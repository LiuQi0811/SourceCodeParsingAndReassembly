...
export class Row extends Component {
  ...
  render(): Node | string {
    ...
    const row = h('div', { class: classes, style }, ...kids) as HTMLElement;
...
export interface ColResponsiveProp {
  span?: number;
  offset?: number;
  push?: number;
  pull?: number;
...
export type ColResponsiveValue = number | ColResponsiveProp;
...
function parseResponsive(value: ColResponsiveValue | undefined): ColResponsiveProp | null {
...
export class Col extends Component {
  ...
  render(): Node | string {
    ...
    const restProps = rest as Record<string, ColResponsiveValue>;
    for (const bp of BREAKPOINTS) {
      const parsed = parseResponsive(restProps[bp]);
      if (!parsed) continue;
      if (parsed.span != null) col.style.setProperty(`--col-${bp}-span`, String(parsed.span));
      if (parsed.offset) col.style.setProperty(`--col-${bp}-offset`, String(parsed.offset));
      if (parsed.span != null) {
