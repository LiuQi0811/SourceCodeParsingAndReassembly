...
export interface MenuProps extends Props {
  ...
  openKeys?: string[];
  multiple?: boolean;
  onSelect?: (info: { key: string; item: MenuItem; keyPath: string[] | null }) => void;
 ...
 */
function normalizeItems(items: MenuItem[] | undefined): MenuItem[] {
  if (!Array.isArray(items)) return [];
  return items.filter(Boolean).map((it: MenuItem) => ({
    key: it.key,
    label: it.label,
    icon: it.icon,
...
export class Menu extends Component {
  ...
  _onScrollResize: (() => void) | null = null;
  _popupEl: HTMLElement | null = null;
  _hideTimer: ReturnType<typeof setTimeout> | undefined = undefined;
  ...
  componentDidMount(): void {
    ...
    if (this._isHorizontal()) {
      this._onDocClick = (e: Event) => {
        ...
        if (this.el && this.el.contains(e.target as Node)) return;
        if (this._popupEl && this._popupEl.contains(e.target as Node)) return;
        this._closePopup();
      };
  ...
  _isHorizontal(): boolean { return this.props.mode === 'horizontal' || !this.props.mode; }
  ...
  _keyPath(items: MenuItem[], key: string, path: string[] = []): string[] | null {
    for (const it of items) {
  ...
  _openPopup(key: string, item: MenuItem): void {
    clearTimeout(this._hideTimer);
    // 构建 popup 内容
    const popup = h('ul', {
      class: 'menu__popup',
    ...
    // 移除旧 popup
    this._popupEl?.remove();
    this._popupEl = popup as HTMLElement;
  ...
  _positionPopup(): void {
    ...
    const trigger = (this.el as Element | null)?.querySelector(`[data-submenu-key="${this.state.popupKey}"]`);
    if (!trigger) return;
  ...
  /** 渲染单个菜单项（叶子节点） */
  _renderItem(item: MenuItem): Node {
  ...
  /** 渲染子菜单（含子项） */
  _renderSubMenu(item: MenuItem): Node {
  ...
  _hasSelectedChild(children: MenuItem[] | null | undefined): boolean {
  ...
  _renderChildren(items: MenuItem[]): Node[] {
