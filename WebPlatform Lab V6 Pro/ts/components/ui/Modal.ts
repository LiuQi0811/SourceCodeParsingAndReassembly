...
export function _lockBody(): void {
  _bodyLockCount += 1;
  if (_bodyLockCount === 1) document.body.style.overflow = 'hidden';
}
export function _unlockBody(): void {
 ...
 */
function _findFocusable(container: HTMLElement | null | undefined): HTMLElement[] {
  if (!container) return [];
 ...
 *
 * @param dialogEl 弹层根节点（含 role=dialog）
 * @param triggerEl 打开弹层的触发元素（关闭后归还焦点）
 * @returns cleanup
 */
export function _trapFocus(dialogEl: HTMLElement | null, triggerEl: HTMLElement | null): () => void {
  if (!dialogEl) return () => {};

  // 1) 进入即聚焦首个可聚焦元素（让读屏/键盘用户立刻处于弹层内）
  const focusables = _findFocusable(dialogEl);
  const initial: HTMLElement = focusables[0] || dialogEl;
  ...
  const onKeyDown = (e: KeyboardEvent): void => {
    ...
    if (e.shiftKey) {
      ...
      if (active === first || !dialogEl.contains(active as Node)) {
        e.preventDefault();
        last.focus();
      }
    } else {
...
export interface ModalProps extends Props {
  ...
  footer?: Node | string | (Node | string)[] | null;
  width?: number;
...
export class Modal extends Component {
  ...
  initialState(): ModalState { return { open: !!this.props.open }; }
  ...
  render(): Node | string {
    ...
    return h('div', { style: { display: 'contents' } },
      h('div', { class: 'modal-mask', onClick: (e: MouseEvent) => {
        if (maskClosable && e.target === e.currentTarget) this.close();
      }}),
      h('div', { class: 'modal', role: 'dialog', 'aria-modal': 'true',
        'aria-label': typeof title === 'string' ? title : undefined,
        onClick: (e: MouseEvent) => { if (maskClosable && e.target === e.currentTarget) this.close(); } },
        h('div', { class: 'modal__dialog', style: width ? { width: `${width}px` } : null },
