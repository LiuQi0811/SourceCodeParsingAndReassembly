...
export interface PaginationState extends State {
  current: number;
  pageSize: number;
  jumperValue: string;
}
  ...
  render(): Node | string {
    ...
    return h('div', { class: 'pagination' },
      ...
      showSizeChanger && h('select', {
        ...
        onChange: (e: Event) => {
          const ps = Number((e.target as HTMLSelectElement).value);
      ...
      showQuickJumper && h('span', { class: 'pagination__jumper-wrap' },
        ...
        h('input', {
          ...
          onInput: (e: Event) => { this.state.jumperValue = (e.target as HTMLInputElement).value; },
