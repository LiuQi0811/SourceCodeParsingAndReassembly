...
export interface QRCodeProps extends Props {
  value?: string;
  size?: number;
  color?: string;
  bgColor?: string;
...
export class QRCode extends Component {
  declare props: QRCodeProps;

  /** 基于字符串的伪随机数生成器：保证相同 value 出图一致 */
  _makeRng(seed: string): RngFn {
  ...
  /** 判断坐标是否落在三个定位角区域 */
  _inFinder(i: number, j: number): boolean {
    const inBox = (x: number, y: number): boolean => x >= 0 && x < 7 && y >= 0 && y < 7;
    return inBox(i, j)
      || inBox(i - (MODULES - 7), j)
      || inBox(i, j - (MODULES - 7));
  }
  ...
  _drawFinder(ctx: CanvasRenderingContext2D, x: number, y: number, cell: number, color: string): void {
    ctx.fillStyle = color;
    // 外框（上、下、左、右各一条）
  ...
  /** Canvas 绘制 */
  _drawCanvas(canvas: HTMLCanvasElement): void {
    const {
  ...
  /** SVG 绘制：返回 <svg> 元素 */
  _renderSVG(): SVGElement {
    const {
      size = 96, color = '#000', bgColor = '#fff',
    } = this.props;
    const svg = document.createElementNS(SVG_NS, 'svg') as unknown as unknown as SVGElement;
    ...
    const drawFinder = (x: number, y: number): void => {
      ...
      outer.setAttribute('x', String(x * cell));
      outer.setAttribute('y', String(y * cell));
      outer.setAttribute('width', String(7 * cell));
      outer.setAttribute('height', String(7 * cell));
      outer.setAttribute('fill', 'none');
      outer.setAttribute('stroke', color);
      outer.setAttribute('stroke-width', String(cell));
      svg.appendChild(outer);
      const inner = document.createElementNS(SVG_NS, 'rect');
      inner.setAttribute('x', String((x + 2) * cell));
      inner.setAttribute('y', String((y + 2) * cell));
      inner.setAttribute('width', String(3 * cell));
      inner.setAttribute('height', String(3 * cell));
    ...
    for (let i = 0; i < MODULES; i++) {
      for (let j = 0; j < MODULES; j++) {
        ...
        if (rng() > 0.5) {
          ...
          r.setAttribute('x', String(i * cell));
          r.setAttribute('y', String(j * cell));
