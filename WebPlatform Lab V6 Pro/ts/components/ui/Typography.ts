// Typography.ts —— 排版组件（Title/Paragraph/Text）
...
/** copyable 配置对象（true 表示启用默认配置） */
interface CopyableConfig {
  text?: string;
  icon?: Node | string | null;
  onCopy?: (content: string, ok: boolean) => void;
}
type CopyableProp = boolean | CopyableConfig;
...
function extractText(children: unknown): string {
  ...
  return arr.map((c) => {
    ...
    if (typeof c === 'object' && c !== null && 'nodeType' in c) {
      return (c as Node).textContent || '';
    }
...
/**
 * 渲染复制按钮
 * @param host 宿主组件，用于 emit 事件
 * @param copyable 配置：true 或 { text, icon, onCopy }
 * @param children 用于提取默认复制文本
 */
function renderCopyable(host: Component, copyable: CopyableProp, children: unknown): Node | null {
  if (!copyable) return null;
  const config: CopyableConfig = typeof copyable === 'object' ? copyable : {};
...
export class Title extends Component {
  declare props: TitleProps;

  render(): Node | string {
    const { level = 1, copyable = false, className = '', children } = this.props;
    const lv = Math.min(Math.max(parseInt(level as string, 10) || 1, 1), 5);
    const tag = `h${lv}`;
    ...
    if (copyable) {
      const cp = renderCopyable(this, copyable, children);
      if (cp) kids.push(cp);
    }
