...
// 根据当前路由动态生成菜单
function getMenuForPath(path: string): SidebarItem[] {
...
export interface SidebarState extends State {
  collapsed: boolean;
  open: boolean;
  currentPath: string;
}
...
export class Sidebar extends Component {
  ...
  render(): Node | string {
    ...
    if (items.length === 0) return h('aside', { 'aria-hidden': 'true' }); // 占位，保持网格稳定
    ...
    return h('aside', { class: classes, 'aria-label': '页面导航' },
      ...items.map((item) => {
        if (item.group) {
        ...
        return h('a', {
          ...
          h('span', { class: 'app-sidebar__item-icon', 'aria-hidden': 'true' }, item.icon),
          h('span', { class: 'app-sidebar__item-label' }, item.label),
        );
      }),
    );
  }
