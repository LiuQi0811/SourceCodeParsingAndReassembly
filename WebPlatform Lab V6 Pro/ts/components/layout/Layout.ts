// Layout.ts —— 应用外壳：Header + Sidebar + Content Outlet + Breadcrumb
...
import type { Props, State, RouteRecord } from '../../core/types.js';
...
export interface LayoutState extends State {}
...
export class Layout extends Component {
  ...
  _sidebar: Sidebar | null = null;
  _breadcrumbEl: HTMLElement | null = null;
  ...
  render(): Node | string {
    ...
    this._breadcrumbEl = h('nav', { class: 'app-breadcrumb', style: { display: 'none' } }) as HTMLElement;
  ...
  componentDidMount(): void {
    ...
    this._unsubAfter = eventBus.on(EVENTS.ROUTER_AFTER, ({ to }: { to: BreadcrumbInput }) => {
    ...
    const to: BreadcrumbInput = currentRoute
  ...
  _renderBreadcrumb(crumbs: BreadcrumbItem[]): void {
  ...
  _buildBreadcrumb(to: BreadcrumbInput): BreadcrumbItem[] {
