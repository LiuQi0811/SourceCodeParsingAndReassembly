...
export interface UserDetailPageProps extends Props {
  router: Router;
  params?: Record<string, string>;
}

export interface UserDetailPageState extends State {
  user: any;
  loading: boolean;
  error: string | null;
}
...
export class UserDetailPage extends Page {
  declare props: UserDetailPageProps;
  declare state: UserDetailPageState;

  initialState(): UserDetailPageState { return { user: null, loading: true, error: null }; }

  async onRouteEnter(params: Record<string, string>): Promise<void> {
    const id = params?.id || this.props.params?.id;
    if (!id) return;
    this.setState({ loading: true, error: null });
    try {
      const all = await request('/assets/data/users.json');
      const user = all.find((u: any) => String(u.id) === String(id));
