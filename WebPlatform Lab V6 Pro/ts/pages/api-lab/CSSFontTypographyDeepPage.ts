...
interface CSSFontTypographyDeepPageState extends State {
  logs: LogEntry[];
  capsSummary: string;
  paletteInfo: string;
  synthesisInfo: string;
  fontFaceInfo: string;
  featureSettingsInfo: string;
...
export class CSSFontTypographyDeepPage extends Page {
  declare state: CSSFontTypographyDeepPageState;
  _inited: boolean = false;
  _dynamicStyles: HTMLStyleElement[] = [];


  initialState(): CSSFontTypographyDeepPageState {
    return {
      logs: [],
      capsSummary: '',
  ...
  componentDidMount(): void {
    ...
    if (!f.palette) this._addLog('warn', '@font-palette-values 不可用（Chrome 101+ 才支持）');
    if (!f.variationSettings) this._addLog('warn', 'font-variation-settings 不可用（Chrome 62+ 才支持）');
    if (!f.autoPhrase) this._addLog('warn', 'word-break: auto-phrase 不可用（Chrome 119+ 才支持，日语形态学断词）');
  }
  ...
  _renderCard1(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['@font-palette-values', f.palette]]),
        h(Tag, { color: 'primary' }, 'Fonts L4'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'COLRv1 彩色字体（Noto Color Emoji / Bootstrap Icons）内置多套调色板，@font-palette-values 让开发者改写调色板：base-palette 选择内置第 N 套调色板，override-color 覆盖特定颜色索引。font-palette 应用自定义调色板（normal/light/dark/--name）。配合 prefers-color-scheme 实现 Emoji 深色模式适配，替代 SVG/PNG 图标的繁琐改色方案。Chrome 101+ 支持。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行 @font-palette-values 演示', { type: 'primary', size: 'sm', onClick: () => this._runPaletteDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.paletteInfo || '（点击按钮查看 @font-palette-values 完整用法）')),
      ],
    });
  ...
  _renderCard2(): Node | string {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '字体缺失某字重/斜体/小型大写时，浏览器默认会合成伪粗/伪斜/伪小型大写（效果差）。font-synthesis-weight/style/small-caps 分别控制是否合成（auto/none）。专业排版应禁用合成并加载真字重，确保设计一致性与 WCAG 对比度。font-synthesis 简写可一次性设置 weight style small-caps。Chrome 97+ 支持 weight/style，Chrome 120+ 支持 small-caps。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行 font-synthesis 演示', { type: 'primary', size: 'sm', onClick: () => this._runSynthesisDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '480px', overflow: 'auto' } },
          h('code', {}, s.synthesisInfo || '（点击按钮查看 font-synthesis 完整用法）')),
      ],
    });
    this.registerChild(card);
    return card.render() as Node;
  ...
  _renderCard3(): Node | string {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '@font-face 全家桶：src 配 format()/tech() 精确指定字体格式与能力、unicode-range 子集化（仅加载用到的字符，CJK 字体体积优化关键）、font-display 加载策略（swap/fallback/optional 避免 FOIT）、字体度量覆盖（size-adjust + ascent/descent/line-gap-override 让 fallback 字体模拟目标字体度量，消除 CLS 布局抖动）、FontFace API（JS 动态加载 + document.fonts.ready 检测）。配合 preload + 自托管优化性能。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行 @font-face 演示', { type: 'primary', size: 'sm', onClick: () => this._runFontFaceDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '600px', overflow: 'auto' } },
          h('code', {}, s.fontFaceInfo || '（点击按钮查看 @font-face 完整用法）')),
      ],
    });
    this.registerChild(card);
    return card.render() as Node;
  ...
  _renderCard4(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['font-feature-settings', f.featureSettings], ['@font-feature-values', f.featureValues]]),
        h(Tag, { color: 'primary' }, 'OpenType'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'font-feature-settings 是访问 OpenType 字体特性的钥匙：liga/calt 连字（编程字体 => → 单字形）、kern 字偶距、smcp 小型大写、tnum/onum/lnum 数字风格（表格对齐/旧式/齐线）、frac 分数、ss01-ss20 风格集、swsh 花体、locl 本地化字形。推荐优先用语义化的 font-variant-* 高级属性。@font-feature-values 命名映射不同字体的特性集，统一引用。@font-feature-values 子规则 @stylistic/@styleset/@swash 等。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行 font-feature-settings 演示', { type: 'primary', size: 'sm', onClick: () => this._runFeatureSettingsDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '600px', overflow: 'auto' } },
          h('code', {}, s.featureSettingsInfo || '（点击按钮查看 font-feature-settings 完整用法）')),
      ],
    });
    this.registerChild(card);
    return card.render() as Node;
  ...
  _renderCard5(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['font-variation-settings', f.variationSettings]]),
        h(Tag, { color: 'primary' }, '可变字体'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '可变字体单文件包含所有字重/宽度/样式，可无级调节与动画化。5 个注册轴：wght 字重 / wdth 宽度 / ital 斜体 / slnt 倾斜 / opsz 视觉尺寸（小字号清晰、大字号精致）。自定义轴大写命名（GRAD 等级暗色加粗不影响布局 / CASL 随性 / SOFT 柔和 / MONO 等宽）。font-weight/font-stretch/font-style 是简写映射，推荐用简写，自定义轴才用 font-variation-settings。配合 @keyframes 实现字重呼吸动画。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行可变字体演示', { type: 'primary', size: 'sm', onClick: () => this._runVariationDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '600px', overflow: 'auto' } },
          h('code', {}, s.variationInfo || '（点击按钮查看可变字体完整用法）')),
      ],
    });
    this.registerChild(card);
    return card.render() as Node;
  ...
  _renderCard6(): Node | string {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'CJK 排版特性：text-spacing-trim 标点压缩（行首/行尾/连续标点）、hanging-punctuation 标点悬挂（标点移到行外让正文边缘整齐）、word-break: auto-phrase 日语形态学断词（避免切断词组，Chrome 119+）、text-autospace CJK 与拉丁/数字间自动加空隙（替代手写空格）、line-break 严格度、overflow-wrap/word-break 断行策略、text-justify: inter-character 两端对齐字间调整、writing-mode 竖排配合 text-orientation。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行 CJK 排版演示', { type: 'primary', size: 'sm', onClick: () => this._runCJKDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '600px', overflow: 'auto' } },
          h('code', {}, s.cjkInfo || '（点击按钮查看 CJK 排版完整用法）')),
      ],
    });
    this.registerChild(card);
    return card.render() as Node;
