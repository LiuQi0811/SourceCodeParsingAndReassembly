...
interface WebSocketStreamPageState extends State {
  logs: any;
  capsSummary: any;
  overviewInfo: any;
  connInfo: any;
...
export class WebSocketStreamPage extends Page {
  declare state: WebSocketStreamPageState;
  _ac!: any;
  _dynamicStyles!: any[];
  _inited!: boolean;
  _sim!: any;
  ...
  _runReadDemo() {
    ...
    if (f.readableStream) {
      try {
        ...
        })().catch((e: any) => this._addLog('warn', `模拟读取失败：${e && e.message}`));
      } catch (err: any) {
...
await readable.pipeThrough(through).pipeTo(logSink);`)),
  ...
  _runBackpressureSim() {
    this._stopSim();
    const sim = { stopped: false, producerTimer: null as any, consumerTimer: null as any, reportTimer: null };
    this._sim = sim;
    ...
    const scheduleProducer = () => {
      ...
      sim.producerTimer = (setTimeout(produce, producerRate) as any);
    };
    const produce = () => {
      if (sim.stopped) return;
    ...
    const scheduleConsumer = () => {
      if (sim.stopped) return;
      sim.consumerTimer = (setTimeout(consume, consumerRate) as any);
    };
    const consume = () => {
      if (sim.stopped) return;
      if (queue.length > 0) {
    ...
    const report = () => {
      ...
      sim.reportTimer = (setTimeout(report, 200) as any);
    };
