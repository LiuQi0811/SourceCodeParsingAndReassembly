...
interface SchedulerTasksPageCaps {
  scheduler: boolean;
...
export interface SchedulerTasksPageProps extends Props {}

export interface SchedulerTasksPageState extends State {
  logs: LogEntry[];
  capsSummary: string;
...
export class SchedulerTasksPage extends Page {
  declare props: SchedulerTasksPageProps;
  declare state: SchedulerTasksPageState;
  ...
  _loafObserver!: any;
  _loafEntries!: any[];
  _pendingTaskController!: any;


  // —— 初始 state ——
  initialState(): SchedulerTasksPageState {
    return {
      logs: [],
      capsSummary: '',
  ...
  async _compareWithSetTimeout() {
    ...
    try {
      ...
      const order: any[] = [];
      const mark = (label: any) => { order.push(label); this._addLog('compare', `触发：${label}`); };
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'window.scheduler.postTask(callback, options) → Promise 按指定优先级调度任务；(options as any).priority ∈ "user-blocking"（最高，~0ms）/ "user-visible"（默认，~4ms）/ "background"（最低，~50ms）。调度器按优先级而非提交顺序执行；同优先级 FIFO。options.delay（毫秒延迟）。可与 setTimeout / queueMicrotask / requestIdleCallback 对比，理解不同调度机制的优先级与延迟特性。'),
...
requestIdleCallback(cb); // 空闲时才执行`)),
  ...
  async _demoTaskController() {
    ...
    try {
      ...
      const signalInfo = `(controller.signal as any).priority = ${(controller.signal as any).priority}`;
  ...
  async _demoPriorityChange() {
    ...
    try {
      ...
      const onChange = (e: any) => {
        const line = `${e.previousPriority} → ${(controller.signal as any).priority}`;
        changes.push(line);
...
try { await taskPromise; } catch (e: any) { /* AbortError */ }`)),
  ...
  _stopIdleDetector(): void {
    if (this._idleDetector) {
      try {
        if (this._idleHandler) (this._idleDetector as any).removeEventListener('change', this._idleHandler);
...
observer.disconnect();        // 断开所有`)),
  ...
  _startLoAFObserver(): void {
    ...
    this._loafEntries = [];
    const handleEntries = (entries: any,entryType: any) => {
      for (const e of entries) {
        this._loafEntries.push(e);
        if (entryType === 'long-animation-frame') {
          const scripts = (e.scripts || []).map((s: any) =>
          ...
          const line =
            `LoAF：duration=${(e.duration as any).toFixed(1)}ms blocking=${(e.blockingDuration ?? 0).toFixed(1)}ms ` +
          ...
          // longtask 退化分支
          const line = `longtask：duration=${(e.duration as any).toFixed(1)}ms startTime=${(e.startTime as any).toFixed(1)} name=${e.name || '-'}`;
          this._addLog('loaf', line);
          this.setState({
            loafState:
    ...
    try {
      ...
      try {
        ...
        this.setState({
          loafState:
            `当前环境不支持 'long-animation-frame'（${errInfo(err).message}）\n` +
...
// 监控内存增长以检测泄漏：定时调用并记录 bytes 变化`)),
  ...
  async _demoYield() {
    ...
    try {
      ...
      this._addLog('yield', `长任务完成：sum=${(sum as any).toFixed(0)}，总耗时 ${elapsed}ms（yield 让出后用户输入可穿插响应）`);
      this.setState({
        yieldState:
          `scheduler.yield() 演示完成 ✓\n` +
          `长任务总耗时 ${elapsed}ms（含 yield 让出时间）\n` +
          `sum = ${(sum as any).toFixed(0)}\n\n` +
