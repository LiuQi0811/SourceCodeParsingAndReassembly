...
interface WebSmartCardAPIPageState extends State {
  logs: any;
  capsSummary: any;
  overviewInfo: any;
  contextInfo: any;
...
export class WebSmartCardAPIPage extends Page {
  declare state: WebSmartCardAPIPageState;
  _connection!: any;
  _context!: any | null;
  _dynamicStyles!: any[];
  _inited!: boolean;
  ...
  componentDidMount() {
    ...
    if (!f.smartCard) this._addLog('warn', 'navigator.smartCard 不可用（jsdom 无 Smart Card API，真实浏览器需 Chromium 提案 flag）');
    if (!f.smartCardContext) this._addLog('warn', 'SmartCardContext 不可用（establishContext 入口缺失）');
    if (!f.pcscInterface) this._addLog('warn', 'PCSCInterface 不可用（pcsc 透传 PC/SC 错误码的接口未暴露）');
    if (!f.smartCardConnection) this._addLog('warn', 'SmartCardConnection 不可用（connect/transmit 入口缺失）');
    if (!f.permissionPolicy) this._addLog('warn', 'Permissions-Policy: smart-card 未声明或 document.policy 不可用');
  ...
  componentWillUnmount() {
    ...
    if (this._context) {
      try { this._context.releaseContext?.(); } catch { /* noop */ }
      this._context = null;
    }
    // 3. 解绑读卡器热插拔事件
    if (this._readerUnbind) {
      try { this._readerUnbind(); } catch { /* noop */ }
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'Web Smart Card API 基于 PC/SC 工作组模型，通过 navigator.smartCard 把系统 PC/SC 资源管理器暴露到浏览器，应用以 APDU（CLA/INS/P1/P2/Lc/Data/Le 命令、SW1/SW2 响应）与卡片通信。与 WebUSB/WebHID 直连读卡器的差异：Web Smart Card 走资源管理器复用驱动并自动处理 T=0/T=1 协议与 ATR，WebUSB 需自行实现 CCID 协议且独占读卡器，WebHID 仅能被动接收键盘楔形读卡器扫描码。浏览器支持：Chromium 提案阶段需 flag，Safari/Firefox 未实现。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行概念演示', { type: 'primary', size: 'sm', onClick: () => this._runOverviewDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, `// Web Smart Card API 入口检测
if ('smartCard' in navigator) {
  const sc = navigator.smartCard;
  ...
  await ctx.releaseContext();
        ...
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.overviewInfo || '（点击按钮查看 Web Smart Card 概念与 PC/SC 模型完整说明）')),
      ],
    });
  ...
  _renderCard2() {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'SmartCardContext.establishContext() 建立 PC/SC 上下文（等价 SCardEstablishContext），返回持有资源管理器句柄的 ctx，后续 listReaders/connect 都基于此。PCSCInterface（interface: "pcsc"）透传 PC/SC 工作组错误码（SCARD_E_NO_SERVICE 0x8010001D、SCARD_E_CARD_NOT_PRESENT 0x80100070 等）。releaseContext 释放句柄（等价 SCardReleaseContext），务必在 finally 或 componentWillUnmount 调用避免泄漏。需声明 Permissions-Policy: smart-card=(self)。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('建立/释放上下文', { type: 'primary', size: 'sm', onClick: () => this._runContextDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, `// 建立 PC/SC 上下文（PCSCInterface 透传错误码）
const ctx = await navigator.smartCard.establishContext({
  interface: 'pcsc',
});
...
try {
  ...
  console.log(err.pcscMessage);  // "SCARD_E_NO_SERVICE"
        ...
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.contextInfo || '（点击按钮查看 SmartCardContext 生命周期完整说明）')),
      ],
    });
    this.registerChild(card);
    return card.render();
...
// status.atr: Uint8Array (Answer To Reset)`)),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.readersInfo || '（点击按钮查看读卡器列举与热插拔完整说明）')),
      ],
    });
    this.registerChild(card);
    return card.render();
...
function parseATR(atr: any) {
  ...
  return { ts, t0 };
        ...
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.connectInfo || '（点击按钮查看连接与协议完整说明）')),
      ],
    });
    this.registerChild(card);
    return card.render();
  ...
  _renderCard5() {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'SmartCardConnection.transmit(PCSCCommand) 发送 APDU（等价 SCardTransmit），命令含 CLA/INS/P1/P2/Lc/Data/Le 四种 CASE（CASE1 无数据无响应、CASE2 有响应、CASE3 有数据、CASE4 有数据有响应）。响应 SW1/SW2：0x9000 成功、0x61xx 需 GET RESPONSE 取回、0x6Cxx Le 错误用 SW2 重发、0x6982 安全状态不满足、0x6A82 文件未找到。T=0 CASE4 长响应需 GET RESPONSE 链式取回。Uint8Array 与十六进制互转辅助拼装。',
        ),
        h('div', { class: 'wsc-apdu-grid' },
          h('div', { class: 'wsc-apdu-cell' }, h('b', {}, 'CLA'), ' 0x00'),
          h('div', { class: 'wsc-apdu-cell' }, h('b', {}, 'INS'), ' 0xA4'),
          h('div', { class: 'wsc-apdu-cell' }, h('b', {}, 'P1'), ' 0x04'),
          h('div', { class: 'wsc-apdu-cell' }, h('b', {}, 'P2'), ' 0x00'),
          h('div', { class: 'wsc-apdu-cell' }, h('b', {}, 'Lc'), ' 0x0C'),
          h('div', { class: 'wsc-apdu-cell' }, h('b', {}, 'Data'), ' A000...'),
          h('div', { class: 'wsc-apdu-cell' }, h('b', {}, 'Le'), ' 0x00'),
          h('div', { class: 'wsc-apdu-cell' }, h('b', {}, 'SW'), ' 9000'),
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('发送 APDU', { type: 'primary', size: 'sm', onClick: () => this._runApduDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, `// 发送 APDU（SELECT by DF name）
const resp = await conn.transmit({
  cla: 0x00, ins: 0xA4, p1: 0x04, p2: 0x00,
...
async function transmitFull(conn: any,  cmd: any) {
  ...
  return resp;
        ...
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.apduInfo || '（点击按钮查看 APDU 传输完整说明）')),
      ],
    });
    this.registerChild(card);
    return card.render();
  ...
  _renderCard6() {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'eID（中国居民身份证）遵循 ISO/IEC 7816-4 文件结构 MF/DF/EF：SELECT（INS=0xA4）选择 MF/DF by AID/EF by FID，READ BINARY（INS=0xB0）按偏移读取 TLV 编码个人信息（姓名/性别/民族/出生/住址/照片）。PIN 验证用 VERIFY（INS=0x20，P2 指定 PIN 引用），3 次错误锁死。数字签名用 PSO（INS=0x2A）：先 MSE SET（INS=0x22）指定算法 RSA-PKCS 与私钥引用，再 PSO HASH（9E9A）+ PSO SIGN（9E9A）生成 RSA 签名，配合卡片证书验签。',
        ),
...
└── DF 签名应用            PSO 签名私钥`),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('读取 eID 信息', { type: 'primary', size: 'sm', onClick: () => this._runEidDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, `// SELECT DF by AID（电子证件应用）
...
const sig = await conn.transmit({
  cla: 0x00, ins: 0x2A, p1: 0x9E, p2: 0x9A, le: 256,
        ...
        h('pre', { class: 'code-block', style: { maxHeight: '600px', overflow: 'auto' } },
          h('code', {}, s.eidInfo || '（点击按钮查看 eID 实战完整说明）')),
      ],
    });
    this.registerChild(card);
    return card.render();
  ...
  _renderCard7() {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'PAdES（ETSI TS 102 778）在 PDF 嵌入高级电子签名（B-B/B-T/B-LT/B-LTA）。Web 方案替代传统 PKCS#11：PDF.js 准备 ByteRange，crypto.subtle.digest 算哈希，Smart Card PSO 在卡片内签名（私钥不出卡），构造 PKCS#7/CAdES 容器嵌入 /Contents，B-T 加 RFC 3161 时间戳。与 WebAuthn 协同：WebAuthn 负责用户登录认证（高层），Smart Card 负责法定签名（低层 PSO，符合 eIDAS QES）。',
        ),
  ...
  └── fetch TSA ── RFC 3161 时间戳（B-T）
      ...
      PAdES B-T 签名 PDF`),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('PAdES 签名流程', { type: 'primary', size: 'sm', onClick: () => this._runPadesDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, `// PAdES 签名：PDF.js + Web Crypto + Smart Card
const hash = await crypto.subtle.digest('SHA-256', signedBytes);
...
const assertion = await navigator.credentials.get({ publicKey: { challenge } });`)),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '600px', overflow: 'auto' } },
          h('code', {}, s.padesInfo || '（点击按钮查看 PAdES 数字签名完整说明）')),
      ],
    });
    this.registerChild(card);
    return card.render();
...
async function getSigner() {
  ...
  return new RemoteSigner('/api/sign'); // 降级2
        ...
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '600px', overflow: 'auto' } },
          h('code', {}, s.securityInfo || '（点击按钮查看权限与安全完整说明）')),
      ],
    });
    this.registerChild(card);
    return card.render();
