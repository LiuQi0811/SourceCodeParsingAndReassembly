...
interface CSSStartingStylePageState extends State {
  logs: LogEntry[];
  capsSummary: string;
  startingStyleInfo: string;
  transitionBehaviorInfo: string;
  interpolateSizeInfo: string;
  displayTransitionInfo: string;
  synergyInfo: string;
  supportInfo: string;
}
...
export class CSSStartingStylePage extends Page {
  declare state: CSSStartingStylePageState;
  ...
  // —— 初始 state ——
  initialState(): CSSStartingStylePageState {
    return {
      logs: [],
      capsSummary: '',
  ...
  componentDidMount(): void {
    ...
    if (!caps.startingStyle) this._addLog('warn', '@starting-style 不支持（Chrome 117+ / Safari 17.5+ / Firefox 129+，jsdom 不模拟）');
    if (!caps.transitionBehavior) this._addLog('warn', 'transition-behavior: allow-discrete 不支持（Chrome 117+）');
    if (!caps.interpolateSize) this._addLog('warn', 'interpolate-size: allow-keywords 不支持（Chrome 129+）');
  }

  componentWillUnmount(): void {
    // 1. 取消所有未执行的 requestAnimationFrame 句柄（polyfill 模拟时注册）
    for (const id of this._rafIds || []) {
      try { if (typeof cancelAnimationFrame === 'function') cancelAnimationFrame(id); } catch { /* noop */ }
    }
    this._rafIds = ([] as any);
  ...
  _caps(): any {
    ...
    if (cssSupports) {
      try { startingStyle = CSS.supports('@starting-style { div { opacity: 0 } }'); } catch { /* jsdom CSS.supports 不识别 at-rule */ }
      try { transitionBehavior = CSS.supports('transition-behavior: allow-discrete'); } catch { /* noop */ }
      try { interpolateSize = CSS.supports('interpolate-size: allow-keywords'); } catch { /* noop */ }
    }
  ...
  _showStartingStyleCaps(): void {
    const caps = this._caps();
    this.setState({ startingStyleInfo:
      '===== @starting-style 能力检测 =====\n\n' +
  ...
  _compareWithWithout(): void {
    const caps = this._caps();
    this.setState({ startingStyleInfo:
      '===== 无 @starting-style vs 有 @starting-style 对比 =====\n\n' +
      '【无 @starting-style】\n' +
  ...
  _renderCard1(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.startingStyle ? 'success' : 'error' }, caps.startingStyle ? '@starting-style ✓' : '@starting-style ✗'),
        h(Tag, { color: 'primary' }, 'first frame'),
        h(Tag, { color: 'info' }, 'declarative 进入过渡')),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '@starting-style 规则定义元素「起始样式」（首次出现时的过渡起点）。当元素从 display:none 进入 display:block 时，浏览器跳过 first frame，常规 transition 无法触发；@starting-style 显式提供起点，让浏览器从该起点过渡到当前样式。语法：@starting-style { .popover { opacity: 0; transform: scale(0.9); } }。仅解决「进入」过渡，退出需配合 transition-behavior: allow-discrete。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('能力检测', { type: 'primary', size: 'sm', onClick: () => this._showStartingStyleCaps() }),
          this._btn('展示语法', { size: 'sm', onClick: () => this._showStartingStyleSyntax() }),
          this._btn('对比无/有', { size: 'sm', onClick: () => this._compareWithWithout() }),
          this._btn('first frame 问题', { size: 'sm', onClick: () => this._explainFirstFrame() })),
  ...
  _showTransitionBehavior(): void {
    const caps = this._caps();
    this.setState({ transitionBehaviorInfo:
      '===== transition-behavior: allow-discrete 语法 =====\n\n' +
  ...
  _compareDefaultVsAllowDiscrete(): void {
    const caps = this._caps();
    this.setState({ transitionBehaviorInfo:
      '===== 默认（normal）vs allow-discrete 对比 =====\n\n' +
  ...
  _renderCard2(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.transitionBehavior ? 'success' : 'error' }, caps.transitionBehavior ? 'allow-discrete ✓' : 'allow-discrete ✗'),
        h(Tag, { color: 'primary' }, 'discrete 属性'),
        h(Tag, { color: 'warning' }, '退出过渡关键')),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '默认 display 是 discrete 属性（瞬变，不参与过渡）；transition-behavior: allow-discrete 配合 transition: display 0.3s 让 display 在过渡期间保持中间值。进入：display none→block + opacity 0→1（配合 @starting-style）；退出：display block→none（保持可过渡状态到过渡结束才真正 none）。是退出过渡的关键，与 @starting-style 协同实现完整进入/退出动画。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('展示 transition-behavior', { type: 'primary', size: 'sm', onClick: () => this._showTransitionBehavior() }),
          this._btn('对比默认 vs allow-discrete', { size: 'sm', onClick: () => this._compareDefaultVsAllowDiscrete() }),
          this._btn('discrete 属性说明', { size: 'sm', onClick: () => this._explainDiscrete() }),
          this._btn('进入/退出流程', { size: 'sm', onClick: () => this._explainEnterExitTransition() })),
  ...
  _showInterpolateSize(): void {
    const caps = this._caps();
    this.setState({ interpolateSizeInfo:
      '===== interpolate-size: allow-keywords 语法 =====\n\n' +
      '/* 全局开启（推荐放在 :root）*/\n' +
      ':root {\n' +
  ...
  _compareFixedVsAuto(): void {
    this.setState({ interpolateSizeInfo:
      '===== fixed 长度 vs auto 关键字过渡对比 =====\n\n' +
  ...
  _showInterpolateSizeSupport(): void {
    const caps = this._caps();
    this.setState({ interpolateSizeInfo:
      '===== interpolate-size 浏览器支持 =====\n\n' +
      '浏览器          | 版本       | 支持状态\n' +
      '----------------|------------|------------------\n' +
  ...
  _renderCard3(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.interpolateSize ? 'success' : 'error' }, caps.interpolateSize ? 'allow-keywords ✓' : 'allow-keywords ✗'),
        h(Tag, { color: 'primary' }, 'auto 插值'),
        h(Tag, { color: 'info' }, 'calc-size()')),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '传统 height/width: auto 不能插值（只能 fixed 长度）；interpolate-size: allow-keywords 允许 auto/min-content/max-content/fit-content 关键字参与过渡。用法：:root { interpolate-size: allow-keywords; } .acc { height: 0; transition: height 0.3s; } .acc.open { height: auto; }（无需 JS 测量高度）。Chrome 129+；与 calc-size() 协同（calc-size(auto, size + 10px)）。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('展示 interpolate-size', { type: 'primary', size: 'sm', onClick: () => this._showInterpolateSize() }),
          this._btn('对比 fixed vs auto', { size: 'sm', onClick: () => this._compareFixedVsAuto() }),
          this._btn('展示 calc-size', { size: 'sm', onClick: () => this._showCalcSize() }),
          this._btn('浏览器支持', { size: 'sm', onClick: () => this._showInterpolateSizeSupport() })),
  ...
  _showFullPattern(): void {
    this.setState({ displayTransitionInfo:
      '===== display 过渡完整模式（popover / tooltip） =====\n\n' +
      '/* 默认（隐藏）状态 */\n' +
      '.popover {\n' +
      '  display: none;\n' +
      '  opacity: 0;\n' +
  ...
  _comparePopoverApi(): void {
    this.setState({ displayTransitionInfo:
      '===== 完整模式 vs popover API 原生过渡对比 =====\n\n' +
      '【完整模式（手动 display + 类切换）】\n' +
  ...
  _renderCard4(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: 'primary' }, '@starting-style + allow-discrete'),
        h(Tag, { color: 'warning' }, '进入/退出双过渡'),
        h(Tag, { color: 'info' }, 'popover 友好')),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '完整 popover/tooltip 进入退出过渡组合：.popover 内 transition: display 0.3s allow-discrete + opacity/transform；.popover.open 内 display:block + opacity:1 + transform:scale(1) + @starting-style 提供起点。进入：@starting-style 提供 opacity:0 起点 → opacity:1；退出：display 保持 block 到过渡结束才 none。与 popover API :popover-open 伪类写法几乎一致，仅触发方式不同。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('展示完整模式', { type: 'primary', size: 'sm', onClick: () => this._showFullPattern() }),
          this._btn('进入流程', { size: 'sm', onClick: () => this._explainEnterFlow() }),
          this._btn('退出流程', { size: 'sm', onClick: () => this._explainExitFlow() }),
          this._btn('对比 popover API', { size: 'sm', onClick: () => this._comparePopoverApi() })),
  ...
  _showThreeWayCompare(): void {
    this.setState({ synergyInfo:
      '===== @starting-style vs Web Animations API vs View Transitions =====\n\n' +
  ...
  _startingStyleWithWAAPI(): void {
    this.setState({ synergyInfo:
      '===== @starting-style + Web Animations API 协同 =====\n\n' +
  ...
  _startingStyleWithViewTransitions(): void {
    this.setState({ synergyInfo:
      '===== @starting-style + View Transitions 协同 =====\n\n' +
  ...
  _renderCard5(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: 'primary' }, '@starting-style'),
        h(Tag, { color: 'warning' }, 'WAAPI'),
        h(Tag, { color: 'info' }, 'View Transitions')),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '@starting-style 是「declarative 进入过渡」；Web Animations API 是「imperative 程序化动画」；View Transitions 是「DOM 切换快照过渡」。协同：@starting-style 处理元素显隐；WAAPI 处理复杂关键帧；View Transitions 处理整页切换。决策：单元素进入/退出 → @starting-style；运行时动态动画 → WAAPI；DOM 结构变化 → View Transitions。三者不互斥，可组合使用。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('三方案对比表', { type: 'primary', size: 'sm', onClick: () => this._showThreeWayCompare() }),
          this._btn('@starting-style + WAAPI', { size: 'sm', onClick: () => this._startingStyleWithWAAPI() }),
          this._btn('@starting-style + View Transitions', { size: 'sm', onClick: () => this._startingStyleWithViewTransitions() }),
          this._btn('决策矩阵', { size: 'sm', onClick: () => this._showDecisionMatrix() })),
  ...
  _showJSPolyfill(): void {
    this.setState({ supportInfo:
      '===== JS 模拟 polyfill（requestAnimationFrame 双 rAF） =====\n\n' +
      '为何无法完美 polyfill：\n' +
  ...
  _renderCard6(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.startingStyle ? 'success' : 'error' }, caps.startingStyle ? '@starting-style ✓' : '@starting-style ✗'),
        h(Tag, { color: caps.transitionBehavior ? 'success' : 'error' }, caps.transitionBehavior ? 'allow-discrete ✓' : 'allow-discrete ✗'),
        h(Tag, { color: caps.interpolateSize ? 'success' : 'error' }, caps.interpolateSize ? 'interpolate-size ✓' : 'interpolate-size ✗'),
        h(Tag, { color: 'primary' }, 'polyfill')),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '@starting-style（Chrome 117+ / Safari 17.5+ / Firefox 129+）、transition-behavior（Chrome 117+）、interpolate-size（Chrome 129+）。polyfill：无法完美 polyfill（依赖浏览器渲染时机）；降级策略：不支持时元素立即显隐（无过渡，仍可用）；JS 模拟（requestAnimationFrame + class toggle 模拟 first frame）；@supports 渐进增强。检测：CSS.supports(\'@starting-style {...}\') / CSS.supports(\'transition-behavior: allow-discrete\') / CSS.supports(\'interpolate-size: allow-keywords\')。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          ...
          this._btn('JS 模拟 polyfill', { size: 'sm', onClick: () => this._showJSPolyfill() }),
          this._btn('@supports 渐进增强', { size: 'sm', onClick: () => this._showSupportsProgressive() })),
  ...
  render(): Node {
    const s = this.state;
    return h('div', { class: 'api-lab-page css-starting-style-page' },
      h('h2', { class: 'section-title' }, 'CSS @starting-style 与过渡实验室'),
      h('p', { class: 'fs-sm text-secondary mb-md' },
        '本页演示 CSS @starting-style（元素首次出现过渡）、transition-behavior: allow-discrete（discrete 属性过渡）、interpolate-size: allow-keywords（auto 关键字插值）、display 过渡完整模式、与 Web Animations API / View Transitions 协同，以及浏览器支持与 polyfill 策略。jsdom 中 CSS 渲染不可用，所有演示通过 CSS 代码片段说明。'),
