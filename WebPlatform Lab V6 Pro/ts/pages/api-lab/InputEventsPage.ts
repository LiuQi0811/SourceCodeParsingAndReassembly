// InputEventsPage.ts —— 输入事件系统 API 实验室
...
// 对比表表头样式
const TH_STYLE: Record<string, string> = {
  padding: '8px 10px',
  textAlign: 'left',
...
// 对比表单元格样式
const TD_STYLE: Record<string, string> = {
  padding: '8px 10px',
...
export interface InputEventsPageProps extends Props {}

export interface InputEventsLog {
  type: string;
  content: string;
  time: string;
}

export interface InputEventsPageState extends State {
  logs: InputEventsLog[];
  pointerInfo: string;
  pointerTrail: PointerPoint[];
  touchInfo: string;
  touchTrail: TouchPoint[];
  keyInfo: string;
  keyHistory: KeyHistoryEntry[];
  gamepadConnected: boolean;
  gamepadInfo: string;
  gamepadState: GamepadState | null;
  eventComparison: CompareItem[];
}

export class InputEventsPage extends Page {
  declare props: InputEventsPageProps;
  declare state: InputEventsPageState;
  _pointerArea: HTMLElement | null = null;
  _pointerCanvas: HTMLCanvasElement | null = null;
  _pointerCtx: CanvasRenderingContext2D | null = null;
  _pointerPoints: PointerPoint[] = [];
  _lastPointerMoveState: number = 0;
  _touchSupported: boolean = false;
  _touchArea: HTMLElement | null = null;
  _touchCanvas: HTMLCanvasElement | null = null;
  _touchCtx: CanvasRenderingContext2D | null = null;
  _touchPoints: Record<number, TouchPoint[]> = {};
  _touchColors: string[] = [];
  _lastTouchMoveState: number = 0;
  _keyboardArea: HTMLElement | null = null;
  _keyboardNeedsFocus: boolean = false;
  _keyboardDidFocus: boolean = false;
  _gamepadInitChecked: boolean = false;
  _gamepadRaf: number | null = null;
  _lastGamepadState: number = 0;
  _compareBatch: CompareItem[] = [];
  _compareBatchStart: number = 0;
  _compareFlushTimer: number | null = null;
  _preventDefaultCompare: boolean = false;

  initialState(): InputEventsPageState {
    return {
      logs: [],
  ...
  _setupPointer(): void {
    const area = this.$<HTMLElement>('.pointer-area');
    if (!area) return;
    ...
    // 监听五种指针事件
    this.on(area, 'pointerdown', (e: any) => this._onPointerDown(e as PointerEvent));
    this.on(area, 'pointermove', (e: any) => this._onPointerMove(e as PointerEvent));
    this.on(area, 'pointerup', (e: any) => this._onPointerEvent(e as PointerEvent, 'pointerup'));
    this.on(area, 'pointercancel', (e: any) => this._onPointerEvent(e as PointerEvent, 'pointercancel'));
    this.on(area, 'pointerleave', (e: any) => this._onPointerEvent(e as PointerEvent, 'pointerleave'));
  }

  // 初始化指针画布：设置尺寸、2D context、样式，并重绘已有轨迹
  _initPointerCanvas(): void {
    const canvas = this.$<HTMLCanvasElement>('.pointer-canvas');
    if (!canvas) return;
  ...
  _ensurePointerCtx(): CanvasRenderingContext2D | null {
  ...
  _redrawPointerTrail(): void {
    if (!this._pointerCtx || !this._pointerPoints || !this._pointerCanvas) return;
    ...
    ctx.beginPath();
    this._pointerPoints.forEach((p: PointerPoint, i: number) => {
  ...
  // 计算指针相对于画布的坐标
  _pointerCoords(e: PointerEvent): { x: number; y: number } {
    if (!this._pointerCanvas) return { x: 0, y: 0 };
  ...
  _onPointerEvent(e: PointerEvent, phase: string): void {
  ...
  _updatePointerInfo(e: PointerEvent, phase: string): void {
    const info = [
      `[${phase}]`,
      `pointerType: ${e.pointerType}`,
      `pointerId: ${e.pointerId}`,
      `isPrimary: ${e.isPrimary}`,
      `pressure: ${(e.pressure as any).toFixed(3)}`,
      `tiltX: ${e.tiltX}° / tiltY: ${e.tiltY}°`,
      `width × height: ${(e.width as any).toFixed(1)} × ${(e.height as any).toFixed(1)}`,
  ...
  _clearPointerTrail(): void {
    this._pointerPoints = [];
    const ctx = this._ensurePointerCtx();
    if (ctx && this._pointerCanvas) ctx.clearRect(0, 0, this._pointerCanvas.width, this._pointerCanvas.height);
  ...
  _setupTouch(): void {
    const area = this.$<HTMLElement>('.touch-area');
    if (!area) return;
    // 能力检测：桌面浏览器可能无触摸事件
    ...
    // 监听四种触摸事件
    this.on(area, 'touchstart', (e: any) => this._onTouchStart(e as TouchEvent));
    this.on(area, 'touchmove', (e: any) => this._onTouchMove(e as TouchEvent));
    this.on(area, 'touchend', (e: any) => this._onTouchEnd(e as TouchEvent, 'touchend'));
    this.on(area, 'touchcancel', (e: any) => this._onTouchEnd(e as TouchEvent, 'touchcancel'));
  }

  _initTouchCanvas(): void {
    const canvas = this.$<HTMLCanvasElement>('.touch-canvas');
    if (!canvas) return;
  ...
  _ensureTouchCtx(): CanvasRenderingContext2D | null {
  ...
  _touchCoords(touch: Touch): { x: number; y: number } {
    if (!this._touchCanvas) return { x: 0, y: 0 };
  ...
  _onTouchEnd(e: TouchEvent, phase: string): void {
    Array.from(e.changedTouches).forEach((t: Touch) => {
  ...
  _updateTouchInfo(e: TouchEvent, phase: string): void {
    ...
    touches.forEach((t: Touch, i: number) => {
      lines.push(
        `  #${i} id=${t.identifier} (${(t.clientX as any).toFixed(0)},${(t.clientY as any).toFixed(0)}) force=${(t.force as any).toFixed(2)} radius=${(t.radiusX as any).toFixed(0)}×${(t.radiusY as any).toFixed(0)}`,
      );
    });
    if (lines.length === 2) lines.push('  （当前无活跃触点）');
    // 收集所有轨迹点用于状态展示
    const allPoints: TouchPoint[] = (Object as any).values(this._touchPoints).flat();
    this.setState({
  ...
  _clearTouchTrail(): void {
    this._touchPoints = {};
    const ctx = this._ensureTouchCtx();
    if (ctx && this._touchCanvas) ctx.clearRect(0, 0, this._touchCanvas.width, this._touchCanvas.height);
  ...
  _setupKeyboard(): void {
    const area = this.$<HTMLElement>('.keyboard-area');
    if (!area) return;
    ...
    this.on(area, 'keydown', (e: any) => this._onKeyDown(e as KeyboardEvent));
    this.on(area, 'keyup', (e: any) => this._onKeyUp(e as KeyboardEvent));
  }
  ...
  _onKeyDown(e: KeyboardEvent): void {
    ...
    const histEntry: KeyHistoryEntry = { key: e.key, code: e.code, time: formatTime() };
  ...
  _setupGamepad(): void {
    // 监听手柄连接/断开事件
    this.on(window, 'gamepadconnected', (e: any) => this._onGamepadConnected(e as GamepadEvent));
    this.on(window, 'gamepaddisconnected', (e: any) => this._onGamepadDisconnected(e as GamepadEvent));

    // 首次检查已连接的手柄（需用户先按下按钮才会出现在列表中）
    if (!this._gamepadInitChecked) {
      this._gamepadInitChecked = true;
      const pads = navigator.getGamepads ? navigator.getGamepads() : [];
      const connected = Array.from(pads).filter(Boolean) as Gamepad[];
      if (connected.length > 0) {
        this._onGamepadConnected({ gamepad: connected[0] } as GamepadEvent);
      }
    }
  }

  _onGamepadConnected(e: GamepadEvent): void {
    const pad = e.gamepad;
    const info = [
      `已连接游戏手柄`,
      `id: ${pad.id}`,
  ...
  _onGamepadDisconnected(e: GamepadEvent): void {
  ...
  _pollGamepad(): void {
    ...
    this.setState({
      gamepadState: {
        ...
        buttons: pad.buttons.map((b: GamepadButton) => ({
          pressed: b.pressed,
          value: b.value,
          touched: b.touched,
        })),
  ...
  _setupComparison(): void {
    const area = this.$<HTMLElement>('.comparison-area');
    if (!area) return;
    this._compareBatch = [];
    this._preventDefaultCompare = false;

    // 同一区域同时监听 pointerdown / touchstart / mousedown，观察触发顺序
    this.on(area, 'pointerdown', (e: any) => this._onComparePointerDown(e as PointerEvent));
    this.on(area, 'touchstart', (e: any) => this._onCompareTouchStart(e as TouchEvent));
    this.on(area, 'mousedown', (e: any) => this._onCompareMouseDown(e as MouseEvent));
  }

  _togglePreventDefault(): void {
  ...
  _getDeviceCapabilities(e: PointerEvent): string {
    const caps = (e as any).sourceCapabilities;
  ...
  _onComparePointerDown(e: PointerEvent): void {
    ...
    if (this._compareFlushTimer) clearTimeout(this._compareFlushTimer);
    this._compareFlushTimer = window.setTimeout(() => this._flushCompareBatch(), 300);
  }

  _onCompareTouchStart(e: TouchEvent): void {
  ...
  _onCompareMouseDown(_e: MouseEvent): void {
    if (!this._compareBatch || this._compareBatch.length === 0) return;
    const t = performance.now() - this._compareBatchStart;
  ...
  _flushCompareBatch(): void {
    if (!this._compareBatch || this._compareBatch.length === 0) return;
    const batch = [...this._compareBatch];
    this._compareBatch = [];
    const order = batch.map((b: CompareItem) => `${b.type}(${(b.time as any).toFixed(1)}ms)`).join(' → ');
    ...
    return [
      ...
      h(Card, {
        ...
        this.state.keyHistory.length > 0 ? h('div', { class: 'log-panel mt-sm', style: { maxHeight: '120px' } },
          ...this.state.keyHistory.map((k: KeyHistoryEntry) => h('div', { class: 'log-panel__line' },
      ...
      h(Card, {
        ...
        gs ? h('div', { class: 'mt-sm' },
          ...
          h('div', { class: 'flex flex-col gap-sm' },
            ...gs.axes.map((v: number, i: number) => h('div', { class: 'flex items-center', style: { gap: '8px', fontSize: '12px' } },
              ...
              h('span', { style: { width: '50px', textAlign: 'right', fontFamily: 'var(--font-family-mono)', color: 'var(--color-text-tertiary)' } }, (v as any).toFixed(3)),
            )),
          ),
          // 按钮状态可视化
          ...
          h('div', { class: 'flex', style: { flexWrap: 'wrap', gap: '6px' } },
            ...gs.buttons.map((b: GamepadButtonState, i: number) => h('div', {
              style: {
                width: '36px',
                height: '36px',
                borderRadius: '6px',
              ...
              title: `Button ${i}: pressed=${b.pressed} value=${(b.value as any).toFixed(2)} touched=${b.touched}`,
            }, String(i))),
          ),
        ) : null,
        !this.state.gamepadConnected ? h('p', { class: 'fs-sm text-tertiary mt-sm' }, '提示：连接手柄后按下任意按钮，将触发 gamepadconnected 事件。') : null as any,
      ),
      ...
      h(Card, {
        ...
        this.state.eventComparison.length > 0 ? h('div', { class: 'mt-sm' },
          ...
          h('div', { class: 'flex', style: { gap: '8px', flexWrap: 'wrap' } },
            ...this.state.eventComparison.map((ev: CompareItem, i: number) => h(Tag, {
              color: ev.type === 'pointerdown' ? 'primary' : ev.type === 'touchstart' ? 'success' : 'warning',
            }, `${i + 1}. ${ev.type} (${(ev.time as any).toFixed(1)}ms${ev.prevented ? ', 已阻止' : ''})`)),
          ),
        ) : null,
        // 对比表
        h('div', { class: 'mt-lg' },
          ...
          h('table', { style: { width: '100%', borderCollapse: 'collapse' } },
            ...
            h('tbody', {},
              ...COMPARISON_TABLE.map((row: ComparisonRow) => h('tr', {},
      ...
      h('div', { class: 'log-panel' },
        ...this.state.logs.map((log: InputEventsLog) => h('div', { class: 'log-panel__line' },
