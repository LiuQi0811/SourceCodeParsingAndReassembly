...
interface CSSViewportUnitsMulticolPageState extends State {
  logs: LogEntry[];
  capsSummary: string;
  viewportInfo: string;
...
export class CSSViewportUnitsMulticolPage extends Page {
  declare state: CSSViewportUnitsMulticolPageState;
  _inited: boolean = false;
  _dynamicStyles: HTMLStyleElement[] = [];


  initialState(): CSSViewportUnitsMulticolPageState {
    return {
      logs: [],
      capsSummary: '',
  ...
  componentDidMount(): void {
    ...
    if (!f.svh) this._addLog('warn', 'svh/lvh/dvh 不可用（Chrome 108+/Safari 15.4+/Firefox 101+）');
    if (!f.cqw) this._addLog('warn', 'cqw/cqh/cqi/cqb/cqmin/cqmax 不可用（需 Chrome 105+/Safari 16+ 容器查询）');
    if (!f.displayContents) this._addLog('warn', 'display: contents 不可用（Firefox 37+/Chrome 65+/Safari 11.1+）');
  ...
  _renderCard1(): Node | string {
    ...
    return h(Card, {
      ...
      h('p', { class: 'fs-sm text-secondary' },
        'CSS Viewport Module Level 5 引入 Small/Large/Dynamic 三档视口单位，解决移动端动态工具栏导致 100vh 溢出问题：svh=最小可见（工具栏展开）、lvh=最大可见（工具栏收起）、dvh=当前可见（动态跟随）。'),
      h('div', { class: 'vp-viewport-strip' },
        h('div', { class: 'vp-vh-cell' }, '10vh'),
        h('div', { class: 'vp-svh-cell' }, '10svh'),
        h('div', { class: 'vp-lvh-cell' }, '10lvh'),
        h('div', { class: 'vp-dvh-cell' }, '10dvh'),
      ),
      h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' },
        this._btn('运行视口单位演示', { type: 'primary', size: 'sm', onClick: () => this._runViewportDemo() }),
      ),
      s.viewportInfo ? h('pre', { class: 'code-block mt-md' }, s.viewportInfo) : null as any,
    );
  }

  // ===================== Card 2：容器查询单位 =====================
  _runContainerDemo(): void {
    const f = this._flags();
  ...
  _renderCard2(): Node | string {
    ...
    return h(Card, {
      ...
      h('p', { class: 'fs-sm text-secondary' },
        '相对最近祖先「容器查询上下文」尺寸的单位，而非视口。配合 container-type 使用，实现组件级响应式排版（同组件在 sidebar/main/footer 不同宽度自适应）。'),
      h('div', { class: 'vp-cq-host' },
        h('div', { class: 'vp-cq-title' }, '容器查询单位演示 (font-size: 5cqi)'),
        h('div', { class: 'vp-cq-text' }, '字号会随容器宽度缩放（min 12px）。resize 容器查看效果。'),
        h('div', { class: 'vp-cq-box' }, 'width: 50cqw; padding: 4cqi'),
        h('div', { class: 'vp-cq-grid' },
          h('div', {}, '3cqi'),
          h('div', {}, '3cqi'),
          h('div', {}, '3cqi'),
        ),
      ...
      h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' },
        this._btn('运行容器单位演示', { type: 'primary', size: 'sm', onClick: () => this._runContainerDemo() }),
      ),
      s.containerInfo ? h('pre', { class: 'code-block mt-md' }, s.containerInfo) : null as any,
    );
  }

  // ===================== Card 3：Multi-column Layout =====================
  _runMulticolDemo(): void {
    const f = this._flags();
    ...
    `);
    const demoHtml = [
      h('div', { class: 'vp-multicol-demo' },
        h('p', {}, '栏目一：这是第一段文字。多栏布局让内容像报纸一样分栏展示，提升阅读体验。break-inside: avoid-column 防止元素被分栏打断。'),
        h('p', {}, '栏目二：第二段内容。column-rule 在栏间画分隔线，column-gap 控制栏间距。'),
        h('div', { class: 'vp-span-all' }, 'column-span: all 跨所有列'),
        h('p', {}, '栏目三：第三段。column-fill: balance 默认尽量让各栏高度平衡；auto 让内容自然填充。'),
        h('p', {}, '栏目四：column-count: 3 强制 3 栏；column-width: 120px 让浏览器按宽度自适应栏数（窄则少栏）。'),
      ),
      h('div', { class: 'vp-multicol-narrow' },
        h('p', {}, 'column-width: 120px 的自适应栏：浏览器根据容器宽度自动计算栏数（container 宽 / 120px），窄屏自动减栏。'),
        h('p', {}, '与 column-count 互斥，columns: 12em 3 简写同时声明（取较小值）。'),
      ),
    ];
  ...
  _renderCard3(): Node | string {
    ...
    return h(Card, {
      ...
      h('p', { class: 'fs-sm text-secondary' },
        '报纸式多栏布局：columns 简写、column-count/column-width 控制栏数与栏宽、column-rule 画分隔线、column-span: all 跨栏标题、break-inside: avoid-column 防止元素被分栏打断。'),
      h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' },
        this._btn('运行多栏布局演示', { type: 'primary', size: 'sm', onClick: () => this._runMulticolDemo() }),
      ),
      s.multicolDemoHtml ? h('div', { class: 'mt-md' }, s.multicolDemoHtml) : null as any,
      s.multicolInfo ? h('pre', { class: 'code-block mt-md' }, s.multicolInfo) : null as any,
    );
  }
  ...
  _renderCard4(): Node | string {
    ...
    return h(Card, {
      ...
      h('p', { class: 'fs-sm text-secondary' },
        'CSS 内在尺寸关键字（min-content/max-content/fit-content/stretch）+ contain-intrinsic-size 占位 + display 阶梯（contents/flow-root/list-item/ruby）。flow-root 创建 BFC 清除浮动替代 clearfix hack；contents 让子元素穿透父元素参与祖父布局。'),
      h('div', { class: 'vp-sizing-grid' },
        h('div', { class: 'vp-sizing-item' },
          h('div', { class: 'fs-sm' }, 'min-content'),
          h('div', { class: 'vp-sz-min' }, '不被断行的最小宽'),
        ),
        h('div', { class: 'vp-sizing-item' },
          h('div', { class: 'fs-sm' }, 'max-content'),
          h('div', { class: 'vp-sz-max' }, '所有内容放一行'),
        ),
        h('div', { class: 'vp-sizing-item' },
          h('div', { class: 'fs-sm' }, 'fit-content'),
          h('div', { class: 'vp-sz-fit' }, '最多 max，最少 min'),
        ),
      ),
      h('div', { class: 'vp-display-row' },
        h('div', { class: 'vp-display-box vp-display-flow-root' },
          h('div', { class: 'vp-float' }, 'float:left'),
          h('div', { class: 'vp-float' }, 'float:left'),
        ),
        h('div', { class: 'vp-display-box' },
          h('div', { style: 'display: contents;' },
            h('div', { style: 'background:#bfdbfe; padding:4px 8px; border-radius:4px;' }, '穿透：父 display:contents'),
          ),
        ),
      ),
      h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' },
        this._btn('运行尺寸+display 演示', { type: 'primary', size: 'sm', onClick: () => this._runSizingDemo() }),
      ),
      s.sizingInfo ? h('pre', { class: 'code-block mt-md' }, s.sizingInfo) : null as any,
    );
  }
