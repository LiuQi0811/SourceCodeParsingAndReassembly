...
export interface TextMediaPageProps extends Props {}

export interface TextMediaLog {
  type: string;
  content: string;
  time: string;
}
...
export interface TextMediaPageState extends State {
  logs: TextMediaLog[];
...
export class TextMediaPage extends Page {
  declare props: TextMediaPageProps;
  declare state: TextMediaPageState;
  _selectionRAF: number | null = null;
  _lastSelectionText: string | null = null;
  _onSelectionChange: ((e: Event) => void) | null = null;
  _editableRAF: number | null = null;
  _onEditableInput: ((e: Event) => void) | null = null;
  _onVoicesChanged: (() => void) | null = null;
  _mediaStream: MediaStream | null = null;
  _mediaRecorder: MediaRecorder | null = null;
  _recordChunks: Blob[] = [];
  _recordTimer: number | null = null;
  _recordStart: number = 0;
  _animation: Animation | null = null;

  initialState(): TextMediaPageState {
    return {
      logs: [],
      selectionText: '',
  ...
  componentDidMount(): void {
    ...
    this._onSelectionChange = () => {
      if (this._selectionRAF) return; // 已有挂起的更新，跳过
      this._selectionRAF = requestAnimationFrame(() => {
        ...
        if (text === this._lastSelectionText) return;
    ...
    const editable = this.$<HTMLElement>('.editable-area');
    if (editable) {
      this._editableRAF = null;
      this._onEditableInput = () => {
        if (this._editableRAF) return;
    ...
    if ('speechSynthesis' in window) {
      const loadVoices = () => {
        ...
        if (voices.length) {
          // 中文优先排序
          const sorted = [...voices].sort((a: SpeechSynthesisVoice, b: SpeechSynthesisVoice) => {
  ...
  componentWillUnmount(): void {
    document.removeEventListener('selectionchange', this._onSelectionChange as EventListener);
    if (this._selectionRAF) { cancelAnimationFrame(this._selectionRAF); this._selectionRAF = null; }
    if (this._editableRAF) { cancelAnimationFrame(this._editableRAF); this._editableRAF = null; }
    const editable = this.$<HTMLElement>('.editable-area');
  ...
  // ============ 1. Selection / Range ============
  _selectByCode(): void {
    const demo = this.$<HTMLElement>('.selection-demo');
    if (!demo) return;
  ...
  _exec(cmd: string, value: string | null = null): void {
    if (typeof document.execCommand === 'function') {
      document.execCommand(cmd, false, (value as any));
      const editable = this.$<HTMLElement>('.editable-area');
  ...
  _speak(): void {
    ...
    const voice = this.state.voices.find((v: SpeechSynthesisVoice) => v.voiceURI === this.state.selectedVoiceURI);
    ...
    u.onerror = (e: SpeechSynthesisErrorEvent) => { this._addLog('error', '朗读错误: ' + e.error); };
    u.onboundary = (e: SpeechSynthesisEvent) => { this._addLog('info', `边界: charIndex=${e.charIndex}`); };
    window.speechSynthesis.speak(u);
    this.setState({ ttsState: { speaking: false, paused: false, pending: true } });
  }

  _pauseTts(): void { window.speechSynthesis?.pause(); this.setState({ ttsState: { speaking: true, paused: true, pending: false } }); }
  _resumeTts(): void { window.speechSynthesis?.resume(); this.setState({ ttsState: { speaking: true, paused: false, pending: false } }); }
  _cancelTts(): void { window.speechSynthesis?.cancel(); this.setState({ ttsState: { speaking: false, paused: false, pending: false } }); }
  ...
  async _startCamera(): Promise<void> {
    try {
      ...
      const video = this.$<HTMLVideoElement>('.media-video');
  ...
  _stopMedia(): void {
    if (this._mediaRecorder && this._mediaRecorder.state !== 'inactive') {
      try { this._mediaRecorder.stop(); } catch { /* noop */ }
    }
    if (this._mediaStream) {
      this._mediaStream.getTracks().forEach((t: MediaStreamTrack) => t.stop());
      this._mediaStream = null;
    }
  ...
  _startRecording(): void {
    ...
    this._mediaRecorder.ondataavailable = (e: BlobEvent) => { if (e.data.size > 0) this._recordChunks.push(e.data); };
    this._mediaRecorder.onstop = () => {
      ...
      const playback = this.$<HTMLVideoElement>('.media-playback');
  ...
  _playAnim(keyframes: Keyframe[] | PropertyIndexedKeyframes, label: string): void {
    const box = this.$<HTMLElement>('.waa-demo-box');
    if (!box) return;
  ...
  _animControl(action: string): void {
  ...
  async _createBlob(): Promise<void> {
    ...
    return [
        ...
        ttsSupported ? h('div', { class: 'flex flex-col gap-sm' },
          h('div', { class: 'flex items-center gap-sm' },
            ...
            h('select', {
              ...
              onChange: (e: Event) => this.setState({ selectedVoiceURI: (e.target as HTMLSelectElement).value }),
            },
              ...this.state.voices.map((v: SpeechSynthesisVoice) => h('option', { value: v.voiceURI }, `${v.name} (${v.lang})`)),
            ),
          ),
          h('textarea', {
            class: 'input',
            ...
            // 直接改 state 字段，不触发 setState/rerender（否则 textarea 重建丢光标）
            onInput: (e: Event) => { this.state.ttsText = (e.target as HTMLTextAreaElement).value; },
          }),
      ...
      h(Card, { title: h('div', { class: 'flex items-center gap-sm' },
        ...
        mediaSupported ? h('div', { class: 'flex flex-col gap-sm' },
          ...
          this.state.mediaError ? h(Alert, { type: 'error', message: '错误', description: this.state.mediaError }) : null as any,
      ...
      h(Card, { title: '6. FormData / Blob / URL.createObjectURL（文件处理）' },
        h('div', { class: 'flex flex-col gap-sm' },
          h('input', {
            type: 'file',
            onChange: (e: Event) => this._onFileChange(e),
            style: { fontSize: '14px' },
          }),
      ...
      h(Card, { title: '事件日志' },
        h('div', { class: 'log-panel' },
          ...this.state.logs.map((log: TextMediaLog) => h('div', { class: 'log-panel__line' },
