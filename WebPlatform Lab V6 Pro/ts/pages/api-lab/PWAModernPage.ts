...
export interface PWAModernPageProps extends Props {}

export interface PWAModernPageState extends State {}

export class PWAModernPage extends Page {
  declare props: PWAModernPageProps;
  declare state: PWAModernPageState;
  _deferredPrompt: any = null;
  _dmMql: MediaQueryList | null = null;
  _inited: boolean = false;
  _onAppInstalled: any = null;
  _onBeforeInstallPrompt: any = null;
  _onDisplayModeChange: any = null;
  _onFreeze: (() => void) | null = null;
  _onGeometryChange: any = null;
  _onNavigate: any = null;
  _onNavigateError: (() => void) | null = null;
  _onNavigateSuccess: (() => void) | null = null;
  _onPageHide: any = null;
  _onPageShow: any = null;
  _onPipEnter: any = null;
  _onPipLeave: any = null;
  _onResume: (() => void) | null = null;
  _onVisibilityChange: any = null;
  _pipWindow: any = null;
  initialState(): PWAModernPageState {
    return {
      logs: [],
      // Card 1
  ...
  componentDidMount(): void {
    ...
    if (c.beforeInstallPrompt) {
      this._onBeforeInstallPrompt = (e: any) => {
    ...
    if (typeof window.matchMedia === 'function') {
      try {
        ...
        sync();
        this._onDisplayModeChange = (e: any) => {
    ...
    if (c.launchQueue && window.launchQueue) {
      try {
        window.launchQueue.setConsumer((params: any) => {
    ...
    if (c.navigation && window.navigation) {
      ...
      const syncNav = () => {
        let cur: any = ''; let count = 0;
        try { cur = nav.currentEntry ? nav.currentEntry.url : ''; } catch { /* noop */ }
        try { count = typeof nav.entries === 'function' ? (nav as any).entries().length : 0; } catch { /* noop */ }
        this.setState({ navCurrentURL: cur, navEntryCount: count });
      };
      syncNav();
      this._onNavigate = (e: any) => {
      ...
      this._onNavigateError = ((e: any) => this._addLog('err', `navigateerror 事件：${e.message || ''}`)) as any;
      try {
        nav.addEventListener('navigate', this._onNavigate);
        nav.addEventListener('navigatesuccess', this._onNavigateSuccess as any);
        nav.addEventListener('navigateerror', this._onNavigateError as any);
      } catch (err: any) {
  ...
  _statusTag(label: any, ok: any) {
    return h(Tag, { color: ok ? 'success' : 'default' }, `${label}:${ok ? '是' : '否'}`);
  }

  _kvRow(label: any, value: any, unit: any = '') {
  ...
  _logApiErr(tag: any, api: any, err: any) {
    const name = err && err.name ? err.name : 'Error';
    const msg = err && err.message ? err.message : String(err);
    this._addLog(tag, `${api} 失败：${name} — ${msg}`);
  }

  _summarizeLaunchParams(params: any) {
    const out: any = { targetURL: '', filesCount: 0, files: [] };
    try { out.targetURL = params.targetURL || ''; } catch { /* noop */ }
    try {
      const files = params.files;
      if (files && files.length) {
        out.filesCount = files.length;
        out.files = Array.from(files).map((f: any) => ({
          name: f.name || '',
          kind: f.kind || '',
          type: f.type || '',
        }));
  ...
  _checkDisplayMode() {
    ...
    const hit = modes.find((m: any) => window.matchMedia(`(display-mode: ${m})`).matches) || 'unknown';
  ...
  async _badgeSet5() {
    ...
    } catch (err: any) { this._logApiErr('badge', 'setAppBadge(5)', err); }
  }
  ...
  async _badgeSetDot() {
    ...
    } catch (err: any) { this._logApiErr('badge', 'setAppBadge()', err); }
  }
  ...
  async _badgeClear() {
    ...
    } catch (err: any) { this._logApiErr('badge', 'clearAppBadge()', err); }
  }
  ...
  _wcoReadRect() {
    ...
    } catch (err: any) { this._logApiErr('wco', 'getTitlebarAreaRect()', err); }
  }
  ...
  async _docPipOpen() {
    ...
    try {
      ...
      const src = (this.$('.js-pip-source') as any);
      if (src) {
    ...
    } catch (err: any) { this._logApiErr('dppip', 'requestWindow', err); }
  }
  ...
  _docPipClose() {
    ...
    } catch (err: any) { this._logApiErr('dppip', 'close()', err); }
  }
  ...
  _protoRegister() {
    ...
    try {
      ...
      (navigator as any).registerProtocolHandler('web+study', url, 'API 实验室学习协议');
    ...
    } catch (err: any) { this._logApiErr('proto', 'registerProtocolHandler', err); }
  }
  ...
  _protoUnregister() {
    ...
    try {
      const url = new URL('/handle?uri=%s', window.location.origin).href;
      (navigator as any).unregisterProtocolHandler!('web+study', url);
      this._addLog('proto', `unregisterProtocolHandler('web+study', '${url}') 完成`);
    } catch (err: any) { this._logApiErr('proto', 'unregisterProtocolHandler', err); }
  }
  ...
  _protoRegisterMailto() {
    ...
    try {
      ...
      (navigator as any).registerProtocolHandler('mailto', url, 'API 实验室邮件处理');
      this._addLog('proto', `registerProtocolHandler('mailto', '${url}', …) 完成（注册 mailto 会让 PWA 出现在邮件应用列表）`);
    } catch (err: any) { this._logApiErr('proto', 'registerProtocolHandler(mailto)', err); }
  }
  ...
  _navBack() {
    ...
    } catch (err: any) { this._logApiErr('nav', 'back()', err); }
  }
  ...
  _navForward() {
    ...
    } catch (err: any) { this._logApiErr('nav', 'forward()', err); }
  }
  ...
  _navReload() {
    ...
    } catch (err: any) { this._logApiErr('nav', 'reload()', err); }
  }
  ...
  _navNavigate() {
    ...
    } catch (err: any) { this._logApiErr('nav', 'navigate()', err); }
  }

  _navListEntries() {
    if (!this._caps().navigation) { this._addLog('err', '当前浏览器不支持 Navigation API'); return; }
    try {
      const entries = (window.navigation as any).entries();
    ...
    } catch (err: any) { this._logApiErr('nav', 'entries()', err); }
  }
    ...
    return [
      ...
      h('div', { class: 'feature-grid mt-lg' },
        ...
        h(Card, {
          ...
          h('div', { class: 'flex flex-col gap-sm' },
            ...
            s.launchParams && h('div', { class: 'log-panel', style: { height: 'auto', padding: '8px' } },
              ...
              ...(s.launchParams.files || []).map((f: any, i: any) => this._kvRow(`file[${i}].name`, `${f.name} (${f.kind}/${f.type})`)),
            ),
        ...
        h(Card, {
          ...
          h('div', { class: 'flex flex-col gap-sm' },
            h('p', { class: 'fs-sm text-tertiary' },
              'Navigation API：', h('code', {}, '(navigation as any).entries()'), ' 返回历史记录列表，', h('code', {}, 'navigation.currentEntry'), ' 是当前条目（含 url/key/id/getState()）；监听 ', h('code', {}, 'navigate'), ' 事件并调用 ', h('code', {}, 'e.intercept({ handler })'), ' 拦截；', h('code', {}, 'navigation.navigate(url)'), ' / ', h('code', {}, 'back()'), ' / ', h('code', {}, 'forward()'), ' / ', h('code', {}, 'reload()'), ' 编程导航；另有 ', h('code', {}, 'navigatesuccess'), ' / ', h('code', {}, 'navigateerror'), ' 事件。'),
            h('p', { class: 'fs-sm text-tertiary' },
            ...
            h('div', { class: 'log-panel', style: { height: 'auto', padding: '8px' } },
              ...
              this._kvRow('(navigation as any).entries().length', s.navEntryCount),
