...
export interface FormAPIDeepPageProps extends Props {}

export interface FormAPIDeepPageState extends State {}

export class FormAPIDeepPage extends Page {
  declare props: FormAPIDeepPageProps;
  declare state: FormAPIDeepPageState;
  _calcForm: any = null;
  _calcHandler: any = null;
  _formdataForm: any = null;
  _formdataHandler: any = null;
  _inited: boolean = false;
  _invalidHandler: any = null;
  _pickerInput: any = null;
  _submitForm: any = null;
  _submitFormHandler: any = null;
  _submitFormdataHandler: any = null;
  _submitHandler: any = null;
  _validationInput: any = null;
  // —— 初始 state ——
  initialState(): FormAPIDeepPageState {
    return {
      logs: [],
      capsSummary: '',
  ...
  componentDidMount(): void {
    ...
    const hasCheckValidity = typeof document !== 'undefined'
      && typeof (document.createElement('input') as any).checkValidity === 'function';
    const hasReportValidity = typeof document !== 'undefined'
      && typeof (document.createElement('input') as any).reportValidity === 'function';
    ...
    if (!hasFormdataEvent) this._addLog('warn', 'FormDataEvent 不可用（需 Chrome 77+ / Firefox 72+，jsdom 未实现）');
    if (!hasReportValidity) this._addLog('warn', 'reportValidity 不可用（jsdom 可能未实现原生提示）');
    if (!hasShowPicker) this._addLog('warn', 'showPicker 不可用（需 Chrome 99+，jsdom 未实现）');
    if (!hasRequestSubmit) this._addLog('warn', 'requestSubmit 不可用（jsdom 可能未实现）');
    if (!hasValidityState) this._addLog('warn', 'ValidityState 不可用（jsdom 可能未实现完整 validity 对象）');
    // Card 9：field-sizing 能力检测
    let hasFieldSizing = false;
    try { hasFieldSizing = typeof CSS !== 'undefined' && typeof CSS.supports === 'function' && CSS.supports('field-sizing', 'content'); } catch { hasFieldSizing = false; }
    if (!hasFieldSizing) this._addLog('warn', 'CSS field-sizing: content 不可用（Chrome 123+，jsdom 不识别）');
  }

  componentWillUnmount(): void {
    // 移除事件监听器并释放引用，便于 GC，每个调用包 try/catch
    if (this._formdataForm && this._formdataHandler) {
      try { this._formdataForm.removeEventListener('formdata', this._formdataHandler); } catch { /* noop */ }
    }
    if (this._formdataForm && this._submitHandler) {
      try { this._formdataForm.removeEventListener('submit', this._submitHandler); } catch { /* noop */ }
    }
    if (this._validationInput && this._invalidHandler) {
      try { this._validationInput.removeEventListener('invalid', this._invalidHandler); } catch { /* noop */ }
    }
    if (this._calcForm && this._calcHandler) {
      try { this._calcForm.removeEventListener('input', this._calcHandler); } catch { /* noop */ }
    }
    if (this._submitForm && this._submitFormHandler) {
      try { this._submitForm.removeEventListener('submit', this._submitFormHandler); } catch { /* noop */ }
    }
    if (this._submitForm && this._submitFormdataHandler) {
      try { this._submitForm.removeEventListener('formdata', this._submitFormdataHandler); } catch { /* noop */ }
    }
    // 重置表单状态
    const resetForm = (form: any) => {
      if (form) { try { form.reset(); } catch { /* noop */ } }
    };
  ...
  _caps() {
    ...
    return {
      ...
      fieldSizing: (() => { try { return typeof CSS !== 'undefined' && typeof CSS.supports === 'function' && CSS.supports('field-sizing', 'content'); } catch { return false; } })(),
    };
  }
  ...
  _ensureFormdataForm() {
    ...
    if (this._formdataForm) return this._formdataForm;
    try {
  ...
  _listenFormdataEvent() {
    ...
    if (!caps.formdataEvent) {
      ...
      this.setState({
        formdataInfo:
          `formdata 事件能力检测：'FormDataEvent' in window = ${caps.formdataEvent}\n` +
    ...
    try {
      ...
      if (this._formdataHandler) {
        try { form.removeEventListener('formdata', this._formdataHandler); } catch { /* noop */ }
      }
      const handler = (e: any) => {
        ...
        try {
          ...
          const entries: any[] = [];
          for (const [k, v] of (e.formData as any).entries()) {
            entries.push(`${k}=${v}`);
          }
      ...
      this.setState({
        formdataInfo:
          `form.addEventListener('formdata', handler) 已注册 ✓\n` +
  ...
  _triggerRequestSubmit() {
    ...
    if (!caps.requestSubmit) {
      ...
      this.setState({
        formdataInfo:
          `form.requestSubmit() 能力检测：HTMLFormElement.prototype.requestSubmit = ${caps.requestSubmit ? 'function' : 'undefined'}\n` +
  ...
  _triggerRawSubmit() {
    ...
    try {
      ...
      this.setState({
        formdataInfo:
          `form.submit() 已调用（jsdom 中无网络层，通常无副作用）\n` +
  ...
  _renderCard1() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.formdataEvent ? 'success' : 'error' }, caps.formdataEvent ? 'FormDataEvent ✓' : '不可用'),
        h(Tag, { color: 'primary' }, 'formdata / requestSubmit'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'formdata 事件在表单提交时触发（submit 之后、FormData 构造后、提交前）。回调接收 FormDataEvent，其 formData 属性返回可修改的 FormData 对象，可直接 append 追加字段或 delete 删除字段，无需重新构造。vs 传统 submit 事件 + new FormData(form)：formdata 事件让你在提交前最后一刻修改数据。form.requestSubmit() 程序化触发表单提交（走完整事件链 submit → formdata → 提交），vs form.submit() 不触发表单事件、不验证、不走 formdata 事件。浏览器支持 Chrome 77+ / Firefox 72+，用 "FormDataEvent" in window 检测。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('注册 formdata 监听', { type: 'primary', size: 'sm', onClick: () => this._listenFormdataEvent() }),
          this._btn('requestSubmit 触发', { type: 'primary', size: 'sm', disabled: !caps.requestSubmit, onClick: () => this._triggerRequestSubmit() }),
          this._btn('form.submit() 对比', { size: 'sm', onClick: () => this._triggerRawSubmit() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'formdata 事件状态：'),
        h('pre', { class: 'code-block', style: { maxHeight: '220px', overflow: 'auto' } },
          h('code', {}, s.formdataInfo || '（点击「注册 formdata 监听」然后「requestSubmit 触发」测试）')),
...
if ('FormDataEvent' in window) { /* 支持 formdata 事件 */ }`)),
  ...
  _ensureValidationInput() {
    ...
    if (this._validationInput) return this._validationInput;
    try {
  ...
  _runCheckValidity() {
    ...
    if (!caps.checkValidity) {
      ...
      this.setState({
        validationInfo:
          `checkValidity 能力检测：input.checkValidity = ${caps.checkValidity ? 'function' : 'undefined'}\n` +
  ...
  _runSetCustomValidity() {
    ...
    try {
      ...
      this.setState({
        validationInfo:
          `element.setCustomValidity(msg) 演示 ✓\n` +
  ...
  _runCustomInvalidUI() {
    ...
    try {
      ...
      if (this._invalidHandler) {
        try { input.removeEventListener('invalid', this._invalidHandler); } catch { /* noop */ }
      }
      const handler = (e: any) => {
  ...
  _renderCard2() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.checkValidity ? 'success' : 'error' }, caps.checkValidity ? 'checkValidity ✓' : '不可用'),
        h(Tag, { color: caps.reportValidity ? 'success' : 'warning' }, caps.reportValidity ? 'reportValidity ✓' : 'report ✗'),
        h(Tag, { color: caps.validityState ? 'success' : 'error' }, caps.validityState ? 'ValidityState ✓' : 'ValidityState ✗'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
        ...
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('checkValidity + validity', { type: 'primary', size: 'sm', disabled: !caps.checkValidity, onClick: () => this._runCheckValidity() }),
          this._btn('reportValidity', { type: 'primary', size: 'sm', disabled: !caps.reportValidity, onClick: () => this._runReportValidity() }),
          this._btn('setCustomValidity', { size: 'sm', disabled: !caps.checkValidity, onClick: () => this._runSetCustomValidity() }),
          this._btn('自定义 invalid UI', { size: 'sm', disabled: !caps.checkValidity, onClick: () => this._runCustomInvalidUI() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'Validation 状态：'),
        h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } },
          h('code', {}, s.validationInfo || '（点击「checkValidity + validity」开始）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
        h('pre', { class: 'code-block', style: { maxHeight: '130px', overflow: 'auto' } },
          h('code', {},
`input.addEventListener('invalid', (e: any) => {
...
// validity 对象：valueMissing / typeMismatch / patternMismatch / valid ...`)),
  ...
  _ensurePickerInput() {
    ...
    if (this._pickerInput) return this._pickerInput;
    try {
      const input = (document.createElement('input') as any);
      input.type = 'color';
      input.value = '#3a7bd5';
  ...
  _callShowPicker() {
    ...
    if (!caps.showPicker) {
      ...
      this.setState({
        showPickerInfo:
          `showPicker 能力检测：'showPicker' in HTMLInputElement.prototype = ${caps.showPicker}\n` +
    ...
    try {
      ...
      this.setState({
        showPickerInfo:
          `input.showPicker() 抛出：${err.name} - ${err.message}\n` +
  ...
  _renderCard3() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.showPicker ? 'success' : 'error' }, caps.showPicker ? 'showPicker ✓' : '不可用'),
        h(Tag, { color: 'primary' }, 'Chrome 99+'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'HTMLInputElement.showPicker() 程序化弹出 date / time / color / month / week / datetime-local / file / range 类型 input 的原生选择器。必须在用户手势（click 等）中调用，否则抛 DOMException（SecurityError）。vs 点击 input 原生弹出：showPicker 可在其他元素点击时触发选择器（如点按钮弹另一处 input 的颜色选择器）。浏览器支持 Chrome 99+，用 "showPicker" in HTMLInputElement.prototype 检测。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('showPicker() 弹出', { type: 'primary', size: 'sm', disabled: !caps.showPicker, onClick: () => this._callShowPicker() }),
          this._btn('检测各类型支持', { size: 'sm', onClick: () => this._detectPickerTypes() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'showPicker 状态：'),
        h('pre', { class: 'code-block', style: { maxHeight: '220px', overflow: 'auto' } },
          h('code', {}, s.showPickerInfo || '（点击「showPicker() 弹出」或「检测各类型支持」）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
...
// 非用户手势调用抛 SecurityError；类型不匹配抛 NotSupportedError`)),
  ...
  _demoFormDataBasics() {
    ...
    if (!caps.formData) {
      ...
      this.setState({
        formDataInfo:
          `FormData 能力检测：typeof FormData = ${caps.formData ? 'function' : 'undefined'}\n` +
    ...
    try {
      ...
      const entriesArr = Array.from((fd as any).entries());
      const keysArr = Array.from((fd as any).keys());
      const valuesArr = Array.from((fd as any).values());
      // delete
      fd.delete('age');
  ...
  _demoFormDataFileUpload() {
    ...
    try {
      ...
      // 读取文件字段
      const fileEntry: any = fd.get('file');
      const fileName = fileEntry && typeof (fileEntry as any).name === 'string' ? (fileEntry as any).name : '(无 name 属性)';
      const fileType = fileEntry && typeof (fileEntry as any).type === 'string' ? (fileEntry as any).type : '(无 type)';
      const fileSize = fileEntry && typeof (fileEntry as any).size === 'number' ? (fileEntry as any).size : '(无 size)';
      const tagsAll = fd.getAll('tags');
      const keysArr = Array.from((fd as any).keys());
      // FormData 没有 size 属性，但可用 entries 遍历计数
      let count = 0;
      for (const _ of (fd as any).entries()) count++;           // eslint-disable-line no-unused-vars
      this.setState({
        formDataInfo:
          `手动构造 FormData 上传文件演示 ✓\n\n` +
          `① new Blob([content], { type: 'text/plain' }) 构造模拟文件\n` +
          `② fd.append('title', '我的文件标题')（普通字段）\n` +
          `③ fd.append('tags', 'js') + ('tags', 'formdata') + ('tags', 'upload')（多值字段）\n` +
          `④ fd.append('file', blob, 'demo.txt')（文件字段，blob + filename）\n\n` +
          `fd.get('file') → ${fileEntry ? (fileEntry as any).constructor.name : 'null'}\n` +
  ...
  _renderCard4() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.formData ? 'success' : 'error' }, caps.formData ? 'FormData ✓' : '不可用'),
        h(Tag, { color: 'primary' }, 'append / set / get / entries'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'FormData API 表示表单数据键值对。new FormData(form?) 构造，传 form 元素则自动提取所有命名控件字段（checkbox 仅 checked、radio 仅选中、file 为 File 对象）。append(name, value) / append(name, blob, filename) 追加（同名多值）；set(name, value) 设置（覆盖同名）；get(name) 取第一个 / getAll(name) 取全部；has(name) 检查存在 / delete(name) 删除；entries() / keys() / values() 返回迭代器。FormData 没有 size 属性，需遍历 entries 计数。手动构造 FormData 可用于 fetch 上传文件（body: fd 自动设 multipart/form-data）。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('基础接口演示', { type: 'primary', size: 'sm', disabled: !caps.formData, onClick: () => this._demoFormDataBasics() }),
          this._btn('new FormData(form)', { type: 'primary', size: 'sm', disabled: !caps.formData, onClick: () => this._demoFormDataFromForm() }),
          this._btn('手动构造上传文件', { size: 'sm', disabled: !caps.formData, onClick: () => this._demoFormDataFileUpload() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'FormData 状态：'),
        h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } },
          h('code', {}, s.formDataInfo || '（点击「基础接口演示」开始）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
        h('pre', { class: 'code-block', style: { maxHeight: '130px', overflow: 'auto' } },
          h('code', {},
`const fd = new FormData(form);              // 自动提取字段
...
for (const [k, v] of (fd as any).entries()) { /* 遍历 */ }`)),
  ...
  _demoNativeComponents() {
    ...
    try {
      ...
      } catch (e: any) { lines.push(`① datalist 检测失败：${e.message}`); }
      // ② progress
      try {
      ...
      } catch (e: any) { lines.push(`② progress 检测失败：${e.message}`); }
      // ③ meter
      try {
      ...
      } catch (e: any) { lines.push(`③ meter 检测失败：${e.message}`); }
      // ④ output
      try {
      ...
      } catch (e: any) { lines.push(`④ output 检测失败：${e.message}`); }
      lines.push('');
  ...
  _demoLiveCalculator() {
    ...
    try {
      ...
      document.body.appendChild(form);
      this._calcForm = form;
      const out = form.elements.namedItem('result') as any;      // form.elements.namedItem 取控件
      const aInput = form.elements.namedItem('a') as any;
      const bInput = form.elements.namedItem('b') as any;
      // 计算函数
      const compute = () => {
  ...
  _renderCard5() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.document ? 'success' : 'error' }, caps.document ? 'DOM ✓' : '不可用'),
        h(Tag, { color: 'primary' }, 'datalist / progress / meter / output'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '原生表单组件提供常用 UI 而无需 JS 库。<datalist id="..."> + <input list="..."> 实现原生自动补全（input.list 属性引用 datalist）；<progress value max> 进度条，position = value/max（无 value 时为 -1 表示不确定）；<meter value min max low high optimum> 度量条，low/high/optimum 决定颜色（value 在 optimum 区间绿、靠近边界黄、超出红）；<output for="a b"> 计算结果展示元素，for 关联输入 id（空格分隔），value 设当前结果、defaultValue 是初始值。常配合 input + oninput 实现实时计算器。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('组件属性检测', { type: 'primary', size: 'sm', disabled: !caps.document, onClick: () => this._demoNativeComponents() }),
          this._btn('实时计算器', { type: 'primary', size: 'sm', disabled: !caps.document, onClick: () => this._demoLiveCalculator() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '原生组件状态：'),
        h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } },
          h('code', {}, s.nativeFormInfo || '（点击「组件属性检测」或「实时计算器」）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
        h('pre', { class: 'code-block', style: { maxHeight: '130px', overflow: 'auto' } },
          h('code', {},
`<datalist id="fruits">
...
<form oninput="result.value = Number(a.value) + Number(b.value)">
  ...
  _ensureSubmitForm() {
    ...
    if (this._submitForm) return this._submitForm;
    try {
      const form = document.createElement('form');
      form.innerHTML =
  ...
  _demoRequestSubmitVsSubmit() {
    ...
    if (caps.formdataEvent && !this._submitFormdataHandler) {
      try {
        this._submitFormdataHandler = (e: any) => {
    ...
    try {
      ...
      if (caps.requestSubmit) {
        try {
          ...
          let submitter = null;
          if (publishBtn && publishBtn.length >= 2) submitter = publishBtn[1]; // 发布按钮
          if (submitter) {
  ...
  _demoValidateThenSubmit() {
    ...
    if (!caps.requestSubmit) {
      ...
      this.setState({
        submitInfo:
          `JS 验证后 requestSubmit 演示：requestSubmit 不可用（${caps.requestSubmit ? '✓' : '✗'}）\n` +
    ...
    try {
      ...
      try {
        ...
        if (!r1) lines.push('   → 验证失败，跳过提交，应提示用户填写必填字段');
      } catch (e: any) { lines.push(`① 验证失败抛错：${e.message}`); }
      ...
      try {
        ...
        if (r2) {
          ...
          if (!this._submitFormHandler) {
            this._submitFormHandler = (e: any) => { e.preventDefault(); this._addLog('submit', 'submit 事件触发并 preventDefault'); };
            try { form.addEventListener('submit', this._submitFormHandler); } catch { /* noop */ }
          }
          form.requestSubmit();                            // 走完整事件链
          lines.push('   → form.requestSubmit() 已调用，触发 submit → formdata → 提交 事件链');
        }
      } catch (e: any) { lines.push(`② 验证后提交抛错：${e.name} - ${e.message}`); }
      lines.push('');
  ...
  _renderCard6() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.requestSubmit ? 'success' : 'error' }, caps.requestSubmit ? 'requestSubmit ✓' : '不可用'),
        h(Tag, { color: 'primary' }, 'requestSubmit / submit / elements'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'form.requestSubmit(submitter?) 是现代表单提交入口，程序化触发表单事件链（submit → formdata → 提交），会执行约束验证（required 等失败则不提交）。vs form.submit() 底层直接提交，不触发 submit/formdata 事件、不做验证。submitter 参数指定提交按钮，影响 formAction / formMethod / formEnctype / formNoValidate / formTarget。form.elements 返回 HTMLFormControlsCollection 访问所有控件；form.elements.namedItem(name) 按 name 取控件，同名多控件返回 RadioNodeList。实战：JS 验证通过后 requestSubmit() 走完整事件链，可在 submit 回调 preventDefault 改用 fetch/AJAX 提交。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('form.elements / namedItem', { type: 'primary', size: 'sm', disabled: !caps.document, onClick: () => this._demoFormElements() }),
          this._btn('requestSubmit vs submit', { type: 'primary', size: 'sm', disabled: !caps.document, onClick: () => this._demoRequestSubmitVsSubmit() }),
          this._btn('验证后 requestSubmit', { size: 'sm', disabled: !caps.document, onClick: () => this._demoValidateThenSubmit() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '提交模式状态：'),
        h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } },
          h('code', {}, s.submitInfo || '（点击「form.elements / namedItem」开始）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
        h('pre', { class: 'code-block', style: { maxHeight: '140px', overflow: 'auto' } },
          h('code', {},
`// JS 验证后 requestSubmit 走完整事件链
...
const actionList = form.elements.namedItem('action'); // RadioNodeList`)),
  ...
  _renderCard9() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.fieldSizing ? 'success' : 'error' }, caps.fieldSizing ? 'field-sizing ✓' : 'field-sizing ✗'),
        h(Tag, { color: 'primary' }, 'CSSWG 2024'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'field-sizing: content（CSSWG 2024）让 textarea/input 按内容自动调整尺寸，覆盖 cols/rows 默认值，告别 JS 监听 input 测量 scrollHeight 的传统方案。配合 min-height/max-height 约束自适应范围，超出出现滚动条。Chrome 123+ 支持，Firefox/Safari 未实现。降级：不支持时按 cols/rows 固定尺寸，用 JS input 事件手动设 height。jsdom 不识别该属性，演示仅展示文档与代码。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('展示 field-sizing 文档与代码', { type: 'primary', size: 'sm', onClick: () => this._runFieldSizingDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'field-sizing 文档与示例：'),
        h('pre', { class: 'code-block', style: { maxHeight: '520px', overflow: 'auto' } },
          h('code', {}, s.fieldSizingInfo || '（点击按钮查看 field-sizing 完整文档与自适应输入框示例）')),
        h(Alert, {
          type: 'info',
  ...
  render() {
    ...
    return h('div', { class: 'page api-lab-page' },
      h('h2', { class: 'section-title' }, '表单 API 深度'),
      h('p', { class: 'fs-sm text-secondary mb-md' },
        '本页演示 formdata 事件与 FormDataEvent、Constraint Validation API（checkValidity/reportValidity/setCustomValidity/validity）、HTMLInputElement.showPicker()、FormData 完整接口、原生表单组件（datalist/progress/meter/output）、表单提交现代模式（requestSubmit vs submit）。所有 API 调用前做 typeof/in 检测，不可用时仅记日志。'),
