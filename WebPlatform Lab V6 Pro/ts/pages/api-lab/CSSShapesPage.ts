...
interface CSSShapesPageState extends State {
  logs: LogEntry[];
  capsSummary: string;
  overviewInfo: string;
  shapeOutsideInfo: string;
  circleInfo: string;
  insetPolygonInfo: string;
  marginThresholdInfo: string;
  imageShapeInfo: string;
  magazineInfo: string;
  pitfallsInfo: string;
}
...
export class CSSShapesPage extends Page {
  declare state: CSSShapesPageState;
  _inited: boolean = false;
  _dynamicStyles: HTMLStyleElement[] = [];
  _shapeOutsideMode: string = '';
  _circleMode: string = '';
  _circleRadius: string = '';
  _polygonMode: string = '';
  _insetRound: string = '';
  _shapeMargin: string = '';
  _imageThreshold: string = '';
  _boxModel: string = '';
  declare _destroyed: boolean;


  // —— 初始 state ——
  initialState(): CSSShapesPageState {
    return {
      logs: [],
      capsSummary: '',
  ...
  _readMarginThresholdInfo(): string {
    const f = this._flags();
    try {
