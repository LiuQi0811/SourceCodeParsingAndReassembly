...
function* fib(count: number): Generator<number> {
  let a = 0, b = 1;
...
function* delegate(): Generator<number | string> {
...
export interface ModernJSPageProps extends Props {}

export interface ModernJSLog {
  type: string;
  content: string;
  time: string;
}

export interface ModernJSPageState extends State {
  logs: ModernJSLog[];
  weakRefResult: string;
  gcTriggered: boolean;
  iterResults: number[];
  generatorResults: GeneratorResult[];
  asyncIterResults: number[];
  promiseAllResult: string;
  promiseRaceResult: string;
  promiseAllSettledResult: string;
  promiseAnyResult: string;
  arrayMethodDemo: string;
  objectStringDemo: string;
  mapSetDemo: string;
  logicDemo: string;
  dynamicImportResult: string;
}

export class ModernJSPage extends Page {
  declare props: ModernJSPageProps;
  declare state: ModernJSPageState;
  _target: WeakTarget | null = null;
  _weakRef: WeakRef<WeakTarget> | null = null;
  _registry: FinalizationRegistry<string> | null = null;

  initialState(): ModernJSPageState {
    return {
      logs: [],
  ...
  componentDidMount(): void {
    ...
    try {
      this._registry = new FinalizationRegistry((heldValue: string) => {
  ...
  _createWeakRef(): void {
    ...
    const bigObj: WeakTarget = {
      name: 'BigCache-' + Date.now(),
      data: new Array(10000).fill(0).map((_: any, i: any) => i),
    };
  ...
  _runIterators(): void {
    ...
    const iterable: any = {
      data: [10, 20, 30],
      [Symbol.iterator]() {
        const data = (this as any).data as number[];
        let i = 0;
        return {
          next: (): IteratorResult<number> => i < data.length
  ...
  _makePromises(): Promise<string>[] {
    const make = (label: string, shouldResolve: boolean): Promise<string> =>
  ...
  async _runPromiseAll(): Promise<void> {
  ...
  async _runPromiseRace(): Promise<void> {
  ...
  async _runPromiseAllSettled(): Promise<void> {
  ...
  async _runPromiseAny(): Promise<void> {
    ...
    try {
      ...
      const reasons = (err.errors || []).map((e: Error) => e.message).join(', ');
  ...
  async _runPromiseAnyAllReject(): Promise<void> {
    this._addLog('crypto', 'Promise.any（4 个全 reject）开始，预期抛 AggregateError');
    const allReject = ['E1', 'E2', 'E3', 'E4'].map((label: any) =>
      new Promise<string>((_: any, reject: any) => setTimeout(() => reject(new Error(label + ' 失败')), 100 + Math.random() * 400))
    );
    try {
  ...
  _runArrayMethods(): void {
    ...
    // flatMap：map + flat(1)
    L.push(`flatMap(split) = ${JSON.stringify(['hello world', 'foo bar'].flatMap((s: any) => s.split(' ')))}`);
    // findLast / findLastIndex：从末尾查找
    L.push(`findLast(偶数)      = ${arr.findLast((x: any) => x % 2 === 0)}`);
    L.push(`findLastIndex(偶数) = ${arr.findLastIndex((x: any) => x % 2 === 0)}`);
    // 不可变版本：toSorted/toReversed/toSpliced/with（返回新数组，不改原数组）
    L.push(`toSorted        = ${JSON.stringify(arr.toSorted((a: any, b: any) => a - b))}`);
    ...
    if (typeof (Object as any).groupBy === 'function') {
      L.push(`Object.groupBy(奇偶) = ${JSON.stringify((Object as any).groupBy(nums, (x: number) => (x % 2 ? 'odd' : 'even')))}`);
    } else {
      L.push('Object.groupBy 不可用（需 ES2024 环境）');
    }
    if (typeof (Map as any).groupBy === 'function') {
      const mg = (Map as any).groupBy(nums, (x: number) => (x % 2 ? 'odd' : 'even')) as Map<string, number[]>;
  ...
  _runObjectStringNumber(): void {
    const L: string[] = [];
    // Object.hasOwn：比 hasOwnProperty 更安全（对 Object.create(null) 仍可用）
    const protoless = Object.create(null) as Record<string, any>;
    protoless.foo = 'bar';
    ...
    L.push(`matchAll 命中 ${matches.length} 个日期：${matches.map((m: any) => m[0]).join(', ')}`);
    ...
    L.push(`Number.isFinite('100') = ${Number.isFinite('100')}  vs 全局 isFinite('100') = ${isFinite('100' as any)}`);
    L.push(`Number.isNaN('NaN')    = ${Number.isNaN('NaN' as any)}  vs 全局 isNaN('NaN') = ${isNaN('NaN' as any)}`);
  ...
  _runMapSet(): void {
    ...
    const map = new Map<object | string | number, string>();
    const objKey = {};
    map.set(objKey, '对象作键').set('str', '字符串键').set(1, '数字键');
    L.push(`Map.size = ${map.size}，键类型=[${[...(map as any).keys()].map((k: any) => typeof k).join(', ')}]（保持插入顺序）`);
    L.push(`Map vs Object：Map 可用对象作键，Object 只能用字符串/Symbol 键`);
    // Set：去重 + 集合运算
    const setA = new Set([1, 2, 3, 4]);
    const setB = new Set([3, 4, 5, 6]);
    const union = new Set([...setA, ...setB]);
    const intersect = new Set([...setA].filter((x: any) => setB.has(x)));
    const difference = new Set([...setA].filter((x: any) => !setB.has(x)));
  ...
  _runLogic(): void {
    ...
    const demoZero: any = 0;
    const demoEmptyStr: any = '';
    const demoNull: any = null;
    ...
    // ?. 可选链：安全访问深层属性/方法
    const user: { profile?: { name: string }; settings?: { theme: string }; greet?: () => void } = { profile: { name: 'Alice' } };
    ...
    // ??= / ||= / &&= 逻辑赋值
    let x: string | null = null; x ??= '赋值'; L.push(`let x=null; x??='赋值' → ${x}`);
  ...
  async _runDynamicImport(): Promise<void> {
    try {
  ...
  renderPage(): Node | string | (Node | string)[] {
    ...
    const genText = this.state.generatorResults.length
      ? this.state.generatorResults.map((g: any) => `${g.name} = [${g.values.join(', ')}]`).join('\n')
      : '（点击「运行 Generator」）';
      ...
      h(Card, {
        ...
        this.state.logs.length === 0
          ...
          : h('div', { class: 'log-panel' },
            ...this.state.logs.map((log: ModernJSLog) => h('div', { class: 'log-panel__line' },
