...
interface CSSTextAdvancedPageState extends State {
  logs: LogEntry[];
  capsSummary: string;
  textWrapInfo: string;
  textSpacingInfo: string;
  hangingHyphensInfo: string;
  textBoxInfo: string;
  whiteSpaceCollapseInfo: string;
  selectionComboInfo: string;
}
...
export class CSSTextAdvancedPage extends Page {
  declare state: CSSTextAdvancedPageState;
  _inited: boolean = false;
  _dynamicStyles: HTMLStyleElement[] = [];
  _textWrapMode: string = '';
  _trimOn: boolean = false;
  _autospaceOn: boolean = false;
  _hangingOn: boolean = false;
  _hyphensOn: boolean = false;
  _textBoxOn: boolean = false;
  _breakMode: string = '';


  // —— 初始 state ——
  initialState(): CSSTextAdvancedPageState {
    return {
      logs: [],
      capsSummary: '',
  ...
  componentDidMount(): void {
    ...
    if (!f.textWrap) this._addLog('warn', 'text-wrap:balance 不可用或 jsdom 未识别（Chrome 114+ balance / Chrome 117+ pretty / stable）');
    if (!f.textSpacingTrim) this._addLog('warn', 'text-spacing-trim 不可用或 jsdom 未识别（CJK 标点挤压，Chrome 128+ 部分支持）');
    if (!f.textAutospace) this._addLog('warn', 'text-autospace 不可用或 jsdom 未识别（中英自动加空格，采纳缓慢）');
    if (!f.textBox) this._addLog('warn', 'text-box:trim-both cap 不可用或 jsdom 未识别（Chrome 129+ 部分支持）');
    if (!f.hangingPunctuation) this._addLog('warn', 'hanging-punctuation 不可用或 jsdom 未识别（Safari 已支持，Chrome/Firefox 仍实验性）');
  ...
  _setTextWrap(mode: any) {
    ...
    const desc = ({ wrap: '默认溢出断行', nowrap: '不换行', balance: '平衡换行（限 ~6-8 行）', pretty: '避免孤行（异步渐进）', stable: 'contenteditable 稳定' } as Record<string, string>)[(mode as any)];
    this._addLog('textwrap', `切换 text-wrap → ${mode}（${desc}）`);
  }

  _runTextWrapDemo(): void {
    this.setState({ textWrapInfo: this._readTextWrapInfo() });
  ...
  _renderCard1(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['text-wrap', f.textWrap], ['text-wrap-mode', f.textWrapMode], ['text-wrap-style', f.textWrapStyle]]),
        h(Tag, { color: 'primary' }, 'balance / pretty / stable'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'text-wrap 控制换行算法：wrap 默认溢出断行、nowrap 不换行、balance 平衡换行（浏览器选最优断点使最后一行不过短，适合标题，限约 6-8 行）、pretty 避免孤行（orphans/widows，异步渐进，适合正文）、stable 用于 contenteditable（光标在底部输入时不重排上方行）。新拆分 text-wrap-mode（wrap|nowrap）+ text-wrap-style（natural|balance|pretty|stable）两个长写属性。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runTextWrapDemo() }),
          this._btn('wrap', { size: 'sm', disabled: !f.textWrap, onClick: () => this._setTextWrap('wrap') }),
          this._btn('balance', { size: 'sm', disabled: !f.textWrap, onClick: () => this._setTextWrap('balance') }),
          this._btn('pretty', { size: 'sm', disabled: !f.textWrap, onClick: () => this._setTextWrap('pretty') }),
        ),
        h('div', { class: 'text-wrap-stage' },
          h('h4', {}, 'CSS Text Module Level 4 文本换行演示标题（balance 让多行标题更平衡）'),
          h('div', {}, '当前 text-wrap = ' + this._textWrapMode + '。balance 限制约 6-8 行（性能）；pretty 异步渐进处理避免孤行；stable 用于 contenteditable。'),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '260px', overflow: 'auto' } },
          h('code', {}, s.textWrapInfo || '（点击「读取信息」或切换 text-wrap 模式）')),
        h(Alert, {
          type: 'info',
  ...
  _renderCard2(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['text-spacing-trim', f.textSpacingTrim], ['text-autospace', f.textAutospace]]),
        h(Tag, { color: 'primary' }, 'CJK 排版'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'text-spacing-trim 是 CJK 全角标点挤压属性：trim-start 去除行首全角标点空白（避免句号顶头），trim-end 去除行尾，trim-adjacent 合并连续标点间距。text-autospace 在 CJK 与拉丁字母/数字间自动插入空格（中英文混排自动加空格）：ideograph-alpha（CJK↔拉丁）、ideograph-numeric（CJK↔数字）。两者均为 CJK 排版关键属性但浏览器采纳缓慢，生产可用 JS 库（如 pangu.js）兜底。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn(`text-spacing-trim（${this._trimOn ? '开启挤压' : '默认'}）`, { type: 'primary', size: 'sm', disabled: !f.textSpacingTrim, onClick: () => this._toggleTrim() }),
          this._btn(`text-autospace（${this._autospaceOn ? '加空格' : '不加'}）`, { size: 'sm', disabled: !f.textAutospace, onClick: () => this._toggleAutospace() }),
        ),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '中文段落（含全角标点，句号顶头问题）：'),
        h('div', { class: 'text-spacing-cn' },
          '前端技术日新月异。CSS Text Module Level 4 引入了 text-spacing-trim 属性，用于控制 CJK 栫点。开启后，行首句号不再顶头，连续标点间距被合并。这是中文排版的关键属性。'),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '中英文混排（CJK 与拉丁/数字间）：'),
        h('div', { class: 'text-spacing-mix' },
          '使用React 18与TypeScript 5构建应用，CSS Text L4的text-autospace会在中文与English 123之间自动加空格，避免挤在一起。'),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '260px', overflow: 'auto' } },
          h('code', {}, s.textSpacingInfo || '（点击按钮切换 text-spacing-trim / text-autospace）')),
        h(Alert, {
          type: 'warning',
  ...
  _renderCard3(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['hanging-punctuation', f.hangingPunctuation], ['hyphens', f.hyphens], ['hyphenate-limit-chars', f.hyphenateLimitChars]]),
        h(Tag, { color: 'primary' }, '标点悬挂 / 断字'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'hanging-punctuation 让标点悬挂到文本框外：first（行首标点悬挂到左边距外）、last（行尾标点悬挂到右边距外）、force-end/allow-end（换行处标点悬挂），影响文本对齐使正文边缘更整齐。hyphens: auto 自动断字（连字符断字）需配合 lang 属性（如 lang="en"）与浏览器对应语言词典；manual 仅在 &shy; 软连字符处断字。hyphenate-character 自定义连字符字符，hyphenate-limit-chars 控制断字处前后最小字符数与最小单词长度。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn(`hanging-punctuation（${this._hangingOn ? 'first last' : 'none'}）`, { type: 'primary', size: 'sm', disabled: !f.hangingPunctuation, onClick: () => this._toggleHanging() }),
          this._btn(`hyphens（${this._hyphensOn ? 'auto' : 'manual'}）`, { size: 'sm', disabled: !f.hyphens, onClick: () => this._toggleHyphens() }),
        ),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '标点悬挂段落（hanging-punctuation: first last）：'),
        h('div', { class: 'hang-punct-stage' },
          '「CSS Text Module Level 4」是 W3C 的文本排版规范。其中 hanging-punctuation 让行首引号悬挂到边距外，使正文边缘整齐。「开启后」行首标点不再挤进行内。'),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '自动断字段落（hyphens: auto，lang="en"）：'),
        h('div', { class: 'hyphens-stage', lang: 'en' },
          'The internationalization and localization features of modern browsers enable sophisticated typography. Long words like internationalization and responsibility can be hyphenated automatically when hyphens is set to auto and the correct lang attribute is provided.'),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '280px', overflow: 'auto' } },
          h('code', {}, s.hangingHyphensInfo || '（点击按钮切换 hanging-punctuation / hyphens）')),
        h(Alert, {
          type: 'info',
  ...
  _renderCard4(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['text-box', f.textBox], ['text-box-trim', f.textBoxTrim], ['text-box-edge', f.textBoxEdge]]),
        h(Tag, { color: 'primary' }, 'half-leading'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
        ...
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn(`text-box（${this._textBoxOn ? 'trim-both cap' : 'none'}）`, { type: 'primary', size: 'sm', disabled: !f.textBoxTrim, onClick: () => this._toggleTextBox() }),
        ),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '按钮对比（左侧 trim 后精确居中，右侧虚线框显示容器边缘）：'),
        h('div', { class: 'text-box-row' },
          h('button', { class: 'text-box-btn', type: 'button' }, '按钮文字'),
          h('div', { class: 'text-box-box' }, '容器'),
        ),
        h('p', { class: 'fs-sm text-tertiary mt-sm' },
          '真实浏览器中开启 text-box: trim-both cap 后，按钮内文字精确贴合容器上下边缘（无半行距空白），垂直居中更精确。jsdom 不渲染，仅展示用法。'),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '280px', overflow: 'auto' } },
          h('code', {}, s.textBoxInfo || '（点击按钮切换 text-box）')),
        h(Alert, {
          type: 'info',
  ...
  _setBreakMode(mode: any) {
    ...
    this._injectStyle('css-ws-collapse-mode', ((map as any)[(mode as any)]) || map.normal);
    this.setState({ whiteSpaceCollapseInfo: this._readWSCollapseInfo() });
    const desc = ({ normal: '默认 normal', breakall: 'break-all 任意字符断', overflow: 'overflow-wrap:break-word 仅溢出断', keepall: 'keep-all CJK 不断' } as Record<string, string>)[(mode as any)];
  ...
  _renderCard5(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['white-space-collapse', f.whiteSpaceCollapse], ['word-break', f.wordBreak], ['overflow-wrap', f.overflowWrap]]),
        h(Tag, { color: 'primary' }, 'break-all / keep-all'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
        ...
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ whiteSpaceCollapseInfo: this._readWSCollapseInfo() }) }),
          this._btn('normal', { size: 'sm', onClick: () => this._setBreakMode('normal') }),
          this._btn('break-all', { size: 'sm', disabled: !f.wordBreak, onClick: () => this._setBreakMode('breakall') }),
          this._btn('overflow-wrap', { size: 'sm', disabled: !f.overflowWrap, onClick: () => this._setBreakMode('overflow') }),
          this._btn('keep-all', { size: 'sm', disabled: !f.wordBreak, onClick: () => this._setBreakMode('keepall') }),
        ),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '长 URL（break-all vs overflow-wrap:break-word）：'),
        h('div', { class: 'mt-xs' },
          h('div', { class: 'ws-collapse-url ws-collapse-url-breakall' }, longUrl),
          h('div', { class: 'ws-collapse-url ws-collapse-url-overflow' }, longUrl),
        ),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, 'CJK 段落（keep-all 让汉字间不断行）：'),
        h('div', { class: 'ws-collapse-cjk' },
          '层叠样式表是一种用来表现HTML或XML等文件样式的计算机语言。CSS Text Module Level 4 定义了文本排版相关属性。keep-all 模式下汉字之间不会断行，仅在标点或空格处换行。'),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '300px', overflow: 'auto' } },
          h('code', {}, s.whiteSpaceCollapseInfo || '（点击按钮切换换行模式）')),
        h(Alert, {
          type: 'info',
  ...
  _readSelectionComboInfo(): string {
    const f = this._flags();
    try {
  ...
  _renderCard6(): Node | string {
    ...
    const card = new Card({
      title: '6. 文本选择与排版组合',
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['user-select', f.userSelect], ['field-sizing', f.fieldSizing], ['writing-mode', f.writingMode], ['text-justify', f.textJustify]]),
        h(Tag, { color: 'primary' }, 'CSS Text L4 汇总'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '文本选择与排版组合：user-select 控制文本可选性、::selection 选中样式、accent-color 表单强调色、caret-color 光标色、field-sizing 表单自动增高、text-align[-last] 对齐、text-justify 分散对齐方法、text-indent 首行缩进、tab-size Tab 宽度、unicode-bidi 双向文本、writing-mode 书写方向（竖排）、direction 文本方向。本卡片汇总 CSS Text Module Level 4 全部特性检测。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('读取汇总信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runSelectionComboDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '竖排书写（writing-mode: vertical-rl; text-orientation: mixed）：'),
        h('div', { class: 'wm-vertical' },
          'CSS Text Module Level 4 竖排书写模式演示。vertical-rl 是中文古籍排版方向，从右到左竖排。'),
        ...
        h('div', { class: 'sel-demo' },
          '选中这段文字查看 ::selection 效果（蓝底白字）。',
          h('label', { style: { display: 'inline-block', marginLeft: '12px' } },
            h('input', { type: 'checkbox', style: { accentColor: '#1677ff', marginRight: '4px' } }), 'accent-color',
          ),
          h('input', { type: 'text', placeholder: 'caret-color', style: { caretColor: '#ef4444', marginLeft: '12px' } }),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '320px', overflow: 'auto' } },
          h('code', {}, s.selectionComboInfo || '（点击「读取汇总信息」查看 CSS Text L4 全家桶检测）')),
        h(Alert, {
          type: 'info',
  ...
  render(): Node {
    const s = this.state;
    return h('div', { class: 'api-lab-page css-text-advanced-page' },
      h('h2', { class: 'section-title' }, 'CSS Text Module Level 4 高级排版 实验室'),
      h('p', { class: 'fs-sm text-secondary mb-md' },
        '本页演示 CSS Text Module Level 4 高级排版：text-wrap 换行、text-spacing-trim / text-autospace CJK 排版、hanging-punctuation 标点悬挂、hyphens 断字、text-box 半行距、white-space-collapse 与 word-break 换行控制、user-select / ::selection / writing-mode / field-sizing 文本选择与排版组合。所有特性通过 typeof / CSS.supports 能力检测，不可用时仅记日志，绝不抛异常。'),
