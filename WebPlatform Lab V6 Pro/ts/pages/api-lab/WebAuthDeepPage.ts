...
function bufToHex(buf: any) {
  ...
  return Array.from(bytes).map((b: any) => b.toString(16).padStart(2, '0')).join(' ');
}

// 工具：把 ArrayBuffer / Uint8Array 转成 Base64URL 字符串（模拟 credential.id 的编码形式）
function bufToBase64Url(buf: any) {
  const bytes = buf instanceof Uint8Array ? buf : new Uint8Array(buf);
  let bin = '';
...
interface WebAuthDeepPageState extends State {
  logs: any;
  capsSummary: any;
  supportInfo: any;
  platformAuthInfo: any;
  creationOptionsInfo: any;
  attestationResult: any;
  requestOptionsInfo: any;
  assertionResult: any;
  attachmentInfo: any;
  residentKeyInfo: any;
  jsonInfo: any;
  extensionsInfo: any;
  digitalCredentialsInfo: any;
}

export class WebAuthDeepPage extends Page {
  declare state: WebAuthDeepPageState;
  _abortCtrl!: any;
  _inited!: boolean;
  _mockAssertion!: any;
  _mockCredential!: any;
  ...
  componentDidMount() {
    ...
    const hasSignal = hasPKC && typeof (PublicKeyCredential as any).isExternalSignalCTAPBridgingAvailable === 'function';
  ...
  _caps() {
    ...
    return {
      ...
      signal: hasPKC && typeof (PublicKeyCredential as any).isExternalSignalCTAPBridgingAvailable === 'function',
...
pubKeyCredParams: [
  ...
  _buildCreationOptions() {
    ...
    lines.push(JSON.stringify({
      ...
      excludeCredentials: options.excludeCredentials.map((c: any) => ({ ...c, id: 'Uint8Array(...)' })),
...
// cred.response.attestationObject / clientDataJSON / getPublicKey() … 发服务器存公钥`)),
  ...
  _buildRequestOptions() {
    ...
    lines.push(JSON.stringify({
      ...
      allowCredentials: options.allowCredentials.map((c: any) => ({ ...c, id: 'Uint8Array(...)' })),
      extensions: options.extensions,
    }, null, 2));
    lines.push('');
    lines.push('字段说明：');
...
// assertion.response.signature / authenticatorData / userHandle … 发服务器验签`)),
  ...
  _explainAttachments() {
    ...
    const transportsText = TRANSPORTS.map((t: any) => `  • ${t.padEnd(12)} ${((transportDesc as any)[(t as any)])}`).join('\n');
  ...
  _demoParseFromJSON() {
    ...
    if (caps.parseCreation) {
      try {
        ...
        lines.push(`  opts.challenge = Uint8Array(${(opts.challenge as any).byteLength})，opts.user.id = Uint8Array(${(opts.user.id as any).byteLength})`);
