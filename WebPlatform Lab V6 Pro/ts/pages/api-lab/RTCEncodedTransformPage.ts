...
interface RTCEncodedTransformPageState extends State {
  logs: any;
  capsSummary: any;
  conceptInfo: any;
  transformBindingInfo: any;
  workerEventInfo: any;
  encodedFrameInfo: any;
  sframeInfo: any;
  webcodecsInfo: any;
  scenarioInfo: any;
  detectionInfo: any;
...
export class RTCEncodedTransformPage extends Page {
  declare state: RTCEncodedTransformPageState;
  _dynamicStyles!: any[];
  _inited!: boolean;
  ...
  _runTransformBindingDemo() {
    ...
    if (!f.rtcRtpScriptTransform) {
      ...
      try {
        ...
        const t = new RTCRtpScriptTransform(worker, 'noop', { tag: 'demo' } as any);
    ...
    if (!f.webCrypto) {
      ...
      try {
        crypto.subtle.generateKey(
          ...
          this._addLog('info', `密钥导出成功，长度=${(raw as any).byteLength} 字节（可透传给 RTCRtpScriptTransform）`);
        }).catch((err: any) => {
