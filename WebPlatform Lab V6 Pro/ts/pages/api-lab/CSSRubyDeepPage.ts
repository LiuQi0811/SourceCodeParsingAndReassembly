...
interface CSSRubyDeepPageState extends State {
  logs: LogEntry[];
  capsSummary: string;
  rubySemanticInfo: string;
  rubyPositionInfo: string;
  rubyAlignInfo: string;
  rubyMergeInfo: string;
  doubleRubyInfo: string;
  cjkComboInfo: string;
  dictCardInfo: string;
  a11yFallbackInfo: string;
}
...
export class CSSRubyDeepPage extends Page {
  declare state: CSSRubyDeepPageState;
  ...
  _verticalMode: boolean = false;


  // —— 初始 state ——
  initialState(): CSSRubyDeepPageState {
    return {
      logs: [],
      capsSummary: '',
  ...
  _setRubyPosition(mode: any) {
    ...
    this._injectStyle('css-ruby-position-demo',
      `.ruby-pos-demo { ruby-position: ${mode}; -webkit-ruby-position: ${((webkitMap as any)[(mode as any)]) || 'before'}; }`);
    this.setState({ rubyPositionInfo: this._readRubyPositionInfo() });
    const desc = ({ over: '注音在主文上方', under: '注音在主文下方', 'inter-character': '注音在字符之间（注音符号专用）' } as Record<string, string>)[(mode as any)];
  ...
  _setRubyAlign(mode: any) {
    ...
    const desc = ({ start: '起始对齐（左对齐）', center: '居中对齐（默认）', 'space-between': '两端对齐', 'space-around': '分散对齐' } as Record<string, string>)[(mode as any)];
  ...
  _setRubyMerge(mode: any) {
    ...
    const desc = ({ separate: '每字独立对齐（默认）', collapse: '多字合并为一基底', auto: 'CJK 合并 / 拉丁独立' } as Record<string, string>)[(mode as any)];
    this._addLog('merge', `切换 ruby-merge → ${mode}（${desc}）`);
  }

  _setOverhang(mode: any) {
    this._overhang = mode;
    this._injectStyle('css-ruby-overhang-demo', `.ruby-overhang-demo { ruby-overhang: ${mode}; }`);
    this.setState({ rubyMergeInfo: this._readRubyMergeInfo() });
    const desc = ({ auto: '注音悬挂到相邻字符（默认）', none: '注音不悬挂（限制在基底内）' } as Record<string, string>)[(mode as any)];
