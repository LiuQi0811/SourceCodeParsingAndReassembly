...
interface CSSMediaUserPreferencePageState extends State {
  logs: LogEntry[];
  capsSummary: string;
  colorSchemeInfo: string;
  reducedMotionInfo: string;
  contrastForcedInfo: string;
  transparencyDataInfo: string;
  invertedDynamicInfo: string;
  hoverPointerInfo: string;
}
...
export class CSSMediaUserPreferencePage extends Page {
  declare state: CSSMediaUserPreferencePageState;
  ...
  // —— 初始 state ——
  initialState(): CSSMediaUserPreferencePageState {
    return {
      logs: [],
      capsSummary: '',
  ...
  _injectStyle(): void {
    ...
    try {
      // 若已存在则先移除（防止重复注入）
      const existed = (document.getElementById(STYLE_ID) as any);
...
`;
  ...
  _watchMedia(query: any,label: any) {
    try {
  ...
  _showReducedMotionPattern(): void {
    this.setState({ reducedMotionInfo:
  ...
  _showReducedMotionWithWAAPI(): void {
    const caps = this._caps();
    this.setState({ reducedMotionInfo:
  ...
  _showReducedMotionDecision(): void {
    this.setState({ reducedMotionInfo:
  ...
  _showContrastForcedSyntax(): void {
    this.setState({ contrastForcedInfo:
  ...
  _showContrastForcedPractical(): void {
    this.setState({ contrastForcedInfo:
      '===== 高对比度 / 强制配色实战要点 =====\n\n' +
      '1. 边框与可见性\n' +
  ...
  _showContrastForcedDecision(): void {
    const caps = this._caps();
    this.setState({ contrastForcedInfo:
  ...
  _showTransparencyDataCaps(): void {
    const caps = this._caps();
  ...
  _showTransparencyDataSyntax(): void {
    this.setState({ transparencyDataInfo:
  ...
  _showTransparencyPractical(): void {
    this.setState({ transparencyDataInfo:
  ...
  _showInvertedColorsPractical(): void {
    this.setState({ invertedDynamicInfo:
  ...
  _showDynamicRangePractical(): void {
    this.setState({ invertedDynamicInfo:
  ...
  _showInvertedDynamicDecision(): void {
    const caps = this._caps();
    this.setState({ invertedDynamicInfo:
  ...
  _showHoverPointerPractical(): void {
    this.setState({ hoverPointerInfo:
      '===== hover / pointer 实战要点 =====\n\n' +
      '1. 触屏「粘滞 hover」问题\n' +
  ...
  _showHoverPointerDecision(): void {
    const caps = this._caps();
    this.setState({ hoverPointerInfo:
