...
interface CSSInlineLayoutPageState extends State {
  logs: LogEntry[];
  capsSummary: string;
  overviewInfo: string;
  dominantBaselineInfo: string;
  baselineShiftInfo: string;
  initialLettersInfo: string;
  inlineSizingInfo: string;
  alignmentBaselineInfo: string;
...
export class CSSInlineLayoutPage extends Page {
  declare state: CSSInlineLayoutPageState;
  _inited: boolean = false;
  _dynamicStyles: HTMLStyleElement[] = [];
  _domBaselineMode: string = '';
  _baselineShiftMode: string = '';
  _initialLettersOn: boolean = false;
  _initialLettersSize: number = 0;
  _inlineSizingOn: boolean = false;
  _alignBaselineMode: string = '';
  _textAnchorMode: string = '';


  // —— 初始 state ——
  initialState(): CSSInlineLayoutPageState {
    return {
      logs: [],
      capsSummary: '',
      // Card 1：概述
  ...
  componentDidMount(): void {
    ...
    if (!f.dominantBaseline) this._addLog('warn', 'dominant-baseline 不可用或 jsdom 未识别（SVG 1.1 起支持，CSS Inline Layout L3 引入 CSS 上下文）');
    if (!f.baselineShift) this._addLog('warn', 'baseline-shift 不可用或 jsdom 未识别（实验性，Firefox 仅 SVG 上下文支持）');
    if (!f.initialLetter) this._addLog('warn', 'initial-letter 不可用或 jsdom 未识别（Chrome 110+/Safari 17+，Firefox 仍 behind flag）');
    if (!f.inlineSizing) this._addLog('warn', 'inline-sizing 不可用或 jsdom 未识别（仍实验性，仅 Firefox 部分实现）');
    ...
    const card = new Card({
      ...
      children: [
        ...
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '320px', overflow: 'auto' } },
          h('code', {}, s.overviewInfo || '（点击「读取概述信息」查看 CSS Inline Layout Module 全景）')),
        h(Alert, {
          type: 'info',
  ...
  _readDominantBaselineInfo(): string {
    const f = this._flags();
    try {
  ...
  _renderCard2(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['dominant-baseline', f.dominantBaseline]]),
        h(Tag, { color: 'primary' }, 'SVG + CSS 共用'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'dominant-baseline 指定 inline-level box 的主导基线：auto|text-bottom|alphabetic|ideographic|middle|central|mathematical|hanging|text-top。alphabetic 是西文默认字母基线，central 是行盒中央（em-box 中点，常用于图标与文字垂直居中），middle 是 x-height 中点（与 central 略不同），ideographic 是 CJK 表意文字基线，hanging 是天城文/藏文顶部基线。该属性最初定义于 SVG 1.1（用于 <text>/<tspan>），后被 CSS Inline Layout L3 引入 CSS 上下文，SVG 上下文支持广泛。与 alignment-baseline 区别：dominant-baseline 设置自身主导基线，alignment-baseline 设置对齐到父级哪个基线。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ dominantBaselineInfo: this._readDominantBaselineInfo() }) }),
          this._btn('alphabetic', { size: 'sm', disabled: !f.dominantBaseline, onClick: () => this._setDominantBaseline('alphabetic') }),
          this._btn('central', { size: 'sm', disabled: !f.dominantBaseline, onClick: () => this._setDominantBaseline('central') }),
          this._btn('middle', { size: 'sm', disabled: !f.dominantBaseline, onClick: () => this._setDominantBaseline('middle') }),
          this._btn('text-bottom', { size: 'sm', disabled: !f.dominantBaseline, onClick: () => this._setDominantBaseline('text-bottom') }),
          this._btn('text-top', { size: 'sm', disabled: !f.dominantBaseline, onClick: () => this._setDominantBaseline('text-top') }),
          this._btn('ideographic', { size: 'sm', disabled: !f.dominantBaseline, onClick: () => this._setDominantBaseline('ideographic') }),
        ),
        h('div', { class: 'dom-baseline-stage' },
          h('div', { class: 'row' },
            h('span', { class: 'icon-box' }, '★'),
            h('span', {}, '图标与文字基线对齐（当前 dominant-baseline = ' + this._domBaselineMode + '）'),
          ),
          h('div', { class: 'row' },
            h('span', { class: 'icon-box' }, '◆'),
            h('span', {}, 'central 常用于图标垂直居中，alphabetic 是默认字母基线。'),
          ),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '300px', overflow: 'auto' } },
          h('code', {}, s.dominantBaselineInfo || '（点击按钮切换 dominant-baseline）')),
        h(Alert, {
          type: 'info',
  ...
  _setBaselineShift(mode: any) {
    ...
    this._injectStyle('css-baseline-shift-mode', ((map as any)[(mode as any)]) || map.none);
  ...
  _renderCard3(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['baseline-shift', f.baselineShift]]),
        h(Tag, { color: 'primary' }, 'sub/super/<length>'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'baseline-shift 调整 inline-level box 相对其主导基线的偏移：baseline（默认无偏移）| sub（下标 ≈ -0.33em 向下）| super（上标 ≈ +0.33em 向上）| <length>（指定长度）| <percentage>（相对 line-height）。与 HTML <sup>/<sub> 元素对比：<sup>/<sub> 是语义元素，默认用 vertical-align: super/sub 且缩小字号（smaller ≈ 0.83em）；baseline-shift 是 CSS 属性，可在任意 inline 元素施加，更精确控制偏移。语义上化学式 H₂O 用 <sub>，数学公式 x² 用 <sup>，自定义上下标用 baseline-shift。该属性仍实验性（CSS Inline Layout L3 WD），Firefox 仅 SVG 上下文支持，生产多用 vertical-align 或 <sup>/<sub> 兜底。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ baselineShiftInfo: this._readBaselineShiftInfo() }) }),
          this._btn('无偏移', { size: 'sm', disabled: !f.baselineShift, onClick: () => this._setBaselineShift('none') }),
          this._btn('super/sub', { size: 'sm', disabled: !f.baselineShift, onClick: () => this._setBaselineShift('super') }),
          this._btn('自定义长度', { size: 'sm', disabled: !f.baselineShift, onClick: () => this._setBaselineShift('len') }),
        ),
        h('div', { class: 'baseline-shift-stage' },
          h('div', { class: 'formula' },
            '数学公式：x',
            h('span', { class: 'bs-shift' }, '2'),
            ' + y',
            h('span', { class: 'bs-sub' }, '1'),
            ' = z',
            h('span', { class: 'bs-len' }, 'n'),
          ),
          ...
          h('div', { style: { marginTop: '6px', fontSize: '14px', color: 'var(--color-text-secondary, #888)' } },
            'baseline-shift 在支持的浏览器中显示偏移；不支持时仅展示缩小字号（<sup>/<sub> 始终生效）。',
          ),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '300px', overflow: 'auto' } },
          h('code', {}, s.baselineShiftInfo || '（点击按钮切换 baseline-shift 模式）')),
        h(Alert, {
          type: 'warning',
  ...
  _readInitialLettersInfo(): string {
    const f = this._flags();
    try {
  ...
  _toggleInitialLetters(): void {
    ...
    if (this.el) {
      ...
      if (stage) {
        if (this._initialLettersOn) stage.classList.remove('initial-letters-off');
        else stage.classList.add('initial-letters-off');
      }
    }
  ...
  _renderCard4(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['initial-letter', f.initialLetter]]),
        h(Tag, { color: 'primary' }, 'Chrome 110+ / Safari 17+'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'initial-letter 控制 drop cap（首字下沉）排版：normal | <integer> <integer>? | drop <integer>? | raise <integer>?。initial-letter: 3 让首字占据 3 行高（drop cap），initial-letter: 3 2 第一个是行数、第二个是下沉深度（基线下方行数），raise 1 让首字上升约 super。必须配合 ::first-letter 伪元素使用（针对块级元素首字符），浏览器自动计算字号/行高/对齐，无需手动 float+font-size hack。Chrome 110+/Safari 17+ 支持，Firefox 仍 behind flag（layout.css.initial-letter.enabled）。生产建议渐进增强 + ::first-letter + float 兜底。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ initialLettersInfo: this._readInitialLettersInfo() }) }),
          this._btn(`initial-letter（${this._initialLettersOn ? '开启' : '关闭'}）`, { size: 'sm', disabled: !f.initialLetter, onClick: () => this._toggleInitialLetters() }),
          this._btn('2 行', { size: 'sm', disabled: !f.initialLetter, onClick: () => this._setInitialLettersSize(2) }),
          this._btn('3 行', { size: 'sm', disabled: !f.initialLetter, onClick: () => this._setInitialLettersSize(3) }),
          this._btn('4 行', { size: 'sm', disabled: !f.initialLetter, onClick: () => this._setInitialLettersSize(4) }),
        ),
        h('div', { class: 'initial-letters-stage' },
          h('p', {},
            '层叠样式表（CSS）是一种用来表现 HTML 或 XML 等文件样式的计算机语言。CSS Inline Layout Module Level 3 引入了 initial-letter 属性，用于实现 drop cap（首字下沉）排版——常见于杂志、书籍、新闻报道的首段。开启后首字"层"会占据多行高度，正文环绕其右侧。'),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '300px', overflow: 'auto' } },
          h('code', {}, s.initialLettersInfo || '（点击按钮开启 initial-letter drop cap）')),
        h(Alert, {
          type: 'info',
  ...
  _renderCard5(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['inline-sizing', f.inlineSizing]]),
        h(Tag, { color: 'primary' }, '实验性'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'inline-sizing 控制 inline-level box 是否可设置宽高：normal（默认，inline 元素 width/height 不生效）| stretch（让 inline 元素可设置宽高，自动 stretch 到指定尺寸）。与 display: inline-block 对比：inline-block 让元素既能设宽高又保持行内布局，但会创建 BFC 影响基线对齐（默认对齐到自身底部）；inline-sizing: stretch 保持 inline 显示类型，仅启用宽高，不创建 BFC，基线对齐与普通 inline 一致，更"轻量"。该属性仍实验性（CSS Inline Layout L3 WD），仅 Firefox 部分实现（behind flag），Chrome/Safari 未支持，生产继续用 inline-block 或 inline-flex 兜底。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ inlineSizingInfo: this._readInlineSizingInfo() }) }),
          this._btn(`inline-sizing（${this._inlineSizingOn ? 'stretch' : 'normal'}）`, { size: 'sm', disabled: !f.inlineSizing, onClick: () => this._toggleInlineSizing() }),
        ),
        h('div', { class: 'inline-sizing-stage' },
          h('div', {},
            'inline-sizing: ',
            h('span', { class: 'is-target' }, 'is-target（min-width: 80px）'),
            ' 对比 display: inline-block ',
            h('span', { class: 'is-ib' }, 'is-ib（width: 120px）'),
            ' 末尾文字。',
          ),
          h('div', { style: { marginTop: '6px', fontSize: '13px', color: 'var(--color-text-secondary, #888)' } },
            'inline-sizing: stretch 后 .is-target 可设宽高且不创建 BFC；inline-block 创建 BFC 影响基线对齐。',
          ),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '300px', overflow: 'auto' } },
          h('code', {}, s.inlineSizingInfo || '（点击按钮切换 inline-sizing）')),
        h(Alert, {
          type: 'warning',
  ...
  _readAlignmentBaselineInfo(): string {
    const f = this._flags();
    try {
  ...
  _renderCard6(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['alignment-baseline', f.alignmentBaseline], ['text-anchor', f.textAnchor]]),
        h(Tag, { color: 'primary' }, 'SVG 文本对齐'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'alignment-baseline 指定 inline-level box 对齐到父级的哪个基线：auto|baseline|before-edge|text-before-edge|middle|central|after-edge|text-after-edge|ideographic|alphabetic|hanging|mathematical。与 dominant-baseline 协同：dominant-baseline 设置自身主导基线（"我是哪种基线"），alignment-baseline 设置对齐到父级哪个基线（"我对齐到父的哪个基线"）。常见组合：图标用 dominant-baseline: central + 文字默认 alphabetic 自动垂直居中。text-anchor 控制 SVG <text> 文本水平对齐（相对锚点）：start|middle|end，是 SVG 专属属性但 CSS Inline Layout L3 引入 CSS 上下文支持。SVG <text> 用 text-anchor: middle + dominant-baseline: central 实现完全居中。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ alignmentBaselineInfo: this._readAlignmentBaselineInfo() }) }),
          this._btn('ab: auto', { size: 'sm', disabled: !f.alignmentBaseline, onClick: () => this._setAlignBaseline('auto') }),
          this._btn('ab: central', { size: 'sm', disabled: !f.alignmentBaseline, onClick: () => this._setAlignBaseline('central') }),
          this._btn('ab: middle', { size: 'sm', disabled: !f.alignmentBaseline, onClick: () => this._setAlignBaseline('middle') }),
          this._btn('ab: baseline', { size: 'sm', disabled: !f.alignmentBaseline, onClick: () => this._setAlignBaseline('baseline') }),
          this._btn('ta: start', { size: 'sm', disabled: !f.textAnchor, onClick: () => this._setTextAnchor('start') }),
          this._btn('ta: middle', { size: 'sm', disabled: !f.textAnchor, onClick: () => this._setTextAnchor('middle') }),
          this._btn('ta: end', { size: 'sm', disabled: !f.textAnchor, onClick: () => this._setTextAnchor('end') }),
        ),
        h('div', { class: 'align-baseline-stage' },
          h('div', { class: 'svg-row' },
            h('svg', { width: '120', height: '60', viewBox: '0 0 120 60', xmlns: 'http://www.w3.org/2000/svg' },
              h('line', { x1: '0', y1: '30', x2: '120', y2: '30', stroke: '#ccc', 'stroke-dasharray': '2,2' }),
              h('circle', { cx: '60', cy: '30', r: '3', fill: '#1677ff' }),
              h('text', { x: '60', y: '30', fill: '#333', 'font-size': '14', class: 'ab-text' },
                'SVG 文本（text-anchor=' + this._textAnchorMode + '）'),
            ),
            h('svg', { width: '120', height: '60', viewBox: '0 0 120 60', xmlns: 'http://www.w3.org/2000/svg' },
              h('rect', { x: '10', y: '10', width: '20', height: '20', fill: '#1677ff' }),
              h('text', { x: '40', y: '25', fill: '#333', 'font-size': '14', class: 'ab-text' }, '图标+文字'),
            ),
          ),
          h('div', { style: { marginTop: '6px', fontSize: '13px', color: 'var(--color-text-secondary, #888)' } },
            '上方虚线为 y=30 中线，蓝色圆点为锚点。text-anchor 控制文字相对锚点的水平对齐；alignment-baseline 控制文字相对图标的垂直对齐。',
          ),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '320px', overflow: 'auto' } },
          h('code', {}, s.alignmentBaselineInfo || '（点击按钮切换 alignment-baseline / text-anchor）')),
        h(Alert, {
          type: 'info',
  ...
  _renderCard7(): Node | string {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'CSS Inline Layout 实战场景：图标与文字垂直居中（dominant-baseline: central 比 vertical-align: middle 更精确）、数学公式排版（baseline-shift: super/sub 或 <sup>/<sub>）、多语言混排基线对齐（CJK 用 ideographic 基线、西文用 alphabetic、数字用 central）、drop cap 杂志排版（initial-letter: 3 替代 float+font-size hack）。每个场景展示真实 CSS 代码与渲染效果对比。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('读取实战信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runScenariosDemo() }),
        ),
        h('div', { class: 'scenarios-stage' },
          // 场景 1：图标与文字垂直居中
          h('div', { class: 'scene' },
            h('div', { class: 'fs-sm text-secondary' }, '场景 1：图标与文字垂直居中（dominant-baseline: central）'),
          ...
          // 场景 2：数学公式排版
          h('div', { class: 'scene' },
            h('div', { class: 'fs-sm text-secondary' }, '场景 2：数学公式排版（baseline-shift: super/sub）'),
            h('div', { class: 'math' },
              '爱因斯坦质能方程：E = mc',
              h('span', { class: 'sup' }, '2'),
              '，化学式 H',
          ...
          // 场景 3：多语言混排基线对齐
          h('div', { class: 'scene' },
            h('div', { class: 'fs-sm text-secondary' }, '场景 3：多语言混排基线对齐（数字用 dominant-baseline: central）'),
            h('div', { class: 'mix' },
              h('span', { class: 'cn' }, '中国'),
              h('span', { class: 'num' }, '2024'),
              h('span', { class: 'en' }, 'GDP'),
              h('span', { class: 'num' }, '17.96'),
              h('span', { class: 'cn' }, '万亿美元'),
            ),
          ),
          // 场景 4：drop cap 杂志排版
          h('div', { class: 'scene' },
            h('div', { class: 'fs-sm text-secondary' }, '场景 4：drop cap 杂志排版（initial-letter: 3）'),
            h('div', { class: 'magazine' },
              '在数字时代，CSS Inline Layout Module Level 3 为 Web 排版带来了专业的行内盒对齐能力。从 dominant-baseline 的图标垂直居中，到 initial-letter 的杂志首字下沉，这些特性让 Web 排版逐步逼近印刷品质。',
            ),
          ),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '280px', overflow: 'auto' } },
          h('code', {}, s.scenariosInfo || '（点击「读取实战信息」查看 4 个场景的 CSS 代码）')),
        h(Alert, {
          type: 'success',
  ...
  _renderCard8(): Node | string {
    ...
    const card = new Card({
      ...
      children: [
        ...
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('读取兼容性信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runCompatDemo() }),
        ),
        h('div', { class: 'compat-stage' },
          h('div', { class: 'fs-sm text-secondary' }, 'vertical-align 历史方案对比：'),
          h('div', { class: 'va-demo' },
            h('span', {}, '文字'),
            h('span', { class: 'va-middle', style: { display: 'inline-block', width: '20px', height: '20px', background: '#1677ff', verticalAlign: 'middle' } }),
            'middle ',
            h('span', { class: 'va-top', style: { display: 'inline-block', width: '20px', height: '20px', background: '#10b981', verticalAlign: 'top' } }),
            'top ',
            h('span', { class: 'va-text-bottom', style: { display: 'inline-block', width: '20px', height: '20px', background: '#f59e0b', verticalAlign: 'text-bottom' } }),
            'text-bottom',
          ),
          h('div', { class: 'fs-sm text-secondary', style: { marginTop: '8px' } }, 'flexbox align-items: baseline 替代：'),
          h('div', { class: 'flex-baseline' },
            h('span', { class: 'big' }, '大字'),
            h('span', { class: 'small' }, '小字（基线对齐）'),
            h('span', { class: 'big' }, 'Big'),
          ),
          h('div', { class: 'fs-sm text-secondary', style: { marginTop: '8px' } }, '浏览器支持矩阵：'),
          h('table', { class: 'matrix' },
            ...
            h('tbody', {},
              h('tr', {}, h('td', {}, 'dominant-baseline (SVG)'), h('td', { class: 'yes' }, '✓ 全支持'), h('td', { class: 'yes' }, '✓ 全支持'), h('td', { class: 'yes' }, '✓ 全支持')),
              h('tr', {}, h('td', {}, 'dominant-baseline (CSS)'), h('td', { class: 'partial' }, '部分'), h('td', { class: 'partial' }, '部分'), h('td', { class: 'partial' }, '部分')),
              h('tr', {}, h('td', {}, 'baseline-shift'), h('td', { class: 'partial' }, '部分'), h('td', { class: 'no' }, '仅 SVG'), h('td', { class: 'partial' }, '部分')),
              h('tr', {}, h('td', {}, 'initial-letter'), h('td', { class: 'yes' }, '110+'), h('td', { class: 'no' }, 'flag'), h('td', { class: 'yes' }, '17+')),
              h('tr', {}, h('td', {}, 'inline-sizing'), h('td', { class: 'no' }, '✗'), h('td', { class: 'partial' }, 'flag'), h('td', { class: 'no' }, '✗')),
              h('tr', {}, h('td', {}, 'alignment-baseline (SVG)'), h('td', { class: 'yes' }, '✓'), h('td', { class: 'yes' }, '✓'), h('td', { class: 'yes' }, '✓')),
              h('tr', {}, h('td', {}, 'text-anchor (SVG)'), h('td', { class: 'yes' }, '✓'), h('td', { class: 'yes' }, '✓'), h('td', { class: 'yes' }, '✓')),
              h('tr', {}, h('td', {}, 'vertical-align'), h('td', { class: 'yes' }, '✓ CSS1'), h('td', { class: 'yes' }, '✓ CSS1'), h('td', { class: 'yes' }, '✓ CSS1')),
            ),
          ),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '320px', overflow: 'auto' } },
          h('code', {}, s.compatInfo || '（点击「读取兼容性信息」查看降级方案与浏览器支持矩阵）')),
        h(Alert, {
          type: 'info',
  ...
  render(): Node {
    const s = this.state;
    return h('div', { class: 'api-lab-page css-inline-layout-page' },
      h('h2', { class: 'section-title' }, 'CSS Inline Layout Module Level 3 实验室'),
      h('p', { class: 'fs-sm text-secondary mb-md' },
        '本页演示 CSS Inline Layout Module Level 3 行内盒排版：dominant-baseline 主导基线、baseline-shift 上下标偏移、initial-letter drop cap 首字下沉、inline-sizing 行内盒尺寸控制、alignment-baseline 与 text-anchor SVG 文本对齐、4 个实战场景、兼容性降级方案。所有特性通过 typeof / CSS.supports 能力检测，不可用时仅记日志，绝不抛异常。'),
