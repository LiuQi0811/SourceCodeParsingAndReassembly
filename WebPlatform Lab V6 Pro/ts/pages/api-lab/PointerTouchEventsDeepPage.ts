...
export interface PointerTouchEventsDeepPageProps extends Props {}

export interface PointerTouchEventsDeepPageState extends State {}

export class PointerTouchEventsDeepPage extends Page {
  declare props: PointerTouchEventsDeepPageProps;
  declare state: PointerTouchEventsDeepPageState;
  _dragOriginX: any = null;
  _dragOriginY: any = null;
  _dragStartX: any = null;
  _dragStartY: any = null;
  _dynamicStyles: any[] | null = null;
  _gestureInit: any = null;
  _inited: boolean = false;
  _pointers: any = null;
  _sliderDragging: boolean = false;
  initialState(): PointerTouchEventsDeepPageState {
    return {
      logs: [],
      capsSummary: '',
  ...
  _renderCard3() {
    ...
    const dragBox = h('div', { class: 'pt-drag-box', style: { touchAction: 'none' },
      onPointerDown: (e: any) => {
        try { if ((e.currentTarget as HTMLElement).setPointerCapture) (e.currentTarget as HTMLElement).setPointerCapture(e.pointerId); } catch { /* noop */ }
    ...
    const viz = h('div', { class: 'pt-gesture-viz', style: { touchAction: 'none' },
      onPointerDown: (e: any) => {
        ...
        if (this._pointers.size === 2) {
          const [a, b] = [...(this._pointers as any).values()];
      ...
      onPointerMove: (e: any) => {
        ...
        if (this._pointers.size >= 2 && this._gestureInit && this._gestureInit.dist) {
          ...
          const p = [...(this._pointers as any).values()][0];
  ...
  _updatePenReadout(readoutEl: any, e: any, count: any) {
    ...
    try {
      ...
      cells[1].textContent = 'pressure: ' + (e.pressure == null ? '-' : (e.pressure as any).toFixed(2));
  ...
  _renderCard8() {
    ...
    const fillEl = h('div', { class: 'pt-slider__fill' }) as HTMLElement;
    const thumbEl = h('div', { class: 'pt-slider__thumb', style: { touchAction: 'none' },
      ...
      onPointerMove: (e: any) => {
        ...
        (e.currentTarget as HTMLElement).style.left = 'calc(' + pctStr + '% + ' + (thumbRect.width / 2) + 'px)';
