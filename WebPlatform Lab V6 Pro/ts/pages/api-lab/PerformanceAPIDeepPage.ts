...
export interface PerformanceAPIDeepPageProps extends Props {}

export interface PerformanceAPIDeepPageState extends State {}

export class PerformanceAPIDeepPage extends Page {
  declare props: PerformanceAPIDeepPageProps;
  declare state: PerformanceAPIDeepPageState;
  _cls: any = null;
  _fid: any = null;
  _inited: boolean = false;
  _lcp: any = null;
  _loafObserver: any = null;
  _perfObserver: any = null;
  _rafId: any = null;
  _visHandler: any = null;
  _visUnsub: any = null;
  _vitalsObserver: any = null;
  initialState(): PerformanceAPIDeepPageState {
    return {
      // 共享事件日志（所有卡片写入同一面板）
      logs: [],
      ...
      this._visHandler = () => {
        if (typeof document !== 'undefined' && document.hidden) {
          this._addLog('info',
            ...
            `LCP=${this._lcp != null ? (this._lcp as any).toFixed(2) + 'ms' : '—'}，` +
            `CLS=${(this._cls as any).toFixed(4)}，` +
            `FID=${this._fid != null ? (this._fid as any).toFixed(2) + 'ms' : '—'}`);
        }
      };
  ...
  _compareNow() {
    ...
    try {
      ...
      const text =
        'performance.now() 增量 = ' + (perfDelta as any).toFixed(4) + ' ms（高精度，受 timeOrigin 影响）\n' +
      ...
      this._addLog('now', `performance.now Δ=${(perfDelta as any).toFixed(4)}ms，Date.now Δ=${dateDelta}ms`);
    } catch (err: any) {
  ...
  _renderEntriesList() {
    ...
    return h('div', { class: 'log-panel', style: { maxHeight: '160px' } },
      list.map((e: any, i: any) => h('div', { class: 'log-panel__line' },
      ...
      setTimeout(() => {
        try {
          ...
          const text = 'measure("gap", "mark-A", "mark-B") → duration = ' +
            (dur != null ? (dur as any).toFixed(3) + ' ms' : '（无法读取）');
          this.setState({ measureResult: text });
          this._addLog('measure', `gap duration=${dur != null ? (dur as any).toFixed(3) + 'ms' : '?'}`);
        } catch (err: any) {
    ...
    try {
      ...
      this.setState({
        ...
        measureResult: 'batch-span（batch-0 → batch-99）duration = ' +
          (spanDur != null ? (spanDur as any).toFixed(3) + ' ms' : '（无法读取）'),
      });
      this._addLog('mark', `批量标记 ${N} 个 mark，mark 数=${markCount}`);
      this._addLog('measure', `batch-span duration=${spanDur != null ? (spanDur as any).toFixed(3) + 'ms' : '?'}`);
    } catch (err: any) {
...
obs.disconnect(); // 停止订阅`),
  ...
  _readNavTiming() {
    ...
    const data = {
      ...
      domInteractive: (nav as any).domInteractive,
    ...
    this._addLog('nav',
      `type=${data.type}，TTFB=${(ttfb as any).toFixed(2)}ms，DOM解析=${(domParse as any).toFixed(2)}ms，` +
  ...
  _listResources() {
    ...
    const list = res.slice(0, 10).map((r: any) => ({
      name: r.name,
  ...
  _renderNavTable(nav: any) {
    ...
    const f = (v: any) => (typeof v === 'number' ? (v as any).toFixed(2) + ' ms' : String(v));
    const stages = [
    ...
    return h('div', {},
        ...
        h('tbody', {},
          ...stages.map((row: any) => {
            ...
            return h('tr', {},
              ...
              h('td', { style: TD }, (dur as any).toFixed(2)),
            );
          }),
        ),
      ),
      h('div', { class: 'flex items-center gap-sm flex-wrap mt-sm' },
        ...
        h(Tag, { color: 'success' }, 'TTFB: ' + (nav.ttfb as any).toFixed(2) + 'ms'),
        h(Tag, { color: 'warning' }, 'DOM解析: ' + (nav.domParse as any).toFixed(2) + 'ms'),
      ),
    );
  }
...
res.forEach((r) => {
  ...
  _startVitals() {
    ...
    const subscribe = (type: any, handler: any, label: any) => {
      try {
    ...
    subscribe('largest-contentful-paint', (e: any) => {
      this._lcp = e.startTime || 0;
      const el = e.element ? e.element.tagName : '—';
      this._addLog('lcp', `LCP=${(this._lcp as any).toFixed(2)}ms，element=${el}，size=${e.size || 0}`);
    }, 'LCP');

    // CLS：layout-shift，累加 value（忽略 hadRecentInput）
    subscribe('layout-shift', (e: any) => {
      if (!e.hadRecentInput) {
        this._cls += e.value || 0;
        this._addLog('cls', `CLS 累计=${(this._cls as any).toFixed(4)}（本次=${(e.value || 0).toFixed(4)}）`);
      }
    }, 'CLS');

    // FID：first-input，processingStart - startTime
    subscribe('first-input', (e: any) => {
      this._fid = (e.processingStart || 0) - (e.startTime || 0);
      this._addLog('fid', `FID=${(this._fid as any).toFixed(2)}ms`);
    }, 'FID');

    // Long Task：longtask，duration > 50ms
    subscribe('longtask', (e: any) => {
      if ((e.duration || 0) > 50) {
    ...
    this._vitalsObserver = {
      ...
      disconnect() {
        observers.forEach((o: any) => { try { o.disconnect(); } catch { /* noop */ } });
      },
    };
  ...
  _makeLongTask() {
    ...
    try {
      ...
      this._addLog('longtask', `长任务完成，阻塞约 ${(elapsed as any).toFixed(2)}ms（若 longtask observer 已启动应回调）`);
    } catch (err: any) {
  ...
  async _measureMemory() {
    ...
    try {
      ...
      const parts = breakdown.map((b: any) =>
  ...
  _renderCard5() {
    ...
    return this._card(
      ...
      [
        ...
        h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' },
          h(Tag, { color: 'success' }, 'LCP: ' + (this._lcp != null ? (this._lcp as any).toFixed(2) + 'ms' : '—')),
          h(Tag, { color: 'warning' }, 'CLS: ' + ((this._cls || 0).toFixed(4))),
          h(Tag, { color: 'primary' }, 'FID: ' + (this._fid != null ? (this._fid as any).toFixed(2) + 'ms' : '—')),
        ),
        s.memoryResult
...
performance.measureUserAgentSpecificMemory!(); // 需 crossOriginIsolated`),
      ...
      const observer = new PerformanceObserver((list: any) => {
        try {
          ...
          entries.forEach((entry: any) => {
            try {
              const scripts = (entry.scripts || []).map((sc: any) => ({
                name: sc.name,
  ...
  _renderLoAFInfo() {
    ...
    const TD = { padding: '4px 8px', borderBottom: '1px solid var(--color-border, #f0f0f0)', fontSize: '12px' };
    const TH = { padding: '4px 8px', borderBottom: '1px solid var(--color-border, #eee)', textAlign: 'left', fontSize: '12px' };
    const f = (v: any) => (typeof v === 'number' ? (v as any).toFixed(2) + ' ms' : String(v == null ? '—' : v));
    const trimName = (n: any) => {
      const s = n || '(无名)';
