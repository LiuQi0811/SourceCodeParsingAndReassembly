...
interface AsyncCookbookPageCaps {
  promise: boolean;
  allSettled: boolean;
  any: boolean;
...
export interface AsyncCookbookPageProps extends Props {}

export interface AsyncCookbookPageState extends State {
  logs: LogEntry[];
  capsSummary: string;
  combinatorResult: string;
  taskOrder: string;
  asyncAwaitResult: string;
  asyncGenResult: string;
  syncGenResult: string;
  abortResult: string;
}

export class AsyncCookbookPage extends Page {
  declare props: AsyncCookbookPageProps;
  declare state: AsyncCookbookPageState;
  ...
  // —— 初始 state ——
  initialState(): AsyncCookbookPageState {
    return {
      logs: [],
      capsSummary: '',
  ...
  componentDidMount(): void {
    ...
    if (!hasAllSettled) this._addLog('warn', 'Promise.allSettled 不可用（ES2020）');
    if (!hasAny) this._addLog('warn', 'Promise.any 不可用（ES2021）');
    if (!hasTimeout) this._addLog('warn', 'AbortSignal.timeout 不可用（需 Node 17.3+）');
    if (!hasAny2) this._addLog('warn', 'AbortSignal.any 不可用（较新环境才支持）');
    if (!hasRAF) this._addLog('warn', 'requestAnimationFrame 不可用（Node 通常无此 API）');
  }
  ...
  _delay(ms: any,value: any, label: string): any  {
    return new Promise((resolve) => setTimeout(() => resolve({ value, label, ms }), ms));
  }
  _delayReject(ms: any,reason: any, label: string): any  {
  ...
  async _demoAll() {
    if (!this._caps().promise) { this._addLog('warn', 'Promise 不可用'); return; }
    try {
      const t0 = Date.now();
      ...
      } catch (err: any) { partialErr = err; }
      const tPartial = Date.now() - t1;
      this.setState({
        combinatorResult:
          `Promise.all([p1(80ms), p2(40ms), p3(120ms)]) —— 全成功才成功\n` +
  ...
  async _demoAllSettled() {
    if (!this._caps().allSettled) { this._addLog('warn', 'Promise.allSettled 不可用（ES2020）'); return; }
    try {
      const t0 = Date.now();
      ...
      const t = Date.now() - t0;
      const pretty = results.map((r, i) =>
        r.status === 'fulfilled'
          ? `[${i}] fulfilled, value: ${JSON.stringify(r.value)}`
          : `[${i}] rejected, reason: ${r.reason.message}`,
      ).join('\n');
      this.setState({
        combinatorResult:
          `Promise.allSettled([p1(80ms ok), p2-bad(50ms reject), p3(120ms ok)])\n` +
  ...
  async _demoRace() {
    ...
    try {
      ...
      } catch (err: any) { fastestErr = err; }
      const tErr = Date.now() - t1;
      this.setState({
        combinatorResult:
          `Promise.race([p1, p2, p3]) —— 第一个 settle 的（成功或失败）作为结果\n` +
  ...
  async _demoAny() {
    if (!this._caps().any) { this._addLog('warn', 'Promise.any 不可用（ES2021）'); return; }
    try {
      const t0 = Date.now();
      ...
      } catch (err: any) { aggErr = err; }
      const tAgg = Date.now() - t1;
      this.setState({
        combinatorResult:
          `Promise.any([p1, p2, p3]) —— 第一个成功的；全失败才抛 AggregateError\n` +
  ...
  _renderCard1(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.promise ? 'success' : 'error' }, caps.promise ? 'Promise ✓' : '不可用'),
        h(Tag, { color: caps.allSettled && caps.any ? 'primary' : 'warning' }, 'allSettled / any'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'Promise.all：全成功才成功，任一失败立即失败（短路）。Promise.allSettled：等全部 settle，返回 [{status, value/reason}]，永不短路。Promise.race：第一个 settle 的（成功或失败）作为结果。Promise.any：第一个成功的；全失败才抛 AggregateError（errors 数组聚合所有原因）。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('Promise.all', { type: 'primary', size: 'sm', disabled: !caps.promise, onClick: () => this._demoAll() }),
          this._btn('allSettled', { type: 'primary', size: 'sm', disabled: !caps.allSettled, onClick: () => this._demoAllSettled() }),
          this._btn('race', { type: 'primary', size: 'sm', disabled: !caps.promise, onClick: () => this._demoRace() }),
          this._btn('any', { type: 'primary', size: 'sm', disabled: !caps.any, onClick: () => this._demoAny() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '组合器结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '240px', overflow: 'auto' } },
          h('code', {}, s.combinatorResult || '（点击 all / allSettled / race / any 任一按钮）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '组合器对照速查：'),
        h('pre', { class: 'code-block', style: { maxHeight: '130px', overflow: 'auto' } },
          h('code', {},
`// all       全成功→值数组；任一失败→立即 reject（短路）
...
// any       第一个成功的；全失败→AggregateError(errors)`)),
  ...
  _demoTaskOrder(): void {
    ...
    const seq: any[] = [];
    const record = (tag: any) => seq.push(`${seq.length + 1}. ${tag}`);
  ...
  _finalizeTaskOrder(seq: any,mcLine: any): void  {
    setTimeout(() => {
      this.setState({
        taskOrder:
          `执行顺序（按 seq 编号）：\n${seq.join('\n')}\n\n` +
  ...
  _renderCard2(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.queueMicrotask ? 'success' : 'error' }, caps.queueMicrotask ? 'queueMicrotask ✓' : '不可用'),
        h(Tag, { color: caps.messageChannel ? 'primary' : 'warning' }, caps.messageChannel ? 'MessageChannel ✓' : 'MessageChannel ✗'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '微任务（microtask）：Promise.then / queueMicrotask / MutationObserver，在每个宏任务结束后、下一个宏任务开始前清空。宏任务（macrotask）：setTimeout / setInterval / MessageChannel / I/O / UI 事件，按调度顺序逐个执行。事件循环：执行一个宏任务 → 清空所有微任务 → 渲染（rAF）→ 取下一个宏任务。本卡片安排一组微/宏任务，记录实际落地顺序。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('安排微/宏任务', { type: 'primary', size: 'sm', disabled: !caps.queueMicrotask, onClick: () => this._demoTaskOrder() }),
          this._btn('加测 rAF', { size: 'sm', disabled: !caps.raf, onClick: () => this._demoRAF() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '实际执行顺序：'),
        h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } },
          h('code', {}, s.taskOrder || '（点击「安排微/宏任务」）')),
...
// 宏任务：setTimeout / MessageChannel / I/O / UI 事件`)),
  ...
  async _demoAsyncBasics() {
    ...
    try {
      ...
      let caught = null;
      try { await this._delayReject(20, 'intended', 'p-bad'); } catch (err: any) { caught = err.message; }
      // 4) .catch() 链式捕获（与 try/catch 等价）
      const viaCatch = await this._delayReject(20, 'via-catch', 'p-bad').catch((err: any) => `recovered: ${err.message}`);
      // 5) finally 总会执行
      let finallyRan = false;
      try { await this._delay(10, 1, 'p'); } finally { finallyRan = true; }
      this.setState({
        asyncAwaitResult:
          `async function 基础：\n` +
  ...
  async _demoParallelVsSequential() {
    if (!this._caps().promise) { this._addLog('warn', 'Promise 不可用'); return; }
    try {
      const makeTask = (ms: any,label: any) => this._delay(ms, label, label);
      ...
      this.setState({
        asyncAwaitResult:
          `并行 await（Promise.all）vs 顺序 await 耗时对比（三个任务各 80ms）：\n` +
  ...
  _renderCard3(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.promise ? 'success' : 'error' }, caps.promise ? 'async/await ✓' : '不可用'),
        h(Tag, { color: 'primary' }, 'try/catch · Promise.all'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'async function 隐式返回 Promise；await 暂停函数执行直到 Promise settle；错误处理可用 try/catch 或 .catch()（等价）；finally 不论成败都执行。并行 await：await Promise.all([p1, p2, p3]) 同时发起、等待全部完成（耗时 ≈ 最慢者）；顺序 await：await p1; await p2; await p3; 串行累加（耗时 ≈ 三者之和）。ES2022 top-level await 允许模块顶层直接 await。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('async 基础', { type: 'primary', size: 'sm', disabled: !caps.promise, onClick: () => this._demoAsyncBasics() }),
          this._btn('并行 vs 顺序', { type: 'primary', size: 'sm', disabled: !caps.promise, onClick: () => this._demoParallelVsSequential() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'async/await 结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } },
          h('code', {}, s.asyncAwaitResult || '（点击「async 基础」或「并行 vs 顺序」）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
        h('pre', { class: 'code-block', style: { maxHeight: '140px', overflow: 'auto' } },
          h('code', {},
`// async function 隐式返回 Promise
...
// top-level await（ES2022，仅模块作用域）`)),
  ...
  async _demoAsyncGenerator() {
    ...
    try {
      ...
      async function* counter(start: any, end: any, delayMs: any) {
      ...
      this.setState({
        asyncGenResult:
          `async function* 异步生成器 + for await：\n` +
  ...
  async _demoCustomAsyncIterator() {
    ...
    try {
      // 自定义异步迭代器类
      class AsyncRange {
        _cur: any; _end: any; _stepMs: any;
        constructor(start: any, end: any, stepMs: any) {
          this._cur = start; this._end = end; this._stepMs = stepMs;
        }
        [Symbol.asyncIterator]() { return this; }     // 自身就是迭代器
        next(): any {
          return new Promise((resolve) => {
            setTimeout(() => {
              if (this._cur > this._end) resolve({ value: 'end', done: true });
              else resolve({ value: this._cur++, done: false });
            }, this._stepMs);
          });
        }
      }
      const collected = [];
      const t0 = Date.now();
      for await (const v of new AsyncRange(100, 103, 30)) collected.push(v);
      const t = Date.now() - t0;
  ...
  _renderCard4(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.asyncIterator ? 'success' : 'error' }, caps.asyncIterator ? 'asyncIterator ✓' : '不可用'),
        h(Tag, { color: 'primary' }, 'async function* · for await'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'async function* 声明异步生成器：内部可 await，yield 暂停并把值交给消费者。for await (const v of asyncIterable) 异步消费——每次迭代都 await 上一次 yield 的 Promise。手动调用 gen.next() 返回 Promise<{value, done}>。Symbol.asyncIterator 协议：对象实现 [Symbol.asyncIterator]() 方法（返回 { next(): Promise<{value, done}> }）即可被 for await 消费。这是流式数据（fetch 流、Node Stream）的基础。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('async 生成器 + for await', { type: 'primary', size: 'sm', disabled: !caps.asyncIterator, onClick: () => this._demoAsyncGenerator() }),
          this._btn('自定义异步迭代器', { size: 'sm', disabled: !caps.asyncIterator, onClick: () => this._demoCustomAsyncIterator() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '异步迭代结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } },
          h('code', {}, s.asyncGenResult || '（点击「async 生成器 + for await」）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
        h('pre', { class: 'code-block', style: { maxHeight: '130px', overflow: 'auto' } },
          h('code', {},
`// async function* 异步生成器 + for await 消费
...
for await (const batch of fetchPages('/api/list')) render(batch);`)),
  ...
  _demoSyncGenerator(): any {
    try {
      ...
      this.setState({
        syncGenResult:
          `同步生成器双向通信（next(value) 把 value 传回生成器）：\n` +
  ...
  _demoGeneratorControl(): any {
    try {
      function* counter(): Generator<any, any, any> {
        let i = 0;
        while (true) {
          try {
            const injected = yield i;            // 接收 next(injected) 的值
            if (injected !== undefined) i = injected; else i++;
          } catch (err: any) {
      ...
      const delegated = [];
      for (const v of outer()) delegated.push(v);

      this.setState({
        syncGenResult:
          `生成器三种推进方式：next(value) / return(value) / throw(err)\n\n` +
  ...
  _renderCard5(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: 'success' }, 'function* ✓'),
        h(Tag, { color: 'primary' }, 'yield · yield* · next(value)'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'function* 声明同步生成器；yield 暂停并把值交给调用方；yield* 委托给另一个可迭代对象。三种推进方式：next(value) 把 value 作为上一个 yield 的返回值（双向通信）；return(value) 在当前 yield 处强制终结；throw(err) 在当前 yield 处抛出错误（可被生成器内 try/catch 捕获）。Symbol.iterator 协议：实现 [Symbol.iterator]() 返回 { next(): {value, done} } 即可被 for...of / 展开运算符消费。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('双向通信', { type: 'primary', size: 'sm', onClick: () => this._demoSyncGenerator() }),
          this._btn('next/return/throw', { type: 'primary', size: 'sm', onClick: () => this._demoGeneratorControl() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '生成器结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '280px', overflow: 'auto' } },
          h('code', {}, s.syncGenResult || '（点击「双向通信」或「next/return/throw」）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
        h('pre', { class: 'code-block', style: { maxHeight: '130px', overflow: 'auto' } },
          h('code', {},
`// 双向通信：yield 的返回值 = 下次 next(value) 的 value
function* dialog(): Generator<any, any, any> {
...
// yield* 委托：function* outer() { yield 'a'; yield* ['x','y']; yield 'b'; }`)),
  ...
  _createAbortController(): void {
    if (!this._caps().abortController) { this._addLog('warn', 'AbortController 不可用'); return; }
    try {
      // 清理上一个 controller
      if (this._abortTimeoutId !== null) { clearTimeout(this._abortTimeoutId); this._abortTimeoutId = null; }
  ...
  _manualAbort(): void {
    if (!this._caps().abortController) { this._addLog('warn', 'AbortController 不可用'); return; }
    if (!this._abortController) { this._addLog('warn', '请先点击「创建 controller」'); return; }
    try {
  ...
  _demoAbortSignalTimeout(): void {
    ...
    try {
      ...
      this._abortListener = () => {
        ...
        this.setState({
          abortResult:
            `AbortSignal.timeout(2000) 自动 abort：\n` +
      ...
      this.setState({
        abortResult:
          `AbortSignal.timeout(2000) 已创建，等待 2 秒自动 abort…\n` +
          `const signal = AbortSignal.timeout(2000);\n` +
  ...
  _demoAbortSignalAny(): void {
    ...
    try {
      ...
      this.setState({
        abortResult:
          `AbortSignal.any([s1, s2]) 已创建，300ms 后将 abort c2…\n` +
          `const combined = AbortSignal.any([c1.signal, c2.signal]);\n` +
  ...
  _renderCard6(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.abortController ? 'success' : 'error' }, caps.abortController ? 'AbortController ✓' : '不可用'),
        h(Tag, { color: caps.abortSignalTimeout ? 'primary' : 'warning' }, caps.abortSignalTimeout ? 'timeout ✓' : 'timeout ✗'),
        h(Tag, { color: caps.abortSignalAny ? 'primary' : 'warning' }, caps.abortSignalAny ? 'any ✓' : 'any ✗'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'new AbortController() 创建控制器；controller.signal 获取 AbortSignal；controller.abort() 触发取消（signal.aborted 置 true 并派发 abort 事件）；signal.addEventListener("abort", cb) 监听取消。signal 可传给 fetch(url, { signal })、addEventListener(name, cb, { signal }) 等实现统一取消。AbortSignal.timeout(ms) 静态方法：N 毫秒后自动 abort（无需手动 controller）。AbortSignal.any([s1, s2]) 组合多个信号，任一 abort 即触发（适合「用户取消 OR 超时」合并场景）。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('创建 controller', { type: 'primary', size: 'sm', disabled: !caps.abortController, onClick: () => this._createAbortController() }),
          this._btn('手动 abort', { danger: true, size: 'sm', disabled: !caps.abortController, onClick: () => this._manualAbort() }),
          this._btn('AbortSignal.timeout', { type: 'primary', size: 'sm', disabled: !caps.abortSignalTimeout, onClick: () => this._demoAbortSignalTimeout() }),
          this._btn('AbortSignal.any', { size: 'sm', disabled: !caps.abortSignalAny, onClick: () => this._demoAbortSignalAny() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'AbortController 状态：'),
        h('pre', { class: 'code-block', style: { maxHeight: '240px', overflow: 'auto' } },
          h('code', {}, s.abortResult || '（点击「创建 controller」或「AbortSignal.timeout」）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
        h('pre', { class: 'code-block', style: { maxHeight: '140px', overflow: 'auto' } },
          h('code', {},
`// 手动取消 fetch
...
// 取消事件监听：btn.addEventListener('click', handler, { signal: ctrl.signal })`)),
  ...
  render(): Node {
    ...
    return h('div', { class: 'page api-lab-page' },
      h('h2', { class: 'section-title' }, '异步编程深度实验室'),
      h('p', { class: 'fs-sm text-secondary mb-md' },
        'Promise 组合器（all / allSettled / race / any）、微任务 vs 宏任务、async/await 深度、异步迭代器/生成器、同步生成器双向通信、AbortController 取消异步。'),
