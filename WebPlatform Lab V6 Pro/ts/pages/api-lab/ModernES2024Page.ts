...
interface ModernES2024PageState extends State {
  logs: any;
  capsSummary: any;
  setOpsResult: any;
  iteratorResult: any;
  groupByResult: any;
  stringPromiseResult: any;
  bufferResult: any;
  proxyResult: any;
}

export class ModernES2024Page extends Page {
  declare state: ModernES2024PageState;
  _inited!: boolean;
  _proxy!: any;
  _resizableBuffer!: any;
  ...
  componentDidMount() {
    ...
    if (!caps.setMethods) this._addLog('warn', 'Set 新方法（union 等）不可用，需 Node 22+');
    if (!caps.iterator) this._addLog('warn', 'Iterator helpers 不可用（Iterator 全局对象或 .map 不存在），需 Node 22+');
    if (!caps.fromAsync) this._addLog('warn', 'Array.fromAsync 不可用，需 Node 22+');
    if (!caps.objectGroupBy || !caps.mapGroupBy) this._addLog('warn', 'Object.groupBy / Map.groupBy 不可用，需 Node 22+');
    if (!caps.stringWellFormed) this._addLog('warn', 'String.prototype.isWellFormed / toWellFormed 不可用，需 Node 20+');
    if (!caps.withResolvers) this._addLog('warn', 'Promise.withResolvers 不可用，需 Node 22+');
    if (!caps.resizableBuffer) this._addLog('warn', 'Resizable ArrayBuffer 不可用，需 Node 22+');
  }
  ...
  _caps() {
    ...
    try { resizableBuffer = 'resizable' in ArrayBuffer.prototype; } catch { /* 老引擎无此属性 */ }
  ...
  _setRelations() {
    ...
    try {
      ...
      this.setState({
        setOpsResult:
          `A = {1,2,3,4}，B = {3,4,5,6}，C = {3,4}，D = {10,20}\n\n` +
  ...
  _renderCard1() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.setMethods ? 'success' : 'error' }, caps.setMethods ? 'Set 新方法 ✓' : '不可用'),
        h(Tag, { color: 'primary' }, '7 个方法'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'ES2024 为 Set 新增 7 个方法：union / intersection / difference / symmetricDifference 返回新 Set（并集/交集/差集/对称差集）；isSubsetOf / isSupersetOf / isDisjointFrom 返回 boolean（子集/超集/不相交）。所有方法接受任意可迭代对象作为参数，不修改原 Set。检测方式：typeof Set.prototype.union === "function"。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('并集/交集/差集/对称差', { type: 'primary', size: 'sm', disabled: !caps.setMethods, onClick: () => this._setBasicOps() }),
          this._btn('子集/超集/不相交', { size: 'sm', disabled: !caps.setMethods, onClick: () => this._setRelations() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'Set 运算结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '220px', overflow: 'auto' } },
          h('code', {}, s.setOpsResult || '（点击按钮演示 Set 新方法）')),
...
A.isDisjointFrom(new Set([10,20])); // true（无交集）`)),
  ...
  _iteratorChain() {
    ...
    try {
      ...
      // 链式：×2 → 保留 >4 → 取前 3 → 转数组
      const chain = it.map((x: any) => x * 2).filter((x: any) => x > 4).take(3);
      ...
      const lazy = Iterator.from([1, 2, 3, 4, 5, 6, 7, 8])
        .map((x: any) => { consumed++; return x * 10; })
        .take(2)
        .toArray();
      this.setState({
        iteratorResult:
          `Iterator.from([1,2,3,4,5]) → Iterator 实例（instanceof Iterator = ${isIterator}）\n\n` +
  ...
  _iteratorMore() {
    ...
    try {
      ...
      const flat = Iterator.from([1, 2, 3]).flatMap((x: any) => [x, x * 10]).toArray(); // [1,10,2,20,3,30]
      const sum = Iterator.from([1, 2, 3, 4]).reduce((acc: any, x: any) => acc + x, 0);     // 10
      const collected: any[] = [];
      Iterator.from(['a', 'b', 'c']).forEach((x: any) => collected.push(x.toUpperCase()));
      this.setState({
        iteratorResult:
          `drop(n)：跳过前 n 个\n` +
  ...
  _iteratorFrom() {
    ...
    try {
      ...
      const squared = it.map((x: any) => x * x).toArray();   // [1,4,9,16,25]
      const gen = function* () { yield 'x'; yield 'y'; yield 'z'; };
      const fromGen = Iterator.from(gen()).filter((s: any) => s !== 'y').toArray(); // ['x','z']
      this.setState({
        iteratorResult:
          `自定义可迭代对象 range = { from:1, to:5, [Symbol.iterator]() {...} }\n` +
  ...
  _renderCard2() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.iterator ? 'success' : 'error' }, caps.iterator ? 'Iterator ✓' : '不可用'),
        h(Tag, { color: 'primary' }, '惰性求值'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'Iterator.prototype 新增 map / filter / take / drop / flatMap / reduce / forEach / toArray 等方法，返回 Iterator 实例（非数组），惰性求值——只有调用 toArray() / reduce() / forEach() 等终结操作才真正计算。Iterator.from(iterable) 把任意可迭代对象（含生成器）转为 Iterator。检测方式：typeof Iterator !== "undefined" && typeof Iterator.prototype.map === "function"。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('链式 map/filter/take', { type: 'primary', size: 'sm', disabled: !caps.iterator, onClick: () => this._iteratorChain() }),
          this._btn('drop/flatMap/reduce', { size: 'sm', disabled: !caps.iterator, onClick: () => this._iteratorMore() }),
          this._btn('Iterator.from', { size: 'sm', disabled: !caps.iterator, onClick: () => this._iteratorFrom() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'Iterator 演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '220px', overflow: 'auto' } },
          h('code', {}, s.iteratorResult || '（点击按钮演示 Iterator helpers）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
        h('pre', { class: 'code-block', style: { maxHeight: '140px', overflow: 'auto' } },
          h('code', {},
`// 惰性链式：每步返回 Iterator，toArray() 才求值
...
Iterator.from([1,2,3]).flatMap((x: any) => [x, x*10]).toArray(); // [1,10,2,20,3,30]`)),
  ...
  async _fromAsync() {
    ...
    try {
      ...
      const asyncGen = async function* () {
        for (let i = 1; i <= 5; i++) { await Promise.resolve(); yield i * i; } // 1,4,9,16,25
      };
      const arr = await Array.fromAsync(asyncGen());
      const arr2 = await Array.fromAsync(asyncGen(), (x: any) => `=${x}`);   // mapFn(item, index)
      const fromSync = await Array.fromAsync([10, 20, 30]);             // 普通数组也接受
      this.setState({
        groupByResult:
          `Array.fromAsync(asyncGenerator) → Promise<Array>\n` +
  ...
  _objectGroupBy() {
    ...
    try {
      ...
      const grouped = Object.groupBy(items, (item: any) => item.cat);
      const hasProto = Object.getPrototypeOf(grouped) === Object.prototype; // false（无原型）
      const keys = (Object as any).keys(grouped);                                    // ['水果','蔬菜','饮品']
      this.setState({
        groupByResult:
          `Object.groupBy(items, item => item.cat)\n` +
          `  items 名称 = ${JSON.stringify(items.map((i: any) => i.name))}\n\n` +
          `分组结果（JSON）：${JSON.stringify(grouped)}\n\n` +
          `(Object as any).keys(grouped) = ${JSON.stringify(keys)}\n` +
          `  grouped['水果'] = ${JSON.stringify(grouped['水果']!.map((i: any) => i.name))}\n` +
          `  grouped['蔬菜'] = ${JSON.stringify(grouped['蔬菜']!.map((i: any) => i.name))}\n` +
          `  grouped['饮品'] = ${JSON.stringify(grouped['饮品']!.map((i: any) => i.name))}\n` +
  ...
  _mapGroupBy() {
    ...
    try {
      ...
      for (const [key, group] of grouped) {
        result.push(`键 {id:'${key.id}'} → [${group.map((i: any) => i.name).join(', ')}]`);
      }
      // 演示：对象作为键，引用相等 → 同一组
      const keyObj = { k: 'a' };
      const m = Map.groupBy([{ v: 1 }, { v: 2 }], (_item: any, i: any) => (i === 0 ? keyObj : keyObj));
  ...
  _renderCard3() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.fromAsync ? 'success' : 'error' }, caps.fromAsync ? 'fromAsync ✓' : '不可用'),
        h(Tag, { color: caps.objectGroupBy && caps.mapGroupBy ? 'success' : 'error' },
          caps.objectGroupBy && caps.mapGroupBy ? 'groupBy ✓' : 'groupBy ✗'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'Array.fromAsync(asyncIterable, mapFn?) → Promise<Array>：把异步迭代器（或同步可迭代/ArrayLike）转成数组，mapFn(item, index) 可选。Object.groupBy(items, cb) → 无原型对象（键为 string/Symbol，防原型污染）；Map.groupBy(items, cb) → Map（键可为任意值，含对象）。cb(item, index) 返回分组键。检测：typeof Array.fromAsync === "function" / typeof Object.groupBy === "function"。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('Array.fromAsync', { type: 'primary', size: 'sm', disabled: !caps.fromAsync, onClick: () => this._fromAsync() }),
          this._btn('Object.groupBy', { size: 'sm', disabled: !caps.objectGroupBy, onClick: () => this._objectGroupBy() }),
          this._btn('Map.groupBy', { size: 'sm', disabled: !caps.mapGroupBy, onClick: () => this._mapGroupBy() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'fromAsync / groupBy 结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '240px', overflow: 'auto' } },
          h('code', {}, s.groupByResult || '（点击按钮演示 fromAsync / groupBy）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
        h('pre', { class: 'code-block', style: { maxHeight: '140px', overflow: 'auto' } },
          h('code', {},
`const arr = await Array.fromAsync(asyncGen());          // 异步迭代器 → 数组
...
const byType = Map.groupBy(items, x => x.type);         // Map（键可为对象）`)),
  ...
  _stringWellFormed() {
    ...
    try {
      ...
      this.setState({
        stringPromiseResult:
          `String.prototype.isWellFormed() → boolean（是否为合法 UTF-16，无孤立代理项）\n` +
  ...
  _renderCard4() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.stringWellFormed ? 'success' : 'error' }, caps.stringWellFormed ? 'String ✓' : 'String ✗'),
        h(Tag, { color: caps.withResolvers ? 'success' : 'error' }, caps.withResolvers ? 'withResolvers ✓' : 'withResolvers ✗'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'String.prototype.isWellFormed() → boolean：判断字符串是否为合法 UTF-16（无孤立代理项 lone surrogate）；toWellFormed() → string：把孤立代理项替换为 U+FFFD（不改变长度）。Promise.withResolvers() → { promise, resolve, reject }：等价于 new Promise + 把 resolve/reject 解构到外部，省去手动捕获样板。检测：typeof String.prototype.isWellFormed === "function" / typeof Promise.withResolvers === "function"。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('isWellFormed/toWellFormed', { type: 'primary', size: 'sm', disabled: !caps.stringWellFormed, onClick: () => this._stringWellFormed() }),
          this._btn('Promise.withResolvers', { type: 'primary', size: 'sm', disabled: !caps.withResolvers, onClick: () => this._withResolvers() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'String / Promise 演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '240px', overflow: 'auto' } },
          h('code', {}, s.stringPromiseResult || '（点击按钮演示 String 新方法 / Promise.withResolvers）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
        h('pre', { class: 'code-block', style: { maxHeight: '140px', overflow: 'auto' } },
          h('code', {},
`'abc\\uD800def'.isWellFormed();  // false
...
const v = await promise;            // 42`)),
  ...
  _createResizableBuffer() {
    ...
    try {
      ...
      this.setState({
        bufferResult:
          `new ArrayBuffer(8, { maxByteLength: 32 })\n` +
          `  (ab as any).byteLength = ${(ab as any).byteLength}（初始 8）\n` +
      ...
      this._addLog('buf', `创建可调整 ArrayBuffer：byteLength=${(ab as any).byteLength}，max=${ab.maxByteLength}，resizable=${ab.resizable}`);
    } catch (err: any) {
  ...
  _transferBuffer() {
    ...
    try {
      ...
      let accessOldLine = '';
      try { (ab as any).byteLength; } catch (e: any) { accessOldLine = `\n访问已分离 (ab as any).byteLength → ${e.name}（detached 后无法访问）`; }
      this.setState({
        bufferResult:
          `ab = new ArrayBuffer(8, { maxByteLength: 32 })，写入 [10..80]\n\n` +
  ...
  _renderCard5() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.resizableBuffer ? 'success' : 'error' }, caps.resizableBuffer ? 'resizable ✓' : 'resizable ✗'),
        h(Tag, { color: caps.transferBuffer ? 'success' : 'error' }, caps.transferBuffer ? 'transfer ✓' : 'transfer ✗'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'new ArrayBuffer(length, { maxByteLength }) 创建可调整大小的 ArrayBuffer；ab.resizable / ab.maxByteLength / ab.grow(newLength)（只能扩容到 maxByteLength，grow 后同一引用、数据保留、新区域填 0）。ab.transfer() 转移数据到新 ArrayBuffer 并使原 buffer detached；ab.transferToFixedLength() 转为不可调整的固定长度；ab.detached 判断是否已分离。SharedArrayBuffer 同理用 { maxByteLength } 创建可增长（growable）共享缓冲区。检测："resizable" in ArrayBuffer.prototype / typeof ArrayBuffer.prototype.transfer === "function"。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('创建 Resizable', { type: 'primary', size: 'sm', disabled: !caps.resizableBuffer, onClick: () => this._createResizableBuffer() }),
          this._btn('grow(扩容)', { size: 'sm', disabled: !caps.resizableBuffer, onClick: () => this._growBuffer() }),
          this._btn('transfer/detached', { size: 'sm', disabled: !caps.transferBuffer, onClick: () => this._transferBuffer() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'ArrayBuffer 状态：'),
        h('pre', { class: 'code-block', style: { maxHeight: '240px', overflow: 'auto' } },
          h('code', {}, s.bufferResult || '（点击按钮演示 Resizable ArrayBuffer）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
        h('pre', { class: 'code-block', style: { maxHeight: '140px', overflow: 'auto' } },
          h('code', {},
`const ab = new ArrayBuffer(8, { maxByteLength: 32 });
...
const ab3 = ab2.transferToFixedLength(); // 转为不可调整（resizable=false）`)),
  ...
  _proxyReflect() {
    ...
    try {
      ...
      const handler = {
        get(obj: any, key: any, receiver: any) { accesses.push(`get:${String(key)}`); return Reflect.get(obj, key, receiver); },
        set(obj: any, key: any, value: any, receiver: any) {
          accesses.push(`set:${String(key)}=${value}`);
          if (key === 'count' && typeof value === 'number' && value < 0) throw new RangeError('count 不能为负数');
          return Reflect.set(obj, key, value, receiver);
        },
        has(obj: any,  key: any) { accesses.push(`has:${String(key)}`); return Reflect.has(obj, key); },
      };
      ...
      let rejectLine = '';
      try { proxy.count = -1; } catch (e: any) { rejectLine = `\nset 拦截验证：proxy.count = -1 → ${e.name}（${e.message}）`; }
      this.setState({
        proxyResult:
          `new Proxy(target, handler) + Reflect 转发\n` +
  ...
  _symbols() {
    ...
    try {
      ...
      const IsEven = { [Symbol.hasInstance](val: any) { return typeof val === 'number' && val % 2 === 0; } };
const evenCheck = (4 as any) instanceof IsEven;  // true
const oddCheck = (5 as any) instanceof IsEven;   // false
      // 4) Symbol.isConcatSpreadable：concat 是否展开
      const spreadable = [1, 2]; (spreadable as any)[Symbol.isConcatSpreadable] = true;
      const notSpread = [3, 4]; (notSpread as any)[Symbol.isConcatSpreadable] = false;
      ...
      const ma = new (MyArray as any)(1, 2, 3);
      const mapped = ma.map((x: any) => x * 2);
  ...
  _renderCard6() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.proxy && caps.reflect ? 'success' : 'error' }, caps.proxy && caps.reflect ? 'Proxy/Reflect ✓' : '不可用'),
        h(Tag, { color: caps.symbol ? 'primary' : 'error' }, caps.symbol ? 'Symbol ✓' : 'Symbol ✗'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
        ...
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('Proxy + Reflect', { type: 'primary', size: 'sm', disabled: !caps.proxy || !caps.reflect, onClick: () => this._proxyReflect() }),
          this._btn('Well-known Symbols', { size: 'sm', disabled: !caps.symbol, onClick: () => this._symbols() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'Proxy / Symbol 演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } },
          h('code', {}, s.proxyResult || '（点击按钮演示 Proxy/Reflect/Symbol）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
        h('pre', { class: 'code-block', style: { maxHeight: '150px', overflow: 'auto' } },
          h('code', {},
`// Proxy + Reflect：响应式 / 验证
...
class MyArray extends Array { static get [Symbol.species](){ return Array; } }`)),
  ...
  render() {
    ...
    return h('div', { class: 'page api-lab-page' },
      h('h2', { class: 'section-title' }, 'ES2023 / ES2024+ 新特性实验室'),
      h('p', { class: 'fs-sm text-secondary mb-md' },
        '本页演示 ES2024 新特性：Set 新方法、Iterator Helpers、Array.fromAsync、Object/Map.groupBy、String.isWellFormed/toWellFormed、Promise.withResolvers、Resizable ArrayBuffer，以及 Proxy/Reflect/Symbol 元编程。需 Node 22+ / 较新浏览器。'),
