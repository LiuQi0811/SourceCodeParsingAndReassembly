...
export interface FileAccessPageProps extends Props {}

export interface FileAccessPageState extends State {}

export class FileAccessPage extends Page {
  declare props: FileAccessPageProps;
  declare state: FileAccessPageState;
  _fileReader: any = null;
  _inited: boolean = false;
  initialState(): FileAccessPageState {
  ...
  componentDidMount(): void {
    ...
    const s = this.state;
    this._addLog('fsa',
      `能力检测：FSA(open=${s.fsaOpen}/save=${s.fsaSave}/dir=${s.fsaDir})；FileReader=${s.hasFileReader}；`
    ...
    this.on(document, 'dragover', (e: any) => {
    ...
    this.on(document, 'drop', (e: any) => {
      const zone = e.target && e.target.closest && e.target.closest('.drop-zone');
      if (!zone) return;
      e.preventDefault();
      this._handleDrop(e);
    });
    this.on(document, 'dragleave', (e: any) => {
      // relatedTarget 为空表示离开窗口，复位高亮
      if (this.state.dropActive && !e.relatedTarget) this.setState({ dropActive: false });
    });
  }
  ...
  componentWillUnmount(): void {
    ...
    if (this.state.blobUrl) {
      try { URL.revokeObjectURL(this.state.blobUrl); } catch { /* noop */ }
    }
  }
  ...
  _handlePickerError(err: any, action: any) {
  ...
  async _openFile() {
    if (!this.state.fsaOpen) return this._addLog('err', 'showOpenFilePicker 不可用');
  ...
  async _saveFile() {
    if (!this.state.fsaSave) return this._addLog('err', 'showSaveFilePicker 不可用');
    const content = (this.$('.fsa-save-textarea') as any)?.value ?? this.state.saveContent;
  ...
  async _openDir() {
    if (!this.state.fsaDir) return this._addLog('err', 'showDirectoryPicker 不可用');
    ...
    try {
      ...
      for await (const [name, handle] of (dirHandle as any).entries()) {
        ...
        if (handle.kind === 'directory') {
          try {
            for await (const [subName, subHandle] of (handle as any).entries()) {
      ...
      entries.sort((a: any, b: any) => a.depth - b.depth || a.name.localeCompare(b.name));
  ...
  async _createFileInDir() {
    if (!this.state.dirHandle) return this._addLog('err', '请先打开一个目录');
    try {
      const name = `fsa-${Date.now().toString(36)}.txt`;
      // getFileHandle(name, { create: true }) 在目录中创建新文件句柄
      const fileHandle = await (this.state.dirHandle as any).getFileHandle(name, { create: true });
  ...
  async _removeFirstEntry() {
    if (!this.state.dirHandle) return this._addLog('err', '请先打开一个目录');
    const first = this.state.dirEntries.find((e: any) => e.depth === 0 && e.kind !== 'error');
    if (!first) return this._addLog('err', '目录中没有可删除的条目');
    try {
      await (this.state.dirHandle as any).removeEntry(first.name);
  ...
  async _queryPerm(mode: any) {
    const handle = this.state.fileHandle || this.state.dirHandle;
    if (!handle) return this._addLog('err', '请先打开文件或目录');
    try {
  ...
  _ingestFiles(files: any, source: any) {
    if (!files || files.length === 0) return this._addLog('file', `${source} 未提供文件`);
    // FileList -> 数组化，提取属性；保留原始 File 引用以便后续读取
    const list = Array.from(files).map((f: any) => ({
  ...
  async _readFirstAsText() {
    ...
    if (!f) return this._addLog('err', '没有可读取的文件');
    try {
  ...
  _startFileReader(method: any) {
    if (!this.state.hasFileReader) return this._addLog('err', 'FileReader 不可用');
    const f = this.state.pickedFiles[0];
    if (!f) return this._addLog('err', '请先在上方 Card 2 选择或拖入一个文件');
    // 中止上一个 reader，避免事件串扰
    if (this._fileReader) {
      try { this._fileReader.abort(); } catch { /* noop */ }
    }
    ...
    this.on(reader, 'progress', (ev: any) => {
    ...
    this.on(reader, 'loadend', () => {
      ...
      this.setState({
        ...
        fileReaderResult: method === 'readAsArrayBuffer' ? `(ArrayBuffer, ${(result as any)?.byteLength || 0} 字节)` : String(result).slice(0, 2000),
    ...
    try {
      if (method === 'readAsText') reader.readAsText(f._file, 'utf-8');
      else if (method === 'readAsArrayBuffer') reader.readAsArrayBuffer(f._file);
      else if (method === 'readAsDataURL') reader.readAsDataURL(f._file);
    } catch (err: any) {
  ...
  _abortFileReader() {
    if (!this._fileReader) return this._addLog('reader', '当前没有进行中的 FileReader');
    try {
  ...
  _runEncodeDecode() {
    ...
    try {
      const input = (this.$('.encode-textarea') as any)?.value ?? this.state.encodeInput;
      ...
      this._addLog('encode',
        `编码：${input.length} 字符 => ${bytes.length} 字节；encodeInto => { read: ${into.read}, written: ${into.written} }；解码回环一致=${roundtrip === input}`);
    } catch (err: any) {
    ...
    try {
      ...
      const source = new ReadableStream({
        start(controller: any) {
          chunks.forEach((c: any) => controller.enqueue(c));
          controller.close();
        },
      });
  ...
  _testDecoderLabels() {
    if (!this.state.hasTextDecoder) return this._addLog('err', 'TextDecoder 不可用');
  ...
  _buildBlobUrl() {
    ...
    try {
      const text = (this.$('.blob-textarea') as any)?.value ?? this.state.blobText;
      ...
      this._addLog('file',
        `Blob 已创建：${blob.size} 字节 / ${blob.type}，object URL=${url.slice(0, 50)}...`);
    } catch (err: any) {
  ...
  _sliceBlob() {
    if (!this.state.hasBlob) return this._addLog('err', 'Blob 不可用');
    try {
      const text = (this.$('.blob-textarea') as any)?.value ?? this.state.blobText;
  ...
  _constructFile() {
    if (!this.state.hasFile) return this._addLog('err', 'File 构造不可用');
    try {
  ...
  renderPage(): Node | string | (Node | string)[] {
    ...
    return [
      ...
      h(Card, {
        ...
        h('div', { class: 'flex flex-col gap-sm' },
          h('p', { class: 'fs-sm text-secondary' },
            '(window as any).showOpenFilePicker / showSaveFilePicker / showDirectoryPicker 返回 FileSystemHandle，可跨会话持有。handle.getFile().text() 读取；createWritable().write().close() 写入；for await...of 遍历目录；getFileHandle/getDirectoryHandle({create:true}) 创建；removeEntry 删除；queryPermission/requestPermission 申请持久权限。'),
          h('div', { class: 'flex items-center gap-sm flex-wrap' },
            h(Tag, { color: s.fsaOpen ? 'success' : 'warning' }, s.fsaOpen ? 'showOpenFilePicker ✓' : 'showOpenFilePicker ✗'),
            h(Tag, { color: s.fsaSave ? 'success' : 'warning' }, s.fsaSave ? 'showSaveFilePicker ✓' : 'showSaveFilePicker ✗'),
            h(Tag, { color: s.fsaDir ? 'success' : 'warning' }, s.fsaDir ? 'showDirectoryPicker ✓' : 'showDirectoryPicker ✗'),
          ),
          !hasFsaAny
            ? h(Alert, { type: 'warning', message: '当前环境不支持 File System Access API（需 Chromium 系浏览器 + 安全上下文）' })
            : h('div', { class: 'flex items-center gap-sm flex-wrap' },
                this._btn('打开文件', { type: 'primary', size: 'sm', onClick: () => this._openFile(), disabled: !s.fsaOpen }),
                this._btn('保存文件', { type: 'primary', size: 'sm', onClick: () => this._saveFile(), disabled: !s.fsaSave }),
                this._btn('打开目录', { type: 'primary', size: 'sm', onClick: () => this._openDir(), disabled: !s.fsaDir }),
                this._btn('目录中新建文件', { size: 'sm', onClick: () => this._createFileInDir(), disabled: !s.dirHandle }),
                this._btn('删除首条目', { danger: true, size: 'sm', onClick: () => this._removeFirstEntry(), disabled: !s.dirHandle }),
                this._btn('queryPermission(read)', { size: 'sm', onClick: () => this._queryPerm('read'), disabled: !(s.fileHandle || s.dirHandle) }),
                this._btn('requestPermission(rw)', { size: 'sm', onClick: () => this._requestPerm('readwrite'), disabled: !(s.fileHandle || s.dirHandle) })),
          h('div', { class: 'flex flex-col gap-xs' },
            h('div', { class: 'fs-sm fw-medium' }, '保存内容（编辑后点「保存文件」写入）：'),
            h('textarea', { class: 'fsa-save-textarea', rows: 4, onInput: (e: any) => { this.state.saveContent = e.target.value; } }, s.saveContent),
          ),
          s.fileName && h('div', { class: 'flex flex-col gap-xs' },
            h('div', { class: 'fs-sm fw-medium' }, `已打开文件：${s.fileName}`),
            h('pre', { class: 'code-block', style: { whiteSpace: 'pre-wrap', wordBreak: 'break-all' } }, s.fileContent.slice(0, 1000) + (s.fileContent.length > 1000 ? '\n...(截断)' : '')),
          ),
          s.dirName && h('div', { class: 'flex flex-col gap-xs' },
            h('div', { class: 'fs-sm fw-medium' }, `目录：${s.dirName}（${s.dirEntries.length} 条目）`),
            h('div', { class: 'log-panel' },
              s.dirEntries.length === 0
                ? h('div', { class: 'log-panel__line' }, h('span', {}, '（空目录）'))
                : s.dirEntries.map((e: any) => h('div', { class: 'log-panel__line' },
                    h('span', { class: `log-panel__tag log-panel__tag--${e.kind === 'directory' ? 'dir' : e.kind === 'file' ? 'file' : 'err'}` }, e.kind === 'directory' ? 'DIR' : e.kind === 'file' ? 'FILE' : 'ERR'),
                    h('span', { style: { paddingLeft: `${e.depth * 18}px` } }, `${e.parent ? '└ ' : ''}${e.name}`))),
            ),
          ),
          s.permStatus && h('div', { class: 'fs-sm text-tertiary' }, `权限：${s.permStatus}`),
        ),
      ),
      ...
      h(Card, {
        ...
        h('div', { class: 'flex flex-col gap-sm' },
          h('p', { class: 'fs-sm text-secondary' },
            '<input type="file"> 的 .files 是 FileList；File 继承 Blob，含 name/size/type/lastModified。方法：text() / arrayBuffer() / slice() / stream()。拖放通过 dataTransfer.files 获取，dragover 必须 preventDefault 才能触发 drop。'),
          h('div', { class: 'flex items-center gap-sm flex-wrap' },
            h('input', { type: 'file', accept: 'image/*,text/*,.json,.csv,.txt', multiple: true, onChange: (e: any) => this._onFilePick(e) }),
            this._btn('读取首个文件为文本', { type: 'primary', size: 'sm', onClick: () => this._readFirstAsText(), disabled: s.pickedFiles.length === 0 })),
          h('div', {
            ...
            h('div', { class: 'fs-sm text-secondary' }, `将文件拖放到此处（dragover 已 preventDefault，当前状态：${s.dropActive ? '拖入中' : '空闲'}）`)),
          s.pickedFiles.length === 0
            ? h('div', { class: 'fs-sm text-tertiary' }, '（暂未选择文件）')
            : h('table', { class: 'data-table', style: { width: '100%', borderCollapse: 'collapse', fontSize: 'var(--font-size-sm)' } },
                h('thead', {}, h('tr', {},
                  h('th', { style: thStyle }, '名称'), h('th', { style: thStyle }, '大小(B)'),
                  h('th', { style: thStyle }, '类型'), h('th', { style: thStyle }, 'lastModified'))),
                h('tbody', {},
                  ...s.pickedFiles.map((f: any) => h('tr', {},
                    h('td', { style: tdStyle }, f.name), h('td', { style: tdStyle }, String(f.size)),
                    h('td', { style: tdStyle }, f.type), h('td', { style: tdStyle }, f.lastModifiedDate)))),
              ),
          s.firstFileText && h('div', { class: 'flex flex-col gap-xs' },
            h('div', { class: 'fs-sm fw-medium' }, '首个文件文本预览（前 2000 字符）：'),
            h('pre', { class: 'code-block', style: { whiteSpace: 'pre-wrap', wordBreak: 'break-all' } }, s.firstFileText)),
        ),
      ),
      ...
      h(Card, {
        ...
        h('div', { class: 'flex flex-col gap-sm' },
          h('p', { class: 'fs-sm text-secondary' },
            'new FileReader() 提供回调式读取：readAsText / readAsArrayBuffer / readAsDataURL / readAsBinaryString。事件 onloadstart/onprogress/onload/onloadend/onerror/onabort；readyState 0=EMPTY/1=LOADING/2=DONE；abort() 中止。需先用上方 Card 2 选择文件。'),
          !s.hasFileReader
            ? h(Alert, { type: 'warning', message: '当前环境不支持 FileReader' })
            : h('div', { class: 'flex items-center gap-sm flex-wrap' },
                this._btn('readAsText', { type: 'primary', size: 'sm', onClick: () => this._startFileReader('readAsText'), disabled: s.pickedFiles.length === 0 }),
                this._btn('readAsArrayBuffer', { type: 'primary', size: 'sm', onClick: () => this._startFileReader('readAsArrayBuffer'), disabled: s.pickedFiles.length === 0 }),
                this._btn('readAsDataURL', { type: 'primary', size: 'sm', onClick: () => this._startFileReader('readAsDataURL'), disabled: s.pickedFiles.length === 0 }),
                this._btn('abort()', { danger: true, size: 'sm', onClick: () => this._abortFileReader() })),
          h('div', { class: 'flex items-center gap-sm' },
            ...
            h('div', { style: { flex: '1', height: '8px', background: 'var(--color-fill)', borderRadius: '4px', overflow: 'hidden' } },
              h('div', { style: { height: '100%', width: `${s.fileReaderProgress}%`, background: 'var(--color-primary)', transition: 'width 0.2s' } })),
            h('span', { class: 'fs-sm fw-medium' }, `${s.fileReaderProgress}%`),
            h('span', { class: 'fs-sm text-tertiary' }, `readyState=${s.fileReaderReadyState}`)),
          s.fileReaderIsImage && s.fileReaderDataUrl && h('div', { class: 'flex flex-col gap-xs' },
            h('div', { class: 'fs-sm fw-medium' }, 'data URL 图片预览：'),
            h('img', { src: s.fileReaderDataUrl, alt: 'preview', style: { maxWidth: '320px', maxHeight: '240px' } })),
          s.fileReaderResult && h('div', { class: 'flex flex-col gap-xs' },
            h('div', { class: 'fs-sm fw-medium' }, '读取结果（截断 2000 字符）：'),
            h('pre', { class: 'code-block', style: { whiteSpace: 'pre-wrap', wordBreak: 'break-all' } }, s.fileReaderResult)),
        ),
      ),
      ...
      h(Card, {
        ...
        h('div', { class: 'flex flex-col gap-sm' },
          h('p', { class: 'fs-sm text-secondary' },
            'new TextEncoder().encode(str) => Uint8Array（仅 UTF-8）；encodeInto(str, u8) 返回 {read, written}。new TextDecoder(label, {fatal, ignoreBOM}).decode(u8, {stream}) 支持多种编码标签。TextEncoderStream / TextDecoderStream 是 TransformStream，可管道串联做流式编解码。'),
          h('div', { class: 'flex items-center gap-sm flex-wrap' },
            h(Tag, { color: s.hasTextEncoder ? 'success' : 'warning' }, s.hasTextEncoder ? 'TextEncoder ✓' : 'TextEncoder ✗'),
            h(Tag, { color: s.hasTextDecoder ? 'success' : 'warning' }, s.hasTextDecoder ? 'TextDecoder ✓' : 'TextDecoder ✗'),
            h(Tag, { color: s.hasTextEncoderStream ? 'success' : 'warning' }, s.hasTextEncoderStream ? 'TextEncoderStream ✓' : '✗'),
            h(Tag, { color: s.hasTextDecoderStream ? 'success' : 'warning' }, s.hasTextDecoderStream ? 'TextDecoderStream ✓' : '✗')),
          h('div', { class: 'flex flex-col gap-xs' },
            h('div', { class: 'fs-sm fw-medium' }, '输入文本（CJK + Emoji 演示字节 vs 字符差异）：'),
            h('textarea', { class: 'encode-textarea', rows: 3, onInput: (e: any) => { this.state.encodeInput = e.target.value; } }, s.encodeInput),
          ),
          h('div', { class: 'flex items-center gap-sm flex-wrap' },
            this._btn('编码 → 十六进制 → 解码回环', { type: 'primary', size: 'sm', onClick: () => this._runEncodeDecode(), disabled: !(s.hasTextEncoder && s.hasTextDecoder) }),
            this._btn('流式回环', { type: 'primary', size: 'sm', onClick: () => this._runStreamRoundtrip(), disabled: !(s.hasTextEncoderStream && s.hasTextDecoderStream) }),
            this._btn('TextDecoder 编码标签测试', { size: 'sm', onClick: () => this._testDecoderLabels(), disabled: !s.hasTextDecoder })),
          s.encodeBytes && h('div', { class: 'flex flex-col gap-xs' },
            h('div', { class: 'fs-sm fw-medium' }, `UTF-8 字节（${s.encodeCharLen} 字符 => ${s.encodeByteLen} 字节）：`),
            h('pre', { class: 'code-block', style: { whiteSpace: 'pre-wrap', wordBreak: 'break-all' } }, s.encodeBytes),
            h('div', { class: 'fs-sm text-secondary' }, `解码回环：${s.encodeRoundtrip}`)),
          s.streamRoundtrip && h('div', { class: 'flex flex-col gap-xs' },
            h('div', { class: 'fs-sm fw-medium' }, '流式回环拼接结果：'),
            h('pre', { class: 'code-block', style: { whiteSpace: 'pre-wrap', wordBreak: 'break-all' } }, s.streamRoundtrip)),
        ),
      ),
      ...
      h(Card, {
        ...
        h('div', { class: 'flex flex-col gap-sm' },
          h('p', { class: 'fs-sm text-secondary' },
            'new Blob([parts], {type}) 可由字符串/ArrayBuffer/TypedArray/Blob 组合；.size/.type/.slice()/.text()/.arrayBuffer()/.stream()。URL.createObjectURL(blob) 生成 blob: URL，revokeObjectURL 释放。new File([parts], name, {type, lastModified}) 继承 Blob，可结合 a[download] 实现无服务端下载。'),
          !(s.hasBlob && s.hasUrl)
            ? h(Alert, { type: 'warning', message: 'Blob 或 URL.createObjectURL 不可用' })
            : h('div', { class: 'flex flex-col gap-xs' },
                h('div', { class: 'fs-sm fw-medium' }, 'Blob 内容：'),
                h('textarea', { class: 'blob-textarea', rows: 4, onInput: (e: any) => { this.state.blobText = e.target.value; } }, s.blobText),
                h('div', { class: 'flex items-center gap-sm flex-wrap' },
                  this._btn('生成 object URL', { type: 'primary', size: 'sm', onClick: () => this._buildBlobUrl() }),
                  this._btn('blob.slice(0,10)', { size: 'sm', onClick: () => this._sliceBlob() }),
                  this._btn('下载 JSON Blob', { type: 'primary', size: 'sm', onClick: () => this._downloadJsonBlob() }),
                  this._btn('new File() 构造', { size: 'sm', onClick: () => this._constructFile(), disabled: !s.hasFile })),
                s.blobUrl && h('div', { class: 'flex flex-col gap-xs' },
                  h('div', { class: 'fs-sm text-secondary' }, `Blob：${s.blobSize} 字节 / ${s.blobType}`),
                  h('a', { href: s.blobUrl, download: 'blob-demo.txt', class: 'btn btn--primary btn--sm' }, '下载 Blob（a[download]）')),
              ),
        ),
      ),
...
const thStyle = { borderBottom: '1px solid var(--color-border)', textAlign: 'left', padding: '4px 8px' },
  tdStyle = { borderBottom: '1px solid var(--color-border)', padding: '4px 8px' };
