...
interface ScrollEventsPageCaps {
  scrollend: boolean;
  scroll: boolean;
  wheel: boolean;
...
export interface ScrollEventsPageProps extends Props {}

export interface ScrollEventsPageState extends State {
  logs: LogEntry[];
  capsSummary: string;
  scrollendInfo: string;
  compareInfo: string;
  gutterInfo: string;
  matrixInfo: string;
  programInfo: string;
...
export class ScrollEventsPage extends Page {
  declare props: ScrollEventsPageProps;
  declare state: ScrollEventsPageState;

  _inited: boolean = false;
  declare _destroyed: boolean;
  _injectedStyles!: any[];
  _scrollCount!: number;
  _scrollendCount!: number;
  _compareScrollCount!: number;
  _compareScrollendCount!: number;
  _gutterValue!: string;


  initialState(): ScrollEventsPageState {
    return {
      logs: [],
      capsSummary: '',
      scrollendInfo: '',
  ...
  componentDidMount(): void {
    ...
    if (!caps.scrollend) this._addLog('warn', 'scrollend 事件不支持（Chrome 114+，需真实浏览器），可用模拟派发演示回调');
    if (!caps.scrollbarGutter) this._addLog('warn', 'scrollbar-gutter: stable 不支持（Chrome 94+），仅说明用法');
    if (!caps.smoothScroll) this._addLog('warn', 'scrollTo/scrollIntoView smooth 不支持，仅说明用法');
    if (caps.passive) this._addLog('info', 'addEventListener 第三参 passive:true 是选项（非特性），可手动指定提升滚动性能');
  ...
  _caps(): ScrollEventsPageCaps {
    let scrollend = false;
    try {
      scrollend = typeof window !== 'undefined'
        && ('onscrollend' in window
          || (typeof Element !== 'undefined' && 'onscrollend' in Element.prototype));
    } catch { scrollend = false; }
    let scroll = false;
    try { scroll = typeof window !== 'undefined' && 'onscroll' in window; } catch { scroll = false; }
    let wheel = false;
    try { wheel = typeof window !== 'undefined' && 'onwheel' in window; } catch { wheel = false; }
    let scrollbarGutter = false;
    try {
      scrollbarGutter = typeof CSS !== 'undefined'
        && typeof CSS.supports === 'function'
        && CSS.supports('scrollbar-gutter', 'stable');
    } catch { scrollbarGutter = false; }
    let smoothScroll = false;
    try {
      smoothScroll = typeof Element !== 'undefined'
        && typeof Element.prototype.scrollIntoView === 'function'
        && typeof Element.prototype.scrollTo === 'function';
    } catch { smoothScroll = false; }
    let passive = false;
    try { passive = typeof window !== 'undefined' && typeof window.addEventListener === 'function'; } catch { passive = false; }
  ...
  _dispatchScrollend(): void {
    const box = (this.$('#se-scrollend-box') as any);
    if (!box) { this._addLog('warn', '未找到滚动容器'); return; }
    try {
  ...
  _renderCard1(): Node | string {
    ...
    const onScroll = () => {
      this._scrollCount++;
      const el = (this.$('#se-scroll-count') as any);
      if (el) el.textContent = String(this._scrollCount);
    };
    const onScrollend = () => {
      this._scrollendCount++;
      const el = (this.$('#se-scrollend-count') as any);
      if (el) el.textContent = String(this._scrollendCount);
    ...
    const paras = Array.from({ length: 20 }, (_, i) =>
      h('p', {}, `段落 ${i + 1}：滚动事件实验室。scrollend 仅在滚动完全停止后触发一次，避免 scroll 高频回调。`));
    return h(Card, {
      title: 'Card 1 · scrollend 事件',
      extra: h(Tag, { color: caps.scrollend ? 'success' : 'error' }, caps.scrollend ? '已支持' : '未支持'),
    },
      h('p', { class: 'fs-sm text-secondary' },
        'Element.scrollend / Document.scrollend 事件：仅在滚动完全停止（惯性/动画结束）触发一次，' +
        '解决 scroll 事件抖动与高频回调问题。vs scroll 事件（滚动过程持续触发，每帧多次）。' +
        '能力检测：\'onscrollend\' in window 或 Element.prototype。'),
      h('div', { class: 'se-demo-box' },
        h('div', {
          id: 'se-scrollend-box', class: 'se-scroll-box',
          onscroll: onScroll,
          onscrollend: onScrollend,
        }, paras),
        h('div', { class: 'se-counter' },
          h('div', { class: 'se-counter-item' }, 'scroll 触发次数：', h('b', { id: 'se-scroll-count' }, '0')),
          h('div', { class: 'se-counter-item se-counter-item--end' }, 'scrollend 触发次数：', h('b', { id: 'se-scrollend-count' }, '0')),
        ),
      ),
      h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' },
        this._btn('模拟派发 scrollend', { type: 'primary', size: 'sm', onClick: () => this._dispatchScrollend() }),
        h(Tag, { color: 'default' }, 'jsdom 需手动 dispatch'),
      ),
      s.scrollendInfo ? h('p', { class: 'fs-sm mt-sm text-secondary' }, s.scrollendInfo) : null as any,
      h('pre', { class: 'code-block mt-md' },
`el.addEventListener('scrollend', e => { /* 滚动真正停止 */ });
...
// Chrome 114+ / Edge 114+；Safari/Firefox 部分`),
  ...
  _dispatchCompareScrollend(): void {
    const box = (this.$('#se-compare-box') as any);
    if (!box) { this._addLog('warn', '未找到对比容器'); return; }
    try {
  ...
  _simulateCompareScroll(): void {
    ...
    try {
      ...
      const tick = () => {
        ...
        const el = (this.$('#se-compare-scroll-count') as any);
        if (el) el.textContent = String(this._compareScrollCount);
        if (n < 5) {
          setTimeout(tick, 30);
        } else {
  ...
  _renderCard2(): Node | string {
    ...
    const onScrollend = () => {
      ...
      const el = (this.$('#se-compare-scrollend-count') as any);
      if (el) el.textContent = String(this._compareScrollendCount);
      this.setState({ compareInfo: `scroll 高频累计=${this._compareScrollCount}，scrollend 累计=${this._compareScrollendCount}（一次滚动周期仅触发 1 次）` });
    };
    const paras = Array.from({ length: 20 }, (_, i) =>
      h('p', {}, `对比段 ${i + 1}：scroll 在滚动过程持续触发，scrollend 在滚动停止后触发一次。`));
    return h(Card, {
      title: 'Card 2 · scroll 事件 vs scrollend 对比',
      extra: h(Tag, { color: this._caps().scroll ? 'success' : 'error' }, this._caps().scroll ? '已支持' : '未支持'),
    },
      h('p', { class: 'fs-sm text-secondary' },
        'scroll 事件在滚动过程持续触发（每帧多次），需节流/rAF/防抖避免卡顿；' +
        'scrollend 仅在滚动结束触发一次，天然低频。对比同一容器的两个事件触发次数。'),
      h('div', { class: 'se-demo-box' },
        h('div', {
        ...
        h('div', { class: 'se-counter' },
          h('div', { class: 'se-counter-item' }, 'scroll 触发：', h('b', { id: 'se-compare-scroll-count' }, '0')),
          h('div', { class: 'se-counter-item se-counter-item--end' }, 'scrollend 触发：', h('b', { id: 'se-compare-scrollend-count' }, '0')),
        ),
      ),
      h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' },
        this._btn('模拟连续 scroll', { type: 'default', size: 'sm', onClick: () => this._simulateCompareScroll() }),
        this._btn('模拟派发 scrollend', { type: 'primary', size: 'sm', onClick: () => this._dispatchCompareScrollend() }),
      ),
      s.compareInfo ? h('p', { class: 'fs-sm mt-sm text-secondary' }, s.compareInfo) : null as any,
      h('pre', { class: 'code-block mt-md' },
`// 方案一：rAF 节流
let ticking = false;
...
el.addEventListener('scrollend', handle);`),
  ...
  _toggleGutter(): void {
    ...
    const panes = [(this.$('#se-gutter-a') as any), (this.$('#se-gutter-b') as any)];
    panes.forEach((p) => {
      if (!p) return;
      try {
  ...
  _renderCard3(): Node | string {
    const s = this.state;
    const caps = this._caps();
    const mkParas = (label: any) => Array.from({ length: 12 }, (_, i) =>
      h('p', {}, `${label} 段 ${i + 1}：内容溢出产生滚动条。`));
    return h(Card, {
      title: 'Card 3 · scrollbar-gutter',
      extra: h(Tag, { color: caps.scrollbarGutter ? 'success' : 'error' }, caps.scrollbarGutter ? '已支持' : '未支持'),
    },
      h('p', { class: 'fs-sm text-secondary' },
        'scrollbar-gutter: auto | stable | both-edges；强制保留滚动条槽位，消除内容宽度跳变与布局抖动（CLF 问题）。' +
        'stable 始终预留空间，both-edges 两侧对称。能力检测：CSS.supports(\'scrollbar-gutter\', \'stable\')。'),
      h('div', { class: 'se-gutter-compare' },
        h('div', { class: 'se-gutter-pane' },
          h('h4', {}, 'gutter: auto（按需）'),
          h('div', { id: 'se-gutter-a', class: 'se-scroll-box' }, mkParas('A')),
        ),
        h('div', { class: 'se-gutter-pane' },
          h('h4', {}, 'gutter: stable（预留）'),
          h('div', { id: 'se-gutter-b', class: 'se-scroll-box se-gutter-stable' }, mkParas('B')),
        ),
      ),
      h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' },
        this._btn('切换 gutter 值', { type: 'primary', size: 'sm', onClick: () => this._toggleGutter() }),
        h(Tag, { color: 'default' }, 'auto → stable → both-edges'),
      ),
      s.gutterInfo ? h('p', { class: 'fs-sm mt-sm text-secondary' }, s.gutterInfo) : null as any,
      h('pre', { class: 'code-block mt-md' },
`.scroll-box { scrollbar-gutter: stable; }
...
if (CSS.supports('scrollbar-gutter', 'stable')) { /* 安全使用 */ }`),
  ...
  _logMatrixEvent(name: string): void {
  ...
  _renderCard4(): Node | string {
    ...
    return h(Card, {
      ...
      h('p', { class: 'fs-sm text-secondary' },
        '对比 scroll / scrollend / wheel / gesture*（已废弃）/ pointermove 滚动；' +
        'scroll 是布局滚动，wheel 是输入设备事件；passive: true 提升滚动性能；' +
        'addEventListener(\'scroll\', cb, { passive: true }) vs onscroll 属性。'),
      h('div', { class: 'se-demo-box' },
        h('div', {
          id: 'se-matrix-box', class: 'se-scroll-box',
          onscroll: (_: Event) => this._logMatrixEvent('scroll'),
          onscrollend: (_: Event) => this._logMatrixEvent('scrollend'),
          onwheel: mkWheel,
          onpointermove: (e: PointerEvent) => { if (e && e.pointerType === 'touch') this._logMatrixEvent('pointermove'); },
        }, Array.from({ length: 12 }, (_, i) => h('p', {}, `事件对比段 ${i + 1}：滚动/滚轮/触摸都会触发对应事件。`))),
      ),
      h('table', { class: 'se-matrix' },
        h('thead', {}, h('tr', {},
          h('th', {}, '事件'),
          h('th', {}, '对象'),
          h('th', {}, '触发时机'),
      ...
      h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' },
        this._btn('模拟派发 scroll', { type: 'default', size: 'sm', onClick: () => { try { (this.$('#se-matrix-box') as any)?.dispatchEvent(new Event('scroll', { bubbles: true })); } catch { /* noop */ } } }),
        this._btn('模拟派发 scrollend', { type: 'default', size: 'sm', onClick: () => { try { (this.$('#se-matrix-box') as any)?.dispatchEvent(new Event('scrollend', { bubbles: true })); } catch { /* noop */ } } }),
        h(Tag, { color: caps.passive ? 'success' : 'error' }, caps.passive ? 'passive ✓' : 'passive ✗'),
      ),
      s.matrixInfo ? h('p', { class: 'fs-sm mt-sm text-secondary' }, s.matrixInfo) : null as any,
      h('pre', { class: 'code-block mt-md' },
`el.addEventListener('scroll', cb, { passive: true });  // 不阻塞滚动，提升性能
...
// pointermove（pointerType==='touch'）可识别触摸滚动`),
  ...
  _smoothScrollTo(pos: string): void {
    const box = (this.$('#se-program-box') as any);
    if (!box) { this._addLog('warn', '未找到演示容器'); return; }
  ...
  _scrollIntoViewDemo(): void {
    const target = (this.$('#se-program-target') as any);
    if (!target) { this._addLog('warn', '未找到目标元素'); return; }
    if (!this._caps().smoothScroll) {
  ...
  _renderCard5(): Node | string {
    const s = this.state;
    const caps = this._caps();
    const paras = Array.from({ length: 18 }, (_, i) =>
      h('p', {}, `编程式段 ${i + 1}：scrollTo/scrollBy/scrollIntoView 配合 behavior:"smooth" 平滑滚动。`));
    ...
    return h(Card, {
      ...
      h('p', { class: 'fs-sm text-secondary' },
        'scrollTo/scrollBy/scrollIntoView options({ behavior: "smooth"|"instant", block, inline })；' +
        'scrollIntoViewIfNeeded；overscroll-behavior: contain 阻止滚动链；' +
        'element.scrollTop/scrollLeft/scrollHeight/scrollWidth。'),
      h('div', { class: 'se-demo-box' },
        h('div', { id: 'se-program-box', class: 'se-scroll-box', style: 'scrollbar-gutter: stable;' }, paras),
      ),
      h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' },
        this._btn('平滑到顶部', { type: 'default', size: 'sm', onClick: () => this._smoothScrollTo('top') }),
        this._btn('平滑到底部', { type: 'default', size: 'sm', onClick: () => this._smoothScrollTo('bottom') }),
        this._btn('scrollBy 80px', { type: 'default', size: 'sm', onClick: () => this._scrollByStep() }),
        this._btn('scrollIntoView 目标', { type: 'primary', size: 'sm', onClick: () => this._scrollIntoViewDemo() }),
      ),
      s.programInfo ? h('p', { class: 'fs-sm mt-sm text-secondary' }, s.programInfo) : null as any,
      h('pre', { class: 'code-block mt-md' },
`el.scrollTo({ top: 0, behavior: 'smooth' });          // 绝对位置
