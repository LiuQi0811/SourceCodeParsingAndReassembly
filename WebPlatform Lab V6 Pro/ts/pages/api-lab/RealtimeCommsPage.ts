...
export interface RealtimeCommsPageProps extends Props {}

export interface RealtimeCommsLog {
  type: string;
  content: string;
  time: string;
}

export interface RealtimeCommsPageState extends State {
  logs: RealtimeCommsLog[];
  rtcState: string;
  dcState: string;
  iceState: string;
  ...
  chatInput: string;
  wtSupported: boolean;
  wtSupportDetail: Record<string, boolean>;
  wtState: string;
  statsInfo: string;
...
export class RealtimeCommsPage extends Page {
  declare props: RealtimeCommsPageProps;
  declare state: RealtimeCommsPageState;
  _wtChecked: boolean = false;
  _pc1: RTCPeerConnection | null = null;
  _pc2: RTCPeerConnection | null = null;
  _dc: RTCDataChannel | null = null;
  _dc2: RTCDataChannel | null = null;
  _transport: any = null;

  // —— 初始 state ——
  initialState(): RealtimeCommsPageState {
    return {
      logs: [],
      rtcState: 'idle',
      dcState: 'idle',
  ...
  componentDidMount(): void {
    ...
    if (this._wtChecked) return; // 防止 _rerender 后重复进入导致 rAF 链爆炸
  ...
  async _startWebRTC(): Promise<void> {
    ...
    try {
      ...
      const pc1 = new RTCPeerConnection({ iceServers: RTC_ICE_SERVERS } as any);
      const pc2 = new RTCPeerConnection({ iceServers: RTC_ICE_SERVERS } as any);
      this._pc1 = pc1;
      this._pc2 = pc2;
      ...
      pc1.onicecandidate = (e: RTCPeerConnectionIceEvent) => {
        if (e.candidate) {
          ...
          pc2.addIceCandidate(e.candidate).catch(
            (err: unknown) => this._addLog('err', `pc2.addIceCandidate 失败：${errInfo(err).message}`),
          );
        }
      };
      pc2.onicecandidate = (e: RTCPeerConnectionIceEvent) => {
        if (e.candidate) {
          ...
          pc1.addIceCandidate(e.candidate).catch(
            (err: unknown) => this._addLog('err', `pc1.addIceCandidate 失败：${errInfo(err).message}`),
          );
        }
      };
      ...
      // —— pc2 监听数据通道到达 ——
      pc2.ondatachannel = (e: RTCDataChannelEvent) => {
  ...
  _setupChannel(dc: RTCDataChannel, isReceiver: boolean): void {
    ...
    dc.onclose = () => {
      ...
      if (!isReceiver) this.setState({ dcState: 'closed' });
    };
    dc.onerror = (e: RTCErrorEvent) => {
      this._addLog('err', `数据通道错误：${(e as any)?.error?.message || 'unknown'}`);
    };
    dc.onmessage = (e: MessageEvent) => {
      const isBinary = typeof e.data !== 'string';
      const size = isBinary ? ((e.data as ArrayBuffer).byteLength ?? 0) : 0;
      const text = isBinary ? `[binary ${size} 字节]` : (e.data as string);
      if (isReceiver) {
        // pc2 收到 pc1 发来的消息
  ...
  _closeDc(): void {
    if (this._dc) { try { this._dc.close(); } catch { /* noop */ } this._dc = null; }
  ...
  _checkWtSupport(): void {
    const detail: Record<string, boolean> = {};
    for (const name of WT_FEATURES) {
      try {
        detail[name] = typeof (globalThis as any)[name] !== 'undefined';
      } catch {
        detail[name] = false;
      }
    }
    const supportedCount = (Object as any).values(detail).filter(Boolean).length;
  ...
  async _tryWtConnect(): Promise<void> {
    ...
    try {
      ...
      const transport = new (globalThis as any).WebTransport('https://example.com');
  ...
  async _demoWtStreams(): Promise<void> {
    ...
    // 单向流：写入一条消息后关闭
    try {
      const writable: WritableStream<Uint8Array> = await this._transport.createUnidirectionalStream();
      const writer = writable.getWriter();
      const data = new TextEncoder().encode('hello webtransport');
      await writer.write(data);
      await writer.close();
      this._addLog('wt', `createUnidirectionalStream：已写入 ${(data as any).byteLength} 字节并关闭`);
    } catch (err: unknown) {
  ...
  _closeWt(): void {
    if (this._transport) {
      try { this._transport.close(); } catch { /* noop */ }
  ...
  _sendBinary(): void {
    ...
    try {
      ...
      this._dc.send(bytes);
      this._addLog('dc', `pc1 已发送二进制：${(bytes as any).byteLength} 字节`);
      this.setState({
        messages: [...this.state.messages, { dir: 'send', text: `[binary ${(bytes as any).byteLength} 字节]`, time: formatTime() }].slice(-30),
      });
    } catch (err: unknown) {
  ...
  async _refreshStats(): Promise<void> {
    const pc = this._pc1;
    if (!pc) {
    ...
    try {
      ...
      for (const [id, stat] of report as unknown as Map<string, any>) {
  ...
  // —— 状态小芯片（label + value）——
  _stateChip(label: string, value: string): Node {
    return h('div', { class: 'flex items-center gap-xs' },
      h('span', { class: 'text-tertiary' }, `${label}:`),
      h('span', { class: 'fw-medium' }, String(value)),
    );
  }
  ...
  renderPage(): Node | string | (Node | string)[] {
    ...
    const chatInput = new Input({
      ...
      onChange: (v: string) => { this.state.chatInput = v; },
    });
    ...
    return [
      ...
      h(Card, {
        ...
        extra: h('div', { class: 'flex items-center gap-xs' },
          h(Tag, { color: 'primary' }, 'RTCPeerConnection'),
          h(Tag, { color: 'success' }, 'RTCDataChannel'),
        ),
      },
        h('div', { class: 'flex flex-col gap-sm' },
          h('p', { class: 'fs-sm text-secondary' },
            '同页面内创建 pc1 / pc2 两个 RTCPeerConnection，通过 onicecandidate 互投 ICE 候选，并以 createOffer / createAnswer 完成 SDP 握手。pc1 主动 createDataChannel，pc2 经 ondatachannel 接收，构成无信令服务器的本地回环。'),
          h('div', { class: 'flex items-center gap-sm flex-wrap' },
            this._btn('建立连接', { type: 'primary', size: 'sm', onClick: () => this._startWebRTC() }),
            this._btn('断开', { danger: true, size: 'sm', onClick: () => this._disconnectRTC() }),
          ),
          // 状态显示
          h('div', { class: 'flex flex-wrap gap-sm fs-sm' },
            this._stateChip('RTC', this.state.rtcState),
            this._stateChip('DC', this.state.dcState),
            this._stateChip('ICE', this.state.iceState),
            this._stateChip('Signaling', this.state.signalingState),
            this._stateChip('Conn', this.state.connState),
          ),
          // 聊天区
          h('div', { class: 'flex items-center gap-sm flex-wrap' },
            chatInput.render(),
            this._btn('发送', { type: 'primary', size: 'sm', onClick: () => this._sendChat() }),
          ),
          // 消息记录
          this.state.messages.length === 0
            ? h('div', { class: 'fs-sm text-tertiary' }, '（暂无消息，建立连接后发送试试）')
            : h('div', { class: 'log-panel' },
              ...this.state.messages.map((m: ChatMessage) => h('div', { class: 'log-panel__line' },
                h('span', { class: 'log-panel__time' }, m.time),
                h('span', { class: `log-panel__tag log-panel__tag--${m.dir === 'send' ? 'push' : m.dir === 'recv' ? 'pop' : 'info'}` },
                  m.dir === 'send' ? 'SEND' : m.dir === 'recv' ? 'RECV' : 'ECHO'),
                h('span', {}, m.text),
              )),
            ),
        ),
      ...
      h(Card, {
        ...
        extra: h(Tag, { color: this.state.wtSupported ? 'success' : 'warning' },
          this.state.wtSupported ? '已支持' : '未支持'),
      },
        h('div', { class: 'flex flex-col gap-sm' },
          h('p', { class: 'fs-sm text-secondary' },
            'WebTransport 基于 HTTP/3（QUIC），提供低延迟的客户端-服务器通信，支持单向流、双向流与不可靠数据报。该 API 较新，需先做能力检测；无可靠公共测试服务器，故仅做连接尝试与优雅降级。'),
          !this.state.wtSupported
            ? h(Alert, {
              type: 'warning',
          ...
          h('div', { class: 'flex flex-col gap-xs' },
            ...(Object as any).entries(this.state.wtSupportDetail).map(([name, ok]: [string, boolean]) => h('div', { class: 'flex items-center gap-sm fs-sm' },
              h('span', { class: `tag ${ok ? 'tag--success' : 'tag--default'}` }, ok ? '✓' : '✕'),
              h('span', { class: ok ? 'fw-medium' : 'text-tertiary' }, name),
            )),
          ),
          h('div', { class: 'flex items-center gap-sm flex-wrap' },
            this._btn('运行支持检测', { size: 'sm', onClick: () => this._checkWtSupport() }),
            this._btn('尝试连接', { type: 'primary', size: 'sm', disabled: !this.state.wtSupported, onClick: () => this._tryWtConnect() }),
            this._btn('演示流 API', { size: 'sm', disabled: !this.state.wtSupported, onClick: () => this._demoWtStreams() }),
            this._btn('关闭', { danger: true, size: 'sm', onClick: () => this._closeWt() }),
          ),
          h('div', { class: 'fs-sm text-secondary' }, `连接状态：${this.state.wtState}`),
        ),
      ),
      ...
      h(Card, {
        ...
        extra: h('div', { class: 'flex items-center gap-xs' },
          h(Tag, { color: 'primary' }, 'RTCDataChannel'),
          h(Tag, { color: 'warning' }, 'RTCStatsReport'),
        ),
      },
        h('div', { class: 'flex flex-col gap-sm' },
          h('p', { class: 'fs-sm text-secondary' },
            '深入剖析数据通道对象属性（label/id/ordered/maxRetransmits 等）、背压处理（bufferedAmountLowThreshold + onbufferedamountlow）、文本与二进制发送，并通过 RTCPeerConnection.getStats() 获取 RTCStatsReport，过滤 inbound-rtp / outbound-rtp / data-channel 条目。'),
          h('div', { class: 'flex items-center gap-sm flex-wrap' },
            this._btn('重新创建通道', { type: 'primary', size: 'sm', onClick: () => this._recreateChannel() }),
            this._btn('刷新通道属性', { size: 'sm', onClick: () => this._refreshDcInfo() }),
            this._btn('发送二进制', { size: 'sm', onClick: () => this._sendBinary() }),
            this._btn('设置背压阈值', { size: 'sm', onClick: () => this._setupBackpressure() }),
            this._btn('刷新统计', { size: 'sm', onClick: () => this._refreshStats() }),
          ),
          // 通道属性
          h('div', { class: 'fs-sm text-secondary' }, '数据通道属性：'),
          this.state.dcInfo.length === 0
            ? h('div', { class: 'fs-sm text-tertiary' }, '（建立连接后点击「刷新通道属性」）')
            : h('div', { class: 'flex flex-col gap-xs' },
              ...this.state.dcInfo.map((item: DcInfoItem) => h('div', { class: 'api-metric' },
                h('span', { class: 'fs-sm text-secondary', style: { minWidth: '180px' } }, item.label),
                h('span', { class: 'fw-medium fs-sm' }, item.value),
              )),
            ),
          // 统计
          h('div', { class: 'fs-sm text-secondary mt-sm' }, 'RTCStatsReport：'),
          h('div', { class: 'fs-sm text-tertiary' }, this.state.statsInfo || '（点击「刷新统计」获取 getStats() 结果）'),
          this.state.statsEntries.length === 0
            ? null
            : h('div', { class: 'flex flex-col gap-xs mt-xs' },
              ...this.state.statsEntries.map((s: StatsEntry) => h('div', { class: 'api-metric' },
                h('span', { class: 'tag tag--primary', style: { minWidth: '90px' } }, s.type),
                h('span', { class: 'fs-sm text-tertiary', style: { minWidth: '120px' } }, s.id),
                h('span', { class: 'fs-sm' }, `sent=${s.bytesSent}B/${s.messagesSent}msg`),
                h('span', { class: 'fs-sm' }, `recv=${s.bytesReceived}B/${s.messagesReceived}msg`),
              )),
            ),
        ),
      ),
      ...
      h(Card, {
        ...
        this.state.logs.length === 0
          ...
          : h('div', { class: 'log-panel' },
            ...this.state.logs.map((log: RealtimeCommsLog) => h('div', { class: 'log-panel__line' },
