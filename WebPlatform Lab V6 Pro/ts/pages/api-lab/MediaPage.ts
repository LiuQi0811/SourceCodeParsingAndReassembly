...
export interface MediaPageProps extends Props {}

export interface MediaGeo {
  latitude: number;
  longitude: number;
  accuracy: number;
...
export interface MediaPageState extends State {
  geo: MediaGeo | null;
...
export class MediaPage extends Page {
  declare props: MediaPageProps;
  declare state: MediaPageState;
  _audioRaf: number | null = null;
  _audioCtx: AudioContext | null = null;
  _oscillator: OscillatorNode | null = null;
  _gain: GainNode | null = null;
  _analyser: AnalyserNode | null = null;

  initialState(): MediaPageState {
    return {
  ...
  componentWillUnmount(): void {
    if (this._audioRaf != null) cancelAnimationFrame(this._audioRaf);
  ...
  componentDidMount(): void {
    this._setupDnd((this.$('.dnd-area') as any));
  }
  ...
  _getGeo(): void {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos: GeolocationPosition) => {
        ...
        this._addLog('info', `定位成功：${(latitude as any).toFixed(4)}, ${(longitude as any).toFixed(4)}`);
  ...
  // —— Clipboard ——
  async _writeClipboard(): Promise<void> {
  ...
  async _readClipboard(): Promise<void> {
    try {
  ...
  // —— Drag & Drop ——
  _setupDnd(target: Element | null): void {
    if (!target) return;
    this.on(target, 'dragover', (e: Event) => {
    ...
    this.on(target, 'drop', (e: Event) => {
      ...
      // 读第一个文本文件
      const textFile = files.find((f: File) => f.type.startsWith('text/'));
      if (textFile) {
  ...
  _toggleAudio(): void {
    ...
    try {
      ...
      const canvas = this.$<HTMLCanvasElement>('#audio-canvas');
      ...
      const step = (): void => {
        ...
        const freqLabel = (this.$('#audio-freq-label') as any);
        if (freqLabel) freqLabel.textContent = `峰值 ${Math.round(freq)}Hz`;
  ...
  // —— Notification ——
  async _requestNotif(): Promise<void> {
  ...
  renderPage(): Node | string | (Node | string)[] {
    const clipboardInput = new Input({
      ...
      onChange: (v: string) => { this.state.clipboard = v; },
    });
    ...
    return [
      ...
      h('div', { class: 'feature-grid mt-lg' },
        // Geolocation
        h(Card, { title: 'Geolocation 地理位置' },
          h('div', { class: 'flex flex-col gap-sm' },
            this._btn('获取当前位置', { type: 'primary', size: 'sm', onClick: () => this._getGeo() }),
            this.state.geoError ? h('div', { class: 'alert alert--error' }, this.state.geoError) : '',
            this.state.geo ? h('div', { class: 'fs-sm' },
              h('div', {}, h('strong', {}, '纬度：'), (this.state.geo.latitude as any).toFixed(6)),
              h('div', {}, h('strong', {}, '经度：'), (this.state.geo.longitude as any).toFixed(6)),
              h('div', {}, h('strong', {}, '精度：'), `${(this.state.geo.accuracy as any).toFixed(0)}m`),
              h('div', {}, h('strong', {}, '海拔：'), this.state.geo.altitude ?? 'N/A'),
            ) : '',
          ),
        ),

        // Clipboard
        h(Card, { title: 'Clipboard 剪贴板' },
          h('div', { class: 'flex flex-col gap-sm' },
            clipboardInput.render(),
            h('div', { class: 'flex gap-sm' },
              this._btn('写入剪贴板', { type: 'primary', size: 'sm', onClick: () => this._writeClipboard() }),
              this._btn('读取剪贴板', { size: 'sm', onClick: () => this._readClipboard() }),
            ),
            h('p', { class: 'fs-sm text-tertiary' }, 'navigator.clipboard.writeText / readText。读取需要用户授权。'),
          ),
        ),

        // Notification
        h(Card, { title: 'Notification 通知' },
          h('div', { class: 'flex flex-col gap-sm' },
            h('div', { class: 'flex items-center gap-sm' },
              h('span', { class: 'fs-sm' }, '权限：'),
              h(Tag, {
                color: this.state.notifPermission === 'granted' ? 'success'
                  : this.state.notifPermission === 'denied' ? 'error' : 'warning',
              }, this.state.notifPermission),
            ),
            this._btn('请求权限并发送通知', {
      ...
      // Drag & Drop
      h(Card, { title: 'Drag & Drop 拖放 + FileReader' },
        h('p', { class: 'fs-sm text-secondary' }, '将本地文件拖入下方区域。演示 dataTransfer.files、FileReader.readAsText。'),
        h('div', {
          class: 'dnd-area',
        }, h('div', { class: 'dnd-area__hint' }, '拖放文件到此区域')),
        (this.state.droppedFiles || []).length > 0 ? h('div', { class: 'mt-sm' },
          h('div', { class: 'fs-sm text-secondary mb-sm' }, `已接收 ${(this.state.droppedFiles || []).length} 个文件：`),
          h('div', { class: 'log-panel' },
            ...(this.state.droppedFiles || []).map((f: DroppedFile, i: number) => h('div', { class: 'log-panel__line' },
              h('span', { class: 'log-panel__tag log-panel__tag--push' }, String(i + 1)),
              h('span', {}, `${f.name}  (${f.size} bytes, ${f.type || '未知类型'})  ·  ${f.lastModified}`),
            )),
          ),
        ) : '',
      ),
      ...
      // Web Audio
      h(Card, { title: 'Web Audio 振荡器 + 频谱分析', extra: h(Tag, { color: this.state.audioRunning ? 'success' : 'default' }, h('span', { id: 'audio-freq-label' }, this.state.audioRunning ? `峰值 ${this.state.audioFreq}Hz` : '已停止')) },
        h('div', { class: 'flex flex-col gap-sm' },
          h('div', { class: 'flex gap-sm' },
            this._btn(this.state.audioRunning ? '停止' : '启动 440Hz 正弦波', {
              type: 'primary', size: 'sm', onClick: () => this._toggleAudio(),
            }),
          ),
          h('canvas', { id: 'audio-canvas', class: 'canvas-stage', style: { width: '100%', height: '120px' }, width: 600, height: 120 }),
          h('p', { class: 'fs-sm text-tertiary' }, 'OscillatorNode → GainNode → AnalyserNode → destination。AnalyserNode.getByteFrequencyData 实时频谱。'),
        ),
      ),
      ...
      h('div', { class: 'log-panel' },
        ...this.state.events.map((log: MediaEventLog) => h('div', { class: 'log-panel__line' },
