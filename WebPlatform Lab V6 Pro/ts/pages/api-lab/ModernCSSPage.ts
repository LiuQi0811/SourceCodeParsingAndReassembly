...
export interface ModernCSSPageProps extends Props {}

export interface ModernCSSPageState extends State {}

export class ModernCSSPage extends Page {
  declare props: ModernCSSPageProps;
  declare state: ModernCSSPageState;
  _inited: boolean = false;
  _injectedStyles: any[] | null = null;
  initialState(): ModernCSSPageState {
    return {
      logs: [],
      // —— 能力检测结果 ——
...
@media (prefers-color-scheme: dark) {
  ...
  _layoutTrigDots() {
    const stage = (this.$('#trig-stage') as any);
    if (!stage) return;
  ...
  _renderTrigTextWrapCard() {
    ...
    return h(Card, {
      ...
      h('div', {
        ...
        ...[0, 1, 2, 3, 4].map((i: any) => h('div', { class: 'trig-dot' }, `${i + 1}`)),
      ),
      h('p', { class: 'fs-sm text-tertiary mt-sm' },
