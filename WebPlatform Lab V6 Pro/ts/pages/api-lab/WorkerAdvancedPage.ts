...
export interface WorkerAdvancedPageProps extends Props {}

export interface WorkerAdvancedPageState extends State {}

export class WorkerAdvancedPage extends Page {
  declare props: WorkerAdvancedPageProps;
  declare state: WorkerAdvancedPageState;
  _broadcastChannels: any[] | null = null;
  _dedicatedWorker: any = null;
  _inited: boolean = false;
  _messageChannel: any = null;
  _sharedWorker: any = null;
  _workerPort: any = null;
  // —— 初始 state ——
  initialState(): WorkerAdvancedPageState {
    return {
      logs: [],
      capsSummary: '',
      // Card 1：Worker 类型对比
  ...
  componentDidMount(): void {
    ...
    if (!hasSharedWorker) this._addLog('warn', 'SharedWorker 不可用（jsdom 通常无此构造器，需真实浏览器）');
    if (!hasOffscreenCanvas) this._addLog('warn', 'OffscreenCanvas 不可用（jsdom 无此构造器，需真实浏览器）');
    if (!hasTransferControl) this._addLog('warn', 'HTMLCanvasElement.prototype.transferControlToOffscreen 不可用（jsdom 未实现）');
  }
  ...
  _demoDedicatedMock() {
    ...
    if (!caps.worker) {
      // 测试环境无 Worker：记录用法说明
      this.setState({ workerCompare:
        'Dedicated Worker 用法（测试环境不可用，仅说明）：\n\n' +
        "// 主线程\n" +
    ...
    try {
      const mockWorker = {
        ...
        onerror: null,
        onmessageerror: null,
        postMessage(_msg: any, _transfer: any) { /* mock：仅演示签名 */ },
        terminate() { /* mock */ },
      };
      ...
      const instKeys = Object.getOwnPropertyNames(mockWorker).filter((k: any) => k !== 'constructor');
      this.setState({ workerCompare:
        'Dedicated Worker mock 演示（未真正 new Worker，避免依赖脚本文件）：\n\n' +
  ...
  _demoSharedMock() {
    ...
    try {
      ...
      this.setState({ workerCompare:
        'SharedWorker mock 演示（未真正 new SharedWorker）：\n\n' +
  ...
  _renderCard1() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.worker ? 'success' : 'error' }, caps.worker ? 'Worker ✓' : 'Worker ✗'),
        h(Tag, { color: caps.sharedWorker ? 'success' : 'error' }, caps.sharedWorker ? 'SharedWorker ✓' : 'SharedWorker ✗'),
        h(Tag, { color: 'primary' }, 'Dedicated / Shared / Service'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'Dedicated Worker（专用 Worker）只能与创建它的页面一对一通信；SharedWorker（共享 Worker）可被多个页面 / iframe 共享，通过 port（MessagePort）通信；ServiceWorker（服务 Worker）拦截网络请求、提供离线缓存，独立线程无 DOM 访问。三者构造器与通信方式均不同。本卡片用 mock 对象演示 API 表面，不真正创建 Worker（避免依赖脚本文件）。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('类型对比表', { type: 'primary', size: 'sm', onClick: () => this._showWorkerDiff() }),
          this._btn('Dedicated mock', { size: 'sm', disabled: !caps.worker, onClick: () => this._demoDedicatedMock() }),
          this._btn('SharedWorker mock', { size: 'sm', disabled: !caps.sharedWorker, onClick: () => this._demoSharedMock() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'Worker 类型对比 / 用法：'),
        h('pre', { class: 'code-block', style: { maxHeight: '280px', overflow: 'auto' } },
          h('code', {}, s.workerCompare || '（点击「类型对比表」或对应 mock 按钮）')),
        h(Alert, {
          type: 'info',
  ...
  _checkModuleSupport() {
    ...
    this.setState({ moduleWorkerInfo:
      'Module Worker 支持检测：\n' +
  ...
  _demoClassicWorker() {
    ...
    this.setState({ moduleWorkerInfo:
      'Classic Worker（type: "classic"，默认）API 与用法：\n\n' +
  ...
  _demoModuleWorker() {
    ...
    this.setState({ moduleWorkerInfo:
      'Module Worker（type: "module"）API 与用法：\n\n' +
      `typeof Worker === 'function'：${caps.worker}\n` +
  ...
  _renderCard2() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.worker ? 'success' : 'error' }, caps.worker ? 'Worker ✓' : '不可用'),
        h(Tag, { color: 'primary' }, 'importScripts / import'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'Classic Worker（type: "classic"，默认）在 Worker 内用 importScripts("lib.js") 同步加载脚本，阻塞执行，不能用 import/export 语法；Module Worker（type: "module"）用 import { x } from "./lib.js" 静态导入，支持 import() 动态导入与 top-level await，可复用主线程 ES Module 生态。检测：worker.constructor === Worker（两者相同，区分靠 type 选项）。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('检测 Module 支持', { type: 'primary', size: 'sm', disabled: !caps.worker, onClick: () => this._checkModuleSupport() }),
          this._btn('Classic Worker', { size: 'sm', disabled: !caps.worker, onClick: () => this._demoClassicWorker() }),
          this._btn('Module Worker', { size: 'sm', disabled: !caps.worker, onClick: () => this._demoModuleWorker() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'Module / Classic Worker 用法对比：'),
        h('pre', { class: 'code-block', style: { maxHeight: '280px', overflow: 'auto' } },
          h('code', {}, s.moduleWorkerInfo || '（点击「检测 Module 支持」或对应用法按钮）')),
        h(Alert, {
          type: 'info',
  ...
  _checkOffscreen() {
    ...
    this.setState({ offscreenInfo:
      'OffscreenCanvas 能力检测：\n' +
  ...
  _createOffscreen() {
    ...
    if (!caps.offscreenCanvas) {
      this.setState({ offscreenInfo:
        'OffscreenCanvas 用法（测试环境不可用，仅说明）：\n\n' +
        '// 主线程：从 <canvas> 转移控制权\n' +
  ...
  _demoTransferControl() {
    ...
    if (!caps.transferControl) {
      this.setState({ offscreenInfo:
        'transferControlToOffscreen 用法（测试环境不可用，仅说明）：\n\n' +
    ...
    try {
      ...
      const offscreen = canvas.transferControlToOffscreen();
      this.setState({ offscreenInfo:
        '真实演示 transferControlToOffscreen：\n' +
  ...
  _renderCard3() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.offscreenCanvas ? 'success' : 'error' }, caps.offscreenCanvas ? 'OffscreenCanvas ✓' : '不可用'),
        h(Tag, { color: caps.transferControl ? 'success' : 'error' }, caps.transferControl ? 'transferControl ✓' : 'transferControl ✗'),
        h(Tag, { color: 'primary' }, '后台渲染'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'OffscreenCanvas 可在 Worker 内渲染，不阻塞主线程。主线程用 canvas.transferControlToOffscreen() 把 <canvas> 控制权转移给 OffscreenCanvas，再通过 worker.postMessage({ canvas }, [canvas]) 转移给 Worker；Worker 内用 getContext("2d"/"webgl"/"webgl2") 渲染，并可用 requestAnimationFrame（仅 OffscreenCanvas Worker 支持）。convertToBlob(options) 导出为 Blob。jsdom 中 OffscreenCanvas / transferControlToOffscreen 通常不可用。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('检测能力', { type: 'primary', size: 'sm', onClick: () => this._checkOffscreen() }),
          this._btn('创建 OffscreenCanvas', { size: 'sm', disabled: !caps.offscreenCanvas, onClick: () => this._createOffscreen() }),
          this._btn('transferControl', { size: 'sm', disabled: !caps.transferControl, onClick: () => this._demoTransferControl() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'OffscreenCanvas 状态：'),
        h('pre', { class: 'code-block', style: { maxHeight: '280px', overflow: 'auto' } },
          h('code', {}, s.offscreenInfo || '（点击「检测能力」或对应演示按钮）')),
        h(Alert, {
          type: 'warning',
  ...
  _sendFromPort2() {
    ...
    try {
      ...
      this.setState({ channelInfo:
        `Worker 端 → port2.postMessage(result)（模拟回传）：\n` +
  ...
  _renderCard4() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.messageChannel ? 'success' : 'error' }, caps.messageChannel ? 'MessageChannel ✓' : '不可用'),
        h(Tag, { color: 'primary' }, 'port1 / port2'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
        ...
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('创建 MessageChannel', { type: 'primary', size: 'sm', disabled: !caps.messageChannel, onClick: () => this._createMessageChannel() }),
          this._btn('port1 发送', { size: 'sm', disabled: !caps.messageChannel, onClick: () => this._sendFromPort1() }),
          this._btn('port2 回传', { size: 'sm', disabled: !caps.messageChannel, onClick: () => this._sendFromPort2() }),
          this._btn('关闭 channel', { danger: true, size: 'sm', disabled: !caps.messageChannel, onClick: () => this._closeMessageChannel() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'MessageChannel 状态：'),
        h('pre', { class: 'code-block', style: { maxHeight: '240px', overflow: 'auto' } },
          h('code', {}, s.channelInfo || '（点击「创建 MessageChannel」开始双向通信演示）')),
        h(Alert, {
          type: 'info',
  ...
  _createBroadcast() {
    ...
    if (!caps.broadcastChannel) {
      this.setState({ broadcastInfo:
        'BroadcastChannel 用法（测试环境不可用，仅说明）：\n\n' +
  ...
  _renderCard5() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.broadcastChannel ? 'success' : 'error' }, caps.broadcastChannel ? 'BroadcastChannel ✓' : '不可用'),
        h(Tag, { color: 'primary' }, '跨页面 / 跨 Worker'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'new BroadcastChannel(name) 创建同名广播通道：同名的所有 channel（跨标签页 / 跨 Worker / 同源）互通，channel.postMessage(message) 广播给所有同名 channel（除发送者自己），channel.onmessage 接收，channel.close() 关闭。用途：跨标签页同步登录态、数据刷新、通知推送。BroadcastChannel 在 Node 18+ 与 jsdom 中通常可用。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('创建 A/B channel', { type: 'primary', size: 'sm', disabled: !caps.broadcastChannel, onClick: () => this._createBroadcast() }),
          this._btn('A 广播', { size: 'sm', disabled: !caps.broadcastChannel, onClick: () => this._broadcastFromA() }),
          this._btn('B 广播', { size: 'sm', disabled: !caps.broadcastChannel, onClick: () => this._broadcastFromB() }),
          this._btn('关闭所有', { danger: true, size: 'sm', disabled: !caps.broadcastChannel, onClick: () => this._closeBroadcast() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'BroadcastChannel 状态：'),
        h('pre', { class: 'code-block', style: { maxHeight: '240px', overflow: 'auto' } },
          h('code', {}, s.broadcastInfo || '（点击「创建 A/B channel」开始广播演示）')),
        h(Alert, {
          type: 'info',
  ...
  _detectWorkerGlobal() {
    this.setState({ globalScopeInfo:
      'Worker 全局对象检测（在主线程执行，仅检测 typeof）：\n\n' +
  ...
  _renderCard6() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: 'primary' }, 'WorkerGlobalScope'),
        h(Tag, { color: 'warning' }, '有 / 无 API'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'WorkerGlobalScope 是 Worker 的全局作用域：有 self / importScripts / location / navigator / setTimeout / fetch / IndexedDB / WebSocket / BroadcastChannel / Performance API / caches（部分）；无 window / document / parent / localStorage / DOM / UI API。DedicatedWorkerGlobalScope 有 self.name / self.close()；SharedWorkerGlobalScope 有 onconnect / self.close()；ServiceWorkerGlobalScope 有 self.caches / self.clients / fetch 事件。本卡片在主线程检测 typeof 列出差异。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('检测全局对象', { type: 'primary', size: 'sm', onClick: () => this._detectWorkerGlobal() }),
          this._btn('列出 API 清单', { size: 'sm', onClick: () => this._listWorkerAPIs() }),
          this._btn('self 上下文检测', { size: 'sm', onClick: () => this._checkSelfContext() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'Worker 全局对象对比：'),
        h('pre', { class: 'code-block', style: { maxHeight: '320px', overflow: 'auto' } },
          h('code', {}, s.globalScopeInfo || '（点击「检测全局对象」或「列出 API 清单」）')),
        h(Alert, {
          type: 'warning',
  ...
  render() {
    ...
    return h('div', { class: 'page api-lab-page' },
      h('h2', { class: 'section-title' }, 'Worker 与并发通信 实验室'),
      h('p', { class: 'fs-sm text-secondary mb-md' },
        'Web Worker 把耗时任务移出主线程避免 UI 卡顿。本页演示 Dedicated / Shared / Module Worker、OffscreenCanvas、MessageChannel、BroadcastChannel 与 WorkerGlobalScope。'),
