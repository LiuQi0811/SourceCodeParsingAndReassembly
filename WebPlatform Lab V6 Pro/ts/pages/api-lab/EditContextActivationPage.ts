...
export interface EditContextActivationPageProps extends Props {}

export interface EditContextActivationPageState extends State {}

export class EditContextActivationPage extends Page {
  declare props: EditContextActivationPageProps;
  declare state: EditContextActivationPageState;
  _ecBound: boolean = false;
  _ecEventsAttached: boolean = false;
  _ecInstance: any = null;
  _inited: boolean = false;
  _injectedStyles: any[] | null = null;
  initialState(): EditContextActivationPageState {
    return {
      logs: [],
      capsSummary: '',
      activationState: '',
  ...
  _renderCard1() {
    ...
    return h(Card, {
      ...
      s.activationState ? h('p', { class: 'fs-sm mt-sm text-secondary' }, s.activationState) : null as any,
...
// 权限敏感 API 前置判断，避免 try/catch 反复试错`),
  ...
  _createEditContext() {
    ...
    const host = (this.$('#ec-edit-canvas') as any);
  ...
  _renderCard2() {
    ...
    return h(Card, {
      ...
      s.editContextInfo ? h('p', { class: 'fs-sm mt-sm text-secondary' }, s.editContextInfo) : null as any,
      h('pre', { class: 'code-block mt-md' },
`const ctx = new EditContext();
...
// 作者负责把文本渲染到 DOM；浏览器负责 IME/虚拟键盘事件分发`),
  ...
  _attachEditContextEvents() {
    ...
    try {
      ...
      if (typeof ctx.addEventListener === 'function') {
        ctx.addEventListener('textupdate', (e: any) => {
          this._addLog('info',
        ...
        ctx.addEventListener('textformatupdate', (e: any) => {
          let formats = '(无法读取)';
        ...
        ctx.addEventListener('characterboundsupdate', (e: any) => {
  ...
  _renderCard3() {
    ...
    return h(Card, {
      ...
      s.editContextEvents ? h('p', { class: 'fs-sm mt-sm text-secondary' }, s.editContextEvents) : null as any,
      h('pre', { class: 'code-block mt-md' },
...
<div contenteditable="true">可直接编辑</div>`),
  ...
  _preflightPermission(apiName: any) {
    const caps = this._caps();
    if (!caps.userActivation) {
  ...
  _renderCard5() {
    ...
    return h(Card, {
      ...
      h('div', { class: 'ec-perm-list' },
        perms.map((p: any) => h('div', { class: 'ec-perm-row' },
          h('code', {}, p.code),
      ...
      s.permissionInfo ? h('p', { class: 'fs-sm mt-sm text-secondary' }, s.permissionInfo) : null as any,
      h('pre', { class: 'code-block mt-md' },
`// 三态模型
