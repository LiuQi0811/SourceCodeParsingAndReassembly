...
function _randomBytes(len: number): Uint8Array {
...
// 支持的凭据类型字符串
type CredentialTypeName = 'PasswordCredential' | 'FederatedCredential' | 'OTPCredential';
...
// FedCM 配置预览结构
interface FedcmPreview {
  mediation: string;
  identity: any;
}

// 字体数据
interface LocalFontItem {
...
interface IdentityCaps {
  ...
  queryLocalFonts: boolean;
  identityCredential: boolean;
}

export interface IdentityPageProps extends Props {}
...
export interface IdentityPageState extends State {
  logs: IdentityLog[];
  // Card 1: Credential Management
  credSupportedTypes: CredentialTypeName[];
  credPreventSilentResult: string;
  credStoreResult: string;
  // Card 2: WebAuthn
  webauthnPlatformAuth: boolean | null;
  webauthnConditionalMed: boolean | null;
  webauthnOptionsPreview: any | null;
  // Card 3: Payment Request
  paymentCanMake: boolean | null;
  paymentPreview: PaymentPreview | null;
  paymentShowResult: string;
  // Card 4: Presentation + Idle Detection
  presentationRequestPreview: PresentationPreview | null;
  idlePermission: string;
  idleRunning: boolean;
  idleUserState: string;
  idleScreenState: string;
  // Card 5: Local Font + FedCM
  localFonts: LocalFontItem[];
  localFontsLoading: boolean;
  fedcmPreview: FedcmPreview | null;
}

export class IdentityPage extends Page {
  declare props: IdentityPageProps;
  declare state: IdentityPageState;
  _inited: boolean = false;
  _presentationConnection: any | null = null;
  _idleDetector: any | null = null;
  _idleChangeHandler: ((e: Event) => void) | null = null;
  declare _destroyed: boolean;

  // —— 初始 state ——
  initialState(): IdentityPageState {
    return {
      logs: [],
      // Card 1: Credential Management
      credSupportedTypes: [],
  ...
  _caps(): IdentityCaps {
    ...
    return {
      ...
      publicKeyCredential: typeof (win as any).PublicKeyCredential !== 'undefined',
      paymentRequest: typeof (win as any).PaymentRequest !== 'undefined',
      presentation: 'presentation' in nav,
      idleDetector: typeof (win as any).IdleDetector !== 'undefined',
      queryLocalFonts: typeof (win as any).queryLocalFonts === 'function',
  ...
  // 键值行（label + value）
  _kvRow(label: string, value: unknown): Node {
  ...
  // 子区块标题
  _subSection(title: string, ...children: (Node | null)[]): Node {
    return h('div', { class: 'flex flex-col gap-xs' },
      h('div', { class: 'fs-sm fw-medium' }, title),
      ...children,
    );
  }

  // 把对象渲染为代码块（处理 Uint8Array → base64 摘要）
  _jsonBlock(obj: unknown): Node {
    if (!obj) return h('div', { class: 'fs-sm text-tertiary' }, '（未构造）');
    try {
      ...
      const text = JSON.stringify(obj, replacer as any, 2);
  ...
  _detectCredentialTypes(): void {
    const win = typeof window !== 'undefined' ? window : ({} as any);
    const types: CredentialTypeName[] = [];
    if ('PasswordCredential' in win) types.push('PasswordCredential');
    if ('FederatedCredential' in win) types.push('FederatedCredential');
    if ('OTPCredential' in win) types.push('OTPCredential');
  ...
  async _preventSilentAccess(): Promise<void> {
    if (!('credentials' in navigator)) {
  ...
  async _createAndStorePasswordCredential(): Promise<void> {
    if (!('credentials' in navigator)) {
    ...
    try {
      ...
      const cred = new PasswordCredential(fakeForm as any);
  ...
  async _checkPlatformAuthenticator(): Promise<void> {
  ...
  async _checkConditionalMediation(): Promise<void> {
    if (typeof PublicKeyCredential === 'undefined') {
  ...
  _buildPaymentRequest(): PaymentPreview | null {
  ...
  async _canMakePayment(): Promise<void> {
    if (typeof PaymentRequest === 'undefined') {
      this._addLog('err', 'PaymentRequest 不可用');
      return;
    }
    let preview = this.state.paymentPreview;
    if (!preview) preview = this._buildPaymentRequest();
    if (!preview) return;
    try {
  ...
  async _showPayment(): Promise<void> {
    if (typeof PaymentRequest === 'undefined') {
      this._addLog('err', 'PaymentRequest 不可用');
      return;
    }
  ...
  _buildPresentationRequest(): void {
    ...
    try {
      ...
      const preview: PresentationPreview = {
        urls,
        defaultRequest: !!(navigator as any).presentation.defaultRequest,
        receiver: !!(navigator as any).presentation.receiver,
      };
  ...
  _disconnectPresentation(fromCleanup = false): void {
    const conn = this._presentationConnection;
    if (!conn) {
      if (!fromCleanup) this._addLog('pres', '无活跃 PresentationConnection');
      return;
    }
    try {
      conn.onconnect = null;
      ...
      if (conn.state === 'connected') conn.terminate();
    } catch { /* noop */ }
    this._presentationConnection = null;
    if (!fromCleanup) this._addLog('pres', '已断开 PresentationConnection');
  }

  // 请求 'idle-detection' 权限
  async _idleRequestPermission(): Promise<void> {
  ...
  async _startIdleDetector(): Promise<void> {
    if (typeof IdleDetector === 'undefined') {
    ...
    try {
      ...
      (detector as any).addEventListener('change', this._idleChangeHandler);
  ...
  // 停止 IdleDetector
  _stopIdleDetector(fromCleanup = false): void {
    if (!this._idleDetector) {
    ...
    try {
      if (this._idleChangeHandler) {
        (this._idleDetector as any).removeEventListener('change', this._idleChangeHandler);
      }
    } catch { /* noop */ }
  ...
  async _queryLocalFonts(): Promise<void> {
    ...
    try {
      ...
      const list: LocalFontItem[] = fonts.slice(0, 20).map((f: any) => ({
  ...
  renderPage(): Node | string | (Node | string)[] {
    ...
    return [
      ...
      h(Card, {
        ...
        extra: h('div', { class: 'flex items-center gap-xs' },
          h(Tag, { color: 'primary' }, 'navigator.credentials'),
          h(Tag, { color: caps.credentials ? 'success' : 'warning' }, caps.credentials ? '稳定' : '不可用'),
        ),
      },
        h('div', { class: 'flex flex-col gap-sm' },
          h('p', { class: 'fs-sm text-secondary' },
            '统一管理密码 / 联合 / OTP 凭据：get / create / store / preventSilentAccess。mediation 取值 silent / optional / conditional / required，控制自动登录行为；preventSilentAccess 用于退出后阻止下次自动登录。'),
          h('div', { class: 'flex items-center gap-sm flex-wrap fs-sm' },
            h('span', { class: 'text-tertiary' }, '能力：'),
            this._capTag(caps.credentials),
            h('span', { class: 'text-tertiary' }, '已识别类型：'),
            s.credSupportedTypes.length === 0
              ? h('span', { class: 'text-tertiary' }, '（未检测）')
              : s.credSupportedTypes.map((t) => h(Tag, { color: 'primary' }, t)),
          ),
          h('div', { class: 'flex items-center gap-sm flex-wrap' },
            this._btn('检测凭据类型', { size: 'sm', onClick: () => this._detectCredentialTypes() }),
            this._btn('preventSilentAccess', { type: 'primary', size: 'sm', disabled: !caps.credentials, onClick: () => this._preventSilentAccess() }),
            this._btn('创建并 store 密码凭据', { type: 'primary', size: 'sm', disabled: !caps.credentials, onClick: () => this._createAndStorePasswordCredential() }),
          ),
          this._kvRow('preventSilentAccess 结果', s.credPreventSilentResult),
          this._kvRow('store 结果', s.credStoreResult),
          h('div', { class: 'fs-sm text-tertiary' }, 'mediation 取值：' + MEDIATION_VALUES.join(' / ')),
        ),
      ),
      ...
      h(Card, {
        ...
        extra: h('div', { class: 'flex items-center gap-xs' },
          h(Tag, { color: 'primary' }, 'PublicKeyCredential'),
          h(Tag, { color: caps.publicKeyCredential ? 'success' : 'warning' }, caps.publicKeyCredential ? '可用' : '不可用'),
          h(Tag, { color: 'warning' }, '实验性'),
        ),
      },
        h('div', { class: 'flex flex-col gap-sm' },
          h('p', { class: 'fs-sm text-secondary' },
            '基于公钥的无密码认证。create({publicKey}) 注册新凭据，get({publicKey}) 完成断言。可探测平台认证器（TouchID / WindowsHello）与条件 UI（autofill 中弹 WebAuthn UI）。注册选项含 rp / user / challenge / pubKeyCredParams / authenticatorSelection / timeout / attestation。'),
          h('div', { class: 'flex items-center gap-sm flex-wrap' },
            this._btn('检测平台认证器', { type: 'primary', size: 'sm', disabled: !caps.publicKeyCredential, onClick: () => this._checkPlatformAuthenticator() }),
            this._btn('检测条件 UI', { type: 'primary', size: 'sm', disabled: !caps.publicKeyCredential, onClick: () => this._checkConditionalMediation() }),
            this._btn('构造注册选项', { size: 'sm', disabled: !caps.publicKeyCredential, onClick: () => this._buildWebAuthnOptions() }),
          ),
          this._kvRow('平台认证器', s.webauthnPlatformAuth === null ? '未检测' : (s.webauthnPlatformAuth ? '可用' : '不可用')),
          this._kvRow('条件 UI', s.webauthnConditionalMed === null ? '未检测' : (s.webauthnConditionalMed ? '可用' : '不可用')),
          this._subSection('PublicKeyCredentialCreationOptions 预览：',
            this._jsonBlock(s.webauthnOptionsPreview),
          ),
        ),
      ),
      ...
      h(Card, {
        ...
        extra: h('div', { class: 'flex items-center gap-xs' },
          h(Tag, { color: 'primary' }, 'PaymentRequest'),
          h(Tag, { color: caps.paymentRequest ? 'success' : 'warning' }, caps.paymentRequest ? '可用' : '不可用'),
          h(Tag, { color: 'default' }, 'basic-card 已弃用'),
        ),
      },
        h('div', { class: 'flex flex-col gap-sm' },
          h('p', { class: 'fs-sm text-secondary' },
            '原生支付 UI：methodData + details + options 三段构造。show() 弹出 UI，canMakePayment() 探测支持，abort() 取消。basic-card 已弃用，使用 https://google.com/pay 等 URL 形式的支付处理程序。details 含 displayItems / total / shippingOptions / modifiers。'),
          h('div', { class: 'flex items-center gap-sm flex-wrap' },
            this._btn('构造 PaymentRequest', { size: 'sm', disabled: !caps.paymentRequest, onClick: () => this._buildPaymentRequest() }),
            this._btn('canMakePayment', { type: 'primary', size: 'sm', disabled: !caps.paymentRequest, onClick: () => this._canMakePayment() }),
            this._btn('show() 支付', { type: 'primary', size: 'sm', disabled: !caps.paymentRequest, onClick: () => this._showPayment() }),
          ),
          this._kvRow('canMakePayment 结果', s.paymentCanMake === null ? '未检测' : (s.paymentCanMake ? '可支付' : '不可支付')),
          this._kvRow('show 结果', s.paymentShowResult),
          this._subSection('PaymentRequest 结构预览：',
            this._jsonBlock(s.paymentPreview),
          ),
        ),
      ),
      ...
      h(Card, {
        ...
        extra: h('div', { class: 'flex items-center gap-xs' },
          h(Tag, { color: 'primary' }, 'PresentationRequest'),
          h(Tag, { color: caps.presentation ? 'success' : 'warning' }, caps.presentation ? '可用' : '不可用'),
          h(Tag, { color: 'primary' }, 'IdleDetector'),
          h(Tag, { color: caps.idleDetector ? 'success' : 'warning' }, caps.idleDetector ? '可用' : '不可用'),
          h(Tag, { color: 'warning' }, '实验性'),
        ),
      },
        h('div', { class: 'flex flex-col gap-sm' },
          h('p', { class: 'fs-sm text-secondary' },
            'Presentation API 用于第二屏幕投屏：PresentationRequest.start / reconnect / getAvailability，PresentationConnection 经 connect / close / terminate 与 statechange 事件管理。Idle Detection API 经 idle-detection 权限监听 userState / screenState 变化，threshold 控制空闲阈值。'),
          // Presentation 子区块
          this._subSection('Presentation：',
            h('div', { class: 'flex items-center gap-sm flex-wrap' },
              this._btn('构造 PresentationRequest', { size: 'sm', disabled: !caps.presentation, onClick: () => this._buildPresentationRequest() }),
              this._btn('断开连接', { danger: true, size: 'sm', onClick: () => this._disconnectPresentation() }),
            ),
            this._jsonBlock(s.presentationRequestPreview),
          ),
          // Idle Detection 子区块
          this._subSection('Idle Detection：',
            h('div', { class: 'flex items-center gap-sm flex-wrap' },
              this._btn('请求权限', { type: 'primary', size: 'sm', disabled: !caps.idleDetector, onClick: () => this._idleRequestPermission() }),
              this._btn('启动监听', { type: 'primary', size: 'sm', disabled: !caps.idleDetector || s.idleRunning, onClick: () => this._startIdleDetector() }),
              this._btn('停止监听', { danger: true, size: 'sm', disabled: !s.idleRunning, onClick: () => this._stopIdleDetector() }),
            ),
            this._kvRow('权限状态', s.idlePermission),
            this._kvRow('运行中', s.idleRunning ? '是' : '否'),
            this._kvRow('userState', s.idleUserState),
            this._kvRow('screenState', s.idleScreenState),
          ),
        ),
      ),
      ...
      h(Card, {
        ...
        extra: h('div', { class: 'flex items-center gap-xs' },
          h(Tag, { color: 'primary' }, 'queryLocalFonts'),
          h(Tag, { color: caps.queryLocalFonts ? 'success' : 'warning' }, caps.queryLocalFonts ? '可用' : '不可用'),
          h(Tag, { color: 'primary' }, 'IdentityCredential'),
          h(Tag, { color: caps.identityCredential ? 'success' : 'warning' }, caps.identityCredential ? '可用' : '不可用'),
          h(Tag, { color: 'warning' }, '实验性'),
        ),
      },
        h('div', { class: 'flex flex-col gap-sm' },
          h('p', { class: 'fs-sm text-secondary' },
            'Local Font Access API 经 local-fonts 权限枚举本地字体（FontData: postscriptName / fullName / family / style），用于高级排版。FedCM（Federated Credential Management）提供浏览器原生 IdP 联合登录 UI，避免第三方 Cookie 跟踪：navigator.credentials.get({ identity: { providers, mode, context } })。'),
          // Local Font 子区块
          this._subSection('Local Font Access：',
            h('div', { class: 'flex items-center gap-sm flex-wrap' },
              this._btn('枚举本地字体', { type: 'primary', size: 'sm', disabled: !caps.queryLocalFonts || s.localFontsLoading, onClick: () => this._queryLocalFonts() }),
            ),
            s.localFonts.length === 0
              ? h('div', { class: 'fs-sm text-tertiary' }, '（暂无字体数据）')
              : h('div', { class: 'flex flex-col gap-xs' },
                ...s.localFonts.map((f) => h('div', { class: 'api-metric' },
                  h('span', { class: 'fs-sm fw-medium', style: { minWidth: '180px' } }, f.family),
                  h('span', { class: 'fs-sm text-secondary', style: { minWidth: '140px' } }, f.style),
                  h('span', { class: 'fs-sm text-tertiary', style: { wordBreak: 'break-all' } }, f.postscriptName),
                )),
              ),
          ),
          // FedCM 子区块
          this._subSection('FedCM：',
            h('div', { class: 'flex items-center gap-sm flex-wrap' },
              this._btn('构造 FedCM 配置', { type: 'primary', size: 'sm', disabled: !caps.identityCredential, onClick: () => this._buildFedcmConfig() }),
            ),
            h('div', { class: 'fs-sm text-secondary' }, 'IdentityCredential 配置预览：'),
            this._jsonBlock(s.fedcmPreview),
          ),
        ),
      ),
