...
interface MediaTrackConstraintsPageState extends State {
  logs: any;
  capsSummary: any;
  overviewInfo: any;
  aecInfo: any;
...
export class MediaTrackConstraintsPage extends Page {
  declare state: MediaTrackConstraintsPageState;
  _dynamicStyles!: any[];
  _inited!: boolean;
  _micStream!: any | null;
  _micTrack!: any;
  ...
  _stopMicStream() {
    if (this._micStream) {
      try { this._micStream.getTracks().forEach((t: any) => t.stop()); } catch { /* noop */ }
  ...
  _runOverviewDemo() {
    ...
    if (f.supportedConstraints) {
      ...
      catch (err: any) { this._addLog('warn', `getSupportedConstraints 失败：${err.name} - ${err.message}`); }
    }
    const supportedKeys = ALL_AUDIO_CONSTRAINT_KEYS.filter((k: any) => ((supported as any)[(k as any)]));
    const unsupportedKeys = ALL_AUDIO_CONSTRAINT_KEYS.filter((k: any) => !((supported as any)[(k as any)]));
