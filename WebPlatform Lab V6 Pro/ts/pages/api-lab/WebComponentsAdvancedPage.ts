...
export interface WebComponentsAdvancedPageProps extends Props {}

export interface WebComponentsAdvancedPageState extends State {}

export class WebComponentsAdvancedPage extends Page {
  declare props: WebComponentsAdvancedPageProps;
  declare state: WebComponentsAdvancedPageState;
  _composedEl: any = null;
  _cssPartEl: any = null;
  _definedEl: any = null;
  _dsdEl: any = null;
  _formEl: any = null;
  ...
  _observer: any = null;
  _stateEl: any = null;
  _stateInternals: any = null;
  _v: any = null;
  initialState(): WebComponentsAdvancedPageState {
    return {
      logs: [], capsSummary: '',
  ...
  _caps(items: any) {
    return items.map((it: any) => h(Tag, { color: it.ok ? 'success' : 'error' }, `${it.label} ${it.ok ? '✓' : '✗'}`));
  }
  ...
  _injectDSD() {
    const d = this._detected();
    const container = (this.$('.wc-adv-dsd-host') as any);
    ...
    try {
      ...
      if (d.dsd) {
        ...
        const sr = host.attachShadow({ mode: 'open' } as any);
  ...
  _defineFormAssociated() {
    ...
    try {
      ...
      class WcAdvField extends HTMLElement {
        ...
        formResetCallback() { this.value = ''; }
        formDisabledCallback(_d: any) { /* noop */ }
        formStateRestoreCallback(_s: any, _m: any) { /* noop */ }
      }
  ...
  _demoIsAttribute() {
    ...
    try {
      ...
      const container = (this.$('.wc-adv-defined-host') as any);
      if (container) {
        container.innerHTML = '';
  ...
  _dispatchComposed() {
    ...
    try {
      const container = (this.$('.wc-adv-composed-host') as any);
      if (!container) { this._addLog('warn', '找不到挂载容器'); return; }
      container.innerHTML = '';
      const host = document.createElement('div');
      ...
      host.addEventListener('inner-action', (e: any) => {
        ...
        let pathInfo = '（不可读）';
        let targetInfo = '（不可读）';
        try { pathInfo = e.composedPath ? e.composedPath().map((n: any) => n.nodeName || String(n)).join(' → ') : '（无 composedPath）'; } catch { /* noop */ }
  ...
  _injectPartStyle() {
    ...
    try {
      const container = (this.$('.wc-adv-css-host') as any);
      if (!container) { this._addLog('warn', '找不到挂载容器'); return; }
      container.innerHTML = '';
      const el = document.createElement('div');
      el.className = 'wc-adv-css-part-el';
      const sr = el.attachShadow({ mode: 'open' } as any);
      sr.innerHTML =
  ...
  _aggregateDetect() {
    ...
    const lines = items.map((it: any) => `${it.label}：${it.ok ? '✓ 支持' : '✗ 不支持 / 不可用'}`);
