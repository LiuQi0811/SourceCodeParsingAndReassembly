...
export interface AccessibilityInteractionPageProps extends Props {}

export interface AccessibilityInteractionPageState extends State {}

export class AccessibilityInteractionPage extends Page {
  declare props: AccessibilityInteractionPageProps;
  declare state: AccessibilityInteractionPageState;
  _connHandler: any = null;
  _fsHandlers: any = null;
  _inited: boolean = false;
  _lastVpKey: any = null;
  _lastVpLog: any = null;
  _offlineHandler: any = null;
  _onlineHandler: any = null;
  _pointerCancelUnbind: any = null;
  _visibilityUnbind: any = null;
  _vvHandlers: any = null;
  _vvRAF: any = null;
  _wakeLock: any = null;
  _wakeLockAutoReacquire: boolean = false;
  // —— 初始 state ——
  // capsSummary 初始为 ''，能力检测完成后填入字符串，触发条件渲染 Alert
  initialState(): AccessibilityInteractionPageState {
    return {
      logs: [],
      capsSummary: '',
  ...
  _fsEnter() {
    ...
    const target = (this.$('.fs-demo-box') as any);
  ...
  _vvRead() {
    ...
    this._addLog('vp', `visualViewport: ${(vv.width as any).toFixed(0)}×${(vv.height as any).toFixed(0)} scale=${(vv.scale as any).toFixed(2)} offset=(${vv.offsetLeft},${vv.offsetTop}) pageLeft=${vv.pageLeft}`);
  }
  ...
  _vvListenStart() {
    ...
    const onResize = () => {
      ...
      this._vvRAF = requestAnimationFrame(() => {
        ...
        const key = `${(v!.width as any).toFixed(0)},${(v!.height as any).toFixed(0)},${(v!.scale as any).toFixed(2)},${v!.offsetTop}`;
        if (key === this._lastVpKey) return;
        this._lastVpKey = key;
        this._addLog('vp', `resize 事件：${(v!.width as any).toFixed(0)}×${(v!.height as any).toFixed(0)} scale=${(v!.scale as any).toFixed(2)} offsetTop=${v!.offsetTop}`);
        this.setState({
          vpInfo: {
