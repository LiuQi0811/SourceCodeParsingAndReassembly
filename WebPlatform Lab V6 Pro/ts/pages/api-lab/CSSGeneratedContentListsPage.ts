...
interface CSSGeneratedContentListsPageState extends State {
  logs: LogEntry[];
  capsSummary: string;
  genContentInfo: string;
  listsInfo: string;
  counterStyleInfo: string;
  pagedMediaInfo: string;
}
...
export class CSSGeneratedContentListsPage extends Page {
  declare state: CSSGeneratedContentListsPageState;
  _inited: boolean = false;
  _dynamicStyles: HTMLStyleElement[] = [];


  initialState(): CSSGeneratedContentListsPageState {
    return {
      logs: [],
      capsSummary: '',
  ...
  _renderCard1(): Node | string {
    ...
    return h(Card, {
      ...
      s.genContentInfo ? h('pre', { class: 'code-block mt-md' }, s.genContentInfo) : null as any,
    );
  }
  ...
  _renderCard2(): Node | string {
    ...
    return h(Card, {
      ...
      s.listsInfo ? h('pre', { class: 'code-block mt-md' }, s.listsInfo) : null as any,
    );
  }

  // ===================== Card 3：@counter-style 自定义计数器 =====================
  _runCounterStyleDemo(): void {
    const f = this._flags();
  ...
  _renderCard3(): Node | string {
    ...
    return h(Card, {
      ...
      s.counterStyleInfo ? h('pre', { class: 'code-block mt-md' }, s.counterStyleInfo) : null as any,
    );
  }

  // ===================== Card 4：@page + Fragmentation =====================
  _runPagedMediaDemo(): void {
    const f = this._flags();
  ...
  _renderCard4(): Node | string {
    ...
    return h(Card, {
      ...
      s.pagedMediaInfo ? h('pre', { class: 'code-block mt-md' }, s.pagedMediaInfo) : null as any,
    );
  }
