...
interface WebXRAdvancedPageState extends State {
  logs: any;
  capsSummary: any;
  overviewInfo: any;
  hitTestInfo: any;
...
export class WebXRAdvancedPage extends Page {
  declare state: WebXRAdvancedPageState;
  _anchors!: any;
  _dynamicStyles!: any[];
  _gl!: any | null;
  _hitTestSource!: any;
  _inited!: boolean;
  _rafId!: ReturnType<typeof setTimeout> | number | null;
  _referenceSpace!: any;
  _xrSession!: any | null;
  ...
  componentDidMount() {
    ...
    this._anchors = new Set<any>();       // Card 3 XRAnchor 集合
    ...
    if (!f.xr) this._addLog('warn', 'navigator.xr 不可用（Chrome 79+ WebXR，需 HTTPS + XR 设备/模拟器）');
    if (!f.xrSession) this._addLog('warn', 'XRSession 接口不可用（需 navigator.xr.requestSession 返回）');
    if (!f.hitTest) this._addLog('warn', 'XRHitTestSource 不可用（Chrome 81+ AR，hit-test 特性）');
    if (!f.xrAnchor) this._addLog('warn', 'XRAnchor 接口不可用（Chrome 85+ AR，anchors 特性）');
    if (!f.domOverlay) this._addLog('warn', 'XRDOMOverlayState 不可用（Chrome 83+ AR，dom-overlay 特性）');
    if (!f.layers) this._addLog('warn', 'XRLayer 接口不可用（Chrome 90+ 桌面/91+ Android，layers 特性）');
  ...
  componentWillUnmount() {
    ...
    if (this._xrSession && typeof this._xrSession.end === 'function') {
      try { this._xrSession.end(); } catch { /* noop */ }
    }
    this._xrSession = null;
    this._hitTestSource = null;
    this._referenceSpace = null;
    this._gl = null;
    try { this._anchors.clear(); } catch { /* noop */ }
    // 移除动态样式
  ...
  _buildOverviewInfo(f: any, support: any){
    return [
      '===== WebXR AR 进阶概览 =====',
      '',
...
try {
  ...
  // NotSupportedError: Required feature 'non-existent-feature' not supported
        ...
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.overviewInfo || '（点击按钮查看 AR 进阶概览与支持矩阵）')),
      ],
    });
      ...
      try {
        if (this._hitTestSource && typeof frame.getHitTestResults === 'function') {
          ...
          if (hits.length > 0) {
            ...
            if (pose) {
              this._addLog('info', `命中 ${hits.length} 处，首点 transform: (${(pose.transform.position.x as any).toFixed(2)}, ${(pose.transform.position.y as any).toFixed(2)}, ${(pose.transform.position.z as any).toFixed(2)})`);
            }
          }
        }
      } catch { /* noop */ }
  ...
  _buildHitTestInfo(f: any, started: any){
    return [
      '===== Hit Test 射线检测 =====',
      '',
  ...
  _renderCard2() {
    ...
    const card = new Card({
      ...
      children: [
        ...
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行 Hit Test 演示', { type: 'primary', size: 'sm', onClick: () => this._runHitTestDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, `// 创建持久 hit-test 源
const hitTestSource = await session.requestHitTestSource({
  space: referenceSpace,
  entityTypes: ['plane', 'point'],
});
...
hitTestSource.cancel();`)),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.hitTestInfo || '（点击按钮查看 Hit Test 射线检测完整说明）')),
      ],
    });
    this.registerChild(card);
    return card.render();
  ...
  _buildAnchorInfo(f: any, created: any){
    return [
      '===== Anchor 锚点 =====',
      '',
  ...
  _renderCard3() {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'frame.createAnchor(pose, space) 在真实世界坐标创建 XRAnchor 持久锚点，session 生命周期内 frame.trackedAnchors 返回当前有效的锚点集合（XRAnchorSet）。每帧用 anchor.getSpace() + frame.getPose() 更新关联物体矩阵。锚点失效时不再出现在 trackedAnchors，据此清理物体。Anchor 比 hit-test 直接放置更稳定，大范围移动漂移 < 5cm。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行 Anchor 演示', { type: 'primary', size: 'sm', onClick: () => this._runAnchorDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, `// hit-test 命中后创建锚点固定物体
const hits = frame.getHitTestResults(hitTestSource);
if (hits.length > 0) {
  const pose = hits[0].getPose(referenceSpace);
  if (pose) {
...
anchors.delete(anchor);`)),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.anchorInfo || '（点击按钮查看 Anchor 锚点完整说明）')),
      ],
    });
    this.registerChild(card);
    return card.render();
  ...
  _buildDomOverlayInfo(f: any, supported: any){
    return [
      '===== DOM Overlays =====',
      '',
      '【domOverlay: { root } 申请】',
  ...
  _renderCard4() {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'requestSession 时传 domOverlay: { root } 指定 DOM 子树作为 HTML 层叠加在 AR 场景之上（仅 AR 模式）。session.domOverlayState.type 仅 "screen"（屏幕固定）。容器 pointer-events: none 不拦截 AR 交互，按钮设 auto 可点。DOM overlay 走浏览器合成器不占 WebGL 预算，适合 HUD/按钮/文字标签。跟随 3D 物体需手动 worldToNDC 投影同步 CSS transform。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行 DOM Overlay 演示', { type: 'primary', size: 'sm', onClick: () => this._runDomOverlayDemo() }),
        ),
        h('div', { class: 'webxr-overlay-preview' },
          h('div', { class: 'webxr-overlay-elt btn-top' }, '🏠 已放置 3 件家具'),
          h('div', { class: 'webxr-overlay-elt btn-right' }, '⚙️ 设置'),
          h('div', { class: 'webxr-overlay-elt btn-bottom' }, '👆 点击地面放置物体'),
          h('div', { class: 'webxr-overlay-hint' }, 'DOM Overlay 预览（屏幕固定层）'),
        ),
        h('div', { class: 'fs-sm text-secondary', style: { marginTop: '8px' } }, '代码示例：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, `// 申请 AR session 带 DOM overlay
const session = await navigator.xr.requestSession('immersive-ar', {
  requiredFeatures: ['hit-test', 'anchors'],
  optionalFeatures: ['dom-overlay'],
  domOverlay: { root: document.querySelector('#ar-overlay-ui') },
...
function projectToScreen(worldPos: any,  frame: any,  refSpace: any) {
  ...
  labelEl.style.transform =
    \`translate(\${(ndc.x*0.5+0.5)*innerWidth}px, \${(-ndc.y*0.5+0.5)*innerHeight}px)\`;
        ...
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.domOverlayInfo || '（点击按钮查看 DOM Overlays 完整说明）')),
      ],
    });
    this.registerChild(card);
    return card.render();
  ...
  _buildLayersInfo(f: any, supported: any){
    return [
      '===== WebXR Layers 概述 =====',
      '',
      '【默认基础层：XRWebGLLayer】',
  ...
  _renderCard5() {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '传统 XRWebGLLayer 单一 framebuffer 渲染所有内容（背景+UI+3D），重绘开销大。WebXR Layers 支持多层独立 framebuffer 由合成器高效合成：ProjectionLayer（主 3D 场景，每眼独立）、QuadLayer（平面 UI）、CylinderLayer（弧形菜单）、EquirectLayer（360° 视频）、CubeLayer（环境贴图）。静态层只渲染一次复用多帧，360° 背景用 EquirectLayer 比 ProjectionLayer 省 30-50% GPU。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行 Layers 概述演示', { type: 'primary', size: 'sm', onClick: () => this._runLayersDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, `// 传统：单一 XRWebGLLayer
const glLayer = new XRWebGLLayer(session, gl);
session.updateRenderState({ baseLayer: glLayer });
...
session.updateRenderState({
  layers: [equirectLayer, projectionLayer, quadLayer],
        ...
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.layersInfo || '（点击按钮查看 WebXR Layers 概述完整说明）')),
      ],
    });
    this.registerChild(card);
    return card.render();
  ...
  _renderCard6() {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'QuadLayer 平面 UI（眼前悬浮菜单，XRRigidTransform 定位 + width/height 物理尺寸）、EquirectLayer 360° 视频（球面投影背景，radius=0 无限远，视频帧只上传一次复用多 view）、CylinderLayer 弧形菜单（圆柱内壁，centralAngleLength 控制弧度）、ProjectionLayer 多视角高效合成（texture-array 单次绘制双眼，比 XRWebGLLayer 快 20-30%）、CubeLayer 环境反射（6 面 TEXTURE_CUBE_MAP）。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行各 Layer 类型演示', { type: 'primary', size: 'sm', onClick: () => this._runLayerTypesDemo() }),
        ),
        h('div', { class: 'webxr-layer-stack' },
          h('div', { class: 'webxr-layer-row' },
            h('span', { class: 'webxr-layer-name' }, 'QuadLayer'),
            h('span', { class: 'webxr-layer-desc' }, '平面 UI：菜单/信息卡悬浮在 3D 空间，XRRigidTransform 定位'),
          ),
          h('div', { class: 'webxr-layer-row' },
            h('span', { class: 'webxr-layer-name' }, 'EquirectLayer'),
            h('span', { class: 'webxr-layer-desc' }, '360° 视频：球面背景，radius=0 无限远，省 30-50% GPU'),
          ),
          h('div', { class: 'webxr-layer-row' },
            h('span', { class: 'webxr-layer-name' }, 'CylinderLayer'),
            h('span', { class: 'webxr-layer-desc' }, '弧形菜单：圆柱内壁，centralAngleLength 控制弧度'),
          ),
          h('div', { class: 'webxr-layer-row' },
            h('span', { class: 'webxr-layer-name' }, 'ProjectionLayer'),
            h('span', { class: 'webxr-layer-desc' }, '主 3D 场景：texture-array 单次绘制双眼，快 20-30%'),
          ),
        ),
        h('div', { class: 'fs-sm text-secondary', style: { marginTop: '8px' } }, '代码示例：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, `// QuadLayer 平面 UI（悬浮菜单）
const quadLayer = new XRQuadLayer(session, gl, {
  space: refSpace,
...
session.updateRenderState({
  layers: [equirectLayer, projectionLayer, quadLayer, cylinderLayer],
        ...
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.layerTypesInfo || '（点击按钮查看各 Layer 类型实战完整说明）')),
      ],
    });
    this.registerChild(card);
    return card.render();
  ...
  _buildFurnitureInfo(f: any, supported: any){
    return [
      '===== 实战 AR 家具摆放 =====',
      '',
  ...
  _renderCard7() {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '综合 Hit Test（地面检测）+ Anchor（家具固定抗漂移）+ DOM Overlay（放置/旋转/删除按钮）+ Layers（ProjectionLayer 渲染家具、QuadLayer 信息卡、EquirectLayer 房间背景）+ light-estimation 环境光照亮。与 Three.js WebXR 桥接：renderer.xr.enabled + ARButton.createButton + XRController select 事件，setAnimationLoop 自动管理 RAF。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行 AR 家具摆放演示', { type: 'primary', size: 'sm', onClick: () => this._runFurnitureDemo() }),
        ),
        h('div', { class: 'webxr-furniture-stage' },
          h('div', { class: 'webxr-furniture-canvas' },
            h('div', { class: 'webxr-furniture-item', style: { top: '40px', left: '40px' } }, '🛋️'),
            h('div', { class: 'webxr-furniture-item', style: { top: '110px', left: '120px' } }, '🪑'),
            h('div', { class: 'webxr-furniture-item', style: { top: '60px', left: '220px' } }, '🪟'),
            h('div', { class: 'webxr-furniture-item', style: { bottom: '20px', right: '40px' } }, '🪴'),
          ),
          h('div', { class: 'webxr-furniture-controls' },
            this._btn('👆 放置', { type: 'primary', size: 'sm', block: true, onClick: () => this._addLog('warn', 'AR 家具放置需真实 ARCore session（jsdom 无 WebXR）') }),
            this._btn('🔄 旋转', { type: 'default', size: 'sm', block: true, onClick: () => this._addLog('warn', '家具旋转需真实 AR session') }),
            this._btn('🗑️ 删除', { type: 'default', size: 'sm', block: true, onClick: () => this._addLog('warn', '家具删除需真实 AR session') }),
            this._btn('🧹 清空', { type: 'default', size: 'sm', block: true, danger: true, onClick: () => this._addLog('warn', '清空所有家具需真实 AR session') }),
          ),
        ),
        h('div', { class: 'fs-sm text-secondary', style: { marginTop: '8px' } }, '代码示例：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, `// 申请 AR session（一次性申请所有 features）
const session = await navigator.xr.requestSession('immersive-ar', {
  requiredFeatures: ['local-floor', 'hit-test', 'anchors'],
  optionalFeatures: ['dom-overlay', 'layers', 'light-estimation'],
  domOverlay: { root: document.querySelector('#furniture-ui') },
...
renderer.setAnimationLoop(() => renderer.render(scene, camera));`)),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.furnitureInfo || '（点击按钮查看 AR 家具摆放完整说明）')),
      ],
    });
    this.registerChild(card);
    return card.render();
  ...
  _renderCard8() {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'isSessionSupported 异步检测后，requiredFeatures 失败抛 NotSupportedError（catch 后去掉不可用 feature 重新申请），optionalFeatures 失败静默忽略。iOS Safari 不支持 WebXR 降级 <a rel="ar" href=".usdz"> Quick Look（iOS 12+）。Google <model-viewer ar> 自动切换 Android Scene Viewer / iOS Quick Look。8thWall 商业 WebAR 兜底 iOS。优先级：WebXR AR → Quick Look → Model Viewer → 8thWall → 纯 3D 预览。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行降级方案演示', { type: 'primary', size: 'sm', onClick: () => this._runFallbackDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, `// 1. 异步检测 + requiredFeatures 失败回退
const arOK = await navigator.xr.isSessionSupported('immersive-ar');
...
engine.start({ canvas, webxr: 'AR' });`)),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.fallbackInfo || '（点击按钮查看能力检测与降级完整说明）')),
      ],
    });
    this.registerChild(card);
    return card.render();
