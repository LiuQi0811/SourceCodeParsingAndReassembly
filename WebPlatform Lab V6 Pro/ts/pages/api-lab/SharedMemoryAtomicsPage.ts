...
interface SharedMemoryAtomicsPageCaps {
  sab: boolean;
  atomics: boolean;
  crossIsolated: boolean;
  resizable: boolean;
  transfer: boolean;
  transferToFixed: boolean;
  detached: boolean;
  weakRef: boolean;
  finalization: boolean;
  structuredClone: boolean;
  sharedStruct: boolean;
}

export interface SharedMemoryAtomicsPageProps extends Props {}

export interface SharedMemoryAtomicsPageState extends State {
  logs: LogEntry[];
  capsSummary: string;
  sabInfo: string;
  atomicsInfo: string;
  waitNotifyInfo: string;
  resizableInfo: string;
  weakRefInfo: string;
  transferInfo: string;
  sharedStructsInfo: string;
}

export class SharedMemoryAtomicsPage extends Page {
  declare props: SharedMemoryAtomicsPageProps;
  declare state: SharedMemoryAtomicsPageState;
  ...
  initialState(): SharedMemoryAtomicsPageState {
    return {
      logs: [], capsSummary: '',
  ...
  componentDidMount(): void {
    ...
    const hasSharedStruct = (() => { try { return typeof SharedStruct !== 'undefined' || (typeof window !== 'undefined' && typeof (window as any).SharedStruct !== 'undefined'); } catch { return false; } })();
  ...
  _caps(): SharedMemoryAtomicsPageCaps {
    return {
      ...
      sharedStruct: (() => { try { return typeof SharedStruct !== 'undefined' || (typeof window !== 'undefined' && typeof (window as any).SharedStruct !== 'undefined'); } catch { return false; } })(),
    };
  }
  ...
  _createSAB(): void {
    ...
    try {
      ...
      this.setState({
        sabInfo:
          `new SharedArrayBuffer(16) → 共享内存 ✓，byteLength = ${(sab as any).byteLength}\n` +
      ...
      this._addLog('sab', `已创建 SAB：byteLength=${(sab as any).byteLength}，load(0)=${loaded0}，load(1)=${loaded1}`);
    } catch (err: any) {
    ...
    try {
      ...
      this.setState({
        sabInfo:
          ...
          `原 SAB byteLength = ${(this._sab as any).byteLength}，Int32[0..3] = [${viewOrig[0]}, ${viewOrig[1]}, ${viewOrig[2]}, ${viewOrig[3]}]\n` +
          `切片 SAB byteLength = ${(sliced as any).byteLength}，Int32[0..1] = [${viewSlice[0]}, ${viewSlice[1]}]\n` +
          `说明：slice 返回新的 SharedArrayBuffer，数据是拷贝（修改切片不影响原 SAB）。`,
      });
      this._addLog('sab', `slice(0,8)：切片 byteLength=${(sliced as any).byteLength}，Int32=[${viewSlice[0]}, ${viewSlice[1]}]`);
    } catch (err: any) {
  ...
  _growSAB(): void {
    ...
    try {
      ...
      const before = (sab as any).byteLength;
      const maxBefore = sab.maxByteLength;
      // grow 到 16 字节
      sab.grow(16);
      const after = (sab as any).byteLength;
      this.setState({
        sabInfo:
          ...
          `grow 后：byteLength = ${(sab as any).byteLength}，maxByteLength = ${sab.maxByteLength}（新字节初始化为 0）`,
      });
    ...
    try {
      ...
      this.setState({
        atomicsInfo:
          ...
          `Atomics.store(view, 0, 10) → 初始值 10；view.length = ${view.length}，byteLength = ${(sab as any).byteLength}\n` +
...
Atomics.isLockFree(4);                 // true（4 字节原子操作无锁）`)),
  ...
  _createWaitView(): void {
    ...
    try {
      ...
      this._addLog('wait', `已创建 wait 视图：store(0, 0)，byteLength=${(sab as any).byteLength}`);
    } catch (err: any) {
  ...
  _waitNotifyDemo(): void {
    ...
    try {
      ...
      let wokenAll: any = 0;
      try { wokenAll = Atomics.notify(view, 0, Infinity); } catch (e: any) { wokenAll = `抛错:${e.name}`; }
    ...
    try {
      ...
      this.setState({
        resizableInfo:
          ...
          `(ab as any).byteLength = ${(ab as any).byteLength}，ab.maxByteLength = ${ab.maxByteLength}，ab.resizable = ${ab.resizable}\n` +
          `说明：ES2024 起 ArrayBuffer 支持 maxByteLength 创建可调整大小缓冲区；resizable 为 true 时可调用 grow/resize（不超过 maxByteLength）。`,
      });
      this._addLog('resizable', `可调整 AB：byteLength=${(ab as any).byteLength}，maxByteLength=${ab.maxByteLength}，resizable=${ab.resizable}`);
    } catch (err: any) {
  ...
  _growResizeAB(): void {
    ...
    try {
      ...
      if (typeof ab.grow === 'function') {
        ab.grow(16);
        lines.push(`ab.grow(16) → byteLength: ${before} → ${(ab as any).byteLength}（grow 只能增大）`);
      } else { lines.push('ab.grow 不可用'); }
      // resize：可增大也可缩小
      if (typeof ab.resize === 'function') {
        ab.resize(24);
        const afterR24 = (ab as any).byteLength;
        ab.resize(12);
        lines.push(`ab.resize(24) → ${afterR24}，resize(12) → ${(ab as any).byteLength}（resize 可增可缩）`);
      } else { lines.push('ab.resize 不可用（部分实现只有 grow）'); }
      this.setState({
        resizableInfo:
          `ab.grow / ab.resize 调整大小：\n` +
          lines.join('\n') + '\n' +
          `当前：byteLength = ${(ab as any).byteLength}，maxByteLength = ${ab.maxByteLength}，resizable = ${ab.resizable}\n` +
          `说明：grow 只能增大且不能超过 maxByteLength；resize 可增可缩。调整大小后基于原 buffer 的 TypedArray 视图会自动反映新长度。`,
      });
      this._addLog('resizable', `grow/resize 后：byteLength=${(ab as any).byteLength}（max=${ab.maxByteLength}）`);
    } catch (err: any) {
    ...
    try {
      ...
      this.setState({
        resizableInfo:
          ...
          `新 transferred: byteLength = ${(transferred as any).byteLength}，新视图 [0,1] = [${newView[0]}, ${newView[1]}]（数据保留）\n` +
      ...
      this._addLog('resizable', `transfer 后：原 detached=${afterDetached}，新 byteLength=${(transferred as any).byteLength}，view=[${newView[0]}, ${newView[1]}]`);
    } catch (err: any) {
...
ab.transferToFixedLength();   // 转移为固定长度 buffer`)),
  ...
  _createFinalizationRegistry(): void {
  ...
  _tryTriggerGC(): void {
    ...
    try {
      ...
      const hadGc = typeof (globalThis as any).gc === 'function';
      let gcLine = '';
      if (hadGc) {
...
// 注意：GC 时机不确定，回调可能不触发！`)),
  ...
  _demoTransfer(): void {
    ...
    try {
      ...
      const beforeByteLength = (ab as any).byteLength;
      ...
      let afterByteLength;
      try { afterByteLength = (ab as any).byteLength; }
      catch (e: any) { afterByteLength = `抛错：${e.name}（detached 后访问抛 TypeError）`; }
  ...
  _listTransferable(): void {
    try {
      ...
      const lines = types.map((([name, avail]: any) => `• ${name}: ${avail ? '可用' : '不可用'}` as any));
      ...
      if (typeof ArrayBuffer.prototype.transfer === 'function') {
        ...
        const t = ab.transfer();
        detachLine = `\n\nArrayBuffer.transfer() 演示：\n  原 ab.detached = ${ab.detached}（分离）\n  新 ab byteLength = ${(t as any).byteLength}，Int32[0] = ${new Int32Array(t)[0]}（数据保留）`;
      }
      this.setState({
        transferInfo:
