...
export interface EditingInputEventsPageProps extends Props {}

export interface EditingInputEventsPageState extends State {}

export class EditingInputEventsPage extends Page {
  declare props: EditingInputEventsPageProps;
  declare state: EditingInputEventsPageState;
  _beforeInputHandler: any = null;
  _editableEl: any = null;
  _inited: boolean = false;
  _inputHandler: any = null;
  _interceptBeforeInputHandler: any = null;
  _interceptEl: any = null;
  _plainEl: any = null;
  _plainInputHandler: any = null;
  _richEl: any = null;
  _richInputHandler: any = null;
  _selectionChangeHandler: any = null;
  // —— 初始 state ——
  initialState(): EditingInputEventsPageState {
    return {
      logs: [],
      capsSummary: '',
  ...
  componentDidMount(): void {
    ...
    if (!caps.inputEvent) this._addLog('warn', 'InputEvent 不可用（typeof undefined），无法派发合成事件');
    if (!caps.undoManager) this._addLog('warn', 'document.undoManager 不可用（浏览器支持有限），将用 execCommand("undo"/"redo") 演示');
    ...
    this._beforeInputHandler = (e: any) => {
      try {
        ...
        if (typeof e.getTargetRanges === 'function') {
          try { ranges = `${e.getTargetRanges().length} 个 StaticRange`; } catch { /* noop */ }
        }
    ...
    this._inputHandler = (e: any) => {
      try { this._addLog('input', `input 事件触发：inputType=${e.inputType || '(未知)'}，data=${e.data == null ? '(null)' : JSON.stringify(e.data)}`); } catch { /* noop */ }
    };
    this._plainInputHandler = (e: any) => {
      try { this._addLog('edit', `plaintext-only input：inputType=${e.inputType || '(未知)'}（已剥离格式）`); } catch { /* noop */ }
    };
    this._richInputHandler = (e: any) => {
      try { this._addLog('edit', `rich input：inputType=${e.inputType || '(未知)'}`); } catch { /* noop */ }
    };
    this._interceptBeforeInputHandler = (e: any) => {
      ...
      try {
        if (e.inputType === 'insertFromPaste' || e.inputType === 'insertFromDrop') {
          ...
          if (e.dataTransfer && typeof e.dataTransfer.getData === 'function') {
            try { raw = e.dataTransfer.getData('text/plain') || ''; } catch { /* noop */ }
          }
          const sanitized = raw.replace(/<[^>]*>/g, '').replace(/\s+/g, ' ').trim();
          e.preventDefault();
          if (caps.execCommand && this._interceptEl) {
            try { document.execCommand('insertText', false, sanitized); } catch { /* noop */ }
          }
    ...
    this._selectionChangeHandler = () => {
      if (!this._detectCaps().selection) return;
      try {
  ...
  _attachEditableListeners() {
    if (!this.el) return;
    const attach = (selector: any, type: any, handler: any) => {
      try {
        const el = this.el!.querySelector(selector);
        if (el && handler) el.addEventListener(type, handler);
        return el || null;
      } catch { /* noop */ }
      return null as any;
    };
  ...
  componentWillUnmount(): void {
    ...
    const q = (sel: any) => { try { return this.el ? this.el.querySelector(sel) : null; } catch { return null; } };
    const detach = (sel: any, type: any, handler: any) => {
      const el = q(sel);
      if (el && handler) { try { el.removeEventListener(type, handler); } catch { /* noop */ } }
    };
    ...
    if (this._selectionChangeHandler && typeof document !== 'undefined') {
      try { document.removeEventListener('selectionchange', this._selectionChangeHandler); } catch { /* noop */ }
    }
    // 释放引用
  ...
  _detectCaps() {
    ...
    return {
      ...
      undoManager: hasDoc && !!(document as any).undoManager,
  ...
  _caps(items: any) {
    return items.map((it: any) =>
      h(Tag, { color: it.ok ? 'success' : 'error' }, `${it.label} ${it.ok ? '✓' : '✗'}`),
    );
  }
  ...
  _showInputTypeList() {
    this.setState({ inputEventInfo:
      '===== InputEvent.inputType 完整类型清单（约 30 种）=====\n\n' +
  ...
  _dispatchBeforeInput() {
    const caps = this._detectCaps();
    if (!caps.inputEvent) { this._addLog('warn', 'InputEvent 不可用，无法派发合成事件'); return; }
    const target = (this.el && this.el.querySelector('[data-editable="card1"]')) || this._editableEl;
    if (!target) { this._addLog('warn', '未找到 Card1 可编辑元素'); return; }
    try {
      ...
      this.setState({ inputEventInfo:
        '已派发合成 InputEvent 演示 beforeinput 流程：\n' +
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'InputEvent 继承 UIEvent。属性：.inputType（约 30 种类型字符串，如 insertText / insertFromPaste / deleteContentBackward / formatBold / historyUndo）、.data（插入/删除的字符串）、.dataTransfer（粘贴/拖放的 DataTransfer）、.isComposing（IME 组合中）。.getTargetRanges() 返回 StaticRange[]（将被影响的范围，仅 beforeinput 可用，input 事件中为空）。事件顺序：beforeinput（可 preventDefault）→ DOM 变化 → input（不可取消）。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('inputType 清单', { type: 'primary', size: 'sm', onClick: () => this._showInputTypeList() }),
          this._btn('派发 beforeinput', { size: 'sm', disabled: !caps.inputEvent, onClick: () => this._dispatchBeforeInput() }),
        ),
        ...
        h('div', { contentEditable: 'true', class: 'demo-editable', dataset: { editable: 'card1' },
          onbeforeinput: this._beforeInputHandler, oninput: this._inputHandler, style: DEMO_EDIT_STYLE },
          '在此输入文本观察 beforeinput/input 事件流'),
        h('div', { class: 'fs-sm text-secondary' }, 'InputEvent 详情：'),
        h('pre', { class: 'code-block', style: { maxHeight: '280px', overflow: 'auto' } },
          h('code', {}, s.inputEventInfo || '（点击「inputType 清单」或「派发 beforeinput」）')),
  ...
  _detectContentEditable() {
    ...
    try { if (rich) richEditable = String(rich.isContentEditable); } catch { /* noop */ }
    try { if (plain) plainEditable = String(plain.isContentEditable); } catch { /* noop */ }
    try { if (typeof document !== 'undefined') designMode = document.designMode || 'off'; } catch { /* noop */ }
    this.setState({ contentEditableInfo:
      'contentEditable / designMode 检测：\n\n' +
  ...
  _showPasteDiff() {
    this.setState({ contentEditableInfo:
      '===== contentEditable="true" vs "plaintext-only" 粘贴对比 =====\n\n' +
  ...
  _renderCard2() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: 'primary' }, 'contentEditable'),
        h(Tag, { color: 'warning' }, 'designMode'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'element.contentEditable = "true" | "false" | "plaintext-only" | "inherit" 控制元素可编辑性；document.designMode = "on" | "off" 使整个文档可编辑；HTMLElement.isContentEditable（只读）反映当前状态。contentEditable="plaintext-only" 剥离粘贴的 HTML 格式（适合代码编辑器）；designMode 全局生效，contentEditable 针对单个元素。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('检测 isContentEditable', { type: 'primary', size: 'sm', disabled: !caps.execCommand, onClick: () => this._detectContentEditable() }),
          this._btn('粘贴格式对比', { size: 'sm', onClick: () => this._showPasteDiff() }),
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          h('div', { style: { flex: '1', minWidth: '180px' } },
            h('div', { class: 'fs-sm text-secondary mb-xs' }, 'contentEditable="true"（富文本）：'),
            h('div', { contentEditable: 'true', class: 'demo-editable', dataset: { editable: 'card2-rich' },
              oninput: this._richInputHandler, style: DEMO_EDIT_STYLE }, '粘贴富文本会保留格式'),
          ),
          h('div', { style: { flex: '1', minWidth: '180px' } },
            h('div', { class: 'fs-sm text-secondary mb-xs' }, 'contentEditable="plaintext-only"（纯文本）：'),
            h('div', { contentEditable: 'plaintext-only', class: 'demo-editable', dataset: { editable: 'card2-plain' },
        ...
        h('pre', { class: 'code-block', style: { maxHeight: '280px', overflow: 'auto' } },
          h('code', {}, s.contentEditableInfo || '（点击「检测 isContentEditable」或「粘贴格式对比」）')),
  ...
  _runExecCommand(cmd: any, value: any = null) {
    const caps = this._detectCaps();
    if (!caps.execCommand) { this._addLog('warn', `document.execCommand 不可用，无法执行 ${cmd}`); return; }
    let result = '(异常)', enabled = '(未知)', supported = '(未知)', stateVal = '(未知)';
    try {
      if (typeof document.queryCommandSupported === 'function') supported = String(document.queryCommandSupported(cmd));
      if (typeof document.queryCommandEnabled === 'function') enabled = String(document.queryCommandEnabled(cmd));
      result = String(document.execCommand(cmd, false, value));
      if (typeof document.queryCommandValue === 'function') stateVal = String(document.queryCommandValue(cmd));
    } catch (err: any) {
      result = `异常：${err.name}`;
    }
    this._addLog('exec', `execCommand("${cmd}"${value ? `, "${value}"` : ''}) = ${result}（supported=${supported}, enabled=${enabled}, value=${stateVal}）`);
    this.setState({ execCommandInfo:
      `document.execCommand("${cmd}"${value ? `, false, "${value}"` : ', false, null'}) 演示：\n\n` +
  ...
  _showExecCommandList() {
    this.setState({ execCommandInfo:
      '===== document.execCommand(commandId, showUI=false, value=null) 命令清单 =====\n\n' +
  ...
  _renderCard3() {
    ...
    const card = new Card({
      ...
      children: [
        ...
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('命令清单', { type: 'primary', size: 'sm', onClick: () => this._showExecCommandList() }),
          this._btn('Bold', { size: 'sm', disabled: !caps.execCommand, onClick: () => this._runExecCommand('bold') }),
          this._btn('Italic', { size: 'sm', disabled: !caps.execCommand, onClick: () => this._runExecCommand('italic') }),
          this._btn('Underline', { size: 'sm', disabled: !caps.execCommand, onClick: () => this._runExecCommand('underline') }),
          this._btn('插入有序列表', { size: 'sm', disabled: !caps.execCommand, onClick: () => this._runExecCommand('insertOrderedList') }),
          this._btn('formatBlock H1', { size: 'sm', disabled: !caps.execCommand, onClick: () => this._runExecCommand('formatBlock', '<h1>') }),
          this._btn('queryCommandState', { size: 'sm', disabled: !caps.execCommand, onClick: () => this._runExecCommand('bold') }),
        ),
        ...
        h('pre', { class: 'code-block', style: { maxHeight: '280px', overflow: 'auto' } },
          h('code', {}, s.execCommandInfo || '（点击「命令清单」或任意命令按钮）')),
  ...
  _detectUndoManager() {
    ...
    try {
      ...
      if (el && 'undoManager' in el) elUm = !!el.undoManager;
    } catch { /* noop */ }
    this.setState({ undoManagerInfo:
      '===== UndoManager 能力检测 =====\n\n' +
  ...
  _demoExecUndoRedo() {
    const caps = this._detectCaps();
    if (!caps.execCommand) { this._addLog('warn', 'execCommand 不可用，无法演示 undo/redo'); return; }
    let undoResult = '(异常)', redoResult = '(异常)', stateUndo = '(未知)', stateRedo = '(未知)';
    try { undoResult = String(document.execCommand('undo', false, (null as any))); } catch (e: any) { undoResult = `异常：${e.name}`; }
    try { redoResult = String(document.execCommand('redo', false, (null as any))); } catch (e: any) { redoResult = `异常：${e.name}`; }
    try { if (caps.queryCommand) { stateUndo = String(document.queryCommandSupported('undo')); stateRedo = String(document.queryCommandSupported('redo')); } } catch { /* noop */ }
    this._addLog('undo', `execCommand("undo")=${undoResult}，execCommand("redo")=${redoResult}`);
    this.setState({ undoManagerInfo:
      'execCommand("undo" / "redo") 演示（浏览器内部撤销栈）：\n\n' +
  ...
  _renderCard4() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([{ label: 'undoManager', ok: caps.undoManager }]),
        h(Tag, { color: caps.undoManager ? 'success' : 'warning' }, caps.undoManager ? '原生可用' : '需降级'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'element.undoManager（需 undoscope 属性，无浏览器支持）或 document.undoManager：.item/.length/.position/.add(transaction, merge)/.remove/.clear/.undo/.redo/.canUndo/.canRedo/.onundo/.onredo。UndoTransaction 接口（label/apply/unapply/reapply）支持自定义事务。⚠️ 浏览器支持极有限，多数通过 execCommand("undo"/"redo") 内部管理撤销栈（栈深度不可直接访问）。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('检测 undoManager', { type: 'primary', size: 'sm', onClick: () => this._detectUndoManager() }),
          this._btn('execCommand undo/redo', { size: 'sm', disabled: !caps.execCommand, onClick: () => this._demoExecUndoRedo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'UndoManager / 撤销栈详情：'),
        h('pre', { class: 'code-block', style: { maxHeight: '320px', overflow: 'auto' } },
          h('code', {}, s.undoManagerInfo || '（点击「检测 undoManager」或「execCommand undo/redo」）')),
  ...
  _readSelection() {
    const caps = this._detectCaps();
    if (!caps.selection) { this._addLog('warn', 'window.getSelection 不可用'); return; }
    try {
      const sel = window.getSelection();
      if (!sel) { this._addLog('warn', 'getSelection() 返回 null'); return; }
      const text = sel.toString();
      const lines = [
      ...
      lines.push('', '===== Selection API =====',
        '  .rangeCount / .getRangeAt(i) / .addRange / .removeAllRanges / .removeRange / .collapse(node, offset) / .extend(node, offset)',
        '  .selectAllChildren(node) / .deleteFromDocument() / .toString() / .anchorNode / .anchorOffset / .focusNode / .focusOffset / .isCollapsed / .type');
  ...
  _surroundWithStrong() {
    const caps = this._detectCaps();
    if (!caps.selection || !caps.range) { this._addLog('warn', 'Selection / Range 不可用'); return; }
    try {
      const sel = window.getSelection();
      if (!sel || sel.rangeCount === 0 || sel.isCollapsed) {
        this._addLog('warn', '无选区或选区已折叠，无法 surroundContents（请先在浏览器中选中文字）');
        this.setState({ selectionInfo:
          'Range.surroundContents() 演示（需先选中文本）：\n\n' +
  ...
  _demoCreateRange() {
    const caps = this._detectCaps();
    if (!caps.createRange) { this._addLog('warn', 'document.createRange 不可用'); return; }
    try {
      ...
      if (host && host.firstChild) {
        ...
        // 还原：把 extract 的内容插回
        try { range.insertNode(extracted); } catch { /* noop */ }
      }
      this.setState({ selectionInfo:
        'document.createRange() 与 Range 操作演示：\n\n' +
  ...
  _renderCard5() {
    ...
    const card = new Card({
      ...
      children: [
        ...
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('读取选区', { type: 'primary', size: 'sm', disabled: !caps.selection, onClick: () => this._readSelection() }),
          this._btn('surroundContents <strong>', { size: 'sm', disabled: !caps.selection || !caps.range, onClick: () => this._surroundWithStrong() }),
          this._btn('createRange 演示', { size: 'sm', disabled: !caps.createRange, onClick: () => this._demoCreateRange() }),
        ),
        ...
        h('pre', { class: 'code-block', style: { maxHeight: '320px', overflow: 'auto' } },
          h('code', {}, s.selectionInfo || '（点击「读取选区」或「surroundContents」）')),
  ...
  _dispatchPaste() {
    const caps = this._detectCaps();
    if (!caps.inputEvent) { this._addLog('warn', 'InputEvent 不可用，无法派发合成 paste 事件'); return; }
    const target = (this.el && this.el.querySelector('[data-editable="card6"]')) || this._interceptEl;
    if (!target) { this._addLog('warn', '未找到 Card6 拦截宿主'); return; }
    try {
      // 构造带 dataTransfer 的 InputEvent（模拟粘贴）
      let dataTransfer = null;
      try { dataTransfer = new DataTransfer(); } catch { /* jsdom 可能无 DataTransfer */ }
      if (dataTransfer) dataTransfer.setData('text/plain', '<b>粗体</b> <script>alert(1)</script> 普通文本');
      const ev = new InputEvent('beforeinput', {
      ...
      if (!dataTransfer) {
        this.setState({ interceptInfo:
          '已派发合成 insertFromPaste（但 DataTransfer 不可用，dataTransfer=null）：\n' +
  ...
  _renderCard6() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([{ label: 'InputEvent', ok: caps.inputEvent }]),
        h(Tag, { color: 'primary' }, 'preventDefault + getTargetRanges'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '现代自定义编辑方案：监听 beforeinput，检查 e.inputType，调用 e.preventDefault() 取消默认，用 e.getTargetRanges() 拿到 StaticRange[] 手动应用变更。用例：自动补全、@mention、emoji 选择器、linkify、粘贴净化。e.dataTransfer.getData("text/plain"/"text/html") 读取粘贴/拖放内容，净化后插入。本卡片拦截 insertFromPaste，preventDefault 后用 execCommand("insertText") 插入净化纯文本。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('拦截方案说明', { type: 'primary', size: 'sm', onClick: () => this._showInterceptGuide() }),
          this._btn('派发 insertFromPaste', { size: 'sm', disabled: !caps.inputEvent, onClick: () => this._dispatchPaste() }),
        ),
        ...
        h('pre', { class: 'code-block', style: { maxHeight: '280px', overflow: 'auto' } },
          h('code', {}, s.interceptInfo || '（点击「拦截方案说明」或「派发 insertFromPaste」）')),
  ...
  render() {
    const s = this.state;
    return h('div', { class: 'page api-lab-page editing-input-events-page' },
      h('h2', { class: 'section-title' }, '编辑与输入事件深入 实验室'),
      h('p', { class: 'fs-sm text-secondary mb-md' },
        '深入演示编辑与输入事件全家桶：InputEvent 与 beforeinput（inputType / getTargetRanges）、contentEditable / designMode、execCommand（已废弃）、UndoManager、Selection / Range、beforeinput 拦截自定义编辑。所有 API 调用前做 typeof 能力检测，jsdom 中 Selection/Range 可能返回 null，用 try/catch 保护。'),
