...
interface DecoratorsResourceManagementPageState extends State {
  logs: any;
  capsSummary: any;
  overviewInfo: any;
  syntaxInfo: any;
...
export class DecoratorsResourceManagementPage extends Page {
  declare state: DecoratorsResourceManagementPageState;
  __disposed!: any;
  _decoratorCallLog!: any;
  declare _destroyed: boolean;
  _disposableStack!: any;
  _dynamicStyles!: any[];
  _inited!: boolean;
  _memoizeCache!: any;
  ...
  componentDidMount() {
    ...
    if (!f.decoratorSupport) this._addLog('warn', '装饰器语法不可用（TC39 Stage 3，需 Chrome flag 或 TS 5.0+/Babel 转译；与 legacy 装饰器不兼容）');
    if (!f.usingSupport) this._addLog('warn', 'using 语法不可用（Explicit Resource Management 提案，需 Chrome flag 或 TS 5.6+/Babel 转译）');
    if (!f.symbolDispose) this._addLog('warn', 'Symbol.dispose 不可用（using 依赖的 well-known symbol，Chrome 124+ 起 flag 可用）');
    if (!f.disposableStack) this._addLog('warn', 'DisposableStack 不可用（资源栈 API，Chrome 124+ 起 flag 可用）');
  ...
  componentWillUnmount() {
    ...
    if (this._memoizeCache) this._memoizeCache.clear();
  }
  ...
  _flags() {
    ...
    return {
      ...
      reflectMetadata: safe(() => typeof Reflect !== 'undefined' && typeof (Reflect as any).metadata === 'function'),
    };
  }
  ...
  _stackPush(name: any, disposeFn: any){
    ...
    const card = new Card({
      ...
      children: [
        ...
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.overviewInfo || '（点击「运行演示」查看装饰器与资源管理完整说明）')),
        h(Alert, {
          type: 'info',
  ...
  _runSyntaxDemo() {
    ...
    // 模拟装饰器执行顺序记录
    this._decoratorCallLog = [];
    ['@trace class', '@trace method', '@trace field', '@trace accessor', '@trace getter'].forEach((tag: any, i: number) => {
  ...
  _renderCard2() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['class', f.classSupport], ['decorator', f.decoratorSupport]]),
        h(Tag, { color: 'primary' }, '5 种目标'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '装饰器五种目标：类（@dec class）、方法（@dec method）、字段（@dec field = 1）、访问器（@dec accessor x = 1，accessor 关键字自动生成 getter/setter + 私有存储）、getter/setter（@dec get x(){}）。装饰器工厂 @dec(args) 先调用工厂返回真正装饰器。多装饰器叠加 @a @b @c：求值顺序 a→b→c（工厂先调用），应用顺序 c(b(a(method)))（逆序，c 最外层）。类装饰器在所有成员装饰器之后执行。陷阱：不能用于普通函数（仅类成员+类本身）、accessor 是 Stage 3 新增、应用顺序逆序易混淆。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行演示', { type: 'primary', size: 'sm', onClick: () => this._runSyntaxDemo() }),
        ),
        h('div', { class: 'drm-code' },
          h('span', { class: 'com' }, '// 五种装饰目标'),
          '\n',
          h('span', { class: 'dec' }, '@trace'),
          ' ',
          h('span', { class: 'kw' }, 'class'),
          ' Service {\n',
          '  ',
          h('span', { class: 'dec' }, '@trace'),
          ' ',
          h('span', { class: 'fn' }, 'method'),
          '() {}\n',
          '  ',
          h('span', { class: 'dec' }, '@trace'),
          ' field = 1;\n',
          ...
          h('span', { class: 'kw' }, 'get'),
          ' ',
          h('span', { class: 'fn' }, 'computed'),
          '() { return 0; }\n',
          '}',
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.syntaxInfo || '（点击「运行演示」查看装饰器语法与种类完整说明）')),
        h(Alert, {
          type: 'info',
  ...
  _renderCard3() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['decorator', f.decoratorSupport], ['Reflect.metadata', f.reflectMetadata]]),
        h(Tag, { color: 'primary' }, 'context.kind'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
        ...
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行演示', { type: 'primary', size: 'sm', onClick: () => this._runApiDemo() }),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.apiInfo || '（点击「运行演示」查看装饰器实现 API 完整说明）')),
        h(Alert, {
          type: 'info',
  ...
  _runClassMethodDemo() {
    ...
    if (!f.decoratorSupport) this._addLog('warn', '装饰器语法不可用，使用模拟调用流程演示；真实环境需 TS 5.0+/Babel 转译');
  }
  ...
  _renderCard4() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['decorator', f.decoratorSupport]]),
        h(Tag, { color: 'primary' }, '5 大实战装饰器'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '五大实战方法装饰器：@logged（日志，包装方法记录调用与返回）、@bound（自动 bind，用 addInitializer 在构造后绑定 this，解构后 this 不丢失）、@deprecated（标记废弃，调用时 console.warn）、@memoize（缓存，序列化 args 作 key）、@debounce（防抖工厂 @debounce(ms)，clearTimeout+setTimeout）。组合使用：@logged @memoize getUser(id)。陷阱：@bound 用 addInitializer 不能返回新函数、@memoize key 需稳定序列化、@debounce 改变返回值时机调用方需感知。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行 @logged 演示', { type: 'primary', size: 'sm', onClick: () => this._runClassMethodDemo() }),
          this._btn('读取信息', { size: 'sm', onClick: () => this.setState({ classMethodInfo: this._readClassMethodInfo() }) }),
        ),
        h('div', { class: 'drm-decorator-log' },
          (!this._decoratorCallLog || this._decoratorCallLog.length === 0)
            ? '（暂无调用记录，点击「运行 @logged 演示」查看模拟执行流程）'
            : this._decoratorCallLog.map((l: any) => l).join('\n'),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.classMethodInfo || '（点击「运行 @logged 演示」查看 5 大方法装饰器完整代码）')),
        h(Alert, {
          type: 'info',
  ...
  _renderCard5() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['decorator', f.decoratorSupport]]),
        h(Tag, { color: 'primary' }, 'accessor 关键字'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '字段装饰器 @field：value 是 undefined（不是初始值），返回初始值函数 init(initial) 可修改/替换初始值。accessor 关键字 + @accessor 装饰器：accessor 自动生成 getter/setter + 私有存储，装饰器收到 { get, set }，返回 { get, set } 替换自定义行为（如响应式 set 时触发更新）。与 Object.defineProperty 协同（内部用 defineProperty 应用）。addInitializer：字段/accessor 装饰器可调用，fn 在类定义完成时执行（静态初始化，类似 static block）。静态字段装饰器：ctx.static === true 标识，逻辑同实例字段。陷阱：field 的 value 是 undefined、accessor 返回 { get, set } 不能返回函数、accessor 是 Stage 3 新增。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行演示', { type: 'primary', size: 'sm', onClick: () => this._runFieldAccessorDemo() }),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.fieldAccessorInfo || '（点击「运行演示」查看字段与访问器装饰器完整代码）')),
        h(Alert, {
          type: 'info',
  ...
  _readResourceInfo() {
    const f = this._flags();
    try {
      const stackItems = this._disposableStack.map((e: any) => '  [' + e.order + '] ' + e.name + (e.disposed ? ' (已释放)' : ''));
  ...
  _renderCard6() {
    ...
    const card = new Card({
      ...
      children: [
        ...
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行演示', { type: 'primary', size: 'sm', onClick: () => this._runResourceDemo() }),
          this._btn('压栈资源', { size: 'sm', onClick: () => this._runResourcePushDemo() }),
          this._btn('LIFO 释放', { size: 'sm', danger: true, onClick: () => this._runResourceDisposeDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, 'DisposableStack 可视化（LIFO 逆序释放，栈底在上方）：'),
        h('div', { class: 'drm-stack' },
          (!this._disposableStack || this._disposableStack.length === 0)
            ? h('div', { style: { color: '#94a3b8', fontSize: '12px', textAlign: 'center', padding: '20px' } }, '栈空（点击「压栈资源」添加）')
            : this._disposableStack.map((e: any) =>
                h('div', { class: 'drm-stack-item' + (e.disposed ? ' drm-disposed' : '') },
                  h('span', {}, e.name),
                  h('span', { class: 'drm-order' }, '#' + e.order + (e.disposed ? ' 释放' : ' 持有')),
                ),
              ),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.resourceInfo || '（点击「运行演示」查看 Explicit Resource Management 完整说明）')),
        h(Alert, {
          type: 'info',
  ...
  _runUsingDemo() {
    ...
    if (!f.usingSupport) this._addLog('warn', 'using 语法不可用，使用模拟流程演示；真实环境需 Chrome flag 或 TS 5.6+/Babel 转译');
  }
  ...
  _renderCard7() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['using', f.usingSupport], ['await using', f.awaitUsingSupport]]),
        h(Tag, { color: 'success' }, '避免泄漏'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '四大 using 实战场景：数据库连接（using conn = acquireConn()，块结束自动 release 避免泄漏）、文件句柄（await using fh = await fs.open()，自动关闭）、锁（navigator.locks.request + await using，自动释放；降级用 AsyncDisposableStack）、事务 scope（using tx = transaction()，自动 commit；异常时需判断 rollback）。自动释放避免泄漏：传统 try-finally 易遗漏 finally，using 强制块结束释放，编译器保证，异常路径也释放。陷阱：事务异常时 commit vs rollback（dispose 内判断 SuppressedError 或用 DisposableStack 手动控制）。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行演示', { type: 'primary', size: 'sm', onClick: () => this._runUsingDemo() }),
        ),
        h('div', { class: 'drm-flow' },
          h('div', { class: 'step' }, h('span', { class: 'step-num' }, '1'), h('span', {}, 'using conn = acquireConn()  // 获取 DB 连接')),
          h('div', { class: 'step' }, h('span', { class: 'step-num' }, '2'), h('span', {}, 'await using fh = fs.open()  // 打开文件句柄')),
          h('div', { class: 'step' }, h('span', { class: 'step-num' }, '3'), h('span', {}, 'await using lock = navigator.locks.request()  // 获取锁')),
          h('div', { class: 'step' }, h('span', { class: 'step-num' }, '4'), h('span', {}, '} // 块结束：LIFO 释放 lock → fh → conn')),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.usingInfo || '（点击「运行演示」查看 using 资源管理 4 大场景完整代码）')),
        h(Alert, {
          type: 'info',
  ...
  _renderCard8() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['decorator', f.decoratorSupport], ['using', f.usingSupport], ['Reflect.metadata', f.reflectMetadata]]),
        h(Tag, { color: 'warning' }, '8 大陷阱'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '八大陷阱：装饰器不能用于普通函数（仅类成员，普通函数用高阶函数）；this 绑定问题（用 @bound addInitializer）；metadata 元数据提案（独立 Stage 3，Reflect.metadata/Symbol.metadata）；与 legacy 装饰器不兼容（TS 5.0 前 --experimentalDecorators，语义不同需重写）；using 必须块作用域（顶层不行，await using 需顶层 await）；异常时仍 dispose（try-finally 语义，dispose 抛异常抑制原异常 SuppressedError）；性能考虑（装饰器一次性开销，using 编译 try-finally 有微小开销，高频热点避免）；转译工具链配置（TS 5.0+ 装饰器 / TS 5.6+ using / Babel 对应插件 / Chrome flag）。最佳实践清单 12 条覆盖使用边界、this、kind 分支、field/accessor 返回、using 作用域、dispose 副作用、异常 rollback、性能、转译、迁移、元数据。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行演示', { type: 'primary', size: 'sm', onClick: () => this._runPitfallsDemo() }),
        ),
        h('table', { class: 'drm-matrix' },
          h('thead', {},
            h('tr', {}, h('th', {}, '特性'), h('th', {}, 'Chrome'), h('th', {}, 'Firefox'), h('th', {}, 'Safari'), h('th', {}, 'TS'), h('th', {}, 'Babel')),
          ),
          h('tbody', {},
            h('tr', {}, h('td', {}, 'Decorators (S3)'), h('td', { class: 'partial' }, 'flag'), h('td', { class: 'no' }, '✗'), h('td', { class: 'no' }, '✗'), h('td', { class: 'yes' }, '5.0+'), h('td', { class: 'yes' }, '插件')),
            h('tr', {}, h('td', {}, 'using/await using'), h('td', { class: 'partial' }, '124+ flag'), h('td', { class: 'no' }, '✗'), h('td', { class: 'no' }, '✗'), h('td', { class: 'yes' }, '5.6+'), h('td', { class: 'yes' }, '插件')),
            h('tr', {}, h('td', {}, 'Symbol.dispose'), h('td', { class: 'partial' }, '124+ flag'), h('td', { class: 'no' }, '✗'), h('td', { class: 'no' }, '✗'), h('td', { class: 'yes' }, '5.6+'), h('td', { class: 'yes' }, '插件')),
            h('tr', {}, h('td', {}, 'DisposableStack'), h('td', { class: 'partial' }, '124+ flag'), h('td', { class: 'no' }, '✗'), h('td', { class: 'no' }, '✗'), h('td', { class: 'yes' }, '5.6+'), h('td', { class: 'yes' }, '插件')),
            h('tr', {}, h('td', {}, 'Reflect.metadata'), h('td', { class: 'no' }, '✗ 提案'), h('td', { class: 'no' }, '✗'), h('td', { class: 'no' }, '✗'), h('td', { class: 'no' }, '✗'), h('td', { class: 'partial' }, 'polyfill')),
          ),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '600px', overflow: 'auto' } },
          h('code', {}, s.pitfallsInfo || '（点击「运行演示」查看 8 大陷阱与 12 条最佳实践完整说明）')),
        h(Alert, {
          type: 'warning',
  ...
  render() {
    const s = this.state;
    return h('div', { class: 'api-lab-page decorators-resource-management-page' },
      h('h2', { class: 'section-title' }, 'TC39 Stage 3 装饰器与资源管理完整实验室'),
      h('p', { class: 'fs-sm text-secondary mb-md' },
        '本页演示 TC39 Stage 3 前瞻性语言特性：Decorators 装饰器（历史演进 legacy→stage2→stage3、5 种装饰目标、装饰器 API (value, context)、@logged/@bound/@deprecated/@memoize/@debounce、字段与 accessor 装饰器）+ Explicit Resource Management（using/await using、Symbol.dispose/asyncDispose、DisposableStack/AsyncDisposableStack、LIFO 释放、DB连接/文件句柄/锁/事务实战）、陷阱与最佳实践。所有特性通过 typeof / in / new Function 语法探测能力检测，不可用时仅记日志（部分用模拟流程演示），绝不抛异常。jsdom/Node 通常不支持，需 Chrome flag 或 TS 5.0+/Babel 转译。'),
