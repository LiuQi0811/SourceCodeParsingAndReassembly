...
interface CSSPerformanceLayoutPageState extends State {
  logs: LogEntry[];
  capsSummary: string;
  overviewInfo: string;
  containInfo: string;
  contentVisibilityAutoInfo: string;
  contentVisibilityHiddenInfo: string;
  intrinsicSizeInfo: string;
  masonryInfo: string;
  perfMonitorInfo: string;
  pitfallsInfo: string;
  beforematchInfo: string;
}
...
export class CSSPerformanceLayoutPage extends Page {
  declare state: CSSPerformanceLayoutPageState;
  _inited: boolean = false;
  _dynamicStyles: HTMLStyleElement[] = [];
  _intersectionObservers: any = undefined as any;
  _perfObservers: any = undefined as any;
  _containMode: string = '';
  _cvAutoMode: string = '';
  _cvHiddenMode: string = '';
  _intrinsicMode: string = '';
  _masonryMode: string = '';
  _perfMode: string = '';


  // —— 初始 state ——
  initialState(): CSSPerformanceLayoutPageState {
    return {
      logs: [],
      capsSummary: '',
  ...
  componentDidMount(): void {
    ...
    if (!f.containLayout) this._addLog('warn', 'contain: layout 不可用或 jsdom 未识别（CSS Containment L1，Chrome 52+/Firefox 69+/Safari 15.4+ 支持）');
    if (!f.containPaint) this._addLog('warn', 'contain: paint 不可用或 jsdom 未识别（CSS Containment L2，Chrome 52+/Firefox 69+/Safari 15.4+）');
    if (!f.containSize) this._addLog('warn', 'contain: size 不可用或 jsdom 未识别（CSS Containment L1，Chrome 52+/Firefox 69+/Safari 15.4+）');
    if (!f.contentVisibilityAuto) this._addLog('warn', 'content-visibility: auto 不可用或 jsdom 未识别（CSS Containment L2，Chrome 85+/Firefox 125+/Safari 17+）');
    if (!f.masonry) this._addLog('warn', 'grid-template-rows: masonry 不可用或 jsdom 未识别（CSS Grid L3，Firefox 默认开启，Chrome/Safari 实验 flag）');
    if (!f.intersectionObserver) this._addLog('warn', 'IntersectionObserver 不可用（jsdom 通常不实现，现代浏览器全支持：Chrome 51+/Firefox 55+/Safari 12.1+）');
    if (!f.performanceObserver) this._addLog('warn', 'PerformanceObserver 不可用（jsdom 通常不实现，现代浏览器全支持：Chrome 52+/Firefox 57+/Safari 11+）');
    if (!f.beforematch) this._addLog('warn', 'beforematch 事件 / hidden=until-found 不可用（Chrome 102+/105+，jsdom 不识别）');
  ...
  _flags(): any {
    ...
    const supportsPV = (p: string, v: string) => this._safe(() =>
    ...
    return {
      ...
      performanceObserver: this._safe(() =>
        typeof PerformanceObserver !== 'undefined' && typeof performance !== 'undefined'),
      // 测量 layout/paint 入口
      performanceMeasureUserAgent: this._safe(() =>
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '现代 Web 性能瓶颈：DOM 规模爆炸（单页应用节点数万+）、长列表渲染卡顿（10000+ 行滚动掉帧）、布局重计算成本（任一元素尺寸变化触发全文档 reflow）、屏外内容浪费、瀑布流布局复杂。三大性能型 CSS 标准：1. CSS Containment（contain 属性）隔离元素布局/绘制/尺寸影响，让浏览器跳过 containment 边界外的 reflow/paint；2. content-visibility 跳过屏外内容渲染（auto 自动跳过/hidden 隐藏保留状态），配合 contain-intrinsic-size 长列表性能数量级提升；3. Masonry Layout（CSS Grid L3）原生瀑布流无需 JS。浏览器支持：contain Chrome 52+/Firefox 69+/Safari 15.4+；content-visibility Chrome 85+/Firefox 125+/Safari 17+；Masonry Firefox 默认开启 Chrome/Safari 实验 flag。'),
        ...
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.overviewInfo || '（点击「读取概述信息」查看三大性能型 CSS 标准全景）')),
        h(Alert, {
          type: 'info',
  ...
  _renderCard2(): Node | string {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'contain 属性全集：layout（布局隔离创建独立格式化上下文，内部布局变化不影响外部）、paint（绘制隔离裁剪到 padding 边界 + 自动 stacking context + containing block，防止子元素溢出）、size（尺寸隔离子树尺寸视为 0，浏览器跳过子树布局性能提升最大，但未指定尺寸会塌陷）、style（样式隔离计数器/引号不泄漏）、inline-size/block-size（CSS Containment L3 单轴隔离，Chrome 105+/Firefox 101+/Safari 17+）。组合值：strict = size layout paint style（最强，需显式尺寸）；content = layout paint style（不含 size 较安全，推荐不确定尺寸元素）。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ containInfo: this._readContainInfo() }) }),
          this._btn('none', { size: 'sm', onClick: () => this._setContainMode('none') }),
          this._btn('layout', { size: 'sm', disabled: !f.containLayout, onClick: () => this._setContainMode('layout') }),
          this._btn('paint', { size: 'sm', disabled: !f.containPaint, onClick: () => this._setContainMode('paint') }),
          this._btn('size', { size: 'sm', disabled: !f.containSize, onClick: () => this._setContainMode('size') }),
          this._btn('style', { size: 'sm', disabled: !f.containStyle, onClick: () => this._setContainMode('style') }),
          this._btn('strict', { size: 'sm', disabled: !f.containStrict, onClick: () => this._setContainMode('strict') }),
          this._btn('content', { size: 'sm', disabled: !f.containContent, onClick: () => this._setContainMode('content') }),
          this._btn('inline-size', { size: 'sm', disabled: !f.containInlineSize, onClick: () => this._setContainMode('inline-size') }),
        ),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, 'contain 演示元素（当前 contain: ' + this._containMode + '）：'),
        h('div', { class: 'contain-box dynamic' },
          'contain: ' + this._containMode + '\n（jsdom 不做真实布局，但 DOM 与样式正确）\n真实浏览器可观察隔离效果',
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.containInfo || '（点击按钮切换 contain 取值查看完整说明）')),
        h(Alert, {
          type: 'warning',
  ...
  _readContentVisibilityAutoInfo(): string {
    const f = this._flags();
    try {
  ...
  _renderCard3(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['content-visibility:auto', f.contentVisibilityAuto]]),
        h(Tag, { color: 'primary' }, 'CSS Containment L2'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'content-visibility: auto 让浏览器自动跳过屏外内容渲染（布局/绘制/合成），屏外元素保留 DOM 但不参与渲染，滚动到可见时才渲染。长列表（10000+ 行）性能数量级提升（首屏渲染时间减少 50-90%）。与 IntersectionObserver 等价但浏览器自动处理无 JS 开销。配合 contain-intrinsic-size 兜底尺寸防滚动条跳变（屏外元素尺寸为 0 导致跳变，兜底尺寸维持稳定）。浏览器记录最后已知尺寸（auto 200px 100px）更精确。浏览器支持：Chrome 85+/Firefox 125+/Safari 17+。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ contentVisibilityAutoInfo: this._readContentVisibilityAutoInfo() }) }),
          this._btn('auto 开', { type: 'primary', size: 'sm', disabled: !f.contentVisibilityAuto, onClick: () => this._setCvAutoMode('on') }),
          this._btn('visible 关', { size: 'sm', onClick: () => this._setCvAutoMode('off') }),
        ),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '长列表演示（content-visibility: auto 跳过屏外，滚动可见时渲染）：'),
        h('div', { class: 'cv-list' },
          Array.from({ length: 20 }, (_, i) =>
            h('div', { class: 'cv-item ' + (this._cvAutoMode === 'on' ? 'cv-auto' : 'cv-visible') },
              `第 ${i + 1} 行 · content-visibility: ${this._cvAutoMode === 'on' ? 'auto' : 'visible'} · contain-intrinsic-size: 0 44px`),
          ),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.contentVisibilityAutoInfo || '（点击按钮切换 content-visibility: auto 查看说明）')),
        h(Alert, {
          type: 'info',
  ...
  _readContentVisibilityHiddenInfo(): string {
    const f = this._flags();
    try {
  ...
  _renderCard4(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['content-visibility:hidden', f.contentVisibilityHidden]]),
        h(Tag, { color: 'primary' }, 'vs display:none / visibility:hidden'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'content-visibility: hidden 完全隐藏元素（不渲染/不绘制/不参与布局）但保留渲染状态（滚动位置/表单输入/CSS 动画进度），切换回 visible 时恢复状态无需重建。vs display: none（不保留状态，子元素状态丢失，切换需重建渲染树）；vs visibility: hidden（仍参与布局占空间，子元素可单独 visible，CSS 动画继续运行）。content-visibility: hidden 暂停 CSS 动画/过渡节省 CPU，切换回时从暂停位置继续。配合 IntersectionObserver 主动控制标签页/折叠面板。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ contentVisibilityHiddenInfo: this._readContentVisibilityHiddenInfo() }) }),
          this._btn('hidden 开', { type: 'primary', size: 'sm', disabled: !f.contentVisibilityHidden, onClick: () => this._setCvHiddenMode('on') }),
          this._btn('visible 关', { size: 'sm', onClick: () => this._setCvHiddenMode('off') }),
        ),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '三种隐藏方式对比（content-visibility:hidden / display:none / visibility:hidden）：'),
        h('div', { class: 'cv-hidden-stage' },
          h('div', { class: 'cv-hidden-box dynamic' },
            'content-visibility: ' + (this._cvHiddenMode === 'on' ? 'hidden' : 'visible') + '\n保留渲染状态',
          ),
          h('div', { class: 'cv-hidden-box cv-display-none' },
            'display: none\n不保留状态（不可见）',
          ),
          h('div', { class: 'cv-hidden-box cv-visibility-hidden' },
            'visibility: hidden\n保留布局占空间',
          ),
          h('div', { class: 'cv-hidden-box' },
            '正常显示（对照）',
          ),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.contentVisibilityHiddenInfo || '（点击按钮切换 content-visibility: hidden 查看说明）')),
        h(Alert, {
          type: 'info',
  ...
  _setIntrinsicMode(mode: any) {
    ...
    this._injectStyle('css-intrinsic-dynamic', ((map as any)[(mode as any)]) || map.fixed);
  ...
  _renderCard5(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['contain-intrinsic-size', f.containIntrinsicSize]]),
        h(Tag, { color: 'primary' }, 'content-visibility 必备'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
        ...
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ intrinsicSizeInfo: this._readIntrinsicSizeInfo() }) }),
          this._btn('固定 200×100', { size: 'sm', disabled: !f.containIntrinsicSize, onClick: () => this._setIntrinsicMode('fixed') }),
          this._btn('auto 200×100', { size: 'sm', disabled: !f.containIntrinsicSizeAuto, onClick: () => this._setIntrinsicMode('auto') }),
          this._btn('单轴 200px', { size: 'sm', disabled: !f.containIntrinsicSize, onClick: () => this._setIntrinsicMode('single') }),
          this._btn('none', { size: 'sm', onClick: () => this._setIntrinsicMode('none') }),
        ),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, 'contain-intrinsic-size 演示（当前模式: ' + this._intrinsicMode + '）：'),
        h('div', { class: 'intrinsic-stage' },
          h('div', { class: 'intrinsic-box dynamic' },
            'contain-intrinsic-size: ' + this._intrinsicMode + '\n（jsdom 不做真实布局，真实浏览器可见尺寸占位）',
          ),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.intrinsicSizeInfo || '（点击按钮切换 contain-intrinsic-size 模式查看说明）')),
        h(Alert, {
          type: 'info',
  ...
  _setMasonryMode(mode: any) {
    ...
    this._injectStyle('css-masonry-dynamic', ((map as any)[(mode as any)]) || map.rows);
  ...
  _renderCard6(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['masonry', f.masonry]]),
        h(Tag, { color: 'primary' }, 'CSS Grid L3'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'Masonry Layout（CSS Grid L3）原生支持 Pinterest 风格瀑布流：grid-template-rows: masonry（行方向瀑布流，元素按列排列自动填充最短列）/ grid-template-columns: masonry（列方向）。masonry-auto-flow: pack（默认，紧凑减少空隙）/ next（有序空隙多）。align-tracks/justify-tracks 多轨道对齐。vs 传统方案：JS 计算最短列（性能差复杂）/ CSS columns（顺序按列填充非最短列）。浏览器支持：Firefox 默认开启（自 70+），Chrome/Safari 实验 flag。生产需降级：CSS columns + break-inside: avoid 或 JS 计算最短列，@supports 检测 + 降级。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ masonryInfo: this._readMasonryInfo() }) }),
          this._btn('rows: masonry', { size: 'sm', disabled: !f.masonry, onClick: () => this._setMasonryMode('rows') }),
          this._btn('columns: masonry', { size: 'sm', disabled: !f.masonryCols, onClick: () => this._setMasonryMode('columns') }),
          this._btn('pack', { size: 'sm', disabled: !f.masonry, onClick: () => this._setMasonryMode('pack') }),
          this._btn('next', { size: 'sm', disabled: !f.masonry, onClick: () => this._setMasonryMode('next') }),
        ),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, 'Masonry 瀑布流演示（Firefox 默认支持，Chrome/Safari 需实验 flag）：'),
        h('div', { class: 'masonry-stage' },
          h('div', { class: 'masonry-grid' },
            h('div', { class: 'masonry-item short' }, '短卡片 1'),
            h('div', { class: 'masonry-item tall' }, '高卡片 1\n\n\n\n内容较多' + this._masonryMode),
            h('div', { class: 'masonry-item medium' }, '中卡片 1\n\n内容'),
            h('div', { class: 'masonry-item short' }, '短卡片 2'),
            h('div', { class: 'masonry-item tall' }, '高卡片 2\n\n\n\n\n\n内容更多'),
            h('div', { class: 'masonry-item medium' }, '中卡片 2\n\n内容'),
            h('div', { class: 'masonry-item short' }, '短卡片 3'),
            h('div', { class: 'masonry-item tall' }, '高卡片 3\n\n\n\n内容'),
            h('div', { class: 'masonry-item short' }, '短卡片 4'),
          ),
        ),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '降级方案：CSS columns（顺序按列填充，非最短列）：'),
        h('div', { class: 'masonry-stage' },
          h('div', { class: 'masonry-columns' },
            h('div', { class: 'masonry-item short' }, '短卡片 1'),
            h('div', { class: 'masonry-item tall' }, '高卡片 1\n\n\n\n内容较多'),
            h('div', { class: 'masonry-item medium' }, '中卡片 1\n\n内容'),
            h('div', { class: 'masonry-item short' }, '短卡片 2'),
        ...
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.masonryInfo || '（点击按钮切换 Masonry 模式查看说明）')),
        h(Alert, {
          type: 'warning',
  ...
  _runPerfMonitorDemo(): void {
    ...
    try {
      ...
      for (const type of types) {
        try { po.observe({ type, buffered: true }); } catch { /* 类型不支持，跳过 */ }
      }
      ...
      if (f.performanceMeasureUserAgent) {
        ...
        if (boxes) boxes.forEach(b => { void b.offsetWidth; });
  ...
  _renderCard7(): Node | string {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '性能监控与验证：Performance API 测量 Layout/Paint 耗时（performance.mark/measure）；PerformanceObserver 监听 layout-shift（CLS 布局抖动）/ paint（FP/FCP/LCP）/ longtask（长任务 >50ms 阻塞）；Lighthouse 10+ 检测长列表建议 content-visibility: auto（"Defer rendering offscreen content" 审计，Performance 分数 +5~15）；Render Blocking 监控（longtask）；Layout Shift 跳变检测（CLS 应 <0.1，过高调整 contain-intrinsic-size）；DevTools Performance 面板查看 skipped subtrees（content-visibility: auto 跳过子树标记 skipped）。jsdom 通常不实现 PerformanceObserver，真实浏览器可完整监控。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行性能监控演示', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runPerfMonitorDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '性能监控输出（PerformanceObserver 监听 layout-shift/paint/longtask）：'),
        h('div', { class: 'perf-output perf-output-area' },
        ...
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.perfMonitorInfo || '（点击「运行性能监控演示」查看完整说明）')),
        h(Alert, {
          type: 'info',
  ...
  _renderCard8(): Node | string {
    ...
    const card = new Card({
      ...
      children: [
        ...
        h('div', { class: 'perf-stage' },
          h('div', { class: 'fs-sm' }, '长列表演示（content-visibility: auto 跳过屏外，1000-10000 行推荐）：'),
          h('div', { class: 'cv-list' },
            Array.from({ length: 10 }, (_, i) =>
              h('div', { class: 'cv-item cv-auto' },
                `第 ${i + 1} 行 · content-visibility: auto · contain-intrinsic-size: 0 44px · 适用 1000-10000 行`),
            ),
          ),
          h('div', { class: 'fs-sm', style: { marginTop: '8px' } }, '↑ content-visibility: auto 零 JS 原生支持；100000+ 行用虚拟列表（react-window）'),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '600px', overflow: 'auto' } },
          h('code', {}, s.pitfallsInfo || '（点击按钮查看 6 大实战场景与 10 大陷阱完整说明）')),
        h(Alert, {
          type: 'warning',
  ...
  _runBeforematchDemo(): void {
    const f = this._flags();
    const info =
      'beforematch 事件 / hidden="until-found" 完整说明：\n\n' +
    ...
    this._addLog('pitfalls',
      `beforematch 演示：beforematch=${f.beforematch}, hiddenUntilFound=${f.hiddenUntilFound}`);
  }
  ...
  _renderCard9(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['beforematch', f.beforematch], ['hidden=until-found', f.hiddenUntilFound]]),
        h(Tag, { color: 'primary' }, 'Chrome 102+'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'hidden="until-found" 是 HTML hidden 属性的新状态：内容默认不渲染，但浏览器的 find-in-page（Ctrl+F）匹配时会触发 beforematch 事件并自动显示内容。典型场景：长文档 FAQ 默认折叠答案、搜索时自动展开；渐进式披露与懒加载。beforematch 事件仅在 document.documentElement 上支持（Chrome 102+），hidden="until-found" 在 Chrome 105+ 实现，Firefox/Safari 尚未支持。jsdom 不实现这两个特性，能力检测返回 false。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行 beforematch 演示', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runBeforematchDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, 'beforematch / hidden=until-found 用法与对比：'),
        h('div', { class: 'perf-stage' },
          h('div', { class: 'fs-sm', style: { marginBottom: '6px' } },
            '示例：hidden="until-found" 内容（真实浏览器中用 Ctrl+F 搜索"答案"可触发显示）：'),
          h('div', { class: 'contain-box', style: { background: '#ede9fe', borderColor: '#8b5cf6' } },
            h('div', { hidden: 'until-found' },
              '这段内容默认隐藏（hidden="until-found"），find-in-page 匹配"答案"时自动显示并触发 beforematch 事件。'),
          ),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '560px', overflow: 'auto' } },
          h('code', {}, s.beforematchInfo || '（点击按钮查看 beforematch / hidden=until-found 完整说明）')),
        h(Alert, {
          type: 'warning',
  ...
  _renderLogPanel(): Node | string {
    ...
    return h('div', { class: 'log-panel' },
      ...
      s.logs.length === 0
        ...
        : s.logs.slice().reverse().map((log: LogEntry) => h('div', { class: 'log-panel__line' },
  ...
  render(): Node {
    const s = this.state;
    return h('div', { class: 'api-lab-page css-performance-layout-page' },
      h('h2', { class: 'section-title' }, 'CSS Containment + content-visibility + Masonry 深度实验室'),
      h('p', { class: 'fs-sm text-secondary mb-md' },
        '本页演示三大性能型 CSS 标准：contain 属性全集（layout/paint/size/strict/inline-size）、content-visibility:auto（跳过屏外渲染长列表性能数量级提升）、content-visibility:hidden（完全隐藏保留渲染状态）、contain-intrinsic-size（兜底尺寸防滚动条跳变）、Masonry Layout（CSS Grid L3 原生 Pinterest 瀑布流）、性能监控（Performance API/Lighthouse/DevTools）、6 大实战场景与 10 大陷阱。所有特性通过 typeof / CSS.supports / PerformanceObserver 能力检测，不可用时仅记日志，绝不抛异常。'),
