...
export interface CanvasDeepPageProps extends Props {}

export interface CanvasDeepPageState extends State {}

export class CanvasDeepPage extends Page {
  declare props: CanvasDeepPageProps;
  declare state: CanvasDeepPageState;
  _inited: boolean = false;
  _rafId: any = null;
  _starPath: any = null;
  // —— 初始 state ——
  initialState(): CanvasDeepPageState {
    return {
      // 共享事件日志（所有卡片写入同一面板）
      logs: [],
  ...
  _caps(): any {
    ...
    const caps: any = { canvas: false, path2d: false, globalCompositeOperation: false };
    for (const m of METHODS) ((caps as any)[(m as any)]) = false;
    try {
      ...
      for (const m of METHODS) ((caps as any)[(m as any)]) = typeof ((ctx as any)[(m as any)]) === 'function';
  ...
  _makeOffscreen(size: any = 100) {
    try {
      if (typeof document === 'undefined' ||
        typeof document.createElement !== 'function') {
        return null as any;
      }
      const off = (document.createElement('canvas') as any);
      off.width = size;
      off.height = size;
  ...
  _processPixels(name: any, fn: any) {
    const caps = this._caps();
    if (!caps.getImageData || !caps.putImageData) {
  ...
  _applyGrayscale() {
    this._processPixels('灰度化', (d: any, i: any) => {
  ...
  _applyInvert() {
    this._processPixels('反色', (d: any, i: any) => {
      d[i] = 255 - d[i];
      d[i + 1] = 255 - d[i + 1];
      d[i + 2] = 255 - d[i + 2];
    });
  ...
  _applyBrightness() {
    const delta = 40;
    this._processPixels(`亮度调整 +${delta}`, (d: any, i: any) => {
      d[i] = d[i] + delta;
      d[i + 1] = d[i + 1] + delta;
      d[i + 2] = d[i + 2] + delta;
    });
  ...
  _applyTransformRotate() {
    ...
    try {
      ...
      this._addLog('transform', `transform(${(cos as any).toFixed(3)}, ${(sin as any).toFixed(3)}, ${(-sin).toFixed(3)}, ${(cos as any).toFixed(3)}, ${(e as any).toFixed(1)}, ${(f as any).toFixed(1)}) 围绕 (${cx},${cy}) 旋转 45°`);
