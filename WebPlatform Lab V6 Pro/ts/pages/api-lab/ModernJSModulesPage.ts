...
interface ModernJSModulesPageState extends State {
  logs: any;
  capsSummary: any;
  promiseTryInfo: any;
  importMapsInfo: any;
...
export class ModernJSModulesPage extends Page {
  declare state: ModernJSModulesPageState;
  _inited!: boolean;
  _injectedStyles!: any;
  ...
  componentDidMount() {
    ...
    if (!caps.promiseTry) this._addLog('warn', 'Promise.try 不可用（Chrome 128+/Firefox 129+/Safari 18+），演示仅记日志');
    if (!caps.getHTML) this._addLog('warn', 'Element.getHTML 不可用（Chrome 125+/Firefox 128+），演示仅记日志');
    if (!caps.shadowRoot) this._addLog('warn', 'attachShadow 不可用，序列化演示仅记日志');
    if (!caps.importMaps) this._addLog('warn', 'Import Maps 检测在 jsdom 中不准（HTMLScriptElement.prototype.type 仅作存在性判断），需真实浏览器验证');
  ...
  _caps() {
    ...
    try { promiseTry = typeof Promise !== 'undefined' && typeof Promise.try === 'function'; } catch { promiseTry = false; }
    let importMaps = false;
    try { importMaps = typeof HTMLScriptElement !== 'undefined' && 'type' in HTMLScriptElement.prototype; } catch { importMaps = false; }
    let getHTML = false;
    try { getHTML = typeof Element !== 'undefined' && typeof Element.prototype.getHTML === 'function'; } catch { getHTML = false; }
    let shadowRoot = false;
    try { shadowRoot = typeof Element !== 'undefined' && typeof Element.prototype.attachShadow === 'function'; } catch { shadowRoot = false; }
    // dynamic import 是语法而非全局函数，无法用 typeof 直接检测
    let dynamicImport = true;
    try { new Function('return typeof import'); } catch { dynamicImport = false; }
    ...
    Promise.try(() => { throw new Error('同步抛错示例'); })
      ...
      .finally(() => {
        ...
        Promise.try(() => 42)
          ...
          .finally(() => {
            // 情况 3：异步返回 Promise
            Promise.try(() => new Promise((resolve: any) => setTimeout(() => resolve('async'), 0)))
              .then((v: any) => {
  ...
  _renderCard1() {
    ...
    return h(Card, {
      ...
      h('p', { class: 'fs-sm text-secondary' },
        'Promise.try(fn) 把同步或会抛错的函数包装为 Promise，正确捕获 fn 内同步抛错（vs new Promise(resolve=>resolve(fn())) 旧写法某些情况变成未捕获异常）；是 Lodash _.attempt / Bluebird Promise.try 的官方标准化。'),
      h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' },
        this._btn('运行 Promise.try', { type: 'primary', size: 'sm', onClick: () => this._runPromiseTry() }),
      ),
      s.promiseTryInfo ? h('pre', { class: 'code-block mt-md' }, s.promiseTryInfo) : null as any,
      h('pre', { class: 'code-block mt-md' },
`// 旧写法：fn 内同步抛错可能变成未捕获异常
...
// 情况3：异步返回 Promise → then 收到 unwrapped 值`),
  ...
  _resolveImportMap() {
    ...
    const resolve = (specifier: any) => {
      try {
        // 完全匹配
        if (((importMap.imports as any)[(specifier as any)])) return ((importMap.imports as any)[(specifier as any)]);
        // 前缀映射（带尾斜杠）
        for (const key of (Object as any).keys(importMap.imports)) {
          if (key.endsWith('/') && specifier.startsWith(key)) {
            return ((importMap.imports as any)[(key as any)]) + specifier.slice(key.length);
          }
        }
    ...
    const lines = cases.map((sp: any) => sp + '  →  ' + resolve(sp));
  ...
  _renderCard2() {
    ...
    return h(Card, {
      ...
      h('p', { class: 'fs-sm text-secondary' },
        '<script type="importmap"> JSON（imports/scopes）；bare module specifier（如 import "react"）解析到 URL；可跳过打包工具直接用 ES Modules；微前端/CDN 依赖映射事实标准。jsdom 不支持 importmap 检测，仅记日志说明。'),
      h('div', { class: 'mj-json-block' },
`{
  "imports": {
    "react": "https://esm.sh/react@18.3.1",
    "react-dom": "https://esm.sh/react-dom@18.3.1",
  ...
  "scopes": {
    "/vendor/": { "react": "https://esm.sh/react@17.0.2" }
      ...
      h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' },
        this._btn('解析演示', { type: 'primary', size: 'sm', onClick: () => this._resolveImportMap() }),
      ),
      s.importMapsInfo ? h('pre', { class: 'code-block mt-md' }, s.importMapsInfo) : null as any,
      h('pre', { class: 'code-block mt-md' },
...
import React from "react";   // bare specifier → 解析到 esm.sh
  ...
  _renderCard3() {
    ...
    return h(Card, {
      ...
      h('p', { class: 'fs-sm text-secondary' },
        'import x from "./data.json" with { type: "json" }；import mod from "./mod.wasm" with { type: "webassembly" }；with 关键字替代旧 assert；声明模块类型供浏览器安全加载。能力检测：无法直接检测（语法层面），用 try/catch 动态 import 测试或记日志说明。'),
      h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' },
        this._btn('说明 Import Attributes', { type: 'primary', size: 'sm', onClick: () => this._testImportAttrs() }),
      ),
      s.importAttrsInfo ? h('p', { class: 'fs-sm mt-sm text-secondary' }, s.importAttrsInfo) : null as any,
      h('pre', { class: 'code-block mt-md' },
`// 新语法 with（推荐）
...
// - 浏览器仅对未声明类型的模块拒绝执行（安全）`),
  ...
  _serializeShadowDOM() {
    ...
    const host = (this.$('#mj-shadow-host') as any);
    if (!host) { this._addLog('warn', '未找到 shadow host 元素'); return; }
    try {
      // 清理旧 shadow
      if (host.shadowRoot) { /* 已有 open shadow，无需重建 */ }
      else { host.attachShadow({ mode: 'open' } as any); }
      ...
      let outer = '';
      try { outer = host.getHTML({ serializableShadowRoots: true }); } catch (e: any) { this._addLog('warn', 'getHTML 调用失败：' + (e && e.message)); }
  ...
  _renderCard4() {
    ...
    return h(Card, {
      ...
      h('p', { class: 'fs-sm text-secondary' },
        'ShadowRoot.getHTML({ serializableShadowRoots, shadowRoots }) 把 Shadow DOM 输出为带 <template shadowrootmode="open"> 的 Declarative Shadow DOM 字符串；Element.getHTML() 同；是 SSR/组件缓存/复制粘贴的关键缺口（innerHTML 序列化丢失 Shadow 树）。Chrome 125+/Firefox 128+。'),
      h('div', { id: 'mj-shadow-host', class: 'mj-shadow-host' },
        h('p', { class: 'fs-sm' }, 'shadow host 容器：点击下方按钮将在此 attachShadow 并序列化对比。'),
      ),
      h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' },
        this._btn('序列化 Shadow DOM', { type: 'primary', size: 'sm', onClick: () => this._serializeShadowDOM() }),
        h(Tag, { color: caps.shadowRoot ? 'success' : 'error' }, caps.shadowRoot ? 'attachShadow 已支持' : 'attachShadow 未支持'),
      ),
      s.getHtmlInfo ? h('pre', { class: 'code-block mt-md' }, s.getHtmlInfo) : null as any,
      h('pre', { class: 'code-block mt-md' },
`const host = document.querySelector('#x');
...
// <div id="x"><template shadowrootmode="open"><p>shadow</p></template></div>`),
  ...
  _renderCard5() {
    ...
    return h(Card, {
      ...
      h('p', { class: 'fs-sm text-secondary' },
        '现代 JS 模块加载生态各有分工：Import Maps 做 specifier 解析、Import Attributes 做类型声明、dynamic import 做按需加载、Module Workers 让 Worker 也能用 ESM、Top-level await 让模块初始化可异步、JSON/CSS modules 静态导入非 JS 资源。'),
      h('table', { class: 'mj-matrix' },
        h('thead', {},
          h('tr', {},
            h('th', {}, '特性'),
            h('th', {}, '作用'),
      ...
      h('pre', { class: 'code-block mt-md' },
`// 综合：importmap + import attributes + dynamic import + top-level await
<script type="importmap">
{ "imports": { "react": "https://esm.sh/react@18" } }
</script>
<script type="module">
