...
// 安全创建带命名空间的 SVG 元素
function svgEl(tag: any) {
  return document.createElementNS(SVG_NS, tag);
}

export interface SVGDeepPageProps extends Props {}

export interface SVGDeepPageState extends State {}

export class SVGDeepPage extends Page {
  declare props: SVGDeepPageProps;
  declare state: SVGDeepPageState;
  _clipDef: any = null;
  _currentFilter: any = null;
  _filterDefs: any = null;
  _inited: boolean = false;
  _maskDef: any = null;
  _rafId: any = null;
  _smilAnim: any = null;
  _viewBoxScale: any = null;
  // —— 初始 state ——
  initialState(): SVGDeepPageState {
    return {
      logs: [],                // 共享事件日志
  ...
  _samplePathPoints() {
    ...
    try {
      ...
      markersOld.forEach((m: any) => m.remove());
      const points: any[] = [];
      for (let i = 0; i < N; i++) {
        const d = (total * i) / (N - 1);
        const pt = path.getPointAtLength(d);
        points.push(`#${i + 1} @${(d as any).toFixed(1)} → (${(pt.x as any).toFixed(1)}, ${(pt.y as any).toFixed(1)})`);
      ...
      children: [
        ...
        h('div', { class: 'table-wrap', style: { overflowX: 'auto' } },
          h('table', { class: 'data-table', style: { width: '100%', borderCollapse: 'collapse', fontSize: '12px' } },
            ...
            h('tbody', {},
              ...PATH_COMMANDS_TABLE.map((r: any) => h('tr', {},
