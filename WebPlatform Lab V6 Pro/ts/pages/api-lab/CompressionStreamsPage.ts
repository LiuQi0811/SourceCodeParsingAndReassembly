...
export interface CompressionStreamPageProps extends Props {}

export interface CompressionStreamPageState extends State {}

export class CompressionStreamPage extends Page {
  declare props: CompressionStreamPageProps;
  declare state: CompressionStreamPageState;
  _abortControllers: any[] | null = null;
  _dynamicStyles: any[] | null = null;
  _inited: boolean = false;
  // —— 初始 state ——
  initialState(): CompressionStreamPageState {
    return {
      logs: [],
      capsSummary: '',
  ...
  componentDidMount(): void {
    ...
    if (!f.compressionStream) this._addLog('warn', 'CompressionStream 不可用（Chrome 80+/Firefox 113+/Safari 16.4+ 全部稳定，jsdom 较新版本支持）');
    if (!f.decompressionStream) this._addLog('warn', 'DecompressionStream 不可用（与 CompressionStream 同期稳定）');
    if (f.compressionStream && !f.gzipSupported) this._addLog('warn', 'gzip 格式不被支持（罕见，仅极老浏览器）');
    if (f.compressionStream && !f.deflateRawSupported) this._addLog('warn', 'deflate-raw 格式不被支持（Chrome 103+ 支持，Safari 16.4+ 支持）');
    if (!f.readableStream) this._addLog('warn', 'ReadableStream 不可用（pipeThrough 演示跳过）');
  ...
  _flags() {
    ...
    return {
      ...
      gzipSupported: safe(() => { try { new CompressionStream('gzip'); return true; } catch { return false; } }),
      deflateSupported: safe(() => { try { new CompressionStream('deflate'); return true; } catch { return false; } }),
      deflateRawSupported: safe(() => { try { new CompressionStream('deflate-raw'); return true; } catch { return false; } }),
  ...
  // —— 真实压缩辅助（jsdom 较新版本可用）——
  async _compress(text: any, format: any = 'gzip') {
    ...
    for (;;) {
      ...
      if (done) break;
      chunks.push(value);
      totalBytes += (value as any).byteLength;
    }
  ...
  async _decompress(bytes: any, format: any = 'gzip') {
  ...
  _bytesToHex(bytes: any, max: any = 64) {
    const arr: any[] = [];
    ...
    const card = new Card({
      ...
      children: [
        ...
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, `// CompressionStream + DecompressionStream
...
// ... 同样 writable.getWriter() / readable.getReader()`)),
        ...
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.overviewInfo || '（点击按钮查看 Compression Streams API 概述完整说明）')),
      ],
    });
  ...
  _runCompressionDemo() {
    ...
    if (!f.compressionStream || !f.gzipSupported) {
      ...
      const formats = ['gzip', 'deflate', 'deflate-raw'].filter((fmt: any) => {
        try { new CompressionStream(fmt); return true; } catch { return false; }
      });
      Promise.all(formats.map(async (fmt: any) => {
        const compressed = await this._compress(sample, fmt);
        const ratio = ((compressed as any).byteLength / originalBytes * 100).toFixed(1);
        return `${fmt}: 原始 ${originalBytes}B → 压缩 ${(compressed as any).byteLength}B（压缩率 ${ratio}%）\n  hex 前 32 字节: ${this._bytesToHex(compressed, 32)}`;
      })).then((lines: any) => {
  ...
  _renderCard2() {
    ...
    const card = new Card({
      ...
      children: [
        ...
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行压缩演示', { type: 'primary', size: 'sm', disabled: !f.compressionStream, onClick: () => this._runCompressionDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, `// CompressionStream 构造（三种格式）
...
await fetch('/upload', {
  ...
  body: cs.readable,  // ReadableStream
        ...
        h('div', { class: `cs-output ${s.compressResult.startsWith('压缩失败') ? 'cs-output--warn' : (s.compressResult ? 'cs-output--success' : '')}` },
          s.compressResult || '（点击按钮尝试真实压缩 700B+ 文本，对比三种格式压缩率）'),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.compressionInfo || '（点击按钮查看 CompressionStream 构造完整用法）')),
      ],
    });
    this.registerChild(card);
    return card.render();
  ...
  _runDecompressionDemo() {
    ...
    if (!f.decompressionStream || !f.gzipSupported || !f.readableStream) {
      ...
      Promise.all(samples.map(async (text: any) => {
  ...
  _renderCard3() {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'new DecompressionStream(format) 与 CompressionStream 对称，writable 接收压缩数据，readable 输出解压后数据。format 必须与压缩时一致（gzip 数据用 deflate 解压抛 EncodingError）。截断或损坏的压缩流抛异常需 try/catch。与 fetch response.body 协同：服务端返回 Content-Type: application/gzip 时手动解压（Content-Encoding: gzip 浏览器自动处理）。TextDecoder stream:true + 末尾 decode() flush 不可少。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行解压演示', { type: 'primary', size: 'sm', disabled: !f.decompressionStream, onClick: () => this._runDecompressionDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, `// 解压流程
...
try {
  ...
  console.error('解压失败:', err.name); // EncodingError
        ...
        h('div', { class: `cs-output ${s.decompressResult.startsWith('解压失败') ? 'cs-output--warn' : (s.decompressResult ? 'cs-output--success' : '')}` },
          s.decompressResult || '（点击按钮尝试真实压缩+解压往返验证 3 个样本）'),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.decompressionInfo || '（点击按钮查看 DecompressionStream 构造完整用法）')),
      ],
    });
    this.registerChild(card);
    return card.render();
  ...
  _renderCard4() {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'fetch(url).body.pipeThrough(new DecompressionStream("gzip")) 解压响应（Content-Type: application/gzip 需手动，Content-Encoding: gzip 浏览器自动）。上传压缩：CompressionStream.readable 直接作为 fetch body（必设 duplex: "half"）。流式压缩大文件：file.stream() → pipeTo(cs.writable)，边读边压缩内存恒定。pipeThrough 链式：解压 → TextDecoderStream → JSON。Service Worker 缓存压缩资源。ByteLengthQueuingStrategy 自定义背压。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行 fetch + Streams 演示', { type: 'primary', size: 'sm', disabled: !f.compressionStream, onClick: () => this._runFetchStreamsDemo() }),
        ),
        // 流式压缩流程图
        h('div', { class: 'cs-flow' },
          h('span', { class: 'cs-flow-node' }, 'file.stream()'),
          h('span', { class: 'cs-flow-arrow' }, '→'),
          h('span', { class: 'cs-flow-node cs-flow-node--stream' }, 'CompressionStream.writable'),
          h('span', { class: 'cs-flow-arrow' }, '→'),
          h('span', { class: 'cs-flow-node cs-flow-node--stream' }, 'CompressionStream.readable'),
          h('span', { class: 'cs-flow-arrow' }, '→'),
          h('span', { class: 'cs-flow-node' }, 'fetch body'),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, `// 解压 fetch 响应
...
const reader = fetch(url)
  ...
  .getReader();`)),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.fetchStreamsInfo || '（点击按钮查看与 fetch + Streams 协同完整用法）')),
      ],
    });
    this.registerChild(card);
    return card.render();
  ...
  _renderCard5() {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'FormData + CompressionStream：file.stream() pipeTo cs.writable，cs.readable 作为 FormData 字段或 fetch body（duplex:"half"）。大日志文件上传：边产生边写入 writer.write()，结束时 writer.close()。分片压缩：1MB 一片，await writer.ready 背压。Service Worker 协同：拦截响应压缩后缓存，读取时解压。上传进度：用 ReadableStream 包装 tee 计数（无法精确监控压缩后字节）。大文件压缩建议用 Web Worker 避免阻塞主线程。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行上传压缩演示', { type: 'primary', size: 'sm', disabled: !f.compressionStream, onClick: () => this._runUploadDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, `// FormData + 压缩
...
self.addEventListener('fetch', (event: any) => {
  // 拦截 + 压缩 + 缓存
        ...
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.uploadInfo || '（点击按钮查看上传大文件压缩完整方案）')),
      ],
    });
    this.registerChild(card);
    return card.render();
  ...
  _renderCard6() {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'Content-Encoding: gzip 浏览器自动处理（response.json() 直接可用，无需手动 DecompressionStream）；Content-Type: application/gzip 需手动 pipeThrough(new DecompressionStream)。手动解压场景：.gz 文件下载、自定义协议、WebSocket 消息、IndexedDB 压缩存储、用户上传预览。Web Worker 并行压缩避免阻塞主线程。ByteLengthQueuingStrategy 提供背压（highWaterMark 默认 0 需显式设）。下载进度只能监控下载层（无法监控解压后字节）。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行下载解压演示', { type: 'primary', size: 'sm', disabled: !f.decompressionStream, onClick: () => this._runDownloadDemo() }),
        ),
        // 下载解压流程图
        h('div', { class: 'cs-flow' },
          h('span', { class: 'cs-flow-node' }, 'fetch(url)'),
          h('span', { class: 'cs-flow-arrow' }, '→'),
          h('span', { class: 'cs-flow-node cs-flow-node--stream' }, 'resp.body'),
          h('span', { class: 'cs-flow-arrow' }, '→'),
          h('span', { class: 'cs-flow-node cs-flow-node--stream' }, 'DecompressionStream'),
          h('span', { class: 'cs-flow-arrow' }, '→'),
          h('span', { class: 'cs-flow-node cs-flow-node--stream' }, 'TextDecoderStream'),
          h('span', { class: 'cs-flow-arrow' }, '→'),
          h('span', { class: 'cs-flow-node' }, 'string'),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, `// Content-Encoding: gzip 浏览器自动处理
const resp = await fetch('/api/data');  // 服务端 gzip
...
const strategy = new ByteLengthQueuingStrategy({ highWaterMark: 1024 * 1024 });`)),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.downloadInfo || '（点击按钮查看下载解压流完整方案）')),
      ],
    });
    this.registerChild(card);
    return card.render();
  ...
  _runPerformanceDemo() {
    ...
    if (!f.compressionStream || !f.readableStream) {
      ...
      const samples = {
        ...
        '随机字符（低压缩率）': Array.from({ length: 500 }, () =>
          String.fromCharCode(33 + Math.floor(Math.random() * 90))).join(''),
        'JSON 数据': JSON.stringify({ items: Array.from({ length: 50 }, (_: any, i: any) => ({ id: i, name: `item-${i}` })) }),
      };
      const formats = ['gzip', 'deflate', 'deflate-raw'].filter((fmt: any) => {
        try { new CompressionStream(fmt); return true; } catch { return false; }
      });
      Promise.all((Object as any).entries(samples).map(async ([name, text]: any) => {
        ...
        for (const fmt of formats) {
          ...
          const ratio = ((compressed as any).byteLength / original * 100).toFixed(1);
          lines.push(`  ${fmt.padEnd(12)} → ${(compressed as any).byteLength}B（压缩率 ${ratio}%，耗时 ${(t1 - t0).toFixed(1)}ms）`);
        }
        return lines.join('\n');
      })).then((results: any) => {
        this.setState({ perfResult: results.join('\n\n') });
        this._addLog('info', `性能对比完成（${(Object as any).keys(samples).length} 类文本 × ${formats.length} 格式）`);
      }).catch((err: any) => {
  ...
  _renderCard7() {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'CompressionStream 原生 C++ 实现速度 ~500MB/s（pako JS ~200MB/s，fflate WASM ~300MB/s），内存流式恒定 ~1MB（pako 全量 2x）。压缩级别不可配置（固定 level 6，介于 pako 6-7）。不同文本压缩率：重复文本 ~1%、JSON ~15-25%、英文 ~30-40%、中文 ~40-50%、随机/已压缩数据 ~100%（无效）。gzip 通用性最好（浏览器自动处理）；deflate zlib 兼容；deflate-raw 体积最小但需自管理边界。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行性能对比演示', { type: 'primary', size: 'sm', disabled: !f.compressionStream, onClick: () => this._runPerformanceDemo() }),
        ),
        // 三种实现对比矩阵
        h('div', { class: 'cs-matrix' },
          h('div', { class: 'cs-matrix-cell' },
            h('div', { class: 'cs-matrix-title' }, 'CompressionStream'),
            h('div', {}, '原生 C++ · ~500MB/s · 0KB · level 6 固定'),
          ),
          h('div', { class: 'cs-matrix-cell' },
            h('div', { class: 'cs-matrix-title' }, 'pako'),
            h('div', {}, 'JS · ~200MB/s · 45KB · level 1-9 可配'),
          ),
          h('div', { class: 'cs-matrix-cell' },
            h('div', { class: 'cs-matrix-title' }, 'fflate'),
            h('div', {}, 'JS · ~250MB/s · 8KB · level 1-9 可配'),
          ),
          h('div', { class: 'cs-matrix-cell' },
            h('div', { class: 'cs-matrix-title' }, 'fflate-wasm'),
            h('div', {}, 'WASM · ~300MB/s · 30KB · level 1-9 可配'),
          ),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
...
// pako 内存 = 原始 + 压缩（2x，大文件 OOM 风险）`)),
        ...
        h('div', { class: `cs-output ${s.perfResult.startsWith('性能测试失败') ? 'cs-output--warn' : (s.perfResult ? 'cs-output--success' : '')}` },
          s.perfResult || '（点击按钮尝试真实压缩 5 类文本 × 3 种格式，对比压缩率与耗时）'),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.performanceInfo || '（点击按钮查看性能与压缩率对比完整说明）')),
      ],
    });
    this.registerChild(card);
    return card.render();
  ...
  _renderCard8() {
    ...
    const card = new Card({
      ...
      children: [
        ...
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行陷阱与最佳实践演示', { type: 'primary', size: 'sm', disabled: !f.compressionStream, onClick: () => this._runPitfallsDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, `// 陷阱 4：小数据不压缩
...
await fetch(url, {
  ...
  duplex: 'half',  // 必需
        ...
        h('pre', { class: 'code-block', style: { maxHeight: '600px', overflow: 'auto' } },
          h('code', {}, s.pitfallsInfo || '（点击按钮查看 8 大陷阱与 12 条最佳实践完整说明）')),
        h(Alert, {
          type: 'warning',
