...
interface CSSConditionalLevel5PageState extends State {
  logs: LogEntry[];
  capsSummary: string;
  whenInfo: string;
  customMediaInfo: string;
  ifFuncInfo: string;
  customSelInfo: string;
  applyInfo: string;
  timelineInfo: string;
}
...
export class CSSConditionalLevel5Page extends Page {
  declare state: CSSConditionalLevel5PageState;
  _inited: boolean = false;
  _dynamicStyles: HTMLStyleElement[] = [];

  // —— 初始 state ——
  initialState(): CSSConditionalLevel5PageState {
    return {
      logs: [],
      capsSummary: '',
      whenInfo: '',
  ...
  _flags(): CondFlags {
    ...
    const supportsPV = (p: string, v: string): boolean => {
      try { return hasCSS && typeof CSS.supports === 'function' && CSS.supports(p, v); }
      catch { return false; }
    };
    const supportsCond = (cond: string): boolean => {
