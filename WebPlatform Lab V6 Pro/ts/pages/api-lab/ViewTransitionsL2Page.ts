...
interface ViewTransitionsL2PageCaps {
  l1: boolean;
  pageswap: boolean;
  pagereveal: boolean;
  navigation: boolean;
...
export interface ViewTransitionsL2PageProps extends Props {}

export interface ViewTransitionsL2PageState extends State {
  logs: LogEntry[];
  capsSummary: string;
  overviewInfo: string;
  pageswapInfo: string;
  pagerevealInfo: string;
  typesCssInfo: string;
  sameOriginInfo: string;
  navApiInfo: string;
}

export class ViewTransitionsL2Page extends Page {
  declare props: ViewTransitionsL2PageProps;
  declare state: ViewTransitionsL2PageState;

  _inited: boolean = false;
  declare _destroyed: boolean;
  _pageswapHandler!: any;
  _pagerevealHandler!: any;
  _transitionstartHandler!: any;
  _navigateHandler!: any;


  // —— 初始 state ——
  initialState(): ViewTransitionsL2PageState {
    return {
      logs: [],
      capsSummary: '',
  ...
  _caps(): ViewTransitionsL2PageCaps {
    const hasL1 = typeof document !== 'undefined' && typeof document.startViewTransition === 'function';
    const hasPageswap = typeof window !== 'undefined' && 'pageswap' in window;
    const hasPagereveal = typeof window !== 'undefined' && 'pagereveal' in window;
    const hasNavigation = typeof navigation !== 'undefined';
  ...
  _registerPageswap(): void {
    ...
    try {
      ...
      this._pageswapHandler = (event: any) => {
        try {
  ...
  _registerPagereveal(): void {
    ...
    try {
      ...
      this._pagerevealHandler = (event: any) => {
        try {
          ...
          if (hasVT && event.viewTransition.ready) {
            event.viewTransition.ready.then(() => this._addLog('pagereveal', 'viewTransition.ready resolve（过渡动画开始）'))
              .catch((e: any) => this._addLog('warn', `viewTransition.ready reject：${e.message}`));
          ...
          if (hasVT && event.viewTransition.finished) {
            event.viewTransition.finished.then(() => this._addLog('pagereveal', 'viewTransition.finished resolve（过渡动画完成）'))
              .catch((e: any) => this._addLog('warn', `viewTransition.finished reject：${e.message}`));
          }
      ...
      this._transitionstartHandler = (event: any) => {
        try {
  ...
  _registerNavigate(): void {
    ...
    try {
      ...
      this._navigateHandler = (event: any) => {
        try {
