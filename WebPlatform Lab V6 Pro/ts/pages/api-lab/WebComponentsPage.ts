...
// ---------------------------------------------------------------------------
const _hasCE: boolean = typeof customElements !== 'undefined';

/** mdn-counter 自定义元素实例接口 */
interface MdnCounterElement extends HTMLElement {
  increment(): void;
  decrement(): void;
}

/** mdn-shadow-box 自定义元素实例接口 */
interface MdnShadowBoxElement extends HTMLElement {}

/** mdn-lifecycle 自定义元素实例接口 */
interface MdnLifecycleElement extends HTMLElement {}
...
if (_hasCE && !customElements.get('mdn-counter')) {
  class MdnCounter extends HTMLElement {
    ...
    static get observedAttributes(): string[] { return ['count']; }
    attributeChangedCallback(name: string, _oldVal: string, newVal: string): void {
      if (name === 'count') { this._count = Number(newVal); this.render(); }
    }
...
let wcShadowMode: 'open' | 'closed' = 'open';
...
if (_hasCE && !customElements.get('mdn-shadow-box')) {
  class MdnShadowBox extends HTMLElement {
    constructor() {
      super();
      const shadow: ShadowRoot = this.attachShadow({ mode: wcShadowMode });
      shadow.innerHTML = `
        <style>
...
if (_hasCE && !customElements.get('mdn-lifecycle')) {
  class MdnLifecycle extends HTMLElement {
    ...
    static get observedAttributes(): string[] { return ['data-status']; }
    ...
    _render(): void {
      const s: string = this.getAttribute('data-status') || '(未设置)';
...
// 用于填充 <slot> 的示例内容（克隆模板时轮换使用）
interface TemplateSlotSample {
  title: string;
  body: string;
}
const TEMPLATE_SLOT_SAMPLES: TemplateSlotSample[] = [
...
const CARD_TEMPLATE_HTML: string = `
  <style>
...
export interface WebComponentsPageProps extends Props {}

interface WcLogEntry {
  time: string;
  text: string;
  tag: string;
}
...
export interface WebComponentsPageState extends State {
  counterValue: number;        // mdn-counter 当前计数（用于跨重渲染恢复）
  counterRemoved: boolean;      // 计数器是否被移除
  counterLog: WcLogEntry[];     // 自定义元素生命周期回调记录
  shadowMode: 'open' | 'closed'; // Shadow DOM 当前 mode
  shadowLog: WcLogEntry[];      // shadowRoot 访问记录
  lifecycleAttached: boolean;   // 生命周期元素是否已挂载
  lifecycleStatus: string;      // 生命周期元素当前 data-status
  lifecycleLogs: WcLogEntry[];  // 生命周期回调记录
  templateCount: number;         // 已克隆的模板实例数
  templateInstances: TemplateInstance[]; // 已克隆实例数据（用于跨重渲染恢复）
}

export class WebComponentsPage extends Page {
  declare props: WebComponentsPageProps;
  declare state: WebComponentsPageState;
  _counterEl: MdnCounterElement | null = null;
  _shadowEl: MdnShadowBoxElement | null = null;
  _lifecycleEl: MdnLifecycleElement | null = null;
  _adoptIframe: HTMLIFrameElement | null = null;

  initialState(): WebComponentsPageState {
    return {
  ...
  _logEntry(tag: string, text: string): WcLogEntry {
    return { time: formatTime(), text, tag };
  }

  // —— 1) Custom Elements：mdn-counter 恢复 ——

  _restoreCounter(): void {
    this._counterEl = null;
    if (this.state.counterRemoved) return;
    const container = (this.$('.wc-counter-container') as any);
    if (!container) return;
    const el = document.createElement('mdn-counter') as MdnCounterElement;
    el.setAttribute('count', String(this.state.counterValue));
    container.appendChild(el);
    this._counterEl = el;
  }

  _counterStep(delta: number): void {
    ...
    if (delta > 0) {
      if (typeof this._counterEl.increment === 'function') this._counterEl.increment();
      else this._counterEl.setAttribute('count', String(Number(this._counterEl.getAttribute('count') || '0') + 1));
    } else {
      if (typeof this._counterEl.decrement === 'function') this._counterEl.decrement();
      else this._counterEl.setAttribute('count', String(Number(this._counterEl.getAttribute('count') || '0') - 1));
    }
  ...
  _restoreShadow(): void {
    ...
    const container = (this.$('.wc-shadow-container') as any);
    if (!container) return;
    container.appendChild(this._buildShadowBox());
  }

  _buildShadowBox(): MdnShadowBoxElement {
    const box = document.createElement('mdn-shadow-box') as MdnShadowBoxElement;
  ...
  _shadowToggle(): void {
    const next: 'open' | 'closed' = this.state.shadowMode === 'open' ? 'closed' : 'open';
    wcShadowMode = next;
  ...
  _cloneTemplate(title: string, body: string): DocumentFragment | null {
    const tpl = document.getElementById('mdn-card-template') as (HTMLTemplateElement | null);
    if (!tpl || !tpl.content) return null;
    // 关键：通过 template.content.cloneNode(true) 复制模板内容
    const content: DocumentFragment = tpl.content.cloneNode(true) as DocumentFragment;
    // 用实际内容替换命名插槽
    const titleSlot = content.querySelector('slot[name="title"]');
    if (titleSlot) titleSlot.replaceWith(h('span', {}, title));
    // 替换默认插槽（无名 slot）
    const defaultSlot = content.querySelector('slot:not([name])');
    if (defaultSlot) defaultSlot.replaceWith(h('span', {}, body));
    return content; // DocumentFragment：appendChild 后其子节点被移入目标容器
  }

  _restoreTemplates(): void {
    const container = (this.$('.wc-template-container') as any);
    if (!container) return;
  ...
  _restoreLifecycle(): void {
    this._lifecycleEl = null;
    if (!this.state.lifecycleAttached) return;
    const container = (this.$('.wc-lifecycle-container') as any);
    if (!container) return;
    const el = document.createElement('mdn-lifecycle') as MdnLifecycleElement;
  ...
  _lifecycleAdd(): void {
    const container = (this.$('.wc-lifecycle-container') as any);
    if (!container) return;
    const el = document.createElement('mdn-lifecycle') as MdnLifecycleElement; // constructor
  ...
  _lifecycleAdopt(): void {
    ...
    if (!this._adoptIframe || !this._adoptIframe.contentDocument) {
      ...
      const host = (this.$('.wc-adopt-container') as any);
      if (host) host.appendChild(iframe); else document.body.appendChild(iframe);
    ...
    const adopted = iframeDoc.adoptNode(this._lifecycleEl) as MdnLifecycleElement;
  ...
  _lifecycleNoop(msg: string): void {
    this.setState({
      lifecycleLogs: [...this.state.lifecycleLogs,
    ...
    return [
      ...
      h(Card, {
        ...
        h('p', { class: 'fs-sm text-secondary' },
          '自定义元素 ', h('code', {}, '<mdn-counter>'),
          ' 实现了 observedAttributes / attributeChangedCallback / disconnectedCallback。+1 / -1 通过 setAttribute 改变 count 属性，元素内部自动 re-render。'),
        h('div', { class: 'flex gap-sm' },
          this._btn('+1', { size: 'sm', type: 'primary', onClick: () => this._counterStep(1) }),
          this._btn('-1', { size: 'sm', onClick: () => this._counterStep(-1) }),
          this._btn('移除元素', { size: 'sm', danger: true, onClick: () => this._counterRemove() }),
          this._btn('重新添加', { size: 'sm', onClick: () => this._counterReadd() }),
        ),
        h('div', { class: 'wc-counter-container mt-sm' }),
        h('p', { class: 'fs-sm text-tertiary mt-sm' },
          '调用 element.remove() 会触发 disconnectedCallback；customElements.define 不可逆，组件卸载时不撤销定义。'),
        h('div', { class: 'log-panel mt-sm', style: { maxHeight: '140px' } },
          ...this.state.counterLog.map((l: WcLogEntry) => h('div', { class: 'log-panel__line' },
            h('span', { class: 'log-panel__time' }, l.time),
            h('span', { class: `log-panel__tag log-panel__tag--${l.tag}` }, 'ce'),
            h('span', {}, l.text),
          )),
        ),
      ),
      ...
      h(Card, {
        ...
        h('p', { class: 'fs-sm text-secondary' },
          '自定义元素 ', h('code', {}, '<mdn-shadow-box>'),
          ' 在 constructor 中 attachShadow，内部样式（:host / .inner / ::slotted）与外部隔离。'),
        h('div', { class: 'flex gap-sm' },
          this._btn('切换 mode: open/closed', { size: 'sm', type: 'primary', onClick: () => this._shadowToggle() }),
          this._btn('查看 element.shadowRoot', { size: 'sm', onClick: () => this._shadowInspect() }),
        ),
        // 外部样式：尝试修改 .inner 颜色，但因 Shadow DOM 隔离无法穿透
        h('style', {}, '/* 外部规则：试图改 .inner 颜色，但无法穿透 Shadow DOM */\n.wc-shadow-section .inner { color: #fa541c !important; border: 2px solid #fa541c; }'),
        h('div', { class: 'wc-shadow-section' },
          h('div', { class: 'wc-shadow-container' }),
          h('p', { class: 'fs-sm text-tertiary mt-sm' },
            '上方虚线框内蓝色 .inner 文本不受外部 .inner 规则影响；通过 slot 分发的 span 被 ::slotted 染为橙色。'),
        ),
        h('p', { class: 'fs-sm text-tertiary' },
          'mode=closed 时 element.shadowRoot 返回 null，外部 JS 无法访问 Shadow 树。'),
        h('div', { class: 'log-panel mt-sm', style: { maxHeight: '140px' } },
          ...this.state.shadowLog.map((l: WcLogEntry) => h('div', { class: 'log-panel__line' },
            h('span', { class: 'log-panel__time' }, l.time),
            h('span', { class: `log-panel__tag log-panel__tag--${l.tag}` }, 'shadow'),
            h('span', {}, l.text),
          )),
        ),
      ),
      ...
      h(Card, {
        ...
        h('p', { class: 'fs-sm text-secondary' },
          '下方 ', h('code', {}, '<template>'),
          ' 是惰性的，其内容不在渲染树中。点击“实例化模板”会通过 ',
          h('code', {}, 'template.content.cloneNode(true)'),
          ' 克隆内容，并用实际文本替换其中的 ', h('code', {}, '<slot>'), '。'),
        h('div', { class: 'flex gap-sm' },
          this._btn('实例化模板', { size: 'sm', type: 'primary', onClick: () => this._templateInstantiate() }),
          this._btn('清空', { size: 'sm', danger: true, onClick: () => this._templateClear() }),
        ),
        // 模板元素本身：用 html prop 填充 innerHTML（template 的 .content 会被填充）
        h('template', { id: 'mdn-card-template', html: CARD_TEMPLATE_HTML }),
        h('div', { class: 'wc-template-container mt-sm' }),
        h('p', { class: 'fs-sm text-tertiary mt-sm' },
          '同一个 ', h('code', {}, '<template>'),
          ' 可被多次克隆；每次克隆可填充不同的 slot 内容。'),
      ),

      // —— 4. 生命周期回调 ——
      h(Card, {
        title: '4. 生命周期回调（Lifecycle）',
        extra: h(Tag, { color: this.state.lifecycleAttached ? 'success' : 'default' },
          this.state.lifecycleAttached ? '已挂载' : '未挂载'),
      },
        h('p', { class: 'fs-sm text-secondary' },
          '自定义元素 ', h('code', {}, '<mdn-lifecycle>'),
          ' 完整实现 constructor / connected / disconnected / adopted / attributeChanged 五个回调。'),
        h('div', { class: 'flex gap-sm' },
          this._btn('添加元素', { size: 'sm', type: 'primary', onClick: () => this._lifecycleAdd() }),
          this._btn('移除元素', { size: 'sm', onClick: () => this._lifecycleRemove() }),
          this._btn('修改属性', { size: 'sm', onClick: () => this._lifecycleAttr() }),
          this._btn('adopt 到 iframe', { size: 'sm', onClick: () => this._lifecycleAdopt() }),
        ),
        h('div', { class: 'wc-lifecycle-container mt-sm' }),
        // 隐藏容器，用于承载 adopt 演示所需的 iframe（新文档）
        h('div', { class: 'wc-adopt-container', style: { display: 'none' } }),
        h('p', { class: 'fs-sm text-tertiary mt-sm' },
          h('code', {}, 'adoptedCallback'),
          ' 通过 Document.adoptNode 跨文档移动元素触发（此处移入一个隐藏 iframe 的文档）。'),
        h('div', { class: 'log-panel mt-sm', style: { maxHeight: '160px' } },
          ...this.state.lifecycleLogs.map((l: WcLogEntry) => h('div', { class: 'log-panel__line' },
            h('span', { class: 'log-panel__time' }, l.time),
            h('span', { class: `log-panel__tag log-panel__tag--${l.tag}` }, 'lc'),
            h('span', {}, l.text),
          )),
        ),
      ),
