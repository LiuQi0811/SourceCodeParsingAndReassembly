...
export interface DragDropDeepPageProps extends Props {}

export interface DragDropDeepPageState extends State {}

export class DragDropDeepPage extends Page {
  declare props: DragDropDeepPageProps;
  declare state: DragDropDeepPageState;
  _dragImageEl: any = null;
  _effectsDt: any = null;
  _fileDt: any = null;
  _formatsDt: any = null;
  _inited: boolean = false;
  _itemsDt: any = null;
  _lifecycleDt: any = null;
  // —— 初始 state ——
  initialState(): DragDropDeepPageState {
    return {
      logs: [],
      capsSummary: '',
      // Card 1：拖拽生命周期
  ...
  _makeDragEvent(type: any, dt: any) {
    let ev = null;
  ...
  _dispatchLifecycleEvent(target: any, type: any) {
    ...
    try {
      ...
      const handler = (e: any) => {
        ...
        if (e.type === 'dragstart' && e.dataTransfer) {
          ...
          catch (err: any) { extra = ' · setData 失败'; }
        }
        if (e.type === 'dragover' || e.type === 'dragenter' || e.type === 'drop') {
          try { e.preventDefault(); extra += ' · preventDefault()（允许放置）'; } catch (err: any) { /* noop */ }
        }
        if (e.type === 'drop' && e.dataTransfer) {
          let got = '';
          try { got = e.dataTransfer.getData('text/plain'); } catch (err: any) { got = '(读取失败)'; }
      ...
      const el = target === 'source' ? (this.$('.dd-source') as any) : (this.$('.dd-drop') as any);
  ...
  _runFullLifecycle() {
    ...
    try { this._lifecycleDt = new DataTransfer(); } catch (e: any) { this._lifecycleDt = null; }
  ...
  _demoEffects() {
    ...
    try {
      ...
      for (const de of DROP_EFFECT_VALUES) {
        try {
          dt.dropEffect = (de as any);
          const got = dt.dropEffect;
          const ok = got === de ? '✓' : `✗(实际=${got})`;
          lines.push(`  dropEffect = "${de}"  → 读取 = "${got}"  ${ok}  光标：${((cursorHint as any)[(de as any)])}`);
        } catch (err: any) {
  ...
  async _demoFileDrop() {
    ...
    try {
      ...
      if (items && typeof items.add === 'function') {
        try { items.add(file1); added++; } catch (e: any) { lines.push(`items.add(note.txt) 失败：${e.message}`); }
        try { items.add(file2); added++; } catch (e: any) { lines.push(`items.add(pic.png) 失败：${e.message}`); }
      } else {
