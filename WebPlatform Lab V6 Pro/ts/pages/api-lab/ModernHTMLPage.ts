...
interface ModernHTMLPageState extends State {
  logs: any;
  capsSummary: any;
  untilFoundState: any;
  accordionInfo: any;
  inertState: any;
  lazyInfo: any;
  popoverHintState: any;
  dialogInfo: any;
}

export class ModernHTMLPage extends Page {
  declare state: ModernHTMLPageState;
  _beforematchAttached!: any;
  _inited!: boolean;
  _injectedStyles!: any;
  _lazyToggle!: any;
  ...
  componentDidMount() {
    ...
    if (!caps.untilFound) this._addLog('warn', 'hidden="until-found" 不可用（Chrome 102+），演示仅记日志');
    if (!caps.detailsName) this._addLog('warn', '<details name> 互斥手风琴不可用（Chrome 120+），演示仅记日志');
    if (!caps.inert) this._addLog('warn', 'inert 属性不可用（全主流 2022+ 已稳定），演示仅记日志');
    if (!caps.popoverHint) this._addLog('warn', 'popover="hint" 不可用（Chrome 114+），演示仅记日志');
  ...
  componentWillUnmount() {
    if (Array.isArray(this._injectedStyles)) {
      this._injectedStyles.forEach((el: any) => el?.remove());
      this._injectedStyles = [];
    }
  }
  ...
  _caps() {
    ...
    } catch { untilFound = false; }
    let beforematch = false;
    try { beforematch = typeof window !== 'undefined' && ('onbeforematch' in window || (typeof document !== 'undefined' && document.body && 'onbeforematch' in document.body)); } catch { beforematch = false; }
    ...
    } catch { detailsName = false; }
    let inert = false;
    try { inert = typeof HTMLElement !== 'undefined' && 'inert' in HTMLElement.prototype; } catch { inert = false; }
    let loadingLazyImg = false, loadingLazyIframe = false;
    try { loadingLazyImg = typeof HTMLImageElement !== 'undefined' && 'loading' in HTMLImageElement.prototype; } catch { /* noop */ }
    try { loadingLazyIframe = typeof HTMLIFrameElement !== 'undefined' && 'loading' in HTMLIFrameElement.prototype; } catch { /* noop */ }
    let fetchpriority = false;
    try { fetchpriority = typeof HTMLImageElement !== 'undefined' && 'fetchPriority' in HTMLImageElement.prototype; } catch { fetchpriority = false; }
    let popover = false, popoverHint = false;
    try { popover = typeof HTMLElement !== 'undefined' && typeof HTMLElement.prototype.showPopover === 'function'; } catch { popover = false; }
    try {
      if (popover) {
        const d = document.createElement('div');
    ...
    } catch { popoverHint = false; }
    let search = false;
    try { search = typeof document !== 'undefined' && document.createElement('search') instanceof HTMLElement; } catch { search = false; }
    let dialog = false, showModal = false;
    try { dialog = typeof HTMLDialogElement !== 'undefined'; } catch { dialog = false; }
    try { showModal = dialog && typeof HTMLDialogElement.prototype.showModal === 'function'; } catch { showModal = false; }
    return {
  ...
  _toggleUntilFound() {
    const el = (this.$('#mh-until-found') as any);
    if (!el) { this._addLog('warn', '未找到演示元素'); return; }
    try {
  ...
  _attachBeforematch() {
    ...
    if (this._beforematchAttached) { this._addLog('info', 'beforematch 监听已存在'); return; }
  ...
  _renderCard1() {
    ...
    return h(Card, {
      ...
      h('p', { class: 'fs-sm text-secondary' },
        'hidden="until-found"：内容默认隐藏，但浏览器查找（Ctrl+F）命中时自动展开并滚动定位（SEO 友好的渐进披露）；' +
        'beforematch 事件在展开前触发，可做埋点/懒加载。vs hidden（彻底隐藏不可搜索）vs display:none。'),
      h('div', { class: 'mh-demo-box' },
        h('div', {
          ...
          h('p', {}, '这是 hidden="until-found" 区域。设置后用 Ctrl+F 搜索 "稀缺知识" 即可触发展开。'),
          h('p', {}, '稀缺知识：beforematch 事件是 2022 年新增的渐进披露原语。'),
        ),
      ),
      h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' },
        this._btn('切换 hidden=until-found', { type: 'primary', size: 'sm', onClick: () => this._toggleUntilFound() }),
        this._btn('说明 beforematch', { type: 'default', size: 'sm', onClick: () => this._attachBeforematch() }),
      ),
      s.untilFoundState ? h('p', { class: 'fs-sm mt-sm text-secondary' }, s.untilFoundState) : null as any,
...
// vs hidden（不可搜索） vs display:none（不占空间）`),
  ...
  _renderCard2() {
    ...
    const onToggle = (e: any) => {
      ...
      const grp = (this.$('#mh-details-group') as any);
    ...
    const mk = (summary: any, body: any) => h('details', { name: 'faq', ontoggle: onToggle },
      h('summary', {}, summary),
      h('p', { class: 'fs-sm' }, body),
    );
    return h(Card, {
      ...
      h('p', { class: 'fs-sm text-secondary' },
        '<details name="grp">：同 name 的 details 同一时间只展开一个，纯 HTML 无需 JS（Chrome 120+）；' +
        'toggle 事件 + details.open 属性；vs 手写 JS 互斥逻辑。'),
      h('div', { id: 'mh-details-group', class: 'mh-details-group' },
        mk('FAQ 1 · 什么是互斥手风琴？', '同 name 的 details 同时只展开一个，展开新的会自动关闭同组其他已展开的。'),
        mk('FAQ 2 · 浏览器支持？', 'Chrome 120+/Edge 120+ 已稳定；Safari 17.2+；Firefox 暂不支持。'),
        mk('FAQ 3 · 与 JS 方案对比？', '纯 HTML 声明式、可 SSR、无 JS 依赖、键盘可访问；JS 方案需手写状态管理。'),
      ),
      s.accordionInfo ? h('p', { class: 'fs-sm mt-sm text-secondary' }, s.accordionInfo) : null as any,
      h('pre', { class: 'code-block mt-md' },
...
// 同 name 同时仅 1 个 open，纯 HTML`),
  ...
  _toggleInert() {
    const pane = (this.$('#mh-inert-pane') as any);
    if (!pane) { this._addLog('warn', '未找到 inert 演示面板'); return; }
    if (!this._caps().inert) {
  ...
  _renderCard3() {
    ...
    return h(Card, {
      ...
      h('p', { class: 'fs-sm text-secondary' },
        'inert：一属性完成块级禁用——不可聚焦/编辑/点击、被屏幕阅读器跳过、不参与 Tab 序列。' +
        '模态对话框遮罩、抽屉导航、加载中表单的标准做法。' +
        'vs pointer-events:none + aria-hidden + tabindex=-1 组合。'),
      h('div', { class: 'mh-inert-stage' },
        h('div', { id: 'mh-inert-pane', class: 'mh-inert-pane' },
          h('p', { class: 'fs-sm' }, '可被 inert 的面板：'),
          h('p', {}, h('input', { placeholder: '输入框', class: 'input' })),
          h('p', {}, h('a', { href: '#', onclick: (e: any) => e.preventDefault() }, '一个链接')),
          h('p', {}, this._btn('按钮', { type: 'default', size: 'sm', onClick: () => this._addLog('info', '按钮被点击（若 inert 则无法点击）') })),
        ),
      ),
      h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' },
        this._btn('切换 inert', { type: 'primary', size: 'sm', onClick: () => this._toggleInert() }),
      ),
      s.inertState ? h('p', { class: 'fs-sm mt-sm text-secondary' }, s.inertState) : null as any,
      h('pre', { class: 'code-block mt-md' },
`<div inert>...整个区块禁用...</div>
...
// 模态遮罩：<div class="backdrop" inert>背景内容</div> + <dialog>`),
  ...
  _toggleLazyLoad() {
    ...
    try {
      const imgs = (this.$('#mh-lazy-grid img') as unknown as any[]);
      imgs.forEach((img: any) => img.setAttribute('loading', mode));
  ...
  _renderCard4() {
    ...
    return h(Card, {
      ...
      h('p', { class: 'fs-sm text-secondary' },
        'loading="lazy"：img/iframe 接近视口才加载（Chrome 121+ iframe 稳定）；' +
        'fetchpriority=high|low|auto 精细控制资源抢占顺序（LCP 图片用 high）。' +
        'vs IntersectionObserver 手写懒加载。'),
      h('div', { id: 'mh-lazy-grid', class: 'mh-lazy-grid' },
        [1, 2, 3, 4].map((i: any) =>
          h('img', {
      ...
      h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' },
        this._btn('切换 loading 模式', { type: 'primary', size: 'sm', onClick: () => this._toggleLazyLoad() }),
        h(Tag, { color: caps.fetchpriority ? 'success' : 'error' }, caps.fetchpriority ? 'fetchpriority ✓' : 'fetchpriority ✗'),
      ),
      s.lazyInfo ? h('p', { class: 'fs-sm mt-sm text-secondary' }, s.lazyInfo) : null as any,
      h('pre', { class: 'code-block mt-md' },
`<img loading="lazy" src="...">        // 接近视口才加载
...
<img fetchpriority="low" src="...">   // 非关键低优先级`),
  ...
  _toggleHint() {
    ...
    const el = (this.$('#mh-hint') as any);
    if (!el) { this._addLog('warn', '未找到 hint 元素'); return; }
    try {
      if (typeof el.togglePopover === 'function') {
        el.togglePopover();
        let isOpen = false;
        try { isOpen = el.matches(':popover-open'); } catch { isOpen = el.hasAttribute('open'); }
  ...
  _renderCard5() {
    ...
    return h(Card, {
      ...
      h('p', { class: 'fs-sm text-secondary' },
        'popover="hint"：轻量提示模式，点击外部即关闭，不进顶层栈（vs popover="auto" 进栈且 Esc 关闭，vs "manual" 需手动关闭）。' +
        '适合 tooltip/简短提示。配合 commandfor 声明式触发。'),
      h('div', { class: 'mh-demo-box' },
        h('div', { id: 'mh-hint', popover: 'hint', class: 'mh-hint' },
          '这是一个 hint 提示：点击外部任意位置即可关闭。'),
      ),
      h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' },
        this._btn('togglePopover()', { type: 'primary', size: 'sm', onClick: () => this._toggleHint() }),
        h(Tag, { color: 'default' }, 'vs auto/manual'),
      ),
      s.popoverHintState ? h('p', { class: 'fs-sm mt-sm text-secondary' }, s.popoverHintState) : null as any,
      h('pre', { class: 'code-block mt-md' },
`<div popover="hint">轻量提示</div>
...
<button commandfor="id" command="toggle-popover">触发</button>`),
  ...
  _showDialog() {
    const dlg = (this.$('#mh-dialog') as any);
    if (!dlg) { this._addLog('warn', '未找到 dialog 元素'); return; }
  ...
  _renderCard6() {
    ...
    return h(Card, {
      ...
      h('p', { class: 'fs-sm text-secondary' },
        '<search>：搜索区语义元素（Chrome 118+），提升可访问性；' +
        '<dialog>：原生模态框，showModal() 模态+::backdrop+焦点陷阱+Esc 关闭，show() 非模态，' +
        'close(returnValue) + close 事件 + returnValue。'),
      h('div', { class: 'mh-demo-box' },
        h('search', {},
          h('p', { class: 'fs-sm' }, '<search> 语义容器：'),
          h('input', { class: 'input', placeholder: '搜索...', type: 'search' }),
        ),
      ),
      h('dialog', {
        ...
        h('p', {}, '原生 dialog 模态框：showModal() 提供 ::backdrop + 焦点陷阱 + Esc 关闭。'),
        h('p', { class: 'fs-sm text-secondary' }, '点击下方按钮或按 Esc 关闭。'),
        h('div', { class: 'flex gap-sm mt-md' },
          this._btn('关闭(returnValue=ok)', { type: 'primary', size: 'sm', onClick: () => { try { (this.$('#mh-dialog') as any)?.close('ok'); } catch { /* noop */ } } }),
        ),
      ),
      h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' },
        this._btn('showModal() / close()', { type: 'primary', size: 'sm', onClick: () => this._showDialog() }),
        h(Tag, { color: caps.search ? 'success' : 'error' }, caps.search ? '<search> ✓' : '<search> ✗'),
      ),
      s.dialogInfo ? h('p', { class: 'fs-sm mt-sm text-secondary' }, s.dialogInfo) : null as any,
      h('pre', { class: 'code-block mt-md' },
