...
export interface FetchLaterAPIPageProps extends Props {}

export interface FetchLaterAPIPageState extends State {}

export class FetchLaterAPIPage extends Page {
  declare props: FetchLaterAPIPageProps;
  declare state: FetchLaterAPIPageState;
  _abortControllers: Set<any> | null = null;
  _fetchLaterResults: any[] | null = null;
  _inited: boolean = false;
  _pagehideHandler: any = null;
  _telemetryQueue: any = null;
  _timer: ReturnType<typeof setTimeout> | null = null;
  _visibilityHandler: any = null;
  initialState(): FetchLaterAPIPageState {
    return {
      logs: [],
      capsSummary: '',
  ...
  componentDidMount(): void {
    ...
    this._abortControllers = new Set<any>();   // 所有活跃 AbortController（在途 fetch 中止）
    ...
    if (!caps.fetchLater) this._addLog('warn', 'fetchLater 不可用（Chrome 123+ 实验性，需开启实验标志；jsdom 无此 API）');
    if (!caps.sendBeacon) this._addLog('warn', 'navigator.sendBeacon 不可用（jsdom 通常无此方法，需真实浏览器）');
    if (!caps.scheduling) this._addLog('info', 'navigator.scheduling.isInputPending 不可用（仅 Chromium 系浏览器）');
  }

  componentWillUnmount(): void {
    // 中止所有进行中的 AbortController（在途 fetch）
    if (this._abortControllers) {
      this._abortControllers.forEach((c: any) => {
        try { if (c && c.signal && !c.signal.aborted) c.abort('page-unmount'); } catch { /* noop */ }
      });
      try { this._abortControllers.clear(); } catch { /* noop */ }
    }
    if (this._visibilityHandler && typeof document !== 'undefined') {
      try { document.removeEventListener('visibilitychange', this._visibilityHandler); } catch { /* noop */ }
    }
    this._visibilityHandler = null;
    if (this._pagehideHandler && typeof window !== 'undefined') {
      try { window.removeEventListener('pagehide', this._pagehideHandler); } catch { /* noop */ }
    }
    this._pagehideHandler = null;
    // dismiss 所有 fetchLater 结果（取消延迟请求）
    if (this._fetchLaterResults) {
      for (const r of this._fetchLaterResults) {
        try { if (r && typeof r.dismiss === 'function') r.dismiss(); } catch { /* noop */ }
      }
      try { this._fetchLaterResults.length = 0; } catch { /* noop */ }
    }
  ...
  _demoFetchLaterBasic() {
    ...
    try {
      ...
      try { stateStr = String(result.state); } catch { /* noop */ }
  ...
  _demoFetchLaterDismiss() {
    ...
    try {
      ...
      try { before = String(result.state); } catch { /* noop */ }
      result.dismiss();
      try { after = String(result.state); } catch { /* noop */ }
  ...
  _demoBeaconDataTypes() {
    ...
    try {
      ...
      const results: any[] = [];
      let ok;
      ok = navigator.sendBeacon(url, 'plain-string'); results.push(`string → ${ok}`);
      try { ok = navigator.sendBeacon(url, new Blob(['{"a":1}'], { type: 'application/json' })); results.push(`Blob(application/json) → ${ok}`); } catch (e: any) { results.push(`Blob → 失败:${e.message}`); }
      try { ok = navigator.sendBeacon(url, new ArrayBuffer(8)); results.push(`ArrayBuffer(8) → ${ok}`); } catch (e: any) { results.push(`ArrayBuffer → 失败:${e.message}`); }
      try { ok = navigator.sendBeacon(url, new Uint8Array([1, 2, 3, 4])); results.push(`Uint8Array → ${ok}`); } catch (e: any) { results.push(`Uint8Array → 失败:${e.message}`); }
      try { ok = navigator.sendBeacon(url, new URLSearchParams({ a: '1', b: '2' })); results.push(`URLSearchParams → ${ok}`); } catch (e: any) { results.push(`URLSearchParams → 失败:${e.message}`); }
      try { const fd = new FormData(); fd.append('x', '1'); ok = navigator.sendBeacon(url, fd); results.push(`FormData → ${ok}`); } catch (e: any) { results.push(`FormData → 失败:${e.message}`); }
  ...
  _renderCard2() {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'navigator.sendBeacon(url, data) 返回 boolean（true 表示已成功加入浏览器发送队列）。data 可为 ArrayBuffer / ArrayBufferView / Blob / string / URLSearchParams / FormData，浏览器据此推断 Content-Type。限制：单次约 64KB（浏览器相关，超出返回 false）、仅 POST、无自定义 headers、无 Response。请求在下一个空闲期或页面卸载时发出，survives page close。visibilitychange → document.visibilityState === "hidden" 是 RELIABLE 上报时机（pagehide 已废弃用于此目的，且影响 bfcache）。sendBeacon 适合小体积遥测。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('发送 JSON beacon', { type: 'primary', size: 'sm', disabled: !caps.sendBeacon, onClick: () => this._demoSendBeacon() }),
          this._btn('data 类型演示', { size: 'sm', disabled: !caps.sendBeacon, onClick: () => this._demoBeaconDataTypes() }),
          this._btn('64KB 限制说明', { size: 'sm', onClick: () => this._explainBeaconLimit() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'sendBeacon 状态 / 用法：'),
        h('pre', { class: 'code-block', style: { maxHeight: '280px', overflow: 'auto' } },
          h('code', {}, s.beaconInfo || '（点击「发送 JSON beacon」或「64KB 限制说明」）')),
        h(Alert, {
          type: 'warning',
  ...
  _demoAllFour() {
    ...
    if (caps.sendBeacon) { lines.push('   navigator.sendBeacon(url, JSON.stringify(payload));  // 返回 boolean'); }
    else { lines.push('   // sendBeacon 不可用'); }
    ...
    } else { lines.push('   // fetchLater 不可用（Chrome 123+ 实验性）'); }
    lines.push('');
    lines.push('差异要点：');
  ...
  _buildReportingHeader() {
    ...
    const headerValue = (Object as any).entries(endpoints).map(([name, url]: any) => `${name}="${url}"`).join(', ');
    ...
    this._addLog('reporting', `构造 Reporting-Endpoints 头值（${(Object as any).keys(endpoints).length} 个命名端点）`);
  }
  ...
  _registerPagehide() {
    ...
    this._pagehideHandler = (event: any) => {
  ...
  _renderCard5() {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '可靠遥测信号链：1) visibilitychange → hidden（用户切标签页 / 最小化 / 导航离开，移动端尤其可靠，★ 最佳 flush 时机）；2) pagehide（页面卸载中，用 event.persisted 检测 bfcache）；3) beforeunload（移动端不可靠）；4) unload（已废弃、不可靠、且阻止 bfcache）。模式：visibilitychange → hidden 时用 fetchLater / sendBeacon flush 所有排队遥测；在途 fetch 用 AbortController 中止。navigator.scheduling.isInputPending() 检测是否安全发送。不要用 unload（破坏 bfcache）。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('注册 visibilitychange', { type: 'primary', size: 'sm', disabled: !caps.visibilityState, onClick: () => this._registerVisibility() }),
          this._btn('注册 pagehide', { size: 'sm', onClick: () => this._registerPagehide() }),
          this._btn('flush 模式 / unload', { size: 'sm', onClick: () => this._showFlushPattern() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '生命周期 / 上报时机：'),
        h('pre', { class: 'code-block', style: { maxHeight: '320px', overflow: 'auto' } },
          h('code', {}, s.lifecycleInfo || '（点击「注册 visibilitychange」或「flush 模式」）')),
        h(Alert, {
          type: 'warning',
  ...
  _demoBatchSend() {
    ...
    if (caps.fetchLater) {
      ...
      } catch (e: any) { lines.push(`    调用异常：${e.message}`); }
    } else if (caps.sendBeacon) {
      lines.push('  → navigator.sendBeacon(url, body)');
      try { const ok = navigator.sendBeacon(url, body); lines.push(`    返回 ${ok}`); } catch (e: any) { lines.push(`    异常：${e.message}`); }
    } else if (caps.fetch) {
  ...
  _renderCard6() {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '完整遥测管线：1) 采集（内存数组收集 click/error/timing）；2) 批量（每 N 条 OR 每 M 秒 OR visibilitychange hidden 触发 flush）；3) 序列化（JSON.stringify）；4) 发送（fetchLater 优先 / sendBeacon 小 / fetch+keepalive 需响应）；5) 重试（fetchLater attemptCount 或失败重新入队）；6) 退避（指数退避）；7) 采样（高频事件采 10%）。navigator.storage.estimate() 检查配额做持久化；Service Worker sync 事件保证送达（Background Sync）。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('TelemetryQueue 代码', { type: 'primary', size: 'sm', onClick: () => this._buildTelemetryQueue() }),
          this._btn('批量发送演示', { size: 'sm', onClick: () => this._demoBatchSend() }),
          this._btn('聚合能力检测', { size: 'sm', onClick: () => this._showAggregate() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '遥测架构 / 队列管理：'),
        h('pre', { class: 'code-block', style: { maxHeight: '360px', overflow: 'auto' } },
          h('code', {}, s.telemetryInfo || '（点击「TelemetryQueue 代码」或「批量发送演示」）')),
        h(Alert, {
          type: 'info',
  ...
  render() {
    const s = this.state;
    return h('div', { class: 'api-lab-page fetch-later-api-page' },
      h('h2', { class: 'section-title' }, 'fetchLater 与可靠上报 实验室'),
      h('p', { class: 'fs-sm text-secondary mb-md' },
        'fetchLater 是 Chrome 123+ 实验性延迟请求 API，配合 sendBeacon / fetch+keepalive / Reporting-Endpoints / visibilitychange 构成完整可靠上报体系。本页演示 fetchLater、sendBeacon 深入、四方式对比、Reporting-Endpoints、生命周期时机与遥测队列架构。'),
