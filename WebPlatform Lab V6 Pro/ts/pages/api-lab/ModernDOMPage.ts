...
export interface ModernDOMPageProps extends Props {}

export interface ModernDOMLog {
  type: string;
  content: string;
  time: string;
}

export interface ModernDOMPageState extends State {
  logs: ModernDOMLog[];
  dialogSupported: boolean;
  popoverSupported: boolean;
  vtSupported: boolean;
  registerPropertySupported: boolean;
  containerQuerySupported: boolean;
  anchorSupported: boolean;
  dsdSupported: boolean;
  vtView: string;
  vtTypesEnabled: boolean;
...
export class ModernDOMPage extends Page {
  declare props: ModernDOMPageProps;
  declare state: ModernDOMPageState;
  _inited: boolean = false;
  _demoStyleEl: HTMLStyleElement | null = null;

  initialState(): ModernDOMPageState {
    return {
      logs: [],
  ...
  componentWillUnmount(): void {
    ...
    const dialog = this.$<HTMLDialogElement>('#demo-dialog');
    if (dialog && typeof dialog.close === 'function') {
      try { dialog.close(); } catch { /* noop */ }
    }
    const popover = this.$<HTMLElement>('#demo-popover');
    if (popover && typeof (popover as any).hidePopover === 'function') {
      try { (popover as any).hidePopover(); } catch { /* noop */ }
    }
  }
  ...
  _closeDialog(value: string): void {
    const dialog = this.$<HTMLDialogElement>('#demo-dialog');
    if (!dialog || typeof dialog.close !== 'function') {
...
::backdrop { background: rgba(0,0,0,.45); }`),
  ...
  _togglePopover(): void {
    ...
    try { (popover as any).togglePopover(); } catch (err: any) {
  ...
  _showPopover(): void {
    ...
    try { (popover as any).showPopover(); } catch (err: any) {
  ...
  _hidePopover(): void {
    ...
    const popover = this.$<HTMLElement>('#demo-popover');
    if (!popover || !this.state.popoverSupported) return;
    try { (popover as any).hidePopover(); } catch (err: any) {
      setTimeout(() => this._addLog('error', `hidePopover 失败: ${err.message}`), 0);
    }
  }

  _renderPopoverCard(): Node {
    const s = this.state;
    return h(Card, {
      ...
      h('div', {
        ...
        onbeforetoggle: (e: Event) => {
          ...
          const status = this.$<HTMLElement>('#popover-status');
          if (status) status.textContent = `beforetoggle: ${te.oldState} → ${te.newState}`;
        },
        ontoggle: (e: Event) => {
          const te = e as ToggleEvent;
          const status = this.$<HTMLElement>('#popover-status');
          if (status) status.textContent = `状态: ${te.newState === 'open' ? '打开' : '关闭'}`;
        },
      },
...
el.addEventListener('toggle', e => console.log('now:', e.newState));`),
  ...
  _switchView(): void {
    ...
    if (transition?.updateCallbackDone) {
      transition.updateCallbackDone
        ...
        .catch((e: any) => this._addLog('error', `updateCallbackDone 失败: ${e.message}`));
    }
    if (transition?.ready) {
      transition.ready
        .then(() => this._addLog('info', 'transition.ready —— ::view-transition-* 伪元素已就绪'))
        .catch((e: any) => this._addLog('error', `transition.ready 失败: ${e.message}`));
    }
    if (transition?.finished) {
      transition.finished
        ...
        .catch((e: any) => this._addLog('error', `transition.finished 失败: ${e.message}`));
    }
  ...
  _renderViewTransitionsCard(): Node {
    const s = this.state;
    return h(Card, {
...
.typed-box {
  ...
  _renderContainerQueriesCard(): Node {
    const s = this.state;
    return h(Card, {
      ...
      h('div', { class: 'flex items-center gap-sm mt-sm' },
        h('input', {
          ...
          oninput: (e: Event) => {
            ...
            const container = this.$<HTMLElement>('#cq-demo');
            if (container) container.style.width = w + 'px';
            const display = this.$<HTMLElement>('#cq-width-display');
...
ro.observe(containerEl);`),
  ...
  _renderDsd(): void {
    const host = this.$<HTMLElement>('#dsd-host');
    if (!host) return;
    host.innerHTML = `
  ...
  _renderAnchorDsdCard(): Node {
    ...
    return h(Card, {
      ...
      s.dsdReady ? h('p', { class: 'fs-sm mt-sm', style: { color: 'var(--color-success)' } },
        '✓ host.shadowRoot 已建立，内部样式封装在 shadow 内，::part(label) 可被外部 CSS 穿透。') : null,
