...
export interface StreamsFetchDeepPageProps extends Props {}

export interface StreamsFetchDeepPageState extends State {}

export class StreamsFetchDeepPage extends Page {
  declare props: StreamsFetchDeepPageProps;
  declare state: StreamsFetchDeepPageState;
  _abortControllers: Set<any> | null = null;
  _abortCtrl: any = null;
  _autoAbortTimer: any = null;
  _demoReader: any = null;
  _demoStream: any = null;
  _inited: boolean = false;
  _readers: any[] | null = null;
  _streams: any[] | null = null;
  _timers: Set<any> | null = null;
  // —— 初始 state ——
  initialState(): StreamsFetchDeepPageState {
    return {
  ...
  componentDidMount(): void {
    ...
    if (!this._abortControllers) this._abortControllers = new Set<any>(); // 所有活跃 AbortController
    if (!this._readers) this._readers = [];                          // 所有活跃 reader（releaseLock）
    if (!this._streams) this._streams = [];                          // 所有活跃 stream（cancel）
    if (!this._timers) this._timers = new Set<any>();                     // 所有 setTimeout 句柄
  ...
  componentWillUnmount(): void {
    ...
    // 清除所有定时器
    if (this._timers) {
      this._timers.forEach((t: any) => { try { clearTimeout(t); } catch { /* noop */ } });
      this._timers.clear();
    }
  ...
  _trackAbort(c: any) { if (c) this._abortControllers!.add(c); return c; }
  _untrackAbort(c: any) { try { this._abortControllers!.delete(c); } catch { /* noop */ } }
  _trackReader(r: any) { if (r) this._readers!.push(r); return r; }
  _untrackReader(r: any) {
    try { const i = this._readers!.indexOf(r); if (i >= 0) this._readers!.splice(i, 1); } catch { /* noop */ }
  }
  _trackStream(s: any) { if (s) this._streams!.push(s); return s; }
  _setTimer(fn: any, ms: any) { const id = setTimeout(fn, ms); this._timers!.add(id); return id; }
  _clearTimer(id: any) { try { clearTimeout(id); this._timers!.delete(id); } catch { /* noop */ } }
  ...
  _buildRequest() {
    ...
    try {
      ...
      try { headers.forEach((v: any, k: any) => headerList.push(`${k}: ${v}`)); } catch { /* noop */ }
  ...
  async _sendFetch() {
    ...
    try {
      ...
      const hs: any[] = [];
      try { resp.headers.forEach((v: any, k: any) => hs.push(`${k}: ${v}`)); } catch { /* noop */ }
      let bodyText = '';
...
resp.text(); resp.json(); resp.blob(); resp.arrayBuffer(); resp.formData(); resp.clone();`),
  ...
  async _demoTimeoutSignal() {
    ...
    let signal;
    try { signal = AbortSignal.timeout(800); } catch (err: any) {
  ...
  _buildStream() {
    ...
    const mkStream = (withType: any) => {
      ...
      const underlying = {
        ...
        cancel(reason: any) { /* 被取消时回调 */ },
      };
      return withType ? new ReadableStream((underlying as any), ({ type: 'bytes' } as any)) : new ReadableStream(underlying);
    };
    let stream = null, usedFallback = false;
    try {
      stream = mkStream(true); // 优先尝试 type:'bytes'（启用 BYOB）
    } catch (err: any) {
      try { stream = mkStream(false); usedFallback = true; } catch (e2: any) {
        ...
        s.streamChunks.length > 0
          ? h('div', { class: 'mt-md' },
              ...
              h('div', { class: 'flex items-center gap-xs flex-wrap' },
                ...s.streamChunks.map((c: any, i: any) => h(Tag, { color: 'primary' }, `#${i + 1} ${JSON.stringify(c)}`)),
              ),
            )
          : null,
        h('pre', { class: 'code-block mt-md' },
  ...
  async _pipeDemo() {
    ...
    try {
      ...
      const upper = new TransformStream({
        transform(chunk: any, controller: any) { controller.enqueue(String(chunk).toUpperCase()); },
        flush(controller: any) { try { controller.enqueue('!'); } catch { /* noop */ } controller.close?.(); },
      });
      // sink：收集结果
      const sink: any[] = [];
      let totalBytes = 0;
  ...
  async _teeDemo() {
    ...
    try {
      ...
      const src = new ReadableStream({
        start(controller: any) {
          [1, 2, 3, 4, 5].forEach((n: any) => controller.enqueue(n));
          controller.close();
        },
      });
      this._trackStream(src);
      let branches;
      try { branches = src.tee(); } catch (err: any) {
  ...
  async _streamDownload() {
    ...
    try {
      ...
      while (true) {
        ...
        if (done) break;
        const bytes = value && (value as any).byteLength != null ? (value as any).byteLength : 0;
  ...
  _demoHeadersAdvanced() {
    ...
    try {
      ...
      if (hasGetSetCookie) {
        ...
        cookies.forEach((c: any, i: any) => lines.push(`  [${i}] ${c}`));
      } else {
      ...
      const entries: any[] = [];
      headers.forEach((v: any, k: any) => entries.push(`  ${k}: ${v}`));
      lines.push(`forEach 遍历 ${entries.length} 项（注意 Set-Cookie 被合并为一条）：`);
      entries.forEach((e: any) => lines.push(e));
      ...
      try {
        const req = new Request('https://example.com', ({
          ...
          body: new ReadableStream({
            start(c: any) { c.enqueue(new TextEncoder().encode('chunk1\n')); c.close(); },
          }),
          duplex: 'half',
        } as any));
