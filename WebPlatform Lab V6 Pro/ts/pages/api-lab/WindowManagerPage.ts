...
class _MockBroadcastChannel {
  ...
  constructor(name: any) {
    ...
    this._closed = false;
    if (!_BC_REGISTRY.has(this.name)) _BC_REGISTRY.set(this.name, new Set<any>());
  ...
  postMessage(message: any) {
    ...
    } catch { cloned = JSON.parse(JSON.stringify(message)); }
    ...
    setTimeout(() => {
      ...
      for (const ch of cur) {
        ...
        if (typeof ch.onmessage === 'function') {
          try { ch.onmessage({ data: cloned, origin: 'mock', target: ch, source: ch }); }
          catch (e: any) { /* 单个 listener 异常不影响其它 */ }
        }
      }
    }, 0);
  }
...
interface WindowManagerPageCaps {
  getScreenDetails: boolean;
  isExtended: boolean;
  orientation: boolean;
  windowOpen: boolean;
  postMessage: boolean;
...
export interface WindowManagerPageProps extends Props {}

export interface WindowManagerPageState extends State {
  logs: LogEntry[];
  capsSummary: string;
  screenDetailsInfo: string;
  windowOpenInfo: string;
  broadcastInfo: string;
  visibilityInfo: string;
  windowControlInfo: string;
  permissionInfo: string;
}

export class WindowManagerPage extends Page {
  declare props: WindowManagerPageProps;
  declare state: WindowManagerPageState;
  ...
  _bcTx!: any;
  _bcUsedMock!: boolean;
  _visibilityBound!: boolean;
  _onCurrentScreenChange!: any;
  _onScreensChange!: any;
  _closed!: boolean;


  // —— 初始 state ——
  initialState(): WindowManagerPageState {
    return {
  ...
  componentDidMount(): void {
    ...
    const summary =
      `Window Management / 多窗口 能力检测：${parts.join(' · ')}。\n` +
    ...
    if (!caps.getScreenDetails) this._addLog('warn', 'window.getScreenDetails 不可用（需 Window Management API + 用户授权 + 安全上下文）');
    if (!caps.isExtended) this._addLog('warn', 'screen.isExtended 不可用（jsdom 中为 undefined，浏览器中需多屏环境）');
    if (!caps.orientation) this._addLog('warn', 'screen.orientation 不可用（jsdom 未实现）');
    if (!caps.broadcastChannel) this._addLog('warn', 'BroadcastChannel 不可用，将使用 mock 实现演示跨标签通信');
    if (!caps.permissions) this._addLog('warn', 'navigator.permissions 不可用（jsdom 未实现 Permissions API）');
    if (!caps.isSecureContext) this._addLog('warn', 'window.isSecureContext 非 true（Window Management / 窗口放置 API 要求安全上下文）');
    ...
    if (caps.postMessage && typeof window.addEventListener === 'function') {
      this.on(window, 'message', (ev) => {
        ...
        if (data.from !== 'wm-demo-child') return;
  ...
  componentWillUnmount(): void {
    if (this._screenDetails && typeof this._screenDetails.removeEventListener === 'function') {
      try { this._screenDetails.removeEventListener('currentscreenchange', this._onCurrentScreenChange); } catch { /* noop */ }
      try { this._screenDetails.removeEventListener('screenschange', this._onScreensChange); } catch { /* noop */ }
    }
    this._screenDetails = null;
    if (this._openedWindow && !this._openedWindow.closed && typeof this._openedWindow.close === 'function') {
      try { this._openedWindow.close(); } catch { /* noop */ }
    }
    this._openedWindow = null;
    if (this._bcRx) { try { this._bcRx.close(); } catch { /* noop */ } this._bcRx = null; }
    if (this._bcTx) { try { this._bcTx.close(); } catch { /* noop */ } this._bcTx = null; }
  }
  ...
  async _getScreenDetails() {
    ...
    try {
      ...
      const lines = screens.map((s: any, i: number) =>
        `[${i}] label="${s.label}"  ${s.width}×${s.height} @ (${s.left}, ${s.top})` +
      ...
      this.setState({
        screenDetailsInfo:
          `window.getScreenDetails() → ScreenDetails ✓\n` +
      ...
      this.setState({
        screenDetailsInfo:
          `getScreenDetails 失败：${err.name} - ${err.message}\n` +
  ...
  _renderCard1(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.getScreenDetails ? 'success' : 'error' }, caps.getScreenDetails ? 'getScreenDetails ✓' : '不可用'),
        h(Tag, { color: caps.isExtended ? 'success' : 'warning' }, caps.isExtended ? 'isExtended ✓' : 'isExtended ✗'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'window.getScreenDetails() → Promise<ScreenDetails>（需用户授权 window-management 权限 + 安全上下文）。ScreenDetails.screens 是 ScreenDetailed[] 数组，ScreenDetails.currentScreen 指向当前屏幕；ScreenDetailed 暴露 label / top / left / width / height / availLeft / availTop / availWidth / availHeight / isPrimary / isInternal / devicePixelRatio。screen.isExtended（布尔，只读）可免授权检测是否多屏。ScreenDetails 还提供 currentscreenchange / screenschange 事件。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('getScreenDetails()', { type: 'primary', size: 'sm', onClick: () => this._getScreenDetails() }),
          this._btn('查询 screen 对象', { size: 'sm', onClick: () => this._queryScreenObject() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '屏幕信息：'),
        h('pre', { class: 'code-block', style: { maxHeight: '240px', overflow: 'auto' } },
          h('code', {}, s.screenDetailsInfo || '（点击「getScreenDetails()」或「查询 screen 对象」）')),
...
console.log(screen.isExtended); // 是否多屏（免授权）`)),
  ...
  _openWindow(): void {
    ...
    try {
      ...
      if (w && typeof w.postMessage === 'function') {
        ...
        this.setState({
          windowOpenInfo:
            `window.open('${url}', '${name}', '${features}') → 真实 WindowProxy ✓\n` +
        ...
        this.setState({
          windowOpenInfo:
            `window.open('${url}', '${name}', '${features}') → null（jsdom 无真实窗口）\n` +
  ...
  _makeMockWindow(url: any, name: string,features: any): any  {
    const page = this;
    ...
    const mock: any = {
      ...
      postMessage(message: any, targetOrigin: any) {
        ...
        setTimeout(() => {
          if (mock.closed || !opener) return;
          const reply = { from: 'wm-demo-child', echo: message, t: Date.now() };
          if (typeof MessageEvent !== 'function') { page._addLog('warn', 'MessageEvent 不可用，无法派发回执'); return; }
          try {
  ...
  _renderCard2(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.windowOpen ? 'success' : 'error' }, caps.windowOpen ? 'window.open ✓' : '不可用'),
        h(Tag, { color: caps.postMessage ? 'success' : 'error' }, caps.postMessage ? 'postMessage ✓' : '不可用'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'window.open(url, name, features) → WindowProxy（子窗引用）；childWin.postMessage(message, targetOrigin) 跨窗口通信（targetOrigin 限定来源，"*" 表示任意）；childWin.close() 关闭子窗；window.opener 指向打开者；window.closed 标记是否已关闭；window.name 跨导航保持。jsdom 中 window.open 返回 null（无真实窗口），本页创建 mock WindowProxy 演示 postMessage 双向流程。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('window.open()', { type: 'primary', size: 'sm', disabled: !caps.windowOpen, onClick: () => this._openWindow() }),
          this._btn('postMessage', { type: 'primary', size: 'sm', disabled: !caps.postMessage, onClick: () => this._postMessageToChild() }),
          this._btn('close()', { size: 'sm', disabled: !caps.windowOpen, onClick: () => this._closeWindow() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '窗口通信状态：'),
        h('pre', { class: 'code-block', style: { maxHeight: '220px', overflow: 'auto' } },
          h('code', {}, s.windowOpenInfo || '（点击「window.open()」）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
        h('pre', { class: 'code-block', style: { maxHeight: '150px', overflow: 'auto' } },
          h('code', {},
`const child = window.open('child.html', 'demo', 'width=480,height=320');
...
child.close();                                                 // 关闭子窗`)),
  ...
  _createChannels(): void {
    ...
    try {
      // 先关闭旧的频道
      if (this._bcRx) { try { this._bcRx.close(); } catch { /* noop */ } }
      if (this._bcTx) { try { this._bcTx.close(); } catch { /* noop */ } }
      ...
      this.setState({
        broadcastInfo:
          `已创建 BroadcastChannel 对（name="${channelName}"）\n` +
  ...
  _bcClose(): void {
    ...
    try {
      ...
      if (this._bcRx) { this._bcRx.onmessage = null; try { this._bcRx.close(); } catch { /* noop */ } }
      if (this._bcTx) { try { this._bcTx.close(); } catch { /* noop */ } }
      this._bcRx = null;
      this._bcTx = null;
      this.setState({
        broadcastInfo:
          `BroadcastChannel 已 close() 释放 ✓\n` +
  ...
  _renderCard3(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.broadcastChannel ? 'success' : 'warning' }, caps.broadcastChannel ? '原生 BC ✓' : 'mock BC'),
        h(Tag, { color: 'primary' }, '同源跨标签页'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'new BroadcastChannel(name) 创建同名频道；channel.postMessage(message) 广播给所有同名频道（不包含自己）；channel.onmessage 接收消息；channel.close() 释放。同源的所有标签页/窗口中同名频道互相可见，用于跨标签页状态同步。本页创建一对同名频道（发送端 + 接收端）演示；环境不支持时用 mock 实现（基于模块级注册表 + setTimeout 异步派发）。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('创建 Channel 对', { type: 'primary', size: 'sm', onClick: () => this._createChannels() }),
          this._btn('postMessage', { type: 'primary', size: 'sm', disabled: !this._bcTx, onClick: () => this._bcPost() }),
          this._btn('close 释放', { size: 'sm', onClick: () => this._bcClose() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'BroadcastChannel 状态：'),
        h('pre', { class: 'code-block', style: { maxHeight: '220px', overflow: 'auto' } },
          h('code', {}, s.broadcastInfo || '（点击「创建 Channel 对」）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
...
bc.close(); // 不用时释放`)),
  ...
  _queryVisibility(): void {
    ...
    try {
      ...
      this.setState({
        visibilityInfo:
          `document.visibilityState = "${state}"   document.hidden = ${hidden}\n` +
  ...
  _queryOrientation(): void {
    if (typeof screen === 'undefined' || !screen.orientation) {
      this._addLog('warn', 'screen.orientation 不可用（jsdom 未实现）');
      this.setState({
        visibilityInfo:
          (this.state.visibilityInfo ? this.state.visibilityInfo + '\n\n' : '') +
  ...
  _bindVisibility(): void {
    ...
    this.setState({
      visibilityInfo:
        `已注册 document 'visibilitychange' 事件监听 ✓\n` +
        `当前 visibilityState = "${document.visibilityState}"，hidden = ${document.hidden}\n` +
  ...
  _renderCard4(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.visibility ? 'success' : 'error' }, caps.visibility ? 'visibility ✓' : '不可用'),
        h(Tag, { color: caps.orientation ? 'success' : 'warning' }, caps.orientation ? 'orientation ✓' : 'orientation ✗'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'document.visibilityState（"visible"/"hidden"/"prerender"）与 document.hidden（布尔）反映页面可见性；visibilitychange 事件在切换标签页/最小化时触发，常用于暂停后台轮询、视频播放。screen.orientation.type（portrait-primary/landscape-primary 等）与 angle 反映屏幕方向。jsdom 中 visibilityState 恒为 "visible"，orientation 通常未实现。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('查询可见性', { type: 'primary', size: 'sm', disabled: !caps.visibility, onClick: () => this._queryVisibility() }),
          this._btn('查询 orientation', { size: 'sm', onClick: () => this._queryOrientation() }),
          this._btn('监听 visibilitychange', { size: 'sm', disabled: !caps.visibility, onClick: () => this._bindVisibility() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '可见性 / 方向状态：'),
        h('pre', { class: 'code-block', style: { maxHeight: '200px', overflow: 'auto' } },
          h('code', {}, s.visibilityInfo || '（点击「查询可见性」/「查询 orientation」/「监听 visibilitychange」）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
...
console.log(screen.orientation.type);   // 'landscape-primary'`)),
  ...
  // 各操作的参数文本（用于日志展示）
  _opArgs(op: any): any  {
    return ({ moveTo: '100, 100', moveBy: '50, 50', resizeTo: '800, 600', resizeBy: '100, 100', focus: '' } as any)[(op as any)] || '';
  }

  // 统一执行窗口几何操作（moveTo / moveBy / resizeTo / resizeBy / focus）
  _doWindowOp(op: any): void  {
    ...
    try {
      ...
      const args = ({ moveTo: [100, 100], moveBy: [50, 50], resizeTo: [800, 600], resizeBy: [100, 100], focus: [] } as any)[(op as any)] || [];
      (window as any)[op](...args);   // jsdom 中为 no-op（存在但不改变窗口几何）
      const after = this._windowSnapshot();
      const argText = this._opArgs(op);
      this.setState({
        windowControlInfo:
          `调用 window.${op}(${argText})\n` +
  ...
  _renderCard5(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.moveTo && caps.resizeTo ? 'success' : 'error' }, caps.moveTo && caps.resizeTo ? 'moveTo/resizeTo ✓' : '不可用'),
        h(Tag, { color: caps.focus ? 'success' : 'error' }, caps.focus ? 'focus ✓' : '不可用'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'window.moveTo(x, y) 移动窗口到绝对坐标；window.moveBy(dx, dy) 相对移动；window.resizeTo(w, h) 设置窗口外尺寸；window.resizeBy(dw, dh) 相对调整尺寸；window.focus() 聚焦窗口；window.blur() 取消聚焦。这些方法受弹窗策略限制（通常仅对 window.open 打开的窗口生效）。jsdom 中均为 no-op 函数（存在但不改变窗口几何），本页记录调用前后 screenX/screenY/outerWidth/outerHeight 以展示无变化。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('moveTo(100,100)', { type: 'primary', size: 'sm', disabled: !caps.moveTo, onClick: () => this._doWindowOp('moveTo') }),
          this._btn('moveBy(50,50)', { size: 'sm', disabled: !caps.moveTo, onClick: () => this._doWindowOp('moveBy') }),
          this._btn('resizeTo(800,600)', { type: 'primary', size: 'sm', disabled: !caps.resizeTo, onClick: () => this._doWindowOp('resizeTo') }),
          this._btn('resizeBy(100,100)', { size: 'sm', disabled: !caps.resizeTo, onClick: () => this._doWindowOp('resizeBy') }),
          this._btn('focus()', { size: 'sm', disabled: !caps.focus, onClick: () => this._doWindowOp('focus') }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '窗口几何状态：'),
        h('pre', { class: 'code-block', style: { maxHeight: '200px', overflow: 'auto' } },
          h('code', {}, s.windowControlInfo || '（点击上方按钮调用窗口几何 API）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
        h('pre', { class: 'code-block', style: { maxHeight: '120px', overflow: 'auto' } },
          h('code', {},
`// 仅对 window.open 打开的子窗通常有效
...
child.focus();               // 聚焦子窗`)),
  ...
  async _queryPermission(name: any) {
    if (typeof navigator === 'undefined' || !navigator.permissions || typeof navigator.permissions.query !== 'function') {
      this._addLog('warn', `navigator.permissions 不可用，无法 query({name:'${name}'})`);
      this.setState({
        permissionInfo:
          `navigator.permissions 不可用（jsdom 未实现 Permissions API）。\n` +
    ...
    try {
      ...
      this.setState({
        permissionInfo:
          `navigator.permissions.query({ name: '${name}' } as any) → PermissionStatus ✓\n` +
      ...
      this.setState({
        permissionInfo:
          `navigator.permissions.query({ name: '${name}' } as any) 抛错：${err.name} - ${err.message}\n` +
  ...
  _renderCard6(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.permissions ? 'success' : 'error' }, caps.permissions ? 'Permissions ✓' : '不可用'),
        h(Tag, { color: caps.isSecureContext ? 'success' : 'warning' }, caps.isSecureContext ? 'Secure ✓' : '非 Secure'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'navigator.permissions.query({ name }) → Promise<PermissionStatus>，status.state ∈ granted/denied/prompt。Window Management API 的权限名为 "window-management"（新规范，旧名 "window-placement"）。调用 getScreenDetails 前应先查询权限：prompt 时需用户手势触发授权。PermissionStatus.onchange 在权限状态变化时触发。要求安全上下文（HTTPS / isSecureContext）。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn("query('window-management')", { type: 'primary', size: 'sm', onClick: () => this._queryPermission('window-management') }),
          this._btn("query('window-placement')", { size: 'sm', onClick: () => this._queryPermission('window-placement') }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '权限查询结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '200px', overflow: 'auto' } },
          h('code', {}, s.permissionInfo || '（点击 query 按钮）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
        h('pre', { class: 'code-block', style: { maxHeight: '160px', overflow: 'auto' } },
          h('code', {},
`// 查询 window-management 权限
...
status.onchange = () => { console.log('权限变为:', status.state); };`)),
  ...
  _renderLogPanel(): Node | string {
    ...
    return h('div', { class: 'log-panel' },
      ...
      h('pre', { class: 'code-block log-pre', style: { maxHeight: '300px', overflow: 'auto' } },
        s.logs.length === 0
          ? h('code', {}, '（暂无日志）')
          : s.logs.map((log) => h('div', { class: 'log-line' },
              h('span', { class: 'log-time' }, log.time),
              h('span', { class: `log-tag log-tag-${log.type}` }, log.type),
  ...
  render(): Node {
    ...
    return h('div', { class: 'page api-lab-page' },
      h('h2', { class: 'section-title' }, 'Window Management 实验室'),
      h('p', { class: 'fs-sm text-secondary mb-md' },
        'Window Management API / 多窗口 / 屏幕信息 / 跨窗口通信。本页演示 getScreenDetails、window.open + postMessage、BroadcastChannel、Page Visibility、窗口几何控制、Permission API。'),
