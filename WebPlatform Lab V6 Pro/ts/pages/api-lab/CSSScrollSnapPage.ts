...
interface CSSScrollSnapPageState extends State {
  logs: LogEntry[];
  capsSummary: string;
  overviewInfo: string;
  snapTypeInfo: string;
  snapAlignInfo: string;
  paddingMarginInfo: string;
  snapStopInfo: string;
  carouselInfo: string;
  galleryTabsInfo: string;
  pitfallsInfo: string;
  scrollStartInfo: string;
}
...
export class CSSScrollSnapPage extends Page {
  declare state: CSSScrollSnapPageState;
  _inited: boolean = false;
  _dynamicStyles: HTMLStyleElement[] = [];
  _snapTypeMode: string = '';
  _snapAlignMode: string = '';
  _scrollPaddingMode: string = '';
  _snapStopMode: string = '';
  _carouselIndex: number = 0;
  _scrollStartTargetMode: string = '';


  // —— 初始 state ——
  initialState(): CSSScrollSnapPageState {
    return {
      logs: [],
      capsSummary: '',
  ...
  componentDidMount(): void {
    ...
    if (!f.snapTypeX) this._addLog('warn', 'scroll-snap-type: x mandatory 不可用或 jsdom 未识别（Chrome 69+ / Firefox 68+ / Safari 11+ 全部稳定）');
    if (!f.snapAlignStart) this._addLog('warn', 'scroll-snap-align: start 不可用或 jsdom 未识别（Chrome 69+ / Firefox 68+ / Safari 11+ 全部稳定）');
    if (!f.snapStop) this._addLog('warn', 'scroll-snap-stop: always 不可用或 jsdom 未识别（Chrome 81+ / Firefox 75+ / Safari 14.1+ 较新）');
    if (!f.scrollStart0) this._addLog('warn', 'scroll-start: 0 不可用或 jsdom 未识别（CSS Scroll Snap Module Level 2 草案，仅 Chrome 124+ 实现，Safari/Firefox 截至 2024 未实现）');
    if (!f.scrollStartTargetStart) this._addLog('warn', 'scroll-start-target: start 不可用或 jsdom 未识别（CSS Scroll Snap Module Level 2 草案，仅 Chrome 124+ 实现）');
    ...
    const card = new Card({
      ...
      children: [
        ...
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.overviewInfo || '（点击「读取概述信息」查看 CSS Scroll Snap Module 全景）')),
        h(Alert, {
          type: 'info',
  ...
  _renderCard2(): Node | string {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'scroll-snap-type 是容器属性，定义吸附轴（none/x/y/block/inline/both）+ 严格度（mandatory 强制 / proximity 接近）。mandatory 强制吸附到最近 snap 点（适合轮播图、全屏分页），proximity 仅接近时吸附（适合画廊、列表）。必须配合 overflow 非 visible。block/inline 随 writing-mode 翻转（默认 block=y、inline=x，vertical-rl 时翻转）。浏览器支持完整：Chrome 69+ / Firefox 68+ / Safari 11+。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ snapTypeInfo: this._readSnapTypeInfo() }) }),
          this._btn('x mandatory', { size: 'sm', disabled: !f.snapTypeX, onClick: () => this._setSnapType('x mandatory') }),
          this._btn('x proximity', { size: 'sm', disabled: !f.snapTypeX, onClick: () => this._setSnapType('x proximity') }),
          this._btn('y mandatory', { size: 'sm', disabled: !f.snapTypeY, onClick: () => this._setSnapType('y mandatory') }),
          this._btn('none', { size: 'sm', onClick: () => this._setSnapType('none') }),
        ),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '横滑相册演示（当前 scroll-snap-type: ' + this._snapTypeMode + '，真实浏览器拖动观察吸附）：'),
        h('div', { class: 'snap-type-stage' },
          h('div', { class: 'snap-item' }, 'Item 1'),
          h('div', { class: 'snap-item' }, 'Item 2'),
          h('div', { class: 'snap-item' }, 'Item 3'),
          h('div', { class: 'snap-item' }, 'Item 4'),
          h('div', { class: 'snap-item' }, 'Item 5'),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.snapTypeInfo || '（点击按钮切换 scroll-snap-type 查看说明）')),
        h(Alert, {
          type: 'info',
  ...
  _renderCard3(): Node | string {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'scroll-snap-align 是子项属性，定义子项对齐到容器 snap 点的方式：none（不作为 snap 点）/ start（左/上对齐）/ center（居中对齐）/ end（右/下对齐）。可与 scroll-snap-type 配合生效，单个子项可设两个值（start end）让两端都是 snap 点。多元素可设不同 align，浏览器滚动停止时选择最近 snap 点。常见用法：轮播图 slide 设 start（左对齐）、画廊图片设 center（居中）、宽横幅设 start end（两端吸附）。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ snapAlignInfo: this._readSnapAlignInfo() }) }),
          this._btn('start', { size: 'sm', disabled: !f.snapAlignStart, onClick: () => this._setSnapAlign('start') }),
          this._btn('center', { size: 'sm', disabled: !f.snapAlignCenter, onClick: () => this._setSnapAlign('center') }),
          this._btn('end', { size: 'sm', disabled: !f.snapAlignStart, onClick: () => this._setSnapAlign('end') }),
          this._btn('none', { size: 'sm', onClick: () => this._setSnapAlign('none') }),
        ),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '横滑演示（当前 scroll-snap-align: ' + this._snapAlignMode + '）：'),
        h('div', { class: 'snap-align-stage' },
          h('div', { class: 'snap-item' }, 'A'),
          h('div', { class: 'snap-item' }, 'B'),
          h('div', { class: 'snap-item' }, 'C'),
          h('div', { class: 'snap-item' }, 'D'),
          h('div', { class: 'snap-item' }, 'E'),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.snapAlignInfo || '（点击按钮切换 scroll-snap-align 查看说明）')),
        h(Alert, {
          type: 'info',
  ...
  _renderCard4(): Node | string {
    ...
    const card = new Card({
      ...
      children: [
        ...
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ paddingMarginInfo: this._readPaddingMarginInfo() }) }),
          this._btn('padding 0', { size: 'sm', disabled: !f.scrollPadding, onClick: () => this._setScrollPadding('0px') }),
          this._btn('padding 20px', { size: 'sm', disabled: !f.scrollPadding, onClick: () => this._setScrollPadding('20px') }),
          this._btn('padding 40px', { size: 'sm', disabled: !f.scrollPadding, onClick: () => this._setScrollPadding('40px') }),
        ),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '横滑演示（scroll-padding: ' + this._scrollPaddingMode + '，子项 scroll-margin: 10px）：'),
        h('div', { class: 'scroll-pad-stage' },
          h('div', { class: 'snap-item' }, 'Card 1'),
          h('div', { class: 'snap-item' }, 'Card 2'),
          h('div', { class: 'snap-item' }, 'Card 3'),
          h('div', { class: 'snap-item' }, 'Card 4'),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.paddingMarginInfo || '（点击按钮切换 scroll-padding 查看说明）')),
        h(Alert, {
          type: 'info',
  ...
  _renderCard5(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['snap-stop:always', f.snapStop]]),
        h(Tag, { color: 'primary' }, '移动端体验'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'scroll-snap-stop 控制是否允许快速滑动跳过多个 snap 点：normal（默认，允许跳过，适合画廊）/ always（强制每个 snap 点停止，适合轮播图）。移动端原生体验：iOS 相册、Android ViewPager 默认一页一滑，scroll-snap-stop: always 让 Web 体验接近原生。与触摸惯性协同：normal 时惯性可越过多个 snap 点，always 时在第一个 snap 点强制停止。浏览器支持较新：Chrome 81+ / Firefox 75+ / Safari 14.1+。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ snapStopInfo: this._readSnapStopInfo() }) }),
          this._btn('normal', { size: 'sm', onClick: () => this._setSnapStop('normal') }),
          this._btn('always', { type: 'primary', size: 'sm', disabled: !f.snapStop, onClick: () => this._setSnapStop('always') }),
        ),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '全屏轮播图演示（当前 scroll-snap-stop: ' + this._snapStopMode + '，真实浏览器用力滑动观察是否跳过）：'),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.snapStopInfo || '（点击按钮切换 scroll-snap-stop 查看说明）')),
        h(Alert, {
          type: 'info',
  ...
  _renderCard6(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['snap-type', f.snapTypeX], ['snap-stop', f.snapStop]]),
        h(Tag, { color: 'primary' }, '5 大场景'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '全屏轮播图五大场景：水平横滑相册（scroll-snap-type: x mandatory + flex: 0 0 100% + scroll-snap-stop: always 一页一滑）、垂直分页滚动（scroll-snap-type: y mandatory + scroll-padding-top 留固定导航空）、嵌套滚动容器（外层垂直分页 + 内层横滑相册，互不影响）、与 IntersectionObserver 懒加载协同（监听 slide 进入视口加载图片）、焦点管理（滑动停止后聚焦到当前 slide 可聚焦元素）。配合 scroll-behavior: smooth 实现平滑过渡。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('读取实战信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runCarouselDemo() }),
          this._btn('Slide 1', { size: 'sm', onClick: () => this._carouselGoTo(0) }),
          this._btn('Slide 2', { size: 'sm', onClick: () => this._carouselGoTo(1) }),
          this._btn('Slide 3', { size: 'sm', onClick: () => this._carouselGoTo(2) }),
          this._btn('Slide 4', { size: 'sm', onClick: () => this._carouselGoTo(3) }),
          this._btn('Slide 5', { size: 'sm', onClick: () => this._carouselGoTo(4) }),
        ),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '全屏轮播图演示（点击按钮切换，真实浏览器可拖动滑动）：'),
        h('div', { class: 'snap-carousel' },
          h('div', { class: 'slide slide-1' }, 'Slide 1'),
          h('div', { class: 'slide slide-2' }, 'Slide 2'),
          h('div', { class: 'slide slide-3' }, 'Slide 3'),
          h('div', { class: 'slide slide-4' }, 'Slide 4'),
          h('div', { class: 'slide slide-5' }, 'Slide 5'),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '600px', overflow: 'auto' } },
          h('code', {}, s.carouselInfo || '（点击按钮查看 5 大轮播图场景完整代码）')),
        h(Alert, {
          type: 'info',
  ...
  _renderCard7(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['snap-type', f.snapTypeX], ['snap-align', f.snapAlignStart]]),
        h(Tag, { color: 'primary' }, '4 大场景'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '图片画廊与 Tabs 四大场景：Tabs 选项卡切换 + scroll-snap（顶部 tab 栏横滑 proximity 吸附 + 点击滚动到内容）、图片网格 + 分页（每页 4 张 grid + 横滑切换页）、移动端 Drawer（底部抽屉上下 snap 展开/收起）、iOS/Android 原生体验对比（iOS UIScrollView pagingEnabled 等价 x mandatory + always，Android ViewPager 等价 x mandatory + flex 100%）。Web 优势：纯 CSS 无需 JS 库；劣势：触摸惯性体验略逊原生。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('读取实战信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runGalleryTabsDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, 'Tabs 选项卡 + scroll-snap 演示：'),
        h('div', { class: 'snap-tabs' },
          h('div', { class: 'tab active' }, '首页'),
          h('div', { class: 'tab' }, '产品'),
          h('div', { class: 'tab' }, '解决方案'),
          h('div', { class: 'tab' }, '案例'),
          h('div', { class: 'tab' }, '关于我们'),
          h('div', { class: 'tab' }, '联系我们'),
        ),
        h('div', { class: 'snap-tab-panel' },
          '当前 Tab：首页（小屏下 tab 栏可横滑，scroll-snap-type: x proximity 接近吸附）',
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '600px', overflow: 'auto' } },
          h('code', {}, s.galleryTabsInfo || '（点击按钮查看 4 大画廊/Tabs 场景完整代码）')),
        h(Alert, {
          type: 'info',
  ...
  _setScrollStartTarget(mode: any) {
    this._scrollStartTargetMode = mode;
    this._injectStyle('css-scroll-start-dynamic',
      `.scroll-start-stage .snap-item.is-target { scroll-start-target: ${mode}; }`);
  ...
  _runScrollStartDemo(): void {
    this.setState({ scrollStartInfo: this._readScrollStartInfo() });
    const f = this._flags();
    this._addLog('scroll-start',
      `scroll-start 演示：target=${this._scrollStartTargetMode}，scroll-start:0=${f.scrollStart0}，scroll-start:50%=${f.scrollStart50}（Level 2 草案，仅 Chrome 124+）`);
  }
  ...
  _renderCard9(): Node | string {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'scroll-start / scroll-start-target 是 CSS Scroll Snap Module Level 2 草案新增属性，声明式指定滚动容器初始位置（scroll-start: 0 / 50% / 100px / center，单轴 scroll-start-x/y、逻辑 scroll-start-block/inline）和子元素被滚动到时的对齐位置（scroll-start-target: start / center / end / nearest）。vs 命令式 scrollIntoView()：声明式无需 JS、SSR 友好、初始渲染即定位、无 FOUC 闪烁。浏览器支持仅 Chrome 124+（2024-05+），Safari/Firefox 未实现需降级到 JS scrollTo。实战：Tab 选项卡默认选中第 2 个、长列表默认滚动到某项、与 scroll-snap-stop 协同、分页文章阅读位置持久化（sessionStorage + history.scrollRestoration = manual）。陷阱：与 scroll-snap-type 双跳吸附、仅初始定位不持续、降级 scrollTo({behavior:"instant"})。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runScrollStartDemo() }),
          this._btn('target:start', { size: 'sm', disabled: !f.scrollStartTargetStart, onClick: () => this._setScrollStartTarget('start') }),
          this._btn('target:center', { type: 'primary', size: 'sm', disabled: !f.scrollStartTargetStart, onClick: () => this._setScrollStartTarget('center') }),
          this._btn('target:end', { size: 'sm', disabled: !f.scrollStartTargetStart, onClick: () => this._setScrollStartTarget('end') }),
          this._btn('target:nearest', { size: 'sm', disabled: !f.scrollStartTargetStart, onClick: () => this._setScrollStartTarget('nearest') }),
        ),
        ...
        h('div', { class: 'scroll-start-stage' },
          h('div', { class: 'snap-item' }, 'Item 1'),
          h('div', { class: 'snap-item' }, 'Item 2'),
          h('div', { class: 'snap-item is-target' }, 'Item 3 (target)'),
          h('div', { class: 'snap-item' }, 'Item 4'),
          h('div', { class: 'snap-item' }, 'Item 5'),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '700px', overflow: 'auto' } },
          h('code', {}, s.scrollStartInfo || '（点击按钮查看 scroll-start / scroll-start-target 完整说明）')),
        h(Alert, {
          type: 'warning',
  ...
  render(): Node {
    const s = this.state;
    return h('div', { class: 'api-lab-page css-scroll-snap-page' },
      h('h2', { class: 'section-title' }, 'CSS Scroll Snap Module Level 1 实验室'),
      h('p', { class: 'fs-sm text-secondary mb-md' },
        '本页演示 CSS Scroll Snap Module 滚动吸附：scroll-snap-type 容器属性（x mandatory / y proximity / both）、scroll-snap-align 子项属性（start / center / end）、scroll-padding 与 scroll-margin 偏移（固定导航协同）、scroll-snap-stop 强制停止（防快速跳过）、全屏轮播图实战（横滑相册 / 垂直分页 / 嵌套 / 懒加载 / 焦点）、图片画廊与 Tabs 选项卡（Tabs 切换 / 网格分页 / Drawer / 原生对比）、scroll-start / scroll-start-target 声明式滚动定位（Level 2 草案 / Chrome 124+ / 声明式初始位置 vs 命令式 scrollIntoView）、陷阱与最佳实践（mandatory/proximity 取舍 / 焦点跳变 / sticky 冲突 / RTL / smooth）。所有特性通过 typeof / CSS.supports 能力检测，不可用时仅记日志，绝不抛异常。'),
