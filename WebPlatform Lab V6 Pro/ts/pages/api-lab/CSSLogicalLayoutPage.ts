...
interface CSSLogicalLayoutPageState extends State {
  logs: LogEntry[];
  capsSummary: string;
  logicalPropsInfo: string;
  directionAwareInfo: string;
  writingModeInfo: string;
  subgridInfo: string;
  masonryInfo: string;
  boxModelInfo: string;
}
...
export class CSSLogicalLayoutPage extends Page {
  declare state: CSSLogicalLayoutPageState;
  _inited: boolean = false;
  _dynamicStyles: HTMLStyleElement[] = [];
  _rtlDemoOn: boolean = false;
  _writingMode: string = '';


  // —— 初始 state ——
  initialState(): CSSLogicalLayoutPageState {
    return {
      logs: [],
      capsSummary: '',
  ...
  componentDidMount(): void {
    ...
    if (!caps.marginInlineStart) this._addLog('warn', 'margin-inline-start 不支持（Chrome 87+ / Firefox 66+ / Safari 15+，jsdom 不模拟）');
    if (!caps.dirPseudo) this._addLog('warn', ':dir() 伪类不支持或 jsdom 未识别（Chrome 120+ / Firefox 49+ / Safari 17.4+）');
    if (!caps.subgrid) this._addLog('warn', 'grid-template-columns: subgrid 不支持（Chrome 117+ / Firefox 71+ / Safari 16+）');
    if (!caps.masonry) this._addLog('warn', 'grid-template-rows: masonry 不支持（仅 Firefox 实验性，需 flag）');
  ...
  _caps(): any {
    ...
    const sup = (p: any,v: any) => {
      try { return cssSupports && CSS.supports(p, v); } catch { return false; }
    };
    const supSel = (sel: any) => {
      try { return cssSupports && CSS.supports(`selector(${sel})`); }
      catch { return false; }
    };
    return {
      cssSupports,
  ...
  _showLogicalPropsCaps(): void {
    const caps = this._caps();
    this.setState({ logicalPropsInfo:
      '===== CSS 逻辑属性能力检测 =====\n\n' +
  ...
  _showWritingDirectionMapping(): void {
    const caps = this._caps();
    this.setState({ logicalPropsInfo:
      '===== 逻辑属性随 writing-mode / direction 自动适配 =====\n\n' +
  ...
  _renderCard1(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._capTags([['margin-inline-start', caps.marginInlineStart], ['inset', caps.inset]]),
        h(Tag, { color: 'primary' }, 'inline / block'),
        h(Tag, { color: 'info' }, '流相对')),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '物理属性 vs 逻辑属性：margin-left vs margin-inline-start、padding-top vs padding-block-start、width/height vs inline-size/block-size、top/left vs inset-block-start/inset-inline-start。inset: 0 = top/right/bottom/left:0；inset-inline/inset-block 逻辑简写。逻辑属性随 writing-mode 和 direction 自动适配：horizontal-tb+ltr 时 inline=水平左→右、block=垂直上→下；vertical-rl 时 inline=垂直上→下、block=水平右→左。Chrome 87+、Firefox 66+、Safari 15+，CSS.supports(\'margin-inline-start\', \'10px\') 检测。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('能力检测', { type: 'primary', size: 'sm', onClick: () => this._showLogicalPropsCaps() }),
          this._btn('物理 vs 逻辑', { size: 'sm', onClick: () => this._showPhysicalVsLogical() }),
          this._btn('inset 简写', { size: 'sm', onClick: () => this._showInsetShorthand() }),
          this._btn('writing-mode/direction 映射', { size: 'sm', onClick: () => this._showWritingDirectionMapping() })),
  ...
  _showRtlAutoFlip(): void {
    const caps = this._caps();
    this.setState({ directionAwareInfo:
      '===== direction: rtl 下逻辑属性自动翻转 =====\n\n' +
      '【direction 属性】\n' +
  ...
  _showUnicodeBidi(): void {
    this.setState({ directionAwareInfo:
      '===== unicode-bidi 与 direction 协同处理双向文本 =====\n\n' +
      '【双向文本问题】\n' +
  ...
  _renderCard2(): Node | string {
    ...
    const card = new Card({
      title: '2. 方向感知布局',
      extra: h('div', { class: 'flex gap-xs' },
        ...this._capTags([[':dir()', caps.dirPseudo], ['margin-inline-start', caps.marginInlineStart]]),
        h(Tag, { color: 'primary' }, 'direction: rtl'),
        h(Tag, { color: 'warning' }, '双向文本')),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'direction: rtl（阿拉伯语/希伯来语）下逻辑属性自动翻转：margin-inline-start 在 ltr 是左边、rtl 是右边。:dir() 伪类选择器（:dir(rtl) / :dir(ltr)）选择特定方向的元素（区别于 [dir] 属性选择器，:dir 匹配计算方向含继承）。unicode-bidi 与 direction 协同处理双向文本（isolate 隔离、plaintext 自动判断）。实战：图标+文字布局用 margin-inline-end 而非 margin-right，rtl 下图标自动到右侧。vs 物理属性：物理属性 margin-right 在 rtl 下不会翻转，需手动 :dir() 判断。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('rtl 自动翻转', { type: 'primary', size: 'sm', onClick: () => this._showRtlAutoFlip() }),
          this._btn(':dir() 伪类', { size: 'sm', onClick: () => this._showDirPseudo() }),
          this._btn('unicode-bidi', { size: 'sm', onClick: () => this._showUnicodeBidi() }),
          this._btn(`切换 rtl/ltr（当前：${this._rtlDemoOn ? 'rtl' : 'ltr'}）`, { size: 'sm', onClick: () => this._toggleRtlDemo() })),
        h('div', { class: 'fs-sm text-secondary' }, '方向感知布局：'),
        h('div', { class: 'css-da-row demo mt-xs' },
          h('div', { class: 'icon' }),
          h('div', { class: 'text' }, '图标 + 文字（margin-inline-end: 8px，真实浏览器中切换 rtl 图标自动到右侧）')),
  ...
  _showWritingModeValues(): void {
    const caps = this._caps();
    this.setState({ writingModeInfo:
      '===== writing-mode 取值详解 =====\n\n' +
      '【writing-mode 属性】\n' +
  ...
  _showFlowMapping(): void {
    const caps = this._caps();
    this.setState({ writingModeInfo:
      '===== 流相对映射（Flow-Relative Mapping）详解 =====\n\n' +
  ...
  _showBlockInlineSize(): void {
    const caps = this._caps();
    this.setState({ writingModeInfo:
      '===== block-size / inline-size 尺寸属性 =====\n\n' +
      '【基础尺寸属性】\n' +
  ...
  _renderCard3(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._capTags([['writing-mode', caps.writingMode], ['block-size', caps.blockSize]]),
        h(Tag, { color: 'primary' }, 'vertical-rl'),
        h(Tag, { color: 'info' }, '流映射')),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'writing-mode: horizontal-tb（默认）/vertical-rl（竖排右到左）/vertical-lr（竖排左到右）/sideways-rl/sideways-lr。逻辑属性映射随 writing-mode 变化：vertical-rl 时 inline-size=高度、block-size=宽度。block-size/inline-size 是块方向尺寸/行内方向尺寸，配合 min-block-size/min-inline-size/max-block-size/max-inline-size。实战：竖排卡片布局用 block-size 控制水平宽度，一套逻辑属性样式适配横/竖排。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('writing-mode 取值', { type: 'primary', size: 'sm', onClick: () => this._showWritingModeValues() }),
          this._btn('流相对映射', { size: 'sm', onClick: () => this._showFlowMapping() }),
          this._btn('block-size/inline-size', { size: 'sm', onClick: () => this._showBlockInlineSize() }),
          this._btn(`切换 writing-mode（当前：${this._writingMode}）`, { size: 'sm', onClick: () => this._toggleWritingMode() })),
        h('div', { class: 'fs-sm text-secondary' }, 'writing-mode 演示：'),
        h('div', { class: 'css-wm-stage mt-xs' },
          h('div', { class: 'css-wm-card demo' }, '卡片 1'),
          h('div', { class: 'css-wm-card demo' }, '卡片 2'),
          h('div', { class: 'css-wm-card demo' }, '卡片 3')),
  ...
  _showSubgridCaps(): void {
    const caps = this._caps();
    this.setState({ subgridInfo:
      '===== CSS Subgrid 能力检测 =====\n\n' +
      `  CSS.supports                                       : ${caps.cssSupports ? 'function（可用）' : 'undefined（不可用）'}\n` +
  ...
  _showSubgridLineNames(): void {
    const caps = this._caps();
    this.setState({ subgridInfo:
      '===== Subgrid line-names 继承 =====\n\n' +
      '【父网格定义列线名】\n' +
      '  .parent {\n' +
      '    display: grid;\n' +
  ...
  _renderCard4(): Node | string {
    ...
    const card = new Card({
      title: '4. CSS Subgrid',
      extra: h('div', { class: 'flex gap-xs' },
        ...this._capTags([['subgrid', caps.subgrid]]),
        h(Tag, { color: 'primary' }, '嵌套对齐'),
        h(Tag, { color: 'info' }, 'line-names')),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'grid-template-columns: subgrid / grid-template-rows: subgrid：子网格继承父网格的轨道，解决嵌套网格对齐问题（父网格列线被子网格共享，无需重复定义）。subgrid 值只能用于嵌套的 grid 容器（display:grid 的子元素且自身也是 grid），需配合 grid-column/grid-row: span N 指定跨越轨道数。line-names 继承：subgrid 可附加自定义轨道名，子网格内可引用父网格线名。浏览器支持：Chrome 117+、Firefox 71+、Safari 16+，CSS.supports(\'grid-template-columns\', \'subgrid\') 检测。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('能力检测', { type: 'primary', size: 'sm', onClick: () => this._showSubgridCaps() }),
          this._btn('Subgrid 语法', { size: 'sm', onClick: () => this._showSubgridSyntax() }),
          this._btn('嵌套对齐问题', { size: 'sm', onClick: () => this._showNestedAlignment() }),
          this._btn('line-names 继承', { size: 'sm', onClick: () => this._showSubgridLineNames() })),
        h('div', { class: 'fs-sm text-secondary' }, 'Subgrid 嵌套演示（真实浏览器中子网格列线与父网格对齐）：'),
        h('div', { class: 'css-sg-parent mt-xs' },
          h('div', { class: 'cell' }, '父-1'),
          h('div', { class: 'cell' }, '父-2'),
          h('div', { class: 'cell' }, '父-3'),
          h('div', { class: 'css-sg-nested' },
            h('div', { class: 'sub-cell' }, '子-1（对齐父-1）'),
            h('div', { class: 'sub-cell' }, '子-2（对齐父-2）'),
            h('div', { class: 'sub-cell' }, '子-3（对齐父-3）'))),
  ...
  _showMasonryCaps(): void {
    const caps = this._caps();
    this.setState({ masonryInfo:
      '===== CSS Masonry 能力检测 =====\n\n' +
      `  CSS.supports                                    : ${caps.cssSupports ? 'function（可用）' : 'undefined（不可用）'}\n` +
  ...
  _showMasonrySyntax(): void {
    const caps = this._caps();
    this.setState({ masonryInfo:
      '===== CSS Masonry 语法（早期 vs 当前） =====\n\n' +
  ...
  _showMasonryVsFlex(): void {
    const caps = this._caps();
    this.setState({ masonryInfo:
      '===== Masonry vs Flexbox flex-wrap 对比 =====\n\n' +
  ...
  _renderCard5(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._capTags([['masonry', caps.masonry], ['align-tracks', caps.alignTracks]]),
        h(Tag, { color: 'primary' }, '瀑布流'),
        h(Tag, { color: 'warning' }, '实验性')),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'grid-template-rows: masonry：瀑布流布局，自动填充不留空隙（子元素按列填入最短列）。grid-auto-flow: masonry（早期语法，已废弃）vs grid-template-rows: masonry（当前语法）。align-tracks/justify-tracks：多轨道对齐（masonry 每列独立行轨道，需分别对齐）。vs Flexbox flex-wrap + 固定高度：masonry 支持不定高项目，列高度自动均衡。浏览器支持：仅 Firefox 实验性（需 flag），CSS.supports(\'grid-template-rows\', \'masonry\') 检测。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('能力检测', { type: 'primary', size: 'sm', onClick: () => this._showMasonryCaps() }),
          this._btn('Masonry 语法', { size: 'sm', onClick: () => this._showMasonrySyntax() }),
          this._btn('Masonry vs Flex', { size: 'sm', onClick: () => this._showMasonryVsFlex() }),
          this._btn('align-tracks/justify-tracks', { size: 'sm', onClick: () => this._showAlignTracks() })),
        h('div', { class: 'fs-sm text-secondary' }, 'Masonry 演示（真实浏览器中瀑布流，jsdom 不渲染）：'),
        h('div', { class: 'css-ms-grid mt-xs' },
          ...heights.map((hgt, i) => h('div', {
  ...
  _showBorderLogical(): void {
    const caps = this._caps();
    this.setState({ boxModelInfo:
      '===== 边框逻辑属性 =====\n\n' +
      '【边框宽度逻辑属性】\n' +
  ...
  _showMarginPaddingShorthand(): void {
    const caps = this._caps();
    this.setState({ boxModelInfo:
      '===== margin / padding 逻辑简写 =====\n\n' +
      '【margin 逻辑简写】\n' +
  ...
  _renderCard6(): Node | string {
    ...
    const card = new Card({
      title: '6. 逻辑属性与盒模型',
      extra: h('div', { class: 'flex gap-xs' },
        ...this._capTags([['border-start-start-radius', caps.borderRadiusLogical], ['margin-inline', caps.marginInline]]),
        h(Tag, { color: 'primary' }, '边框/圆角'),
        h(Tag, { color: 'info' }, '简写')),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '边框逻辑属性（border-inline-start-width/border-block-start-width/border-inline-color）；圆角逻辑属性（border-start-start-radius/border-start-end-radius/border-end-start-radius/border-end-end-radius，start-end 对应 block/inline）；轮廓逻辑属性（outline-inline-start/outline-block-start）；margin-inline/margin-block 简写（start+end）；padding-inline/padding-block 简写。实战：卡片在 rtl/竖排下边框、圆角、间距自动适配。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('边框逻辑属性', { type: 'primary', size: 'sm', onClick: () => this._showBorderLogical() }),
          this._btn('圆角逻辑属性', { size: 'sm', onClick: () => this._showRadiusLogical() }),
          this._btn('轮廓逻辑属性', { size: 'sm', onClick: () => this._showOutlineLogical() }),
          this._btn('margin/padding 简写', { size: 'sm', onClick: () => this._showMarginPaddingShorthand() })),
        h('div', { class: 'fs-sm text-secondary' }, '逻辑盒模型演示（border-inline-start / border-start-start-radius / padding-block 等）：'),
        h('div', { class: 'css-bm-card mt-xs' },
          h('div', {}, '卡片 1（ltr，border-inline-start 蓝色、对角圆角、padding-block/padding-inline）')),
        h('div', { class: 'css-bm-card rtl mt-xs' },
          h('div', {}, '卡片 2（rtl，border-inline-start 自动到右侧并变红）')),
  ...
  render(): Node {
    const s = this.state;
    return h('div', { class: 'api-lab-page css-logical-layout-page' },
      h('h2', { class: 'section-title' }, 'CSS 逻辑属性与布局'),
      h('p', { class: 'fs-sm text-secondary mb-md' },
        '本页演示 CSS 逻辑属性基础（物理 vs 逻辑、inset 简写、writing-mode/direction 自动适配）、方向感知布局（direction:rtl、:dir() 伪类、unicode-bidi）、writing-mode 与流相对映射（block-size/inline-size）、CSS Subgrid、CSS Masonry 瀑布流、逻辑盒模型（边框/圆角/轮廓/简写）。所有特性通过 CSS.supports 能力检测，不可用时仅记日志，绝不抛异常。jsdom 中 CSS 渲染不可用，所有演示通过 CSS 代码片段说明。'),
