...
interface CSSCascadeLayersPageCaps {
  css: boolean;
  supports: boolean;
  layerBase: boolean;
  layerList: boolean;
  layerNested: boolean;
  layerAnonymous: boolean;
  importLayer: boolean;
  layerBlockRule: boolean;
  layerStatementRule: boolean;
}

export interface CSSCascadeLayersPageProps extends Props {}

export interface CSSCascadeLayersPageState extends State {
  logs: LogEntry[];
  capsSummary: string;
  overviewInfo: string;
  syntaxInfo: string;
...
export class CSSCascadeLayersPage extends Page {
  declare props: CSSCascadeLayersPageProps;
  declare state: CSSCascadeLayersPageState;
  ...
  _syntaxMode!: string;
  _orderMode!: string;
  _importantMode!: string;
  _importMode!: string;
  _conditionalMode!: string;
  _patternsMode!: string;


  // —— 初始 state ——
  initialState(): CSSCascadeLayersPageState {
    return {
      logs: [],
      capsSummary: '',
  ...
  componentDidMount(): void {
    ...
    if (!f.layerBase) this._addLog('warn', '@layer base 不可用或 jsdom 未识别（CSS Cascading L5，Chrome 99+/Firefox 97+/Safari 15.4+ 支持）');
    if (!f.layerList) this._addLog('warn', '@layer 多层声明语法不可用或 jsdom 未识别');
    if (!f.importLayer) this._addLog('warn', '@import url() layer(name) 不可用或 jsdom 未识别');
    if (!f.layerBlockRule) this._addLog('warn', 'CSSLayerBlockRule 类型未定义（jsdom 可能不实现 CSSOM 层规则接口，真实浏览器可用 document.styleSheets 遍历层）');
    if (!f.layerStatementRule) this._addLog('warn', 'CSSLayerStatementRule 类型未定义（jsdom 可能不实现）');
  ...
  _flags(): CSSCascadeLayersPageCaps {
    const hasCSS = this._safe(() => typeof CSS !== 'undefined');
    const supportsStr = (s: any) => this._safe(() =>
      hasCSS && typeof CSS.supports === 'function' && CSS.supports(s));
    return {
      css: hasCSS,
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'CSS Cascading and Inheritance Level 5 引入 @layer 解决级联痛点：!important 滥用、选择器特异性战争、第三方库覆盖难、代码顺序依赖、CSS-in-JS 与全局 CSS 冲突。@layer 显式声明级联层，层优先级由声明顺序决定（后声明更高），同层内按特异性+源码顺序，unlayered 规则优先级最高。!important 在层中反转（低层 !important 优先级更高，保护底层库 reset）。浏览器支持：Chrome 99+/Firefox 97+/Safari 15.4+（2022 年起），老浏览器忽略 @layer 规则需 fallback 双写。CSSLayerBlockRule/CSSLayerStatementRule CSSOM 接口可遍历 document.styleSheets 查看层。'),
        ...
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.overviewInfo || '（点击「读取概述信息」查看 @layer 全景）')),
        h(Alert, {
          type: 'info',
  ...
  _renderCard2(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['@layer base', f.layerBase], ['嵌套层', f.layerNested]]),
        h(Tag, { color: 'primary' }, '4 种语法'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '@layer 四种语法：命名层块 @layer name { ... }（规则归入 name 层）、层顺序声明 @layer name1, name2, name3;（仅声明顺序不含规则，后声明优先级更高）、匿名层 @layer { ... }（一次性隔离无法引用）、嵌套层 @layer framework.components { ... }（子层优先级高于父层，用于框架内部组织）。隐式层（unlayered）即未分组规则，优先级最高（vs 普通 layered 规则），让应用代码默认覆盖框架层。同名层合并：多个 @layer base { ... } 块合并到同一 base 层按源码顺序。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ syntaxInfo: this._readSyntaxInfo() }) }),
          this._btn('命名层', { size: 'sm', disabled: !f.layerBase, onClick: () => this._setSyntaxMode('named') }),
          this._btn('层顺序', { size: 'sm', disabled: !f.layerList, onClick: () => this._setSyntaxMode('order') }),
          this._btn('匿名层', { size: 'sm', disabled: !f.layerAnonymous, onClick: () => this._setSyntaxMode('anonymous') }),
          this._btn('嵌套层', { size: 'sm', disabled: !f.layerNested, onClick: () => this._setSyntaxMode('nested') }),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.syntaxInfo || '（点击按钮切换 @layer 语法模式查看说明）')),
        h(Alert, {
          type: 'info',
  ...
  _readOrderInfo(): any {
    const f = this._flags();
    try {
      const target = this.el && (this.el as any as Element | null)?.querySelector('.order-target');
      let computed = '(未渲染)';
      if (target) {
  ...
  _setOrderMode(mode: any): void  {
    ...
    this._injectStyle('css-layer-order-dynamic', ((map as any)[(mode as any)]) || map.abc);
  ...
  _renderCard3(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['@layer base', f.layerBase]]),
        h(Tag, { color: 'primary' }, '优先级规则'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '层优先级规则：1. 声明顺序决定层优先级（后声明更高），@layer a, b, c → a < b < c；2. 同层内按特异性+源码顺序（与无层时一致）；3. unlayered 规则优先级最高（vs 普通 layered 规则），让应用代码默认覆盖框架层。嵌套层内部排序：子层优先级高于父层（framework < framework.base < framework.components），同父层子层按声明顺序。@layer 重排序不可行：层顺序由"首次声明"决定，后续声明仅补充规则不改变顺序，调整需修改文件顶部 @layer 声明。unlayered 视为"匿名层"优先级高于所有显式层。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ orderInfo: this._readOrderInfo() }) }),
          this._btn('a<b<c', { size: 'sm', disabled: !f.layerBase, onClick: () => this._setOrderMode('abc') }),
          this._btn('重排序说明', { size: 'sm', disabled: !f.layerBase, onClick: () => this._setOrderMode('cba') }),
        ),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '层顺序演示（预期紫色 #6366f1，unlayered 优先级最高）：'),
        h('div', { class: 'layer-stage' },
          h('div', { class: 'layer-demo-box order-target' },
            '我的背景色 = unlayered 紫色 #6366f1（优先级最高，覆盖 layer-a/b/c）',
          ),
          h('div', { class: 'fs-sm text-secondary', style: { marginTop: '8px' } },
            'layer-a 红 < layer-b 绿 < layer-c 蓝（layered 最高）< unlayered 紫（最高）',
          ),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.orderInfo || '（点击按钮查看层顺序与优先级完整说明）')),
        h(Alert, {
          type: 'info',
  ...
  _readImportantInfo(): any {
    const f = this._flags();
    try {
      const target = this.el && (this.el as any as Element | null)?.querySelector('.imp-target');
      let computed = '(未渲染)';
      if (target) {
        computed = window.getComputedStyle(target).getPropertyValue('background-color') || '(空)';
      }
  ...
  _setImportantMode(mode: any): void  {
  ...
  _renderCard4(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['@layer base', f.layerBase]]),
        h(Tag, { color: 'warning' }, '!important 反转'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '!important 在 @layer 中优先级反转：普通规则下层优先级越高越优先（layerA < layerB），!important 规则下层优先级越低越优先（layerA !important > layerB !important，反转！）。原因：允许底层库（reset）用 !important 保护关键样式不被上层应用覆盖。与 specificity 解耦：层优先级（反转后）决定 !important 胜负，特异性仅在同层内起作用。完整优先级：普通规则按层顺序（layerA < ... < unlayered）< !important 规则反转层顺序（unlayered !important < ... < layerA !important）。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ importantInfo: this._readImportantInfo() }) }),
          this._btn('normal', { size: 'sm', disabled: !f.layerBase, onClick: () => this._setImportantMode('normal') }),
          this._btn('反转演示', { size: 'sm', disabled: !f.layerBase, onClick: () => this._setImportantMode('reversed') }),
        ),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '!important 反转演示（预期红色 #ef4444，imp-low !important 反转后最高）：'),
        h('div', { class: 'layer-stage' },
          h('div', { class: 'layer-demo-box imp-target' },
            '我的背景色 = 红色 #ef4444（imp-low !important 反转后优先级最高）',
          ),
          h('div', { class: 'fs-sm text-secondary', style: { marginTop: '8px' } },
            'imp-low !important（红，反转后最高）> imp-high !important（蓝）> unlayered 普通（紫，被 !important 覆盖）',
          ),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.importantInfo || '（点击按钮查看 !important 反转完整说明）')),
        h(Alert, {
          type: 'warning',
  ...
  _renderCard5(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['@import layer()', f.importLayer]]),
        h(Tag, { color: 'primary' }, '框架集成'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '@import url("framework.css") layer(name) 将第三方 CSS 强制归入特定层，便于控制优先级。问题：第三方 CSS（Tailwind/Bootstrap）默认 unlayered 优先级最高，应用样式难覆盖；解决：@import "tailwind.css" layer(vendor) 归入 vendor 层优先级低于应用层。Tailwind v3+ 推荐 @import "tailwindcss" layer(tailwind)，Tailwind v4 内置 @layer base/components/utilities。@import 顺序约束：必须在文件顶部（除 @charset 和 @layer 声明外），其他规则后 @import 被浏览器忽略。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ importInfo: this._readImportInfo() }) }),
          this._btn('Tailwind', { size: 'sm', disabled: !f.importLayer, onClick: () => this._setImportMode('tailwind') }),
          this._btn('Bootstrap', { size: 'sm', disabled: !f.importLayer, onClick: () => this._setImportMode('bootstrap') }),
          this._btn('多框架', { size: 'sm', disabled: !f.importLayer, onClick: () => this._setImportMode('multi') }),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.importInfo || '（点击按钮查看 @import layer 完整说明）')),
        h(Alert, {
          type: 'info',
  ...
  _setConditionalMode(mode: any): void  {
  ...
  _renderCard6(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['@layer base', f.layerBase]]),
        h(Tag, { color: 'primary' }, '条件层'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'cascade-layer() 条件检测：@supports (cascade-layer: base) { @layer base { ... } } 浏览器支持时执行（CSS.supports(\'@layer base\') 同义）。@media 嵌套 @layer：@media (min-width: 600px) { @layer responsive { ... } } 媒体查询内声明层仅匹配时生效，用于响应式层。动态层优先级：层顺序由首次声明决定，但 @media 可动态启用/禁用层内规则（如 mobile/desktop 不同视口启用不同层避免冲突）。与容器查询结合：@container (min-width: 400px) { @layer card { ... } } 组件级响应式层。@supports 检测 + fallback 双写保证兼容性。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ conditionalInfo: this._readConditionalInfo() }) }),
          this._btn('@supports', { size: 'sm', disabled: !f.layerBase, onClick: () => this._setConditionalMode('supports') }),
          this._btn('@media', { size: 'sm', disabled: !f.layerBase, onClick: () => this._setConditionalMode('media') }),
          this._btn('@container', { size: 'sm', disabled: !f.layerBase, onClick: () => this._setConditionalMode('container') }),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.conditionalInfo || '（点击按钮查看 cascade-layer 条件完整说明）')),
        h(Alert, {
          type: 'info',
  ...
  _readPatternsInfo(): any {
    const f = this._flags();
    try {
      const box = this.el && (this.el as any as Element | null)?.querySelector('.arch-box');
  ...
  _setPatternsMode(mode: any): void  {
  ...
  _renderCard7(): Node | string {
    ...
    const card = new Card({
      ...
      children: [
        ...
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ patternsInfo: this._readPatternsInfo() }) }),
          this._btn('四层架构', { size: 'sm', disabled: !f.layerBase, onClick: () => this._setPatternsMode('four-layer') }),
          this._btn('Tailwind', { size: 'sm', disabled: !f.layerBase, onClick: () => this._setPatternsMode('tailwind') }),
          this._btn('CSS-in-JS', { size: 'sm', disabled: !f.layerBase, onClick: () => this._setPatternsMode('css-in-js') }),
          this._btn('降级第三方', { size: 'sm', disabled: !f.layerBase, onClick: () => this._setPatternsMode('vendor') }),
        ),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '四层架构演示（预期 color=#1e40af utilities 层，border-radius=6px components 层）：'),
        h('div', { class: 'layer-stage' },
          h('div', { class: 'layer-demo-box arch-box' },
            '我是 .arch-box，样式来自 reset + framework + components + utilities 四层叠加\nutilities 层 color=#1e40af font-weight=600 优先级最高（layered）',
          ),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '600px', overflow: 'auto' } },
          h('code', {}, s.patternsInfo || '（点击按钮查看 5 大架构模式完整代码）')),
        h(Alert, {
          type: 'success',
  ...
  _renderCard8(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['@layer', f.layerBase], ['CSSLayerBlockRule', f.layerBlockRule]]),
        h(Tag, { color: 'warning' }, '8 大陷阱'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '八大陷阱与最佳实践：1. 层命名约定（采用 reset→framework→components→utilities 行业通用命名）；2. 过度细分层（3-6 层为宜，嵌套层用于框架内部）；3. CSS Modules 互操作（配置 generateLayerName 归层或手动 @layer 包裹）；4. DevTools 查看层顺序（Chrome 96+ Styles 面板分组，document.styleSheets 遍历 CSSLayerBlockRule）；5. 兼容性 fallback（@supports cascade-layer() 检测 + 双写规则）；6. 与 @scope 关系（@layer 控制优先级，@scope 控制作用域，正交可组合）；7. @import 顺序约束（必须文件顶部，其他规则后静默失败）；8. unlayered 意外覆盖（应用代码归 @layer app 避免意外）。10 条最佳实践清单。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
  ...
  render(): Node {
    const s = this.state;
    return h('div', { class: 'api-lab-page css-cascade-layers-page' },
      h('h2', { class: 'section-title' }, 'CSS Cascade Layers @layer 深度实验室'),
      h('p', { class: 'fs-sm text-secondary mb-md' },
        '本页演示 CSS Cascading and Inheritance Level 5 的 @layer 级联层：@layer 声明语法（命名层/层顺序/匿名层/嵌套层）、层顺序与优先级（声明顺序/unlayered 最高/嵌套排序）、!important 在层中的反转（低层 !important 优先级更高保护底层库）、@import 与 layer（第三方 CSS 归层/Tailwind 集成）、cascade-layer() 条件（@supports/@media/容器查询）、5 大实战架构模式（四层架构/Tailwind/CSS-in-JS/BEM/降级）、8 大陷阱与最佳实践。所有特性通过 typeof / CSS.supports 能力检测，不可用时仅记日志，绝不抛异常。'),
