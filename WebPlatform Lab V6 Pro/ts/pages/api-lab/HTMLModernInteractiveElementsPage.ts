...
interface HTMLModernInteractiveElementsPageState extends State {
  logs: any;
  capsSummary: any;
  detailsInfo: any;
  searchInfo: any;
...
export class HTMLModernInteractiveElementsPage extends Page {
  declare state: HTMLModernInteractiveElementsPageState;
  _dynamicStyles!: any[];
  _inited!: boolean;
  _internals!: any;
  ...
  componentDidMount() {
    ...
    if (!f.detailsContent) this._addLog('warn', '::details-content 不可用（Chrome 129+ 才支持）');
    if (!f.search) this._addLog('warn', '<search> 元素不可用（Chrome 124+/Safari 17.4+ 才支持）');
    if (!f.modal) this._addLog('warn', ':modal 伪类不可用（Chrome 111+ 才支持）');
    if (!f.permission) this._addLog('warn', '<permission> 元素不可用（实验性，仅 Chrome Canary）');
  }
  ...
  _renderCard1() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['<details>', f.details], ['::details-content', f.detailsContent], ['::marker', f.detailsMarker]]),
        h(Tag, { color: 'primary' }, '零 JS 折叠'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '<details>/<summary> 原生折叠面板零 JS 即可用；::details-content（Chrome 129+）选中展开内容区域可独立样式化（padding/background/动画）；::marker 自定义三角箭头；name 属性（Chrome 120+）实现互斥手风琴（同 name 仅一个展开，替代手写 JS）；toggle 事件监听展开状态变化（e.newState/e.oldState）。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行 <details> 演示', { type: 'primary', size: 'sm', onClick: () => this._runDetailsDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.detailsInfo || '（点击按钮查看 <details>/::details-content 完整用法）')),
      ],
    });
  ...
  _renderCard2() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['<search>', f.search]]),
        h(Tag, { color: 'primary' }, 'landmark'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '<search> 是 2024 新增的语义元素，用于搜索/过滤区域，自动获得 role="search" landmark 语义，屏幕阅读器提供快捷键跳转。替代传统 <form role="search"> ARIA 补丁。可包含表单、输入框、筛选控件。Chrome 124+/Safari 17.4+/Firefox 124+ 支持。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行 <search> 演示', { type: 'primary', size: 'sm', onClick: () => this._runSearchDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '440px', overflow: 'auto' } },
          h('code', {}, s.searchInfo || '（点击按钮查看 <search> 完整用法）')),
      ],
    });
    this.registerChild(card);
    return card.render();
  ...
  _renderCard3() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['<dialog>', f.dialog], [':modal', f.modal], ['::backdrop', f.backdrop]]),
        h(Tag, { color: 'primary' }, '原生模态'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '<dialog> 原生模态/非模态对话框：show() 非模态、showModal() 模态（top-layer + 焦点陷阱 + ESC 关闭 + ::backdrop 遮罩）、open 属性声明式。:modal 伪类（Chrome 111+）区分模态/非模态样式。form method="dialog" 表单提交关闭对话框并把按钮 value 设为 returnValue。cancel 事件可阻止 ESC 关闭（有未保存内容时提示）。配合 @starting-style + overlay allow-discrete 实现打开/关闭过渡动画。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行 <dialog> 演示', { type: 'primary', size: 'sm', onClick: () => this._runDialogDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '560px', overflow: 'auto' } },
          h('code', {}, s.dialogInfo || '（点击按钮查看 <dialog>/:modal 完整用法）')),
      ],
    });
    this.registerChild(card);
    return card.render();
  ...
  _renderCard4() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['::target-text', f.targetText], [':target', f.target], [':target-within', f.targetWithin]]),
        h(Tag, { color: 'primary' }, '深链接'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '文本片段（Text Fragment）让 URL 锚点定位到「元素内的某段文字」：#:~:text=关键词 自动滚动并高亮。::target-text 伪元素（Chrome 89+）控制高亮样式（默认黄色背景，可改色/加动画）。支持 prefix/suffix 过滤、textStart-textEnd 范围选择。FragmentDirective API（实验性）程序化生成链接。:target 反映锚点目标元素，:target-within（提案）反映包含目标的祖先。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行 ::target-text 演示', { type: 'primary', size: 'sm', onClick: () => this._runTargetTextDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '540px', overflow: 'auto' } },
          h('code', {}, s.targetTextInfo || '（点击按钮查看 ::target-text 完整用法）')),
      ],
    });
    this.registerChild(card);
    return card.render();
  ...
  _renderCard5() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['<permission>', f.permission]]),
        h(Tag, { color: 'primary' }, '实验性'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '<permission> 是实验性声明式权限按钮：type 属性指定权限（geolocation/camera/microphone/notifications），浏览器自动渲染按钮、触发原生权限对话框、反映权限状态（granted/denied/prompt）。:state() 伪类响应状态切换样式。替代传统「按钮 + Permissions API + try/catch」的命令式方案。截至 2025 仅 Chrome Canary behind flag，需 JS 检测降级。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行 <permission> 演示', { type: 'primary', size: 'sm', onClick: () => this._runPermissionDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '540px', overflow: 'auto' } },
          h('code', {}, s.permissionInfo || '（点击按钮查看 <permission> 完整用法）')),
      ],
    });
    this.registerChild(card);
    return card.render();
  ...
  _renderCard6() {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '统一反映元素「交互状态」的伪类：:modal（showModal 打开的 dialog/popover）、:popover-open（打开的 popover）、:fullscreen（requestFullscreen 全屏）、:picture-in-picture（画中画）、:target（URL 锚点目标）、:defined（自定义元素升级完成，配合 :not() 做加载前占位）、:state()（自定义元素 ElementInternals.states 内部状态，与样式解耦）。可用 :is() 组合多状态响应。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行状态伪类演示', { type: 'primary', size: 'sm', onClick: () => this._runStatePseudoDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '600px', overflow: 'auto' } },
          h('code', {}, s.statePseudoInfo || '（点击按钮查看状态伪类完整用法）')),
      ],
    });
    this.registerChild(card);
    return card.render();
