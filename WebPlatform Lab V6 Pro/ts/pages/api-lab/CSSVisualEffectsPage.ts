...
interface CSSVisualEffectsPageState extends State {
  logs: LogEntry[];
  capsSummary: string;
  aspectRatio: string;
  ...
  objectPositionIndex: number;
  objectFitInfo: string;
  shape: string;
  shapeKind: string;
  shapeIndex: number;
  shapeMargin: number;
...
export class CSSVisualEffectsPage extends Page {
  declare state: CSSVisualEffectsPageState;
  _inited: boolean = false;
  _injectedStyles: any[] = [];


  // —— 初始 state ——
  initialState(): CSSVisualEffectsPageState {
    return {
      logs: [],
      capsSummary: '',
...
.bg-box {
  ...
  _changeShapeThreshold(delta: any) {
