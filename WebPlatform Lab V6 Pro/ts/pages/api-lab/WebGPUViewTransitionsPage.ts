...
interface WebGPUViewTransitionsPageCaps {
  gpu: boolean;
...
export interface WebGPUViewTransitionsPageProps extends Props {}

export interface WebGPUViewTransitionsPageState extends State {
  logs: LogEntry[];
  capsSummary: string;
  adapterInfo: string;
  resourceInfo: string;
...
export class WebGPUViewTransitionsPage extends Page {
  declare props: WebGPUViewTransitionsPageProps;
  declare state: WebGPUViewTransitionsPageState;
  ...
  _shaderModule!: any;
  _pipeline!: any;
  _bindGroupLayout!: any;
  _bindGroup!: any;
  _errorHandler!: any;
  _transitionEl!: any;
  _popover!: any;
  _popoverToggleHandler!: any;
  _dialog!: any;
  _dialogCloseHandler!: any;
  _dialogCancelHandler!: any;


  // —— 初始 state ——
  initialState(): WebGPUViewTransitionsPageState {
    return {
      logs: [],
      capsSummary: '',
  ...
  componentWillUnmount(): void {
    ...
    const detach = (target: any,type: any,handler: any) => {
      if (target && handler) { try { target.removeEventListener(type, handler); } catch { /* noop */ } }
    };
    const remove = (el: any) => { if (el && el.parentNode) el.parentNode.removeChild(el); };
  ...
  _caps(): WebGPUViewTransitionsPageCaps {
    const hasGPU = typeof navigator !== 'undefined' && !!navigator.gpu;
    const hasDialog = typeof HTMLDialogElement !== 'undefined';
    const tryProbe = (fn: any) => { try { return fn(); } catch { return false; } };
...
// ... 用完调用 device.destroy()`)),
  ...
  _createBuffer(): void {
    ...
    try {
      ...
      const bufUsageLine = ['VERTEX', 'INDEX', 'UNIFORM', 'STORAGE', 'COPY_SRC', 'COPY_DST', 'MAP_READ', 'MAP_WRITE']
        .map((k) => `${k}=${((GPUBufferUsage as any)[(k as any)])}`).join(' / ');
      this.setState({
        resourceInfo:
  ...
  _createTexture(): void {
    ...
    try {
      ...
      const texUsageLine = ['RENDER_ATTACHMENT', 'COPY_SRC', 'COPY_DST', 'TEXTURE_BINDING', 'STORAGE_BINDING']
        .map((k) => `${k}=${((GPUTextureUsage as any)[(k as any)])}`).join(' / ');
      this.setState({
        resourceInfo:
  ...
  _createPipeline(): void {
    ...
    try {
      ...
      catch (e: any) { bglLine = `getBindGroupLayout(0) 抛错：${e.message}\n`; }
      this.setState({
        resourceInfo:
          (this.state.resourceInfo || '') + '\n\n' +
  ...
  _encodeAndSubmit(): void {
    ...
    try {
      ...
      if (typeof this._device.queue.onSubmittedWorkDone === 'function') {
        ...
        }).catch((e: any) => this._addLog('warn', `onSubmittedWorkDone reject：${e.message}`));
        doneLine = 'queue.onSubmittedWorkDone() → Promise（已发起，resolve 后记日志）\n';
      }
      this._device.popErrorScope().then((err: any) => {
      ...
      }).catch((e: any) => this._addLog('warn', `popErrorScope reject：${e.message}`));
      this.setState({
        renderInfo:
          `完整渲染流程（已 submit）：\n` +
    ...
    try {
      ...
      this.setState({
        renderInfo:
          ...
          `写入 ${(data as any).byteLength} 字节（6 个 float32 = 3 个二维顶点）${texLine}\n\n` +
          `说明：writeBuffer / writeTexture 是 queue 上的便捷方法，会在 submit 之前按队列顺序执行，省去 mapAsync 的同步开销。`,
      });
      this._addLog('queue', `queue.writeBuffer 写入 ${(data as any).byteLength}B；${this._texture ? 'writeTexture ✓' : '无 texture'}`);
    } catch (err: any) {
...
device.popErrorScope().then(err => err && console.error(err));`)),
  ...
  async _startViewTransition() {
    ...
    try {
      ...
      const ms = (k: any) => (((timing as any)[(k as any)]) = performance.now() - t0).toFixed(1);
      ...
      transition.finished.then(() => {
        ...
        this.setState({
          transitionInfo:
            ...
            `updateCallbackDone resolve：DOM 已更新（耗时 ${(timing as any).update?.toFixed(1)}ms）\n` +
            `ready resolve：伪元素就绪，开始动画（耗时 ${(timing as any).ready?.toFixed(1)}ms）\n` +
            `finished resolve：过渡完成（耗时 ${(timing as any).finished?.toFixed(1)}ms）\n\n` +
...
.hero { view-transition-name: hero; }   /* 命名元素独立过渡 */`)),
  ...
  _createPopover(): void {
    ...
    try {
      ...
      this._popoverToggleHandler = (ev: any) => {
