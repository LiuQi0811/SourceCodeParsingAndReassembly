...
export interface ObserverPageProps extends Props {}
...
export interface ObserverPageState extends State {
  intersectionLog: ObserverLog[];
  mutationLog: ObserverLog[];
  resizeLog: ObserverLog[];
...
export class ObserverPage extends Page {
  ...
  _addIntersection(entry: IntersectionObserverEntry): void {
    this.setState({
  ...
  _setupIntersection(): void {
    const target = (this.$('#intersection-target') as any);
    if (!target) return;
    this._intersectionObserver = new IntersectionObserver((entries: IntersectionObserverEntry[]) => {
      (entries || []).forEach((e: IntersectionObserverEntry) => this._addIntersection(e));
    }, { threshold: [0, 0.25, 0.5, 0.75, 1] });
    this._intersectionObserver.observe(target);
  }

  _setupMutation(): void {
    const target = (this.$('#mutation-target') as any);
    if (!target) return;
    this._mutationObserver = new MutationObserver((mutations: MutationRecord[]) => {
      const logs: string[] = [];
      for (const m of (mutations || [])) {
        if (m.type === 'childList') {
          m.addedNodes.forEach((n: Node) => logs.push(`+ ${n.nodeName}`));
          m.removedNodes.forEach((n: Node) => logs.push(`- ${n.nodeName}`));
        } else if (m.type === 'attributes') {
          logs.push(`attr ${m.attributeName} = ${(m.target as Element).getAttribute(m.attributeName || '')}`);
      ...
      this.setState((s: any) => ({
        ...
        mutationLog: [...s.mutationLog, ...logs.map((t: string) => ({ time: formatTime(), text: t }))].slice(-20),
      }));
    });
  ...
  _setupResize(): void {
    const target = (this.$('#resize-target') as any);
    if (!target) return;
    this._resizeObserver = new ResizeObserver((entries: ResizeObserverEntry[]) => {
      for (const e of (entries || [])) {
        const cr = e.contentRect;
        const borderBoxSize = (e as any).borderBoxSize?.[0]?.inlineSize;
        this.setState({
          boxWidth: Math.round(cr.width),
          resizeLog: [...this.state.resizeLog, {
            time: formatTime(),
            text: `${Math.round(cr.width)} × ${Math.round(cr.height)}  (borderBox: ${borderBoxSize ? (borderBoxSize as any).toFixed(0) : '?'}px)`,
          }].slice(-15),
        });
      }
    });
  ...
  _mutate(type: string): void {
    const target = this.$<HTMLElement>('#mutation-target');
    if (!target) return;
    if (type === 'add') {
  ...
  _resize(delta: number): void {
    const target = this.$<HTMLElement>('#resize-target');
    if (!target) return;
    ...
    return [
      ...
      h(Card, { title: 'IntersectionObserver', extra: h(Tag, { color: 'primary' }, `${this.state.intersectionRatio}%`) },
        ...
        h('div', { class: 'log-panel mt-sm', style: { maxHeight: '120px' } },
          ...(this.state.intersectionLog || []).map((l: ObserverLog) => h('div', { class: 'log-panel__line' },
      ...
      h(Card, { title: 'MutationObserver', extra: h(Tag, { color: 'success' }, `${this.state.mutationCount} 次`) },
        ...
        h('div', { class: 'log-panel mt-sm', style: { maxHeight: '120px' } },
          ...(this.state.mutationLog || []).map((l: ObserverLog) => h('div', { class: 'log-panel__line' },
            h('span', { class: 'log-panel__time' }, l.time),
      ...
      h(Card, { title: 'ResizeObserver', extra: h(Tag, { color: 'warning' }, `宽 ${this.state.boxWidth}px`) },
        ...
        h('input', {
          ...
          oninput: (e: Event) => {
            const target = this.$<HTMLElement>('#resize-target');
            if (target) target.style.width = (e.target as HTMLInputElement).value + 'px';
          },
        }),
        h('div', {
          id: 'resize-target',
        ...
        h('div', { class: 'log-panel mt-sm', style: { maxHeight: '120px' } },
          ...(this.state.resizeLog || []).map((l: ObserverLog) => h('div', { class: 'log-panel__line' },
            h('span', { class: 'log-panel__time' }, l.time),
