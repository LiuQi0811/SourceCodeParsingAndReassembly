...
interface CodecSupportResult extends CodecItem {
  supported: boolean;
  error?: string;
}

const VIDEO_CODECS: CodecItem[] = [
...
interface WebglCompareRow {
  feature: string;
  v1: string;
  v2: string;
}

const WEBGL_COMPARE: WebglCompareRow[] = [
...
export interface AdvancedGraphicsPageProps extends Props {}

export interface AdvancedGraphicsLog {
  type: string;
  content: string;
  time: string;
}

export interface AdvancedGraphicsPageState extends State {
  logs: AdvancedGraphicsLog[];
  // Card 1: WebGL2
  ...
  webgpuDeviceInfo: string;
  webgpuBufferCreated: boolean;
  // Card 3: CSS Houdini
  paintWorkletSupported: boolean;
  paintWorkletAdded: boolean;
  registerPropertySupported: boolean;
  registerPropertyDone: boolean;
  // Card 4: WebCodecs
  webcodecsSupported: boolean;
  webcodecsVideo: CodecSupportResult[];
  webcodecsAudio: CodecSupportResult[];
  // Card 5: Scroll-driven
  scrollAnimSupported: boolean;
}

export class AdvancedGraphicsPage extends Page {
  declare props: AdvancedGraphicsPageProps;
  declare state: AdvancedGraphicsPageState;
  _inited: boolean = false;
  _webgl2Raf: number | null = null;
  _webgl2ShaderErr: boolean = false;
  _gpuDevice: any | null = null;
  _demoStyleEl: HTMLStyleElement | null = null;
  _paintWorkletUrl: string | null = null;
  _paintAdded: boolean = false;
  _regPropDone: boolean = false;
  _savedScrollTop: number | null = null;
  _lastLoggedScroll: number | null = null;

  initialState(): AdvancedGraphicsPageState {
    return {
      logs: [],
      // Card 1: WebGL2
  ...
  _compileWebGL2Shader(gl: WebGL2RenderingContext, type: number, src: string): WebGLShader {
  ...
  _drawWebGL2(): void {
    ...
    const canvas = this.$<HTMLCanvasElement>('.webgl2-canvas');
    if (!canvas) return;
    const gl = canvas.getContext?.('webgl2');
    if (!gl) return;
  ...
  async _initHoudini(): Promise<void> {
  ...
  async _initWebCodecs(): Promise<void> {
    ...
    const videoResults: CodecSupportResult[] = [];
    ...
    // 检测音频编解码器支持
    const audioResults: CodecSupportResult[] = [];
    for (const { codec, label } of AUDIO_CODECS) {
      try {
        const result = await AudioDecoder.isConfigSupported({ codec } as AudioDecoderConfig);
  ...
  _bindScrollListener(): void {
    const c = this.$<HTMLElement>('.scroll-anim-container');
    if (!c) return;
    // 节流：避免滚动时频繁触发 setState 导致容器重置
    const handler = throttle((): void => {
  ...
  _refreshScrollText(): void {
    const c = this.$<HTMLElement>('.scroll-anim-container');
    const pos = this.$<HTMLElement>('.scroll-position');
    if (c && pos) {
      const pct = ((c.scrollTop / (c.scrollHeight - c.clientHeight || 1)) * 100).toFixed(0);
    ...
    return [
      ...
      h(Card, {
        ...
        h('div', { class: 'flex flex-col gap-sm' },
          ...
          s.webgpuAdapterInfo ? h('div', { class: 'fs-sm text-secondary' }, `Adapter: ${s.webgpuAdapterInfo}`) : null as any,
          s.webgpuDeviceInfo ? h('div', { class: 'fs-sm text-secondary' }, `Device limits: ${s.webgpuDeviceInfo}`) : null as any,
      ...
      h(Card, {
        ...
        h('div', { class: 'flex flex-col gap-sm' },
          h('p', { class: 'fs-sm text-secondary' },
            '(CSS as any).paintWorklet.addModule(url) 注册 Paint Worklet（此处用 Blob URL 内联 worklet 代码），CSS.registerProperty 注册自定义属性（Properties & Values API），元素背景使用 paint(worklet-name) 调用。能力检测：typeof (CSS as any).paintWorklet。layoutWorklet / animationWorklet 同理。'),
          h('div', { class: 'flex gap-sm flex-wrap' },
