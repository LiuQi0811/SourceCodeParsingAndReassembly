...
function _writeStr(view: any,  offset: any,  str: any) {
  for (let i = 0; i < str.length; i++) view.setUint8(offset + i, str.charCodeAt(i));
}

/** 生成一段正弦波 16-bit PCM 单声道 WAV 的 ArrayBuffer */
function generateSineWav(sampleRate: any,  duration: any,  freq: any) {
...
interface AdvancedWebAudioPageState extends State {
  logs: any;
  audioSupported: any;
  audioCtxInfo: any;
  audioState: any;
...
export class AdvancedWebAudioPage extends Page {
  declare state: AdvancedWebAudioPageState;
  _analyser!: any;
  _analyserRaf!: any;
  _audioCtx!: any | null;
  _autoTimer!: any;
  ...
  componentWillUnmount() {
    ...
    if (this._workletUrl) {
      try { URL.revokeObjectURL(this._workletUrl); } catch { /* noop */ }
  ...
  _bindControls() {
    const freq = (this.$('.freq-slider') as any);
    if (freq) {
      freq.value = this._freq ?? 440;
      this.on(freq, 'input', () => {
        this._freq = Number(freq.value);
        const label = (this.$('.freq-val') as any);
        if (label) label.textContent = `${this._freq} Hz`;
      });
    }
    const gain = (this.$('.gain-slider') as any);
    if (gain) {
      gain.value = this._gain ?? 0.3;
      this.on(gain, 'input', () => {
        this._gain = Number(gain.value);
        const label = (this.$('.gain-val') as any);
        if (label) label.textContent = `${Math.round(this._gain * 100)}%`;
      });
    }
    const filter = (this.$('.filter-select') as any);
    if (filter) {
    ...
    const osc = (this.$('.osc-select') as any);
    if (osc) {
  ...
  _resumeCtx() {
    if (!this._audioCtx) { this._addLog('error', 'AudioContext 尚未创建'); return; }
    if (typeof this._audioCtx.resume !== 'function') { this._addLog('error', 'AudioContext.resume 不可用'); return; }
    ...
    }).catch((err: any) => this._addLog('error', `resume 失败：${errInfo(err).message}`));
  }

  _suspendCtx() {
    if (!this._audioCtx) { this._addLog('error', 'AudioContext 尚未创建'); return; }
    if (typeof this._audioCtx.suspend !== 'function') { this._addLog('error', 'AudioContext.suspend 不可用'); return; }
    this._audioCtx.suspend().then(() => {
      this.setState({ audioState: this._audioCtx.state });
      this._addLog('info', `suspend() → state=${this._audioCtx.state}`);
    }).catch((err: any) => this._addLog('error', `suspend 失败：${errInfo(err).message}`));
  }

  _closeCtx() {
    if (!this._audioCtx) { this._addLog('error', 'AudioContext 尚未创建'); return; }
    if (typeof this._audioCtx.close !== 'function') { this._addLog('error', 'AudioContext.close 不可用'); return; }
    this._audioCtx.close().then(() => {
      ...
      this._audioCtx = null;
    }).catch((err: any) => this._addLog('error', `close 失败：${errInfo(err).message}`));
  }
  ...
  _createOfflineCtx() {
    ...
    try {
      ...
      gain.gain.value = 0.2;
      osc.start(0);
      off.startRendering().then((buf: any) => {
        this._addLog('info', `OfflineAudioContext.startRendering() ✓ → duration=${(buf.duration as any).toFixed(2)}s, channels=${buf.numberOfChannels}`);
      }).catch((err: any) => this._addLog('error', `startRendering 失败：${err.message}`));
    } catch (err: any) {
  ...
  _playNote() {
    ...
    // 1 秒后自动断开
    setTimeout(() => {
      try { osc.disconnect(); gain.disconnect(); filter.disconnect(); } catch { /* noop */ }
    }, 1100);
  ...
  _playAutomation() {
    ...
    this._autoTimer = setTimeout(() => {
      ...
      try { osc.disconnect(); gain.disconnect(); analyser.disconnect(); } catch { /* noop */ }
      this._stopSpectrum();
  ...
  _startSpectrum() {
    this._stopSpectrum();
    const draw = () => {
      const canvas = (this.$('.analyser-canvas') as any);
  ...
  async _initWorklet() {
    ...
    try {
      ...
      if (gainParam) gainParam.value = 0.5;
  ...
  async _decodeAudio() {
    ...
    try {
      ...
      this._addLog('info', `decodeAudioData：已生成 ${(wavBuf as any).byteLength} 字节 WAV（440Hz, 0.5s, mono 16-bit）`);
      ...
      this.setState({
        decodeResult: [
          ...
          `duration = ${(audioBuf.duration as any).toFixed(3)} s`,
          `getChannelData(0)[0..1000] peak = ${(peak as any).toFixed(3)}`,
        ].join('\n'),
      });
      this._addLog('audio', `decodeAudioData ✓ → channels=${audioBuf.numberOfChannels}, length=${audioBuf.length}, duration=${(audioBuf.duration as any).toFixed(3)}s`);
    } catch (err: any) {
  ...
  _startRecognition() {
    ...
    (rec as any).onaudiostart = () => this._addLog('speech', 'audiostart：音频采集已开始');
    (rec as any).onspeechstart = () => this._addLog('speech', 'speechstart：检测到语音输入');
    (rec as any).onspeechend = () => this._addLog('speech', 'speechend：语音输入结束');
    rec.onerror = (e: any) => this._addLog('error', `SpeechRecognition error：${e.error}${e.message ? ' / ' + e.message : ''}`);
    rec.onend = () => {
  ...
  renderPage() {
    ...
    return [
      ...
      h(Card, {
        ...
        extra: h(Tag, { color: s.audioSupported === null ? 'default' : (s.audioSupported ? 'success' : 'error') },
          s.audioSupported === null ? '检测中' : (s.audioSupported ? '稳定' : '不可用')),
      },
        h('div', { class: 'flex flex-col gap-sm' },
          h('p', { class: 'fs-sm text-secondary' },
            'new AudioContext() 创建实时音频上下文；new OfflineAudioContext(numberOfChannels, length, sampleRate) 创建离线渲染上下文。state 为 running / suspended / closed，可通过 resume() / suspend() / close() 切换。destination 为最终输出节点。'),
          h('div', { class: 'flex gap-sm flex-wrap' },
            this._btn('创建 AudioContext', { type: 'primary', size: 'sm', onClick: () => this._createCtx() }),
            this._btn('resume', { size: 'sm', onClick: () => this._resumeCtx() }),
            this._btn('suspend', { size: 'sm', onClick: () => this._suspendCtx() }),
            this._btn('close', { size: 'sm', danger: true, onClick: () => this._closeCtx() }),
            this._btn('OfflineAudioContext', { size: 'sm', onClick: () => this._createOfflineCtx() }),
          ),
          h('div', { class: 'flex items-center gap-sm' },
            h('span', { class: 'fs-sm' }, '当前状态：'),
            h(Tag, { color: s.audioState === 'running' ? 'success' : (s.audioState === 'closed' ? 'error' : 'warning') }, s.audioState),
          ),
          s.audioCtxInfo && h('pre', { class: 'code-block' }, s.audioCtxInfo),
          // 节点拓扑图：Source → Effect → Destination
          h('div', { class: 'fs-sm text-secondary mt-xs' }, '节点拓扑（Source → Effect → Destination）：'),
          h('div', { style: { display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap', padding: '8px', background: 'var(--color-fill, #f5f5f5)', borderRadius: '6px' } },
            h('span', { style: { padding: '4px 10px', background: '#1677ff', color: '#fff', borderRadius: '4px', fontSize: '12px' } }, 'OscillatorNode'),
            h('span', { style: { fontSize: '14px', color: '#999' } }, '→'),
            h('span', { style: { padding: '4px 10px', background: '#52c41a', color: '#fff', borderRadius: '4px', fontSize: '12px' } }, 'GainNode'),
            h('span', { style: { fontSize: '14px', color: '#999' } }, '→'),
            h('span', { style: { padding: '4px 10px', background: '#722ed1', color: '#fff', borderRadius: '4px', fontSize: '12px' } }, 'BiquadFilterNode'),
            h('span', { style: { fontSize: '14px', color: '#999' } }, '→'),
            h('span', { style: { padding: '4px 10px', background: '#fa541c', color: '#fff', borderRadius: '4px', fontSize: '12px' } }, 'destination'),
          ),
        ),
      ),
      ...
      h(Card, {
        ...
        h('div', { class: 'flex flex-col gap-sm' },
          h('p', { class: 'fs-sm text-secondary' },
            'OscillatorNode.type（sine/square/sawtooth/triangle/custom）、frequency 与 detune（AudioParam）、start()/stop()。GainNode.gain 控制音量包络。BiquadFilterNode.type 共 8 种滤波器，含 frequency / Q / gain 参数。节点连接：source.connect(gain).connect(filter).connect(destination)。'),
          // 频率滑块
          h('div', { class: 'flex items-center gap-sm' },
            h('span', { class: 'fs-sm', style: { width: '72px' } }, '频率'),
            h('input', { class: 'freq-slider', type: 'range', min: 80, max: 2000, step: 1, style: { flex: 1 } }),
            h('span', { class: 'freq-val fs-sm fw-medium', style: { minWidth: '80px' } }, `${this._freq ?? 440} Hz`),
          ),
          // 音量滑块
          h('div', { class: 'flex items-center gap-sm' },
            h('span', { class: 'fs-sm', style: { width: '72px' } }, '音量'),
            h('input', { class: 'gain-slider', type: 'range', min: 0, max: 1, step: 0.01, style: { flex: 1 } }),
            h('span', { class: 'gain-val fs-sm fw-medium', style: { minWidth: '80px' } }, `${Math.round((this._gain ?? 0.3) * 100)}%`),
          ),
          // 振荡器类型
          h('div', { class: 'flex items-center gap-sm' },
            h('span', { class: 'fs-sm', style: { width: '72px' } }, '波形'),
            h('select', { class: 'osc-select', style: { flex: 1 } },
              ...OSC_TYPES.map((t: any) => h('option', { value: t }, t)),
            ),
          ),
          // 滤波器类型
          h('div', { class: 'flex items-center gap-sm' },
            h('span', { class: 'fs-sm', style: { width: '72px' } }, '滤波器'),
            h('select', { class: 'filter-select', style: { flex: 1 } },
              ...FILTER_TYPES.map((t: any) => h('option', { value: t }, t)),
            ),
          ),
          h('div', { class: 'flex gap-sm flex-wrap' },
            this._btn('播放音符（1秒）', { type: 'primary', size: 'sm', onClick: () => this._playNote() }),
          ),
          h('p', { class: 'fs-sm text-tertiary' }, 'OscillatorNode → GainNode → BiquadFilterNode → destination。播放 1 秒后自动 stop。'),
        ),
      ),
      ...
      h(Card, {
        ...
        h('div', { class: 'flex flex-col gap-sm' },
          h('p', { class: 'fs-sm text-secondary' },
            'AudioParam 自动化：setValueAtTime / linearRampToValueAtTime（线性）/ exponentialRampToValueAtTime（指数，不能从 0）/ setTargetAtTime（指数渐近）/ setValueCurveAtTime（曲线）/ cancelScheduledValues。AnalyserNode：fftSize（2 的幂，32-32768）、frequencyBinCount=fftSize/2、getByteFrequencyData（频域）/ getByteTimeDomainData（时域）、smoothingTimeConstant。'),
          h('div', { class: 'flex items-center gap-sm flex-wrap' },
            this._btn(s.autoPlaying ? '播放中…' : '播放 ADSR 自动化', {
              type: 'primary', size: 'sm', onClick: () => this._playAutomation(), disabled: s.autoPlaying,
            }),
            h('span', { class: 'fs-sm text-secondary' }, 'fftSize：'),
            h(Tag, { color: 'primary' }, String(s.fftSize || '—')),
            h('span', { class: 'fs-sm text-secondary' }, 'frequencyBinCount：'),
            h(Tag, { color: 'primary' }, String(s.frequencyBinCount || '—')),
          ),
          h('canvas', { class: 'analyser-canvas', width: 600, height: 140, style: { width: '100%', height: '140px', background: '#0d1117', borderRadius: '6px' } }),
          h('p', { class: 'fs-sm text-tertiary' }, 'sawtooth 220Hz → GainNode(ADSR) → AnalyserNode → destination。canvas 用 requestAnimationFrame + getByteFrequencyData 实时绘制频谱条形图。'),
        ),
      ),
      ...
      h(Card, {
        ...
        extra: h(Tag, { color: s.workletSupported === null ? 'default' : (s.workletSupported ? 'success' : 'error') },
          s.workletSupported === null ? '检测中' : (s.workletSupported ? '实验性' : '不可用')),
      },
        h('div', { class: 'flex flex-col gap-sm' },
          h('p', { class: 'fs-sm text-secondary' },
            'AudioWorklet：audioContext.audioWorklet.addModule(url) 注册 AudioWorkletProcessor，new AudioWorkletNode(ctx, name) 创建节点，parameters 为参数端口。decodeAudioData(arrayBuffer) → Promise<AudioBuffer>，AudioBuffer 含 numberOfChannels / length / sampleRate / duration / getChannelData(channel)。'),
          h('div', { class: 'flex gap-sm flex-wrap' },
            this._btn('加载 AudioWorklet', { type: 'primary', size: 'sm', onClick: () => this._initWorklet() }),
            this._btn('decodeAudioData', { size: 'sm', onClick: () => this._decodeAudio() }),
          ),
          s.workletResult && h('div', { class: 'fs-sm' },
            h('strong', {}, 'AudioWorklet：'), h('span', { class: 'text-secondary' }, s.workletResult)),
          s.decodeResult && h('div', {}, h('div', { class: 'fs-sm text-secondary mb-xs' }, 'decodeAudioData 返回的 AudioBuffer：'), h('pre', { class: 'code-block' }, s.decodeResult)),
          h('div', { class: 'fs-sm text-secondary mt-xs' }, 'AudioWorklet Processor 源码（通过 Blob URL 内联，addModule 加载）：'),
          h('pre', { class: 'code-block' }, WORKLET_SRC.trim()),
        ),
      ),
      ...
      h(Card, {
        ...
        extra: h(Tag, { color: s.speechSupported === null ? 'default' : (s.speechSupported ? 'warning' : 'error') },
          s.speechSupported === null ? '检测中' : (s.speechSupported ? '实验性' : '不可用')),
      },
        h('div', { class: 'flex flex-col gap-sm' },
          h('p', { class: 'fs-sm text-secondary' },
            'SpeechRecognition（webkitSpeechRecognition 前缀）：continuous / interimResults / lang / maxAlternatives 属性，start()/stop()/abort() 方法，audiostart / speechstart / speechend / result / error / end 事件。SpeechRecognitionResult 含 SpeechRecognitionAlternative（transcript / confidence）。SpeechGrammarList 可选。能力检测：\'SpeechRecognition\' in window || \'webkitSpeechRecognition\' in window。'),
          h('div', { class: 'flex gap-sm flex-wrap' },
            this._btn(s.speechRunning ? '停止识别' : '开始识别', {
          ...
          h('div', { class: 'flex items-center gap-sm' },
            h('span', { class: 'fs-sm' }, '识别状态：'),
            h(Tag, { color: s.speechRunning ? 'success' : 'default' }, s.speechRunning ? '运行中' : '已停止'),
            h('span', { class: 'fs-sm text-tertiary' }, 'lang=zh-CN, interimResults=true, maxAlternatives=3'),
          ),
          s.speechResults.length > 0
            ? h('div', { class: 'log-panel' },
                ...s.speechResults.map((r: any) => h('div', { class: 'log-panel__line' },
                  h('span', { class: `log-panel__tag log-panel__tag--${r.isFinal ? 'speech' : 'info'}` }, r.isFinal ? 'final' : 'interim'),
                  h('span', { class: 'log-panel__content' }, `${r.transcript}（${(r.confidence ?? 0).toFixed(2)}）`),
                )),
              )
