...
export interface AdvancedPageProps extends Props {}
...
export interface AdvancedPageState extends State {
  logs: AdvancedLog[];
  wsState: string;
  ...
  cryptoDecrypted: string;
  compressInput: string;
  compressResult: string;
}

export class AdvancedPage extends Page {
  declare props: AdvancedPageProps;
  declare state: AdvancedPageState;
  _bc: BroadcastChannel | null = null;
  _ws: WebSocket | null = null;
  _db: IDBDatabase | null = null;
  _perfObs: PerformanceObserver | null = null;

  initialState(): AdvancedPageState {
    return {
      logs: [],
      wsState: '未连接',
      wsMessages: [],
  ...
  componentDidMount(): void {
    ...
    try {
      ...
      this._bc.onmessage = (e: MessageEvent) => {
        const { from, text } = (e.data || {}) as { from: number; text: string };
    ...
    // 加载已有 IndexedDB 笔记
    this._loadNotes().catch((err: any) => this._addLog('error', `读取笔记失败：${err.message}`));

    // PerformanceObserver：监听 measure 条目
    try {
      this._perfObs = new PerformanceObserver((list: PerformanceObserverEntryList) => {
        for (const entry of list.getEntries()) {
          this._addLog('perf', `PerformanceObserver 捕获 measure「${entry.name}」= ${(entry.duration as any).toFixed(2)}ms`);
        }
      });
  ...
  _openDB(): Promise<IDBDatabase> {
    return new Promise((resolve: any, reject: any) => {
      ...
      req.onerror = (e: Event) => {
        reject(new Error(`打开数据库失败：${(e.target as IDBRequest).error?.message || 'unknown'}`));
      };
    });
  }

  async _addNote(): Promise<void> {
    ...
    try {
      ...
      const req = store.add({ title, content, createdAt: Date.now() } as Note);
      tx.oncomplete = () => {
        this._addLog('db', `oncomplete：已添加笔记「${title}」`);
        this.setState({ noteTitle: '', noteContent: '' });
        this._loadNotes();
      };
      tx.onerror = (e: Event) => this._addLog('error', `事务失败：${(e.target as IDBTransaction).error?.message}`);
      req.onerror = (e: Event) => this._addLog('error', `添加失败：${(e.target as IDBRequest).error?.message}`);
    } catch (err: any) {
      this._addLog('error', `添加笔记异常：${err.message}`);
    }
  }

  async _loadNotes(): Promise<void> {
    try {
      const db = await this._openDB();
      ...
      req.onsuccess = (e: Event) => {
        const notes = ((e.target as IDBRequest).result || []) as Note[];
        this.setState({ notes });
        this._addLog('db', `查询完成：当前共 ${notes.length} 条笔记`);
      };
      req.onerror = (e: Event) => this._addLog('error', `查询失败：${(e.target as IDBRequest).error?.message}`);
    } catch (err: any) {
      this._addLog('error', `读取笔记异常：${err.message}`);
    }
  }

  async _deleteNote(id: number): Promise<void> {
    try {
      const db = await this._openDB();
      const tx = db.transaction('notes', 'readwrite');
      const store = tx.objectStore('notes');
      ...
      req.onerror = (e: Event) => this._addLog('error', `删除失败：${(e.target as IDBRequest).error?.message}`);
    } catch (err: any) {
      this._addLog('error', `删除笔记异常：${err.message}`);
    }
  }

  async _clearNotes(): Promise<void> {
    try {
      const db = await this._openDB();
      const tx = db.transaction('notes', 'readwrite');
      const store = tx.objectStore('notes');
      ...
      req.onerror = (e: Event) => this._addLog('error', `清空失败：${(e.target as IDBRequest).error?.message}`);
    } catch (err: any) {
  ...
  _runPerfTest(): void {
    ...
    const results: Array<{ name: string; duration: number | string; count: number | string }> = [
      { name: 'now() 素数循环', duration: nowDuration, count },
      ...measures.map((m: PerformanceEntry) => ({ name: m.name, duration: m.duration, count: sum })),
    ];
    ...
    this._addLog('perf', `now() 测量耗时 ${(nowDuration as any).toFixed(2)}ms，找到 ${count} 个素数`);
    this.setState({ perfResults: [...this.state.perfResults, ...results.map((r: any) => ({ ...r, time: formatTime() } as PerfResult))].slice(-20) });
  }

  // =================== 5. Web Crypto ===================
  async _computeHash(): Promise<void> {
    try {
  ...
  async _runCompress(): Promise<void> {
    try {
      const text = this.state.compressInput;
      // TextEncoder 将字符串编码为 Uint8Array
      const encoder = new TextEncoder();
      const data = encoder.encode(text);
      const originalSize = (data as any).byteLength;
      ...
      while (true) {
        ...
        if (value) {
          ...
          compressedSize += (value as any).byteLength;
        }
      }
      ...
      for (const chunk of chunks) {
        ...
        offset += (chunk as any).byteLength;
      }
      ...
      while (true) {
        ...
        if (done) break;
        if (value) dChunks.push(value as Uint8Array);
      }
  ...
  renderPage(): Node | string | (Node | string)[] {
    ...
    const wsInput = new Input({
      ...
      onChange: (v: string) => { this.state.wsInput = v; },
    });
    ...
    const noteTitleInput = new Input({
      ...
      placeholder: '笔记标题',
      onChange: (v: string) => { this.state.noteTitle = v; },
    });
    ...
    const noteContentInput = new Input({
      ...
      placeholder: '笔记内容',
      onChange: (v: string) => { this.state.noteContent = v; },
    });
    ...
    const bcInput = new Input({
      ...
      onChange: (v: string) => { this.state.bcInput = v; },
    });
    ...
    const cryptoInput = new Input({
      ...
      onChange: (v: string) => { this.state.cryptoInput = v; },
    });
    ...
    const compressInput = new Input({
      ...
      onChange: (v: string) => { this.state.compressInput = v; },
    });
    ...
    return [
        ...
        h('div', { class: 'flex flex-col gap-sm' },
          ...
          this.state.wsMessages.length === 0
            ...
            : h('div', { class: 'log-panel' },
              ...this.state.wsMessages.map((m: WsMessage) => h('div', { class: 'log-panel__line' },
      ...
      h(Card, {
        ...
        h('div', { class: 'flex flex-col gap-sm' },
          ...
          this.state.notes.length === 0
            ...
            : h('div', { class: 'flex flex-col gap-sm' },
              ...this.state.notes.map((note: Note) => h('div', { class: 'flex items-center gap-sm' },
                ...
                h('button', {
                  ...
                  onClick: () => this._deleteNote(note.id as number),
                }, '×'),
              )),
            ),
        ),
      ...
      h(Card, {
        ...
        h('div', { class: 'flex flex-col gap-sm' },
          ...
          this.state.perfResults.length === 0
            ...
            : h('div', { class: 'flex flex-col gap-sm' },
              ...this.state.perfResults.map((r: PerfResult) => h('div', { class: 'api-metric' },
                h('span', { class: 'fs-sm text-secondary', style: { minWidth: '160px' } }, r.name),
                h('span', { class: 'fw-medium fs-sm' }, typeof r.duration === 'string' ? r.duration : `${(r.duration as number).toFixed(2)}ms`),
      ...
      h(Card, {
        ...
        this.state.logs.length === 0
          ...
          : h('div', { class: 'log-panel' },
            ...this.state.logs.map((log: AdvancedLog) => h('div', { class: 'log-panel__line' },
