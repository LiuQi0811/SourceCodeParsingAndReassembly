...
interface CanvasRecordingPageCaps {
  canvas: boolean;
  toDataURL: boolean;
  toBlob: boolean;
  ...
  mediaStreamDest: boolean;
}

export interface CanvasRecordingPageProps extends Props {}

export interface CanvasRecordingPageState extends State {
  logs: LogEntry[];
  capsSummary: string;
  exportResult: string;
  captureInfo: string;
...
export class CanvasRecordingPage extends Page {
  declare props: CanvasRecordingPageProps;
  declare state: CanvasRecordingPageState;
  ...
  // —— 初始 state ——
  initialState(): CanvasRecordingPageState {
    return {
  ...
  componentWillUnmount(): void {
    ...
    try {
      if (this._animIds) {
        for (const id of (Object as any).keys(this._animIds)) {
    ...
    try {
      if (this._audioDest && this._audioDest.stream && typeof this._audioDest.stream.getTracks === 'function') {
        this._audioDest.stream.getTracks().forEach((t: any) => this._stopTrack(t));
      }
    } catch { /* noop */ }
  ...
  _drawAnimFrame(ctx: any,w: any,h: any,t: any): void  {
    try {
      ...
      if (typeof ctx.fillText === 'function') {
        ...
        ctx.fillText(`t=${(t as any).toFixed(1)}s`, 8, 22);
      }
    } catch { /* jsdom 桩函数可能抛错，忽略 */ }
  }

  // —— 辅助：绘制静态场景（渐变 + 三圆 + 文字），供 Card 1 / 5 复用 ——
  _drawStaticScene(ctx: any,w: any,h: any, label: string): void  {
    try {
      if (typeof ctx.clearRect === 'function') ctx.clearRect(0, 0, w, h);
      if (typeof ctx.createLinearGradient === 'function') {
        const g = ctx.createLinearGradient(0, 0, w, h);
  ...
  _stopTrack(track: any): void  {
    try { if (track && typeof track.stop === 'function') track.stop(); } catch { /* noop */ }
  }

  _stopStream(stream: any): void  {
    try {
      if (stream && typeof stream.getTracks === 'function') {
        stream.getTracks().forEach((t: any) => { try { if (typeof t.stop === 'function') t.stop(); } catch { /* noop */ } });
      }
    } catch { /* noop */ }
  }

  _safeStopRecorder(recorder: any): void  {
    try {
  ...
  _buildPlaybackUrl(chunks: any,mime: any,videoId: any): any  {
    try {
      ...
      if (videoId && url) {
        const v = (document.getElementById(videoId) as any);
        if (v) v.src = url;
      }
  ...
  _toDataURLDemo(type: string,quality: any): void  {
    const caps = this._caps();
  ...
  _toBlobDemo(type: string,quality: any): void  {
    const caps = this._caps();
  ...
  _convertToBlobDemo(type: string,quality: any): void  {
    const caps = this._caps();
  ...
  _startCapture(fps: any): void  {
    ...
    const canvas = (document.getElementById('canvas-capture') as any);
  ...
  _startRecorder(mime: any,timeslice: any): void  {
    const caps = this._caps();
    ...
    const canvas = (document.getElementById('canvas-recorder') as any);
    // 清理上一次录制
  ...
  _runE2E(seconds: any = 3): void  {
    ...
    const canvas = (document.getElementById('canvas-e2e') as any);
  ...
  _runWebCodecs(): void {
    ...
    const canvas = (document.getElementById('canvas-codecs') as any);
    const w = (canvas && canvas.width) || 320;
    const h = (canvas && canvas.height) || 200;
    ...
    try {
      const encoder = new VideoEncoder({
        output: (chunk) => {
          ...
          const cs = (chunk && typeof (chunk as any).byteLength === 'number') ? (chunk as any).byteLength : '?';
  ...
  _runAudioRecording(seconds: any = 3): void  {
    const caps = this._caps();
    if (!caps.mediaRecorder) { this._addLog('warn', 'MediaRecorder 不可用'); return; }
    ...
    const canvas = (document.getElementById('canvas-audio') as any);
  ...
  _stopAudioRecording(silent: any): void  {
    ...
    const card = new Card({
      ...
      children: [
        ...
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          ...
          this._btn('toDataURL PNG', { size: 'sm', disabled: !caps.toDataURL, onClick: () => this._toDataURLDemo('image/png', undefined) }),
          ...
          this._btn('toBlob PNG', { size: 'sm', disabled: !caps.toBlob, onClick: () => this._toBlobDemo('image/png', undefined) }),
          this._btn('toBlob WebP q=0.8', { size: 'sm', disabled: !caps.toBlob, onClick: () => this._toBlobDemo('image/webp', 0.8) }),
          this._btn('OffscreenCanvas convertToBlob', { size: 'sm', disabled: !caps.offscreen || !caps.convertToBlob, onClick: () => this._convertToBlobDemo('image/png', undefined) }),
  ...
  _renderCard3(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...
        s.recorderMime ? h(Tag, { color: 'primary' }, s.recorderMime) : null as any,
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
