...
export interface WindowPiPViewportPageProps extends Props {}

export interface WindowPiPViewportPageState extends State {
  logs: LogEntry[];
  capsSummary: string;
  docPiPInfo: string;
  viewportInfo: string;
  wakeLockInfo: string;
  orientationFsInfo: string;
  pointerLockInfo: string;
  virtualKeyboardInfo: string;
}

export class WindowPiPViewportPage extends Page {
  declare props: WindowPiPViewportPageProps;
  declare state: WindowPiPViewportPageState;
  ...
  _onVVResize!: any;
  _onVVScroll!: any;
  _wakeLockSentinel!: any;
  _wakeLockReleaseHandler!: any;
  _demoFsEl!: any;
  _demoPointerEl!: any;
  _pipEnterBound!: boolean;


  // —— 初始 state ——
  initialState(): WindowPiPViewportPageState {
    return {
      logs: [],
      capsSummary: '',
  ...
  async _demoDocPiP() {
    ...
    try {
      ...
      if (movedEl && pipWindow && pipWindow.document && pipWindow.document.body) {
        ...
        } catch (e: any) { this._addLog('warn', `移动 DOM 进 PiP 失败：${errInfo(e).message}`); }
      }
  ...
  _bindViewport(): void {
    ...
    this._onVVResize = (e: any) => { const t = e && e.target ? e.target : vv; this._addLog('event', `visualViewport resize：width=${t.width}，height=${t.height}，scale=${t.scale}`); };
    this._onVVScroll = (e: any) => { const t = e && e.target ? e.target : vv; this._addLog('event', `visualViewport scroll：pageLeft=${t.pageLeft}，pageTop=${t.pageTop}`); };
    try { vv!.addEventListener('resize', this._onVVResize); } catch (err: any) { this._addLog('warn', `注册 resize 失败：${errInfo(err).message}`); }
    try { vv!.addEventListener('scroll', this._onVVScroll); } catch (err: any) { this._addLog('warn', `注册 scroll 失败：${errInfo(err).message}`); }
    this.setState({ viewportInfo:
  ...
  // 程序化显示 / 隐藏虚拟键盘
  _toggleVirtualKeyboard(show: any): void  {
    const caps = this._caps(undefined);
