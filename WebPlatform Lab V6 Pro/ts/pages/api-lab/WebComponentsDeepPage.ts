...
export interface WebComponentsDeepPageProps extends Props {}

export interface WebComponentsDeepPageState extends State {}

export class WebComponentsDeepPage extends Page {
  declare props: WebComponentsDeepPageProps;
  declare state: WebComponentsDeepPageState;
  _adoptIframe: any = null;
  _cardDefined: any = null;
  _counterDefined: any = null;
  _counterEl: any = null;
  _inited: boolean = false;
  _inputDefined: any = null;
  _inputEl: any = null;
  _internals: any = null;
  _partEl: any = null;
  _partElDefined: any = null;
  _shadowEl: any = null;
  _slotHost: any = null;
  _slotHostDefined: any = null;
  _v: any = null;
  // —— 初始 state ——
  initialState(): WebComponentsDeepPageState {
    return {
  ...
  componentDidMount(): void {
    ...
    ].forEach(([ok, msg]: any) => { if (!ok) this._addLog('warn', msg); });
  }
  ...
  _insertCounter() {
    ...
    const container = (this.$('.wc-deep-counter-host') as any);
      ...
      class WcDeepCard extends HTMLElement {
        constructor() {
          ...
          if (typeof this.attachShadow === 'function') {
            const sr = (this.attachShadow({ mode: wcDeepShadowMode as ShadowRootMode }) as any);
            sr.innerHTML =
  ...
  _attachShadowMode(mode: any) {
    if (!this._ensureShadowCardDefined()) return;
    wcDeepShadowMode = mode;
    const container = (this.$('.wc-deep-shadow-host') as any);
    if (!container) { this._addLog('warn', '找不到挂载容器'); return; }
    try {
      ...
      class WcDeepSlotHost extends HTMLElement {
        constructor() {
          ...
          if (typeof this.attachShadow === 'function') {
            ...
            sr.querySelectorAll('slot').forEach((slot: any) => {
  ...
  _triggerSlotchange() {
    ...
    const container = (this.$('.wc-deep-slot-host') as any);
    if (!container) { this._addLog('warn', '找不到挂载容器'); return; }
    try {
      if (!this._slotHost) {
  ...
  _listAssignedNodes() {
    ...
    try {
      ...
      const lines: any[] = [];
      const readSlot = (slot: any, label: any) => {
        ...
        nodes.forEach((n: any, i: any) => lines.push(`  [${i}] ${n.nodeName}: ${(n.textContent || '').slice(0, 40)}`));
  ...
  _setPart() {
    ...
    const container = (this.$('.wc-deep-part-host') as any);
    if (!container) { this._addLog('warn', '找不到挂载容器'); return; }
    try {
  ...
  _cloneTemplate() {
    ...
    try {
      const tpl = (this.$('#wc-deep-template') as any);
      ...
      const fillSlot = (sel: any, text: any) => {
      ...
      const container = (this.$('.wc-deep-template-host') as any);
      if (!container) { this._addLog('warn', '找不到挂载容器'); return; }
  ...
  _attachInternalsDemo() {
    ...
    const container = (this.$('.wc-deep-input-host') as any);
    if (!container) { this._addLog('warn', '找不到挂载容器'); return; }
    try {
      if (!this._inputEl) {
