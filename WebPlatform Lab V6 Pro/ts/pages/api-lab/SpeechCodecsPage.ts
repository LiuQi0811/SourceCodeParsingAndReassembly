...
interface SpeechCodecsPageState extends State {
  logs: any;
  capsSummary: any;
  ttsInfo: any;
  voicesInfo: any;
  recognitionInfo: any;
  videoEncoderInfo: any;
  videoDecoderInfo: any;
  imageDecoderInfo: any;
}

export class SpeechCodecsPage extends Page {
  declare state: SpeechCodecsPageState;
  _encodedChunks!: any;
  _imageDecoder!: any;
  _inited!: boolean;
  _recognition!: any;
  ...
  componentDidMount() {
    ...
    const c = (name: any, has: any) => `${name} ${has ? '✓' : '✗'}`;
    const parts = [
  ...
  _speak() {
    ...
    try {
      ...
      u.onboundary = (e: any) => this._addLog('tts', `onboundary：charIndex=${e.charIndex}（到达词/句边界）`);
      u.onerror = (e: any) => this._addLog('warn', `onerror：${e.error}`);
      this._utterance = u;
...
speechSynthesis.cancel();            // 立即停止并清空队列`)),
  ...
  _loadVoices() {
    ...
    try {
      const renderList = (voices: any) => {
        const lines = voices.map((v: any, i: number) =>
  ...
  _pickDefaultVoice() {
    ...
    try {
      ...
      const def = voices.find((v: any) => v.default) || voices[0];
...
loadVoices();`)),
  ...
  _startRecognition() {
    ...
    try {
      ...
      (r as any).onaudiostart = () => this._addLog('asr', 'onaudiostart：音频采集开始');
      (r as any).onaudioend = () => this._addLog('asr', 'onaudioend：音频采集结束');
      (r as any).onspeechstart = () => this._addLog('asr', 'onspeechstart：检测到语音');
      (r as any).onspeechend = () => this._addLog('asr', 'onspeechend：语音停止');
      r.onresult = (e: any) => {
        ...
        this._addLog('asr', `onresult：transcript="${top.transcript}" confidence=${(top.confidence as any).toFixed(2)} isFinal=${last.isFinal}`);
        // 拼接所有结果的 transcript
        let fullText = '';
      ...
      (r as any).onnomatch = () => this._addLog('warn', 'onnomatch：未识别到匹配内容');
      r.onerror = (e: any) => this._addLog('warn', `onerror：${e.error}（如 not-allowed / no-speech / network）`);
    ...
    try {
      ...
      const encoder = new VideoEncoder({
        output: (chunk: any, metadata: any) => {
          ...
          this._addLog('encode', `output chunk：type="${chunk.type}" timestamp=${chunk.timestamp} duration=${chunk.duration} byteLength=${(chunk as any).byteLength}`);
        ...
        error: (e: any) => this._addLog('warn', `VideoEncoder error：${e.message}`),
      });
      encoder.configure({
  ...
  async _encodeMockFrame() {
    ...
    try {
      ...
      const chunkText = chunk
        ? `chunk.type="${chunk.type}" timestamp=${chunk.timestamp} duration=${chunk.duration} byteLength=${(chunk as any).byteLength}`
        : '（无 chunk 产出）';
      this.setState({
        videoEncoderInfo:
  ...
  _createVideoDecoder() {
    ...
    try {
      const decoder = new VideoDecoder({
        ...
        error: (e: any) => this._addLog('warn', `VideoDecoder error：${e.message}`),
      });
  ...
  async _decodeMockChunk() {
    ...
    try {
      ...
      if (realChunk) {
        ...
        chunkDesc = `使用 Card 4 编码产出的真实 chunk：type="${realChunk.type}" timestamp=${realChunk.timestamp} byteLength=${(realChunk as any).byteLength}`;
        this._addLog('decode', `decode(realChunk)：type="${realChunk.type}" byteLength=${(realChunk as any).byteLength}`);
      } else {
        ...
        chunkDesc = `无 Card 4 真实 chunk，使用 mock chunk：type="key" timestamp=0 byteLength=${(mock as any).byteLength}（仅 NAL 起始码，可能触发 error）`;
        this._addLog('decode', `decode(mock chunk)：type="key" byteLength=${(mock as any).byteLength}`);
      }
