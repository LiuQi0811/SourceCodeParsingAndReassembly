...
interface CSSGradientsDeepPageState extends State {
  logs: LogEntry[];
  capsSummary: string;
  linearInfo: string;
  radialInfo: string;
  conicInfo: string;
  repeatingInfo: string;
  colorStopsInfo: string;
  interpolationInfo: string;
  layeredInfo: string;
  patternInfo: string;
}
...
export class CSSGradientsDeepPage extends Page {
  declare state: CSSGradientsDeepPageState;
  _inited: boolean = false;
  _dynamicStyles: HTMLStyleElement[] = [];


  initialState(): CSSGradientsDeepPageState {
    return {
      logs: [],
      capsSummary: '',
  ...
  componentDidMount(): void {
    ...
    if (!f.conicGradient) this._addLog('warn', 'conic-gradient() 不可用（Chrome 69+/Firefox 83+/Safari 12.1+）');
    if (!f.oklchInterpolation) this._addLog('warn', 'oklch 颜色插值不可用（Chrome 111+ / CSS Color L4）');
  ...
  _renderCard1(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['linear-gradient()', f.linearGradient]]),
        h(Tag, { color: 'primary' }, 'Images L4'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'linear-gradient(<angle>|<side-or-corner>, <color-stop>+) 线性渐变。角度 0deg=向上、90deg=向右、180deg=向下（默认）、270deg=向左，顺时针递增；关键字 to top|to right|to bottom|to left|to top right（须带 to 前缀）。颜色停止点 <color> <position> 可省略位置，多色默认均匀分布（如 4 色自动 0%/33.3%/66.7%/100%）。注意 0deg 在渐变里是向上，与 transform: rotate(0deg) 方向约定不同。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行 linear-gradient 演示', { type: 'primary', size: 'sm', onClick: () => this._runLinearDemo() }),
        ),
        ...
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.linearInfo || '（点击按钮查看 linear-gradient() 完整用法）')),
      ],
    });
  ...
  _renderCard3(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['conic-gradient()', f.conicGradient]]),
        h(Tag, { color: 'primary' }, 'Images L4'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'conic-gradient([from angle] [at position], <color-stop>+) 锥形渐变，颜色沿圆周方向变化。from 0deg 起始方向为正上方（12 点钟），顺时针旋转；at center 设中心位置。应用：饼图（双位置/单值链式停止点硬切）、色轮（hsl 色相环）、雷达图（配合 transform: rotate 旋转容器扫描）。首尾颜色不同会出现硬切线 → 用同色闭合。Chrome 69+/Firefox 83+/Safari 12.1+ 支持。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行 conic-gradient 演示', { type: 'primary', size: 'sm', onClick: () => this._runConicDemo() }),
        ),
        h('div', { class: 'gr-conic' }),
        h('pre', { class: 'code-block' }, h('code', {}, code)),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.conicInfo || '（点击按钮查看 conic-gradient() 完整用法）')),
      ],
    });
    this.registerChild(card);
    return card.render() as Node;
  ...
  _renderCard4(): Node | string {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'repeating-linear/radial/conic-gradient() 重复渐变，语法与普通渐变相同，但停止点图案会无限平铺。重复周期 = 末停止点位置 - 首停止点位置。经典条纹：repeating-linear-gradient(45deg, #3b82f6 0, #3b82f6 10px, #fff 10px, #fff 20px)。应用：条纹图案、棋盘格（两层正交叠加）、波纹（repeating-radial）、放射条纹（repeating-conic）。像素值需精确，高 DPI 屏用整数 px 避免模糊。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行 repeating 演示', { type: 'primary', size: 'sm', onClick: () => this._runRepeatingDemo() }),
        ),
        h('div', { class: 'gr-repeating' }),
        h('pre', { class: 'code-block' }, h('code', {}, code)),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.repeatingInfo || '（点击按钮查看 repeating-* 完整用法）')),
      ],
    });
    this.registerChild(card);
    return card.render() as Node;
  ...
  _renderCard5(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: 'primary' }, 'Color Stops'),
        h(Tag, { color: f.linearGradient ? 'success' : 'error' }, `双位置 ${f.linearGradient ? '✓' : '✗'}`),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '颜色停止点高级用法。双位置 <color> <pos1> <pos2> 实现硬过渡（区间内纯色）；同位置两色（#3b82f6 50%, #ef4444 50%）实现硬边缘。transparent 关键字实际是 rgba(0,0,0,0)，与彩色插值产生灰边 → 用 rgba 同色相透明替代。currentColor 取元素 color 计算值。渐变提示 (gradient hint) 在两色间插入纯位置指定中点（如 linear-gradient(to right, blue, 25%, red) 让 25% 处为中点）。单值链式停止点（L4）用于饼图硬切。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行 color stops 演示', { type: 'primary', size: 'sm', onClick: () => this._runColorStopsDemo() }),
        ),
        h('div', { class: 'gr-colorstops' }),
        h('pre', { class: 'code-block' }, h('code', {}, code)),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.colorStopsInfo || '（点击按钮查看颜色停止点高级用法）')),
      ],
    });
    this.registerChild(card);
    return card.render() as Node;
  ...
  _renderCard7(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: 'primary' }, 'Blend'),
        h(Tag, { color: f.linearGradient ? 'success' : 'error' }, `blend-mode ${f.linearGradient ? '✓' : '✗'}`),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '多层 background-image 叠加：先写的渲染在上层（与 z-index 无关，按声明顺序堆叠）。background-blend-mode 控制本元素多层背景间混合（multiply|screen|overlay|darken|lighten|color-dodge|color-burn|hard-light|soft-light|difference|exclusion|hue|saturation|color|luminosity），多层可分别指定。与 mix-blend-mode 区别：background-blend-mode 仅混合本元素背景层，mix-blend-mode 混合元素与背后内容（需 isolation: isolate 隔离）。渐变+图片混合实现暗角/调色；多层 radial 叠加实现斑点/光晕复杂图案。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行 layered 演示', { type: 'primary', size: 'sm', onClick: () => this._runLayeredDemo() }),
        ),
        h('div', { class: 'gr-layered' }),
        h('pre', { class: 'code-block' }, h('code', {}, code)),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.layeredInfo || '（点击按钮查看多层叠加与 mix-blend-mode 完整用法）')),
      ],
    });
    this.registerChild(card);
    return card.render() as Node;
