...
interface CSSScrollLayoutPageState extends State {
  logs: LogEntry[];
  capsSummary: string;
  scrollSnapInfo: string;
  scrollBehaviorInfo: string;
  contentVisibilityInfo: string;
  scrollbarInfo: string;
  overflowClipInfo: string;
  containerQueryInfo: string;
}
...
export class CSSScrollLayoutPage extends Page {
  declare state: CSSScrollLayoutPageState;
  _inited: boolean = false;
  _dynamicStyles: HTMLStyleElement[] = [];
  _dynamicNodes: any[] = [];
  _observers: any = undefined as any;
  _snapStrictness: string = '';


  // —— 初始 state ——
  initialState(): CSSScrollLayoutPageState {
    return {
      logs: [],
      capsSummary: '',
  ...
  _flags(): any {
    ...
    const supportsCond = (cond: string) => {
  ...
  _scrollToSection(block: any,behavior: any) {
    const f = this._flags();
    if (!f.scrollIntoView) {
    ...
    try {
      ...
      target.scrollIntoView((opts as any));
  ...
  _runContentVisibilityDemo(): void {
    const f = this._flags();
    try {
  ...
  _renderCard4(): Node | string {
    const s = this.state;
    const f = this._flags();
    const filler = (text: any) => h('div', { style: { height: '300px', padding: '8px' } }, text);
    const card = new Card({
