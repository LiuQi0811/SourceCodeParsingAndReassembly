...
export interface WebAssemblyPageProps extends Props {}

export interface WebAssemblyLog {
  type: string;
  content: string;
  time: string;
}

export interface WebAssemblyPageState extends State {
  logs: WebAssemblyLog[];
  capsSummary: string;
  moduleInfo: string;
  memorySize: string;
...
export class WebAssemblyPage extends Page {
  declare props: WebAssemblyPageProps;
  declare state: WebAssemblyPageState;
  _inited: boolean = false;
  _module: WebAssembly.Module | null = null;
  _instance: WebAssembly.Instance | null = null;
  _memory: WebAssembly.Memory | null = null;
  _table: WebAssembly.Table | null = null;
  _global: WebAssembly.Global | null = null;
  _importInstance: WebAssembly.Instance | null = null;

  initialState(): WebAssemblyPageState {
    return {
      logs: [],
      capsSummary: '',
      moduleInfo: '',
  ...
  componentDidMount(): void {
    ...
    const hasException = typeof WebAssembly !== 'undefined' && typeof (WebAssembly as any).Exception !== 'undefined';
    const hasTag = typeof WebAssembly !== 'undefined' && typeof (WebAssembly as any).Tag !== 'undefined';
  ...
  _caps(): WasmCaps {
    const has = (n: string): boolean => typeof WebAssembly !== 'undefined' && typeof (WebAssembly as any)[n] !== 'undefined';
    return {
      wasm: has('Module'),
      memory: has('Memory'),
      table: has('Table'),
  ...
  async _compileModule(): Promise<void> {
    if (!this._caps().wasm) {
  ...
  async _instantiateModule(): Promise<void> {
    if (!this._caps().wasm) {
    ...
    try {
      ...
      if (this._caps().streaming && this._caps().fetch) {
        try {
          ...
          const streamExports: any = streamInst.exports;
...
0a 09 01 07 00 20 00 20 01 6a 0b ; code: local.get 0/1, i32.add, end`)),
  ...
  _createMemory(): void {
    ...
    try {
      const memory = new WebAssembly.Memory({ initial: 1, maximum: 10, shared: false } as WebAssembly.MemoryDescriptor);
      this._memory = memory;
      const len = (memory.buffer as any).byteLength;
  ...
  _growMemory(): void {
    ...
    try {
      const beforeLen = (this._memory.buffer as any).byteLength;
      const beforePages = beforeLen / 65536;
      const prevPages = this._memory.grow(2);
      const afterLen = (this._memory.buffer as any).byteLength;
...
// grow 后 memory.buffer 变为新 ArrayBuffer，旧 view 失效`)),
  ...
  _createTable(): void {
    ...
    try {
      const table = new WebAssembly.Table({ element: 'anyfunc', initial: 2, maximum: 10 } as WebAssembly.TableDescriptor);
      this._table = table;
      this.setState({
        tableInfo:
  ...
  _setTableFunc(): void {
    ...
    try {
      const fnDouble = (x: number): number => x * 2;
      const fnInc = (x: number): number => x + 1;
      this._table.set(0, fnDouble as any);
      this._table.set(1, fnInc as any);
  ...
  _getTableFunc(): void {
    ...
    try {
      const fn0 = this._table.get(0) as ((x: number) => number) | null;
      const fn1 = this._table.get(1) as ((x: number) => number) | null;
      const r0 = fn0 ? fn0(21) : null;
      const r1 = fn1 ? fn1(99) : null;
      const lenBefore = this._table.length;
      const prev = this._table.grow(1, null as any);
...
const prev = table.grow(1, null);               // 扩容，新槽位用 null 填充`)),
  ...
  _createGlobal(): void {
    ...
    try {
      const global = new WebAssembly.Global({ value: 'i32', mutable: true } as WebAssembly.GlobalDescriptor, 42);
      this._global = global;
  ...
  _writeGlobal(): void {
    ...
    try {
      ...
      let readonlyLine = '';
      try {
        const ro = new WebAssembly.Global({ value: 'i32', mutable: false } as WebAssembly.GlobalDescriptor, 7);
        readonlyLine = `\n\n只读 Global：new Global({ value:'i32', mutable:false }, 7)\n  ro.value = ${ro.value}（只读，赋值会抛 TypeError）`;
        try { ro.value = 999; } catch (e: any) { readonlyLine += `\n  尝试 ro.value = 999 → ${e.name}（不可写）`; }
      } catch (e: any) { readonlyLine = `\n\n只读 Global 创建失败：${e.message}`; }
      this.setState({
        globalValue:
...
// ro.value = 9n;               // mutable:false 赋值会抛 TypeError`)),
  ...
  async _demoInterop(): Promise<void> {
    if (!this._caps().wasm) {
    ...
    try {
      ...
      const importObject = {
        env: {
          log: (n: number) => this._addLog('export', `WASM 调用 JS env.log(${n}) —— 跨边界互操作成功`),
          abort: () => { throw new Error('WASM 调用 abort'); },
        },
      };
      const { module, instance } = await WebAssembly.instantiate(IMPORT_BYTES, importObject as any);
  ...
  async _triggerRuntimeError(): Promise<void> {
    if (!this._caps().wasm) {
      this._addLog('warn', 'WebAssembly 不可用');
      return;
    }
    ...
    try {
      ...
      const isRuntime = typeof (WebAssembly as any).RuntimeError !== 'undefined' && err instanceof (WebAssembly as any).RuntimeError;
    ...
    try {
      ...
      const isCompile = typeof (WebAssembly as any).CompileError !== 'undefined' && err instanceof (WebAssembly as any).CompileError;
    ...
    try {
      ...
      const isLink = typeof (WebAssembly as any).LinkError !== 'undefined' && err instanceof (WebAssembly as any).LinkError;
    ...
    if (this._caps().tag && this._caps().exception) {
      try {
        const tag = new (WebAssembly as any).Tag({ parameters: ['i32'] });
        const exc = new (WebAssembly as any).Exception(tag, [42]);
        const isExc = exc instanceof (WebAssembly as any).Exception;
