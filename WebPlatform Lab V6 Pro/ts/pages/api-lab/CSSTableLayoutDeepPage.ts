...
interface CSSTableLayoutDeepPageState extends State {
  logs: LogEntry[];
  capsSummary: string;
  anonymousBoxesInfo: string;
  tableLayoutInfo: string;
  borderCollapseInfo: string;
  captionValignInfo: string;
  colgroupInfo: string;
  semanticsA11yInfo: string;
  stickyHeaderInfo: string;
  printTrapsInfo: string;
}
...
export class CSSTableLayoutDeepPage extends Page {
  declare state: CSSTableLayoutDeepPageState;
  _inited: boolean = false;
  _dynamicStyles: HTMLStyleElement[] = [];
  _tableLayoutMode: string = '';
  _collapseMode: string = '';
  _emptyCells: string = '';
  _captionSide: string = '';
  _valignMode: string = '';
  _stickyOn: boolean = false;


  // —— 初始 state ——
  initialState(): CSSTableLayoutDeepPageState {
    return {
      logs: [],
      capsSummary: '',
      // Card 1：匿名表格盒模型
  ...
  componentDidMount(): void {
    ...
    if (!f.captionSide) this._addLog('warn', 'caption-side:inline-start|inline-end 不可用或 jsdom 未识别（CSS Logical Properties 写法，新浏览器支持）');
    if (!f.breakInside) this._addLog('warn', 'break-inside:avoid 不可用或 jsdom 未识别（旧名 page-break-inside:avoid 兼容性更好）');
    if (!f.positionSticky) this._addLog('warn', 'position:sticky 不可用或 jsdom 未识别（Chrome 56+/Firefox 32+/Safari 13+ 全面支持）');
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '表格布局不限于 <table> 标签：display: table / table-row / table-cell / table-column / table-column-group / table-caption / table-header-group / table-footer-group 任意元素都可参与表格布局。若 DOM 层级缺失，浏览器按 CSS2.1 §17.2.1 自动生成「匿名盒」补全（如孤立的 table-cell 会被自动包上匿名 table-row + 匿名 table）。匿名盒无法用 CSS/JS 命中。table-column / table-column-group 仅承载 border/background/width/visibility，不渲染内容。',
        ),
        h('div', { class: 'tbl-demo' },
          h('div', { class: 'fs-sm text-secondary mb-xs' }, '用 div 模拟表格（display: table-*）：'),
          h('div', { class: 'anon-stage' },
            h('div', { class: 'anon-table' },
              h('div', { class: 'anon-row' },
                h('div', { class: 'anon-cell anon-cell--hl' }, 'A'),
                h('div', { class: 'anon-cell' }, 'B'),
                h('div', { class: 'anon-cell' }, 'C'),
              ),
            ),
          ),
          h('div', { class: 'fs-sm text-secondary mt-sm' }, '孤立的 table-cell（缺 table/row → 自动包匿名盒）：'),
          h('div', { class: 'anon-bare-cell' }, '我是裸 cell，浏览器自动包了匿名 row + table'),
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap mt-sm' },
          this._btn('运行匿名盒演示', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runAnonymousBoxesDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '460px', overflow: 'auto' } },
          h('code', {}, s.anonymousBoxesInfo || '（点击按钮查看 display:table-* 全家桶 + 匿名盒生成规则）')),
      ],
    });
  ...
  _renderCard2(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['table-layout', f.tableLayout]]),
        h(Tag, { color: 'primary' }, 'auto / fixed'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'table-layout: auto（默认）扫描全部单元格内容二次计算列宽，适应内容但首屏慢、大表格性能差；table-layout: fixed 仅看首行（或 <col>）宽度一次锁定列宽，性能稳定（1000 行表格约 10 倍速）但需配合 width:100% 与手动设列宽。fixed 下长内容可用 overflow:hidden + text-overflow:ellipsis 截断。大数据表格务必用 fixed。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行算法演示', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runTableLayoutDemo() }),
          this._btn('auto', { size: 'sm', disabled: !f.tableLayout, onClick: () => this._setTableLayout('auto') }),
          this._btn('fixed', { size: 'sm', disabled: !f.tableLayout, onClick: () => this._setTableLayout('fixed') }),
        ),
        h('div', { class: 'tbl-demo' },
          h('div', { class: 'fs-sm text-secondary mb-xs' }, `当前 table-layout: ${this._tableLayoutMode}（fixed 模式下 .col-w 锁定 120px，长内容自动截断）`),
          h('table', { class: 'tl-switch' },
            h('thead', {},
              h('tr', {},
                h('th', { class: 'col-w' }, 'ID'),
                h('th', {}, '名称'),
                h('th', { class: 'col-long' }, '描述（长内容，fixed 下截断）'),
              ),
            ),
            h('tbody', {},
              h('tr', {},
                h('td', {}, '1'),
                h('td', {}, 'CSS 表格布局'),
                h('td', {}, '这是非常非常非常长的描述文字，用来演示 table-layout: fixed 配合 overflow:hidden + text-overflow:ellipsis 的截断效果'),
              ),
              h('tr', {},
                h('td', {}, '2'),
                h('td', {}, '匿名盒模型'),
                h('td', {}, '另一个长内容：display: table-cell 的孤立元素会被浏览器自动包裹匿名 table-row 和 table 层级'),
              ),
            ),
          ),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '420px', overflow: 'auto' } },
          h('code', {}, s.tableLayoutInfo || '（点击「运行算法演示」查看 auto vs fixed 完整对比）')),
        h(Alert, {
          type: 'info',
  ...
  _renderCard3(): Node | string {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'border-collapse: separate（默认）单元格独立边框 + border-spacing 间距 + empty-cells 控制空单元格；collapse 相邻边按 CSS2.1 §17.6.2.1 优先级合并（hidden > none > width 大 > 样式强 > cell>row>table）。collapse 下 border-spacing/empty-cells/border-radius 失效，且 position:sticky 边框会消失（见 Card 7）。collapse 适合单线紧凑表格，separate 适合网格风格 + 圆角。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行边框演示', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runBorderCollapseDemo() }),
          this._btn('separate', { size: 'sm', disabled: !f.borderCollapse, onClick: () => this._setCollapse('separate') }),
          this._btn('collapse', { size: 'sm', disabled: !f.borderCollapse, onClick: () => this._setCollapse('collapse') }),
          this._btn(`empty-cells: ${this._emptyCells}`, { size: 'sm', disabled: !f.emptyCells, onClick: () => this._setEmptyCells(this._emptyCells === 'show' ? 'hide' : 'show') }),
        ),
        h('div', { class: 'tbl-demo' },
          h('div', { class: 'fs-sm text-secondary mb-xs' }, `当前 border-collapse: ${this._collapseMode}，empty-cells: ${this._emptyCells}（注意空单元格行为）`),
          h('table', { class: 'bc-switch' },
            h('thead', {},
              h('tr', {}, h('th', {}, '姓名'), h('th', {}, '年龄'), h('th', {}, '邮箱')),
            ),
            h('tbody', {},
              h('tr', {}, h('td', {}, '张三'), h('td', {}, '28'), h('td', {}, 'zhang@example.com')),
              h('tr', {}, h('td', {}, '李四'), h('td', { class: 'empty' }, ''), h('td', {}, 'li@example.com')),
              h('tr', {}, h('td', {}, '王五'), h('td', {}, '35'), h('td', { class: 'empty' }, '')),
            ),
          ),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '420px', overflow: 'auto' } },
          h('code', {}, s.borderCollapseInfo || '（点击「运行边框演示」查看 collapse 边框合并规则 + separate 间距 + empty-cells）')),
        h(Alert, {
          type: 'warning',
  ...
  _renderCard4(): Node | string {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'caption-side 控制 <caption> 位置：top（默认）/bottom（CSS2.1）/inline-start/inline-end（CSS Logical，受 direction/writing-mode 影响）。<caption> 是表格语义标题，屏幕阅读器朗读为表格标签，应作为 <table> 首个子元素，视觉隐藏用 sr-only 而非删除。table-cell 中 vertical-align 控制整段内容垂直对齐（top/middle/bottom/baseline），baseline 让同行单元格首行文字基线对齐，与行内元素 vertical-align 行为不同。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行演示', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runCaptionValignDemo() }),
          this._btn('caption: top', { size: 'sm', disabled: !f.captionSideTop, onClick: () => this._setCaptionSide('top') }),
          this._btn('caption: bottom', { size: 'sm', disabled: !f.captionSideTop, onClick: () => this._setCaptionSide('bottom') }),
          this._btn('caption: inline-start', { size: 'sm', disabled: !f.captionSide, onClick: () => this._setCaptionSide('inline-start') }),
        ...
        h('div', { class: 'tbl-demo' },
          h('div', { class: 'fs-sm text-secondary mb-xs' }, `caption-side: ${this._captionSide}，vertical-align: ${this._valignMode}（注意 caption 位置 + 单元格内容垂直对齐）`),
          h('table', { class: 'cap-switch va-switch' },
            h('caption', {}, '表 1：员工信息（caption 位置受 caption-side 控制）'),
            h('thead', {},
              h('tr', {}, h('th', { class: 'va-cell-h' }, '姓名'), h('th', { class: 'va-cell-h' }, '简介')),
            ),
            h('tbody', {},
              h('tr', {},
                h('td', {}, h('strong', {}, '张三')),
                h('td', {}, '前端工程师，负责 React/Vue 应用开发，5 年经验，熟悉 CSS 表格布局与 Flexbox/Grid'),
              ),
              h('tr', {},
                h('td', {}, h('strong', {}, '李四')),
                h('td', {}, '后端工程师，负责 Node.js/Go 微服务，3 年经验'),
              ),
            ),
          ),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '420px', overflow: 'auto' } },
          h('code', {}, s.captionValignInfo || '（点击「运行演示」查看 caption-side + vertical-align 全套用法）')),
        h(Alert, {
          type: 'info',
  ...
  _renderCard5(): Node | string {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '<colgroup> 包裹 <col> 对列应用统一样式；<col> 是 table-column 盒不渲染内容，仅 width/background/border/visibility:collapse 4 类属性生效，color/font/padding/text-align 无效需设在 cell 上。span 属性（HTML）让一个 <col> 代表多列。table-layout:fixed 下 <col> width 优先级最高（> 首行 cell width > 均分）。visibility:collapse 隐藏整列是响应式列控制的标准手段。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行列控制演示', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runColgroupDemo() }),
        ),
        h('div', { class: 'tbl-demo' },
          h('div', { class: 'fs-sm text-secondary mb-xs' }, 'fixed + colgroup 锁定列宽（col 设 width，cell 透明透出列背景）：'),
          h('table', { class: 'tl-fixed', style: { width: '100%', borderCollapse: 'collapse' } },
            h('colgroup', {},
              h('col', { style: { width: '60px', background: '#fef3c7' } }),
              h('col', { style: { width: '140px' } }),
              h('col', { style: { width: '200px' } }),
              h('col', { style: { width: '100px', background: '#dbeafe' } }),
            ),
            h('thead', {},
              h('tr', {},
                h('th', {}, 'ID'),
            ...
            h('tbody', {},
              h('tr', {},
                h('td', { style: { background: 'transparent' } }, '1'),
                h('td', { style: { background: 'transparent' } }, '张三'),
                h('td', { style: { background: 'transparent' } }, 'zhang@example.com'),
                h('td', { style: { background: 'transparent' } }, '编辑'),
              ),
              h('tr', {},
                h('td', { style: { background: 'transparent' } }, '2'),
                h('td', { style: { background: 'transparent' } }, '李四'),
                h('td', { style: { background: 'transparent' } }, 'li@example.com'),
                h('td', { style: { background: 'transparent' } }, '编辑'),
              ),
            ),
          ),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '460px', overflow: 'auto' } },
          h('code', {}, s.colgroupInfo || '（点击「运行列控制演示」查看 <colgroup>/<col>/span + 列宽优先级链）')),
        h(Alert, {
          type: 'warning',
  ...
  _renderCard6(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['display:table', f.displayTable]]),
        h(Tag, { color: 'primary' }, 'scope / ARIA'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'thead/tbody/tfoot/th/td 提供表格语义；<th scope="col|row|colgroup|rowgroup"> 让屏幕阅读器建立「单元格↔表头」关联；colspan/rowspan 合并单元格（复杂场景用 headers 显式关联多个表头）。原生 <table> 自带 ARIA table 语义，仅 div 模拟表格才需 role="table"/row/cell/columnheader。原则：能用原生 <table> 就用原生，不用 <table> 做布局（改用 Grid/Flexbox）。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行语义演示', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runSemanticsA11yDemo() }),
        ),
        h('div', { class: 'tbl-demo' },
          h('div', { class: 'fs-sm text-secondary mb-xs' }, '语义化表格（scope + colspan 合并表头）：'),
          h('table', { class: 'tbl', style: { borderCollapse: 'collapse', width: '100%' } },
            h('caption', {}, '表 2：2024/2025 年度区域销售额（万元）'),
            h('thead', {},
              h('tr', {},
                h('th', { scope: 'col', rowspan: '2', class: 'sem-th' }, '区域'),
                h('th', { scope: 'col', colspan: '2', class: 'sem-th' }, '销售额'),
                h('th', { scope: 'col', rowspan: '2', class: 'sem-th' }, '增长率'),
              ),
              h('tr', {},
                h('th', { scope: 'col', class: 'sem-th' }, '2024'),
                h('th', { scope: 'col', class: 'sem-th' }, '2025'),
              ),
            ),
            h('tbody', {},
              h('tr', {},
                h('th', { scope: 'row', class: 'sem-th' }, '华东'),
                h('td', {}, '120'),
                h('td', {}, '150'),
                h('td', {}, '+25%'),
              ),
              h('tr', {},
                h('th', { scope: 'row', class: 'sem-th' }, '华北'),
                h('td', {}, '90'),
                h('td', {}, '108'),
                h('td', {}, '+20%'),
              ),
        ...
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '460px', overflow: 'auto' } },
          h('code', {}, s.semanticsA11yInfo || '（点击「运行语义演示」查看 scope/colspan/rowspan/headers/ARIA 全套用法）')),
        h(Alert, {
          type: 'warning',
  ...
  _renderCard7(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...
        h(Tag, { color: 'primary' }, 'sticky / overflow'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'position: sticky 让 thead th 钉在 top:0、tbody th 钉在 left:0 实现表头+首列冻结。陷阱：父容器 overflow:hidden 失效、必须有滚动容器+高度限制、左上角交叉格 z-index 要最高（3>2>1）。严重问题：border-collapse:collapse 下 sticky 边框会随滚动消失，需改用 separate+border-spacing:0 或用 box-shadow 模拟边框。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行 sticky 演示', { type: 'primary', size: 'sm', disabled: !f.positionSticky, onClick: () => this._runStickyDemo() }),
          this._btn(`sticky 表头：${this._stickyOn ? '开启' : '关闭'}`, { size: 'sm', disabled: !f.positionSticky, onClick: () => this._toggleSticky() }),
        ),
        h('div', { class: 'tbl-demo' },
          h('div', { class: 'fs-sm text-secondary mb-xs' }, '可滚动表格容器（sticky 表头 + 首列，左上角交叉格 z-index 最高）：'),
          h('div', { class: 'tbl-stage sticky-toggle' },
            h('table', { class: 'tbl sticky-table', style: { borderCollapse: 'separate', borderSpacing: '0', width: '100%' } },
              h('thead', {},
                h('tr', {},
                  h('th', { style: { position: 'sticky', top: '0', left: '0', zIndex: '4', background: '#1e3a8a', color: '#fff' } }, '姓名\\指标'),
                  h('th', { style: { position: 'sticky', top: '0', zIndex: '3', background: '#1e3a8a', color: '#fff' } }, '1月'),
                  h('th', { style: { position: 'sticky', top: '0', zIndex: '3', background: '#1e3a8a', color: '#fff' } }, '2月'),
                  h('th', { style: { position: 'sticky', top: '0', zIndex: '3', background: '#1e3a8a', color: '#fff' } }, '3月'),
                  h('th', { style: { position: 'sticky', top: '0', zIndex: '3', background: '#1e3a8a', color: '#fff' } }, '4月'),
                  h('th', { style: { position: 'sticky', top: '0', zIndex: '3', background: '#1e3a8a', color: '#fff' } }, '5月'),
                  h('th', { style: { position: 'sticky', top: '0', zIndex: '3', background: '#1e3a8a', color: '#fff' } }, '6月'),
                ),
              ),
              h('tbody', {},
                ...['张三', '李四', '王五', '赵六', '钱七', '孙八', '周九', '吴十'].map((name, i) =>
                  h('tr', { key: i },
                    h('th', { style: { position: 'sticky', left: '0', zIndex: '2', background: '#e0e7ff' } }, name),
                    ...Array.from({ length: 6 }, (_, j) =>
                      h('td', { key: j, style: { background: '#fff' } }, String(100 + i * 10 + j)),
                    ),
                  ),
                ),
              ),
        ...
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '480px', overflow: 'auto' } },
          h('code', {}, s.stickyHeaderInfo || '（点击「运行 sticky 演示」查看 sticky 表头/首列 + overflow 陷阱 + border-collapse 边框 hack）')),
        h(Alert, {
          type: 'warning',
  ...
  _renderCard8(): Node | string {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'break-inside:avoid 避免行内分页、break-before:page 强制章节分页、thead display:table-header-group 每页重复表头。widows/orphans 控制段落孤行寡行（默认 2）。陷阱：rowspan 跨页裁剪、width:100% 列宽收缩、背景色默认不打印（需 print-color-adjust:exact）。@page 控制纸张 size/margin。生产建议 break-* 新名 + page-break-* 旧名都写。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行打印演示', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runPrintTrapsDemo() }),
          this._btn('打印预览（Ctrl+P）', { size: 'sm', onClick: () => { try { window.print(); } catch (e: any) { this._addLog('warn', `window.print 不可用：${e.message}`); } } }),
        ),
        h('div', { class: 'tbl-demo' },
          h('div', { class: 'fs-sm text-secondary mb-xs' }, '打印分页规则示意（@media print 内 break-inside:avoid / break-before:page）：'),
          h('pre', { style: { fontSize: '12px', lineHeight: '1.6', background: '#fff', padding: '8px', borderRadius: '4px', overflow: 'auto' } },
            `@media print {
  ...
  * { print-color-adjust: exact; }         /* 强制打印背景 */
  @page { size: A4 landscape; margin: 15mm; }
        ...
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.printTrapsInfo || '（点击「运行打印演示」查看 break-*/widows/orphans/@page + 合并单元格陷阱 + width 收缩）')),
        h(Alert, {
          type: 'info',
