...
interface WebMIDINFCGamepadPageState extends State {
  logs: any;
  capsSummary: any;
  midiInfo: any;
  nfcInfo: any;
  ...
  otpInfo: any;
  pressureInfo: any;
  digitalGoodsInfo: any;
}

export class WebMIDINFCGamepadPage extends Page {
  declare state: WebMIDINFCGamepadPageState;
  _gamepadTimer!: any;
  _gpConnUnbind!: any;
  _gpDiscUnbind!: any;
  _gpLogTs!: any;
  ...
  componentDidMount() {
    ...
    if (!caps.midi) this._addLog('warn', 'Web MIDI 不可用（typeof navigator.requestMIDIAccess !== "function"，jsdom 无此 API）');
    if (!caps.digitalGoods) this._addLog('warn', 'Digital Goods API 不可用（typeof window.digitalGoods === "undefined"，仅 Android Chrome + Play Billing）');

    // 注册 gamepadconnected / gamepaddisconnected 监听
    if (caps.gamepad && typeof win.addEventListener === 'function') {
      this._gpConnUnbind = this.on(win, 'gamepadconnected', (e: any) => {
        const gp = (e && e.gamepad) || {};
        this._addLog('pad', `gamepadconnected → index=${gp.index} id=${gp.id} buttons=${(gp.buttons || []).length}`);
      });
      this._gpDiscUnbind = this.on(win, 'gamepaddisconnected', (e: any) => {
        const gp = (e && e.gamepad) || {};
  ...
  componentWillUnmount() {
    ...
    if (this._nfcAbort) { try { this._nfcAbort.abort(); } catch { /* noop */ } this._nfcAbort = null; }
    if (this._nfcReader) {
      try { this._nfcReader.onreading = null; } catch { /* noop */ }
      try { this._nfcReader.onreadingerror = null; } catch { /* noop */ }
      this._nfcReader = null;
    }
    // 3. 断开 PressureObserver
    if (this._pressureObserver) { try { this._pressureObserver.disconnect(); } catch { /* noop */ } this._pressureObserver = null; }
    // 4. 停止 gamepad 轮询定时器
    if (this._gamepadTimer) { try { clearInterval(this._gamepadTimer); } catch { /* noop */ } this._gamepadTimer = null; }
    ...
    // 6. 取消 OTP 请求
    if (this._otpAbort) { try { this._otpAbort.abort(); } catch { /* noop */ } this._otpAbort = null; }
  }
  ...
  _caps(items: any) {
    return items.map(([name, ok]: any) =>
      h(Tag, { color: ok ? 'success' : 'error' }, `${name} ${ok ? '✓' : '✗'}`)
    );
  }
  ...
  async _midiRequest() {
    if (!this._has('midi')) {
      this.setState({ midiInfo:
        'Web MIDI 不可用（typeof navigator.requestMIDIAccess !== "function"）。\n\n' +
    ...
    try {
      ...
      const inputs: any[] = []; const outputs: any[] = [];
      access.inputs.forEach((p: any) => inputs.push({ name: p.name, manufacturer: p.manufacturer, state: p.state }));
      access.outputs.forEach((p: any) => outputs.push({ name: p.name, manufacturer: p.manufacturer, state: p.state }));
      ...
      const list = (inputs.length ? inputs.map((p: any) => `[in]  ${p.name || '?'} | ${p.manufacturer || '-'} | state=${p.state}`).join('\n') + '\n' : '') +
        (outputs.length ? outputs.map((p: any) => `[out] ${p.name || '?'} | ${p.manufacturer || '-'} | state=${p.state}`).join('\n') + '\n' : '') +
        (inputs.length === 0 && outputs.length === 0 ? '（jsdom 无真实 MIDI 设备，inputs/outputs 均为空 Map）\n' : '');
      this.setState({ midiInfo:
        'navigator.requestMIDIAccess({ sysex: false }) → MIDIAccess：\n' +
  ...
  _midiSendDemo() {
    if (!this._has('midi')) { this._addLog('warn', 'Web MIDI 不可用，无法演示 send 用法'); return; }
    this.setState({ midiInfo:
      'MIDIOutput.send 演示字节序列（中央 C = 60，力度 100）：\n\n' +
  ...
  _renderCard1() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['requestMIDIAccess', ok]]),
        h(Tag, { color: 'primary' }, 'MIDIAccess / Input / Output'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'navigator.requestMIDIAccess({ sysex }) 返回 Promise<MIDIAccess>。MIDIAccess 含 .inputs / .outputs（Map）、.onstatechange、.sysexEnabled。MIDIInput 监听 .onmidimessage 取 e.data（Uint8Array [status, data1, data2]）；MIDIOutput 提供 .send(data, timestamp) / .clear()。状态字节高 4 位命令（0x8 Note Off / 0x9 Note On / 0xB CC / 0xC PC / 0xE Pitch Bend），低 4 位通道（0-15）；音高与力度均为 0-127。sysex 需显式授权。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('请求 MIDI 访问', { type: 'primary', size: 'sm', disabled: !ok, onClick: () => this._midiRequest() }),
          this._btn('消息格式 / sysex', { size: 'sm', onClick: () => this._midiExplain() }),
          this._btn('send 字节演示', { size: 'sm', disabled: !ok, onClick: () => this._midiSendDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'MIDI 访问 / 用法：'),
        h('pre', { class: 'code-block', style: { maxHeight: '300px', overflow: 'auto' } },
          h('code', {}, s.midiInfo || '（点击「请求 MIDI 访问」或对应说明按钮）')),
        h(Alert, {
          type: 'info',
  ...
  async _nfcScan() {
    if (!this._has('nfc')) {
      this.setState({ nfcInfo:
        'Web NFC 不可用（typeof NDEFReader === "undefined"）。\n\n' +
    ...
    try {
      if (this._nfcAbort) { try { this._nfcAbort.abort(); } catch { /* noop */ } }
  ...
  _nfcBuildMessage() {
    if (!this._has('nfc')) { this._addLog('warn', 'Web NFC 不可用，无法构造 NDEFMessage'); return; }
    try {
      let built = false; let recordInfo = '';
      try {
        const msg = new NDEFMessage({ records: [{ recordType: 'text', data: 'Hello NFC' }] });
        built = true;
        const recs = msg.records || [];
        recordInfo = recs.map((r: any, i: number) => `record#${i}: recordType=${r.recordType} mediaType=${r.mediaType || '-'} id=${r.id || '-'}`).join('\n');
      } catch (e: any) {
  ...
  async _nfcWriteDemo() {
    if (!this._has('nfc')) { this._addLog('warn', 'Web NFC 不可用，无法演示 write'); return; }
    try {
  ...
  _renderCard2() {
    ...
    const card = new Card({
      title: '2. Web NFC API',
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['NDEFReader', ok]]),
        h(Tag, { color: 'primary' }, 'NDEFMessage / Record'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          "new NDEFReader() 构造。reader.scan({ signal }) 返回 Promise 开始监听标签；reader.write(message, { overwrite, signal }) 写入 NDEF 消息。reader.onreading = (e: any) => 取 e.serialNumber / e.message（NDEFMessage）；e.message.records 是 NDEFRecord 数组，recordType 含 'empty'|'text'|'url'|'smart-poster'|'absolute-url'|'mime'|'unknown'，可调用 toRecords() / toText() / toJSON() / toDataURL() 解码。仅 Android Chrome 可用。"),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('NFC 扫描', { type: 'primary', size: 'sm', disabled: !ok, onClick: () => this._nfcScan() }),
          this._btn('构造 NDEF 消息', { size: 'sm', disabled: !ok, onClick: () => this._nfcBuildMessage() }),
          this._btn('write 演示', { size: 'sm', disabled: !ok, onClick: () => this._nfcWriteDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'NFC 状态 / 用法：'),
        h('pre', { class: 'code-block', style: { maxHeight: '300px', overflow: 'auto' } },
          h('code', {}, s.nfcInfo || '（点击「NFC 扫描」或对应演示按钮）')),
        h(Alert, {
          type: 'warning',
  ...
  _gamepadScan() {
    if (!this._has('gamepad')) { this._addLog('warn', 'Gamepad API 不可用（typeof navigator.getGamepads !== "function"）'); return; }
    let pads: any[] = [];
    try { pads = Array.from((navigator as any).getGamepads() || []); } catch { pads = []; }
    const connected = pads.filter((p: any) => p);
    const list = connected.length
      ? connected.map((gp: any) =>
          `#${gp.index} id="${gp.id}" connected=${gp.connected} mapping="${gp.mapping}"\n  axes=[${gp.axes ? Array.from(gp.axes).map((a: any) => (a as any).toFixed(2)).join(', ') : ''}] (${gp.axes ? gp.axes.length : 0} 轴)\n` +
          `  buttons=${gp.buttons ? gp.buttons.length : 0} 个 timestamp=${gp.timestamp}\n  vibrationActuator=${gp.vibrationActuator ? '存在' : '无'}`
        ).join('\n')
      : '（jsdom polyfill 返回 [null,null,null,null]，无真实手柄；按下手柄任意键后才会出现在 getGamepads）';
    this.setState({ gamepadInfo:
      'navigator.getGamepads() 扫描结果：\n' +
  ...
  _gamepadPoll() {
    if (!this._has('gamepad')) return;
    let pads: any[] = [];
    try { pads = Array.from((navigator as any).getGamepads() || []).filter((p: any) => p); } catch { return; }
    if (pads.length === 0) return;
    const gp = pads[0];
    const now = Date.now();
    if (!this._gpLogTs || now - this._gpLogTs >= 1000) {
      this._gpLogTs = now;
      this._addLog('pad', `轮询 #${gp!.index} axes=[${gp!.axes ? Array.from(gp!.axes).map((a: any) => (a as any).toFixed(2)).join(',') : ''}] buttons=${gp!.buttons ? gp!.buttons.length : 0}`);
    }
  }
  ...
  _gamepadLayout() {
    this.setState({ gamepadInfo:
      '===== Gamepad 标准映射按键布局（mapping === "standard"）=====\n\n' +
  ...
  async _gamepadRumble() {
    if (!this._has('gamepad')) { this._addLog('warn', 'Gamepad API 不可用，无法触发振动'); return; }
    let pads: any[] = [];
    try { pads = Array.from((navigator as any).getGamepads() || []).filter((p: any) => p); } catch { pads = []; }
    if (pads.length === 0) {
      this.setState({ gamepadInfo:
        'GamepadHapticActuator 振动用法（无真实手柄，仅说明）：\n\n' +
    ...
    try {
      ...
      this.setState({ gamepadInfo:
        `GamepadHapticActuator.playEffect 演示：\n\n  手柄 #${gp.index} id="${gp.id}"\n` +
  ...
  _renderCard3() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['getGamepads', ok]]),
        h(Tag, { color: 'primary' }, 'Gamepad / Button / Actuator'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'navigator.getGamepads() 返回 Array<Gamepad|null>（始终长度 4）。Gamepad 含 .id / .index / .connected / .timestamp（输入变化时更新）/ .axes（Float32Array -1 到 1）/ .buttons（GamepadButton[]）/ .mapping（"standard"|""|"xr-standard"）/ .vibrationActuator。GamepadButton 含 .pressed / .touched / .value（0-1 模拟扳机）。window 监听 gamepadconnected / gamepaddisconnected。振动通过 vibrationActuator.playEffect("dual-rumble", { duration, startDelay, strongMagnitude, weakMagnitude })。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('扫描手柄', { type: 'primary', size: 'sm', disabled: !ok, onClick: () => this._gamepadScan() }),
          this._btn('停止轮询', { danger: true, size: 'sm', disabled: !ok, onClick: () => this._gamepadStop() }),
          this._btn('标准按键布局', { size: 'sm', onClick: () => this._gamepadLayout() }),
          this._btn('模拟振动', { type: 'primary', size: 'sm', disabled: !ok, onClick: () => this._gamepadRumble() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '手柄扫描 / 布局 / 振动：'),
        h('pre', { class: 'code-block', style: { maxHeight: '300px', overflow: 'auto' } },
          h('code', {}, s.gamepadInfo || '（点击「扫描手柄」或「标准按键布局」）')),
        h(Alert, {
          type: 'info',
  ...
  async _otpRequest() {
    if (!this._has('otp')) {
      this.setState({ otpInfo:
        'WebOTP 不可用（credentials.get 或 OTPCredential 缺失）。\n\n' +
    ...
    if (this._otpAbort) { try { this._otpAbort.abort(); } catch { /* noop */ } }
    const ac = new AbortController();
    this._otpAbort = ac;
    this._addLog('otp', "navigator.credentials.get({ otp: { transport:['sms'] } }) 调用中，等待 SMS…");
    try {
      const cred = await (navigator as any).credentials.get({ otp: { transport: ['sms'] }, signal: ac.signal });
      if (cred) {
        this.setState({ otpInfo:
          'navigator.credentials.get({ otp: { transport:["sms"] } }) → OTPCredential：\n\n' +
  ...
  _renderCard4() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['credentials.get(otp)', ok]]),
        h(Tag, { color: 'primary' }, 'OTPCredential'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          "navigator.credentials.get({ otp: { transport: ['sms'] } }) 返回 Promise<OTPCredential>。OTPCredential 含 .code（OTP 字符串）与 .type === 'otp'。要求页面 HTTPS 且 SMS 以 @origin #code 前缀开头（如 @example.com #12345 Your code is 123456）。配合 <input autocomplete=\"one-time-code\" inputmode=\"numeric\"> 接收。AbortController.signal 取消等待。"),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('请求 OTP', { type: 'primary', size: 'sm', disabled: !ok, onClick: () => this._otpRequest() }),
          this._btn('SMS 格式说明', { size: 'sm', onClick: () => this._otpExplain() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'WebOTP 状态 / 用法：'),
        h('pre', { class: 'code-block', style: { maxHeight: '300px', overflow: 'auto' } },
          h('code', {}, s.otpInfo || '（点击「请求 OTP」或「SMS 格式说明」）')),
        h(Alert, {
          type: 'info',
  ...
  _pressureStart() {
    if (!this._has('pressure')) {
      this.setState({ pressureInfo:
        'Compute Pressure 不可用（typeof PressureObserver === "undefined"）。\n\n' +
    ...
    try {
      if (this._pressureObserver) { try { this._pressureObserver.disconnect(); } catch { /* noop */ } }
      let observed = false;
      ...
      try { observer.observe('cpu'); observed = true; }
      catch (e: any) { this._addLog('warn', `observe('cpu') 失败：${errInfo(e).name || 'Error'} — ${errInfo(e).message || ''}`); }
      const knownSources = PressureObserver.knownSources || PressureObserver.supportedSources || [];
      this.setState({ pressureInfo:
        '已创建 PressureObserver 并观察 cpu：\n\n  new PressureObserver(callback, { sampleRate: 1.0 })\n' +
  ...
  _pressureStop() {
    if (!this._pressureObserver) { this._addLog('warn', '无 PressureObserver 可断开'); return; }
    try {
  ...
  _renderCard5() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['PressureObserver', ok]]),
        h(Tag, { color: 'primary' }, 'PressureRecord'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          "new PressureObserver(callback, { sampleRate }) 创建观察器。callback 收到 (changes, observer)，changes 为 PressureRecord[]。.observe('cpu'|'gpu') 开始观察，.unobserve(source) / .disconnect() / .takeRecords() 控制。PressureRecord 含 .source / .state（'nominal'|'fair'|'serious'|'critical'）/ .time / .averages / .maxes。静态 PressureObserver.knownSources 列已知源。用途：自适应流媒体画质、节流后台任务、游戏特效降级。"),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('开始观察 cpu', { type: 'primary', size: 'sm', disabled: !ok, onClick: () => this._pressureStart() }),
          this._btn('断开 observer', { danger: true, size: 'sm', disabled: !ok, onClick: () => this._pressureStop() }),
          this._btn('档位 / 场景说明', { size: 'sm', onClick: () => this._pressureExplain() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'Compute Pressure 状态 / 用法：'),
        h('pre', { class: 'code-block', style: { maxHeight: '300px', overflow: 'auto' } },
          h('code', {}, s.pressureInfo || '（点击「开始观察 cpu」或「档位说明」）')),
        h(Alert, {
          type: 'info',
  ...
  _dgDetect() {
    ...
    this.setState({ digitalGoodsInfo:
      'Digital Goods API 能力检测：\n\n' +
  ...
  _dgExplainFlow() {
    this.setState({ digitalGoodsInfo:
      '===== Digital Goods / Play Billing 集成流程 =====\n\n' +
  ...
  async _dgListPurchases() {
    if (!this._has('digitalGoods')) {
      this.setState({ digitalGoodsInfo:
        'listPurchases 不可用（window.digitalGoods === undefined）。\n\n' +
    ...
    try {
      ...
      const items = list.length
        ? list.map((it: any) => `  [${it.itemId}] ${it.title} | ${it.description || '-'} | ${it.price || '-'}`).join('\n')
        : '（无已购商品）';
      this.setState({ digitalGoodsInfo:
        `DigitalGoodsService 真实调用：\n\n  getPublisherName() → "${publisher}"\n  listPurchases() → ${list.length} 个已购商品\n\n` + items });
  ...
  _renderCard6() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['digitalGoods', ok]]),
        h(Tag, { color: 'primary' }, 'Play Billing'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
        ...
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('检测能力', { type: 'primary', size: 'sm', onClick: () => this._dgDetect() }),
          this._btn('Play Billing 流程', { size: 'sm', onClick: () => this._dgExplainFlow() }),
          this._btn('listPurchases', { size: 'sm', disabled: !ok, onClick: () => this._dgListPurchases() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'Digital Goods 状态 / 用法：'),
        h('pre', { class: 'code-block', style: { maxHeight: '300px', overflow: 'auto' } },
          h('code', {}, s.digitalGoodsInfo || '（点击「检测能力」或「Play Billing 流程」）')),
        h(Alert, {
          type: 'warning',
  ...
  render() {
    const s = this.state;
    return h('div', { class: 'api-lab-page midi-nfc-gamepad-page' },
      h('h2', { class: 'section-title' }, '设备通信与输入 API 实验室'),
      h('p', { class: 'fs-sm text-secondary mb-md' },
        'Web MIDI / Web NFC / Gamepad / WebOTP / Compute Pressure / Digital Goods —— 6 类与外部设备 / 系统状态通信的 API。本页聚焦 NativeInteropPage 与 HardwareDevicesPage 未深入的消息格式、记录结构与购买流程。'),
