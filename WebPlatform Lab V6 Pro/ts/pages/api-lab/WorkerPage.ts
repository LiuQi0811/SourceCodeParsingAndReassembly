...
// 主线程版本：埃氏筛
function sieveMain(n: number): number {
...
// 主线程版本：fib（BigInt）
function fibMain(n: number): string {
  let a = 0n, b = 1n;
  for (let i = 0; i < n; i++) {
    [a, b] = [b, a + b];
  }
...
export interface WorkerPageProps extends Props {}
...
export interface WorkerPageState extends State {
  sieveN: number;
  fibN: number;
  running: boolean;
...
export class WorkerPage extends Page {
  declare props: WorkerPageProps;
  declare state: WorkerPageState;
  _worker: Worker | null = null;
  _onWorkerMessage: ((e: MessageEvent) => void) | null = null;

  initialState(): WorkerPageState {
    return {
      sieveN: 5_000_000,
      fibN: 1000,
      running: false,
  ...
  componentDidMount(): void {
    ...
    this._onWorkerMessage = (e: MessageEvent) => {
      const { type, payload } = e.data as WorkerMessageData;
    ...
    this._worker.addEventListener('error', (err: ErrorEvent) => {
  ...
  _append(type: string, content: string): void {
  ...
  async _runSieveCompare(): Promise<void> {
  ...
  async _runFibCompare(): Promise<void> {
    if (this.state.running) return;
  ...
  renderPage(): Node | string | (Node | string)[] {
    ...
    return [
      ...
      // 操作面板
      h(Card, { title: '任务面板', extra: h(Tag, { color: this.state.running ? 'error' : 'success' }, this.state.running ? '运行中' : '空闲') },
        h('div', { class: 'flex flex-col gap-md' },
          // 埃氏筛
          h('div', { class: 'flex items-center gap-sm flex-wrap' },
            h('span', { class: 'fw-medium' }, '埃氏筛'),
            h('span', { class: 'fs-sm text-secondary' }, 'N'),
            sieveInput.render(),
            this._btn('主线程 vs Worker 对比', {
          ...
          // fib
          h('div', { class: 'flex items-center gap-sm flex-wrap' },
            h('span', { class: 'fw-medium' }, 'Fibonacci（BigInt）'),
            h('span', { class: 'fs-sm text-secondary' }, 'N'),
            fibInput.render(),
            this._btn('主线程 vs Worker 对比', {
              type: 'primary', size: 'sm',
          ...
          h('p', { class: 'fs-sm text-tertiary' },
            '提示：在 Worker 运行期间，尝试拖动页面、点击按钮 —— UI 不会卡顿；而主线程版本运行时会冻结界面。'),
        ),
      ),

      // 结果对比表
      h(Card, { title: '执行结果对比', extra: h(Tag, { color: 'primary' }, `${this.state.results.length} 条`) },
        this.state.results.length === 0
          ? h('div', { class: 'fs-sm text-tertiary' }, '尚未执行任务，点击上方按钮开始')
          : h('div', { class: 'log-panel' },
            ...this.state.results.map((r: WorkerResult) => h('div', { class: 'log-panel__line' },
              h('span', { class: 'log-panel__time' }, r.time),
              h('span', { class: 'log-panel__tag log-panel__tag--push' }, r.task),
              h('span', {}, `耗时 ${formatDuration(r.elapsed)}  ·  结果长度 ${String(r.result).length}`),
            )),
          ),
      ),

      // 代码片段
      h(Card, { title: 'compute.worker.js 关键代码' },
