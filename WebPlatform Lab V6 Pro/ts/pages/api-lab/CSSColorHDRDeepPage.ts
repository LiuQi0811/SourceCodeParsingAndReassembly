...
interface CSSColorHDRDeepPageCaps {
  css: boolean;
  supports: boolean;
  colorFunc: boolean;
  oklch: boolean;
  ...
  lch: boolean;
  colorMix: boolean;
  relativeColor: boolean;
  relativeOklch: boolean;
  dynamicRangeHigh: boolean;
  colorGamutP3: boolean;
  colorGamutRec2020: boolean;
  prefersContrast: boolean;
  canvasColorSpace: boolean;
}

export interface CSSColorHDRDeepPageProps extends Props {}

export interface CSSColorHDRDeepPageState extends State {
  logs: LogEntry[];
  capsSummary: string;
  overviewInfo: string;
  colorFuncInfo: string;
  oklchInfo: string;
  colorMixInfo: string;
  relativeColorInfo: string;
  hdrInfo: string;
...
export class CSSColorHDRDeepPage extends Page {
  declare props: CSSColorHDRDeepPageProps;
  declare state: CSSColorHDRDeepPageState;

  _inited: boolean = false;
  declare _destroyed: boolean;
  _dynamicStyles!: any[];
  _colorSpaceMode!: string;
  _oklchMode!: string;
  _mixMode!: string;
  _relativeMode!: string;
  _hdrMode!: string;
  _renderingMode!: string;


  // —— 初始 state ——
  initialState(): CSSColorHDRDeepPageState {
    return {
      logs: [],
      capsSummary: '',
  ...
  _flags(): CSSColorHDRDeepPageCaps {
    const hasCSS = this._safe(() => typeof CSS !== 'undefined');
    const supportsPV = (p: any,v: any) => this._safe(() =>
      hasCSS && typeof CSS.supports === 'function' && CSS.supports(p, v));
    const matchMQ = (mq: any) => this._safe(() =>
  ...
  _readColorFuncInfo(): any {
    const f = this._flags();
    try {
      const swatch = this.el && (this.el as any as Element | null)?.querySelector('.cs-display-p3');
      let computed = '(未渲染)';
      if (swatch) {
  ...
  _setColorSpaceMode(mode: any): void  {
  ...
  _readOklchInfo(): any {
    const f = this._flags();
    try {
      const step = this.el && (this.el as any as Element | null)?.querySelector('.oklch-l70');
      let computed = '(未渲染)';
      if (step) {
  ...
  _readColorMixInfo(): any {
    const f = this._flags();
    try {
      const mix = this.el && (this.el as any as Element | null)?.querySelector('.mix-oklch-50');
      let computed = '(未渲染)';
      if (mix) {
  ...
  _readRelativeColorInfo(): any {
    const f = this._flags();
    try {
      const rel = this.el && (this.el as any as Element | null)?.querySelector('.rel-lighten');
      let computed = '(未渲染)';
      if (rel) {
  ...
  _setRelativeMode(mode: any): void  {
  ...
  _runRenderingDemo(): void {
    ...
    try {
      const canvas = this.el && (this.el as any as Element | null)?.querySelector('.canvas-stage canvas') as HTMLCanvasElement | null;
      if (!canvas) {
      ...
      const out = (this.el as any as Element | null)?.querySelector('.rendering-output');
      if (out) {
