...
function createNaiveAsyncContext() {
  ...
  function Variable(name: any) {
    ...
    const v = {
      name,
      run(value: any, callback: any) {
        stack.push(value);
        try { return callback(); } finally { stack.pop(); }
      },
  ...
  function Snapshot() {
    ...
    for (const v of registry) captured.set(v, v._stack[v._stack.length - 1]);
    return {
      run(callback: any) {
        for (const [v, val] of captured) v._stack.push(val);
        try { return callback(); } finally {
          for (const [v] of captured) v._stack.pop();
        }
      },
    };
  }
  // Snapshot.wrap(fn)：捕获当前快照，返回的函数在调用时恢复快照后执行 fn
  Snapshot.wrap = function (fn: any) {
    const snap = Snapshot();
    return function (this: any, ...args: any[]) { return snap.run(() => fn.apply(this, args)); };
  };
...
interface AsyncContextPageCaps {
  asyncContext: boolean;
  variable: boolean;
  snapshot: boolean;
  snapshotWrap: boolean;
}

export interface AsyncContextPageProps extends Props {}

export interface AsyncContextPageState extends State {
  logs: LogEntry[];
  capsSummary: string;
  problemInfo: string;
  variableInfo: string;
  snapshotInfo: string;
  awaitBoundaryInfo: string;
  fetchInfo: string;
  schedulerWorkerInfo: string;
  apmInfo: string;
  comparisonInfo: string;
}

export class AsyncContextPage extends Page {
  declare props: AsyncContextPageProps;
  declare state: AsyncContextPageState;
  ...
  // —— 初始 state ——
  initialState(): AsyncContextPageState {
    return {
      logs: [],
      capsSummary: '',
  ...
  componentDidMount(): void {
    ...
    if (!c.asyncContext) this._addLog('warn', 'AsyncContext 不可用：TC39 Stage 2/3 提案，jsdom/Node 当前未实现；Card 2/3/4/7 使用朴素 polyfill 演示');
    if (!f.fetch) this._addLog('warn', 'fetch 不可用（jsdom 通常提供，Node 18+ 内置）');
    if (!f.abortSignalAny) this._addLog('warn', 'AbortSignal.any 不可用（较新环境才支持）');
    if (!f.schedulerPostTask) this._addLog('warn', 'scheduler.postTask 不可用（Chrome 94+，Firefox/Safari 无）');
    if (!f.worker) this._addLog('warn', 'Worker 不可用（jsdom 不支持）');
  ...
  _renderCard1(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: c.asyncContext ? 'success' : 'warning' }, c.asyncContext ? 'AsyncContext ✓' : '提案未落地'),
        h(Tag, { color: 'primary' }, 'zone.js · AsyncLocalStorage'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '异步调用栈中状态丢失：每个 await 进入新微任务，调用栈上的本地变量不再可达。传统显式传参会污染所有层签名；用全局变量临时存放会被并发请求互相覆盖（串号、跨用户泄漏）。zone.js 通过 monkey-patch 异步 API 传播上下文（侵入式、有性能开销）；Node AsyncLocalStorage 基于 async_hooks 在运行时层传播（仅 Node）。AsyncContext（TC39 Stage 2/3）目标是在语言层标准化上下文容器语义，由引擎原生实现，零开销。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('展开问题与方案', { type: 'primary', size: 'sm', onClick: () => this._demoProblemHistory() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '问题与历史方案：'),
        h('pre', { class: 'code-block', style: { maxHeight: '320px', overflow: 'auto' } },
          h('code', {}, s.problemInfo || '（点击「展开问题与方案」）')),
        ...
        h('pre', { class: 'code-block', style: { maxHeight: '160px', overflow: 'auto' } },
          h('code', {},
`// AsyncContext.Variable：声明一个「上下文变量」，跨 await 自动保留
...
traceId.get();                     // undefined（run 作用域外）`)),
  ...
  _demoVariable(): any {
    ...
    this.setState({
      variableInfo:
        `AsyncContext.Variable 演示（朴素 polyfill，同步部分真实运行）：\n${L.join('\n')}\n\n` +
        `参考 API：\n` +
  ...
  _renderCard2(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: c.variable ? 'success' : 'warning' }, c.variable ? 'Variable ✓' : '用 polyfill 演示'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'new AsyncContext.Variable(name?) 声明一个上下文变量；var.run(value, callback) 设值并执行 callback，callback 内（含同步子调用）var.get() 读取该值；run 返回 callback 的返回值。嵌套 run 隔离：内层 run 的值不影响外层，退出内层后 get() 恢复外层值。多个 Variable 互相独立。run 作用域外 get() 返回 undefined。本卡片用「朴素 polyfill（栈式）」真实运行同步部分；跨 await 行为见 Card 4。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('演示 Variable', { type: 'primary', size: 'sm', onClick: () => this._demoVariable() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'Variable 演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '280px', overflow: 'auto' } },
          h('code', {}, s.variableInfo || '（点击「演示 Variable」）')),
...
v.get();                // undefined（run 外）`)),
  ...
  _demoSnapshot(): any {
    ...
    this.setState({
      snapshotInfo:
        `AsyncContext.Snapshot 演示（朴素 polyfill，同步部分真实运行）：\n${L.join('\n')}\n\n` +
        `参考 API：\n` +
  ...
  _renderCard3(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: c.snapshot ? 'success' : 'warning' }, c.snapshot ? 'Snapshot ✓' : '用 polyfill 演示'),
        h(Tag, { color: c.snapshotWrap ? 'primary' : 'warning' }, c.snapshotWrap ? 'wrap ✓' : 'wrap ✗'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'new AsyncContext.Snapshot() 一次性捕获「所有 Variable 的当前值」；snap.run(callback) 把所有 Variable 恢复到捕获时状态后执行 callback。Snapshot.wrap(fn) 是工厂：返回一个函数，调用时先恢复快照再执行 fn——适合把「注册时上下文」带给「触发时回调」（如 setTimeout/addEventListener 回调）。Variable.run 只设一个变量；Snapshot.run 整体恢复一组。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('演示 Snapshot', { type: 'primary', size: 'sm', onClick: () => this._demoSnapshot() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'Snapshot 演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '300px', overflow: 'auto' } },
          h('code', {}, s.snapshotInfo || '（点击「演示 Snapshot」）')),
...
// 用户点击时 v.get() 在 wrap 内仍是 'req-1'，而非 undefined`)),
    ...
    try {
      ...
      traceId.run('timer-ctx', () => {
        timerId = setTimeout(() => {
          ...
          this.setState({
            awaitBoundaryInfo:
              `AsyncContext 跨 await/Promise 边界演示（朴素 polyfill，标注失效点）：\n${L.join('\n')}\n\n` +
              `参考（真实 API 行为）：\n` +
              `  const v = new AsyncContext.Variable('v');\n` +
              `  v.run('ctx', async () => {\n` +
        ...
        if (timerId) this._pendingTimers.push(timerId);
      });
    } catch (err: any) {
  ...
  _renderCard4(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: c.asyncContext ? 'success' : 'warning' }, c.asyncContext ? '真实 API ✓' : 'polyfill 标注失效'),
        h(Tag, { color: 'primary' }, 'await · setTimeout · rAF'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'AsyncContext 的核心承诺：在 var.run(value, callback) 作用域内，callback 触发的所有异步续体（await、Promise.then、queueMicrotask、setTimeout、requestAnimationFrame）都能 var.get() 读到 value。机制：引擎在「调度续体」时携带「当前上下文快照」，续体执行前恢复。本卡用朴素 polyfill 演示「跨 await 失效」（栈在 finally 已 pop），并对照真实 API 行为；若环境有真实 AsyncContext，会一并跑真实用例。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('演示跨 await 边界', { type: 'primary', size: 'sm', onClick: () => this._demoAwaitBoundary() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '跨 await 演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '300px', overflow: 'auto' } },
          h('code', {}, s.awaitBoundaryInfo || '（点击「演示跨 await 边界」）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考：续体类型与是否保留'),
        h('pre', { class: 'code-block', style: { maxHeight: '150px', overflow: 'auto' } },
          h('code', {},
`// 真实 AsyncContext 在以下续体中均自动保留 v.get()
v.run('ctx', async () => {
...
// ✗ 不保留：addEventListener 回调（注册与触发跨多次任务，需 Snapshot.wrap）`)),
  ...
  // =================== Card 5：与 fetch 协同 ===================
  _demoFetchCoordination(): any {
    const c = this._caps();
    const f = this._flags();
    const info =
      '===== AsyncContext 与 fetch / Response.body / Service Worker 协同 =====\n\n' +
  ...
  _renderCard5(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: f.fetch ? 'success' : 'error' }, f.fetch ? 'fetch ✓' : 'fetch ✗'),
        h(Tag, { color: f.abortSignalAny ? 'success' : 'warning' }, f.abortSignalAny ? 'AbortSignal.any ✓' : 'any ✗'),
        h(Tag, { color: c.asyncContext ? 'success' : 'warning' }, c.asyncContext ? 'AsyncContext ✓' : '提案未落地'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'AsyncContext 与 fetch 协同的三个典型场景：(1) 全局 fetch 包装从上下文读 traceId 注入 header，所有请求自动带 traceId；(2) Response.body 流式读取跨 chunk 保留上下文；(3) AbortSignal.any 合并「用户取消/超时/父任务取消」与 AsyncContext 配合实现可取消的可追踪请求。Service Worker 的 fetch 事件是独立全局，AsyncContext 不跨进程边界——需把 traceId 序列化进 request header 显式传递。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('展开 fetch 协同', { type: 'primary', size: 'sm', onClick: () => this._demoFetchCoordination() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'fetch 协同说明：'),
        h('pre', { class: 'code-block', style: { maxHeight: '360px', overflow: 'auto' } },
          h('code', {}, s.fetchInfo || '（点击「展开 fetch 协同」）')),
...
traceId.run('req-abc', async () => {
  ...
  _demoSchedulerWorker(): void {
    const c = this._caps();
    const f = this._flags();
    const info =
      '===== AsyncContext 与 scheduler / Worker 协同：传播矩阵 =====\n\n' +
  ...
  _renderCard6(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: f.schedulerPostTask ? 'success' : 'warning' }, f.schedulerPostTask ? 'postTask ✓' : 'postTask ✗'),
        h(Tag, { color: f.schedulerYield ? 'success' : 'warning' }, f.schedulerYield ? 'yield ✓' : 'yield ✗'),
        h(Tag, { color: f.worker ? 'success' : 'warning' }, f.worker ? 'Worker ✓' : 'Worker ✗'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'scheduler.postTask/scheduler.yield（Chrome 94+）是浏览器优先级调度 API，真实 AsyncContext 在其续体中保留上下文。Worker postMessage 跨进程，AsyncContext 不传播——必须把上下文值序列化进 message，Worker 内 onmessage 重新 run 建立。可转移对象（Transferable）只能零拷贝移交 ArrayBuffer/MessagePort 等，不能传播 AsyncContext；structuredClone 限制 Function/DOM 节点不可克隆，所以上下文里只能传「值」。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('展开传播矩阵', { type: 'primary', size: 'sm', onClick: () => this._demoSchedulerWorker() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'scheduler/Worker 协同说明：'),
        h('pre', { class: 'code-block', style: { maxHeight: '380px', overflow: 'auto' } },
          h('code', {}, s.schedulerWorkerInfo || '（点击「展开传播矩阵」）')),
...
self.onmessage = (e) => traceId.run(e.data.traceId, () => handle(e.data.payload));`)),
  ...
  _demoApmMultiTenant(): any {
    ...
    this.setState({
      apmInfo:
        `AsyncContext 实战：APM tracing + 多租户 + 多语言 SSR（朴素 polyfill 同步模拟）：\n${L.join('\n')}\n\n` +
        `参考：多租户 fetch 包装\n` +
  ...
  _renderCard7(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: c.asyncContext ? 'success' : 'warning' }, c.asyncContext ? 'AsyncContext ✓' : 'polyfill 模拟'),
        h(Tag, { color: 'primary' }, 'otel · tenantId · locale'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '三大实战场景：(1) APM tracing——OpenTelemetry Web SDK 的 context API 与 AsyncContext 同构，traceId 自动随异步续体传播，子 span 无需显式传 context；(2) 多语言 SSR——locale 在 run 作用域内跨 await 渲染保留，i18n 不需每个组件传 locale；(3) 多租户 SaaS——tenantId 透传到所有 fetch，杜绝逐层传参与全局变量串号。本卡用朴素 polyfill 同步模拟「3 个子 fetch 自动带 tid/tenant」与「并发租户隔离」。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('演示 APM/多租户', { type: 'primary', size: 'sm', onClick: () => this._demoApmMultiTenant() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'APM/多租户演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '360px', overflow: 'auto' } },
          h('code', {}, s.apmInfo || '（点击「演示 APM/多租户」）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考：多语言 SSR locale 透传'),
        h('pre', { class: 'code-block', style: { maxHeight: '150px', overflow: 'auto' } },
          h('code', {},
`const locale = new AsyncContext.Variable('locale');
...
async function renderApp() {
  ...
  // =================== Card 8：对照与陷阱 ===================
  _demoComparisonPitfalls(): any {
    const c = this._caps();
    const f = this._flags();
    const info =
      '===== AsyncContext 与 AsyncLocalStorage 对照 + 陷阱 + 协同 =====\n\n' +
  ...
  _renderCard8(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: c.asyncContext ? 'success' : 'warning' }, c.asyncContext ? 'AsyncContext ✓' : '提案未落地'),
        h(Tag, { color: f.symbolDispose ? 'success' : 'warning' }, f.symbolDispose ? 'Symbol.dispose ✓' : 'dispose ✗'),
        h(Tag, { color: 'primary' }, 'ALS · zone.js · using'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'AsyncContext.Variable 与 Node AsyncLocalStorage API 形态接近（run/get/wrap），未来 Node 可基于 AsyncContext 重实现 ALS。Polyfill（zone.js / asynccontext npm）有 10-30% 开销，原生实现目标零开销。常见陷阱：new Promise 续体调度点决定是否保留、addEventListener 回调需 Snapshot.wrap、Worker/SW/iframe 跨进程不传播。与 using/Symbol.dispose（ERM 提案）协同可实现「作用域级资源 + 上下文」统一管理。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('展开对照与陷阱', { type: 'primary', size: 'sm', onClick: () => this._demoComparisonPitfalls() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '对照与陷阱说明：'),
        h('pre', { class: 'code-block', style: { maxHeight: '420px', overflow: 'auto' } },
          h('code', {}, s.comparisonInfo || '（点击「展开对照与陷阱」）')),
...
v.run('req-1', () => {
  ...
  render(): Node {
    ...
    return h('div', { class: 'page api-lab-page' },
      h('h2', { class: 'section-title' }, 'AsyncContext 异步上下文透传实验室'),
      h('p', { class: 'fs-sm text-secondary mb-md' },
        'TC39 Stage 2/3 提案：AsyncContext.Variable / AsyncContext.Snapshot。跨 await/Promise 边界自动保留上下文，解决「异步调用栈状态丢失」与「全局变量并发串号」。含 zone.js/AsyncLocalStorage 对照、fetch/scheduler/Worker 协同、APM tracing/多租户实战、polyfill 与陷阱。'),
