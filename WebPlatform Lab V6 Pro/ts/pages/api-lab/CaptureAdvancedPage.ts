...
interface CaptureAdvancedPageCaps {
  getDisplayMedia: boolean;
  captureController: boolean;
  restrictionTarget: boolean;
  cropTo: boolean;
  clipboardItem: boolean;
  clipboardItemSupports: boolean;
  clipboardWrite: boolean;
}

export interface CaptureAdvancedPageProps extends Props {}

export interface CaptureAdvancedPageState extends State {
  logs: LogEntry[];
  capsSummary: string;
  gdmState: string;
  controllerState: string;
  elementCaptureState: string;
  clipboardState: string;
}

export class CaptureAdvancedPage extends Page {
  declare props: CaptureAdvancedPageProps;
  declare state: CaptureAdvancedPageState;
  ...
  initialState(): CaptureAdvancedPageState {
    return {
      logs: [],
      capsSummary: '',
      gdmState: '',
  ...
  _renderCard1(): Node | string {
    ...
    return h(Card, {
      ...
      s.gdmState ? h('p', { class: 'fs-sm mt-sm text-secondary' }, s.gdmState) : null as any,
...
track.onended = () => { /* 用户停止共享 */ };`),
  ...
  _demoCaptureController(): void {
    ...
    try {
      ...
      if (focusSupported) {
        try { controller.setFocus!(true); this._addLog('info', 'controller.setFocus(true) 已调用（聚焦共享源窗口）'); } catch (e: any) { this._addLog('warn', 'setFocus 调用失败：' + (e && e.message)); }
      }
      if (surfaceSupported) {
        try { controller.setDisplaySurface!('monitor'); this._addLog('info', 'controller.setDisplaySurface("monitor") 已调用（运行时切换共享源 surfaceSwitching）'); } catch (e: any) { this._addLog('warn', 'setDisplaySurface 调用失败：' + (e && e.message)); }
      }
      this.setState({
  ...
  _renderCard2(): Node | string {
    ...
    return h(Card, {
      ...
      s.controllerState ? h('p', { class: 'fs-sm mt-sm text-secondary' }, s.controllerState) : null as any,
      h('pre', { class: 'code-block mt-md' },
`const controller = new CaptureController();
const stream = await navigator.mediaDevices.getDisplayMedia({
...
controller.cropTo(restrictionTarget); // Element/Region Capture 裁剪`),
  ...
  _convertToRestrictionTarget(): void {
    const caps = this._caps();
    ...
    const el = (this.$('#ca-target') as any);
  ...
  _renderCard3(): Node | string {
    ...
    return h(Card, {
      ...
      h('p', { class: 'fs-sm text-secondary' },
        '(RestrictionTarget as any).fromElement(element) 把 DOM 元素转为 RestrictionTarget；controller.cropTo(restrictionTarget) 把整页捕获裁剪到该元素；' +
      ...
      s.elementCaptureState ? h('p', { class: 'fs-sm mt-sm text-secondary' }, s.elementCaptureState) : null as any,
      h('pre', { class: 'code-block mt-md' },
    ...
    return h(Card, {
      ...
      h('div', { class: 'ca-compare' },
        ...
        h('div', { class: 'ca-compare-row' },
          ...
          h('div', { class: 'fs-sm' }, '(RestrictionTarget as any).fromElement(el) + cropTo(rt) 只共享该元素，元素滚动/变换自动跟随，最精确。'),
        ),
      ),
...
// displaySurface: 'browser' | 'window' | 'screen' | 'application'`),
  ...
  _asyncWriteClipboard(): void {
    ...
    try {
      ...
      const textBlobPromise = new Promise<Blob>((resolve) => {
        setTimeout(() => {
      ...
      const item = (new ClipboardItem({ 'text/plain': textBlobPromise }) as any);
  ...
  _renderCard5(): Node | string {
    ...
    return h(Card, {
      ...
      s.clipboardState ? h('p', { class: 'fs-sm mt-sm text-secondary' }, s.clipboardState) : null as any,
      h('pre', { class: 'code-block mt-md' },
...
await navigator.clipboard.write([item]);  // 一次写多种格式，目标应用自选`),
  ...
  _renderCard6(): Node | string {
    const caps = this._caps();
    const mk = (api: any,version: any,scene: any,ok: any) => h('tr', {},
      h('td', {}, api),
      h('td', {}, version),
      h('td', {}, scene),
      h('td', {}, ok ? '✓' : '✗'),
