...
export interface SecurityPrivacyPageProps extends Props {}

export interface SecurityPrivacyPageState extends State {}

export class SecurityPrivacyPage extends Page {
  declare props: SecurityPrivacyPageProps;
  declare state: SecurityPrivacyPageState;
  _inited: boolean = false;
  _onSpv: any = null;
  initialState(): SecurityPrivacyPageState {
    return {
      logs: [],
      // Card 1: CSP / SRI
  ...
  _logCapabilities() {
    const caps = [
      ...
      ['element.setHTML', typeof Element !== 'undefined' && typeof (Element.prototype as any).setHTML === 'function'],
    ...
    const summary = caps.map(([n, ok]: any) => `${n}:${ok ? '✓' : '✗'}`).join('  ');
  ...
  _detectCspMeta() {
    try {
      ...
      const contents = Array.from(metas).map((m: any) => m.getAttribute('content') || '').filter(Boolean);
  ...
  _checkAllowsFeature() {
    ...
    try {
      ...
      const lines = probes.map(([feature, origin]: any) => {
        let allowed: boolean | string = false;
        try {
  ...
  async _queryPermissions() {
    ...
    for (const name of names) {
      try {
        ...
        ((result as any)[(name as any)]) = status.state; // 'granted' | 'denied' | 'prompt'
        // 监听变化（一次性）
        status.onchange = () => {
          this._addLog('perm', `权限 ${name} 状态变更 => ${status.state}`);
        };
      } catch (err: any) {
        ((result as any)[(name as any)]) = 'unsupported';
        this._addLog('perm', `query(${name}) 失败：${err.name}`);
      }
    }
    this.setState({ permissionsStatus: result });
    const summary = (Object as any).entries(result).map(([k, v]: any) => `${k}=${v}`).join(', ');
  ...
  _runPermissionsPolicyDemo() {
    ...
    try {
      ...
      if (info.hasFeaturePolicy) {
        ...
        } catch (e: any) { info.notes.push(`allowedFeatures err:${e.name}`); }
        ...
        } catch (e: any) { info.notes.push(`features err:${e.name}`); }
        ...
        for (const f of probes) {
          try {
            if (typeof document.featurePolicy.getAllowlistForFeature === 'function') {
              ((info.allowlist as any)[(f as any)]) = document.featurePolicy.getAllowlistForFeature(f);
            }
          } catch (e: any) { ((info.allowlist as any)[(f as any)]) = `err:${e.name}`; }
          const perOrigin: Record<string, any> = {};
          for (const origin of [undefined, 'https://trusted.example.com', 'https://evil.example.com']) {
            ...
            } catch (e: any) { perOrigin[origin || 'self'] = `err:${e.name}`; }
          }
          ((info.allowsFeature as any)[(f as any)]) = perOrigin;
        }
      }
  ...
  _checkTrustedTypes() {
    ...
    try {
      ...
      const policy = window.trustedTypes.createPolicy('api-lab-tt', {
        createHTML: (s: any) => s.replace(/<script>/gi, '&lt;script&gt;'),
      });
      ...
      const target = (this.$('.tt-demo-target') as any);
      if (target) {
  ...
  _sanitizerDemo() {
    ...
    try {
      // 自定义允许/拒绝清单
      const sanitizer: any = new window.Sanitizer({
      ...
      const target = (this.$('.san-demo-target') as any);
  ...
  renderPage(): Node | string | (Node | string)[] {
    ...
    const hasSetHTML = typeof Element !== 'undefined' && typeof (Element.prototype as any).setHTML === 'function';

    const permEntries = (Object as any).entries(s.permissionsStatus);
        ...
        h('div', { class: 'flex flex-col gap-sm' },
          ...
          permEntries.length > 0
            ? h('div', { class: 'flex items-center gap-xs flex-wrap' },
                ...permEntries.map(([name, state]: any) =>
  ...
  _renderCard9() {
    ...
    const card = new Card({
      ...
      children: h('div', { class: 'flex flex-col gap-sm' },
        ...
        info && (Object as any).keys(info.allowlist).length > 0 && h('div', { class: 'fs-sm text-tertiary', style: { wordBreak: 'break-all' } },
          `getAllowlistForFeature：${(Object as any).entries(info.allowlist).map(([k, v]: [any, any]) => `${k}=${JSON.stringify(v)}`).join(' | ')}`),
        info && (Object as any).keys(info.allowsFeature).length > 0 && h('div', { class: 'fs-sm text-tertiary', style: { wordBreak: 'break-all' } },
          `allowsFeature 探测：${(Object as any).entries(info.allowsFeature).map(([f, per]: [any, any]) =>
