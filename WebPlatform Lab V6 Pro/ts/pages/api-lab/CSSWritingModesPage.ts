...
interface CSSWritingModesPageState extends State {
  logs: LogEntry[];
  capsSummary: string;
  overviewInfo: string;
  writingModeInfo: string;
  directionBidiInfo: string;
  textOrientationInfo: string;
  logicalPropsInfo: string;
  nestingInfo: string;
  cjkVerticalInfo: string;
  i18nInfo: string;
}
...
export class CSSWritingModesPage extends Page {
  declare state: CSSWritingModesPageState;
  ...
  // —— 初始 state ——
  initialState(): CSSWritingModesPageState {
    return {
      logs: [],
      capsSummary: '',
  ...
  _readTextOrientationInfo(): string {
    const f = this._flags();
    try {
