...
export interface TextEncodingIntlPageProps extends Props {}

export interface TextEncodingIntlPageState extends State {}

export class TextEncodingIntlPage extends Page {
  declare props: TextEncodingIntlPageProps;
  declare state: TextEncodingIntlPageState;
  _abortController: any = null;
  _destroyed: any = null;
  _fontHandler: any = null;
  _inited: boolean = false;
  initialState(): TextEncodingIntlPageState {
    return {
      logs: [],
      capsSummary: '',
      // Card 1: Encoding API
  ...
  _runEncode() {
    ...
    try {
      ...
      const zhBytes = Array.from(str).map((ch: any) => {
    ...
    (async () => {
      try {
        ...
        try {
          ...
          bytesHex = Array.from(bytes).map((b: any) => b.toString(16).padStart(2, '0')).join(' ');
        } catch { /* noop */ }
  ...
  _runSegmentZh() {
    ...
    try {
      ...
      const lines = segments.map((s: any, i: any) =>
        `[${String(i).padStart(2, '0')}] segment="${s.segment}"  isWordLike=${s.isWordLike}  index=${s.index}`);
      const words = segments.filter((s: any) => s.isWordLike).map((s: any) => s.segment);
      lines.push('');
  ...
  _runSegmentEn() {
    ...
    try {
      ...
      sentences.forEach((s: any, i: any) => lines.push(`  句[${i}] "${s.segment.trim()}"`));
      this.setState({ segResult: lines.join('\n') });
      ...
      for (const locale of ['zh', 'en', 'ja']) {
        for (const type of ['conjunction', 'disjunction']) {
          for (const style of ['long', 'short', 'narrow']) {
            try {
              const fmt = (new Intl.ListFormat(locale, { type, style } as any) as any);
  ...
  _runRelativeTime() {
    ...
    try {
      ...
      ['year', 'month', 'week', 'day', 'hour', 'minute', 'second'].forEach((u: any) => {
  ...
  _runDisplayNames() {
    ...
    try {
      ...
      lines.push('【language】:');
      ['en', 'ja', 'fr', 'ko'].forEach((c: any) => lines.push(`  ${c} → ${lang.of(c)}`));

      const region = new Intl.DisplayNames('zh', { type: 'region' });
      lines.push('');
      lines.push('【region】:');
      ['US', 'CN', 'JP', 'FR'].forEach((c: any) => lines.push(`  ${c} → ${region.of(c)}`));

      const currency = new Intl.DisplayNames('zh', { type: 'currency' });
      lines.push('');
      lines.push('【currency】:');
      ['USD', 'CNY', 'JPY', 'EUR'].forEach((c: any) => lines.push(`  ${c} → ${currency.of(c)}`));

      const script = new Intl.DisplayNames('zh', { type: 'script' });
      lines.push('');
      lines.push('【script】:');
      ['Latn', 'Hans', 'Hant', 'Cyrl'].forEach((c: any) => lines.push(`  ${c} → ${script.of(c)}`));
  ...
  _checkFonts() {
    ...
    try {
      ...
      lines.push(`typeof fonts.ready:  ${(fonts.ready as any) ? 'Promise' : 'undefined'}`);
      lines.push(`typeof fonts.forEach:${typeof fonts.forEach}`);
      lines.push('');
      lines.push('【check(font, text) 测试】:');
      ['16px Arial', '16px "Times New Roman"', '16px "NonExistent-Font-XYZ"', 'bold 16px sans-serif'].forEach((f: any) => {
        try {
