...
interface CSSFunctionsDeepPageState extends State {
  logs: LogEntry[];
  capsSummary: string;
  overviewInfo: string;
  bezierStepsInfo: string;
  linearInfo: string;
  calcInfo: string;
  minMaxClampInfo: string;
  mathFunctionsInfo: string;
  practiceInfo: string;
  pitfallsInfo: string;
  randomInfo: string;
}
...
export class CSSFunctionsDeepPage extends Page {
  declare state: CSSFunctionsDeepPageState;
  ...
  // —— 初始 state ——
  initialState(): CSSFunctionsDeepPageState {
    return {
      logs: [],
      capsSummary: '',
  ...
  componentDidMount(): void {
    ...
    if (!f.linear) this._addLog('warn', 'linear() 缓动函数不可用或 jsdom 未识别（Chrome 113+ / Firefox 113+ 较新，Safari 部分支持）');
    if (!f.round) this._addLog('warn', 'round() 数学函数不可用或 jsdom 未识别（Chrome 113+ / Firefox 113+ 较新）');
    if (!f.sin) this._addLog('warn', 'sin()/cos() 三角函数不可用或 jsdom 未识别（Chrome 113+ / Firefox 113+ 较新，CSS Values L4）');
    if (!f.random) this._addLog('warn', 'random() 随机函数不可用或 jsdom 未识别（CSS Values L5 草案，Chrome/Safari 实验性支持，Firefox 未实现）');
    ...
    const card = new Card({
      ...
      children: [
        ...
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.overviewInfo || '（点击「读取概述信息」查看 CSS Easing + Math Functions 全景）')),
        h(Alert, {
          type: 'info',
  ...
  _setEasing(mode: any) {
    ...
    this._injectStyle('css-easing-dynamic', ((map as any)[(mode as any)]) || map['cubic-bezier']);
    // 触发动画
    const box1 = this.el && this.el.querySelector('.easing-box');
    const box2 = this.el && this.el.querySelector('.steps-box');
    if (box1) { box1.classList.remove('moved'); requestAnimationFrame(() => box1.classList.add('moved')); }
    if (box2) { box2.classList.remove('moved'); requestAnimationFrame(() => box2.classList.add('moved')); }
  ...
  _renderCard2(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['cubic-bezier', f.cubicBezier], ['steps', f.steps]]),
        h(Tag, { color: 'primary' }, '缓动函数'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'cubic-bezier(x1,y1,x2,y2) 定义贝塞尔曲线控制点（P0/P3 固定 0,0 / 1,1），x 范围 [0,1]，y 可超出产生回弹。标准关键字：linear/ease/ease-in/ease-out/ease-in-out（均等价特定 cubic-bezier）。steps(n, position) 阶梯动画：jump-start/jump-end(默认)/jump-none/jump-both。常见用途：Material Design cubic-bezier(0.4,0,0.2,1)、iOS 弹性 cubic-bezier(0.36,1.56,0.64,1)、精灵图动画 steps(8,end)、打字机效果 steps(20,end)。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ bezierStepsInfo: this._readBezierStepsInfo() }) }),
          this._btn('cubic-bezier', { size: 'sm', disabled: !f.cubicBezier, onClick: () => this._setEasing('cubic-bezier') }),
          this._btn('ease', { size: 'sm', disabled: !f.cubicBezier, onClick: () => this._setEasing('ease') }),
          this._btn('ease-in-out', { size: 'sm', disabled: !f.cubicBezier, onClick: () => this._setEasing('ease-in-out') }),
          this._btn('steps start', { size: 'sm', disabled: !f.steps, onClick: () => this._setEasing('steps-start') }),
          this._btn('steps none', { size: 'sm', disabled: !f.steps, onClick: () => this._setEasing('steps-none') }),
        ),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '缓动演示（点击按钮触发动画，当前模式: ' + this._easingMode + '）：'),
        h('div', { class: 'easing-stage' },
          h('div', { class: 'fs-sm' }, 'cubic-bezier 类缓动：'),
          h('div', { class: 'easing-box' }, ''),
          h('div', { class: 'fs-sm' }, 'steps 类阶梯动画：'),
          h('div', { class: 'steps-box' }, ''),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.bezierStepsInfo || '（点击按钮切换缓动模式查看说明）')),
        h(Alert, {
          type: 'info',
  ...
  _renderCard3(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['linear()', f.linear]]),
        h(Tag, { color: 'primary' }, 'CSS Easing L1'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
        ...
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('读取 linear() 信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ linearInfo: this._readLinearInfo() }) }),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.linearInfo || '（点击按钮查看 linear() 自定义缓动完整说明）')),
        h(Alert, {
          type: 'info',
  ...
  _renderCard4(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['calc', f.calc]]),
        h(Tag, { color: 'primary' }, '四则运算'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'calc() 支持四则运算（+ - * /）与单位混合计算（calc(100% - 20px)、calc(50vw + 10em)）。⚠ + 和 - 前后必须有空格（calc(100%-20px) 错误，calc(100% - 20px) 正确），* 和 / 不要求空格。可嵌套 calc() 或用括号（calc((100% - 20px) / 2)）。与 var() 协同实现 CSS 变量数学运算（calc(var(--gap) * 2)）。浏览器渲染时计算（非预编译），可使用运行时值如 100%。全浏览器支持（Chrome 19+ / Firefox 16+ / Safari 6+）。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ calcInfo: this._readCalcInfo() }) }),
          this._btn('100% - 20px', { size: 'sm', disabled: !f.calc, onClick: () => this._setCalc('100% - 20px') }),
          this._btn('50vw + 10em', { size: 'sm', disabled: !f.calc, onClick: () => this._setCalc('50vw + 10em') }),
          this._btn('100% / 3', { size: 'sm', disabled: !f.calc, onClick: () => this._setCalc('100% / 3') }),
        ),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, 'calc() 演示（当前 calc(' + this._calcExpr + ')）：'),
        h('div', { class: 'calc-stage' },
          h('div', { class: 'calc-box' },
            'width: calc(' + this._calcExpr + ')\n（真实浏览器查看计算后宽度）',
          ),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.calcInfo || '（点击按钮切换 calc 表达式查看说明）')),
        h(Alert, {
          type: 'warning',
  ...
  _renderCard5(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['min', f.min], ['max', f.max], ['clamp', f.clamp]]),
        h(Tag, { color: 'primary' }, '极值函数'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'min(<v1>, <v2>, ...) 返回最小值（限制最大宽度），max(<v1>, <v2>, ...) 返回最大值（保证最小宽度），clamp(<min>, <preferred>, <max>) 限制首选值在 [min, max] 范围。响应式字体大小：font-size: clamp(14px, 2vw, 20px) 小屏 14px/中屏 2vw/大屏 20px，替代 @media 查询更简洁。流式布局：width: clamp(300px, 80vw, 1200px)。与媒体查询取舍：单值响应式用 clamp（流式无突变），多属性变化用 @media。浏览器支持：Chrome 79+ / Firefox 75+ / Safari 13.1+。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ minMaxClampInfo: this._readMinMaxClampInfo() }) }),
          this._btn('100px, 50vw, 500px', { size: 'sm', disabled: !f.clamp, onClick: () => this._setClamp('100px, 50vw, 500px') }),
          this._btn('200px, 80vw, 800px', { size: 'sm', disabled: !f.clamp, onClick: () => this._setClamp('200px, 80vw, 800px') }),
        ),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, 'clamp() 演示（width: clamp(' + this._clampExpr + '), font-size: clamp(14px, 2vw, 20px)）：'),
        h('div', { class: 'clamp-stage' },
          h('div', { class: 'clamp-box' },
            '响应式宽度 + 字体\nwidth: clamp(' + this._clampExpr + ')\nfont-size: clamp(14px, 2vw, 20px)\n（缩放浏览器窗口观察变化）',
          ),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.minMaxClampInfo || '（点击按钮切换 clamp 表达式查看说明）')),
        h(Alert, {
          type: 'info',
  ...
  _renderCard6(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['round', f.round], ['mod', f.mod], ['sin', f.sin], ['pow', f.pow]]),
        h(Tag, { color: 'primary' }, 'CSS Values L4'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'CSS Values L4 数学函数全集：abs()（绝对值）、sign()（符号 -1/0/1）、round(mode, value, step)（四舍五入 down/up/to-zero/nearest）、mod()（取模，符号同除数）、rem()（取余，符号同被除数）、sin()/cos()/tan()（三角函数，角度 deg/rad/turn）、asin()/acos()/atan()/atan2()（反三角）、pow(base, exp)/sqrt()/hypot()（幂函数）、exp()/log()（指数对数）。让 CSS 具备完整数学计算能力，无需 JS 辅助。浏览器支持：Chrome 113+ / Firefox 113+ / Safari 16.4+ 较新。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('读取数学函数信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ mathFunctionsInfo: this._readMathFunctionsInfo() }) }),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.mathFunctionsInfo || '（点击按钮查看 CSS Values L4 数学函数全集）')),
        h(Alert, {
          type: 'info',
  ...
  _renderCard7(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['sin/cos', f.sin], ['pow', f.pow], ['calc', f.calc]]),
        h(Tag, { color: 'primary' }, '6 大场景'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '数学函数六大实战场景：圆周运动（transform: translate(calc(cos(angle) * r), calc(sin(angle) * r))，需 @property 注册 angle 为 <angle> 才能动画插值）、弹簧动画（sin(8t) * exp(-t) 衰减振荡）、复杂路径（Lissajous 曲线 sin(3t)/sin(2t)）、SVG path 与 CSS math（部分浏览器有限）、响应式设计（clamp + calc）、计算复杂布局（clamp + floor 计算网格列数）。配合 @property 注册自定义属性类型实现动画插值。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('读取实战信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runPracticeDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '圆周运动演示（transform: translate(calc(cos*sin) * 80px)，真实浏览器查看动画）：'),
        h('div', { class: 'math-stage' },
          h('div', { class: 'orbit-container' },
            h('div', { class: 'orbit-center' }, ''),
            h('div', { class: 'orbit-planet' }, ''),
          ),
          h('div', { class: 'fs-sm', style: { textAlign: 'center', marginTop: '8px' } },
            '中心点 + 行星（真实浏览器中行星绕中心圆周运动）',
          ),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '600px', overflow: 'auto' } },
          h('code', {}, s.practiceInfo || '（点击按钮查看 6 大数学函数实战场景完整代码）')),
        h(Alert, {
          type: 'info',
  ...
  _runRandomDemo(): void {
    ...
    this._addLog('practice',
      `random() 演示：random=${f.random}, randomItem=${f.randomItem}`);
  }
  ...
  _renderCard9(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['random()', f.random], ['random-item()', f.randomItem]]),
        h(Tag, { color: 'warning' }, 'L5 草案'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
        ...
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行 random() 演示', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runRandomDemo() }),
        ),
        ...
        h('div', { class: 'fn-stage' },
          h('div', { class: 'fs-sm', style: { marginBottom: '6px' } },
            '示例：random() 生成随机宽度（真实浏览器 behind flag 可用，jsdom 仅展示用法）：'),
          h('div', { class: 'calc-stage' },
            h('div', { class: 'fs-sm' },
              'width: random(100px, 500px)  // 100~500px 随机宽度'),
            h('div', { class: 'fs-sm', style: { marginTop: '4px' } },
              'color: random-item(--seed, #ef4444, #3b82f6, #10b981)  // 随机颜色'),
          ),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '600px', overflow: 'auto' } },
          h('code', {}, s.randomInfo || '（点击按钮查看 random() / random-item() 完整说明）')),
        h(Alert, {
          type: 'warning',
  ...
  render(): Node {
    const s = this.state;
    return h('div', { class: 'api-lab-page css-functions-deep-page' },
      h('h2', { class: 'section-title' }, 'CSS Easing + Math Functions 实验室'),
      h('p', { class: 'fs-sm text-secondary mb-md' },
        '本页演示 CSS Easing Functions Level 1 + CSS Values and Units Level 4：cubic-bezier() 与 steps()（贝塞尔曲线 + 阶梯动画 + ease 关键字）、linear() 自定义缓动（任意分段线性 + 弹簧回弹）、calc() 与嵌套（单位混合 + var() 协同 + 空格陷阱）、min()/max()/clamp()（响应式字体 + 流式布局）、数学函数全集（abs/sign/round/mod/rem/sin/cos/tan/pow/sqrt/exp/log）、数学函数实战（圆周运动 + 弹簧 + Lissajous + @property 协同）、陷阱与最佳实践（空格 / min vs @media / 性能 / 降级 / DevTools / sin 单位）。所有特性通过 typeof / CSS.supports 能力检测，不可用时仅记日志，绝不抛异常。'),
