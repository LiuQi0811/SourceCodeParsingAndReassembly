...
interface CSSGridDeepPageState extends State {
  logs: LogEntry[];
  capsSummary: string;
  basicsInfo: string;
  repeatInfo: string;
...
export class CSSGridDeepPage extends Page {
  declare state: CSSGridDeepPageState;
  _inited: boolean = false;
  _dynamicStyles: HTMLStyleElement[] = [];


  initialState(): CSSGridDeepPageState {
    return {
      logs: [],
      capsSummary: '',
  ...
  componentDidMount(): void {
    ...
    if (!f.subgrid) this._addLog('warn', 'subgrid 不可用（Chrome 117+/Firefox 71+/Safari 16+ 起支持）');
    if (!f.gridAutoFlowDense) this._addLog('warn', 'grid-auto-flow: dense 不可用');
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'display: grid / inline-grid 声明网格容器，grid-template-columns / grid-template-rows 定义显式网格。轨道尺寸取值：length / percentage / fr（弹性份额，类似 flex-grow）/ auto / min-content / max-content / minmax(min, max) / fit-content()。fr 按比例分配剩余空间，minmax 约束最小最大值。常见防溢出技巧：minmax(0, 1fr) 强制 fr 不被内容撑大。',
        ),
        h('div', { class: 'cg-demo' },
          h('div', { class: 'fs-sm text-secondary' }, '固定列：100px 200px 100px'),
          h('div', { class: 'cg-grid cg-basics-fixed' },
            h('div', { class: 'cg-cell' }, '100'),
            h('div', { class: 'cg-cell cg-alt' }, '200'),
            h('div', { class: 'cg-cell' }, '100'),
          ),
          h('div', { class: 'fs-sm text-secondary mt-sm' }, 'fr 比例：1fr 2fr 1fr'),
          h('div', { class: 'cg-grid cg-basics-fr' },
            h('div', { class: 'cg-cell' }, '1fr'),
            h('div', { class: 'cg-cell cg-alt' }, '2fr'),
            h('div', { class: 'cg-cell' }, '1fr'),
          ),
          h('div', { class: 'fs-sm text-secondary mt-sm' }, 'minmax 约束：minmax(120px,1fr) minmax(200px,2fr)'),
          h('div', { class: 'cg-grid cg-basics-minmax' },
            h('div', { class: 'cg-cell' }, 'minmax(120,1fr)'),
            h('div', { class: 'cg-cell cg-alt' }, 'minmax(200,2fr)'),
          ),
          h('div', { class: 'fs-sm text-secondary mt-sm' }, 'auto 1fr auto（表单布局）'),
          h('div', { class: 'cg-grid cg-basics-auto' },
            h('div', { class: 'cg-cell' }, 'auto'),
            h('div', { class: 'cg-cell cg-alt' }, '1fr'),
            h('div', { class: 'cg-cell' }, 'auto'),
          ),
        ),
        ...
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.basicsInfo || '（点击按钮查看 Grid 容器基础与显式网格完整用法）')),
      ],
    });
  ...
  _renderCard2(): Node | string {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'repeat(n, track) 重复固定次数，repeat(auto-fill, minmax(200px, 1fr)) 自动填充（尽可能多列，空列保留），repeat(auto-fit, minmax(200px, 1fr)) 自动适配（空列折叠，现有列拉伸填满）。minmax(200px, 1fr) 实现无媒体查询的响应式卡片网格。auto-fill vs auto-fit 关键区别：项目少于列数时 auto-fit 拉伸项目，auto-fill 保留空位。repeat() 内可混合多轨道：repeat(3, 100px 1fr) = 100px 1fr 100px 1fr 100px 1fr。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行 repeat 演示', { type: 'primary', size: 'sm', onClick: () => this._runRepeatDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.repeatInfo || '（点击按钮查看 repeat() / auto-fill / auto-fit 完整用法）')),
      ],
    });
    this.registerChild(card);
    return card.render() as Node;
  ...
  _renderCard3(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['grid', f.grid], ['grid-area', f.gridArea]]),
        h(Tag, { color: 'primary' }, '命名区域'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'grid-template-areas 用字符串命名区域，每行引号包裹、空格分隔区域名，. 表示空单元格，同名区域必须相邻形成矩形。项目用 grid-area: <name> 放入命名区域。实战：经典三栏布局 header/sidebar/main/footer 一目了然。响应式重排只需用 media query 改变 areas 定义，项目代码不变。是 Grid 最直观、最易维护的特性。',
        ),
        h('div', { class: 'cg-areas' },
          h('div', { class: 'cg-area cg-area--header' }, 'header'),
          h('div', { class: 'cg-area cg-area--sidebar' }, 'sidebar'),
          h('div', { class: 'cg-area cg-area--main' }, 'main'),
          h('div', { class: 'cg-area cg-area--footer' }, 'footer'),
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' },
          this._btn('运行 areas 演示', { type: 'primary', size: 'sm', onClick: () => this._runAreasDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.areasInfo || '（点击按钮查看 grid-template-areas 完整用法）')),
      ],
    });
    this.registerChild(card);
    return card.render() as Node;
  ...
  _renderCard4(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['grid-area', f.gridArea]]),
        h(Tag, { color: 'primary' }, '网格线'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '命名网格线：grid-template-columns: [start] 200px [content-start] 1fr [content-end end]，一条线可有多个名字。grid-column / grid-row: <start-line> / <end-line>，线号从 1 开始，可用 span N 跨越、负数从末尾计数（-1 是显式网格最后线）。grid-area 简写：<row-start> / <column-start> / <row-end> / <column-end>，单值时视为命名区域。默认 auto 由 grid-auto-flow 自动放置。线名可重复，用 "name N" 引用第 N 条同名线。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行命名线演示', { type: 'primary', size: 'sm', onClick: () => this._runLinesDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.linesInfo || '（点击按钮查看命名网格线/grid-column/grid-row/grid-area 完整用法）')),
      ],
    });
    this.registerChild(card);
    return card.render() as Node;
  ...
  _renderCard5(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['dense', f.gridAutoFlowDense]]),
        h(Tag, { color: 'primary' }, '自动放置'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'grid-auto-flow: row(默认逐行)/column(逐列)/dense(密集回填)/row dense/column dense。稀疏模式（默认）光标只前进不回头，留下空位；密集模式（dense）每个项目从 (1,1) 重新扫描，小项目填进大项目留下的空位，适合瀑布流近似但打乱视觉顺序（影响无障碍）。隐式网格用 grid-auto-columns / grid-auto-rows 控制超出显式网格时自动生成轨道的尺寸。',
        ),
        h('div', { class: 'cg-demo' },
          h('div', { class: 'fs-sm text-secondary' }, 'row（逐行填充）'),
          h('div', { class: 'cg-flow-row' },
            h('div', { class: 'cg-cell' }, '1'),
            h('div', { class: 'cg-cell cg-alt' }, '2'),
            h('div', { class: 'cg-cell' }, '3'),
            h('div', { class: 'cg-cell cg-alt' }, '4'),
            h('div', { class: 'cg-cell' }, '5'),
            h('div', { class: 'cg-cell cg-alt' }, '6'),
          ),
          h('div', { class: 'fs-sm text-secondary mt-sm' }, 'row dense（密集回填，不同 span 混合）'),
          h('div', { class: 'cg-flow-dense' },
            h('div', { class: 'cg-cell cg-warn' }, '1 (span2)'),
            h('div', { class: 'cg-cell cg-ok' }, '2 (2x2)'),
            h('div', { class: 'cg-cell' }, '3'),
            h('div', { class: 'cg-cell' }, '4'),
            h('div', { class: 'cg-cell' }, '5'),
            h('div', { class: 'cg-cell cg-alt' }, '6 (span2)'),
          ),
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' },
          this._btn('运行 auto-flow 演示', { type: 'primary', size: 'sm', onClick: () => this._runFlowDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.flowInfo || '（点击按钮查看 grid-auto-flow 完整用法）')),
      ],
    });
    this.registerChild(card);
    return card.render() as Node;
  ...
  _renderCard6(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['gap', f.gap]]),
        h(Tag, { color: 'primary' }, '对齐'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'gap 简写 row-gap + column-gap（轨道间距，不作用容器边缘）。justify-items: stretch(默认)/start/end/center/baseline 项目在单元格内水平对齐；align-items 同值垂直对齐；place-items 简写。justify-content/align-content/place-content 控制整个网格在容器内对齐（start/end/center/stretch/space-around/between/evenly）。与 Flexbox 对应：justify-content/align-items 相同，但 Grid 多了 justify-items（Flexbox 无），且 items 系列作用在「单元格内」。',
        ),
        h('div', { class: 'cg-demo' },
          h('div', { class: 'fs-sm text-secondary' }, 'justify-items: start + align-items: center'),
          h('div', { class: 'cg-align-items cg-ji-start cg-ai-center' },
            h('div', { class: 'cg-cell' }, 'start'),
            h('div', { class: 'cg-cell' }, 'start'),
            h('div', { class: 'cg-cell' }, 'start'),
          ),
          h('div', { class: 'fs-sm text-secondary mt-sm' }, 'justify-items: center + align-items: end'),
          h('div', { class: 'cg-align-items cg-ji-center cg-ai-end' },
            h('div', { class: 'cg-cell' }, 'center'),
            h('div', { class: 'cg-cell' }, 'center'),
            h('div', { class: 'cg-cell' }, 'center'),
          ),
          h('div', { class: 'fs-sm text-secondary mt-sm' }, 'place-content: center（网格在容器内居中）'),
          h('div', { class: 'cg-content-center' },
            h('div', { class: 'cg-cell' }, 'A'),
            h('div', { class: 'cg-cell' }, 'B'),
            h('div', { class: 'cg-cell' }, 'C'),
          ),
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' },
          this._btn('运行对齐演示', { type: 'primary', size: 'sm', onClick: () => this._runAlignDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.alignInfo || '（点击按钮查看 gap/align-items/justify-items 完整用法）')),
      ],
    });
    this.registerChild(card);
    return card.render() as Node;
  ...
  _renderCard7(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['subgrid', f.subgrid]]),
        h(Tag, { color: 'primary' }, 'Grid L2'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'grid-template-columns: subgrid / grid-template-rows: subgrid 让子网格继承父网格轨道定义，孙项目列与父项目列严格对齐。继承内容包括轨道尺寸、命名网格线（line-names）、gap。与嵌套 display:grid 区别：嵌套各自独立定义轨道不对齐，subgrid 继承并对齐。实战：表单标签输入对齐、卡片内容跨列对齐、表头表体列对齐。浏览器支持：Chrome 117+/Firefox 71+/Safari 16+，全球 ~95%。',
        ),
        h('div', { class: 'cg-demo' },
          h('div', { class: 'fs-sm text-secondary' }, 'subgrid：子网格跨 2 列继承父轨道（与父列对齐）'),
          h('div', { class: 'cg-subgrid-parent' },
            h('div', { class: 'cg-cell' }, '父-1'),
            h('div', { class: 'cg-subgrid-child' },
              h('div', { class: 'cg-cell' }, '子-1（对齐父-2）'),
              h('div', { class: 'cg-cell' }, '子-2（对齐父-3）'),
            ),
          ),
          h('div', { class: 'fs-sm text-secondary mt-sm' }, '嵌套 grid（非 subgrid）：子网格独立定义不对齐'),
          h('div', { class: 'cg-nested-parent' },
            h('div', { class: 'cg-cell' }, '父-1'),
            h('div', { class: 'cg-nested-child' },
              h('div', { class: 'cg-cell' }, '子-1（独立）'),
              h('div', { class: 'cg-cell' }, '子-2（独立）'),
            ),
          ),
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' },
          this._btn('运行 subgrid 演示', { type: 'primary', size: 'sm', onClick: () => this._runSubgridDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.subgridInfo || '（点击按钮查看 subgrid 完整用法）')),
      ],
    });
    this.registerChild(card);
    return card.render() as Node;
  ...
  _renderCard8(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...
        h(Tag, { color: f.subgrid ? 'success' : 'error' }, `subgrid ${f.subgrid ? '✓' : '✗'}`),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '实战模式：经典三栏（areas + 1fr）/ 照片墙（auto-fit + minmax + gap）/ 12 列网格系统（repeat(12, 1fr) + span）/ 瀑布流近似（dense + 不同 span）/ 暗黑模式仪表盘（areas 响应式重排 + prefers-color-scheme）/ 圣杯布局。vs Flexbox 决策树：一维→Flex，二维→Grid；内容驱动→Flex，布局驱动→Grid；居中单元素→Flex，整页布局→Grid areas。生产常见组合：外层 Grid 控结构 + 内层 Flex 控局部。Chrome DevTools Grid 调试：网格线/area/track sizes 高亮。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
        ...
        h('pre', { class: 'code-block', style: { maxHeight: '600px', overflow: 'auto' } },
          h('code', {}, s.patternInfo || '（点击按钮查看实战布局模式与 vs Flexbox 完整代码）')),
      ],
    });
    this.registerChild(card);
    return card.render() as Node;
