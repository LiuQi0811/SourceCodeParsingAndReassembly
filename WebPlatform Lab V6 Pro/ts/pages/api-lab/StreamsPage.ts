...
export interface StreamsPageProps extends Props {}

export interface StreamsPageState extends State {}

export class StreamsPage extends Page {
  declare props: StreamsPageProps;
  declare state: StreamsPageState;
  _abortCtrl: any = null;
  _activeReader: any = null;
  _onVisChange: any = null;
  _simSseCtrl: any = null;
  _sse: any = null;
  _streamTimers: Set<any> | null = null;
  _visInited: boolean = false;
  initialState(): StreamsPageState {
    return {
      logs: [],
  ...
  componentWillUnmount(): void {
    ...
    if (this._streamTimers) {
      this._streamTimers.forEach((t: any) => { try { clearInterval(t); } catch { /* noop */ } });
    ...
    try {
      ...
      const stream = new ReadableStream({
        start: (controller: any) => {
          ...
          if (!this._streamTimers) this._streamTimers = new Set<any>();
  ...
  async _demoAsyncIter() {
    ...
    try {
      ...
      const stream = new ReadableStream({
        start(controller: any) {
          ['流', '式', '迭', '代'].forEach((c: any) => controller.enqueue(c));
          controller.close();
        },
      });
  ...
  async _demoTee() {
    ...
    try {
      ...
      const src = new ReadableStream({
        start(controller: any) { [1, 2, 3].forEach((n: any) => controller.enqueue(n)); controller.close(); },
      });
      const [a, b] = src.tee();
  ...
  async _runPipeline() {
    ...
    try {
      ...
      const upper = new TransformStream({ // 自定义转换：转大写
        transform(chunk: any, controller: any) { controller.enqueue(chunk.toUpperCase()); },
      });
      const sink: any[] = [];
      let totalBytes = 0;
      const writable = new WritableStream({ // sink：收集结果
        write(chunk: any) { sink.push(chunk); totalBytes += new TextEncoder().encode(chunk).byteLength; },
      });
    ...
    return [
        ...
        h('div', { class: 'flex flex-col gap-sm' },
          ...
          this.state.sseMessages.length === 0
            ...
            : h('div', { class: 'log-panel' },
              ...this.state.sseMessages.map((m: any) => h('div', { class: 'log-panel__line' },
      ...
      h(Card, {
        ...
        this.state.logs.length === 0
          ...
          : h('div', { class: 'log-panel' },
            ...this.state.logs.map((log: any) => h('div', { class: 'log-panel__line' },
