...
interface ES2025LanguagePageState extends State {
  logs: any;
  capsSummary: any;
  asyncIterResult: any;
  shadowRealmResult: any;
  errorRegExpResult: any;
  float16Result: any;
  suppressedResult: any;
  decoratorResult: any;
}

export class ES2025LanguagePage extends Page {
  declare state: ES2025LanguagePageState;
  _inited!: boolean;
  _shadowRealm!: any;
  ...
  async _asyncIterChain() {
    ...
    try {
      ...
      // 链式：×2 → 保留 >5 → 取前 3 → 转数组
      const chain = (asyncGen() as any).map((x: any) => x * 2).filter((x: any) => x > 5).take(3);
      const arr = await chain.toArray();           // [6,8,10]
      // 惰性求值验证：take(n) 取够即停，不消费剩余元素
      let consumed = 0;
      const lazy = await (asyncGen() as any)
        .map((x: any) => { consumed++; return x * 10; })
        .take(2)
        .toArray();
      // drop 演示
      const dropped = await (asyncGen() as any).drop(5).take(2).toArray(); // [6,7]
      this.setState({
        asyncIterResult:
  ...
  async _asyncIterFrom() {
    ...
    try {
      ...
      const squared = await it.map((x: any) => x * x).toArray();   // [1,4,9,16,25]
      this.setState({
        asyncIterResult:
...
const sq = await it.map((x: any) => x*x).toArray();     // Promise<Array>`)),
  ...
  _createShadowRealm() {
    ...
    try {
      ...
      const mainHacked = typeof ([] as any).hacked !== 'undefined'; // false（主 realm 未受污染）
    ...
    if (!caps.errorIsError) {
      ...
      try {
        ...
        const duckCheck = e instanceof Error ||
          (e && typeof e === 'object' && typeof (e as any).message === 'string' && typeof (e as any).name === 'string');
  ...
  _testRegExpEscape() {
    ...
    if (!caps.regExpEscape) {
      ...
      try {
        ...
        // 手动转义：转义所有正则元字符
        const manualEscape = (s: any) => s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  ...
  _float16Demo() {
    ...
    try {
      ...
      if (caps.float16Array) {
        ...
        const f32Sum = f32Arr.reduce((a: any, b: any) => a + b, 0);
        float16ArrLines =
          `\nFloat16Array 演示：\n` +
      ...
      try {
        ...
        try {
          ...
          try {
            ...
            try { fakeDispose(); } catch (e: any) { caught = e; }
          }
        } catch (e: any) {
          caught = e;
        }
      ...
      if (caps.disposableStack) {
        ...
        try {
          try {
            ...
            try { stack[Symbol.dispose](); } catch (e: any) { throw e; }
          }
        } catch (e: any) {
          caughtErr = e;
...
// err.message === 'cleanup failed'（dispose 异常作为主错误）`)),
  ...
  _manualDecorators() {
    try {
      ...
      const logged = (target: any, context: any) => {
      ...
      const bound = (target: any, context: any) => {
        const original = target;
        return function(this: any, ...args: any[]) {
      ...
      class Calculator {
        ...
        fib(n: any): any { return n < 2 ? n : this.fib(n - 1) + this.fib(n - 2); }
      }
      ...
      // 包装计数 + memoize
      const memoizedFib = memoize(function(this: any, n: any) { fibCalls++; return originalFib.call(this, n); }, { name: 'fib', kind: 'method' });
