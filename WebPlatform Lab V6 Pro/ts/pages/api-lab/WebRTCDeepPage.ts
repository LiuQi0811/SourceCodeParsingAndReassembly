...
interface WebRTCDeepPageState extends State {
  logs: any;
  capsSummary: any;
  pcLifecycle: any;
  offerAnswer: any;
...
export class WebRTCDeepPage extends Page {
  declare state: WebRTCDeepPageState;
  _dc1!: any;
  _dc2!: any;
  _inited!: boolean;
  _mediaStream!: any | null;
  _pc1!: any;
  _pc2!: any;
  ...
  componentDidMount() {
    ...
    if (!hasGetUserMedia) this._addLog('warn', 'navigator.mediaDevices.getUserMedia 不可用（jsdom 无摄像头 / 麦克风），Card 4 将以 mock 演示');
    if (!hasRTCStatsReport) this._addLog('warn', 'RTCStatsReport 构造器不可用（getStats 返回的 Map 仍可遍历）');
  }

  componentWillUnmount() {
    // 释放数据通道
    if (this._dc1) { try { this._dc1.close(); } catch { /* noop */ } this._dc1 = null; }
    if (this._dc2) { try { this._dc2.close(); } catch { /* noop */ } this._dc2 = null; }
    // 关闭两条 RTCPeerConnection
    if (this._pc1) { try { this._pc1.close(); } catch { /* noop */ } this._pc1 = null; }
    if (this._pc2) { try { this._pc2.close(); } catch { /* noop */ } this._pc2 = null; }
    // 停止 MediaStream 所有 track
    if (this._mediaStream) {
      try {
        const tracks = typeof this._mediaStream.getTracks === 'function' ? this._mediaStream.getTracks() : [];
        for (const t of tracks) { try { t.stop(); } catch { /* noop */ } }
      } catch { /* noop */ }
  ...
  _caps(items: any) {
    return items.map((it: any) => h(Tag, { color: it.ok ? 'success' : 'error' }, `${it.key} ${it.ok ? '✓' : '✗'}`));
  }
  ...
  // 安全读取对象属性（防御性）
  _safeGet(obj: any, key: any){ try { return obj[key]; } catch { return undefined; } }
  ...
  _createPeerConnections() {
    ...
    if (!caps.rtcpc) {
      ...
      this.setState({ pcLifecycle:
        'RTCPeerConnection 用法（测试环境不可用，仅说明）：\n\n' +
    ...
    try {
      if (this._pc1) { try { this._pc1.close(); } catch { /* noop */ } }
      if (this._pc2) { try { this._pc2.close(); } catch { /* noop */ } }
      ...
      const pc1 = new RTCPeerConnection((config as any));
      const pc2 = new RTCPeerConnection((config as any));
      this._pc1 = pc1;
      this._pc2 = pc2;

      // 绑定状态变化事件（mock 不会主动触发，但绑定 API 表面）
      const logState = (who: any, pc: any) => this._addLog('rtc', `${who}.connectionState=${pc.connectionState}, iceConnectionState=${pc.iceConnectionState}, signalingState=${pc.signalingState}`);
      const bindState = (pc: any, who: any) => {
        try { pc.onconnectionstatechange = () => logState(who, pc); } catch { /* noop */ }
        try { pc.oniceconnectionstatechange = () => logState(who, pc); } catch { /* noop */ }
        try { pc.onsignalingstatechange = () => logState(who, pc); } catch { /* noop */ }
      };
      bindState(pc1, 'pc1'); bindState(pc2, 'pc2');
  ...
  _renderCard1() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([{ key: 'RTCPeerConnection', ok: caps.rtcpc }]),
        h(Tag, { color: 'primary' }, 'configuration / states'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'new RTCPeerConnection(configuration) 接受 iceServers（STUN/TURN 服务器列表）、iceTransportPolicy（all/relay）、bundlePolicy（balanced/max-compat/max-bundle）、rtcpMuxPolicy（require/negotiate）。每个连接维护三套状态：connectionState（整体）、iceConnectionState（ICE 层）、signalingState（信令层）。本卡片创建 pc1 / pc2 并绑定状态变化事件，后续卡片在此基础上完成协商。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('创建 pc1/pc2', { type: 'primary', size: 'sm', disabled: !caps.rtcpc, onClick: () => this._createPeerConnections() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'RTCPeerConnection 配置与状态：'),
        h('pre', { class: 'code-block', style: { maxHeight: '280px', overflow: 'auto' } },
          h('code', {}, s.pcLifecycle || '（点击「创建 pc1/pc2」开始演示）')),
        h(Alert, {
          type: 'info',
  ...
  async _performOfferAnswer() {
    const caps = this._capFlags();
    if (!caps.rtcpc) { this._addLog('warn', 'RTCPeerConnection 不可用'); return; }
    if (!this._pc1 || !this._pc2) { this._addLog('warn', '请先点击 Card 1 的「创建 pc1/pc2」'); return; }
    try {
      const pc1 = this._pc1;
      const pc2 = this._pc2;
      const steps = ['开始 Offer/Answer 协商流程：\n'];

      steps.push('1) pc1.createOffer() ...'); this._addLog('offer', 'pc1.createOffer() ...');
      const offer = await pc1.createOffer();
      steps.push(`   offer.type = ${offer.type}, offer.sdp = ${JSON.stringify(offer.sdp).slice(0, 60)}`);

      steps.push('2) pc1.setLocalDescription(offer) ...'); this._addLog('offer', `pc1.setLocalDescription(type=${offer.type})`);
      await pc1.setLocalDescription(offer);
      steps.push(`   pc1.signalingState = ${pc1.signalingState}（应为 have-local-offer）`);

      steps.push('3) pc2.setRemoteDescription(offer) ...'); this._addLog('offer', 'pc2.setRemoteDescription(offer)');
      await pc2.setRemoteDescription(offer);
      steps.push(`   pc2.signalingState = ${pc2.signalingState}（应为 have-remote-offer）`);

      steps.push('4) pc2.createAnswer() ...'); this._addLog('answer', 'pc2.createAnswer() ...');
      const answer = await pc2.createAnswer();
      steps.push(`   answer.type = ${answer.type}, answer.sdp = ${JSON.stringify(answer.sdp).slice(0, 60)}`);

      steps.push('5) pc2.setLocalDescription(answer) ...'); this._addLog('answer', `pc2.setLocalDescription(type=${answer.type})`);
      await pc2.setLocalDescription(answer);
      steps.push(`   pc2.signalingState = ${pc2.signalingState}（应回到 stable）`);

      steps.push('6) pc1.setRemoteDescription(answer) ...'); this._addLog('answer', 'pc1.setRemoteDescription(answer) —— 握手完成');
  ...
  _renderCard2() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([{ key: 'createOffer', ok: caps.rtcpc }, { key: 'createAnswer', ok: caps.rtcpc }]),
        h(Tag, { color: 'primary' }, 'SDP 交换'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'SDP（Session Description Protocol）协商是 WebRTC 握手核心：呼叫方 createOffer() 生成 Offer SDP，setLocalDescription() 设为本地描述后经信令通道发给被叫；被叫 setRemoteDescription() 设为远端描述，createAnswer() 生成 Answer SDP，setLocalDescription() 后回传；呼叫方 setRemoteDescription() 完成握手。本页用同页 pc1 / pc2 模拟信令转发，演示完整六步流程。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('执行 Offer/Answer', { type: 'primary', size: 'sm', disabled: !caps.rtcpc, onClick: () => this._performOfferAnswer() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'Offer/Answer 协商步骤与状态：'),
        h('pre', { class: 'code-block', style: { maxHeight: '320px', overflow: 'auto' } },
          h('code', {}, s.offerAnswer || '（请先在 Card 1 创建 pc1/pc2，再点击「执行 Offer/Answer」）')),
        h(Alert, {
          type: 'warning',
  ...
  async _wireIceCandidates() {
    ...
    try {
      ...
      const bindIce = (pcFrom: any, pcTo: any, fromName: any, toName: any, counter: any) => {
        try {
          pcFrom.onicecandidate = (e: any) => {
            if (e.candidate) {
              counter.n += 1;
              ...
              pcTo.addIceCandidate(e.candidate).catch((err: any) => this._addLog('warn', `${toName}.addIceCandidate 失败：${err.message}`));
            } else {
      ...
      const tryAdd = async (pc: any, who: any) => {
        try { await pc.addIceCandidate(mockCandidate); this._addLog('ice', `${who}.addIceCandidate(mockCandidate) 成功（演示 API 表面）`); return true; }
        catch (err: any) { this._addLog('warn', `${who}.addIceCandidate(mockCandidate) 失败：${err.message}`); return false; }
      };
      const added2 = await tryAdd(pc2, 'pc2');
      const added1 = await tryAdd(pc1, 'pc1');

      this.setState({ iceInfo:
        '已绑定 ICE 候选交换回调（同页内直接互投，模拟信令服务器转发）：\n' +
  ...
  _renderCard3() {
    ...
    const card = new Card({
      title: '3. ICE 候选协商',
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([{ key: 'onicecandidate', ok: caps.rtcpc }, { key: 'addIceCandidate', ok: caps.rtcpc }]),
        h(Tag, { color: 'primary' }, 'trickle ICE'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'ICE（Interactive Connectivity Establishment）协商负责在 NAT / 防火墙后建立 P2P 连接。浏览器通过 STUN/TURN 服务器收集候选（host / srflx / prflx / relay），每收集到一个触发 onicecandidate 事件，e.candidate 经信令通道发给对端，对端调用 addIceCandidate(candidate) 添加。Trickle ICE 允许边收集边交换，无需等全部完成。oniceconnectionstatechange 反映 ICE 层连接状态迁移。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('绑定 ICE 互投 + 注入候选', { type: 'primary', size: 'sm', disabled: !caps.rtcpc, onClick: () => this._wireIceCandidates() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'ICE 候选协商状态：'),
        h('pre', { class: 'code-block', style: { maxHeight: '320px', overflow: 'auto' } },
          h('code', {}, s.iceInfo || '（请先在 Card 1 创建 pc1/pc2，再点击「绑定 ICE 互投」）')),
        h(Alert, {
          type: 'info',
  ...
  async _demoMediaStream() {
    const caps = this._capFlags();
    if (!caps.rtcpc) { this._addLog('warn', 'RTCPeerConnection 不可用'); this.setState({ mediaInfo: 'RTCPeerConnection 不可用，无法演示 MediaStream 接入。' }); return; }
    if (!this._pc1) { this._addLog('warn', '请先点击 Card 1 的「创建 pc1/pc2」'); return; }
    const pc1 = this._pc1;
    ...
    const mockStream = caps.mediaStream && typeof MediaStream === 'function'
      ? (() => { try { return new MediaStream(); } catch { return { addTrack() {}, removeTrack() {}, getTracks() { return []; } }; } })()
    ...
    if (typeof pc1.getSenders === 'function') {
      try { const s = pc1.getSenders(); sendersCount = Array.isArray(s) ? String(s.length) : '?'; lines.push(`   pc1.getSenders() → ${sendersCount} 个 RTCRtpSender`); } catch (err: any) { lines.push(`   pc1.getSenders() 失败：${err.message}`); }
    } else { lines.push('   pc1.getSenders 不可用（typeof undefined）'); }

    if (typeof pc1.getReceivers === 'function') {
      try { const r = pc1.getReceivers(); receiversCount = Array.isArray(r) ? String(r.length) : '?'; lines.push(`   pc1.getReceivers() → ${receiversCount} 个 RTCRtpReceiver`); } catch (err: any) { lines.push(`   pc1.getReceivers() 失败：${err.message}`); }
    } else { lines.push('   pc1.getReceivers 不可用（typeof undefined）'); }

    if (addTrackOk && typeof pc1.removeTrack === 'function') {
      try { pc1.removeTrack(null); removeTrackOk = true; lines.push('4) pc1.removeTrack(sender) 已调用（演示 API 表面）'); } catch (err: any) { lines.push(`4) pc1.removeTrack 失败：${err.message}`); }
    } else { lines.push('4) pc1.removeTrack 不可用或 addTrack 未成功'); }
  ...
  _renderCard4() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([{ key: 'getUserMedia', ok: caps.getUserMedia }, { key: 'addTrack', ok: caps.rtcpc }]),
        h(Tag, { color: 'primary' }, 'video / audio'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'navigator.mediaDevices.getUserMedia({ video, audio }) 请求摄像头 / 麦克风权限并返回 MediaStream；pc.addTrack(track, stream) 把媒体加入连接（返回 RTCRtpSender），pc.removeTrack(sender) 移除；pc.ontrack 接收对端 track 到达事件；pc.getSenders() / getReceivers() 列出本地发送与远端接收的 track。jsdom 无真实设备，getUserMedia 会失败，用 try/catch 兜底并以 mock track 演示 addTrack API 表面。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('演示媒体接入', { type: 'primary', size: 'sm', disabled: !caps.rtcpc, onClick: () => this._demoMediaStream() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'MediaStream 接入状态：'),
        h('pre', { class: 'code-block', style: { maxHeight: '320px', overflow: 'auto' } },
          h('code', {}, s.mediaInfo || '（请先在 Card 1 创建 pc1/pc2，再点击「演示媒体接入」）')),
        h(Alert, {
          type: 'warning',
  ...
  _demoDataChannel() {
    const caps = this._capFlags();
    if (!caps.rtcpc) { this._addLog('warn', 'RTCPeerConnection 不可用'); this.setState({ dcInfo: 'RTCPeerConnection 不可用，无法演示 DataChannel。' }); return; }
    if (!this._pc1) { this._addLog('warn', '请先点击 Card 1 的「创建 pc1/pc2」'); return; }
    const pc1 = this._pc1;
    const pc2 = this._pc2;
    try {
      if (this._dc1) { try { this._dc1.close(); } catch { /* noop */ } }
      if (this._dc2) { try { this._dc2.close(); } catch { /* noop */ } }
      this._dc1 = null; this._dc2 = null;
      ...
      try { dc.binaryType = 'arraybuffer'; } catch { /* noop */ }
      try { dc.onopen = () => this._addLog('dc', `dc.onopen：通道「${dc.label}」已打开 (readyState=${dc.readyState})`); } catch { /* noop */ }
      try { dc.onmessage = (e: any) => { const text = typeof e.data !== 'string' ? `[binary ${(e.data?.byteLength ?? 0)} 字节]` : e.data; this._addLog('dc', `dc.onmessage 收到：${text}`); }; } catch { /* noop */ }
      try { dc.onclose = () => this._addLog('dc', `dc.onclose：通道「${dc.label}」已关闭 (readyState=${dc.readyState})`); } catch { /* noop */ }
      try { dc.onerror = (e: any) => this._addLog('warn', `dc.onerror：${e?.error?.message || 'unknown'}`); } catch { /* noop */ }
      try { dc.onbufferedamountlow = () => this._addLog('dc', `dc.onbufferedamountlow 触发（bufferedAmount=${dc.bufferedAmount}）`); } catch { /* noop */ }

      // pc2 监听 ondatachannel
      if (pc2) {
        try {
          pc2.ondatachannel = (e: any) => { this._addLog('dc', `pc2.ondatachannel：收到通道「${e.channel?.label || '?'}」`); this._dc2 = e.channel; };
        } catch { /* noop */ }
      }
      ...
      try {
        if (typeof dc.send === 'function' && dc.readyState === 'open') {
          dc.send('hello from pc1'); sentOk = true;
      ...
      // 收集通道属性快照
      const get = (k: any) => this._safeGet(dc, k);
      const nullable = (k: any) => get(k) == null ? 'null' : get(k);
      const props = [
  ...
  _renderCard5() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([{ key: 'createDataChannel', ok: caps.rtcpc }, { key: 'RTCDataChannel', ok: caps.rtcdc }]),
        h(Tag, { color: 'primary' }, 'P2P 数据'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'pc.createDataChannel(label, options) 创建 P2P 数据通道：options 接受 ordered（顺序保证）、maxRetransmits（最大重传次数，与 maxPacketLifeTime 互斥）、negotiated（带外协商，需指定 id）、protocol。事件 onopen / onmessage / onclose / onerror；binaryType（blob|arraybuffer）控制二进制接收格式；bufferedAmount / bufferedAmountLowThreshold + onbufferedamountlow 处理背压。通道基于 SCTP over DTLS，提供可靠 / 不可选两种传输模式。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('创建数据通道', { type: 'primary', size: 'sm', disabled: !caps.rtcpc, onClick: () => this._demoDataChannel() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '数据通道状态与属性：'),
        h('pre', { class: 'code-block', style: { maxHeight: '320px', overflow: 'auto' } },
          h('code', {}, s.dcInfo || '（请先在 Card 1 创建 pc1/pc2，再点击「创建数据通道」）')),
        h(Alert, {
          type: 'info',
  ...
  async _demoStatsAndSession() {
    const caps = this._capFlags();
    if (!caps.rtcpc) { this._addLog('warn', 'RTCPeerConnection 不可用'); this.setState({ statsInfo: 'RTCPeerConnection 不可用，无法演示 getStats。' }); return; }
    if (!this._pc1) { this._addLog('warn', '请先点击 Card 1 的「创建 pc1/pc2」'); return; }
    const pc1 = this._pc1;
      ...
      try {
        ...
        try {
          for (const [id, stat] of report) {
            ...
            ((typeCounts as any)[(t as any)]) = (((typeCounts as any)[(t as any)]) || 0) + 1;
            if (samples.length < 3) samples.push(`  [${t}] ${String(id).slice(0, 12)}: ${JSON.stringify(stat).slice(0, 80)}`);
          }
        } catch (err: any) {
        ...
        lines.push('\nRTCStats 类型分布：');
        for (const t of knownTypes) { const n = ((typeCounts as any)[(t as any)]) || 0; if (n > 0) lines.push(`  ${t}: ${n}`); }
        const others = (Object as any).keys(typeCounts).filter((k: any) => !knownTypes.includes(k));
        if (others.length) lines.push(`  其它: ${others.map((k: any) => `${k}=${((typeCounts as any)[(k as any)])}`).join(', ')}`);

        lines.push('\n样本（前 3 条）：');
        lines.push(...samples);
        this._addLog('stats', `getStats 完成：${size} 条，类型 ${(Object as any).keys(typeCounts).join('/') || '无'}`);
      } catch (err: any) {
    ...
    if (typeof pc1.getConfiguration === 'function') {
      ...
      } catch (err: any) { lines.push(`   getConfiguration 失败：${err.message}`); }
    } else { lines.push('   getConfiguration 不可用（typeof undefined）'); }
    if (typeof pc1.setConfiguration === 'function') {
      ...
      } catch (err: any) { lines.push(`   setConfiguration 失败：${err.message}`); }
    } else { lines.push('   setConfiguration 不可用（需先 setLocalDescription）'); }

    // 3) restartIce()
    lines.push('\n3) ICE 重启：');
    if (typeof pc1.restartIce === 'function') {
      try { pc1.restartIce(); lines.push('   pc1.restartIce() 已调用（标记需重新收集 ICE 候选，下次 createOffer 携带 iceRestart）'); this._addLog('stats', 'pc1.restartIce() 已调用'); }
      catch (err: any) { lines.push(`   restartIce 失败：${err.message}`); }
    } else { lines.push('   restartIce 不可用（typeof undefined）'); }
  ...
  _renderCard6() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([{ key: 'getStats', ok: caps.rtcpc }, { key: 'RTCStatsReport', ok: caps.rtcStatsReport }]),
        h(Tag, { color: 'primary' }, 'RTCStats / session'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'pc.getStats() 返回 Promise<RTCStatsReport>，是一个 Map<id, RTCStats>，包含各类统计：candidate-pair（ICE 候选对）、inbound-rtp / outbound-rtp（收发 RTP）、remote-inbound-rtp / remote-outbound-rtp（远端收发）、data-channel（数据通道）、transport（传输）、candidate（候选）、codec（编解码器）。每条统计含 id / type / timestamp 及 type 专属字段。配合 getConfiguration / setConfiguration / restartIce / close 完成会话管理。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('刷新统计 + 会话管理', { type: 'primary', size: 'sm', disabled: !caps.rtcpc, onClick: () => this._demoStatsAndSession() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'getStats 结果与会话管理：'),
        h('pre', { class: 'code-block', style: { maxHeight: '360px', overflow: 'auto' } },
          h('code', {}, s.statsInfo || '（请先在 Card 1 创建 pc1/pc2，再点击「刷新统计 + 会话管理」）')),
        h(Alert, {
          type: 'info',
  ...
  render() {
    const s = this.state;
    return h('div', { class: 'page api-lab-page webrtc-deep-page' },
      h('h2', { class: 'section-title' }, 'WebRTC 深入 实验室'),
      h('p', { class: 'fs-sm text-secondary mb-md' },
        'WebRTC 用于浏览器间 P2P 音视频与数据通信。本页演示 RTCPeerConnection 配置与生命周期、Offer/Answer 协商、ICE 候选交换、MediaStream 媒体接入、DataChannel 数据通道与 getStats 统计。'),
