...
export interface EnvironmentPageProps extends Props {}
...
export interface EnvironmentPageState extends State {
  logs: EnvLog[];
...
export class EnvironmentPage extends Page {
  declare props: EnvironmentPageProps;
  declare state: EnvironmentPageState;
  _globalReady: boolean = false;
  _fullscreenTarget: HTMLElement | null = null;
  _wakeSentinel: any = null;
  _wakeLockDesired: boolean = false;
  _idleHandle: number | null = null;
  _rafHandle: number | null = null;
  _battery: any = null;

  initialState(): EnvironmentPageState {
    return {
      logs: [],
  ...
  componentDidMount(): void {
    ...
    if (!this._globalReady) {
      ...
      const supported = (Object as any).entries(capabilities).filter(([, v]: any) => v).map(([k]: any) => k);
    ...
    this._fullscreenTarget = this.$<HTMLElement>('.fullscreen-target');
  }
  ...
  _onVisibilityChange(): void {
    ...
    if (isHidden) {
      ...
      this._addLog('info', awaySec > 0
        ? `页面回到前台，离开时长 ${(awaySec as any).toFixed(2)}s`
        : `页面回到前台 (${state})`);
  ...
  _runIdleTask(): void {
    if (typeof window.requestIdleCallback === 'function') {
      this._idleHandle = window.requestIdleCallback((deadline: IdleDeadline) => {
        ...
        this._addLog('info', `idle 触发：timeRemaining=${(remaining as any).toFixed(1)}ms didTimeout=${deadline.didTimeout}`);
      });
  ...
  _onFullscreenChange(): void {
    ...
    this.setState({
      isFullscreen: !!el,
      fullscreenElement: el ? (el.className || el.tagName.toLowerCase()) : null as any,
    });
  ...
  _enterFullscreen(): void {
    ...
    const target = this._fullscreenTarget || this.$<HTMLElement>('.fullscreen-target');
    if (!target) return;
    try {
      const fn = (target as any).requestFullscreen || (target as any).webkitRequestFullscreen;
      if (fn) {
        fn.call(target);
  ...
  // —— 4. Vibration ——
  _vibrate(pattern: number | number[]): void {
  ...
  // —— 辅助：能力标签 ——
  _capTag(key: keyof Capabilities): Node {
  ...
  // —— 辅助：格式化电池时间 ——
  _formatBatteryTime(sec: number | null): string {
  ...
  renderPage(): Node | string | (Node | string)[] {
    const caps = this.state.capabilities as Capabilities;
    ...
    return [
          ...
          h('div', { class: 'flex flex-col gap-sm' },
            ...
            this.state.idleResults.length > 0
              ? h('div', { class: 'log-panel', style: { maxHeight: '120px' } },
                  ...this.state.idleResults.map((r: IdleResult) => h('div', { class: 'log-panel__line' },
                    h('span', { class: 'log-panel__time' }, r.time),
                    h('span', { class: 'log-panel__tag log-panel__tag--push' }, r.source),
                    h('span', {}, `timeRemaining=${(r.timeRemaining as any).toFixed(1)}ms didTimeout=${r.didTimeout}`),
                  )),
                )
              : h('p', { class: 'fs-sm text-tertiary' },
      ...
      h(Card, { title: '事件日志', extra: h('span', { class: 'fs-sm text-tertiary' }, '实时') },
        h('div', { class: 'log-panel' },
          ...this.state.logs.map((log: EnvLog) => h('div', { class: 'log-panel__line' },
