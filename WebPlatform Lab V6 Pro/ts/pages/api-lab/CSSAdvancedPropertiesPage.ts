...
interface CSSAdvancedPropertiesPageCaps {
  css: boolean;
  supports: boolean;
  attrTyped: boolean;
  attrLength: boolean;
  attrColor: boolean;
  attrFallback: boolean;
  attrMulti: boolean;
  attrString: boolean;
  carousel: boolean;
  carouselDirection: boolean;
  carouselLoop: boolean;
  carouselSnapping: boolean;
  scrollMarker: boolean;
  scrollMarkerGroup: boolean;
...
export interface CSSAdvancedPropertiesPageProps extends Props {}

export interface CSSAdvancedPropertiesPageState extends State {
  logs: LogEntry[];
  capsSummary: string;
  attrInfo: string;
  carouselInfo: string;
...
export class CSSAdvancedPropertiesPage extends Page {
  declare props: CSSAdvancedPropertiesPageProps;
  declare state: CSSAdvancedPropertiesPageState;
  ...
  // —— 初始 state ——
  initialState(): CSSAdvancedPropertiesPageState {
    return {
      logs: [],
      capsSummary: '',
      // Card 1：attr() 增强
  ...
  componentDidMount(): void {
    ...
    if (!f.attrTyped) this._addLog('warn', 'attr() 类型化语法不可用（CSS Values Level 5，仅 Chrome Canary 实验）');
    if (!f.carousel) this._addLog('warn', 'carousel-* 不可用（CSSWG 2024 提案，尚未在任何浏览器稳定支持）');
    if (!f.zoom) this._addLog('warn', 'zoom 属性不可用或 jsdom 未识别（Chrome/Edge/Safari 早已支持，Firefox 126+）');
    if (!f.env) this._addLog('warn', 'env() 不可用或 jsdom 未识别（Chrome/Safari/Firefox 已稳定）');
    if (!f.maskImage) this._addLog('warn', 'mask-image 不可用或 jsdom 未识别（Chrome 120+ 标准语法，无需 -webkit- 前缀）');
    if (!f.modal) this._addLog('warn', ':modal 伪类不可用或 jsdom 未识别（Chrome 111+ / Firefox 116+ / Safari 16.4+）');
    if (!f.nthChildOf) this._addLog('warn', ':nth-child(an+b of S) 不可用或 jsdom 未识别（Chrome 111+）');
  }
  ...
  _flags(): CSSAdvancedPropertiesPageCaps {
  ...
  _runAttrDemo(): void {
    ...
    try {
      ...
      this.setState({ attrInfo:
        'attr() 增强（CSS Values Level 5）演示：\n' +
  ...
  _renderCard1(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['attr(<type>)', f.attrTyped], ['attr+fallback', f.attrFallback], ['attr(<color>)', f.attrColor]]),
        h(Tag, { color: 'primary' }, '类型化 / fallback'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'attr() 增强让 CSS 属性值直接读取 HTML 属性，并支持类型推断与 fallback。语法：attr(<name> <type>?, <fallback>?)。<type> 取值：<length>/<color>/<url>/<number>/<percentage>/<angle>/<time>/<resolution>/<integer>/"*"/<type>+（多值）/ <type>#（逗号列表）。历史 Level 3 仅支持 content 字符串；Level 5 扩展到任意属性 + 类型 + fallback。让数据驱动样式无需 JS 中转。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行 attr() 演示', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runAttrDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'attr() 演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } },
          h('code', {}, s.attrInfo || '（点击「运行 attr() 演示」）')),
...
@supports (width: attr(data-w <length>)) {
    ...
    try {
      if (typeof CSSStyleSheet !== 'undefined') {
        try {
          ...
          sheet.replaceSync(
            '.cap-carousel { carousel: auto; carousel-direction: inline; carousel-loop: on; carousel-snapping: mandatory; } ' +
      ...
      host.innerHTML =
        '<div class="cap-carousel-track">' +
          Array.from({ length: 5 }, (_, i) =>
            `<div class="cap-carousel-slide">Slide ${i + 1}</div>`).join('') +
        '</div>';
      ...
      this.setState({ carouselInfo:
        'CSS Carousels（CSSWG 2024 提案）演示：\n' +
  ...
  _renderCard2(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['carousel-*', f.carousel], ['scroll-marker', f.scrollMarker], ['scroll-buttons', f.scrollButtons]]),
        h(Tag, { color: 'primary' }, '纯 CSS 轮播'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'CSS Carousels 提案让纯 CSS 实现轮播图：carousel-* 属性族（carousel / carousel-direction / carousel-loop / carousel-snapping）控制行为；scroll-marker / scroll-marker-group 生成分页指示器；scroll-buttons 生成前进/后退按钮；::carousel-content / ::scroll-marker / ::scroll-button-* 等伪元素由浏览器自动生成。键盘方向键、触屏滑动、Tab 焦点循环全部原生支持，无需 JS。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行 Carousel 演示', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runCarouselDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'Carousel 演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '280px', overflow: 'auto' } },
          h('code', {}, s.carouselInfo || '（点击「运行 Carousel 演示」）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
        h('pre', { class: 'code-block', style: { maxHeight: '180px', overflow: 'auto' } },
          h('code', {},
`<div carousel="auto">
...
[carousel]::scroll-marker:current { background: #3b82f6; }`)),
  ...
  _renderCard3(): Node | string {
    ...
    const card = new Card({
      title: '3. CSS zoom 属性',
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['zoom', f.zoom], ['zoom:reset', f.zoomReset]]),
        h(Tag, { color: 'primary' }, 'vs transform: scale()'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'zoom 属性让元素整体缩放并影响布局（占据缩放后空间），与 transform: scale() 仅做视觉变换不占空间形成关键差异。语法：zoom: <number> | <percentage> | normal | reset。reset 值（Chrome 110+）用于响应用户缩放操作。历史上仅 IE/Edge/Chrome/Safari 支持，Firefox 126+（2024）起正式支持，现已全主流可用。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行 zoom 演示', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runZoomDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'zoom 演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '320px', overflow: 'auto' } },
          h('code', {}, s.zoomInfo || '（点击「运行 zoom 演示」）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
        h('pre', { class: 'code-block', style: { maxHeight: '150px', overflow: 'auto' } },
          h('code', {},
`.container { zoom: 1.25; }              /* 整体放大 1.25 倍，影响布局 */
...
if (CSS.supports('zoom', '1.5')) { /* 安全使用 zoom */ }`)),
  ...
  _runEnvDemo(): void {
    ...
    try {
      ...
      this.setState({ envInfo:
        'CSS env() 与 viewport-fit 演示：\n' +
  ...
  _renderCard4(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['env()', f.env], ['constant()', f.constant], ['titlebar-area', f.envTitlebar]]),
        h(Tag, { color: 'primary' }, '刘海屏 / 折叠屏 / PWA'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'env() 读取浏览器环境变量，主要用于响应刘海屏 / 折叠屏 / PWA 桌面窗口的环境边距。env(safe-area-inset-*) 提供安全区边距；env(titlebar-area-*) 用于 PWA 桌面应用自定义标题栏拖拽区；env(viewport-segment-*) 用于折叠屏双屏尺寸与铰链位置；env(keyboard-inset-height) 用于虚拟键盘避让。前置：HTML <meta name="viewport" content="viewport-fit=cover">。constant() 是 iOS < 11.2 的旧前缀。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行 env() 演示', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runEnvDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'env() 演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '320px', overflow: 'auto' } },
          h('code', {}, s.envInfo || '（点击「运行 env() 演示」）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
...
if (CSS.supports('padding-top', 'env(safe-area-inset-top)')) { /* env() 可用 */ }`)),
  ...
  _runMaskDemo(): void {
    ...
    try {
      ...
      const maskImageVal = window.getComputedStyle(g as Element)!.getPropertyValue('mask-image');
      const webkitMaskImageVal = window.getComputedStyle(g as Element)!.getPropertyValue('-webkit-mask-image');
      const maskModeVal = window.getComputedStyle(g as Element)!.getPropertyValue('mask-mode');
      const maskCompositeVal = window.getComputedStyle((host.querySelector('.cap-mask-composite') as any)).getPropertyValue('mask-composite');
      this.setState({ maskInfo:
        'CSS Masking 高级演示：\n' +
  ...
  _renderCard5(): Node | string {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['mask-image', f.maskImage], ['mask-mode', f.maskMode], ['mask-composite', f.maskComposite], ['mask-border', f.maskBorder]]),
        h(Tag, { color: 'primary' }, '与 background-* 同构'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'CSS Masking 通过 mask-* 属性族（与 background-* 同构）实现图像蒙版：mask-image / mask-mode / mask-repeat / mask-position / mask-size / mask-origin / mask-clip / mask-composite。mask-mode: alpha|luminance|match-source 决定用蒙版图像的 alpha 通道还是亮度作为遮罩；mask-composite: add|subtract|intersect|exclude 控制多个蒙版的合成方式。Chrome 120+（2023.12）起标准语法无需 -webkit- 前缀；Safari 仍需 -webkit-；Firefox 53+ 早已支持。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行 Mask 演示', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runMaskDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'Mask 演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '340px', overflow: 'auto' } },
          h('code', {}, s.maskInfo || '（点击「运行 Mask 演示」）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
...
.svg-mask {
  ...
  _runPseudoDemo(): void {
    ...
    try {
      ...
      host.innerHTML =
        ...
        '<div class="cap-pseudo-list">' +
          Array.from({ length: 6 }, (_, i) =>
            `<div class="cap-pseudo-item ${i % 2 === 0 ? 'even' : 'odd'}">Item ${i + 1}</div>`).join('') +
        '</div>';
      document.body.appendChild(host);
      this._dynamicNodes.push(host);
      ...
      try {
        const dlg = host.querySelector('.cap-pseudo-dialog') as any;
        if (dlg && typeof dlg.showModal === 'function') {
          // jsdom 中 showModal 可能抛错；真实浏览器会显示模态
          try { dlg.showModal(); modalMatches = dlg.matches(':modal'); } catch { /* noop */ }
        }
      } catch { /* noop */ }
      try {
        const pop = host.querySelector('#cap-pseudo-popover') as any;
        if (pop && typeof pop.showPopover === 'function') {
          try { pop.showPopover(); popoverMatches = pop.matches(':popover-open'); } catch { /* noop */ }
        }
      } catch { /* noop */ }
  ...
  _renderCard6(): Node | string {
    ...
    const card = new Card({
      title: '6. CSS 高级伪类全家桶',
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([[':modal', f.modal], [':popover-open', f.popoverOpen], [':nth-child(of)', f.nthChildOf], [':dir()', f.dir]]),
        h(Tag, { color: 'primary' }, 'modal / media / selector'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'CSS 高级伪类覆盖三类：交互状态（:modal / :popover-open / :fullscreen / :picture-in-picture）、选择器增强（:target-within / :dir() / :defined / :nth-child(an+b of S)）、媒体状态（:playing / :paused / :seeking / :buffering / :muted / :volume-locked）。:nth-child(of S) 仅在 S 匹配的子元素中计数（vs :nth-child 先选再过滤）；:dir() 匹配计算方向（vs [dir] 属性选择器仅匹配显式声明）；:defined 区分自定义元素是否已注册，常用于加载占位样式。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行伪类演示', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runPseudoDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '伪类演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '360px', overflow: 'auto' } },
          h('code', {}, s.pseudoInfo || '（点击「运行伪类演示」）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
        h('pre', { class: 'code-block', style: { maxHeight: '220px', overflow: 'auto' } },
          h('code', {},
`/* 模态框样式（仅 showModal 触发） */
...
my-widget:defined { animation: fade-in 0.3s; }`)),
  ...
  render(): Node {
    const s = this.state;
    return h('div', { class: 'api-lab-page css-advanced-properties-page' },
      h('h2', { class: 'section-title' }, 'CSS 高级属性 实验室'),
      h('p', { class: 'fs-sm text-secondary mb-md' },
        '本页演示 attr() 类型化（CSS Values Level 5）、CSS Carousels 提案、CSS zoom 属性、env() 与 viewport-fit、CSS Masking 高级、CSS 高级伪类全家桶（:modal / :popover-open / :nth-child(of) / :dir() / 媒体伪类等）。所有特性通过 typeof / CSS.supports 能力检测，不可用时仅记日志，绝不抛异常。'),
