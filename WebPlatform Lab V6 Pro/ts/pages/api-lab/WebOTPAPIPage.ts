...
interface WebOTPAPIPageState extends State {
  logs: any;
  capsSummary: any;
  overviewInfo: any;
  getOtpInfo: any;
...
export class WebOTPAPIPage extends Page {
  declare state: WebOTPAPIPageState;
  ...
  componentDidMount() {
    ...
    if (!f.otpCredential) this._addLog('warn', 'OTPCredential 不可用（仅 Android Chrome 84+ 支持；iOS Safari/Firefox/桌面浏览器不支持，需降级 autocomplete="one-time-code" + 手动输入）');
    if (!f.credentialsGet) this._addLog('warn', 'navigator.credentials.get 不可用（jsdom 不实现，真实浏览器 Chrome 51+/Firefox 60+ 支持 Credential Management API）');
    if (!f.secureContext) this._addLog('warn', '当前非安全上下文（window.isSecureContext=false），WebOTP 在非安全上下文被禁用（需 HTTPS 或 localhost）');
  ...
  componentWillUnmount() {
    ...
    // 清理模拟 OTP 定时器
    if (this._otpTimer) {
      try { clearTimeout(this._otpTimer); } catch { /* noop */ }
      this._otpTimer = null;
    }
  }
  ...
  _flags() {
    ...
    return {
      ...
      oneTimeCode: safe(() => typeof document !== 'undefined' && typeof document.createElement === 'function' && 'autocomplete' in (document.createElement('input') as any)),
    };
  }
  ...
  _tryGetOtp(onCode: any) {
    ...
    navigator.credentials.get({
      ...
      if (cred && cred.code) {
        ...
        if (typeof onCode === 'function') onCode(cred.code);
      } else {
    ...
    const card = new Card({
      ...
      children: [
        ...
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.overviewInfo || '（点击「运行演示」查看 WebOTP API 完整说明）')),
        h(Alert, {
          type: 'info',
  ...
  _readGetOtpInfo() {
    const f = this._flags();
    try {
      const input = this.el && (this.el as Element).querySelector('.otp-input-2') as HTMLInputElement | null;
      let currentVal = '(未渲染)';
      if (input) currentVal = input.value || '(空)';
  ...
  _renderCard2() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['OTPCredential', f.otpCredential], ['credentials.get', f.credentialsGet]]),
        h(Tag, { color: 'primary' }, 'Promise 异步'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'navigator.credentials.get({ otp: { transport: ["sms"] }, signal }) 异步等待短信，返回 OTPCredential（含 code 验证码、transport、id、type）。transport 目前仅支持 "sms"（必须为数组）。Promise 异步等待短信到达，无短信则永久 pending（必须 AbortController 超时中止）。credential.code 可能含空格需 trim()。完整流程：等待短信 → 拿到 code → 填充 input → 提交表单。非 HTTPS/非 Android Chrome 直接 reject 或 API 不存在。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('接收验证码', { type: 'primary', size: 'sm', disabled: !f.credentialsGet && !f.oneTimeCode, onClick: () => this._runGetOtpDemo() }),
          this._btn('读取信息', { size: 'sm', onClick: () => this.setState({ getOtpInfo: this._readGetOtpInfo() }) }),
        ),
        h('div', { class: 'otp-input-wrap' },
          h('input', { class: 'otp-input otp-input-2', type: 'text', inputmode: 'numeric', autocomplete: 'one-time-code', placeholder: '点击「接收验证码」', maxlength: '6' }),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.getOtpInfo || '（点击「接收验证码」查看 OTPCredential 完整说明；不支持环境用模拟 OTP）')),
        h(Alert, {
          type: 'warning',
  ...
  _renderCard3() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['OTPCredential', f.otpCredential]]),
        h(Tag, { color: 'warning' }, '格式严格'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'WebOTP 短信必须包含格式：@origin https://example.com #123456 [8mA3Kp9xQ2L]（最后一行）。@origin 后跟当前页面 URL 的 origin（防钓鱼：浏览器校验 origin 匹配），# 后跟验证码（提取为 credential.code），App Hash 11 位哈希（可选，Android App Hash 唯一标识发送 App）。格式不匹配不触发（缺关键字/origin 不匹配/App Hash 不匹配/不在最后一行）。与运营商短信过滤：Chrome Android 用 SMS Retriever API 自动读取，运营商附加内容需保证格式在末尾。兼容旧格式 <#> 前缀。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行演示', { type: 'primary', size: 'sm', onClick: () => this._runSmsFormatDemo() }),
        ),
        h('div', { class: 'otp-sms-preview' },
          '模拟 SMS 内容：\n',
          h('span', {}, 'Your verification code is '),
          h('span', { class: 'tag-code' }, '123456'),
          h('span', {}, '.\n'),
          h('span', { class: 'tag-origin' }, '@https://example.com'),
          h('span', {}, ' '),
          h('span', { class: 'tag-code' }, '#123456'),
          h('span', {}, ' '),
          h('span', { class: 'tag-hash' }, '8mA3Kp9xQ2L'),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.smsFormatInfo || '（点击「运行演示」查看短信格式规范完整说明）')),
        h(Alert, {
          type: 'warning',
  ...
  _renderCard4() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['AbortController', f.abortController], ['AbortSignal', f.abortSignal]]),
        h(Tag, { color: 'primary' }, '防悬挂 Promise'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'navigator.credentials.get({ otp, signal: ac.signal }) 通过 AbortController 中止等待。中止场景：超时（setTimeout 60 秒，防永久 pending）、用户切换页面（visibilitychange → hidden 时 abort）、用户手动取消（取消按钮）。中止后 Promise reject，err.name === "AbortError"（应静默处理）。防止悬挂 Promise：不中止会导致 Promise 永不 resolve/reject 内存泄漏、页面切换后仍占用 SMS 监听资源、多次调用累积 pending。解决：始终绑定 AbortController + 超时 + visibilitychange。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行演示（3 秒后中止）', { type: 'primary', size: 'sm', disabled: !f.abortController, onClick: () => this._runAbortDemo() }),
          this._btn('读取信息', { size: 'sm', onClick: () => this.setState({ abortInfo: this._readAbortInfo() }) }),
        ),
        h('div', { class: 'otp-output' }, '点击「运行演示」后 3 秒将触发 abort()（见日志）'),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.abortInfo || '（点击「运行演示」查看 abort 信号完整说明）')),
        h(Alert, {
          type: 'warning',
  ...
  _readAutocompleteInfo() {
    const f = this._flags();
    try {
      const input = this.el && (this.el as Element).querySelector('.otp-input-otc');
      let hasAutocomplete = '(未渲染)';
      if (input) hasAutocomplete = input.getAttribute('autocomplete') || '(空)';
  ...
  _renderCard5() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['one-time-code', f.oneTimeCode]]),
        h(Tag, { color: 'success' }, 'iOS Safari 支持'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'autocomplete="one-time-code" 是 HTML 标准属性（非 WebOTP API），浏览器识别后收到短信自动填充。与 WebOTP API 协同：WebOTP API 主动请求（JS 控制，Android Chrome 首选），autocomplete 声明式自动检测（降级）。iOS Safari 支持 autocomplete（但不支持 WebOTP API），收到短信弹"填充验证码"建议。跨平台降级：Android Chrome 用 WebOTP API + autocomplete，iOS Safari 仅 autocomplete，桌面部分支持 + 手动输入。最佳实践：同时使用，配 inputmode="numeric" 弹数字键盘。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行演示', { type: 'primary', size: 'sm', onClick: () => this._runAutocompleteDemo() }),
        ),
        h('div', { class: 'otp-autocomplete-stage' },
          '跨平台降级输入框（iOS Safari 也能自动填充）：',
          h('div', { class: 'otp-input-wrap' },
            h('input', { class: 'otp-input otp-input-otc', type: 'text', inputmode: 'numeric', autocomplete: 'one-time-code', pattern: '[0-9]*', maxlength: '6', placeholder: '123456' }),
          ),
          h('div', { style: { marginTop: '6px', fontSize: '12px' } }, 'iOS Safari 收到短信会弹出"填充验证码"建议；Android Chrome 同时支持 WebOTP API 和 autocomplete。'),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.autocompleteInfo || '（点击「运行演示」查看 autocomplete="one-time-code" 完整说明）')),
        h(Alert, {
          type: 'info',
  ...
  _runLoginFlowDemo() {
    ...
    // 模拟流程：标记 step 状态
    const steps = this.el && (this.el as Element).querySelectorAll('.otp-login-flow .step');
    if (steps && steps.length >= 5) {
      steps[0].classList.add('done');
      steps[1].classList.add('active');
    }
    this._tryGetOtp((code: any) => {
      const input = this.el && (this.el as Element).querySelector('.otp-input-login') as HTMLInputElement | null;
      if (input) {
        input.value = code;
        input.classList.add('otp-filled');
      }
  ...
  _renderCard6() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['OTPCredential', f.otpCredential], ['AbortController', f.abortController]]),
        h(Tag, { color: 'primary' }, '渐进增强'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '完整流程：发送短信 → input focus + credentials.get → 浏览器等待 → 短信到达 resolve → 自动填充 → 自动提交。用户切换/超时处理：visibilitychange/60 秒超时/手动输入 abort()。与现有登录流程集成：渐进增强（不支持跳过，保持手动输入）、不破坏现有流程（WebOTP 仅加速）、表单仍需 pattern+maxlength 校验、提交仍需服务端校验（前端填充不可信）。陷阱：不要 await 阻塞 UI（用 .then 异步）、自动提交前确认用户意图、服务端必须校验。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行演示', { type: 'primary', size: 'sm', onClick: () => this._runLoginFlowDemo() }),
        ),
        h('div', { class: 'otp-login-flow' },
          h('div', { class: 'step' }, h('span', { class: 'step-num' }, '1'), h('span', {}, '发送短信（用户输入手机号，点击发送）')),
          h('div', { class: 'step' }, h('span', { class: 'step-num' }, '2'), h('span', {}, 'input focus + navigator.credentials.get({ otp })')),
          h('div', { class: 'step' }, h('span', { class: 'step-num' }, '3'), h('span', {}, '浏览器等待短信到达（Promise pending）')),
          h('div', { class: 'step' }, h('span', { class: 'step-num' }, '4'), h('span', {}, '自动填充 input.value = cred.code')),
          h('div', { class: 'step' }, h('span', { class: 'step-num' }, '5'), h('span', {}, '自动提交表单 form.submit()')),
        ),
        h('div', { class: 'otp-input-wrap' },
          h('input', { class: 'otp-input otp-input-login', type: 'text', inputmode: 'numeric', autocomplete: 'one-time-code', maxlength: '6', placeholder: '验证码自动填充到此' }),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.loginFlowInfo || '（点击「运行演示」查看登录验证码自动填充完整流程）')),
        h(Alert, {
          type: 'info',
  ...
  _renderCard7() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['OTPCredential', f.otpCredential], ['WebAuthn', f.webAuthn]]),
        h(Tag, { color: 'primary' }, 'WebAuthn 协同'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'TOTP 与 SMS OTP 区别：SMS OTP（WebOTP 支持，依赖手机号+短信通道，易被 SIM 交换攻击，体验好但安全性低）；TOTP（Google Authenticator 基于 time+共享密钥离线生成，安全性高但需装 App，WebOTP 不支持 TOTP 仅 SMS）。与 WebAuthn 协同：WebAuthn（指纹/Face ID/安全密钥，抗钓鱼）作主凭据日常登录，SMS OTP 作备用/账户恢复。降级方案链：WebAuthn → WebOTP API → autocomplete → 手动输入 → TOTP。陷阱：SMS OTP 不应作唯一凭据（SIM 交换攻击）。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行演示', { type: 'primary', size: 'sm', onClick: () => this._runTwoFaDemo() }),
        ),
        h('table', { class: 'otp-2fa-table' },
          h('thead', {},
            h('tr', {}, h('th', {}, '方案'), h('th', {}, '凭据类型'), h('th', {}, 'WebOTP 支持'), h('th', {}, '抗钓鱼'), h('th', {}, '体验')),
          ),
          h('tbody', {},
            h('tr', {}, h('td', {}, 'WebAuthn'), h('td', {}, '生物/硬件'), h('td', { class: 'no' }, '✗'), h('td', { class: 'yes' }, '✓'), h('td', {}, '好')),
            h('tr', {}, h('td', {}, 'SMS OTP'), h('td', {}, '短信验证码'), h('td', { class: 'yes' }, '✓'), h('td', { class: 'no' }, '✗'), h('td', {}, '好')),
            h('tr', {}, h('td', {}, 'TOTP'), h('td', {}, '时间一次性码'), h('td', { class: 'no' }, '✗'), h('td', { class: 'partial' }, '部分'), h('td', {}, '中')),
          ),
        ),
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.twoFaInfo || '（点击「运行演示」查看 2FA WebAuthn + SMS OTP 协同完整代码）')),
        h(Alert, {
          type: 'warning',
    ...
    const card = new Card({
      ...
      children: [
        ...
        h('pre', { class: 'code-block mt-sm', style: { maxHeight: '600px', overflow: 'auto' } },
          h('code', {}, s.pitfallsInfo || '（点击「运行演示」查看 7 大陷阱与 12 条最佳实践完整说明）')),
        h(Alert, {
          type: 'warning',
