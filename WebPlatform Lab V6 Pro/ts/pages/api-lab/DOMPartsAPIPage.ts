...
interface DOMPartsAPIPageState extends State {
  logs: any;
  capsSummary: any;
  conceptInfo: any;
  rootInfo: any;
...
export class DOMPartsAPIPage extends Page {
  declare state: DOMPartsAPIPageState;
  _childContainer!: any;
  _dynamicStyles!: any[];
  _formContainer!: any;
  _formTpl!: any;
...
const createInstanceOK =
  ...
  _runRootDemo() {
    ...
    `);
    const container = (this.$('.domparts-root-host') as any);
...
part.disconnect();`)),
  ...
  _runChildNodeDemo() {
    ...
    `);
    const container = (this.$('.domparts-child-host') as any);
    ...
    if (f.createChildNodePart && f.childNodePart) {
      try {
        ...
        if (container) { container.innerHTML = ''; container.appendChild(host); }
        const root = typeof host.getPartRoot === 'function' ? (host as any).getPartRoot() : null;
...
function renderList(host: any,  items: any) {
  ...
  _runInstanceDemo() {
    ...
    `);
    const container = (this.$('.domparts-instance-host') as any);
...
class StylePart extends Part {
  ...
  _runListDemo() {
    ...
    `);
    const container = (this.$('.domparts-list-host') as any);
...
customElements.define('item-list', ItemList);`)),
  ...
  _runFormDemo() {
    ...
    `);
    const container = (this.$('.domparts-form-host') as any);
    ...
    if (f.createInstance && f.propertyPart) {
      try {
        ...
        if (instance && typeof instance.update === 'function') {
          instance.update({ name: 'Alice', onInput: (e: any) => this._addLog('info', 'input: ' + e.target.value) });
