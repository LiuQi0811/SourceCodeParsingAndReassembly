...
interface DecodeConfigItem {
  name: string;
  config: any;
}

const DECODE_CONFIGS: DecodeConfigItem[] = [
...
const ENCODE_CONFIGS: DecodeConfigItem[] = [
...
export interface AdvancedMediaPageProps extends Props {}

export interface AdvancedMediaLog {
  type: string;
  content: string;
  time: string;
}

export interface AdvancedMediaPageState extends State {
  logs: AdvancedMediaLog[];
  mseSupported: boolean | null;
  mimeResults: MimeResult[];
  mseStatus: string;
  pipSupported: boolean | null;
  docPipSupported: boolean | null;
  pipActive: boolean;
  docPipActive: boolean;
  pipWindowSize: string;
  mcSupported: boolean | null;
  decodeResults: CapResultRow[];
  encodeResults: CapResultRow[];
  emeSupported: boolean | null;
  emeResults: EmeResultRow[];
  textTracksList: TextTrackInfo[];
  vttCues: { start: number; end: number; text: string }[];
  trackSupported: boolean | null;
  generatorSupported: boolean | null;
  trackInfo: string;
}

export class AdvancedMediaPage extends Page {
  declare props: AdvancedMediaPageProps;
  declare state: AdvancedMediaPageState;
  _inited: boolean = false;
  declare _destroyed: boolean;
  _mediaSource: MediaSource | null = null;
  _mseUrl: string | null = null;
  _sourceBuffer: SourceBuffer | null = null;
  _videoEl: HTMLVideoElement | null = null;
  _trackVideoEl: HTMLVideoElement | null = null;
  _pipWindow: any | null = null;
  _docPipWindow: Window | null = null;
  _docPipPlaceholder: Node | null = null;
  _mediaKeys: MediaKeys | null = null;
  _keySession: MediaKeySession | null = null;
  _mediaStream: MediaStream | null = null;
  _streamTrack: MediaStreamTrack | null = null;

  initialState(): AdvancedMediaPageState {
    return {
      logs: [],
  ...
  _bindControls(): void {
    const video = this.$<HTMLVideoElement>('.mse-video');
    if (video) {
      this._videoEl = video;
    ...
    const trackVideo = this.$<HTMLVideoElement>('.track-video');
    if (trackVideo) {
  ...
  _createMSE(): void {
    ...
    try {
      ...
      this.on(ms, 'sourceopen', () => {
        ...
        this.on(sb, 'error', (e) => this._addLog('error', `SourceBuffer.error：${(e as any)?.message || 'unknown'}`));
  ...
  _requestDocPiP(): void {
    ...
    const target = this.$<HTMLElement>('.doc-pip-content');
    ...
    dpi.requestWindow({ width: 360, height: 240 })
      ...
      .catch((err: any) => this._addLog('error', `requestWindow 失败：${errInfo(err).message}`));
  }
  ...
  async _runDecodeInfo(): Promise<void> {
  ...
  async _runEncodeInfo(): Promise<void> {
    if (!this.state.mcSupported) {
  ...
  async _probeKeySystems(): Promise<void> {
  ...
  async _createMediaKeys(): Promise<void> {
    ...
    try {
      ...
      this.on(session, 'message', (e) => {
        const ev = e as MediaKeyMessageEvent;
  ...
  _addTextTrack(): void {
    ...
    for (const c of SAMPLE_CUES) {
      ...
      cue.line = 'auto' as any; cue.position = 50; cue.size = 100; cue.align = 'center' as any;
      track.addCue(cue);
    }
    ...
    this.on(track, 'cuechange', () => {
      ...
      const active = activeCues && activeCues.length ? [...activeCues as any].map((c: any) => c.text).join(' / ') : '（无）';
      this._addLog('eme', `TextTrack.cuechange · activeCues: ${active}`);
    });
    this._refreshTextTracks();
  }

  _refreshTextTracks(): void {
    const v = this._trackVideoEl;
    if (!v) return;
    const list: TextTrackInfo[] = [];
  ...
  _removeFirstCue(): void {
    ...
    const cue = cues[0];
    if (cue) t.removeCue(cue as VTTCue);
  ...
  _detectTrack(): void {
    ...
    const generatorSupported = typeof (window as any).MediaStreamTrackGenerator !== 'undefined';
    this.setState({
      trackSupported,
      generatorSupported,
  ...
  async _getUserMedia(): Promise<void> {
  ...
  _probeGenerator(): void {
    ...
    try {
      ...
      const GeneratorCtor = (window as any).MediaStreamTrackGenerator;
  ...
  _renderCapabilitiesTable(rows: CapResultRow[]): Node {
    if (!rows || rows.length === 0) return h('div', { class: 'log-panel__empty' }, '（点击上方按钮运行检测）');
    const TH: Props = { padding: '6px 10px', borderBottom: '1px solid var(--color-border, #eee)', textAlign: 'left', fontSize: '12px' };
    const TD: Props = { padding: '6px 10px', borderBottom: '1px solid var(--color-border, #f0f0f0)', fontSize: '12px' };
    const tagFor = (v: boolean): Node => v ? h(Tag, { color: 'success' }, '是') : h(Tag, { color: 'error' }, '否');
      ...
      h(Card, {
        ...
        h('div', { class: 'flex flex-col gap-sm' },
          ...
          h('div', { class: 'flex items-center gap-sm flex-wrap' },
            ...
            s.pipWindowSize ? h('span', { class: 'fs-sm text-secondary' }, `window=${s.pipWindowSize}`) : null as any,
            ...
            ? h('div', {},
                ...
                h('div', { class: 'flex flex-col gap-xs' },
                  ...s.vttCues.map((c) => h('div', { class: 'fs-sm', style: { padding: '4px 8px', background: 'var(--color-fill, #f5f5f5)', borderRadius: '4px' } },
                    h('span', { class: 'text-tertiary' }, `[${(c.start as any).toFixed(1)}→${(c.end as any).toFixed(1)}] `),
                    h('span', {}, c.text),
                  )),
                ),
              )
