...
export interface WebLocksAPIPageProps extends Props {}

export interface WebLocksAPIPageState extends State {
  logs: LogEntry[];
  capsSummary: string;
  requestInfo: string;
  modeInfo: string;
...
export class WebLocksAPIPage extends Page {
  declare props: WebLocksAPIPageProps;
  declare state: WebLocksAPIPageState;

  _inited: boolean = false;
  declare _destroyed: boolean;
  _simLocks!: any;
  _simClientId!: string;
  _simClientSeq!: number;
  _pendingLocks!: any;


  // —— 初始 state ——
  initialState(): WebLocksAPIPageState {
    return {
      logs: [],
      capsSummary: '',
  ...
  async _demoExclusiveQueue() {
    ...
    if (!caps.request) {
      ...
      const order: any[] = [];
      const make = (id: any) => new Promise((resolve) => {
        setTimeout(() => {
    ...
    try {
      const order: any[] = [];
      const make = (id: any) => navigator.locks.request(name, async (lock) => {
  ...
  async _demoSharedCoexist() {
    ...
    try {
      const acquired: any[] = [];
      const make = (id: any) => navigator.locks.request(name, { mode: 'shared' } as any, async (lock) => {
  ...
  _renderCard2(): Node | string {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          "默认 exclusive（排他锁，同名同时仅 1 个持有者）；shared（共享锁，多个 shared 可共存，但与 exclusive 互斥）。用法：navigator.locks.request(name, { mode: 'shared' } as any, cb)。典型：shared 用于读（并发）、exclusive 用于写（互斥），类似读写锁。"),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
  ...
  async _demoIfAvailable() {
    ...
    try {
      ...
      const fast = navigator.locks.request(name, { ifAvailable: true } as any, async (lock) => {
  ...
  async _demoSteal() {
    ...
    try {
      const events: any[] = [];
      // A：长持有，监听 signal
      const a = navigator.locks.request(name, (async (lock: any, signal: any) => {
      ...
      // B：steal 抢占
      await new Promise((r) => setTimeout(r, 50));
      const b = navigator.locks.request(name, { steal: true } as any, async (lock) => {
  ...
  async _demoLockObject() {
    ...
    try {
      let captured = null;
      await navigator.locks.request(name, async (lock) => {
        captured = lock ? { name: lock.name, mode: lock.mode, keys: (Object as any).keys(lock) } : null;
  ...
  _compareWithBroadcastChannel(): void {
    this.setState({ scenarioInfo:
