...
interface HTMLMediaElementAdvancedPageState extends State {
  logs: any;
  capsSummary: any;
  overviewInfo: any;
  sinkInfo: any;
  ...
  rangesInfo: any;
  patternInfo: any;
  audioSessionInfo: any;
}

export class HTMLMediaElementAdvancedPage extends Page {
  declare state: HTMLMediaElementAdvancedPageState;
  _addAvailabilityId!: any;
  _audioPitchEl!: any;
  _audioRangesEl!: any;
  _audioSinkEl!: any;
  _audioTracksEl!: any;
  ...
  _initRefs() {
    ...
    this._audioSinkEl = (this.$('.sink-audio') as any);
    this._audioPitchEl = (this.$('.pitch-audio') as any);
    this._videoRemoteEl = (this.$('.remote-video') as any);
    this._audioTracksEl = (this.$('.tracks-audio') as any);
    this._audioRangesEl = (this.$('.ranges-audio') as any);
  }

  _registerMediaEl(el: any) {
  ...
  _flags() {
    ...
    if (typeof document !== 'undefined' && typeof document.createElement === 'function') {
      try { audio = (document.createElement('audio') as any); } catch { audio = {}; }
    }
    return {
      setSinkId: safe(() => typeof (proto as any).setSinkId === 'function' || typeof (audio as any).setSinkId === 'function'),
...
const caps = {
  ...
  async _runSinkDemo() {
    ...
    `);
    const audio = (this.$('.sink-audio') as any);
    ...
    if (f.enumerateDevices) {
      try {
        ...
        const list = (this.$('.hme-sink-list') as any);
        if (list) {
          list.innerHTML = '';
    ...
    if (f.setSinkId && audio) {
      try {
        ...
        if (f.enumerateDevices) {
          const devices = await navigator.mediaDevices.enumerateDevices();
          const outs = devices.filter((d: any) => d.kind === 'audiooutput');
          if (outs.length > 1) targetId = outs[1].deviceId;
        }
        if (targetId) {
          await (audio as any).setSinkId(targetId);
...
audioElement.audioSink = '';`)),
  ...
  _runPitchDemo() {
    ...
    `);
    const audio = (this.$('.pitch-audio') as any);
    this._registerMediaEl(audio);
...
[0.5, 0.75, 1, 1.25, 1.5, 2].forEach((rate: any) => {
  ...
  _runControlsDemo() {
    ...
    `);
    const video = (this.$('.cl-video') as any);
    ...
    if (f.controlsList && video) {
      try {
        ...
        // 演示 toggle 各 token
        CONTROLS_LIST_TOKENS.forEach((token: any) => {
          try {
    ...
    const card = new Card({
      ...
      children: [
        ...
        h('div', { class: 'hme-controls-host' },
          ...
          h('div', {},
            ...CONTROLS_LIST_TOKENS.map((t: any) => h('span', { class: 'hme-cl-token hme-cl-token--on' }, t)),
          ),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
...
obs.observe(video, { attributes: true });`)),
  ...
  async _runRemoteDemo() {
    ...
    `);
    const video = (this.$('.remote-video') as any);
    this._registerMediaEl(video);
      ...
      try {
        ...
        if (remote) {
          ...
          if (typeof remote.watchAvailability === 'function') {
            try {
              const id = await remote.watchAvailability((available: any) => {
...
video.disableRemotePlayback = true;`)),
  ...
  _runTracksDemo() {
    ...
    `);
    const audio = (this.$('.tracks-audio') as any);
    this._registerMediaEl(audio);
...
at.addEventListener('addtrack', e => console.log('新增音轨', e.track));`)),
  ...
  _runRangesDemo() {
    ...
    `);
    const audio = (this.$('.ranges-audio') as any);
    this._registerMediaEl(audio);

    const firedEvents = new Set<any>();
    if (audio) {
      // 绑定核心媒体事件，统计触发情况
      MEDIA_EVENTS.forEach((evt: any) => {
        this.on(audio, evt, () => {
      ...
      // 演示 TimeRanges 读取
      try {
        const readRanges = (name: any, ranges: any) => {
...
function isBuffered(time: any) {
  ...
  _runPatternDemo() {
    ...
    // 综合演示：在多语言播放器上联动各特性
    const audio = (this.$('.pattern-audio') as any);
    this._registerMediaEl(audio);
