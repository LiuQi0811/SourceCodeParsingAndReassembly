...
function truncate(str: any, max: any = 64) {
  if (typeof str !== 'string') return String(str);
  return str.length > max ? str.slice(0, max) + `…（共 ${str.length} 字符）` : str;
}

export interface WebCryptoDeepPageProps extends Props {}

export interface WebCryptoDeepPageState extends State {}

export class WebCryptoDeepPage extends Page {
  declare props: WebCryptoDeepPageProps;
  declare state: WebCryptoDeepPageState;
  _abortCtrl: any = null;
  _aesCipherBuf: any = null;
  _aesIv: any = null;
  _aesKey: any = null;
  _derivedKey: any = null;
  _ecdsaKeyPair: any = null;
  _ecdsaSig: any = null;
  _ecdsaSignedData: any = null;
  _inited: boolean = false;
  _lastJwk: any = null;
  _rsaKeyPair: any = null;
  _unwrappedKey: any = null;
  _wrapIv: any = null;
  _wrappedBuf: any = null;
  _wrappedKeyAlgo: any = null;
  _wrappedKeyUsages: any = null;
  _wrappingKey: any = null;
  // —— 初始 state ——
  initialState(): WebCryptoDeepPageState {
    return {
      logs: [],
      supported: false,
      capsSummary: '',
  ...
  async _aesEncrypt() {
    ...
    try {
      ...
      this._addLog('aes', `已加密 "Hello AES-GCM"：明文 ${data.length} 字节 → 密文 ${(cipherBuf as any).byteLength} 字节（含 16 字节 GCM auth tag）`);
  ...
  async _generateRsaKey() {
    ...
    try {
      ...
      this._addLog('rsa', `公钥加密 "Hello RSA-OAEP" → ${(cipherBuf as any).byteLength} 字节（= modulusLength/8 = 256）`);
      const plainBuf = await crypto.subtle.decrypt(
        { name: 'RSA-OAEP' },
        keyPair.privateKey,
        cipherBuf,
  ...
  async _ecdsaSign() {
    ...
    try {
      ...
      this._addLog('sign', `已用 ECDSA P-256 + SHA-256 对 "${text}" 签名：${(sigBuf as any).byteLength} 字节 / ${hex.length} hex 字符`);
    } catch (err: any) {
  ...
  async _ecdsaVerify(tampered: any = false) {
    if (!this._caps().subtle) {
    ...
    try {
      ...
      const sigBytes = new Uint8Array(
        this._ecdsaSig.match(/.{2}/g).map((h: any) => parseInt(h, 16)),
      );
  ...
  async _wrapKey() {
    ...
    try {
      ...
      this._addLog('wrap', `封装结果：${(wrapped as any).byteLength} 字节；IV=${bufToHex(iv)}`);
      ...
      children: [
        ...
        h('div', { class: 'table-wrap', style: { overflowX: 'auto' } },
          h('table', { class: 'data-table', style: { width: '100%', borderCollapse: 'collapse', fontSize: '12px' } },
            ...
            h('tbody', {},
              ...ALGO_CAPS_TABLE.map((r: any) => h('tr', {},
