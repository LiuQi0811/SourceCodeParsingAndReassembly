...
export interface RouterPageProps extends Props {
  router: Router;
}

export interface RouterPageState extends State {
  logs: LogEntry[];
  userId: string;
  queryKey: string;
  queryValue: string;
...
export class RouterPage extends Page {
  declare props: RouterPageProps;
  declare state: RouterPageState;
  _unsubBefore: (() => void) | null = null;
  _unsubAfter: (() => void) | null = null;
  _onPopState: ((e: PopStateEvent) => void) | null = null;

  initialState(): RouterPageState {
    return { logs: [], userId: '5', queryKey: 'tab', queryValue: 'profile' };
  }

  componentDidMount(): void {
    // 监听路由事件，写入日志面板
    this._unsubBefore = eventBus.on(EVENTS.ROUTER_BEFORE, ({ to, from }: { to: RouteLocation; from: RouteLocation | null }) => {
      this._addLog('guard', `beforeEach  ${from?.path || '∅'} → ${to.path}`);
    });
    this._unsubAfter = eventBus.on(EVENTS.ROUTER_AFTER, ({ to, from }: { to: RouteLocation; from: RouteLocation | null }) => {
  ...
  _makeBtn(label: string, opts: ButtonProps): Node | string {
            ...
            h('div', { class: 'card__body flex items-center gap-sm' },
              ...
              (() => {
                const input = new Input({
                  ...
                  onChange: (v: string) => { this.state.userId = v; },
                });
            ...
            h('div', { class: 'card__body flex flex-col gap-sm' },
              h('div', { class: 'flex items-center gap-sm' },
                ...
                h('input', { class: 'input input--sm', value: this.state.queryKey, style: { width: '120px' },
                  oninput: (e: Event) => { this.state.queryKey = (e.target as HTMLInputElement).value; } }),
                h('span', { class: 'fs-sm text-secondary' }, 'value'),
                h('input', { class: 'input input--sm', value: this.state.queryValue, style: { width: '120px' },
                  oninput: (e: Event) => { this.state.queryValue = (e.target as HTMLInputElement).value; } }),
              ),
