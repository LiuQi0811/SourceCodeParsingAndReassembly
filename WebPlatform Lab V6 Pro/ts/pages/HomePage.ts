...
export class HomePage extends Page {
  declare props: HomePageProps;
  declare state: HomePageState;
  _typeTimer: ReturnType<typeof setInterval> | null = null;
  _canvasRaf: number | null = null;
  _coverageObserver: IntersectionObserver | null = null;

  // typed 初始即完整文本：若 JS 禁用或打字机未启动，标题也能正常显示。
  // 打字机作为渐进增强：启动时清空再逐字打出。重渲染时 state.typed 保持完整文本，
  // 不会因 _loadArticles 的 setState 覆盖打字机已写的进度。
  initialState(): HomePageState { return { typed: TYPE_TEXT, articles: [], loading: true }; }
  ...
  _startTyping(): void {
    ...
    this._typeTimer = setInterval(() => {
      ...
      if (i >= TYPE_TEXT.length) clearInterval(this._typeTimer!);
    }, 80);
  }

  async _loadArticles(): Promise<void> {
    try {
  ...
  _animateCoverage(): void {
    // IntersectionObserver：仪表盘进入视口才动画
    const canvas = this.$<HTMLCanvasElement>('#coverage-canvas');
    if (!canvas) return;
    this._coverageObserver = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) this._drawDonut(canvas);
      });
    }, { threshold: 0.3 });
    this._coverageObserver.observe(canvas);
  }

  _drawDonut(canvas: HTMLCanvasElement): void {
    ...
    const animate = (now: number): void => {
      ...
      if (t < 1) this._canvasRaf = requestAnimationFrame(animate);
    };
    ...
    return [
      ...
      h('section', { class: 'home-hero' },
        h('div', { class: 'home-hero__content' },
          h('span', { class: 'home-hero__badge' }, '✦ 零依赖 · 纯原生 · OOP'),
          h('h1', { class: 'home-hero__title' },
            h('span', { id: 'typed-text' }, this.state.typed),
            h('span', { class: 'cursor' }),
          ),
          h('p', { class: 'home-hero__subtitle' },
            'WebPlatform Lab —— 借鉴 MDN Web 文档组织形式，用浏览器原生 API 自主实现路由系统、UI 组件库与状态管理，覆盖 178 个 Web API 深度页面。每一行代码都可被阅读、调试与学习。'),
      ...
      h('div', { class: 'feature-grid' },
        ...FEATURES.map((f) => {
          const card = new Card({
            title: h('div', { class: 'flex items-center gap-sm' },
              h('span', { class: 'feature-card__icon', style: { display: 'inline-grid' } }, f.icon),
              h('span', {}, f.title),
            ),
            hoverable: true,
            children: [
              h('p', { class: 'feature-card__desc' }, f.desc),
              h('div', { class: 'feature-card__meta' },
                ...f.tags.map((t) => h(Tag, { color: 'primary' }, t)),
              ),
            ],
          });
      ...
      h('div', { class: 'dashboard' },
        h('div', { class: 'dashboard__chart' },
          h('canvas', { id: 'coverage-canvas' }),
        ),
        h('div', { class: 'dashboard__list' },
          ...[
          ...
          ].map((item) => h('div', { class: 'dashboard__item' },
            h('span', {}, item.api),
            h('span', { class: `tag ${item.weight >= 100 ? 'tag--success' : 'tag--warning'}` }, `${item.weight}%`),
          )),
        ),
      ),
      ...
      h('div', { class: 'feature-grid' },
        this.state.loading
          ? [0, 1, 2].map(() => h('div', { class: 'card' },
              h('div', { class: 'card__body' }, h('div', { class: 'skeleton', style: { height: '120px' } })),
            ))
          : this.state.articles.map((a: any) => {
              const card = new Card({
                ...
                hoverable: true,
                children: [
                  h('div', { class: 'flex items-center gap-xs mb-sm' },
                    h(Tag, { color: 'primary' }, a.category),
                    h('span', { class: 'fs-sm text-tertiary' }, `${a.readTime} 分钟阅读`),
                  ),
