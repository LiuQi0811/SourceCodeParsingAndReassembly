...
export interface GeneralComponentsPageProps extends Props {}

export interface GeneralComponentsPageState extends State {
  spinLoading: boolean;
}

export class GeneralComponentsPage extends Page {
  declare props: GeneralComponentsPageProps;
  declare state: GeneralComponentsPageState;

  initialState(): GeneralComponentsPageState {
    return {
      spinLoading: true,
    };
  }
    ...
    const listCard = new Card({
      ...
      children: [
        h(List, {
          ...
          split: true,
          dataSource: listData,
          renderItem: (item: ListItemData) => h('div', {},
    ...
    const uploadCard = new Card({
      ...
      children: [
        h(Upload, {
          listType: 'text',
          maxCount: 5,
          onChange: (files: any[]) => { message.info(`当前 ${files.length} 个文件`); },
