...
export interface ComponentsIndexPageProps extends Props {
  router: Router;
}

export interface ComponentsIndexPageState extends State {}

export class ComponentsIndexPage extends Page {
  declare props: ComponentsIndexPageProps;
  declare state: ComponentsIndexPageState;

  renderPage(): Node | string | (Node | string)[] {
    return [
      h('h2', { class: 'section-title' }, '组件演示'),
      h('p', { class: 'text-secondary mb-lg' },
        '所有组件均以原生 CSS + ES Class 实现，无任何第三方依赖。体现 OOP 继承（Component 基类）、多态（type/size 变体）、组合（父子组件）。'),

      h('div', { class: 'feature-grid' },
        ...CATEGORIES.map((cat) => {
          const card = new Card({
            title: h('div', { class: 'flex items-center gap-sm' },
              h('span', { class: 'feature-card__icon' }, cat.icon),
              h('span', {}, cat.title),
            ),
            hoverable: true,
            children: [
              ...
              h('div', { class: 'feature-card__meta' },
                ...cat.tags.map((t) => h(Tag, { color: 'primary' }, t)),
              ),
            ],
          });
