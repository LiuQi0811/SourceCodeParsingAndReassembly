...
export interface DataComponentsPageProps extends Props {
  router: Router;
}

export interface DataComponentsPageState extends State {
  users: any[];
  loading: boolean;
}

export class DataComponentsPage extends Page {
  declare props: DataComponentsPageProps;
  declare state: DataComponentsPageState;

  initialState(): DataComponentsPageState { return { users: [], loading: true }; }
  ...
  _renderTable(owner: Component): Node | string {
    ...
    const columns = [
      ...
      { title: '角色', dataIndex: 'role', key: 'role',
        render: (v: any) => h(Tag, { color: 'primary' }, v) },
      { title: '年龄', dataIndex: 'age', key: 'age', sorter: (a: any, b: any) => a.age - b.age },
      { title: '城市', dataIndex: 'city', key: 'city' },
      { title: '状态', dataIndex: 'status', key: 'status',
        render: (v: any) => h(Tag, { color: v === 'active' ? 'success' : 'warning' }, v === 'active' ? '在线' : '空闲') },
    ];

    const table = new Table({
      columns, dataSource: this.state.users, pageSize: 5,
      onRowClick: (row: any) => this.props.router.push(`/router/user/${row.id}`),
    });
    owner.registerChild(table);
    return table.render();
  }

  _renderCards(owner: Component): Node | string {
    if (this.state.loading) return h(Skeleton, { lines: 4 });
    return h('div', { class: 'feature-grid' },
      ...this.state.users.slice(0, 6).map((u: any) => {
        const card = new Card({
  ...
  renderPage(): Node | string | (Node | string)[] {
    ...
    return [
      ...
      h(Card, { title: 'Tree 树形控件' },
        ...
        h('div', { class: 'demo-row' }, h('span', { class: 'demo-row__label' }, '可勾选'),
          h(Tree, {
            checkable: true,
            defaultExpandAll: true,
            treeData: [
      ...
      h(Card, { title: 'Carousel 轮播' },
        ...
        h('div', { class: 'mt-md' }, (() => { const c = new Carousel({
          effect: 'fade',
          children: [
      ...
      // Calendar
      h(Card, { title: 'Calendar 日历' },
        h(Calendar, {}),
      ),

      // Descriptions
      h(Card, { title: 'Descriptions 描述列表' },
        h(Descriptions, {
          title: '用户信息',
          column: 2,
          border: true,
      ...
      // QRCode
      h(Card, { title: 'QRCode 二维码' },
        h('div', { class: 'demo-row' }, h('span', { class: 'demo-row__label' }, '基本'),
          h(QRCode, { value: 'https://example.com', size: 128 }),
          h(QRCode, { value: 'Hello World', size: 128, color: '#1677ff' }),
          h(QRCode, { value: 'https://example.com', size: 128, type: 'svg' }),
        ),
      ),

      // Watermark
      h(Card, { title: 'Watermark 水印' },
        h(Watermark, {
          content: 'Demo Watermark',
          gap: [80, 80],
        }, h('div', { style: { height: '160px', background: '#fafafa', padding: '16px' } },
          h('p', {}, '这段内容上方覆盖了水印文字。'),
          h('p', { class: 'text-secondary' }, '水印由 canvas 生成，平铺在内容之上但不阻挡交互。'),
        )),
      ),
    ];
  }
