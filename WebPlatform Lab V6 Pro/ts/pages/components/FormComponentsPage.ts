...
export interface FormComponentsPageProps extends Props {}

export interface FormComponentsPageState extends State {
  name: string;
  email: string;
  city: string | null;
  gender: string;
...
export class FormComponentsPage extends Page {
  declare props: FormComponentsPageProps;
  declare state: FormComponentsPageState;

  initialState(): FormComponentsPageState {
    return {
  ...
  _validate(): Record<string, string> {
  ...
  renderPage(): Node | string | (Node | string)[] {
    ...
    const citySelect = new Select({
      ...
      onChange: (v: string) => { this.state.city = v; },
    });
      ...
      h(Card, { title: 'InputNumber 数字输入框' },
        h('div', { style: 'display:flex; gap:16px; flex-wrap:wrap; align-items:center;' },
          (() => { const c = new InputNumber({ min: 0, max: 100, defaultValue: 5, size: 'middle',
            onChange: (v: number) => message.info(`值变更：${v}`) }); this.registerChild(c); return c.render(); })(),
          ...
          (() => { const c = new InputNumber({ min: 0, max: 1000, defaultValue: 100, step: 10,
            ...
            parser: (s: string) => s.replace(/¥\s?|(,*)/g, '') }); this.registerChild(c); return c.render(); })(),
        ),
