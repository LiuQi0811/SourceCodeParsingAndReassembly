...
import type { IconStatic } from '../../components/ui/Icon.js';
...
export interface NavigationComponentsPageProps extends Props {
  router: Router;
}

export interface NavigationComponentsPageState extends State {
  page: number;
  pageSize: number;
  currentStep: number;
  rateValue: number;
...
export class NavigationComponentsPage extends Page {
  declare props: NavigationComponentsPageProps;
  declare state: NavigationComponentsPageState;

  initialState(): NavigationComponentsPageState {
    return {
      page: 1,
      pageSize: 10,
      currentStep: 1,
  ...
  renderPage(): Node | string | (Node | string)[] {
    ...
    const collapseCard = new Card({
      ...
      children: [
        h(Collapse, {
          ...
          onChange: (keys: string[]) => this.setState({ collapseActive: keys }),
          items: [
    ...
    const paginationCard = new Card({
      ...
      children: [
        h(Pagination, {
          ...
          onChange: (page: number) => { this.setState({ page }); message.info(`跳转到第 ${page} 页`); },
        }),
    ...
    const rateCard = new Card({
      ...
      children: [
        h('div', { class: 'demo-row' },
          ...
          h(Rate, { value: this.state.rateValue, allowHalf: true, onChange: (v: number) => { this.setState({ rateValue: v }); message.info(`评分：${v} 星`) } }),
          h('span', { class: 'ml-sm text-secondary' }, `${this.state.rateValue} 星`),
        ),
        h('div', { class: 'demo-row' },
    ...
    const sliderCard = new Card({
      ...
      children: [
        h('div', { class: 'mb-md' },
          h(Slider, { value: this.state.sliderValue, min: 0, max: 100, onChange: (v: number) => this.setState({ sliderValue: v }) }),
        ),
        h('div', { class: 'mb-md' },
          h(Slider, { value: this.state.sliderValue, showInput: true, onChange: (v: number) => this.setState({ sliderValue: v }) }),
        ),
      ...
      children: [
        h('div', { class: 'demo-row' },
          h(Segmented, {
            ...
            onChange: (v: string) => this.setState({ segmentedValue: v }),
            options: [
              { value: 'a', label: '选项 A' },
              { value: 'b', label: '选项 B' },
              { value: 'c', label: '选项 C' },
      ...
      children: [
        ...
        (() => {
          const affix = new Affix({
            offsetTop: 0,
            onChange: (fixed: boolean) => message.info(fixed ? '已固定' : '已取消固定'),
    ...
    const menuCard = new Card({
      ...
      children: [
        ...
        (() => { const c = new Menu({
          ...
          onSelect: ({ key, keyPath }: { key: string; keyPath: string[] }) => message.info(`Menu 选中：${key}，keyPath: ${JSON.stringify(keyPath)}`),
        ...
        (() => { const c = new Menu({
          ...
          onSelect: ({ key }: { key: string }) => message.info(`Menu 选中：${key}`),
        }); this.registerChild(c); return c.render(); })(),
      ],
    });
    ...
    const iconCard = new Card({
      ...
      children: [
        h('p', { class: 'text-secondary' }, `内置 ${(Icon as unknown as IconStatic).names.length} 个常用图标，支持 size / color / spin / rotate。`),
        h('div', { style: 'display:flex; flex-wrap:wrap; gap:24px; font-size:24px; margin-top:12px;' },
          ...(Icon as unknown as IconStatic).names.map((name: string) => h('div', {
