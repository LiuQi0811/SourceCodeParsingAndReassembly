// Comprehensive fixup v5 - combines caps interface regeneration with safe line-by-line transforms.
...
function extractPairsFromObject(src, objOpenIdx) {
  ...
  for (const seg of segments) {
    ...
    if (m) { pairs.push({ key: m[1], type: inferType(m[2]) }); continue; }
    // shorthand: just key
...
function fix(name) {
  ...
  // === 1) Regenerate caps interface from _caps()/_flags() return value ===
  ...
  // === 2) Fix polymorphic `_caps(items): Node[]` with overloads ===
  ...
  // === 4) Cast `document.getElementById(...)` to `any` (any arg, not just string literals) ===
