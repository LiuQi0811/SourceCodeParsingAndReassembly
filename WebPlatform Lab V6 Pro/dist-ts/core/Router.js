...
export class Router {
    ...
    pushByName(name, params = {}, query = {}) {
        ...
        if (!route)
            throw new Error(`[Router] 路由名 "${name}" 不存在`);
        let path = route.path;
    ...
    async _navigate(rawPath, { type = 'push', state = null } = {}) {
        ...
        if (this._currentInstance && typeof this._currentInstance.onRouteLeave === 'function') {
        ...
        // 写入 history
        if (type === 'push')
            this.strategy.push(rawPath, state);
        else if (type === 'replace')
            this.strategy.replace(rawPath, state);
        // 切换页面组件
        ...
        // 设置文档标题
        if (route.meta?.title)
            document.title = `${route.meta.title} · 原生 SPA`;
    }
    async _renderPage(route, { to, from, params, query }) {
        ...
        const cleanupPrev = () => {
            ...
            try {
                ...
                console.error('[Router] 旧页 destroy 失败', err);
            }
        };
        ...
        // 实例化新页面并立即挂载
        const pageProps = { router: this, route, params, query, to, from };
        const instance = new TargetComponent(pageProps);
        instance.mount(outlet);
        this._currentInstance = instance;
        // 调用页面级生命周期
        if (typeof instance.onRouteEnter === 'function') {
            try {
                await instance.onRouteEnter(params, query);
            }
            catch (err) {
                console.error('[Router] onRouteEnter error:', err);
            }
        }
        // 滚动到顶部
