...
export class WebAnimationsPage extends Page {
    _animCancelHandler = null;
    _animFinishHandler = null;
    _cssAnim = null;
    _inited = false;
    _injectedStyleEl = null;
    _mainAnim = null;
    _multiAnims = null;
    _rafId = null;
    _waveAnims = null;
    ...
    componentDidMount() {
        ...
        if (this._mainAnim === undefined)
            this._mainAnim = null; // Card 2 主 Animation 句柄
        if (this._waveAnims === undefined)
            this._waveAnims = []; // Card 3 wave / composite 动画集合
        if (this._multiAnims === undefined)
            this._multiAnims = []; // Card 4 多动画集合
        if (this._cssAnim === undefined)
            this._cssAnim = null; // Card 5 CSS 动画引用
        if (this._rafId === undefined)
            this._rafId = null; // Card 2 requestAnimationFrame 句柄
        if (this._injectedStyleEl === undefined)
            this._injectedStyleEl = null; // Card 5 注入的 <style> 元素
        if (this._animFinishHandler === undefined)
            this._animFinishHandler = null; // Card 2 finish 处理器
        if (this._animCancelHandler === undefined)
            this._animCancelHandler = null; // Card 2 cancel 处理器
        ...
        const summary = '能力检测：element.animate=' + (animSupported ? '✓' : '✗') +
    ...
    _card(title, desc, extra, children) {
        const kids = [];
        if (desc)
            kids.push(h('p', { class: 'fs-sm text-secondary' }, desc));
    ...
    _playFadeMove() {
        ...
        if (!el) {
            this._addLog('warn', '未找到演示元素 #wa-box-1');
            return;
        }
        try {
    ...
    _playBounce() {
        ...
        try {
            const anim = el.animate([{ transform: 'translateY(0)' }, { transform: 'translateY(-60px)' }], { duration: 600, iterations: Infinity, direction: 'alternate-reverse', easing: 'ease-in-out' });
    ...
    _renderCard1() {
        return this._card('Card 1 · element.animate() 基础 + 关键帧', 'element.animate(keyframes, options) 返回 Animation。keyframes 可为数组形式 ' +
            ...
            'iterationStart、playbackRate。', h(Tag, { color: 'primary' }, 'animate()'), [
            h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('播放淡入+位移（cubic-bezier）', { type: 'primary', size: 'sm', onClick: () => this._playFadeMove() }), this._btn('alternate-reverse 无限弹跳', { size: 'sm', onClick: () => this._playBounce() }), this._btn('fill:forwards 保持终态', { size: 'sm', onClick: () => this._playFillForwards() })),
            this._demoBox('wa-box-1', undefined),
            h('pre', { class: 'code-block mt-md' }, `// 数组形式关键帧 + cubic-bezier 回弹缓动
el.animate(
...
el.animate([...], { duration: 1000, fill: 'forwards' });`),
    ...
    _ensureMainAnim() {
        ...
        if (!el) {
            this._addLog('warn', '未找到演示元素 #wa-box-2');
            return null;
        }
        try {
            const anim = el.animate([
    ...
    _detachMainAnimListeners() {
        ...
        try {
            if (anim && typeof anim.removeEventListener === 'function') {
                if (this._animFinishHandler)
                    anim.removeEventListener('finish', this._animFinishHandler);
                if (this._animCancelHandler)
                    anim.removeEventListener('cancel', this._animCancelHandler);
            }
        }
        catch { /* noop */ }
    ...
    _syncMainState() {
        const anim = this._mainAnim;
        if (!anim) {
            this.setState({ animState: 'idle', currentTime: null });
            return;
        }
        let st = 'idle';
        let ct = null;
        try {
            st = anim.playState || 'idle';
        }
        catch { /* noop */ }
        try {
            ct = anim.currentTime;
    ...
    _pauseMain() {
        const anim = this._mainAnim;
        if (!anim) {
            this._addLog('warn', '主动画尚未创建，请先 play');
            return;
        }
        try {
            anim.pause();
            this._addLog('anim', 'pause()：已暂停');
            this._syncMainState();
        }
        catch (err) {
            this._addLog('warn', 'pause 失败：' + (err && err.message));
        }
    }
    _reverseMain() {
        const anim = this._mainAnim;
        ...
        try {
            ...
            this._addLog('warn', 'reverse 失败：' + (err && err.message));
        }
    }
    _finishMain() {
        const anim = this._mainAnim;
        ...
        try {
            anim.finish();
            this._addLog('anim', 'finish()：跳到动画终点');
            this._syncMainState();
        }
        catch (err) {
            this._addLog('warn', 'finish 失败：' + (err && err.message));
        }
    }
    _cancelMain() {
        const anim = this._mainAnim;
        if (!anim) {
            this._addLog('warn', '主动画尚未创建');
            return;
        }
        try {
            anim.cancel();
            this._addLog('anim', 'cancel()：已取消并清除效果');
            this._syncMainState();
        }
        catch (err) {
            this._addLog('warn', 'cancel 失败：' + (err && err.message));
        }
    }
    _commitStylesMain() {
        const anim = this._mainAnim;
        ...
        try {
            ...
            this._addLog('warn', 'commitStyles 失败：' + (err && err.message));
        }
    }
    _renderCard2() {
        const s = this.state;
        const ct = s.currentTime;
        return this._card('Card 2 · Animation 对象控制（play/pause/reverse/cancel/finish/commitStyles）', 'const anim = element.animate(...) 返回 Animation 对象。属性：currentTime、playbackRate、' +
            'playState(\'idle\'|\'running\'|\'paused\'|\'finished\')、startTime、effect（KeyframeEffect）、timeline；' +
            '方法：play/pause/reverse/cancel/finish/commitStyles/persist；事件：finish、cancel、remove。', h(Tag, { color: s.animState === 'running' ? 'success' : (s.animState === 'paused' ? 'warning' : 'default') }, 'playState: ' + s.animState), [
            h('div', { class: 'flex items-center gap-sm flex-wrap mt-sm' }, h(Tag, { color: 'primary' }, 'currentTime: ' + (ct != null ? Number(ct).toFixed(1) + ' ms' : '—')), h(Tag, { color: 'default' }, '主动画：' + (this._mainAnim ? '已创建' : '未创建'))),
            h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('play', { type: 'primary', size: 'sm', onClick: () => this._playMain() }), this._btn('pause', { size: 'sm', onClick: () => this._pauseMain() }), this._btn('reverse', { size: 'sm', onClick: () => this._reverseMain() }), this._btn('finish（跳到终点）', { size: 'sm', onClick: () => this._finishMain() }), this._btn('cancel', { size: 'sm', danger: true, onClick: () => this._cancelMain() }), this._btn('commitStyles 后 cancel', { size: 'sm', onClick: () => this._commitStylesMain() })),
            this._demoBox('wa-box-2', undefined),
            h('pre', { class: 'code-block mt-md' }, `const anim = el.animate(keyframes, { duration: 6000, iterations: 2 });
...
anim.addEventListener('cancel', (e: any) => { /* ... */ });`),
    ...
    _createWave() {
        ...
        const els = ids.map((id) => this._el(id));
        if (els.some((e) => !e)) {
            this._addLog('warn', '未找到演示元素（wa-box-3a/3b/3c）');
            return;
        }
        ...
        try {
            ...
            let timeline = null;
            try {
                timeline = typeof document !== 'undefined' ? document.timeline : null;
            }
            catch {
                timeline = null;
            }
            els.forEach((el, i) => {
                try {
                    const effect = new KeyframeEffect(el, keyframes, { ...opts, delay: i * 200 });
    ...
    _createWaveFallback() {
        ...
        els.forEach((el, i) => {
            try {
                const anim = el.animate([{ transform: 'translateY(0)' }, { transform: 'translateY(-40px)' }, { transform: 'translateY(0)' }], { duration: 900, delay: i * 200, iterations: Infinity, easing: 'ease-in-out' });
                this._waveAnims.push(anim);
            }
            catch (err) {
    ...
    _compositeRotate() {
        ...
        if (!el) {
            this._addLog('warn', '未找到演示元素 #wa-box-3a');
            return;
        }
        try {
            // 基础位移动画（replace，fill:forwards 保留）
            const base = el.animate([{ transform: 'translateX(0)' }, { transform: 'translateX(60px)' }], { duration: 1500, fill: 'forwards', easing: 'ease-out', composite: 'replace' });
            this._waveAnims.push(base);
            this._addLog('kfe', '基础位移动画（composite:replace，fill:forwards）');
            // 叠加旋转动画（composite:add，与位移 transform 累加）
            const rot = el.animate([{ transform: 'rotate(0deg)' }, { transform: 'rotate(360deg)' }], { duration: 1500, iterations: Infinity, easing: 'linear', composite: 'add' });
    ...
    _renderCard3() {
        return this._card('Card 3 · KeyframeEffect + 序列动画（wave / composite）', 'new KeyframeEffect(target, keyframes, options) 独立创建关键帧效果；new Animation(effect, timeline) ' +
            '用效果创建动画。CompositeOperation: \'replace\' | \'add\' | \'accumulate\'（多动画叠加）；' +
            'iterationComposite: \'replace\' | \'accumulate\'（每次迭代累加）。多元素用 delay 错开形成序列。', h(Tag, { color: 'success' }, 'KeyframeEffect'), [
            h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('创建 wave 序列动画', { type: 'primary', size: 'sm', onClick: () => this._createWave() }), this._btn('composite:add 叠加旋转', { size: 'sm', onClick: () => this._compositeRotate() }), this._btn('停止 wave / composite', {
                size: 'sm', danger: true, onClick: () => {
                    this._cancelWaveAnims();
                    this._addLog('wave', '已停止所有 wave / composite 动画');
                },
            })),
            h('div', { class: 'flex items-center mt-md', style: { gap: '24px' } }, this._demoBox('wa-box-3a', undefined), this._demoBox('wa-box-3b', { background: 'var(--color-success, #52c41a)' }), this._demoBox('wa-box-3c', { background: 'var(--color-warning, #faad14)' })),
            h('pre', { class: 'code-block mt-md' }, `const effect = new KeyframeEffect(el, keyframes, {
...
el.animate([...], { iterationComposite: 'accumulate' });`),
    ...
    _applyThreeAnims() {
        ...
        if (!el) {
            this._addLog('warn', '未找到演示元素 #wa-box-4');
            return;
        }
        this._cancelMultiAnims();
        try {
            const a1 = el.animate([{ opacity: 1 }, { opacity: 0.3 }, { opacity: 1 }], { duration: 1500, iterations: Infinity, easing: 'ease-in-out' });
            const a2 = el.animate([{ transform: 'scale(1)' }, { transform: 'scale(1.3)' }, { transform: 'scale(1)' }], { duration: 2000, iterations: Infinity, easing: 'ease-in-out' });
            const a3 = el.animate([{ backgroundColor: 'rgb(22,119,255)' }, { backgroundColor: 'rgb(255,77,79)' }, { backgroundColor: 'rgb(22,119,255)' }], { duration: 2500, iterations: Infinity, easing: 'linear' });
    ...
    _listElementAnims(el) {
        ...
        try {
            ...
            const list = anims.map((a, i) => {
                ...
                try {
                    composite = (a.effect && a.effect.composite) || '—';
                }
                catch { /* noop */ }
    ...
    _cancelMultiAnims() {
        try {
            if (Array.isArray(this._multiAnims)) {
                this._multiAnims.forEach((a) => { try {
                    a && a.cancel && a.cancel();
                }
                catch { /* noop */ } });
            }
    ...
    _renderAnimList() {
        ...
        const hasTarget = list.some((x) => x.target != null);
        const rows = list.map((x) => h('tr', {}, h('td', { style: TD }, String(x.idx)), h('td', { style: TD }, x.playState), h('td', { style: TD }, x.currentTime != null ? Number(x.currentTime).toFixed(1) + ' ms' : '—'), h('td', { style: TD }, hasTarget ? (x.target || '—') : (x.composite || '—'))));
        return h('table', { style: { width: '100%', borderCollapse: 'collapse', marginTop: '8px' } }, h('thead', {}, h('tr', {}, h('th', { style: TH }, '#'), h('th', { style: TH }, 'playState'), h('th', { style: TH }, 'currentTime'), h('th', { style: TH }, hasTarget ? 'target' : 'composite'))), h('tbody', {}, ...rows));
    }
    _renderCard4() {
        const s = this.state;
        return this._card('Card 4 · getAnimations() + 动画监听', 'element.getAnimations() 返回当前作用于元素的 Animation[]；document.getAnimations() 返回页面所有动画。' +
            'Animation.persist() 防止 fill:forwards 动画被自动回收；可监听 finish/cancel/remove 事件。', h(Tag, { color: 'primary' }, '动画数: ' + s.animCount), [
            h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('施加 3 个动画 → getAnimations 列出', { type: 'primary', size: 'sm', onClick: () => this._applyThreeAnims() }), this._btn('persist 一个动画', { size: 'sm', onClick: () => this._persistAnim() }), this._btn('document.getAnimations 全局列表', { size: 'sm', onClick: () => this._listDocAnims() }), this._btn('清理多动画', {
                size: 'sm', danger: true, onClick: () => {
                    this._cancelMultiAnims();
            ...
            h('pre', { class: 'code-block mt-md' }, `el.getAnimations();        // 作用于 el 的所有 Animation[]
...
anim.addEventListener('remove', (e: any) => { /* 自动移除时触发 */ });`),
    ...
    _controlCssAnim(method, okStatus, errLabel) {
        const el = this._el('wa-box-5');
        if (!el) {
            this._addLog('warn', '未找到演示元素 #wa-box-5');
            return;
        }
        if (typeof el.getAnimations !== 'function') {
    ...
    _renderCard5() {
        ...
        return this._card('Card 5 · DocumentTimeline + 自定义 timeline + 与 CSS 动画互操作', 'document.timeline 为默认时间轴；new DocumentTimeline({ originTime }) 自定义时间起点；' +
            'animation.timeline = customTimeline 切换时间轴。CSS @keyframes 定义的动画也出现在 getAnimations() 中，' +
            '可被 JS pause()/reverse() 控制——WAAPI 与 CSS 动画共享同一引擎。', h(Tag, { color: hasTimeline ? 'success' : 'warning' }, s.cssAnimStatus), [
            h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, h(Tag, { color: 'primary' }, 'DocumentTimeline: ' + (hasTimeline ? '✓' : '✗'))),
            h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('注入 CSS @keyframes spin 动画', { type: 'primary', size: 'sm', onClick: () => this._injectCssSpin() }), this._btn('JS 拦截并 pause CSS 动画', { size: 'sm', onClick: () => this._pauseCssAnim() }), this._btn('reverse CSS 动画', { size: 'sm', onClick: () => this._reverseCssAnim() })),
            this._demoBox('wa-box-5', { background: 'var(--color-success, #52c41a)' }),
            h('pre', { class: 'code-block mt-md' }, `// 默认时间轴
document.timeline;
// 自定义时间起点
...
cssAnim.reverse();  // JS 反向 CSS 动画`),
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'page api-lab-page' }, h('h2', { class: 'section-title' }, 'Web Animations 动画实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, '演示 Web Animations API（WAAPI）：element.animate 关键帧 / Animation 控制 / ' +
            'KeyframeEffect + 序列动画 / getAnimations / DocumentTimeline 与 CSS 互操作。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderLogPanel());
    }
}
//# sourceMappingURL=WebAnimationsPage.js.map