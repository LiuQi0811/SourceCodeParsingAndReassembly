...
export class SpeechCodecsPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!hasSS)
            this._addLog('warn', 'speechSynthesis 不可用（jsdom/Node 不支持，需真实浏览器）');
        if (!hasSR)
            this._addLog('warn', 'SpeechRecognition 不可用（仅 Chrome/Edge 支持，需 HTTPS 或 localhost）');
        if (!hasVE || !hasVD)
            this._addLog('warn', 'VideoEncoder/Decoder 不可用（WebCodecs 仅浏览器支持，需 HTTPS）');
        if (!hasID)
    ...
    _speak() {
        ...
        try {
            ...
            this.setState({
                ttsInfo: `new SpeechSynthesisUtterance("${text}")\n` +
    ...
    _cancelSpeaking() {
        ...
        try {
            ...
            this._utterance = null;
            this.setState({
                ttsInfo: `speechSynthesis.cancel()\n` +
                    `取消前：${before}\n` +
                    `取消后：${after}\n` +
    ...
    _resumeSpeaking() {
        ...
        try {
            ...
            this.setState({
                ttsInfo: `speechSynthesis.resume()\n` +
                    `paused = ${speechSynthesis.paused}\n` +
                    `说明：resume() 恢复已暂停的播报。`,
            });
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.speechSynthesis ? 'success' : 'error' }, caps.speechSynthesis ? 'speechSynthesis ✓' : '不可用'), h(Tag, { color: 'primary' }, 'speak / cancel / pause')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'speechSynthesis.speak(utterance) 异步播报文本；cancel() 立即停止并清空队列；pause() / resume() 暂停与恢复。SpeechSynthesisUtterance 字段：text / lang / rate（0.1-10）/ pitch（0-2）/ volume（0-1）/ voice（来自 getVoices()）。事件：start / end / pause / resume / mark / boundary / error。speechSynthesis.speaking / paused / pending 反映当前队列状态。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('speak', { type: 'primary', size: 'sm', disabled: !caps.speechSynthesis || !caps.utterance, onClick: () => this._speak() }), this._btn('cancel', { danger: true, size: 'sm', disabled: !caps.speechSynthesis, onClick: () => this._cancelSpeaking() }), this._btn('pause', { size: 'sm', disabled: !caps.speechSynthesis, onClick: () => this._pauseSpeaking() }), this._btn('resume', { size: 'sm', disabled: !caps.speechSynthesis, onClick: () => this._resumeSpeaking() })),
                h('div', { class: 'fs-sm text-secondary' }, 'TTS 状态：'),
                h('pre', { class: 'code-block', style: { maxHeight: '200px', overflow: 'auto' } }, h('code', {}, s.ttsInfo || '（点击 speak 开始播报）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '160px', overflow: 'auto' } }, h('code', {}, `const u = new SpeechSynthesisUtterance('你好');
...
speechSynthesis.cancel();            // 立即停止并清空队列`)),
    ...
    _loadVoices() {
        ...
        try {
            const renderList = (voices) => {
                const lines = voices.map((v, i) => `[${i}] name="${v.name}" lang="${v.lang}" default=${v.default} localService=${v.localService} voiceURI="${v.voiceURI}"`);
                this.setState({
                    voicesInfo: `speechSynthesis.getVoices() → SpeechSynthesisVoice[${voices.length}]\n` +
                        `${lines.join('\n')}\n\n` +
    ...
    _pickDefaultVoice() {
        ...
        try {
            ...
            speechSynthesis.speak(u);
            this.setState({
                voicesInfo: `已选取 default voice 并绑定到 utterance.voice：\n` +
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.voice ? 'success' : 'error' }, caps.voice ? 'Voice ✓' : '不可用'), h(Tag, { color: 'primary' }, 'voiceschanged')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'speechSynthesis.getVoices() 返回 SpeechSynthesisVoice[]。voice 字段：name（语音名）/ lang（BCP 47 标签，如 zh-CN、en-US）/ voiceURI（唯一标识）/ default（是否默认 voice）/ localService（本地引擎为 true，云端为 false）。voices 异步加载，首次调用可能返回空数组，需监听 voiceschanged 事件后再取。可把 voice 赋给 utterance.voice 切换发音人。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('加载 voices', { type: 'primary', size: 'sm', disabled: !caps.speechSynthesis, onClick: () => this._loadVoices() }), this._btn('选默认 voice 并 speak', { size: 'sm', disabled: !caps.speechSynthesis || !caps.utterance, onClick: () => this._pickDefaultVoice() })),
                h('div', { class: 'fs-sm text-secondary' }, 'voice 列表：'),
                h('pre', { class: 'code-block', style: { maxHeight: '240px', overflow: 'auto' } }, h('code', {}, s.voicesInfo || '（点击「加载 voices」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '140px', overflow: 'auto' } }, h('code', {}, `function loadVoices() {
  const voices = speechSynthesis.getVoices();
  if (voices.length === 0) {                 // 首次返回空
    speechSynthesis.onvoiceschanged = loadVoices;  // 异步加载
    return;
  }
  const zh = voices.find((v: any) => v.lang.startsWith('zh'));
...
loadVoices();`)),
        ...
        try {
            ...
            r.onresult = (e) => {
                ...
                for (let i = 0; i < resultList.length; i++)
                    fullText += resultList[i][0].transcript;
                this.setState({
                    recognitionInfo: `event.results = SpeechRecognitionResultList（length=${resultList.length}）\n` +
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.recognition ? 'success' : 'error' }, caps.recognition ? 'Recognition ✓' : '不可用'), h(Tag, { color: 'primary' }, 'start / stop / abort')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'new SpeechRecognition()（或 webkitSpeechRecognition）创建识别器。lang 设置识别语言（BCP 47）；continuous=true 连续识别；interimResults=true 返回中间结果；maxAlternatives 限制每条候选数。start() 开始、stop() 请求停止（仍触发 onresult）、abort() 立即中止。事件：audiostart / audioend / speechstart / speechend / result / nomatch / error / end。event.results 是 SpeechRecognitionResultList，每项含 transcript / confidence / isFinal。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('开始识别', { type: 'primary', size: 'sm', disabled: !caps.recognition, onClick: () => this._startRecognition() }), this._btn('stop', { size: 'sm', disabled: !caps.recognition, onClick: () => this._stopRecognition() }), this._btn('abort', { danger: true, size: 'sm', disabled: !caps.recognition, onClick: () => this._abortRecognition() })),
                h('div', { class: 'fs-sm text-secondary' }, '识别结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '220px', overflow: 'auto' } }, h('code', {}, s.recognitionInfo || '（点击「开始识别」并允许麦克风）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '160px', overflow: 'auto' } }, h('code', {}, `const SR = window.SpeechRecognition || window.webkitSpeechRecognition;
const r = new SR();
...
r.onerror = (e: any) => console.error(e.error);
r.start();   // 需 HTTPS 或 localhost + 麦克风授权`)),
    ...
    _createVideoEncoder() {
        ...
        try {
            ...
            this.setState({
                videoEncoderInfo: `new VideoEncoder({ output(chunk, metadata), error(e) })\n` +
    ...
    async _encodeMockFrame() {
        ...
        try {
            ...
            const chunkText = chunk
                ...
                : '（无 chunk 产出）';
            this.setState({
                videoEncoderInfo: `new VideoFrame(canvas, { timestamp: 0, duration: 33333 })\n` +
    ...
    _resetVideoEncoder() {
        ...
        try {
            ...
            this._encodedChunks = [];
            this.setState({
                videoEncoderInfo: `encoder.reset() ✓\n` +
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.videoEncoder ? 'success' : 'error' }, caps.videoEncoder ? 'VideoEncoder ✓' : '不可用'), h(Tag, { color: caps.encodedVideoChunk ? 'primary' : 'warning' }, caps.encodedVideoChunk ? 'EncodedChunk ✓' : 'Chunk ✗')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'new VideoEncoder({ output(chunk, metadata), error(e) }) 创建视频编码器；encoder.configure({ codec, width, height, bitrate, framerate }) 配置；encoder.encode(frame, { keyFrame }) 编码 VideoFrame；await encoder.flush() 等待所有 output；encoder.reset() 清空队列；encoder.close() 释放；encoder.encodeQueueSize 当前队列长度。output 回调收到 EncodedVideoChunk（type ∈ key/delta，含 timestamp/duration/byteLength/copyTo）+ metadata（含 decoderConfig）。VideoFrame 来源：canvas / ImageBitmap / 另一个 VideoFrame。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('创建 VideoEncoder', { type: 'primary', size: 'sm', disabled: !caps.videoEncoder, onClick: () => this._createVideoEncoder() }), this._btn('encode mock frame', { type: 'primary', size: 'sm', disabled: !caps.videoEncoder || !caps.videoFrame, onClick: () => this._encodeMockFrame() }), this._btn('reset', { size: 'sm', disabled: !caps.videoEncoder, onClick: () => this._resetVideoEncoder() })),
                h('div', { class: 'fs-sm text-secondary' }, '编码结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '220px', overflow: 'auto' } }, h('code', {}, s.videoEncoderInfo || '（点击「创建 VideoEncoder」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '180px', overflow: 'auto' } }, h('code', {}, `const encoder = new VideoEncoder({
  output: (chunk: any, meta: any) => { /* chunk: EncodedVideoChunk */ },
...
encoder.close();         // 彻底释放`)),
    ...
    _createVideoDecoder() {
        ...
        try {
            ...
            this.setState({
                videoDecoderInfo: `new VideoDecoder({ output(frame), error(e) })\n` +
                    `decoder.configure({ codec: "vp8", codedWidth: 320, codedHeight: 240 }) ✓\n` +
    ...
    async _decodeMockChunk() {
        ...
        try {
            ...
            this.setState({
                videoDecoderInfo: `decoder.decode(chunk) ✓\n` +
                    `${chunkDesc}\n` +
                    `await decoder.flush() ✓\n` +
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.videoDecoder ? 'success' : 'error' }, caps.videoDecoder ? 'VideoDecoder ✓' : '不可用'), h(Tag, { color: 'primary' }, 'decode / flush')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'new VideoDecoder({ output(frame), error(e) }) 创建视频解码器；decoder.configure({ codec, codedWidth, codedHeight, description? }) 配置；decoder.decode(chunk) 把 EncodedVideoChunk 入队解码；await decoder.flush() 等待所有 output；decoder.reset() 清空队列；decoder.close() 释放。output 回调收到解码后的 VideoFrame（必须 close()）。AudioEncoder / AudioDecoder / EncodedAudioChunk / AudioData 用法完全同构，仅 codec 不同（如 mp4a.40.2 / opus）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('创建 VideoDecoder', { type: 'primary', size: 'sm', disabled: !caps.videoDecoder, onClick: () => this._createVideoDecoder() }), this._btn('decode mock chunk', { type: 'primary', size: 'sm', disabled: !caps.videoDecoder || !caps.encodedVideoChunk, onClick: () => this._decodeMockChunk() })),
                h('div', { class: 'fs-sm text-secondary' }, '解码结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '220px', overflow: 'auto' } }, h('code', {}, s.videoDecoderInfo || '（点击「创建 VideoDecoder」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法（含 AudioDecoder 同构）：'),
                h('pre', { class: 'code-block', style: { maxHeight: '180px', overflow: 'auto' } }, h('code', {}, `const decoder = new VideoDecoder({
  output: (frame: any) => { frame.close(); },   // VideoFrame 用完必须 close()
  error: (e: any) => console.error(e),
});
decoder.configure({ codec: 'vp8', codedWidth: 320, codedHeight: 240 });
...
// AudioDecoder 同构：
const ad = new AudioDecoder({ output: (d: any) => d.close(), error: console.error });
...
ad.decode(audioChunk); await ad.flush();`)),
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.imageDecoder ? 'success' : 'error' }, caps.imageDecoder ? 'ImageDecoder ✓' : '不可用'), h(Tag, { color: 'primary' }, 'png / gif / webp')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'new ImageDecoder({ type, data }) 异步解码图像（type ∈ image/png / image/gif / image/webp / image/avif）。decoder.tracks.ready 等待轨道元数据就绪；decoder.tracks[i].animated / frameCount / type 反映轨道信息；decoder.decode({ frameIndex }) 返回 VideoFrame（与 VideoEncoder 同源，可进一步编码或绘制到 canvas）。支持 GIF/APNG 多帧动画。本卡片用 1x1 透明 PNG 字节演示解码流程。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('创建 ImageDecoder 并 decode', { type: 'primary', size: 'sm', disabled: !caps.imageDecoder, onClick: () => this._createImageDecoder() })),
                h('div', { class: 'fs-sm text-secondary' }, '解码结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '240px', overflow: 'auto' } }, h('code', {}, s.imageDecoderInfo || '（点击「创建 ImageDecoder 并 decode」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '160px', overflow: 'auto' } }, h('code', {}, `const decoder = new ImageDecoder({ type: 'image/png', data: pngBytes });
await decoder.tracks.ready;
...
decoder.close();`)),
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'page api-lab-page' }, h('h2', { class: 'section-title' }, 'Web Speech + WebCodecs 实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, 'Web Speech API 提供语音合成（TTS）与语音识别（ASR）；WebCodecs API 提供底层视频/音频/图像编解码。本页演示 SpeechSynthesis / SpeechRecognition / VideoEncoder / VideoDecoder / ImageDecoder。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderCard6(), this._renderLogPanel());
    }
}
//# sourceMappingURL=SpeechCodecsPage.js.map