...
export class ModernCSSPage extends Page {
    ...
    componentDidMount() {
        ...
        const check = (a, b) => {
            try {
                if (typeof CSS === 'undefined')
                    return false;
                if (a.startsWith('@'))
                    return CSS.supports(a);
                if (a === 'selector')
                    return CSS.supports(`selector(${b})`);
        ...
        const layerSupported = check('@layer base', undefined);
        const scopeSupported = check('@scope', undefined);
        ...
        const startingStyleSupported = check('@starting-style', undefined);
    ...
    _renderLayerCard() {
        ...
        return h(Card, {
            ...
            extra: h(Tag, { color: s.layerSupported ? 'success' : 'error' }, s.layerSupported ? '已支持' : '未支持'),
        }, h('p', { class: 'fs-sm text-secondary' }, '@layer 用于显式分组 CSS 规则并控制级联优先级。声明顺序决定优先级：后声明的层优先级更高。!important 在 layer 中反转：先声明的层 !important 优先级更高。可用 @import url() layer(name); 把外部样式表归入指定层。'), !s.layerSupported && h(Alert, {
            type: 'warning',
            message: '当前浏览器不支持 @layer',
            description: 'CSS.supports("@layer base") 返回 false。Chrome 99+ / Firefox 97+ / Safari 15.4+ 支持。jsdom 不支持，但页面不会报错。',
        }), h('div', { class: 'flex gap-sm mt-sm flex-wrap' }, this._btn(`切换层顺序（当前：${s.layerOrder === 'normal' ? 'base → theme' : 'theme → base'}）`, {
        ...
        })), h('div', { class: 'layer-demo-box layer-target' }, `目标元素 .layer-target —— 当前层顺序：${s.layerOrder === 'normal' ? 'base, theme（theme 覆盖 base → 蓝色）' : 'theme, base（base 覆盖 theme → 橙色）'}${s.layerImportantOn ? '；!important 已注入（先声明的 base 层 !important 优先级更高 → 橙色）' : ''}`), h('p', { class: 'fs-sm text-tertiary mt-sm' }, '正常规则：后声明的层优先级更高（theme 后声明 → 覆盖 base）。点击「注入 !important」后规则反转：先声明的 base 层的 !important 反而优先级最高，强制变橙色。'), h('pre', { class: 'code-block mt-sm' }, `@layer base, theme;          /* 声明顺序：base < theme */
...
@import url("reset.css") layer(reset);`));
    ...
    _renderScopeHasCard() {
        ...
        return h(Card, {
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: s.scopeSupported ? 'success' : 'error' }, s.scopeSupported ? '@scope 已支持' : '@scope 未支持'), h(Tag, { color: s.hasSupported ? 'success' : 'error' }, s.hasSupported ? ':has 已支持' : ':has 未支持')),
        }, h('p', { class: 'fs-sm text-secondary' }, ':has() 是 CSS 首个真正的「父选择器」，根据子元素状态选择父元素；@scope (.host) to (.limit) { ... } 限定规则仅作用于 host 内、limit 之外的元素，避免全局污染。:scope 关键字引用当前作用域根。'), (!s.scopeSupported || !s.hasSupported) && h(Alert, {
            type: 'warning',
        ...
        }), h('h4', { class: 'fs-md mt-sm', style: { fontWeight: 'bold' } }, ':has() 演示'), h('div', { class: 'flex gap-sm mt-sm' }, this._btn(`子元素 ${s.hasActiveChild ? '取消 .active' : '加 .active'}`, {
            type: 'primary', size: 'sm', onClick: () => this._toggleHasChild(),
        })), h('div', { class: 'has-demo mt-sm' }, h('div', { class: 'fs-sm text-secondary' }, '父容器 .has-demo —— 当子元素含 .active 时通过 :has(.has-child.active) 变蓝并加阴影'), h('span', {
            class: `has-child${s.hasActiveChild ? ' active' : ''}`,
        }, `子元素 A${s.hasActiveChild ? '（active）' : ''}`), h('span', { class: 'has-child' }, '子元素 B（恒非 active）')), h('h4', { class: 'fs-md mt-sm', style: { fontWeight: 'bold' } }, '@scope 演示'), h('div', { class: 'scope-host' }, h('p', {}, '@scope (.scope-host) to (.scope-limit) 内的 <p> —— 被作用域规则染成蓝色加粗'), h('p', {}, '这是另一段受作用域影响的 <p>，同样变蓝'), h('div', { class: 'scope-limit' }, h('p', {}, '.scope-limit 之内的 <p> —— 不受 @scope 影响，保持默认样式'))), h('pre', { class: 'code-block mt-sm' }, `/* :has() 父选择器 */
  ...
  img { max-width: 100%; }
    ...
    _renderColorCard() {
        ...
        return h(Card, {
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: s.colorMixSupported ? 'success' : 'error' }, s.colorMixSupported ? 'color-mix' : 'color-mix ✕'), h(Tag, { color: s.relativeColorSupported ? 'success' : 'error' }, s.relativeColorSupported ? '相对颜色' : '相对颜色 ✕'), h(Tag, { color: s.lightDarkSupported ? 'success' : 'error' }, s.lightDarkSupported ? 'light-dark' : 'light-dark ✕')),
        }, h('p', { class: 'fs-sm text-secondary' }, 'color-mix(in srgb|oklch, c1, c2 N%) 按比例混合两色；相对颜色 rgb(from c r g b) 从现有颜色派生（可对 r/g/b 做 calc）；light-dark(light, dark) 根据 color-scheme 自动取值，配合 prefers-color-scheme 媒体查询实现主题切换。'), (!s.colorMixSupported || !s.relativeColorSupported || !s.lightDarkSupported) && h(Alert, {
            type: 'warning',
            message: '部分特性当前浏览器不支持',
        ...
        }), h('h4', { class: 'fs-md mt-sm', style: { fontWeight: 'bold' } }, 'color-mix() —— 红 + 蓝（5 个比例）'), h('div', { class: 'mix-row' }, h('div', { class: 'mix-swatch mix-swatch--r1' }, '20%'), h('div', { class: 'mix-swatch mix-swatch--r2' }, '40%'), h('div', { class: 'mix-swatch mix-swatch--r3' }, '50%'), h('div', { class: 'mix-swatch mix-swatch--r4' }, '60%'), h('div', { class: 'mix-swatch mix-swatch--r5' }, '80%')), h('p', { class: 'fs-sm text-tertiary mt-sm' }, 'color-mix(in srgb, #ff4d4f, #1677ff N%) —— N 为蓝色占比。in oklch 模式更符合人眼感知（混合更平滑）。'), h('h4', { class: 'fs-md mt-sm', style: { fontWeight: 'bold' } }, '相对颜色 —— from #1677ff 派生'), h('div', { class: 'flex mt-sm', style: { flexWrap: 'wrap' } }, h('div', { class: 'rel-box rel-base' }, '基色'), h('div', { class: 'rel-box rel-lighter' }, 'lighter\nr+60'), h('div', { class: 'rel-box rel-darker' }, 'darker\nr-60 g-30'), h('div', { class: 'rel-box rel-alpha' }, 'alpha\n/0.4')), h('p', { class: 'fs-sm text-tertiary mt-sm' }, 'rgb(from #1677ff calc(r + 60) g b) —— 派生亮色；rgb(from #1677ff r g b / 0.4) —— 派生半透明色。也可用 hsl() / oklch() 派生。'), h('h4', { class: 'fs-md mt-sm', style: { fontWeight: 'bold' } }, 'light-dark() —— 配合 color-scheme'), h('div', { class: 'flex gap-sm mt-sm' }, this._btn(`切换 color-scheme（当前：${s.colorScheme}）`, {
            type: 'primary', size: 'sm', onClick: () => this._toggleColorScheme(),
        })), h('div', { class: 'light-dark-demo' }, `当前 color-scheme = ${s.colorScheme}。light-dark(#fff, #1f1f1f) 取${s.colorScheme === 'dark' ? '第二参数（深色）' : '第一参数（浅色）'}。背景 / 边框 / 文字色均由 light-dark() 计算。`), h('p', { class: 'fs-sm text-tertiary mt-sm' }, 'prefers-color-scheme 媒体查询会自动设置 color-scheme；也可手动设置 :root { color-scheme: light | dark | light dark }。'), h('pre', { class: 'code-block mt-sm' }, `/* color-mix */
  ...
  :root { color-scheme: dark; }
    ...
    _renderNestingCard() {
        ...
        return h(Card, {
            ...
            extra: h(Tag, { color: s.nestingSupported ? 'success' : 'error' }, s.nestingSupported ? '已支持' : '未支持'),
        }, h('p', { class: 'fs-sm text-secondary' }, '原生 CSS 嵌套用 & 引用父选择器，不再需要 SCSS / Less 等预处理器。可嵌套 @media / @supports 等规则。注意：原生嵌套中 & 必须出现在选择器开头（如 &.active、& .child），与 SCSS 的 & 可任意位置略有差异。'), !s.nestingSupported && h(Alert, {
            type: 'warning',
        ...
        }), h('div', { class: 'flex gap-sm mt-sm' }, this._btn(`.nest-card ${s.nestActive ? '取消 .active' : '加 .active'}`, {
            type: 'primary', size: 'sm', onClick: () => this._toggleNestActive(),
        })), h('div', { class: `nest-card mt-sm${s.nestActive ? ' active' : ''}` }, h('div', { class: 'nest-title' }, '嵌套卡片标题（.nest-title）'), h('div', { class: 'nest-desc' }, `样式由原生嵌套 & .nest-title / &.active 应用。当前 ${s.nestActive ? 'active（蓝边、蓝字）' : '非 active（默认灰）'}。嵌套 @media (max-width:600px) 还会缩小内边距。`)), h('p', { class: 'fs-sm text-tertiary mt-sm' }, '与 SCSS 区别：SCSS 在编译期展开为逗号分隔选择器；原生 CSS 由浏览器解析，& 必须是相对父级的引用，且嵌套层级过深时性能略降。'), h('pre', { class: 'code-block mt-sm' }, `.nest-card {
  padding: 16px;
  ...
  @media (max-width: 600px) {              /* 嵌套 @media */
    ...
    _renderTrigTextWrapCard() {
        ...
        return h(Card, {
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: s.trigSupported ? 'success' : 'error' }, s.trigSupported ? '三角函数' : '三角函数 ✕'), h(Tag, { color: s.textWrapSupported ? 'success' : 'error' }, s.textWrapSupported ? 'text-wrap' : 'text-wrap ✕'), h(Tag, { color: s.startingStyleSupported ? 'success' : 'error' }, s.startingStyleSupported ? '@starting-style' : '@starting-style ✕')),
        }, h('p', { class: 'fs-sm text-secondary' }, 'CSS 三角函数：sin() / cos() / tan() / asin() / acos() / atan() / atan2()，以及 pow() / sqrt() / hypot() / log() / exp() / abs() / sign()。角度单位 deg / rad / turn / grad 可互转。配合 calc() 可实现纯 CSS 几何布局。'), 
        // —— 三角函数演示：圆形排列 5 个点 ——
        h('h4', { class: 'fs-md mt-sm', style: { fontWeight: 'bold' } }, '三角函数 sin/cos —— 圆形排列 5 个点'), !s.trigSupported && h(Alert, {
            type: 'warning',
        ...
        }), h('div', { class: 'flex gap-sm mt-sm' }, this._btn('旋转 72°', { type: 'primary', size: 'sm', onClick: () => this._rotateTrig() }), this._btn('半径 +10', { size: 'sm', onClick: () => this._changeTrigRadius(10) }), this._btn('半径 -10', { size: 'sm', onClick: () => this._changeTrigRadius(-10) })), h('div', {
            id: 'trig-stage',
            class: 'trig-stage',
            // 渲染后下一帧布局一次（_initTrigLayoutIfReady 内部用 rAF，安全）
        }, ...[0, 1, 2, 3, 4].map((i) => h('div', { class: 'trig-dot' }, `${i + 1}`))), h('p', { class: 'fs-sm text-tertiary mt-sm' }, `5 个点均匀分布在半径 ${s.trigRadius}px 的圆上，起始角度偏移 ${s.trigAngleOffset}°。计算公式：x = cx + R*cos(θ), y = cy + R*sin(θ)。`), h('pre', { class: 'code-block mt-sm' }, `/* 纯 CSS 三角函数圆形排列 */
.dot {
...
transform: rotate(atan2(1, 1));     /* 45deg */`), 
        // —— text-wrap 演示 ——
        h('h4', { class: 'fs-md mt-sm', style: { fontWeight: 'bold' } }, 'text-wrap: balance / pretty'), !s.textWrapSupported && h(Alert, {
            type: 'warning',
        ...
        }), h('div', { class: 'tw-balance' }, 'text-wrap: balance —— 标题文本自动均衡换行，避免最后一行只剩一个字。本段会尽量让每行长度接近。'), h('div', { class: 'tw-pretty' }, 'text-wrap: pretty —— 段落文本智能换行，避免孤行（最后一行单字）和断点不美观。适合正文段落。'), h('div', { class: 'tw-normal' }, 'text-wrap: wrap（默认）—— 普通换行，由浏览器按宽度自动断行，可能出现孤行或不平衡的换行。'), h('p', { class: 'fs-sm text-tertiary mt-sm' }, 'text-wrap-mode: wrap|nowrap 控制是否换行；text-wrap-style: balance|pretty|stable|nowrap 控制换行算法（二者可分开设置）。'), 
        // —— @starting-style 演示 ——
        h('h4', { class: 'fs-md mt-sm', style: { fontWeight: 'bold' } }, '@starting-style + transition-behavior: allow-discrete'), !s.startingStyleSupported && h(Alert, {
            type: 'warning',
        ...
        }), h('div', { class: 'flex gap-sm mt-sm' }, this._btn(`${s.startingVisible ? '隐藏' : '显示'} 元素`, {
            type: 'primary', size: 'sm', onClick: () => this._toggleStartingStyle(),
        })), s.startingVisible && h('div', { class: 'ss-target' }, '@starting-style 让元素首次出现时从透明 + 缩小 0.8 过渡到正常。配合 transition-behavior: allow-discrete 还能过渡 display 属性（隐藏时先过渡再 display:none）。'), h('p', { class: 'fs-sm text-tertiary mt-sm' }, 'interpolate-size: allow-keywords 可让 height: auto 也能被过渡（不再需要测量高度）。field-sizing: content 让 textarea/input 跟随内容自动调整大小（下方 textarea 演示）。'), 
        // —— field-sizing 演示 ——
        h('h4', { class: 'fs-md mt-sm', style: { fontWeight: 'bold' } }, 'field-sizing: content —— textarea 自适应高度'), !s.fieldSizingSupported && h(Alert, {
            type: 'warning',
        ...
        }), h('h4', { class: 'fs-md mt-sm', style: { fontWeight: 'bold' } }, '能力检测汇总'), h('div', { class: 'fs-sm mt-sm', style: { lineHeight: '1.8' } }, h('div', {}, `三角函数 sin/cos/...: ${s.trigSupported ? '✓ 已支持' : '✗ 未支持'}`), h('div', {}, `text-wrap (balance/pretty): ${s.textWrapSupported ? '✓ 已支持' : '✗ 未支持'}`), h('div', {}, `@starting-style: ${s.startingStyleSupported ? '✓ 已支持' : '✗ 未支持'}`), h('div', {}, `transition-behavior: allow-discrete: ${s.transitionBehaviorSupported ? '✓ 已支持' : '✗ 未支持'}`), h('div', {}, `interpolate-size: allow-keywords: ${s.interpolateSizeSupported ? '✓ 已支持' : '✗ 未支持'}`), h('div', {}, `field-sizing: content: ${s.fieldSizingSupported ? '✓ 已支持' : '✗ 未支持'}`)), h('pre', { class: 'code-block mt-sm' }, `/* @starting-style：元素首次出现时的初始样式 */
.ss-target {
  opacity: 1;
...
//# sourceMappingURL=ModernCSSPage.js.map