...
export class WindowPiPViewportPage extends Page {
    ...
    componentDidMount() {
        ...
        const caps = this._caps(undefined);
        const mark = (b) => (b ? '✓' : '✗');
        const parts = [
        ...
        if (!caps.docPiP)
            this._addLog('warn', 'window.documentPictureInPicture 不可用（需真实浏览器，jsdom 已 polyfill）');
        if (!caps.visualViewport)
            this._addLog('warn', 'window.visualViewport 不可用');
        if (!caps.wakeLock)
            this._addLog('warn', 'navigator.wakeLock 不可用（需用户手势 + 活跃文档）');
        if (!caps.orientation)
            this._addLog('warn', 'screen.orientation 不可用（jsdom 未实现）');
        if (!caps.fullscreen)
            this._addLog('warn', 'Fullscreen API 不可用（jsdom 未实现 fullscreenEnabled）');
        if (!caps.pointerLock)
            this._addLog('warn', 'Pointer Lock API 不可用（jsdom 未实现 requestPointerLock）');
    ...
    componentWillUnmount() {
        ...
        if (vv && typeof vv.removeEventListener === 'function') {
            try {
                if (this._onVVResize)
                    vv.removeEventListener('resize', this._onVVResize);
            }
            catch { /* noop */ }
            try {
                if (this._onVVScroll)
                    vv.removeEventListener('scroll', this._onVVScroll);
            }
            catch { /* noop */ }
        }
    ...
    async _demoDocPiP() {
        const caps = this._caps(undefined);
        if (!caps.docPiP) {
            this.setState({ docPiPInfo: 'window.documentPictureInPicture 不可用。\n\n真实浏览器用法：\n' +
                    ...
                    "// 事件：'enter' (e.window) / 'leave'；documentPictureInPicture.addEventListener('enter', (e: any) => e.window);" });
        ...
        try {
            ...
            if (movedEl && pipWindow && pipWindow.document && pipWindow.document.body) {
                ...
                try {
                    ...
                    else if (typeof body.appendChild === 'function') {
                        body.appendChild(movedEl);
                        movedOk = true;
                    }
                }
                catch (e) {
                    this._addLog('warn', `移动 DOM 进 PiP 失败：${errInfo(e).message}`);
                }
            }
            const currentWin = (typeof dpip.window !== 'undefined') ? dpip.window : null;
            this.setState({ docPiPInfo: 'documentPictureInPicture.requestWindow() → PiP 窗口 ✓\n' +
    ...
    _closeDocPiP() {
        ...
        try {
            if (typeof this._pipWindow.close === 'function')
                this._pipWindow.close();
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['DocPiP', caps.docPiP], ['任意DOM', caps.docPiP]])),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'window.documentPictureInPicture.requestWindow({width,height,disallowReturnToOpener}) 返回 Promise<Window>，得到一个全新 window 对象，可把任意 DOM 元素「移动」进去（非仅 video，元素被移动而非复制）。window.documentPictureInPicture.window 指向当前 PiP 窗口；事件 enter（e.window）/ leave。适合把视频控件、聊天面板、笔记等独立到浮动窗口。jsdom 无原生支持，polyfill 已 mock。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('requestWindow + 移动 DOM', { type: 'primary', size: 'sm', disabled: !caps.docPiP, onClick: () => this._demoDocPiP() }), this._btn('关闭 PiP', { danger: true, size: 'sm', disabled: !caps.docPiP, onClick: () => this._closeDocPiP() })),
                h('div', { class: 'fs-sm text-secondary' }, 'Document PiP 状态：'),
                h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, s.docPiPInfo || '（点击「requestWindow + 移动 DOM」开始演示）')),
                h(Alert, {
                    type: 'warning',
    ...
    _readViewport() {
        const caps = this._caps(undefined);
        if (!caps.visualViewport) {
            this.setState({ viewportInfo: 'window.visualViewport 不可用。\n\n真实浏览器用法：\n' +
                    'const vv = window.visualViewport;\n' +
                    'vv.width/height/scale/offsetLeft/offsetTop/pageLeft/pageTop\n' +
                    "vv.addEventListener('resize', (e: any) => e.target.width);\n" +
                    "vv.addEventListener('scroll', (e: any) => e.target.pageTop);" });
            this._addLog('warn', 'visualViewport 不可用，已记录用法');
            return;
        }
        try {
            const vv = window.visualViewport;
            this.setState({ viewportInfo: 'window.visualViewport 属性快照：\n' +
    ...
    _bindViewport() {
        ...
        try {
            ...
            this._addLog('warn', `注册 resize 失败：${errInfo(err).message}`);
        }
        try {
            vv.addEventListener('scroll', this._onVVScroll);
        }
        catch (err) {
            this._addLog('warn', `注册 scroll 失败：${errInfo(err).message}`);
        }
        this.setState({ viewportInfo: '已注册 visualViewport 事件监听 ✓\n' +
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['visualViewport', caps.visualViewport], ['resize/scroll', caps.visualViewport]])),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'window.visualViewport 反映用户实际可见的视口（受双指缩放与移动端键盘影响），区别于 layout viewport。属性：width / height / scale / offsetLeft / offsetTop / pageLeft / pageTop。事件：resize（视口尺寸或缩放变化）/ scroll（视口滚动）/ scrollend。e.target 指向 visualViewport 自身。常用于：移动端键盘弹出时调整布局、双指缩放适配、固定元素定位。jsdom 中由 polyfill mock 提供静态值。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取属性', { type: 'primary', size: 'sm', disabled: !caps.visualViewport, onClick: () => this._readViewport() }), this._btn('监听 resize/scroll', { size: 'sm', disabled: !caps.visualViewport, onClick: () => this._bindViewport() })),
                h('div', { class: 'fs-sm text-secondary' }, 'Visual Viewport 状态：'),
                h('pre', { class: 'code-block', style: { maxHeight: '240px', overflow: 'auto' } }, h('code', {}, s.viewportInfo || '（点击「读取属性」或「监听 resize/scroll」）')),
                h(Alert, {
                    type: 'info',
    ...
    async _requestWakeLock() {
        const caps = this._caps(undefined);
        if (!caps.wakeLock) {
            this.setState({ wakeLockInfo: 'navigator.wakeLock 不可用。\n\n真实浏览器用法：\n' +
        ...
        try {
            ...
            if (this._wakeLockSentinel && !this._wakeLockSentinel.released) {
                try {
                    if (this._wakeLockReleaseHandler)
                        this._wakeLockSentinel.removeEventListener('release', this._wakeLockReleaseHandler);
                }
                catch { /* noop */ }
                try {
            ...
            if (typeof sentinel.addEventListener === 'function') {
                try {
                    sentinel.addEventListener('release', this._wakeLockReleaseHandler);
                }
                catch { /* noop */ }
            }
            this.setState({ wakeLockInfo: "navigator.wakeLock.request('screen') → WakeLockSentinel ✓\n" +
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['wakeLock', caps.wakeLock], ['screen', caps.wakeLock]])),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, "navigator.wakeLock.request('screen') 返回 Promise<WakeLockSentinel>，阻止屏幕进入休眠（如视频播放、导航、阅读时）。WakeLockSentinel.type 始终为 'screen'；released 布尔标记是否已释放；release() 返回 Promise 主动释放；'release' 事件在释放时触发（含系统自动释放）。要求用户手势 + 文档可见（visibilityState='visible'）。页面隐藏时系统可能自动释放，需在 visibilitychange 中重新请求。jsdom 中由 polyfill mock 提供。"),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn("request('screen')", { type: 'primary', size: 'sm', disabled: !caps.wakeLock, onClick: () => this._requestWakeLock() }), this._btn('release 释放', { danger: true, size: 'sm', disabled: !caps.wakeLock, onClick: () => this._releaseWakeLock() })),
                h('div', { class: 'fs-sm text-secondary' }, 'Wake Lock 状态：'),
                h('pre', { class: 'code-block', style: { maxHeight: '220px', overflow: 'auto' } }, h('code', {}, s.wakeLockInfo || '（点击 "request(\'screen\')" 申请唤醒锁）')),
                h(Alert, {
                    type: 'warning',
    ...
    _readOrientation() {
        const caps = this._caps(undefined);
        if (!caps.orientation) {
            this.setState({ orientationFsInfo: 'screen.orientation 不可用（jsdom 未实现）。\n\n真实浏览器：\n' +
        ...
        try {
            ...
            this.setState({ orientationFsInfo: `screen.orientation 快照：\n  type = "${ori.type}"   angle = ${ori.angle}\n\n` +
    ...
    async _attemptFullscreen() {
        if (typeof document === 'undefined' || document.fullscreenEnabled !== true) {
            ...
            this.setState({ orientationFsInfo: 'Fullscreen API 不可用。\n\n真实浏览器用法：\n' +
        ...
        try {
            ...
            this.setState({ orientationFsInfo: `requestFullscreen 失败：${errInfo(err).name} - ${errInfo(err).message}\n常见原因：未由用户手势触发 / 元素不可全屏 / 浏览器拒绝。jsdom 中 fullscreenEnabled 为 false，无法真实进入全屏。` });
        }
    }
    // 退出全屏
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['orientation', caps.orientation], ['Fullscreen', caps.fullscreen]])),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, "screen.orientation.type（portrait-primary / landscape-secondary 等）与 angle 反映屏幕方向；lock(type) 锁定方向（要求文档已全屏）、unlock() 解除、'change' 事件监听变化。Fullscreen API：element.requestFullscreen({navigationUI:'auto'|'show'|'hide'}) 进入全屏（需用户手势）、document.exitFullscreen() 退出、document.fullscreenElement 当前全屏元素、document.fullscreenEnabled 能力检测、fullscreenchange / fullscreenerror 事件。jsdom 中 orientation 与 fullscreen 通常未实现。"),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取 orientation', { type: 'primary', size: 'sm', disabled: !caps.orientation, onClick: () => this._readOrientation() }), this._btn('requestFullscreen', { size: 'sm', disabled: !caps.fullscreen, onClick: () => this._attemptFullscreen() }), this._btn('exitFullscreen', { danger: true, size: 'sm', disabled: !caps.fullscreen, onClick: () => this._exitFullscreen() })),
                h('div', { class: 'fs-sm text-secondary' }, 'Orientation / Fullscreen 状态：'),
                h('pre', { class: 'code-block', style: { maxHeight: '240px', overflow: 'auto' } }, h('code', {}, s.orientationFsInfo || '（点击「读取 orientation」或「requestFullscreen」）')),
                h(Alert, {
                    type: 'warning',
    ...
    async _attemptPointerLock() {
        const caps = this._caps(undefined);
        if (!caps.pointerLock) {
            this.setState({ pointerLockInfo: 'Pointer Lock API 不可用（element.requestPointerLock 不存在）。\n\n真实浏览器用法：\n' +
        ...
        try {
            ...
            this.setState({ pointerLockInfo: `requestPointerLock 失败：${errInfo(err).name} - ${errInfo(err).message}\n常见原因：未由用户手势触发 / unadjustedMovement 不被支持 / 用户拒绝。典型场景：FPS 游戏、3D 模型查看器。` });
        }
    }
    // 退出指针锁
    _exitPointerLock() {
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['PointerLock', caps.pointerLock], ['movementX/Y', caps.pointerLock]])),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'element.requestPointerLock({ unadjustedMovement: true }) 请求指针锁（新版返回 Promise，旧版返回 void）；锁定后鼠标光标隐藏、MouseEvent.movementX / movementY 提供无限相对位移（不再受屏幕边界限制），适合 FPS 游戏瞄准、3D 模型 / 全景查看器。unadjustedMovement:true 使用原始鼠标输入（绕过 OS 鼠标加速度，更精准）。document.exitPointerLock() 退出；document.pointerLockElement 当前锁定元素；pointerlockchange / pointerlockerror 事件。需用户手势触发。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('requestPointerLock', { type: 'primary', size: 'sm', disabled: !caps.pointerLock, onClick: () => this._attemptPointerLock() }), this._btn('exitPointerLock', { danger: true, size: 'sm', disabled: !caps.pointerLock, onClick: () => this._exitPointerLock() })),
                h('div', { class: 'fs-sm text-secondary' }, 'Pointer Lock 状态：'),
                h('pre', { class: 'code-block', style: { maxHeight: '240px', overflow: 'auto' } }, h('code', {}, s.pointerLockInfo || '（点击「requestPointerLock」尝试锁定指针）')),
                h(Alert, {
                    type: 'info',
    ...
    _detectVirtualKeyboard() {
        const caps = this._caps(undefined);
        if (!caps.virtualKeyboard) {
            this.setState({ virtualKeyboardInfo: 'navigator.virtualKeyboard 不可用（仅无物理键盘设备，如平板 / 手机）。\n\n真实浏览器用法：\n' +
        ...
        try {
            ...
            this.setState({ virtualKeyboardInfo: 'navigator.virtualKeyboard 检测 ✓\n' +
    ...
    _toggleVirtualKeyboard(show) {
        const caps = this._caps(undefined);
        if (!caps.virtualKeyboard) {
            this._addLog('warn', 'navigator.virtualKeyboard 不可用');
            return;
        }
        try {
            const vk = navigator.virtualKeyboard;
            ...
            if (typeof fn === 'function') {
                fn.call(vk);
                this._addLog('vkbd', `virtualKeyboard.${show ? 'show' : 'hide'}() 已调用`);
            }
            else {
                this._addLog('warn', `virtualKeyboard.${show ? 'show' : 'hide'} 不可用`);
            }
        }
        catch (err) {
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['virtualKeyboard', caps.virtualKeyboard], ['geometrychange', caps.virtualKeyboard]])),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, "navigator.virtualKeyboard（仅无物理键盘设备，如平板 / 手机）提供程序化控制：show() 唤起 / hide() 收起 / boundingRect 键盘几何 / overlaysContent 控制是否由应用自行处理遮挡布局 / 'geometrychange' 事件监听几何变化。元素 virtualKeyboardPolicy='auto'（浏览器自动，默认）/ 'manual'（应用自行 show/hide，配合 contenteditable + inputmode 实现自定义键盘 UI）。jsdom 中由 polyfill mock 提供。"),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('检测 virtualKeyboard', { type: 'primary', size: 'sm', disabled: !caps.virtualKeyboard, onClick: () => this._detectVirtualKeyboard() }), this._btn('show()', { size: 'sm', disabled: !caps.virtualKeyboard, onClick: () => this._toggleVirtualKeyboard(true) }), this._btn('hide()', { size: 'sm', disabled: !caps.virtualKeyboard, onClick: () => this._toggleVirtualKeyboard(false) })),
                h('div', { class: 'fs-sm text-secondary' }, 'Virtual Keyboard 状态：'),
                h('pre', { class: 'code-block', style: { maxHeight: '240px', overflow: 'auto' } }, h('code', {}, s.virtualKeyboardInfo || '（点击「检测 virtualKeyboard」开始演示）')),
                h(Alert, {
                    type: 'info',
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'api-lab-page window-pip-viewport-page' }, h('h2', { class: 'section-title' }, 'Window / PiP / Viewport 显示控制 实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, '文档画中画 / 可视视口 / 屏幕唤醒锁 / 方向与全屏 / 指针锁 / 虚拟键盘。本页演示 Window/PiP/Viewport 显示控制类 API，覆盖任意 DOM 画中画、移动端键盘与缩放适配、唤醒锁、全屏与方向锁定、指针锁、虚拟键盘控制。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderCard6(), this._renderLogPanel());
    }
}
//# sourceMappingURL=WindowPiPViewportPage.js.map