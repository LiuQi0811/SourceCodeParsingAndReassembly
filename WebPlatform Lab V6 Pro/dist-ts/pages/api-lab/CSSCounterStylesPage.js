...
export class CSSCounterStylesPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!f.counterStyleAtRule)
            this._addLog('warn', '@counter-style at-rule 不可用或 jsdom 未识别（Chrome 91+ / Firefox 33+ / Safari 17+ 全部稳定）');
        if (!f.counterStyleRule)
            this._addLog('warn', 'CSSCounterStyleRule 接口未定义（jsdom 通常不实现，现代浏览器全支持）');
        if (!f.counterSet)
            this._addLog('warn', 'counter-set 不可用或 jsdom 未识别（Chrome 85+ / Firefox 68+ / Safari 17.4+）');
    ...
    _flags() {
        ...
        const supportsDecl = (decl) => safe(() => hasCSS && typeof CSS.supports === 'function' && CSS.supports(decl));
        return {
            css: hasCSS,
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            children: [
                ...
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.overviewInfo || '（点击「读取概述信息」查看 CSS Counter Styles 全景）')),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['@counter-style', f.counterStyleAtRule]]), h(Tag, { color: 'primary' }, 'at-rule 语法')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '@counter-style at-rule 语法：@counter-style <name> { system; symbols; additive-symbols; prefix; suffix; range; pad; negative; fallback; speak-as; }。10 个描述符：system（算法类型，必需）、symbols（符号集，必需）、additive-symbols（加法符号，仅 additive system）、prefix/suffix（前后缀，suffix 默认 ". "）、range（适用范围）、pad（补齐）、negative（负数符号）、fallback（兜底样式）、speak-as（无障碍朗读）。name 必须是合法 CSS 标识符，不能与内置样式重名。用 list-style-type: name 或 content: counter(item, name) 引用。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取语法信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ syntaxInfo: this._readSyntaxInfo() }) })),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.syntaxInfo || '（点击按钮查看 @counter-style 语法完整说明）')),
                h(Alert, {
                    type: 'info',
    ...
    _setSystem(mode) {
        ...
        this._injectStyle('css-system-dynamic', (styles[mode]) || styles.cyclic);
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['@counter-style', f.counterStyleAtRule]]), h(Tag, { color: 'primary' }, '7 种 system')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'system 描述符定义算法类型：cyclic（循环，适合天干地支）、symbolic（符号重复，如 * ** ***）、additive（加法，适合罗马数字 I/V/X）、fixed（固定，超出用 fallback）、numeric（数字进制，适合阿拉伯-印度数字）、alphabetic（字母进制，适合希腊字母 α β γ）、extends（继承已有样式微调）。不同 system 表现差异显著，fallback 默认 decimal 兜底超范围情况。Chrome 91+ / Firefox 33+ / Safari 17+ 支持。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ systemInfo: this._readSystemInfo() }) }), this._btn('cyclic', { size: 'sm', disabled: !f.counterStyleAtRule, onClick: () => this._setSystem('cyclic') }), this._btn('symbolic', { size: 'sm', disabled: !f.counterStyleAtRule, onClick: () => this._setSystem('symbolic') }), this._btn('additive', { size: 'sm', disabled: !f.counterStyleAtRule, onClick: () => this._setSystem('additive') }), this._btn('numeric', { size: 'sm', disabled: !f.counterStyleAtRule, onClick: () => this._setSystem('numeric') }), this._btn('alphabetic', { size: 'sm', disabled: !f.counterStyleAtRule, onClick: () => this._setSystem('alphabetic') })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '当前 system: ' + this._currentSystem + '（真实浏览器查看自定义序号）：'),
                h('div', { class: 'system-stage' }, h('ol', { class: 'sys-cyclic' }, h('li', {}, '第一项'), h('li', {}, '第二项'), h('li', {}, '第三项'), h('li', {}, '第四项'), h('li', {}, '第五项'))),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.systemInfo || '（点击按钮切换 system 查看说明）')),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['@counter-style', f.counterStyleAtRule]]), h(Tag, { color: 'primary' }, '符号集')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'symbols 描述符定义符号集（用于 cyclic/symbolic/fixed/numeric/alphabetic），支持字符串、多字符符号、Unicode 转义（\\2605 ★）、图像 url()。多字符符号将整个字符串作为单个符号。additive-symbols 仅用于 system: additive（罗马数字等），语法 additive-symbols: <value> <symbol>, ...，value 按降序排列，算法从大到小减法表示（2024 = MMXXIV）。Unicode 符号集：天干地支、阿拉伯-印度数字、希腊字母、八卦、星星等任意 Unicode 字符。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取 symbols 信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ symbolsInfo: this._readSymbolsInfo() }) })),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.symbolsInfo || '（点击按钮查看 symbols/additive-symbols 完整说明）')),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['@counter-style', f.counterStyleAtRule]]), h(Tag, { color: 'primary' }, '6 个描述符')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '6 个格式描述符：prefix/suffix（前后缀，suffix 默认 ". "）、range（适用范围，超出用 fallback）、pad（补齐，如 pad: 3 "0" 补零到 3 位）、fallback（兜底样式，默认 decimal）、negative（负数符号，默认 "-"，可定义括号 negative: "(" ")"）、组合自定义完整序号格式（如发票号 INV-0001）。prefix: "(" + suffix: ") " 实现括号序号 (1) (2)。range: 1 12 限制月份序号。pad: 4 "0" 补零到 4 位。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取格式描述符信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ descriptorsInfo: this._readDescriptorsInfo() }) })),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.descriptorsInfo || '（点击按钮查看 6 个格式描述符完整说明）')),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['@counter-style', f.counterStyleAtRule]]), h(Tag, { color: 'primary' }, '无障碍')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'speak-as 描述符决定屏幕阅读器如何朗读序号：auto（默认，浏览器决定）/ bullets（朗读"项目"，适合符号型）/ numbers（朗读"第一项"，适合数字型）/ words（朗读符号作为单词）/ spell-out（逐字符朗读符号本身）/ <<counter-style>>（引用另一个 counter-style 的朗读方式）。数字型建议 speak-as: numbers，符号型建议 speak-as: spell-out 或 bullets。无障碍最佳实践：避免依赖序号传达关键信息，配合 aria-label 提供完整描述。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取 speak-as 信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ speakAsInfo: this._readSpeakAsInfo() }) })),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.speakAsInfo || '（点击按钮查看 speak-as 与无障碍完整说明）')),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard7() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['@counter-style', f.counterStyleAtRule]]), h(Tag, { color: 'primary' }, '6 大场景')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '中文序号六大场景：天干（甲乙丙丁戊己庚辛壬癸，cyclic 循环）、地支（子丑寅卯辰巳午未申酉戌亥，cyclic）、二十四节气（立春雨水...大寒，fixed + fallback）、农历日期（初一初二...三十，fixed）、阿拉伯-印度数字（٠١٢٣٤٥٦٧٨٩，numeric）、希腊字母（αβγ...ω，alphabetic 字母进制，25=αα）。每个场景展示真实 @counter-style 代码与渲染效果。点击按钮注入自定义 @counter-style（真实浏览器查看效果）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('注入并运行演示', { type: 'primary', size: 'sm', disabled: !f.counterStyleAtRule, onClick: () => this._runCjkCustomDemo() })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '天干/地支/希腊字母序号演示（真实浏览器查看）：'),
                h('div', { class: 'cjk-stage' }, h('div', { class: 'grid-cols' }, h('div', {}, h('div', { class: 'fs-sm text-secondary' }, '天干（cyclic）：'), h('ol', { class: 'tiangan-list' }, h('li', {}, '第一'), h('li', {}, '第二'), h('li', {}, '第三'), h('li', {}, '第四'), h('li', {}, '第五'))), h('div', {}, h('div', { class: 'fs-sm text-secondary' }, '地支（cyclic）：'), h('ol', { class: 'dizhi-list' }, h('li', {}, '第一'), h('li', {}, '第二'), h('li', {}, '第三'), h('li', {}, '第四'))), h('div', {}, h('div', { class: 'fs-sm text-secondary' }, '希腊字母（alphabetic）：'), h('ol', { class: 'greek-list' }, h('li', {}, '第一'), h('li', {}, '第二'), h('li', {}, '第三'))), h('div', {}, h('div', { class: 'fs-sm text-secondary' }, '说明：'), '点击「注入并运行演示」注入 @counter-style，真实浏览器可查看自定义序号。jsdom 不渲染序号但 DOM 正确。'))),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.cjkCustomInfo || '（点击按钮注入 @counter-style 并查看 6 大场景完整代码）')),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard8() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '实战与陷阱：list-style-type: name 引用自定义 @counter-style；counter-reset/increment/set 控制计数器（counter-set 较新 Chrome 85+/Safari 17.4+）；多级目录章节号（1.1.1）用 counters() 函数嵌套；counters(name, separator, style?) 拼接祖先作用域计数器。5 大陷阱：fallback 处理超范围（默认 decimal）、counter-set 浏览器支持较新（用 counter-reset 替代）、@counter-style 浏览器差异（Firefox 33+ / Chrome 91+ / Safari 17+）、DevTools 调试（Firefox 更强大）、counter-reset 作用域（嵌套需重置子计数器）。'),
                ...
                h('div', { class: 'toc-stage' }, h('ol', { class: 'toc', style: { counterReset: 'section' } }, h('li', {}, '第一章'), h('li', {}, '第二章'), h('li', {}, '第三章'))),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.pitfallsInfo || '（点击按钮查看 5 大陷阱与 8 条最佳实践完整说明）')),
                h(Alert, {
                    type: 'warning',
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'api-lab-page css-counter-styles-page' }, h('h2', { class: 'section-title' }, 'CSS Counter Styles @counter-style 实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, '本页演示 CSS Counter Styles Level 3 自定义列表序号样式：@counter-style 语法（name + 10 个描述符）、system 描述符全集（cyclic/symbolic/additive/fixed/numeric/alphabetic/extends）、symbols 与 additive-symbols（符号集 + 罗马数字）、prefix/suffix/range/pad/fallback/negative（完整序号格式）、speak-as 与无障碍（屏幕阅读器朗读）、中文天干地支与节气实战（cyclic 天干地支 / fixed 节气 / numeric 阿拉伯-印度数字 / alphabetic 希腊字母）、实战与陷阱（list-style 引用 / counter-set / 多级目录章节号 / counters() 嵌套 / fallback / DevTools 调试）。所有特性通过 typeof / CSS.supports 能力检测，不可用时仅记日志，绝不抛异常。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderCard6(), this._renderCard7(), this._renderCard8(), this._renderLogPanel());
    }
}
//# sourceMappingURL=CSSCounterStylesPage.js.map