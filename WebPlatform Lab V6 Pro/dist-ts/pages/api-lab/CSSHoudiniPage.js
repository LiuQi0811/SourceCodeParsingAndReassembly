...
export class CSSHoudiniPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!caps.paintWorklet)
            this._addLog('warn', 'CSS.paintWorklet 不可用（Paint Worklet）');
        if (!caps.attributeStyleMap)
            this._addLog('warn', 'element.attributeStyleMap 不可用（CSS Typed OM）');
    }
    ...
    _caps() {
        ...
        const hasFn = (n) => hasCSS && typeof (CSS[n]) === 'function';
        const hasProp = (n) => hasCSS && typeof (CSS[n]) !== 'undefined';
        return {
            css: hasCSS,
            registerProperty: hasFn('registerProperty'),
            paintWorklet: hasProp('paintWorklet') && typeof CSS.paintWorklet.addModule === 'function',
    ...
    _registerProperty() {
        ...
        if (!caps.registerProperty) {
            this._addLog('warn', 'CSS.registerProperty 不可用（Properties & Values API）');
            this.setState({
                propertyInfo: 'CSS.registerProperty 不可用\n' +
        ...
        try {
            ...
            this.setState({
                propertyInfo: 'CSS.registerProperty({ name, syntax, inherits, initialValue }) ✓\n' +
    ...
    _detectSyntax() {
        ...
        try {
            ...
            const sp = (p, v) => `CSS.supports('${p}', '${v}') = ${CSS.supports(p, v)}`;
            const results = [
            ...
            this.setState({
                propertyInfo: `CSS.supports(prop, value) / CSS.supports(condition) → boolean\n${results.join('\n')}\n\n` +
    ...
    _showSyntaxList() {
        this.setState({
            propertyInfo: 'CSS.registerProperty 各 syntax 取值与含义：\n' +
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.registerProperty ? 'success' : 'error' }, caps.registerProperty ? 'registerProperty ✓' : '不可用'), h(Tag, { color: 'primary' }, 'syntax / inherits')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'CSS.registerProperty({ name, syntax, inherits, initialValue }) 注册自定义属性，使其具有强类型语法、初始值与继承语义。注册后的属性可正确参与 @keyframes 动画与 transition 过渡（未注册的 --var 默认视为 * 语法，无法平滑插值）。syntax 取值如 <color> / <length> / <percentage> / <number> / <length+> / * 等。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('注册 --my-color', { type: 'primary', size: 'sm', disabled: !caps.registerProperty, onClick: () => this._registerProperty() }), this._btn('CSS.supports 检测', { type: 'primary', size: 'sm', disabled: !caps.supports, onClick: () => this._detectSyntax() }), this._btn('syntax 取值表', { size: 'sm', onClick: () => this._showSyntaxList() })),
                h('div', { class: 'fs-sm text-secondary' }, '注册 / 检测结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '240px', overflow: 'auto' } }, h('code', {}, s.propertyInfo || '（点击「注册 --my-color」或「CSS.supports 检测」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '150px', overflow: 'auto' } }, h('code', {}, `// 注册强类型自定义属性
...
CSS.supports('color', 'oklch(70% 0.15 145)'); CSS.supports('width', '10cqw');`)),
    ...
    async _addPaintModule() {
        ...
        if (!caps.paintWorklet) {
            ...
            this.setState({
                paintInfo: '(CSS as any).paintWorklet.addModule(url) 不可用\n' +
        ...
        try {
            ...
            await CSS.paintWorklet.addModule(mockUrl);
            this.setState({
                paintInfo: '(CSS as any).paintWorklet.addModule(url) → Promise<void> ✓\n' +
    ...
    _showRegisterPaint() {
        this.setState({
            paintInfo: 'registerPaint(name, class) —— 在 Paint Worklet 中注册绘制类\n\n' +
    ...
    _showWorkletOverview() {
        this.setState({
            paintInfo: 'CSS Houdini 三大 Worklet 概述：\n\n' +
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.paintWorklet ? 'success' : 'error' }, caps.paintWorklet ? 'paintWorklet ✓' : '不可用'), h(Tag, { color: 'primary' }, 'registerPaint / paint()')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '(CSS as any).paintWorklet.addModule(url) 加载 Paint Worklet 模块；worklet 脚本内用 registerPaint(name, class) 注册绘制类，类需实现 static get inputProperties() / inputArguments() / contextOptions() 与 paint(ctx, geometry, properties, args) 方法。CSS 中用 background: paint(name, ...args) 调用。绘制在独立线程执行，输入属性变化时自动重绘。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('addModule', { type: 'primary', size: 'sm', disabled: !caps.paintWorklet, onClick: () => this._addPaintModule() }), this._btn('registerPaint 类', { type: 'primary', size: 'sm', onClick: () => this._showRegisterPaint() }), this._btn('三大 Worklet 概述', { size: 'sm', onClick: () => this._showWorkletOverview() })),
                h('div', { class: 'fs-sm text-secondary' }, 'Paint Worklet 说明：'),
                h('pre', { class: 'code-block', style: { maxHeight: '280px', overflow: 'auto' } }, h('code', {}, s.paintInfo || '（点击「addModule」或「registerPaint 类」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '160px', overflow: 'auto' } }, h('code', {}, `// paint-worklet.js（运行在 Worklet 线程）
...
.el { background: paint(circle); --radius: 30px; }        // CSS 调用`)),
    ...
    _typedOMSetGet() {
        ...
        if (!caps.attributeStyleMap) {
            ...
            this.setState({
                typedOMInfo: 'element.attributeStyleMap 不可用\n' +
        ...
        try {
            ...
            this.setState({
                typedOMInfo: `element.attributeStyleMap.set(prop, value) / .get(prop) ✓\n` +
    ...
    _cssFactories() {
        ...
        if (!caps.cssPx) {
            ...
            this.setState({
                typedOMInfo: 'CSS.px / CSS.percent 工厂方法不可用\n' +
    ...
    _computedStyleMap() {
        ...
        if (!caps.computedStyleMap) {
            ...
            this.setState({
                typedOMInfo: 'element.computedStyleMap() 不可用\n' +
        ...
        try {
            ...
            this.setState({
                typedOMInfo: `element.computedStyleMap() → StylePropertyMap ✓\n` +
                    `前 8 项计算样式：\n${entries.join('\n')}\n\n` +
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.attributeStyleMap ? 'success' : 'error' }, caps.attributeStyleMap ? 'Typed OM ✓' : '不可用'), h(Tag, { color: 'primary' }, 'attributeStyleMap / computedStyleMap')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'CSS Typed OM 用类型化对象替代字符串表示 CSS 值。element.attributeStyleMap.set(prop, value) / .get(prop) 类型化读写内联样式；element.computedStyleMap() 返回可迭代的计算样式 StylePropertyMap。CSS.px(n) / CSS.percent(n) / CSS.number(n) 等工厂返回 CSSUnitValue；CSSMathSum / CSSMathProduct 等表示数学表达式。相比 el.style.width = "100px"，Typed OM 保留类型，避免字符串 parse。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('attributeStyleMap', { type: 'primary', size: 'sm', disabled: !caps.attributeStyleMap, onClick: () => this._typedOMSetGet() }), this._btn('CSS.px/percent 工厂', { type: 'primary', size: 'sm', disabled: !caps.cssPx, onClick: () => this._cssFactories() }), this._btn('computedStyleMap', { size: 'sm', disabled: !caps.computedStyleMap, onClick: () => this._computedStyleMap() })),
                h('div', { class: 'fs-sm text-secondary' }, 'Typed OM 结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, s.typedOMInfo || '（点击「attributeStyleMap」或「CSS.px/percent 工厂」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '160px', overflow: 'auto' } }, h('code', {}, `// 类型化内联样式读写
el.attributeStyleMap.set('width', CSS.px(100));
el.attributeStyleMap.get('width');   // CSSUnitValue { value:100, unit:'px' }
// 工厂方法与数学表达式
...
el.attributeStyleMap.get('width').value; // 直接 100`)),
    ...
    _detectSelectors() {
        ...
        try {
            const sel = (s) => CSS.supports(`selector(${s})`);
            const results = [
                `:has(*)         = ${sel(':has(*)')}`,
            ...
            this.setState({
                selectorInfo: `CSS.supports('selector(...)') → boolean（Selectors Level 4 检测）\n${results.join('\n')}\n\n` +
    ...
    _testSelectors() {
        try {
            ...
            this.setState({
                selectorInfo: `实际 DOM 匹配测试（host 内：section×2 + article×1，含 h2/p/img）\n${lines.join('\n')}\n\n` +
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.supports ? 'success' : 'error' }, caps.supports ? 'CSS.supports ✓' : '不可用'), h(Tag, { color: 'primary' }, 'Selectors Level 4')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, ':has(rel) 是 CSS Selectors Level 4 的关系选择器，匹配包含指定子/兄弟元素的元素（:has(> .child) 直接子、:has(+ .sibling) 相邻兄弟、:has(~ .general) 后续兄弟）。:is(a,b,c) 匹配任一并保持特异性；:where(a,b) 匹配任一但特异性为 0（便于被覆盖）；:not(x) 取反。CSS.supports(\'selector(...)\') 可同步检测选择器是否被引擎支持。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('CSS.supports 检测', { type: 'primary', size: 'sm', disabled: !caps.supports, onClick: () => this._detectSelectors() }), this._btn('querySelector 实测', { type: 'primary', size: 'sm', onClick: () => this._testSelectors() })),
                h('div', { class: 'fs-sm text-secondary' }, '选择器检测结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '280px', overflow: 'auto' } }, h('code', {}, s.selectorInfo || '（点击「CSS.supports 检测」或「querySelector 实测」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '150px', overflow: 'auto' } }, h('code', {}, `/* :has() 关系选择器 */
...
input:not([disabled]) { cursor: pointer; }       /* :not 取反 */`)),
    ...
    _detectContainer() {
        ...
        try {
            const ct = (v) => `container-type: ${v.padEnd(11)}= ${CSS.supports(`container-type: ${v}`)}`;
            const cq = (u) => `width: 10${u.padEnd(6)}             = ${CSS.supports('width', `10${u}`)}`;
            const results = [
                ct('size'), ct('inline-size'), ct('normal'),
                cq('cqw'), cq('cqi'), cq('cqmin'), cq('cqmax'),
            ];
            this.setState({
                containerInfo: `CSS.supports 检测 Container Queries 支持：\n${results.join('\n')}\n\n` +
    ...
    _createContainer() {
        try {
            ...
            try {
                ...
                rulesInfo = `读取 cssRules 抛错：${e.name}（jsdom 可能不解析 @container）`;
            }
            this.setState({
                containerInfo: `已创建容器元素（container-type: inline-size, container-name: my-box, width: 400px）✓\n` +
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.supports ? 'success' : 'error' }, caps.supports ? 'supports ✓' : '不可用'), h(Tag, { color: 'primary' }, 'container-type / @container / cqw')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'Container Queries 让组件根据自身容器尺寸（而非视口）响应样式。container-type: size | inline-size | normal 声明查询容器；container-name 命名容器；@container name (inline-size > Npx) { ... } 定义容器查询规则。容器查询单位 cqw/cqh/cqi/cqb/cqmin/cqmax 基于容器尺寸。相比 @media 基于视口，@container 基于组件容器，更适合组件化设计系统。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('CSS.supports 检测', { type: 'primary', size: 'sm', disabled: !caps.supports, onClick: () => this._detectContainer() }), this._btn('创建容器 + @container', { type: 'primary', size: 'sm', onClick: () => this._createContainer() })),
                h('div', { class: 'fs-sm text-secondary' }, 'Container Queries 结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '280px', overflow: 'auto' } }, h('code', {}, s.containerInfo || '（点击「CSS.supports 检测」或「创建容器 + @container」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '160px', overflow: 'auto' } }, h('code', {}, `.sidebar { container-type: inline-size; container-name: sidebar; }
...
.card { padding: 2cqi; }       /* 内联轴的 2% */`)),
    ...
    _injectLayers() {
        try {
            ...
            try {
                ...
                rulesInfo = `读取 cssRules 抛错：${e.name}`;
            }
            this.setState({
                layerInfo: `已注入 @layer 规则 ✓\n` +
    ...
    _inspectLayers() {
        ...
        try {
            ...
            if (style && style.sheet) {
                try {
                    rulesText = Array.from(style.sheet.cssRules).map((r, i) => `  [${i}] ${r.constructor.name}${r.name ? ` name="${r.name}"` : ''}${r.cssText ? `\n      cssText: ${r.cssText.slice(0, 80)}` : ''}`).join('\n');
                }
                catch (e) {
            ...
            this.setState({
                layerInfo: `CSS.supports('@layer base') = ${layerSupported}\n\n` +
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.supports ? 'success' : 'error' }, caps.supports ? 'supports ✓' : '不可用'), h(Tag, { color: 'primary' }, '@layer / 层优先级')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '@layer 声明级联层并显式控制优先级。@layer base, components, utilities; 声明层顺序（后者优先级更高）；@layer name { ... } 定义层内容。优先级规则：越后声明的层优先级越高；未分层样式优先级高于所有分层样式；!important 反转层顺序。@layer 让开发者摆脱特异性 hack，便于集成第三方 CSS（如把第三方样式放进低优先级层）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('注入 @layer 规则', { type: 'primary', size: 'sm', onClick: () => this._injectLayers() }), this._btn('内省 cssRules', { type: 'primary', size: 'sm', disabled: !caps.supports, onClick: () => this._inspectLayers() })),
                h('div', { class: 'fs-sm text-secondary' }, '@layer 级联层结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '300px', overflow: 'auto' } }, h('code', {}, s.layerInfo || '（点击「注入 @layer 规则」或「内省 cssRules」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '160px', overflow: 'auto' } }, h('code', {}, `/* 声明层顺序：base < components < utilities */
@layer base, components, utilities;
...
.btn { color: red; }  /* 未分层 —— 永远胜过分层样式 */`)),
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'page api-lab-page' }, h('h2', { class: 'section-title' }, 'CSS Houdini 与现代 CSS 实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, 'CSS Houdini 把 CSS 引擎底层暴露给 JS（Worklets / registerProperty / Typed OM）；现代 CSS 带来 :has() 选择器、Container Queries、@layer 级联层、oklch/color-mix 颜色等。本页演示 registerProperty、Paint Worklet、Typed OM、关系选择器、容器查询、级联层。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderCard6(), this._renderLogPanel());
    }
}
//# sourceMappingURL=CSSHoudiniPage.js.map