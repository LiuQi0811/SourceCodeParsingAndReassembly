...
export class FetchPage extends Page {
    ...
    renderPage() {
        ...
        return [
            ...
            // 基础 fetch
            h(Card, { title: '基础请求 + Headers / 状态码', extra: h(Tag, { color: 'primary' }, 'GET') }, h('div', { class: 'flex flex-col gap-sm' }, h('div', { class: 'flex items-center gap-sm flex-wrap' }, h('span', { class: 'fs-sm text-secondary' }, 'URL'), urlInput.render(), this._btn('发起请求', { type: 'primary', size: 'sm', onClick: () => this._fetchBasic() })), this.state.status != null ? h('div', { class: 'flex items-center gap-md fs-sm' }, h('span', {}, h('strong', {}, '状态：'), h(Tag, { color: this.state.status < 400 ? 'success' : 'error' }, String(this.state.status))), h('span', {}, h('strong', {}, '耗时：'), formatDuration(this.state.timing ?? 0))) : '', this.state.headers.length > 0 ? h('div', {}, h('div', { class: 'fs-sm text-secondary mb-sm' }, '响应头：'), h('div', { class: 'log-panel' }, ...this.state.headers.map((h_) => h('div', { class: 'log-panel__line' }, h('span', { class: 'log-panel__tag log-panel__tag--push' }, h_.k), h('span', {}, h_.v))))) : '', this.state.result ? h('pre', { class: 'code-block', style: { maxHeight: '260px' } }, this.state.result) : '')),
            // 流式 + 中止
            h(Card, { title: 'ReadableStream 流式读取 + AbortController 中止', extra: h(Tag, { color: this.state.abortableRunning ? 'error' : 'default' }, this.state.abortableRunning ? '运行中' : '空闲') }, h('div', { class: 'flex flex-col gap-sm' }, h('div', { class: 'flex flex-wrap gap-sm' }, this._btn('启动流式请求', { type: 'primary', size: 'sm', onClick: () => this._fetchWithAbort(), disabled: this.state.abortableRunning }), this._btn('abort() 中止', { danger: true, size: 'sm', onClick: () => this._abortAll(), disabled: !this.state.abortableRunning }), this._btn('AbortSignal.timeout(1)', { size: 'sm', onClick: () => this._fetchTimeout() })), this.state.streamingText ? h('div', {}, h('div', { class: 'fs-sm text-secondary mb-sm' }, `流式接收中（${this.state.streamingText.length} bytes）：`), h('pre', { class: 'code-block', style: { maxHeight: '200px' } }, this.state.streamingText.slice(-2000))) : '')),
            // 日志
            h(Card, { title: '请求日志', extra: h('span', { class: 'fs-sm text-tertiary' }, '实时') }),
            h('div', { class: 'log-panel' }, ...this.state.logs.map((log) => h('div', { class: 'log-panel__line' }, h('span', { class: 'log-panel__time' }, log.time), h('span', { class: `log-panel__tag log-panel__tag--${log.type === 'info' ? 'push' : log.type}` }, log.type), h('span', {}, log.content)))),
        ];
    }
}
