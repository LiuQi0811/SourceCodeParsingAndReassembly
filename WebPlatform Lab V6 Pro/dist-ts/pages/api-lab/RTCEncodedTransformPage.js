...
export class RTCEncodedTransformPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!f.rtcRtpScriptTransform)
            this._addLog('warn', 'RTCRtpScriptTransform 不可用（jsdom 无 WebRTC，真实浏览器需 Chrome 86+ / Edge 86+）');
        if (!f.senderTransform)
            this._addLog('warn', 'RTCRtpSender.prototype.transform 不可用（新 API 标志位）');
        if (!f.createEncodedStreams)
            this._addLog('warn', 'RTCRtpSender.createEncodedStreams 不可用（旧 Insertable Streams API，Chrome 86-105）');
        if (!f.encodedVideoFrame)
            this._addLog('warn', 'RTCEncodedVideoFrame 不可用（仅 worker 内可见）');
        if (!f.videoFrame)
            this._addLog('warn', 'VideoFrame 不可用（WebCodecs，Chrome 94+ / Safari 16.4+）');
        if (!f.webCrypto)
            this._addLog('warn', 'crypto.subtle 不可用（SFrame 加密依赖，需 HTTPS 或 localhost）');
...
【工作线程模型（RTCRtpScriptTransform）】
  ...
  Worker（rtc-transform-worker.js）：
    self.onrtcrtpscripttransform = (event: any) => {
    ...
    controller.enqueue(frame);
        ...
        const card = new Card({
            ...
            children: [
                ...
                h('p', { class: 'fs-sm text-secondary' }, 'WebRTC Insertable Streams（可插入流）允许 JS 在 RTP 编解码器输出与 RTP 打包之间插入自定义处理管线。API 三阶段演进：createEncodedStreams()（旧，Chrome 86-105）→ RTCEncodedTransform（过渡，主线程）→ RTCRtpScriptTransform（新，Worker 隔离，Chrome 105+，规范方向）。新 API 处理逻辑跑在 DedicatedWorkerGlobalScope，主线程仅做绑定不阻塞。与 WebCodecs 互补：WebCodecs 通用编解码，Insertable Streams 处理 WebRTC 管线注入。Safari/Firefox 暂不支持。'),
                h('div', { class: 'rtc-et-pipeline' }, h('span', { class: 'rtc-et-pipe-node' }, 'Encoder'), h('span', { class: 'rtc-et-pipe-arrow' }, '→'), h('span', { class: 'rtc-et-pipe-node rtc-et-pipe-node--worker' }, 'RTCRtpScriptTransform (Worker)'), h('span', { class: 'rtc-et-pipe-arrow' }, '→'), h('span', { class: 'rtc-et-pipe-node' }, 'RTP Packetizer'), h('span', { class: 'rtc-et-pipe-arrow' }, '→'), h('span', { class: 'rtc-et-pipe-node' }, 'Network')),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行概念演示', { type: 'primary', size: 'sm', onClick: () => this._runConceptDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '320px', overflow: 'auto' } }, h('code', {}, code)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.conceptInfo || '（点击按钮查看概念与演进完整说明）')),
            ],
        });
...
【双向绑定（发送 + 接收）】
  ...
  pc.getSenders().forEach((s: any) => { if (s.track) s.transform = sendTransform; });
  pc.getReceivers().forEach((r: any) => { if (r.track) r.transform = recvTransform; });
...
receiver.transform = new RTCRtpScriptTransform(worker, 'decrypt', {
  ...
  mode: 'aes-gcm',
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'RTCRtpSender.transform / RTCRtpReceiver.transform 绑定 RTCRtpScriptTransform 实例。构造签名 new RTCRtpScriptTransform(worker, name, options)：worker 是 DedicatedWorker 实例（同源 .js），name 字符串用于 worker 内路由，options 任意可结构化克隆的值透传给 transformer.options。options 不能传函数 / DOM / 闭包（DataCloneError）。一个 transform 实例只能绑一个 sender 或 receiver，绑定后所有新编码帧自动进 worker。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行绑定演示', { type: 'primary', size: 'sm', onClick: () => this._runTransformBindingDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '320px', overflow: 'auto' } }, h('code', {}, code)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.transformBindingInfo || '（点击按钮查看 transform 绑定完整说明）')),
            ],
        });
        this.registerChild(card);
        return card.render();
...
【pipeThrough TransformStream 处理每帧】
  self.onrtcrtpscripttransform = (event: any) => {
    ...
    readable
      ...
      .pipeTo(writable)
      .catch((err: any) => console.error('管道错误:', err));
  };
...
【多绑定共用一个 Worker】
  ...
  const worker = new Worker('transform-worker.js');
  pc.getSenders().forEach((s: any, i: number) => {
    if (s.track) {
  ...
  readable
    ...
    .catch((err: any) => console.error('管道错误:', err));
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'Worker 内 self.onrtcrtpscripttransform 事件接收 RTCRtpScriptTransformer 实例：transformer.name（主线程构造时传入的路由名）、transformer.options（结构化克隆透传）、transformer.readable（ReadableStream<RTCEncodedFrame> 编码后帧输入）、transformer.writable（WritableStream 处理后帧输出）。端到端管道：readable.pipeThrough(new TransformStream({ transform })).pipeTo(writable)。transform 函数处理每帧后必须 controller.enqueue(frame)，否则下游收不到画面。'),
                h('div', { class: 'rtc-et-pipeline' }, h('span', { class: 'rtc-et-pipe-node' }, 'readable'), h('span', { class: 'rtc-et-pipe-arrow' }, '→'), h('span', { class: 'rtc-et-pipe-node rtc-et-pipe-node--worker' }, 'TransformStream'), h('span', { class: 'rtc-et-pipe-arrow' }, '→'), h('span', { class: 'rtc-et-pipe-node' }, 'writable')),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 Worker 事件演示', { type: 'primary', size: 'sm', onClick: () => this._runWorkerEventDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '360px', overflow: 'auto' } }, h('code', {}, code)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.workerEventInfo || '（点击按钮查看 Worker 端事件完整说明）')),
            ],
        });
        this.registerChild(card);
        return card.render();
...
transform: (frame: any, controller: any) => {
  ...
  controller.enqueue(frame);    // 必须 enqueue，否则下游收不到
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'RTCEncodedVideoFrame / RTCEncodedAudioFrame 接口（仅 worker 内可见）：type（key/delta）、timestamp（微秒）、data（ArrayBuffer 编码后字节）、getMetadata()（返回 synchronizationSource/contributingSources/frameId/width/height/spatialIndex/temporalIndex 等）、getContributingSources()（CSRC 数组）。data 可写：frame.data = newArrayBuffer 修改帧内容，修改后必须 enqueue。关键帧（key）处理错误会导致解码端长时间花屏。timestamp 微秒级，与 Date.now()（毫秒）单位不同。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 Frame 接口演示', { type: 'primary', size: 'sm', onClick: () => this._runEncodedFrameDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '360px', overflow: 'auto' } }, h('code', {}, code)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.encodedFrameInfo || '（点击按钮查看 EncodedFrame 接口完整说明）')),
            ],
        });
        this.registerChild(card);
        return card.render();
...
【密钥轮换（forward secrecy）】
  ...
  async function rotateKey(oldKey: any) {
    ...
    transform: async (frame: any, controller: any) => {
      ...
      controller.enqueue(frame);
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'SFrame（Secure Frame，IETF draft-omara-sframe）专为实时视频会议设计的端到端加密方案：在 RTP 编码后帧级别加密（非包级别 SRTP），与 SFU 兼容（SFU 不解密即可转发）。密钥协商用 ECDH（P-256）通过 DataChannel 交换公钥派生共享密钥，AES-GCM 256 加密每帧。nonce 管理：ssrc(4) + 单调递增 counter(8) = 12 字节，重复 nonce 会致命（AES-GCM 安全性崩溃）。crypto.subtle 异步，transform 需 async。GCM tag 校验失败必须丢帧。密钥轮换用 HKDF 派生 + 重叠窗口。'),
                h('div', { class: 'rtc-et-pipeline' }, h('span', { class: 'rtc-et-pipe-node' }, 'Encoder'), h('span', { class: 'rtc-et-pipe-arrow' }, '→'), h('span', { class: 'rtc-et-pipe-node rtc-et-pipe-node--crypto' }, 'AES-GCM 加密'), h('span', { class: 'rtc-et-pipe-arrow' }, '→'), h('span', { class: 'rtc-et-pipe-node' }, 'RTP / SFU'), h('span', { class: 'rtc-et-pipe-arrow' }, '→'), h('span', { class: 'rtc-et-pipe-node rtc-et-pipe-node--crypto' }, 'AES-GCM 解密'), h('span', { class: 'rtc-et-pipe-arrow' }, '→'), h('span', { class: 'rtc-et-pipe-node' }, 'Decoder')),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 SFrame 加密演示', { type: 'primary', size: 'sm', onClick: () => this._runSframeDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '420px', overflow: 'auto' } }, h('code', {}, code)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.sframeInfo || '（点击按钮查看 SFrame 端到端加密完整说明）')),
            ],
        });
        this.registerChild(card);
        return card.render();
...
【RTCEncodedVideoFrame → EncodedVideoChunk → VideoFrame（解码）】
  ...
  function encodedFrameToChunk(frame: any) {
  ...
  let decoder = new VideoDecoder({
    ...
    error: (e: any) => console.error('解码错误:', e),
  });
  decoder.configure({
...
【VideoFrame → EncodedVideoChunk → RTCEncodedVideoFrame（重编码）】
  ...
  let encoder = new VideoEncoder({
    ...
    error: (e: any) => console.error('编码错误:', e),
  });
  encoder.configure({
    codec: 'vp8',  // 转码为 VP8
    width: 1280,
    ...
    const encoder = new VideoEncoder({
      output: (chunk: any, meta: any) => {
        ...
        const frame = new RTCEncodedVideoFrame({
          ...
          data: (chunk as any).byteLength ? chunk.copyTo(new Uint8Array((chunk as any).byteLength)) : new Uint8Array(0),
        });
        writer.write(frame);
      },
      error: (e: any) => console.error(e),
    ...
    readable.pipeThrough(new TransformStream({
      start: async (controller: any) => {
        const ws = new WritableStream({
          write: (f: any) => decoder.decode(encodedFrameToChunk(f)),
        });
        writer = ws.getWriter();
      },
      transform: (frame: any, controller: any) => {
...
【OffscreenCanvas 背景虚化（worker 内）】
  ...
  function blurBackground(vframe: any) {
...
encoder.configure({ codec: 'vp8', width: 1280, height: 720, bitrate: 1_000_000 });`;
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'RTCEncodedVideoFrame（已编码，H.264 NALU / VP8）↔ VideoFrame（WebCodecs 未编码，YUV/RGB）互转：RTCEncodedVideoFrame → EncodedVideoChunk（type/timestamp/data 直接复用）→ VideoDecoder.decode() → VideoFrame → Canvas/AI 处理 → VideoEncoder.encode() → EncodedVideoChunk → 重封装写回 writable。典型场景：H.264 → VP8 转码、实时背景虚化（解码 → OffscreenCanvas blur → 重编码）、视频分析（解码后送 AI 模型）。VideoFrame.close() 必须调用释放显存。串联会增加 1-2 帧延迟。'),
                h('div', { class: 'rtc-et-pipeline' }, h('span', { class: 'rtc-et-pipe-node' }, 'EncodedFrame'), h('span', { class: 'rtc-et-pipe-arrow' }, '→'), h('span', { class: 'rtc-et-pipe-node rtc-et-pipe-node--worker' }, 'VideoDecoder'), h('span', { class: 'rtc-et-pipe-arrow' }, '→'), h('span', { class: 'rtc-et-pipe-node' }, 'VideoFrame'), h('span', { class: 'rtc-et-pipe-arrow' }, '→'), h('span', { class: 'rtc-et-pipe-node rtc-et-pipe-node--worker' }, 'Canvas/AI'), h('span', { class: 'rtc-et-pipe-arrow' }, '→'), h('span', { class: 'rtc-et-pipe-node rtc-et-pipe-node--worker' }, 'VideoEncoder')),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 WebCodecs 互转演示', { type: 'primary', size: 'sm', onClick: () => this._runWebCodecsDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '420px', overflow: 'auto' } }, h('code', {}, code)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.webcodecsInfo || '（点击按钮查看 WebCodecs 互转完整说明）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _runScenarioDemo() {
        ...
        if (!f.videoFrame)
            this._addLog('warn', 'VideoFrame 不可用，背景虚化 / 水印演示仅展示代码');
        if (!f.trackProcessor)
            this._addLog('warn', 'MediaStreamTrackProcessor 不可用，本地摄像头处理管线降级');
        if (!f.audioData)
            this._addLog('warn', 'AudioData 不可用，音频降噪 WASM 管线仅展示代码');
        const info = `
===== 实战场景：背景虚化 / 水印注入 / 音频降噪 =====
...
【场景 2：水印注入（Insertable Streams + NALU SEI）】
  ...
  // H.264 SEI NALU 构造
  function injectSEI(frameData: any,  text: any) {
...
【场景 3：音频降噪（AudioData 走 WASM RNNoise）】
  ...
  self.onmessage = async (e: any) => {
    ...
    readable.pipeThrough(new TransformStream({
      transform: (audioData: any, controller: any) => {
    ...
    transform: (frame: any, controller: any) => {
      ...
      frame.close();  // 必须 close 释放显存
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '三大实战场景：1）实时背景虚化用 MediaStreamTrackProcessor 拆分摄像头流为 VideoFrame 流，worker 内 OffscreenCanvas 缩小再放大近似 box blur，进阶用 MediaPipe Selfie Segmentation 精确人体分割；2）水印注入在 RTCEncodedVideoFrame 关键帧注入 H.264 SEI NALU（NAL type 6 user_data_unregistered），纯字节操作 <1ms；3）音频降噪用 MediaStreamTrackProcessor 拆分音频为 AudioData 流，worker 内调用 WASM RNNoise（需 480ms 帧长）降噪后 MediaStreamTrackGenerator 重合成。性能：背景虚化 10-20ms、水印 <1ms、降噪 5-10ms、转码 20-40ms、SFrame 1-3ms。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行实战场景演示', { type: 'primary', size: 'sm', onClick: () => this._runScenarioDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '420px', overflow: 'auto' } }, h('code', {}, code)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.scenarioInfo || '（点击按钮查看实战场景完整说明）')),
            ],
        });
        this.registerChild(card);
        return card.render();
...
【能力检测：新 API 优先 + 旧 API 降级】
  ...
  function attachTransform(sender: any,  worker: any,  name: any,  options: any) {
    if (hasNewAPI()) {
...
【降级到 createEncodedStreams()（旧 API）】
  ...
  function legacyEncrypt(sender: any,  key: any) {
    const { readable, writable } = sender.createEncodedStreams();
    let nonceCounter = 0;
    readable.pipeThrough(new TransformStream({
      transform: async (frame: any, controller: any) => {
...
const caps = {
  ...
  webCrypto: typeof crypto !== 'undefined' && !!crypto.subtle,
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '能力检测优先级：新 API（RTCRtpScriptTransform + \'transform\' in RTCRtpSender.prototype，Chrome 105+）→ 旧 API（createEncodedStreams，Chrome 86-105 主线程处理）→ MediaStreamTrackProcessor + WebCodecs 自建管线（Safari 17+）→ 服务端转码兜底（Safari<17 / Firefox）。\'transform\' in prototype 是新 API 标志位（getter/setter，不能用 typeof 检测）。与 MediaStreamTrackProcessor 协同：本地流 TrackProcessor 拆 VideoFrame 处理 → 编码 → Insertable Streams 加密。createEncodedAudioStreams 是音频专用旧 API。'),
                h('div', { class: 'rtc-et-matrix' }, h('div', { class: 'rtc-et-matrix-cell' }, h('b', {}, 'Chrome 105+'), h('br'), '✓ 新 API', h('br'), '✓ 旧 API（降级）'), h('div', { class: 'rtc-et-matrix-cell' }, h('b', {}, 'Chrome 86-104'), h('br'), '✗ 新 API', h('br'), '✓ 旧 API'), h('div', { class: 'rtc-et-matrix-cell' }, h('b', {}, 'Safari 17+'), h('br'), '✗ 新 API', h('br'), '✗ 旧 API'), h('div', { class: 'rtc-et-matrix-cell' }, h('b', {}, 'Firefox'), h('br'), '✗ 新 API', h('br'), '✗ 旧 API')),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行能力检测演示', { type: 'primary', size: 'sm', onClick: () => this._runDetectionDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '440px', overflow: 'auto' } }, h('code', {}, code)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.detectionInfo || '（点击按钮查看能力检测与降级完整说明）')),
            ],
        });
        this.registerChild(card);
        return card.render();
...
//# sourceMappingURL=RTCEncodedTransformPage.js.map