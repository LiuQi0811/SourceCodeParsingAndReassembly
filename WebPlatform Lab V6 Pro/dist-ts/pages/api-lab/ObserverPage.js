// ObserverPage.ts —— Observer 三剑客：Intersection / Mutation / Resize
...
export class ObserverPage extends Page {
    ...
    _setupResize() {
        ...
        this._resizeObserver = new ResizeObserver((entries) => {
            for (const e of (entries || [])) {
                ...
                const borderBoxSize = e.borderBoxSize?.[0]?.inlineSize;
                this.setState({
                    boxWidth: Math.round(cr.width),
                    resizeLog: [...this.state.resizeLog, {
                            time: formatTime(),
                            text: `${Math.round(cr.width)} × ${Math.round(cr.height)}  (borderBox: ${borderBoxSize ? borderBoxSize.toFixed(0) : '?'}px)`,
                        }].slice(-15),
                });
            }
        });
    ...
    renderPage() {
        return [
            ...
            h(Card, { title: 'IntersectionObserver', extra: h(Tag, { color: 'primary' }, `${this.state.intersectionRatio}%`) }, h('p', { class: 'fs-sm text-secondary' }, '滚动下方容器，目标方块进入/离开视口时会触发回调并计算交叉比例。'), h('div', {
            ...
            }, `${this.state.intersectionRatio}%`)), h('div', { class: 'log-panel mt-sm', style: { maxHeight: '120px' } }, ...(this.state.intersectionLog || []).map((l) => h('div', { class: 'log-panel__line' }, h('span', { class: 'log-panel__time' }, l.time), h('span', { class: 'log-panel__tag log-panel__tag--push' }, 'intersect'), h('span', {}, l.text))))),
            // MutationObserver
            h(Card, { title: 'MutationObserver', extra: h(Tag, { color: 'success' }, `${this.state.mutationCount} 次`) }, h('p', { class: 'fs-sm text-secondary' }, '观察下方容器内的 DOM 变化（添加/删除节点、属性变更、文本变更）。'), h('div', { class: 'flex gap-sm mb-sm' }, this._btn('+ 添加节点', { size: 'sm', type: 'primary', onClick: () => this._mutate('add') }), this._btn('- 删除节点', { size: 'sm', onClick: () => this._mutate('remove') }), this._btn('改属性', { size: 'sm', onClick: () => this._mutate('attr') }), this._btn('改文本', { size: 'sm', onClick: () => this._mutate('text') }), this._btn('清空', { size: 'sm', danger: true, onClick: () => this._mutate('clear') })), h('div', {
                id: 'mutation-target',
                'data-version': '0',
                class: 'mutation-target',
            }, h('span', { 'data-mut-text': true }, '初始文本')), h('div', { class: 'log-panel mt-sm', style: { maxHeight: '120px' } }, ...(this.state.mutationLog || []).map((l) => h('div', { class: 'log-panel__line' }, h('span', { class: 'log-panel__time' }, l.time), h('span', { class: 'log-panel__tag log-panel__tag--info' }, 'mutate'), h('span', {}, l.text))))),
            // ResizeObserver
            h(Card, { title: 'ResizeObserver', extra: h(Tag, { color: 'warning' }, `宽 ${this.state.boxWidth}px`) }, h('p', { class: 'fs-sm text-secondary' }, '拖动下方滑块改变方块宽度，ResizeObserver 会捕获 contentRect 变化。'), h('div', { class: 'flex gap-sm mb-sm' }, this._btn('- 20px', { size: 'sm', onClick: () => this._resize(-20) }), this._btn('+ 20px', { size: 'sm', onClick: () => this._resize(20) })), h('input', {
            ...
            }, `${this.state.boxWidth}px`), h('div', { class: 'log-panel mt-sm', style: { maxHeight: '120px' } }, ...(this.state.resizeLog || []).map((l) => h('div', { class: 'log-panel__line' }, h('span', { class: 'log-panel__time' }, l.time), h('span', { class: 'log-panel__tag log-panel__tag--error' }, 'resize'), h('span', {}, l.text))))),
        ];
    }
}
//# sourceMappingURL=ObserverPage.js.map