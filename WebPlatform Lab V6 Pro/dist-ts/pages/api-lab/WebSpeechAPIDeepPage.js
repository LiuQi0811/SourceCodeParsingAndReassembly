...
export class WebSpeechAPIDeepPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!f.speechSynthesis)
            this._addLog('warn', 'speechSynthesis 不可用（jsdom 无 TTS，真实浏览器需 Chrome/Edge/Safari）');
        if (!f.speechSynthesisUtterance)
            this._addLog('warn', 'SpeechSynthesisUtterance 不可用');
        if (!f.speechRecognition)
            this._addLog('warn', 'SpeechRecognition 不可用（需 webkit 前缀，Chrome/Edge 支持，Firefox 默认禁用需 flag）');
        if (!f.speechGrammarList)
            this._addLog('warn', 'SpeechGrammarList 不可用（Chrome 部分支持）');
...
navigator.permissions.query({ name: 'microphone' } as any)
  .then((r: any) => console.log('mic permission:', r.state));`)),
                ...
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.overviewInfo || '（点击按钮查看 Web Speech API 概述与能力检测完整说明）')),
            ],
        });
    ...
    `);
        ...
        if (f.speechSynthesis && f.speechSynthesisUtterance) {
            try {
                ...
                u.onerror = (e) => this._addLog('warn', `TTS 朗读错误 onerror: ${e.error}`);
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'window.speechSynthesis 单例对象，speak(utterance)/cancel()/pause()/resume() 方法 + pending/speaking/paused 状态。new SpeechSynthesisUtterance(text) 创建话语，属性 text/lang/pitch(0-2)/rate(0.1-10)/volume(0-1)/voice。8 个事件：onstart/onend/onerror/onpause/onresume/onmark/onboundary（word boundary 词边界）。getVoices() 获取可用语音，voiceschanged 事件处理异步加载。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('朗读示例文本', { type: 'primary', size: 'sm', onClick: () => this._runSynthBasicDemo() })),
                h('div', { class: 'ws-waveform' }, h('div', { class: 'ws-wave-bar', style: { animationDelay: '0s' } }), h('div', { class: 'ws-wave-bar', style: { animationDelay: '0.1s' } }), h('div', { class: 'ws-wave-bar', style: { animationDelay: '0.2s' } }), h('div', { class: 'ws-wave-bar', style: { animationDelay: '0.3s' } }), h('div', { class: 'ws-wave-bar', style: { animationDelay: '0.4s' } }), h('div', { class: 'ws-wave-bar', style: { animationDelay: '0.5s' } }), h('div', { class: 'ws-wave-bar', style: { animationDelay: '0.6s' } }), h('div', { class: 'ws-wave-bar', style: { animationDelay: '0.7s' } }), h('div', { class: 'ws-wave-bar', style: { animationDelay: '0.8s' } }), h('div', { class: 'ws-wave-bar', style: { animationDelay: '0.9s' } }), h('div', { class: 'ws-wave-bar', style: { animationDelay: '0.2s' } }), h('div', { class: 'ws-wave-bar', style: { animationDelay: '0.4s' } })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `const synth = window.speechSynthesis;
...
// synth.getVoices(); // 获取可用语音（首次可能为空）`)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.synthBasicInfo || '（点击按钮查看 SpeechSynthesis 合成基础完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['getVoices', f.speechSynthesis]]), h(Tag, { color: 'primary' }, 'Voice 深潜')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'SpeechSynthesisVoice 属性：name/lang/voiceURI/localService（本地 vs 云端）/default。getVoices() 返回 Voice 数组，按 lang 筛选（zh-CN/en-US/ja-JP），按 localService 筛选本地无网络延迟。utterance.voice = selectedVoice 设置。voice 加载时机：首次 getVoices 可能空，监听 voiceschanged。不同 OS/浏览器 voice 差异：Windows SAPI、macOS NSSpeechSynthesizer、Android Google TTS。实战：多语言切换、voice 偏好持久化（localStorage 存 voiceURI）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('获取语音列表', { type: 'primary', size: 'sm', onClick: () => this._runVoiceDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '语音列表示例（mock）：'),
                h('div', { class: 'ws-voice-list' }, h('div', { class: 'ws-voice-item' }, h('span', { class: 'ws-voice-name' }, 'Microsoft Huihui'), h('span', { class: 'ws-voice-lang' }, 'zh-CN'), h('span', { class: 'ws-voice-tag ws-voice-tag--local' }, '本地'), h('span', { class: 'ws-voice-tag ws-voice-tag--default' }, '默认')), h('div', { class: 'ws-voice-item' }, h('span', { class: 'ws-voice-name' }, 'Google 普通话'), h('span', { class: 'ws-voice-lang' }, 'zh-CN'), h('span', { class: 'ws-voice-tag ws-voice-tag--cloud' }, '云端')), h('div', { class: 'ws-voice-item' }, h('span', { class: 'ws-voice-name' }, 'Microsoft David'), h('span', { class: 'ws-voice-lang' }, 'en-US'), h('span', { class: 'ws-voice-tag ws-voice-tag--local' }, '本地')), h('div', { class: 'ws-voice-item' }, h('span', { class: 'ws-voice-name' }, 'Samantha'), h('span', { class: 'ws-voice-lang' }, 'en-US'), h('span', { class: 'ws-voice-tag ws-voice-tag--local' }, '本地')), h('div', { class: 'ws-voice-item' }, h('span', { class: 'ws-voice-name' }, 'Kyoko'), h('span', { class: 'ws-voice-lang' }, 'ja-JP'), h('span', { class: 'ws-voice-tag ws-voice-tag--local' }, '本地'))),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `const synth = window.speechSynthesis;
let voices = synth.getVoices();
...
localStorage.setItem('preferred-voice', u.voice.voiceURI);`)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.voiceInfo || '（点击按钮查看 SpeechSynthesisVoice 语音选择完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['队列/节奏', f.speechSynthesis]]), h(Tag, { color: 'primary' }, 'TTS 实战')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'speak() 自动队列（多 utterance 顺序播放），cancel() 清空整个队列，pause()/resume() 暂停恢复。rate（0.1-10 默认 1）控制语速，pitch（0-2 默认 1）控制音调，volume（0-1 默认 1）音量。长文本分句队列避免单 utterance 过长截断。mark 标记与 SSML（部分浏览器支持 break/emphasis）。boundary 事件实现卡拉OK式词高亮。实战：朗读器、字幕同步、多角色对话（不同 voice+pitch）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('朗读队列演示', { type: 'primary', size: 'sm', onClick: () => this._runSynthControlDemo() })),
                h('div', { class: 'ws-karaoke' }, h('span', { class: 'ws-word ws-word--active' }, '你好'), h('span', { class: 'ws-word' }, '，'), h('span', { class: 'ws-word' }, '世界'), h('span', { class: 'ws-word' }, '！'), h('span', { class: 'ws-word' }, '这是'), h('span', { class: 'ws-word' }, '卡拉OK'), h('span', { class: 'ws-word' }, '高亮'), h('span', { class: 'ws-word' }, '演示')),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `const synth = window.speechSynthesis;
...
synth.speak(u);`)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.synthControlInfo || '（点击按钮查看队列与节奏控制完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['SpeechRecognition', f.speechRecognition]]), h(Tag, { color: 'primary' }, 'ASR 基础')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'new (SpeechRecognition || webkitSpeechRecognition)() 创建识别器。属性 lang（zh-CN/en-US）、continuous（单次 vs 持续）、interimResults（中间结果）、maxAlternatives（备选数）。方法 start()/stop()/abort()。事件 onstart/onend/onerror/onspeechstart/onspeechend/onsoundstart/onsoundend/onresult。onerror 错误类型：no-speech/aborted/audio-capture/network/not-allowed/service-not-allowed/bad-grammar。必须 HTTPS/localhost + 麦克风权限 + 用户激活。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('启动识别', { type: 'primary', size: 'sm', onClick: () => this._runRecogBasicDemo() })),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, h('div', { class: 'ws-mic' }, '🎤'), h('span', { class: 'ws-status ws-status--no' }, '未启动')),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `const SR = window.SpeechRecognition || window.webkitSpeechRecognition;
const rec = new SR();
rec.lang = 'zh-CN';
...
// rec.abort(); // 立即中止`)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.recogBasicInfo || '（点击按钮查看 SpeechRecognition 识别基础完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['Result API', f.speechRecognition]]), h(Tag, { color: 'primary' }, '结果结构')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'event.results 是 SpeechRecognitionResultList（类数组），results[i] 是 SpeechRecognitionResult（含 isFinal），result[0] 是 SpeechRecognitionAlternative（.transcript 文本 + .confidence 置信度 0-1）。maxAlternatives 控制备选数 result[0..n]。event.resultIndex 当前结果起始索引。continuous + interimResults 配合实现实时字幕。transcript 处理：首字母大写、标点、敏感词过滤。实战：语音指令、语音搜索、实时字幕。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('实时字幕演示', { type: 'primary', size: 'sm', onClick: () => this._runResultDemo() })),
                h('div', { class: 'ws-caption' }, h('span', { class: 'ws-final' }, '你好世界'), h('span', { class: 'ws-interim' }, ' 这是中间结果...')),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `rec.continuous = true;
rec.interimResults = true;
rec.maxAlternatives = 3;
let finalText = '';
...
rec.onresult = (e: any) => {
  ...
  caption.innerHTML =
    ...
    '<span class="interim">' + interim + '</span>';
                ...
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.resultInfo || '（点击按钮查看 SpeechRecognitionEvent 与 Result 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard7() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['GrammarList', f.speechGrammarList]]), h(Tag, { color: 'primary' }, '语法约束')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'new SpeechGrammarList() 创建语法列表，addFromString(grammarString, weight) 添加 JSGF 语法，addFromURI(uri, weight) 从 URL 加载。JSGF（JSpeech Grammar Format）格式：#JSGF V1.0; grammar commands; public <command> = (打开 | 关闭 | 切换) (灯 | 窗帘 | 音乐);。recognition.grammars = grammarList 设置，weight 权重 0-1。语法约束提升识别准确率，适合语音指令系统、IVR 替代。浏览器支持有限（Chrome 部分支持，Safari/Firefox 不支持）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('语法约束演示', { type: 'primary', size: 'sm', onClick: () => this._runGrammarDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `const grammar = ` + '`#JSGF V1.0;\ngrammar commands;\npublic <command> = (打开 | 关闭 | 切换) (灯 | 窗帘 | 音乐);`' + `;
...
rec.start();`)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.grammarInfo || '（点击按钮查看 SpeechGrammarList 语法约束完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard8() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'primary' }, '实战'), h(Tag, { color: f.speechSynthesis || f.speechRecognition ? 'success' : 'error' }, `Web Speech ${f.speechSynthesis || f.speechRecognition ? '✓' : '✗'}`)),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '实战模式：语音助手（TTS+ASR 闭环）、语音搜索框、无障碍朗读、多语言切换、离线识别（WASM+Vosk/whisper.cpp）。9 大陷阱：getVoices 异步需 voiceschanged、ASR 需 HTTPS/localhost、长文本 speak 截断需分句、cancel 中断所有 utterance、Safari 与 Chrome voice 名差异、连续识别自动 stop 需 onend 重启、移动端屏幕锁定中断、权限需用户激活、Chrome 走云端隐私问题。降级方案：Web Audio API + MediaRecorder 录音 + 服务端 ASR。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行实战模式演示', { type: 'primary', size: 'sm', onClick: () => this._runPatternDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// 语音助手闭环：ASR 听 → 处理 → TTS 回应
const rec = new (window.SpeechRecognition || window.webkitSpeechRecognition)();
rec.lang = 'zh-CN';
rec.onresult = (e: any) => {
  const heard = e.results[0][0].transcript;
...
async function recordAndRecognize() {
  ...
  return (await resp.json()).text;
                ...
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.patternInfo || '（点击按钮查看实战模式与陷阱完整清单）')),
            ],
        });
        this.registerChild(card);
        return card.render();
...
//# sourceMappingURL=WebSpeechAPIDeepPage.js.map