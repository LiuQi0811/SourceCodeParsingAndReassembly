...
export class CSSAdvancedFeaturesPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!f.anchor)
            this._addLog('warn', 'anchor-name 不可用（CSS Anchor Positioning，Chrome 125+）');
        if (!f.animationTimeline)
            this._addLog('warn', 'animation-timeline: scroll() 不可用（Scroll-driven Animations，Chrome 115+）');
        if (!f.scope)
            this._addLog('warn', '@scope 不可用（Chrome 118+）');
        if (!f.statePseudo)
            this._addLog('warn', ':state() 伪类不可用或无法检测（Chrome 125+）');
        if (!f.cssFunction)
            this._addLog('warn', '@function 不可用（CSS Functions and Mixins，实验性，尚未广泛支持）');
    }
    ...
    _runAnchorDemo() {
        ...
        try {
            ...
            try {
                anchor.style.setProperty('anchor-name', '--myanchor');
            }
            catch { /* jsdom 拒绝未知属性 */ }
            ...
            try {
                positioned.style.setProperty('top', 'anchor(--myanchor bottom)');
            }
            catch { /* noop */ }
            try {
                positioned.style.setProperty('left', 'anchor(--myanchor left)');
            }
            catch { /* noop */ }
            ...
            this.setState({ anchorInfo: 'CSS Anchor Positioning 演示：\n' +
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['anchor-name', f.anchor], ['position-area', f.positionArea], ['position-try', f.positionTry]]), h(Tag, { color: 'primary' }, 'tooltip / popover')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'CSS Anchor Positioning 让元素相对另一个「锚点」元素定位，无需 JS 计算坐标。anchor-name 给锚点命名（--x 形式）；定位元素用 top/left: anchor(--name side) 引用锚点边界；anchor-default 指定默认锚点；anchor-scroll 控制滚动行为；anchor-scope 限定查找范围；position-area 用网格区域定位；position-try 自动避让翻转（flip-block/flip-inline 等）。inset-area 是已弃用旧名，被 position-area 取代。适用于 tooltip / popover / 菜单跟随触发元素。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行锚点演示', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runAnchorDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, 'Anchor 演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '240px', overflow: 'auto' } }, h('code', {}, s.anchorInfo || '（点击「运行锚点演示」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '150px', overflow: 'auto' } }, h('code', {}, `.anchor { anchor-name: --myanchor; }
.tooltip {
  position: absolute;
...
@layer { .btn { anchor-default: --myanchor; } }   /* 默认锚点 */`)),
    ...
    _runScrollTimelineDemo() {
        ...
        try {
            ...
            const tlVal = window.getComputedStyle(progress).getPropertyValue('animation-timeline');
            this.setState({ scrollTimelineInfo: 'Scroll-driven Animations 演示：\n' +
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['animation-timeline', f.animationTimeline], ['ScrollTimeline', f.scrollTimeline], ['ViewTimeline', f.viewTimeline]]), h(Tag, { color: 'primary' }, 'scroll() / view()')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'Scroll-driven Animations 让动画由滚动或元素进入视口驱动，无需 scroll 事件 + requestAnimationFrame。CSS：scroll-timeline / view-timeline 声明时间线；animation-timeline: scroll()/view() 绑定到动画；timeline-scope 跨子树扩展作用域；animation-range: cover 0% 50% 限定动画区间。JS：AnimationTimeline 基类、ScrollTimeline（{ source, axis }）、ViewTimeline（{ subject, inset }）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行滚动动画演示', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runScrollTimelineDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, 'Scroll-driven 结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, s.scrollTimelineInfo || '（点击「运行滚动动画演示」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '150px', overflow: 'auto' } }, h('code', {}, `.scroller { scroll-timeline: --st; }     /* 声明滚动时间线 */
...
/* JS：new ScrollTimeline({ source: el, axis: 'block' }); */`)),
    ...
    _runScopeDemo() {
        ...
        try {
            ...
            const p1Color = window.getComputedStyle(p1).getPropertyValue('color');
            this.setState({ scopeInfo: '@scope at-rule 演示：\n' +
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['@scope', f.scope]]), h(Tag, { color: 'primary' }, '上/下边界 / proximity')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '@scope (.root) to (.limit) { ... } 限定规则仅作用于 .root 内、.limit 之外的元素，避免全局污染。:scope 引用作用域根；下边界用 (.stop) 或 :scope to .stop 声明；邻近规则（proximity）在多个 :scope 匹配时取 DOM 距离最近的根。CSSStyleSheet.replaceSync 可同步检测 @scope 是否能被解析（不可用则抛错）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 @scope 演示', { type: 'primary', size: 'sm', onClick: () => this._runScopeDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '@scope 演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, s.scopeInfo || '（点击「运行 @scope 演示」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '150px', overflow: 'auto' } }, h('code', {}, `@scope (.card) to (.card-footer) {
...
@scope (.article) { :scope to .comments { ... } }`)),
    ...
    _runStateDemo() {
        ...
        try {
            ...
            if (!customElements.get(elName)) {
                customElements.define(elName, class extends HTMLElement {
                    ...
                    constructor() {
                        ...
                        } // jsdom 可能未实现 attachInternals
                    }
                    toggleOpen() {
                        if (this._internals && this._internals.states) {
                            if (this._internals.states.contains('open'))
                                this._internals.states.remove('open');
                            else
            ...
            const colorAfter = window.getComputedStyle(el).getPropertyValue('color');
            this.setState({ stateInfo: ':state() 伪类演示：\n' +
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['customElements', f.customElements], ['ElementInternals', f.elementInternals], [':state()', f.statePseudo]]), h(Tag, { color: 'primary' }, 'target / internalTarget')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, ':state() 伪类匹配自定义元素的内置受保护状态。自定义元素在构造函数中 this.attachInternals() 获取 ElementInternals，其 .states 是 DOMTokenList，用 add/remove/contains/toggle 管理状态名。CSS 用 my-el:state(open) 匹配当前 states 含 open 的元素。与 [data-open] 区别：states 不会被 attribute 反射，外部无法直接篡改，更语义化、更安全。target / internalTarget 选择器配合 :state 可实现纯 CSS 的状态驱动样式。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 :state() 演示', { type: 'primary', size: 'sm', disabled: !f.customElements, onClick: () => this._runStateDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, ':state() 演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, s.stateInfo || '（点击「运行 :state() 演示」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '160px', overflow: 'auto' } }, h('code', {}, `class MyEl extends HTMLElement {
...
my-el:state(loading) { opacity: .5; }`)),
    ...
    _runLayerDemo() {
        ...
        try {
            ...
            document.body.appendChild(el);
            this._dynamicNodes.push(el);
            const colorVal = window.getComputedStyle(el).getPropertyValue('color');
            ...
            this.setState({ layerInfo: 'CSS Triage / Cascade Layers 演示：\n' +
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['@layer', f.layer], ['@property', f.property], ['@container', f.container]]), h(Tag, { color: 'primary' }, 'layer() / revert-layer')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'Cascade Layers 用 @layer 显式控制级联优先级：@layer base, components; 声明顺序（后者优先级更高）；@layer name { ... } 定义层。layer(name) 函数配合 @import 把外部样式表归入层；revert-layer 关键字回退到上一层值。@property 注册强类型自定义属性（等价于 Houdini CSS.registerProperty）；@container 基于容器尺寸响应（区别于 @media 视口）。优先级总则：未分层样式 > 分层样式；!important 反转层顺序。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行级联层演示', { type: 'primary', size: 'sm', onClick: () => this._runLayerDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '级联层演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '280px', overflow: 'auto' } }, h('code', {}, s.layerInfo || '（点击「运行级联层演示」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '160px', overflow: 'auto' } }, h('code', {}, `@layer base, components, utilities;     /* 声明顺序：base < components < utilities */
...
@container (inline-size > 400px) { .widget { display: grid; } }`)),
    ...
    _runFunctionDemo() {
        ...
        try {
            ...
            const tryParse = (prop, val) => {
                if (!hasCSSStyleValue)
                    return null;
                try {
                    return CSSStyleValue.parse(prop, val);
                }
                catch (e) {
                    return `解析抛错：${e.name}`;
                }
            ...
            const results = probes.map(([name, prop, val, ok]) => {
                let readBack = '';
                try {
            ...
            this.setState({ functionInfo: 'CSS Custom Functions & 数学函数 演示：\n' +
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['@function', f.cssFunction], ['数学函数', f.mathSin], ['abs/sign', f.mathAbs]]), h(Tag, { color: 'primary' }, '@if / @else')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'CSS Functions and Mixins 模块引入 @function 定义可复用函数、@if/@else 条件分支（实验性，极新）。calc() 已支持丰富数学函数：abs/sign（绝对值/符号）、mod/rem（取模/取余，符号规则不同）、round（四舍五入，可带步长）、sin/cos/tan（三角）、pow/sqrt/hypot（幂/平方根/直角边）、log/exp（对数/指数）、atan2（反正切）、pi()/e()（常量）。可用 CSSStyleValue.parse(prop, value) 类型化解析（Typed OM），或直接 setProperty 后读回验证支持。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行函数演示', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runFunctionDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '函数 / 数学函数演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '300px', overflow: 'auto' } }, h('code', {}, s.functionInfo || '（点击「运行函数演示」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '170px', overflow: 'auto' } }, h('code', {}, `/* @function（实验性，CSS Functions and Mixins） */
...
.r  { transform: rotate(atan2(1, 1)); }     /* 45deg */`)),
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'api-lab-page css-advanced-features-page' }, h('h2', { class: 'section-title' }, 'CSS 高级特性 实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, '本页演示 CSS Anchor Positioning、Scroll-driven Animations、@scope 作用域、:state() 自定义元素状态、Cascade Layers / @property / @container、@function 与 calc 数学函数。所有特性通过 typeof / CSS.supports 能力检测，不可用时仅记日志，绝不抛异常。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderCard6(), this._renderLogPanel());
    }
}
//# sourceMappingURL=CSSAdvancedFeaturesPage.js.map