...
export class PointerLockAPIPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!f.requestPointerLock)
            this._addLog('warn', 'Element.prototype.requestPointerLock 不可用（jsdom 不实现；真实浏览器 Chrome 22+/Firefox 14+/Safari 10.1+ 全支持，需安全上下文 HTTPS/localhost）');
        if (!f.movement)
            this._addLog('warn', 'movementX/movementY 不可用（PointerEvent.movementX，锁定后提供无边界相对位移；jsdom 不实现）');
        if (!f.secureContext)
            this._addLog('warn', '当前非安全上下文（window.isSecureContext=false），Pointer Lock 在非安全上下文被禁用');
    ...
    _flags() {
        ...
        return {
            ...
            lockChangeEvent: safe(() => typeof window !== 'undefined' && (typeof window.onpointerlockchange !== 'undefined' || 'pointerlockchange' in document)),
    ...
    _setupDocumentListeners() {
        ...
        const onChange = () => {
            ...
            const out = this.el?.querySelector('.pl-lock-state');
            if (out)
                out.textContent = '当前锁定状态：' + (this._locked ? '已锁定（document.pointerLockElement = ' + tag + '）' : '未锁定');
        };
        const onError = () => {
            this._addLog('warn', 'pointerlockerror 触发：指针锁定失败（用户拒绝/系统策略/安全上下文/无效元素）');
            const out = this.el?.querySelector('.pl-lock-state');
            if (out)
                out.textContent = '当前锁定状态：锁定失败（pointerlockerror）';
        };
        try {
    ...
    _readRequestInfo() {
        const f = this._flags();
        try {
            const target = this.el?.querySelector('.pl-target');
    ...
    _runRequestDemo() {
        ...
        const target = this.el?.querySelector('.pl-target') ?? null;
        this._tryRequestLock(target);
        if (target)
            target.classList.toggle('pl-active', !!this._locked);
    }
    _renderCard2() {
        const s = this.state;
        const f = this._flags();
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['requestPointerLock', f.requestPointerLock], ['SecureContext', f.secureContext]]), h(Tag, { color: 'warning' }, '需用户手势')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'element.requestPointerLock() 请求将鼠标锁定到该元素。新版浏览器（Chrome 89+/Firefox 50+）返回 Promise<void>，旧版本无返回值（依赖 pointerlockchange 事件）。调用约束：必须在用户手势（click/keydown）事件循环内调用、必须安全上下文（HTTPS/localhost）、元素必须已连接到 DOM、同一时刻只能锁定一个元素。锁定成功后 document.pointerLockElement === el，光标隐藏，鼠标移动产生 mousemove + movementX/movementY。非用户手势调用会触发 pointerlockerror。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('请求锁定', { type: 'primary', size: 'sm', disabled: !f.requestPointerLock, onClick: () => this._runRequestDemo() }), this._btn('读取信息', { size: 'sm', onClick: () => this.setState({ requestInfo: this._readRequestInfo() }) })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '点击下方元素请求指针锁定（真实浏览器需用户手势触发）：'),
                h('div', { class: 'pl-target', onClick: (e) => this._runRequestDemo() }, '点击我请求 Pointer Lock\n（真实浏览器将锁定鼠标到此元素）\n当前锁定状态见下方'),
                h('div', { class: 'mt-sm' }, h('span', { class: 'pl-lock-state' }, '当前锁定状态：' + (this._locked ? '已锁定' : '未锁定'))),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.requestInfo || '（点击「请求锁定」或「读取信息」查看 requestPointerLock 完整说明）')),
                h(Alert, {
                    type: 'warning',
    ...
    _runExitDemo() {
        ...
        const target = this.el?.querySelector('.pl-target');
        if (target)
            target.classList.remove('pl-active');
    }
    _renderCard3() {
        const s = this.state;
        const f = this._flags();
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['exitPointerLock', f.exitPointerLock]]), h(Tag, { color: 'error' }, 'Esc 强制解锁')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'document.exitPointerLock() 主动解除指针锁定，无返回值。调用后触发 pointerlockchange 事件，document.pointerLockElement 变为 null，光标恢复显示。自动解锁场景：用户按 Esc 键（浏览器系统强制解锁，且 Esc 后进入约 1 秒"短期锁定期"防钓鱼）、标签页失焦（切换/最小化）、锁定元素移出 DOM、主动调用 exitPointerLock。pointerlockerror 错误原因：用户拒绝/系统策略/非安全上下文/非用户手势/Esc 锁定期内再请求/元素未连接 DOM。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('解除锁定', { type: 'primary', size: 'sm', disabled: !f.exitPointerLock, onClick: () => this._runExitDemo() }), this._btn('读取信息', { size: 'sm', onClick: () => this.setState({ exitInfo: this._readExitInfo() }) })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '当前锁定状态（点击「解除锁定」主动解锁）：'),
                h('div', {}, h('span', { class: 'pl-lock-state' }, '当前锁定状态：' + (this._locked ? '已锁定' : '未锁定'))),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.exitInfo || '（点击「解除锁定」或「读取信息」查看 exitPointerLock 完整说明）')),
                h(Alert, {
                    type: 'warning',
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['pointerlockchange', f.lockChangeEvent], ['pointerLockElement', f.pointerLockElement]]), h(Tag, { color: 'primary' }, 'document 级事件')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'pointerlockchange 与 pointerlockerror 事件均注册在 document 上（非 element）。pointerlockchange 在锁定/解锁时均触发，通过 document.pointerLockElement 判断当前状态（null 或元素引用）。pointerlockerror 在锁定失败时触发，无错误详情。推荐模式：在 pointerlockchange 锁定时注册 mousemove，解锁时移除，避免非锁定期间收到无效 movementX/Y。pointerlockchange 触发时 pointerLockElement 已是新值。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行演示', { type: 'primary', size: 'sm', onClick: () => this._runEventsDemo() })),
                h('div', { class: 'pl-stage' }, h('div', { class: 'fs-sm text-secondary' }, 'document 级监听已在 componentDidMount 注册（真实浏览器锁定/解锁将更新下方状态）：'), h('div', { class: 'mt-sm' }, h('span', { class: 'pl-lock-state pl-lock-state-live' }, '当前锁定状态：' + (this._locked ? '已锁定' : '未锁定')))),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.eventsInfo || '（点击「运行演示」查看 pointerlock 事件完整说明）')),
                h(Alert, {
                    type: 'info',
    ...
    _readMovementInfo() {
        ...
        try {
            ...
            const sampleStr = samples.length === 0
                ...
                : samples.map((s, i) => '  [' + i + '] movementX=' + s.x + ' movementY=' + s.y + ' clientX=' + s.cx + ' clientY=' + s.cy).join('\n');
    ...
    _setupMovementListener() {
        const f = this._flags();
        const area = this.el?.querySelector('.pl-move-area');
        if (!area) {
        ...
        const onMove = (e) => {
            ...
            if (this._movementSamples.length > 8)
                this._movementSamples.shift();
            const out = this.el?.querySelector('.pl-move-output');
            if (out) {
    ...
    _runMovementDemo() {
        ...
        if (!f.movement)
            this._addLog('warn', 'movementX/Y 不可用（jsdom 不实现 PointerEvent.movementX），真实浏览器可观察');
    }
    _renderCard5() {
        const s = this.state;
        const f = this._flags();
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['movementX/Y', f.movement]]), h(Tag, { color: 'primary' }, '高频更新')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'MouseEvent.movementX/movementY 是相对上次 mousemove 事件的位移增量。锁定前受视口边界限制（到边归零），锁定后持续更新无边界限制，clientX/clientY 锁定不变。3D 相机控制只用 movementX/Y（绝对坐标无意义）。高频事件（每秒数十至上百次），建议 requestAnimationFrame 合并。推荐模式：pointerlockchange 锁定时注册 mousemove，解锁时移除。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行演示', { type: 'primary', size: 'sm', onClick: () => this._runMovementDemo() })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '在下方区域移动鼠标查看 movementX/Y（真实浏览器锁定后无边界）：'),
                h('div', { class: 'pl-move-area' }, '移动鼠标到此区域\nmovementX/Y 将实时记录\n（jsdom 不触发 mousemove，真实浏览器可观察）'),
                h('div', { class: 'pl-output pl-move-output' }, '等待 mousemove 事件...'),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.movementInfo || '（点击「运行演示」查看 movementX/Y 完整说明与采样）')),
                h(Alert, {
                    type: 'info',
    ...
    _readFpsInfo() {
        ...
        try {
            return `===== 实战：第一人称 3D 相机控制 =====\n` +
                ...
                `\n` +
                `【当前相机状态】\n` +
                `  yaw   = ${this._yaw.toFixed(3)} rad (${(this._yaw * 180 / Math.PI).toFixed(1)}°)\n` +
                `  pitch = ${this._pitch.toFixed(3)} rad (${(this._pitch * 180 / Math.PI).toFixed(1)}°)\n` +
    ...
    _setupFpsCamera() {
        const f = this._flags();
        const viewport = this.el?.querySelector('.pl-viewport');
        if (!viewport) {
    ...
    _updateFpsView() {
        const hud = this.el?.querySelector('.pl-viewport .pl-hud');
        if (hud) {
            hud.textContent = 'yaw=' + this._yaw.toFixed(2) + ' (' + (this._yaw * 180 / Math.PI).toFixed(0) + '°)  pitch=' + this._pitch.toFixed(2) + ' (' + (this._pitch * 180 / Math.PI).toFixed(0) + '°)';
        }
        const cube = this.el?.querySelector('.pl-viewport .pl-cube');
        if (cube) {
    ...
    _runFpsDemo() {
        ...
        this._addLog('info', `3D 相机演示已启动：yaw=${this._yaw.toFixed(2)}, pitch=${this._pitch.toFixed(2)}（点击视口请求锁定，移动鼠标旋转视角）`);
        if (!f.movement)
            this._addLog('warn', 'movementX/Y 不可用（jsdom 不实现），相机视角不会更新；真实浏览器可观察');
    }
    _renderCard6() {
        const s = this.state;
        const f = this._flags();
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['requestPointerLock', f.requestPointerLock], ['movementX/Y', f.movement]]), h(Tag, { color: 'primary' }, 'WebGL/WebGPU 协同')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'FPS 游戏第一人称视角：requestPointerLock + mousemove movementX/Y → 旋转相机。yaw += movementX * sensitivity（偏航左右转），pitch -= movementY * sensitivity（俯仰，反转 Y 轴符合直觉），pitch 需 clamp 到 [-π/2, π/2] 防翻转。与 WebGL/WebGPU 协同：每帧 requestAnimationFrame 用 yaw/pitch 计算 view 矩阵并渲染。sensitivity 通常 0.002~0.005。反转 Y 轴原理：屏幕 Y 向下增大，但向下看 pitch 应减小。陷阱：不 clamp pitch 导致万向锁、sensitivity 过大抖动、不要在 mousemove 内直接渲染。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行演示', { type: 'primary', size: 'sm', onClick: () => this._runFpsDemo() })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '点击视口请求锁定，移动鼠标旋转视角（yaw/pitch 实时更新）：'),
                h('div', { class: 'pl-viewport' }, h('div', { class: 'pl-hud' }, 'yaw=0.00 (0°)  pitch=0.00 (0°)'), h('div', { class: 'pl-cube' }, '◆'), h('div', { class: 'pl-crosshair' }, '+')),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.fpsInfo || '（点击「运行演示」查看 3D 相机控制完整代码）')),
                h(Alert, {
                    type: 'info',
    ...
    _readPanoramaInfo() {
        ...
        try {
            return `===== 实战：全景图 / 360° 图片浏览 =====\n` +
                ...
                `\n` +
                `【当前全景偏移】\n` +
                `  angle = ${this._panoAngle.toFixed(1)}°\n` +
    ...
    _setupPanorama() {
        const f = this._flags();
        const pano = this.el?.querySelector('.pl-panorama');
        if (!pano) {
            this._addLog('warn', '全景图：未找到 .pl-panorama 元素');
            return;
        }
        // 移除旧 mousemove（注意区分 fps 的监听）
        const existing = this._moveListeners.filter(([t]) => t === pano);
        ...
        this._moveListeners = this._moveListeners.filter(([t]) => t !== pano);
        const onMove = (e) => {
            const me = e;
    ...
    _updatePanoramaView() {
        const marker = this.el?.querySelector('.pl-pano-marker');
        const hud = this.el?.querySelector('.pl-pano-hud');
        if (marker) {
            // 0-360° 映射到 0-100% 宽度
        ...
        if (hud) {
            hud.textContent = '偏移角度：' + this._panoAngle.toFixed(1) + '°';
        }
    }
    _runPanoramaDemo() {
        const f = this._flags();
        ...
        this._setupPanorama();
        this._addLog('info', `全景图演示已启动：angle=${this._panoAngle.toFixed(1)}°（点击全景图请求锁定，移动鼠标浏览）`);
        if (!f.movement)
            this._addLog('warn', 'movementX/Y 不可用（jsdom 不实现），全景图不会滚动；真实浏览器可观察');
    }
    _renderCard7() {
        const s = this.state;
        const f = this._flags();
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['requestPointerLock', f.requestPointerLock], ['Fullscreen', f.fullscreen]]), h(Tag, { color: 'primary' }, '全屏 + 锁定')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '全景图/360° 图片浏览：拖拽锁定浏览全景，全屏 + 指针锁定 = 沉浸式体验。实现模式：拖拽（mousedown 锁定 + mousemove 滚动 + mouseup 解锁）适合偶尔浏览；指针锁定（click 锁定 + mousemove 持续滚动 + Esc 解锁）适合沉浸式浏览。与 DeviceOrientation API 移动端对比：桌面端用 Pointer Lock + mousemove，移动端用 DeviceOrientation（gamma/beta），iOS Safari 不支持 Pointer Lock 必须降级。虚拟现实场景：WebXR API 有独立输入系统，但 WebXR 之前的简易 VR（Cardboard 风格）可用 Pointer Lock。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行演示', { type: 'primary', size: 'sm', onClick: () => this._runPanoramaDemo() })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '点击全景图请求锁定，移动鼠标浏览（水平偏移实时更新）：'),
                h('div', { class: 'pl-panorama' }, h('div', { class: 'pl-pano-marker' }, '▼'), h('div', { class: 'pl-pano-hud' }, '偏移角度：0.0°')),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.panoramaInfo || '（点击「运行演示」查看全景图浏览完整代码）')),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard8() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['requestPointerLock', f.requestPointerLock], ['SecureContext', f.secureContext]]), h(Tag, { color: 'warning' }, '7 大陷阱')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '七大陷阱：必须用户手势触发（非手势调用被拒，触发 pointerlockerror）；Esc 键系统强制解锁 + 约 1 秒短期锁定期（防钓鱼）；标签页失焦自动解锁（切换/最小化）；与 Fullscreen API 协同（先全屏再锁定）；无障碍键盘等价（方向键控制 yaw/pitch）；iOS Safari 不支持（降级 DeviceOrientation/拖拽+touch）；安全考虑（钓鱼防护：用户手势/Esc 逃逸/短期锁定期/失焦解锁）。最佳实践清单 10 条覆盖用户手势、事件监听、Esc 锁定期、全屏顺序、键盘等价、iOS 降级、mousemove 生命周期、pitch clamp、渲染时机、防钓鱼告知。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行演示', { type: 'primary', size: 'sm', onClick: () => this._runPitfallsDemo() })),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.pitfallsInfo || '（点击「运行演示」查看 7 大陷阱与 10 条最佳实践完整说明）')),
                h(Alert, {
                    type: 'warning',
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'api-lab-page pointer-lock-api-page' }, h('h2', { class: 'section-title' }, 'Pointer Lock API 鼠标锁定完整实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, '本页演示 Pointer Lock API（前 Mouse Lock API，W3C 规范）全套能力：概述与动机、requestPointerLock 基础（用户手势/Promise/安全上下文）、document.exitPointerLock、pointerlockchange/error 事件、movementX/Y 相对位移、第一人称 3D 相机控制实战、全景图/360° 浏览实战、陷阱与最佳实践。所有特性通过 typeof / in 能力检测，不可用时仅记日志，绝不抛异常。jsdom 不实现 Pointer Lock，真实浏览器（Chrome 22+/Firefox 14+/Safari 10.1+）全支持。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, h('div', { class: 'feature-grid' }, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderCard6(), this._renderCard7(), this._renderCard8()), this._renderLogPanel());
    }
}
//# sourceMappingURL=PointerLockAPIPage.js.map