...
export class StreamsFetchDeepPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!this._abortControllers)
            this._abortControllers = new Set(); // 所有活跃 AbortController
        if (!this._readers)
            this._readers = []; // 所有活跃 reader（releaseLock）
        if (!this._streams)
            this._streams = []; // 所有活跃 stream（cancel）
        if (!this._timers)
            this._timers = new Set(); // 所有 setTimeout 句柄
        ...
        const hasReqRespHeaders = typeof Request !== 'undefined' &&
        ...
        if (!hasFetch)
            this._addLog('warn', 'fetch 不可用，Card 1/2/5 的 fetch 演示将跳过');
        if (!hasReqRespHeaders)
            this._addLog('warn', 'Request/Response/Headers 不完整，相关演示将跳过');
        if (!hasAbort)
            this._addLog('warn', 'AbortController/AbortSignal.timeout 不可用，取消演示将受限');
        if (!hasStreams)
            this._addLog('warn', 'Streams API 不完整，管道演示将跳过');
        if (!hasGetReader)
            this._addLog('warn', 'ReadableStream.prototype.getReader 不可用，流式读取演示将跳过');
        if (!hasTextDecoderStream)
            this._addLog('warn', 'TextDecoderStream 不可用，Card 5 解码演示将跳过');
    }
    // —— 生命周期：卸载 ——
    ...
    componentWillUnmount() {
            ...
            this._readers.forEach((r) => {
                try {
                    if (r && typeof r.cancel === 'function')
                        r.cancel('page-unmount');
                }
                catch { /* noop */ }
                try {
                    if (r && typeof r.releaseLock === 'function')
                        r.releaseLock();
                }
                catch { /* noop */ }
            });
        ...
        if (this._streams) {
            this._streams.forEach((s) => {
                try {
                    if (s && !s.locked && typeof s.cancel === 'function')
                        s.cancel('page-unmount');
                }
                catch { /* noop */ }
            });
            this._streams = [];
    ...
    _trackAbort(c) { if (c)
        this._abortControllers.add(c); return c; }
    _untrackAbort(c) { try {
        this._abortControllers.delete(c);
    }
    catch { /* noop */ } }
    _trackReader(r) { if (r)
        this._readers.push(r); return r; }
    _untrackReader(r) {
        try {
            const i = this._readers.indexOf(r);
            if (i >= 0)
                this._readers.splice(i, 1);
        }
        catch { /* noop */ }
    }
    _trackStream(s) { if (s)
        this._streams.push(s); return s; }
    ...
    _buildRequest() {
        ...
        try {
            ...
            const info = 'Request.url = ' + req.url + '\n' +
    ...
    async _sendFetch() {
        ...
        try {
            ...
            const respInfo = 'response.ok = ' + resp.ok + '\n' +
    ...
    _renderCard1() {
        const s = this.state;
        return this._card('Card 1 · Fetch API 基础', 'fetch(input, init)：init 含 method/headers/body/mode/credentials/cache/redirect/referrer/integrity；' +
            ...
            'response.text()/json()/blob()/arrayBuffer()/formData()/clone()。', h(Tag, { color: 'primary' }, 'Fetch'), [
            h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('构造 Request', {
            ...
            s.reqInfo
                ...
                : null,
            h('pre', { class: 'code-block mt-md' }, `fetch(input, init);   // init: { method, headers, body, mode, credentials, cache, redirect, referrer, integrity }
new Request(url, init); new Response(body, init); new Headers();
h.append(name, value); h.get/set/delete/has(name); (h as any).entries(); h.forEach(cb);
...
resp.text(); resp.json(); resp.blob(); resp.arrayBuffer(); resp.formData(); resp.clone();`),
    ...
    async _startAbortableFetch() {
        ...
        try {
            ctrl.signal.addEventListener('abort', () => {
                let reason = '—';
                try {
                    reason = JSON.stringify(ctrl.signal.reason);
                }
                catch {
        ...
        try {
            ...
            if (this._autoAbortTimer) {
                this._clearTimer(this._autoAbortTimer);
                this._autoAbortTimer = null;
            }
    ...
    _renderCard2() {
        ...
        return this._card('Card 2 · AbortController + 超时取消', 'new AbortController() → controller.signal (AbortSignal)；signal.aborted / signal.reason / signal.throwIfAborted()；' +
            'controller.abort(reason)；fetch(url, { signal })；AbortSignal.timeout(ms) 静态方法（自动超时）；' +
            'AbortSignal.any([sig1, sig2]) 组合；signal.addEventListener("abort", cb)。', h(Tag, { color: running ? 'warning' : (s.abortState === '已中止' ? 'error' : 'default') }, s.abortState), [
            h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('开始 fetch', {
            ...
            h('pre', { class: 'code-block mt-md' }, `const ctrl = new AbortController();
...
signal.addEventListener('abort', cb);`),
    ...
    _releaseDemoStream() {
        if (this._demoReader) {
            try {
                this._demoReader.cancel('rebuild');
            }
            catch { /* noop */ }
            try {
                this._demoReader.releaseLock();
        ...
        if (this._demoStream) {
            try {
                if (!this._demoStream.locked)
                    this._demoStream.cancel('rebuild');
            }
            catch { /* noop */ }
            this._demoStream = null;
        }
    ...
    _renderCard3() {
        const s = this.state;
        return this._card('Card 3 · ReadableStream + 流式读取', 'new ReadableStream({ start(controller){ controller.enqueue(chunk); controller.close(); controller.error(e); }, pull(controller), cancel(reason), type:"bytes" })；' +
            'stream.getReader() → reader.read() → { value, done } / reader.releaseLock() / reader.cancel()；' +
            'stream.locked；for await (const chunk of stream) 异步迭代；BYOB Reader：stream.getReader({ mode: "byob" }) / reader.read(view)。', h(Tag, { color: 'success' }, 'ReadableStream'), [
            h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('构造流', {
            ...
            s.streamChunks.length > 0
                ? h('div', { class: 'mt-md' }, h('div', { class: 'fs-sm text-secondary mb-xs' }, `已读取 ${s.streamChunks.length} 个 chunk：`), h('div', { class: 'flex items-center gap-xs flex-wrap' }, ...s.streamChunks.map((c, i) => h(Tag, { color: 'primary' }, `#${i + 1} ${JSON.stringify(c)}`))))
                : null,
            h('pre', { class: 'code-block mt-md' }, `const stream = new ReadableStream({
...
const byob = stream.getReader({ mode: 'byob' }); byob.read(new Uint8Array(1024)); // BYOB`),
    ...
    async _pipeDemo() {
        ...
        try {
            ...
            const upper = new TransformStream({
                ...
                flush(controller) { try {
                    controller.enqueue('!');
                }
                catch { /* noop */ } controller.close?.(); },
            });
            // sink：收集结果
            const sink = [];
            let totalBytes = 0;
            const writable = new WritableStream({
                write(chunk) {
                    sink.push(chunk);
                    try {
                        totalBytes += new TextEncoder().encode(String(chunk)).byteLength;
                    }
                    catch { /* noop */ }
                },
                close() { },
    ...
    _renderCard4() {
        const s = this.state;
        return this._card('Card 4 · WritableStream + TransformStream + 管道', 'new WritableStream({ start(controller), write(chunk, controller), close(), abort(reason) })；' +
            ...
            'readable.pipeTo(writable) / readable.pipeThrough(transformStream)；stream.tee() → [branch1, branch2] 分流。', h(Tag, { color: 'warning' }, 'Pipe'), [
            h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('管道演示', {
            ...
            s.pipeResult
                ...
                : null,
            h('pre', { class: 'code-block mt-md' }, `const writable = new WritableStream({
...
const [branch1, branch2] = stream.tee(); // 分流`),
    ...
    _renderCard5() {
        const s = this.state;
        return this._card('Card 5 · 流式 Fetch + 响应体流', 'response.body 是 ReadableStream；const reader = response.body.getReader(); while(true){ const { value, done } = await reader.read(); if(done) break; ... }；' +
            '流式下载大文件实时显示进度；TextDecoderStream 串接：response.body.pipeThrough(new TextDecoderStream())。', h(Tag, { color: s.bodyProgress.startsWith('完成') ? 'success' : (s.bodyProgress ? 'warning' : 'default') }, s.bodyProgress || '空闲'), [
            h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('开始流式下载', {
            ...
            s.bodyProgress
                ...
                : null,
            h('pre', { class: 'code-block mt-md' }, `// response.body 是 ReadableStream
...
const { value } = await r.read(); // value 已是解码后的字符串`),
    ...
    _renderCard6() {
        ...
        return this._card('Card 6 · Headers 进阶 + Fetch 流式上传 + Response 静态方法', 'Headers.getSetCookie() 处理多个 Set-Cookie（vs get/getAll）+ fetch duplex:"half" 流式上传请求体 + Response.error()/json()/redirect() 静态方法。', h('div', { class: 'flex gap-xs' }, h(Tag, { color: hasHeaders ? 'success' : 'error' }, hasHeaders ? 'Headers ✓' : '不可用')), [
            h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('Headers.getSetCookie', { type: 'primary', size: 'sm', disabled: !hasHeaders, onClick: () => this._demoHeadersAdvanced() }), this._btn('Response 静态方法', { type: 'primary', size: 'sm', disabled: !hasHeaders, onClick: () => this._demoResponseStatic() }), this._btn('fetch duplex:half', { type: 'primary', size: 'sm', onClick: () => this._demoFetchDuplex() })),
            s.advancedHeadersState
                ? h('pre', { class: 'code-block mt-md', style: { maxHeight: '300px', overflow: 'auto' } }, s.advancedHeadersState)
                : null,
            h('pre', { class: 'code-block mt-md' }, `// 1. Headers.getSetCookie() —— 多 Set-Cookie 处理
const h = new Headers();
...
Response.redirect(url, 302); // 重定向响应`),
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'page api-lab-page' }, h('h2', { class: 'section-title' }, '流与 Fetch 深度实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, 'Streams API 是浏览器处理流式数据的基础设施，Fetch API 借助 ReadableStream 实现真正的流式响应。' +
            '本页演示流式 fetch、管道组合、AbortController 取消、字节流。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderCard6(), this._renderLogPanel());
    }
}
//# sourceMappingURL=StreamsFetchDeepPage.js.map