...
export class CanvasPage extends Page {
    ...
    _setupCanvas(canvas, w = 320, h = 200) {
    ...
    _drawShapes() {
        const canvas = this.$('#canvas-shapes');
        if (!canvas)
            return;
    ...
    _drawGradients() {
        const canvas = this.$('#canvas-gradient');
        if (!canvas)
            return;
        const ctx = this._setupCanvas(canvas);
        if (!ctx)
    ...
    _drawTransform() {
        const canvas = this.$('#canvas-transform');
        if (!canvas)
            return;
        const ctx = this._setupCanvas(canvas);
        if (!ctx)
    ...
    _drawText() {
        ...
        ctx.fillText(`width=${m.width.toFixed(1)}px`, 20, 160);
    }
    _toggleAnimation() {
    ...
    _toggleParticles() {
        if (this.state.particleCount > 0) {
            ...
            const canvas = this.$('#canvas-particles');
            if (canvas) {
                const ctx = canvas.getContext('2d');
                if (ctx)
    ...
    renderPage() {
        return [
            ...
            // 静态图元
            h('div', { class: 'feature-grid mt-lg' }, h(Card, { title: '路径与图元' }, h('canvas', { id: 'canvas-shapes', class: 'canvas-stage' })), h(Card, { title: '渐变（Linear / Radial / Conic）' }, h('canvas', { id: 'canvas-gradient', class: 'canvas-stage' })), h(Card, { title: '变换（translate / rotate / save / restore）' }, h('canvas', { id: 'canvas-transform', class: 'canvas-stage' })), h(Card, { title: '文字与阴影（fillText / measureText）' }, h('canvas', { id: 'canvas-text', class: 'canvas-stage' }))),
            // 动画
            h(Card, { title: 'requestAnimationFrame 动画', extra: h(Tag, { color: this.state.animRunning ? 'success' : 'default' }, this.state.animRunning ? '运行中' : '已停止') }, h('div', { class: 'flex flex-col gap-sm' }, h('div', { class: 'flex gap-sm' }, this._btn(this.state.animRunning ? '停止动画' : '启动动画', {
            ...
            // 粒子
            h(Card, { title: '粒子系统', extra: h(Tag, { color: this.state.particleCount > 0 ? 'success' : 'default' }, `${this.state.particleCount} 粒子`) }, h('div', { class: 'flex flex-col gap-sm' }, h('div', { class: 'flex gap-sm' }, this._btn(this.state.particleCount > 0 ? '停止粒子' : '启动粒子', {
