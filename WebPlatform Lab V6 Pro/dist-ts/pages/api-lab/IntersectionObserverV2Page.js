...
export class IntersectionObserverV2Page extends Page {
    ...
    componentDidMount() {
        ...
        if (!caps.v2)
            this._addLog('warn', 'trackVisibility (v2) 不可用（jsdom polyfill 静默忽略该选项，不强制 delay ≥ 100）');
    }
    ...
    _caps() {
        ...
        if (hasIO) {
            ...
            try {
                probe = new IntersectionObserver(() => { }, { trackVisibility: true, delay: 50 });
                // 未抛 → trackVisibility 被静默忽略（jsdom polyfill / v1-only），v2 未真正生效
                v2Supported = false;
            }
            catch (e) {
                if (e instanceof TypeError)
                    v2Supported = true; // 真正支持 v2（强制 delay ≥ 100）
                else
                    v2Supported = false;
            }
    ...
    _makeFakeEntry({ isIntersecting, isVisible, ratio }) {
    ...
    _showIoCaps() {
        const caps = this._caps();
        this.setState({ trackVisibilityInfo: '===== IntersectionObserver v2 能力检测 =====\n\n' +
    ...
    _createV2Observer() {
        const caps = this._caps();
        if (!caps.io) {
            this.setState({ trackVisibilityInfo: "IntersectionObserver v2 用法（不可用，仅说明）：\n\n" +
        ...
        try {
            ...
            let mode = 'v2';
            try {
                observer = new IntersectionObserver(() => { }, { trackVisibility: true, delay: 100, threshold: [0, 0.5, 1], rootMargin: '0px' });
            }
            catch (e) {
            ...
            this.setState({ trackVisibilityInfo: `${mode === 'v2-error' ? '（v2 选项触发异常，已退回 v1）' : '真实构造 IntersectionObserver（v2 选项）'}：\n` +
    ...
    _compareV1V2Options() {
        this.setState({ trackVisibilityInfo: '===== IntersectionObserver v1 vs v2 options 对比 =====\n\n' +
    ...
    _explainDelayLimit() {
        this.setState({ trackVisibilityInfo: '===== delay 限制与 rootMargin 限制详解 =====\n\n' +
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.io ? 'success' : 'error' }, caps.io ? 'IO ✓' : 'IO ✗'), h(Tag, { color: caps.v2 ? 'success' : 'error' }, caps.v2 ? 'trackVisibility ✓' : 'trackVisibility ✗'), h(Tag, { color: 'primary' }, 'delay ≥ 100ms')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'new IntersectionObserver(cb, { trackVisibility: true, delay: 100, threshold, rootMargin, root }) 创建 v2 观察者。delay 控制「可见性稳定时间」（最小 100ms，否则抛 TypeError）；trackVisibility 启用后 entries 多 isVisible 字段。构造器签名与 v1 相同，v2 通过 options 区分。delay 让目标连续满足可见条件 delay 毫秒后才上报 isVisible=true，避免快速遮挡闪烁。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('能力检测', { type: 'primary', size: 'sm', onClick: () => this._showIoCaps() }), this._btn('创建 v2 observer', { size: 'sm', disabled: !caps.io, onClick: () => this._createV2Observer() }), this._btn('对比 v1/v2 选项', { size: 'sm', onClick: () => this._compareV1V2Options() }), this._btn('delay 限制说明', { size: 'sm', onClick: () => this._explainDelayLimit() })),
    ...
    _simulateVisible() {
        ...
        const timer = setTimeout(() => {
            ...
            this.setState({ visibilityInfo: '===== 模拟 v2 回调（目标实际可见）=====\n\n' +
    ...
    _simulateObscured() {
        ...
        const timer = setTimeout(() => {
            this._addLog('visible', `模拟 v2 回调：isIntersecting=${entry.isIntersecting}, isVisible=${entry.isVisible}, ratio=${entry.intersectionRatio}`);
            this.setState({ visibilityInfo: '===== 模拟 v2 回调（目标几何相交但不可见）=====\n\n' +
                    'IntersectionObserver v2 回调 entry：\n' +
    ...
    _showVisibilityCompare() {
        this.setState({ visibilityInfo: '===== isIntersecting vs isVisible 对比表 =====\n\n' +
    ...
    _explainVisibilitySemantics() {
        this.setState({ visibilityInfo: '===== isVisible 语义详解 =====\n\n' +
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.v2 ? 'success' : 'error' }, caps.v2 ? 'isVisible ✓' : 'isVisible 模拟'), h(Tag, { color: 'primary' }, '几何相交 vs 实际可见'), h(Tag, { color: 'warning' }, 'opacity/clip/scale')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'entry.isVisible 区分「几何相交」（isIntersecting）与「实际可见」（isVisible）。opacity:0 / visibility:hidden / 被其他元素覆盖 / clip-path / transform: scale(0) 都会让 isVisible=false 但 isIntersecting 可能仍 true。因 jsdom 无真实回调，本页用 setTimeout 模拟 v2 entry 对象 { isIntersecting, isVisible, intersectionRatio, target, time, boundingClientRect, intersectionRect, rootBounds } 展示语义。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('模拟可见', { type: 'primary', size: 'sm', onClick: () => this._simulateVisible() }), this._btn('模拟遮挡', { danger: true, size: 'sm', onClick: () => this._simulateObscured() }), this._btn('对比表', { size: 'sm', onClick: () => this._showVisibilityCompare() }), this._btn('语义说明', { size: 'sm', onClick: () => this._explainVisibilitySemantics() })),
    ...
    _demoRootMargin() {
        this.setState({ marginThresholdInfo: '===== rootMargin 计算演示 =====\n\n' +
    ...
    _demoThresholdArray() {
        this.setState({ marginThresholdInfo: '===== threshold 数组演示 =====\n\n' +
    ...
    _explainRootContainer() {
        this.setState({ marginThresholdInfo: '===== root 容器说明 =====\n\n' +
    ...
    _explainV2RootMarginLimit() {
        this.setState({ marginThresholdInfo: '===== v2 rootMargin 限制（trackVisibility: true 时）=====\n\n' +
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.io ? 'success' : 'error' }, caps.io ? 'IO ✓' : 'IO ✗'), h(Tag, { color: 'primary' }, 'rootMargin / threshold'), h(Tag, { color: 'warning' }, 'v2 rootMargin = 0px')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'rootMargin（"10px 20px 30px 40px" / "-50% 0px" 预加载）扩展/收缩 root 判定矩形；threshold（[0, 0.25, 0.5, 0.75, 1]）控制相交比例穿过阈值时回调；root（默认 viewport，可为指定元素）决定相对哪个容器判定。注意：trackVisibility: true 时 rootMargin 限制为 0px（非零抛 TypeError），因 rootMargin 偏移会破坏几何相交与实际可见的语义一致性。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('演示 rootMargin', { type: 'primary', size: 'sm', onClick: () => this._demoRootMargin() }), this._btn('演示 threshold 数组', { size: 'sm', onClick: () => this._demoThresholdArray() }), this._btn('说明 root 容器', { size: 'sm', onClick: () => this._explainRootContainer() }), this._btn('v2 rootMargin 限制', { danger: true, size: 'sm', onClick: () => this._explainV2RootMarginLimit() })),
    ...
    _simulateObserve() {
        ...
        if (!caps.io || !caps.observe) {
            this.setState({ lifecycleInfo: "observer.observe 用法（不可用，仅说明）：\n\n" +
        ...
        try {
            ...
            this.setState({ lifecycleInfo: `模拟 observe（真实 API 调用）：\n  this._observer.observe(<div>)  // target 已加入观察列表\n\n` +
    ...
    _simulateUnobserve() {
        ...
        if (!caps.io || !caps.unobserve) {
            this.setState({ lifecycleInfo: "observer.unobserve 用法（不可用，仅说明）：\n\n" +
        ...
        try {
            ...
            this.setState({ lifecycleInfo: `模拟 unobserve（真实 API 调用）：\n  this._observer.unobserve(<div>)  // target 已移出观察列表\n\n` +
    ...
    _simulateTakeRecords() {
        const caps = this._caps();
        if (!caps.io || !caps.takeRecords) {
            this.setState({ lifecycleInfo: "observer.takeRecords 用法（不可用，仅说明）：\n\n" +
    ...
    _simulateDisconnect() {
        ...
        if (!caps.io || !caps.disconnect) {
            this.setState({ lifecycleInfo: "observer.disconnect 用法（不可用，仅说明）：\n\n" +
        ...
        try {
            ...
            this._observedTargets = [];
            this.setState({ lifecycleInfo: `模拟 disconnect（真实 API 调用）：\n  this._observer.disconnect()  // 停止观察所有 target\n\n` +
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.observe ? 'success' : 'error' }, caps.observe ? 'observe ✓' : 'observe ✗'), h(Tag, { color: caps.takeRecords ? 'success' : 'error' }, caps.takeRecords ? 'takeRecords ✓' : 'takeRecords ✗'), h(Tag, { color: 'primary' }, 'v2 = v1 API')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'observer.observe(target) 加入观察；unobserve(target) 移除单个 target；disconnect() 移除全部并停止；takeRecords() 返回未交付的 entry 数组并清空队列。v2 与 v1 API 完全相同，差异仅在 options（trackVisibility/delay）与 entry 字段（isVisible）。takeRecords 在回调前同步读取 pending entries，常用于 disconnect 前避免丢失最后一批变更。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('模拟 observe', { type: 'primary', size: 'sm', disabled: !caps.observe, onClick: () => this._simulateObserve() }), this._btn('模拟 unobserve', { size: 'sm', disabled: !caps.unobserve, onClick: () => this._simulateUnobserve() }), this._btn('模拟 takeRecords', { size: 'sm', disabled: !caps.takeRecords, onClick: () => this._simulateTakeRecords() }), this._btn('模拟 disconnect', { danger: true, size: 'sm', disabled: !caps.disconnect, onClick: () => this._simulateDisconnect() })),
    ...
    // =================== Card 5：使用场景与限制 ===================
    _showIabViewability() {
        this.setState({ scenariosInfo: '===== IAB Viewability 广告可见性监测示例 =====\n\n' +
    ...
    _compareLazyLoadStrategies() {
        this.setState({ scenariosInfo: '===== 懒加载策略对比 =====\n\n' +
                '策略             | 触发条件                       | 适用场景              | 缺点\n' +
                '-----------------|--------------------------------|-----------------------|--------------------------\n' +
    ...
    _showExposureTrackingCode() {
        this.setState({ scenariosInfo: '===== 曝光埋点代码示例（v2）=====\n\n' +
    ...
    _showLimitsAndBestPractices() {
        this.setState({ scenariosInfo: '===== v2 限制与最佳实践 =====\n\n' +
                '限制：\n' +
    ...
    _renderCard5() {
        ...
        const card = new Card({
            title: '5. 使用场景与限制',
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.v2 ? 'success' : 'error' }, caps.v2 ? 'v2 ✓' : 'v2 模拟'), h(Tag, { color: 'primary' }, 'IAB Viewability'), h(Tag, { color: 'warning' }, '性能开销')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '使用场景：广告可见性监测（IAB Viewability：50% 像素 ≥1s 连续）、懒加载（isIntersecting 提前 + isVisible 兜底）、曝光埋点（isVisible=true 才计入）、防盗刷（isVisible=false 不计播放）。限制：trackVisibility 性能开销大（仅必要时启用）、delay 最小 100ms、rootMargin 必须 0、SecureContext（HTTPS）要求、移动端节电。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('IAB Viewability 示例', { type: 'primary', size: 'sm', onClick: () => this._showIabViewability() }), this._btn('懒加载策略对比', { size: 'sm', onClick: () => this._compareLazyLoadStrategies() }), this._btn('曝光埋点代码', { size: 'sm', onClick: () => this._showExposureTrackingCode() }), this._btn('限制与最佳实践', { danger: true, size: 'sm', onClick: () => this._showLimitsAndBestPractices() })),
    ...
    _showV1V2Diff() {
        this.setState({ compareInfo: '===== IntersectionObserver v1 vs v2 差异表 =====\n\n' +
    ...
    _compareIoPo() {
        this.setState({ compareInfo: '===== IntersectionObserver vs PerformanceObserver 对比 =====\n\n' +
    ...
    _showIoRoSynergy() {
        this.setState({ compareInfo: '===== IntersectionObserver + ResizeObserver 协同 =====\n\n' +
                '各自职责：\n' +
    ...
    _compareIoScroll() {
        this.setState({ compareInfo: '===== IntersectionObserver vs scroll 事件对比 =====\n\n' +
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'primary' }, 'v1 vs v2'), h(Tag, { color: 'info' }, 'IO vs PO'), h(Tag, { color: 'warning' }, 'IO + RO / vs scroll')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'v1 vs v2 API 差异表（options/entry 字段/限制）；与 PerformanceObserver 对比（PO 监听性能条目 vs IO 监听几何可见）；与 ResizeObserver 协同（IO 触发后 RO 监听尺寸，离开视口停止 RO 省性能）；与 scroll 事件对比（IO 替代 scroll+getBoundingClientRect 性能更优，浏览器内部相交检测主线程负担小）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('v1/v2 差异表', { type: 'primary', size: 'sm', onClick: () => this._showV1V2Diff() }), this._btn('IO vs PO 对比', { size: 'sm', onClick: () => this._compareIoPo() }), this._btn('IO + RO 协同', { size: 'sm', onClick: () => this._showIoRoSynergy() }), this._btn('IO vs scroll 对比', { size: 'sm', onClick: () => this._compareIoScroll() })),
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'api-lab-page intersection-observer-v2-page' }, h('h2', { class: 'section-title' }, 'IntersectionObserver v2 可见性追踪 实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, '本页深入演示 IntersectionObserver v2 的 trackVisibility / isVisible 可见性追踪（区分几何相交与实际可见）、rootMargin/threshold 配置（v2 rootMargin 必须 0px）、observe/unobserve/disconnect/takeRecords 生命周期，以及广告可见性、懒加载、曝光埋点等使用场景与限制，并对比 v1、PerformanceObserver、ResizeObserver、scroll 事件。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderCard6(), this._renderLogPanel());
    }
}
//# sourceMappingURL=IntersectionObserverV2Page.js.map