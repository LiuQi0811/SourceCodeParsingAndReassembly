...
export class SpatialAudioDeepPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!f.audioContext)
            this._addLog('warn', 'AudioContext 不可用（jsdom 无 Web Audio，真实浏览器需 Chrome/Firefox/Edge/Safari）');
        if (!f.pannerNode)
            this._addLog('warn', 'PannerNode 不可用（3D 空间化核心节点缺失）');
        if (!f.stereoPannerNode)
            this._addLog('warn', 'StereoPannerNode 不可用（简易立体声节点缺失，部分老浏览器）');
        if (!f.webxr)
            this._addLog('warn', 'WebXR 不可用（navigator.xr 缺失，头部跟踪演示不可用）');
        ...
        if (f.audioContext && f.pannerNode) {
            ...
            if (ctx) {
                try {
                    ...
                    const sourceEl = this.$('.spatial-source');
                    const tick = () => {
    ...
    _runWebXRDemo() {
        ...
        if (f.webxr) {
            try {
                ...
                Promise.resolve(supported).then((ok) => {
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '空间音频（Spatial Audio）= 3D 定位音频，模拟真实世界中声音随距离/方位/朝向衰减。核心：听者（AudioListener，BaseAudioContext.listener 单例）+ 声源（PannerNode）+ 双耳渲染（HRTF）。AudioListener 有 9 个 AudioParam：positionX/Y/Z（位置）、forwardX/Y/Z（朝向，默认 (0,0,-1)）、upX/Y/Z（头顶，默认 (0,1,0)）。AudioParam 可用 linearRampToValueAtTime 动画化，或 WebXR requestAnimationFrame 实时同步头部姿态。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 AudioListener 演示', { type: 'primary', size: 'sm', onClick: () => this._runListenerDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// BaseAudioContext.listener 只读，每 ctx 一个
const ctx = new AudioContext();
const listener = ctx.listener;
...
// 旧版（已废弃）：listener.setPosition(x,y,z) / setOrientation(fx,fy,fz,ux,uy,uz)`)),
                ...
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.listenerInfo || '（点击按钮查看空间音频概念与 AudioListener 完整说明）')),
            ],
        });
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'PannerNode 是 3D 空间化声源节点，构造时传 panningModel/distanceModel/positionXYZ/orientationXYZ/refDistance/maxDistance/rolloffFactor/cone* 等。panningModel 决定空间化算法：equalpower（等功率，CPU 低，无前后区分）vs HRTF（头相关传递函数，双耳渲染，CPU 高 ~5-10x，定位清晰）。connect 拓扑：Source → PannerNode → destination。运行时可动态切换 panningModel。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 PannerNode 基础演示', { type: 'primary', size: 'sm', onClick: () => this._runPannerBasicDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// 构造 PannerNode（推荐直接传 options）
const panner = new PannerNode(ctx, {
...
panner.panningModel = 'equalpower'; // 等功率，CPU 低`)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.pannerBasicInfo || '（点击按钮查看 PannerNode 构造与 panningModel 完整说明）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'PannerNode.positionX/Y/Z 是声源世界坐标（AudioParam），orientationX/Y/Z 是声源朝向（指向性声源辐射方向）。可用 value 直接赋值、linearRampToValueAtTime 平滑过渡、setValueCurveAtTime 自定义轨迹曲线，或 requestAnimationFrame 实时更新（游戏中声源跟随物体）。下方场景可视化声源绕听者旋转（戴耳机体验 HRTF 双耳渲染）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('启动声源旋转动画', { type: 'primary', size: 'sm', onClick: () => this._runPositionDemo() })),
                h('div', { class: 'spatial-scene' }, h('div', { class: 'spatial-axes' }, h('div', { class: 'spatial-axis-x' }), h('div', { class: 'spatial-axis-z' })), h('div', { class: 'spatial-listener' }), h('div', { class: 'spatial-source', style: { left: '75%', top: '50%' } })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// 声源位置（AudioParam）
const panner = new PannerNode(ctx, { panningModel: 'HRTF' });
...
animate();`)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.positionInfo || '（点击按钮查看声源位置与朝向完整说明，真实浏览器可听到旋转声效）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'PannerNode.distanceModel 控制声源音量随距离衰减：linear（线性，物理不真实但简单，已被建议废弃）、inverse（反比，物理真实的点声源球面发散，默认推荐）、exponential（指数，更陡峭）。refDistance 参考距离（内全音量）、maxDistance 最大距离（外不再衰减）、rolloffFactor 衰减速率（越大衰减越快，linear 限制 0-1）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行距离衰减演示', { type: 'primary', size: 'sm', onClick: () => this._runDistanceDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// 三种 distanceModel
...
oscillator.connect(panner).connect(ctx.destination);`)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.distanceInfo || '（点击按钮查看距离衰减模型完整说明）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'PannerNode 声锥（Sound Cone）实现指向性声源：coneInnerAngle（内锥全音量）、coneOuterAngle（外锥衰减到 coneOuterGain）、配合 orientationX/Y/Z 朝向。默认 360° 全向。StereoPannerNode 是 2D 简化版（仅 pan -1~+1 左右声像，无距离/朝向，CPU 极低）。channelCount 默认 2，HRTF 模式强制双声道输出。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行声锥与立体声演示', { type: 'primary', size: 'sm', onClick: () => this._runConeDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// 声锥（指向性声源）
...
const panner = new PannerNode(ctx, {
  ...
  positionX: 3, positionZ: -5,
                ...
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.coneInfo || '（点击按钮查看声锥与立体声完整说明）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'WebXR 沉浸式 VR/AR 中，用 requestAnimationFrame 同步头部姿态到 AudioListener：XRRigidTransform 提供 position + orientation（四元数），将四元数转换为 forward/up 向量写入 listener 的 6 个朝向 AudioParam，position 写入 3 个位置 AudioParam。声源与听者在同一 XR 参考空间（local-floor）坐标系。无 WebXR 时可用 DeviceOrientationEvent 近似头部跟踪。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 WebXR 协同演示', { type: 'primary', size: 'sm', onClick: () => this._runWebXRDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// WebXR 头部姿态 → AudioListener 同步
let xrRefSpace = null;
session.requestReferenceSpace('local-floor').then((rs: any) => {
  xrRefSpace = rs;
  session.requestAnimationFrame(onXRFrame);
...
window.addEventListener($1, (e: any) => {
  // alpha/beta/gamma → 欧拉角 → 写入 listener
                ...
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.webxrInfo || '（点击按钮查看 WebXR 协同完整说明）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard7() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'ChannelMergerNode 将多个单声道合并为多声道信号（5.1 环绕 6 声道），ChannelSplitterNode 反向拆分。MediaElementAudioSourceNode 把 <video>/<audio> 接入 Web Audio 图进行空间化处理。Ambisonics（高阶环绕声）用球谐函数编码全景声：FOA 4 声道 W/X/Y/Z，可整体旋转（头部跟踪高效），再用 ConvolverNode 做 HRTF 卷积解码为双耳。大量声源 + 头部跟踪时 Ambisonics 比 PannerNode 更高效。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行多声道与 Ambisonics 演示', { type: 'primary', size: 'sm', onClick: () => this._runMultichannelDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// ChannelMergerNode 多声道合并（5.1）
const merger = new ChannelMergerNode(ctx, { numberOfInputs: 6 });
src0.connect(merger, 0, 0);  // 第 1 声道
src1.connect(merger, 0, 1);  // 第 2 声道
// ... 6 个声道
...
// ... X/Y/Z 各做一次卷积`)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.multichannelInfo || '（点击按钮查看多声道与 Ambisonics 完整说明）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard8() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'HRTF 比 equalpower CPU 开销高 5-10x（频域卷积），桌面 Chrome HRTF 流畅 ~32 个、移动端 ~16 个，equalpower 可达数百个。AudioWorklet 可实现自定义 HRTF 卷积（个性化测量数据）。ResonanceAudio（Google）是基于 Web Audio 的高质量空间音频库，自实现 HRTF + 房间声学 + 高阶 Ambisonics。性能优化：静态声源用 equalpower、超 maxDistance 断开、Ambisonics 合并、移动端降级 StereoPannerNode。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行性能与生态演示', { type: 'primary', size: 'sm', onClick: () => this._runPerfDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// AudioWorklet 自定义 HRTF 卷积
class HRTFProcessor extends AudioWorkletProcessor {
  process(inputs: any, outputs: any){
...
oscillator.connect(source.input);`)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.perfInfo || '（点击按钮查看性能与生态完整说明）')),
            ],
        });
        this.registerChild(card);
        return card.render();
...
//# sourceMappingURL=SpatialAudioDeepPage.js.map