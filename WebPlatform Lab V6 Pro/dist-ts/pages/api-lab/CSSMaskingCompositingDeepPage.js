...
export class CSSMaskingCompositingDeepPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!f.maskMode)
            this._addLog('warn', 'mask-mode 不可用（Chrome 120+ 标准无前缀）');
        if (!f.maskBorderSource)
            this._addLog('warn', 'mask-border 浏览器支持有限（Chrome 部分支持，Firefox 实验性）');
        if (!f.maskComposite)
            this._addLog('info', 'mask-composite 部分浏览器需 -webkit- 前缀（Safari）');
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'mask: <mask-layer># 简写遮罩属性。mask-image: none|<image>|<image-set> 指定遮罩图像，alpha 通道决定可见性（黑色不透明可见/透明不可见）。与 clip-path 区别：mask 用 alpha/luminance 渐变切边可羽化，clip-path 硬切边。-webkit-mask-* 前缀历史：Chrome 120+ 标准无前缀，Safari 仍需 -webkit-，Firefox 早支持无前缀。实战双写保证兼容。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 mask 基础演示', { type: 'primary', size: 'sm', onClick: () => this._runOverviewDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '300px', overflow: 'auto' } }, h('code', {}, `.feather {
  -webkit-mask: linear-gradient(black, transparent);
          mask: linear-gradient(black, transparent);
}
...
.multi-mask {
  -webkit-mask:
    ...
    linear-gradient(transparent, black);
                ...
                h('div', { class: 'mk-overview-host' }, h('div', { class: 'mk-overview-box' }, '原图'), h('div', { class: 'mk-overview-box mk-overview-alpha' }, '线性遮罩'), h('div', { class: 'mk-overview-box mk-overview-radial' }, '径向遮罩')),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.overviewInfo || '（点击按钮查看 mask 基础完整用法）')),
            ],
        });
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['mask-image', f.maskImage]]), h(Tag, { color: 'primary' }, '多遮罩')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'mask-image: url(mask.png)/linear-gradient()/radial-gradient()/image-set() 指定遮罩图像。多重遮罩叠加逗号分隔（先写的在上层），与 background-image 同构 API。mask 配合渐变实现羽化边缘。SVG mask 元素引用 mask-image: url(#svgMask)。mask-image（整张遮罩）vs mask-border-source（九宫格切片遮罩）区别。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 mask-image 演示', { type: 'primary', size: 'sm', onClick: () => this._runMaskImageDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '300px', overflow: 'auto' } }, h('code', {}, `/* 多层遮罩叠加（先写的在上层） */
.multi {
  -webkit-mask:
    radial-gradient(circle at 30% 50%, black 25%, transparent 25%),
    radial-gradient(circle at 70% 50%, black 25%, transparent 25%),
...
.retina {
  -webkit-mask: image-set(url(mask.png) 1x, url(mask@2x.png) 2x);
          mask: image-set(url(mask.png) 1x, url(mask@2x.png) 2x);
                ...
                h('div', { class: 'mk-mi-host' }, h('div', { class: 'mk-mi-box mk-mi-linear' }, '线性'), h('div', { class: 'mk-mi-box mk-mi-radial' }, '径向'), h('div', { class: 'mk-mi-box mk-mi-multi' }, '多层'), h('div', { class: 'mk-mi-box mk-mi-svg' }, 'SVG')),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.maskImageInfo || '（点击按钮查看 mask-image 多遮罩完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'mask-mode: match-source|alpha|luminance 三种模式。match-source 默认根据图像类型自动选择（PNG 用 alpha、SVG 用 luminance）。alpha 模式使用图像 alpha 通道，luminance 模式使用图像亮度（白色不透明黑色透明）。mask-type 仅用于 SVG <mask> 元素 mask-type: luminance|alpha（无 match-source）。实战：彩色 SVG 遮罩用 luminance。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 mask-mode 演示', { type: 'primary', size: 'sm', onClick: () => this._runMaskModeDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '300px', overflow: 'auto' } }, h('code', {}, `/* alpha 模式：使用图像 alpha 通道 */
.alpha-mask {
  -webkit-mask: linear-gradient(black, transparent);
  -webkit-mask-mode: alpha;
          mask: linear-gradient(black, transparent);
...
<mask id="m" mask-type="luminance">
  <rect fill="white" .../>
                ...
                h('div', { class: 'mk-mm-host' }, h('div', { class: 'mk-mm-box' }, '原图'), h('div', { class: 'mk-mm-box mk-mm-alpha' }, 'alpha'), h('div', { class: 'mk-mm-box mk-mm-luminance' }, 'luminance')),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.maskModeInfo || '（点击按钮查看 mask-mode 与 mask-type 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['mask', f.mask]]), h(Tag, { color: 'primary' }, '同构 API')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '与 background-* 完全同构的 API：mask-repeat: repeat|no-repeat|repeat-x|repeat-y|space|round；mask-position: <position>（中心/角/百分比）；mask-size: auto|<length>|<percentage>|cover|contain；mask-origin: border-box|padding-box|content-box|fill-box|stroke-box|view-box；mask-clip 同 mask-origin 但影响剪切区域。mask 简写顺序与 background 一致。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 mask 盒模型演示', { type: 'primary', size: 'sm', onClick: () => this._runMaskBoxPropsDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '300px', overflow: 'auto' } }, h('code', {}, `/* 重复网格遮罩 */
.grid-mask {
  -webkit-mask: radial-gradient(circle, black 5px, transparent 6px);
          mask: radial-gradient(circle, black 5px, transparent 6px);
  -webkit-mask-size: 20px 20px;
...
.shorthand {
  mask: url(mask.png) center / cover no-repeat border-box;
                ...
                h('div', { class: 'mk-mb-host' }, h('div', { class: 'mk-mb-box mk-mb-repeat' }, 'repeat'), h('div', { class: 'mk-mb-box mk-mb-no-repeat' }, 'no-repeat'), h('div', { class: 'mk-mb-box mk-mb-cover' }, 'cover'), h('div', { class: 'mk-mb-box mk-mb-position' }, 'position')),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.maskBoxPropsInfo || '（点击按钮查看 mask 盒模型属性完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['mask-composite', f.maskComposite]]), h(Tag, { color: 'primary' }, '合成')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'mask-composite: add|subtract|intersect|exclude 四种合成运算。add 默认新旧遮罩并集，subtract 旧遮罩减新遮罩，intersect 交集，exclude 对称差（异或）。多层 mask 合成顺序从下往上。与 SVG mask-mode/mask-type 配合。实战：圆环（subtract）、十字形（add）、复杂遮罩叠加。mask-composite（alpha 合成）vs background-blend-mode（颜色混合）区别。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 mask-composite 演示', { type: 'primary', size: 'sm', onClick: () => this._runMaskCompositeDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '300px', overflow: 'auto' } }, h('code', {}, `/* 圆环：大圆减中圆 */
.ring {
  -webkit-mask:
    radial-gradient(circle, black 40%, transparent 40%),
    radial-gradient(circle, black 20%, transparent 20%);
...
.cross {
  ...
  -webkit-mask-composite: source-over;
          mask-composite: add;
                ...
                h('div', { class: 'mk-mc-host' }, h('div', { class: 'mk-mc-box mk-mc-add' }, 'add'), h('div', { class: 'mk-mc-box mk-mc-subtract' }, 'subtract'), h('div', { class: 'mk-mc-box mk-mc-intersect' }, 'intersect'), h('div', { class: 'mk-mc-box mk-mc-exclude' }, 'exclude')),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.maskCompositeInfo || '（点击按钮查看 mask-composite 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['mask-border-source', f.maskBorderSource]]), h(Tag, { color: 'warning' }, '支持有限')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'mask-border: <source> <slice> <width> <outset> <repeat> <mode> 简写。mask-border-source 遮罩源，mask-border-slice 九宫格切片，mask-border-width/outset/repeat 与 border-image 同构，mask-border-mode: alpha|luminance。应用：复杂边框遮罩效果。浏览器支持有限（Chrome 部分支持、Firefox 实验性、Safari 不支持）。与 border-image 区别：mask-border 是遮罩（控制可见性），border-image 是边框装饰。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 mask-border 演示', { type: 'primary', size: 'sm', onClick: () => this._runMaskBorderDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '300px', overflow: 'auto' } }, h('code', {}, `/* 完整简写 */
.frame {
...
.frame-detail {
  ...
  -webkit-mask-border-mode: alpha;
          mask-border-mode: alpha;
                ...
                h('div', { class: 'mk-mbd-host' }, h('div', { class: 'mk-mbd-box mk-mbd-stretch' }, 'stretch'), h('div', { class: 'mk-mbd-box mk-mbd-repeat' }, 'repeat')),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.maskBorderInfo || '（点击按钮查看 mask-border 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard7() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['mask-type', f.maskType]]), h(Tag, { color: 'primary' }, 'SVG mask')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'SVG <mask id="x" maskUnits="userSpaceOnUse|objectBoundingBox" maskContentUnits> 元素。mask="url(#x)" SVG 内引用，CSS mask-image: url(#svgMask) HTML 引用 SVG mask。maskUnits 坐标系（userSpaceOnUse 用户坐标/objectBoundingBox 0-1 归一化），maskContentUnits 遮罩内容坐标系。SVG mask 内可放任意图形（rect/circle/path/渐变/文字）。SVG mask vs CSS mask-image 优劣对比。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 SVG mask 演示', { type: 'primary', size: 'sm', onClick: () => this._runSvgMaskDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '300px', overflow: 'auto' } }, h('code', {}, `<!-- SVG mask 元素定义 -->
<svg width="0" height="0">
  <defs>
  ...
  .masked {
    -webkit-mask-image: url(#circleMask);
            mask-image: url(#circleMask);
                ...
                h('div', { class: 'mk-sm-host' }, h('div', { class: 'mk-sm-box mk-sm-circle' }, 'circle'), h('div', { class: 'mk-sm-box mk-sm-rect' }, 'rect'), h('div', { class: 'mk-sm-box mk-sm-path' }, 'path'), h('div', { class: 'mk-sm-box mk-sm-complex' }, 'complex')),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.svgMaskInfo || '（点击按钮查看 SVG mask 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
...
.shine {
  ...
  animation: shine 2s linear infinite;
                ...
                h('div', { class: 'mk-pt-host' }, h('div', { class: 'mk-pt-box mk-pt-feather' }, '羽化'), h('div', { class: 'mk-pt-text' }, 'MASK'), h('div', { class: 'mk-pt-box mk-pt-irregular' }, '三角'), h('div', { class: 'mk-pt-box mk-pt-fade' }, 'hover'), h('div', { class: 'mk-pt-glass' }, 'glass')),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.patternInfo || '（点击按钮查看实战模式与陷阱完整指南）')),
            ],
        });
        this.registerChild(card);
        return card.render();
...
//# sourceMappingURL=CSSMaskingCompositingDeepPage.js.map