...
//   3. Element Capture（元素级捕获）—— (RestrictionTarget as any).fromElement(element)
...
export class CaptureAdvancedPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!caps.getDisplayMedia)
            this._addLog('warn', 'navigator.mediaDevices.getDisplayMedia 不可用（jsdom 无 MediaDevices），演示仅记日志');
        if (!caps.captureController)
            this._addLog('warn', 'CaptureController 不可用（Chrome 116+），surfaceSwitching 仅说明');
        if (!caps.restrictionTarget)
            this._addLog('warn', 'RestrictionTarget.fromElement 不可用（Chrome 121+），Element Capture 仅说明');
        if (!caps.clipboardItem)
            this._addLog('warn', 'ClipboardItem 不可用（全主流 2022+），演示仅记日志');
        if (!caps.clipboardWrite)
            this._addLog('warn', 'navigator.clipboard.write 不可用（需安全上下文+权限），演示仅记日志');
    ...
    _renderCard1() {
        ...
        }, h('p', { class: 'fs-sm text-secondary' }, 'navigator.mediaDevices.getDisplayMedia({video, audio, displaySurface, monitorTypeSurfaces, surfaceSwitching, systemAudio}) 返回 MediaStream；' +
            'displaySurface: monitor|window|browser|application；track.getSettings() 读 displaySurface/cursor；track.onended 监听停止。'), h('div', { class: 'ca-demo-box' }, h('p', { class: 'fs-sm' }, '点击下方按钮请求屏幕共享。jsdom 无 MediaDevices，仅记日志说明调用方式。')), h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('请求屏幕共享', { type: 'primary', size: 'sm', onClick: () => this._requestDisplayMedia() }), h(Tag, { color: 'default' }, 'displaySurface: monitor|window|browser|application')), s.gdmState ? h('p', { class: 'fs-sm mt-sm text-secondary' }, s.gdmState) : null, h('pre', { class: 'code-block mt-md' }, `const stream = await navigator.mediaDevices.getDisplayMedia({
...
track.onended = () => { /* 用户停止共享 */ };`));
        ...
        try {
            ...
            if (focusSupported) {
                try {
                    ...
                    this._addLog('info', 'controller.setFocus(true) 已调用（聚焦共享源窗口）');
                }
                catch (e) {
                    this._addLog('warn', 'setFocus 调用失败：' + (e && e.message));
                }
            }
            if (surfaceSupported) {
                try {
                    controller.setDisplaySurface('monitor');
                    this._addLog('info', 'controller.setDisplaySurface("monitor") 已调用（运行时切换共享源 surfaceSwitching）');
                }
                catch (e) {
                    this._addLog('warn', 'setDisplaySurface 调用失败：' + (e && e.message));
                }
            }
            this.setState({
    ...
    _renderCard2() {
        ...
        }, h('p', { class: 'fs-sm text-secondary' }, 'new CaptureController() 传入 getDisplayMedia({controller}) 后可调用 controller.setFocus(focusable) 聚焦共享源、' +
            'controller.setDisplaySurface(surface) 运行时切换共享源（Chrome 116+ surfaceSwitching），无需重新申请授权。'), h('div', { class: 'ca-demo-box' }, h('p', { class: 'fs-sm' }, '点击按钮演示构造 CaptureController 并调用 setFocus / setDisplaySurface（jsdom 记日志）。')), h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('构造 CaptureController', { type: 'primary', size: 'sm', onClick: () => this._demoCaptureController() }), h(Tag, { color: caps.cropTo ? 'success' : 'error' }, caps.cropTo ? 'cropTo ✓' : 'cropTo ✗')), s.controllerState ? h('p', { class: 'fs-sm mt-sm text-secondary' }, s.controllerState) : null, h('pre', { class: 'code-block mt-md' }, `const controller = new CaptureController();
...
controller.cropTo(restrictionTarget); // Element/Region Capture 裁剪`));
    ...
    _convertToRestrictionTarget() {
        ...
        if (!el) {
            this._addLog('warn', '未找到演示目标元素');
            return;
        }
        try {
            const rt = RestrictionTarget.fromElement(el);
            this._addLog('info', '(RestrictionTarget as any).fromElement(el) 成功：得到 RestrictionTarget 对象');
    ...
    _renderCard3() {
        ...
        }, h('p', { class: 'fs-sm text-secondary' }, '(RestrictionTarget as any).fromElement(element) 把 DOM 元素转为 RestrictionTarget；controller.cropTo(restrictionTarget) 把整页捕获裁剪到该元素；' +
            '配合 sendFocus/event focus；vs Region Capture 整页裁剪。Chrome 121+。'), h('div', { class: 'ca-demo-box' }, h('p', { class: 'fs-sm' }, '下方蓝色框为演示捕获目标，点击按钮可转为 RestrictionTarget：'), h('div', { id: 'ca-target', class: 'ca-target' }, h('p', { class: 'fs-sm' }, '我是被捕获目标元素（demo div）。'), h('p', { class: 'fs-sm' }, 'Element Capture 后，仅此区域被共享，其他 UI 不可见。'))), h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('转为 RestrictionTarget', { type: 'primary', size: 'sm', onClick: () => this._convertToRestrictionTarget() }), h(Tag, { color: caps.cropTo ? 'success' : 'error' }, caps.cropTo ? 'cropTo ✓' : 'cropTo ✗')), s.elementCaptureState ? h('p', { class: 'fs-sm mt-sm text-secondary' }, s.elementCaptureState) : null, h('pre', { class: 'code-block mt-md' }, `const targetEl = document.querySelector('#capture-me');
const rt = await (RestrictionTarget as any).fromElement(targetEl); // Chrome 121+
const controller = new CaptureController();
...
// 安全：不暴露无关 UI/隐私，仅共享该元素`));
    ...
    _renderCard4() {
        ...
        }, h('p', { class: 'fs-sm text-secondary' }, '对比三种捕获模式：getDisplayMedia 原始全标签页捕获 vs Region Capture（cropTo 裁剪到 CSS 区域）vs Element Capture（只共享某元素）。' +
            '安全模型：裁剪后不暴露无关 UI/隐私。displaySurface: browser（标签页）/ window / screen。'), h('div', { class: 'ca-compare' }, h('div', { class: 'ca-compare-row' }, h('div', { class: 'ca-compare-label' }, '全标签页捕获'), h('div', { class: 'fs-sm' }, 'getDisplayMedia({video:true}) 默认共享整个标签页，包含地址栏以外全部内容，无裁剪，可能暴露无关 UI。')), h('div', { class: 'ca-compare-row' }, h('div', { class: 'ca-compare-label' }, 'Region Capture'), h('div', { class: 'fs-sm' }, '整页捕获后用 controller.cropTo(restrictionTarget) 按 CSS 区域裁剪，运行时可调整区域，隐藏区域不外泄。')), h('div', { class: 'ca-compare-row' }, h('div', { class: 'ca-compare-label' }, 'Element Capture'), h('div', { class: 'fs-sm' }, '(RestrictionTarget as any).fromElement(el) + cropTo(rt) 只共享该元素，元素滚动/变换自动跟随，最精确。'))), h('table', { class: 'ca-matrix' }, h('thead', {}, h('tr', {}, h('th', {}, '模式'), h('th', {}, 'displaySurface'), h('th', {}, '裁剪粒度'), h('th', {}, '隐私暴露'), h('th', {}, '运行时调整'))), h('tbody', {}, h('tr', {}, h('td', {}, '全标签页'), h('td', {}, 'browser'), h('td', {}, '无裁剪'), h('td', {}, '高（整页）'), h('td', {}, '不支持')), h('tr', {}, h('td', {}, 'Region Capture'), h('td', {}, 'browser'), h('td', {}, 'CSS 区域'), h('td', {}, '低（仅区域）'), h('td', {}, '支持（cropTo）')), h('tr', {}, h('td', {}, 'Element Capture'), h('td', {}, 'browser'), h('td', {}, '单个元素'), h('td', {}, '最低（仅元素）'), h('td', {}, '支持（跟随元素）')))), h('pre', { class: 'code-block mt-md' }, `// Region Capture：裁剪到 CSS 区域
const rt = await (RestrictionTarget as any).fromElement(regionEl);
...
// displaySurface: 'browser' | 'window' | 'screen' | 'application'`));
    ...
    _renderCard5() {
        ...
        const supportsPlain = caps.clipboardItemSupports
            ? (() => { try {
                return ClipboardItem.supports('text/plain');
            }
            catch {
                return false;
            } })()
        ...
        const supportsWebCustom = caps.clipboardItemSupports
            ? (() => { try {
                return ClipboardItem.supports('web custom/format');
            }
            catch {
                return false;
            } })()
        ...
        }, h('p', { class: 'fs-sm text-secondary' }, 'new ClipboardItem({ "text/plain": Promise.resolve(blob), "image/png": asyncBlobPromise }) 支持懒加载（写入时才 resolve）；' +
            'ClipboardItem.supports(type) 静态检测格式；自定义 MIME 用 web 前缀；一次写多种格式让目标应用自选；navigator.clipboard.write([item])。'), h('div', { class: 'ca-demo-box' }, h('p', { class: 'fs-sm' }, '支持格式检测（ClipboardItem.supports）：'), h('div', { class: 'flex items-center gap-sm flex-wrap' }, h(Tag, { color: supportsPlain ? 'success' : 'error' }, `text/plain ${supportsPlain ? '✓' : '✗'}`), h(Tag, { color: supportsPng ? 'success' : 'error' }, `image/png ${supportsPng ? '✓' : '✗'}`), h(Tag, { color: supportsWebCustom ? 'success' : 'error' }, `web custom/format ${supportsWebCustom ? '✓' : '✗'}`))), h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('异步生成并写入剪贴板', { type: 'primary', size: 'sm', onClick: () => this._asyncWriteClipboard() }), h(Tag, { color: caps.clipboardWrite ? 'success' : 'error' }, caps.clipboardWrite ? 'clipboard.write ✓' : 'clipboard.write ✗')), s.clipboardState ? h('p', { class: 'fs-sm mt-sm text-secondary' }, s.clipboardState) : null, h('pre', { class: 'code-block mt-md' }, `// 懒加载：值是 Promise<Blob>，clipboard.write 时才 resolve
...
await navigator.clipboard.write([item]);  // 一次写多种格式，目标应用自选`));
    ...
    _renderCard6() {
        const caps = this._caps();
        const mk = (api, version, scene, ok) => h('tr', {}, h('td', {}, api), h('td', {}, version), h('td', {}, scene), h('td', {}, ok ? '✓' : '✗'));
        return h(Card, {
            title: 'Card 6 · 能力检测矩阵与决策',
            extra: h(Tag, { color: 'default' }, '场景推荐'),
        }, h('p', { class: 'fs-sm text-secondary' }, '对比 Element/Region Capture/ClipboardItem/CaptureController/getDisplayMedia 的 Chrome 版本与场景；' +
            '场景推荐：远程桌面=Element Capture、演示共享=Region Capture、富文本复制=ClipboardItem 多格式。'), h('table', { class: 'ca-matrix' }, h('thead', {}, h('tr', {}, h('th', {}, 'API'), h('th', {}, 'Chrome 版本'), h('th', {}, '典型场景'), h('th', {}, '当前可用'))), h('tbody', {}, mk('getDisplayMedia', 'Chrome 72+', '屏幕/窗口/标签页共享基础', caps.getDisplayMedia), mk('CaptureController', 'Chrome 116+', '运行时切换共享源 surfaceSwitching', caps.captureController), mk('RestrictionTarget + cropTo (Element)', 'Chrome 121+', '远程桌面/协作工具只共享某元素', caps.restrictionTarget && caps.cropTo), mk('Region Capture (cropTo 区域)', 'Chrome 104+', '演示共享裁剪到 CSS 区域', caps.cropTo), mk('ClipboardItem (Promise 懒加载)', 'Chrome 76+ / 全主流', '富文本/图片多格式复制', caps.clipboardItem), mk('ClipboardItem.supports', 'Chrome 98+', '检测格式可用性', caps.clipboardItemSupports), mk('navigator.clipboard.write', 'Chrome 66+ / 全主流', '异步写剪贴板（需安全上下文）', caps.clipboardWrite))), h('div', { class: 'ca-demo-box' }, h('p', { class: 'fs-sm' }, h('strong', {}, '场景推荐：')), h('p', { class: 'fs-sm' }, '远程桌面/在线协作 → Element Capture：只共享指定元素，最精确、隐私最安全。'), h('p', { class: 'fs-sm' }, '演示共享/教学直播 → Region Capture：裁剪到 CSS 区域，可运行时调整。'), h('p', { class: 'fs-sm' }, '富文本/图片复制 → ClipboardItem 多格式：一次写 text/plain + image/png，目标应用自选最佳格式。'), h('p', { class: 'fs-sm' }, '运行时切换共享源 → CaptureController + surfaceSwitching：无需重新授权。')), h('pre', { class: 'code-block mt-md' }, `// 决策树
...
//# sourceMappingURL=CaptureAdvancedPage.js.map