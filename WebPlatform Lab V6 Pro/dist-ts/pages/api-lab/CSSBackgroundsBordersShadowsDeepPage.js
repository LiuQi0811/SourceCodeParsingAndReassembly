...
export class CSSBackgroundsBordersShadowsDeepPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!f.bgClipText)
            this._addLog('warn', 'background-clip: text 不可用（需 -webkit- 前缀，渐变文字场景常用）');
        if (!f.backdropFilter)
            this._addLog('warn', 'backdrop-filter 不可用（毛玻璃效果需前缀或后备）');
        if (!f.boxDecoBreak)
            this._addLog('info', 'box-decoration-break: clone 在 Firefox 用 -webkit- 前缀');
        if (!f.borderImage)
            this._addLog('info', 'border-image 切片单位/浏览器支持差异较大');
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'background-image: url(a), url(b), linear-gradient(...) 多层叠加，先写的在上层，background-color 永远最底层且单值。background 简写每层用逗号分隔，position 与 size 用 / 分隔。各分项属性（color/image/repeat/attachment/position/size/origin/clip）可多层逗号对应。background-blend-mode: multiply/screen/overlay/... 在同一元素多层 image 间混合，与 mix-blend-mode（跨元素）区别。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行多重背景演示', { type: 'primary', size: 'sm', onClick: () => this._runMultipleBgsDemo() })),
                h('div', { class: 'bb-multi-bg' }, '多层叠加 + blend-mode'),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.multipleBgsInfo || '（点击按钮查看多重背景完整用法）')),
            ],
        });
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'background-clip: border-box|padding-box|content-box|text（text 实现渐变文字，需 -webkit- 前缀 + color: transparent）。background-origin: border-box|padding-box|content-box 决定 background-position 起始原点（与 clip 区别：origin 定位起点，clip 裁剪范围）。background-size: cover|contain|<length>|<percentage>，cover 铺满可能裁剪，contain 完整可能留白。background-attachment: scroll|fixed|local，fixed 视差/local 容器内滚动。简写中 position / size 用 / 分隔。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 clip/origin/size 演示', { type: 'primary', size: 'sm', onClick: () => this._runBgClipDemo() })),
                h('div', { class: 'bb-clip-text' }, '渐变文字 GRADIENT'),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.bgClipInfo || '（点击按钮查看 background-clip/origin/size/attachment 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'border-radius: 1/2/3/4 值语法（左上/右上/右下/左下顺时针，缺失取对角）。"/" 分隔水平/垂直半径实现椭圆角：border-radius: 50px / 30px。分项属性 border-top-left-radius 等（最多两值水平垂直）。百分比圆角（50% 圆形 / 不规则 blob）。outline 不跟随圆角（仍是矩形），table border-collapse: collapse 时圆角无效。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 border-radius 演示', { type: 'primary', size: 'sm', onClick: () => this._runBorderRadiusDemo() })),
                h('div', { class: 'bb-radius-mix' }),
                h('div', { class: 'bb-radius-circle' }),
                h('div', { class: 'bb-radius-percent' }),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.borderRadiusInfo || '（点击按钮查看 border-radius 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'border-image: source slice / width / outset repeat 简写。border-image-slice 九宫格切片（无单位！1=1像素），4 角直接放置、4 边按 repeat 拉伸/平铺、中心默认丢弃（fill 关键字填充）。border-image-repeat: stretch|repeat|round|space。border-image 优先级高于 border-color/style，但必须先设 border-width 占位。不支持 border-radius（渐变圆角边框需两层 div 模拟）。渐变也可作为 source。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 border-image 演示', { type: 'primary', size: 'sm', onClick: () => this._runBorderImageDemo() })),
                h('div', { class: 'bb-bi-box' }, '九宫格切片'),
                h('div', { class: 'bb-bi-stretch' }, '渐变边框'),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.borderImageInfo || '（点击按钮查看 border-image 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'box-shadow: h-offset v-offset blur spread color inset，多层用逗号分隔（先写在上层），inset 内阴影，spread 扩散（0 0 0 3px 实心环做焦点环）。多层不同 blur 模拟真实物理阴影与霓虹光晕。性能：大模糊半径触发重绘，控制在 30px 内、3 层内。filter: drop-shadow() 跟随透明 PNG/SVG 形状但不支持 inset/spread，性能略差。box-shadow 跟随 border-radius 但不跟随 border-image。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 box-shadow 演示', { type: 'primary', size: 'sm', onClick: () => this._runBoxShadowDemo() })),
                h('div', { class: 'bb-shadow-multi' }),
                h('div', { class: 'bb-shadow-inset' }),
                h('div', { class: 'bb-shadow-neon' }, 'NEON'),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.boxShadowInfo || '（点击按钮查看 box-shadow 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['clone', f.boxDecoBreak]]), h(Tag, { color: 'primary' }, 'Visual L3')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'box-decoration-break: slice|clone 控制内联元素跨多行时边框/背景/阴影的绘制方式。slice（默认）：整体切开，边框/圆角仅在首行左和末行右绘制。clone：每行独立绘制完整边框、背景、圆角、阴影（像多个独立小标签拼接）。Firefox 仅支持 -webkit- 前缀。最常用场景：多行高亮关键词标签、文章内链接背景、代码内联背景。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 box-decoration-break 演示', { type: 'primary', size: 'sm', onClick: () => this._runBoxDecoDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, 'clone 演示（多行内联）：'),
                h('p', { style: { maxWidth: '200px', lineHeight: '1.8' } }, '这是一段较长的文字用来测试 box-decoration-break 的 clone 效果，', h('span', { class: 'bb-deco-clone' }, '这段高亮关键词会跨多行，每行都独立绘制完整背景和圆角'), '，看效果。'),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.boxDecoInfo || '（点击按钮查看 box-decoration-break 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard7() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'outline: width style color 简写，类似 border 但不占布局空间、不触发 reflow、不跟随 border-radius、不支持单边。outline-offset 可正可负（正向画在元素外，负向画在元素内）。:focus-visible 仅在键盘焦点时显示轮廓（现代无障碍焦点环最佳实践）。outline 不支持 transition，需过渡时用 box-shadow: 0 0 0 3px ... 替代（跟随圆角）。*:focus { outline: none } 会破坏无障碍，必须配 :focus-visible 恢复。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 outline 演示', { type: 'primary', size: 'sm', onClick: () => this._runOutlineDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '用 Tab 键聚焦下方按钮查看 :focus-visible：'),
                h('button', { class: 'bb-outline-btn', type: 'button' }, 'Focus Me (Tab)'),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.outlineInfo || '（点击按钮查看 outline 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard8() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'primary' }, '实战'), h(Tag, { color: f.bgClipText ? 'success' : 'error' }, `clip:text ${f.bgClipText ? '✓' : '✗'}`), h(Tag, { color: f.backdropFilter ? 'success' : 'error' }, `backdrop ${f.backdropFilter ? '✓' : '✗'}`)),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '10 种实战模式 + 10 个陷阱清单。模式：渐变文字（background-clip:text + -webkit- 前缀）/ 多层背景叠加图案（background-blend-mode）/ 阴影立体感（多层不同 blur）/ 毛玻璃卡片（backdrop-filter+border）/ 霓虹光晕（多层 box-shadow 多层模糊）/ 按钮 hover 阴影动画（transform + box-shadow 过渡）/ 焦点环无障碍（:focus-visible + 双层 box-shadow）/ 渐变边框圆角（两层 div 模拟）/ 纸张纹理 / 双色调滤镜。陷阱：clip:text 必须 -webkit- / 大模糊性能 / slice 无单位 / outline 不跟随圆角 / *:focus 破坏无障碍 / blend-mode 层数匹配 / table 圆角 / backdrop-filter 兼容 / border-image 占位 / position / size 顺序。'),
                ...
                h('div', { class: 'bb-shadow-neon' }, 'NEON'),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.patternInfo || '（点击按钮查看 10 种实战模式 + 10 个陷阱清单）')),
            ],
        });
        this.registerChild(card);
        return card.render();
...
//# sourceMappingURL=CSSBackgroundsBordersShadowsDeepPage.js.map