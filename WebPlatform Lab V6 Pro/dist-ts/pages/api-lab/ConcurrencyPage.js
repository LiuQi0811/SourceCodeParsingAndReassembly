...
export class ConcurrencyPage extends Page {
    ...
    initialState() {
        ...
        return {
            ...
            schedulerSupported: (typeof globalThis !== 'undefined' && 'scheduler' in globalThis) ||
    ...
    componentDidMount() {
        ...
        this._channel = null;
        this._channelWorker = null;
        this._channelBlobUrl = null;
        ...
        if (!s.sabSupported)
            this._addLog('warn', 'SharedArrayBuffer 不可用：需 crossOriginIsolated=true（COOP/COEP）');
        if (!s.schedulerSupported)
            this._addLog('warn', 'Scheduler API 不可用：将回退到 setTimeout/queueMicrotask');
        if (!s.pressureSupported)
            this._addLog('warn', 'PressureObserver 不可用：需较新 Chromium 内核');
    }
    componentWillUnmount() {
        // 终止 SharedWorker
        try {
            this._sharedPorts.forEach((p) => { try {
                p.close();
            }
            catch { /* noop */ } });
            this._sharedPorts = [];
            if (this._sharedBlobUrl) {
                URL.revokeObjectURL(this._sharedBlobUrl);
                this._sharedBlobUrl = null;
            }
        }
        catch { /* noop */ }
        ...
        try {
            ...
            if (this._channelWorker) {
                this._channelWorker.terminate();
                this._channelWorker = null;
            }
            if (this._channelBlobUrl) {
                URL.revokeObjectURL(this._channelBlobUrl);
                this._channelBlobUrl = null;
            }
        }
        catch { /* noop */ }
        ...
        try {
            if (this._sabWorker) {
                this._sabWorker.terminate();
                this._sabWorker = null;
            }
            if (this._sabBlobUrl) {
                URL.revokeObjectURL(this._sabBlobUrl);
                this._sabBlobUrl = null;
            }
        }
        catch { /* noop */ }
        ...
        try {
            this._taskControllers.forEach((c) => { try {
                c.abort();
            }
            catch { /* noop */ } });
            this._taskControllers = [];
    ...
    _renderLogPanel() {
        const s = this.state;
        const logs = s.logs || [];
        return h('div', { class: 'log-panel' }, h('div', { class: 'log-panel__header' }, '事件日志（共享给所有卡片）'), logs.length === 0
            ? h('div', { class: 'log-panel__empty' }, '（暂无日志）')
            : logs.map((log) => h('div', { class: 'log-panel__line' }, h('span', { class: 'log-panel__time' }, log.time), h('span', { class: `log-panel__tag log-panel__tag--${log.type}` }, log.type), h('span', { class: 'log-panel__content' }, log.content))));
    }
    // =================== Card 1：SharedWorker ===================
    _initSharedWorker() {
    ...
    _renderSharedWorkerCard() {
        ...
        return h(Card, {
            ...
            extra: h(Tag, { color: s.sharedSupported ? 'success' : 'error' }, s.sharedSupported ? '稳定' : '不可用'),
        }, h('p', { class: 'fs-sm text-secondary' }, '多个标签页可共享同一个 worker（同源 + 同名）。' +
            '通过 port.postMessage / port.onmessage 双向通信；' +
            'worker 内 onconnect 拿到 port 并广播给所有连接。'), h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('连接 SharedWorker', {
        ...
        }), h(Tag, { color: 'primary' }, `当前连接数：${s.sharedClients}`)), h('pre', { class: 'code-block mt-md' }, `const sw = new SharedWorker(url, 'name');
sw.port.start();
...
self.onconnect = (e) => { e.ports[0].start(); /* ... */ };`));
    ...
    _renderMessageChannelCard() {
        ...
        }, h('p', { class: 'fs-sm text-secondary' }, 'MessageChannel 创建一对 port1/port2，可将其中一端 transfer 给另一个上下文（如 Worker）。' +
            '两端通过 postMessage / onmessage 双向通信，调用 start() 开始派发，close() 关闭。'), h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('建立 MessageChannel', {
            type: 'primary', size: 'sm',
        ...
        })), h('pre', { class: 'code-block mt-md' }, `const ch = new MessageChannel();
...
// worker 内：e.ports[0] 即 port2，可双向通信`));
    ...
    _testAtomicsSync() {
        ...
        try {
            ...
            this._addLog('push', `Atomics 操作：store(0,10)→load=${before}；add(0,5)→返回旧值${added}，load=${after}；` +
    ...
    _renderSABCard() {
        ...
        return h(Card, {
            ...
            extra: h(Tag, { color: s.sabSupported ? 'success' : 'error' }, s.sabSupported ? '可用' : '不可用'),
        }, h('p', { class: 'fs-sm text-secondary' }, 'SharedArrayBuffer 在多线程间共享同一段内存；Atomics 提供原子操作与 wait/notify 同步原语。' +
            '主线程不能用 Atomics.wait（会阻塞 UI），但可用 waitAsync；worker 内可用 wait。'), h(Alert, {
        ...
        }), h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('store/load/add/sub/and/or/xor', {
            type: 'primary', size: 'sm',
            ...
            disabled: !s.sabSupported,
        })), h('pre', { class: 'code-block mt-md' }, `const sab = new SharedArrayBuffer(4);
const view = new Int32Array(sab);
...
// 主线程非阻塞：Atomics.waitAsync(view, 0, 0, 3000).value.then(...);`));
    ...
    _runSchedulerAbort() {
        if (this.state.schedulerSupported && typeof globalThis.scheduler?.postTask === 'function' && typeof TaskController !== 'undefined') {
            ...
            globalThis.scheduler.postTask(() => this._addLog('info', `任务#${id} 执行了（不应出现，因为会被取消）`), { signal: controller.signal }).catch((err) => {
            ...
            setTimeout(() => {
                try {
                    ...
                    this._addLog('err', `abort 失败: ${e.message}`);
                }
            }, 0);
    ...
    _renderSchedulerCard() {
        ...
        return h(Card, {
            ...
            extra: h(Tag, { color: s.schedulerSupported ? 'primary' : 'warning' }, s.schedulerSupported ? '实验性' : '回退'),
        }, h('p', { class: 'fs-sm text-secondary' }, 'scheduler.postTask(cb, {priority, signal, delay}) 按优先级调度任务；' +
            'TaskController（AbortController 子类）可中途取消；scheduler.yield() 让出主线程。'), h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('提交 3 个不同优先级', {
            type: 'primary', size: 'sm',
            ...
            ? h('div', { class: 'mt-md fs-sm' }, '执行顺序：', s.taskOrder.map((t, i) => h(Tag, { key: i, color: 'primary' }, `${i + 1}.${t}`)))
            : null, h('pre', { class: 'code-block mt-md' }, `// 优先级：'user-blocking' > 'user-visible' > 'background'
...
await scheduler.yield(); // 让出主线程（长任务分片）`));
        ...
        try {
            const observer = new PressureObserver((records) => {
                for (const r of records) {
                    ...
                    const src = r.source;
                    const t = r.time;
                    this._addLog('pull', `Pressure → source=${src}, state=${state}, time=${t.toFixed(0)}`);
                    this.setState((s) => ({
                        ...s,
                        pressureState: state,
    ...
    _testWeakRef() {
        ...
        const check = (round) => {
            if (round > 5) {
                ...
                this._addLog(alive ? 'info' : 'pull', alive ? `第${round}轮 deref() 仍存活（未 GC）`
            ...
            try {
                const junk = [];
                for (let i = 0; i < 5; i++)
                    junk.push(new Array(50000).fill(0));
                junk.length = 0;
            }
            catch { /* noop */ }
    ...
    _renderPressureCard() {
        ...
        return h(Card, {
            ...
            extra: h(Tag, { color: s.pressureSupported ? 'primary' : 'error' }, s.pressureSupported ? '实验性' : '不可用'),
        }, h('p', { class: 'fs-sm text-secondary' }, 'PressureObserver 监听 CPU 压力状态（nominal/fair/serious/critical）；' +
            'FinalizationRegistry 在对象被 GC 时回调；WeakRef 持有弱引用，deref() 可能返回 undefined。'), h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('启动 PressureObserver', {
            type: 'primary', size: 'sm',
            ...
            disabled: !s.pressureSupported,
        }), h(Tag, { color: 'primary' }, `state: ${s.pressureState}`), h(Tag, { color: 'default' }, `事件数: ${s.pressureCount}`)), h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('FinalizationRegistry 演示', {
            size: 'sm',
            ...
            ? h(Tag, { color: s.weakRefAlive ? 'success' : 'warning' }, `WeakRef deref: ${s.weakRefAlive ? '存活' : 'undefined'}`)
            : null), h('pre', { class: 'code-block mt-md' }, `// PressureObserver
...
const ref = new WeakRef(bigObj); ref.deref();`));
    ...
    renderPage() {
        ...
        return [
            ...
            h('div', { class: 'grid grid-2 gap-md' }, this._renderSharedWorkerCard(), this._renderMessageChannelCard()),
            h('div', { class: 'grid grid-2 gap-md mt-md' }, this._renderSABCard(), this._renderSchedulerCard()),
...
//# sourceMappingURL=ConcurrencyPage.js.map