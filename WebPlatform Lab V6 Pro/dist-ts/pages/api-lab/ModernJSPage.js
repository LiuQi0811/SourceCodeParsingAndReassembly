// ModernJSPage.ts —— 现代 ES2020-2024 特性实验室
...
export class ModernJSPage extends Page {
    ...
    _makePromises() {
        const make = (label, shouldResolve) => new Promise((resolve, reject) => {
            const delay = 100 + Math.floor(Math.random() * 900);
            setTimeout(() => {
                if (shouldResolve)
                    resolve(`${label}✓(${delay}ms)`);
                else
                    reject(new Error(`${label}✗(${delay}ms)`));
            }, delay);
        });
    ...
    async _runPromiseAnyAllReject() {
        ...
        const allReject = ['E1', 'E2', 'E3', 'E4'].map((label) => new Promise((_, reject) => setTimeout(() => reject(new Error(label + ' 失败')), 100 + Math.random() * 400)));
        try {
    ...
    _runLogic() {
        ...
        const demoEmptyStr = '';
        const demoNull = null;
        L.push(`0 ?? '默认'   = ${demoZero ?? '默认'}   // ?? 保留 0`);
        L.push(`0 || '默认'   = ${0 || '默认'}   // || 视 0 为 falsy`);
        L.push(`'' ?? '默认'  = ${demoEmptyStr ?? '默认'}  // ?? 保留空串`);
        L.push(`'' || '默认'  = ${demoEmptyStr || '默认'}`);
        L.push(`null ?? '默认' = ${demoNull ?? '默认'}`);
        // ?. 可选链：安全访问深层属性/方法
        ...
        let x = null;
        x ??= '赋值';
        L.push(`let x=null; x??='赋值' → ${x}`);
        let y = '';
        y ||= '赋值';
        L.push(`let y='';   y||='赋值' → ${y}`);
        let z = 1;
        z &&= 2;
        L.push(`let z=1;    z&&=2     → ${z}`);
    ...
    async _runDynamicImport() {
        try {
            ...
            let metaInfo = '不可用';
            try {
                metaInfo = (typeof import.meta !== 'undefined') ? '可用' : '不可用';
            }
            catch { /* noop */ }
            this.setState({
    ...
    renderPage() {
        ...
        return [
            ...
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'WeakRef 包装对象而不阻止其被 GC；FinalizationRegistry 注册回调，在对象被回收时触发。WeakRef 用于缓存，FinalizationRegistry 用于资源清理。'), h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('创建 WeakRef', { type: 'primary', size: 'sm', onClick: () => this._createWeakRef() }), this._btn('检查 deref()', { size: 'sm', onClick: () => this._checkWeakRef() }), this._btn('解除强引用', { danger: true, size: 'sm', onClick: () => this._tryGc() })), h('pre', { class: 'code-block' }, this.state.weakRefResult), h('p', { class: 'fs-sm text-tertiary' }, '提示：GC 不可手动触发，点击「解除强引用」后，切换标签页或等待一段时间，再点「检查 deref()」观察是否变为 undefined。'))),
            // ============ 2. Iterator / Generator / async iteration ============
            h(Card, {
                title: '2. Iterator / Generator / async iteration（迭代器与生成器）',
                extra: h(Tag, { color: 'primary' }, 'Symbol.iterator'),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, '自定义 Iterator 实现 Symbol.iterator 协议；Generator 用 function* + yield；yield* 委托其他可迭代对象；async iterator 用 Symbol.asyncIterator + for await...of。'), h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 Iterator', { type: 'primary', size: 'sm', onClick: () => this._runIterators() }), this._btn('运行 Generator', { type: 'primary', size: 'sm', onClick: () => this._runGenerators() }), this._btn('运行 async iter', { type: 'primary', size: 'sm', onClick: () => this._runAsyncIter() })), h('div', { class: 'fs-sm text-secondary' }, '自定义 Iterator（for...of 遍历结果）：'), h('pre', { class: 'code-block' }, this.state.iterResults.length ? `[${this.state.iterResults.join(', ')}]` : '（点击「运行 Iterator」）'), h('div', { class: 'fs-sm text-secondary mt-sm' }, 'Generator（fib / range / yield* 委托）：'), h('pre', { class: 'code-block' }, genText), h('div', { class: 'fs-sm text-secondary mt-sm' }, 'async iterator（for await...of）：'), h('pre', { class: 'code-block' }, this.state.asyncIterResults.length ? `[${this.state.asyncIterResults.join(', ')}]` : '（点击「运行 async iter」）'))),
            // ============ 3. Promise 组合器 ============
            h(Card, {
                title: '3. Promise 组合器（all / race / allSettled / any + AggregateError）',
                extra: h(Tag, { color: 'warning' }, '4 个随机延迟 Promise'),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, '生成 4 个 100-1000ms 随机延迟的 Promise（P1/P3 resolve，P2/P4 reject）。all 全成功才成功；race 第一个完成；allSettled 全部完成；any 第一个成功，全失败抛 AggregateError。'), h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('Promise.all', { type: 'primary', size: 'sm', onClick: () => this._runPromiseAll() }), this._btn('Promise.race', { type: 'primary', size: 'sm', onClick: () => this._runPromiseRace() }), this._btn('Promise.allSettled', { type: 'primary', size: 'sm', onClick: () => this._runPromiseAllSettled() }), this._btn('Promise.any', { type: 'primary', size: 'sm', onClick: () => this._runPromiseAny() }), this._btn('any(全失败→AggregateError)', { danger: true, size: 'sm', onClick: () => this._runPromiseAnyAllReject() })), h('div', { class: 'fs-sm text-secondary' }, 'Promise.all 结果：'), h('pre', { class: 'code-block' }, this.state.promiseAllResult || '（点击「Promise.all」）'), h('div', { class: 'fs-sm text-secondary mt-sm' }, 'Promise.race 结果：'), h('pre', { class: 'code-block' }, this.state.promiseRaceResult || '（点击「Promise.race」）'), h('div', { class: 'fs-sm text-secondary mt-sm' }, 'Promise.allSettled 结果：'), h('pre', { class: 'code-block' }, this.state.promiseAllSettledResult || '（点击「Promise.allSettled」）'), h('div', { class: 'fs-sm text-secondary mt-sm' }, 'Promise.any 结果（含 AggregateError 演示）：'), h('pre', { class: 'code-block' }, this.state.promiseAnyResult || '（点击「Promise.any」）'))),
            // ============ 4. 新数组方法 ============
            h(Card, {
                title: '4. 新数组方法（at / flat / flatMap / findLast / toSorted / with / groupBy）',
                extra: h(Tag, { color: 'primary' }, 'ES2022-2024'),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'at() 负索引取值；flat/flatMap 扁平化；findLast/findLastIndex 从末尾查找；toSorted/toReversed/toSpliced/with 为不可变版本；Object.groupBy/Map.groupBy 分组（ES2024）。'), this._btn('运行数组方法演示', { type: 'primary', size: 'sm', onClick: () => this._runArrayMethods() }), h('pre', { class: 'code-block' }, this.state.arrayMethodDemo || '（点击「运行数组方法演示」）'))),
            // ============ 5. Object / String / Number 新方法 ============
            h(Card, {
                title: '5. Object / String / Number 新方法',
                extra: h(Tag, { color: 'success' }, 'hasOwn / replaceAll / matchAll'),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'Object.hasOwn/fromEntries；String.replaceAll/matchAll/at/padStart/padEnd；Number.isInteger/isFinite/isNaN/isSafeInteger/MAX_SAFE_INTEGER；数值分隔符 1_000_000。'), this._btn('运行 Object/String/Number 演示', { type: 'primary', size: 'sm', onClick: () => this._runObjectStringNumber() }), h('pre', { class: 'code-block' }, this.state.objectStringDemo || '（点击按钮演示）'))),
            // ============ 6. Map / Set / WeakMap / WeakSet ============
            h(Card, {
                title: '6. Map / Set / WeakMap / WeakSet',
                extra: h(Tag, { color: 'warning' }, '集合类型'),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'Map 键值对（键可任意类型，保持顺序）；Set 唯一值集合；WeakMap/WeakSet 键/值为对象且弱引用，不阻止 GC，不可遍历。'), this._btn('运行 Map/Set 演示', { type: 'primary', size: 'sm', onClick: () => this._runMapSet() }), h('pre', { class: 'code-block' }, this.state.mapSetDemo || '（点击按钮演示）'))),
            // ============ 7. 逻辑赋值与空值合并 ============
            h(Card, {
                title: '7. 逻辑赋值与空值合并（?? / ?. / ??= / ||= / &&=）',
                extra: h(Tag, { color: 'primary' }, 'ES2020-2021'),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, '?? 仅对 null/undefined 触发（vs || 对所有 falsy 触发）；?. 可选链安全访问深层属性/方法；??= / ||= / &&= 逻辑赋值运算符。'), this._btn('运行逻辑赋值演示', { type: 'primary', size: 'sm', onClick: () => this._runLogic() }), h('pre', { class: 'code-block' }, this.state.logicDemo || '（点击按钮演示）'))),
            // ============ 8. 顶层 await + 动态 import ============
            h(Card, {
                title: '8. 顶层 await + 动态 import()',
                extra: h(Tag, { color: 'success' }, 'ES2020 / import.meta'),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, '顶层 await 在 ES Module 顶层直接用 await；动态 import() 运行时按需加载模块；import.meta 暴露模块元信息。本页用 async 函数包裹演示动态 import 一个已有模块。'), this._btn('运行动态 import', { type: 'primary', size: 'sm', onClick: () => this._runDynamicImport() }), h('pre', { class: 'code-block' }, this.state.dynamicImportResult || '（点击按钮演示）'))),
            // ============ 日志面板 ============
            h(Card, {
                title: '事件日志',
...
//# sourceMappingURL=ModernJSPage.js.map