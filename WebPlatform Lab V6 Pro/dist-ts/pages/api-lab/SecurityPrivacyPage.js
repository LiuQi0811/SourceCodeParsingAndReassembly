...
const CSP_META_EXAMPLE = `<meta http-equiv="Content-Security-Policy"
...
const SRI_EXAMPLE = `<!-- script/link 标签的 SRI 完整示例 -->
...
const CSP_REPORT_EXAMPLE = `// report-uri / report-to 端点接收 POST application/csp-report
...
const COOP_COEP_EXAMPLE = `# Cross-Origin Isolation 所需的响应头
...
const CORS_FETCH_EXAMPLE = `// 跨域 fetch 的标准配置
...
const PERMISSIONS_POLICY_EXAMPLE = `# Permissions-Policy HTTP 头（替代已弃用的 Feature-Policy）
...
const PP_HTTP_HEADER_SYNTAX = `# Permissions-Policy HTTP 头语法（取代已弃用的 Feature-Policy）
Permissions-Policy: camera=(self "https://trusted.example.com"),
                    geolocation=(),
                    microphone=*,
                    payment=(self "https://checkout.example.com"),
...
const PP_IFRAME_ALLOW_ATTR = `# iframe allow 属性：粒度比 HTTP 头更细，可针对单个 iframe 授权
<iframe src="https://embed.example.com"
...
const PP_FEATURES_LIST = `# Permissions Policy 支持的特性全集（50+ 项，节选）
# 媒体 / 输入 / 显示：
...
const PP_FEATUREPOLICY_JS_API = `# document.featurePolicy / document.policy JS API
...
# - jsdom 中上述 API 均不可用`;
const PP_THIRD_PARTY_SANDBOX = `# 实战 1：第三方 iframe 沙箱（嵌入不可信内容）
...
const PP_OWN_PAGE_STRATEGY = `# 实战 2：自家页面策略（禁用强力特性 + 第三方分析白名单）
...
#       COOP/COEP —— 控制跨域隔离
#       PP       —— 控制特性授权`;
const PP_PITFALLS = `# Permissions Policy 常见陷阱
1) 特性名映射（旧属性 → 新特性）：
...
const SET_COOKIE_EXAMPLE = `# 完整 Set-Cookie 头示例（含所有现代属性）
...
const FETCH_METADATA_EXAMPLE = `# Fetch Metadata 请求头（浏览器自动附加，JS 无法伪造）
...
const SAMESITE_COMPARISON = `# SameSite 属性三档对比：
...
const CSP_DIRECTIVES_TABLE = `# 常见 CSP 指令速查：
...
export class SecurityPrivacyPage extends Page {
        ...
        if (typeof SecurityPolicyViolationEvent !== 'undefined' ||
            ...
            this._onSpv = (e) => {
                try {
                    ...
                    this._addLog('spv', `拦截违规：blocked=${info.blockedURI} directive=${info.violatedDirective} disposition=${info.disposition} @${info.lineNumber}:${info.columnNumber}`);
                }
                catch (err) {
    ...
    componentWillUnmount() {
        ...
        if (this._onSpv) {
            try {
                document.removeEventListener('securitypolicyviolation', this._onSpv);
            }
            catch { /* noop */ }
        }
        this._onSpv = null;
    ...
    _detectCrossOriginIsolated() {
        try {
            ...
            this._addLog('cors', `self.crossOriginIsolated = ${iso}（${iso ? '已隔离，可用 SAB / measureUserAgentSpecificMemory' : '未隔离，需配置 COOP+COEP 头'}）`);
        }
        catch (err) {
        ...
        try {
            ...
            if (info.hasFeaturePolicy) {
                ...
                try {
                    ...
                    info.notes.push(`allowedFeatures err:${e.name}`);
                }
                ...
                try {
                    ...
                    info.notes.push(`features err:${e.name}`);
                }
                ...
                for (const f of probes) {
                    ...
                    for (const origin of [undefined, 'https://trusted.example.com', 'https://evil.example.com']) {
                        try {
                            ...
                            perOrigin[origin || 'self'] = `err:${e.name}`;
                        }
                    }
            ...
            this._addLog('perm', `PP 演示：featurePolicy=${info.hasFeaturePolicy ? '✓' : '✗'} ` +
    ...
    _sanitizerDemo() {
        ...
        try {
            ...
            const input = '<p>正常段落</p><script>alert(1)</script><img src=x onerror="alert(2)"><b>加粗</b>';
    ...
    renderPage() {
        ...
        return [
            ...
            h(Card, {
                ...
                extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: hasSpv ? 'success' : 'default' }, hasSpv ? 'SPV 可用' : 'SPV 不可用'), h(Tag, { color: 'warning' }, 'HTTP 头/meta')),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'CSP 通过 <meta http-equiv="Content-Security-Policy"> 或响应头 Content-Security-Policy 下发；指令包括 default-src / script-src / style-src / img-src / connect-src / font-src / media-src / frame-src / object-src / manifest-src / worker-src；来源关键词：\'self\' / \'none\' / \'unsafe-inline\' / \'unsafe-eval\' / nonce-xxx / sha256-xxx；report-uri / report-to 配置报告端点。SRI 通过 <script integrity="sha256-..." crossorigin="anonymous"> 让浏览器校验资源 hash，不匹配则拒绝加载。'), h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('检测 CSP meta', { type: 'primary', size: 'sm', onClick: () => this._detectCspMeta() }), this._btn('说明 violation 触发', { size: 'sm', onClick: () => this._probeViolationProbe() }), this._btn('清空 violation 缓存', { size: 'sm', onClick: () => this._clearViolation() })), h('div', { class: 'flex items-center gap-sm flex-wrap' }, h(Tag, { color: s.cspMetaFound ? 'success' : 'default' }, s.cspMetaFound ? '已检测到 CSP meta' : '未检测到 CSP meta'), h(Tag, { color: 'info' }, ' disposition: enforce | report')), s.cspMetaFound && h('pre', { class: 'code-block', style: { maxHeight: '80px' } }, h('code', {}, s.cspMetaFound)), h('div', { class: 'fs-sm text-secondary' }, 'CSP meta 标签示例（default-src + 各类 src 指令 + report）：'), h('pre', { class: 'code-block', style: { maxHeight: '180px', overflow: 'auto' } }, h('code', {}, CSP_META_EXAMPLE)), h('div', { class: 'fs-sm text-secondary' }, 'SRI script/link 标签示例：'), h('pre', { class: 'code-block', style: { maxHeight: '120px', overflow: 'auto' } }, h('code', {}, SRI_EXAMPLE)), h('div', { class: 'fs-sm text-secondary' }, 'securitypolicyviolation 事件捕获字段与 report-uri 报告格式：'), h('pre', { class: 'code-block', style: { maxHeight: '180px', overflow: 'auto' } }, h('code', {}, CSP_REPORT_EXAMPLE)), h('div', { class: 'fs-sm text-secondary' }, '常见 CSP 指令与来源关键词速查：'), h('pre', { class: 'code-block', style: { maxHeight: '220px', overflow: 'auto' } }, h('code', {}, CSP_DIRECTIVES_TABLE)), s.violationEvent && h('div', { class: 'alert alert--warning', style: { fontSize: '12px' } }, h('div', {}, h('div', { class: 'fw-medium' }, '最近一次 SecurityPolicyViolationEvent：'), h('div', { class: 'fs-sm' }, `blockedURI=${s.violationEvent.blockedURI}`), h('div', { class: 'fs-sm' }, `violatedDirective=${s.violationEvent.violatedDirective} effectiveDirective=${s.violationEvent.effectiveDirective}`), h('div', { class: 'fs-sm' }, `disposition=${s.violationEvent.disposition} @${s.violationEvent.lineNumber}:${s.violationEvent.columnNumber}`), h('div', { class: 'fs-sm', style: { wordBreak: 'break-all' } }, `sourceFile=${s.violationEvent.sourceFile}`), s.violationEvent.originalPolicy && h('div', { class: 'fs-sm', style: { wordBreak: 'break-all' } }, `originalPolicy=${String(s.violationEvent.originalPolicy).slice(0, 120)}...`))), h(Alert, {
                type: 'info',
            ...
            h(Card, {
                ...
                extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: hasCOI ? 'success' : 'default' }, s.crossOriginIsolated ? '已隔离' : '未隔离'), h(Tag, { color: 'warning' }, 'HTTP 头')),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'fetch 的 mode 取值 cors | no-cors | same-origin；credentials 取值 omit | same-origin | include；简单请求直接发送，非简单请求（自定义头/PUT/PATCH/非简单 Content-Type）先发 OPTIONS 预检。Cross-Origin Isolation 需配置 COOP: same-origin + COEP: require-corp，资源端声明 CORP: same-origin；启用后 self.crossOriginIsolated === true，可解锁 SharedArrayBuffer 与 performance.measureUserAgentSpecificMemory。'), h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('检测 crossOriginIsolated', { type: 'primary', size: 'sm', onClick: () => this._detectCrossOriginIsolated() }), this._btn('查看 CORS fetch 用法', { size: 'sm', onClick: () => this._showCorsUsage() })), h('div', { class: 'flex items-center gap-sm flex-wrap' }, h(Tag, { color: 'primary' }, `self.crossOriginIsolated = ${s.crossOriginIsolated}`), h(Tag, { color: s.crossOriginIsolated ? 'success' : 'warning' }, s.crossOriginIsolated ? 'SAB 可用' : '需 COOP+COEP')), h('div', { class: 'fs-sm text-secondary' }, 'COOP / COEP / CORP HTTP 头示例：'), h('pre', { class: 'code-block', style: { maxHeight: '180px', overflow: 'auto' } }, h('code', {}, COOP_COEP_EXAMPLE)), h('div', { class: 'fs-sm text-secondary' }, 'fetch 跨域配置（mode / credentials / 预检）与服务端响应头：'), h('pre', { class: 'code-block', style: { maxHeight: '200px', overflow: 'auto' } }, h('code', {}, CORS_FETCH_EXAMPLE)), h(Alert, {
                type: 'info',
            ...
            h(Card, {
                ...
                extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: hasPermissions ? 'success' : 'default' }, hasPermissions ? 'permissions 可用' : 'permissions 不可用'), h(Tag, { color: hasIframePP ? 'success' : 'default' }, hasIframePP ? 'PP 可用' : 'PP 不可用')),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'Storage Access API：document.hasStorageAccess() 返回 Promise<boolean>，document.requestStorageAccess() 请求第一方 Cookie 访问权限（弹出提示），适用于第三方 iframe。Permissions Policy（原 Feature Policy，已弃用）通过 Permissions-Policy HTTP 头或 iframe allow 属性控制能力授权；navigator.permissions.query({ name }) 返回 { state: granted|denied|prompt }；iframe.permissionsPolicy.allowedFeatures() 列出 iframe 当前可用的特性。'), h('div', { class: 'fs-sm text-secondary' }, 'Storage Access API：'), h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('hasStorageAccess()', { type: 'primary', size: 'sm', onClick: () => this._checkStorageAccess() }), this._btn('requestStorageAccess()', { size: 'sm', onClick: () => this._requestStorageAccess() }), h(Tag, { color: s.storageAccess === 'granted' ? 'success' : s.storageAccess === 'denied' ? 'warning' : 'default' }, `storageAccess: ${s.storageAccess}`)), h(Alert, {
                type: 'warning',
                message: 'Storage Access API 需第三方 iframe 上下文 + HTTPS',
                description: '在第一方顶层页面调用 hasStorageAccess 通常返回 true；jsdom 中不可用，调用将记日志说明。',
            }), h('div', { class: 'fs-sm text-secondary' }, 'Permissions Policy：'), h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('检测 iframe.permissionsPolicy', { type: 'primary', size: 'sm', onClick: () => this._checkIframePermissionsPolicy() }), this._btn('allowsFeature 探测', { size: 'sm', onClick: () => this._checkAllowsFeature() }), this._btn('查询 4 个权限', { type: 'primary', size: 'sm', onClick: () => this._queryPermissions() })), permEntries.length > 0
                ? h('div', { class: 'flex items-center gap-xs flex-wrap' }, ...permEntries.map(([name, state]) => h(Tag, { color: state === 'granted' ? 'success' : state === 'denied' ? 'error' : state === 'unsupported' ? 'default' : 'warning' }, `${name}: ${state}`)))
                : h('div', { class: 'fs-sm text-tertiary' }, '（暂未查询权限，点击「查询 4 个权限」）'), s.iframeAllowedFeatures.length > 0 && h('div', { class: 'fs-sm text-tertiary', style: { wordBreak: 'break-all' } }, `iframe.permissionsPolicy.allowedFeatures() => [${s.iframeAllowedFeatures.slice(0, 12).join(', ')}${s.iframeAllowedFeatures.length > 12 ? '...' : ''}]`), h('div', { class: 'fs-sm text-secondary' }, 'Permissions-Policy HTTP 头与 iframe allow 属性示例：'), h('pre', { class: 'code-block', style: { maxHeight: '180px', overflow: 'auto' } }, h('code', {}, PERMISSIONS_POLICY_EXAMPLE)), h(Alert, {
                type: 'info',
            ...
            h(Card, {
                ...
                extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: hasTT ? 'success' : 'default' }, hasTT ? 'TT 可用' : 'TT 不可用'), h(Tag, { color: hasSanitizer ? 'success' : 'default' }, hasSanitizer ? 'Sanitizer 可用' : 'Sanitizer 不可用'), h(Tag, { color: 'warning' }, '防御性')),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'Trusted Types 防止 DOM XSS：要求 innerHTML / insertAdjacentHTML / eval 等接收 TrustedHTML 而非字符串。trustedTypes.createPolicy(name, { createHTML }) 创建策略；policy.createHTML(str) 返回 TrustedHTML。配合 CSP: require-trusted-types-for \'script\' 强制启用。Sanitizer API：new Sanitizer({ allowElements, allowAttributes, dropElements, dropAttributes }) 创建净化器；sanitizer.sanitize(input) 返回 DocumentFragment；element.setHTML(html, { sanitizer }) 安全设置 innerHTML（自动剥离危险节点）。'), h('div', { class: 'fs-sm text-secondary' }, 'Trusted Types 演示：'), h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('检测 + 创建 policy', { type: 'primary', size: 'sm', onClick: () => this._checkTrustedTypes() }), h(Tag, { color: s.ttSupport ? 'success' : 'default' }, s.ttSupport ? 'trustedTypes 已检测' : 'trustedTypes 不可用'), h(Tag, { color: s.ttPolicyCreated ? 'success' : 'default' }, s.ttPolicyCreated ? 'policy 已创建' : 'policy 未创建')), h('div', { class: 'tt-demo-target', style: { padding: '8px', border: '1px dashed #555', minHeight: '24px', fontSize: '13px' } }, '（TrustedHTML 输出将显示在此处）'), h('div', { class: 'fs-sm text-secondary' }, 'Sanitizer API 演示（输入含 <script> 与 onerror，将被剥离）：'), h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 Sanitizer 演示', { type: 'primary', size: 'sm', onClick: () => this._sanitizerDemo() }), h(Tag, { color: hasSanitizer ? 'success' : 'default' }, hasSanitizer ? 'Sanitizer 可用' : 'Sanitizer 不可用'), h(Tag, { color: hasSetHTML ? 'success' : 'default' }, hasSetHTML ? 'setHTML 可用' : 'setHTML 不可用')), h('div', { class: 'san-demo-target', style: { padding: '8px', border: '1px dashed #555', minHeight: '24px', fontSize: '13px' } }, '（setHTML 安全输出将显示在此处）'), s.sanitizerOutput && h('pre', { class: 'code-block', style: { maxHeight: '80px', overflow: 'auto' } }, h('code', {}, `sanitize 输出：${s.sanitizerOutput}`)), h(Alert, {
                type: 'info',
            ...
            h(Card, {
                ...
                extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'success' }, 'Cookie'), h(Tag, { color: 'warning' }, '实验性')),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'Cookie 属性：HttpOnly（JS 不可读）、Secure（仅 HTTPS）、SameSite=Strict|Lax|None（需 Secure）、Partitioned（CHIP 第三方隔离）。document.cookie 读写非 HttpOnly Cookie；cookieStore API 提供 get/set/delete/getAll。Privacy Sandbox：Topics API（browsingTopics 推断兴趣）、Protected Audience API（原 FLEDGE，runAdAuction/joinAdInterestGroup）、Attribution Reporting、Storage Partitioning。Fetch Metadata 头（Sec-Fetch-Site/Mode/Dest/User）由浏览器自动附加，JS 无法伪造，服务端可据此防 CSRF。'), h('div', { class: 'fs-sm text-secondary' }, 'Cookie：'), h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取 document.cookie', { type: 'primary', size: 'sm', onClick: () => this._showCurrentCookie() }), h(Tag, { color: s.currentCookie ? 'success' : 'default' }, s.currentCookie ? '有非 HttpOnly Cookie' : '(空或仅 HttpOnly)')), s.currentCookie && h('pre', { class: 'code-block', style: { maxHeight: '60px', overflow: 'auto' } }, h('code', {}, s.currentCookie)), h('div', { class: 'fs-sm text-secondary' }, '完整 Set-Cookie 头示例（含所有现代属性）：'), h('pre', { class: 'code-block', style: { maxHeight: '180px', overflow: 'auto' } }, h('code', {}, SET_COOKIE_EXAMPLE)), h('div', { class: 'fs-sm text-secondary' }, 'SameSite 三档取值对比（防 CSRF 关键属性）：'), h('pre', { class: 'code-block', style: { maxHeight: '120px', overflow: 'auto' } }, h('code', {}, SAMESITE_COMPARISON)), h('div', { class: 'fs-sm text-secondary' }, 'Privacy Sandbox 能力检测：'), h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('检测 Privacy Sandbox', { type: 'primary', size: 'sm', onClick: () => this._checkPrivacySandbox() }), this._btn('尝试 browsingTopics()', { size: 'sm', onClick: () => this._tryBrowsingTopics() })), h('div', { class: 'flex items-center gap-xs flex-wrap' }, h(Tag, { color: s.topicsSupport ? 'success' : 'default' }, `browsingTopics: ${s.topicsSupport ? '✓' : '✗'}`), h(Tag, { color: s.runAdAuctionSupport ? 'success' : 'default' }, `runAdAuction: ${s.runAdAuctionSupport ? '✓' : '✗'}`), h(Tag, { color: s.joinAdInterestGroupSupport ? 'success' : 'default' }, `joinAdInterestGroup: ${s.joinAdInterestGroupSupport ? '✓' : '✗'}`)), h('div', { class: 'fs-sm text-secondary' }, 'Fetch Metadata 请求头（浏览器自动附加，防 CSRF）：'), h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('说明 Fetch Metadata', { size: 'sm', onClick: () => this._showFetchMetadataInfo() })), h('pre', { class: 'code-block', style: { maxHeight: '160px', overflow: 'auto' } }, h('code', {}, FETCH_METADATA_EXAMPLE)), h(Alert, {
                type: 'warning',
    ...
    _renderCard9() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: hasFeaturePolicy ? 'success' : 'default' }, hasFeaturePolicy ? 'featurePolicy 可用' : 'featurePolicy 不可用'), h(Tag, { color: hasPolicy ? 'success' : 'default' }, hasPolicy ? 'policy 可用' : 'policy 不可用'), h(Tag, { color: 'warning' }, 'HTTP 头/iframe allow')),
            children: h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'Permissions Policy（原 Feature Policy，已改名）通过 Permissions-Policy HTTP 头或 iframe allow 属性，按源声明式启用/禁用强力特性（camera/microphone/usb/payment 等）。所有主流浏览器均可用；老语法 Feature-Policy 头已弃用。本卡演示 document.featurePolicy / document.policy JS API 与第三方 iframe 沙箱实战。'), h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 PP 演示', { type: 'primary', size: 'sm', onClick: () => this._runPermissionsPolicyDemo() }), h(Tag, { color: 'info' }, 'document.featurePolicy / document.policy')), info && h('div', { class: 'flex items-center gap-xs flex-wrap' }, h(Tag, { color: info.hasFeaturePolicy ? 'success' : 'default' }, `document.featurePolicy: ${info.hasFeaturePolicy ? '✓' : '✗'}`), h(Tag, { color: info.hasPolicy ? 'success' : 'default' }, `document.policy: ${info.hasPolicy ? '✓' : '✗'}`), h(Tag, { color: info.iframePP ? 'success' : 'default' }, `iframe.permissionsPolicy: ${info.iframePP ? '✓' : '✗'}`)), info && info.allowedFeatures && h('div', { class: 'fs-sm text-tertiary', style: { wordBreak: 'break-all' } }, `document.featurePolicy.allowedFeatures() => [${(info.allowedFeatures || []).slice(0, 12).join(', ')}${(info.allowedFeatures || []).length > 12 ? '...' : ''}]`), info && info.features && h('div', { class: 'fs-sm text-tertiary', style: { wordBreak: 'break-all' } }, `document.featurePolicy.features() => 共 ${(info.features || []).length} 项（浏览器认知全集）`), info && Object.keys(info.allowlist).length > 0 && h('div', { class: 'fs-sm text-tertiary', style: { wordBreak: 'break-all' } }, `getAllowlistForFeature：${Object.entries(info.allowlist).map(([k, v]) => `${k}=${JSON.stringify(v)}`).join(' | ')}`), info && Object.keys(info.allowsFeature).length > 0 && h('div', { class: 'fs-sm text-tertiary', style: { wordBreak: 'break-all' } }, `allowsFeature 探测：${Object.entries(info.allowsFeature).map(([f, per]) => `${f}{self=${per.self},trusted=${per['https://trusted.example.com']},evil=${per['https://evil.example.com']}}`).join(' | ')}`), h('div', { class: 'fs-sm text-secondary' }, 'Permissions-Policy HTTP 头语法（逗号分隔多特性 / self|none|*|源 / 嵌套括号）：'), h('pre', { class: 'code-block', style: { maxHeight: '240px', overflow: 'auto' } }, h('code', {}, PP_HTTP_HEADER_SYNTAX)), h('div', { class: 'fs-sm text-secondary' }, 'iframe allow 属性（与 HTTP 头协同，allowfullscreen → fullscreen 映射，srcdoc 继承基线）：'), h('pre', { class: 'code-block', style: { maxHeight: '220px', overflow: 'auto' } }, h('code', {}, PP_IFRAME_ALLOW_ATTR)), h('div', { class: 'fs-sm text-secondary' }, '特性列表全集（约 50+ 项，节选）：'), h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, PP_FEATURES_LIST)), h('div', { class: 'fs-sm text-secondary' }, 'document.featurePolicy / document.policy JS API（allowedFeatures / allowsFeature / features / getAllowlistForFeature，浏览器支持差异）：'), h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, PP_FEATUREPOLICY_JS_API)), h('div', { class: 'fs-sm text-secondary' }, '实战 1：第三方 iframe 沙箱（嵌入不可信内容 / 禁用 camera/mic/usb / 仅允许必要特性 / 与 sandbox 协同 / CSP frame-src）：'), h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, PP_THIRD_PARTY_SANDBOX)), h('div', { class: 'fs-sm text-secondary' }, '实战 2：自家页面策略（禁用同步 script / 限制 camera 仅 self / 第三方分析白名单 / 与 COOP/COEP 协同）：'), h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, PP_OWN_PAGE_STRATEGY)), h('div', { class: 'fs-sm text-secondary' }, '陷阱（特性名映射 / 与 sandbox 区别 / report-only / 浏览器差异 / DevTools Network 查看头 / iframe allow 与 HTTP 头优先级）：'), h('pre', { class: 'code-block', style: { maxHeight: '280px', overflow: 'auto' } }, h('code', {}, PP_PITFALLS)), h(Alert, {
                type: 'info',
    ...
    _renderLogPanel() {
        ...
        const card = new Card({
            ...
            children: s.logs.length === 0
                ? h('div', { class: 'log-panel__empty fs-sm text-tertiary' }, '（暂无日志）')
                : h('div', { class: 'log-panel' }, h('div', { class: 'log-panel__header' }, '事件日志'), ...s.logs.map((log) => h('div', { class: 'log-panel__line' }, h('span', { class: 'log-panel__time' }, log.time), h('span', { class: `log-panel__tag log-panel__tag--${log.type}` }, log.type), h('span', { class: 'log-panel__content' }, log.content))))
        });
        this.registerChild(card);
        return card.render();
    }
}
//# sourceMappingURL=SecurityPrivacyPage.js.map