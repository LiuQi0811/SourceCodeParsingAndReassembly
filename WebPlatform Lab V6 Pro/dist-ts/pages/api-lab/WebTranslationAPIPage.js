...
export class WebTranslationAPIPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!f.translator)
            this._addLog('warn', 'Translator 不可用（Chrome 130+ origin trial，jsdom/Node/Firefox/Safari 均无）');
        if (!f.languageDetector)
            this._addLog('warn', 'LanguageDetector 不可用（Chrome 130+ origin trial，与 Translator 同期推出）');
        if (!f.translatorAvailability)
            this._addLog('warn', '(Translator as any).availability 不可用，无法探测模型下载状态');
        if (!f.languagePairAvailable)
            this._addLog('warn', 'Translator.languagePairAvailable 不可用，无法查询语言配对');
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            children: [
                ...
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// Translation API：本地翻译
...
detector.destroy();`)),
                ...
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.overviewInfo || '（点击按钮查看 Translation + Language Detector API 概述完整说明）')),
            ],
        });
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'Translator.create({ sourceLanguage, targetLanguage, signal }) 异步创建（首次触发模型下载数十 MB），translate(text) 同步翻译返回 Promise<string>，translate(ReadableStream, signal) 流式翻译（边输入边输出）。复用同一 Translator 性能更好，destroy() 释放模型内存。AbortSignal 中止 create/translate。内置小模型（Gemma/T5 量化版）通过 WebNN/WebGPU 硬件加速，延迟 100ms-1s。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 Translator 演示', { type: 'primary', size: 'sm', onClick: () => this._runTranslatorDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// 同步翻译
const translator = await Translator.create({
  sourceLanguage: 'en',
  targetLanguage: 'zh-Hans',
});
...
translator.destroy();`)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('div', { class: `wt-result ${s.translateResult.startsWith('翻译成功') ? 'wt-result--success' : (s.translateResult ? 'wt-result--warn' : '')}` }, s.translateResult || '（点击按钮尝试真实本地翻译 en → zh-Hans）'),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.translatorInfo || '（点击按钮查看 Translator 入口完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    `);
        if (!f.detectorCreate) {
            ...
            try {
                ...
                LanguageDetector.create().then((detector) => {
                    this._detectors.push(detector);
                    return Promise.all(samples.map((s) => detector.detect(s)));
                }).then((results) => {
                    const lines = results.map((r, i) => `"${samples[i]}" → ${r.map((x) => `${x.detectedLanguage}(${(x.confidence * 100).toFixed(0)}%)`).join(', ')}`);
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'LanguageDetector.create() 异步创建（无参数），detect(text) 返回多候选 [{detectedLanguage, confidence}]，按置信度降序排列。短文本（< 20 字符）识别准确率下降，需累积文本或结合上下文。detect(ReadableStream, signal) 流式检测边输入边更新。与 Translator 协同：先 detect 语言再 languagePairAvailable 检查配对再 create Translator（语言路由）。destroy() 释放模型内存。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 Language Detector 演示', { type: 'primary', size: 'sm', onClick: () => this._runDetectorDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// 多候选语言检测
const detector = await LanguageDetector.create();
const results = await detector.detect('Bonjour le monde');
// [
//   { detectedLanguage: 'fr', confidence: 0.98 },
...
detector.destroy(); // 释放模型`)),
                ...
                h('div', { class: `wt-result ${s.detectResult.startsWith('检测失败') ? 'wt-result--warn' : (s.detectResult ? 'wt-result--success' : '')}` }, s.detectResult || '（点击按钮尝试真实本地语言检测：英/法/中/日样本）'),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.detectorInfo || '（点击按钮查看 Language Detector 入口完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
...
const translator = await Translator.create({
  ...
  monitor: (state: any) => {
    updateProgressBar(state.downloadedBytes / state.totalBytes * 100);
                ...
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.availabilityInfo || '（点击按钮查看可用性检测完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['languagePairAvailable', f.languagePairAvailable]]), h(Tag, { color: 'primary' }, '语言配对')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'Translator.languagePairAvailable(src, tgt) 探测语言对支持（与 availability 语义类似）。支持语言全集约 50+ 种持续扩展。翻译方向不对称（en→zh 支持，zh→en 可能不支持）。BCP-47 语言代码：zh-Hans 简体（推荐）/ zh-Hant 繁体（推荐），而非 zh-CN/zh-TW（地区子标签，不推荐）。中文必须用 script 子标签，否则 API 可能不识别。用户语言偏好从 navigator.languages 匹配，优先具体语言。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行语言配对演示', { type: 'primary', size: 'sm', onClick: () => this._runLanguagesDemo() })),
                // 语言支持速查
                h('div', { class: 'wt-matrix' }, h('div', { class: 'wt-matrix-cell' }, h('div', { class: 'wt-matrix-title' }, '常见语言'), h('div', {}, 'en / zh-Hans / zh-Hant / es / fr / de / ja / ko / ru / ar / hi')), h('div', { class: 'wt-matrix-cell' }, h('div', { class: 'wt-matrix-title' }, '亚洲语言'), h('div', {}, 'ja / ko / zh-Hans / zh-Hant / vi / th / id / ms / hi / ta')), h('div', { class: 'wt-matrix-cell' }, h('div', { class: 'wt-matrix-title' }, '欧洲语言'), h('div', {}, 'en / fr / de / es / pt / it / nl / sv / no / da / fi / pl / uk / cs / el')), h('div', { class: 'wt-matrix-cell' }, h('div', { class: 'wt-matrix-title' }, 'BCP-47 中文'), h('div', {}, 'zh-Hans 简体（推荐）· zh-Hant 繁体（推荐）· zh-CN/zh-TW 不推荐'))),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// 探测语言对支持
...
function pickTargetLang() {
  ...
  return 'en';
                ...
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.languagesInfo || '（点击按钮查看支持语言列表与配对完整说明）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '实时翻译输入框：input 事件 → 300ms debounce → LanguageDetector.detect 检测语言 → languagePairAvailable 检查配对 → Translator.create/复用 → translate 显示结果。AbortController 取消旧请求避免结果错乱。与 Web Speech API 协同：语音识别 interimResults → 实时翻译（麦克风 → Web Speech → Translation API → 显示，< 500ms）。离线场景需 availability==="available"。降级方案：不支持时回退到 Google Translate widget 或后端代理。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行实时翻译演示', { type: 'primary', size: 'sm', onClick: () => this._runRealtimeDemo() })),
                // 实时翻译流程图
                h('div', { class: 'wt-flow' }, h('span', { class: 'wt-flow-node' }, 'input'), h('span', { class: 'wt-flow-arrow' }, '→'), h('span', { class: 'wt-flow-node' }, 'debounce 300ms'), h('span', { class: 'wt-flow-arrow' }, '→'), h('span', { class: 'wt-flow-node wt-flow-node--ai' }, 'LanguageDetector.detect'), h('span', { class: 'wt-flow-arrow' }, '→'), h('span', { class: 'wt-flow-node wt-flow-node--ai' }, 'Translator.translate'), h('span', { class: 'wt-flow-arrow' }, '→'), h('span', { class: 'wt-flow-node' }, '显示结果')),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// 实时翻译输入框
input.addEventListener('input', () => {
  clearTimeout(debounceTimer);
...
recognition.onresult = (e: any) => {
  for (let i = e.resultIndex; i < e.results.length; i++) {
    ...
    if (e.results[i].isFinal) translateFinal(transcript);
                ...
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.realtimeInfo || '（点击按钮查看实时翻译输入框完整方案）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard7() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '长文本分段（按段落/句子，每段 < 5000 字符）。多语言混合：每段 detect 语言，缓存对应 Translator（语言路由）。进度监控：onProgress({ done, total, pct })。AbortSignal 中止（abortButton 触发 ac.abort）。翻译质量评估：回译（翻译后译回原文对比相似度）。与 WebNN 硬件加速协同（navigator.ml 检测，NPU 提速 5-10 倍）。并发翻译：worker pool concurrency=4（受限于硬件并发与 Translator 实例数）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行批量翻译演示', { type: 'primary', size: 'sm', onClick: () => this._runBatchDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// 长文本分段
...
if ('ml' in navigator) console.log('NPU 加速已启用');`)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.batchInfo || '（点击按钮查看批量文档翻译与语言路由完整方案）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard8() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '8 大陷阱：模型体积（30-100MB，先 availability 探测提示下载）/ 隐私敏感文本（医疗/法律/财务，禁用日志）/ 云 vs 本地取舍（隐私+离线+免费 vs 质量+语言+兼容）/ 多语言混合（分段 detect 语言路由）/ 浏览器支持矩阵（Chrome 130+ origin trial）/ 降级 Google Translate widget / 模型缓存清理 / origin trial 过期。10 条最佳实践：availability 探测、复用 Translator、destroy 释放、AbortSignal 中止、长文本分段、混合文本路由、隐私提示、降级方案、监控过期、用户选项。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行陷阱与最佳实践演示', { type: 'primary', size: 'sm', onClick: () => this._runPitfallsDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// 陷阱 1：模型体积 → 先 availability 探测
...
try {
  ...
  return cloudTranslate();
                ...
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.pitfallsInfo || '（点击按钮查看 8 大陷阱与 10 条最佳实践完整说明）')),
                h(Alert, {
                    type: 'warning',
...
//# sourceMappingURL=WebTranslationAPIPage.js.map