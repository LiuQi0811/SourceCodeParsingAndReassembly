...
export class BarcodeScreenCapturePage extends Page {
    ...
    componentDidMount() {
        ...
        if (!hasBarcode)
            this._addLog('warn', 'BarcodeDetector 不可用（typeof BarcodeDetector === "undefined"），需 Chrome/Edge 或开启实验标志');
        if (!hasMediaDevices)
            this._addLog('warn', 'navigator.mediaDevices 不可用（需安全上下文 https/localhost + 真实浏览器）');
        if (!hasImageCapture)
            this._addLog('warn', 'ImageCapture 不可用（typeof ImageCapture === "undefined"），需 Chromium 系浏览器');
        if (hasDisplayMedia)
            this._addLog('info', 'getDisplayMedia 可用，但本页不真正调用（避免触发权限弹窗）');
    }
    ...
    _caps() {
        const hasMedia = typeof navigator !== 'undefined' && navigator.mediaDevices &&
            typeof navigator.mediaDevices === 'object';
        const md = navigator.mediaDevices;
        const has = (fn) => hasMedia && typeof (md[fn]) === 'function';
        return {
    ...
    async _checkBarcodeSupport() {
        ...
        if (!caps.barcode) {
            ...
            this.setState({
                barcodeSupportInfo: `typeof BarcodeDetector = "${typeof BarcodeDetector}"\n` +
        ...
        try {
            ...
            this.setState({
                barcodeSupportInfo: `typeof BarcodeDetector = "${typeof BarcodeDetector}"（构造器可用）\n` +
    ...
    _createBarcodeDetector() {
        ...
        try {
            ...
            this.setState({
                barcodeSupportInfo: `new BarcodeDetector({ formats: ['qr_code', 'code_128', 'ean_13'] }) ✓\n` +
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.barcode ? 'success' : 'error' }, caps.barcode ? 'BarcodeDetector ✓' : '不可用'), h(Tag, { color: 'primary' }, 'qr / ean / code_128')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'Barcode Detection API 提供 new BarcodeDetector(options) 构造器，options.formats 指定识别格式白名单；BarcodeDetector.getSupportedFormats() 是静态方法，返回当前浏览器支持的格式（Promise<string[]>）。支持格式包括 qr_code / code_128 / code_39 / ean_13 / ean_8 / upc_a / upc_e / itf / codabar / pdf417 / aztec / data_matrix。用于扫描二维码、商品条码等。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('检测能力', { type: 'primary', size: 'sm', disabled: !caps.barcode, onClick: () => this._checkBarcodeSupport() }), this._btn('构造 Detector', { type: 'primary', size: 'sm', disabled: !caps.barcode, onClick: () => this._createBarcodeDetector() }), this._btn('仅检测 typeof', { size: 'sm', onClick: () => this._checkBarcodeSupport() })),
                h('div', { class: 'fs-sm text-secondary' }, '能力检测 / 构造结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, s.barcodeSupportInfo || '（点击「检测能力」或「构造 Detector」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '150px', overflow: 'auto' } }, h('code', {}, `if (typeof BarcodeDetector !== 'undefined') {
  ...
  console.warn('BarcodeDetector 不可用，需 Chromium 系浏览器');
    ...
    async _detectFromCanvas() {
        ...
        if (!caps.barcode) {
            ...
            this.setState({
                barcodeDetectInfo: `typeof BarcodeDetector = "${typeof BarcodeDetector}"\n` +
        ...
        try {
            ...
            const detectedText = results.length
                ? results.map((b, i) => `[${i}] rawValue = "${b.rawValue}"\n` +
                    `    boundingBox = { x:${b.boundingBox.x}, y:${b.boundingBox.y}, width:${b.boundingBox.width}, height:${b.boundingBox.height} }\n` +
                    `    cornerPoints = ${b.cornerPoints ? b.cornerPoints.length : 0} 个角点`).join('\n')
                : '（空数组，未识别到任何条码——canvas 画的不是合法条码）';
            this.setState({
                barcodeDetectInfo: `detector.detect(canvas) → Promise<DetectedBarcode[]> ✓\n` +
    ...
    async _detectFromImageBitmap() {
        ...
        if (!caps.createImageBitmap) {
            ...
            this.setState({
                barcodeDetectInfo: `typeof createImageBitmap = "${typeof createImageBitmap}"\n` +
        ...
        try {
            ...
            this.setState({
                barcodeDetectInfo: `createImageBitmap(blob) → ImageBitmap，可作为 detect 的 imageSource：\n\n` +
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.barcode ? 'success' : 'error' }, caps.barcode ? 'detect ✓' : '不可用'), h(Tag, { color: 'primary' }, 'canvas / ImageBitmap')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'detector.detect(imageSource) → Promise<DetectedBarcode[]> 识别条码。imageSource 可为 ImageBitmap / Blob / ImageData / HTMLImageElement / HTMLCanvasElement / HTMLVideoElement。DetectedBarcode 含 rawValue（解码文本）、boundingBox（DOMRectReadOnly）、cornerPoints（四个角点 {x,y} 数组）。本卡片构造含模拟条纹的 canvas 作为 imageSource 演示调用流程（实际不会识别为合法条码）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('detect(canvas)', { type: 'primary', size: 'sm', disabled: !caps.barcode, onClick: () => this._detectFromCanvas() }), this._btn('detect(ImageBitmap)', { size: 'sm', disabled: !caps.barcode, onClick: () => this._detectFromImageBitmap() })),
                h('div', { class: 'fs-sm text-secondary' }, 'detect 识别结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, s.barcodeDetectInfo || '（点击「detect(canvas)」或「detect(ImageBitmap)」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '140px', overflow: 'auto' } }, h('code', {}, `const detector = new BarcodeDetector({ formats: ['qr_code'] });
...
// 也可传入 ImageBitmap / Blob / ImageData / HTMLCanvasElement`)),
    ...
    _checkDisplayMediaSupport() {
        const caps = this._caps();
        if (!caps.displayMedia) {
            this._addLog('warn', 'getDisplayMedia 不可用（navigator.mediaDevices.getDisplayMedia 未定义）');
            this.setState({
                displayMediaInfo: `typeof navigator.mediaDevices = "${navigator.mediaDevices ? 'object' : 'undefined'}"\n` +
        ...
        this.setState({
            displayMediaInfo: `typeof navigator.mediaDevices.getDisplayMedia = "function"（可用）\n` +
    ...
    _demoDisplayMediaUsage() {
        ...
        const mockTrack = {
            ...
            async applyConstraints(constraints) { return Promise.resolve(); },
            onended: null,
            stop() { },
            addEventListener(type, handler) { if (type === 'ended')
        ...
        this.setState({
            displayMediaInfo: `（用 mock track 演示 API 形态，未真正请求屏幕共享）\n\n` +
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.displayMedia ? 'success' : 'error' }, caps.displayMedia ? 'getDisplayMedia ✓' : '不可用'), h(Tag, { color: 'primary' }, 'displaySurface / cursor')),
            children: [
                ...
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('检测能力', { type: 'primary', size: 'sm', disabled: !caps.displayMedia, onClick: () => this._checkDisplayMediaSupport() }), this._btn('演示 track 用法', { type: 'primary', size: 'sm', disabled: !caps.displayMedia, onClick: () => this._demoDisplayMediaUsage() }), this._btn('停止共享', { danger: true, size: 'sm', disabled: !caps.displayMedia, onClick: () => this._stopDisplayShare() })),
                h('div', { class: 'fs-sm text-secondary' }, '屏幕共享状态：'),
                h('pre', { class: 'code-block', style: { maxHeight: '280px', overflow: 'auto' } }, h('code', {}, s.displayMediaInfo || '（点击「检测能力」或「演示 track 用法」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '160px', overflow: 'auto' } }, h('code', {}, `const stream = await navigator.mediaDevices.getDisplayMedia({
...
// track.stop(); 停止共享`)),
    ...
    _checkImageCaptureSupport() {
        ...
        this.setState({
            imageCaptureInfo: `typeof ImageCapture = "${typeof ImageCapture}"（构造器可用）\n` +
    ...
    async _demoImageCaptureUsage() {
        ...
        try {
            ...
            try {
                imageCapture = new ImageCapture(mockTrack);
            ...
            if (imageCapture && typeof imageCapture.getPhotoCapabilities === 'function') {
                ...
                capsLine = `getPhotoCapabilities 不可调用（构造失败或方法不存在）`;
            }
            this.setState({
                imageCaptureInfo: `（mock track 演示 ImageCapture 调用形态，未真正调用 getUserMedia）\n\n` +
                    `${constructLine}\n\n${capsLine}\n\n` +
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.imageCapture ? 'success' : 'error' }, caps.imageCapture ? 'ImageCapture ✓' : '不可用'), h(Tag, { color: 'primary' }, 'takePhoto / grabFrame')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'ImageCapture API 提供 new ImageCapture(videoTrack) 构造器（videoTrack 来自 getUserMedia）。takePhoto(options) → Promise<Blob> 拍照（options: imageWidth/imageHeight/redEyeReduction/fillLightMode）；getPhotoCapabilities() → Promise<PhotoCapabilities> 读取能力（redEyeReduction/imageWidth{min,max,step,current}/imageHeight/fillLightMode）；getPhotoSettings() 读取当前设置；grabFrame() → Promise<ImageBitmap> 快速抓帧（比 takePhoto 快，无编码开销）。本页不真正调用 getUserMedia（避免权限弹窗），用 mock track 演示调用形态。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('检测能力', { type: 'primary', size: 'sm', disabled: !caps.imageCapture, onClick: () => this._checkImageCaptureSupport() }), this._btn('演示拍照流程', { type: 'primary', size: 'sm', disabled: !caps.imageCapture, onClick: () => this._demoImageCaptureUsage() })),
                h('div', { class: 'fs-sm text-secondary' }, 'ImageCapture 状态：'),
                h('pre', { class: 'code-block', style: { maxHeight: '300px', overflow: 'auto' } }, h('code', {}, s.imageCaptureInfo || '（点击「检测能力」或「演示拍照流程」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '160px', overflow: 'auto' } }, h('code', {}, `const stream = await navigator.mediaDevices.getUserMedia({ video: true });
const [track] = stream.getVideoTracks();
...
const bitmap = await imageCapture.grabFrame();      // 快速抓帧`)),
    ...
    _demoTrackCapabilities() {
        const caps = this._caps();
        if (!caps.userMedia) {
            this._addLog('warn', 'getUserMedia 不可用，无法获取真实 track 演示 capabilities');
            this.setState({
                trackCapabilityInfo: `typeof navigator.mediaDevices.getUserMedia = "${caps.mediaDevices ? typeof (navigator.mediaDevices.getUserMedia) : 'undefined'}"\n` +
        ...
        this.setState({
            trackCapabilityInfo: `（mock track 演示 MediaStreamTrack 高级能力，未真正调用 getUserMedia）\n\n` +
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.userMedia ? 'success' : 'error' }, caps.userMedia ? 'getUserMedia ✓' : '不可用'), h(Tag, { color: 'primary' }, 'pan / tilt / zoom / torch')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'MediaStreamTrack 高级能力：track.getCapabilities() → MediaTrackCapabilities（pan/tilt/zoom/torch 等 PTZ 能力，仅高端摄像头支持）；track.getSettings() → MediaTrackSettings（当前实际值）；track.applyConstraints({ advanced: [{ zoom, torch }] }) 动态修改；track.onmute / onunmute 事件（设备被占用/恢复）；track.clone() 克隆 track。本页用 mock track 演示 API 形态（不真正调用 getUserMedia）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('演示 track 能力', { type: 'primary', size: 'sm', disabled: !caps.userMedia, onClick: () => this._demoTrackCapabilities() })),
                h('div', { class: 'fs-sm text-secondary' }, 'MediaTrack 能力 / 设置：'),
                h('pre', { class: 'code-block', style: { maxHeight: '320px', overflow: 'auto' } }, h('code', {}, s.trackCapabilityInfo || '（点击「演示 track 能力」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '160px', overflow: 'auto' } }, h('code', {}, `const stream = await navigator.mediaDevices.getUserMedia({ video: true });
const [track] = stream.getVideoTracks();
...
const cloned = track.clone();              // 克隆独立 track`)),
    ...
    async _demoEnumerateDevices() {
        ...
        if (!caps.enumerate) {
            ...
            this.setState({
                devicesInfo: `typeof navigator.mediaDevices.enumerateDevices = "${caps.mediaDevices ? typeof (navigator.mediaDevices.enumerateDevices) : 'undefined'}"\n` +
        ...
        try {
            ...
            const byKind = { videoinput: [], audioinput: [], audiooutput: [] };
            for (const d of devices)
                if (byKind[d.kind])
                    byKind[d.kind].push(d);
            const devicesText = devices.length
                ? devices.map((d, i) => `[${i}] kind="${d.kind}" label="${d.label || '（空，未授权）'}" ` +
                    `deviceId="${d.deviceId ? d.deviceId.slice(0, 8) + '...' : '（空）'}" groupId="${d.groupId ? d.groupId.slice(0, 8) + '...' : '（空）'}"`).join('\n')
                : '（空数组，无设备或未授权）';
            // 演示 getSupportedConstraints
            let constraintsText = '';
            if (caps.supportedConstraints) {
                const supported = navigator.mediaDevices.getSupportedConstraints();
                const keys = Object.keys(supported).filter((k) => (supported[k]));
            ...
            this.setState({
                devicesInfo: `navigator.mediaDevices.enumerateDevices() → Promise<MediaDeviceInfo[]>（${devices.length} 个设备）\n\n` +
                    `设备列表：\n${devicesText}\n\n` +
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.enumerate ? 'success' : 'error' }, caps.enumerate ? 'enumerate ✓' : '不可用'), h(Tag, { color: caps.onDeviceChange ? 'success' : 'error' }, caps.onDeviceChange ? 'ondevicechange ✓' : 'ondevicechange ✗')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'navigator.mediaDevices.enumerateDevices() → Promise<MediaDeviceInfo[]> 枚举所有媒体设备，MediaDeviceInfo 含 deviceId / kind（videoinput|audioinput|audiooutput）/ label / groupId。getSupportedConstraints() → MediaTrackSupportedConstraints 返回浏览器支持的约束键（值全为 true）。ondevicechange 事件在设备插拔时触发。未授权 getUserMedia 前 label 为空字符串。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('枚举设备', { type: 'primary', size: 'sm', disabled: !caps.enumerate, onClick: () => this._demoEnumerateDevices() }), this._btn('仅检测 typeof', { size: 'sm', disabled: !caps.enumerate, onClick: () => this._demoEnumerateDevices() })),
                h('div', { class: 'fs-sm text-secondary' }, '设备枚举结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '320px', overflow: 'auto' } }, h('code', {}, s.devicesInfo || '（点击「枚举设备」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '160px', overflow: 'auto' } }, h('code', {}, `const devices = await navigator.mediaDevices.enumerateDevices();
...
navigator.mediaDevices.addEventListener('devicechange', async () => {
  ...
  console.log('设备变化', fresh);
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'page api-lab-page' }, h('h2', { class: 'section-title' }, '条码识别 / 屏幕共享 / 图像捕获 实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, '本页演示 Barcode Detection API（BarcodeDetector）、Screen Capture API（getDisplayMedia）、ImageCapture API（takePhoto/grabFrame）、MediaStreamTrack 高级能力（PTZ/torch）、MediaDevices 设备枚举。所有 API 调用前做 typeof 能力检测，jsdom 不可用时仅记日志说明。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderCard6(), this._renderLogPanel());
    }
}
//# sourceMappingURL=BarcodeScreenCapturePage.js.map