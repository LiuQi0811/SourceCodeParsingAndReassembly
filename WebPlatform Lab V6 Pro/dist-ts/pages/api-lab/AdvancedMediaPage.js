// =====================================================================
// AdvancedMediaPage.ts —— 媒体扩展与高级特性实验室
// 演示 MDN：
...
export class AdvancedMediaPage extends Page {
    ...
    componentWillUnmount() {
        ...
        if (this._mseUrl) {
            try {
                URL.revokeObjectURL(this._mseUrl);
            }
            catch { /* noop */ }
            this._mseUrl = null;
        }
        ...
        if (this._mediaStream) {
            try {
                this._mediaStream.getTracks().forEach((t) => { try {
                    t.stop();
                }
                catch { /* noop */ } });
            }
        ...
        if (this._streamTrack) {
            try {
                this._streamTrack.stop();
            }
            catch { /* noop */ }
        ...
        if (this._trackVideoEl) {
            try {
                this._trackVideoEl.pause();
            }
            catch { /* noop */ }
            this._trackVideoEl = null;
        }
        // 6. 关闭 MediaKeys session
        if (this._keySession) {
            try {
                this._keySession.close().catch(() => { });
            }
            catch { /* noop */ }
    ...
    _createMSE() {
        ...
        try {
            ...
            if (this._videoEl)
                this._videoEl.src = this._mseUrl;
    ...
    _mseAbort() {
        ...
        if (!sb) {
            this._addLog('error', 'SourceBuffer 尚未创建');
            return;
        }
        try {
            sb.abort();
            this._addLog('mse', 'sourceBuffer.abort() ✓');
        }
        catch (err) {
            this._addLog('error', `sourceBuffer.abort 失败：${errInfo(err).message}`);
        }
    }
    _mseRemove() {
        const sb = this._sourceBuffer;
        ...
        try {
            sb.remove(0, 1);
            this._addLog('mse', 'sourceBuffer.remove(0, 1) ✓');
        }
        catch (err) {
            this._addLog('error', `sourceBuffer.remove 失败：${errInfo(err).message}`);
        }
    }
    _mseEndOfStream() {
        const ms = this._mediaSource;
        if (!ms) {
            this._addLog('error', 'MediaSource 尚未创建');
            return;
        }
        try {
            ms.endOfStream();
            this._addLog('mse', `mediaSource.endOfStream() ✓ readyState=${ms.readyState}`);
        }
        catch (err) {
            this._addLog('error', `endOfStream 失败：${errInfo(err).message}`);
        }
    }
    ...
    _requestDocPiP() {
        ...
        if (!target) {
            this._addLog('error', '未找到待转移的 DOM 节点');
            return;
        }
        dpi.requestWindow({ width: 360, height: 240 })
            ...
            this._docPipPlaceholder = h('div', { class: 'log-panel__empty', style: { padding: '16px' } }, '（内容已转移到 Document PiP 窗口，关闭窗口后会回来）');
            ...
            this.on(pipWin, 'pagehide', () => {
                try {
                    if (this._docPipPlaceholder?.parentNode)
                        this._docPipPlaceholder.parentNode.replaceChild(target, this._docPipPlaceholder);
    ...
    _closeDocPiP() {
        if (!this._docPipWindow) {
            this._addLog('error', 'Document PiP 窗口未打开');
            return;
        }
        try {
            this._docPipWindow.close();
            this._addLog('pip', 'documentPictureInPicture.window.close() ✓');
        }
        catch (err) {
            this._addLog('error', `close 失败：${errInfo(err).message}`);
        }
    }
    ...
    async _runDecodeInfo() {
        ...
        for (const item of DECODE_CONFIGS) {
            try {
                ...
                results.push({ name: item.name, supported: false, smooth: false, powerEfficient: false, error: info.message });
                this._addLog('error', `decodingInfo("${item.name}") 异常：${info.message}`);
            }
        }
    ...
    async _runEncodeInfo() {
        ...
        for (const item of ENCODE_CONFIGS) {
            try {
                ...
                const info = errInfo(err);
                results.push({ name: item.name, supported: false, smooth: false, powerEfficient: false, error: info.message });
                this._addLog('error', `encodingInfo("${item.name}") 异常：${info.message}`);
            }
        }
    ...
    async _createMediaKeys() {
        ...
        try {
            ...
            this.on(session, 'message', (e) => {
                const ev = e;
                this._addLog('eme', `MediaKeySession.message：messageType=${ev.messageType}, ${ev.message?.byteLength || 0} bytes`);
            });
    ...
    _addTextTrack() {
        ...
        this.on(track, 'cuechange', () => {
            ...
            const active = activeCues && activeCues.length ? [...activeCues].map((c) => c.text).join(' / ') : '（无）';
    ...
    _toggleTrackMode() {
        ...
        if (!v || v.textTracks.length === 0) {
            this._addLog('error', '尚未添加 TextTrack');
            return;
        }
    ...
    _removeFirstCue() {
        const v = this._trackVideoEl;
        if (!v || v.textTracks.length === 0 || !v.textTracks[0].cues || v.textTracks[0].cues.length === 0) {
        ...
        if (cue)
            t.removeCue(cue);
        this._addLog('eme', `TextTrack.removeCue ✓ · 剩余 ${t.cues?.length || 0} 个 cue`);
        this._refreshTextTracks();
    }
    // ===================== Card 5: MediaStreamTrack + Insertable Streams =====================
    _detectTrack() {
        const trackSupported = typeof MediaStreamTrack !== 'undefined';
        const generatorSupported = typeof window.MediaStreamTrackGenerator !== 'undefined';
        this.setState({
            trackSupported,
            generatorSupported,
    ...
    async _getUserMedia() {
        ...
        try {
            ...
            if (!track) {
                this._addLog('error', 'getUserMedia 返回的 stream 中没有 video track');
                return;
            }
            // 监听 track 事件
            ...
            const info = errInfo(err);
            this._addLog('error', `getUserMedia 失败：${info.name || ''} ${info.message}`);
        }
    }
    _applyConstraints() {
        const track = this._streamTrack;
        if (!track) {
            this._addLog('error', '尚无 MediaStreamTrack，先点 getUserMedia');
            return;
        }
        if (typeof track.applyConstraints !== 'function') {
            this._addLog('error', 'track.applyConstraints 不可用');
            return;
        }
    ...
    _cloneTrack() {
        ...
        if (typeof track.clone !== 'function') {
            this._addLog('error', 'track.clone 不可用');
            return;
        }
    ...
    _stopTrack() {
        ...
        try {
            track.stop();
            this._addLog('track', `track.stop() ✓ · readyState=${track.readyState}`);
        }
        catch (err) {
            this._addLog('error', `track.stop 失败：${errInfo(err).message}`);
        }
    }
    _probeGenerator() {
        ...
        try {
            ...
            const GeneratorCtor = window.MediaStreamTrackGenerator;
            const gen = new GeneratorCtor({ kind: 'video' });
            ...
            if (typeof window.MediaStreamTrackProcessor !== 'undefined') {
                ...
                const proc = new ProcessorCtor({ track: gen });
    ...
    _renderCapabilitiesTable(rows) {
        if (!rows || rows.length === 0)
            return h('div', { class: 'log-panel__empty' }, '（点击上方按钮运行检测）');
        ...
        return h('table', { style: { width: '100%', borderCollapse: 'collapse' } }, h('thead', {}, h('tr', {}, h('th', { style: TH }, '配置'), h('th', { style: TH }, 'supported'), h('th', { style: TH }, 'smooth'), h('th', { style: TH }, 'powerEfficient'))), h('tbody', {}, ...rows.map((r) => h('tr', {}, h('td', { style: TD }, r.name), h('td', { style: TD }, r.error ? h(Tag, { color: 'error' }, 'err') : tagFor(r.supported)), h('td', { style: TD }, r.error ? '—' : tagFor(r.smooth)), h('td', { style: TD }, r.error ? '—' : tagFor(r.powerEfficient))))));
    }
    // ===================== 渲染 =====================
    renderPage() {
        const s = this.state;
        return [
            ...
            h(Card, {
                ...
                extra: h(Tag, { color: s.mseSupported === null ? 'default' : (s.mseSupported ? 'success' : 'error') }, s.mseSupported === null ? '检测中' : (s.mseSupported ? '稳定' : '不可用')),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'new MediaSource() → addSourceBuffer(mime) → SourceBuffer.mode（segments|sequence）/ appendBuffer / remove / abort。SourceBufferList 事件 addsourcebuffer / removesourcebuffer。URL.createObjectURL(mediaSource) 生成 video.src。MediaSource.isTypeSupported(mime) 静态检测。endOfStream() 标记流结束。'), h('div', { class: 'flex gap-sm flex-wrap' }, this._btn('创建 MediaSource', { type: 'primary', size: 'sm', onClick: () => this._createMSE() }), this._btn('sourceBuffer.abort', { size: 'sm', onClick: () => this._mseAbort() }), this._btn('sourceBuffer.remove(0,1)', { size: 'sm', onClick: () => this._mseRemove() }), this._btn('endOfStream', { size: 'sm', danger: true, onClick: () => this._mseEndOfStream() })), h('div', { class: 'flex items-center gap-sm' }, h('span', { class: 'fs-sm' }, '状态：'), h(Tag, { color: s.mseStatus === '未创建' ? 'default' : 'primary' }, s.mseStatus)), h('video', { class: 'mse-video', controls: true, muted: true, playsInline: true,
                style: { width: '100%', maxWidth: '480px', background: '#000', borderRadius: '6px' } }), s.mimeResults.length > 0
                ? h('div', {}, h('div', { class: 'fs-sm text-secondary mb-xs' }, 'MediaSource.isTypeSupported() 检测结果：'), h('div', { class: 'flex flex-col gap-xs' }, ...s.mimeResults.map((r) => h('div', { class: 'flex items-center gap-sm fs-sm' }, h(Tag, { color: r.ok ? 'success' : 'error' }, r.ok ? '✓' : '✕'), h('code', { class: 'fs-sm' }, r.mime)))))
                : h('div', { class: 'log-panel__empty' }, '（不可用时无检测结果）'))),
            // ============ Card 2: Picture-in-Picture ============
            h(Card, {
                title: '2. Picture-in-Picture 与 Document PiP',
                extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: s.pipSupported ? 'success' : 'error' }, s.pipSupported ? 'PiP' : 'PiP 不可用'), h(Tag, { color: s.docPipSupported ? 'warning' : 'error' }, s.docPipSupported ? 'Doc PiP 实验' : 'Doc PiP 不可用')),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'HTMLVideoElement.requestPictureInPicture() → Promise<PictureInPictureWindow>；document.pictureInPictureElement 当前 PiP 元素；document.exitPictureInPicture() 退出。事件 enterpictureinpicture / leavepictureinpicture / resize。Document PiP：window.documentPictureInPicture.requestWindow({width,height}) → Window，可把任意 DOM 移过去。能力检测：\'pictureInPictureEnabled\' in document / \'documentPictureInPicture\' in window。'), h('div', { class: 'flex gap-sm flex-wrap' }, this._btn('请求 PiP', { type: 'primary', size: 'sm', onClick: () => this._requestPiP(), disabled: !s.pipSupported }), this._btn('退出 PiP', { size: 'sm', onClick: () => this._exitPiP(), disabled: !s.pipActive }), this._btn('请求 Document PiP', { type: 'primary', size: 'sm', onClick: () => this._requestDocPiP(), disabled: !s.docPipSupported }), this._btn('关闭 Doc PiP 窗口', { size: 'sm', danger: true, onClick: () => this._closeDocPiP(), disabled: !s.docPipActive })), h('div', { class: 'flex items-center gap-sm flex-wrap' }, h('span', { class: 'fs-sm' }, 'PiP 状态：'), h(Tag, { color: s.pipActive ? 'success' : 'default' }, s.pipActive ? '运行中' : '未运行'), s.pipWindowSize ? h('span', { class: 'fs-sm text-secondary' }, `window=${s.pipWindowSize}`) : null, h('span', { class: 'fs-sm' }, 'Doc PiP：'), h(Tag, { color: s.docPipActive ? 'success' : 'default' }, s.docPipActive ? '已打开' : '未打开')), h('div', { class: 'doc-pip-content', style: { padding: '12px', background: 'var(--color-fill, #f5f5f5)', borderRadius: '6px' } }, h('div', { class: 'fw-medium mb-xs' }, '★ 我是会被转移到 Document PiP 窗口的 DOM 节点'), h('div', { class: 'fs-sm text-secondary' }, '点击"请求 Document PiP"后，本 div 会被 appendChild 到 PiP 窗口的 document.body；关闭窗口时通过 pagehide 事件移回原位置。')))),
            // ============ Card 3: Media Capabilities ============
            h(Card, {
                title: '3. Media Capabilities API',
                extra: h(Tag, { color: s.mcSupported === null ? 'default' : (s.mcSupported ? 'success' : 'error') }, s.mcSupported === null ? '检测中' : (s.mcSupported ? '稳定' : '不可用')),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'navigator.mediaCapabilities.decodingInfo({type, video, audio}) → {supported, smooth, powerEfficient}；type: \'file\' | \'media-source\'。encodingInfo({type, video, audio}) → 同结构；type: \'record\' | \'webrtc\'。'), h('div', { class: 'flex gap-sm flex-wrap' }, this._btn('运行 decodingInfo', { type: 'primary', size: 'sm', onClick: () => this._runDecodeInfo(), disabled: !s.mcSupported }), this._btn('运行 encodingInfo', { type: 'primary', size: 'sm', onClick: () => this._runEncodeInfo(), disabled: !s.mcSupported })), h('div', {}, h('div', { class: 'fs-sm text-secondary mb-xs' }, 'decodingInfo 结果：'), this._renderCapabilitiesTable(s.decodeResults)), h('div', {}, h('div', { class: 'fs-sm text-secondary mb-xs' }, 'encodingInfo 结果：'), this._renderCapabilitiesTable(s.encodeResults)))),
            // ============ Card 4: EME + Text Tracks ============
            h(Card, {
                title: '4. Encrypted Media Extensions (EME) 与 Text Tracks',
                extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: s.emeSupported ? 'success' : 'error' }, s.emeSupported ? 'EME' : 'EME 不可用'), h(Tag, { color: 'success' }, 'Text Tracks')),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'EME：navigator.requestMediaKeySystemAccess(keySystem, configs) → MediaKeySystemAccess；getConfiguration() / createMediaKeys()；MediaKeys.createSession(\'temporary\')；MediaKeySession.generateRequest / update / close / load；HTMLMediaElement.setMediaKeys(mediaKeys)。Text Tracks：HTMLMediaElement.textTracks、addTextTrack(kind,label,lang)；TextTrack.kind/label/language/mode/cues/addCue/removeCue；VTTCue.startTime/endTime/text/line/position/size/align。'), h('div', { class: 'flex gap-sm flex-wrap' }, this._btn('探测 keySystem', { type: 'primary', size: 'sm', onClick: () => this._probeKeySystems(), disabled: !s.emeSupported }), this._btn('createMediaKeys + setMediaKeys', { size: 'sm', onClick: () => this._createMediaKeys(), disabled: !s.emeSupported }), this._btn('addTextTrack + VTTCue', { type: 'primary', size: 'sm', onClick: () => this._addTextTrack() }), this._btn('切换 mode', { size: 'sm', onClick: () => this._toggleTrackMode() }), this._btn('removeCue(0)', { size: 'sm', danger: true, onClick: () => this._removeFirstCue() })), s.emeResults.length > 0
                ? h('div', {}, h('div', { class: 'fs-sm text-secondary mb-xs' }, 'requestMediaKeySystemAccess 探测结果：'), h('div', { class: 'flex flex-col gap-xs' }, ...s.emeResults.map((r) => h('div', { class: 'flex items-start gap-sm fs-sm' }, h(Tag, { color: r.supported ? 'success' : 'error' }, r.supported ? '✓' : '✕'), h('div', {}, h('code', { class: 'fw-medium' }, r.keySystem), h('div', { class: 'fs-sm text-tertiary' }, r.detail))))))
                : null, h('video', { class: 'track-video', controls: true, muted: true, playsInline: true,
                style: { width: '100%', maxWidth: '480px', background: '#000', borderRadius: '6px' } }), s.textTracksList.length > 0
                ? h('div', {}, h('div', { class: 'fs-sm text-secondary mb-xs' }, 'TextTracks 列表：'), ...s.textTracksList.map((t) => h('div', { class: 'fs-sm mb-xs', style: { padding: '6px 10px', background: 'var(--color-fill, #f5f5f5)', borderRadius: '4px' } }, h('strong', {}, `${t.kind}`), h('span', { class: 'text-secondary' }, ` · label="${t.label}" · lang=${t.language} · `), h(Tag, { color: t.mode === 'showing' ? 'success' : (t.mode === 'hidden' ? 'warning' : 'default') }, t.mode), h('span', { class: 'text-secondary' }, ` · cues=${t.cues.length}`))))
                : null, s.vttCues.length > 0
                ? h('div', {}, h('div', { class: 'fs-sm text-secondary mb-xs' }, 'VTTCue 内容：'), h('div', { class: 'flex flex-col gap-xs' }, ...s.vttCues.map((c) => h('div', { class: 'fs-sm', style: { padding: '4px 8px', background: 'var(--color-fill, #f5f5f5)', borderRadius: '4px' } }, h('span', { class: 'text-tertiary' }, `[${c.start.toFixed(1)}→${c.end.toFixed(1)}] `), h('span', {}, c.text)))))
                : null)),
            // ============ Card 5: MediaStreamTrack + Insertable Streams ============
            h(Card, {
                title: '5. MediaStreamTrack 深入与 Insertable Streams',
                extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: s.trackSupported ? 'success' : 'error' }, s.trackSupported ? 'Track' : 'Track 不可用'), h(Tag, { color: s.generatorSupported ? 'warning' : 'error' }, s.generatorSupported ? 'Generator 实验' : 'Generator 不可用')),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'MediaStreamTrack 属性：kind / id / label / enabled / muted / readyState（live|ended）；getCapabilities() / getConstraints() / applyConstraints() / clone() / stop()；事件 unmute / mute / ended / overconstrained。Insertable Streams：MediaStreamTrackGenerator({kind}) 创建可写帧的 track；MediaStreamTrackProcessor({track}) 读取视频帧（readable stream）。'), h('div', { class: 'flex gap-sm flex-wrap' }, this._btn('getUserMedia({video:true})', { type: 'primary', size: 'sm', onClick: () => this._getUserMedia(), disabled: !s.trackSupported }), this._btn('applyConstraints', { size: 'sm', onClick: () => this._applyConstraints(), disabled: !s.trackSupported }), this._btn('clone', { size: 'sm', onClick: () => this._cloneTrack(), disabled: !s.trackSupported }), this._btn('stop', { size: 'sm', danger: true, onClick: () => this._stopTrack(), disabled: !s.trackSupported }), this._btn('探测 Generator/Processor', { size: 'sm', onClick: () => this._probeGenerator(), disabled: !s.generatorSupported })), s.trackInfo
                ? h('pre', { class: 'code-block' }, s.trackInfo)
                : h('div', { class: 'log-panel__empty' }, '（点击 getUserMedia 获取 track 信息）'), h(Alert, {
                type: 'warning',
...
//# sourceMappingURL=AdvancedMediaPage.js.map