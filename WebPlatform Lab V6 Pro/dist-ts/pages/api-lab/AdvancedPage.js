// AdvancedPage.ts —— 高级 Web API 实验室
...
export class AdvancedPage extends Page {
    ...
    async _runCompress() {
        try {
            ...
            for (const chunk of dChunks)
                decompressed += decoder.decode(chunk, { stream: true });
    ...
    renderPage() {
        ...
        return [
            ...
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, '连接公共 echo 服务器 wss://echo.websocket.events，发送消息会被原样回显。演示 onopen/onmessage/onerror/onclose 与 readyState（0/1/2/3）状态流转。'), h('div', { class: 'flex items-center gap-sm flex-wrap' }, wsInput.render(), this._btn('连接', { type: 'primary', size: 'sm', onClick: () => this._connectWs() }), this._btn('发送', { type: 'primary', size: 'sm', onClick: () => this._sendWs() }), this._btn('关闭', { danger: true, size: 'sm', onClick: () => this._disconnectWs() })), h('div', { class: 'fs-sm text-tertiary' }, `readyState 映射：0=CONNECTING  1=OPEN  2=CLOSING  3=CLOSED`), 
            // 消息记录
            this.state.wsMessages.length === 0
                ? h('div', { class: 'fs-sm text-tertiary' }, '（暂无消息记录）')
                : h('div', { class: 'log-panel' }, ...this.state.wsMessages.map((m) => h('div', { class: 'log-panel__line' }, h('span', { class: 'log-panel__time' }, m.time), h('span', { class: `log-panel__tag log-panel__tag--${m.dir === 'out' ? 'push' : 'pop'}` }, m.dir === 'out' ? 'SEND' : 'RECV'), h('span', {}, m.text)))))),
            // ============ 2. IndexedDB ============
            h(Card, {
                title: '2. IndexedDB（浏览器事务型数据库）',
                extra: h(Tag, { color: 'warning' }, `${this.state.notes.length} 条笔记`),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, '数据库 mdn-lab-db（v1），object store「notes」（keyPath: id，autoIncrement）。演示 onupgradeneeded/onsuccess/onerror/oncomplete 事件。'), h('div', { class: 'flex items-center gap-sm flex-wrap' }, noteTitleInput.render(), this._btn('添加笔记', { type: 'primary', size: 'sm', onClick: () => this._addNote() }), this._btn('刷新列表', { size: 'sm', onClick: () => this._loadNotes() }), this._btn('清空 store', { danger: true, size: 'sm', onClick: () => this._clearNotes() })), noteContentInput.render(), 
            // 笔记列表
            this.state.notes.length === 0
                ? h('div', { class: 'fs-sm text-tertiary' }, '（暂无笔记，添加一条试试）')
                : h('div', { class: 'flex flex-col gap-sm' }, ...this.state.notes.map((note) => h('div', { class: 'flex items-center gap-sm' }, h('span', { class: 'fs-sm text-secondary', style: { minWidth: '40px' } }, `#${note.id}`), h('div', { class: 'flex-1' }, h('div', { class: 'fw-medium fs-sm' }, note.title), h('div', { class: 'fs-sm text-tertiary' }, note.content)), h('span', { class: 'fs-sm text-tertiary' }, formatTime(new Date(note.createdAt))), h('button', {
                    type: 'button',
            ...
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, '创建名为 mdn-lab-bc 的广播频道，输入消息发送后，其他打开本页面的标签页会收到。消息结构：{ from: timestamp, text: ... }。'), h('div', { class: 'flex items-center gap-sm flex-wrap' }, bcInput.render(), this._btn('广播消息', { type: 'primary', size: 'sm', onClick: () => this._sendBc() })), h('p', { class: 'fs-sm text-tertiary' }, '提示：复制当前页面 URL 到新标签页打开，即可测试跨标签页消息收发。'))),
            // ============ 4. Performance API ============
            h(Card, {
                title: '4. Performance API（性能监控）',
                extra: h(Tag, { color: 'primary' }, `${this.state.perfResults.length} 条测量`),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, '使用 performance.now() 测量耗时、mark() 打标记、measure() 计算区间、PerformanceObserver 监听 measure 条目。'), this._btn('运行性能测试', { type: 'primary', size: 'sm', onClick: () => this._runPerfTest() }), this.state.perfResults.length === 0
                ? h('div', { class: 'fs-sm text-tertiary' }, '（尚未运行测试）')
                : h('div', { class: 'flex flex-col gap-sm' }, ...this.state.perfResults.map((r) => h('div', { class: 'api-metric' }, h('span', { class: 'fs-sm text-secondary', style: { minWidth: '160px' } }, r.name), h('span', { class: 'fw-medium fs-sm' }, typeof r.duration === 'string' ? r.duration : `${r.duration.toFixed(2)}ms`), h('span', { class: 'fs-sm text-tertiary', style: { marginLeft: '12px' } }, String(r.count))))))),
            // ============ 5. Web Crypto ============
            h(Card, {
                title: '5. Web Crypto（SubtleCrypto 加密）',
                extra: h(Tag, { color: 'primary' }, 'SHA-256 / AES-GCM'),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, '演示 crypto.subtle.digest（SHA-256）、generateKey + encrypt/decrypt（AES-GCM）、getRandomValues。所有 subtle 方法返回 Promise。'), h('div', { class: 'flex items-center gap-sm flex-wrap' }, cryptoInput.render(), this._btn('计算 SHA-256', { type: 'primary', size: 'sm', onClick: () => this._computeHash() }), this._btn('AES-GCM 加解密', { type: 'primary', size: 'sm', onClick: () => this._runCrypto() }), this._btn('生成随机数', { size: 'sm', onClick: () => this._genRandom() })), h('div', { class: 'fs-sm text-secondary' }, 'SHA-256 哈希：'), h('pre', { class: 'code-block' }, this.state.cryptoHash || '（点击「计算 SHA-256」）'), h('div', { class: 'fs-sm text-secondary mt-sm' }, 'AES-GCM 加密结果（十六进制）：'), h('pre', { class: 'code-block' }, this.state.cryptoEncrypted || '（点击「AES-GCM 加解密」）'), h('div', { class: 'fs-sm text-secondary mt-sm' }, `解密还原：${this.state.cryptoDecrypted || '—'}`))),
            // ============ 6. CompressionStream + TextEncoder ============
            h(Card, {
                title: '6. CompressionStream + TextEncoder（编码与压缩）',
                extra: h(Tag, { color: 'warning' }, 'gzip'),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'TextEncoder 将字符串编码为 Uint8Array，CompressionStream/DecompressionStream 进行 gzip 流式压缩/解压。'), compressInput.render(), this._btn('运行压缩测试', { type: 'primary', size: 'sm', onClick: () => this._runCompress() }), h('div', { class: 'fs-sm fw-medium' }, this.state.compressResult || '（点击「运行压缩测试」）'))),
            // ============ 日志面板 ============
            h(Card, {
                title: '事件日志',
