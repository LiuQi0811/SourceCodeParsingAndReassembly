...
export class CSSMediaUserPreferencePage extends Page {
    ...
    componentDidMount() {
        ...
        if (!caps.matchMedia) {
            ...
            // 列出当前所有探测结果
            const cur = [];
            if (caps.colorSchemeDark)
                cur.push(`prefers-color-scheme=dark`);
            if (caps.colorSchemeLight)
                cur.push(`prefers-color-scheme=light`);
            if (caps.reducedMotion)
                cur.push(`prefers-reduced-motion=reduce`);
            if (caps.noReducedMotion)
                cur.push(`prefers-reduced-motion=no-preference`);
            if (caps.contrastMore)
                cur.push(`prefers-contrast=more`);
            if (caps.forcedColors)
                cur.push(`forced-colors=active`);
            if (caps.reducedTransparency)
                cur.push(`prefers-reduced-transparency=reduce`);
            if (caps.reducedData)
                cur.push(`prefers-reduced-data=reduce`);
            if (caps.invertedColors)
                cur.push(`inverted-colors=inverted`);
            if (caps.dynamicRangeHigh)
                cur.push(`dynamic-range=high`);
            if (caps.hover)
                cur.push(`hover=hover`);
            if (caps.pointerFine)
                cur.push(`pointer=fine`);
            if (caps.pointerCoarse)
                cur.push(`pointer=coarse`);
...
`;
    ...
    _showColorSchemeCaps() {
        const caps = this._caps();
        this.setState({ colorSchemeInfo: '===== prefers-color-scheme 与 color-scheme 能力检测 =====\n\n' +
    ...
    _showColorSchemeSyntax() {
        this.setState({ colorSchemeInfo: '===== prefers-color-scheme 与 color-scheme 语法 =====\n\n' +
                '/* 1. 媒体查询：根据系统配色应用不同样式 */\n' +
                '@media (prefers-color-scheme: dark) {\n' +
    ...
    _compareSystemVsManual() {
        this.setState({ colorSchemeInfo: '===== 系统偏好 vs 手动主题切换 =====\n\n' +
    ...
    _showColorSchemeListener() {
        const caps = this._caps();
        this.setState({ colorSchemeInfo: '===== 监听系统主题切换（matchMedia change 事件） =====\n\n' +
                '/* 注册监听 */\n' +
                "const mql = window.matchMedia('(prefers-color-scheme: dark)');\n" +
                '\n' +
                '// 现代写法（推荐）\n' +
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.colorScheme ? 'success' : 'error' }, caps.colorScheme ? 'prefers-color-scheme ✓' : 'prefers-color-scheme ✗'), h(Tag, { color: caps.colorSchemeDark ? 'warning' : 'primary' }, caps.colorSchemeDark ? '当前=dark' : '当前=light'), h(Tag, { color: 'info' }, 'light-dark()')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '@media (prefers-color-scheme: dark|light) 检测用户系统深浅色偏好；color-scheme: light dark CSS 属性声明元素支持的配色方案，浏览器自动适配默认表单控件、滚动条、canvas 背景；light-dark() 函数根据当前 color-scheme 自动取值；window.matchMedia(\'(prefers-color-scheme: dark)\').matches JS 检测；matchMedia(...).addEventListener(\'change\', cb) 监听系统主题切换。系统偏好是用户全局设置，应优先尊重（vs 手动 .dark 类 + localStorage）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('能力检测', { type: 'primary', size: 'sm', onClick: () => this._showColorSchemeCaps() }), this._btn('展示语法', { size: 'sm', onClick: () => this._showColorSchemeSyntax() }), this._btn('系统 vs 手动', { size: 'sm', onClick: () => this._compareSystemVsManual() }), this._btn('监听 change', { size: 'sm', onClick: () => this._showColorSchemeListener() })),
                h('div', { class: 'css-mup-cs-demo fs-sm' }, 'color-scheme: light dark 演示容器（背景/文字为系统颜色 Canvas / CanvasText，跟随系统深浅色）'),
                h('div', { class: 'css-mup-light-dark-demo fs-sm' }, 'light-dark(#f5f5f5, #1a1a1a) 演示（真实浏览器中跟随系统切换）'),
    ...
    _showReducedMotionCaps() {
        const caps = this._caps();
        this.setState({ reducedMotionInfo: '===== prefers-reduced-motion 能力检测 =====\n\n' +
    ...
    _showReducedMotionPattern() {
        this.setState({ reducedMotionInfo: '===== prefers-reduced-motion 实战模式 =====\n\n' +
                '/* 全局禁用动画（推荐作为 reset）*/\n' +
                '@media (prefers-reduced-motion: reduce) {\n' +
                '  *, *::before, *::after {\n' +
                '    animation-duration: 0.01ms !important;\n' +
    ...
    _showReducedMotionWithWAAPI() {
        const caps = this._caps();
        this.setState({ reducedMotionInfo: '===== prefers-reduced-motion 与 WAAPI 协同 =====\n\n' +
                '// JS 检测用户偏好\n' +
    ...
    _showReducedMotionDecision() {
        this.setState({ reducedMotionInfo: '===== 动画决策矩阵（reduced-motion 优先级） =====\n\n' +
                '场景                          | 处理方式                                   | 理由\n' +
                '------------------------------|--------------------------------------------|---------------------------\n' +
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.reducedMotion ? 'success' : 'error' }, caps.reducedMotion ? 'reduce ✓' : 'reduce ✗'), h(Tag, { color: caps.reducedMotionActive ? 'warning' : 'primary' }, caps.reducedMotionActive ? '当前=reduce' : '当前=no-preference'), h(Tag, { color: 'info' }, 'WAAPI 协同')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '@media (prefers-reduced-motion: reduce|no-preference) 检测用户是否请求减少动画（前庭障碍、注意力问题、节省电量）。实战模式：@media (prefers-reduced-motion: reduce) { *, *::before, *::after { animation-duration: 0.01ms !important; transition-duration: 0.01ms !important; } }。matchMedia(\'(prefers-reduced-motion: reduce)\').matches JS 检测；与 WAAPI 协同：element.animate(keyframes, { duration: prefersReducedMotion ? 0 : 300 })。CSS @media 处理 declarative，JS 检测用于动态决策（如禁用 rAF 循环）。'),
                h('div', { class: 'css-mup-motion-demo flex items-center gap-sm' }, h('div', { class: 'css-mup-motion-box' }), h('span', { class: 'fs-sm text-secondary' }, '↑ 旋转方块演示（reduce 偏好下应几乎不动）')),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('能力检测', { type: 'primary', size: 'sm', onClick: () => this._showReducedMotionCaps() }), this._btn('实战模式', { size: 'sm', onClick: () => this._showReducedMotionPattern() }), this._btn('与 WAAPI 协同', { size: 'sm', onClick: () => this._showReducedMotionWithWAAPI() }), this._btn('动画决策矩阵', { size: 'sm', onClick: () => this._showReducedMotionDecision() })),
    ...
    _showContrastForcedCaps() {
        const caps = this._caps();
        this.setState({ contrastForcedInfo: '===== prefers-contrast 与 forced-colors 能力检测 =====\n\n' +
    ...
    _showContrastForcedSyntax() {
        this.setState({ contrastForcedInfo: '===== prefers-contrast 与 forced-colors 语法 =====\n\n' +
    ...
    _showContrastForcedPractical() {
        this.setState({ contrastForcedInfo: '===== 高对比度 / 强制配色实战要点 =====\n\n' +
                '1. 边框与可见性\n' +
    ...
    _showContrastForcedDecision() {
        const caps = this._caps();
        this.setState({ contrastForcedInfo: '===== prefers-contrast vs forced-colors 决策 =====\n\n' +
                '二者区别：\n' +
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.contrast ? 'success' : 'error' }, caps.contrast ? 'contrast ✓' : 'contrast ✗'), h(Tag, { color: caps.forcedColors ? 'success' : 'error' }, caps.forcedColors ? 'forced-colors ✓' : 'forced-colors ✗'), h(Tag, { color: caps.forcedColorsActive ? 'warning' : 'primary' }, caps.forcedColorsActive ? '当前=active' : '当前=none')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '@media (prefers-contrast: more|less|custom|no-preference) 用户请求的对比度级别；@media (forced-colors: active|none) Windows 高对比度模式（强制配色）；forced-color-adjust: auto|none|preserve-parent-color 控制元素是否服从强制配色。强制配色下颜色被替换为系统调色板（Canvas/CanvasText/ButtonText 等），需用系统颜色关键字。实战：高对比度模式下调整边框、移除纯装饰性背景、确保焦点可见。matchMedia(\'(forced-colors: active)\').matches JS 检测。'),
                h('div', { class: 'css-mup-contrast-demo css-mup-forced-demo fs-sm', style: { padding: '8px 12px', border: '1px solid #888', borderRadius: '4px' } }, '强制配色 / 高对比度演示容器（forced-colors: active 时背景=Canvas、文字=CanvasText、边框=ButtonText）'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('能力检测', { type: 'primary', size: 'sm', onClick: () => this._showContrastForcedCaps() }), this._btn('展示语法', { size: 'sm', onClick: () => this._showContrastForcedSyntax() }), this._btn('实战要点', { size: 'sm', onClick: () => this._showContrastForcedPractical() }), this._btn('决策对比', { size: 'sm', onClick: () => this._showContrastForcedDecision() })),
    ...
    _showTransparencyDataCaps() {
        const caps = this._caps();
        this.setState({ transparencyDataInfo: '===== prefers-reduced-transparency 与 prefers-reduced-data 能力检测 =====\n\n' +
    ...
    _showTransparencyDataSyntax() {
        this.setState({ transparencyDataInfo: '===== prefers-reduced-transparency 与 prefers-reduced-data 语法 =====\n\n' +
    ...
    _showTransparencyPractical() {
        this.setState({ transparencyDataInfo: '===== prefers-reduced-transparency 实战 =====\n\n' +
                '触发场景：\n' +
    ...
    _showDataPractical() {
        this.setState({ transparencyDataInfo: '===== prefers-reduced-data 实战 =====\n\n' +
                '触发场景：\n' +
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.reducedTransparency ? 'success' : 'error' }, caps.reducedTransparency ? 'transparency ✓' : 'transparency ✗'), h(Tag, { color: caps.reducedData ? 'success' : 'error' }, caps.reducedData ? 'reduced-data ✓' : 'reduced-data ✗'), h(Tag, { color: 'warning' }, '较新特性')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '@media (prefers-reduced-transparency: reduce) 用户请求减少半透明效果（提升可读性）；@media (prefers-reduced-data: reduce) 用户请求减少数据传输（省流量模式）。reduced-data 实战：不加载非必要资源、用系统字体替代 web font、低分辨率图片；reduced-transparency 实战：移除 backdrop-filter、降低 opacity 叠加。两者均为较新特性，浏览器支持有限。matchMedia(\'(prefers-reduced-transparency: reduce)\').matches / matchMedia(\'(prefers-reduced-data: reduce)\').matches 检测。'),
                h('div', { class: 'css-mup-transparency-demo fs-sm' }, '半透明 + backdrop-filter 演示容器（reduce-transparency 时应变为实色）'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('能力检测', { type: 'primary', size: 'sm', onClick: () => this._showTransparencyDataCaps() }), this._btn('展示语法', { size: 'sm', onClick: () => this._showTransparencyDataSyntax() }), this._btn('transparency 实战', { size: 'sm', onClick: () => this._showTransparencyPractical() }), this._btn('data 实战', { size: 'sm', onClick: () => this._showDataPractical() })),
    ...
    _showInvertedDynamicCaps() {
        const caps = this._caps();
        this.setState({ invertedDynamicInfo: '===== inverted-colors 与 dynamic-range 能力检测 =====\n\n' +
    ...
    _showInvertedColorsPractical() {
        this.setState({ invertedDynamicInfo: '===== inverted-colors 实战（避免双重反转） =====\n\n' +
                '问题场景：\n' +
    ...
    _showDynamicRangePractical() {
        this.setState({ invertedDynamicInfo: '===== dynamic-range / color-gamut 实战（HDR / 广色域） =====\n\n' +
    ...
    _showInvertedDynamicDecision() {
        const caps = this._caps();
        this.setState({ invertedDynamicInfo: '===== inverted-colors vs dynamic-range vs color-gamut 决策 =====\n\n' +
                '三者关注不同维度：\n' +
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.invertedColors ? 'success' : 'error' }, caps.invertedColors ? 'inverted-colors ✓' : 'inverted-colors ✗'), h(Tag, { color: caps.dynamicRange ? 'success' : 'error' }, caps.dynamicRange ? 'dynamic-range ✓' : 'dynamic-range ✗'), h(Tag, { color: caps.colorGamutP3 ? 'success' : 'default' }, caps.colorGamutP3 ? 'P3 ✓' : 'P3 ?')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '@media (inverted-colors: inverted|none) 检测系统是否启用颜色反转（macOS 反转颜色辅助功能）；@media (dynamic-range: high|standard) 检测显示器动态范围（HDR/SDR），与 color-gamut 相关；@media (color-gamut: rec2020|p3|srgb) 检测色域支持。inverted-colors 实战：反转图片避免双重反转（图片被系统反转一次再被 CSS 反转一次）；dynamic-range/high 实战：为 HDR 显示器提供更广色域内容。浏览器支持：inverted-colors 仅 Safari/macOS；dynamic-range 较新。 '),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('能力检测', { type: 'primary', size: 'sm', onClick: () => this._showInvertedDynamicCaps() }), this._btn('inverted 实战', { size: 'sm', onClick: () => this._showInvertedColorsPractical() }), this._btn('dynamic-range 实战', { size: 'sm', onClick: () => this._showDynamicRangePractical() }), this._btn('决策对比', { size: 'sm', onClick: () => this._showInvertedDynamicDecision() })),
    ...
    _showHoverPointerCaps() {
        const caps = this._caps();
        this.setState({ hoverPointerInfo: '===== hover 与 pointer 媒体特性能力检测 =====\n\n' +
    ...
    _showHoverPointerSyntax() {
        this.setState({ hoverPointerInfo: '===== hover 与 pointer 媒体特性语法 =====\n\n' +
    ...
    _showHoverPointerPractical() {
        this.setState({ hoverPointerInfo: '===== hover / pointer 实战要点 =====\n\n' +
                '1. 触屏「粘滞 hover」问题\n' +
    ...
    _showHoverPointerDecision() {
        const caps = this._caps();
        this.setState({ hoverPointerInfo: '===== hover vs pointer vs any-hover 决策矩阵 =====\n\n' +
                '场景                          | 推荐检测                    | 理由\n' +
                '------------------------------|-----------------------------|---------------------------\n' +
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.hover ? 'success' : 'error' }, caps.hover ? 'hover ✓' : 'hover ✗'), h(Tag, { color: caps.pointer ? 'success' : 'error' }, caps.pointer ? 'pointer ✓' : 'pointer ✗'), h(Tag, { color: caps.hoverSupported ? 'warning' : 'primary' }, `当前=${caps.hoverSupported ? 'hover' : 'none'} / ${pointerLabel}`)),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '@media (hover: hover|none) 检测主输入设备是否支持 hover（桌面 vs 触屏）；@media (pointer: fine|coarse|none) 检测指针精度（鼠标 fine / 触屏 coarse / 无指针 none）；@media (any-hover: hover) / @media (any-pointer: fine) 检测任意输入设备（多设备场景）。实战：hover 设备显示 hover 效果与工具提示；触屏设备增大点击区域、用 tap 替代 hover。vs 特征检测 window.matchMedia(\'(hover: hover)\').matches 比 ontouchstart 更可靠；与 :hover 伪类协同：@media (hover: hover) { .btn:hover { ... } }。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('能力检测', { type: 'primary', size: 'sm', onClick: () => this._showHoverPointerCaps() }), this._btn('展示语法', { size: 'sm', onClick: () => this._showHoverPointerSyntax() }), this._btn('实战要点', { size: 'sm', onClick: () => this._showHoverPointerPractical() }), this._btn('决策矩阵', { size: 'sm', onClick: () => this._showHoverPointerDecision() })),
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'api-lab-page css-media-user-preference-page' }, h('h2', { class: 'section-title' }, 'CSS 用户偏好媒体查询'), h('p', { class: 'fs-sm text-secondary mb-md' }, '本页演示 CSS Media Queries Level 5+ 用户偏好与交互能力特性：prefers-color-scheme 与 color-scheme、prefers-reduced-motion、prefers-contrast 与 forced-colors、prefers-reduced-transparency 与 prefers-reduced-data、inverted-colors 与 dynamic-range、hover 与 pointer 媒体特性。jsdom 中 matchMedia 默认未实现，所有特性检测会降级为仅说明，真实浏览器中可实时探测与监听系统偏好变化。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderCard6(), this._renderLogPanel());
    }
}
//# sourceMappingURL=CSSMediaUserPreferencePage.js.map