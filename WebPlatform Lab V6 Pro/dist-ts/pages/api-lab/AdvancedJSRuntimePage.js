...
async function* tickGen(count, delay = 200) {
...
export class AdvancedJSRuntimePage extends Page {
    _asyncGenCancel = null;
    _asyncGenRunning = false;
    _disposedStacks = [];
    _inited = false;
    initialState() {
        return {
            ...
            caps: {
                ...
                arrayGroup: typeof Array.prototype.group === 'function',
    ...
    componentDidMount() {
        ...
        if (!c.iteratorHelpers)
            this._addLog('warn', 'Iterator Helpers 不可用：将回退到 Array.from + 数组方法');
        if (!c.symbolDispose && !c.disposableStack)
            this._addLog('warn', 'ERM（using/DisposableStack）部分不可用，将用 try/finally 兜底');
        if (!c.promiseAny)
            this._addLog('warn', 'Promise.any 不可用（ES2021），将跳过相关演示');
        if (!c.structuredClone)
            this._addLog('warn', 'structuredClone 不可用，将回退到 JSON 拷贝');
        if (!c.randomUUID)
            this._addLog('warn', 'crypto.randomUUID 不可用，将回退到 getRandomValues 拼接');
    }
    ...
    _demoReflectVsObject() {
        ...
        try {
            ...
            objectErr = e.constructor.name + ': ' + e.message;
        }
    ...
    _demoApplyConstructProxy() {
        ...
        const originalAdd = function (a, b) { return a + b; };
        ...
        function Gadget(name) { this.name = name; this.kind = 'gadget'; }
    ...
    _demoIteratorHelpers() {
        ...
        if (c.iteratorHelpers && c.iteratorFrom) {
            ...
            L.push(`reduce(求和) → ${Iterator.from([1, 2, 3, 4]).reduce((acc, x) => acc + x, 0)}`);
            L.push(`some(>8) → ${Iterator.from([1, 5, 10]).some((x) => x > 8)}`);
            L.push(`every(>0) → ${Iterator.from([1, 2, 3]).every((x) => x > 0)}`);
            L.push(`find(偶) → ${Iterator.from([1, 2, 3, 4]).find((x) => x % 2 === 0)}`);
            ...
            const processed = arr.filter((x) => x % 2 === 0).map((x) => x * x).slice(1, 4);
    ...
    _demoERM() {
        ...
        const makeResource = (label) => {
            ...
            if (c.symbolDispose)
                resource[Symbol.dispose] = disposeFn;
            else
                resource.dispose = disposeFn;
            return resource;
        };
        ...
        if (c.disposableStack) {
            ...
            stack.adopt(tmpFile, (f) => { f.deleted = true; L.push(`  • adopt() 清理：${f.name} 已删除`); });
            L.push(`  • 注册 3 个资源到 stack（defer/use/adopt）`);
            const stackRef = stack;
            stackRef.dispose();
            L.push(`  • stack.dispose() 完成，所有资源已按 LIFO 顺序释放`);
            this._disposedStacks = this._disposedStacks.filter((s) => s !== stackRef);
        }
        else {
            ...
            try {
                L.push(`  • 开启 ${resources.length} 个资源`);
            }
            finally {
                for (let i = resources.length - 1; i >= 0; i--) {
                    const fn = (resources[i])[Symbol.dispose] || (resources[i]).dispose;
                    fn.call(resources[i]);
                }
            }
        }
        ...
        L.push('', '说明：using 声明是语法关键字，运行时无法能力检测；如浏览器不支持，需用 Babel/SWC 编译为 try/finally。', '      using r = makeRes(); 等价于 try { ... } finally { r[Symbol.dispose]?.(); }');
    ...
    _makePromises() {
        const make = (label, ok, delay = 100 + Math.random() * 400) => new Promise((resolve, reject) => setTimeout(() => ok ? resolve(`${label}✓`) : reject(new Error(`${label}✗`)), delay));
        return [make('A', false), make('B', false), make('C', true)];
    }
    async _runPromiseAny() {
        if (!this.state.caps.promiseAny) {
    ...
    async _runPromiseAllSettled() {
        if (!this.state.caps.promiseAllSettled) {
            this._addLog('warn', 'Promise.allSettled 不可用');
            return;
        }
        ...
        const text = res.map((r, i) => r.status === 'fulfilled' ? `  #${i + 1} fulfilled → ${r.value}` : `  #${i + 1} rejected  → ${r.reason.message}`).join('\n');
    ...
    async _runPromiseAnyAllReject() {
        if (!this.state.caps.promiseAny || !this.state.caps.aggregateError) {
            this._addLog('warn', 'Promise.any 或 AggregateError 不可用');
            return;
        }
        this._addLog('info', 'Promise.any（3 个全 reject）开始，预期抛 AggregateError');
        const allReject = ['X1', 'X2', 'X3'].map((l) => new Promise((_, rej) => setTimeout(() => rej(new Error(l + ' 失败')), 80 + Math.random() * 300)));
        try {
            ...
            this.setState({
                promiseResult: `AggregateError 触发！\n` +
    ...
    _demoErrorCause() {
        ...
        try {
            try {
                ...
                const wrapped = new Error('保存用户失败', { cause: originalErr });
    ...
    _demoArrayNew() {
        ...
        if (c.arrayGroup) {
            L.push(`Array.prototype.group 可用：${JSON.stringify(nums.group((x) => (x % 2 ? 'odd' : 'even')))}`);
        }
        else {
    ...
    _demoStringNew() {
        ...
        matches.forEach((m, i) => L.push(`  #${i + 1} 完整="${m[0]}"，订单号=${m[1]}，金额=${m[2]}`));
        L.push('');
    ...
    _demoMisc() {
        ...
        if (c.structuredClone) {
            ...
            L.push(`  (cloned.buf as any).byteLength        → ${cloned.buf.byteLength}`);
            L.push(`  深拷贝独立：cloned.nested !== original.nested → ${cloned.nested !== original.nested}`);
            try {
                structuredClone({ fn: () => 1 });
            }
            catch (err) {
                L.push(`  structuredClone({fn}) 抛异常：${err.message}（不支持函数/DOM 节点等）`);
            }
        }
        else {
        ...
        if (c.randomUUID) {
            ...
            uuids.forEach((u, i) => L.push(`  ${i + 1}. ${u}`));
    ...
    renderPage() {
        ...
        return [
            ...
            h(Card, {
                ...
                extra: h('div', { class: 'flex items-center gap-xs flex-wrap' }, h(Tag, { color: 'success' }, 'Proxy:稳定'), h(Tag, { color: 'success' }, 'Reflect:稳定')),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'new Proxy(target, handler) 提供 13 个 trap（get/set/has/deleteProperty/ownKeys/apply/construct/getPrototypeOf/setPrototypeOf/defineProperty/getOwnPropertyDescriptor/isExtensible/preventExtensions）。Reflect 与 Proxy trap 一一对应，方法返回 boolean 而非抛异常，便于在 trap 内组合默认行为 + 增强。'), h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('演示 Proxy 拦截', { type: 'primary', size: 'sm', onClick: () => this._demoProxyIntercept() }), this._btn('Reflect 对比 Object', { type: 'primary', size: 'sm', onClick: () => this._demoReflectVsObject() }), this._btn('apply/construct trap', { type: 'primary', size: 'sm', onClick: () => this._demoApplyConstructProxy() })), h('div', { class: 'fs-sm text-secondary' }, 'Proxy 拦截 get/set/has/delete/ownKeys：'), h('pre', { class: 'code-block' }, s.proxyResult), h('div', { class: 'fs-sm text-secondary mt-sm' }, 'Reflect vs Object（返回 boolean vs 抛异常）：'), h('pre', { class: 'code-block' }, s.reflectResult), h('div', { class: 'fs-sm text-secondary mt-sm' }, 'apply trap（函数 Proxy）+ construct trap（构造函数 Proxy）：'), h('pre', { class: 'code-block' }, s.applyProxyResult))),
            // ============ Card 2: Iterator Helpers 与 async iterator ============
            h(Card, {
                title: 'Card 2. Iterator Helpers 与 Symbol.asyncIterator',
                extra: h('div', { class: 'flex items-center gap-xs flex-wrap' }, this._capsBadge(c, 'iteratorHelpers', 'Helpers'), this._capsBadge(c, 'iteratorFrom', 'Iterator.from'), h(Tag, { color: c.iteratorHelpers ? 'success' : 'warning' }, c.iteratorHelpers ? 'ES2025' : '回退')),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'Iterator Helpers（ES2025）：iterator.map/filter/take/drop/toArray/forEach/reduce/some/every/find 链式惰性求值；Iterator.from(iterable) 静态方法。Generator 用 function* + yield，yield* 委托；async function* + for await...of 处理异步迭代。'), h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('Iterator Helpers', { type: 'primary', size: 'sm', onClick: () => this._demoIteratorHelpers() }), this._btn('运行异步生成器', { type: 'primary', size: 'sm', onClick: () => this._demoAsyncIterator() })), h('div', { class: 'fs-sm text-secondary' }, 'Generator + Iterator Helpers：'), h('pre', { class: 'code-block' }, s.iterHelperResult), h('div', { class: 'fs-sm text-secondary mt-sm' }, 'async function* + for await...of：'), h('pre', { class: 'code-block' }, s.asyncIterResult))),
            // ============ Card 3: Explicit Resource Management ============
            h(Card, {
                title: 'Card 3. Explicit Resource Management（using / await using）',
                extra: h('div', { class: 'flex items-center gap-xs flex-wrap' }, this._capsBadge(c, 'symbolDispose', 'Symbol.dispose'), this._capsBadge(c, 'disposableStack', 'DisposableStack'), h(Tag, { color: c.disposableStack ? 'success' : 'warning' }, c.disposableStack ? 'ES2025' : 'Stage 3')),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'using 声明绑定 [Symbol.dispose]() 资源，离开作用域自动释放；await using 绑定 [Symbol.asyncDispose]()；DisposableStack/AsyncDisposableStack 提供 .use()/.adopt()/.defer()/.dispose() 集中管理。由于 using 是语法关键字，运行时无法能力检测，回退到 try/finally + 手动 dispose。'), this._btn('演示 using/DisposableStack', { type: 'primary', size: 'sm', onClick: () => this._demoERM() }), h('pre', { class: 'code-block' }, s.ermResult))),
            // ============ Card 4: Promise 组合与错误处理 ============
            h(Card, {
                title: 'Card 4. Promise 组合与错误处理新特性',
                extra: h('div', { class: 'flex items-center gap-xs flex-wrap' }, this._capsBadge(c, 'promiseAny', 'Promise.any'), this._capsBadge(c, 'aggregateError', 'AggregateError'), h(Tag, { color: 'success' }, 'Error.cause:稳定')),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'Promise.any：任一 fulfill 即返回，全 reject 抛 AggregateError；Promise.allSettled：等所有 settle 返回 {status, value/reason}；AggregateError.errors 是失败原因数组；Error.cause（ES2022）保留原始错误链，便于层层包装定位根因。'), h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('Promise.any（2拒1成）', { type: 'primary', size: 'sm', onClick: () => this._runPromiseAny() }), this._btn('Promise.allSettled', { type: 'primary', size: 'sm', onClick: () => this._runPromiseAllSettled() }), this._btn('any(全拒→AggregateError)', { danger: true, size: 'sm', onClick: () => this._runPromiseAnyAllReject() }), this._btn('演示 Error.cause', { type: 'primary', size: 'sm', onClick: () => this._demoErrorCause() })), h('div', { class: 'fs-sm text-secondary' }, 'Promise 组合结果：'), h('pre', { class: 'code-block' }, s.promiseResult), h('div', { class: 'fs-sm text-secondary mt-sm' }, 'Error.cause 错误链：'), h('pre', { class: 'code-block' }, s.errorCauseResult))),
            // ============ Card 5: 数据结构与 API 新方法 ============
            h(Card, {
                title: 'Card 5. 数据结构与 API 新方法',
                extra: h('div', { class: 'flex items-center gap-xs flex-wrap' }, this._capsBadge(c, 'structuredClone', 'structuredClone'), this._capsBadge(c, 'randomUUID', 'UUID'), h(Tag, { color: 'success' }, 'Array/String:ES2021-2024')),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'Array：at/findLast/findLastIndex/toSorted/toReversed/toSpliced/with/Object.groupBy/Map.groupBy；String：replaceAll/matchAll/at/padStart/padEnd/trimStart/trimEnd；Object：hasOwn/fromEntries；其它：structuredClone/BigInt/BigInt64Array/atob/btoa/crypto.randomUUID。'), h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('Array 新方法', { type: 'primary', size: 'sm', onClick: () => this._demoArrayNew() }), this._btn('String 新方法', { type: 'primary', size: 'sm', onClick: () => this._demoStringNew() }), this._btn('structuredClone/BigInt/UUID', { type: 'primary', size: 'sm', onClick: () => this._demoMisc() })), h('div', { class: 'fs-sm text-secondary' }, 'Array 新方法：'), h('pre', { class: 'code-block' }, s.arrayNewResult), h('div', { class: 'fs-sm text-secondary mt-sm' }, 'String 新方法：'), h('pre', { class: 'code-block' }, s.stringNewResult), h('div', { class: 'fs-sm text-secondary mt-sm' }, 'structuredClone / BigInt / UUID：'), h('pre', { class: 'code-block' }, s.miscResult))),
            // ============ 日志面板 ============
            h(Card, {
                title: '事件日志',
...
//# sourceMappingURL=AdvancedJSRuntimePage.js.map