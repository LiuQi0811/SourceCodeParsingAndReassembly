...
export class WebNNAPIPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!f.createContext)
            this._addLog('warn', 'navigator.ml.createContext 不可用（MLContext 创建演示跳过）');
        if (!f.opBuilder)
            this._addLog('warn', 'opBuilder / MLGraphBuilder 不可用（计算图构建演示跳过）');
        if (!f.mlContext)
            this._addLog('warn', 'MLContext 类型未定义');
        if (!f.mlGraph)
            this._addLog('warn', 'MLGraph 类型未定义（model.build 演示跳过）');
        if (!f.mlCompiledModel)
            this._addLog('warn', 'MLCompiledModel 类型未定义（compile 演示跳过）');
        this._injectBaseStyles();
    }
    componentWillUnmount() {
        if (this._mlContext) {
            try {
                this._mlContext.dispose?.();
            }
            catch { /* noop */ }
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'WebNN API（W3C Web ML WG）是浏览器原生神经网络推理 API，直接调用 NPU/GPU/CPU 硬件加速器，性能数量级优于 WASM/WebGL。与 TF.js/ORT Web 是「底层后端」关系（框架解析模型后调度到 WebNN）。Built-in AI（Translator/Summarizer/Prompt API）是 WebNN 之上的封装层。浏览器支持：Chrome 113+ flag / 121+ 部分 OS 默认 / origin trial；Firefox/Safari 截至 2025 未实现。jsdom 无此 API。'),
                ...
                h('div', { class: 'webnn-flow' }, h('span', { class: 'webnn-flow-node webnn-flow-node--api' }, 'Built-in AI'), h('span', { class: 'webnn-flow-arrow' }, '↓'), h('span', { class: 'webnn-flow-node webnn-flow-node--api' }, 'TF.js / ONNX RT'), h('span', { class: 'webnn-flow-arrow' }, '↓'), h('span', { class: 'webnn-flow-node' }, 'WebNN API'), h('span', { class: 'webnn-flow-arrow' }, '↓'), h('span', { class: 'webnn-flow-node webnn-flow-node--hw' }, 'NPU / GPU / CPU')),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// 检测 WebNN
if ('ml' in navigator) {
  ...
  console.warn('WebNN 不可用，降级到 wasm 后端');
                ...
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.overviewInfo || '（点击按钮查看 WebNN 概述与动机完整说明）')),
            ],
        });
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'navigator.ml.createContext({ deviceType, numThreads, powerPreference }) 创建 MLContext（不透明硬件句柄）。deviceType 三选：cpu（兼容广，AVX/NEON 向量指令）/ gpu（DirectML/Metal/Vulkan，性能高）/ npu（专用芯片，性能最高功耗最低，算子有限）。powerPreference: default/high-performance/low-power（仅建议）。必须 HTTPS + Secure Context。NPU 不可用应 fallback GPU → CPU。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行入口演示', { type: 'primary', size: 'sm', onClick: () => this._runEntryDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '三种 deviceType 速查：'),
                h('div', { class: 'webnn-device-grid' }, h('div', { class: 'webnn-device-card webnn-device-card--cpu' }, h('div', { class: 'webnn-device-title' }, 'cpu'), h('div', {}, 'AVX2/AVX512/NEON\n兼容广 · 性能低\nnumThreads 控制并行')), h('div', { class: 'webnn-device-card webnn-device-card--gpu' }, h('div', { class: 'webnn-device-title' }, 'gpu'), h('div', {}, 'DirectML/Metal/Vulkan\n性能高 10-100x\n移动端功耗高')), h('div', { class: 'webnn-device-card webnn-device-card--npu' }, h('div', { class: 'webnn-device-title' }, 'npu'), h('div', {}, 'Intel NPU/Apple ANE\n性能最高 · 功耗最低\n算子支持有限'))),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// NPU → GPU → CPU 降级
async function createContext() {
  if (!('ml' in navigator)) throw new Error('WebNN 不支持');
  for (const type of ['npu', 'gpu', 'cpu']) {
    try {
    ...
    } catch (e: any) { console.warn(type + ' 不可用:', e.message); }
  }
  throw new Error('无可用 WebNN 后端');
}
...
console.log('isSecureContext:', window.isSecureContext);`)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.entryInfo || '（点击按钮查看 Navigator ML 入口完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'new MLGraphBuilder(context) 创建 builder。Operand 是张量，通过 builder.input(name, { dataType, dimensions }) 创建输入，builder.constant(desc, value) 创建权重。dataType 支持 float32/float16/int32/int8/int4。dimensions 即 NCHW/NHWC shape（layout 选项控制）。算子链式调用：每个算子返回 MLOperand，作为下一算子输入。builder.build({ output }) 编译为 MLGraph（可多输出）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行计算图演示', { type: 'primary', size: 'sm', onClick: () => this._runGraphDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例（简单 MLP）：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `const context = await navigator.ml.createContext({ deviceType: 'gpu' });
const builder = new MLGraphBuilder(context);
...
const model = builder.build({ output: out });`)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.graphInfo || '（点击按钮查看构建计算图完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['MLGraphBuilder', f.opBuilder]]), h(Tag, { color: 'primary' }, '算子')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'WebNN 规范定义 80+ 算子，覆盖主流模型所需：卷积（conv2d/convTranspose2d/depthwise via groups）、矩阵（matmul/transpose/reshape/concat/split/gather/slice）、激活（relu/sigmoid/tanh/leakyRelu/gelu/swish/mish）、池化（maxPool2d/averagePool2d/globalXxx）、归一化（batchNorm/layerNorm/instanceNorm）、归约（reduceMean/Max/Sum/L1/L2/LogSumExp）、注意力（attention 实验性）、RNN（gru/lstm/gruCell/lstmCell）、量化（int8/uint8/int4 算子）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行算子全集演示', { type: 'primary', size: 'sm', onClick: () => this._runOpsDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '算子分类速查：'),
                h('div', { class: 'webnn-ops-grid' }, h('span', { class: 'webnn-op-tag webnn-op-tag--core' }, 'conv2d'), h('span', { class: 'webnn-op-tag webnn-op-tag--core' }, 'matmul'), h('span', { class: 'webnn-op-tag webnn-op-tag--core' }, 'transpose'), h('span', { class: 'webnn-op-tag webnn-op-tag--core' }, 'reshape'), h('span', { class: 'webnn-op-tag' }, 'add/sub/mul'), h('span', { class: 'webnn-op-tag webnn-op-tag--act' }, 'relu'), h('span', { class: 'webnn-op-tag webnn-op-tag--act' }, 'sigmoid'), h('span', { class: 'webnn-op-tag webnn-op-tag--act' }, 'tanh'), h('span', { class: 'webnn-op-tag webnn-op-tag--act' }, 'leakyRelu'), h('span', { class: 'webnn-op-tag webnn-op-tag--act' }, 'gelu'), h('span', { class: 'webnn-op-tag webnn-op-tag--act' }, 'swish'), h('span', { class: 'webnn-op-tag webnn-op-tag--act' }, 'softmax'), h('span', { class: 'webnn-op-tag' }, 'maxPool2d'), h('span', { class: 'webnn-op-tag' }, 'avgPool2d'), h('span', { class: 'webnn-op-tag' }, 'batchNorm'), h('span', { class: 'webnn-op-tag' }, 'layerNorm'), h('span', { class: 'webnn-op-tag webnn-op-tag--reduce' }, 'reduceMean'), h('span', { class: 'webnn-op-tag webnn-op-tag--reduce' }, 'reduceMax'), h('span', { class: 'webnn-op-tag webnn-op-tag--reduce' }, 'reduceSum'), h('span', { class: 'webnn-op-tag webnn-op-tag--reduce' }, 'reduceL2'), h('span', { class: 'webnn-op-tag' }, 'concat'), h('span', { class: 'webnn-op-tag' }, 'split'), h('span', { class: 'webnn-op-tag' }, 'gather'), h('span', { class: 'webnn-op-tag' }, 'slice'), h('span', { class: 'webnn-op-tag' }, 'pad'), h('span', { class: 'webnn-op-tag' }, 'cast'), h('span', { class: 'webnn-op-tag' }, 'where'), h('span', { class: 'webnn-op-tag' }, 'squeeze'), h('span', { class: 'webnn-op-tag' }, 'attention'), h('span', { class: 'webnn-op-tag' }, 'gru/lstm')),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// 常用算子调用
let x = builder.input('x', { dataType: 'float32', dimensions: [1, 3, 224, 224] });
...
const model = builder.build({ output: logits });`)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.opsInfo || '（点击按钮查看 80+ 算子全集完整说明）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '四阶段流水线：builder.build(output) 同步构建 MLGraph（拓扑校验）→ graph.compile() 异步编译为硬件二进制（算子融合 + codegen，首次 100ms~10s）→ compilation.createExecution() 创建执行句柄 → execution.dispatch(inputs) 异步推理返回 outputs。MLTensor 是 GPU/NPU 显存对象，零拷贝；context.readTensor 读回 CPU。dispatch 异步非阻塞但不并发，多输入需多个 execution。编译结果可缓存（同 page 内自动，跨 page 实验性）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行编译执行演示', { type: 'primary', size: 'sm', onClick: () => this._runCompileDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '四阶段流水线：'),
                h('div', { class: 'webnn-flow' }, h('span', { class: 'webnn-flow-node webnn-flow-node--api' }, 'build()'), h('span', { class: 'webnn-flow-arrow' }, '→'), h('span', { class: 'webnn-flow-node' }, 'MLGraph'), h('span', { class: 'webnn-flow-arrow' }, '→'), h('span', { class: 'webnn-flow-node webnn-flow-node--api' }, 'compile()'), h('span', { class: 'webnn-flow-arrow' }, '→'), h('span', { class: 'webnn-flow-node' }, 'MLCompilation')),
                h('div', { class: 'webnn-flow' }, h('span', { class: 'webnn-flow-node webnn-flow-node--api' }, 'createExecution()'), h('span', { class: 'webnn-flow-arrow' }, '→'), h('span', { class: 'webnn-flow-node' }, 'MLExecution'), h('span', { class: 'webnn-flow-arrow' }, '→'), h('span', { class: 'webnn-flow-node webnn-flow-node--api' }, 'dispatch(inputs)'), h('span', { class: 'webnn-flow-arrow' }, '→'), h('span', { class: 'webnn-flow-node webnn-flow-node--hw' }, 'outputs')),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// 完整推理流水线
const context = await navigator.ml.createContext({ deviceType: 'gpu' });
const builder = new MLGraphBuilder(context);
// ... 构建计算图（Card 3/4）
...
for await (const frame of videoFrames) {
  ...
  renderResult(logits);
                ...
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.compileInfo || '（点击按钮查看编译与执行完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    `);
        ...
        const info = [
            ...
            '    const arr = Array.from(probs).map((p: any, i: number) => ({ prob: p, idx: i }));',
            '    arr.sort((a: any, b: any) => b.prob - a.prob);',
            '    return arr.slice(0, k).map(({ prob, idx }: any) => ({',
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['navigator.ml', f.navMl], ['MLGraph', f.mlGraph]]), h(Tag, { color: 'primary' }, '实战')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'MobileNetV3 图像分类完整流程：1）图像预处理（resize 224x224 + normalize mean/std + HWC→CHW）2）构建计算图（conv2d+BN+h-swish+15 个 MBConv blocks+全局池化+全连接+softmax）3）编译缓存（首次 ~2s，缓存后毫秒级）4）dispatch 推理（MLTensor 零拷贝）5）softmax → top-5 解码 ImageNet 标签 6）与 ONNX Runtime Web 互操作（WebNN EP 自动调度）。性能：WebNN GPU ~3ms vs WASM ~280ms，~93x 提升。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 MobileNetV3 实战演示', { type: 'primary', size: 'sm', onClick: () => this._runMobilenetDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '推理流水线：'),
                h('div', { class: 'webnn-flow' }, h('span', { class: 'webnn-flow-node' }, '图像 ImageData'), h('span', { class: 'webnn-flow-arrow' }, '→'), h('span', { class: 'webnn-flow-node webnn-flow-node--api' }, '预处理 resize+normalize'), h('span', { class: 'webnn-flow-arrow' }, '→'), h('span', { class: 'webnn-flow-node webnn-flow-node--hw' }, 'MLTensor'), h('span', { class: 'webnn-flow-arrow' }, '→'), h('span', { class: 'webnn-flow-node webnn-flow-node--api' }, 'dispatch')),
                h('div', { class: 'webnn-flow' }, h('span', { class: 'webnn-flow-node webnn-flow-node--hw' }, 'NPU/GPU 推理'), h('span', { class: 'webnn-flow-arrow' }, '→'), h('span', { class: 'webnn-flow-node webnn-flow-node--api' }, 'softmax'), h('span', { class: 'webnn-flow-arrow' }, '→'), h('span', { class: 'webnn-flow-node' }, 'top-5 标签')),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// 预处理：resize + normalize + HWC→CHW
function preprocess(imageData: any, w: any = 224, h: any = 224) {
...
const top5 = Array.from(probs)
  .map((p: any, i: number) => ({ prob: p, idx: i }))
  .sort((a: any, b: any) => b.prob - a.prob)
  .slice(0, 5)
  .map(({ prob, idx }: any) => ({ label: imagenetLabels[idx], prob }));
...
const session = await ort.InferenceSession.create(url, {
  executionProviders: ['webnn', 'wasm'],  // 优先 WebNN，降级 wasm
                ...
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.mobilenetInfo || '（点击按钮查看 MobileNetV3 实战完整方案）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard7() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['navigator.ml', f.navMl], ['MLGraph', f.mlGraph]]), h(Tag, { color: 'primary' }, '实战')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'BERT 文本分类完整流程：1）Tokenizer（WordPiece/BPE，JS 实现）2）Token Embedding（gather 查表）+ Positional Encoding + LayerNorm 3）Multi-Head Attention（Q/K/V 投影 + 拆多头 + scaled dot-product + attention mask + softmax + 合并多头）4）LayerNorm + 残差 5）FFN（dim→4*dim→dim，GELU 激活）6）重复 12 层 Transformer Block 7）[CLS] token 分类头 + softmax。与 WebGPU 协同：WebGPU 训练 + WebNN 推理，性能 GPU ~8ms vs WASM ~650ms。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 BERT 实战演示', { type: 'primary', size: 'sm', onClick: () => this._runBertDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, 'Transformer Block 结构：'),
                h('div', { class: 'webnn-flow' }, h('span', { class: 'webnn-flow-node' }, 'input'), h('span', { class: 'webnn-flow-arrow' }, '→'), h('span', { class: 'webnn-flow-node webnn-flow-node--api' }, 'Multi-Head Attention'), h('span', { class: 'webnn-flow-arrow' }, '→'), h('span', { class: 'webnn-flow-node webnn-flow-node--api' }, 'Add + LayerNorm')),
                h('div', { class: 'webnn-flow' }, h('span', { class: 'webnn-flow-node webnn-flow-node--api' }, 'FFN (dim→4dim→dim, GELU)'), h('span', { class: 'webnn-flow-arrow' }, '→'), h('span', { class: 'webnn-flow-node webnn-flow-node--api' }, 'Add + LayerNorm'), h('span', { class: 'webnn-flow-arrow' }, '→'), h('span', { class: 'webnn-flow-node' }, 'output')),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例（Multi-Head Attention 核心）：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// Multi-Head Attention（手写，不用实验性 attention 算子）
function mha(builder: any,  x: any,  numHeads: any,  dim: any,  weights: any,  seqLen: any) {
  const headDim = dim / numHeads;
  // Q/K/V 投影
  ...
  return builder.matmul(attn, weights.Wo);
                ...
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.bertInfo || '（点击按钮查看 BERT 文本分类实战完整方案）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard8() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'primary' }, '性能'), h(Tag, { color: f.navMl ? 'success' : 'error' }, `WebNN ${f.navMl ? '✓' : '✗'}`)),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '八大陷阱：1）硬件后端性能数量级差（NPU > GPU > CPU，NPU ~60x CPU）2）首次 compile 耗时大（GPU ~2s，NPU ~5s，需 requestIdleCallback 预热）3）compilation 序列化跨 page 缓存实验性（Chrome 124+，需 IndexedDB fallback）4）privacy 攻击面（deviceType 推断硬件，不能用于指纹）5）与 Built-in AI 共享 NPU 排队 6）仅 Chrome 部分版本支持，必须 Polyfill（WebNN→WebGPU→WebGL→WASM）7）算子不支持需 fallback CPU 8）移动端电量散热（low-power + Battery API）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行性能与陷阱演示', { type: 'primary', size: 'sm', onClick: () => this._runPerfDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '硬件后端性能对比（MobileNetV3 Large）：'),
                h('div', { class: 'webnn-matrix' }, h('div', { class: 'webnn-matrix-cell' }, h('div', { class: 'webnn-matrix-title' }, 'CPU i7-12700'), h('div', {}, '~120ms / 推理\n~500ms compile')), h('div', { class: 'webnn-matrix-cell' }, h('div', { class: 'webnn-matrix-title' }, 'GPU RTX 3060'), h('div', {}, '~3ms / 推理\n~2000ms compile')), h('div', { class: 'webnn-matrix-cell' }, h('div', { class: 'webnn-matrix-title' }, 'NPU Intel NPU'), h('div', {}, '~2ms / 推理\n~5000ms compile')), h('div', { class: 'webnn-matrix-cell' }, h('div', { class: 'webnn-matrix-title' }, 'WASM (TF.js)'), h('div', {}, '~280ms / 推理\n无 compile')), h('div', { class: 'webnn-matrix-cell' }, h('div', { class: 'webnn-matrix-title' }, 'WebGL (TF.js)'), h('div', {}, '~18ms / 推理\n无 compile')), h('div', { class: 'webnn-matrix-cell' }, h('div', { class: 'webnn-matrix-title' }, 'WebGPU (TF.js)'), h('div', {}, '~12ms / 推理\n无 compile'))),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例（预热 + 序列化缓存）：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// 1. 预热：requestIdleCallback 中编译
requestIdleCallback(async () => {
  const graph = buildModel();
...
async function getCachedCompilation(modelKey: any,  buildFn: any,  context: any) {
  const db = await openDB('webnn-cache');
  const cached = await db.get('compilations', modelKey);
  if (cached) {
...
async function getBackend() {
  ...
  return 'wasm';
                ...
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.perfInfo || '（点击按钮查看性能与陷阱完整说明）')),
            ],
        });
        this.registerChild(card);
        return card.render();
...
//# sourceMappingURL=WebNNAPIPage.js.map