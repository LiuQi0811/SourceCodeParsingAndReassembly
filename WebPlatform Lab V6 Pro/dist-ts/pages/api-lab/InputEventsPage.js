...
export class InputEventsPage extends Page {
    ...
    _touchPoints = {};
    _touchColors = [];
    _lastTouchMoveState = 0;
    _keyboardArea = null;
    _keyboardNeedsFocus = false;
    _keyboardDidFocus = false;
    _gamepadInitChecked = false;
    _gamepadRaf = null;
    _lastGamepadState = 0;
    _compareBatch = [];
    _compareBatchStart = 0;
    _compareFlushTimer = null;
    _preventDefaultCompare = false;
    initialState() {
        return {
            logs: [],
    ...
    _ensurePointerCtx() {
        if (this._pointerCanvas && this._pointerCanvas.isConnected)
    ...
    _onPointerDown(e) {
        ...
        try {
            this._pointerArea?.setPointerCapture(e.pointerId);
        }
        catch { /* noop */ }
        ...
        // 控制轨迹点数量
        if (this._pointerPoints.length > 1000)
            this._pointerPoints = this._pointerPoints.slice(-500);
    ...
    _ensureTouchCtx() {
        if (this._touchCanvas && this._touchCanvas.isConnected)
    ...
    _redrawTouchTrail() {
        if (!this._touchCtx || !this._touchPoints || !this._touchCanvas)
            return;
    ...
    _onTouchMove(e) {
        ...
        Array.from(e.changedTouches).forEach((t) => {
            const { x, y } = this._touchCoords(t);
            if (!this._touchPoints[t.identifier])
                this._touchPoints[t.identifier] = [];
    ...
    _updateTouchInfo(e, phase) {
        ...
        touches.forEach((t, i) => {
            lines.push(`  #${i} id=${t.identifier} (${t.clientX.toFixed(0)},${t.clientY.toFixed(0)}) force=${t.force.toFixed(2)} radius=${t.radiusX.toFixed(0)}×${t.radiusY.toFixed(0)}`);
        });
        if (lines.length === 2)
            lines.push('  （当前无活跃触点）');
        // 收集所有轨迹点用于状态展示
    ...
    _pollGamepad() {
        ...
        const now = performance.now();
        if (now - this._lastGamepadState < 100)
            return;
    ...
    _getDeviceCapabilities(e) {
        ...
        if (!caps)
            return '不支持 InputDeviceCapabilities';
        const parts = ['InputDeviceCapabilities'];
        if ('fireTouchEvents' in caps)
            parts.push(`fireTouchEvents=${caps.fireTouchEvents}`);
        return parts.join(' ');
    }
    ...
    _onCompareTouchStart(e) {
        if (!this._compareBatch || this._compareBatch.length === 0)
            return;
    ...
    renderPage() {
        ...
        return [
            ...
            }, h('p', { class: 'fs-sm text-secondary' }, '在下方区域按下并拖动指针（鼠标 / 触摸 / 手写笔）。演示 pointerType / pointerId / isPrimary / pressure / tiltX·tiltY / width·height，以及 setPointerCapture 指针捕获与 canvas 轨迹绘制。'), h('div', { class: 'flex gap-sm mt-sm' }, this._btn('清空轨迹', { size: 'sm', onClick: () => this._clearPointerTrail() })), h('div', {
                class: 'pointer-area',
                style: {
                    height: '200px',
            ...
            }, h('canvas', { class: 'pointer-canvas', style: { width: '100%', height: '100%', display: 'block' } }), h('div', { style: { position: 'absolute', top: '8px', left: '12px', color: 'var(--color-text-tertiary)', fontSize: '12px', pointerEvents: 'none' } }, 'pointer-area（在此交互）')), h('pre', { class: 'code-block mt-sm', style: { whiteSpace: 'pre-wrap', fontSize: '12px', margin: 0 } }, this.state.pointerInfo)),
            // —— 2. Touch Events ——
            h(Card, {
                title: 'Touch Events 触摸事件',
                extra: h(Tag, { color: touchSupported ? 'success' : 'default' }, touchSupported ? '支持' : '不支持'),
            }, h('p', { class: 'fs-sm text-secondary' }, 'Touch Events 是早期触摸 API，通过 touches / changedTouches / targetTouches 处理多指触控。现代推荐使用 Pointer Events 作为统一方案。'), !touchSupported
                ? h(Alert, { type: 'warning', message: '当前设备不支持触摸事件', description: "'ontouchstart' in window 检测为 false。需要触摸屏设备或开启浏览器 DevTools 触摸模拟。" })
                : h('div', {}, h('div', { class: 'flex gap-sm mt-sm' }, this._btn('清空轨迹', { size: 'sm', onClick: () => this._clearTouchTrail() })), h('div', {
                    class: 'touch-area',
                    style: {
                        height: '200px',
                        border: '2px dashed var(--color-border)',
                ...
                }, h('canvas', { class: 'touch-canvas', style: { width: '100%', height: '100%', display: 'block' } }), h('div', { style: { position: 'absolute', top: '8px', left: '12px', color: 'var(--color-text-tertiary)', fontSize: '12px', pointerEvents: 'none' } }, 'touch-area（多指触摸）')), h('pre', { class: 'code-block mt-sm', style: { whiteSpace: 'pre-wrap', fontSize: '12px', margin: 0 } }, this.state.touchInfo))),
            // —— 3. Keyboard Events ——
            h(Card, {
                title: 'Keyboard Events 键盘事件',
                extra: h(Tag, { color: 'primary' }, `${this.state.keyHistory.length} 次`),
            }, h('p', { class: 'fs-sm text-secondary' }, '点击下方区域聚焦后按键。演示 key（逻辑键名）vs code（物理键码）、location、repeat、修饰键，以及 Ctrl+S 组合键拦截。'), h('div', {
                class: 'keyboard-area',
                tabindex: '0',
                style: {
                    height: '80px',
            ...
            }, '点击此处聚焦，然后按任意键'), h('pre', { class: 'code-block mt-sm', style: { whiteSpace: 'pre-wrap', fontSize: '12px', margin: 0 } }, this.state.keyInfo), 
            // 按键历史
            this.state.keyHistory.length > 0 ? h('div', { class: 'log-panel mt-sm', style: { maxHeight: '120px' } }, ...this.state.keyHistory.map((k) => h('div', { class: 'log-panel__line' }, h('span', { class: 'log-panel__time' }, k.time), h('span', { class: 'log-panel__tag log-panel__tag--mutate' }, 'key'), h('span', {}, `key="${k.key}"  code="${k.code}"`)))) : null),
            // —— 4. Gamepad API ——
            h(Card, {
                title: 'Gamepad API 游戏手柄',
                extra: h(Tag, { color: this.state.gamepadConnected ? 'success' : 'default' }, this.state.gamepadConnected ? '已连接' : '未连接'),
            }, h('p', { class: 'fs-sm text-secondary' }, '连接手柄并按任意按钮触发 gamepadconnected 事件。手柄状态需要用 requestAnimationFrame 轮询（事件不会持续触发）。'), h('pre', { class: 'code-block mt-sm', style: { whiteSpace: 'pre-wrap', fontSize: '12px', margin: 0 } }, this.state.gamepadInfo), 
            // 手柄状态可视化
            gs ? h('div', { class: 'mt-sm' }, 
            // 摇杆轴值可视化
            h('div', { class: 'fs-sm text-secondary', style: { marginBottom: '4px' } }, 'Axes 摇杆轴值：'), h('div', { class: 'flex flex-col gap-sm' }, ...gs.axes.map((v, i) => h('div', { class: 'flex items-center', style: { gap: '8px', fontSize: '12px' } }, h('span', { style: { width: '60px', fontFamily: 'var(--font-family-mono)' } }, `Axis ${i}`), h('div', { style: { flex: '1', height: '8px', background: 'var(--color-bg-spotlight)', borderRadius: '4px', position: 'relative', overflow: 'hidden' } }, h('div', { style: { position: 'absolute', left: '50%', top: '0', width: '1px', height: '100%', background: 'var(--color-border)' } }), h('div', { style: {
                    position: 'absolute',
                    left: `${50 + v * 50}%`,
                    top: '0',
                    width: '4px',
            ...
            // 按钮状态可视化
            h('div', { class: 'fs-sm text-secondary', style: { marginTop: '12px', marginBottom: '4px' } }, 'Buttons 按钮状态：'), h('div', { class: 'flex', style: { flexWrap: 'wrap', gap: '6px' } }, ...gs.buttons.map((b, i) => h('div', {
                style: {
                    width: '36px',
                    height: '36px',
                    borderRadius: '6px',
            ...
            }, String(i))))) : null, !this.state.gamepadConnected ? h('p', { class: 'fs-sm text-tertiary mt-sm' }, '提示：连接手柄后按下任意按钮，将触发 gamepadconnected 事件。') : null),
            // —— 5. InputDeviceCapabilities + 事件对比 ——
            h(Card, {
            ...
            }, h('p', { class: 'fs-sm text-secondary' }, '同一区域同时监听 pointerdown / touchstart / mousedown，观察事件触发顺序。开启 preventDefault 可在 pointerdown 上阻止后续兼容事件。'), h('div', { class: 'flex gap-sm mt-sm' }, this._btn(this._preventDefaultCompare ? '关闭 preventDefault' : '开启 preventDefault', {
                size: 'sm',
            ...
            // 事件触发顺序
            this.state.eventComparison.length > 0 ? h('div', { class: 'mt-sm' }, h('div', { class: 'fs-sm text-secondary', style: { marginBottom: '4px' } }, '事件触发顺序：'), h('div', { class: 'flex', style: { gap: '8px', flexWrap: 'wrap' } }, ...this.state.eventComparison.map((ev, i) => h(Tag, {
                color: ev.type === 'pointerdown' ? 'primary' : ev.type === 'touchstart' ? 'success' : 'warning',
            }, `${i + 1}. ${ev.type} (${ev.time.toFixed(1)}ms${ev.prevented ? ', 已阻止' : ''})`)))) : null, 
            // 对比表
            h('div', { class: 'mt-lg' }, h('div', { class: 'fs-sm text-secondary', style: { marginBottom: '4px' } }, 'Mouse / Touch / Pointer 事件模型对比：'), h('table', { style: { width: '100%', borderCollapse: 'collapse' } }, h('thead', {}, h('tr', {}, h('th', { style: TH_STYLE }, '特性'), h('th', { style: TH_STYLE }, 'Mouse Events'), h('th', { style: TH_STYLE }, 'Touch Events'), h('th', { style: TH_STYLE }, 'Pointer Events'))), h('tbody', {}, ...COMPARISON_TABLE.map((row) => h('tr', {}, h('td', { style: TD_STYLE }, h('strong', {}, row.feature)), h('td', { style: TD_STYLE }, row.mouse), h('td', { style: TD_STYLE }, row.touch), h('td', { style: TD_STYLE }, row.pointer))))))),
            // —— 事件日志 ——
            h(Card, { title: '事件日志', extra: h('span', { class: 'fs-sm text-tertiary' }, '实时') }),
            h('div', { class: 'log-panel' }, ...this.state.logs.map((log) => h('div', { class: 'log-panel__line' }, h('span', { class: 'log-panel__time' }, log.time), h('span', { class: `log-panel__tag log-panel__tag--${LOG_TAG_CLASS[log.type] || 'info'}` }, log.type), h('span', {}, log.content)))),
        ];
    }
}
//# sourceMappingURL=InputEventsPage.js.map