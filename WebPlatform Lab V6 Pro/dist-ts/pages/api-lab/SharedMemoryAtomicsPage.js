...
export class SharedMemoryAtomicsPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!hasResizable)
            this._addLog('warn', 'ArrayBuffer.prototype.resizable / maxByteLength 不可用（需 Node 20+ 或现代浏览器）');
        if (!hasTransfer)
        ...
        const hasSharedStruct = (() => { try {
            return typeof SharedStruct !== 'undefined' || (typeof window !== 'undefined' && typeof window.SharedStruct !== 'undefined');
        }
        catch {
            return false;
        } })();
        if (!hasSharedStruct)
            this._addLog('warn', 'SharedStruct 不可用（TC39 Stage 3 提案，Chrome behind flag，jsdom 无）');
    }
    ...
    _createSAB() {
        ...
        try {
            ...
            this.setState({
                sabInfo: `new SharedArrayBuffer(16) → 共享内存 ✓，byteLength = ${sab.byteLength}\n` +
    ...
    _sliceSAB() {
        ...
        try {
            ...
            this.setState({
                sabInfo: `sab.slice(0, 8) → 拷贝出新的 SharedArrayBuffer（与原 SAB 独立）\n` +
    ...
    _growSAB() {
        ...
        try {
            ...
            this.setState({
                sabInfo: `new SharedArrayBuffer(8, { maxByteLength: 32 }) → 可增长 SAB ✓\n` +
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.sab ? 'success' : 'error' }, caps.sab ? 'SAB ✓' : '不可用'), h(Tag, { color: caps.crossIsolated ? 'success' : 'warning' }, caps.crossIsolated ? 'crossOriginIsolated ✓' : '非隔离'), h(Tag, { color: 'primary' }, 'load / store / slice / grow')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'new SharedArrayBuffer(byteLength) 创建跨 Worker 共享的固定长度内存；new Int32Array(sab) 等创建视图；Atomics.load(typedArray, index) / Atomics.store(typedArray, index, value) 原子读写；sab.slice(start, end) 拷贝出新 SAB；new SharedArrayBuffer(byteLength, { maxByteLength }) + sab.grow(newLength) 创建可增长 SAB。使用 SharedArrayBuffer 需 crossOriginIsolated 环境（COOP: same-origin + COEP: require-corp）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('创建 SAB', { type: 'primary', size: 'sm', disabled: !caps.sab, onClick: () => this._createSAB() }), this._btn('slice 拷贝', { size: 'sm', disabled: !caps.sab, onClick: () => this._sliceSAB() }), this._btn('可增长 SAB', { type: 'primary', size: 'sm', disabled: !caps.sab, onClick: () => this._growSAB() })),
                h('div', { class: 'fs-sm text-secondary' }, 'SharedArrayBuffer 状态：'),
                h('pre', { class: 'code-block', style: { maxHeight: '220px', overflow: 'auto' } }, h('code', {}, s.sabInfo || '（点击「创建 SAB」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '140px', overflow: 'auto' } }, h('code', {}, `const sab = new SharedArrayBuffer(16);        // 16 字节
const view = new Int32Array(sab);             // 4 个 Int32 槽位
...
// postMessage(sab, [sab]) —— SharedArrayBuffer 共享不转移`)),
    ...
    _createAtomicsView() {
        ...
        try {
            ...
            this.setState({
                atomicsInfo: `new SharedArrayBuffer(16) + new Int32Array(sab) → Atomics 演示视图 ✓\n` +
    ...
    _atomicsOps() {
        ...
        try {
            ...
            this.setState({
                atomicsInfo: `Atomics add/sub/and/or/xor（返回旧值，原子更新）：\n` +
    ...
    _atomicsVsNormal() {
        ...
        try {
            ...
            const r1 = view[1];
            this.setState({
                atomicsInfo: `原子操作 vs 普通操作（单线程累加 1000 次）：\n` +
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: ok ? 'success' : 'error' }, ok ? 'Atomics ✓' : '不可用'), h(Tag, { color: 'primary' }, 'add/sub/and/or/xor/exchange/CAS')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'Atomics.add/sub/and/or/xor(typedArray, index, value) 都是「读-改-写」原子操作，返回旧值；Atomics.exchange(typedArray, index, value) 无条件交换返回旧值；Atomics.compareExchange(typedArray, index, expectedValue, newValue) 是 CAS 原语，仅当当前值等于 expectedValue 时才写入；Atomics.isLockFree(size) 判断 size 字节原子操作是否无需锁。原子操作保证不可分割，是多线程无锁数据结构的基础。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('创建视图', { type: 'primary', size: 'sm', disabled: !ok, onClick: () => this._createAtomicsView() }), this._btn('原子运算 + CAS', { type: 'primary', size: 'sm', disabled: !ok, onClick: () => this._atomicsOps() }), this._btn('原子 vs 普通', { size: 'sm', disabled: !ok, onClick: () => this._atomicsVsNormal() })),
                h('div', { class: 'fs-sm text-secondary' }, 'Atomics 操作结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '240px', overflow: 'auto' } }, h('code', {}, s.atomicsInfo || '（点击「创建视图」开始）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '120px', overflow: 'auto' } }, h('code', {}, `const view = new Int32Array(new SharedArrayBuffer(16));
Atomics.store(view, 0, 10);
...
Atomics.isLockFree(4);                 // true（4 字节原子操作无锁）`)),
    ...
    _createWaitView() {
        ...
        try {
            ...
            Atomics.store(view, 0, 0);
            this.setState({
                waitNotifyInfo: `new SharedArrayBuffer(8) + new Int32Array(sab) → wait/notify 视图 ✓\n` +
    ...
    _waitNotifyDemo() {
        ...
        try {
            ...
            let wokenAll = 0;
            try {
                wokenAll = Atomics.notify(view, 0, Infinity);
            }
            catch (e) {
                wokenAll = `抛错:${e.name}`;
            }
            ...
            this.setState({
                waitNotifyInfo: `Atomics.wait/notify 同线程模拟（主线程只能 timeout=0）：\n` +
                    lines.join('\n') + '\n' +
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: ok ? 'success' : 'error' }, ok ? 'wait/notify ✓' : '不可用'), h(Tag, { color: 'primary' }, '线程同步原语')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'Atomics.wait(typedArray, index, value, timeout) 阻塞当前线程：若内存值 ≠ value 立即返回 "not-equal"；超时返回 "timed-out"；被 notify 唤醒返回 "ok"。Atomics.notify(typedArray, index, count) 唤醒在指定位置 wait 的 agent，返回实际唤醒数。注意：主线程只能用 timeout=0（否则抛 TypeError 阻塞 UI），真正阻塞等待需在 Worker 中调用。这是基于共享内存的线程同步原语。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('创建视图', { type: 'primary', size: 'sm', disabled: !ok, onClick: () => this._createWaitView() }), this._btn('wait/notify 演示', { type: 'primary', size: 'sm', disabled: !ok, onClick: () => this._waitNotifyDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, 'wait/notify 结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '220px', overflow: 'auto' } }, h('code', {}, s.waitNotifyInfo || '（点击「创建视图」开始）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '140px', overflow: 'auto' } }, h('code', {}, `// Worker 中（可阻塞）
...
// 主线程 Atomics.wait 只能用 timeout=0，否则抛 TypeError`)),
    ...
    _createResizableAB() {
        ...
        try {
            ...
            this.setState({
                resizableInfo: `new ArrayBuffer(8, { maxByteLength: 32 }) → 可调整大小 ArrayBuffer ✓\n` +
                    `(ab as any).byteLength = ${ab.byteLength}，ab.maxByteLength = ${ab.maxByteLength}，ab.resizable = ${ab.resizable}\n` +
    ...
    _growResizeAB() {
        ...
        try {
            ...
            if (typeof ab.resize === 'function') {
                ...
                lines.push('ab.resize 不可用（部分实现只有 grow）');
            }
            this.setState({
                resizableInfo: `ab.grow / ab.resize 调整大小：\n` +
                    lines.join('\n') + '\n' +
    ...
    _transferAB() {
        ...
        try {
            ...
            if (typeof transferred.transferToFixedLength === 'function') {
                try {
                    tflResizable = transferred.transferToFixedLength().resizable;
                }
                catch (e) {
                    tflResizable = `抛错:${e.name}`;
                }
            }
            this.setState({
                resizableInfo: `ab.transfer() / transferToFixedLength() + detached：\n` +
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.resizable ? 'success' : 'error' }, caps.resizable ? 'resizable ✓' : '不可用'), h(Tag, { color: caps.transfer ? 'success' : 'warning' }, caps.transfer ? 'transfer ✓' : 'transfer ✗'), h(Tag, { color: 'primary' }, 'grow/resize/transfer/detached')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'ES2024 起 new ArrayBuffer(byteLength, { maxByteLength }) 创建可调整大小的缓冲区；ab.resizable（只读布尔）判断是否可调整；ab.maxByteLength（只读）最大长度；ab.grow(newLength) 只能增大；ab.resize(newLength) 可增可缩；ab.transfer() 零拷贝转移所有权（原 buffer detached）；ab.transferToFixedLength() 转移为固定长度；ab.detached（只读布尔）判断是否已分离。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('创建可调整 AB', { type: 'primary', size: 'sm', disabled: !caps.resizable, onClick: () => this._createResizableAB() }), this._btn('grow/resize', { type: 'primary', size: 'sm', disabled: !caps.resizable, onClick: () => this._growResizeAB() }), this._btn('transfer/detached', { type: 'primary', size: 'sm', disabled: !caps.transfer, onClick: () => this._transferAB() })),
                h('div', { class: 'fs-sm text-secondary' }, '可调整 ArrayBuffer 状态：'),
                h('pre', { class: 'code-block', style: { maxHeight: '240px', overflow: 'auto' } }, h('code', {}, s.resizableInfo || '（点击「创建可调整 AB」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
...
ab.transferToFixedLength();   // 转移为固定长度 buffer`)),
    ...
    _createWeakRef() {
        ...
        try {
            ...
            this.setState({
                weakRefInfo: `new WeakRef(target) → 弱引用 ✓\n` +
    ...
    _createFinalizationRegistry() {
        ...
        try {
            ...
            this.setState({
                weakRefInfo: `new FinalizationRegistry(callback) → 注册表 ✓\n` +
    ...
    _tryTriggerGC() {
        ...
        try {
            ...
            if (hadGc) {
                globalThis.gc();
                gcLine = '(globalThis as any).gc() 已调用（--expose-gc 启用）\n';
            }
            else {
                for (let i = 0; i < 100000; i++) {
                    const _ = { x: i, y: i * 2, arr: new Array(10).fill(i) };
                }
                gcLine = 'globalThis.gc 不可用，已分配 10 万临时对象施压，GC 时机由引擎决定\n';
            }
            const after = this._weakRef.deref();
            this.setState({
                weakRefInfo: `尝试触发 GC 并观察 WeakRef.deref()：\n` +
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.weakRef ? 'success' : 'error' }, caps.weakRef ? 'WeakRef ✓' : '不可用'), h(Tag, { color: caps.finalization ? 'success' : 'error' }, caps.finalization ? 'FinalizationRegistry ✓' : '不可用'), h(Tag, { color: 'primary' }, '弱引用 / GC 回调')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'new WeakRef(target) 创建弱引用，不阻止 GC 回收 target；weakRef.deref() 返回 target（未 GC）或 undefined（已 GC）。new FinalizationRegistry(callback) 注册对象被 GC 回收时的回调；registry.register(target, heldValue, unregisterToken) 注册；registry.unregister(unregisterToken) 注销。注意：GC 时机不确定，回调可能不触发或延迟，不能依赖它做关键资源清理。典型用途：缓存、可选监听器、资源清理辅助。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('创建 WeakRef', { type: 'primary', size: 'sm', disabled: !caps.weakRef, onClick: () => this._createWeakRef() }), this._btn('FinalizationRegistry', { type: 'primary', size: 'sm', disabled: !caps.finalization, onClick: () => this._createFinalizationRegistry() }), this._btn('尝试触发 GC', { size: 'sm', disabled: !caps.weakRef, onClick: () => this._tryTriggerGC() })),
                h('div', { class: 'fs-sm text-secondary' }, 'WeakRef / FinalizationRegistry 状态：'),
                h('pre', { class: 'code-block', style: { maxHeight: '240px', overflow: 'auto' } }, h('code', {}, s.weakRefInfo || '（点击「创建 WeakRef」开始）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '160px', overflow: 'auto' } }, h('code', {}, `// WeakRef：弱引用对象
let obj = { data: 42 };
...
// 注意：GC 时机不确定，回调可能不触发！`)),
    ...
    _demoTransfer() {
        ...
        try {
            ...
            let afterByteLength;
            try {
                afterByteLength = ab.byteLength;
            }
            catch (e) {
                afterByteLength = `抛错：${e.name}（detached 后访问抛 TypeError）`;
            }
            const clonedData = cloned.data;
            this.setState({
                transferInfo: `structuredClone + transfer 演示：\n` +
                    `原 ab: byteLength=${beforeByteLength}，detached=${beforeDetached}，view[0,1]=[${view[0]}, ${view[1]}]\n` +
    ...
    _listTransferable() {
        try {
            ...
            this.setState({
                transferInfo: `可转移对象（Transferable Objects）类型检测：\n${lines.join('\n')}\n\n` +
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.structuredClone ? 'success' : 'error' }, caps.structuredClone ? 'structuredClone ✓' : '不可用'), h(Tag, { color: caps.transfer ? 'success' : 'warning' }, caps.transfer ? 'transfer ✓' : 'transfer ✗'), h(Tag, { color: 'primary' }, '零拷贝所有权移动')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'postMessage(message, transfer) 的 transfer 数组接受可转移对象（ArrayBuffer / MessagePort / ImageBitmap / OffscreenCanvas / ReadableStream / WritableStream / TransformStream / AudioData / VideoFrame）；structuredClone(value, { transfer: [ab] }) 同样支持转移。转移后原对象 detached 无法再访问；与深拷贝区别：拷贝复制数据，转移零拷贝移动所有权。Worker 间传递大数据用 transfer 避免拷贝开销；SharedArrayBuffer 是共享不转移。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('structuredClone+transfer', { type: 'primary', size: 'sm', disabled: !caps.structuredClone, onClick: () => this._demoTransfer() }), this._btn('可转移类型检测', { size: 'sm', onClick: () => this._listTransferable() })),
                h('div', { class: 'fs-sm text-secondary' }, '可转移对象演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, s.transferInfo || '（点击「structuredClone+transfer」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '140px', overflow: 'auto' } }, h('code', {}, `// postMessage 转移（Worker 间）
...
worker.postMessage(sab, [sab]);  // 不会让原 sab detached`)),
    ...
    _renderCard9() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.sharedStruct ? 'success' : 'error' }, caps.sharedStruct ? 'SharedStruct ✓' : 'SharedStruct ✗'), h(Tag, { color: 'primary' }, 'TC39 Stage 3')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'Shared Structs（TC39 Stage 3 提案）提供跨 Worker 共享的可变结构体，是对象级共享（new SharedStruct() / SharedArray / SharedMap），不同于 SharedArrayBuffer 的字节级共享。字段直接读写，引擎管理同步，Worker 间修改立即可见无需序列化。与 SharedArrayBuffer 对比：SAB 更底层需 Atomics，SharedStruct 更易用更结构化。Chrome behind flag 实验，Firefox/Safari 未实现。降级用 SharedArrayBuffer+Atomics 或 postMessage 拷贝。jsdom 无，演示仅展示文档。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('展示 Shared Structs 文档与代码', { type: 'primary', size: 'sm', onClick: () => this._runSharedStructsDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, 'Shared Structs 文档与示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '520px', overflow: 'auto' } }, h('code', {}, s.sharedStructsInfo || '（点击按钮查看 Shared Structs 完整文档与 Worker 共享状态示例）')),
                h(Alert, {
                    type: 'info',
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'page api-lab-page' }, h('h2', { class: 'section-title' }, '共享内存与原子操作实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, 'SharedArrayBuffer 提供跨 Worker 共享的固定长度内存；Atomics 提供原子操作与 wait/notify 同步原语；ES2024 ArrayBuffer 支持 resizable/transfer；WeakRef + FinalizationRegistry 提供弱引用与 GC 回调；可转移对象实现零拷贝所有权移动。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderCard6(), this._renderCard9(), this._renderLogPanel());
    }
}
//# sourceMappingURL=SharedMemoryAtomicsPage.js.map