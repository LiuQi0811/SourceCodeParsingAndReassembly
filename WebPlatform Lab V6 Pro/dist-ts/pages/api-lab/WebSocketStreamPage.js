...
export class WebSocketStreamPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!f.webSocket)
            this._addLog('warn', 'WebSocket 不可用（jsdom 默认无，仅作对照组展示）');
        if (!f.readableStream)
            this._addLog('warn', 'ReadableStream 不可用（BYOB reader 演示跳过）');
        if (!f.writableStream)
            this._addLog('warn', 'WritableStream 不可用（writer.ready 反压演示跳过）');
        if (!f.abortController)
            this._addLog('warn', 'AbortController 不可用（signal 中止演示跳过）');
    ...
    _stopSim() {
        if (this._sim) {
            ...
            try {
                clearTimeout(this._sim.producerTimer);
            }
            catch { /* noop */ }
            try {
                clearTimeout(this._sim.consumerTimer);
            }
            catch { /* noop */ }
            try {
                clearTimeout(this._sim.reportTimer);
            }
            catch { /* noop */ }
            this._sim = null;
        }
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '传统 WebSocket 无背压：send() 同步返回不感知对端消费速率，仅能轮询 bufferedAmount。WebSocketStream（Chrome 实验 origin trial）将 WebSocket 包装为 readable/writable 双向流，通过 writer.ready / reader.read() Promise 天然反压，并支持 AbortSignal 与 Promise 化关闭握手。浏览器支持：仅 Chrome/Edge 实验性，Firefox/Safari/Node/jsdom 均无，生产需 Polyfill。'),
                ...
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// 传统 WebSocket：无背压
const ws = new WebSocket('wss://example.com/stream');
ws.onmessage = (e: any) => handle(e.data);
...
while (true) {
  ...
  handle(value); // value: Uint8Array
                ...
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.overviewInfo || '（点击按钮查看 WebSocketStream 概述与动机完整说明）')),
            ],
        });
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'new WebSocketStream(url, { protocols, signal }) 构造，三大 Promise 属性：opened（解析 { readable, writable, protocol, extensions }）、closed（解析 { closeCode, reason }）、ready。与 new WebSocket() 的关键差异：无 readyState 数字属性，状态由 Promise 表达；构造后必须 await wss.opened 才能拿到 readable/writable。protocols 协商失败时 opened reject。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行构造演示', { type: 'primary', size: 'sm', onClick: () => this._runConnDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `const wss = new WebSocketStream('wss://example.com/stream', {
  protocols: ['chat'],
...
console.log('关闭 code=%d reason=%s', closeCode, reason);`)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.connInfo || '（点击按钮查看 WebSocketStream 构造与连接完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    `);
        ...
        const info = [
            ...
            '    console.log("收到", (value as any).byteLength, "字节");',
            '  }',
            '',
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '从 wss.opened 拿到 readable: ReadableStream<Uint8Array>。getReader() 默认 reader，循环 await reader.read() 拿 { value, done }；value 是 Uint8Array（一帧消息边界保留），文本需 TextDecoder 解码。getReader({ mode: "byob" }) 拿 BYOB reader，read(new Uint8Array(buffer)) 零拷贝读取到调用方 buffer。支持 tee() 分流、pipeTo/pipeThrough 与 Streams 生态互通。reader.read() 在下游慢时挂起 → TCP 接收窗口收缩 → 对端阻塞，全链路背压。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行读取演示', { type: 'primary', size: 'sm', onClick: () => this._runReadDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `const { readable } = await wss.opened;
const decoder = new TextDecoder('utf-8');
...
await readable.pipeThrough(through).pipeTo(logSink);`)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.readInfo || '（点击按钮查看 readable 流读取完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    `);
        ...
        const info = [
            ...
            '  ByteLengthQueuingStrategy({ highWaterMark })  按 (chunk as any).byteLength 计',
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'writable.getWriter() 拿 writer，await writer.ready 感知下游消费速率（背压核心），await writer.write(chunk) 写入。highWaterMark + ByteLengthQueuingStrategy 决定 desiredSize 何时 ≤0 触发背压（writer.ready 保持 pending，生产者自然挂起）。对比传统 ws.send 仅能轮询 bufferedAmount。下方纯 JS 模拟直观演示生产者全速→队列达 highWaterMark→降速→消费追上→恢复全速的背压闭环。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行写入与背压演示', { type: 'primary', size: 'sm', onClick: () => this._runWriteDemo() }), s.simRunning
                ...
                h('div', { class: 'wss-sim' }, h('div', {}, s.simStats || '（点击「运行写入与背压演示」启动纯 JS 背压模拟）'), h('div', { class: 'wss-sim-bar-wrap' }, h('div', { class: 'wss-sim-bar', style: { width: `${pct}%`, background: bpActive ? '#ef4444' : 'linear-gradient(90deg, #22c55e, #eab308)' } }), h('div', { class: 'wss-sim-threshold', style: { left: `${Math.round((high / (high * 2)) * 100)}%` } })), h('div', { style: { marginTop: '6px', fontSize: '11px', color: '#94a3b8' } }, `队列水位 ${depth} / ${high * 2}（阈值 highWaterMark=${high}，超过则生产者 await writer.ready 降速）`)),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `const { writable } = await wss.opened;
const writer = writable.getWriter();

// —— 背压核心：await writer.ready 感知下游消费速率 ——
async function pump(chunks: any) {
  for (const chunk of chunks) {
...
function legacySend(ws: any,  chunks: any) {
  let i = 0;
  const step = () => {
  ...
  step();
                ...
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.writeInfo || '（点击按钮查看 writable 写入与背压完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    `);
        ...
        const info = [
            ...
            '  console.log((value as any).byteLength, value.buffer);',
            '',
            '  优势：',
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '传统 WebSocket 需手动设 binaryType:"arraybuffer" 否则拿 Blob；WebSocketStream 简化为恒 Uint8Array，文本帧也需 TextDecoder 解码。permessage-deflate（RFC 7692）通过 Sec-WebSocket-Extensions 头协商消息压缩，opened 解析的 extensions 字符串可查询协商结果。文本/JSON 开启压缩收益大，已压缩二进制（Protobuf/原始行情）应关闭避免双压缩。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行协议扩展演示', { type: 'primary', size: 'sm', onClick: () => this._runBinaryDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// 传统 WebSocket：必须设 binaryType
const ws = new WebSocket('wss://example.com/s');
ws.binaryType = 'arraybuffer'; // 否则拿到 Blob
ws.onmessage = (e: any) => { /* e.data: ArrayBuffer */ };
...
// Sec-WebSocket-Protocol: chat`)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.binaryInfo || '（点击按钮查看二进制类型与 permessage-deflate 完整说明）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'new WebSocketStream(url, { signal: ac.signal }) 传入 AbortController 信号，任意时刻 ac.abort(reason) 中止（握手阶段 opened reject AbortError，已连接后 TCP 立即关闭不发 close 帧）。正常关闭用 wss.close(1000, reason) 发起关闭握手，closed Promise 解析 { closeCode, reason }（字段名 closeCode，部分文档记作 code）。与 fetch AbortSignal 语义一致，可共享同一 AbortController。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行中止与关闭演示', { type: 'primary', size: 'sm', onClick: () => this._runAbortDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `const ac = new AbortController();
...
console.log('关闭 code=%d reason=%s', closeCode, reason);`)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.abortInfo || '（点击按钮查看 AbortSignal 中止与关闭握手完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard7() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '服务端高频二进制行情 tick（1000~10000 帧/秒），reader.read() 天然背压：渲染慢则挂起 → TCP 接收窗口收缩 → 服务端自动降速。慢客户端检测：单帧处理耗时 >5ms 累计触发采样降级（rAF 只渲染最新帧）。重连策略：指数退避 + 抖动防雪崩。心跳保活防 TCP 半开慢死。已压缩二进制（Protobuf）应关闭 permessage-deflate 避免双压缩。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行行情实战演示', { type: 'primary', size: 'sm', onClick: () => this._runMarketDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// 背压自适应消费行情 tick
const { readable } = await wss.opened;
const reader = readable.getReader();
while (true) {
  const { value, done } = await reader.read(); // ← 背压点
...
async function reconnect() {
  ...
  setTimeout(() => this.connect(), delay);
                ...
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.marketInfo || '（点击按钮查看低延迟行情推送实战完整方案）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    `);
        const info = [
            ...
            '    const opened = new Promise((res: any, rej: any) => { resolveOpened = res; rejectOpened = rej; });',
            '',
            '    let resolveClosed;',
            '    const closed = new Promise((res: any) => { resolveClosed = res; });',
            '',
            ...
            '    ws.addEventListener($1, (e: any) => resolveClosed({ closeCode: e.code, reason: e.reason }));',
            '',
            ...
            '      start(c) {',
            '        controller = c;',
            '        ws.addEventListener($1, (e: any) => c.enqueue(new Uint8Array(e.data)));',
            ...
            '    }',
            '',
            '    return { opened, closed, close: (code: any, reason: any) => ws.close(code, reason) };',
            '  }',
            '',
    ...
    _renderCard8() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'primary' }, '决策'), h(Tag, { color: f.webSocketStream ? 'success' : 'error' }, `WebSocketStream ${f.webSocketStream ? '✓' : '✗'}`)),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '6 维对比：背压 / 双向 / 可靠 / 低延迟 / HTTP3 多路复用 / 二进制。WebSocketStream = WebSocket 背压增强版（仍 TCP/HTTP1.1 升级）；WebTransport = QUIC 原生多路 + 0-RTT + 数据报不可靠；SSE = 单向文本推送。选型决策树 + 降级链（WebSocketStream → WebSocket → SSE）。Polyfill 思路：WebSocket + ReadableStream/WritableStream + bufferedAmount 轮询模拟 writer.ready 背压，用法与原生一致。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行决策矩阵演示', { type: 'primary', size: 'sm', onClick: () => this._runDecisionDemo() })),
                // 6 维对比速查
                h('div', { class: 'wss-matrix' }, h('div', { class: 'wss-matrix-cell' }, h('div', { class: 'wss-matrix-title' }, 'WebSocket'), h('div', {}, '背压 ✗ · 双向 ✓ · 可靠 ✓ · 低延迟 △ · H3 ✗ · 二进制 ✓')), h('div', { class: 'wss-matrix-cell' }, h('div', { class: 'wss-matrix-title' }, 'WebSocketStream'), h('div', {}, '背压 ✓ · 双向 ✓ · 可靠 ✓ · 低延迟 △ · H3 ✗ · 二进制 ✓')), h('div', { class: 'wss-matrix-cell' }, h('div', { class: 'wss-matrix-title' }, 'WebTransport'), h('div', {}, '背压 ✓ · 双向 ✓ · 可靠 △ · 低延迟 ✓ · H3 ✓ · 二进制 ✓')), h('div', { class: 'wss-matrix-cell' }, h('div', { class: 'wss-matrix-title' }, 'SSE'), h('div', {}, '背压 ✗ · 双向 ✗ · 可靠 ✓ · 低延迟 △ · H3 ✗ · 二进制 ✗'))),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// Polyfill：WebSocket + Streams 包装出 WebSocketStream 接口
function createWebSocketStreamPolyfill(url: any, { protocols, signal } : any= {}) {
  const ws = new WebSocket(url, protocols);
  ws.binaryType = 'arraybuffer';
  const opened = new Promise((res: any, rej: any) => {
  ...
  const closed = new Promise((res: any) =>
    ws.addEventListener($1, (e: any) => res({ closeCode: e.code, reason: e.reason })));
  ...
  return { opened, closed, close: (c: any, r: any) => ws.close(c, r) };
}
...
const { readable, writable } = await wss.opened;`)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.decisionInfo || '（点击按钮查看决策矩阵 + 降级方案 + Polyfill 完整说明）')),
            ],
        });
        this.registerChild(card);
        return card.render();
...
//# sourceMappingURL=WebSocketStreamPage.js.map