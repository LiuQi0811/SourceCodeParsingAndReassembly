...
export class CSSTransitionsAnimationsPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!f.linearFn)
            this._addLog('warn', 'linear() 不可用（Chrome 124+ / CSS Easing Level 2）');
        if (f.prefersReducedMotion)
            this._addLog('warn', '用户偏好减少动态效果（prefers-reduced-motion: reduce）');
        if (!f.viewTransitions)
            this._addLog('info', 'View Transitions API 不可用（Chrome 111+）');
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['animation', f.animation]]), h(Tag, { color: 'primary' }, '关键帧')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '@keyframes name { from { } to { } } 或 0%/25%/50%/75%/100% 多关键帧定义动画蓝图。!important 在关键帧中被忽略（规范规定）；同名 @keyframes 后者覆盖前者；关键帧中可使用 var() CSS 变量与 calc() 表达式；隐式 from/to 自动取元素当前值。动态修改可用 CSSStyleSheet.insertRule。@keyframes 比 transition 更高效（预编译）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 @keyframes 演示', { type: 'primary', size: 'sm', onClick: () => this._runKeyframesDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.keyframesInfo || '（点击按钮查看 @keyframes 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['cubic-bezier()', f.cubicBezier]]), h(Tag, { color: 'primary' }, 'Easing L1')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'cubic-bezier(x1, y1, x2, y2) 定义三次贝塞尔曲线，起点 (0,0) 终点 (1,1)，两个控制点决定曲线形状。x1/x2 必须 [0,1]，y1/y2 可超出 [0,1] 实现回弹/弹性效果（如 cubic-bezier(0.34, 1.56, 0.64, 1)）。预设等价：ease = cubic-bezier(0.25, 0.1, 0.25, 1) 等。UI 反馈用 ease-out，进入用 ease-in，退出用 ease-in-out。工具：cubic-bezier.com / easings.net。'),
                h('div', { class: 'ta-easing-grid' }, h('div', { class: 'ta-easing-cell' }, [h('div', {}, 'ease'), h('div', { class: 'ta-easing-bar', style: { animationTimingFunction: 'ease' } })]), h('div', { class: 'ta-easing-cell' }, [h('div', {}, 'linear'), h('div', { class: 'ta-easing-bar', style: { animationTimingFunction: 'linear' } })]), h('div', { class: 'ta-easing-cell' }, [h('div', {}, 'ease-in'), h('div', { class: 'ta-easing-bar', style: { animationTimingFunction: 'ease-in' } })]), h('div', { class: 'ta-easing-cell' }, [h('div', {}, 'ease-out'), h('div', { class: 'ta-easing-bar', style: { animationTimingFunction: 'ease-out' } })]), h('div', { class: 'ta-easing-cell' }, [h('div', {}, 'ease-in-out'), h('div', { class: 'ta-easing-bar', style: { animationTimingFunction: 'ease-in-out' } })]), h('div', { class: 'ta-easing-cell' }, [h('div', {}, 'back'), h('div', { class: 'ta-easing-bar', style: { animationTimingFunction: 'cubic-bezier(0.34, 1.56, 0.64, 1)' } })])),
                h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('运行 cubic-bezier 演示', { type: 'primary', size: 'sm', onClick: () => this._runCubicBezierDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.cubicBezierInfo || '（点击按钮查看 cubic-bezier() 完整深潜）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'steps(n, start|end) 分 n 步跳跃：steps(4, end) 每步末尾跳，steps(4, start) 每步开头跳，step-start = steps(1, start)。应用：精灵图逐帧动画、打字机效果、进度条分段。linear() 函数（Chrome 124+ / CSS Easing Level 2）：linear(0, 0.25, 1) 自定义多点线性插值，配合百分比 linear(0 0%, 0.5 50%, 1 100%) 显式控制每段时间。比 cubic-bezier 更直观可控但不能回弹。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 steps/linear 演示', { type: 'primary', size: 'sm', onClick: () => this._runStepsDemo() })),
                ...
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.stepsInfo || '（点击按钮查看 steps() / linear() 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['animation 事件', f.animation]]), h(Tag, { color: 'primary' }, 'JS API')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'Transition 事件：transitionrun/transitionstart/transitioncancel/transitionend，事件属性 propertyName/elapsedTime/pseudoElement。Animation 事件：animationstart/animationiteration/animationcancel/animationend，animationiteration 在每次循环时触发（除最后一次）。element.getAnimations() 获取所有 Animation 对象，可 play()/pause()/cancel()/reverse()/finish()。替代方案：Web Animations API element.animate(keyframes, options)，返回 Animation 对象并支持 .finished Promise。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行动画事件演示', { type: 'primary', size: 'sm', onClick: () => this._runEventsDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.eventsInfo || '（点击按钮查看动画事件与 JS 控制完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard7() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '仅过渡 transform/opacity 仅触发 composite（不触发 layout/paint）。will-change: transform 提示浏览器预创建合成层。contain: layout/paint 限制重排范围。避免过渡 width/height/top/left（触发 layout）。@keyframes 比 transition 更高效（预编译）。animation-play-state: paused 暂停时节省资源。IntersectionObserver 离屏暂停动画。prefers-reduced-motion 尊重用户减少动态偏好。60fps 预算：每帧 16.67ms。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行性能优化演示', { type: 'primary', size: 'sm', onClick: () => this._runPerfDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.perfInfo || '（点击按钮查看性能优化与最佳实践完整指南）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard8() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'primary' }, '实战'), h(Tag, { color: f.viewTransitions ? 'success' : 'error' }, `View Transitions ${f.viewTransitions ? '✓' : '✗'}`)),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '14 种实战模式：hover 反馈（scale + ease-out）/ 卡片淡入（opacity + translateY + 错峰延迟）/ 加载旋转（rotate + infinite）/ 脉冲呼吸（scale + alternate）/ 摆动（rotate + alternate）/ 滑入菜单（transform + transition）/ 进度条（width + transition 或 animation）/ 打字机（steps + animation）/ 精灵图逐帧（steps + background-position）/ 模态框淡入淡出（@starting-style + transition-behavior: allow-discrete）/ 视图过渡（View Transitions API）/ 导航栏指示器 / 列表项交错入场 / 滚动视差。'),
                ...
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.patternInfo || '（点击按钮查看 14 种实战动画模式完整代码）')),
            ],
        });
        this.registerChild(card);
        return card.render();
...
//# sourceMappingURL=CSSTransitionsAnimationsPage.js.map