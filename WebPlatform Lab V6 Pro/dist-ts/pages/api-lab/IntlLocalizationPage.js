...
export class IntlLocalizationPage extends Page {
    ...
    initialState() {
        return {
            ...
            sanitizeOutput: '',
            sanitizeRaw: '',
            sanitizerSupported: typeof globalThis.Sanitizer !== 'undefined',
            urlPattern: '/users/:id(\\d+)/posts/:slug',
            urlTest: '/users/42/posts/hello-world',
            urlPatternSupported: typeof globalThis.URLPattern !== 'undefined',
            urlPatternResult: '',
            messageFormatInfo: '',
            messageFormatResult: '',
        };
    ...
    _flags() {
        return {
            ...
            formatMethod: this._safe(() => typeof new window.Intl.MessageFormat('Hello {name}', 'en').format === 'function'),
        };
    }
    ...
    _runDateTime() {
        try {
            ...
            try {
                ...
                rangeStr = `formatRange 不支持：${e.message}`;
            }
    ...
    _runNumberFormat() {
        try {
            ...
            let opts = {};
            if (numStyle === 'currency')
                opts = { style: 'currency', currency: numLocale.startsWith('zh') ? 'CNY' : numLocale.startsWith('ja') ? 'JPY' : numLocale.startsWith('ru') ? 'RUB' : numLocale.startsWith('ko') ? 'KRW' : 'USD' };
            else if (numStyle === 'percent')
                opts = { style: 'percent' };
            else if (numStyle === 'unit')
                opts = { style: 'unit', unit: 'kilometer-per-hour' };
            else if (numStyle === 'compact')
                opts = { notation: 'compact', compactDisplay: 'short' };
            else if (numStyle === 'scientific')
            ...
            for (const type of types) {
                try {
                    ...
                    if (type === 'region')
                        ...
                        samples = ['USD', 'CNY', 'JPY', 'EUR'];
                    else if (type === 'script')
                        samples = ['Latn', 'Hans', 'Hant', 'Cyrl'];
                    else if (type === 'calendar')
                        samples = ['gregory', 'chinese', 'islamic', 'hebrew'];
                    else
                        samples = ['year', 'month', 'day', 'hour'];
                    const row = samples.map((s) => `${s}=${dn.of(s)}`).join(', ');
                    lines.push(`【${type}】${row}`);
                }
                catch (e) {
                    lines.push(`【${type}】不支持：${e.message}`);
                }
            }
    ...
    _runLocale() {
        try {
            ...
            try {
                lines.push(`textInfo:       ${JSON.stringify(loc.textInfo)}`);
            }
            catch {
                lines.push('textInfo:       不支持');
            }
            try {
                lines.push(`weekInfo:       ${JSON.stringify(loc.weekInfo)}`);
            }
            catch {
                lines.push('weekInfo:       不支持');
            }
            try {
                lines.push(`calendars:      ${JSON.stringify(loc.calendars)}`);
            }
            catch {
                lines.push('calendars:      不支持');
            }
    ...
    _runUrlPattern() {
        ...
        try {
            const pattern = new globalThis.URLPattern({ pathname: urlPattern });
    ...
    _runMessageFormatDemo() {
        ...
        try {
            const lines = [];
            const fmt = (mf, values) => {
                if (typeof mf.format === 'function')
                    return mf.format(values);
                return JSON.stringify(mf.formatToParts(values));
            };
            const mf1 = new window.Intl.MessageFormat('Hello, {name}!', 'en');
            lines.push('【1. 简单变量 {name}】');
            lines.push('  模板: Hello, {name}!');
            lines.push(`  → ${fmt(mf1, { name: 'Alice' })}`);
            const mf2 = new window.Intl.MessageFormat('You have {count, plural, one {# item} other {# items}}.', 'en');
            lines.push('\n【2. 复数 plural】');
            lines.push('  模板: You have {count, plural, one {# item} other {# items}}.');
            [0, 1, 5].forEach((n) => {
                try {
                    lines.push(`  count=${n}: ${fmt(mf2, { count: n })}`);
                }
                catch (e) {
                    lines.push(`  count=${n}: 错误 ${e.message}`);
                }
            });
            const mf3 = new window.Intl.MessageFormat('{pos, selectordinal, one {#st} two {#nd} few {#rd} other {#th}}', 'en');
            lines.push('\n【3. 序数 selectordinal】');
            lines.push('  模板: {pos, selectordinal, one {#st} two {#nd} few {#rd} other {#th}}');
            [1, 2, 3, 4, 11].forEach((n) => {
                try {
                    lines.push(`  pos=${n}: ${fmt(mf3, { pos: n })}`);
                }
                catch (e) {
                    lines.push(`  pos=${n}: 错误 ${e.message}`);
                }
            });
            const mf4 = new window.Intl.MessageFormat('{gender, select, male {he} female {she} other {they}}', 'en');
            lines.push('\n【4. 选择 select】');
            lines.push('  模板: {gender, select, male {he} female {she} other {they}}');
            ['male', 'female', 'unknown'].forEach((g) => {
                try {
                    lines.push(`  gender=${g}: ${fmt(mf4, { gender: g })}`);
                }
                catch (e) {
                    lines.push(`  gender=${g}: 错误 ${e.message}`);
                }
            });
            try {
                const mf5 = new window.Intl.MessageFormat('Price: {price, number, ::currency/USD}', 'en');
                lines.push('\n【5. 函数 ::currency/USD】');
            ...
            try {
                const mf6 = new window.Intl.MessageFormat('Today is {date, datetime, ::yMd}', 'en');
                lines.push('\n【6. 函数 ::yMd (datetime)】');
    ...
    _renderCard9() {
        ...
        return h(Card, {
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'primary' }, 'MessageFormat'), h(Tag, { color: 'success' }, 'MF2'), h(Tag, { color: 'warning' }, 'Stage 3')),
        }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'Intl.MessageFormat 是 ICU MessageFormat 2（MF2）的 JS 原生实现，ECMA-402 Stage 3 提案，Chrome 124+ 起支持。与 ListFormat/PluralRules 等「单一功能」API 不同，MessageFormat 是「结构化消息」：一条消息内组合变量插值 / 复数 / 选择 / 函数调用，是 i18n 消息系统的基石。'), h('div', { class: 'flex gap-xs flex-wrap' }, ...this._caps([
            ...
            : null, h('pre', { class: 'code-block' }, this.state.messageFormatInfo || '—'), h('div', { class: 'fs-sm text-secondary mt-sm' }, 'MF2 消息语法：'), h('pre', { class: 'code-block' }, [
        ...
        ].join('\n')), h('div', { class: 'fs-sm text-secondary mt-sm' }, 'MF2 vs MessageFormat 1：'), h('pre', { class: 'code-block' }, [
        ...
        ].join('\n')), h('div', { class: 'fs-sm text-secondary mt-sm' }, 'i18n 消息系统实战：'), h('pre', { class: 'code-block' }, [
        ...
        ].join('\n')), h('div', { class: 'demo-row' }, this._btn('运行 MF2 演示', { type: 'primary', size: 'sm', onClick: () => this._runMessageFormatDemo() })), h('div', { class: 'fs-sm text-secondary' }, '运行结果：'), h('pre', { class: 'code-block', style: { maxHeight: '320px' } }, this.state.messageFormatResult || '（点击「运行 MF2 演示」）')));
    }
    renderPage() {
        return [
            h('h2', { class: 'section-title' }, '国际化 / 本地化 / 字符串 API 实验室'),
            h('p', { class: 'text-secondary mb-lg' }, '演示 Intl 全家桶（DateTimeFormat / NumberFormat / PluralRules / ListFormat / RelativeTimeFormat / DisplayNames / Segmenter / Collator / Locale / MessageFormat MF2）、Sanitizer API、URLPattern API。'),
            h(Card, {
                title: '1. Intl.DateTimeFormat — 日期/时间格式化',
                extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'primary' }, 'DateTimeFormat'), h(Tag, { color: 'success' }, 'formatToParts'), h(Tag, { color: 'warning' }, 'formatRange')),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, '根据 locale 与 options 把 Date 格式化为字符串；formatToParts 暴露每个字段；formatRange 处理区间。'), h('div', { class: 'demo-row' }, h('span', { class: 'demo-row__label' }, 'Locale'), this._localeSelect(this.state.dtLocale, (v) => { this.setState({ dtLocale: v }); })), h('div', { class: 'demo-row' }, h('span', { class: 'demo-row__label' }, '样式'), h('select', {
                class: 'ant-input', style: { width: 'auto', padding: '4px 8px' },
                onChange: (e) => this.setState({ dtStyle: e.target.value }),
            }, h('option', { value: 'full', selected: this.state.dtStyle === 'full' }, 'full（dateStyle+timeStyle）'), h('option', { value: 'long', selected: this.state.dtStyle === 'long' }, 'long（字段化）'), h('option', { value: 'medium', selected: this.state.dtStyle === 'medium' }, 'medium'), h('option', { value: 'short', selected: this.state.dtStyle === 'short' }, 'short（hour12:false）')), this._btn('格式化', { type: 'primary', size: 'sm', onClick: () => this._runDateTime() })), h('div', { class: 'fs-sm text-secondary' }, '结果：'), h('div', { class: 'api-metric', style: { fontSize: '24px' } }, this.state.dtResult || '—'), h('div', { class: 'fs-sm text-secondary mt-sm' }, 'formatToParts：'), h('pre', { class: 'code-block' }, this.state.dtPartsResult || '—'), h('div', { class: 'fs-sm text-secondary mt-sm' }, 'formatRange（now → now+3d）：'), h('pre', { class: 'code-block' }, this.state.dtRangeResult || '—'))),
            h(Card, {
                title: '2. Intl.NumberFormat — 数字 / 货币 / 百分比 / 单位',
                extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'primary' }, 'NumberFormat'), h(Tag, { color: 'success' }, 'compact'), h(Tag, { color: 'warning' }, 'currency')),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, '支持 style: decimal / currency / percent / unit；notation: standard / scientific / compact；signDisplay / useGrouping 等。'), h('div', { class: 'demo-row' }, h('span', { class: 'demo-row__label' }, '数字'), h(Input, {
                value: this.state.numInput,
                style: { width: '180px' },
                onInput: (e) => this.setState({ numInput: e.target.value }),
            })), h('div', { class: 'demo-row' }, h('span', { class: 'demo-row__label' }, 'Locale'), this._localeSelect(this.state.numLocale, (v) => { this.setState({ numLocale: v }); })), h('div', { class: 'demo-row' }, h('span', { class: 'demo-row__label' }, '样式'), h('select', {
                class: 'ant-input', style: { width: 'auto', padding: '4px 8px' },
                onChange: (e) => this.setState({ numStyle: e.target.value }),
            }, ['currency', 'decimal', 'percent', 'unit', 'compact', 'scientific'].map((s) => h('option', { value: s, selected: this.state.numStyle === s }, s))), this._btn('格式化', { type: 'primary', size: 'sm', onClick: () => this._runNumberFormat() })), h('div', { class: 'fs-sm text-secondary' }, '结果：'), h('div', { class: 'api-metric', style: { fontSize: '28px' } }, this.state.numResult || '—'), h('div', { class: 'fs-sm text-secondary mt-sm' }, 'formatToParts：'), h('pre', { class: 'code-block' }, this.state.numPartsResult || '—'))),
            h(Card, {
                title: '3. PluralRules / ListFormat / RelativeTimeFormat / DisplayNames',
                extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'primary' }, 'PluralRules'), h(Tag, { color: 'primary' }, 'ListFormat'), h(Tag, { color: 'primary' }, 'RelativeTimeFormat'), h(Tag, { color: 'primary' }, 'DisplayNames')),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'PluralRules：基数/序数规则（俄语有 few/many 等多形式）；ListFormat：列表连接词；RelativeTimeFormat：相对时间（昨天/明天）；DisplayNames：区域/语言/货币等本地化名称。'), h('div', { class: 'flex gap-sm flex-wrap' }, this._btn('PluralRules', { type: 'primary', size: 'sm', onClick: () => this._runPluralRules() }), this._btn('ListFormat', { type: 'primary', size: 'sm', onClick: () => this._runListFormat() }), this._btn('RelativeTimeFormat', { type: 'primary', size: 'sm', onClick: () => this._runRelativeTime() }), this._btn('DisplayNames', { type: 'primary', size: 'sm', onClick: () => this._runDisplayNames() })), h('div', { class: 'fs-sm text-secondary' }, 'PluralRules 结果：'), h('pre', { class: 'code-block' }, this.state.pluralResult || '（点击 PluralRules）'), h('div', { class: 'fs-sm text-secondary mt-sm' }, 'ListFormat 结果：'), h('pre', { class: 'code-block' }, this.state.listResult || '（点击 ListFormat）'), h('div', { class: 'fs-sm text-secondary mt-sm' }, 'RelativeTimeFormat 结果：'), h('pre', { class: 'code-block' }, this.state.relTimeResult || '（点击 RelativeTimeFormat）'), h('div', { class: 'fs-sm text-secondary mt-sm' }, 'DisplayNames 结果：'), h('pre', { class: 'code-block' }, this.state.displayResult || '（点击 DisplayNames）'))),
            h(Card, {
                title: '4. Intl.Segmenter / Intl.Collator / Intl.Locale',
                extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'success' }, 'Segmenter'), h(Tag, { color: 'success' }, 'Collator'), h(Tag, { color: 'success' }, 'Locale')),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'Segmenter：按 grapheme / word / sentence 切分字符串（中日韩友好）；Collator：本地化字符串比较与排序（numeric/sensitivity/拼音）；Intl.Locale：解析 locale 与 Unicode 扩展。'), h('div', { class: 'demo-row' }, h('span', { class: 'demo-row__label' }, '切分粒度'), h('select', {
                class: 'ant-input', style: { width: 'auto', padding: '4px 8px' },
                onChange: (e) => this.setState({ segGranularity: e.target.value }),
            }, ['word', 'grapheme', 'sentence'].map((g) => h('option', { value: g, selected: this.state.segGranularity === g }, g))), this._btn('切分', { type: 'primary', size: 'sm', onClick: () => this._runSegmenter() }), this._btn('Collator', { type: 'primary', size: 'sm', onClick: () => this._runCollator() }), this._btn('Locale', { type: 'primary', size: 'sm', onClick: () => this._runLocale() })), h('div', { class: 'demo-row' }, h('span', { class: 'demo-row__label' }, '切分输入'), h(Input, {
                value: this.state.segInput,
                style: { flex: '1', minWidth: '240px' },
                onInput: (e) => this.setState({ segInput: e.target.value }),
            })), h('div', { class: 'fs-sm text-secondary' }, 'Segmenter 结果（按行展示每段）：'), h('pre', { class: 'code-block', style: { maxHeight: '200px' } }, this.state.segResult || '—'), h('div', { class: 'fs-sm text-secondary mt-sm' }, 'Collator 结果：'), h('pre', { class: 'code-block' }, this.state.collatorResult || '（点击 Collator）'), h('div', { class: 'fs-sm text-secondary mt-sm' }, 'Intl.Locale 解析结果：'), h('pre', { class: 'code-block' }, this.state.localeResult || '（点击 Locale）'))),
            h(Card, {
                title: '5. Sanitizer API — HTML 净化',
                extra: h(Tag, { color: 'warning' }, 'Sanitizer'),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'Sanitizer API 在浏览器内原生净化 HTML，移除 script/事件处理器/javascript: 等危险内容，仅保留白名单元素与属性。sanitizeFor 返回 Element；Element.setHTML 是另一种用法。'), !this.state.sanitizerSupported
                ? h(Alert, { type: 'warning', message: '当前浏览器不支持 Sanitizer API，下方使用降级正则剥离（仅供演示，不可在生产用）' })
                : null, h('div', { class: 'demo-row' }, h('span', { class: 'demo-row__label' }, '恶意 HTML'), h(Input, {
                value: this.state.sanitizeInput,
                style: { flex: '1', minWidth: '320px' },
                onInput: (e) => this.setState({ sanitizeInput: e.target.value }),
            }), this._btn('净化', { type: 'primary', size: 'sm', onClick: () => this._runSanitize() })), h('div', { class: 'fs-sm text-secondary' }, '原始 HTML（已转义显示）：'), h('pre', { class: 'code-block' }, this.state.sanitizeRaw || '—'), h('div', { class: 'fs-sm text-secondary mt-sm' }, '净化后 HTML（已安全渲染）：'), h('div', {
            ...
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'URLPattern 类似 path-to-regexp，但支持 protocol/host/pathname 全字段；:name 命名组、:name(regex) 正则约束、* 通配；test() 返回布尔，exec() 返回分组详情。'), !this.state.urlPatternSupported
                ? h(Alert, { type: 'warning', message: '当前浏览器不支持 URLPattern API（建议 Chrome 95+ / Edge 95+）' })
                : null, h('div', { class: 'demo-row' }, h('span', { class: 'demo-row__label' }, '模式'), h(Input, {
                value: this.state.urlPattern,
                style: { flex: '1', minWidth: '240px' },
                onInput: (e) => this.setState({ urlPattern: e.target.value }),
            })), h('div', { class: 'demo-row' }, h('span', { class: 'demo-row__label' }, '测试 URL'), h(Input, {
                value: this.state.urlTest,
                style: { flex: '1', minWidth: '240px' },
                onInput: (e) => this.setState({ urlTest: e.target.value }),
            }), this._btn('匹配', { type: 'primary', size: 'sm', onClick: () => this._runUrlPattern() })), h('div', { class: 'fs-sm text-secondary' }, '结果：'), h('pre', { class: 'code-block' }, this.state.urlPatternResult || '—'))),
            this._renderCard9(),
            h(Card, {
                title: '事件日志',
...
//# sourceMappingURL=IntlLocalizationPage.js.map