// =====================================================================
// AdvancedGraphicsPage.ts —— 高级图形与渲染 API 实验室
// 演示 MDN：
...
//   3. CSS Houdini（(CSS as any).paintWorklet.addModule / layoutWorklet / animationWorklet /
...
export class AdvancedGraphicsPage extends Page {
    _inited = false;
    _webgl2Raf = null;
    _webgl2ShaderErr = false;
    _gpuDevice = null;
    _demoStyleEl = null;
    _paintWorkletUrl = null;
    _paintAdded = false;
    _regPropDone = false;
    _savedScrollTop = null;
    _lastLoggedScroll = null;
    initialState() {
        return {
            logs: [],
            // Card 1: WebGL2
    ...
    componentWillUnmount() {
        ...
        if (this._paintWorkletUrl) {
            try {
                URL.revokeObjectURL(this._paintWorkletUrl);
            }
            catch { /* noop */ }
    ...
    _queryWebGL2Exts() {
        try {
            ...
            const gl = canvas.getContext?.('webgl2');
            if (!gl) {
                this._addLog('webgl2', '查询扩展失败：无 WebGL2 上下文');
                return;
            }
    ...
    _renderWebGL2Compare() {
        return h('table', { class: 'gpu-info-table' }, h('thead', {}, h('tr', {}, h('th', {}, '特性'), h('th', {}, 'WebGL1'), h('th', {}, 'WebGL2'))), h('tbody', {}, ...WEBGL_COMPARE.map((row) => h('tr', {}, h('th', {}, row.feature), h('td', {}, row.v1), h('td', {}, row.v2)))));
    }
    // ===================== Card 2: WebGPU =====================
    async _initWebGPU() {
    ...
    async _initHoudini() {
        const cssAny = CSS;
        const paintSupported = typeof CSS !== 'undefined' && typeof cssAny.paintWorklet !== 'undefined';
        const layoutSupported = typeof CSS !== 'undefined' && typeof cssAny.layoutWorklet !== 'undefined';
        const animSupported = typeof CSS !== 'undefined' && typeof cssAny.animationWorklet !== 'undefined';
        ...
        // —— Paint Worklet：(CSS as any).paintWorklet.addModule(Blob URL) ——
    ...
    _restoreScrollTop() {
        ...
        const c = this.$('.scroll-anim-container');
        if (c)
            c.scrollTop = this._savedScrollTop;
    }
    ...
    _renderLogPanel() {
        const s = this.state;
        return h('div', { class: 'log-panel' }, s.logs.length === 0
    ...
    renderPage() {
        ...
        return [
            ...
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'canvas.getContext(\'webgl2\') 获取上下文（能力检测：typeof WebGL2RenderingContext）。WebGL2 内置 VAO（createVertexArray / bindVertexArray）、UBO（getUniformBlockIndex / uniformBlockBinding / bindBufferBase）、Transform Feedback（bindTransformFeedback）、实例化渲染（drawArraysInstanced），并使用 #version 300 es 的 GLSL 3.00 ES（in/out 语法、out fragColor、gl_InstanceID）。'), h('div', { class: 'flex gap-sm flex-wrap' }, this._btn('重绘场景', { type: 'primary', size: 'sm', onClick: () => { this._drawWebGL2(); this._addLog('webgl2', '手动重绘 WebGL2 场景（VAO + UBO + drawArraysInstanced）'); } }), this._btn('查询扩展', { size: 'sm', onClick: () => this._queryWebGL2Exts() })), h('canvas', { class: 'webgl2-canvas', width: 480, height: 200 }), h('div', { class: 'fs-sm text-secondary mt-xs' }, 'GPU 信息（getParameter + WEBGL_debug_renderer_info.UNMASKED_RENDERER_WEBGL）：'), h('pre', { class: 'code-block' }, s.webgl2Info), h('div', { class: 'fs-sm text-secondary mt-xs' }, 'WebGL1 与 WebGL2 特性对比：'), this._renderWebGL2Compare())),
            // ============ Card 2: WebGPU ============
            h(Card, {
                title: '2. WebGPU（navigator.gpu / WGSL / 渲染管线）',
                extra: h(Tag, { color: s.webgpuSupported ? (s.webgpuBufferCreated ? 'success' : 'warning') : 'error' }, s.webgpuSupported ? (s.webgpuBufferCreated ? '已初始化' : '初始化中') : '不支持'),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, '能力检测：\'gpu\' in navigator。流程：navigator.gpu.requestAdapter() → adapter.requestDevice() → device.createBuffer / createShaderModule / createRenderPipeline → createCommandEncoder → queue.submit。着色器语言为 WGSL。jsdom 无 GPU，仅展示调用流程。'), h('div', { class: 'flex gap-sm flex-wrap' }, this._btn('重新初始化', { type: 'primary', size: 'sm', onClick: () => this._initWebGPU() })), h('div', { class: 'fs-sm' }, h('strong', {}, '阶段：'), h('span', { class: 'text-secondary' }, s.webgpuStage)), s.webgpuAdapterInfo ? h('div', { class: 'fs-sm text-secondary' }, `Adapter: ${s.webgpuAdapterInfo}`) : null, s.webgpuDeviceInfo ? h('div', { class: 'fs-sm text-secondary' }, `Device limits: ${s.webgpuDeviceInfo}`) : null, h('div', { class: 'fs-sm text-secondary mt-xs' }, 'WGSL 着色器源码（createShaderModule）：'), h('pre', { class: 'code-block' }, WGSL_SHADER.trim()))),
            // ============ Card 3: CSS Houdini ============
            h(Card, {
            ...
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, '(CSS as any).paintWorklet.addModule(url) 注册 Paint Worklet（此处用 Blob URL 内联 worklet 代码），CSS.registerProperty 注册自定义属性（Properties & Values API），元素背景使用 paint(worklet-name) 调用。能力检测：typeof (CSS as any).paintWorklet。layoutWorklet / animationWorklet 同理。'), h('div', { class: 'flex gap-sm flex-wrap' }, this._btn('重新注册', { type: 'primary', size: 'sm', onClick: () => this._initHoudini() })), h('div', { class: 'flex gap-sm' }, h('div', { class: 'houdini-paint-demo flex-1' }), h('div', { class: 'houdini-paint-demo houdini-paint-demo--alt flex-1' })), h('div', { class: 'fs-sm text-secondary' }, `paintWorklet: ${s.paintWorkletSupported ? '✓' : '✗'} | registerProperty: ${s.registerPropertySupported ? '✓' : '✗'} | 已 addModule: ${s.paintWorkletAdded ? '✓' : '✗'} | 已 registerProperty: ${s.registerPropertyDone ? '✓' : '✗'}`), h('div', { class: 'fs-sm text-secondary mt-xs' }, 'Paint Worklet 源码（通过 Blob URL 内联，addModule 加载）：'), h('pre', { class: 'code-block' }, PAINT_WORKLET_SRC.trim()))),
            // ============ Card 4: WebCodecs API ============
            h(Card, {
            ...
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, '能力检测：typeof VideoDecoder。VideoDecoder/VideoEncoder/AudioDecoder/AudioEncoder 通过 isConfigSupported({ codec }) 查询编解码器支持。EncodedVideoChunk / EncodedAudioChunk / VideoFrame / AudioData 为数据载体。本演示不实际解码（需真实视频数据），仅展示 API 调用流程。'), h('div', { class: 'flex gap-sm flex-wrap' }, this._btn('重新检测', { type: 'primary', size: 'sm', onClick: () => this._initWebCodecs() })), s.webcodecsSupported
                ? h('div', { class: 'flex gap-md' }, h('div', { class: 'flex-1' }, h('div', { class: 'fs-sm fw-medium mb-xs' }, '视频编解码器（VideoDecoder.isConfigSupported）'), h('div', { class: 'flex flex-col gap-xs' }, ...s.webcodecsVideo.map((r) => h('div', { class: 'flex items-center gap-xs fs-sm' }, h(Tag, { color: r.supported ? 'success' : 'default' }, r.supported ? '✓' : '✗'), h('span', {}, r.label), h('code', { class: 'text-tertiary' }, r.codec))))), h('div', { class: 'flex-1' }, h('div', { class: 'fs-sm fw-medium mb-xs' }, '音频编解码器（AudioDecoder.isConfigSupported）'), h('div', { class: 'flex flex-col gap-xs' }, ...s.webcodecsAudio.map((r) => h('div', { class: 'flex items-center gap-xs fs-sm' }, h(Tag, { color: r.supported ? 'success' : 'default' }, r.supported ? '✓' : '✗'), h('span', {}, r.label), h('code', { class: 'text-tertiary' }, r.codec))))))
            ...
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'CSS 原生滚动驱动动画，无需 JS。animation-timeline: scroll() 绑定到最近滚动容器；animation-timeline: view() 在元素进入视口时触发；animation-range 控制动画区间。下方容器可滚动，顶部进度条随滚动伸缩，列表项进入视口时淡入。同时用 JS（scroll 事件）观察并记录 scrollTop 变化。'), h('div', { class: 'flex items-center gap-sm' }, h('span', { class: 'fs-sm text-secondary' }, '滚动位置：'), h('span', { class: 'fs-sm fw-medium scroll-position' }, '0 / 0 px')), h('div', { class: 'scroll-anim-container' }, h('div', { class: 'scroll-progress-track' }, h('div', { class: 'scroll-progress-bar' })), ...Array.from({ length: 12 }, (_, i) => h('div', { class: 'scroll-view-item' }, h('div', { class: 'fw-medium' }, `条目 #${i + 1}`), h('div', { class: 'fs-sm text-secondary' }, '此元素绑定 animation-timeline: view()，进入滚动视口时触发淡入位移动画（animation-range: entry 0% cover 40%）。')))), h('div', { class: 'fs-sm text-secondary mt-xs' }, '关键 CSS：'), h('pre', { class: 'code-block' }, `.scroll-progress-bar {\n  animation: scroll-progress linear both;\n  animation-timeline: scroll();\n}\n.scroll-view-item {\n  animation: scroll-view-fade linear both;\n  animation-timeline: view();\n  animation-range: entry 0% cover 40%;\n}`))),
            // ============ 日志面板 ============
            h(Card, {
                title: '事件日志',
...
//# sourceMappingURL=AdvancedGraphicsPage.js.map