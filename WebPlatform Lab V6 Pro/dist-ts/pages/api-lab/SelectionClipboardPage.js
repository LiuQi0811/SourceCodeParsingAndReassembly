...
export class SelectionClipboardPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!hasClipWrite)
            this._addLog('warn', 'navigator.clipboard 不可用（jsdom 或非安全上下文 http）');
        if (!hasClipItem)
            this._addLog('warn', 'ClipboardItem 未定义（多 MIME 写入演示将记日志说明）');
        if (!hasExec)
            this._addLog('warn', 'document.execCommand 不可用');
        ...
        if (hasClipWrite && typeof document !== 'undefined') {
            ...
            try {
                document.addEventListener('clipboardchange', this._clipboardChangeHandler);
            }
            catch { /* noop */ }
        }
    }
    componentWillUnmount() {
        // 移除事件监听器并释放引用，便于 GC
        const remove = (target, type, handler) => {
        ...
        if (this._selHost && this._selHost.parentNode) {
            try {
                this._selHost.parentNode.removeChild(this._selHost);
            }
            catch { /* noop */ }
        }
        if (this._rangeHost && this._rangeHost.parentNode) {
            try {
                this._rangeHost.parentNode.removeChild(this._rangeHost);
            }
            catch { /* noop */ }
        }
    ...
    _readSelection() {
        ...
        try {
            ...
            if (this._selHost && typeof this._selHost.select === 'function') {
                try {
                    this._selHost.focus();
                    this._selHost.setSelectionRange(6, 14);
                    selectLine = `textarea.setSelectionRange(6, 14) → 选中文本索引 [6, 14)\n`;
                }
                catch (e) {
                    selectLine = `setSelectionRange 失败：${e.message}\n`;
                }
            }
            else {
                selectLine = '（未创建 textarea 宿主，先点「创建选区宿主」）\n';
            }
            const text = sel.toString();
            this.setState({
                selectionInfo: `${selectLine}` +
    ...
    _addRangeAndSelectDemo() {
        ...
        try {
            const sel = window.getSelection();
            if (!sel) {
                this.setState({ selectionInfo: 'getSelection() 返回 null，无法操作' });
                return;
            }
            sel.removeAllRanges();
            ...
            if (this._caps().createRange && this._selHost) {
                try {
                    ...
                    if (typeof sel.containsNode === 'function')
                        lines.push(`sel.containsNode(host, false) = ${sel.containsNode(this._selHost, false)}`);
                    sel.collapse(this._selHost, 0);
                    lines.push(`sel.collapse(host, 0) → 折叠到起始（isCollapsed=${sel.isCollapsed}）`);
                }
                catch (e) {
                    lines.push(`addRange/selectAllChildren/collapse 失败：${e.name} - ${e.message}（jsdom 可能不支持真实选区）`);
                }
            }
            else {
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.selection ? 'success' : 'error' }, caps.selection ? 'Selection ✓' : '不可用'), h(Tag, { color: 'primary' }, 'getSelection / toString')),
            children: [
                ...
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('创建选区宿主', { type: 'primary', size: 'sm', disabled: !caps.selection, onClick: () => this._createSelectionHost() }), this._btn('读取 Selection', { type: 'primary', size: 'sm', disabled: !caps.selection, onClick: () => this._readSelection() }), this._btn('addRange/selectAllChildren', { size: 'sm', disabled: !caps.selection, onClick: () => this._addRangeAndSelectDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, 'Selection 状态：'),
                h('pre', { class: 'code-block', style: { maxHeight: '220px', overflow: 'auto' } }, h('code', {}, s.selectionInfo || '（点击「创建选区宿主」开始）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '120px', overflow: 'auto' } }, h('code', {}, `const sel = window.getSelection();
...
sel.selectAllChildren(node);              // 选中所有子节点`)),
    ...
    _rangeSetStartEnd() {
        ...
        try {
            if (!this._rangeHost) {
                this._addLog('warn', '请先点击「创建 Range 宿主」');
                return;
            }
            const range = document.createRange();
            ...
            if (firstChild && firstChild.nodeType === 3) { // Node.TEXT_NODE = 3
                ...
                try {
                    ...
                    lines.push(`cloneContents 失败：${e.message}`);
                }
                try {
                    ...
                    lines.push(`getBoundingClientRect/getClientRects 失败：${e.message}（jsdom 无布局）`);
                }
            }
            else {
                lines.push('宿主无文本子节点');
    ...
    _rangeSelectAndClone() {
        ...
        try {
            ...
            try {
                ...
                lines.push(`cloneRange/compareBoundaryPoints 失败：${e.message}`);
            }
    ...
    _rangeMutate() {
        ...
        try {
            ...
            try {
                ...
                lines.push(`insertNode 失败：${e.name} - ${e.message}`);
            }
            try {
                host.textContent = 'Range 包裹演示文本';
                const range2 = document.createRange();
                range2.selectNodeContents(host);
                ...
                lines.push(`surroundContents 失败：${e.name} - ${e.message}（须恰好跨完整节点）`);
            }
            try {
                ...
                lines.push(`extractContents/deleteContents 失败：${e.name} - ${e.message}`);
            }
            this.setState({ rangeInfo: lines.join('\n') });
    ...
    _renderCard2() {
        ...
        const card = new Card({
            title: '2. Range 操作',
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.createRange ? 'success' : 'error' }, caps.createRange ? 'createRange ✓' : '不可用'), h(Tag, { color: caps.range ? 'success' : 'warning' }, caps.range ? 'Range ✓' : 'Range ✗')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'document.createRange() 或 new Range() 创建 Range，表示文档片段区间。setStart(node, offset)/setEnd(node, offset) 定位；setStartBefore/setStartAfter/setEndBefore/setEndAfter 用相邻节点定位；selectNode 选中整个节点，selectNodeContents 仅选内容；collapsed 判断重合；cloneContents 复制为 DocumentFragment，extractContents 移出，deleteContents 删除，insertNode 插入，surroundContents 用新父节点包裹；cloneRange 克隆，compareBoundaryPoints 比较边界（用 Range.START_TO_START 等常量）；getBoundingClientRect/getClientRects 取几何矩形。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('创建 Range 宿主', { type: 'primary', size: 'sm', disabled: !caps.createRange, onClick: () => this._createRangeHost() }), this._btn('setStart/setEnd', { type: 'primary', size: 'sm', disabled: !caps.createRange, onClick: () => this._rangeSetStartEnd() }), this._btn('selectNode/cloneRange', { size: 'sm', disabled: !caps.createRange, onClick: () => this._rangeSelectAndClone() }), this._btn('insertNode/surround', { size: 'sm', disabled: !caps.createRange, onClick: () => this._rangeMutate() })),
                h('div', { class: 'fs-sm text-secondary' }, 'Range 状态：'),
                h('pre', { class: 'code-block', style: { maxHeight: '240px', overflow: 'auto' } }, h('code', {}, s.rangeInfo || '（点击「创建 Range 宿主」开始）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '110px', overflow: 'auto' } }, h('code', {}, `const range = document.createRange();
...
range.surroundContents(document.createElement('mark')); // 包裹`)),
    ...
    async _clipboardWriteText() {
        ...
        try {
            ...
            this.setState({
                clipboardInfo: `navigator.clipboard.writeText('剪贴板写入测试 📋') → Promise<void> ✓\n` +
                    `已将文本写入系统剪贴板。\n` +
    ...
    async _clipboardReadText() {
        ...
        try {
            ...
            this.setState({
                clipboardInfo: `navigator.clipboard.readText() → Promise<string> ✓\n` +
                    `读取到的文本："${text}"\n` +
                    `文本长度：${text.length} 字符\n` +
    ...
    async _clipboardWriteItem() {
        ...
        try {
            ...
            this.setState({
                clipboardInfo: `new ClipboardItem({ 'text/plain': blob, 'text/html': blob }) ✓\n` +
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.clipboard ? 'success' : 'error' }, caps.clipboard ? 'clipboard.writeText ✓' : '不可用'), h(Tag, { color: caps.clipboardItem ? 'success' : 'warning' }, caps.clipboardItem ? 'ClipboardItem ✓' : 'ClipboardItem ✗')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'navigator.clipboard.writeText(text) / readText() 是最常用的纯文本异步剪贴板 API；navigator.clipboard.write([ClipboardItem]) / read() 支持多 MIME（text/plain、text/html、image/png）。new ClipboardItem({ "text/plain": blob, "text/html": blob }) 构造多格式项；item.types 列出 MIME，item.getType(mime) 取对应 Blob。所有方法返回 Promise，需 HTTPS 安全上下文 + 用户授权。clipboardchange 事件在剪贴板内容变化时触发。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('writeText', { type: 'primary', size: 'sm', disabled: !caps.clipboard, onClick: () => this._clipboardWriteText() }), this._btn('readText', { type: 'primary', size: 'sm', disabled: !caps.clipboardRead, onClick: () => this._clipboardReadText() }), this._btn('write ClipboardItem', { size: 'sm', disabled: !caps.clipboardWrite || !caps.clipboardItem, onClick: () => this._clipboardWriteItem() })),
                h('div', { class: 'fs-sm text-secondary' }, 'Clipboard 状态：'),
                h('pre', { class: 'code-block', style: { maxHeight: '200px', overflow: 'auto' } }, h('code', {}, s.clipboardInfo || '（点击 writeText / readText / write ClipboardItem）')),
                h(Alert, {
                    type: 'warning',
    ...
    _execCommandDetect() {
        ...
        try {
            ...
            for (const cmd of cmds) {
                ...
                try {
                    value = String(document.queryCommandValue(cmd));
                }
                catch (e) {
                    value = `err(${e.name})`;
                }
            ...
            const exec = (cmd, ...args) => { try {
                return String(document.execCommand(cmd, ...args));
            }
            catch (e) {
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.execCommand ? 'success' : 'error' }, caps.execCommand ? 'execCommand ✓' : '不可用'), h(Tag, { color: 'warning' }, 'Deprecated')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'document.execCommand(cmd, showUI, value) 执行编辑命令：copy/cut/paste（剪贴板）、selectAll（全选）、bold/italic/underline（富文本）、insertText（插入文本）、formatBlock（块级格式 h1/h2/p）、undo/redo（撤销重做）。queryCommandSupported(cmd) 检测支持，queryCommandEnabled(cmd) 检测当前可用，queryCommandValue(cmd) 取值。execCommand 已被 MDN 标记为 Deprecated，推荐用 Clipboard API 替代 copy/cut/paste，但旧浏览器与富文本编辑仍依赖它。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('命令支持检测 + copy/cut/paste', { type: 'primary', size: 'sm', disabled: !caps.execCommand, onClick: () => this._execCommandDetect() }), this._btn('富文本格式命令', { size: 'sm', disabled: !caps.execCommand, onClick: () => this._execFormatCmds() })),
                h('div', { class: 'fs-sm text-secondary' }, 'execCommand 结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, s.execInfo || '（点击按钮执行 execCommand）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '110px', overflow: 'auto' } }, h('code', {}, `// 富文本编辑（contenteditable）
editor.focus();
...
document.queryCommandEnabled('copy');     // 当前是否可用`)),
        ...
        try {
            ...
            this._copyHandler = (e) => {
                if (e.clipboardData) {
                    ...
                    try {
                        e.clipboardData.setData('text/html', '<i>【被拦截的 copy 事件】</i>自定义 HTML');
                    }
                    catch { /* noop */ }
            ...
            this.setState({
                eventInfo: `已注册 document 级 'copy' 事件监听器 ✓\n` +
    ...
    _registerCutPasteIntercept() {
        ...
        try {
            ...
            this._cutHandler = (e) => {
                if (e.clipboardData)
                    e.clipboardData.setData('text/plain', '【cut 拦截】剪切被替换为此文本');
                e.preventDefault();
            ...
            this.setState({
                eventInfo: `已注册 document 级 'cut' 与 'paste' 事件监听器 ✓\n` +
    ...
    _dispatchCopyEvent() {
        if (typeof document === 'undefined' || typeof Event === 'undefined') {
            this._addLog('warn', 'Event/document 不可用');
            return;
        }
        try {
            let evt;
            ...
            if (caps.clipboardEvent) {
                try {
                    evt = new ClipboardEvent('copy', { bubbles: true, cancelable: true, clipboardData: new DataTransfer() });
                }
                catch (e) {
            ...
            this.setState({
                eventInfo: `document.dispatchEvent(${evt.constructor.name}('copy')) → 已派发 copy 事件\n` +
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.event ? 'success' : 'error' }, caps.event ? 'Event ✓' : '不可用'), h(Tag, { color: caps.clipboardEvent ? 'success' : 'warning' }, caps.clipboardEvent ? 'ClipboardEvent ✓' : 'ClipboardEvent ✗')),
            children: [
                ...
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('注册 copy 拦截', { type: 'primary', size: 'sm', disabled: !caps.event, onClick: () => this._registerCopyIntercept() }), this._btn('注册 cut/paste 拦截', { size: 'sm', disabled: !caps.event, onClick: () => this._registerCutPasteIntercept() }), this._btn('手动触发 copy', { type: 'primary', size: 'sm', disabled: !caps.event, onClick: () => this._dispatchCopyEvent() })),
                h('div', { class: 'fs-sm text-secondary' }, '事件拦截状态：'),
                h('pre', { class: 'code-block', style: { maxHeight: '220px', overflow: 'auto' } }, h('code', {}, s.eventInfo || '（点击「注册 copy 拦截」然后「手动触发 copy」测试）')),
                h(Alert, {
                    type: 'info',
    ...
    _modifyExtendDemo() {
        ...
        try {
            ...
            if (this._selHost) {
                try {
                    this._selHost.focus();
                    if (typeof this._selHost.setSelectionRange === 'function') {
                        this._selHost.setSelectionRange(0, 1);
                        prepLine = `初始：textarea.setSelectionRange(0, 1) → 选第 1 个字符\n`;
                    }
                }
                catch (e) {
                    prepLine = `初始选区失败：${e.message}\n`;
                }
            }
            else {
                prepLine = '（未创建 textarea 宿主，先点 Card 1「创建选区宿主」）\n';
            }
            ...
            if (modifySupported) {
                try {
                    ...
                    modifyLine = `modify 抛错：${e.name} - ${e.message}\n`;
                }
            }
            else {
                modifyLine = `selection.modify 不可用（jsdom 未实现，浏览器中可用）\n`;
            }
    ...
    _modifyMoveDemo() {
        ...
        try {
            const sel = window.getSelection();
            if (!sel) {
                this.setState({ modifyInfo: 'getSelection() 返回 null' });
                return;
            }
            let prepLine = '';
            if (this._selHost) {
                try {
                    this._selHost.focus();
                    if (typeof this._selHost.setSelectionRange === 'function') {
                        this._selHost.setSelectionRange(5, 10);
                        prepLine = `初始：textarea.setSelectionRange(5, 10) → 选 [5,10)\n`;
                    }
                }
                catch (e) {
                    prepLine = `初始选区失败：${e.message}\n`;
            ...
            if (modifySupported) {
                try {
                    ...
                    lines.push(`sel.modify('move','backward','word') → 移动光标（选区折叠）`);
                }
                catch (e) {
                    lines.push(`move 抛错：${e.name} - ${e.message}`);
                }
            }
            else {
                lines.push(`selection.modify 不可用（jsdom 未实现）`);
            }
            // collapse / extend
            try {
                ...
                if (anchorNode) {
                    ...
                    try {
                        sel.extend(anchorNode, 3);
                        lines.push(`sel.extend(anchorNode, 3) → 扩展焦点到 offset 3（isCollapsed=${sel.isCollapsed}，toString="${sel.toString()}")`);
                    }
                    catch (e) {
                        lines.push(`extend 抛错：${e.message}`);
                    }
                }
                else {
                    lines.push(`anchorNode 为 null，无法 collapse/extend`);
                }
            }
            catch (e) {
                lines.push(`collapse/extend 抛错：${e.name} - ${e.message}`);
            }
            ...
            if (this._caps().createRange && this._selHost) {
                try {
                    ...
                    if (!fc) {
                        ...
                        const tryOp = (label, fn) => { try {
                            fn();
                            lines.push(label);
                        }
                        catch (e) {
                            lines.push(`${label.split(' →')[0]} 抛错：${e.name} - ${e.message}`);
                        } };
                    ...
                    lines.push(`操作失败：${e.name} - ${e.message}`);
                }
            }
            else {
                lines.push('createRange 或宿主不可用');
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.selection ? 'success' : 'error' }, caps.selection ? 'Selection ✓' : '不可用'), h(Tag, { color: 'warning' }, 'modify 非标准')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'selection.modify(alter, direction, granularity) 调整选区：alter="extend" 扩展选区 / "move" 移动光标；direction="forward"/"backward"/"left"/"right"；granularity="character"/"word"/"line"/"paragraph"/"lineboundary"。collapse(node, offset) 折叠光标；extend(node, offset) 扩展焦点（保持锚点）；deleteFromDocument() 删除选中内容；removeRange(range) 移除指定 Range；removeAllRanges() 清空。注：modify 为非标准 API（Firefox/Chrome 支持，jsdom 未实现）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('modify extend', { type: 'primary', size: 'sm', disabled: !caps.selection, onClick: () => this._modifyExtendDemo() }), this._btn('move + collapse/delete', { size: 'sm', disabled: !caps.selection, onClick: () => this._modifyMoveDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, 'modify 操作结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '240px', overflow: 'auto' } }, h('code', {}, s.modifyInfo || '（先在 Card 1 创建选区宿主，再点本卡按钮）')),
                h(Alert, {
                    type: 'info',
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'page api-lab-page' }, h('h2', { class: 'section-title' }, 'Selection / Clipboard / execCommand 实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, '本页演示 Selection API（选区）、Range API（文档片段区间）、Clipboard API（异步剪贴板读写）、execCommand（已废弃的编辑命令）、copy/cut/paste 事件拦截、selection.modify 高级操作。所有 API 调用前做 typeof 能力检测，不可用时仅记日志。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderCard6(), this._renderLogPanel());
    }
}
//# sourceMappingURL=SelectionClipboardPage.js.map