...
export class CanvasDeepPage extends Page {
    ...
    _makeOffscreen(size = 100) {
        try {
            ...
            if (typeof octx.createLinearGradient === 'function') {
                ...
                if ('fillStyle' in octx)
                    octx.fillStyle = g;
            }
            else if ('fillStyle' in octx) {
                octx.fillStyle = '#722ed1';
            }
            if (typeof octx.fillRect === 'function')
                octx.fillRect(0, 0, size, size);
            // 居中文字
            if (typeof octx.fillText === 'function') {
                if ('font' in octx)
                    octx.font = `bold ${Math.floor(size / 7)}px sans-serif`;
                if ('fillStyle' in octx)
        ...
        try {
            ...
            for (const p of points) {
                ...
                if (caps.isPointInPath) {
                    try {
                        inPath = ctx.isPointInPath(this._starPath, p.x, p.y);
                    }
                    catch { /* noop */ }
                }
                if (caps.isPointInStroke) {
                    try {
                        inStroke = ctx.isPointInStroke(this._starPath, p.x, p.y);
                    }
                    catch { /* noop */ }
                }
    ...
    _cycleComposite() {
        ...
        try {
            ...
            if (caps.globalCompositeOperation)
                ctx.globalCompositeOperation = next;
            if (typeof ctx.beginPath === 'function')
                ctx.beginPath();
            if (typeof ctx.arc === 'function')
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.path2d ? 'success' : 'warning' }, caps.path2d ? 'Path2D 可用' : 'Path2D 不可用'), h(Tag, { color: caps.isPointInPath ? 'success' : 'default' }, caps.isPointInPath ? 'isPointInPath ✓' : 'isPointInPath ✗')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '演示：用 Path2D 构造一个 10 顶点星形路径，复用同一对象做 fill + stroke；isPointInPath / isPointInStroke 检测多个坐标是否命中路径。Path2D 把路径数据封装为对象，可在多处复用、跨 worker 序列化（OffscreenCanvas）、被浏览器 GPU 加速缓存。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('绘制 Path2D 星形', {
                ...
                h('pre', { class: 'code-block', style: { maxHeight: '80px', overflow: 'auto' } }, h('code', {}, s.hitResult || '（点击「绘制 Path2D 星形」→「isPointInPath 命中测试」）')),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.getImageData ? 'success' : 'warning' }, caps.getImageData ? 'getImageData ✓' : 'getImageData ✗'), h(Tag, { color: caps.putImageData ? 'success' : 'warning' }, caps.putImageData ? 'putImageData ✓' : 'putImageData ✗')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '演示：getImageData 读取像素 → 灰度化（Y = 0.299R + 0.587G + 0.114B）/ 反色（new = 255 - old）/ 亮度调整（data[i] += Δ，Uint8ClampedArray 自动 clamp）→ putImageData 写回。createImageData 构造全空 ImageData 并逐像素填入红蓝棋盘格。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('绘制原图', {
                    type: 'primary', size: 'sm',
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.globalCompositeOperation ? 'success' : 'warning' }, caps.globalCompositeOperation ? '可用' : '不可用'), h(Tag, { color: 'primary' }, `当前：${s.currentComposite || 'source-over'}`)),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '演示：先画红色圆（destination），切换合成模式后再画蓝色圆（source），观察两圆重叠区域的混合效果。共 12 种常用模式循环切换。source-over 为默认（新图形覆盖旧）；multiply 模拟透明胶片叠加；difference 取差值；xor 重叠区域透明。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('循环切换 12 种合成模式', {
                    type: 'primary', size: 'sm',
                ...
                h('div', { class: 'fs-sm text-secondary mt-sm' }, `共 ${COMPOSITE_MODES.length} 种循环模式：${COMPOSITE_MODES.join(' / ')}`),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.drawImage ? 'success' : 'warning' }, caps.drawImage ? 'drawImage ✓' : 'drawImage ✗'), h(Tag, { color: caps.transform ? 'success' : 'warning' }, caps.transform ? 'transform ✓' : 'transform ✗')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '演示：drawImage 三种重载（1:1 / 缩放 / 局部源放大），image 源用离屏 canvas（带渐变 + 文字）；transform 矩阵围绕 (160,100) 旋转 45°；save/restore 嵌套绘制 5 个旋转方块。save 入栈：fillStyle/strokeStyle/lineWidth/transform/globalAlpha/globalCompositeOperation/font 等；restore 出栈恢复。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('drawImage 缩放绘制', {
                    type: 'primary', size: 'sm',
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.createPattern ? 'success' : 'warning' }, caps.createPattern ? 'pattern ✓' : 'pattern ✗'), h(Tag, { color: caps.createConicGradient ? 'success' : 'warning' }, caps.createConicGradient ? 'conic ✓' : 'conic ✗')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '演示：4 种填充并排对比 —— createPattern（离屏 canvas 平铺）/ createLinearGradient（线性）/ createRadialGradient（径向，可造光晕）/ createConicGradient（圆锥，较新浏览器）。所有渐变通过 addColorStop(offset 0-1, color) 添加色标。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('绘制 4 种渐变并排对比', {
                    type: 'primary', size: 'sm',
                ...
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '从左到右：pattern(repeat) → linear(120→220) → radial(280,100,r=5→90) → conic(390,100,0°→360°)'),
                h(Alert, {
                    type: 'warning',
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'page api-lab-page' }, h('h2', { class: 'section-title' }, 'Canvas 2D 深度实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, '演示 Canvas 2D 进阶：Path2D 路径对象 / 像素操作 / globalCompositeOperation 合成 / drawImage+坐标变换 / createPattern+渐变。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderLogPanel());
    }
}
//# sourceMappingURL=CanvasDeepPage.js.map