...
export class CSSFlexboxDeepPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!f.gap)
            this._addLog('warn', 'gap 不可用（Chrome 84+/Firefox 63+/Safari 14.1+ 起支持，旧浏览器需用 margin 模拟）');
        if (!f.flexBasisContent)
            this._addLog('warn', 'flex-basis: content 不可用（仅 Firefox 较早支持，Chrome/Safari 较新版本支持）');
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'Flex 容器：display: flex（块级）/ inline-flex（行内级）。flex-direction 设置主轴方向：row(默认)/row-reverse/column/column-reverse。主轴用 justify-content 对齐，交叉轴用 align-items 对齐。容器属性还有 flex-wrap/align-content/gap；项目属性有 order/flex(flex-basis+grow+shrink)/align-self。Chrome 29+/Firefox 22+/Safari 9+ 全面支持，IE 11 实现旧规范有 bug。'),
                h('div', { class: 'fl-demo' }, h('div', { class: 'fl-basics-block' }, h('div', { class: 'fl-item' }, '1'), h('div', { class: 'fl-item' }, '2'), h('div', { class: 'fl-item' }, '3')), h('div', { class: 'fl-basics-inline' }, h('div', { class: 'fl-item fl-item--alt' }, 'A'), h('div', { class: 'fl-item fl-item--alt' }, 'B'))),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行基础演示', { type: 'primary', size: 'sm', onClick: () => this._runBasicsDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.basicsInfo || '（点击按钮查看 display:flex / flex-direction / 主轴交叉轴 完整用法）')),
            ],
        });
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['flex-basis:content', f.flexBasisContent]]), h(Tag, { color: 'primary' }, '核心算法')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'flex 算法五步：收集项目主轴尺寸 → 算可用空间 → 正走 grow 分配 / 负走 shrink 收缩。flex-basis 优先于 width；flex-grow 按比例分剩余；flex-shrink 按 basis 加权收缩。flex 简写：flex:1=1 1 0%（等分）/ flex:auto=1 1 auto（按内容+分剩余）/ flex:none=0 0 auto（不变不缩）。flex-basis:0 让所有项目从 0 开始按 grow 瓜分实现等分；flex-basis:auto 则先按内容排再分剩余。'),
                h('div', { class: 'fl-demo' }, h('div', { class: 'fl-box fl-algo-grow' }, h('div', { class: 'fl-item grow-1' }, 'grow:1'), h('div', { class: 'fl-item grow-2' }, 'grow:2'), h('div', { class: 'fl-item grow-3' }, 'grow:3')), h('div', { class: 'fl-box fl-algo-fixed' }, h('div', { class: 'fl-item fixed' }, 'fixed 80'), h('div', { class: 'fl-item flex-1 fl-item--accent' }, 'flex:1 占剩余'))),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行三件套演示', { type: 'primary', size: 'sm', onClick: () => this._runFlexAlgorithmDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.flexAlgorithmInfo || '（点击按钮查看 flex-basis/grow/shrink 算法完整解析）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['space-evenly', f.alignContentSpaceEvenly]]), h(Tag, { color: 'primary' }, '主轴对齐')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'justify-content 控制项目在主轴上的对齐：flex-start(默认)/flex-end/center/space-between/space-around/space-evenly。space-between 首尾贴边中间均分；space-around 首尾半隙中间全隙；space-evenly 全部等隙（含首尾）。配合 flex-direction: row-reverse 可翻转主轴起点。IE 11 不支持 space-evenly/around，可用 margin:auto 降级。'),
                h('div', { class: 'fl-demo' }, h('div', { class: 'fl-jc-row', style: 'justify-content: flex-start' }, h('span', {}, 'A'), h('span', {}, 'B'), h('span', {}, 'C')), h('div', { class: 'fl-jc-row', style: 'justify-content: center' }, h('span', {}, 'A'), h('span', {}, 'B'), h('span', {}, 'C')), h('div', { class: 'fl-jc-row', style: 'justify-content: space-between' }, h('span', {}, 'A'), h('span', {}, 'B'), h('span', {}, 'C')), h('div', { class: 'fl-jc-row', style: 'justify-content: space-evenly' }, h('span', {}, 'A'), h('span', {}, 'B'), h('span', {}, 'C'))),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 justify-content 演示', { type: 'primary', size: 'sm', onClick: () => this._runJustifyContentDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.justifyContentInfo || '（点击按钮查看 justify-content 6 种取值完整解析）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['align-items', f.flex]]), h(Tag, { color: 'primary' }, '交叉轴对齐')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'align-items 控制所有项目在交叉轴的对齐：stretch(默认)/flex-start/flex-end/center/baseline。align-self 单个项目覆盖。stretch 仅项目无固定交叉轴尺寸时生效；baseline 让所有项目文字基线对齐。Flexbox 实现垂直居中：align-items:center + justify-content:center 两行搞定，替代旧 5+ 种 hack。'),
                h('div', { class: 'fl-demo' }, h('div', { class: 'fl-ai-row', style: 'align-items: flex-start' }, h('span', { class: 'tall' }, 'A'), h('span', {}, 'B'), h('span', { class: 'short' }, 'C')), h('div', { class: 'fl-ai-row', style: 'align-items: center' }, h('span', { class: 'tall' }, 'A'), h('span', {}, 'B'), h('span', { class: 'short' }, 'C')), h('div', { class: 'fl-ai-row', style: 'align-items: baseline' }, h('span', { class: 'tall' }, 'A'), h('span', {}, 'B'), h('span', { class: 'short' }, 'C'))),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 align-items 演示', { type: 'primary', size: 'sm', onClick: () => this._runAlignItemsDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.alignItemsInfo || '（点击按钮查看 align-items/align-self 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'flex-wrap: nowrap(默认)/wrap/wrap-reverse 控制换行。多行时 align-content 控制行间距（7 种取值，仅多行有效）；align-items 控制单行内项目对齐。gap/row-gap/column-gap 替代 margin 实现项目间距（Chrome 84+），优势：无需 :first-child 重置、自动响应换行、CSS calc 可用。响应式卡片网格：flex: 1 1 200px + flex-wrap + gap 自动适应列数，无需 media query。'),
                h('div', { class: 'fl-demo' }, h('div', { class: 'fl-wrap-box' }, h('span', {}, '1'), h('span', {}, '2'), h('span', {}, '3'), h('span', {}, '4'), h('span', {}, '5'), h('span', {}, '6'))),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行多行+gap 演示', { type: 'primary', size: 'sm', onClick: () => this._runFlexWrapAlignContentDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.flexWrapAlignContentInfo || '（点击按钮查看 flex-wrap/align-content/gap 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['order', f.order]]), h(Tag, { color: 'warning' }, '可访问性')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'order: <integer>（默认 0，可为负数）改变项目视觉顺序，但 Tab 焦点顺序和屏幕阅读器朗读仍按 DOM 顺序，造成视觉与可访问性不一致（违反 WCAG 2.1 SC 1.3.2）。推荐用 DOM 顺序而非 order 实现布局；order 仅用于装饰性重排（如媒体对象图片左右切换）。响应式重排也尽量用 DOM + media query 切换而非纯 order。'),
                h('div', { class: 'fl-demo' }, h('div', { class: 'fl-order-box' }, h('span', {}, 'A'), h('span', { class: 'o-last' }, 'B order:99'), h('span', { class: 'o-first' }, 'C order:-1')), h('div', { class: 'fs-xs text-secondary mt-xs' }, 'DOM: A B C → 视觉: C A B')),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 order 演示', { type: 'primary', size: 'sm', onClick: () => this._runOrderDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.orderInfo || '（点击按钮查看 order 与可访问性完整解析）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard7() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '15 种经典布局模式完整代码：垂直水平居中（justify+align center）、顶栏（space-between）、等高三栏（两侧固定+中间 flex:1）、响应式卡片网格（flex:1 1 200px + wrap + gap 无需 media query）、粘性页脚（min-height:100vh + column + main flex:1）、媒体对象、输入框组、垂直等距导航、按钮组、分页器、响应式重排、底部操作栏、聊天气泡、表单行、标签云。Flexbox 适合内容优先的一维布局，整页布局用 Grid。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行布局模式演示', { type: 'primary', size: 'sm', onClick: () => this._runPatternsDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.patternsInfo || '（点击按钮查看 15 种布局模式完整代码）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard8() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '核心陷阱：min-width/min-height: auto（默认不收缩到内容以下，需手动设 0）；flex-basis 优先于 width；百分比相对容器 content-box；绝对定位项目不参与 flex 但受 align-items 影响；IE 11 有诸多 flexbug 需显式写 flex: 1 1 0%。Chrome DevTools 提供 Flex 编辑器、align-items 切换徽章、项目 sizing 可视化（grow=绿/shrink=红）。vs Grid：内容驱动一维用 Flexbox，布局驱动二维用 Grid。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行调试与陷阱演示', { type: 'primary', size: 'sm', onClick: () => this._runTrapsDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.trapsInfo || '（点击按钮查看 Flexbox 调试与陷阱完整解析）')),
            ],
        });
        this.registerChild(card);
        return card.render();
...
//# sourceMappingURL=CSSFlexboxDeepPage.js.map