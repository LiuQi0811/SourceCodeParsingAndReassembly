...
export class WebTransportPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!f.datagramDuplexStream)
            this._addLog('warn', 'WebTransportDatagramDuplexStream 不可用（datagrams API 演示跳过）');
        if (!f.bidiStream)
            this._addLog('warn', 'WebTransportBidirectionalStream 不可用（双向流演示跳过）');
        if (!f.sendStream)
            this._addLog('warn', 'WebTransportSendStream 不可用（单向发送流演示跳过）');
        if (!f.receiveStream)
            this._addLog('warn', 'WebTransportReceiveStream 不可用（单向接收流演示跳过）');
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'WebTransport over HTTP/3 基于 QUIC（UDP）解决 WebSocket 三大痛点：TCP 队头阻塞、单流瓶颈、无不可靠模式。提供三种通信模式：Datagrams（不可靠数据报，游戏/视频帧）、Bidirectional Streams（可靠双向流，RPC/文件）、Unidirectional Streams（可靠单向流，日志/推送），共享一条连接、流间互不阻塞。浏览器支持 Chrome 97+ / Edge 97+，Firefox/Safari 受限。jsdom 无此 API，仅展示用法。'),
                ...
                h('div', { class: 'wt-mode-grid' }, h('div', { class: 'wt-mode-card wt-mode-card--datagram' }, h('div', { class: 'wt-mode-title' }, 'Datagrams 数据报'), h('div', {}, '不可靠 · 无序 · ≤ maxDatagramSize\n游戏输入 / 视频帧 / 遥测')), h('div', { class: 'wt-mode-card wt-mode-card--bidi' }, h('div', { class: 'wt-mode-title' }, 'Bidirectional Streams'), h('div', {}, '可靠 · 全双工 · 流独立\nRPC / 文件传输 / 控制信令')), h('div', { class: 'wt-mode-card wt-mode-card--unidi' }, h('div', { class: 'wt-mode-title' }, 'Unidirectional Streams'), h('div', {}, '可靠 · 单向 · 流独立\n日志上报 / 单向推送 / 流式下发'))),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// 连接 WebTransport（https + /_webtransport 路径）
...
transport.close({ closeCode: 1000, reason: 'done' });`)),
                ...
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.overviewInfo || '（点击按钮查看 WebTransport 概述与动机完整说明）')),
            ],
        });
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['WebTransport', f.webTransport]]), h(Tag, { color: 'primary' }, '构造')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'new WebTransport(url, { allowPooling, serverCertificateHashes, congestionControl }) 构造，url 必须 https（开发期 localhost 可 http）。三大 Promise：ready（连接就绪）、closed（关闭解析 { closeCode, reason }）、draining（服务端 GOAWYE 提示停止新建流）。主动关闭 transport.close({ closeCode, reason }) 立即发 CONNECTION_CLOSE 帧。serverCertificateHashes 允许自签证书指纹白名单（仅开发/内网）。congestionControl 选 throughput / low-latency 倾向。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行构造演示', { type: 'primary', size: 'sm', onClick: () => this._runConnDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// 自签证书 + 低延迟拥塞控制
...
try {
  ...
  transport.close({ closeCode: 1000, reason: 'done' });
                ...
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.connInfo || '（点击按钮查看 WebTransport 构造与连接完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'transport.datagrams 是 WebTransportDatagramDuplexStream，含 readable + writable 双向 + maxDatagramSize 上限。Datagrams 不可靠、可能丢失/乱序，但保留消息边界（一个 write 对应一个 read）。适合游戏输入、视频帧（WebCodecs 编码后发送）、遥测采样等「过时即无价值」场景。与 WebRTC SCTP 相比无需 P2P 协商、开销更低。注意：必须应用层加序列号去重，关键控制信令应走可靠流。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 Datagrams 演示', { type: 'primary', size: 'sm', onClick: () => this._runDatagramDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// Datagrams：不可靠数据报
const writer = transport.datagrams.writable.getWriter();
const reader = transport.datagrams.readable.getReader();
...
encoder.configure({ codec: 'vp8', width: 1280, height: 720 });`)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.datagramInfo || '（点击按钮查看 Datagrams 数据报完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'transport.createBidirectionalStream() 返回 WebTransportBidirectionalStream（含 readable + writable）。一条 WebTransport 连接可并发创建大量独立双向流，QUIC 流间互不阻塞（无 HOL 队头阻塞）。SendStream 写入可靠有序，ReceiveStream 读取可靠有序。适合 RPC（每请求一流）、文件分块传输、控制信令。背压通过 writer.ready 体现（QUIC 流控）。createUnidirectionalStream() 创建单向流。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行双向流演示', { type: 'primary', size: 'sm', onClick: () => this._runBidiDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// 创建双向流
const bidi = await transport.createBidirectionalStream();
const writer = bidi.writable.getWriter();
const reader = bidi.readable.getReader();
...
await uni.getWriter().write(new TextEncoder().encode('log entry'));`)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.bidiInfo || '（点击按钮查看双向流 Bidirectional Streams 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
...
//   minRtt, datagrams: { bytesSent, bytesReceived, ... } }`)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.unidiInfo || '（点击按钮查看单向发送/接收流完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['WebTransport', f.webTransport]]), h(Tag, { color: 'primary' }, '统计')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'congestionControl 选项：throughput（默认，最大化吞吐，NewReno/CUBIC）/ low-latency（优化延迟，BBRv2/Copa，Chrome 110+）。getStats() 返回 { bytesSent, bytesReceived, numOutgoingStreamsCreated, smoothedRtt, rttVariation, minRtt, datagrams } 连接级统计。RTT 监控周期采样判断网络质量，自适应码率根据 RTT + 丢包率动态调整 WebCodecs 编码器 bitrate（AIMD 策略）。allowPooling 共享拥塞控制状态但失去独立优先级。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行拥塞控制演示', { type: 'primary', size: 'sm', onClick: () => this._runCongestionDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// 低延迟拥塞控制
...
// }`)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.congestionInfo || '（点击按钮查看拥塞控制与统计完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard7() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['WebTransport', f.webTransport]]), h(Tag, { color: 'primary' }, '5 大场景')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '五大实战场景：云游戏串流（WebCodecs 编码 → datagrams 下行 + 输入上行 + 双向流控制信令）、实时协作 Figma/Google Docs（光标 datagrams + 编辑双向流 + 快照单向流）、视频会议低延迟（每路视频一条双向流，vs WebRTC SFU 更灵活）、IoT 双向遥测（传感器 datagrams + 控制双向流 + 批量日志单向流）、WebCodecs 协同（编码后通过 datagram 发送，全链路延迟 <50ms）。low-latency 拥塞控制 + 多路复用是核心优势。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行实战场景演示', { type: 'primary', size: 'sm', onClick: () => this._runMarketDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '数据流示意：'),
                h('div', { class: 'wt-flow' }, h('span', { class: 'wt-flow-node' }, 'WebCodecs 编码'), h('span', { class: 'wt-flow-arrow' }, '→'), h('span', { class: 'wt-flow-node wt-flow-node--unreliable' }, 'datagrams 下行'), h('span', { class: 'wt-flow-arrow' }, '→'), h('span', { class: 'wt-flow-node' }, 'WebCodecs 解码'), h('span', { class: 'wt-flow-arrow' }, '→'), h('span', { class: 'wt-flow-node' }, 'Canvas 渲染')),
                h('div', { class: 'wt-flow' }, h('span', { class: 'wt-flow-node wt-flow-node--unreliable' }, '玩家输入 datagrams 上行'), h('span', { class: 'wt-flow-arrow' }, '→'), h('span', { class: 'wt-flow-node' }, '服务端游戏循环')),
                h('div', { class: 'wt-flow' }, h('span', { class: 'wt-flow-node wt-flow-node--reliable' }, '控制信令 双向流'), h('span', { class: 'wt-flow-arrow' }, '↔'), h('span', { class: 'wt-flow-node wt-flow-node--reliable' }, '暂停/恢复/分辨率')),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// 云游戏：WebCodecs 编码 → datagrams 下行
const t = new WebTransport(url, { congestionControl: 'low-latency' });
await t.ready;
...
while (true) {
  ...
  decoder.decode(new EncodedVideoChunk({
    ...
    data: value.subarray(16),
                ...
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.marketInfo || '（点击按钮查看 5 大实战场景完整方案）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard8() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'primary' }, '决策'), h(Tag, { color: f.webTransport ? 'success' : 'error' }, `WebTransport ${f.webTransport ? '✓' : '✗'}`)),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '五大陷阱：服务端实现复杂（Nginx/Node 不支持，需 Cloudflare Quiche/quic-go/msquic 等）、自签证书 serverCertificateHashes 仅 sha-256 + DER 哈希、移动端 QUIC UDP 被运营商 QoS + iOS Safari 不支持、QUIC UDP 被防火墙阻断需 TCP 回退、与 WebSocket 不可互转需 TransportAdapter 抽象降级。6 维决策矩阵：传输层/HOL/多路复用/不可靠/双向/0-RTT。降级链 WebTransport→WebSocketStream→WebSocket→SSE 逐级牺牲能力。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行决策矩阵演示', { type: 'primary', size: 'sm', onClick: () => this._runDecisionDemo() })),
                h('div', { class: 'wt-matrix' }, h('div', { class: 'wt-matrix-cell' }, h('div', { class: 'wt-matrix-title' }, 'WebTransport'), h('div', {}, 'QUIC · 无 HOL · 多路复用 · datagram · 0-RTT · Chrome 97+')), h('div', { class: 'wt-matrix-cell' }, h('div', { class: 'wt-matrix-title' }, 'WebSocketStream'), h('div', {}, 'TCP · 有 HOL · 单流 · 背压 · Chrome 实验')), h('div', { class: 'wt-matrix-cell' }, h('div', { class: 'wt-matrix-title' }, 'WebSocket'), h('div', {}, 'TCP · 有 HOL · 单流 · 无背压 · 全平台')), h('div', { class: 'wt-matrix-cell' }, h('div', { class: 'wt-matrix-title' }, 'SSE'), h('div', {}, 'HTTP · 单向 · 文本 · 自动重连 · 全平台'))),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// 降级链：WebTransport → WebSocketStream → WebSocket → SSE
...
class TransportAdapter {
  ...
  async close() {
                ...
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.decisionInfo || '（点击按钮查看陷阱与决策矩阵完整说明）')),
            ],
        });
        this.registerChild(card);
        return card.render();
...
//# sourceMappingURL=WebTransportPage.js.map