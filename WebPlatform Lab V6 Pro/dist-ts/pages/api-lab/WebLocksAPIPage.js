...
//      navigator.locks.request(name, { mode: 'shared' } as any, cb)；典型：shared 用于读、exclusive 用于写。
...
export class WebLocksAPIPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!caps.locks)
            this._addLog('warn', 'navigator.locks 不可用（jsdom 无此对象，需 SecureContext + Chrome 69+）');
        if (!caps.query)
            this._addLog('warn', 'navigator.locks.query 不可用（jsdom 无此方法）');
    }
    componentWillUnmount() {
            ...
            try {
                if (entry && entry.queue) {
                    for (const q of entry.queue) {
                        try {
                            if (q && typeof q.resolve === 'function')
                                q.resolve(null);
                        }
                        catch { /* noop */ }
                    }
        ...
        for (const [name, holder] of this._pendingLocks || []) {
            try {
                ...
                if (holder && typeof holder.resolve === 'function')
                    holder.resolve();
            }
            catch { /* noop */ }
        }
    ...
    _showRequestCaps() {
        const caps = this._caps();
        this.setState({ requestInfo: '===== LockManager.request 能力检测 =====\n\n' +
    ...
    async _simulateRequest() {
        ...
        if (!caps.request) {
            ...
            this.setState({ requestInfo: `模拟 navigator.locks.request('${name}', cb) 执行流程：\n\n` +
        ...
        try {
            ...
            this.setState({ requestInfo: `真实 navigator.locks.request('${name}', cb)：\n` +
    ...
    _explainCallbackModel() {
        this.setState({ requestInfo: '===== callback 模型详解 =====\n\n' +
    ...
    _explainReleaseTiming() {
        this.setState({ requestInfo: '===== 锁释放时机 =====\n\n' +
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.request ? 'success' : 'error' }, caps.request ? 'request ✓' : 'request ✗'), h(Tag, { color: 'primary' }, 'callback 模型'), h(Tag, { color: 'warning' }, 'FIFO 排队')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'navigator.locks.request(name, callback) 返回 Promise；callback 接收 Lock 对象 { name, mode }。callback 返回的 Promise resolve 后锁释放；锁名跨同源 tab/worker 全局唯一。同名锁请求默认 exclusive 模式排队（FIFO）执行。callback 抛异常时锁自动释放（不会泄漏）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('能力检测', { type: 'primary', size: 'sm', onClick: () => this._showRequestCaps() }), this._btn('模拟请求锁', { size: 'sm', onClick: () => this._simulateRequest() }), this._btn('callback 模型', { size: 'sm', onClick: () => this._explainCallbackModel() }), this._btn('锁释放时机', { size: 'sm', onClick: () => this._explainReleaseTiming() })),
    ...
    async _demoExclusiveQueue() {
        ...
        if (!caps.request) {
            ...
            this.setState({ modeInfo: `模拟 exclusive 排队（锁名 '${name}'，3 个请求）：\n  ${order.join('\n  ')}\n\n` +
        ...
        try {
            ...
            await Promise.all([make(1), make(2), make(3)]);
            this.setState({ modeInfo: `真实 exclusive 排队（锁名 '${name}'，3 个请求）：\n  ${order.join('\n  ')}\n\n` +
                    'exclusive 模式（默认）：同名锁同时仅 1 个持有者；后续请求排队等待，FIFO 顺序执行。' });
        }
        catch (err) {
    ...
    async _demoSharedCoexist() {
        ...
        if (!caps.request) {
            ...
            await Promise.all([make(1), make(2), make(3)]);
            this.setState({ modeInfo: `模拟 shared 共存（锁名 '${name}'，3 个请求）：\n  ${acquired.join('\n  ')}\n\n` +
                    "shared 模式：navigator.locks.request(name, { mode: 'shared' } as any, cb)；多个 shared 可同时持有，但与 exclusive 互斥。" });
            return;
        }
        try {
            const acquired = [];
            ...
            await Promise.all([make(1), make(2), make(3)]);
            this.setState({ modeInfo: `真实 shared 共存（锁名 '${name}'，3 个请求）：\n  ${acquired.join('\n  ')}\n\n` +
                    "shared 模式：navigator.locks.request(name, { mode: 'shared' } as any, cb)；多个 shared 可同时持有，但与 exclusive 互斥。" });
        }
        catch (err) {
    ...
    _showReadWriteMatrix() {
        this.setState({ modeInfo: '===== 读写锁矩阵（exclusive vs shared）=====\n\n' +
                '当前请求 \\ 已持有     | exclusive | shared\n' +
                '---------------------|-----------|--------\n' +
                'exclusive（写）       | 排队      | 排队\n' +
                'shared（读）          | 排队      | 同时持有\n\n' +
                '典型用法：\n' +
                "  写：navigator.locks.request('resource', { mode: 'exclusive' } as any, async (lock) => { /* 写入 */ });\n" +
                "  读：navigator.locks.request('resource', { mode: 'shared' } as any, async (lock) => { /* 读取 */ });\n\n" +
    ...
    _explainFairness() {
        this.setState({ modeInfo: '===== 公平性与饥饿避免 =====\n\n' +
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.request ? 'success' : 'error' }, caps.request ? '可用 ✓' : '不可用'), h(Tag, { color: 'primary' }, 'exclusive / shared'), h(Tag, { color: 'warning' }, '读写锁')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, "默认 exclusive（排他锁，同名同时仅 1 个持有者）；shared（共享锁，多个 shared 可共存，但与 exclusive 互斥）。用法：navigator.locks.request(name, { mode: 'shared' } as any, cb)。典型：shared 用于读（并发）、exclusive 用于写（互斥），类似读写锁。"),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('演示 exclusive 排队', { type: 'primary', size: 'sm', onClick: () => this._demoExclusiveQueue() }), this._btn('演示 shared 共存', { size: 'sm', onClick: () => this._demoSharedCoexist() }), this._btn('读写锁矩阵', { size: 'sm', onClick: () => this._showReadWriteMatrix() }), this._btn('公平性说明', { size: 'sm', onClick: () => this._explainFairness() })),
    ...
    async _demoIfAvailable() {
        ...
        if (!caps.request) {
            ...
            this.setState({ stealInfo: `模拟 ifAvailable（锁名 '${name}'）：\n\n` +
        ...
        try {
            ...
            this.setState({ stealInfo: `真实 ifAvailable（锁名 '${name}'）：\n  ${results.join('\n  ')}\n\n` +
    ...
    async _demoSteal() {
        ...
        if (!caps.request) {
            ...
            this.setState({ stealInfo: `模拟 steal（锁名 '${name}'）：\n\n` +
                    '  持有者 A：navigator.locks.request(name, async (lock, signal) => {\n' +
                    '    // 长任务\n' +
                    '    await longTask();\n' +
                    '    // 若被抢占，signal.aborted === true，应停止工作\n' +
                    '  });\n\n' +
                    '  抢占者 B：navigator.locks.request(name, { steal: true } as any, async (lock) => {\n' +
        ...
        try {
            ...
            this.setState({ stealInfo: `真实 steal（锁名 '${name}'）：\n  ${events.join('\n  ')}\n\n` +
                    '说明：{ steal: true } 抢占当前持有者；被抢占者 callback 第二参数 AbortSignal 收到 aborted=true，应停止工作并让 callback Promise settle（释放锁给抢占者）。' });
        }
        catch (err) {
    ...
    _explainAbortSignal() {
        this.setState({ stealInfo: '===== AbortSignal 详解 =====\n\n' +
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.request ? 'success' : 'error' }, caps.request ? '可用 ✓' : '不可用'), h(Tag, { color: 'primary' }, 'ifAvailable / steal'), h(Tag, { color: 'warning' }, 'AbortSignal')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '{ ifAvailable: true } 立即获取或失败：锁空闲则 callback 接收 Lock，被占用则接收 null（不排队，立即返回）。{ steal: true } 抢占当前持有者：被抢占者 callback 第二参数 AbortSignal 收到 aborted=true。两者互斥（同时设置抛 TypeError）。典型：ifAvailable 用于跨 tab 任务去重，steal 用于高优先级抢占。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('演示 ifAvailable', { type: 'primary', size: 'sm', onClick: () => this._demoIfAvailable() }), this._btn('演示 steal', { danger: true, size: 'sm', onClick: () => this._demoSteal() }), this._btn('AbortSignal 说明', { size: 'sm', onClick: () => this._explainAbortSignal() }), this._btn('抢占语义', { size: 'sm', onClick: () => this._explainStealSemantics() })),
    ...
    async _simulateQuery() {
        ...
        try {
            ...
            this.setState({ queryInfo: `真实 navigator.locks.query() →\n{\n  held: [\n${held.join(',\n') || '    (空)'}\n  ],\n  pending: [\n${pending.join(',\n') || '    (空)'}\n  ]\n}\n\n` +
                    '说明：query() 返回当前 origin 内所有锁的快照（held + pending）。held 是当前持有的锁，pending 是排队等待的锁。' });
        }
        catch (err) {
    ...
    _showHeldStructure() {
        this.setState({ queryInfo: '===== held 结构详解 =====\n\n' +
    ...
    _showPendingStructure() {
        this.setState({ queryInfo: '===== pending 结构详解 =====\n\n' +
                'navigator.locks.query() → Promise<{ held, pending }>\n' +
    ...
    _explainClientIdPrivacy() {
        this.setState({ queryInfo: '===== clientId 隐私说明 =====\n\n' +
                'clientId 是什么：\n' +
    ...
    _renderCard4() {
        ...
        const card = new Card({
            title: '4. query 状态查询',
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.query ? 'success' : 'error' }, caps.query ? 'query ✓' : 'query ✗'), h(Tag, { color: 'primary' }, 'held / pending'), h(Tag, { color: 'warning' }, 'clientId')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'navigator.locks.query() 返回 Promise<{ held, pending }>：held 是当前持有的锁数组，pending 是排队等待的锁数组。每项含 { name, clientId, mode }。clientId 区分 tab/worker（不暴露具体页面）。用途：调试锁争用、监控排队深度、决策是否 steal。返回快照（非实时订阅）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('模拟 query', { type: 'primary', size: 'sm', onClick: () => this._simulateQuery() }), this._btn('held 结构', { size: 'sm', onClick: () => this._showHeldStructure() }), this._btn('pending 结构', { size: 'sm', onClick: () => this._showPendingStructure() }), this._btn('clientId 隐私', { size: 'sm', onClick: () => this._explainClientIdPrivacy() })),
    ...
    async _demoLockObject() {
        ...
        if (!caps.request) {
            this._addLog('lock', `[模拟] request('${name}', cb) → cb({ name:'${name}', mode:'exclusive' })`);
            this.setState({ lockObjInfo: `模拟 Lock 对象（锁名 '${name}'）：\n\n` +
                    '  await navigator.locks.request(name, async (lock) => {\n' +
        ...
        try {
            ...
            this.setState({ lockObjInfo: `真实 Lock 对象（锁名 '${name}'）：\n` +
                    `  lock.name = "${captured.name}"\n` +
                    `  lock.mode = "${captured.mode}"\n` +
                    `  (Object as any).keys(lock) = [${captured.keys.join(', ')}]\n\n` +
    ...
    _explainOnRelease() {
        this.setState({ lockObjInfo: '===== onrelease 说明 =====\n\n' +
    ...
    _explainPersistentLock() {
        this.setState({ lockObjInfo: '===== 持久锁模式 =====\n\n' +
    ...
    _explainPageUnload() {
        this.setState({ lockObjInfo: '===== 页面卸载释放 =====\n\n' +
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.lockMode ? 'success' : 'error' }, caps.lockMode ? 'Lock ✓' : 'Lock ✗'), h(Tag, { color: 'primary' }, 'name / mode'), h(Tag, { color: 'warning' }, '持久锁')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'Lock 对象含 { name, mode } 两个只读属性，无 unlock() 方法（锁不能跨 callback 边界持有）。锁释放触发 onrelease（早期规范，已废弃，现用 finally + AbortSignal 替代）。持久锁模式：callback 内保持 await pending Promise 实现长期持有（长任务、监听器）。页面关闭时浏览器自动释放锁。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('演示 Lock 对象', { type: 'primary', size: 'sm', onClick: () => this._demoLockObject() }), this._btn('onrelease 说明', { size: 'sm', onClick: () => this._explainOnRelease() }), this._btn('持久锁模式', { size: 'sm', onClick: () => this._explainPersistentLock() }), this._btn('页面卸载释放', { size: 'sm', onClick: () => this._explainPageUnload() })),
    ...
    _showScenarios() {
        const scenarios = [
            ...
            {
                name: '跨 tab 任务去重',
                code: "navigator.locks.request('poll-task', { ifAvailable: true } as any, async (lock) => { if (!lock) return; await poll(); });",
                reason: 'ifAvailable 防重入：只有一个 tab 执行任务，其他 tab 跳过',
            },
            {
                name: '轮询任务协调',
                code: "navigator.locks.request('poll', { mode:'shared' } as any, async () => { /* read */ }); query() 监控并发度",
    ...
    _compareWithSharedWorker() {
        this.setState({ scenarioInfo: '===== Web Locks vs SharedWorker 对比 =====\n\n' +
    ...
    _showLimitsAndBestPractices() {
        this.setState({ scenarioInfo: '===== 限制与最佳实践 =====\n\n' +
                '限制：\n' +
    ...
    _renderCard6() {
        ...
        const card = new Card({
            title: '6. 使用场景与组合',
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.locks ? 'success' : 'error' }, caps.locks ? 'WL ✓' : 'WL ✗'), h(Tag, { color: 'primary' }, '4 场景'), h(Tag, { color: 'warning' }, 'vs BC/SW/Atomics')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '场景：IndexedDB 写入串行化（避免读写竞态）、跨 tab 任务去重（ifAvailable 防重入）、轮询任务协调（shared + query）、资源池管理（命名锁表示槽位）。与 BroadcastChannel 对比（广播消息 vs 互斥协调）、与 SharedWorker 对比（状态管理 vs 无状态锁）、与 Atomics 对比（共享内存 vs 命名锁）。限制：同源 + SecureContext + Promise 正确 resolve + 异常自动释放。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('4 场景示例', { type: 'primary', size: 'sm', onClick: () => this._showScenarios() }), this._btn('WL vs BC 对比', { size: 'sm', onClick: () => this._compareWithBroadcastChannel() }), this._btn('WL vs SW 对比', { size: 'sm', onClick: () => this._compareWithSharedWorker() }), this._btn('限制与最佳实践', { size: 'sm', onClick: () => this._showLimitsAndBestPractices() })),
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'api-lab-page web-locks-api-page' }, h('h2', { class: 'section-title' }, 'Web Locks API 实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, '本页深入演示 Web Locks API（navigator.locks）：request 基本用法（callback 模型 + 锁释放时机）、mode exclusive/shared（读写锁矩阵）、ifAvailable/steal（非阻塞尝试 + 强制抢占 + AbortSignal）、query 状态查询（held/pending + clientId）、Lock 对象与 onrelease（持久锁模式 + 页面卸载释放）、使用场景与组合（vs BroadcastChannel/SharedWorker/Atomics + 限制与最佳实践）。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderCard6(), this._renderLogPanel());
    }
}
//# sourceMappingURL=WebLocksAPIPage.js.map