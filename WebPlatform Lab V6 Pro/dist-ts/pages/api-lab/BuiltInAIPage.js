...
export class BuiltInAIPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!caps.translator)
            this._addLog('warn', 'Translator 不可用（Chrome 138+，多数环境需 flag）');
        if (!caps.languageDetector)
            this._addLog('warn', 'LanguageDetector 不可用（Chrome 138+）');
        if (!caps.summarizer)
            this._addLog('warn', 'Summarizer 不可用（Chrome 138+）');
        if (!caps.writer || !caps.rewriter)
            this._addLog('warn', 'Writer / Rewriter 不可用（Chrome 138+）');
        if (!caps.languageModel)
            this._addLog('warn', 'LanguageModel（Prompt API）不可用（Chrome 138+）');
        if (!caps.navigatorMl)
            this._addLog('warn', 'navigator.ml（WebNN）不可用（Chrome 138+，部分平台仅 Origin Trial）');
    }
    ...
    async _checkTranslator() {
        const caps = this._caps();
        if (!caps.translator) {
            this.setState({ translatorInfo: 'Translator API 用法（测试环境不可用，仅说明）：\n\n' +
        ...
        try {
            ...
            this.setState({ translatorInfo: `(Translator as any).availability() = ${JSON.stringify(availability)}\n\n` +
                    'Availability 枚举含义：\n' +
    ...
    async _createAndTranslate() {
        ...
        try {
            ...
            const translator = await Translator.create({
                sourceLanguage: 'en',
                targetLanguage: 'zh',
                monitor: (p) => { this._addLog('trans', `模型下载进度：${typeof p === 'number' ? Math.round(p * 100) + '%' : JSON.stringify(p)}`); },
            });
            ...
            this.setState({ translatorInfo: `Translator.create({ sourceLanguage:'en', targetLanguage:'zh', monitor }) → Translator 实例\n` +
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.translator ? 'success' : 'error' }, caps.translator ? 'Translator ✓' : 'Translator ✗'), h(Tag, { color: 'primary' }, 'Chrome 138+')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'Translator API 提供端侧文本翻译。Translator.create({ sourceLanguage, targetLanguage, monitor }) 返回 Promise<Translator>；实例方法 translator.translate(text) 返回 Promise<string>，translator.destroy() 释放。静态 (Translator as any).availability() 返回 Availability 枚举（available / downloadable / downloading / available-after-download / unavailable）；create 的 monitor 回调接收下载进度。Chrome 138+ 提供，多数环境需在 chrome://flags 开启并下载语言模型。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('检测 availability', { type: 'primary', size: 'sm', disabled: !caps.translator, onClick: () => this._checkTranslator() }), this._btn('创建并翻译', { size: 'sm', disabled: !caps.translator, onClick: () => this._createAndTranslate() })),
                h('div', { class: 'fs-sm text-secondary' }, 'Translator 状态：'),
                h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, s.translatorInfo || '（点击「检测 availability」或「创建并翻译」）')),
                h(Alert, {
                    type: 'warning',
    ...
    async _checkDetector() {
        const caps = this._caps();
        if (!caps.languageDetector) {
            this.setState({ detectorInfo: 'LanguageDetector API 用法（测试环境不可用，仅说明）：\n\n' +
        ...
        try {
            ...
            this.setState({ detectorInfo: `LanguageDetector.availability() = ${JSON.stringify(availability)}\n\n` +
    ...
    async _detectSamples() {
        ...
        try {
            ...
            const lines = results.map((r) => `detect(${JSON.stringify(r.text)}) → ${JSON.stringify(r.detected)}`);
            this.setState({ detectorInfo: `LanguageDetector.create() → detector\n\n` +
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.languageDetector ? 'success' : 'error' }, caps.languageDetector ? 'LanguageDetector ✓' : '不可用'), h(Tag, { color: 'primary' }, '按 confidence 排序')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'LanguageDetector.create() 返回 Promise<LanguageDetector>；detector.detect(text) 返回 Promise<[{ detectedLanguage, confidence }]>，数组按 confidence 降序排列，可能含多个候选语言。静态 LanguageDetector.availability() 返回 Availability。常与 Translator 配合：先用 detector 识别源语言，再创建对应语言对的 Translator。Chrome 138+ 提供。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('检测 availability', { type: 'primary', size: 'sm', disabled: !caps.languageDetector, onClick: () => this._checkDetector() }), this._btn('检测语言样例', { size: 'sm', disabled: !caps.languageDetector, onClick: () => this._detectSamples() })),
                h('div', { class: 'fs-sm text-secondary' }, 'LanguageDetector 状态：'),
                h('pre', { class: 'code-block', style: { maxHeight: '240px', overflow: 'auto' } }, h('code', {}, s.detectorInfo || '（点击「检测 availability」或「检测语言样例」）')),
                h(Alert, {
                    type: 'info',
    ...
    async _checkSummarizer() {
        const caps = this._caps();
        if (!caps.summarizer) {
            this.setState({ summarizerInfo: 'Summarizer / Writer / Rewriter 用法（测试环境不可用，仅说明）：\n\n' +
        ...
        try {
            ...
            this.setState({ summarizerInfo: `Summarizer.availability() = ${JSON.stringify(availability)}\n\n` +
    ...
    async _summarizeSample() {
        ...
        try {
            ...
            const summarizer = await Summarizer.create({
                ...
                monitor: (p) => { this._addLog('sum', `模型下载进度：${typeof p === 'number' ? Math.round(p * 100) + '%' : JSON.stringify(p)}`); },
            });
            ...
            for await (const chunk of await summarizer.summarizeStreaming(sample)) {
                streamed += chunk;
                chunkCount += 1;
            }
            const full = await summarizer.summarize(sample);
            this.setState({ summarizerInfo: `Summarizer.create({ type:'tl;dr', format:'plain-text', length:'short' }) → summarizer\n\n` +
                    `原段落：${sample}\n\n` +
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.summarizer ? 'success' : 'error' }, caps.summarizer ? 'Summarizer ✓' : 'Summarizer ✗'), h(Tag, { color: caps.writer ? 'success' : 'error' }, caps.writer ? 'Writer ✓' : 'Writer ✗'), h(Tag, { color: caps.rewriter ? 'success' : 'error' }, caps.rewriter ? 'Rewriter ✓' : 'Rewriter ✗')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'Summarizer.create({ type, format, length, sharedContext, monitor }) 创建摘要器，type 可为 tl;dr / key-points / teaser / headline，format 为 markdown / plain-text，length 为 short / medium / long。summarizer.summarize(text, { context }) 一次性返回 Promise<string>；summarizer.summarizeStreaming(text) 返回 AsyncIterable<string>，可流式拼接。Writer.create({ sharedContext }) + writer.write(text, { context }) 做生成式写作；Rewriter.create({ sharedContext, tone, format, length }) + rewriter.rewrite(text, { context }) 改写文本，tone 可为 as-is / more-formal / more-casual。Chrome 138+。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('检测 Summarizer', { type: 'primary', size: 'sm', disabled: !caps.summarizer, onClick: () => this._checkSummarizer() }), this._btn('摘要样例（流式）', { size: 'sm', disabled: !caps.summarizer, onClick: () => this._summarizeSample() })),
                h('div', { class: 'fs-sm text-secondary' }, 'Summarizer / Writer / Rewriter 状态：'),
                h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, s.summarizerInfo || '（点击「检测 Summarizer」或「摘要样例」）')),
                h(Alert, {
                    type: 'info',
    ...
    async _checkPrompt() {
        const caps = this._caps();
        if (!caps.languageModel) {
            this.setState({ promptInfo: 'Prompt API（LanguageModel）用法（测试环境不可用，仅说明）：\n\n' +
        ...
        try {
            ...
            this.setState({ promptInfo: `LanguageModel.availability() = ${JSON.stringify(availability)}${paramsLine}\n\n` +
    ...
    async _countTokens() {
        ...
        try {
            ...
            const tokens = await LanguageModel.countPromptTokens(sample);
            this.setState({ promptInfo: (this.state.promptInfo || '') + '\n\n' +
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.languageModel ? 'success' : 'error' }, caps.languageModel ? 'LanguageModel ✓' : '不可用'), h(Tag, { color: 'primary' }, '通用会话')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'LanguageModel（部分浏览器暴露在 window.ai 上）是通用提示词 API。LanguageModel.create({ systemPrompt, initialTemp, topK, monitor }) 返回 Promise<LanguageModel>。实例方法：model.prompt(text) 一次性返回 Promise<string>；model.promptStreaming(text) 返回 ReadableStream 流式输出；model.countPromptTokens(text) 返回 Promise<number> 预估 token；model.destroy() 释放。静态：LanguageModel.availability()、(LanguageModel as any).params() 返回 { defaultTemperature, defaultTopK, maxTemperature, maxTopK }。Chrome 138+。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('检测 availability/params', { type: 'primary', size: 'sm', disabled: !caps.languageModel, onClick: () => this._checkPrompt() }), this._btn('统计 token 数', { size: 'sm', disabled: !caps.languageModel, onClick: () => this._countTokens() })),
                h('div', { class: 'fs-sm text-secondary' }, 'LanguageModel 状态：'),
                h('pre', { class: 'code-block', style: { maxHeight: '240px', overflow: 'auto' } }, h('code', {}, s.promptInfo || '（点击「检测 availability/params」或「统计 token 数」）')),
                h(Alert, {
                    type: 'info',
    ...
    async _aggregateAvailability() {
        const caps = this._caps();
        const check = async (name, ctor) => {
            if (!ctor)
                return `${name}: typeof undefined`;
            try {
        ...
        this.setState({ availabilityInfo: '===== 内置 AI availability 汇总 =====\n\n' +
                lines.join('\n') + aiMgrLine + '\n' + gpuLine + '\n\n' +
                '门控说明：\n' +
        ...
        this._addLog('avail', `汇总完成：${lines.filter((l) => !l.includes('undefined')).length} 项可用`);
    }
    ...
    _explainSession() {
        ...
        this.setState({ availabilityInfo: body +
                '会话管理最佳实践：\n' +
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.navigatorAi || caps.windowAi ? 'success' : 'error' }, caps.navigatorAi || caps.windowAi ? 'navigator.ai ✓' : 'navigator.ai ✗'), h(Tag, { color: caps.navigatorMl ? 'success' : 'error' }, caps.navigatorMl ? 'navigator.ml ✓' : 'navigator.ml ✗'), h(Tag, { color: caps.navigatorGpu ? 'success' : 'error' }, caps.navigatorGpu ? 'gpu ✓' : 'gpu ✗')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'navigator.ai（部分浏览器为 window.ai）是内置 AI 的统一入口，提供 canCreateGenericSession() 与 genericSession.create() 等会话管理。内置 AI 受多重门控：必须 HTTPS（或 localhost）、用户在 chrome://flags 或设置中开启对应 API、首次使用触发模型下载。Availability 枚举：available / downloadable / downloading / available-after-download / unavailable。navigator.gpu（WebGPU，通用计算）与 navigator.ml（WebNN，神经网络专用加速）是两套不同的底层加速通道。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('汇总 availability', { type: 'primary', size: 'sm', onClick: () => this._aggregateAvailability() }), this._btn('会话管理说明', { size: 'sm', onClick: () => this._explainSession() })),
                h('div', { class: 'fs-sm text-secondary' }, '可用性 / 会话管理：'),
                h('pre', { class: 'code-block', style: { maxHeight: '280px', overflow: 'auto' } }, h('code', {}, s.availabilityInfo || '（点击「汇总 availability」或「会话管理说明」）')),
                h(Alert, {
                    type: 'warning',
    ...
    async _detectWebNN() {
        const caps = this._caps();
        if (!caps.navigatorMl) {
            this.setState({ webnnInfo: 'WebNN（Neural Network）API 用法（测试环境不可用，仅说明）：\n\n' +
                    "// 创建上下文（指定设备类型）\n" +
        ...
        try {
            ...
            this.setState({ webnnInfo: `navigator.ml.createContext({ deviceType:'cpu' }) → MLContext\n` +
    ...
    _listWebNNOps() {
        this.setState({ webnnInfo: '===== WebNN MLGraphBuilder 算子清单 =====\n\n' +
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.navigatorMl ? 'success' : 'error' }, caps.navigatorMl ? 'navigator.ml ✓' : 'navigator.ml ✗'), h(Tag, { color: 'primary' }, 'CPU/GPU/NPU')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'navigator.ml（WebNN）提供神经网络专用加速。navigator.ml.createContext({ deviceType: "cpu"|"gpu"|"npu" }) 创建 MLContext；new MLGraphBuilder(context) 构建计算图，用 builder.input(name, { type, dimensions }) / builder.constant(desc, data) 定义输入与常量，用 builder.add/sub/mul/div/matmul/conv2d/relu/softmax/reshape/transpose/gemm/batchNormalization/pooling 等算子组合；builder.build(outputOperand) 编译为 MLGraph；ctx.compute(graph, inputs) 执行得到 outputs。NPU（Neural Processing Unit）是设备内置的神经网络专用加速器，能效比远高于 CPU/GPU。Chrome 138+ 提供，部分平台仅 Origin Trial。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('检测并构建图', { type: 'primary', size: 'sm', disabled: !caps.navigatorMl, onClick: () => this._detectWebNN() }), this._btn('算子与 NPU 说明', { size: 'sm', onClick: () => this._listWebNNOps() })),
                h('div', { class: 'fs-sm text-secondary' }, 'WebNN 状态：'),
                h('pre', { class: 'code-block', style: { maxHeight: '300px', overflow: 'auto' } }, h('code', {}, s.webnnInfo || '（点击「检测并构建图」或「算子与 NPU 说明」）')),
                h(Alert, {
                    type: 'info',
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'api-lab-page builtin-ai-page' }, h('h2', { class: 'section-title' }, '内置 AI 与 WebNN 实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, '本页演示浏览器内置 AI API（Translator / LanguageDetector / Summarizer / Writer / Rewriter / LanguageModel）与 WebNN（navigator.ml）神经网络加速。这些能力为 Chrome 138+ 新特性，需 HTTPS + chrome://flags 开启 + 模型下载；jsdom 环境不可用，所有按钮点击将仅记日志说明用法与支持状态。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderCard6(), this._renderLogPanel());
    }
}
//# sourceMappingURL=BuiltInAIPage.js.map