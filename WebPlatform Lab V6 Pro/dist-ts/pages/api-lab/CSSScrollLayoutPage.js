...
export class CSSScrollLayoutPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!f.scrollSnap)
            this._addLog('warn', 'scroll-snap-type 不可用或 jsdom 未识别（Chrome 69+ / Firefox 68+ / Safari 11+）');
        if (!f.contentVisibility)
            this._addLog('warn', 'content-visibility:auto 不可用或 jsdom 未识别（Chrome 85+）');
        if (!f.scrollbarGutter)
            this._addLog('warn', 'scrollbar-gutter:stable 不可用或 jsdom 未识别（Chrome 94+ / Firefox 97+）');
        if (!f.overflowClip)
            this._addLog('warn', 'overflow:clip 不可用或 jsdom 未识别（Chrome 90+ / Firefox 81+）');
        if (!f.containerType)
            this._addLog('warn', 'container-type:inline-size 不可用或 jsdom 未识别（Chrome 105+ / Firefox 110+ / Safari 16+）');
    ...
    _readSnapInfo() {
        ...
        try {
            ...
            if (stage) {
                ...
                if (firstCard)
                    cardSnapAlign = window.getComputedStyle(firstCard).getPropertyValue('scroll-snap-align') || '(空)';
            }
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['scroll-snap-type', f.scrollSnap], ['scroll-snap-align', f.scrollSnapAlign], ['scroll-snap-stop', f.scrollSnapStop]]), h(Tag, { color: 'primary' }, 'mandatory / proximity')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'CSS Scroll Snap 让滚动容器在滚动结束时自动停在指定的捕捉点。scroll-snap-type: x|y|both [mandatory|proximity] 在容器声明捕捉轴与严格度（mandatory 强制捕捉、proximity 接近时捕捉）；scroll-snap-align: start|center|end|none 在子元素声明对齐点；scroll-snap-stop: always 防止快速滚动跳过捕捉点；scroll-padding 给容器留捕捉偏移（如顶部固定栏遮挡）；scroll-margin 给子元素额外偏移。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取 Snap 信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runScrollSnapDemo() }), this._btn(`切换严格度（当前：${this._snapStrictness}）`, { size: 'sm', disabled: !f.scrollSnap, onClick: () => this._toggleSnapStrictness() })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '横向滚动下方 5 张卡片（真实浏览器中会自动 snap）：'),
                h('div', { class: 'css-snap-stage mt-xs' }, ...colors.map((color, i) => h('div', {
                    class: 'css-snap-card', style: { background: color },
                }, `卡片 ${i + 1}`))),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, s.scrollSnapInfo || '（点击「读取 Snap 信息」）')),
                h(Alert, {
                    type: 'info',
    ...
    _scrollToSection(block, behavior) {
        ...
        try {
            ...
            this.setState({ scrollBehaviorInfo: 'scrollIntoView 演示：\n' +
                    `  ${readBack}\n` +
                    `  目标元素 data-sec="${block}"（${behavior === 'smooth' ? '平滑滚动' : behavior === 'instant' ? '瞬时跳转' : '默认'}）\n\n` +
                    'API 表面：\n' +
                    '  element.scrollIntoView({ behavior: "smooth"|"instant"|"auto", block: "start"|"center"|"end"|"nearest", inline: "start"|"center"|"end"|"nearest" } as any)\n' +
    ...
    _showOverscrollInfo() {
        const f = this._flags();
        this.setState({ scrollBehaviorInfo: 'overscroll-behavior 与 scroll-timeline 说明：\n\n' +
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['scroll-behavior', f.scrollBehavior], ['scrollIntoView', f.scrollIntoView], ['overscroll-behavior', f.overscrollBehavior]]), h(Tag, { color: 'primary' }, 'smooth / contain')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'element.scrollIntoView({ behavior, block, inline }) 让元素滚动到视口；scrollIntoViewIfNeeded() 是 WebKit 私有，仅在不可见时滚动。window.scrollTo({top,left,behavior}) 绝对滚动，scrollBy({left,top}) 相对滚动，element.scroll({top,left}) 容器内滚动。scroll-behavior: smooth 在容器声明后所有程序化滚动与锚点跳转都平滑滚动。overscroll-behavior: contain 阻止滚动链（模态框滚到边界不带动背景）。scroll-timeline 把滚动进度作为动画时间线（详见 CSSAdvancedFeaturesPage）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('滚到 start（smooth）', { type: 'primary', size: 'sm', disabled: !f.scrollIntoView, onClick: () => this._scrollToSection('start', 'smooth') }), this._btn('滚到 center（smooth）', { size: 'sm', disabled: !f.scrollIntoView, onClick: () => this._scrollToSection('center', 'smooth') }), this._btn('滚到 end（instant）', { size: 'sm', disabled: !f.scrollIntoView, onClick: () => this._scrollToSection('end', 'instant') }), this._btn('overscroll 说明', { size: 'sm', onClick: () => this._showOverscrollInfo() })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, 'scroll-behavior: smooth 容器（点击按钮滚动到对应区块）：'),
                h('div', { class: 'css-scroll-stage mt-xs' }, h('div', { class: 'css-scroll-section', style: { background: '#3b82f6' }, 'data-sec': 'start' }, '区块 start'), h('div', { class: 'css-scroll-section', style: { background: '#10b981' }, 'data-sec': 'center' }, '区块 center'), h('div', { class: 'css-scroll-section', style: { background: '#ef4444' }, 'data-sec': 'end' }, '区块 end')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, 'overscroll-behavior: contain 模态框（滚到边界不动外层）：'),
                h('div', { class: 'css-osb-modal mt-xs' }, h('div', { style: { height: '300px' } }, '模态内容（overscroll-behavior: contain）—— 真实浏览器中滚到顶/底不会带动外层页面滚动。')),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '240px', overflow: 'auto' } }, h('code', {}, s.scrollBehaviorInfo || '（点击按钮触发 scrollIntoView 或查看 overscroll 说明）')),
                h(Alert, {
                    type: 'info',
    ...
    _runContentVisibilityDemo() {
        ...
        try {
            ...
            this.setState({ contentVisibilityInfo: 'content-visibility 与 contain-intrinsic-size 演示：\n' +
                    `  .css-cv-item { content-visibility: auto; contain-intrinsic-size: auto 40px; }\n` +
                    `  100 项渲染测量：${renderMs}\n` +
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['content-visibility', f.contentVisibility], ['contain-intrinsic-size', f.containIntrinsicSize], ['contain', f.contain]]), h(Tag, { color: 'primary' }, 'lazy 渲染')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'content-visibility: auto 让浏览器跳过屏外内容的渲染（layout/paint），对长列表/长文档有巨大性能提升。contain-intrinsic-size 提供占位尺寸避免滚动条跳动（auto 前缀让浏览器记住真实尺寸）。contain: layout|paint|style|size 是 containment 原语。content-visibility:hidden 与 display:none 区别：hidden 保留渲染状态，再显示更快；display:none 完全移除需重渲染。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('测量 100 项渲染', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runContentVisibilityDemo() })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '100 项列表（content-visibility:auto; contain-intrinsic-size:auto 40px）：'),
                h('div', { class: 'css-cv-list mt-xs' }, ...items.map((i) => h('div', { class: 'css-cv-item' }, `第 ${i} 项 —— 屏外时跳过渲染，contain-intrinsic-size 占位 40px`))),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, s.contentVisibilityInfo || '（点击「测量 100 项渲染」）')),
                h(Alert, {
                    type: 'warning',
    ...
    _runScrollbarDemo() {
        ...
        try {
            ...
            this.setState({ scrollbarInfo: '自定义滚动条 scrollbar-* 演示：\n' +
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['scrollbar-width', f.scrollbarWidth], ['scrollbar-color', f.scrollbarColor], ['scrollbar-gutter', f.scrollbarGutter]]), h(Tag, { color: 'primary' }, 'thin / stable')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'scrollbar-width: auto|thin|none（Firefox 起源，现已标准化）控制滚动条粗细，none 可隐藏但仍可滚动。scrollbar-color: <thumb> <track> 同时设置滑块与轨道颜色。::-webkit-scrollbar{,-track,-thumb,-button,-corner} 是 WebKit/Blink 旧伪元素，仍可用以细粒度定制。scrollbar-gutter: stable 预留滚动条空间，防止内容从无溢出到溢出时布局抖动。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取滚动条信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runScrollbarDemo() })),
                ...
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, s.scrollbarInfo || '（点击「读取滚动条信息」）')),
                h(Alert, {
                    type: 'info',
    ...
    _runOverflowClipDemo() {
        ...
        try {
            ...
            this.setState({ overflowClipInfo: 'overflow 与 clip 新特性 演示：\n' +
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['overflow:clip', f.overflowClip], ['overflow-clip-margin', f.overflowClipMargin], ['overflow-wrap', f.overflowWrap]]), h(Tag, { color: 'primary' }, 'clip / clip-margin')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'overflow 新增 clip 值：类似 hidden 但不创建滚动容器，无法程序化滚动，开销更低，适合纯裁剪。overflow-clip-margin: <length> 允许内容溢出指定距离再裁剪。overflow-x / overflow-y 可独立设置两轴。text-overflow: clip|ellipsis|<string>（Firefox 支持 string）。overflow-wrap: break-word|anywhere 控制断行（anywhere 影响 min-content 计算，与 word-break:break-all 不同）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取 clip 信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runOverflowClipDemo() })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, 'overflow: hidden vs clip vs clip+margin（内层 200x120，外层 120x80）：'),
                h('div', { class: 'mt-xs' }, h('div', { class: 'css-oc-box css-oc-hidden' }, h('div', { class: 'css-oc-inner' }, 'hidden')), h('div', { class: 'css-oc-box css-oc-clip' }, h('div', { class: 'css-oc-inner' }, 'clip')), h('div', { class: 'css-oc-box css-oc-clip-margin' }, h('div', { class: 'css-oc-inner' }, 'clip+margin 20px'))),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, 'overflow-wrap: anywhere / break-word（长单词 ' + longWord + '）：'),
                h('div', { class: 'mt-xs' }, h('div', { class: 'css-ow-test css-ow-anywhere' }, longWord), h('div', { class: 'css-ow-test css-ow-break' }, longWord), h('div', { class: 'css-ow-test' }, longWord + '（默认 normal）')),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, s.overflowClipInfo || '（点击「读取 clip 信息」）')),
                h(Alert, {
                    type: 'info',
    ...
    _runContainerQueryDemo() {
        ...
        try {
            ...
            this.setState({ containerQueryInfo: '容器查询与布局性能 演示：\n' +
    ...
    _renderCard6() {
        ...
        const card = new Card({
            title: '6. 滚动容器查询与布局性能',
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['container-type', f.containerType], ['container-name', f.containerName], ['@container style', f.containerStyle]]), h(Tag, { color: 'primary' }, '@container')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'container-type: inline-size 声明容器查询维度，container-name 命名容器，@container (min-width) 基于容器尺寸响应（区别于 @media 视口）。@container style(--theme:dark) 样式查询按自定义属性值响应。组合 content-visibility:auto + container queries 可让卡片组件屏外跳过渲染且容器内响应式。性能：will-change: scroll-position 提示滚动优化、contain: layout 隔离布局、overscroll-behavior: contain 防嵌套滚动链；scroll-timeline 滚动驱动动画详见 CSSAdvancedFeaturesPage。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取容器查询信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runContainerQueryDemo() })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '拖动右下角调整容器宽度（container-type: inline-size; container-name: card）：'),
                h('div', { class: 'css-cq-host mt-xs' }, h('div', { class: 'css-cq-widget' }, h('span', { class: 'cq-title' }, '响应式卡片'), h('span', {}, '>280px 横向布局；<200px 纵向紧凑布局'))),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '280px', overflow: 'auto' } }, h('code', {}, s.containerQueryInfo || '（点击「读取容器查询信息」）')),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '150px', overflow: 'auto' } }, h('code', {}, `.card-host { container-type: inline-size; container-name: card; }
@container card (min-width: 280px) {
...
.scroller { contain: layout; overscroll-behavior: contain; will-change: scroll-position; }`)),
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'api-lab-page css-scroll-layout-page' }, h('h2', { class: 'section-title' }, 'CSS 滚动与布局 实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, '本页演示 CSS Scroll Snap、scrollIntoView 与滚动行为、content-visibility 与 contain-intrinsic-size、scrollbar-* 自定义滚动条、overflow:clip 与 clip-margin、容器查询与布局性能。所有特性通过 typeof / CSS.supports 能力检测，不可用时仅记日志，绝不抛异常。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderCard6(), this._renderLogPanel());
    }
}
//# sourceMappingURL=CSSScrollLayoutPage.js.map