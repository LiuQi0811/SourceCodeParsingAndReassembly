...
export class WebShareContactsPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!hasShare)
            this._addLog('warn', 'navigator.share 不可用（需 HTTPS + 用户手势）');
        if (!hasCanShare)
            this._addLog('warn', 'navigator.canShare 不可用（需 HTTPS 环境）');
        if (!hasContacts)
            this._addLog('warn', 'navigator.contacts 不可用（Contact Picker API 仅 Android Chrome 支持）');
        if (!hasSetBadge)
            this._addLog('warn', 'navigator.setAppBadge 不可用（Badging API 需 PWA 安装后有效）');
        if (!hasOtpCred)
            this._addLog('warn', 'OTPCredential 不可用（WebOTP 仅 Android Chrome 支持）');
        if (!hasPwdCred)
            this._addLog('warn', 'PasswordCredential 不可用（Credential Management 需真实浏览器 + HTTPS）');
    }
    componentWillUnmount() {
        // 释放 AbortController 与定时器，避免内存泄漏
        if (this._otpController) {
            try {
                this._otpController.abort();
            }
            catch { /* noop */ }
    ...
    async _shareBasic() {
        ...
        try {
            ...
            this.setState({
                shareInfo: `navigator.share({ title, text, url }) → Promise<void> ✓\n` +
                    `分享数据：\n  title = ${data.title}\n  text = ${data.text}\n  url = ${data.url}\n` +
            ...
            this.setState({
                shareInfo: `navigator.share 失败：${err.name} - ${err.message}\n` +
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.share ? 'success' : 'error' }, caps.share ? 'share ✓' : '不可用'), h(Tag, { color: 'primary' }, 'title / text / url')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'navigator.share({ title, text, url }) → Promise<void> 调用系统原生分享面板，可分享到社交应用、邮件、复制到剪贴板等。必须在用户手势（点击事件）中调用，且需要 HTTPS 环境（localhost 除外）。成功 resolve 表示用户完成分享；失败 reject 中 AbortError 表示用户主动取消（属正常行为，不应作为错误处理）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('分享 title+text+url', { type: 'primary', size: 'sm', onClick: () => this._shareBasic() })),
                h('div', { class: 'fs-sm text-secondary' }, '分享结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '220px', overflow: 'auto' } }, h('code', {}, s.shareInfo || '（点击「分享 title+text+url」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '180px', overflow: 'auto' } }, h('code', {}, `// 必须在点击事件回调中调用（用户手势）
  ...
  try {
    ...
    _canShareBasic() {
        ...
        try {
            ...
            this.setState({
                canShareInfo: `navigator.canShare(data?) → boolean（检测能否分享，不实际触发）\n` +
                    `canShare({ text }) = ${r1}\n` +
                    `canShare({ url }) = ${r2}\n` +
                    `canShare({ title }) = ${r3}\n` +
    ...
    _canShareFiles() {
        ...
        try {
            ...
            this.setState({
                canShareInfo: `navigator.canShare({ files: [...] }) → boolean（Web Share Level 2 文件类型检测）\n` +
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.canShare ? 'success' : 'error' }, caps.canShare ? 'canShare ✓' : '不可用'), h(Tag, { color: 'primary' }, 'Level 2 文件检测')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'navigator.canShare(data?) → boolean 仅检测能否分享，不实际触发分享面板。无参数时返回环境是否支持任意分享；传入 data 时检测特定数据类型是否可分享。Web Share Level 2 中 canShare({ files: [...] }) 可检测文件 MIME 类型是否可分享（不同浏览器支持的类型不同，如 image/png / image/jpeg / text/plain / application/json）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('检测纯文本/URL/标题', { type: 'primary', size: 'sm', disabled: !caps.canShare, onClick: () => this._canShareBasic() }), this._btn('检测文件类型', { type: 'primary', size: 'sm', disabled: !caps.canShare || !caps.file, onClick: () => this._canShareFiles() })),
                h('div', { class: 'fs-sm text-secondary' }, 'canShare 检测结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '220px', overflow: 'auto' } }, h('code', {}, s.canShareInfo || '（点击「检测纯文本/URL/标题」或「检测文件类型」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '160px', overflow: 'auto' } }, h('code', {}, `// 用 canShare 灰掉不可用的分享按钮
  ...
  if (navigator.canShare({ files: [file] })) {
    ...
    async _shareTextFile() {
        ...
        try {
            ...
            this.setState({
                fileShareInfo: `navigator.share({ files: [File], title, text }) → Promise<void> ✓\n` +
    ...
    async _shareImageFile() {
        ...
        try {
            ...
            this.setState({
                fileShareInfo: `navigator.share({ files: [PNG], title, text }) → Promise<void> ✓\n` +
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.share ? 'success' : 'error' }, caps.share ? 'share(files) ✓' : '不可用'), h(Tag, { color: caps.file ? 'primary' : 'warning' }, caps.file ? 'File ✓' : 'File ✗')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'Web Share Level 2 扩展了 navigator.share，支持 data.files: File[] 分享文件。文件可以是 image/png / image/jpeg / text/plain / application/json 等 MIME 类型。data.title / data.text 可与 files 同时传入作为附加信息。File 对象可通过 new File([content], name, { type }) 构造，也可来自 <input type="file"> 或 fetch/blob 转换。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('分享 .txt 文件', { type: 'primary', size: 'sm', disabled: !caps.share || !caps.file, onClick: () => this._shareTextFile() }), this._btn('分享 .png 图片', { type: 'primary', size: 'sm', disabled: !caps.share || !caps.file, onClick: () => this._shareImageFile() })),
                h('div', { class: 'fs-sm text-secondary' }, '文件分享结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '200px', overflow: 'auto' } }, h('code', {}, s.fileShareInfo || '（点击「分享 .txt 文件」或「分享 .png 图片」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '180px', overflow: 'auto' } }, h('code', {}, `// 从 <input type="file"> 获取文件并分享
...
const f = new File(['hi'], 'a.txt', { type: 'text/plain' });`)),
    ...
    async _getContactProperties() {
        ...
        try {
            ...
            this.setState({
                contactsInfo: `navigator.contacts.getProperties() → Promise<string[]> ✓\n` +
    ...
    async _selectContact() {
        ...
        try {
            ...
            this.setState({
                contactsInfo: `navigator.contacts.select(['name','email','tel'], { multiple: false }) → Promise<array> ✓\n` +
    ...
    async _selectMultipleContacts() {
        ...
        try {
            ...
            this.setState({
                contactsInfo: `navigator.contacts.select(['name','email','tel','address','icon'], { multiple: true }) → Promise<array> ✓\n` +
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.contacts ? 'success' : 'error' }, caps.contacts ? 'contacts ✓' : '不可用'), h(Tag, { color: 'primary' }, 'name/email/tel/address/icon')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'navigator.contacts.select(properties, options) → Promise<array> 弹出系统联系人选择器让用户选联系人。properties: ["name","email","tel","address","icon"] 指定要获取的字段；options.multiple: true 允许多选。navigator.contacts.getProperties() → Promise<array> 返回设备支持的属性集合。返回的联系人对象每个属性都是数组（如 { name: ["张三"], tel: ["13800138000"] }）。需 HTTPS + 用户手势，仅 Android Chrome 支持。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('getProperties', { type: 'primary', size: 'sm', disabled: !caps.getProps, onClick: () => this._getContactProperties() }), this._btn('select 单选', { type: 'primary', size: 'sm', disabled: !caps.contacts, onClick: () => this._selectContact() }), this._btn('select 多选', { size: 'sm', disabled: !caps.contacts, onClick: () => this._selectMultipleContacts() })),
                h('div', { class: 'fs-sm text-secondary' }, '联系人选择结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '220px', overflow: 'auto' } }, h('code', {}, s.contactsInfo || '（点击「getProperties」或「select 单选/多选」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '180px', overflow: 'auto' } }, h('code', {}, `// 探测支持的属性
const props = await navigator.contacts.getProperties();
...
// contacts[0].icon[0] 是 base64 data URL`)),
    ...
    async _setBadgeNumber() {
        ...
        try {
            ...
            this.setState({
                badgeInfo: `navigator.setAppBadge(5) → Promise<void> ✓\n` +
    ...
    async _setBadgeDot() {
        ...
        try {
            ...
            this.setState({
                badgeInfo: `navigator.setAppBadge() → Promise<void> ✓\n` +
                    `已设置应用图标角标为通用圆点（无数字）。\n` +
    ...
    async _clearBadge() {
        ...
        try {
            ...
            this.setState({
                badgeInfo: `navigator.clearAppBadge() → Promise<void> ✓\n` +
                    `已清除应用图标角标。\n` +
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.setBadge ? 'success' : 'error' }, caps.setBadge ? 'setAppBadge ✓' : '不可用'), h(Tag, { color: caps.clearBadge ? 'success' : 'error' }, caps.clearBadge ? 'clearAppBadge ✓' : '不可用')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'navigator.setAppBadge(count?) → Promise<void> 在 PWA 应用图标上显示角标（未读消息数）；count 省略则显示通用圆点（无数字）。navigator.clearAppBadge() → Promise<void> 清除角标。用途：邮件/消息应用提醒未读数、待办应用提醒未完成任务。仅在 PWA 安装后有效（普通网页无桌面图标，角标不可见）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('setAppBadge(5)', { type: 'primary', size: 'sm', disabled: !caps.setBadge, onClick: () => this._setBadgeNumber() }), this._btn('setAppBadge() 圆点', { type: 'primary', size: 'sm', disabled: !caps.setBadge, onClick: () => this._setBadgeDot() }), this._btn('clearAppBadge', { danger: true, size: 'sm', disabled: !caps.clearBadge, onClick: () => this._clearBadge() })),
                h('div', { class: 'fs-sm text-secondary' }, '徽章操作结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '180px', overflow: 'auto' } }, h('code', {}, s.badgeInfo || '（点击「setAppBadge」或「clearAppBadge」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '160px', overflow: 'auto' } }, h('code', {}, `// 收到新消息时设置未读数
...
await navigator.setAppBadge();`)),
    ...
    async _receiveOtp() {
        ...
        try {
            ...
            this.setState({
                credInfo: `navigator.credentials.get({ otp: true, signal }) → Promise<OTPCredential> ✓\n` +
            ...
            this.setState({
                credInfo: `navigator.credentials.get({ otp: true }) 失败：${err.name} - ${err.message}\n` +
    ...
    async _createPasswordCredential() {
        ...
        try {
            ...
            this.setState({
                credInfo: `navigator.credentials.create({ password: { id, password } }) → PasswordCredential ✓\n` +
                    `返回凭证：${cred ? cred.constructor.name : 'null'}\n` +
    ...
    async _storeCredential() {
        ...
        try {
            ...
            this.setState({
                credInfo: `navigator.credentials.store(credential) → Promise<Credential> ✓\n` +
    ...
    async _preventSilentAccess() {
        ...
        try {
            ...
            this.setState({
                credInfo: `navigator.credentials.preventSilentAccess() → Promise<void> ✓\n` +
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.credentials ? 'success' : 'error' }, caps.credentials ? 'credentials ✓' : '不可用'), h(Tag, { color: caps.otpCred ? 'primary' : 'warning' }, caps.otpCred ? 'OTPCredential ✓' : 'OTP ✗'), h(Tag, { color: caps.pwdCred ? 'primary' : 'warning' }, caps.pwdCred ? 'PasswordCredential ✓' : 'Pwd ✗')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'WebOTP：navigator.credentials.get({ otp: true }) → Promise<OTPCredential>，返回的 OTPCredential 含 .code（短信验证码）。需 AbortController 取消监听，配合 <input autocomplete="one-time-code"> 自动填充；仅 Android Chrome 支持，短信需含 "@example.com #12345" 格式。Credential Management：create({ password: { id, password } }) 构造 PasswordCredential；store(cred) 存储；get({ password: true }) 取出；preventSilentAccess() 阻止自动登录。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('监听短信 OTP', { type: 'primary', size: 'sm', disabled: !caps.credentials, onClick: () => this._receiveOtp() }), this._btn('取消 OTP 监听', { size: 'sm', onClick: () => this._cancelOtp() }), this._btn('create PasswordCredential', { type: 'primary', size: 'sm', disabled: !caps.credCreate, onClick: () => this._createPasswordCredential() }), this._btn('store 凭证', { size: 'sm', disabled: !caps.credStore || !caps.credCreate, onClick: () => this._storeCredential() }), this._btn('preventSilentAccess', { danger: true, size: 'sm', disabled: !caps.preventSilent, onClick: () => this._preventSilentAccess() })),
                h('div', { class: 'fs-sm text-secondary' }, 'WebOTP / Credential 结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '220px', overflow: 'auto' } }, h('code', {}, s.credInfo || '（点击「监听短信 OTP」或「create/store PasswordCredential」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '220px', overflow: 'auto' } }, h('code', {}, `// WebOTP：监听短信验证码（带超时取消）
const controller = new AbortController();
...
await navigator.credentials.preventSilentAccess();   // 登出后阻止自动登录`)),
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'page api-lab-page' }, h('h2', { class: 'section-title' }, 'Web Share / Contacts / Badging / WebOTP / Credential 实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, '本页演示 Web Share API（基础分享 + 文件分享）、Contact Picker API（联系人选择器）、Badging API（应用图标徽章）、WebOTP（短信验证码自动填充）与 Credential Management API（凭证管理）。这些 API 大多需要 HTTPS + 用户手势，部分仅 PWA 安装后或 Android Chrome 可用。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderCard6(), this._renderLogPanel());
    }
}
//# sourceMappingURL=WebShareContactsPage.js.map