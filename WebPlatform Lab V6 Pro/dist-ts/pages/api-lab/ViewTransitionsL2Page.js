...
export class ViewTransitionsL2Page extends Page {
    ...
    componentDidMount() {
        ...
        if (hasFeaturePolicy) {
            try {
                allowsVT = document.featurePolicy.allowsFeature('view-transition');
            }
            catch { /* noop */ }
        }
        let origin = '';
        ...
        if (hasFeaturePolicy)
            parts.push(`allowsFeature('view-transition') ${allowsVT ? '✓' : '✗'}`);
        ...
        if (!hasL1)
            this._addLog('warn', 'document.startViewTransition 不可用（L1 同文档过渡，需 Chrome 111+）');
        if (!hasPageswap)
            this._addLog('warn', 'pageswap 事件不可用（L2 跨文档，需 Chrome 124+）');
        if (!hasPagereveal)
            this._addLog('warn', 'pagereveal 事件不可用（L2 跨文档，需 Chrome 124+）');
        if (!hasNavigation)
            this._addLog('warn', 'Navigation API 不可用（typeof navigation === "undefined"）');
        if (origin)
            this._addLog('info', `当前 location.origin = ${origin}（同源检测基准：scheme+host+port）`);
    }
    componentWillUnmount() {
        ...
        try {
            detach(window, 'pageswap', this._pageswapHandler);
        }
        catch { /* noop */ }
        try {
            detach(window, 'pagereveal', this._pagerevealHandler);
        }
        catch { /* noop */ }
        try {
            detach(window, 'transitionstart', this._transitionstartHandler);
        }
        catch { /* noop */ }
        try {
            detach(navigation, 'navigate', this._navigateHandler);
        }
        catch { /* noop */ }
    ...
    _showOverview() {
        ...
        this.setState({ overviewInfo: '===== View Transitions L1（同文档）vs L2（跨文档）=====\n\n' +
    ...
    _detectL1() {
        const caps = this._caps();
        if (!caps.l1) {
            this.setState({ overviewInfo: 'L1 同文档 View Transitions 不可用（typeof document.startViewTransition === "undefined"）。\n请用 Chrome 111+ 真实浏览器打开本页。\n\n' +
        ...
        this.setState({ overviewInfo: 'L1 同文档 View Transitions 可用：typeof document.startViewTransition === "function" ✓\n\n' +
    ...
    _showOptIn() {
        this.setState({ overviewInfo: 'L2 跨文档 View Transitions 的 opt-in 方式（两页都需声明）：\n\n' +
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.l1 ? 'success' : 'error' }, caps.l1 ? 'L1 ✓' : 'L1 ✗'), h(Tag, { color: caps.pageswap ? 'success' : 'error' }, caps.pageswap ? 'pageswap ✓' : 'pageswap ✗'), h(Tag, { color: caps.pagereveal ? 'success' : 'error' }, caps.pagereveal ? 'pagereveal ✓' : 'pagereveal ✗'), h(Tag, { color: 'primary' }, '同文档 vs 跨文档')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '同文档（L1）用 document.startViewTransition(callback) 在 SPA 内手动触发过渡，JS callback 控制 DOM swap；跨文档（L2）由浏览器在整页导航（MPA）时自动触发，两页通过 <meta name="view-transition" content="same-origin"> 或 HTTP 头 View-Transition: same-origin 声明启用。L2 要求同源、两页都 opt-in、非下载、未被 Navigation API intercept。架构差异：L1 = JS 控制 DOM swap + 浏览器做动画；L2 = 浏览器控制整页加载 + 动画，JS 仅能通过 pageswap/pagereveal 钩子定制 types。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('L1 vs L2 对比', { type: 'primary', size: 'sm', onClick: () => this._showOverview() }), this._btn('检测 L1 能力', { size: 'sm', onClick: () => this._detectL1() }), this._btn('L2 opt-in 方式', { size: 'sm', onClick: () => this._showOptIn() })),
                h('div', { class: 'fs-sm text-secondary' }, 'L1/L2 概览与能力：'),
                h('pre', { class: 'code-block', style: { maxHeight: '300px', overflow: 'auto' } }, h('code', {}, s.overviewInfo || '（点击「L1 vs L2 对比」或对应按钮）')),
                h(Alert, {
                    type: 'info',
    ...
    _registerPageswap() {
        const caps = this._caps();
        if (!caps.pageswap) {
            this.setState({ pageswapInfo: 'pageswap 事件不可用（"pageswap" in window === false）。\n请用 Chrome 124+ 真实浏览器，并确保两页都 opt-in。\n\n' +
        ...
        try {
            if (this._pageswapHandler && typeof window.removeEventListener === 'function') {
                try {
                    window.removeEventListener('pageswap', this._pageswapHandler);
                }
                catch { /* noop */ }
            }
            ...
            this.setState({ pageswapInfo: '已注册 pageswap 监听器 ✓\n\n' +
    ...
    _showPageswapTypesCode() {
        this.setState({ pageswapInfo: 'pageswap 中定制 types 的代码模板：\n\n  window.addEventListener("pageswap", (event: any) => {\n' +
    ...
    _renderCard2() {
        ...
        const card = new Card({
            title: '2. pageswap 事件',
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.pageswap ? 'success' : 'error' }, caps.pageswap ? 'pageswap ✓' : 'pageswap ✗'), h(Tag, { color: 'primary' }, 'PageSwapEvent / NavigationActivation')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'window.addEventListener("pageswap", (event: any) => {...}) 在旧页"即将被替换"前触发。PageSwapEvent.activation 是 NavigationActivation，含 .navigationType（"push"|"replace"|"reload"|"traverse"）、.from（旧页 URL）、.entry（NavigationHistoryEntry）。PageSwapEvent.viewTransition 是 ViewTransitionTypes（注意：仅 types Set，不是完整 ViewTransition 对象，没有 ready/finished/skipTransition）。通过 event.viewTransition.types.add("forward") 添加自定义类型，配合 CSS :active-view-transition-type() 定制动画。这是唯一能从旧页定制跨文档过渡的钩子。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('注册 pageswap', { type: 'primary', size: 'sm', disabled: !caps.pageswap, onClick: () => this._registerPageswap() }), this._btn('types 代码模板', { size: 'sm', onClick: () => this._showPageswapTypesCode() })),
                h('div', { class: 'fs-sm text-secondary' }, 'pageswap 事件信息：'),
                h('pre', { class: 'code-block', style: { maxHeight: '280px', overflow: 'auto' } }, h('code', {}, s.pageswapInfo || '（点击「注册 pageswap」开始监听，或查看 types 代码模板）')),
                h(Alert, {
                    type: 'warning',
    ...
    _registerPagereveal() {
        const caps = this._caps();
        if (!caps.pagereveal) {
            this.setState({ pagerevealInfo: 'pagereveal 事件不可用（"pagereveal" in window === false）。\n请用 Chrome 124+ 真实浏览器，并确保两页都 opt-in。\n\n' +
        ...
        try {
            if (this._pagerevealHandler && typeof window.removeEventListener === 'function') {
                try {
                    window.removeEventListener('pagereveal', this._pagerevealHandler);
                }
                catch { /* noop */ }
            }
            if (this._transitionstartHandler && typeof window.removeEventListener === 'function') {
                try {
                    window.removeEventListener('transitionstart', this._transitionstartHandler);
                }
                catch { /* noop */ }
            }
            ...
            try {
                window.addEventListener('transitionstart', this._transitionstartHandler);
            }
            catch { /* noop */ }
            this._addLog('pagereveal', '已注册 pagereveal + transitionstart 监听器');
            this.setState({ pagerevealInfo: '已注册 pagereveal + transitionstart 监听器 ✓\n\n' +
    ...
    _showPagerevealCode() {
        this.setState({ pagerevealInfo: 'pagereveal 中操作完整 ViewTransition 的代码模板：\n\n  window.addEventListener("pagereveal", async (event) => {\n' +
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.pagereveal ? 'success' : 'error' }, caps.pagereveal ? 'pagereveal ✓' : 'pagereveal ✗'), h(Tag, { color: 'primary' }, 'PageRevealEvent / 完整 ViewTransition')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'window.addEventListener("pagereveal", (event: any) => {...}) 在新页"首次显示"前（first paint 之前）触发。PageRevealEvent.viewTransition 是完整 ViewTransition 对象（有 .ready/.finished/.updateCallbackDone/.skipTransition()/.types），与 pageswap 的 ViewTransitionTypes 不同。PageRevealEvent.activatedBy 为 "pageswap"（有旧页过渡）或 null（首次加载）。可在此：1) 调用 skipTransition() 跳过动画；2) 修改 types Set；3) await ready 等过渡开始；4) await finished 等过渡完成。时序：pageswap(旧页) → 导航 → pagereveal(新页) → 过渡运行 → finished。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('注册 pagereveal', { type: 'primary', size: 'sm', disabled: !caps.pagereveal, onClick: () => this._registerPagereveal() }), this._btn('代码模板', { size: 'sm', onClick: () => this._showPagerevealCode() })),
                h('div', { class: 'fs-sm text-secondary' }, 'pagereveal 事件信息：'),
                h('pre', { class: 'code-block', style: { maxHeight: '300px', overflow: 'auto' } }, h('code', {}, s.pagerevealInfo || '（点击「注册 pagereveal」开始监听，或查看代码模板）')),
                h(Alert, {
                    type: 'info',
    ...
    _showTypesCss() {
        this.setState({ typesCssInfo: '===== ViewTransition.types 与 CSS 定制 =====\n\n' +
    ...
    _showSetTypesCode() {
        this.setState({ typesCssInfo: '在三种场景中设置 types 的代码：\n\n' +
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.l1 ? 'success' : 'error' }, caps.l1 ? 'L1 types ✓' : 'L1 types ✗'), h(Tag, { color: (caps.pageswap || caps.pagereveal) ? 'success' : 'error' }, (caps.pageswap || caps.pagereveal) ? 'L2 types ✓' : 'L2 types ✗'), h(Tag, { color: 'primary' }, ':active-view-transition-type()')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'types Set 控制 CSS 匹配：在 pageswap（ViewTransitionTypes）、pagereveal（完整 ViewTransition 的 types）或 L1 startViewTransition({ update, types }) 中 add 自定义类型字符串，CSS 用 :active-view-transition-type(forward) ::view-transition-group(root) { animation: ...; } 匹配。@view-transition { navigation: auto; } 是跨文档 opt-in 的 CSS 规则。常见模式：forward 右滑、back 左滑、reload 淡入。::view-transition-group(root) 包裹整体过渡，::view-transition-old/new(root) 是旧/新快照伪元素。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('types 与 CSS 配合', { type: 'primary', size: 'sm', onClick: () => this._showTypesCss() }), this._btn('设置 types 代码', { size: 'sm', onClick: () => this._showSetTypesCode() })),
                h('div', { class: 'fs-sm text-secondary' }, 'types 与 CSS 定制：'),
                h('pre', { class: 'code-block', style: { maxHeight: '320px', overflow: 'auto' } }, h('code', {}, s.typesCssInfo || '（点击「types 与 CSS 配合」或「设置 types 代码」）')),
                h(Alert, {
                    type: 'info',
    ...
    _detectSameOrigin() {
        ...
        this.setState({ sameOriginInfo: '===== 同源与跨源限制 =====\n\n' +
                `${sameOriginCheck}\n\n` +
                '【跨文档 VT 触发条件（全部满足）】\n' +
    ...
    _detectPermissionPolicy() {
        ...
        this.setState({ sameOriginInfo: 'Permission Policy 与 opt-in 构造：\n\n' +
    ...
    _renderCard5() {
        ...
        const card = new Card({
            title: '5. 同源与跨源限制',
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.allowsVT ? 'success' : 'warning' }, caps.allowsVT ? 'allowsFeature ✓' : 'FeaturePolicy n/a'), h(Tag, { color: 'primary' }, 'same-origin / opt-in')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '跨文档 View Transitions 要求：1) 同源（scheme+host+port 完全匹配）；2) 两页都 opt-in（<meta name="view-transition" content="same-origin"> 或 HTTP 头 View-Transition: same-origin 或 CSS @view-transition { navigation: auto; }）；3) 非下载导航；4) 未被 Navigation API event.intercept() 拦截（拦截则降级为 L1）；5) 仅顶层框架，跨源 iframe 内不能使用；6) Permission Policy 允许 view-transition（document.featurePolicy.allowsFeature("view-transition")）。安全模型：同源要求防止泄露跨源页面结构。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('同源检测', { type: 'primary', size: 'sm', onClick: () => this._detectSameOrigin() }), this._btn('Permission Policy', { size: 'sm', onClick: () => this._detectPermissionPolicy() })),
                h('div', { class: 'fs-sm text-secondary' }, '同源与跨源限制：'),
                h('pre', { class: 'code-block', style: { maxHeight: '320px', overflow: 'auto' } }, h('code', {}, s.sameOriginInfo || '（点击「同源检测」或「Permission Policy」）')),
                h(Alert, {
                    type: 'warning',
    ...
    _registerNavigate() {
        const caps = this._caps();
        if (!caps.navigation) {
            this.setState({ navApiInfo: 'Navigation API 不可用（typeof navigation === "undefined"）。\n请用 Chrome 102+ 真实浏览器。\n\n' +
        ...
        try {
            ...
            this.setState({ navApiInfo: '已注册 navigation.navigate 监听器 ✓\n\n' +
    ...
    // 展示 L1/L2 两种路径的代码流程
    _showCodeFlow() {
        this.setState({ navApiInfo: '===== Navigation API 与 View Transitions 协同 =====\n\n' +
                ...
                '  // 旧页：\n  window.addEventListener("pageswap", (e: any) => { if (e.viewTransition) e.viewTransition.types.add("forward"); });\n' +
                '  // 新页：\n  window.addEventListener("pagereveal", (e: any) => { if (e.viewTransition) e.viewTransition.types.add("forward"); });\n' +
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.navigation ? 'success' : 'error' }, caps.navigation ? 'navigation ✓' : 'navigation ✗'), h(Tag, { color: caps.l1 ? 'success' : 'error' }, caps.l1 ? 'L1 ✓' : 'L1 ✗'), h(Tag, { color: 'primary' }, 'intercept / 降级')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '使用 Navigation API navigation.addEventListener("navigate", e => e.intercept({...})) 拦截导航后，导航变为同文档（L1）过渡——可在 intercept handler 内调用 document.startViewTransition({ update, types }) 实现过渡。不 intercept 则执行整页加载，触发跨文档（L2）过渡（需两页 opt-in）。NavigateEvent 的 navigationType（push/replace/reload/traverse）与 destination.url 帮助决定 transition types。跨文档在 pageswap/pagereveal 中加 types；同文档通过 startViewTransition 新重载 { update, types } 传入。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('注册 navigate', { type: 'primary', size: 'sm', disabled: !caps.navigation, onClick: () => this._registerNavigate() }), this._btn('L1/L2 代码流程', { size: 'sm', onClick: () => this._showCodeFlow() })),
                h('div', { class: 'fs-sm text-secondary' }, 'Navigation API 与 VT 协同：'),
                h('pre', { class: 'code-block', style: { maxHeight: '340px', overflow: 'auto' } }, h('code', {}, s.navApiInfo || '（点击「注册 navigate」或「L1/L2 代码流程」）')),
                h(Alert, {
                    type: 'info',
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'page api-lab-page view-transitions-l2-page' }, h('h2', { class: 'section-title' }, '跨文档 View Transitions (L2) 实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, 'View Transitions L2（跨文档）由浏览器在整页导航时自动触发过渡，区别于 L1（同文档，JS 手动 startViewTransition）。本页演示 pageswap/pagereveal 事件、ViewTransition.types 与 CSS :active-view-transition-type() 定制、同源与 opt-in 限制、Navigation API 协同。所有 API 调用前做能力检测，不可用时仅记日志，绝不抛异常。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderCard6(), this._renderLogPanel());
    }
}
//# sourceMappingURL=ViewTransitionsL2Page.js.map