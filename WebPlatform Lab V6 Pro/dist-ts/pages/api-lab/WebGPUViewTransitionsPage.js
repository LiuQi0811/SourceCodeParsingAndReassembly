...
export class WebGPUViewTransitionsPage extends Page {
    ...
    componentDidMount() {
        ...
        this._adapter = null;
        this._device = null; // Card 1 GPUAdapter / GPUDevice
        this._buffer = null;
        this._texture = null;
        this._sampler = null; // Card 2 GPUBuffer/Texture/Sampler
        this._shaderModule = null;
        this._pipeline = null; // Card 2 GPUShaderModule / GPURenderPipeline
        this._bindGroupLayout = null;
        this._bindGroup = null;
        this._errorHandler = null; // Card 2 bindGroup + uncapturederror
        this._transitionEl = null; // Card 4 演示过渡用的容器
        this._popover = null;
        this._popoverToggleHandler = null; // Card 5 popover + toggle 监听
        this._dialog = null;
        this._dialogCloseHandler = null;
        this._dialogCancelHandler = null; // Card 6 dialog + close/cancel
        ...
        if (!hasViewTransition)
            this._addLog('warn', 'document.startViewTransition 不可用（需 Chrome 111+ 真实浏览器）');
        if (!hasPopover)
            this._addLog('warn', 'Popover API 不可用（showPopover 未定义，需 Chrome 114+）');
        if (!hasDialog)
            this._addLog('warn', 'HTMLDialogElement 不可用');
    }
    componentWillUnmount() {
        ...
        const remove = (el) => { if (el && el.parentNode)
    ...
    async _requestAdapter() {
        ...
        try {
            ...
            this.setState({
                adapterInfo: `navigator.gpu.requestAdapter(${JSON.stringify(ADAPTER_OPTIONS)}) → GPUAdapter ✓\n` +
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.gpu ? 'success' : 'error' }, caps.gpu ? 'navigator.gpu ✓' : '不可用'), h(Tag, { color: 'primary' }, 'requestAdapter / requestDevice')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'navigator.gpu 是 WebGPU 入口；gpu.requestAdapter({ powerPreference: "low-power"|"high-performance", forceSoftware }) → Promise<GPUAdapter|null>。adapter.info（GPUAdapterInfo：vendor/architecture/description）描述硬件；adapter.features（GPUFeatureSet，Set-like）列出可用特性；adapter.limits（GPUSupportedLimits：maxTextureDimension2D 等）描述资源上限。adapter.requestDevice(descriptor) → Promise<GPUDevice>，device 是所有资源创建入口，使用后应 destroy()。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('requestAdapter', { type: 'primary', size: 'sm', disabled: !caps.requestAdapter, onClick: () => this._requestAdapter() }), this._btn('requestDevice', { type: 'primary', size: 'sm', disabled: !caps.requestAdapter, onClick: () => this._requestDevice() })),
                h('div', { class: 'fs-sm text-secondary' }, 'adapter / device 信息：'),
                h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, s.adapterInfo || '（点击 requestAdapter / requestDevice）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '160px', overflow: 'auto' } }, h('code', {}, `const adapter = await navigator.gpu.requestAdapter({ powerPreference: 'low-power' });
...
device.addEventListener('uncapturederror', (e: any) => console.error(e.error));
// ... 用完调用 device.destroy()`)),
                h(Alert, {
                    type: 'info',
    ...
    _createBuffer() {
        const caps = this._caps();
        if (!caps.gpu) {
            this._addLog('warn', 'WebGPU 不可用：测试环境不支持，请用真实浏览器');
            return;
        }
        if (!this._device) {
            this._addLog('warn', '请先在 Card 1 点击「requestDevice」');
            return;
        }
        try {
            ...
            this.setState({
                resourceInfo: `device.createBuffer({ size: 24, usage: VERTEX|COPY_DST, label: 'demo-vertex-buffer' }) → GPUBuffer ✓\n` +
    ...
    _createPipeline() {
        ...
        if (!this._device) {
            this._addLog('warn', '请先在 Card 1 点击「requestDevice」，并先创建 shaderModule');
            return;
        }
        if (!this._shaderModule) {
            this._addLog('warn', '请先点击「createShaderModule」');
            return;
        }
        try {
            ...
            let bglLine = '';
            try {
                this._bindGroupLayout = pipeline.getBindGroupLayout(0);
                bglLine = 'pipeline.getBindGroupLayout(0) → GPUBindGroupLayout ✓\n';
            }
            catch (e) {
                bglLine = `getBindGroupLayout(0) 抛错：${e.message}\n`;
            }
            this.setState({
                resourceInfo: (this.state.resourceInfo || '') + '\n\n' +
    ...
    _renderCard2() {
        ...
        const card = new Card({
            title: '2. WebGPU 资源创建',
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.gpu ? 'success' : 'error' }, caps.gpu ? 'Resource ✓' : '不可用'), h(Tag, { color: 'primary' }, 'buffer/texture/sampler/shader/pipeline')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'device.createBuffer({ size, usage }) 创建顶点/索引/Uniform/Storage 缓冲；device.createTexture({ size, format, usage }) 创建可渲染/可采样纹理，texture.createView() 生成 GPUTextureView；device.createSampler({ magFilter, minFilter, addressModeU/V }) 控制采样；device.createShaderModule({ code }) 编译 WGSL；device.createRenderPipeline({ layout:"auto", vertex, fragment, primitive }) 创建不可变渲染管线。usage 用 GPUBufferUsage / GPUTextureUsage 位掩码按位或。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('createBuffer', { type: 'primary', size: 'sm', disabled: !caps.gpu, onClick: () => this._createBuffer() }), this._btn('createTexture', { type: 'primary', size: 'sm', disabled: !caps.gpu, onClick: () => this._createTexture() }), this._btn('createSampler', { size: 'sm', disabled: !caps.gpu, onClick: () => this._createSampler() }), this._btn('createShaderModule', { type: 'primary', size: 'sm', disabled: !caps.gpu, onClick: () => this._createShader() }), this._btn('createRenderPipeline', { type: 'primary', size: 'sm', disabled: !caps.gpu, onClick: () => this._createPipeline() })),
                h('div', { class: 'fs-sm text-secondary' }, '资源创建结果（依次追加）：'),
                h('pre', { class: 'code-block', style: { maxHeight: '280px', overflow: 'auto' } }, h('code', {}, s.resourceInfo || '（依次点击各按钮，建议顺序：buffer → texture → sampler → shader → pipeline）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '演示用的 WGSL（顶点 + 片元）：'),
                h('pre', { class: 'code-block', style: { maxHeight: '180px', overflow: 'auto' } }, h('code', {}, '// 顶点：依据 vertex_index 取三角形三个顶点\n' + VERT_WGSL + '\n// 片元：输出纯色\n' + FRAG_WGSL)),
                h(Alert, {
                    type: 'warning',
    ...
    _encodeAndSubmit() {
        ...
        if (!this._pipeline) {
            this._addLog('warn', '请先在 Card 2 点击「createRenderPipeline」');
            return;
        }
        if (!this._texture) {
            this._addLog('warn', '请先在 Card 2 点击「createTexture」');
            return;
        }
        try {
            ...
            this._device.popErrorScope().then((err) => {
                if (err)
                    this._addLog('warn', `popErrorScope 捕获错误：${err.name} - ${err.message}`);
                else
                    this._addLog('submit', 'popErrorScope resolve(null)：本段编码无 validation 错误');
            }).catch((e) => this._addLog('warn', `popErrorScope reject：${e.message}`));
            this.setState({
                renderInfo: `完整渲染流程（已 submit）：\n` +
    ...
    _writeBuffer() {
        ...
        if (!this._buffer) {
            this._addLog('warn', '请先在 Card 2 点击「createBuffer」');
            return;
        }
        try {
    ...
    _renderCard3() {
        ...
        const card = new Card({
            title: '3. WebGPU 渲染流程',
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.gpu ? 'success' : 'error' }, caps.gpu ? 'Render ✓' : '不可用'), h(Tag, { color: 'primary' }, 'commandEncoder / beginRenderPass / draw / submit')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'device.createCommandEncoder() 创建命令编码器；encoder.beginRenderPass({ colorAttachments: [{ view, loadOp, storeOp, clearValue }] }) 开启渲染通道；passEncoder.setPipeline(pipeline) / setBindGroup(0, bg) / setVertexBuffer(0, buffer) 绑定资源；passEncoder.draw(vertexCount, instanceCount, firstVertex, firstInstance) 发起绘制；passEncoder.end() 结束通道；encoder.finish() → GPUCommandBuffer；device.queue.submit([commandBuffer]) 提交到 GPU。device.pushErrorScope("validation") / popErrorScope() 用于捕获校验错误；device.addEventListener("uncapturederror", ...) 捕获未捕获错误。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('queue.writeBuffer', { type: 'primary', size: 'sm', disabled: !caps.gpu, onClick: () => this._writeBuffer() }), this._btn('编码并 submit', { type: 'primary', size: 'sm', disabled: !caps.gpu, onClick: () => this._encodeAndSubmit() })),
                h('div', { class: 'fs-sm text-secondary' }, '渲染流程结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '280px', overflow: 'auto' } }, h('code', {}, s.renderInfo || '（前置：Card 1 requestDevice → Card 2 createTexture + createShaderModule + createRenderPipeline）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '180px', overflow: 'auto' } }, h('code', {}, `device.pushErrorScope('validation');
...
device.popErrorScope().then(err => err && console.error(err));`)),
    ...
    async _startViewTransition() {
        ...
        if (!caps.viewTransition) {
            ...
            this.setState({
                transitionInfo: `当前环境不支持 View Transitions API（typeof document.startViewTransition === "undefined"）。\n` +
        ...
        try {
            ...
            if (!this._transitionEl) {
                ...
                // 挂到当前页面 DOM（el 已绑定）
                if (this.el)
                    this.el.appendChild(this._transitionEl);
            }
            ...
            transition.finished.then(() => {
                ...
                this.setState({
                    transitionInfo: `document.startViewTransition(callback) → ViewTransition ✓，当前 count = ${now}\n` +
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.viewTransition ? 'success' : 'error' }, caps.viewTransition ? 'startViewTransition ✓' : '不可用'), h(Tag, { color: 'primary' }, 'ready / updateCallbackDone / finished')),
            children: [
                ...
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('startViewTransition', { type: 'primary', size: 'sm', disabled: !caps.viewTransition, onClick: () => this._startViewTransition() })),
                h('div', { class: 'fs-sm text-secondary' }, '过渡时序记录：'),
                h('pre', { class: 'code-block', style: { maxHeight: '220px', overflow: 'auto' } }, h('code', {}, s.transitionInfo || '（点击 startViewTransition，会在下方容器上切换背景色并记录三个 Promise 的时序）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '160px', overflow: 'auto' } }, h('code', {}, `const t = document.startViewTransition(() => {
...
.hero { view-transition-name: hero; }   /* 命名元素独立过渡 */`)),
    ...
    _createPopover() {
        ...
        if (!caps.popover) {
            ...
            this.setState({
                popoverInfo: `当前环境不支持 Popover API（typeof element.showPopover === "function" 为 false）。\n` +
        ...
        try {
            ...
            this.setState({
                popoverInfo: `document.createElement('div') + div.popover = 'auto' ✓，已 append 到 document.body（top layer 渲染）\n` +
    ...
    _showPopover() {
        if (!this._popover) {
            this._addLog('warn', '请先点击「创建 Popover」');
            return;
        }
        if (typeof this._popover.showPopover !== 'function') {
            this._addLog('warn', 'showPopover 不可用');
            return;
        }
        try {
    ...
    _hidePopover() {
        ...
        if (typeof this._popover.hidePopover !== 'function') {
            this._addLog('warn', 'hidePopover 不可用');
            return;
        }
        try {
    ...
    _togglePopover() {
        ...
        if (typeof this._popover.togglePopover !== 'function') {
    ...
    _renderCard5() {
        ...
        const card = new Card({
            title: '5. Popover API',
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.popover ? 'success' : 'error' }, caps.popover ? 'Popover ✓' : '不可用'), h(Tag, { color: 'primary' }, 'show/hide/toggle + beforetoggle/toggle')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'element.popover = "auto" | "manual" | null 声明元素为弹层；element.showPopover() / hidePopover() / togglePopover() 命令式控制。auto 模式点击外部或 Esc 自动关闭、同时只允许一个打开；manual 模式需手动关闭。beforetoggle / toggle 事件携带 newState/oldState（"open"|"closed"）。声明式：button 元素加 popovertarget="id" + popovertargetaction="toggle|show|hide" 即可绑定。:popover-open 伪类匹配已打开的 popover。Popover 渲染在 Top Layer，不受父级 z-index/overflow 影响。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('创建 Popover', { type: 'primary', size: 'sm', disabled: !caps.popover, onClick: () => this._createPopover() }), this._btn('showPopover', { type: 'primary', size: 'sm', disabled: !caps.popover, onClick: () => this._showPopover() }), this._btn('hidePopover', { size: 'sm', disabled: !caps.popover, onClick: () => this._hidePopover() }), this._btn('togglePopover', { type: 'primary', size: 'sm', disabled: !caps.popover, onClick: () => this._togglePopover() })),
                h('div', { class: 'fs-sm text-secondary' }, 'Popover 状态与 toggle 事件流：'),
                h('pre', { class: 'code-block', style: { maxHeight: '200px', overflow: 'auto' } }, h('code', {}, s.popoverInfo || '（点击「创建 Popover」后，再 show/hide/toggle 观察 toggle 事件流）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '160px', overflow: 'auto' } }, h('code', {}, `// 命令式
const pop = document.createElement('div');
pop.popover = 'auto';
document.body.appendChild(pop);
pop.addEventListener('toggle', (e: any) => console.log(e.oldState, '->', e.newState));
...
<div id="my-pop" popover="manual">内容</div>`)),
    ...
    _createDialog() {
        ...
        if (!caps.dialog) {
            ...
            this.setState({
                dialogInfo: `当前环境不支持 HTML <dialog> 元素（typeof HTMLDialogElement === "undefined"）。\n` +
                    `请用现代浏览器打开本页。\n` +
        ...
        try {
            ...
            this.setState({
                dialogInfo: `document.createElement('dialog') ✓（HTMLDialogElement），已 append 到 document.body（top layer 渲染）\n` +
    ...
    _showDialog() {
        if (!this._dialog) {
            this._addLog('warn', '请先点击「创建 dialog」');
            return;
        }
        if (typeof this._dialog.show !== 'function') {
            this._addLog('warn', 'dialog.show 不可用');
            return;
        }
        try {
            this._dialog.show();
    ...
    _showModalDialog() {
        ...
        if (typeof this._dialog.showModal !== 'function') {
            this._addLog('warn', 'dialog.showModal 不可用');
            return;
        }
        try {
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.dialog ? 'success' : 'error' }, caps.dialog ? '<dialog> ✓' : '不可用'), h(Tag, { color: caps.showModal ? 'primary' : 'warning' }, caps.showModal ? 'showModal ✓' : 'showModal ✗')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'document.createElement("dialog") 创建原生对话框（HTMLDialogElement）。dialog.show() 非模态打开（不阻塞、无遮罩）；dialog.showModal() 模态打开（::backdrop 伪元素生成遮罩、焦点陷阱、Esc 触发 cancel 事件）；dialog.close(returnValue) 关闭并把 returnValue 存到 dialog.returnValue；dialog.open 反映当前打开态。close 事件在关闭后触发，cancel 事件在 showModal 时按 Esc 触发（preventDefault 可阻止关闭）。dialog 渲染在 Top Layer，不受父级 z-index/overflow 影响。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('创建 dialog', { type: 'primary', size: 'sm', disabled: !caps.dialog, onClick: () => this._createDialog() }), this._btn('show()', { type: 'primary', size: 'sm', disabled: !caps.dialog, onClick: () => this._showDialog() }), this._btn('showModal()', { type: 'primary', size: 'sm', disabled: !caps.showModal, onClick: () => this._showModalDialog() }), this._btn('close()', { danger: true, size: 'sm', disabled: !caps.dialog, onClick: () => this._closeDialog() })),
                h('div', { class: 'fs-sm text-secondary' }, 'dialog 状态与事件流：'),
                h('pre', { class: 'code-block', style: { maxHeight: '220px', overflow: 'auto' } }, h('code', {}, s.dialogInfo || '（点击「创建 dialog」后，再 show / showModal / close 观察事件流）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '160px', overflow: 'auto' } }, h('code', {}, `const dlg = document.createElement('dialog');
...
dlg.addEventListener('cancel', (e: any) => e.preventDefault());  // 禁用 Esc 关闭
...
/* dialog::backdrop { background: rgba(0,0,0,.5); } */`)),
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'page api-lab-page' }, h('h2', { class: 'section-title' }, 'WebGPU / View Transitions / Popover / <dialog> 实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, '本页演示四类现代 Web API：WebGPU（替代 WebGL 的现代图形 API）、View Transitions（DOM 间动画过渡）、Popover（声明式弹层）、HTML <dialog>（原生对话框）。所有 API 调用前做 typeof 能力检测，不可用时仅记日志说明，绝不抛异常。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderCard6(), this._renderLogPanel());
    }
}
//# sourceMappingURL=WebGPUViewTransitionsPage.js.map