...
export class StreamsPage extends Page {
        ...
        this._onVisChange = () => {
            if (document.visibilityState === 'hidden' && this.state.beaconSupported) {
                try {
                    ...
                    const ok = navigator.sendBeacon('https://httpbin.org/post', new Blob([payload], { type: 'application/json' }));
    ...
    componentWillUnmount() {
        ...
        if (this._simSseCtrl) { // 中止模拟 SSE
            try {
                this._simSseCtrl.abort('page-unmount');
            }
            catch { /* noop */ }
        ...
        if (this._activeReader) { // 关闭可能残留的 stream reader
            try {
                this._activeReader.cancel('page-unmount');
            }
            catch { /* noop */ }
        ...
        if (this._streamTimers) {
            this._streamTimers.forEach((t) => { try {
                clearInterval(t);
            }
            catch { /* noop */ } });
    ...
    async _demoReadLoop() {
        if (typeof ReadableStream === 'undefined')
            return this._addLog('err', 'ReadableStream 不可用');
        try {
    ...
    async _demoAsyncIter() {
        ...
        if (!hasAsyncIter)
            return this._addLog('err', 'ReadableStream 异步迭代不被当前环境支持');
        try {
    ...
    async _runPipeline() {
        ...
        if (this.state.streamRunning)
            return this._addLog('stream', '流水线运行中，请稍候');
            ...
            const source = new ReadableStream({
                start: (controller) => {
                    ...
                    const timer = setInterval(() => {
                        ...
                        try {
                            controller.enqueue(sourceWords[i++]);
                        }
                        catch { /* noop */ }
                    }, 150);
                    if (!this._streamTimers)
        ...
        if (this._abortCtrl) {
            try {
                if (!this._abortCtrl.signal.aborted)
                    this._abortCtrl.abort('page-unmount');
            }
            catch { /* noop */ }
    ...
    async _startSlowFetch() {
        if (typeof AbortController === 'undefined')
            return this._addLog('err', 'AbortController 不可用');
        if (this._abortCtrl)
            return this._addLog('abort', '已有进行中的请求，请先中止或等待');
    ...
    _abortFetch() {
        if (!this._abortCtrl)
            return this._addLog('abort', '当前没有可中止的请求');
        try {
    ...
    async _demoSignalAny() {
        ...
        setTimeout(() => { try {
            ctrl.abort('manual-before-timeout');
        }
        catch { /* noop */ } }, 600);
        try {
            ...
            if (err.name === 'AbortError')
                this._addLog('abort', 'any demo 被 AbortError 中止（来自 combined 信号）');
            else if (err.name === 'TimeoutError')
                this._addLog('abort', 'any demo 被 TimeoutError 中止');
            else
                this._addLog('err', `any demo 失败：${err.name} - ${err.message}`);
        }
    }
    // =================== 3. Beacon API ===================
    _sendBeaconJson() {
        if (!this.state.beaconSupported)
            return this._addLog('err', 'navigator.sendBeacon 不可用');
        try {
    ...
    _connectSse() {
        if (typeof EventSource === 'undefined')
            return this._addLog('err', 'EventSource 不可用');
        if (this._sse) {
    ...
    async _simulateSseViaFetch() {
        if (typeof ReadableStream === 'undefined')
            return this._addLog('err', 'ReadableStream 不可用，无法模拟 SSE');
        if (this._simSseCtrl) {
            this._addLog('sse', '已有进行中的模拟 SSE，先中止');
            try {
                this._simSseCtrl.abort('restart');
            }
            catch { /* noop */ }
            this._simSseCtrl = null;
        }
        ...
        try {
            ...
            if (!resp.ok)
                return this._addLog('err', `fetch /stream/5 失败：HTTP ${resp.status}`);
            if (!resp.body)
                return this._addLog('err', 'resp.body 不可用（浏览器不支持流式响应体）');
            ...
            if (err.name === 'AbortError')
                this._addLog('sse', '模拟 SSE 被中止');
            else
                this._addLog('err', `模拟 SSE 异常：${err.message}`);
        }
        finally {
            this._simSseCtrl = null;
        }
    ...
    _abortSimSse() {
        if (this._simSseCtrl) {
            try {
                this._simSseCtrl.abort('user-stop');
            }
            catch { /* noop */ }
        }
        if (this._activeReader) {
            try {
                this._activeReader.cancel('user-stop');
            }
            catch { /* noop */ }
            this._activeReader = null;
        }
    ...
    renderPage() {
        ...
        return [
            ...
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'ReadableStream 提供可读数据流：通过 getReader().read() 循环或 for await...of 异步迭代消费；tee() 将流分叉为两个；pipeThrough(TransformStream) 应用转换；pipeTo(WritableStream) 写入 sink。下面演示完整流水线。'), !hasStreams
                ? h(Alert, { type: 'warning', message: '当前环境不支持 Streams API（ReadableStream 未定义）' })
                : h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('read 循环', { type: 'primary', size: 'sm', onClick: () => this._demoReadLoop() }), this._btn('for await 迭代', { type: 'primary', size: 'sm', onClick: () => this._demoAsyncIter() }), this._btn('tee() 分叉', { size: 'sm', onClick: () => this._demoTee() }), this._btn('运行完整流水线', { type: 'primary', size: 'sm', onClick: () => this._runPipeline() })), this.state.streamProgress && h('div', { class: 'fs-sm fw-medium' }, this.state.streamProgress))),
            // ============ 2. AbortController / AbortSignal ============
            h(Card, {
            ...
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'new AbortController() 创建控制器，将 controller.signal 传入 fetch(..., { signal }) 即可中止。AbortSignal.timeout(ms) 自动超时；AbortSignal.any([...]) 组合多个信号；signal.aborted / signal.reason 反映中止状态与原因。'), !hasAbort
                ? h(Alert, { type: 'warning', message: '当前环境不支持 AbortController' })
                : h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('启动慢请求', { type: 'primary', size: 'sm', onClick: () => this._startSlowFetch() }), this._btn('中止请求', { danger: true, size: 'sm', onClick: () => this._abortFetch() }), this._btn('AbortSignal.timeout', { size: 'sm', onClick: () => this._demoTimeoutSignal() }), this._btn('AbortSignal.any', { size: 'sm', onClick: () => this._demoSignalAny() })), this.state.abortReason && h('div', { class: 'fs-sm text-secondary' }, `reason: ${this.state.abortReason}`))),
            // ============ 3. Beacon API ============
            h(Card, {
                title: '3. Beacon API（navigator.sendBeacon）',
                extra: h(Tag, { color: 'success' }, `已发送 ${this.state.beaconSent} 次`),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'navigator.sendBeacon(url, data) 是 fire-and-forget 的 POST 投递，专为页面卸载场景设计。可用 Blob 指定 application/json，或用 URLSearchParams 表单编码。与 fetch(..., { keepalive: true }) 相比，sendBeacon 无法读取响应，但卸载时更可靠。'), !hasBeacon
                ? h(Alert, { type: 'warning', message: '当前环境不支持 navigator.sendBeacon' })
                : h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('发送 JSON Beacon', { type: 'primary', size: 'sm', onClick: () => this._sendBeaconJson() }), this._btn('发送表单 Beacon', { type: 'primary', size: 'sm', onClick: () => this._sendBeaconForm() }), this._btn('对比 fetch keepalive', { size: 'sm', onClick: () => this._compareKeepalive() })), h('p', { class: 'fs-sm text-tertiary' }, '提示：页面切到后台（visibilitychange => hidden）会自动发送一条 beacon，演示卸载场景下的可靠投递（详见控制台）。'))),
            // ============ 4. EventSource (SSE) ============
            h(Card, {
                title: '4. EventSource（Server-Sent Events / SSE）',
                extra: h(Tag, { color: this.state.sseState === 'OPEN' ? 'success' : 'default' }, this.state.sseState),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'new EventSource(url) 建立长连接接收 server push。onopen/onmessage/onerror 反映状态流转，readyState 0=CONNECTING / 1=OPEN / 2=CLOSED，close() 主动断开。公共 SSE 端点可能不可达；「模拟 SSE」按钮用 fetch + ReadableStream 解析 data: 行作为兼容降级方案。'), h('div', { class: 'flex items-center gap-sm flex-wrap' }, !hasSse
                ? h(Alert, { type: 'warning', message: '当前环境不支持 EventSource，仍可使用「模拟 SSE」' })
                : this._btn('连接 EventSource', { type: 'primary', size: 'sm', onClick: () => this._connectSse() }), this._btn('关闭 EventSource', { danger: true, size: 'sm', onClick: () => this._closeSse() }), this._btn('模拟 SSE（fetch 流）', { type: 'primary', size: 'sm', onClick: () => this._simulateSseViaFetch() }), this._btn('中止模拟 SSE', { danger: true, size: 'sm', onClick: () => this._abortSimSse() })), this.state.sseMessages.length === 0
                ? h('div', { class: 'fs-sm text-tertiary' }, '（暂无 SSE 消息）')
                : h('div', { class: 'log-panel' }, ...this.state.sseMessages.map((m) => h('div', { class: 'log-panel__line' }, h('span', { class: 'log-panel__time' }, m.time), h('span', { class: 'log-panel__tag log-panel__tag--sse' }, m.type === 'data' ? 'DATA' : 'MSG'), h('span', {}, m.data)))))),
            // ============ 日志面板 ============
            h(Card, {
                title: '事件日志',
...
//# sourceMappingURL=StreamsPage.js.map