...
//      (CSS as any).layoutWorklet.addModule(url) 加载
...
//      (CSS as any).animationWorklet.addModule(url) 加载
...
export class CSSHoudiniWorkletsDeepPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!f.layoutWorklet)
            this._addLog('warn', 'CSS.layoutWorklet 不可用（Layout Worklet 仅 Chromium 实验）');
        if (!f.animationWorklet)
            this._addLog('warn', 'CSS.animationWorklet 不可用（Chrome 85+ / Safari 17+ / Firefox 暂不支持）');
        if (!f.workletAnimation)
            this._addLog('warn', 'WorkletAnimation 不可用（Animation Worklet API）');
        if (!f.scrollTimeline)
            this._addLog('warn', 'ScrollTimeline 不可用（scroll-driven animations 基础）');
...
【三大 Worklet 对照】
  ...
  -----------------|--------------------------------|-----------------|------------------
  Paint Worklet    | (CSS as any).paintWorklet.addModule()   | registerPaint   | paint(name, ...)
  Layout Worklet   | (CSS as any).layoutWorklet.addModule()  | registerLayout  | display: layout(name)
  Animation Worklet| (CSS as any).animationWorklet.addModule()| registerAnimator| new WorkletAnimation(name, ...)
...
【Worklet 生命周期与 addModule 加载机制】
  ...
  async function loadAllWorklets() {
    await Promise.all([
      (CSS as any).paintWorklet.addModule("/worklets/paint.js"),
      (CSS as any).layoutWorklet.addModule("/worklets/layout.js"),
      (CSS as any).animationWorklet.addModule("/worklets/animator.js"),
    ]);
...
async function loadAllWorklets() {
  ...
  console.log('所有 Worklet 加载完成');
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'Houdini 把 CSS 引擎底层暴露给 JS。三大 Worklet（Paint/Layout/Animation）+ Properties & Values API + Typed OM + CSS Parser API + Font Metrics API 全景。Worklet 是轻量 Worker，可被渲染线程调用，无 DOM 访问，独立全局作用域。addModule 异步加载模块，引擎按需实例化。Layout Worklet 仅 Chromium 实验，Animation Worklet Chrome 85+/Safari 17+/Firefox 暂不支持。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 Houdini 概述演示', { type: 'primary', size: 'sm', onClick: () => this._runOverviewDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '240px', overflow: 'auto' } }, h('code', {}, code)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.overviewInfo || '（点击按钮查看 Houdini 概述与 Worklets 体系完整说明）')),
            ],
        });
...
【加载入口：(CSS as any).layoutWorklet.addModule(url)】
  // 主线程
  if (CSS.layoutWorklet) {
    await (CSS as any).layoutWorklet.addModule('/worklets/masonry.js');
  } else {
...
// .box { display: layout(my-layout); }`;
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['layoutWorklet', f.layoutWorklet]]), h(Tag, { color: f.layoutWorklet ? 'success' : 'error' }, '仅 Chromium 实验')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '(CSS as any).layoutWorklet.addModule(url) 加载布局模块，registerLayout(name, class) 注册布局类。类需实现 static get inputProperties() / childInputProperties() / layoutChildren() / async layout(children, edges, constraints, styleMap, breakToken)。display: layout(name) 触发自定义布局，与 Flexbox/Grid 并列的「作者定义布局」能力。Layout Worklet 是 Houdini 最具想象力但浏览器支持最弱的部分，仅 Chromium 实验。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 registerLayout 概述演示', { type: 'primary', size: 'sm', onClick: () => this._runLayoutBasicDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '300px', overflow: 'auto' } }, h('code', {}, code)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.layoutBasicInfo || '（点击按钮查看 registerLayout 完整说明）')),
            ],
        });
        this.registerChild(card);
        return card.render();
...
// }`;
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['masonry', f.layoutWorklet]]), h(Tag, { color: 'primary' }, '实战')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'masonry 布局实现：列宽固定、元素按高度分配到最短列。registerLayout("masonry", class) 完整代码示例，inputProperties: ["--column-count", "--gap"]，children.map(c => c.layoutNextFragment({fixedInlineSize})) 生成片段。对比 CSS Grid Level 3 masonry 提案（grid-template-rows: masonry）。其他实战：circle 圆形布局、flow 流式布局、自定义对齐。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 masonry 瀑布流演示', { type: 'primary', size: 'sm', onClick: () => this._runLayoutMasonryDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, 'masonry 视觉 mockup（CSS multi-column 模拟，真实 Worklet 不在 jsdom 运行）：'),
                h('div', { class: 'hw-masonry-viz' }, h('div', { class: 'hw-masonry-item hw-masonry-item--a' }, '1'), h('div', { class: 'hw-masonry-item hw-masonry-item--b' }, '2'), h('div', { class: 'hw-masonry-item hw-masonry-item--c' }, '3'), h('div', { class: 'hw-masonry-item hw-masonry-item--d' }, '4'), h('div', { class: 'hw-masonry-item hw-masonry-item--e' }, '5'), h('div', { class: 'hw-masonry-item hw-masonry-item--f' }, '6')),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '320px', overflow: 'auto' } }, h('code', {}, code)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.layoutMasonryInfo || '（点击按钮查看 masonry 瀑布流完整实现）')),
            ],
        });
        this.registerChild(card);
        return card.render();
...
【加载入口：(CSS as any).animationWorklet.addModule(url)】
  // 主线程
  if (CSS.animationWorklet) {
    await (CSS as any).animationWorklet.addModule('/worklets/animator.js');
  } else {
...
// WorkletAnimation:   跑在合成线程，不受主线程阻塞，60fps 稳定`;
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '(CSS as any).animationWorklet.addModule(url) 加载动画模块，registerAnimator(name, class) 注册动画类。类需实现 static get inputProperties() / outputProperties() / constructor(options) / animate(currentTime, effect)。new WorkletAnimation(name, keyframes, timeline, options) 创建实例。element.animate() 跑主线程受阻塞，WorkletAnimation 跑合成线程不受主线程阻塞。Animation Worklet 解决「输入驱动的滚动动画」问题。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 registerAnimator 概述演示', { type: 'primary', size: 'sm', onClick: () => this._runAnimBasicDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '320px', overflow: 'auto' } }, h('code', {}, code)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.animBasicInfo || '（点击按钮查看 registerAnimator 完整说明）')),
            ],
        });
        this.registerChild(card);
        return card.render();
...
// 因为 animate() 跑在独立合成线程`;
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'animate(currentTime, effect) 参数：currentTime 当前时间线进度，effect WorkletAnimationEffect（含 localTime/progress/currentIteration）。在 animate 内修改 effect.localTime 或直接 set output properties。与 ScrollTimeline/ViewTimeline 协同（scroll-driven animations 跑在 compositor）。状态保持（stateful animator 通过 constructor options 与实例属性）。性能优势：60fps 即使 main thread 卡顿。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 animate() 方法深潜演示', { type: 'primary', size: 'sm', onClick: () => this._runAnimMethodDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '时间线对照可视化：'),
                h('div', { class: 'hw-timeline-viz' }, h('div', { class: 'hw-timeline-row' }, h('span', {}, 'DocumentTimeline'), h('div', { class: 'hw-timeline-bar' }, h('div', { class: 'hw-timeline-fill', style: { width: '35%' } }))), h('div', { class: 'hw-timeline-row' }, h('span', {}, 'ScrollTimeline'), h('div', { class: 'hw-timeline-bar' }, h('div', { class: 'hw-timeline-fill', style: { width: '60%' } }))), h('div', { class: 'hw-timeline-row' }, h('span', {}, 'ViewTimeline  '), h('div', { class: 'hw-timeline-bar' }, h('div', { class: 'hw-timeline-fill', style: { width: '45%' } })))),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '320px', overflow: 'auto' } }, h('code', {}, code)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.animMethodInfo || '（点击按钮查看 animate() 方法完整说明）')),
            ],
        });
        this.registerChild(card);
        return card.render();
...
【主线程：创建视差 WorkletAnimation】
  // 1. 加载 animator 模块
  await (CSS as any).animationWorklet.addModule('/worklets/parallax.js');
...
// @keyframes parallax { from { transform: translateY(0); } to { transform: translateY(-200px); } }`;
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '视差滚动实现：多层背景以不同速率移动。registerAnimator("parallax", class) 完整代码，static get inputProperties() return ["--parallax-rate"]，animate(currentTime, effect) 中 effect.localTime = currentTime * rate。new WorkletAnimation("parallax", keyframes, new ScrollTimeline({...}), { rate: 0.5 })。对比 CSS scroll-driven animations（CSS 声明式 vs JS Worklet 可编程）。其他实战：sticky header 渐显、scroll-linked progress bar、自定义缓动。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行视差滚动演示', { type: 'primary', size: 'sm', onClick: () => this._runAnimParallaxDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, 'parallax 视觉 mockup（三层不同速率，真实 Worklet 不在 jsdom 运行）：'),
                h('div', { class: 'hw-parallax-viz' }, h('div', { class: 'hw-parallax-layer hw-parallax-layer--back' }, h('span', {}, '远景层 rate=0.2'), h('span', { class: 'hw-parallax-rate' }, 'slow')), h('div', { class: 'hw-parallax-layer hw-parallax-layer--mid' }, h('span', {}, '中景层 rate=0.5'), h('span', { class: 'hw-parallax-rate' }, 'mid')), h('div', { class: 'hw-parallax-layer hw-parallax-layer--front' }, h('span', {}, '近景层 rate=1.0'), h('span', { class: 'hw-parallax-rate' }, 'fast'))),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '320px', overflow: 'auto' } }, h('code', {}, code)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.animParallaxInfo || '（点击按钮视差滚动完整实现）')),
            ],
        });
        this.registerChild(card);
        return card.render();
...
【3. addModule 是 Promise 异步加载】
  ...
  // 错误
  (CSS as any).animationWorklet.addModule("/worklets/a.js");
  new WorkletAnimation("a", ...);   // 可能未注册，失败

  // 正确
  await (CSS as any).animationWorklet.addModule("/worklets/a.js");
  ...
  开发时修改 Worklet 模块后需重新 addModule（不会自动重载）：
    ...
    if (module.hot) {
      module.hot.accept("/worklets/a.js", () => {
        // 重新加载（部分浏览器支持覆盖注册）
        (CSS as any).animationWorklet.addModule("/worklets/a.js?t=" + Date.now());
      });
    }
...
【6. 陷阱清单（完整版）】
  ...
  降级策略：
    ...
    - JS 能力检测
      if (CSS.animationWorklet) {
        await (CSS as any).animationWorklet.addModule(...);
...
// - 与现有 CSS 布局协同（display: layout() 替换默认布局）`;
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'DevTools 不直接支持 Worklet 调试（需 chrome://inspect/#worklets）。Worklet 代码运行在独立上下文（无 window/document/localStorage）。addModule 是 Promise 异步加载。错误处理（worklet 内 throw 不冒泡到 main）。模块热替换。陷阱清单：Layout Worklet 浏览器支持极弱仅 Chromium 实验性/Animation Worklet Chrome 85+/Safari 17+/Firefox 暂不支持/Worklet 内不能访问 DOM/不能 console.log 到主页面控制台/需 HTTPS 或 localhost/布局重计算性能开销/与现有 CSS 布局协同/降级策略（@supports 检测 display: layout()）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 Worklet 调试与陷阱演示', { type: 'primary', size: 'sm', onClick: () => this._runDebugDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '320px', overflow: 'auto' } }, h('code', {}, code)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.debugInfo || '（点击按钮查看 Worklet 调试与陷阱完整清单）')),
            ],
        });
        this.registerChild(card);
        return card.render();
...
//# sourceMappingURL=CSSHoudiniWorkletsDeepPage.js.map