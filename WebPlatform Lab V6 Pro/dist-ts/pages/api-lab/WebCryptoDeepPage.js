...
export class WebCryptoDeepPage extends Page {
    ...
    _wrapIv = null;
    _wrappedBuf = null;
    _wrappedKeyAlgo = null;
    _wrappedKeyUsages = null;
    _wrappingKey = null;
    // —— 初始 state ——
    initialState() {
        return {
    ...
    componentDidMount() {
        ...
        this._addLog(supported ? 'info' : 'warn', `能力检测：crypto.subtle ${supported ? '可用 ✓' : '不可用 ✗'}；TextEncoder ${typeof TextEncoder !== 'undefined' ? '✓' : '✗'}；getRandomValues ${typeof crypto !== 'undefined' && typeof crypto.getRandomValues === 'function' ? '✓' : '✗'}`);
    }
    ...
    async _aesEncrypt() {
        ...
        try {
            ...
            const cipherBuf = await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, this._aesKey, data);
            this._aesIv = iv;
    ...
    async _aesDecrypt() {
        ...
        try {
            const plainBuf = await crypto.subtle.decrypt({ name: 'AES-GCM', iv: this._aesIv }, this._aesKey, this._aesCipherBuf);
    ...
    async _generateRsaKey() {
        ...
        try {
            ...
            const cipherBuf = await crypto.subtle.encrypt({ name: 'RSA-OAEP' }, keyPair.publicKey, msg);
    ...
    async _ecdsaSign() {
        ...
        try {
            ...
            const data = new TextEncoder().encode(text);
            const sigBuf = await crypto.subtle.sign({ name: 'ECDSA', hash: 'SHA-256' }, this._ecdsaKeyPair.privateKey, data);
    ...
    async _ecdsaVerify(tampered = false) {
        ...
        try {
            ...
            const sigBytes = new Uint8Array(this._ecdsaSig.match(/.{2}/g).map((h) => parseInt(h, 16)));
            ...
            const ok = await crypto.subtle.verify({ name: 'ECDSA', hash: 'SHA-256' }, this._ecdsaKeyPair.publicKey, sigBytes, data);
            this.setState({ verifyResult: ok ? '通过 ✓' : '失败 ✗' });
            this._addLog('verify', `验证结果：${ok ? '通过 ✓' : '失败 ✗'}${tampered ? '（篡改后应失败）' : '（原文应通过）'}`);
        }
        catch (err) {
    ...
    async _deriveKeyPbkdf2() {
        ...
        try {
            ...
            const baseKey = await crypto.subtle.importKey('raw', new TextEncoder().encode(password), { name: 'PBKDF2' }, false, ['deriveKey', 'deriveBits']);
    ...
    async _deriveKeyHkdf() {
        ...
        try {
            ...
            const baseKey = await crypto.subtle.importKey('raw', ikm, { name: 'HKDF' }, false, ['deriveKey', 'deriveBits']);
            const derived = await crypto.subtle.deriveKey({ name: 'HKDF', hash: 'SHA-256', salt, info }, baseKey, { name: 'AES-GCM', length: 256 }, true, ['encrypt', 'decrypt']);
    ...
    async _importJwk() {
        ...
        try {
            const key = await crypto.subtle.importKey('jwk', this._lastJwk, { name: 'AES-GCM', length: 256 }, true, ['encrypt', 'decrypt']);
    ...
    async _wrapKey() {
        ...
        try {
            ...
            const ecdsaKeyPair = await crypto.subtle.generateKey({ name: 'ECDSA', namedCurve: 'P-256' }, true, ['sign', 'verify']);
            // 3. 用 AES-GCM 封装 ECDSA 私钥（format='pkcs8'，对应私钥；'spki' 对应公钥）
            const iv = crypto.getRandomValues(new Uint8Array(12));
            const wrapped = await crypto.subtle.wrapKey('pkcs8', ecdsaKeyPair.privateKey, wrappingKey, { name: 'AES-GCM', iv });
    ...
    async _unwrapKey() {
        ...
        try {
            const unwrapped = await crypto.subtle.unwrapKey('pkcs8', this._wrappedBuf, this._wrappingKey, { name: 'AES-GCM', iv: this._wrapIv }, this._wrappedKeyAlgo, true, this._wrappedKeyUsages);
    ...
    _renderCard1() {
        ...
        const card = new Card({
            title: '1. 哈希摘要 (digest)',
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.subtle ? 'success' : 'warning' }, caps.subtle ? 'SubtleCrypto 可用' : 'SubtleCrypto 不可用'), h(Tag, { color: 'primary' }, 'SHA-1/256/384/512')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'crypto.subtle.digest(algo, data) 返回 Promise<ArrayBuffer>；输入需为 BufferSource（用 new TextEncoder().encode(str) 转换）。SHA-1 输出 160 位、SHA-256 输出 256 位、SHA-384 输出 384 位、SHA-512 输出 512 位。所有算法返回原始比特，常用 bufToHex 转十六进制展示。digest 是单向不可逆函数，常用于完整性校验、内容寻址、密码存储（需加盐）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('计算 SHA-256("hello")', { type: 'primary', size: 'sm', disabled: !caps.subtle, onClick: () => this._computeSHA256() }), this._btn('对比 4 种算法输出长度', { size: 'sm', disabled: !caps.subtle, onClick: () => this._compareDigests() })),
                h('div', { class: 'fs-sm text-secondary' }, 'SHA-256("hello") 十六进制（256 位 = 64 hex 字符）：'),
                h('pre', { class: 'code-block', style: { maxHeight: '60px', overflow: 'auto' } }, h('code', {}, s.digestResult || '（点击「计算 SHA-256("hello")」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '4 种算法输出长度对比（SHA-1=160 / SHA-256=256 / SHA-384=384 / SHA-512=512 位）：'),
                h('pre', { class: 'code-block', style: { maxHeight: '180px', overflow: 'auto' } }, h('code', {}, s.digestCompare || '（点击「对比 4 种算法输出长度」）')),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.subtle ? 'success' : 'warning' }, caps.subtle ? '可用' : '不可用'), h(Tag, { color: 'primary' }, 'AES-GCM 256'), h(Tag, { color: 'warning' }, 'AEAD')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'crypto.subtle.generateKey({ name:\'AES-GCM\', length:256 }, true, [\'encrypt\',\'decrypt\']) 生成对称密钥；encrypt({ name:\'AES-GCM\', iv }, key, data) 返回的密文末尾自带 16 字节 GCM auth tag（AEAD 同时保证机密性与完整性）；iv 推荐 12 字节（96 位），用 crypto.getRandomValues!(new Uint8Array(12)) 生成，每次加密必须随机且不重复；decrypt 用相同 iv + key 还原，tag 校验失败会抛 OperationError。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('生成 AES 密钥', { type: 'primary', size: 'sm', disabled: !caps.subtle, onClick: () => this._generateAesKey() }), this._btn('加密 "Hello AES-GCM"', { type: 'primary', size: 'sm', disabled: !caps.subtle, onClick: () => this._aesEncrypt() }), this._btn('解密还原', { size: 'sm', disabled: !caps.subtle, onClick: () => this._aesDecrypt() })),
                h('div', { class: 'fs-sm text-secondary' }, '密文（十六进制，末 16 字节为 GCM auth tag）：'),
                h('pre', { class: 'code-block', style: { maxHeight: '80px', overflow: 'auto' } }, h('code', {}, s.aesCipher || '（点击「生成 AES 密钥」→「加密」）')),
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.subtle ? 'success' : 'warning' }, caps.subtle ? '可用' : '不可用'), h(Tag, { color: 'primary' }, 'RSA-OAEP 2048'), h(Tag, { color: 'primary' }, 'ECDSA P-256')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'RSA-OAEP：generateKey({ name:\'RSA-OAEP\', modulusLength:2048, publicExponent:new Uint8Array([1,0,1]) /* 65537 */, hash:\'SHA-256\' }, true, [\'encrypt\',\'decrypt\']) 生成密钥对；encrypt(publicKey) 加密、decrypt(privateKey) 解密。ECDSA：generateKey({ name:\'ECDSA\', namedCurve:\'P-256\' }, true, [\'sign\',\'verify\']) 生成密钥对；sign({ name:\'ECDSA\', hash:\'SHA-256\' }, privateKey, data) 签名；verify(..., publicKey, signature, data) 返回 boolean。RSA 加密适合短数据（≤ modulusLength/8 - padding），长数据应改用混合加密（RSA 包 AES 密钥）。'),
                h('div', { class: 'fs-sm text-secondary' }, 'RSA-OAEP：'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('生成 RSA 密钥并加解密', { type: 'primary', size: 'sm', disabled: !caps.subtle, onClick: () => this._generateRsaKey() }), h(Tag, { color: s.rsaPubJwk ? 'success' : 'default' }, s.rsaPubJwk ? '已生成' : '未生成')),
                s.rsaPubJwk && h('pre', { class: 'code-block', style: { maxHeight: '80px', overflow: 'auto' } }, h('code', {}, `RSA 公钥 JWK 摘要：${s.rsaPubJwk}`)),
                h('div', { class: 'fs-sm text-secondary' }, `RSA 解密还原："Hello RSA-OAEP" → ${s.rsaDecrypt || '—'}`),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, 'ECDSA 签名 / 验证：'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('生成 ECDSA 密钥', { type: 'primary', size: 'sm', disabled: !caps.subtle, onClick: () => this._generateEcdsaKey() }), this._btn('签名', { type: 'primary', size: 'sm', disabled: !caps.subtle, onClick: () => this._ecdsaSign() }), this._btn('验证', { size: 'sm', disabled: !caps.subtle, onClick: () => this._ecdsaVerify(false) }), this._btn('篡改数据后验证', { size: 'sm', disabled: !caps.subtle, onClick: () => this._ecdsaVerify(true) })),
                h('div', { class: 'fs-sm text-secondary' }, 'ECDSA 签名（hex）：'),
                h('pre', { class: 'code-block', style: { maxHeight: '60px', overflow: 'auto' } }, h('code', {}, s.ecdsaSig || '（点击「生成 ECDSA 密钥」→「签名」）')),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, h(Tag, { color: s.verifyResult.startsWith('通过') ? 'success' : s.verifyResult ? 'error' : 'default' }, `验证结果：${s.verifyResult || '未验证'}`)),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.subtle ? 'success' : 'warning' }, caps.subtle ? '可用' : '不可用'), h(Tag, { color: 'primary' }, 'PBKDF2'), h(Tag, { color: 'primary' }, 'HKDF'), h(Tag, { color: 'warning' }, 'JWK')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'PBKDF2：importKey(\'raw\', passwordBytes, { name:\'PBKDF2\' }, false, [\'deriveKey\',\'deriveBits\']) 把密码包装为 baseKey；deriveKey({ name:\'PBKDF2\', salt, iterations:100000, hash:\'SHA-256\' }, baseKey, { name:\'AES-GCM\', length:256 }, true, [...]) 派生 AES 密钥；deriveBits 派生原始比特。HKDF：importKey raw + deriveKey { name:\'HKDF\', hash:\'SHA-256\', salt, info }，单轮派生（比 PBKDF2 快，不适合低熵密码）。exportKey(\'jwk\'|\'raw\'|\'pkcs8\'|\'spki\', key) 导出；importKey(\'jwk\'|\'raw\', ...) 反向导入。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('从密码派生 AES 密钥（100000 次迭代）', { type: 'primary', size: 'sm', disabled: !caps.subtle, onClick: () => this._deriveKeyPbkdf2() }), this._btn('HKDF 派生（对比）', { size: 'sm', disabled: !caps.subtle, onClick: () => this._deriveKeyHkdf() }), this._btn('导出密钥为 JWK', { size: 'sm', disabled: !caps.subtle, onClick: () => this._exportJwk() }), this._btn('从 JWK 导入', { size: 'sm', disabled: !caps.subtle, onClick: () => this._importJwk() })),
                s.deriveTiming && h('div', { class: 'fs-sm fw-medium' }, s.deriveTiming),
                h('div', { class: 'fs-sm text-secondary' }, '派生 / 导出的 AES-GCM 密钥 JWK：'),
                h('pre', { class: 'code-block', style: { maxHeight: '100px', overflow: 'auto' } }, h('code', {}, s.deriveKeyJwk || '（点击「从密码派生 AES 密钥」）')),
                h(Alert, {
                    type: 'warning',
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.subtle ? 'success' : 'warning' }, caps.subtle ? '可用' : '不可用'), h(Tag, { color: 'primary' }, 'wrapKey/unwrapKey')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'crypto.subtle.wrapKey(format, key, wrappingKey, wrapAlgo) 把一个密钥用 wrappingKey 加密导出；format 取 \'raw\'（对称/EC 公钥）/ \'pkcs8\'（私钥）/ \'spki\'（公钥）。unwrapKey(format, wrappedKey, unwrappingKey, unwrapAlgo, unwrappedKeyAlgo, extractable, usages) 反向解密并导入。常用于跨设备传输私钥（先用 RSA/AES 协商 wrappingKey，再封装业务密钥）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('用 AES-GCM 封装 ECDSA 私钥', { type: 'primary', size: 'sm', disabled: !caps.subtle, onClick: () => this._wrapKey() }), this._btn('解封装还原', { size: 'sm', disabled: !caps.subtle, onClick: () => this._unwrapKey() }), this._btn('把能力表输出到日志', { size: 'sm', onClick: () => this._logAlgoCaps() })),
                h('div', { class: 'fs-sm text-secondary' }, '封装结果（pkcs8 + AES-GCM，hex）：'),
                h('pre', { class: 'code-block', style: { maxHeight: '80px', overflow: 'auto' } }, h('code', {}, s.wrapResult || '（点击「用 AES-GCM 封装 ECDSA 私钥」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '算法能力汇总表（算法 | 用途 | 密钥长度 | 支持的操作）：'),
                h('div', { class: 'table-wrap', style: { overflowX: 'auto' } }, h('table', { class: 'data-table', style: { width: '100%', borderCollapse: 'collapse', fontSize: '12px' } }, h('thead', {}, h('tr', {}, h('th', { style: thStyle }, '算法'), h('th', { style: thStyle }, '用途'), h('th', { style: thStyle }, '密钥长度'), h('th', { style: thStyle }, '支持的操作'))), h('tbody', {}, ...ALGO_CAPS_TABLE.map((r) => h('tr', {}, h('td', { style: tdStyle }, h('code', {}, r.algo)), h('td', { style: tdStyle }, r.use), h('td', { style: tdStyle }, r.keylen), h('td', { style: tdStyle }, r.ops)))))),
                h(Alert, {
                    type: 'info',
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'page api-lab-page' }, h('h2', { class: 'section-title' }, 'Web Crypto 加密实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, '演示 SubtleCrypto 全套：哈希摘要 / 对称加密 / 非对称签名 / 密钥派生 / 密钥封装。所有操作在浏览器内完成，私钥不离开设备。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderLogPanel());
    }
}
...
//# sourceMappingURL=WebCryptoDeepPage.js.map