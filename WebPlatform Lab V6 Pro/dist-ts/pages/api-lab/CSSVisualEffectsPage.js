...
export class CSSVisualEffectsPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!caps.aspectRatio)
            this._addLog('warn', 'aspect-ratio: 16/9 不可用或 jsdom 未识别（Chrome 88+ / Firefox 89+ / Safari 15+）');
        if (!caps.shapeOutside)
            this._addLog('warn', 'shape-outside: circle(50%) 不可用或 jsdom 未识别（Chrome 37+ / Firefox 62+ / Safari 11.1+）');
        if (!caps.backdropFilter)
            this._addLog('warn', 'backdrop-filter: blur(10px) 不可用或 jsdom 未识别（Chrome 76+ / Safari 9+ -webkit- / Firefox 103+）');
        if (!caps.willChange)
            this._addLog('warn', 'will-change: transform 不可用或 jsdom 未识别（Chrome 36+ / Firefox 36+ / Safari 9.1+）');
    ...
    _readAspectRatioInfo() {
        ...
        try {
            ...
            this.setState({ aspectRatioInfo: 'aspect-ratio 宽高比演示：\n' +
    ...
    _renderAspectRatioCard() {
        ...
        return h(Card, {
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._capTags([['aspect-ratio', caps.aspectRatio], ['auto', caps.aspectRatioAuto]]), h(Tag, { color: 'primary' }, '16/9 · 1 · auto')),
        }, h('p', { class: 'fs-sm text-secondary' }, 'aspect-ratio 维持元素宽高比，width 变化时高度按比例自动计算，响应式布局避免高度塌陷。值支持 "16 / 9"（比例）、"1"（正方形，等价 1/1）、"auto 16 / 9"（auto 优先使用固有尺寸如图片原始宽高比，无固有尺寸时回退）。vs padding-top: 56.25% hack：aspect-ratio 更直观，不影响内容布局。实战：响应式视频容器、卡片网格统一比例、骨架屏占位。'), !caps.aspectRatio && h(Alert, {
            type: 'warning',
        ...
        }), h('div', { class: 'flex gap-sm mt-sm flex-wrap' }, this._btn(`切换宽高比（当前：${s.aspectRatio}）`, {
        ...
        })), h('div', { class: 'fs-sm text-secondary mt-sm' }, `aspect-ratio: ${s.aspectRatio} 的演示框（width:100% 自动算高度）：`), h('div', { class: 've-ar-box', style: { aspectRatio: s.aspectRatio } }, `aspect-ratio: ${s.aspectRatio}`), h('div', { class: 'fs-sm text-secondary mt-sm' }, '卡片网格统一比例（每个 div: aspect-ratio: 4/3）：'), h('div', { class: 've-ar-grid' }, h('div', { style: { aspectRatio: '4 / 3' } }, '4:3'), h('div', { style: { aspectRatio: '4 / 3' } }, '4:3'), h('div', { style: { aspectRatio: '4 / 3' } }, '4:3')), h('div', { class: 'fs-sm text-secondary mt-sm' }, 'padding-top: 56.25% hack（= 9/16 * 100%，16:9；需绝对定位子元素）：'), h('div', { class: 've-ar-padhack' }, h('div', { class: 've-ar-padhack-inner' }, 'padding-top: 56.25% hack')), h('pre', { class: 'code-block mt-sm', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, s.aspectRatioInfo || '（点击「读取计算值」查看 aspect-ratio 计算值与说明）')), h('pre', { class: 'code-block mt-sm' }, `/* aspect-ratio —— 维持宽高比 */
.video-box {
  width: 100%;
  ...
  inset: 0;
    ...
    _readObjectFitInfo() {
        ...
        try {
            ...
            this.setState({ objectFitInfo: 'object-fit / object-position 演示：\n' +
    ...
    _renderObjectFitCard() {
        ...
        return h(Card, {
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._capTags([['object-fit', caps.objectFit], ['object-position', caps.objectPosition]]), h(Tag, { color: 'primary' }, 'fill · contain · cover')),
        }, h('p', { class: 'fs-sm text-secondary' }, 'object-fit: fill|contain|cover|none|scale-down 控制替换元素（img/video/iframe）内容适配方式：fill 拉伸变形、contain 完整显示留白、cover 填满裁剪（常用封面）、none 原始尺寸、scale-down 取 none/contain 较小者。object-position: 50% 50%|left top|center bottom 控制内容定位点。vs background-size：object-fit 用于 img/video 内容，background-size 用于背景图。实战：头像裁剪（cover + center top 显示头部）。全平台浏览器支持。'), !caps.objectFit && h(Alert, {
            type: 'warning',
        ...
        }), h('div', { class: 'flex gap-sm mt-sm flex-wrap' }, this._btn(`切换 object-fit（当前：${s.objectFit}）`, {
            type: 'primary', size: 'sm',
            disabled: !caps.css,
        ...
        })), h('div', { class: 'fs-sm text-secondary mt-sm' }, `5 种 object-fit 对比（容器 140x140，图片 240x140 横向；当前演示框：object-fit=${s.objectFit}, object-position=${s.objectPosition}）：`), h('div', { class: 've-of-stage' }, ...fits.map((fit) => h('div', { class: 've-of-cell' }, h('img', { class: `ve-of-img ${fit}`, src: DEMO_IMG, alt: fit }), h('div', { class: 'label' }, fit)))), h('div', { class: 'fs-sm text-secondary mt-sm' }, `当前演示框（object-fit: ${s.objectFit}; object-position: ${s.objectPosition}）：`), h('div', { class: 've-of-cell', style: { width: '180px', height: '180px', display: 'inline-block' } }, h('img', {
            class: `ve-of-img ${s.objectFit}`,
            src: DEMO_IMG,
            alt: 'demo',
            style: { objectPosition: s.objectPosition },
        }), h('div', { class: 'label' }, `${s.objectFit} / ${s.objectPosition}`)), h('pre', { class: 'code-block mt-sm', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, s.objectFitInfo || '（点击「读取计算值」查看 object-fit/object-position 计算值与说明）')), h('pre', { class: 'code-block mt-sm' }, `/* object-fit —— 替换元素内容适配 */
.avatar img {
  width: 100%;
  height: 100%;
  ...
  background-position: center;
    ...
    _readShapesInfo() {
        ...
        try {
            ...
            this.setState({ shapesInfo: 'CSS Shapes 演示：\n' +
    ...
    _renderShapesCard() {
        ...
        return h(Card, {
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._capTags([['shape-outside', caps.shapeOutside], ['shape-margin', caps.shapeMargin], ['shape-image-threshold', caps.shapeImageThreshold]]), h(Tag, { color: 'primary' }, 'float 元素')),
        }, h('p', { class: 'fs-sm text-secondary' }, 'CSS Shapes 让文字环绕自定义形状而非矩形：shape-outside: circle()|ellipse()|polygon()|inset()|url(image.png) 定义浮动元素外轮廓，文字环绕该轮廓。shape-margin 控制文字与形状的距离，shape-image-threshold (0-1) 基于 url() 图片 alpha 通道决定环绕区域。重要约束：shape-outside 仅对浮动元素（float: left/right）生效。实战：圆形头像 + 文字环绕、不规则图片文字环绕。Chrome 37+ / Firefox 62+ / Safari 11.1+ 支持。'), !caps.shapeOutside && h(Alert, {
            type: 'warning',
        ...
        }), h('div', { class: 'flex gap-sm mt-sm flex-wrap' }, this._btn(`切换 shape-outside（当前：${s.shapeKind}）`, {
            type: 'primary', size: 'sm',
            disabled: !caps.css,
        ...
        })), h('div', { class: 'fs-sm text-secondary mt-sm' }, `浮动元素 + shape-outside: ${s.shape}（shape-margin: ${s.shapeMargin}px）—— 真实浏览器中右侧文字会环绕形状轮廓：`), h('div', { class: 've-shape-stage' }, h('div', {
            class: `ve-shape-float ${s.shapeKind}`,
            style: { shapeMargin: `${s.shapeMargin}px` },
        }, ''), h('p', {}, '这段文字会环绕左侧浮动元素的形状轮廓（而非矩形边框）。在真实浏览器中可见：圆形时文字贴合圆弧，菱形（polygon）时文字贴合菱形边，none 时回退为矩形环绕。'), h('p', {}, 'shape-outside 仅对 float: left/right 的元素生效。元素需要有明确的宽高，shape-outside 定义其外轮廓，浏览器据此计算文字可流入的区域。'), h('p', {}, `当前形状：${s.shape}，shape-margin: ${s.shapeMargin}px（文字与形状保持距离）。shape-image-threshold: ${s.shapeThreshold}（配合 url() 图片使用，基于 alpha 通道阈值决定环绕区域）。`), h('p', {}, 'polygon() 用顶点坐标定义多边形（如菱形 polygon(50% 0, 100% 50%, 50% 100%, 0 50%)）；url(image.png) 基于图片不透明区域生成形状，配合 shape-image-threshold 控制灵敏度。')), h('pre', { class: 'code-block mt-sm', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, s.shapesInfo || '（点击「读取计算值」查看 shape-outside 计算值与说明）')), h('pre', { class: 'code-block mt-sm' }, `/* CSS Shapes —— 文字环绕自定义形状 */
.avatar-wrap {
  float: left;
  width: 100px;
  height: 100px;
...
.shape-elem { float: left; shape-outside: circle(50%); }`));
    ...
    _readBlendModeInfo() {
        ...
        try {
            ...
            if (stage) {
                ...
                if (text)
                    mbmComputed = window.getComputedStyle(text).getPropertyValue('mix-blend-mode') || '(空)';
                isoComputed = window.getComputedStyle(stage).getPropertyValue('isolation') || '(空)';
            }
            this.setState({ blendModeInfo: 'mix-blend-mode / isolation 演示：\n' +
    ...
    _renderBlendModeCard() {
        ...
        return h(Card, {
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._capTags([['mix-blend-mode', caps.mixBlendMode], ['isolation', caps.isolation], ['background-blend-mode', caps.backgroundBlendMode]]), h(Tag, { color: 'primary' }, '16 种模式')),
        }, h('p', { class: 'fs-sm text-secondary' }, 'mix-blend-mode 让元素内容与下层背景混合，共 16 种模式（normal/multiply/screen/overlay/darken/lighten/color-dodge/color-burn/hard-light/soft-light/difference/exclusion/hue/saturation/color/luminosity）。isolation:isolate 创建独立合成层隔离混合范围，防止混合泄漏到祖先。vs background-blend-mode：mix-blend-mode 混合元素与背景（跨元素），background-blend-mode 混合元素自身多层背景。实战：文字叠加图片 difference 反色、颜色滤镜。全平台浏览器支持。'), !caps.mixBlendMode && h(Alert, {
            type: 'warning',
        ...
        }), h('div', { class: 'flex gap-sm mt-sm flex-wrap' }, this._btn(`切换混合模式（当前：${s.blendMode}）`, {
            type: 'primary', size: 'sm',
            disabled: !caps.css,
        ...
        })), h('div', { class: 'fs-sm text-secondary mt-sm' }, `mix-blend-mode: ${s.blendMode} 文字（${s.isolationOn ? '父容器已 isolate 隔离' : '父容器未隔离'}）—— 真实浏览器可见混合效果：`), h('div', { class: `ve-blend-stage${s.isolationOn ? ' isolated' : ''}` }, h('div', { class: 've-blend-text', style: { mixBlendMode: s.blendMode } }, `MIX ${s.blendMode.toUpperCase()}`)), h('div', { class: 'fs-sm text-secondary mt-sm' }, 'background-blend-mode: screen（多层背景混合，区别于 mix-blend-mode 跨元素）：'), h('div', { class: 've-blend-layers' }, h('div', { class: 'bg1' }, ''), h('div', { class: 'bg2' }, '')), h('div', { class: 'fs-sm text-tertiary mt-sm' }, `16 种模式：${allModes.join(' / ')}。点击「切换混合模式」循环演示。isolation:isolate 防止混合泄漏到祖先层（如模态框内混合不影响背景页面）。`), h('pre', { class: 'code-block mt-sm', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, s.blendModeInfo || '（点击「读取计算值」查看 mix-blend-mode/isolation 计算值与说明）')), h('pre', { class: 'code-block mt-sm' }, `/* mix-blend-mode —— 元素与下层背景混合 */
.text-overlay {
  ...
  background-blend-mode: screen, multiply;  /* 按层声明 */
    ...
    _readWillChangeInfo() {
        ...
        try {
            ...
            this.setState({ willChangeInfo: 'will-change / GPU 合成层 演示：\n' +
    ...
    _renderWillChangeCard() {
        ...
        return h(Card, {
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._capTags([['will-change', caps.willChange]]), h(Tag, { color: 'primary' }, 'transform · opacity')),
        }, h('p', { class: 'fs-sm text-secondary' }, 'will-change: transform|opacity|scroll-position|contents 提示浏览器该属性即将变化，提前优化（创建 GPU 合成层、预渲染）。触发合成层的属性：transform/opacity/filter/will-change。合成层优势：GPU 加速、独立线程绘制、变化只触发 paint/composite 不触发 layout。滥用警告：过多 will-change 导致内存激增、合成层爆炸，应在动画前设置、动画后重置 will-change:auto。vs transform:translateZ(0) hack：will-change 是语义化优化提示。Chrome 36+ / Firefox 36+ / Safari 9.1+ 支持。'), !caps.willChange && h(Alert, {
            type: 'warning',
        ...
        }), h('div', { class: 'flex gap-sm mt-sm flex-wrap' }, this._btn(`will-change ${s.willChangeOn ? '关闭' : '开启'}（当前：${s.willChangeOn ? 'transform' : 'auto'}）`, {
            type: 'primary', size: 'sm',
        ...
        })), h('div', { class: 'fs-sm text-secondary mt-sm' }, `动画演示框（${s.willChangeOn ? 'will-change:transform 已开启' : 'will-change 未开启'}；点击「触发动画」平移+旋转）：`), h('div', { class: 've-wc-stage' }, h('div', {
            class: `ve-wc-box${s.animating ? ' animating' : ''}${s.willChangeOn ? ' willchange' : ''}`,
        }, s.willChangeOn ? 'WC' : 'box')), h('p', { class: 'fs-sm text-tertiary mt-sm' }, '真实浏览器中，开启 will-change:transform 后动画更流畅（GPU 合成层加速）。但长期保留 will-change 会占用 GPU 内存，动画结束后应重置为 auto。'), h('pre', { class: 'code-block mt-sm', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, s.willChangeInfo || '（点击「读取计算值」查看 will-change 计算值与说明）')), h('pre', { class: 'code-block mt-sm' }, `/* will-change —— 提示浏览器提前优化 */
.animated-box {
  transition: transform 0.6s ease;
...
/* .bad { will-change: transform; } 给 100 个元素都设置 → 内存激增 */`));
    ...
    _readFilterInfo() {
        ...
        try {
            ...
            this.setState({ filterInfo: 'filter / backdrop-filter 演示：\n' +
    ...
    _renderFilterCard() {
        ...
        return h(Card, {
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._capTags([['filter', caps.filter], ['backdrop-filter', caps.backdropFilter], ['drop-shadow', caps.dropShadow]]), h(Tag, { color: 'primary' }, 'blur · 毛玻璃')),
        }, h('p', { class: 'fs-sm text-secondary' }, 'filter 给元素加滤镜：blur/brightness/contrast/grayscale/hue-rotate/invert/opacity/saturate/sepia/drop-shadow，可多滤镜组合（filter: blur(2px) brightness(1.2) contrast(1.1)）。backdrop-filter 滤镜元素后面的内容（毛玻璃效果），Safari 需 -webkit- 前缀。backdrop-filter vs filter：前者滤镜元素后面内容，后者滤镜元素自身。drop-shadow() vs box-shadow：drop-shadow 跟随元素实际形状（含透明区域），box-shadow 是矩形阴影。实战：毛玻璃导航栏、图片滤镜、星形图标阴影。filter 全平台；backdrop-filter Chrome 76+/Safari 9+(-webkit-)/Firefox 103+。'), !caps.filter && h(Alert, {
            type: 'warning',
        ...
        }), h('div', { class: 'flex gap-sm mt-sm flex-wrap' }, this._btn(`切换 filter（当前：${s.combineFilters ? '组合' : s.filter}）`, {
            type: 'primary', size: 'sm',
        ...
        })), h('div', { class: 'fs-sm text-secondary mt-sm' }, `9 种 filter 对比（当前演示框 filter: ${activeFilter}）：`), h('div', { class: 've-filter-stage' }, ...allFilters.map((f) => h('div', { class: 've-filter-cell' }, h('div', { class: 'img-wrap' }, h('img', { src: DEMO_IMG, alt: f, style: { filter: f } })), h('div', { class: 'label' }, f)))), h('div', { class: 'fs-sm text-secondary mt-sm' }, `当前演示框（filter: ${activeFilter}）：`), h('div', { class: 've-filter-cell', style: { display: 'inline-block' } }, h('div', { class: 'img-wrap' }, h('img', { src: DEMO_IMG, alt: 'demo', style: { filter: activeFilter } })), h('div', { class: 'label' }, activeFilter)), h('div', { class: 'fs-sm text-secondary mt-sm' }, `backdrop-filter: blur(10px) 毛玻璃（${s.backdropFilterOn ? '开启' : '关闭'} —— 真实浏览器可见毛玻璃效果）：`), h('div', { class: 've-bd-stage' }, h('div', { class: `ve-bd-panel${s.backdropFilterOn ? '' : ' off'}` }, s.backdropFilterOn ? 'backdrop-filter: blur(10px)' : 'backdrop-filter: none')), h('div', { class: 'fs-sm text-secondary mt-sm' }, 'drop-shadow() vs box-shadow（星形 clip-path，drop-shadow 跟随形状，box-shadow 是矩形）：'), h('div', { class: 've-shadow-stage' }, h('div', { class: 've-shadow-cell' }, h('div', { class: 've-shadow-icon' }, h('div', { class: 've-shadow-star box-shadow' }, '')), h('div', { class: 'label' }, 'box-shadow（矩形）')), h('div', { class: 've-shadow-cell' }, h('div', { class: 've-shadow-icon' }, h('div', { class: 've-shadow-star drop-shadow' }, '')), h('div', { class: 'label' }, 'drop-shadow（跟随形状）')), h('div', { class: 've-shadow-cell' }, h('div', { class: 've-shadow-icon' }, h('div', { class: 've-shadow-star' }, '')), h('div', { class: 'label' }, '无阴影'))), h('pre', { class: 'code-block mt-sm', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, s.filterInfo || '（点击「读取计算值」查看 filter/backdrop-filter 计算值与说明）')), h('pre', { class: 'code-block mt-sm' }, `/* filter —— 元素滤镜 */
...
//# sourceMappingURL=CSSVisualEffectsPage.js.map