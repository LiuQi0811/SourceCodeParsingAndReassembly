...
export class CSSColorHDRDeepPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!f.colorFunc)
            this._addLog('warn', 'color() 函数不可用或 jsdom 未识别（CSS Color L4，Chrome 111+/Firefox 113+/Safari 15+ 支持）');
        if (!f.oklch)
            this._addLog('warn', 'oklch() 不可用或 jsdom 未识别（CSS Color L4，Chrome 111+/Firefox 113+/Safari 15.4+ 支持）');
        if (!f.colorMix)
            this._addLog('warn', 'color-mix() 不可用或 jsdom 未识别（CSS Color L5，Chrome 111+/Firefox 113+/Safari 16.2+ 支持）');
        if (!f.relativeColor)
            this._addLog('warn', '相对颜色 rgb(from ...) 不可用或 jsdom 未识别（CSS Color L5，Chrome 119+/Safari 16.4+ 支持）');
        if (!f.dynamicRangeHigh)
            this._addLog('warn', 'matchMedia("(dynamic-range: high)") 不可用或未匹配（jsdom 通常不实现 matchMedia；需真实 HDR 显示器）');
        if (!f.colorGamutP3)
            this._addLog('warn', 'matchMedia("(color-gamut: p3)") 不可用或未匹配（jsdom 通常不实现；需 Display P3 显示器）');
    ...
    _flags() {
        ...
        const matchMQ = (mq) => this._safe(() => typeof window !== 'undefined' && typeof window.matchMedia === 'function' &&
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            children: [
                ...
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.overviewInfo || '（点击「读取概述信息」查看 CSS Color Level 4/5 与 HDR 全景）')),
                h(Alert, {
                    type: 'info',
    ...
    _setColorSpaceMode(mode) {
        ...
        this._injectStyle('css-color-space-dynamic', `.cs-swatch.active { background: color(${mode} 1 0 0); }`);
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['color()', f.colorFunc]]), h(Tag, { color: 'primary' }, 'CSS Color L4')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'color() 函数指定任意色彩空间：color(display-p3 1 0 0) 表达 Display P3 鲜艳红。预定义色彩空间全集：srgb / srgb-linear / display-p3（Apple 主推）/ a98-rgb（Adobe RGB）/ prophoto-rgb（色域最广）/ rec2020（UHDTV）/ xyz / xyz-d50 / xyz-d65。自定义 @color-profile 通过 ICC profile 加载自定义色彩空间（如 CMYK 印刷、Pantone 专色）。在 HDR/广色域显示器上 color(display-p3 1 0 0) 比 rgb(255,0,0) 更鲜艳，sRGB 显示器上两者视觉一致（浏览器降级渲染）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ colorFuncInfo: this._readColorFuncInfo() }) }), this._btn('srgb', { size: 'sm', disabled: !f.colorFunc, onClick: () => this._setColorSpaceMode('srgb') }), this._btn('display-p3', { size: 'sm', disabled: !f.colorFunc, onClick: () => this._setColorSpaceMode('display-p3') }), this._btn('rec2020', { size: 'sm', disabled: !f.colorFunc, onClick: () => this._setColorSpaceMode('rec2020') }), this._btn('a98-rgb', { size: 'sm', disabled: !f.colorFunc, onClick: () => this._setColorSpaceMode('a98-rgb') }), this._btn('prophoto-rgb', { size: 'sm', disabled: !f.colorFunc, onClick: () => this._setColorSpaceMode('prophoto-rgb') }), this._btn('xyz-d65', { size: 'sm', disabled: !f.colorFunc, onClick: () => this._setColorSpaceMode('xyz-d65') })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '各色彩空间红色对比（HDR 显示器可见差异）：'),
                h('div', { class: 'cs-swatch-row' }, h('div', { class: 'cs-swatch cs-srgb' }, 'srgb'), h('div', { class: 'cs-swatch cs-display-p3' }, 'display-p3'), h('div', { class: 'cs-swatch cs-rec2020' }, 'rec2020'), h('div', { class: 'cs-swatch cs-a98' }, 'a98-rgb'), h('div', { class: 'cs-swatch cs-prophoto' }, 'prophoto'), h('div', { class: 'cs-swatch cs-xyz-d65' }, 'xyz-d65')),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.colorFuncInfo || '（点击按钮切换 color() 色彩空间查看说明）')),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['oklch()', f.oklch], ['lab()', f.lab], ['lch()', f.lch]]), h(Tag, { color: 'primary' }, '感知均匀')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'oklch(L C H) / oklab(L a b) 是 CSS Color L4 推荐的感知均匀色彩空间，由 Björn Ottosson 2020 提出改进 CIE Lab。vs HSL 优势：HSL 感知不均匀（H 0° 与 60° 视觉亮度差异巨大），调 L 时饱和度失真；oklch L/C/H 三轴独立可调，亮度线性递增生成设计系统色阶（Tailwind v4 / Radix Colors / Stripe 采用）。lab()/lch() 是 CIE 1976 老牌标准（Lab 直角坐标 a 红绿/b 黄蓝，LCH 极坐标 C 彩度/H 色相），仍广泛使用但蓝色区域感知不完美。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ oklchInfo: this._readOklchInfo() }) }), this._btn('oklch', { size: 'sm', disabled: !f.oklch, onClick: () => this._setOklchMode('oklch') }), this._btn('oklab', { size: 'sm', disabled: !f.oklab, onClick: () => this._setOklchMode('oklab') }), this._btn('lab', { size: 'sm', disabled: !f.lab, onClick: () => this._setOklchMode('lab') }), this._btn('lch', { size: 'sm', disabled: !f.lch, onClick: () => this._setOklchMode('lch') })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, 'oklch 蓝色色阶（亮度 0.5→0.9 线性递增，C/H 不变）：'),
                h('div', { class: 'oklch-scale' }, h('div', { class: 'oklch-step oklch-l50' }, 'L=0.5'), h('div', { class: 'oklch-step oklch-l60' }, 'L=0.6'), h('div', { class: 'oklch-step oklch-l70' }, 'L=0.7'), h('div', { class: 'oklch-step oklch-l80' }, 'L=0.8'), h('div', { class: 'oklch-step oklch-l90' }, 'L=0.9')),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.oklchInfo || '（点击按钮查看 oklch/lab/lch 完整说明与色阶生成代码）')),
                h(Alert, {
                    type: 'info',
    ...
    _setMixMode(mode) {
        this._mixMode = mode;
        this._injectStyle('css-color-mix-dynamic', `.mix-dynamic { background: color-mix(in ${mode}, red 50%, blue); }`);
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['color-mix()', f.colorMix]]), h(Tag, { color: 'primary' }, 'CSS Color L5')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'color-mix(in <space>, <color1> <pct>?, <color2> <pct>?) 在指定色彩空间混合两色。支持中间空间：srgb/srgb-linear/oklch/oklab/lab/lch/xyz，可指定色相插值方法（longer/shorter/increasing/decreasing hue）。百分比语法：省略=50/50，自动补全，总和超 100% 归一化。透明度合并：alpha 通道按混合规则计算。与 design tokens 协同：一行 color-mix(in oklch, var(--brand), white 15%) 派生 hover/active/disabled 变体。in oklch 比 in srgb 混色更鲜艳（srgb 中间色偏灰暗"灰色死区"，oklch 感知均匀保持饱和度）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ colorMixInfo: this._readColorMixInfo() }) }), this._btn('in oklch', { size: 'sm', disabled: !f.colorMix, onClick: () => this._setMixMode('oklch') }), this._btn('in srgb', { size: 'sm', disabled: !f.colorMix, onClick: () => this._setMixMode('srgb') }), this._btn('in lab', { size: 'sm', disabled: !f.colorMix, onClick: () => this._setMixMode('lab') }), this._btn('in xyz-d65', { size: 'sm', disabled: !f.colorMix, onClick: () => this._setMixMode('xyz-d65') })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, 'color-mix 混色对比（oklch vs srgb 中间色差异）：'),
                h('div', { class: 'mix-stage' }, h('div', { class: 'mix-box mix-a' }, 'red'), h('div', { class: 'mix-box mix-oklch-50' }, 'oklch 50%'), h('div', { class: 'mix-box mix-srgb-50' }, 'srgb 50%'), h('div', { class: 'mix-box mix-oklch-25' }, 'oklch 25%'), h('div', { class: 'mix-box mix-oklch-75' }, 'oklch 75%'), h('div', { class: 'mix-box mix-alpha' }, 'alpha 0.5'), h('div', { class: 'mix-box mix-b' }, 'blue')),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.colorMixInfo || '（点击按钮切换 color-mix 中间空间查看说明）')),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['rgb(from)', f.relativeColor], ['oklch(from)', f.relativeOklch]]), h(Tag, { color: 'primary' }, 'CSS Color L5')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '相对颜色用 from 关键字派生颜色：<func>(from <base> <channel>...)，支持 rgb/hsl/lab/lch/oklab/oklch/color()。通道变量：rgb→r g b，oklch→l c h，hsl→h s l，alpha 通用。提取并修改通道：rgb(from #2563eb r 0 0) 仅保留红色通道，rgb(from red r g b / 0.5) 改 alpha。主题色调派生：oklch(from var(--brand) calc(l + 0.15) c h) 派生亮色变体，比 color-mix 更灵活（可单独修改某一通道）。color-contrast() 提案自动选择 WCAG 对比度最高颜色，仅 Safari 16.4+ 部分支持。浏览器：Chrome 119+/Firefox 128+/Safari 16.4+。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ relativeColorInfo: this._readRelativeColorInfo() }) }), this._btn('lighten', { size: 'sm', disabled: !f.relativeOklch, onClick: () => this._setRelativeMode('lighten') }), this._btn('darken', { size: 'sm', disabled: !f.relativeOklch, onClick: () => this._setRelativeMode('darken') }), this._btn('saturate', { size: 'sm', disabled: !f.relativeOklch, onClick: () => this._setRelativeMode('saturate') }), this._btn('hue+60', { size: 'sm', disabled: !f.relativeOklch, onClick: () => this._setRelativeMode('hue') }), this._btn('alpha', { size: 'sm', disabled: !f.relativeColor, onClick: () => this._setRelativeMode('alpha') })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '相对颜色派生变体（基础色 #2563eb）：'),
                h('div', { class: 'rel-stage' }, h('div', { class: 'rel-box rel-base' }, 'base'), h('div', { class: 'rel-box rel-lighten' }, 'lighten'), h('div', { class: 'rel-box rel-darken' }, 'darken'), h('div', { class: 'rel-box rel-saturate' }, 'saturate'), h('div', { class: 'rel-box rel-hue' }, 'hue+60'), h('div', { class: 'rel-box rel-alpha' }, 'alpha 0.4')),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.relativeColorInfo || '（点击按钮切换相对颜色模式查看说明）')),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'HDR 与广色域媒体查询：@media (dynamic-range: high) 检测 HDR 显示器，@media (color-gamut: p3|rec2020) 检测广色域。prefers-contrast: more|less|custom 用户对比度偏好。system-color（Canvas/LinkText/ButtonText/Field 等）反映操作系统主题色，CSS Color L4 重新定义为函数可混色。HDR 视频背景在 HDR 显示器上呈现鲜艳颜色。HDR 显示检测需三方支持：HDR 显示器 + 浏览器 + 操作系统（Windows HDR 模式 / macOS MacBook Pro 等）。jsdom 通常不实现 matchMedia，真实浏览器 + HDR 显示器才能完整检测。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ hdrInfo: this._readHdrInfo() }) }), this._btn('dynamic-range', { size: 'sm', onClick: () => this._setHdrMode('dynamic-range') }), this._btn('color-gamut', { size: 'sm', onClick: () => this._setHdrMode('color-gamut') }), this._btn('prefers-contrast', { size: 'sm', onClick: () => this._setHdrMode('prefers-contrast') })),
                h('div', { class: 'hdr-stage' }, h('div', { class: 'hdr-swatch hdr-sdr' }, 'SDR 渐变（sRGB red→blue）'), h('div', { class: 'hdr-swatch hdr-p3' }, 'P3 鲜艳红（HDR 显示器可见差异）'), h('div', { class: 'hdr-swatch hdr-rec2020' }, 'Rec2020 超广色域红'), h('div', { class: 'hdr-swatch hdr-contrast' }, 'prefers-contrast: more（高对比度边框）')),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.hdrInfo || '（点击按钮查看 HDR 媒体查询完整说明）')),
                h(Alert, {
                    type: 'warning',
    ...
    _readRenderingInfo() {
        ...
        try {
            return `===== 浏览器渲染与色彩管理 =====\n` +
                ...
                `  const img = new Image();\n` +
                `  img.onload = () => {\n` +
                `    const srgbCanvas = (document.createElement('canvas') as any);\n` +
    ...
    _renderCard7() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['canvas P3', f.canvasColorSpace]]), h(Tag, { color: 'primary' }, '色彩管理')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '浏览器渲染管线：CSS 颜色值解析（保留色彩空间）→ 计算样式 → 绘制时转换为目标色域 → 显示器呈现，遵循 ICC profile 与色彩管理协议。canvas colorSpace 选项：getContext("2d", { colorSpace: "display-p3" }) 创建 P3 画布绘制 HDR 图像（Chrome 98+/Safari 16.4+，Firefox 部分支持）。drawImage 跨色彩空间自动转换（P3 图 → sRGB canvas 色域裁剪）。ImageData.colorSpace 反映像素色彩空间，putImageData 不匹配时自动转换。自定义 @color-profile 加载外部 ICC profile（如 CMYK 印刷、Pantone 专色）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行渲染演示', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runRenderingDemo() })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, 'canvas P3 vs sRGB 红色对比（HDR 显示器可见差异）：'),
                h('div', { class: 'canvas-stage' }, h('canvas', { width: '240', height: '80' }, '（您的浏览器不支持 canvas）'), h('div', { class: 'color-output rendering-output' }, f.canvasColorSpace
                    ? '点击「运行渲染演示」在 canvas 上绘制 P3 vs sRGB 红色对比...'
                    : 'canvas colorSpace: "display-p3" 不可用（Firefox 部分支持，Chrome 98+/Safari 16.4+ 支持），跳过真实绘制。')),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.renderingInfo || '（点击「运行渲染演示」查看色彩管理完整说明）')),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard8() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['color-mix', f.colorMix], ['relative-color', f.relativeColor]]), h(Tag, { color: 'warning' }, '5 大场景')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '五大实战场景：暗色主题切换 + 色阶生成器（oklch 反转色阶）、设计系统 color-mix 派生变体（hover/active/soft/border）、浏览器降级（双写 / @supports / CSS.supports 三方案）、WCAG 对比度（color-contrast 提案 + JS 计算 relativeLuminance，AA ≥4.5:1 / AAA ≥7:1）、渐变色带修复（in oklch 替代 in srgb 消除"灰色死区"）。8 大陷阱：sRGB 显示器无差异、oklch L 范围 0~1、color-mix 百分比归一化、通道变量大小写、prophoto-rgb 超可见光、canvas colorSpace 兼容、color-contrast 仅 Safari、HDR 三方支持。'),
                ...
                h('div', { class: 'theme-stage light' }, h('div', { class: 'fs-sm' }, '亮色主题 oklch 蓝色色阶（L 0.97→0.30 线性递减）：'), h('div', { class: 'scale-row' }, h('div', { class: 'scale-step', style: { background: 'oklch(0.97 0.02 250)', color: '#000' } }, '50'), h('div', { class: 'scale-step', style: { background: 'oklch(0.80 0.12 250)' } }, '300'), h('div', { class: 'scale-step', style: { background: 'oklch(0.60 0.20 250)' } }, '500'), h('div', { class: 'scale-step', style: { background: 'oklch(0.40 0.14 250)' } }, '800'), h('div', { class: 'scale-step', style: { background: 'oklch(0.30 0.10 250)' } }, '900')), h('div', { class: 'fs-sm', style: { marginTop: '8px' } }, '渐变色带修复（srgb 灰色死区 vs oklch 感知均匀）：'), h('div', { class: 'gradient-bad' }), h('div', { class: 'fs-sm', style: { marginTop: '4px', fontSize: '10px' } }, '↑ linear-gradient(in srgb, blue, red) —— 中间偏灰暗'), h('div', { class: 'gradient-good' }), h('div', { class: 'fs-sm', style: { marginTop: '4px', fontSize: '10px' } }, '↑ linear-gradient(in oklch, blue, red) —— 中间保持饱和度')),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.pitfallsInfo || '（点击按钮查看 5 大实战场景与 8 大陷阱完整说明）')),
                h(Alert, {
                    type: 'warning',
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'api-lab-page css-color-hdr-deep-page' }, h('h2', { class: 'section-title' }, 'CSS Color Level 4/5 与 HDR 深度实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, '本页演示 CSS Color Module Level 4/5 现代色彩标准与 HDR 广色域：color() 函数与色彩空间全集（display-p3/rec2020/xyz）、oklch/lab/lch 感知均匀色彩、color-mix() 混色、相对颜色（from 关键字）派生主题色调、HDR 与 dynamic-range 媒体查询、canvas colorSpace 色彩管理、5 大实战场景与 8 大陷阱。所有特性通过 typeof / CSS.supports / matchMedia 能力检测，不可用时仅记日志，绝不抛异常。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderCard6(), this._renderCard7(), this._renderCard8(), this._renderLogPanel());
    }
}
//# sourceMappingURL=CSSColorHDRDeepPage.js.map