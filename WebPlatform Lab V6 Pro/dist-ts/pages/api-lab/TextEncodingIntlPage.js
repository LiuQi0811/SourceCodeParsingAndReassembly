...
export class TextEncodingIntlPage extends Page {
    ...
    _renderCard1() {
        return this._card('1. Encoding API — TextEncoder / TextDecoder', 'TextEncoder 仅支持 UTF-8，encode(str) → Uint8Array；TextDecoder 支持 utf-8 / gbk / shift_jis / euc-kr 等多种编码，decode(bytes) → str；options: { fatal, ignoreBOM }；encodeInto(str, uint8) → { read, written }。', h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'primary' }, 'TextEncoder'), h(Tag, { color: 'success' }, 'TextDecoder'), h(Tag, { color: 'warning' }, 'encodeInto')), [
            h('div', { class: 'flex gap-sm flex-wrap' }, this._btn('编码演示', { type: 'primary', size: 'sm', onClick: () => this._runEncode() }), this._btn('decode 多编码', { size: 'sm', onClick: () => this._runDecode() }), this._btn('encodeInto 性能对比', { size: 'sm', onClick: () => this._runEncodeInto() })),
    ...
    _renderCard2() {
        return this._card('2. TextEncoderStream / TextDecoderStream — 流式编码', 'new TextEncoderStream() / new TextDecoderStream(label, { fatal, ignoreBOM })；拥有 readable / writable 属性，可 pipeThrough；常用于 fetch response.body 流式解码。', h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'primary' }, 'TextEncoderStream'), h(Tag, { color: 'success' }, 'TextDecoderStream'), h(Tag, { color: 'warning' }, 'pipeThrough')), [
            h('div', { class: 'flex gap-sm flex-wrap' }, this._btn('流式编码-解码往返', { type: 'primary', size: 'sm', onClick: () => this._runStream() })),
    ...
    _runSegmentZh() {
        ...
        try {
            ...
            const lines = segments.map((s, i) => `[${String(i).padStart(2, '0')}] segment="${s.segment}"  isWordLike=${s.isWordLike}  index=${s.index}`);
    ...
    _runSegmentEn() {
        ...
        try {
            ...
            const lines = ['【word 分词】'];
            segments.forEach((s, i) => lines.push(`  [${String(i).padStart(2, '0')}] seg="${s.segment}"  isWordLike=${s.isWordLike}  index=${s.index}`));
            const words = segments.filter((s) => s.isWordLike).map((s) => s.segment);
            lines.push('');
    ...
    _renderCard3() {
        return this._card('3. Intl.Segmenter — 分词 / 分段 / 字素', 'new Intl.Segmenter(locale, { granularity: "grapheme" | "word" | "sentence" | "line" })；segment(str) 返回可迭代的 Segments；每段 { segment, index, input, isWordLike }。', h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'primary' }, 'Segmenter'), h(Tag, { color: 'success' }, 'word'), h(Tag, { color: 'success' }, 'sentence'), h(Tag, { color: 'warning' }, 'grapheme')), [
            h('div', { class: 'flex gap-sm flex-wrap' }, this._btn('中文分词', { type: 'primary', size: 'sm', onClick: () => this._runSegmentZh() }), this._btn('英文分词', { size: 'sm', onClick: () => this._runSegmentEn() }), this._btn('grapheme 字素', { size: 'sm', onClick: () => this._runSegmentGrapheme() })),
    ...
    _renderCard4() {
        return this._card('4. Intl 高级格式化 — ListFormat / PluralRules / RelativeTimeFormat / DisplayNames / DurationFormat', 'ListFormat 列表连接词；PluralRules 基数/序数规则（select → one/other/two/few/many）；RelativeTimeFormat 相对时间（昨天/明天）；DisplayNames 语言/地区/货币/脚本本地化名称；DurationFormat 时长格式化（较新）。', h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'primary' }, 'ListFormat'), h(Tag, { color: 'success' }, 'PluralRules'), h(Tag, { color: 'warning' }, 'RelativeTimeFormat'), h(Tag, { color: 'warning' }, 'DisplayNames'), h(Tag, { color: 'default' }, 'DurationFormat')), [
            h('div', { class: 'flex gap-sm flex-wrap' }, this._btn('ListFormat', { type: 'primary', size: 'sm', onClick: () => this._runListFormat() }), this._btn('PluralRules 0-5', { type: 'primary', size: 'sm', onClick: () => this._runPluralRules() }), this._btn('RelativeTime', { type: 'primary', size: 'sm', onClick: () => this._runRelativeTime() }), this._btn('DisplayNames', { type: 'primary', size: 'sm', onClick: () => this._runDisplayNames() })),
    ...
    _checkFonts() {
        ...
        try {
            ...
            if (!this._fontHandler) {
                ...
                try {
                    fonts.addEventListener('loadingdone', this._fontHandler);
                }
                catch { /* noop */ }
            }
    ...
    _renderCard5() {
        return this._card('5. CSS Font Loading API — FontFace / document.fonts', 'new FontFace(family, source, { weight, style }).load() → Promise<FontFace>；document.fonts（FontFaceSet）：add / has / delete / check / load / ready / clear / onloadingdone。', h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'primary' }, 'FontFace'), h(Tag, { color: 'success' }, 'FontFaceSet'), h(Tag, { color: 'warning' }, 'document.fonts')), [
            h('div', { class: 'flex gap-sm flex-wrap' }, this._btn('检测 document.fonts', { type: 'primary', size: 'sm', onClick: () => this._checkFonts() }), this._btn('模拟 FontFace.load', { size: 'sm', onClick: () => this._loadFontFace() }), this._btn('document.fonts.ready', { size: 'sm', onClick: () => this._checkFontsReady() })),
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'page api-lab-page' }, h('h2', { class: 'section-title' }, '文本编码与国际化实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, '演示 Encoding API / TextEncoderStream / Intl.Segmenter / Intl 高级格式化 / CSS Font Loading API。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderLogPanel());
    }
}
//# sourceMappingURL=TextEncodingIntlPage.js.map