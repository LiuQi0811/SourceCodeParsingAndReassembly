// =====================================================================
// ModernDOMPage.ts —— 现代平台 DOM 能力实验室
// 演示 MDN：
...
export class ModernDOMPage extends Page {
    ...
    componentDidMount() {
        ...
        const anchorSupported = (() => {
            try {
                return typeof CSS !== 'undefined' && CSS.supports('position-anchor', '--x');
            }
            catch {
                return false;
            }
        ...
        if (registerPropertySupported) {
            try {
                ...
                propLog = { type: 'info', content: 'CSS.registerProperty 成功注册 --typed-color (syntax: <color>, initialValue: #1677ff)', time: formatTime() };
            }
            catch (err) {
                propLog = { type: 'error', content: `CSS.registerProperty 失败: ${err.message}`, time: formatTime() };
            }
        }
        else {
            propLog = { type: 'error', content: '当前环境不支持 CSS.registerProperty（jsdom 或旧浏览器）', time: formatTime() };
        }
    ...
    _renderDialogCard() {
        ...
        return h(Card, {
            ...
            extra: h(Tag, { color: s.dialogSupported ? 'success' : 'error' }, s.dialogSupported ? '已支持' : '未支持'),
        }, h('p', { class: 'fs-sm text-secondary' }, '<dialog> 是原生模态元素。showModal() 模态打开（显示 ::backdrop 遮罩、阻塞背景交互、Esc 触发 cancel）；show() 非模态打开；close(returnValue) 关闭并传回返回值。监听 close / cancel 事件。'), !s.dialogSupported ? h(Alert, {
            type: 'warning',
            message: '当前环境不支持 <dialog>',
            description: 'jsdom 不实现 HTMLDialogElement；Chrome 37+ / Firefox 98+ / Safari 15.4+ 均已支持。',
        }) : null, h('div', { class: 'flex gap-sm mt-sm' }, this._btn('showModal()', { type: 'primary', size: 'sm', onClick: () => this._showModal() }), this._btn('show() 非模态', { size: 'sm', onClick: () => this._showNonModal() }), this._btn('close("ok")', { size: 'sm', onClick: () => this._closeDialog('ok') }), this._btn('close("cancel")', { danger: true, size: 'sm', onClick: () => this._closeDialog('cancel') })), h('div', { class: 'fs-sm text-tertiary mt-sm' }, `当前 returnValue: "${s.dialogReturnValue || '(空)'}" —— 按 Esc 触发 cancel 事件（默认会关闭 dialog）。`), h('dialog', {
            id: 'demo-dialog',
            onClose: (e) => {
                const target = e.target;
                const rv = (target && target.returnValue) || '';
                setTimeout(() => {
        ...
        }, h('h4', { style: { marginTop: '0', marginBottom: 'var(--spacing-xs)' } }, '对话框内容'), h('p', { class: 'fs-sm text-secondary' }, '这是 <dialog> 元素。showModal() 时 ::backdrop 遮罩覆盖页面并阻塞背景交互；按 Esc 触发 cancel 事件并自动关闭。'), h('div', { class: 'flex gap-sm mt-sm' }, h('button', {
            type: 'button',
            class: 'btn btn--sm btn--primary',
            onClick: () => this._closeDialog('ok'),
        }, '确认 (returnValue="ok")'), h('button', {
            type: 'button',
            class: 'btn btn--sm',
            onClick: () => this._closeDialog('cancel'),
        }, '取消 (returnValue="cancel")'))), h('pre', { class: 'code-block mt-sm' }, `const dlg = document.querySelector('dialog');
...
::backdrop { background: rgba(0,0,0,.45); }`));
    ...
    _renderPopoverCard() {
        ...
        return h(Card, {
            ...
            extra: h(Tag, { color: s.popoverSupported ? 'success' : 'error' }, s.popoverSupported ? '已支持' : '未支持'),
        }, h('p', { class: 'fs-sm text-secondary' }, '通过 popover 属性（auto/manual）声明弹出层，浏览器自动处理 Top Layer 层级、焦点管理、Light Dismiss（auto 模式点击外部自动关闭）。监听 beforetoggle / toggle 事件获取 oldState / newState。'), !s.popoverSupported ? h(Alert, {
            type: 'warning',
            message: '当前浏览器不支持 Popover API',
            description: 'showPopover / hidePopover / togglePopover 不可用。Chrome 114+ / Safari 17+ / Firefox 125+ 支持。',
        }) : null, h('div', { class: 'flex gap-sm mt-sm' }, this._btn('togglePopover()', { type: 'primary', size: 'sm', onClick: () => this._togglePopover() }), this._btn('showPopover()', { size: 'sm', onClick: () => this._showPopover() }), this._btn('hidePopover()', { size: 'sm', onClick: () => this._hidePopover() })), h('div', { class: 'flex items-center gap-sm mt-sm' }, h('button', {
            type: 'button',
            class: 'btn btn--sm',
            popovertarget: 'demo-popover',
        }, '声明式触发（popovertarget）'), h('span', { id: 'popover-status', class: 'fs-sm text-tertiary' }, '状态: 关闭')), h('div', {
            id: 'demo-popover',
            popover: 'auto',
            onbeforetoggle: (e) => {
                const te = e;
                ...
                if (status)
                    status.textContent = `beforetoggle: ${te.oldState} → ${te.newState}`;
            },
            ontoggle: (e) => {
                const te = e;
                const status = this.$('#popover-status');
                if (status)
                    status.textContent = `状态: ${te.newState === 'open' ? '打开' : '关闭'}`;
            },
        }, h('h4', { style: { marginTop: '0', marginBottom: 'var(--spacing-xs)' } }, 'Popover 内容'), h('p', { class: 'fs-sm text-secondary' }, 'popover="auto" 点击外部自动关闭（Light Dismiss）。beforetoggle 在切换前触发，toggle 在切换后触发，均提供 oldState / newState。')), h('pre', { class: 'code-block mt-sm' }, `<div popover="auto" id="p"></div>
...
el.addEventListener('toggle', e => console.log('now:', e.newState));`));
    ...
    _renderViewTransitionsCard() {
        ...
        return h(Card, {
            ...
            extra: h(Tag, { color: s.vtSupported ? 'success' : 'error' }, s.vtSupported ? '已支持' : '未支持'),
        }, h('p', { class: 'fs-sm text-secondary' }, 'document.startViewTransition(callback) 包裹 DOM 更新，浏览器自动捕获前后快照并通过 ::view-transition-old / ::view-transition-new 伪元素生成过渡动画。transition 对象提供 updateCallbackDone / ready / finished 三个 Promise 以及 types 集合。'), !s.vtSupported ? h(Alert, {
            type: 'warning',
            message: '当前浏览器不支持 View Transitions',
            description: '已降级为直接 setState，无过渡动画。Chrome 111+ / Edge 111+ 支持。',
        }) : null, h('div', { class: 'flex gap-sm mt-sm' }, this._btn('切换视图 A / B', { type: 'primary', size: 'sm', onClick: () => this._switchView() }), this._btn(s.vtTypesEnabled ? 'types: ON（点击关闭）' : 'types: OFF（点击开启）', { size: 'sm', onClick: () => this._toggleVtTypes() })), h('div', { class: 'flex items-center gap-sm mt-sm' }, h('div', {
            id: 'vt-view-box',
            style: {
        ...
        }, `视图 ${s.vtView}`), h('span', { class: 'fs-sm text-tertiary' }, '← view-transition-name: vt-view-box')), h('pre', { class: 'code-block mt-sm' }, `const t = document.startViewTransition(() => {
...
::view-transition-new(root) { animation-duration: .4s; }`));
    ...
    _renderPropertyCard() {
        ...
        return h(Card, {
            ...
            extra: h(Tag, { color: s.registerPropertySupported ? 'success' : 'error' }, s.registerPropertySupported ? '已支持' : '未支持'),
        }, h('p', { class: 'fs-sm text-secondary' }, 'CSS.registerProperty() 或 @property at-rule 为自定义属性声明类型（<color>/<length>/<integer> 等）、初始值与继承性。类型化后浏览器可在过渡/动画中对值进行插值；未类型化只能离散跳变。'), !s.registerPropertySupported ? h(Alert, {
            type: 'warning',
            message: '当前环境不支持 CSS.registerProperty',
            description: 'jsdom 不实现该 API。Chrome 85+ / Safari 16.4+ / Firefox 128+ 支持。下方演示仍可工作但无类型化过渡效果。',
        }) : null, h('div', { class: 'flex gap-sm mt-sm' }, this._btn('切换类型化颜色', { type: 'primary', size: 'sm', onClick: () => this._toggleTypedColor() }), this._btn('切换非类型化颜色', { size: 'sm', onClick: () => this._toggleUntypedColor() })), h('div', { class: 'flex gap-sm mt-sm' }, h('div', { class: 'flex flex-col items-center gap-xs' }, h('div', {
            class: 'typed-box',
            style: { '--typed-color': s.typedColor, background: s.typedColor },
        }), h('span', { class: 'fs-sm text-tertiary' }, '类型化（@property）')), h('div', { class: 'flex flex-col items-center gap-xs' }, h('div', {
            class: 'untyped-box',
            style: { '--untyped-color': s.untypedColor, background: s.untypedColor },
        }), h('span', { class: 'fs-sm text-tertiary' }, '非类型化（普通变量）'))), h('p', { class: 'fs-sm text-tertiary mt-sm' }, '类型化方块应平滑过渡（0.6s），非类型化方块瞬间跳变。差异源于 @property 告诉浏览器该变量是 <color> 类型，可以在过渡中插值。'), h('pre', { class: 'code-block mt-sm' }, `/* JS 注册 */
CSS.registerProperty({
  name: '--typed-color',
  syntax: '<color>',
  inherits: false,
  ...
  background: var(--typed-color);
    ...
    _renderContainerQueriesCard() {
        ...
        return h(Card, {
            ...
            extra: h(Tag, { color: s.containerQuerySupported ? 'success' : 'error' }, s.containerQuerySupported ? '已支持' : '未支持'),
        }, h('p', { class: 'fs-sm text-secondary' }, 'container-type: inline-size 声明容器，@container 规则根据容器（而非视口）宽度响应式布局。jsdom 不响应 CSS 响应式，真实浏览器中拖动滑块即可看到列数与颜色变化。配合 ResizeObserver 可在 JS 中感知容器尺寸变化。'), !s.containerQuerySupported ? h(Alert, {
            type: 'warning',
            message: '当前环境可能不支持 Container Queries',
            description: 'CSS.supports 在 jsdom 中返回 false。Chrome 105+ / Safari 16+ / Firefox 110+ 支持。',
        }) : null, h('div', { class: 'flex items-center gap-sm mt-sm' }, h('input', {
            id: 'cq-range',
            type: 'range',
            min: '200',
            max: '600',
        ...
        }), h('span', { id: 'cq-width-display', class: 'fs-sm text-tertiary' }, `${s.containerWidth}px`)), h('div', {
            id: 'cq-demo',
            class: 'cq-demo mt-sm',
            style: { width: s.containerWidth + 'px' },
        }, h('div', { class: 'cq-grid' }, h('div', { class: 'cq-card' }, '卡片 1'), h('div', { class: 'cq-card' }, '卡片 2'), h('div', { class: 'cq-card' }, '卡片 3'), h('div', { class: 'cq-card' }, '卡片 4'))), h('p', { class: 'fs-sm text-tertiary mt-sm' }, '容器 < 350px 单列（蓝），350-500px 双列（绿），> 500px 三列（橙）。@container 依据父容器宽度，而非视口宽度。'), h('pre', { class: 'code-block mt-sm' }, `.cq-demo {
...
ro.observe(containerEl);`));
    ...
    _renderAnchorDsdCard() {
        ...
        return h(Card, {
            ...
            extra: h(Tag, { color: s.anchorSupported ? 'success' : 'error' }, s.anchorSupported ? 'Anchor 已支持' : 'Anchor 未支持'),
        }, h('p', { class: 'fs-sm text-secondary' }, 'anchor-name 为元素命名锚点，position-anchor 引用锚点，anchor() 函数获取锚点边/中心位置实现无需 JS 计算的相对定位；position-try-fallbacks 提供翻转回退。Declarative Shadow DOM 通过 <template shadowrootmode="open"> 实现服务端渲染式 shadow DOM。'), (!s.anchorSupported || !s.dsdSupported) ? h(Alert, {
            type: 'warning',
            message: '兼容性提示',
        ...
        }) : null, h('h4', { class: 'fs-md mt-sm', style: { fontWeight: 'bold' } }, 'Anchor Positioning'), h('div', {
        ...
        }, '我通过 position-anchor 锚定到按钮')), h('p', { class: 'fs-sm text-tertiary mt-sm' }, 'tooltip 样式：position-anchor: --anchor-demo; top: anchor(bottom); left: anchor(center); position-try-fallbacks: flip-block;'), h('h4', { class: 'fs-md mt-sm', style: { fontWeight: 'bold' } }, 'Declarative Shadow DOM'), h('div', { class: 'flex gap-sm mt-sm' }, this._btn('渲染声明式 Shadow DOM', { type: 'primary', size: 'sm', onClick: () => this._renderDsd() })), h('div', { id: 'dsd-host', class: 'dsd-host mt-sm' }, h('span', { class: 'fs-sm text-tertiary' }, '点击上方按钮后，此处通过 innerHTML 注入 <template shadowrootmode="open">')), s.dsdReady ? h('p', { class: 'fs-sm mt-sm', style: { color: 'var(--color-success)' } }, '✓ host.shadowRoot 已建立，内部样式封装在 shadow 内，::part(label) 可被外部 CSS 穿透。') : null, h('pre', { class: 'code-block mt-sm' }, `/* CSS Anchor Positioning */
...
//# sourceMappingURL=ModernDOMPage.js.map