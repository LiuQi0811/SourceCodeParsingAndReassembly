...
export class CSSCustomHighlightAPIPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!f.highlightsAPI)
            this._addLog('warn', '当前 jsdom 版本不支持 CSS.highlights；点击按钮查看代码示例与说明');
        if (!f.priority)
            this._addLog('warn', 'priority 属性不可用（Chrome 105+/Safari 17.2+）');
    ...
    // —— 安全注册/清理 Highlight ——
    _registerHighlight(name, highlight) {
        if (!this._flags().highlightsAPI)
            return false;
        try {
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'CSS.highlights 是全局 HighlightRegistry（类似 CSS.stylesheets），用 set/get/delete/clear/entries/keys/values/forEach/size 管理。Highlight 是 Range 集合（Set-like：add/delete/has/clear/forEach/size/迭代），构造器可传任意多个 Range。修改 Highlight 内 Range 后浏览器自动重绘，无需 refresh。Chrome 105+/Safari 17.2+ 支持。Range API 全浏览器可用。'),
                ...
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.basicsInfo || '（点击按钮查看 CSS.highlights + Highlight 完整用法）')),
            ],
        });
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['::highlight()', f.pseudo]]), h(Tag, { color: 'primary' }, '伪元素')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '::highlight(<name>) 伪元素用 CSS 样式化注册的高亮。仅支持文本相关属性白名单：color/background-color/text-decoration 及子属性/caret-color/outline 及子属性/stroke/fill/stroke-width（SVG 文本）。不支持 box-shadow/border/padding/transform/font-* 等会触发布局的属性（高亮是视觉装饰层不参与布局）。可配合 @prefers-color-scheme 自适应深色模式。'),
                h('div', { class: 'ch-demo' }, h('p', { class: 'ch-demo-text ch-pseudo-text' }, '这段演示文本中的 Highlight 和 API 单词会被自动高亮。')),
                h('div', { class: 'ch-highlight-legend' }, h('span', { class: 'ch-legend-item' }, h('span', { class: 'ch-legend-swatch', style: { background: '#fef08a' } }), 'ch-yellow (Highlight 单词)'), h('span', { class: 'ch-legend-item' }, h('span', { class: 'ch-legend-swatch', style: { background: '#bbf7d0' } }), 'ch-green (API 单词)')),
                h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('运行 ::highlight() 演示', { type: 'primary', size: 'sm', onClick: () => this._runPseudoDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.pseudoInfo || '（点击按钮查看 ::highlight() 伪元素完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _runPriorityDemo() {
        ...
        const info = [
            ...
            '',
            '【浏览器支持】',
            `  Highlight.prototype.priority: ${f.priority ? '✓' : '✗'} (Chrome 105+/Safari 17.2+)`,
            '',
            '【调试技巧】',
        ...
        this._addLog('css', `priority 演示完成；supports=${f.priority}`);
    }
    _renderCard3() {
        const s = this.state;
        const f = this._flags();
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['priority', f.priority]]), h(Tag, { color: 'primary' }, '叠层')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '(highlight as any).priority = <number> 控制多重高亮重叠时的叠层顺序（数值越大越上层，默认 0 可为负）。相同 priority 按注册顺序。各属性独立覆盖：A 设 color，B 设 background，即使 A priority 高 B 的 background 仍生效。协同编辑场景：当前用户选区 priority 高于他人。修改 priority 立即生效无需重新注册。Chrome 105+/Safari 17.2+ 支持。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 priority 演示', { type: 'primary', size: 'sm', onClick: () => this._runPriorityDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.priorityInfo || '（点击按钮查看 priority 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['type', f.type]]), h(Tag, { color: 'primary' }, '语义')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'highlight.type = "search-result"|"spell-error"|"grammar-error"|"text-selection"|"highlight" 为高亮声明语义分类。规范预定义 5 个值，便于辅助技术（屏幕阅读器）识别高亮用途。type 不影响样式（::highlight 仍按 name 匹配），与 priority 独立。AT 支持有限但持续改进，未来可提供更好的无障碍体验。Chrome 105+/Safari 17.2+ 支持。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 type 演示', { type: 'primary', size: 'sm', onClick: () => this._runTypeDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.typeInfo || '（点击按钮查看 type 属性完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _runLiveSearchDemo() {
        ...
        const input = document.querySelector('.ch-search-input');
    ...
    `);
        // 在所有文本节点中搜索
        let matchCount = 0;
        const walker = document.createTreeWalker(demoText, NodeFilter.SHOW_TEXT, null);
        let node;
        while ((node = walker.nextNode())) {
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'primary' }, '实战'), h(Tag, { color: f.highlightsAPI ? 'success' : 'error' }, `实时演示 ${f.highlightsAPI ? '✓' : '✗'}`)),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '7 大典型场景完整实战：搜索结果高亮（实时输入匹配 + TreeWalker 遍历文本节点）/ 拼写检查标注（后端 API + type=spell-error）/ 协同编辑他人选区（WebSocket + 每用户独立 Highlight + priority）/ 代码评审批注（行号范围 Range）/ 阅读进度（IntersectionObserver + 最低 priority）/ 富文本编辑器装饰（当前行高亮）/ 表单错误标记。含性能优化（复用 Highlight、requestIdleCallback 分批、清理失效 Range）与 React/Vue 集成模式。'),
                h('div', { class: 'ch-demo' }, h('div', { class: 'ch-search-bar' }, h('input', {
                    type: 'text',
                    class: 'ch-search-input',
                    value: 'Highlight',
                    placeholder: '输入要搜索的关键词',
                }), this._btn('搜索高亮', { type: 'primary', size: 'sm', onClick: () => this._runLiveSearchDemo() })), h('div', { class: 'ch-demo-text ch-search-text' }, 'CSS Custom Highlight API 是浏览器原生的高亮 API。' +
                    '它让开发者可以用 Range 对象标记文本区间，并用 ::highlight() 伪元素样式化。' +
                    'Highlight 对象是 Range 的集合，类似 Set 但不是 Set 的子类。' +
                    '通过 CSS.highlights 全局注册表管理所有高亮。'), s.liveSearchResult
                    ? h('pre', { class: 'ch-output' }, s.liveSearchResult)
                    : h('div', { class: 'ch-stats' }, '点击「搜索高亮」按钮运行实时搜索演示')),
                h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('查看 7 大场景完整代码', { type: 'default', size: 'sm', onClick: () => this._runPatternsDemo() })),
                s.patternsInfo
                    ? h('pre', { class: 'code-block', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.patternsInfo))
                    : null,
            ],
        });
        this.registerChild(card);
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'primary' }, '方案对比')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '7 种高亮方案对比：::selection（仅用户选区，无 JS）/ <mark>（HTML 语义但改 DOM）/ <span class="hl">（同 mark 但无语义）/ execCommand hiliteColor（已废弃）/ Canvas 叠加（性能差但完全控制视觉）/ SVG 叠加（矢量但同 Canvas 问题）/ CSS Custom Highlight API（不改 DOM + 性能优秀 + 仅文本 CSS 属性）。含决策树与混合方案（临时高亮用 Custom Highlight + 持久化用 <mark>）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('查看方案对比', { type: 'primary', size: 'sm', onClick: () => this._runCompareDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.compareInfo || '（点击按钮查看 7 种高亮方案完整对比）')),
            ],
        });
        this.registerChild(card);
        return card.render();
...
//# sourceMappingURL=CSSCustomHighlightAPIPage.js.map