...
const PHASE_NAMES = ['NONE', 'CAPTURING', 'AT_TARGET', 'BUBBLING'];
export class DOMTraversalPage extends Page {
    ...
    _traverseElements() {
        ...
        try {
            ...
            this._addLog('walker', `TreeWalker(SHOW_ELEMENT) 遍历到 ${visited.length} 个元素：`
    ...
    _traverseText() {
        ...
        try {
            ...
            while ((n = iter.nextNode())) {
                ...
                if (t)
                    texts.push(t.length > 16 ? t.slice(0, 16) + '…' : t);
            }
            ...
            this._addLog('iter', `NodeIterator(SHOW_TEXT) 遍历到 ${texts.length} 个文本节点：`
    ...
    _traverseLiOnly() {
        ...
        try {
            ...
            this._addLog('walker', `TreeWalker+filter(仅 LI) → 命中 ${lis.length} 个：`
                + lis.slice(0, 5).join('; ')
                + (lis.length > 5 ? ' ...' : ''));
        }
        catch (err) {
    ...
    _runFragmentPerf() {
        ...
        const directEl = this.$('.dt-perf-direct');
        const fragEl = this.$('.dt-perf-fragment');
        if (!directEl || !fragEl)
            return;
        const N = 500;
        ...
        this._addLog('frag', `N=${N} | 直接 appendChild: ${directMs.toFixed(3)}ms | `
    ...
    _runRangeExtract() {
        ...
        const container = this.$('.dt-range-container');
        if (!container)
            return;
        ...
        try {
            ...
            if (typeof range.getClientRects === 'function') {
                try {
                    rectsCount = range.getClientRects().length;
                }
                catch {
                    rectsCount = 0;
                }
            ...
            this._addLog('range', `range.selectNode(span) → extractContents() 取出 ${extracted.childNodes.length} 节点 | 剩余: "${remain}"`);
        }
        catch (err) {
    ...
    _demoSelection() {
        ...
        try {
            ...
            if (container && container.firstChild) {
                ...
                this._addLog('range', `Selection: rangeCount ${before}→${sel.rangeCount} | `
    ...
    _testTypedOM() {
        ...
        try {
            ...
            if (typeof el.computedStyleMap === 'function') {
                try {
                    const cfs = el.computedStyleMap().get('font-size');
            ...
            if (typeof CSSStyleValue !== 'undefined' && typeof CSSStyleValue.parse === 'function') {
                try {
                    const p = CSSStyleValue.parse('width', '50%');
            ...
            this._addLog('typed', `attributeStyleMap.set/get ✓：font-size=${fs.value}${fs.unit}, width=${w.value}${w.unit}`);
        }
        catch (err) {
    ...
    _testHighlight() {
        ...
        const container = this.$('.dt-highlight-text');
        if (!container)
            return;
        ...
        try {
            ...
            this._addLog('highlight', `CSS.highlights.set('dt-highlight', ...) ✓ | size=${size} | ::highlight 伪元素样式生效`);
        }
        catch (err) {
    ...
    _addScenario(target, type, handler, options) {
        try {
    ...
    _getEventTargets() {
        return {
            parent: this.$('.dt-event-parent'),
            child: this.$('.dt-event-child'),
            grand: this.$('.dt-event-grandchild'),
        };
    }
    // 普通监听：默认冒泡，从 grandchild(target) → child → parent
    _setupNormalListeners() {
        this._clearEventScenario();
        ...
        const mk = (label) => (e) => {
            ...
            this._addLog('event', `[普通/${label}] phase=${PHASE_NAMES[e.eventPhase] || e.eventPhase} | `
    ...
    _setupCaptureListeners() {
        ...
        const mk = (label) => (e) => {
            const target = e.target;
            const current = e.currentTarget;
            this._addLog('event', `[捕获/${label}] phase=${PHASE_NAMES[e.eventPhase] || e.eventPhase} | `
    ...
    _triggerGrandchildClick() {
        const grand = this.$('.dt-event-grandchild');
    ...
    _dispatchCustomEvent() {
        const parent = this.$('.dt-event-parent');
        if (!parent)
            return;
        ...
        const onCustom = (e) => {
            const ce = e;
            this._addLog('event', `[CustomEvent] type=${ce.type} | detail=${JSON.stringify(ce.detail)} | `
        ...
        this._addLog('event', `>>> dispatchEvent(new CustomEvent('demo-custom', { detail: ${JSON.stringify(detail)} }))`);
        try {
    ...
    _runScheduling() {
        ...
        const record = (name) => { results.push({ name, t: performance.now() - t0 }); };
        ...
        setTimeout(() => {
            ...
            this._addLog('sched', '理论顺序：微任务(queueMicrotask/Promise.then) → rAF → MessageChannel → setTimeout → requestIdleCallback');
        }, 80);
    }
    // AbortSignal 演示：addEventListener({ signal }) 通过 abort 主动取消
    _setupAbortableListener() {
        if (!this.state.abortSupported) {
            this._addLog('error', 'AbortController/AbortSignal 不可用');
            return;
        }
        const target = this.$('.dt-abort-target');
        if (!target)
            return;
        ...
        const handler = () => this._addLog('abort', '可中止监听触发（监听仍存活，未被 abort）');
    ...
    renderPage() {
        ...
        return [
            ...
            h(Card, {
                ...
                extra: h(Tag, { color: s.traversalSupported ? 'success' : 'error' }, s.traversalSupported ? '稳定' : '不支持'),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'document.createTreeWalker(root, whatToShow, filter) 创建深度优先遍历器，支持 parentNode()/firstChild()/lastChild()/previousSibling()/nextSibling()/nextNode()/previousNode()；document.createNodeIterator(root, whatToShow, filter) 创建单向迭代器，nextNode()/previousNode()/detach()。whatToShow 用 NodeFilter.SHOW_* 常量，filter.acceptNode 返回 FILTER_ACCEPT/REJECT/SKIP。'), h('div', { class: 'flex gap-sm flex-wrap' }, this._btn('遍历元素', { type: 'primary', size: 'sm', onClick: () => this._traverseElements() }), this._btn('遍历文本', { size: 'sm', onClick: () => this._traverseText() }), this._btn('仅遍历 LI', { size: 'sm', onClick: () => this._traverseLiOnly() })), h('div', { class: 'fs-sm text-secondary mt-xs' }, '目标 DOM 容器（用 h() 构造，包含 div/ul/li/span 多层嵌套）：'), h('div', { class: 'dt-tree' }, h('div', { class: 'dt-tree__a' }, '顶层 div（class=dt-tree__a）', h('ul', {}, h('li', {}, '列表项 1 — ', h('span', { class: 'tag-text' }, '强调文本')), h('li', {}, '列表项 2 — 普通'), h('li', {}, '列表项 3 — ', h('span', { class: 'tag-text' }, '另一段强调'))), h('div', { class: 'dt-tree__b' }, '嵌套 div（class=dt-tree__b）', h('ul', {}, h('li', {}, '嵌套列表项 A'), h('li', {}, '嵌套列表项 B'))))))),
            // ============ Card 2: DocumentFragment 与 Range ============
            h(Card, {
                title: '2. DocumentFragment 与 Range 深入',
                extra: h(Tag, { color: s.rangeSupported ? 'success' : 'error' }, s.rangeSupported ? '稳定' : '不支持'),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'DocumentFragment 是轻量文档片段，appendChild 多个节点只触发一次 reflow，性能优于循环 appendChild。Range 表示连续 DOM 区间，支持 setStart/setEnd/selectNode/selectNodeContents/collapse/cloneContents/extractContents/deleteContents/insertNode/surroundContents 及 getBoundingClientRect/getClientRects。Selection API（window.getSelection/removeAllRanges/addRange/getRangeAt/toString）配合 Range 管理用户选区。'), h('div', { class: 'flex gap-sm flex-wrap' }, this._btn('Fragment 性能测试', { type: 'primary', size: 'sm', onClick: () => this._runFragmentPerf() }), this._btn('Range extractContents', { size: 'sm', onClick: () => this._runRangeExtract() }), this._btn('Selection API', { size: 'sm', onClick: () => this._demoSelection() })), s.fragmentPerf ? h('div', { class: 'fs-sm' }, h('strong', {}, '性能对比：'), h('span', { class: 'text-secondary' }, ` N=${s.fragmentPerf.count} | 直接 appendChild: ${s.fragmentPerf.directMs}ms | `
                + `DocumentFragment: ${s.fragmentPerf.fragMs}ms | 加速比: ${s.fragmentPerf.ratio}x`)) : null, h('div', { class: 'dt-perf-row' }, h('div', { class: 'dt-perf-col' }, h('div', { class: 'fs-sm text-secondary mb-xs' }, '直接 appendChild 容器（每次插入触发 reflow）：'), h('ul', { class: 'dt-perf-list dt-perf-direct' })), h('div', { class: 'dt-perf-col' }, h('div', { class: 'fs-sm text-secondary mb-xs' }, 'DocumentFragment 容器（一次性插入）：'), h('ul', { class: 'dt-perf-list dt-perf-fragment' }))), h('div', { class: 'fs-sm text-secondary mt-xs' }, 'Range 操作结果（selectNode + extractContents）：'), h('div', { class: 'dt-range-container fs-sm' }), s.rangeExtractResult ? h('div', { class: 'fs-sm' }, s.rangeExtractResult) : null)),
            // ============ Card 3: CSS Typed OM 与 Custom Highlight ============
            h(Card, {
                title: '3. CSS Typed Object Model 与 CSS Custom Highlight API',
                extra: h(Tag, { color: s.typedOMSupported || s.highlightSupported ? 'warning' : 'error' }, s.typedOMSupported || s.highlightSupported ? '实验性' : '不支持'),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'CSS Typed OM：element.attributeStyleMap.set/get 返回 CSSUnitValue 等 CSSStyleValue 子类（{value, unit}），element.computedStyleMap() 返回只读 StylePropertyMapReadOnly。CSS.px/percent/vw/em 创建单位值，CSSMathSum/Product/Negate 表达数学表达式，CSSKeywordValue/ImageValue 表示其他类型，CSSStyleValue.parse/parseAll 解析字符串。CSS Custom Highlight API：new Highlight(...ranges) + CSS.highlights.set(name, highlight) 注册高亮，通过 ::highlight(name) 伪元素样式化，无需包裹 DOM 节点。'), h(Alert, {
                type: 'warning',
            ...
            }), h('div', { class: 'flex gap-sm flex-wrap' }, this._btn('测试 Typed OM', { type: 'primary', size: 'sm', onClick: () => this._testTypedOM() }), this._btn('测试 Highlight', { type: 'primary', size: 'sm', onClick: () => this._testHighlight() }), this._btn('清除 Highlight', { size: 'sm', onClick: () => this._clearHighlight() })), h('div', { class: 'dt-typed-demo' }, 'Typed OM 演示元素（点击按钮后通过 attributeStyleMap.set 修改 font-size/width）'), s.typedOMInfo ? h('pre', { class: 'code-block' }, s.typedOMInfo) : null, h('div', { class: 'fs-sm text-secondary mt-xs' }, 'Custom Highlight 演示文本（注册后 ::highlight(dt-highlight) 高亮其中片段）：'), h('div', { class: 'dt-highlight-text' }, '这是一段用于演示 CSS Custom Highlight API 的文本，点击按钮注册 Highlight 后将通过 ::highlight 伪元素高亮其中片段。'), s.highlightInfo ? h('pre', { class: 'code-block' }, s.highlightInfo) : null)),
            // ============ Card 4: Event 系统深入 ============
            h(Card, {
                title: '4. Event 系统深入（capture / once / stopPropagation / CustomEvent）',
                extra: h(Tag, { color: s.eventSupported ? 'success' : 'error' }, s.eventSupported ? '稳定' : '不支持'),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'Event 对象含 bubbles/cancelable/composed/defaultPrevented/target/currentTarget/eventPhase(0/1/2/3)/timeStamp/type；event.composedPath() 返回跨 Shadow DOM 的事件路径；stopPropagation/stopImmediatePropagation/preventDefault 控制事件流。addEventListener 第三参 options: capture（捕获阶段触发）/passive（不可 preventDefault，优化滚动）/once（触发一次后自动移除）/signal（AbortSignal，可主动取消）。CustomEvent(type, { detail, bubbles, cancelable }) 携带数据；dispatchEvent 派发；EventTarget 类可被继承实现自定义事件目标。'), h('div', { class: 'flex gap-sm flex-wrap' }, this._btn('普通监听', { type: 'primary', size: 'sm', onClick: () => this._setupNormalListeners() }), this._btn('捕获监听', { type: 'primary', size: 'sm', onClick: () => this._setupCaptureListeners() }), this._btn('stopPropagation', { size: 'sm', onClick: () => this._setupStopPropagation() }), this._btn('once 监听', { size: 'sm', onClick: () => this._setupOnceListeners() }), this._btn('触发 grandchild 点击', { type: 'primary', size: 'sm', onClick: () => this._triggerGrandchildClick() }), this._btn('派发 CustomEvent', { size: 'sm', onClick: () => this._dispatchCustomEvent() })), h('div', { class: 'fs-sm text-secondary' }, '三层嵌套 div（parent → child → grandchild），先选择场景再点击触发：'), h('div', { class: 'dt-event-parent' }, h('div', { class: 'fs-sm fw-medium' }, 'parent（蓝色边框）'), h('div', { class: 'dt-event-child' }, h('div', { class: 'fs-sm fw-medium' }, 'child（绿色边框）'), h('div', { class: 'dt-event-grandchild' }, h('div', { class: 'fs-sm fw-medium' }, 'grandchild（橙色边框，点击我或上方按钮）')))))),
            // ============ Card 5: 调度对比 ============
            h(Card, {
                title: '5. 调度对比（queueMicrotask / Promise / setTimeout / rAF / MessageChannel / requestIdleCallback）',
                extra: h(Tag, { color: s.schedulingSupported ? 'success' : 'error' }, s.schedulingSupported ? '稳定' : '不支持'),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'queueMicrotask 与 Promise.resolve().then 为微任务，在当前同步任务后立即执行；setTimeout(cb, 0) 为宏任务，嵌套调用会被 clamp 到 4ms；requestAnimationFrame 在下一帧渲染前执行（约 16ms）；MessageChannel.postMessage 也是宏任务，但无 4ms clamp；requestIdleCallback 在浏览器空闲时执行（可用 timeout 强制）。理论顺序：微任务 → rAF → MessageChannel → setTimeout → requestIdleCallback。'), h('div', { class: 'flex gap-sm flex-wrap' }, this._btn('调度对比', { type: 'primary', size: 'sm', onClick: () => this._runScheduling() })), h(Alert, {
                type: 'info',
            ...
            }), h('div', { class: 'flex gap-sm flex-wrap' }, this._btn('添加 signal 监听', { type: 'primary', size: 'sm', onClick: () => this._setupAbortableListener() }), this._btn('abort 中止', { danger: true, size: 'sm', onClick: () => this._abortListener() }), this._btn('触发目标点击', { size: 'sm', onClick: () => this._triggerAbortDemo() })), h('div', { class: 'dt-abort-target' }, '可中止监听目标（点击触发监听）'))),
            // ============ 日志面板 ============
            h(Card, {
                title: '事件日志',
...
//# sourceMappingURL=DOMTraversalPage.js.map