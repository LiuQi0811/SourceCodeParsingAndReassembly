...
export class DeviceSensorsPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!this._sensorChecked) {
            ...
            this._addLog('sensor', `传感器能力检测完成：可用 ${supported.length}/${SENSOR_CTOR_NAMES.length}` +
    ...
    componentWillUnmount() {
        ...
        if (this._orientUnbind) {
            try {
                this._orientUnbind();
            }
            catch { /* noop */ }
            this._orientUnbind = null;
        }
        if (this._motionUnbind) {
            try {
                this._motionUnbind();
            }
            catch { /* noop */ }
            this._motionUnbind = null;
        }
        // 3. 停止活跃的 Sensor 对象
        if (this._accelSensor) {
            try {
                this._accelSensor.stop();
            }
            catch { /* noop */ }
    ...
    _geoGet() {
        ...
        try {
            navigator.geolocation.getCurrentPosition((pos) => this._onGeoSuccess(pos, 'getCurrentPosition'), (err) => this._onGeoError(err), { enableHighAccuracy: true, timeout: 5000, maximumAge: 0 });
        }
        catch (err) {
    ...
    _geoWatch() {
        ...
        try {
            this._geoWatchId = navigator.geolocation.watchPosition((pos) => this._onGeoSuccess(pos, 'watchPosition'), (err) => this._onGeoError(err), { enableHighAccuracy: true, timeout: 5000, maximumAge: 1000 });
    ...
    _onGeoSuccess(pos, source) {
        ...
        this._addLog('geo', `[${source}] lat=${coords.latitude.toFixed(5)} lng=${coords.longitude.toFixed(5)} ±${coords.accuracy.toFixed(0)}m`);
    }
    _onGeoError(err) {
    ...
    _onDeviceOrientation(e) {
        ...
        const now = Date.now();
        if (this._lastOrientTs && now - this._lastOrientTs < 100)
            return;
        ...
        if (!this._lastOrientLogTs || now - this._lastOrientLogTs >= 1000) {
            ...
            this._addLog('orient', `alpha=${alpha.toFixed(1)}° beta=${beta.toFixed(1)}° gamma=${gamma.toFixed(1)}° absolute=${e.absolute}`);
        }
    }
    _onDeviceMotion(e) {
        // 节流日志：每 ~500ms 记一条
        const now = Date.now();
        if (this._lastMotionTs && now - this._lastMotionTs < 500)
            return;
        this._lastMotionTs = now;
        const acc = e.accelerationIncludingGravity || {};
        const rate = e.rotationRate || {};
        this._addLog('orient', `devicemotion accG(x,y,z)=${(acc.x ?? 0).toFixed(1)},${(acc.y ?? 0).toFixed(1)},${(acc.z ?? 0).toFixed(1)} ` +
    ...
    async _accelStart() {
        ...
        try {
            const sensor = new window.Accelerometer({ frequency: 10 }); // 10 Hz
            ...
            this._addLog('sensor', `Accelerometer.start() frequency=10Hz activated=${sensor.activated} hasReading=${sensor.hasReading}`);
        }
        catch (err) {
    ...
    _accelStop() {
        ...
        try {
            ...
            s.stop();
            this._addLog('sensor', `Accelerometer.stop() activated=${s.activated} hasReading=${s.hasReading} timestamp=${s.timestamp ?? 'N/A'}`);
        }
        catch (err) {
    ...
    async _barcodeDetect(file) {
        ...
        if (!file)
            return;
        if (!this._barcodeFormats)
            await this._barcodeInitFormats();
        ...
        const results = await Promise.all(PERMISSION_NAMES.map(async (name) => {
            try {
                ...
                status.onchange = () => {
                    ...
                    this.setState({
                        permissionsList: this.state.permissionsList.map((p) => p.name === name ? { ...p, state: status.state } : p),
                    });
                };
    ...
    _kvRow(label, value, unit = '') {
        ...
        return h('div', { class: 'flex items-center gap-sm' }, h('span', { class: 'fs-sm text-secondary', style: { minWidth: '120px' } }, label), h('span', { class: 'fs-sm' }, text));
    }
    renderPage() {
        ...
        return [
            ...
            h(Card, {
                ...
                extra: h('span', { class: 'flex items-center gap-xs' }, h(Tag, { color: 'primary' }, 'navigator.geolocation'), this._capTag(caps.geo)),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-tertiary' }, 'getCurrentPosition(success, error, options) 单次定位；watchPosition 持续监听并返回 watchId，clearWatch(id) 停止。options 可设 enableHighAccuracy / timeout / maximumAge。position.coords 含 latitude、longitude、accuracy、altitude、altitudeAccuracy、heading、speed，position.timestamp 为采集时间戳。'), h('div', { class: 'flex flex-wrap gap-sm' }, this._btn('获取定位', {
                ...
                ? h('div', { class: 'log-panel', style: { height: 'auto', padding: '12px' } }, this._kvRow('latitude', this.state.geoCoords.latitude.toFixed(6), '°'), this._kvRow('longitude', this.state.geoCoords.longitude.toFixed(6), '°'), this._kvRow('accuracy', this.state.geoCoords.accuracy.toFixed(1), 'm'), this._kvRow('altitude', this.state.geoCoords.altitude, 'm'), this._kvRow('altitudeAccuracy', this.state.geoCoords.altitudeAccuracy, 'm'), this._kvRow('heading', this.state.geoCoords.heading, '°'), this._kvRow('speed', this.state.geoCoords.speed, 'm/s'), this._kvRow('timestamp', this.state.geoCoords.timestamp, 'ms'))
                : h('p', { class: 'fs-sm text-tertiary' }, '（点击「获取定位」后，坐标会显示在这里）'), h('p', { class: 'fs-xs text-tertiary' }, '错误码：1=PERMISSION_DENIED，2=POSITION_UNAVAILABLE，3=TIMEOUT。调用需用户授权，且通常要求 HTTPS。'))), 
            // ============ 2. Device Orientation & Motion ============
            h(Card, {
                title: '2. Device Orientation & Motion 设备方向与运动',
                extra: h('span', { class: 'flex items-center gap-xs' }, h(Tag, { color: 'primary' }, 'deviceorientation'), this._capTag(caps.orient)),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-tertiary' }, 'deviceorientation 事件提供 alpha（罗盘方向 0-360）、beta（前后倾斜 -180~180）、gamma（左右倾斜 -90~90）、absolute。devicemotion 事件提供 accelerationIncludingGravity {x,y,z}、acceleration、rotationRate {alpha,beta,gamma}、interval。'), caps.orient === false ? h(Alert, {
                type: 'warning',
            ...
            }), h('div', { class: 'flex flex-wrap gap-sm' }, this._btn('开始方向监听', {
                type: 'primary', size: 'sm',
            ...
            })), h('div', { class: 'flex items-center gap-md' }, h('div', {
            ...
            }, 'N')), h('div', { class: 'fs-sm' }, this._kvRow('alpha', ov.alpha.toFixed(1), '°'), this._kvRow('beta', ov.beta.toFixed(1), '°'), this._kvRow('gamma', ov.gamma.toFixed(1), '°'))), h('p', { class: 'fs-xs text-tertiary' }, '左侧方块 transform: rotate(alpha) 跟随罗盘方向旋转。方向事件已节流至约 10 次/秒，devicemotion 日志每 500ms 一条。'))), 
            // ============ 3. Generic Sensor API ============
            h(Card, {
                title: '3. Generic Sensor API 通用传感器',
                extra: h('span', { class: 'flex items-center gap-xs' }, h(Tag, { color: 'primary' }, 'Sensor APIs'), this._capTag(sensorSupport.Accelerometer)),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-tertiary' }, 'Generic Sensor API 将传感器抽象为对象：new Accelerometer({ frequency: 10 }) 构造，addEventListener(\'reading\') 读取 x/y/z，\'error\' 监听错误，start()/stop() 控制。sensor.activated / hasReading / timestamp 反映状态，可结合 navigator.permissions.query({ name: \'accelerometer\' }) 查权限。多为 Chromium 实验性支持。'), h('div', { class: 'flex flex-wrap gap-xs' }, ...SENSOR_CTOR_NAMES.map((name) => h('span', { class: 'flex items-center gap-xs' }, h('span', { class: 'fs-xs text-secondary' }, name), sensorSupport[name]
                ? h(Tag, { color: 'success' }, '支持')
                : h(Tag, { color: 'error' }, '不支持')))), h('div', { class: 'flex flex-wrap gap-sm' }, this._btn('启动加速度计', {
                type: 'primary', size: 'sm',
            ...
            })), h('div', { class: 'log-panel', style: { height: 'auto', padding: '12px' } }, this._kvRow('x', av.x.toFixed(3)), this._kvRow('y', av.y.toFixed(3)), this._kvRow('z', av.z.toFixed(3)), this._kvRow('activated', this.state.accelActive ? 'true' : 'false')), h('p', { class: 'fs-xs text-tertiary' }, 'Accelerometer 以 10Hz 采样；点击启动前会先查询 accelerometer 权限。其余传感器（Gyroscope / Magnetometer 等）API 用法一致。'))), 
            // ============ 4. Barcode Detection ============
            h(Card, {
                title: '4. Barcode Detection 条码检测',
                extra: h('span', { class: 'flex items-center gap-xs' }, h(Tag, { color: 'primary' }, 'BarcodeDetector'), this._capTag(caps.barcode)),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-tertiary' }, 'BarcodeDetector.getSupportedFormats() 返回支持的格式数组；new BarcodeDetector({ formats: [...] }) 构造；detector.detect(canvasOrImage) 返回 [{ rawValue, boundingBox, cornerPoints, format }]。'), caps.barcode === false ? h(Alert, {
                type: 'warning',
            ...
            }) : null, h('div', { class: 'flex flex-wrap gap-sm items-center' }, this._btn('查询支持格式', {
                size: 'sm',
            ...
            }, caps.barcode ? '选择图片检测' : '（不支持）', h('input', {
                type: 'file',
                accept: 'image/*',
                disabled: !caps.barcode,
                style: { display: 'none' },
                ...
                ? h('div', { class: 'log-panel', style: { height: 'auto', padding: '12px', whiteSpace: 'pre-wrap' } }, this.state.barcodeResult)
                : h('p', { class: 'fs-sm text-tertiary' }, '（选择包含条码的图片后，检测结果会显示在这里）'))), 
            // ============ 5. Permissions API ============
            h(Card, {
                title: '5. Permissions API 权限查询',
                extra: h('span', { class: 'flex items-center gap-xs' }, h(Tag, { color: 'primary' }, 'navigator.permissions'), this._capTag(caps.permissions)),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-tertiary' }, 'navigator.permissions.query({ name }) 返回 PermissionStatus { state, onchange }，状态为 granted / denied / prompt。监听 onchange 可捕获权限变化。部分权限名称在不支持时会抛 TypeError，需 try/catch。'), h('div', { class: 'flex flex-wrap gap-sm' }, this._btn('刷新权限', {
                type: 'primary', size: 'sm',
                onClick: () => this._refreshPermissions(),
                disabled: !caps.permissions,
            })), this.state.permissionsList.length > 0
                ? h('div', { class: 'flex flex-col gap-xs' }, ...this.state.permissionsList.map((p) => h('div', {
                    class: 'flex items-center gap-sm',
                    style: { padding: '6px 8px', background: 'rgba(255,255,255,0.03)', borderRadius: '6px' },
                }, h('span', { class: 'fs-sm', style: { minWidth: '140px' } }, p.name), h(Tag, { color: this._permStateColor(p.state) }, p.state), h('span', { class: 'fs-xs text-tertiary' }, p.note))))
...
//# sourceMappingURL=DeviceSensorsPage.js.map