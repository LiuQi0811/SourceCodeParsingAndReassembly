...
export class CanvasRecordingPage extends Page {
    ...
    componentDidMount() {
        ...
        if (this._animIds == null)
            this._animIds = {};
        // Card 2：captureStream
        if (this._capStream === undefined)
            this._capStream = null;
        if (this._capTrack === undefined)
            this._capTrack = null;
        // Card 3：MediaRecorder
        if (this._recStream === undefined)
            this._recStream = null;
        if (this._recTrack === undefined)
            this._recTrack = null;
        if (this._recorder === undefined)
            this._recorder = null;
        if (this._recChunks === undefined)
            this._recChunks = [];
        if (this._recUrl === undefined)
            this._recUrl = null;
        if (this._recMime === undefined)
            this._recMime = '';
        // Card 4：端到端
        if (this._e2eStream === undefined)
            this._e2eStream = null;
        if (this._e2eTrack === undefined)
            this._e2eTrack = null;
        if (this._e2eRecorder === undefined)
            this._e2eRecorder = null;
        if (this._e2eChunks === undefined)
            this._e2eChunks = [];
        if (this._e2eUrl === undefined)
            this._e2eUrl = null;
        if (this._e2eMime === undefined)
            this._e2eMime = '';
        if (this._e2eTimer === undefined)
            this._e2eTimer = null;
        if (this._e2eStopTimer === undefined)
            this._e2eStopTimer = null;
        if (this._e2eStart === undefined)
            this._e2eStart = 0;
        // Card 5：WebCodecs
        if (this._encoder === undefined)
            this._encoder = null;
        if (this._encodedChunks === undefined)
            this._encodedChunks = [];
        // Card 6：音频混合
        if (this._audioCtx === undefined)
            this._audioCtx = null;
        if (this._audioDest === undefined)
            this._audioDest = null;
        if (this._audioOsc === undefined)
            this._audioOsc = null;
        if (this._audioVideoStream === undefined)
            this._audioVideoStream = null;
        if (this._audioRecorder === undefined)
            this._audioRecorder = null;
        if (this._audioChunks === undefined)
            this._audioChunks = [];
        if (this._audioUrl === undefined)
            this._audioUrl = null;
        if (this._audioMime === undefined)
            this._audioMime = '';
        if (this._audioTimer === undefined)
            this._audioTimer = null;
        if (this._audioStopTimer === undefined)
            this._audioStopTimer = null;
        if (this._audioStart === undefined)
            this._audioStart = 0;
        ...
        const summary = '能力检测：HTMLCanvasElement=' + (caps.canvas ? '✓' : '✗') +
        ...
        if (!caps.canvas) {
            ...
            if (!caps.toDataURL)
                this._addLog('warn', 'toDataURL 不可用或会抛 "Not implemented"（jsdom 限制），Card 1 将记录错误');
            if (!caps.toBlob)
                this._addLog('warn', 'toBlob 不可用（jsdom 限制），Card 1 toBlob 演示将跳过');
            if (!caps.offscreen || !caps.convertToBlob)
                this._addLog('warn', 'OffscreenCanvas/convertToBlob 不可用，Card 1 convertToBlob 将跳过');
            if (!caps.captureStream)
                this._addLog('warn', 'canvas.captureStream 不可用（jsdom 限制）；需 Chrome 51+/Firefox 43+/Safari 14+，Card 2/3/4/6 实时流演示将仅记流程');
            if (!caps.mediaRecorder)
                this._addLog('warn', 'MediaRecorder 不可用，Card 3/4/6 录制演示将仅记流程');
            if (!caps.isTypeSupported)
                this._addLog('warn', 'MediaRecorder.isTypeSupported 不可用（polyfill 限制），mime 检测将标记为未知');
            if (!caps.videoFrame || !caps.videoEncoder)
                this._addLog('warn', 'WebCodecs 不可用（VideoFrame/VideoEncoder）；需 Chrome 94+，Card 5 将仅记流程');
            if (!caps.audioContext || !caps.mediaStreamDest)
                this._addLog('warn', 'AudioContext/createMediaStreamDestination 不可用，Card 6 音频混合将仅记流程');
        }
    }
    componentWillUnmount() {
        ...
        // —— Card 2：captureStream ——
        this._stopTrack(this._capTrack);
        this._capTrack = null;
        this._stopStream(this._capStream);
        this._capStream = null;
        // —— Card 3：MediaRecorder ——
        this._safeStopRecorder(this._recorder);
        this._stopTrack(this._recTrack);
        this._recTrack = null;
        this._stopStream(this._recStream);
        this._recStream = null;
        ...
        try {
            if (this._e2eTimer) {
                clearInterval(this._e2eTimer);
                this._e2eTimer = null;
            }
        }
        catch { /* noop */ }
        try {
            if (this._e2eStopTimer) {
                clearTimeout(this._e2eStopTimer);
                this._e2eStopTimer = null;
            }
        }
        catch { /* noop */ }
        this._safeStopRecorder(this._e2eRecorder);
        this._stopTrack(this._e2eTrack);
        this._e2eTrack = null;
        this._stopStream(this._e2eStream);
        this._e2eStream = null;
        this._revoke(this._e2eUrl);
        this._e2eUrl = null;
        // —— Card 5：WebCodecs ——
        try {
            if (this._encoder && typeof this._encoder.close === 'function')
                this._encoder.close();
        }
        catch { /* noop */ }
        this._encoder = null;
        ...
        try {
            if (this._audioOsc && typeof this._audioOsc.stop === 'function')
                this._audioOsc.stop();
        }
        catch { /* noop */ }
        this._audioOsc = null;
        this._safeStopRecorder(this._audioRecorder);
        this._stopStream(this._audioVideoStream);
        this._audioVideoStream = null;
        try {
    ...
    _caps() {
        ...
        try {
            ...
            if (caps.offscreen) {
                try {
                    caps.convertToBlob = typeof OffscreenCanvas.prototype.convertToBlob === 'function';
                }
                catch { /* noop */ }
            }
            ...
            if (caps.mediaRecorder) {
                try {
                    caps.isTypeSupported = typeof MediaRecorder.isTypeSupported === 'function';
                }
                catch { /* noop */ }
            }
            ...
            if (AC) {
                try {
                    caps.mediaStreamDest = typeof AC.prototype.createMediaStreamDestination === 'function';
                }
                catch { /* noop */ }
            }
        }
        ...
        try {
            ...
            for (let i = 0; i < 3; i++) {
                if (typeof ctx.beginPath === 'function')
                    ctx.beginPath();
                if (typeof ctx.arc === 'function')
                    ctx.arc(48 + i * 64, h / 2, 26, 0, Math.PI * 2);
                if ('fillStyle' in ctx)
    ...
    _startAnim(id) {
        ...
        if (!ctx) {
            this._addLog('warn', `${id} 元素不可用，无法启动动画`);
            return false;
        }
        const canvas = document.getElementById(id);
    ...
    _drawExportScene() {
        ...
        if (!ctx) {
            this._addLog('warn', 'canvas-export 元素不可用');
            return;
        }
    ...
    _toDataURLDemo(type, quality) {
        const caps = this._caps();
        if (!caps.toDataURL) {
            this._addLog('warn', 'canvas.toDataURL 不可用（jsdom 限制）');
            return;
        }
        const canvas = document.getElementById('canvas-export');
        if (!canvas) {
    ...
    _toBlobDemo(type, quality) {
        const caps = this._caps();
        if (!caps.toBlob) {
            this._addLog('warn', 'canvas.toBlob 不可用（jsdom 限制）；真实浏览器异步返回 Blob，比 toDataURL 内存友好');
            return;
        }
        const canvas = document.getElementById('canvas-export');
        if (!canvas) {
        ...
        try {
            canvas.toBlob((blob) => {
                if (!blob) {
                    this._addLog('warn', 'toBlob 回调收到 null');
                    return;
                }
    ...
    _convertToBlobDemo(type, quality) {
        ...
        try {
            ...
            if (octx)
                this._drawStaticScene(octx, 160, 100, 'OFFSCREEN');
    ...
    _startCapture(fps) {
        ...
        if (!ctx) {
            this._addLog('warn', 'canvas-capture 元素不可用');
            return;
        }
    ...
    _stopCapture(silent) {
        ...
        this._stopAnim('canvas-capture');
        if (!silent)
            this._addLog('capture', '已停止捕获：track.stop() + 取消 rAF 动画');
        this.setState({ captureInfo: '已停止' });
    }
    _requestManualFrame() {
        if (!this._capTrack) {
            this._addLog('warn', '请先点击「captureStream(0) 手动」启动手动模式');
            return;
        }
        if (typeof this._capTrack.requestFrame !== 'function') {
            this._addLog('warn', 'track.requestFrame 不可用（环境限制）');
            return;
        }
        try {
    ...
    _checkMimeSupport() {
        const caps = this._caps();
        if (!caps.mediaRecorder) {
            this._addLog('warn', 'MediaRecorder 不可用，无法检测 mimeType');
            return;
        }
        ...
        for (const mt of RECORDER_MIME_TYPES) {
            ...
            if (caps.isTypeSupported) {
                try {
                    supported = MediaRecorder.isTypeSupported(mt);
                }
                catch { /* noop */ }
            }
    ...
    _startRecorder(mime, timeslice) {
        const caps = this._caps();
        if (!caps.mediaRecorder) {
            this._addLog('warn', 'MediaRecorder 不可用');
            return;
        }
        if (!caps.captureStream) {
            this._addLog('warn', 'canvas.captureStream 不可用，无法录制');
            return;
        }
        const ctx = this._getCtx('canvas-recorder');
        if (!ctx) {
            this._addLog('warn', 'canvas-recorder 元素不可用');
            return;
        }
        ...
        recorder.onstop = () => {
            ...
            this._recUrl = url;
            if (blob)
                this._addLog('rec', `onstop：录制结束，可点击下方 <video> 回放`);
    ...
    _pauseRecorder() {
        if (!this._recorder) {
            this._addLog('warn', '尚未开始录制');
            return;
        }
        if (typeof this._recorder.pause !== 'function') {
            this._addLog('warn', 'recorder.pause 不可用（polyfill 限制）');
            return;
        }
        try {
    ...
    _resumeRecorder() {
        ...
        if (typeof this._recorder.resume !== 'function') {
            this._addLog('warn', 'recorder.resume 不可用（polyfill 限制）');
            return;
        }
        try {
    ...
    _requestData() {
        ...
        if (typeof this._recorder.requestData !== 'function') {
            this._addLog('warn', 'recorder.requestData 不可用（polyfill 限制）');
            return;
        }
        try {
    ...
    _stopRecorder() {
        ...
        if (this._recorder.state === 'inactive') {
            this._addLog('warn', 'recorder 已 inactive');
            return;
        }
        try {
            this._recorder.stop();
    ...
    _runE2E(seconds = 3) {
        ...
        if (!ctx) {
            this._addLog('warn', 'canvas-e2e 元素不可用');
            return;
        }
        ...
        this._e2eMime = mime;
        this._e2eChunks = [];
        recorder.ondataavailable = (e) => { if (e && e.data && e.data.size > 0)
            this._e2eChunks.push(e.data); };
        recorder.onstop = () => {
            this._revoke(this._e2eUrl);
        ...
        this._e2eStopTimer = setTimeout(() => this._stopE2E(undefined), seconds * 1000);
    }
    _stopE2E(silent) {
        if (this._e2eTimer) {
            try {
    ...
    _runWebCodecs() {
        ...
        if (!ctx) {
            this._addLog('warn', 'canvas-codecs 元素不可用');
            return;
        }
    ...
    _runAudioRecording(seconds = 3) {
        ...
        if (!caps.captureStream) {
            this._addLog('warn', 'canvas.captureStream 不可用');
            return;
        }
        if (!caps.audioContext || !caps.mediaStreamDest) {
        ...
        if (!ctx) {
            this._addLog('warn', 'canvas-audio 元素不可用');
            return;
        }
        ...
        try {
            ...
            if (typeof osc.connect === 'function')
                osc.connect(gain);
            if (typeof gain.connect === 'function')
                gain.connect(dest);
        ...
        recorder.onstop = () => {
            ...
            this._audioUrl = url;
            if (blob)
                this._addLog('audio', `onstop：含音轨录制完成 ${blob.size} bytes (${(blob.size / 1024).toFixed(1)} KB)`);
        ...
        this._audioStopTimer = setTimeout(() => this._stopAudioRecording(undefined), seconds * 1000);
    }
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.toDataURL ? 'success' : 'warning' }, caps.toDataURL ? 'toDataURL ✓' : 'toDataURL ✗'), h(Tag, { color: caps.toBlob ? 'success' : 'warning' }, caps.toBlob ? 'toBlob ✓' : 'toBlob ✗'), h(Tag, { color: caps.convertToBlob ? 'success' : 'warning' }, caps.convertToBlob ? 'convertToBlob ✓' : 'convertToBlob ✗')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '演示：先绘制静态场景，再分别用 toDataURL（同步 base64）/ toBlob（异步二进制）/ OffscreenCanvas.convertToBlob（Promise，Worker 友好）导出。toDataURL 内存占用约为原始数据 1.33 倍（base64 编码）；toBlob 不进 base64 更省内存；convertToBlob 可在 Worker 中运行。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('绘制静态场景', { type: 'primary', size: 'sm', onClick: () => this._drawExportScene() }), this._btn('toDataURL PNG', { size: 'sm', disabled: !caps.toDataURL, onClick: () => this._toDataURLDemo('image/png', undefined) }), this._btn('toDataURL JPEG q=0.5', { size: 'sm', disabled: !caps.toDataURL, onClick: () => this._toDataURLDemo('image/jpeg', 0.5) }), this._btn('toDataURL WebP q=0.8', { size: 'sm', disabled: !caps.toDataURL, onClick: () => this._toDataURLDemo('image/webp', 0.8) }), this._btn('toBlob PNG', { size: 'sm', disabled: !caps.toBlob, onClick: () => this._toBlobDemo('image/png', undefined) }), this._btn('toBlob WebP q=0.8', { size: 'sm', disabled: !caps.toBlob, onClick: () => this._toBlobDemo('image/webp', 0.8) }), this._btn('OffscreenCanvas convertToBlob', { size: 'sm', disabled: !caps.offscreen || !caps.convertToBlob, onClick: () => this._convertToBlobDemo('image/png', undefined) }), this._btn('跨域污染说明', { size: 'sm', onClick: () => this._explainTainted() })),
                h('canvas', {
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.captureStream ? 'success' : 'warning' }, caps.captureStream ? 'captureStream ✓' : 'captureStream ✗'), h(Tag, { color: caps.requestFrame ? 'success' : 'warning' }, caps.requestFrame ? 'requestFrame ✓' : 'requestFrame ✗')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '演示：启动 rAF 动画后调用 captureStream，把 canvas 动画作为媒体源。frameRate=0 时需手动 requestFrame() 推送每一帧；不传或 >0 时浏览器按帧率自动捕获。该 MediaStream 可用于 WebRTC 推流、MediaRecorder 录制、<video> 实时显示。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('captureStream(30)', { type: 'primary', size: 'sm', disabled: !caps.captureStream, onClick: () => this._startCapture(30) }), this._btn('captureStream() 自动', { size: 'sm', disabled: !caps.captureStream, onClick: () => this._startCapture(undefined) }), this._btn('captureStream(0) 手动', { size: 'sm', disabled: !caps.captureStream, onClick: () => this._startCapture(0) }), this._btn('requestFrame()', { size: 'sm', disabled: !caps.requestFrame, onClick: () => this._requestManualFrame() }), this._btn('停止捕获', { size: 'sm', onClick: () => this._stopCapture(false) })),
                h('canvas', {
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.mediaRecorder ? 'success' : 'warning' }, caps.mediaRecorder ? 'MediaRecorder ✓' : 'MediaRecorder ✗'), h(Tag, { color: caps.isTypeSupported ? 'success' : 'warning' }, caps.isTypeSupported ? 'isTypeSupported ✓' : 'isTypeSupported ✗'), h(Tag, { color: 'primary' }, `state: ${s.recorderState}`), s.recorderMime ? h(Tag, { color: 'primary' }, s.recorderMime) : null),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '演示：先检测 mimeType 支持，再用 captureStream(30) 的流构造 MediaRecorder。start(timeslice) 中 timeslice>0 每 N ms 产出一次 chunk；=0 仅在 stop 时一次性产出。pause/resume 暂停恢复；requestData 手动触发 dataavailable；stop 后合并 chunks 为 Blob 经 URL.createObjectURL 在 <video> 回放。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('检测 mimeType 支持', { size: 'sm', disabled: !caps.mediaRecorder, onClick: () => this._checkMimeSupport() }), this._btn('start 默认 webm', { type: 'primary', size: 'sm', disabled: recDisabled, onClick: () => this._startRecorder('video/webm', 0) }), this._btn('start vp9 timeslice=1000', { size: 'sm', disabled: recDisabled, onClick: () => this._startRecorder('video/webm;codecs=vp9', 1000) }), this._btn('pause', { size: 'sm', disabled: !caps.mediaRecorder, onClick: () => this._pauseRecorder() }), this._btn('resume', { size: 'sm', disabled: !caps.mediaRecorder, onClick: () => this._resumeRecorder() }), this._btn('requestData', { size: 'sm', disabled: !caps.mediaRecorder, onClick: () => this._requestData() }), this._btn('stop', { size: 'sm', disabled: !caps.mediaRecorder, onClick: () => this._stopRecorder() })),
                h('canvas', {
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: disabled ? 'warning' : 'success' }, disabled ? '不可用' : '可用'), h(Tag, { color: 'primary' }, s.e2eProgress || '尚未录制')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '演示：一键执行完整流程——启动 rAF 动画 → captureStream(30) → MediaRecorder（2.5Mbps）→ start → 3 秒后自动 stop → onstop 合并 Blob → URL.createObjectURL → <video> 回放。录制时长与文件大小正相关；码率越高画质越好但文件越大。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('录制 3 秒 (30fps, 2.5Mbps)', { type: 'primary', size: 'sm', disabled, onClick: () => this._runE2E(3) }), this._btn('停止', { size: 'sm', disabled, onClick: () => this._stopE2E(false) })),
                h('canvas', {
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.videoFrame ? 'success' : 'warning' }, caps.videoFrame ? 'VideoFrame ✓' : 'VideoFrame ✗'), h(Tag, { color: caps.videoEncoder ? 'success' : 'warning' }, caps.videoEncoder ? 'VideoEncoder ✓' : 'VideoEncoder ✗')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '演示：从 canvas 创建 10 个 VideoFrame（每帧 timestamp = i × 1e6/30 微秒），用 VideoEncoder 编码为 vp09（VP9）。每个 EncodedVideoChunk 经 output 回调收集，最后 flush() 刷出剩余数据。WebCodecs 提供帧级精确控制，比 MediaRecorder 黑盒更灵活，但需自行用 mp4-muxer / webm-muxer 等库封装容器。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('VideoFrame + VideoEncoder 编码 10 帧', { type: 'primary', size: 'sm', disabled, onClick: () => this._runWebCodecs() })),
                h('canvas', {
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.audioContext ? 'success' : 'warning' }, caps.audioContext ? 'AudioContext ✓' : 'AudioContext ✗'), h(Tag, { color: caps.mediaStreamDest ? 'success' : 'warning' }, caps.mediaStreamDest ? 'MediaStreamDest ✓' : 'MediaStreamDest ✗'), h(Tag, { color: 'primary' }, s.audioInfo || '尚未录制')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '演示：创建 AudioContext + MediaStreamDestination，振荡器（440Hz 正弦波）经 Gain 连接到音频目标；canvas.captureStream(30) 取视频轨；new MediaStream([...视频轨, ...音频轨]) 合并；MediaRecorder 录制 3 秒含音轨视频。同时 Gain 连接到 audioCtx.destination 便于实时监听。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('录制 3 秒 (canvas + 440Hz 音频)', { type: 'primary', size: 'sm', disabled, onClick: () => this._runAudioRecording(3) }), this._btn('停止', { size: 'sm', disabled, onClick: () => this._stopAudioRecording(false) })),
                h('canvas', {
    ...
    renderPage() {
        ...
        return [
            ...
            h('p', { class: 'fs-sm text-secondary mb-md' }, '演示 Canvas 媒体输出全链路：静态导出（toDataURL/toBlob/convertToBlob）→ 实时流捕获（captureStream）→ MediaRecorder 录制 → 端到端实战 → WebCodecs 帧级编码 → 音频混合录制。'),
...
//# sourceMappingURL=CanvasRecordingPage.js.map