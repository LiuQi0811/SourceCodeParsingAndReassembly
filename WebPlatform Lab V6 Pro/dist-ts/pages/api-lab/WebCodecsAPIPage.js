...
export class WebCodecsAPIPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!f.videoEncoder)
            this._addLog('warn', 'VideoEncoder 不可用（jsdom 不实现；Chrome 94+/Safari 17+ 支持，Firefox 部分支持 behind flag）');
        if (!f.videoDecoder)
            this._addLog('warn', 'VideoDecoder 不可用（jsdom 不实现；Chrome 94+/Safari 17+ 支持）');
        if (!f.videoFrame)
            this._addLog('warn', 'VideoFrame 不可用（jsdom 不实现；WebCodecs 帧数据容器）');
        if (!f.audioEncoder)
            this._addLog('warn', 'AudioEncoder 不可用（jsdom 不实现；Chrome 94+ 支持）');
        if (!f.imageDecoder)
            this._addLog('warn', 'ImageDecoder 不可用（jsdom 不实现；Chrome 94+/Safari 17+ 支持）');
    ...
    _readVideoEncoderInfo() {
        ...
        try {
            return `===== VideoEncoder 编码器 =====\n` +
                ...
                `      console.log('编码输出', chunk.type, (chunk as any).byteLength);\n` +
                `    },\n` +
    ...
    async _runVideoEncoderDemo() {
        ...
        if (f.isConfigSupported) {
            try {
                ...
                this._addLog(result.supported ? 'info' : 'warn', `VideoEncoder.isConfigSupported(${this._probeCodec}) → supported=${result.supported}` +
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'VideoEncoder 通过 new VideoEncoder({ output, error }) 构造，configure({ codec, width, height, bitrate, framerate, hardwareAcceleration }) 配置编码参数。encode(frame, { keyFrame }) 编码单个 VideoFrame，flush() 等待所有挂起帧完成，reset() 重置到未配置状态，close() 释放资源。output 回调收到 EncodedVideoChunk（type: key/delta，timestamp/duration 微秒，copyTo 拷贝数据）。hardwareAcceleration 支持 no-preference/prefer-hardware/prefer-software。codec 字符串如 avc1.42E01E（H.264 Baseline）、vp09.00.10.08（VP9）、av01.0.04M.08（AV1）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行编码演示', { type: 'primary', size: 'sm', onClick: () => this._runVideoEncoderDemo() }), ...codecs.map(([codec, name]) => this._btn(name, { size: 'sm', onClick: () => this._setProbeCodec(codec) }))),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '当前探测 codec：' + this._probeCodec),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.videoEncoderInfo || '（点击「运行编码演示」探测 codec 支持并查看完整代码）')),
                h(Alert, {
                    type: 'info',
    ...
    async _runVideoDecoderDemo() {
        ...
        if (f.isConfigSupported) {
            try {
                ...
                this._addLog(result.supported ? 'info' : 'warn', `VideoDecoder.isConfigSupported(${this._decoderCodec}, hw=${this._hwAccel}) → supported=${result.supported}` +
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'VideoDecoder 通过 new VideoDecoder({ output, error }) 构造，configure({ codec, codedWidth, codedHeight, hardwareAcceleration, description }) 配置解码参数。description 接收 codec 私有数据（AVC SPS/PPS）。decode(EncodedVideoChunk) 解码单 chunk，flush() 等待完成，output 回调收到 VideoFrame（必须手动 close）。hardwareAcceleration: no-preference（默认）/prefer-hardware（GPU 省电）/prefer-software（兼容性）。VideoDecoder.isConfigSupported() 探测硬件加速实际可用性。Chrome 94+ 完整支持。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行解码演示', { type: 'primary', size: 'sm', onClick: () => this._runVideoDecoderDemo() }), ...codecs.map(([codec, name]) => this._btn(name, { size: 'sm', onClick: () => this._setDecoderCodec(codec) }))),
                h('div', { class: 'flex items-center gap-sm flex-wrap mt-sm' }, h('span', { class: 'fs-sm text-secondary' }, '硬件加速：'), ...hwModes.map((mode) => this._btn(mode, { size: 'sm', onClick: () => this._setHwAccel(mode) }))),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '当前 codec：' + this._decoderCodec + ' / hardwareAcceleration：' + this._hwAccel),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.videoDecoderInfo || '（点击「运行解码演示」探测硬件加速并查看完整代码）')),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['VideoFrame', f.videoFrame]]), h(Tag, { color: 'warning' }, '⚠ 必须 close')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'VideoFrame 是原始视频帧容器，可从 Canvas/ImageBitmap/ArrayBuffer/另一个 VideoFrame 构造。init 指定 format（I420/NV12/I444/RGBA/BGRA 等）、codedWidth/codedHeight、timestamp（微秒，必填）、duration、displayWidth/displayHeight、colorSpace。format 分 YUV 系列（I420 最常用、NV12 硬件友好、I444 高质量）与 RGBA/BGRA 系列（屏幕/Canvas）。allocationSize({ rect, format }) 返回拷贝字节数，copyTo(buf, { rect, format }) 拷贝像素（可自动格式转换）。⚠ VideoFrame 必须手动 close() 释放显存，否则泄漏。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 VideoFrame 演示', { type: 'primary', size: 'sm', onClick: () => this._runVideoFrameDemo() }), ...formats.map(([fmt]) => this._btn(fmt, { size: 'sm', onClick: () => this._setFrameFormat(fmt) }))),
                h('div', { class: 'wc-frame-grid' }, formats.map(([fmt, desc, type]) => h('div', { class: 'wc-frame-cell' }, h('div', { class: 'fmt' }, fmt), h('div', { class: 'desc' }, desc + ' [' + type + ']')))),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.videoFrameInfo || '（点击按钮查看 VideoFrame 完整说明）')),
                h(Alert, {
                    type: 'warning',
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'AudioEncoder 通过 new AudioEncoder({ output, error }) 构造，configure({ codec, sampleRate, numberOfChannels, bitrate }) 配置。codec 支持 mp3/aac(mp4a.40.2)/flac/opus/pcm。encode(AudioData) 编码原始采样。AudioData 构造需指定 format（s16/s32/f32 带 -planar 后缀为 Planar 平面，不带为 Packed 交错）、sampleRate、numberOfFrames、numberOfChannels、timestamp、data。AudioDecoder.decode(EncodedAudioChunk) 解码，output 回调收到 AudioData（allocationSize/copyTo/close）。AudioData 也必须手动 close()。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行音频编解码演示', { type: 'primary', size: 'sm', onClick: () => this._runAudioCodecDemo() }), ...codecs.map(([codec, name]) => this._btn(name, { size: 'sm', onClick: () => this._setAudioCodec(codec) }))),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '当前音频 codec：' + this._audioCodec),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.audioCodecInfo || '（点击按钮查看音频编解码完整说明）')),
                h(Alert, {
                    type: 'info',
    ...
    async _runImageDecoderDemo() {
        ...
        try {
            ...
            this._addLog(supported ? 'info' : 'warn', `ImageDecoder.isTypeSupported('${this._imageType}') → ${supported}`);
        }
        catch (err) {
    ...
    _renderCard7() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'Canvas → VideoEncoder → MP4/WebM 完整管线：相比 canvas.captureStream()+MediaRecorder（仅整体文件），WebCodecs 提供逐帧控制与自定义封装。流程：OffscreenCanvas 绘制 → new VideoFrame(canvas, {timestamp, duration}) → VideoEncoder.encode(frame, {keyFrame}) → output 回调收到 EncodedVideoChunk → mp4box.js 封装 MP4 或 ebml.js 封装 WebM。扩展场景：实时视频处理（Camera → 像素处理 → 编码）、WebRTC RTCEncodedTransform 协同（WebRTC 帧中间处理）、录屏直播（getDisplayMedia → 编码 → WHIP 推流）。latencyMode: realtime 实现低延迟。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行管线演示', { type: 'primary', size: 'sm', onClick: () => this._runPipelineDemo() })),
                h('div', { class: 'wc-pipeline' }, h('span', { class: 'step' }, 'Canvas'), h('span', { class: 'arrow' }, '→'), h('span', { class: 'step' }, 'VideoFrame'), h('span', { class: 'arrow' }, '→'), h('span', { class: 'step active' }, 'VideoEncoder'), h('span', { class: 'arrow' }, '→'), h('span', { class: 'step' }, 'EncodedVideoChunk'), h('span', { class: 'arrow' }, '→'), h('span', { class: 'step' }, 'mp4box.js'), h('span', { class: 'arrow' }, '→'), h('span', { class: 'step' }, 'MP4 文件')),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.pipelineInfo || '（点击「运行管线演示」查看完整 Canvas→MP4 管线代码）')),
                h(Alert, {
                    type: 'info',
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'api-lab-page webcodecs-api-page' }, h('h2', { class: 'section-title' }, 'WebCodecs API 编解码管线 完整实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, '本页演示 W3C WebCodecs API 低层级音视频编解码：VideoEncoder/VideoDecoder（H.264/VP9/AV1）、VideoFrame（I420/NV12/RGBA）、AudioEncoder/AudioDecoder（opus/aac/flac）、ImageDecoder（AVIF/WebP/GIF 动图）、Canvas→MP4/WebM 管线（mp4box.js/RTCEncodedTransform）、陷阱与最佳实践（codec 字符串/硬件加速/close/Worker）。所有特性通过 typeof 能力检测，不可用时仅记日志，绝不抛异常。jsdom 不实现 WebCodecs，真实浏览器（Chrome 94+）可运行实际编解码。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderCard6(), this._renderCard7(), this._renderCard8(), this._renderLogPanel());
    }
}
//# sourceMappingURL=WebCodecsAPIPage.js.map