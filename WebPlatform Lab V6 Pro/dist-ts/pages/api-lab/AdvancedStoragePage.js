...
export class AdvancedStoragePage extends Page {
    ...
    initialState() {
        ...
        return {
            ...
            cookieStoreSupported: (typeof cookieStore !== 'undefined') ||
    ...
    componentDidMount() {
        ...
        if (s.cookieStoreSupported) {
            try {
                const cs = this._cs();
                this._cookieChangeHandler = (e) => {
                    const changed = (e.changed || []).map((c) => c.name);
                    const deleted = (e.deleted || []).map((c) => c.name);
                    this.setState((st) => ({ cookieEvents: st.cookieEvents + 1 }));
                    this._addLog('pull', `CookieChangeEvent：changed=[${changed.join(',') || '空'}]，deleted=[${deleted.join(',') || '空'}]`);
                };
    ...
    async _opfsDemo() {
        if (!this.state.opfsSupported) {
            this._addLog('err', 'OPFS 不可用（navigator.storage.getDirectory 缺失）');
            return;
        }
        try {
            ...
            const content = `OPFS demo @ ${formatTime()}\n` +
            ...
            this._addLog('info', 'OPFS 对用户不可见；Worker 内 createSyncAccessHandle 同步读写性能远超 File System Access API，主线程用 createWritable 异步写入');
        }
        catch (err) {
    ...
    _renderOPFSCard() {
        ...
        return h(Card, {
            ...
            extra: h(Tag, { color: s.opfsSupported ? 'primary' : 'error' }, s.opfsSupported ? '实验性' : '不可用'),
        }, h('p', { class: 'fs-sm text-secondary' }, 'navigator.storage.getDirectory() 返回 OPFS 根目录（对用户不可见、性能高）。' +
            'getFileHandle/createWritable/getFile().text()/removeEntry 全流程；Worker 内可用 createSyncAccessHandle 同步访问。'), h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('运行 OPFS 完整演示', { type: 'primary', size: 'sm', onClick: () => this._opfsDemo(), disabled: !s.opfsSupported }), s.opfsContent
            ? h(Tag, { color: 'success' }, `已读 ${s.opfsContent.length} 字节`)
            : h(Tag, { color: 'default' }, '未读取')), s.opfsContent
            ? h('pre', { class: 'code-block mt-md' }, s.opfsContent)
            : null, h('pre', { class: 'code-block mt-md' }, `const root = await navigator.storage.getDirectory();
...
// Worker 同步：const sh = await fh.createSyncAccessHandle();`));
    ...
    _locksQuery() {
        if (!this.state.locksSupported) {
            this._addLog('err', 'navigator.locks 不可用');
            return;
        }
        navigator.locks.query().then((snapshot) => {
            const held = (snapshot.held || []).map((l) => `${l.name}(${l.mode})`);
            const pending = (snapshot.pending || []).map((l) => `${l.name}(${l.mode})`);
            this.setState({ locksHeld: held.length, locksPending: pending.length });
            this._addLog('pull', `query() → held=[${held.join(',') || '空'}]，` +
    ...
    _locksExclusiveHold() {
        ...
        if (this._locksAbort)
            opts.signal = this._locksAbort.signal;
        navigator.locks.request(LOCK_NAME, opts, async (lock) => {
            if (!lock) {
                this._activeLocks--;
                this._addLog('warn', 'exclusive 锁 callback 收到 null（未获取到，可能被中止）');
                return;
            }
            this._addLog('push', `持有 exclusive 锁 → name=${lock.name}, mode=${lock.mode}，将保持 3 秒`);
    ...
    _locksExclusiveWait() {
        ...
        navigator.locks.request(LOCK_NAME, opts, async (lock) => {
            ...
            this._addLog('pull', `第二个 exclusive 请求终于获取到锁（等待 ${waited}s），name=${lock.name}`);
    ...
    _locksSharedMultiple() {
        ...
        for (let i = 1; i <= 3; i++) {
            const idx = i;
            this._activeLocks++;
            const opts = { mode: 'shared' };
            if (this._locksAbort)
                opts.signal = this._locksAbort.signal;
            navigator.locks.request(sharedName, opts, async (lock) => {
                if (!lock) {
                    this._activeLocks--;
                    return;
                }
                acquired++;
                this._addLog('push', `shared 锁#${idx} 获取成功 → name=${lock.name}, mode=${lock.mode}（同时持有 ${acquired} 个）`);
    ...
    _locksIfAvailable() {
        ...
        this._activeLocks++;
        navigator.locks.request(LOCK_NAME + '-ifavail', { mode: 'exclusive', ifAvailable: true }, async (lock) => {
            if (!lock) {
                this._activeLocks--;
    ...
    _renderLocksCard() {
        ...
        return h(Card, {
            ...
            extra: h(Tag, { color: s.locksSupported ? 'success' : 'error' }, s.locksSupported ? '稳定' : '不可用'),
        }, h('p', { class: 'fs-sm text-secondary' }, 'navigator.locks.request(name, {mode, ifAvailable, steal, signal} as any, cb) 协调同源跨标签页锁。' +
            'exclusive 互斥/shared 可并发；callback Promise settle 后自动释放；locks.query() 返回 {held, pending}。'), h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('持有 exclusive 锁 3s', { type: 'primary', size: 'sm', onClick: () => this._locksExclusiveHold(), disabled: !s.locksSupported }), this._btn('查询 locks.query()', { size: 'sm', onClick: () => this._locksQuery(), disabled: !s.locksSupported }), this._btn('发起 3 个 shared 锁', { size: 'sm', onClick: () => this._locksSharedMultiple(), disabled: !s.locksSupported }), this._btn('ifAvailable 演示', { size: 'sm', onClick: () => this._locksIfAvailable(), disabled: !s.locksSupported }), h(Tag, { color: 'primary' }, `held: ${s.locksHeld}`), h(Tag, { color: 'warning' }, `pending: ${s.locksPending}`)), h('pre', { class: 'code-block mt-md' }, `await navigator.locks.request('my-lock', { mode: 'exclusive' } as any, async (lock) => {
...
// snap.held: [{name, clientId, mode}]；snap.pending: [...]`));
    ...
    async _bucketsDemo() {
        if (!this.state.bucketsSupported) {
            this._addLog('err', 'navigator.storageBuckets 不可用（ES2024/新，仅较新 Chromium）');
            return;
        }
        try {
            // openOrCreate：不存在则创建
            ...
            this._addLog('push', "storageBuckets.openOrCreate('demo-bucket', {quota:10MB, durability:'strict'}) → 已创建/打开");
            // 查询 quota
            ...
            this._addLog('info', '提示：IndexedDB / Cache / Service Worker 可绑定到特定 bucket（通过 bucket 目录隔离）');
        }
        catch (err) {
    ...
    async _estimateDemo() {
        if (!this.state.storageManagerSupported) {
            this._addLog('err', 'navigator.storage.estimate 不可用');
            return;
        }
        try {
            ...
            const details = est.usageDetails
                ? Object.entries(est.usageDetails)
                    .map(([k, v]) => `${k}:${(v / 1048576).toFixed(2)}MB`).join(', ')
                : '(无 usageDetails)';
            this._addLog('pull', `estimate() → quota=${quotaMB}MB, usage=${usageMB}MB, usageDetails={${details}}`);
        }
        catch (err) {
    ...
    async _persistQuery() {
        if (!this.state.storageManagerSupported || typeof navigator.storage.persisted !== 'function') {
            this._addLog('err', 'navigator.storage.persisted 不可用');
            return;
        }
        try {
    ...
    async _persistRequest() {
        if (!this.state.storageManagerSupported || typeof navigator.storage.persist !== 'function') {
            this._addLog('err', 'navigator.storage.persist 不可用');
            return;
        }
        try {
            const ok = await navigator.storage.persist();
            this.setState({ persisted: ok });
            this._addLog(ok ? 'push' : 'warn', `persist() → ${ok ? '请求被批准，已持久化' : '请求被拒绝（需用户授权或站点被频繁使用）'}`);
        }
        catch (err) {
    ...
    _renderBucketsCard() {
        ...
        return h(Card, {
            ...
            extra: h(Tag, { color: s.bucketsSupported ? 'primary' : 'error' }, s.bucketsSupported ? '实验性' : '不可用'),
        }, h('p', { class: 'fs-sm text-secondary' }, 'storageBuckets.openOrCreate({quota, durability, persisted}) 创建独立存储桶；bucket.getQuota/setQuota/delete 管理配额。' +
            'storage.estimate() 返回 {quota, usage, usageDetails}；persist()/persisted() 请求/查询持久化。'), h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('创建/删除 bucket', { type: 'primary', size: 'sm', onClick: () => this._bucketsDemo(), disabled: !s.bucketsSupported }), this._btn('storage.estimate()', { size: 'sm', onClick: () => this._estimateDemo(), disabled: !s.storageManagerSupported }), this._btn('查询 persisted()', { size: 'sm', onClick: () => this._persistQuery(), disabled: !s.storageManagerSupported }), this._btn('请求 persist()', { size: 'sm', onClick: () => this._persistRequest(), disabled: !s.storageManagerSupported })), h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, h(Tag, { color: 'primary' }, `quota: ${quotaMB} MB`), h(Tag, { color: 'success' }, `usage: ${usageMB} MB`), s.persisted !== null
            ? h(Tag, { color: s.persisted ? 'success' : 'warning' }, `persisted: ${s.persisted ? '是' : '否'}`)
            : null), h('pre', { class: 'code-block mt-md' }, `const bucket = await navigator.storageBuckets.openOrCreate('logs', {
...
const ok = await navigator.storage.persist();  // 请求持久化`));
        ...
        return new Promise((resolve, reject) => {
            ...
            req.onupgradeneeded = () => {
                ...
                if (!db.objectStoreNames.contains(IDB_STORE)) {
                    ...
                    this._addLog('push', `onupgradeneeded：createObjectStore('${IDB_STORE}', {keyPath:'id', autoIncrement:true})` +
                        ` + createIndex('byEmail',{unique:true}) + createIndex('byAge')`);
                }
            };
            req.onsuccess = () => resolve(req.result);
            req.onerror = () => reject(req.error);
            req.onblocked = () => this._addLog('warn', 'IDB open 被阻塞（onblocked），可能有旧连接未关闭');
        });
    }
    _idbReq(req) {
        return new Promise((resolve, reject) => {
    ...
    async _idbSetup() {
        if (!this.state.idbSupported) {
            this._addLog('err', 'indexedDB 不可用');
            return;
        }
        try {
            // 关闭旧连接
            ...
            this._addLog('push', `数据库 '${IDB_DB_NAME}' v${IDB_VERSION} 已打开，含仓库 '${IDB_STORE}'`);
    ...
    async _idbCursorAll() {
        ...
        try {
            ...
            this._addLog('pull', `游标遍历完成 → ${result.length} 条：` +
    ...
    async _idbIndexGetByEmail() {
        ...
        try {
            ...
            if (!user) {
                ...
                this._addLog('pull', `index('byEmail').get('bob@demo.dev') → #${user.id} ${user.name}, age=${user.age}`);
            }
        }
        catch (err) {
    ...
    async _idbKeyRangeAge() {
        ...
        try {
            ...
            this._addLog('pull', `index('byAge').getAll(IDBKeyRange.bound(20,30)) → ${result.length} 条：` +
                `[${result.map((u) => `${u.name}(${u.age})`).join(', ')}]`);
        }
        catch (err) {
    ...
    _renderIndexedDBCard() {
        ...
        return h(Card, {
            ...
            extra: h(Tag, { color: s.idbSupported ? 'success' : 'error' }, s.idbSupported ? '稳定' : '不可用'),
        }, h('p', { class: 'fs-sm text-secondary' }, '事务 readonly/readwrite/versionchange；createObjectStore({keyPath, autoIncrement})；createIndex({unique, multiEntry})；' +
            '游标 openCursor/continue（null 即结束）；IDBKeyRange.only/lowerBound/upperBound/bound 范围查询（Promise 包装）。'), h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('初始化数据库 + 插 5 条', { type: 'primary', size: 'sm', onClick: () => this._idbSetup(), disabled: !s.idbSupported }), this._btn('游标遍历全部', { size: 'sm', onClick: () => this._idbCursorAll(), disabled: !s.idbSupported }), this._btn('索引 byEmail 查 Bob', { size: 'sm', onClick: () => this._idbIndexGetByEmail(), disabled: !s.idbSupported }), this._btn('IDBKeyRange age 20-30', { size: 'sm', onClick: () => this._idbKeyRangeAge(), disabled: !s.idbSupported }), h(Tag, { color: 'primary' }, `users: ${s.idbCount}`)), s.idbUsers.length > 0
            ? h('div', { class: 'mt-md fs-sm text-secondary', style: { maxHeight: '120px', overflow: 'auto' } }, ...s.idbUsers.map((u) => h('div', {}, `#${u.id}  ${u.name}  <${u.email}>  age=${u.age}`)))
            : null, h('pre', { class: 'code-block mt-md' }, `const store = db.createObjectStore('users', { keyPath: 'id', autoIncrement: true });
store.createIndex('byEmail', 'email', { unique: true });
...
// 事务事件：tx.oncomplete / onerror / onabort`));
    ...
    async _cacheDemo() {
        if (!this.state.cacheSupported) {
            this._addLog('err', 'caches API 不可用');
            return;
        }
        try {
            ...
            const resp = new Response(JSON.stringify({ hello: 'cache', t: Date.now() }), { headers: { 'Content-Type': 'application/json' } });
            ...
            this.setState({ cacheItems: keys.map((r) => r.url) });
            ...
            this.setState({ cacheItems: keys2.map((r) => r.url) });
    ...
    async _cookieSet() {
        ...
        try {
            ...
            this._addLog('push', `cookieStore.set({name:'${COOKIE_NAME}', value:'${value}', sameSite:'lax', secure:true, expires:+5min})`);
        }
        catch (err) {
    ...
    async _cookieGet() {
        if (!this.state.cookieStoreSupported) {
            this._addLog('err', 'cookieStore 不可用');
            return;
        }
        try {
            ...
            if (!c) {
                ...
                this._addLog('pull', `cookieStore.get → name=${c.name}, value=${c.value}, domain=${c.domain}, path=${c.path}`);
            }
        }
        catch (err) {
    ...
    async _cookieGetAll() {
        ...
        try {
            ...
            this._addLog('pull', `cookieStore.getAll() → ${all.length} 条：[${all.map((c) => c.name).join(', ') || '空'}]`);
        }
        catch (err) {
    ...
    _renderCacheCookieCard() {
        ...
        return h(Card, {
            ...
            extra: h(Tag, { color: bothOk ? 'success' : 'warning' }, bothOk ? '稳定' : '部分可用'),
        }, h('p', { class: 'fs-sm text-secondary' }, 'Cache：caches.open、put/add/addAll、match/matchAll/keys/delete（ignoreSearch/ignoreMethod/ignoreVary）；response.clone()（stream 只能读一次）。' +
            'Cookie Store：set/get/getAll/delete + change 事件（CookieChangeEvent.changed/deleted）。'), h('div', { class: 'mt-md' }, h('div', { class: 'fs-sm fw-medium mb-xs' }, 'Cache API'), h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('put/match/keys/delete', { type: 'primary', size: 'sm', onClick: () => this._cacheDemo(), disabled: !s.cacheSupported }), h(Tag, { color: 'default' }, `cache 条目: ${s.cacheItems.length}`))), h('div', { class: 'mt-md' }, h('div', { class: 'fs-sm fw-medium mb-xs' }, 'Cookie Store API'), h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('set cookie', { type: 'primary', size: 'sm', onClick: () => this._cookieSet(), disabled: !s.cookieStoreSupported }), this._btn('get', { size: 'sm', onClick: () => this._cookieGet(), disabled: !s.cookieStoreSupported }), this._btn('getAll', { size: 'sm', onClick: () => this._cookieGetAll(), disabled: !s.cookieStoreSupported }), this._btn('delete', { size: 'sm', danger: true, onClick: () => this._cookieDelete(), disabled: !s.cookieStoreSupported }), h(Tag, { color: 'primary' }, `change 事件: ${s.cookieEvents}`))), s.cookieList.length > 0
            ? h('div', { class: 'fs-sm text-tertiary mt-md' }, `当前 cookie：${s.cookieList.map((c) => `${c.name}=${c.value}`).join('; ') || '(空)'}`)
            : null, h('pre', { class: 'code-block mt-md' }, `const cache = await caches.open('v1');
...
cookieStore.addEventListener('change', (e: any) => { e.changed; e.deleted; });`));
    ...
    renderPage() {
        return [
            ...
            h('div', { class: 'grid grid-2 gap-md' }, this._renderOPFSCard(), this._renderLocksCard()),
            h('div', { class: 'grid grid-2 gap-md mt-md' }, this._renderBucketsCard(), this._renderIndexedDBCard()),
...
//# sourceMappingURL=AdvancedStoragePage.js.map