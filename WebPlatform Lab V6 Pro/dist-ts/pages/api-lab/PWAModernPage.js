...
export class PWAModernPage extends Page {
    ...
    _onBeforeInstallPrompt = null;
    _onDisplayModeChange = null;
    _onFreeze = null;
    ...
    componentDidMount() {
        ...
        if (c.navigation && window.navigation) {
            ...
            const syncNav = () => {
                ...
                let count = 0;
                try {
                    cur = nav.currentEntry ? nav.currentEntry.url : '';
                }
                catch { /* noop */ }
                try {
                    count = typeof nav.entries === 'function' ? nav.entries().length : 0;
                }
                catch { /* noop */ }
    ...
    componentWillUnmount() {
        ...
        if (this._caps().docPip && window.documentPictureInPicture) {
            if (this._onPipEnter) {
                try {
                    window.documentPictureInPicture.removeEventListener('enter', this._onPipEnter);
                }
                catch { /* noop */ }
            ...
            if (this._onPipLeave) {
                try {
                    window.documentPictureInPicture.removeEventListener('leave', this._onPipLeave);
                }
                catch { /* noop */ }
        ...
        if (this._caps().navigation && window.navigation) {
            ...
            if (this._onNavigate) {
                try {
                    nav.removeEventListener('navigate', this._onNavigate);
                }
                catch { /* noop */ }
            }
            if (this._onNavigateSuccess) {
                try {
                    nav.removeEventListener('navigatesuccess', this._onNavigateSuccess);
                }
                catch { /* noop */ }
            }
            if (this._onNavigateError) {
                try {
                    nav.removeEventListener('navigateerror', this._onNavigateError);
                }
                catch { /* noop */ }
            }
    ...
    _preCode(obj) {
        ...
        return h('pre', { class: 'code-block', style: { maxHeight: '280px', overflow: 'auto' } }, h('code', {}, text));
    }
    ...
    _summarizeLaunchParams(params) {
        ...
        try {
            out.targetURL = params.targetURL || '';
        }
        catch { /* noop */ }
        try {
    ...
    async _badgeSet5() {
        if (!this._caps().badging) {
            this._addLog('err', '当前浏览器不支持 App Badging API');
            return;
        }
        try {
            ...
            this._logApiErr('badge', 'setAppBadge(5)', err);
        }
    }
    async _badgeSetDot() {
        if (!this._caps().badging) {
        ...
        try {
            ...
            this._logApiErr('badge', 'setAppBadge()', err);
        }
    }
    async _badgeClear() {
        if (!this._caps().badging) {
        ...
        try {
            ...
            this._logApiErr('badge', 'clearAppBadge()', err);
        }
    }
    _wcoReadRect() {
        if (!this._caps().wco) {
            this._addLog('err', '当前浏览器不支持 Window Controls Overlay API');
            return;
        }
        try {
            ...
            this._logApiErr('wco', 'getTitlebarAreaRect()', err);
        }
    }
    // ============ Card 3: Document PiP + Protocol Handler ============
    async _docPipOpen() {
        if (!this._caps().docPip) {
            this._addLog('err', '当前浏览器不支持 Document Picture-in-Picture API');
            return;
        }
        try {
            ...
            this._logApiErr('dppip', 'requestWindow', err);
        }
    }
    _docPipClose() {
        if (!this._pipWindow) {
            this._addLog('err', '当前没有打开的 Document PiP 窗口');
            return;
        }
        try {
            this._pipWindow.close();
            ...
            this._logApiErr('dppip', 'close()', err);
        }
    }
    _protoRegister() {
        if (!this._caps().protocolHandler) {
            this._addLog('err', '当前浏览器不支持 Protocol Handler API');
            return;
        }
        try {
            ...
            this._logApiErr('proto', 'registerProtocolHandler', err);
        }
    }
    _protoUnregister() {
        if (!this._caps().protocolHandler) {
        ...
        try {
            ...
            this._logApiErr('proto', 'unregisterProtocolHandler', err);
        }
    }
    _protoRegisterMailto() {
        ...
        try {
            ...
            this._logApiErr('proto', 'registerProtocolHandler(mailto)', err);
        }
    }
    ...
    _navBack() {
        if (!this._caps().navigation) {
            this._addLog('err', '当前浏览器不支持 Navigation API');
            return;
        }
        try {
    ...
    _navForward() {
        ...
        try {
            ...
            this._logApiErr('nav', 'forward()', err);
        }
    }
    _navReload() {
        if (!this._caps().navigation) {
        ...
        try {
            ...
            this._logApiErr('nav', 'reload()', err);
        }
    }
    _navNavigate() {
        if (!this._caps().navigation) {
        ...
        try {
            ...
            this._logApiErr('nav', 'navigate()', err);
        }
    }
    _navListEntries() {
        if (!this._caps().navigation) {
        ...
        try {
            ...
            this._logApiErr('nav', 'entries()', err);
        }
    }
    _lifecycleTabAway() {
    ...
    renderPage() {
        ...
        return [
            ...
            h(Card, {
                ...
                extra: h('span', { class: 'flex items-center gap-xs' }, h(Tag, { color: 'primary' }, 'BeforeInstallPromptEvent'), this._capTag(c.beforeInstallPrompt)),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-tertiary' }, '通过 ', h('code', {}, '<link rel="manifest">'), ' 关联 manifest JSON；字段含 name / short_name / start_url / scope / display (fullscreen | standalone | minimal-ui | browser) / orientation / theme_color / background_color / icons / shortcuts / share_target / file_handlers / protocol_handlers。'), h('p', { class: 'fs-sm text-tertiary' }, 'BeforeInstallPromptEvent：监听 ', h('code', {}, 'window.onbeforeinstallprompt'), ' 缓存事件，', h('code', {}, 'event.prompt()'), ' 触发安装提示，', h('code', {}, 'event.userChoice'), ' 是 Promise<{outcome, platform}>；', h('code', {}, 'appinstalled'), ' 事件在安装完成后触发；可用 ', h('code', {}, 'matchMedia(\'(display-mode: standalone)\')'), ' 或 iOS ', h('code', {}, 'navigator.standalone'), ' 检测运行模式。'), h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._statusTag('已捕获 beforeinstallprompt', s.deferredPromptAvailable), this._statusTag('isStandalone', s.isStandalone), this._statusTag('appInstalled', s.appInstalled), h(Tag, { color: 'primary' }, `display-mode:${s.displayMode}`), s.installOutcome && h(Tag, { color: s.installOutcome === 'accepted' ? 'success' : 'warning' }, `outcome:${s.installOutcome}`)), h('div', { class: 'flex flex-wrap gap-sm' }, this._btn('安装应用 (prompt)', { type: 'primary', size: 'sm', onClick: () => this._triggerInstall(), disabled: !c.beforeInstallPrompt || !s.deferredPromptAvailable }), this._btn('检测 display-mode', { size: 'sm', onClick: () => this._checkDisplayMode() }), this._btn('查找 <link rel="manifest">', { size: 'sm', onClick: () => this._checkManifestLink() })), h('div', { class: 'fs-sm text-secondary mt-xs' }, '完整 manifest 示例（含 share_target / file_handlers / protocol_handlers）：'), this._preCode(MANIFEST_EXAMPLE), h('p', { class: 'fs-xs text-tertiary' }, '注：beforeinstallprompt 可能仅在页面加载后由浏览器择机触发；本页已在 componentDidMount 中尽早注册监听，事件触发后会自动启用"安装应用"按钮。'))), 
            // ============ Card 2: App Badging + Window Controls Overlay ============
            h(Card, {
                title: '2. App Badging 与 Window Controls Overlay',
                extra: h('span', { class: 'flex items-center gap-xs' }, h(Tag, { color: 'primary' }, 'setAppBadge'), this._capTag(c.badging), h(Tag, { color: 'primary' }, 'windowControlsOverlay'), this._capTag(c.wco)),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-tertiary' }, 'App Badging API（被动 API，无事件）：', h('code', {}, 'navigator.setAppBadge(count)'), ' 设置数字 badge；', h('code', {}, 'navigator.setAppBadge()'), ' 设置无数字 badge（仅一个点）；', h('code', {}, 'navigator.clearAppBadge()'), ' 清除。需要已安装 PWA。'), h('p', { class: 'fs-sm text-tertiary' }, 'Window Controls Overlay：', h('code', {}, 'navigator.windowControlsOverlay.visible'), ' 反映系统控件是否占用标题栏；', h('code', {}, 'getTitlebarAreaRect()'), ' 返回 {x,y,width,height,top,left,right,bottom}；监听 ', h('code', {}, 'geometrychange'), '；CSS 用 ', h('code', {}, 'env(titlebar-area-x/y/width/height)'), '，需在 manifest ', h('code', {}, 'display_override'), ' 中声明 ', h('code', {}, '\'window-controls-overlay\''), '。'), h('div', { class: 'flex flex-wrap gap-sm' }, this._btn('设置 Badge (5)', { type: 'primary', size: 'sm', onClick: () => this._badgeSet5(), disabled: !c.badging }), this._btn('设置无数字 Badge', { size: 'sm', onClick: () => this._badgeSetDot(), disabled: !c.badging }), this._btn('清除 Badge', { size: 'sm', danger: true, onClick: () => this._badgeClear(), disabled: !c.badging }), this._btn('读取 titlebar rect', { size: 'sm', onClick: () => this._wcoReadRect(), disabled: !c.wco })), h('div', { class: 'log-panel', style: { height: 'auto', padding: '8px' } }, this._kvRow('windowControlsOverlay.visible', s.wcoVisible), this._kvRow('titlebar.x', s.titlebarRect ? s.titlebarRect.x : 'N/A'), this._kvRow('titlebar.y', s.titlebarRect ? s.titlebarRect.y : 'N/A'), this._kvRow('titlebar.width', s.titlebarRect ? s.titlebarRect.width : 'N/A'), this._kvRow('titlebar.height', s.titlebarRect ? s.titlebarRect.height : 'N/A')), (!c.badging || !c.wco) && h(Alert, {
                type: 'warning',
                message: '部分 API 不支持',
            ...
            h(Card, {
                ...
                extra: h('span', { class: 'flex items-center gap-xs' }, h(Tag, { color: 'primary' }, 'documentPictureInPicture'), this._capTag(c.docPip), h(Tag, { color: 'primary' }, 'registerProtocolHandler'), this._capTag(c.protocolHandler)),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-tertiary' }, 'Document Picture-in-Picture：', h('code', {}, 'window.documentPictureInPicture.requestWindow({width,height})'), ' 返回独立 Window 对象，可向其 ', h('code', {}, 'document.body'), ' appendChild 任意 DOM；', h('code', {}, 'documentPictureInPicture.window'), ' 是当前 PiP 窗口；监听 ', h('code', {}, 'enter'), ' / ', h('code', {}, 'leave'), ' 事件。与视频 PiP 不同，可承载任意 HTML。'), h('p', { class: 'fs-sm text-tertiary' }, 'Protocol Handler：', h('code', {}, 'navigator.registerProtocolHandler(scheme, url, title)'), '；自定义 scheme 需 ', h('code', {}, 'web+'), ' 前缀（如 ', h('code', {}, 'web+study'), '），也可注册 ', h('code', {}, 'mailto'), ' 让 PWA 出现在邮件应用列表；url 必须同源且含 ', h('code', {}, '%s'), ' 占位符；', h('code', {}, 'unregisterProtocolHandler(scheme, url)'), ' 注销。'), h('div', { class: 'flex flex-wrap gap-sm' }, this._btn('打开 Document PiP', { type: 'primary', size: 'sm', onClick: () => this._docPipOpen(), disabled: !c.docPip || s.docPipActive }), this._btn('关闭 PiP', { size: 'sm', danger: true, onClick: () => this._docPipClose(), disabled: !s.docPipActive }), this._btn('注册 web+study', { type: 'primary', size: 'sm', onClick: () => this._protoRegister(), disabled: !c.protocolHandler }), this._btn('注销 web+study', { size: 'sm', onClick: () => this._protoUnregister(), disabled: !c.protocolHandler }), this._btn('注册 mailto', { size: 'sm', onClick: () => this._protoRegisterMailto(), disabled: !c.protocolHandler })), h('div', { class: 'flex items-center gap-sm' }, h('span', { class: 'fs-sm text-secondary' }, 'Document PiP 状态：'), h(Tag, { color: s.docPipActive ? 'success' : 'default' }, s.docPipActive ? '已打开' : '未打开')), 
            // 会被克隆到 PiP 窗口的源 DOM
            h('div', { class: 'js-pip-source', style: { padding: '12px', borderRadius: '8px', background: 'linear-gradient(135deg,#1677ff,#722ed1)', color: '#fff', textAlign: 'center' } }, h('div', { style: { fontSize: '18px', fontWeight: 600 } }, '画中画内容'), h('div', { style: { fontSize: '12px', opacity: 0.85, marginTop: '6px' } }, '此 div 会被克隆 appendChild 到 PiP 窗口的 document.body')), (!c.docPip || !c.protocolHandler) && h(Alert, {
                type: 'warning',
                message: '部分 API 不支持',
            ...
            h(Card, {
                ...
                extra: h('span', { class: 'flex items-center gap-xs' }, h(Tag, { color: 'primary' }, 'launchQueue'), this._capTag(c.launchQueue)),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-tertiary' }, 'Launch Handler API：', h('code', {}, 'window.launchQueue.setConsumer((params) => …)'), ' 注册启动消费者；', h('code', {}, 'LaunchParams'), ' 含 ', h('code', {}, 'targetURL'), ' 与 ', h('code', {}, 'files'), '（FileSystemFileHandle 数组）；替代方案 ', h('code', {}, 'navigation.addEventListener(\'launch\')'), '。'), h('p', { class: 'fs-sm text-tertiary' }, 'File Handling API：在 manifest ', h('code', {}, 'file_handlers'), ' 数组中声明 ', h('code', {}, '{ action, accept: { mime: [extensions] } }'), '；安装 PWA 后，系统会将匹配文件以 LaunchParams.files 传入 setConsumer 回调，通过 ', h('code', {}, 'files[i].getFile()'), ' 读取内容。'), h('div', { class: 'flex flex-wrap gap-sm' }, this._btn('能力检测', { size: 'sm', onClick: () => this._launchCheckCap() }), this._btn('读取 LaunchParams 快照', { type: 'primary', size: 'sm', onClick: () => this._launchReadParams(), disabled: !c.launchQueue })), s.launchParams && h('div', { class: 'log-panel', style: { height: 'auto', padding: '8px' } }, this._kvRow('targetURL', s.launchParams.targetURL || '(空)'), this._kvRow('files.length', s.launchParams.filesCount), ...(s.launchParams.files || []).map((f, i) => this._kvRow(`file[${i}].name`, `${f.name} (${f.kind}/${f.type})`))), h('div', { class: 'fs-sm text-secondary mt-xs' }, 'manifest file_handlers 示例：'), this._preCode(FILE_HANDLERS_EXAMPLE), h(Alert, {
                type: 'info',
            ...
            h(Card, {
                ...
                extra: h('span', { class: 'flex items-center gap-xs' }, h(Tag, { color: 'primary' }, 'navigation'), this._capTag(c.navigation)),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-tertiary' }, 'Navigation API：', h('code', {}, '(navigation as any).entries()'), ' 返回历史记录列表，', h('code', {}, 'navigation.currentEntry'), ' 是当前条目（含 url/key/id/getState()）；监听 ', h('code', {}, 'navigate'), ' 事件并调用 ', h('code', {}, 'e.intercept({ handler })'), ' 拦截；', h('code', {}, 'navigation.navigate(url)'), ' / ', h('code', {}, 'back()'), ' / ', h('code', {}, 'forward()'), ' / ', h('code', {}, 'reload()'), ' 编程导航；另有 ', h('code', {}, 'navigatesuccess'), ' / ', h('code', {}, 'navigateerror'), ' 事件。'), h('p', { class: 'fs-sm text-tertiary' }, 'Page Lifecycle：状态机 active → passive → hidden → frozen → terminated；', h('code', {}, 'document.visibilityState'), ' (visible | hidden) + ', h('code', {}, 'visibilitychange'), ' 事件；', h('code', {}, 'freeze'), ' / ', h('code', {}, 'resume'), ' 事件对应后台冻结/恢复；', h('code', {}, 'pagehide'), ' / ', h('code', {}, 'pageshow'), ' 的 ', h('code', {}, 'persisted'), ' 反映 BFCache 命中。'), h('div', { class: 'flex flex-wrap gap-sm' }, this._btn('navigation.back()', { size: 'sm', onClick: () => this._navBack(), disabled: !c.navigation }), this._btn('navigation.forward()', { size: 'sm', onClick: () => this._navForward(), disabled: !c.navigation }), this._btn('navigation.reload()', { size: 'sm', onClick: () => this._navReload(), disabled: !c.navigation }), this._btn('navigation.navigate(#hash)', { type: 'primary', size: 'sm', onClick: () => this._navNavigate(), disabled: !c.navigation }), this._btn('列出 entries()', { size: 'sm', onClick: () => this._navListEntries(), disabled: !c.navigation }), this._btn('提示：切换标签页测试 lifecycle', { size: 'sm', onClick: () => this._lifecycleTabAway() })), h('div', { class: 'log-panel', style: { height: 'auto', padding: '8px' } }, this._kvRow('visibilityState', s.visibilityState), this._kvRow('(navigation as any).entries().length', s.navEntryCount), this._kvRow('navigation.currentEntry.url', s.navCurrentURL || '(N/A)'), this._kvRow('matchMedia display-mode', `${s.displayMode} (standalone=${s.isStandalone})`)), !c.navigation && h(Alert, {
                type: 'warning',
                message: 'Navigation API 不可用',
                description: '当前浏览器不支持 Navigation API（仅 Chromium 系较新版本支持）。Page Lifecycle 事件仍可监听。',
            })))),
            // —— 事件日志 ——
            h(Card, { title: '事件日志', extra: h('span', { class: 'fs-sm text-tertiary' }, '实时') }, h('div', { class: 'log-panel' }, s.logs.length === 0
...
//# sourceMappingURL=PWAModernPage.js.map