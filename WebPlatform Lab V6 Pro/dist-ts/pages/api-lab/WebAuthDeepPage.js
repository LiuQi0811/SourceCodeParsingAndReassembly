...
export class WebAuthDeepPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!hasCredStore)
            this._addLog('warn', 'navigator.credentials.create 不可用（需安全上下文 HTTPS/localhost）');
        if (!hasUVPA)
            this._addLog('warn', 'isUserVerifyingPlatformAuthenticatorAvailable 不可用');
        if (!hasParseCreation)
            this._addLog('warn', 'parseCreationOptionsFromJSON 不可用（ES2024 新方法）');
        const hasDigitalCred = (() => { try {
        ...
        if (!hasDigitalCred)
            this._addLog('warn', 'DigitalCredential 不可用（W3C WebAuth 2024-2025，Chrome behind flag，jsdom 无）');
    }
    ...
    async _detectPlatformAuthenticator() {
        ...
        if (!caps.uvpa) {
            ...
            this.setState({
                platformAuthInfo: 'PublicKeyCredential.isUserVerifyingPlatformAuthenticatorAvailable 不可用\n' +
        ...
        try {
            ...
            this.setState({
                platformAuthInfo: 'PublicKeyCredential.isUserVerifyingPlatformAuthenticatorAvailable() → Promise<boolean>\n' +
                    `返回值：${available}\n` +
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.pkc ? 'success' : 'error' }, caps.pkc ? 'PublicKeyCredential ✓' : '不可用'), h(Tag, { color: 'primary' }, 'isUVPA / COSE')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'PublicKeyCredential.isUserVerifyingPlatformAuthenticatorAvailable() → Promise<boolean> 检测平台认证器（TouchID/Windows Hello），不触发弹窗；isExternalSignalCTAPBridgingAvailable() 检测 CTAP 桥接（较新）。所有 API 前做 typeof 检测，jsdom/Node 中 PublicKeyCredential 通常 undefined，需真实浏览器 + HTTPS。pubKeyCredParams.alg 用 COSE 注册表编号标识公钥算法（ES256=-7、RS256=-257、EdDSA=-8）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('同步检测', { type: 'primary', size: 'sm', onClick: () => this._detectSupport() }), this._btn('检测平台认证器', { type: 'primary', size: 'sm', disabled: !caps.uvpa, onClick: () => this._detectPlatformAuthenticator() }), this._btn('COSE 算法表', { size: 'sm', onClick: () => this._listCoseAlgs() })),
                h('div', { class: 'fs-sm text-secondary' }, '能力检测结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '220px', overflow: 'auto' } }, h('code', {}, s.supportInfo || '（点击「同步检测」或「COSE 算法表」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '平台认证器检测结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '180px', overflow: 'auto' } }, h('code', {}, s.platformAuthInfo || '（点击「检测平台认证器」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '140px', overflow: 'auto' } }, h('code', {}, `// 检测平台认证器（不触发弹窗，可安全调用）
...
pubKeyCredParams: [
  ...
  { type: 'public-key', alg: -257 },  // RS256（兼容）
    ...
    _demoCreateFlow() {
        ...
        const mockClientData = new Uint8Array(Array.from('{"type":"webauthn.create","challenge":"AQIDBAU","origin":"https://localhost"}')
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.credCreate ? 'success' : 'error' }, caps.credCreate ? 'create ✓' : '不可用'), h(Tag, { color: 'primary' }, 'credentials.create')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'navigator.credentials.create({ publicKey: creationOptions }) → Promise<PublicKeyCredential> 触发注册：认证器生成密钥对，私钥留存认证器，公钥经 attestationObject 返回。返回的 AuthenticatorAttestationResponse 含 clientDataJSON（客户端上下文）、attestationObject（CBOR 编码的签名数据）；getAuthenticatorData()/getPublicKey()/getPublicKeyAlgorithm()/getTransports() 为较新的便捷访问器。本页不真正调用 create（会弹认证器），仅构造 options 并用 mock 返回值演示解析。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('构造 CreationOptions', { type: 'primary', size: 'sm', onClick: () => this._buildCreationOptions() }), this._btn('演示 create 解析', { type: 'primary', size: 'sm', onClick: () => this._demoCreateFlow() })),
                h('div', { class: 'fs-sm text-secondary' }, 'PublicKeyCredentialCreationOptions：'),
                h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, s.creationOptionsInfo || '（点击「构造 CreationOptions」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, 'create 返回值解析（mock）：'),
                h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, s.attestationResult || '（点击「演示 create 解析」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '180px', overflow: 'auto' } }, h('code', {}, `const creationOptions = {
...
// cred.response.attestationObject / clientDataJSON / getPublicKey() … 发服务器存公钥`)),
    ...
    _demoGetFlow() {
        ...
        const mockRawId = new Uint8Array([0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff]);
        const mockClientData = new Uint8Array(Array.from('{"type":"webauthn.get","challenge":"AQIDBAU","origin":"https://localhost"}')
            .map((c) => c.charCodeAt(0)));
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.credGet ? 'success' : 'error' }, caps.credGet ? 'get ✓' : '不可用'), h(Tag, { color: 'primary' }, 'credentials.get')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'navigator.credentials.get({ publicKey: requestOptions }) → Promise<PublicKeyCredential> 触发认证：认证器用留存的私钥对挑战签名，返回 AuthenticatorAssertionResponse。response 含 clientDataJSON、authenticatorData（认证器数据）、signature（签名）、userHandle（用户 ID，可发现凭据时返回；存在表示 UV 成功）。服务器用注册时的公钥验签。本页不真正调用 get，仅构造 options 并用 mock 返回值演示解析。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('构造 RequestOptions', { type: 'primary', size: 'sm', onClick: () => this._buildRequestOptions() }), this._btn('演示 get 解析', { type: 'primary', size: 'sm', onClick: () => this._demoGetFlow() })),
                h('div', { class: 'fs-sm text-secondary' }, 'PublicKeyCredentialRequestOptions：'),
                h('pre', { class: 'code-block', style: { maxHeight: '240px', overflow: 'auto' } }, h('code', {}, s.requestOptionsInfo || '（点击「构造 RequestOptions」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, 'get 返回值解析（mock）：'),
                h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, s.assertionResult || '（点击「演示 get 解析」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '160px', overflow: 'auto' } }, h('code', {}, `const requestOptions = {
  challenge: new Uint8Array([/* 16 字节随机 */]),
  rpId: 'example.com',
  userVerification: 'preferred',
...
assertion 中 response.userHandle 存在也表示 UV 成功（可发现凭据）。`;
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'primary' }, 'platform / cross-platform'), h(Tag, { color: 'primary' }, 'UV 三模式')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'authenticatorSelection.authenticatorAttachment 控制 platform（TouchID/Windows Hello，内置不可移除）或 cross-platform（USB 安全密钥/手机，漫游可拔插）。transports（getTransports() 返回 usb/nfc/ble/internal/hybrid/smart-card）描述认证器传输方式，存库供后续 allowCredentials.transports 用。userVerification 三种模式：required（必须生物识别/PIN）、preferred（优先验证，降级为 presence）、discouraged（仅触控）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('对比说明', { type: 'primary', size: 'sm', onClick: () => this._explainAttachments() })),
                h('div', { class: 'fs-sm text-secondary' }, 'Attachment / transports / UV 说明：'),
                h('pre', { class: 'code-block', style: { maxHeight: '320px', overflow: 'auto' } }, h('code', {}, s.attachmentInfo || '（点击「对比说明」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '160px', overflow: 'auto' } }, h('code', {}, `// 注册时筛选认证器类型
authenticatorSelection: {
  ...
  认证时必须 allowCredentials: [{ type, id, transports }]，否则认证器不知用哪个凭据。`;
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'primary' }, 'residentKey 三模式'), h(Tag, { color: 'primary' }, '无密码登录')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '可发现凭据（Discoverable Credential，又称 Resident Key）存储在认证器内并绑定用户身份。residentKey 三种值：required（必须创建，认证时可省略 allowCredentials，认证器自行匹配用户）、preferred（优先创建，不支持则退化）、discouraged（不创建，认证时必须提供 allowCredentials）。requireResidentKey 为旧字段，true 等价于 residentKey: "required"。可发现凭据是无密码登录的基础。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('解释可发现凭据', { type: 'primary', size: 'sm', onClick: () => this._explainResidentKey() })),
                h('div', { class: 'fs-sm text-secondary' }, 'Resident Key 说明：'),
                h('pre', { class: 'code-block', style: { maxHeight: '320px', overflow: 'auto' } }, h('code', {}, s.residentKeyInfo || '（点击「解释可发现凭据」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '160px', overflow: 'auto' } }, h('code', {}, `// 无密码登录：注册时强制可发现凭据
authenticatorSelection: {
  ...
  // { prf: { results: { first: ArrayBuffer(32) } }, largeBlob: { blob: ArrayBuffer } }`;
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.parseCreation ? 'success' : 'warning' }, caps.parseCreation ? 'parseFromJSON ✓' : 'parseFromJSON ✗'), h(Tag, { color: 'primary' }, 'credProps / largeBlob / prf')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'ES2024 新增 PublicKeyCredential.parseCreationOptionsFromJSON()/parseRequestOptionsFromJSON() 从纯 JSON 构造 options（自动 Base64URL→Uint8Array），credential.toJSON() 反向序列化，省去手写二进制转码。Extensions（扩展）在 options.extensions 指定，结果在 getClientExtensionResults() 返回：credProps（查询 resident key）、largeBlob（约 1KB 状态存储）、prf（伪随机函数密钥派生）、payment（SPC 支付确认）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('演示 JSON 序列化', { type: 'primary', size: 'sm', onClick: () => this._demoParseFromJSON() }), this._btn('演示 Extensions', { type: 'primary', size: 'sm', onClick: () => this._demoExtensions() })),
                h('div', { class: 'fs-sm text-secondary' }, 'JSON 序列化结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '300px', overflow: 'auto' } }, h('code', {}, s.jsonInfo || '（点击「演示 JSON 序列化」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, 'Extensions 说明：'),
                h('pre', { class: 'code-block', style: { maxHeight: '300px', overflow: 'auto' } }, h('code', {}, s.extensionsInfo || '（点击「演示 Extensions」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '140px', overflow: 'auto' } }, h('code', {}, `// ES2024：从 JSON 构造 options（自动解码 Base64URL）
...
// credential.toJSON() 反向：ArrayBuffer → Base64URL，可直接 JSON.stringify 传输`)),
    ...
    _renderCard9() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.digitalCredential ? 'success' : 'error' }, caps.digitalCredential ? 'DigitalCredential ✓' : 'DigitalCredential ✗'), h(Tag, { color: 'primary' }, 'W3C WebAuth 2024-2025')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'Digital Credentials API（W3C WebAuth WG 2024-2025）通过 navigator.credentials.get({ digital: {...} }) 请求政府签发数字凭证（驾照、护照），用于年龄/身份验证。返回 DigitalCredential 实例（protocol/data）。与 WebAuthn 互补但独立：WebAuthn 用于认证登录，Digital Credentials 传递身份信息。两者都走 navigator.credentials.get 但选项不同。Chrome behind flag，Firefox/Safari 未实现。降级：上传证件照片或第三方 IDV 服务。jsdom 无，演示仅展示文档。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('展示 Digital Credentials 文档与代码', { type: 'primary', size: 'sm', onClick: () => this._runDigitalCredentialsDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, 'Digital Credentials 文档与示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '520px', overflow: 'auto' } }, h('code', {}, s.digitalCredentialsInfo || '（点击按钮查看 Digital Credentials API 完整文档与数字身份验证示例）')),
                h(Alert, {
                    type: 'info',
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'page api-lab-page' }, h('h2', { class: 'section-title' }, 'Web Authentication（WebAuthn）深入实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, 'WebAuthn 基于公钥密码学，用认证器（TouchID/Windows Hello/USB 安全密钥）替代密码实现抗钓鱼强认证。本页演示注册（attestation）、认证（assertion）、Attachment/transports/UV、可发现凭据与 ES2024 JSON 序列化。测试环境仅展示用法与 mock 解析。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderCard6(), this._renderCard9(), this._renderLogPanel());
    }
}
//# sourceMappingURL=WebAuthDeepPage.js.map