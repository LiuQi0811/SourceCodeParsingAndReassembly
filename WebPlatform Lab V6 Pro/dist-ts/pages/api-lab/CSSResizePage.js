...
export class CSSResizePage extends Page {
    _inited = false;
    _dynamicStyles = [];
    _resizeObservers = undefined;
    _resizeMode = '';
    _overflowMode = '';
    _logicalMode = '';
    _writingMode = '';
    _resizerHidden = false;
    _resizerCustom = false;
    // —— 初始 state ——
    initialState() {
        return {
            logs: [],
    ...
    componentDidMount() {
        ...
        if (!f.resizeBlock)
            this._addLog('warn', 'resize: block 不可用或 jsdom 未识别（CSS UI L4 逻辑值，Chrome 111+ / Firefox 119+ / Safari 16.4+ 支持）');
        if (!f.resizeInline)
            this._addLog('warn', 'resize: inline 不可用或 jsdom 未识别（CSS UI L4 逻辑值，Chrome 111+ / Firefox 119+ / Safari 16.4+ 支持）');
        if (!f.resizeObserver)
            this._addLog('warn', 'ResizeObserver 不可用（jsdom 通常不实现，现代浏览器全支持：Chrome 64+ / Firefox 69+ / Safari 13.1+）');
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['resize:both', f.resizeBoth], ['resize:block', f.resizeBlock], ['resize:inline', f.resizeInline]]), h(Tag, { color: 'primary' }, 'CSS UI L4')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'resize 属性定义于 CSS UI Module Level 4，允许用户通过鼠标拖拽右下角手柄调整元素尺寸。规范要求 overflow 不为 visible 才能生效（必备组合：resize: both; overflow: auto;）。resize: both/horizontal/vertical 全部现代浏览器支持；resize: block/inline 为 CSS UI L4 逻辑值，Chrome 111+ 支持。textarea 元素 UA 默认设置 resize: both，需显式 resize: none 关闭。::-webkit-resizer 伪元素仅 Webkit/Blink 支持可自定义手柄。ResizeObserver API 可监听拖拽尺寸变化。'),
                ...
                h('div', { class: 'rs-box' }, '拖拽右下角手柄调整尺寸\n（jsdom 不渲染手柄，但 DOM 与样式正确）\n当前 resize: ' + this._resizeMode),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.overviewInfo || '（点击「读取概述信息」查看 resize 属性完整说明）')),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'resize 取值：none（默认，不可拖拽）/ both（横纵双向）/ horizontal（仅横向）/ vertical（仅纵向）/ block（逻辑块方向）/ inline（逻辑行方向）。物理值 horizontal/vertical 始终相对物理屏幕方向；逻辑值 block/inline 相对书写方向（writing-mode）：horizontal-tb 时 block=vertical、inline=horizontal；vertical-rl/lr 时翻转。block/inline 为 CSS UI L4 逻辑值，Chrome 111+ / Firefox 119+ / Safari 16.4+ 支持。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('none', { size: 'sm', onClick: () => this._setResizeMode('none') }), this._btn('both', { type: 'primary', size: 'sm', disabled: !f.resizeBoth, onClick: () => this._setResizeMode('both') }), this._btn('horizontal', { size: 'sm', disabled: !f.resizeHorizontal, onClick: () => this._setResizeMode('horizontal') }), this._btn('vertical', { size: 'sm', disabled: !f.resizeVertical, onClick: () => this._setResizeMode('vertical') }), this._btn('block', { size: 'sm', disabled: !f.resizeBlock, onClick: () => this._setResizeMode('block') }), this._btn('inline', { size: 'sm', disabled: !f.resizeInline, onClick: () => this._setResizeMode('inline') })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '可拖拽元素（当前 resize: ' + this._resizeMode + '）：'),
                h('div', { class: 'rs-box' }, '切换上方按钮观察不同 resize 取值\n当前 resize: ' + this._resizeMode + '\n（jsdom 不渲染手柄，DOM 与样式正确）'),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.valuesInfo || '（点击按钮切换 resize 取值并查看完整说明）')),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['resize:both', f.resizeBoth]]), h(Tag, { color: 'warning' }, 'overflow 非 visible')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'resize 必须配合 overflow 非 visible 值才能生效：overflow: visible 时手柄不显示、resize 无效；overflow: hidden/auto/scroll/clip 均可触发 resize。最常用组合 resize: both; overflow: auto;（内容超出自动滚动）。overflow: hidden 仍可 resize（创建 BFC + 隐藏滚动容器，手柄附着其上）。分轴 overflow-x/overflow-y 任一非 visible 即可触发对应方向 resize。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('visible', { size: 'sm', danger: true, onClick: () => this._setOverflowMode('visible') }), this._btn('hidden', { size: 'sm', onClick: () => this._setOverflowMode('hidden') }), this._btn('auto', { type: 'primary', size: 'sm', onClick: () => this._setOverflowMode('auto') }), this._btn('scroll', { size: 'sm', onClick: () => this._setOverflowMode('scroll') }), this._btn('clip', { size: 'sm', onClick: () => this._setOverflowMode('clip') })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '可拖拽元素（当前 overflow: ' + this._overflowMode + '）：'),
                h('div', { class: 'rs-overflow' }, '切换上方按钮观察不同 overflow 取值\n当前 overflow: ' + this._overflowMode + '\nresize: both（visible 时手柄不显示）\n\n这段内容较长用于触发滚动条测试：CSS Resize 与 overflow 必须协同工作，overflow: visible 时 resize 无效，overflow: hidden 仍可 resize。'),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.overflowInfo || '（点击按钮切换 overflow 取值并查看协同说明）')),
                h(Alert, {
                    type: 'warning',
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['resize:both', f.resizeBoth]]), h(Tag, { color: 'success' }, '✓ 防拖出布局')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'resize 拖拽改变元素 width/height（inline style），但 min-width/max-width/min-height/max-height 仍生效，拖拽不会突破边界。约束规则：min-width > max-width > width（优先级），拖拽超出范围自动钳制到边界值。最佳实践：始终设置 min-width/min-height（防拖到 0 不可见）+ max-width/max-height（防拖出父容器破坏布局）；flex/grid 子元素需配 flex-shrink: 0；min/max 用相对单位响应式适配。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 min/max 约束演示', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runConstraintsDemo() })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '可拖拽元素（拖拽范围 100-500px × 80-400px）：'),
                h('div', { class: 'rs-constrained' }, '拖拽右下角手柄\nmin-width: 100px / max-width: 500px\nmin-height: 80px / max-height: 400px\n（拖拽不会突破 min/max 边界）'),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.constraintsInfo || '（点击按钮查看 min/max 约束完整说明）')),
                h(Alert, {
                    type: 'info',
    ...
    _setLogicalMode(mode) {
        this._logicalMode = mode;
        this._injectStyle('css-resize-logical-dynamic', `.rs-logical { resize: ${mode}; writing-mode: ${this._writingMode}; }`);
    ...
    _setWritingMode(mode) {
        ...
        this._injectStyle('css-resize-wm-dynamic', `.rs-logical { resize: ${this._logicalMode}; writing-mode: ${mode}; }`);
        this.setState({ logicalInfo: this._readLogicalInfo() });
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['resize:block', f.resizeBlock], ['resize:inline', f.resizeInline]]), h(Tag, { color: 'primary' }, 'CSS UI L4 逻辑值')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'resize: block/inline 是 CSS UI L4 逻辑值，随 writing-mode 自适应翻转。默认 horizontal-tb 时 block=vertical、inline=horizontal；vertical-rl/lr 时翻转（block=horizontal、inline=vertical）。逻辑值让竖排古中文/日文布局的拖拽行为与书写方向一致，更自然。浏览器支持：Chrome 111+ / Edge 111+ / Firefox 119+ / Safari 16.4+，老浏览器回退为 none。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('resize: block', { type: 'primary', size: 'sm', disabled: !f.resizeBlock, onClick: () => this._setLogicalMode('block') }), this._btn('resize: inline', { size: 'sm', disabled: !f.resizeInline, onClick: () => this._setLogicalMode('inline') }), this._btn('horizontal-tb', { size: 'sm', onClick: () => this._setWritingMode('horizontal-tb') }), this._btn('vertical-rl', { size: 'sm', onClick: () => this._setWritingMode('vertical-rl') })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '当前 resize: ' + this._logicalMode + ' / writing-mode: ' + this._writingMode + '：'),
                h('div', { class: 'rs-logical' }, '切换 resize 逻辑值与 writing-mode\n当前 resize: ' + this._logicalMode + '\nwriting-mode: ' + this._writingMode + '\n（竖排时逻辑值翻转，物理值不变）'),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.logicalInfo || '（点击按钮切换 resize 逻辑值 / writing-mode 查看说明）')),
                h(Alert, {
                    type: 'warning',
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['::-webkit-resizer', f.webkitResizer], ['resize:both', f.resizeBoth]]), h(Tag, { color: 'warning' }, 'Webkit 私有')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '::-webkit-resizer 是 Webkit/Blink 私有伪元素（非标准），用于样式化滚动容器右下角的 resize 手柄。仅 Chrome/Safari/Edge 支持，Firefox 不支持。可隐藏手柄（display: none，但 resize 仍可用）或自定义手柄样式（background/background-color 等）。必须配合 resize 非 none + overflow 非 visible 才生效。跨浏览器兼容需 JS 检测或用 JS 拖拽库。长期方案应使用 CSS UI L4 标准属性（仍在草案）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn(`隐藏手柄（${this._resizerHidden ? '已隐藏' : '显示中'}）`, { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._toggleResizerHidden() }), this._btn(`自定义手柄（${this._resizerCustom ? '已开启' : '默认'}）`, { size: 'sm', disabled: !f.css, onClick: () => this._toggleResizerCustom() })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '可拖拽元素（仅 Webkit/Blink 浏览器可见手柄变化）：'),
                h('div', { class: 'rs-resizer' }, '拖拽右下角手柄\n::-webkit-resizer 当前：\n  hidden = ' + (this._resizerHidden ? '是' : '否') + '\n  custom = ' + (this._resizerCustom ? '是' : '否') + '\n（Firefox 不支持该伪元素）'),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.resizerInfo || '（点击按钮切换 ::-webkit-resizer 样式查看说明）')),
                h(Alert, {
                    type: 'warning',
    ...
    _renderCard7() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['resize:both', f.resizeBoth], ['ResizeObserver', f.resizeObserver]]), h(Tag, { color: 'primary' }, '5 大场景')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '五大实战场景：IDE 多面板布局（sidebar resize: horizontal + terminal resize: vertical + flex-shrink: 0 防压缩）、表格列宽调整（th resize: horizontal，通常配合 JS 拖拽手柄更可控）、调试器面板（左右分栏可拖拽）、textarea 自适应（默认 both，限制为 vertical 防横向破坏布局）、与 JS 拖拽库对比（interact.js/jQuery UI/react-resizable 任意位置手柄 + 完整触摸/键盘支持）。选型建议：简单场景用原生 resize，复杂场景用 JS 库，混合方案原生 resize + ResizeObserver + JS 增强。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行实战场景演示', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runPatternsDemo() })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, 'IDE 多面板布局演示（侧栏 resize: horizontal，底部 resize: vertical）：'),
                h('div', { class: 'rs-ide' }, h('div', { class: 'rs-ide-sidebar' }, '文件树\n  src/\n    pages/\n    components/\n  package.json\n\n← 拖右边界调整宽度 →'), h('div', { class: 'rs-ide-main' }, '// 编辑器主区域\nimport { Page } from \'../../core/Component.js\';\n\nclass Demo extends Page {\n  renderPage(): Node | string | (Node | string)[] {\n    return h(\'div\', {}, \'Hello\');\n  }\n}')),
                h('div', { class: 'rs-ide-bottom-wrap' }, h('div', { class: 'rs-ide-bottom' }, '$ npm run build\n> building...\n✓ built in 1.23s\n\n↑ 拖上边界调整高度 ↓')),
                ...
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.patternsInfo || '（点击按钮查看 5 大实战场景完整代码与对比）')),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard8() {
        ...
        const card = new Card({
            ...
            children: [
                ...
                h('div', { class: 'rs-observer-target' }, '拖拽右下角手柄\nResizeObserver 将监听尺寸变化\n（jsdom 通常不实现 ResizeObserver，真实浏览器可观察）'),
                h('div', { class: 'rs-output rs-observer-output' }, f.resizeObserver
                    ? '等待 ResizeObserver 回调（拖拽上方元素）...'
                    : 'ResizeObserver 不可用（jsdom 通常不实现）。在真实浏览器中拖拽上方元素将触发回调，输出实时尺寸。'),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.pitfallsInfo || '（点击按钮查看 5 大陷阱与 10 条最佳实践完整说明）')),
                h(Alert, {
                    type: 'warning',
...
//# sourceMappingURL=CSSResizePage.js.map