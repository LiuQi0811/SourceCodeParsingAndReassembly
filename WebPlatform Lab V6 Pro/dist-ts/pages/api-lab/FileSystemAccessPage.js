...
export class FileSystemAccessPage extends Page {
    ...
    _inited = false;
    _mockDirHandle = null;
    _mockFileHandle = null;
    _mockSaveHandle = null;
    _pickedHandle = null;
    _saveContent = null;
    _saveHandle = null;
    initialState() {
    ...
    componentDidMount() {
        ...
        if (!hasOpen || !hasSave || !hasDir) {
            this._addLog('warn', 'File System Access API 部分不可用（需 Chromium 系浏览器 + 安全上下文 HTTPS/localhost）；'
        ...
        if (!hasFh || !hasDh || !hasWfs) {
            this._addLog('warn', 'FileSystemFileHandle / FileSystemDirectoryHandle / FileSystemWritableFileStream 类型在当前环境缺失');
        }
    }
    componentWillUnmount() {
        ...
        if (this.state.blobInfo && this.state.blobInfo.url) {
            try {
                URL.revokeObjectURL(this.state.blobInfo.url);
            }
            catch { /* noop */ }
        }
    ...
    async _openFilePicker() {
        ...
        if (typeof w.showOpenFilePicker !== 'function') {
            ...
            try {
                const mockFile = new File(['Hello, File System Access API!\n这是 mock 文件内容。\n第三行示例。'], 'mock-doc.txt', { type: 'text/plain', lastModified: Date.now() });
                this._mockFileHandle = this._makeMockFileHandle(mockFile);
                this._addLog('pick', `mock showOpenFilePicker({ multiple:false, types:[text/plain] }) → [${this._mockFileHandle.name}]（kind=${this._mockFileHandle.kind}）`);
            }
            catch (err) {
    ...
    async _readFileContent() {
        ...
        try {
            ...
            this._addLog('read', `读取完成：${file.name}（${file.size}B / ${file.type || '未知类型'}），共 ${text.length} 字符`);
        }
        catch (err) {
    ...
    _makeMockFileHandle(file) {
        return {
            name: file.name,
            kind: 'file',
            getFile: async () => file,
            // createWritable 返回 mock FileSystemWritableFileStream
            createWritable: async (_opts) => {
                let buffer = '';
                return {
                    write: async (data) => { if (typeof data === 'string')
                        buffer += data; },
                    seek: async (_pos) => { },
                    truncate: async (_size) => { buffer = buffer.slice(0, _size); },
                    close: async () => { },
                    _getBuffer: () => buffer,
                };
            },
    ...
    async _saveFilePicker() {
        ...
        if (typeof w.showSaveFilePicker !== 'function') {
            ...
            try {
                ...
                this._addLog('save', `mock showSaveFilePicker({ suggestedName:'fsa-save.txt', types:[text/plain] }) → ${this._mockSaveHandle.name}`);
            }
            catch (err) {
    ...
    async _writeAndClose() {
        ...
        const content = this.$('.fsa-save-textarea')?.value
            ?? 'Hello, File System Access API!\n这是写入测试内容。';
        try {
            this._addLog('write', `handle.createWritable({ keepExistingData:false }) → write(${content.length} 字符) → seek/truncate → close()`);
            ...
            try {
                await stream.truncate(content.length);
            }
            catch { /* noop */ }
    ...
    async _openDirPicker() {
        ...
        if (typeof w.showDirectoryPicker !== 'function') {
            ...
            this._addLog('dir', `mock showDirectoryPicker({ mode:'read' }) → ${this._mockDirHandle.name}（kind=${this._mockDirHandle.kind}）`);
            return;
        }
    ...
    async _walkDirectory() {
        ...
        try {
            ...
            const entries = [];
            // 异步递归：限制深度避免深层目录卡顿
            const walk = async (dirHandle, depth, prefix) => {
            ...
            this._addLog('walk', `遍历完成：${entries.length} 条目（文件 ${fileCount} / 目录 ${dirCount}，深度≤2）`);
        }
        catch (err) {
    ...
    _makeMockDirHandle() {
        const build = (name, tree) => {
            ...
            if (tree.kind === 'directory') {
                ...
                handle.getFileHandle = async (n, _opts) => build(n, { kind: 'file' });
                handle.removeEntry = async (_n, _opts) => { };
                handle.values = handle.entries;
                handle.keys = () => ({ async *[Symbol.asyncIterator]() {
    ...
    _createBlob() {
        ...
        try {
            const text = this.$('.fsa-blob-textarea')?.value
                ?? 'Hello Blob\n第二行\n含中文与 emoji 🌍';
            ...
            this._addLog('blob', `new Blob([text], { type }) → ${blob.size} 字节 / ${blob.type}`
    ...
    async _readBlobArrayBuffer() {
        ...
        try {
            ...
            this.setState({
                blobInfo: {
                    ...this.state.blobInfo,
                    arrayBuffer: `${buf.byteLength} 字节；hex(前32): ${hex}${buf.byteLength > 32 ? ' ...' : ''}`,
                },
            });
            this._addLog('blob', `blob.arrayBuffer() → ${buf.byteLength} 字节`);
        }
        catch (err) {
    ...
    _sliceBlob() {
        ...
        try {
            ...
            this._addLog('slice', `blob.slice(0, ${end}, 'text/plain') → ${slice.size} 字节（原 ${blob.size}，type=${slice.type}）`);
            // 异步读取 slice 内容展示
            slice.text().then((t) => {
                if (this._destroyed)
                    return;
    ...
    _newFileConstruct() {
        ...
        try {
            const file = new File(['Hello File!', '\n第二行'], `constructed-${Date.now().toString(36)}.txt`, { type: 'text/plain', lastModified: Date.now() });
            const isBlob = typeof Blob !== 'undefined' && (file instanceof Blob);
            this._addLog('blob', `new File([parts], name, { type, lastModified }) → ${file.name}，${file.size}B，type=${file.type}，File instanceof Blob = ${isBlob}`);
        }
        catch (err) {
    ...
    _readAsDataURL() {
        ...
        reader.onloadend = () => {
            ...
            this._addLog('dataurl', `onloadend：readAsDataURL 完成${isImage ? '（图片，已生成 data:image/png;base64,...）' : ''}，前缀=${String(result).slice(0, 30)}...`);
        };
        reader.onerror = () => {
            if (this._destroyed)
                return;
    ...
    _checkFileReaderSync() {
        if (typeof FileReaderSync !== 'undefined') {
            this._addLog('reader', `FileReaderSync 可用（typeof = ${typeof FileReaderSync}），仅 Worker 内可同步调用 readAsText 等`);
        }
        else {
            this._addLog('warn', 'FileReaderSync 不可用（仅 Worker 内可用，主线程无此构造器；同步阻塞读取会冻结 UI）');
        }
    }
    _renderCard1() {
        const s = this.state;
        return this._card('1. File System Access API — showOpenFilePicker', '(window as any).showOpenFilePicker({ multiple, types, excludeAcceptAllOption }) → Promise<FileSystemFileHandle[]>；'
            + 'FileSystemFileHandle：name / kind("file") / getFile() → Promise<File> / createWritable() → Promise<FileSystemWritableFileStream>。', h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'primary' }, 'showOpenFilePicker'), h(Tag, { color: 'success' }, 'FileSystemFileHandle'), h(Tag, { color: 'warning' }, 'getFile()')), [
            h('div', { class: 'flex gap-sm flex-wrap' }, this._btn('打开文件选择器', { type: 'primary', size: 'sm', onClick: () => this._openFilePicker() }), this._btn('读取文件内容', { type: 'primary', size: 'sm', onClick: () => this._readFileContent() })),
            s.openedFile
                ? h('div', { class: 'flex flex-col gap-xs mt-sm' }, h('div', { class: 'fs-sm fw-medium' }, `已读取：${s.openedFile.name}（${s.openedFile.size}B / ${s.openedFile.type}）`), h('pre', { class: 'code-block', style: { whiteSpace: 'pre-wrap', wordBreak: 'break-all', maxHeight: '180px', overflow: 'auto' } }, s.openedFile.text.slice(0, 1000) + (s.openedFile.text.length > 1000 ? '\n...(截断)' : '')))
                : h('div', { class: 'fs-sm text-tertiary mt-sm' }, '（点击「打开文件选择器」后「读取文件内容」；能力不可用时返回 mock 句柄）'),
        ]);
    }
    _renderCard2() {
        const s = this.state;
        return this._card('2. showSaveFilePicker + FileSystemWritableFileStream', '(window as any).showSaveFilePicker({ suggestedName, types }) → Promise<FileSystemFileHandle>；'
            + 'handle.createWritable({ keepExistingData }) → Promise<FileSystemWritableFileStream>；'
            + 'stream.write(textOrBlobOrBuffer) / seek(position) / truncate(size) / close()；keepExistingData:false（默认）会清空。', h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'primary' }, 'showSaveFilePicker'), h(Tag, { color: 'success' }, 'createWritable'), h(Tag, { color: 'warning' }, 'write/seek/truncate/close')), [
            h('div', { class: 'flex flex-col gap-xs' }, h('div', { class: 'fs-sm fw-medium' }, '待写入内容（编辑后点「写入并 close」）：'), h('textarea', {
                class: 'fsa-save-textarea',
                rows: 3,
                onInput: (e) => { this._saveContent = e.target.value; },
            }, 'Hello, File System Access API!\n这是写入测试内容。')),
            h('div', { class: 'flex gap-sm flex-wrap' }, this._btn('保存文件', { type: 'primary', size: 'sm', onClick: () => this._saveFilePicker() }), this._btn('写入并 close', { type: 'primary', size: 'sm', onClick: () => this._writeAndClose() })),
            s.savedFile
                ? h('div', { class: 'flex flex-col gap-xs mt-sm' }, h('div', { class: 'fs-sm fw-medium' }, `已写入：${s.savedFile.name}（${s.savedFile.size} 字节）`), h('pre', { class: 'code-block', style: { whiteSpace: 'pre-wrap', wordBreak: 'break-all' } }, s.savedFile.content))
                : h('div', { class: 'fs-sm text-tertiary mt-sm' }, '（点击「保存文件」选择目标后「写入并 close」）'),
        ]);
    }
    _renderCard3() {
        const s = this.state;
        return this._card('3. showDirectoryPicker + 递归遍历', '(window as any).showDirectoryPicker({ mode }) → Promise<FileSystemDirectoryHandle>；'
            + '(dir as any).entries() / values() / keys() 异步迭代器；getFileHandle(name,{create:true}) / removeEntry(name,{recursive:true})；'
            + 'for await (const [name, handle] of (dir as any).entries()) 递归遍历，handle.kind === "file" / "directory"。', h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'primary' }, 'showDirectoryPicker'), h(Tag, { color: 'success' }, 'entries()'), h(Tag, { color: 'warning' }, 'for await')), [
            h('div', { class: 'flex gap-sm flex-wrap' }, this._btn('打开目录', { type: 'primary', size: 'sm', onClick: () => this._openDirPicker() }), this._btn('递归遍历', { type: 'primary', size: 'sm', onClick: () => this._walkDirectory() })),
            s.dirTree
                ? h('div', { class: 'flex flex-col gap-xs mt-sm' }, h('div', { class: 'fs-sm fw-medium' }, `目录：${s.dirTree.name}（${s.dirTree.entries.length} 条目）`), h('div', { class: 'log-panel', style: { maxHeight: '280px', overflow: 'auto' } }, s.dirTree.entries.map((e) => h('div', { class: 'log-panel__line' }, h('span', {
                    class: `log-panel__tag log-panel__tag--${e.kind === 'directory' ? 'dir' : e.kind === 'file' ? 'file' : 'err'}`,
                }, e.kind === 'directory' ? 'DIR' : e.kind === 'file' ? 'FILE' : 'ERR'), h('span', { style: { paddingLeft: `${e.depth * 18}px` } }, e.path || e.name)))))
    ...
    _renderCard4() {
        const s = this.state;
        const bi = s.blobInfo;
        return this._card('4. File / Blob API', 'new File([bits], name, { type, lastModified })；new Blob([data], { type })；'
            + 'blob.size / blob.type；blob.text() → Promise<string> / arrayBuffer() → Promise<ArrayBuffer> / stream() → ReadableStream；'
            + 'blob.slice(start, end, contentType)；URL.createObjectURL(blob) / revokeObjectURL(url)。', h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'primary' }, 'Blob'), h(Tag, { color: 'success' }, 'File'), h(Tag, { color: 'warning' }, 'createObjectURL')), [
            h('div', { class: 'flex flex-col gap-xs' }, h('div', { class: 'fs-sm fw-medium' }, 'Blob 内容：'), h('textarea', {
                class: 'fsa-blob-textarea',
                rows: 3,
                onInput: (e) => { this._blobText = e.target.value; },
            }, 'Hello Blob\n第二行\n含中文与 emoji 🌍')),
            h('div', { class: 'flex gap-sm flex-wrap' }, this._btn('创建 Blob', { type: 'primary', size: 'sm', onClick: () => this._createBlob() }), this._btn('读取 text', { type: 'primary', size: 'sm', onClick: () => this._readBlobText() }), this._btn('读取 arrayBuffer', { type: 'primary', size: 'sm', onClick: () => this._readBlobArrayBuffer() }), this._btn('slice', { size: 'sm', onClick: () => this._sliceBlob() }), this._btn('new File()', { size: 'sm', onClick: () => this._newFileConstruct() })),
            bi
                ? h('div', { class: 'flex flex-col gap-xs mt-sm' }, h('div', { class: 'fs-sm text-secondary' }, `Blob：${bi.size} 字节 / ${bi.type}${bi.url ? '，object URL 已生成' : '（URL.createObjectURL 不可用）'}`), bi.text && h('div', { class: 'fs-sm' }, `blob.text() → ${JSON.stringify(bi.text.slice(0, 60))}`), bi.arrayBuffer && h('div', { class: 'fs-sm', style: { wordBreak: 'break-all' } }, `blob.arrayBuffer() → ${bi.arrayBuffer}`), bi.slice && h('div', { class: 'fs-sm' }, `blob.slice() → ${bi.slice}`))
    ...
    _renderCard5() {
        ...
        return this._card('5. FileReader + FileReaderSync + 数据 URL', 'new FileReader()；readAsText(blob, encoding) / readAsArrayBuffer(blob) / readAsDataURL(blob) / readAsBinaryString(blob)；'
            + '事件 onloadstart / onprogress / onload / onerror / onloadend / onabort；result / error / readyState(0 EMPTY/1 LOADING/2 DONE)；'
            + 'abort()；FileReaderSync（仅 Worker）为同步版；数据 URL 格式 data:[<mediatype>][;base64],<data>。', h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'primary' }, 'FileReader'), h(Tag, { color: 'success' }, 'readAsDataURL'), h(Tag, { color: 'warning' }, 'FileReaderSync')), [
            h('div', { class: 'flex gap-sm flex-wrap' }, this._btn('readAsDataURL', { type: 'primary', size: 'sm', onClick: () => this._readAsDataURL() }), this._btn('abort 演示', { danger: true, size: 'sm', onClick: () => this._abortDemo() }), this._btn('检测 FileReaderSync', { size: 'sm', onClick: () => this._checkFileReaderSync() })),
            rr
                ? h('div', { class: 'flex flex-col gap-xs mt-sm' }, h('div', { class: 'flex items-center gap-sm' }, h('span', { class: 'fs-sm text-secondary' }, '进度：'), h('div', { style: { flex: '1', height: '8px', background: 'var(--color-fill)', borderRadius: '4px', overflow: 'hidden' } }, h('div', { style: { height: '100%', width: `${rr.progress}%`, background: 'var(--color-primary)', transition: 'width 0.2s' } })), h('span', { class: 'fs-sm fw-medium' }, `${rr.progress}%`), h('span', { class: 'fs-sm text-tertiary' }, `readyState=${rr.readyState}`), h('span', { class: 'fs-sm text-tertiary' }, `(${rr.method})`)), rr.isImage && rr.dataUrl && h('div', { class: 'flex flex-col gap-xs' }, h('div', { class: 'fs-sm fw-medium' }, 'data URL 图片预览：'), h('img', {
                    src: rr.dataUrl, alt: 'preview',
                    style: { maxWidth: '320px', maxHeight: '240px', border: '1px solid var(--color-border)' },
                })), rr.result && h('div', { class: 'flex flex-col gap-xs' }, h('div', { class: 'fs-sm fw-medium' }, '读取结果（截断 120 字符）：'), h('pre', { class: 'code-block', style: { whiteSpace: 'pre-wrap', wordBreak: 'break-all' } }, rr.result)))
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'page api-lab-page' }, h('h2', { class: 'section-title' }, '文件系统访问 API 实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, 'File System Access API 让 Web 直接读写本地文件；File/Blob/FileReader 是处理二进制数据的基石。本页演示文件选择/保存/目录遍历与二进制读写。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderLogPanel());
    }
}
//# sourceMappingURL=FileSystemAccessPage.js.map