...
export class WebComponentsDeepPage extends Page {
    ...
    componentDidMount() {
        ...
        if (this._counterEl === undefined)
            this._counterEl = null;
        if (this._shadowEl === undefined)
            this._shadowEl = null;
        if (this._slotHost === undefined)
            this._slotHost = null;
        if (this._partEl === undefined)
            this._partEl = null;
        if (this._inputEl === undefined)
            this._inputEl = null;
        if (this._adoptIframe === undefined)
            this._adoptIframe = null;
        if (this._counterDefined === undefined)
            this._counterDefined = false;
        if (this._cardDefined === undefined)
            this._cardDefined = false;
        if (this._slotHostDefined === undefined)
            this._slotHostDefined = false;
        if (this._partElDefined === undefined)
            this._partElDefined = false;
        if (this._inputDefined === undefined)
        ...
        const summary = '能力检测：customElements.define=' + (ceSupported ? '✓' : '✗') +
    ...
    _defineCounter() {
        ...
        if (this._counterDefined) {
            this._addLog('define', '<wc-deep-counter> 已定义（跳过）');
            return;
        }
    ...
    _insertCounter() {
        if (!this._caps().ce) {
            this._addLog('warn', 'customElements 不可用');
            return;
        }
        if (!this._counterDefined) {
            this._addLog('warn', '请先点击「定义元素」');
            return;
        }
        const container = this.$('.wc-deep-counter-host');
        if (!container) {
            this._addLog('warn', '找不到挂载容器');
            return;
        }
        try {
    ...
    _removeCounter() {
        if (!this._counterEl) {
            this._addLog('warn', '当前没有 <wc-deep-counter> 元素可移除');
            return;
        }
        try {
            ...
            class WcDeepCard extends HTMLElement {
                constructor() {
                    ...
                    if (typeof this.attachShadow === 'function') {
                        ...
                        self._addLog('shadow', `attachShadow({ mode: '${wcDeepShadowMode}' } as any) → 返回 ShadowRoot`);
                    }
                    else {
    ...
    _attachShadowMode(mode) {
        if (!this._ensureShadowCardDefined())
            return;
    ...
    _triggerSlotchange() {
        if (!this._ensureSlotHostDefined())
            return;
        ...
        try {
            ...
            setTimeout(() => {
                try {
                    span.remove();
                    this._addLog('change', '已 removeChild，分发内容再次变化应触发 slotchange');
                }
                catch { /* noop */ }
            }, 100);
    ...
    _listAssignedNodes() {
        if (!this._slotHost) {
            this._addLog('warn', '请先点击「触发 slotchange」创建 <wc-deep-slot-host>');
            return;
        }
        ...
        try {
            ...
            if (!sr) {
                this._addLog('warn', 'element.shadowRoot 为 null（mode=closed 或未升级）');
                return;
            }
            const lines = [];
            const readSlot = (slot, label) => {
                if (!slot) {
                    lines.push(`未找到 ${label}（HTMLSlotElement 可能不可用）`);
                    return;
                }
                let nodes = [], elems = [];
            ...
            if (!caps.slotEl)
                this._addLog('warn', 'HTMLSlotElement 类型不可用，assignedNodes 可能为 stub');
        }
        catch (err) {
    ...
    _adoptedStyleSheets() {
        ...
        if (!this._partEl) {
            this._addLog('warn', '请先点击「设置 part」创建 <wc-deep-part>');
            return;
        }
        try {
            // 构造 Constructable Stylesheet：new CSSStyleSheet() + replaceSync(cssText)
            const sheet = new CSSStyleSheet();
            sheet.replaceSync(':host { border-color:#13c2c2 !important; background:#e6fffb; }' +
                '::part(title) { color:#13c2c2 !important; border-left-color:#13c2c2 !important; }');
            const sr = this._partEl.shadowRoot;
            if (!sr) {
                this._addLog('warn', 'element.shadowRoot 为 null（未升级或 closed）');
                return;
            }
    ...
    _cloneTemplate() {
        ...
        try {
            ...
            if (!tpl || !tpl.content) {
                this._addLog('warn', '找不到 <template> 或 .content 为空');
                return;
            }
    ...
    _defineFormAssociated() {
        ...
        if (this._inputDefined) {
            this._addLog('template', '<wc-deep-input> 已定义（跳过）');
            return;
        }
    ...
    _attachInternalsDemo() {
        ...
        if (!this._inputDefined) {
            this._addLog('warn', '请先点击「定义 form-associated 元素」');
            return;
        }
        ...
        try {
            ...
            if (!internals) {
                this._addLog('warn', '元素未升级或 attachInternals 未执行，ElementInternals 不可用');
                return;
            }
            const lines = [];
            ...
            if ('willValidate' in internals)
                lines.push(`internals.willValidate = ${internals.willValidate}`);
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.ce ? 'success' : 'warning' }, caps.ce ? 'customElements 可用' : '不可用'), h(Tag, { color: 'primary' }, 'wc-deep-counter')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'customElements.define(name, constructor, options) 注册自定义元素；name 必须含连字符（如 \'wc-deep-counter\'）。生命周期回调：connectedCallback（插入 DOM）/ disconnectedCallback（移除）/ attributeChangedCallback（observedAttributes 属性变化）/ adoptedCallback（adopt 到新文档）。static get observedAttributes() 返回要观察的属性数组；customElements.get / whenDefined → Promise / upgrade(node) 升级未注册元素。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('定义元素', { type: 'primary', size: 'sm', disabled: !caps.ce, onClick: () => this._defineCounter() }), this._btn('插入元素触发 connectedCallback', { size: 'sm', disabled: !caps.ce, onClick: () => this._insertCounter() }), this._btn('改属性触发 attributeChangedCallback', { size: 'sm', disabled: !caps.ce, onClick: () => this._changeCounterAttr() }), this._btn('移除元素触发 disconnectedCallback', { size: 'sm', danger: true, disabled: !caps.ce, onClick: () => this._removeCounter() })),
                h('div', { class: 'wc-deep-counter-host mt-sm' }),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '生命周期回调轨迹（最近 12 条）：'),
                h('pre', { class: 'code-block', style: { maxHeight: '180px', overflow: 'auto' } }, h('code', {}, s.lifecycleLog || '（点击「定义元素」→「插入元素」→「改属性」→「移除元素」）')),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.attachShadow ? 'success' : 'warning' }, caps.attachShadow ? 'attachShadow 可用' : '不可用'), h(Tag, { color: 'primary' }, 'wc-deep-card'), h(Tag, { color: 'warning' }, s.shadowStatus)),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'element.attachShadow({ mode: \'open\' | \'closed\' }) 返回 ShadowRoot；open 时 element.shadowRoot 可访问，closed 返回 null。Shadow DOM 提供样式隔离（内部 <style> 不外泄）与 DOM 隔离（querySelector 查不到内部）。slot 分发：Light DOM 子节点带 slot="x" → 分发到 <slot name="x">；无 slot 属性 → 默认 <slot>。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('attachShadow open', { type: 'primary', size: 'sm', disabled: !caps.attachShadow, onClick: () => this._attachShadowOpen() }), this._btn('attachShadow closed', { size: 'sm', disabled: !caps.attachShadow, onClick: () => this._attachShadowClosed() }), this._btn('slot 分发演示', { size: 'sm', disabled: !caps.attachShadow, onClick: () => this._slotDistribute() })),
                h('div', { class: 'wc-deep-shadow-host mt-sm' }),
                h('p', { class: 'fs-sm text-tertiary mt-sm' }, 'open 模式 element.shadowRoot 返回 ShadowRoot 对象；closed 模式返回 null，外部无法访问 Shadow 树。::slotted(span) 可在 Shadow 内样式化被分发的 Light DOM 节点。'),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.slotEl ? 'success' : 'warning' }, caps.slotEl ? 'HTMLSlotElement 可用' : '不可用'), h(Tag, { color: 'primary' }, 'slotchange / assignedNodes')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '<slot> 元素的 slotchange 事件在分发内容变化时触发；slot.assignedNodes({ flatten: true }) 返回分发节点数组（flatten 递归展开嵌套 slot）；assignedElements() 仅返回 Element。<slot name="header"> 命名插槽 vs 默认插槽 <slot></slot>。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('触发 slotchange', { type: 'primary', size: 'sm', disabled: !caps.attachShadow, onClick: () => this._triggerSlotchange() }), this._btn('assignedNodes 列表', { size: 'sm', disabled: !caps.attachShadow, onClick: () => this._listAssignedNodes() })),
                h('div', { class: 'wc-deep-slot-host mt-sm' }),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, 'assignedNodes / assignedElements 输出：'),
                h('pre', { class: 'code-block', style: { maxHeight: '160px', overflow: 'auto' } }, h('code', {}, s.slotNodes || '（点击「触发 slotchange」→「assignedNodes 列表」）')),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.cssSheet ? 'success' : 'warning' }, caps.cssSheet ? 'CSSStyleSheet 可用' : '不可用'), h(Tag, { color: 'primary' }, '::part() / :host()'), h(Tag, { color: 'warning' }, s.partStatus)),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'element.part = \'foo bar\' 或 setAttribute(\'part\', \'foo\') 暴露 Shadow 内部元素；外部 CSS 用 ::part(foo) { ... } 设置样式（穿透 Shadow 样式隔离）。:host 选择宿主；:host([disabled]) 匹配带属性宿主；:host-context(.dark) 上下文匹配。shadowRoot.adoptedStyleSheets = [sheet] 应用 Constructable Stylesheet（new CSSStyleSheet() + replaceSync），可跨 ShadowRoot 共享。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('设置 part', { type: 'primary', size: 'sm', disabled: !caps.attachShadow, onClick: () => this._setPart() }), this._btn(':host 选择器演示', { size: 'sm', disabled: !caps.attachShadow, onClick: () => this._hostSelector() }), this._btn('adoptedStyleSheets 注入', { size: 'sm', disabled: !caps.cssSheet || !caps.attachShadow, onClick: () => this._adoptedStyleSheets() })),
                h('div', { class: 'wc-deep-part-host mt-sm' }),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '120px', overflow: 'auto' } }, `/* Shadow 内部 */
<div part="title">...</div>
/* 外部 CSS 穿透 ::part() */
...
shadowRoot.adoptedStyleSheets = [sheet];`),
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.template ? 'success' : 'warning' }, caps.template ? 'template 可用' : '不可用'), h(Tag, { color: caps.attachInternals ? 'success' : 'warning' }, caps.attachInternals ? 'attachInternals 可用' : '不可用'), h(Tag, { color: 'primary' }, 'wc-deep-input')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '<template>.content 返回 DocumentFragment（惰性）；cloneNode(true) 深拷贝。customElements.define(name, ctor, { formAssociated: true }) 或 static formAssociated = true 声明表单关联元素；element.attachInternals() 返回 ElementInternals；internals.setFormValue(value) 提交给 <form>；internals.states（CustomStateSet）驱动 :--valid 伪类；willValidate / validity 表单校验；formResetCallback / formStateRestoreCallback 等回调。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('克隆 template', { type: 'primary', size: 'sm', disabled: !caps.template, onClick: () => this._cloneTemplate() }), this._btn('定义 form-associated 元素', { size: 'sm', disabled: !caps.ce, onClick: () => this._defineFormAssociated() }), this._btn('attachInternals 演示', { size: 'sm', disabled: !caps.attachInternals, onClick: () => this._attachInternalsDemo() })),
                // <template> 元素：用 html prop 填充 .content
                h('template', { id: 'wc-deep-template', html: '<style>' +
                ...
                h('pre', { class: 'code-block', style: { maxHeight: '120px', overflow: 'auto', whiteSpace: 'pre-wrap' } }, h('code', {}, s.internalsStatus || '（点击「定义 form-associated 元素」→「attachInternals 演示」）')),
                h(Alert, {
                    type: 'info',
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'page api-lab-page' }, h('h2', { class: 'section-title' }, 'Web Components 深度实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, '演示 Web Components：customElements.define 生命周期 / Shadow DOM attachShadow / HTMLSlotElement slotchange / CSS ::part()+:host()+adoptedStyleSheets / HTMLTemplateElement+form-associated+ElementInternals。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderLogPanel());
    }
}
//# sourceMappingURL=WebComponentsDeepPage.js.map