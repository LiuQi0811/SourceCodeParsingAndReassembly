// =====================================================================
// WebAssemblyPage.ts —— WebAssembly 实验室
// 演示 MDN：
...
// import 模块：调用 run() 会通过 importObject 调用 JS 的 env.log(42)
...
export class WebAssemblyPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!hasStreaming)
            this._addLog('warn', 'WebAssembly.instantiateStreaming 不可用（需 http(s) 且响应 Content-Type: application/wasm）');
        if (!hasException)
            this._addLog('warn', 'WebAssembly.Exception 不可用（需 wasm exceptions proposal 支持）');
        if (!hasTag)
            this._addLog('warn', 'WebAssembly.Tag 不可用（需 wasm exceptions proposal 支持）');
    }
    ...
    _validateBytes() {
        ...
        try {
            ...
            this.setState({
                moduleInfo: `WebAssembly.validate(bufferSource) → boolean（仅校验，不实例化）\n` +
    ...
    async _compileModule() {
        ...
        try {
            ...
            const module = await WebAssembly.compile(ADDER_BYTES);
            this._module = module;
            ...
            const importsText = importsMeta.length
                ...
                : '（无导入）';
            this.setState({
                moduleInfo: `WebAssembly.compile(ADDER_BYTES) → WebAssembly.Module ✓\n` +
    ...
    async _instantiateModule() {
        ...
        try {
            ...
            const { module, instance } = await WebAssembly.instantiate(ADDER_BYTES, {});
            this._module = module;
            this._instance = instance;
            const exports = instance.exports;
            const r1 = exports.add(2, 3);
            const r2 = exports.add(40, 2);
            let streamLine = '';
            if (this._caps().streaming && this._caps().fetch) {
                try {
                    const resp = new Response(ADDER_BYTES, { headers: { 'Content-Type': 'application/wasm' } });
                    const streamResult = await WebAssembly.instantiateStreaming(Promise.resolve(resp), {});
                    const streamInst = streamResult.instance;
                    const streamExports = streamInst.exports;
                    const r3 = streamExports.add(7, 8);
            ...
            this.setState({
                moduleInfo: `WebAssembly.instantiate(ADDER_BYTES, {}) → { module, instance } ✓\n` +
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.wasm ? 'success' : 'error' }, caps.wasm ? 'WebAssembly ✓' : '不可用'), h(Tag, { color: 'primary' }, 'compile / instantiate')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'WebAssembly.validate(bufferSource) → boolean 验证字节码合法性；compile(bufferSource) → Promise<Module>；instantiate(bufferSource, importObject) → Promise<{module, instance}>；instantiateStreaming(fetch(url), importObject) 流式编译（响应必须是 application/wasm MIME 类型）。Module.exports(module) / Module.imports(module) 返回导出/导入描述 { name, kind }，kind ∈ function/memory/table/global。本卡片用最小的 WAT 文本对应的二进制（adder 模块）演示编译流程。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('validate', { type: 'primary', size: 'sm', disabled: !caps.wasm, onClick: () => this._validateBytes() }), this._btn('compile', { type: 'primary', size: 'sm', disabled: !caps.wasm, onClick: () => this._compileModule() }), this._btn('instantiate', { type: 'primary', size: 'sm', disabled: !caps.wasm, onClick: () => this._instantiateModule() })),
                h('div', { class: 'fs-sm text-secondary' }, '编译 / 实例化 / 内省结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '220px', overflow: 'auto' } }, h('code', {}, s.moduleInfo || '（点击 validate / compile / instantiate）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, 'adder 模块对应的 WAT 文本与字节布局：'),
                h('pre', { class: 'code-block', style: { maxHeight: '180px', overflow: 'auto' } }, h('code', {}, `; (module (func (export "add") (param i32 i32) (result i32)
...
0a 09 01 07 00 20 00 20 01 6a 0b ; code: local.get 0/1, i32.add, end`)),
    ...
    _createMemory() {
        ...
        try {
            ...
            this.setState({
                memorySize: `new WebAssembly.Memory({ initial: 1, maximum: 10, shared: false })\n` +
            ...
            this._addLog('memory', `已创建 Memory：(buffer as any).byteLength=${len}（${pages} 页），maximum=10 页`);
        }
        catch (err) {
    ...
    _growMemory() {
        ...
        try {
            ...
            const prevPages = this._memory.grow(2);
            const afterLen = this._memory.buffer.byteLength;
            const afterPages = afterLen / 65536;
            this.setState({
                memorySize: `memory.grow(2) → 返回之前页数 ${prevPages}\n` +
                    `grow 前：(buffer as any).byteLength = ${beforeLen}（${beforePages} 页）\n` +
                    `grow 后：(buffer as any).byteLength = ${afterLen}（${afterPages} 页）\n` +
    ...
    _readWriteBuffer() {
        ...
        try {
            ...
            this.setState({
                memorySize: `通过 DataView 读写 memory.buffer：\n` +
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.memory ? 'success' : 'error' }, caps.memory ? 'Memory ✓' : '不可用'), h(Tag, { color: 'primary' }, '64KB / 页')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'new WebAssembly.Memory({ initial: 1, maximum: 10, shared: false }) 创建线性内存；memory.buffer → ArrayBuffer（长度 = initial × 64KB）；memory.grow(delta) 返回之前的页数（每页 64KB），grow 后 buffer 引用会变为新的更大 ArrayBuffer（旧引用作废）。共享内存 shared: true 需要 crossOriginIsolated 环境（COOP/COEP）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('创建 Memory', { type: 'primary', size: 'sm', disabled: !caps.memory, onClick: () => this._createMemory() }), this._btn('grow(扩容)', { type: 'primary', size: 'sm', disabled: !caps.memory, onClick: () => this._growMemory() }), this._btn('读写 buffer', { size: 'sm', disabled: !caps.memory, onClick: () => this._readWriteBuffer() })),
                h('div', { class: 'fs-sm text-secondary' }, 'Memory 状态：'),
                h('pre', { class: 'code-block', style: { maxHeight: '180px', overflow: 'auto' } }, h('code', {}, s.memorySize || '（点击「创建 Memory」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '140px', overflow: 'auto' } }, h('code', {}, `const memory = new WebAssembly.Memory({ initial: 1, maximum: 10, shared: false });
...
// grow 后 memory.buffer 变为新 ArrayBuffer，旧 view 失效`)),
    ...
    _createTable() {
        ...
        try {
            ...
            this._table = table;
            this.setState({
                tableInfo: `new WebAssembly.Table({ element: 'anyfunc', initial: 2, maximum: 10 })\n` +
    ...
    _setTableFunc() {
        ...
        try {
            ...
            this.setState({
                tableInfo: `table.set(0, (x) => x * 2) / table.set(1, (x) => x + 1)\n` +
    ...
    _getTableFunc() {
        ...
        try {
            ...
            const prev = this._table.grow(1, null);
            const lenAfter = this._table.length;
            this.setState({
                tableInfo: `table.get(0)(21) = ${r0}（取出 fn×2 并调用）\n` +
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.table ? 'success' : 'error' }, caps.table ? 'Table ✓' : '不可用'), h(Tag, { color: 'primary' }, 'anyfunc / externref')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'new WebAssembly.Table({ element: "anyfunc" | "externref", initial, maximum }) 创建函数引用表；table.length 获取长度；table.get(index) 取出函数引用；table.set(index, value) 设置引用；table.grow(delta, value) 扩容。用途：动态函数表，支持 call_indirect 间接调用。element 类型：anyfunc（函数引用，规范名 funcref）/ externref（外部引用）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('创建 Table', { type: 'primary', size: 'sm', disabled: !caps.table, onClick: () => this._createTable() }), this._btn('set 函数', { type: 'primary', size: 'sm', disabled: !caps.table, onClick: () => this._setTableFunc() }), this._btn('get 函数', { size: 'sm', disabled: !caps.table, onClick: () => this._getTableFunc() })),
                h('div', { class: 'fs-sm text-secondary' }, 'Table 状态：'),
                h('pre', { class: 'code-block', style: { maxHeight: '180px', overflow: 'auto' } }, h('code', {}, s.tableInfo || '（点击「创建 Table」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '120px', overflow: 'auto' } }, h('code', {}, `const table = new WebAssembly.Table({ element: 'anyfunc', initial: 2, maximum: 10 });
...
const prev = table.grow(1, null);               // 扩容，新槽位用 null 填充`)),
    ...
    _createGlobal() {
        ...
        try {
            ...
            this.setState({
                globalValue: `new WebAssembly.Global({ value: 'i32', mutable: true }, 42)\n` +
    ...
    _writeGlobal() {
        ...
        try {
            ...
            this.setState({
                globalValue: `写入 Global（mutable: true）：\n` +
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.global ? 'success' : 'error' }, caps.global ? 'Global ✓' : '不可用'), h(Tag, { color: 'primary' }, 'i32 / i64 / f32 / f64')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'new WebAssembly.Global({ value: "i32" | "i64" | "f32" | "f64", mutable: true }, initialValue) 创建全局变量；global.value 读写值；global.valueOf() 返回值（与 .value 等价）。value 类型：i32 / i64 / f32 / f64 / v128 / externref。mutable: false 时 value 不可写（读取仍可）。用途：WASM 模块导出的全局变量，可被 JS 读写。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('创建 Global', { type: 'primary', size: 'sm', disabled: !caps.global, onClick: () => this._createGlobal() }), this._btn('读 value', { size: 'sm', disabled: !caps.global, onClick: () => this._readGlobal() }), this._btn('写 value', { type: 'primary', size: 'sm', disabled: !caps.global, onClick: () => this._writeGlobal() })),
                h('div', { class: 'fs-sm text-secondary' }, 'Global 状态：'),
                h('pre', { class: 'code-block', style: { maxHeight: '180px', overflow: 'auto' } }, h('code', {}, s.globalValue || '（点击「创建 Global」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '120px', overflow: 'auto' } }, h('code', {}, `const g = new WebAssembly.Global({ value: 'i32', mutable: true }, 42);
...
// ro.value = 9n;               // mutable:false 赋值会抛 TypeError`)),
    ...
    async _demoInterop() {
        ...
        try {
            ...
            exports.run();
            this.setState({
                runtimeResult: `importObject = { env: { log(n), abort() } }（JS 函数传给 WASM）\n` +
    ...
    async _triggerRuntimeError() {
        ...
        // 4) WebAssembly.Tag / WebAssembly.Exception
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.wasm ? 'success' : 'error' }, caps.wasm ? '互操作 ✓' : '不可用'), h(Tag, { color: caps.exception && caps.tag ? 'primary' : 'warning' }, caps.exception && caps.tag ? 'Exception/Tag ✓' : 'Exception/Tag ✗')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'importObject 传入 JS 函数给 WASM 调用（{ env: { log, abort } }）；instance.exports 暴露 WASM 的函数/内存/表/全局给 JS。WebAssembly.RuntimeError=运行时错误（除零、unreachable）；CompileError=编译错误（字节码非法）；LinkError=链接错误（导入不匹配）；WebAssembly.Exception=异常对象，WebAssembly.Tag=异常标签，可与 wasm exceptions proposal 配合跨边界抛出。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('模拟互操作', { type: 'primary', size: 'sm', disabled: !caps.wasm, onClick: () => this._demoInterop() }), this._btn('触发 RuntimeError', { danger: true, size: 'sm', disabled: !caps.wasm, onClick: () => this._triggerRuntimeError() })),
                h('div', { class: 'fs-sm text-secondary' }, '互操作 / 异常处理结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, s.runtimeResult || '（点击「模拟互操作」或「触发 RuntimeError」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '160px', overflow: 'auto' } }, h('code', {}, `// WASM 调用 JS：importObject 把 JS 函数交给 WASM
...
const exc = new WebAssembly.Exception(tag, [42]);`)),
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'page api-lab-page' }, h('h2', { class: 'section-title' }, 'WebAssembly 实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, 'WebAssembly（WASM）让 C/C++/Rust 等编译产物在浏览器中近原生速度运行。本页演示 Module/Instance/Memory/Table/Global 与 instantiateStreaming。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderLogPanel());
    }
}
//# sourceMappingURL=WebAssemblyPage.js.map