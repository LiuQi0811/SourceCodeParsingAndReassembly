...
export class MediaSessionAPIPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!f.mediaSession)
            this._addLog('warn', 'navigator.mediaSession 不可用（jsdom 通常不实现，Chrome 73+ / Firefox 82+ / Safari 15+ 全主流稳定）');
        if (!f.mediaMetadataCtor)
            this._addLog('warn', 'window.MediaMetadata 构造函数不可用（jsdom 通常不实现，Chrome 57+ / Firefox 82+ / Safari 15+ 支持）');
        if (!f.setActionHandler)
            this._addLog('warn', 'navigator.mediaSession.setActionHandler 不可用（jsdom 通常不实现）');
        if (!f.setPositionState)
            this._addLog('warn', 'navigator.mediaSession.setPositionState 不可用（较新 API，Chrome 81+ / Safari 15+ 支持，Firefox 仍部分支持）');
        if (!f.isSecureContext)
            this._addLog('warn', '当前非安全上下文（Media Session API 要求 secure context：https 或 localhost）');
    ...
    _flags() {
        ...
        return {
            ...
            cameraAction: safe(() => typeof navigator.mediaSession === 'object' &&
                navigator.mediaSession !== null &&
                typeof navigator.mediaSession.cameraAction !== 'undefined'), // Chrome 扩展
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'Media Session API 由 W3C Media Working Group 维护，让网页媒体"穿透"到 OS 层级：锁屏界面显示标题 / 艺术家 / 封面，通知栏显示媒体控件，硬件媒体键（键盘 Fn 键 / 耳机线控 / 蓝牙耳机）触发，OS 媒体中心（Windows / macOS / Android / iOS 控制中心）集成。vs HTMLMediaElement API：后者仅控制页面内播放，无法显示锁屏 / 通知栏控件。核心三件套：metadata（元数据）/ playbackState（播放状态）/ setActionHandler（动作处理）。浏览器支持：Chrome 73+ / Firefox 82+ / Safari 15+ 全主流稳定，需安全上下文（https 或 localhost）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取概述信息', { type: 'primary', size: 'sm', onClick: () => this._runOverviewDemo() })),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.overviewInfo || '（点击「读取概述信息」查看 Media Session API 全景与完整代码示例）')),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'navigator.mediaSession 是 Media Session API 的根入口（只读属性，类型 MediaSession 对象）。可读写属性：metadata（MediaMetadata 实例）、playbackState（none/paused/playing）。可调用方法：setActionHandler(action, callback)、setPositionState(state)。要求安全上下文（window.isSecureContext === true，即 https 或 localhost）。与 HTMLMediaElement 自动同步：audio.play()/pause() 浏览器自动设置 playbackState，但 metadata 不会自动同步需手动设置。建议显式监听 play/pause 事件手动设置 playbackState 确保跨浏览器一致。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取入口信息', { type: 'primary', size: 'sm', disabled: !f.mediaSession, onClick: () => this._runEntryDemo() })),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.entryInfo || '（点击「读取入口信息」查看 navigator.mediaSession 全貌）')),
                h(Alert, {
                    type: 'warning',
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'MediaMetadata 构造函数：new MediaMetadata({ title, artist, album, artwork })。artwork 是 MediaImage 数组，每项含 src（必填，URL）/ sizes（如 "96x96" 或 "96x96 256x256"）/ type（MIME 类型）。OS 不同 UI 需要不同尺寸：通知栏小图（96）、锁屏大图（512+）、高 DPI（2x/3x），建议至少提供 96/128/192/256/384/512 六档，浏览器根据 sizes 自动选择。元数据显示在 Android 通知栏 / 锁屏、iOS 控制中心 / 锁屏、macOS Now Playing、Windows 媒体条。与 MediaTrack 元数据区别：MediaTrack 描述音轨（"英语配音"）用于切换，MediaMetadata 描述作品本身（标题/艺术家）用于 OS UI 显示，两者互补。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行元数据演示', { type: 'primary', size: 'sm', onClick: () => this._runMetadataDemo() })),
                h('div', { class: 'ms-metadata-card' }, h('div', { class: 'artwork' }, '♪'), h('div', { class: 'info' }, h('div', { class: 'title' }, 'Never Gonna Give You Up'), h('div', { class: 'artist' }, 'Rick Astley'), h('div', { class: 'album' }, 'Whenever You Need Somebody · 模拟锁屏 / 通知栏显示'))),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.metadataInfo || '（点击「运行元数据演示」设置 MediaMetadata 并查看说明）')),
                h(Alert, {
                    type: 'info',
    ...
    _readPlaybackStateInfo() {
        ...
        try {
            ...
            if (f.playbackState) {
                try {
                    liveState = navigator.mediaSession.playbackState;
                }
                catch (e) {
                    liveState = '读取失败：' + e.message;
                }
            }
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['playbackState', f.playbackState]]), h(Tag, { color: 'primary' }, 'OS UI 联动')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'playbackState 取值：none（默认，未播放）/ paused（暂停，OS UI 显示 ▶）/ playing（播放中，OS UI 显示 ⏸）。反映网页媒体当前状态，OS 媒体键 UI 据此联动。浏览器自动同步 <audio>/<video> 的 play/pause 事件到 playbackState，但建议显式监听并设置（确保跨浏览器一致 + 处理 ended 等边界）。setActionHandler 回调中应同时调用 audio.play()/pause() 并设置 playbackState。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取信息', { type: 'primary', size: 'sm', onClick: () => this.setState({ playbackStateInfo: this._readPlaybackStateInfo() }) }), this._btn('none', { size: 'sm', disabled: !f.playbackState, onClick: () => this._setPlaybackState('none') }), this._btn('paused', { size: 'sm', disabled: !f.playbackState, onClick: () => this._setPlaybackState('paused') }), this._btn('playing', { size: 'sm', disabled: !f.playbackState, onClick: () => this._setPlaybackState('playing') })),
                h('div', { class: 'ms-stage' }, h('div', { class: 'fs-sm text-secondary' }, '当前 playbackState 模拟 OS UI：'), h('div', { class: 'ms-state-pill ' + this._playbackStateMode, style: { marginTop: '6px' } }, h('span', { class: 'dot' }), this._playbackStateMode === 'playing' ? '播放中 ⏸ (OS 显示暂停键)' :
                    this._playbackStateMode === 'paused' ? '已暂停 ▶ (OS 显示播放键)' :
                        '未播放 (OS 不显示控件)')),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.playbackStateInfo || '（点击按钮切换 playbackState 并查看说明）')),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['setActionHandler', f.setActionHandler]]), h(Tag, { color: 'primary' }, '9 大动作')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'setActionHandler(action, callback) 注册 OS 媒体键动作处理。action 全集：play/pause/previoustrack/nexttrack/seekbackward/seekforward/seekto/stop/skipad。callback 接收 details 参数：seekbackward/seekforward 的 details.seekOffset（默认 10s），seekto 的 details.seekTime（绝对时间）+ details.fastSeek（快速 seek）。callback 传 null 清除该动作处理。页面卸载时建议清除所有 handler 避免通知栏占用残留。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取信息', { type: 'primary', size: 'sm', onClick: () => this._runActionHandlerDemo() })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '点击下方动作格注册对应 handler（真实浏览器可在 OS UI 触发）：'),
                h('div', { class: 'ms-action-grid' }, ...actions.map((a) => h('div', {
                    class: 'ms-action-cell ' + (this._activeAction === a ? 'active' : ''),
                    onClick: () => this._setActiveAction(a),
                }, a))),
                h('div', { class: 'ms-output' }, '当前选中：' + this._activeAction + '\n' +
                    'setActionHandler 可用：' + (f.setActionHandler ? '是' : '否（jsdom 不实现）') + '\n' +
                    '真实浏览器：点击 OS 媒体键 / 锁屏控件 / 通知栏按钮 → 触发对应 handler'),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.actionHandlerInfo || '（点击动作格或「读取信息」查看 setActionHandler 全集与代码示例）')),
                h(Alert, {
                    type: 'warning',
    ...
    _runMusicPlayerDemo() {
        ...
        if (!f.mediaSession) {
            ...
            try {
                ...
                navigator.mediaSession.playbackState = this._playerState;
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'HTMLAudioElement + MediaMetadata + setActionHandler 全套集成：loadTrack 函数同时设置 audio.src、播放、new MediaMetadata（锁屏 / 通知栏显示）。监听 play/pause 同步 playbackState，ended 自动下一曲。注册 play/pause/previoustrack/nexttrack/seekbackward/seekforward/seekto 全套 handler。setPositionState 让 OS UI 显示进度条（duration/position/playbackRate），用户拖动触发 seekto action。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行播放器演示', { type: 'primary', size: 'sm', onClick: () => this._runMusicPlayerDemo() })),
                h('div', { class: 'ms-player' }, h('div', { class: 'track' }, h('div', { class: 'cover' }, '♪'), h('div', { class: 'meta' }, h('div', { class: 'title' }, currentTrack.split(' · ')[0]), h('div', { class: 'artist' }, currentTrack.split(' · ')[1] + ' · 索引 ' + this._playerTrackIndex))), h('div', { class: 'controls' }, h('div', { class: 'ctrl-btn', onClick: () => this._playerPrev() }, '⏮'), h('div', { class: 'ctrl-btn main', onClick: () => this._playerTogglePlay() }, this._playerState === 'playing' ? '⏸' : '▶'), h('div', { class: 'ctrl-btn', onClick: () => this._playerNext() }, '⏭')), h('div', { style: { textAlign: 'center', fontSize: '11px', color: '#64748b', marginTop: '8px' } }, '点击控件模拟 OS 媒体键触发（真实浏览器在锁屏 / 通知栏触发）')),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.musicPlayerInfo || '（点击「运行播放器演示」查看完整音乐播放器集成代码）')),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard7() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'video 元素 + Media Session：与音频类似但元数据含"视频"语义（海报）。Chromecast 投屏：metadata 自动同步到接收端（Chromecast 设备），投屏控制条显示标题/封面，用户在手机锁屏可控制投屏。与 Picture-in-Picture 协同：PiP 浮窗显示播放控件，触发 Media Session action handler，用户切到其他应用后仍可控制视频。后台播放控制：页面切到后台时通过 visibilitychange 监听 + 设置 playbackState 让 OS 媒体控件继续可用。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行视频与投屏演示', { type: 'primary', size: 'sm', onClick: () => this._runVideoCastDemo() })),
                h('div', { class: 'ms-video-stage' }, h('div', { class: 'placeholder' }, '🎬 视频播放区域（演示）'), h('div', { class: 'pip-row' }, h('div', { class: 'ms-state-pill ' + (f.mediaSession ? 'playing' : 'none') }, h('span', { class: 'dot' }), 'Media Session ' + (f.mediaSession ? '可用' : '不可用')), h('div', { class: 'ms-state-pill ' + (pipAvailable ? 'playing' : 'none') }, h('span', { class: 'dot' }), 'PiP ' + (pipAvailable ? '可用' : '不可用')))),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.videoCastInfo || '（点击「运行视频与投屏演示」查看完整代码）')),
                h(Alert, {
                    type: 'info',
        ...
        const card = new Card({
            ...
            children: [
                ...
                h('div', { class: 'ms-output' }, '页面卸载清理示例：\n' +
                    ...
                    '});'),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.pitfallsInfo || '（点击按钮查看 7 大陷阱与 11 条最佳实践完整说明）')),
                h(Alert, {
                    type: 'warning',
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'api-lab-page media-session-page' }, h('h2', { class: 'section-title' }, 'Media Session API 媒体会话完整实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, '本页演示 W3C Media Session API 全套能力：OS 级媒体控制集成（锁屏 / 通知栏 / 硬件键）、navigator.mediaSession 入口与安全上下文、MediaMetadata 元数据（title/artist/album/artwork 多尺寸封面）、playbackState 状态联动、setActionHandler 9 大动作（play/pause/prev/next/seekbackward/seekforward/seekto/stop/skipad）、音乐播放器集成（audio + setPositionState 进度条）、视频与投屏（Chromecast + Picture-in-Picture + 后台播放）、陷阱与最佳实践（CORS / 清理 / iOS / 电量 / 无障碍）。所有特性通过 safe 能力检测，不可用时仅记日志，绝不抛异常。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderCard6(), this._renderCard7(), this._renderCard8(), this._renderLogPanel());
    }
}
//# sourceMappingURL=MediaSessionAPIPage.js.map