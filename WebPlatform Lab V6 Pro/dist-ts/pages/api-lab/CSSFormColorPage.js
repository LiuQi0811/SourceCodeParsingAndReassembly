...
export class CSSFormColorPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!caps.userValid)
            this._addLog('warn', ':user-invalid 不可用或 jsdom 未识别（Chrome 119+/Edge 119+），演示仅记日志');
        if (!caps.colorFn)
            this._addLog('warn', 'color(display-p3 ...) 不可用或 jsdom 未识别（Chrome 111+），演示仅记日志');
        if (!caps.oklch)
            this._addLog('warn', 'oklch() 不可用或 jsdom 未识别（Chrome 111+），演示仅记日志');
        if (!caps.lightDark)
            this._addLog('warn', 'light-dark() 不可用或 jsdom 未识别（Chrome 123+），演示仅记日志');
    ...
    componentWillUnmount() {
        ...
        if (Array.isArray(this._injectedStyles)) {
            this._injectedStyles.forEach((el) => { try {
                el && el.remove();
            }
            catch { /* noop */ } });
            this._injectedStyles = [];
    ...
    _checkUserInvalid() {
        ...
        const form = this.$('#fc-form');
        if (!form) {
            this._addLog('warn', '未找到演示表单');
            return;
        }
        let matched = 0;
        try {
    ...
    _simulateInteraction() {
        const req = this.$('#fc-required');
        const email = this.$('#fc-email');
        if (!req && !email) {
            this._addLog('warn', '未找到演示输入框');
            return;
        }
        try {
    ...
    _renderCard1() {
        ...
        }, h('p', { class: 'fs-sm text-secondary' }, ':user-valid / :user-invalid 区别于 :valid / :invalid（页面加载即匹配），仅在用户已交互（blur 或修改）后才匹配，' +
            '解决"页面一加载就一片红"痛点。演示：required input + email input，未交互时 :user-invalid 不匹配，blur 后才匹配。'), h('form', { id: 'fc-form', class: 'fc-form', onsubmit: (e) => e.preventDefault() }, h('div', {}, h('input', {
            id: 'fc-required', class: 'fc-field', type: 'text',
            placeholder: 'required 字段（未填）', required: true,
        }), h('div', { class: 'fc-field-hint' }, 'required：未交互时不红，blur 后变红')), h('div', {}, h('input', {
            id: 'fc-email', class: 'fc-field', type: 'email',
            placeholder: 'email 字段（输入非法邮箱）', value: 'not-an-email',
        }), h('div', { class: 'fc-field-hint' }, 'type=email：输入非法值，blur 后 :user-invalid 匹配'))), h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('查询 :user-invalid 匹配', { type: 'primary', size: 'sm', onClick: () => this._checkUserInvalid() }), this._btn('模拟用户 blur 交互', { type: 'default', size: 'sm', onClick: () => this._simulateInteraction() })), s.userValidInfo ? h('p', { class: 'fs-sm mt-sm text-secondary' }, s.userValidInfo) : null, h('pre', { class: 'code-block mt-md' }, `.field:user-invalid { border-color: red; }
...
/* 检测：CSS.supports('selector', ':user-invalid') */`));
    ...
    _renderCard2() {
        ...
        }, h('p', { class: 'fs-sm text-secondary' }, 'text-underline-offset: auto|<length> 控制下划线偏移；text-decoration-thickness: auto|from-font|<length> 控制下划线粗细；' +
            '配合 text-decoration-skip-ink: none|auto|all 控制是否避让下行字母（g/y/p）。像素级控制下划线，避开下行字母。'), h('div', { class: 'fc-link-list' }, h('a', { id: 'fc-link-offset-2', class: 'fc-link offset-2', href: '#', onclick: (e) => e.preventDefault() }, 'offset:2px（贴近基线）testing glyphs g y p'), h('a', { id: 'fc-link-offset-5', class: 'fc-link offset-5', href: '#', onclick: (e) => e.preventDefault() }, 'offset:5px（远离基线，避开下行字母）g y p'), h('a', { id: 'fc-link-thick-3', class: 'fc-link thick-3', href: '#', onclick: (e) => e.preventDefault() }, 'thickness:3px + offset:4px（粗下划线）g y p'), h('a', { id: 'fc-link-skip-none', class: 'fc-link skip-ink-none', href: '#', onclick: (e) => e.preventDefault() }, 'skip-ink:none（下划线穿过 g y p 连续）'), h('a', { id: 'fc-link-skip-all', class: 'fc-link skip-ink-all', href: '#', onclick: (e) => e.preventDefault() }, 'skip-ink:all（下划线在 g y p 处全部断开）')), h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('读取计算值', { type: 'primary', size: 'sm', disabled: !caps.css, onClick: () => this._readUnderlineInfo() }), h(Tag, { color: caps.decorationThickness ? 'success' : 'error' }, caps.decorationThickness ? 'thickness ✓' : 'thickness ✗'), h(Tag, { color: caps.skipInk ? 'success' : 'error' }, caps.skipInk ? 'skip-ink ✓' : 'skip-ink ✗')), s.underlineInfo ? h('pre', { class: 'code-block mt-md', style: { maxHeight: '240px', overflow: 'auto' } }, s.underlineInfo) : null, h('pre', { class: 'code-block mt-md' }, `a { text-decoration: underline;
    ...
    _setContainMode(mode) {
        const caps = this._caps();
        const box = this.$('#fc-contain-box');
        if (!box) {
            this._addLog('warn', '未找到 contain 演示容器');
            return;
        }
            ...
            const supported = (mode === 'strict') ? caps.contain
                : (mode === 'layout') ? caps.containLayout
                    : (mode === 'paint') ? caps.containPaint
                        : true;
            this._addLog(supported ? 'info' : 'warn', `contain → "${mode}"${supported ? '' : '（jsdom 可能未识别，仅说明）'}：隔离重排/重绘/尺寸范围，是 content-visibility 生效前提`);
            this.setState({
                containInfo: `contain: ${mode} —— ${{ strict: 'layout+paint+size（完全隔离，子元素尺寸不影响外部）',
                    layout: '隔离布局（子元素重排不外溢）',
                    paint: '隔离绘制（子元素绘制不超出边界，可裁剪）',
                    content: 'layout+paint（常用，安全）',
                    none: '关闭 containment' }[mode]}`,
            });
        }
        catch (err) {
    ...
    _renderCard3() {
        ...
        }, h('p', { class: 'fs-sm text-secondary' }, 'contain: layout|paint|size|style|strict(=layout paint size)|content(=layout paint)|inline-size；' +
            '隔离元素重排/重绘/尺寸/样式计算范围，是 content-visibility 生效前提。对长列表/虚拟滚动/卡片网格/广告 iframe 性能关键。'), h('div', { class: 'fc-contain-stage' }, h('div', { id: 'fc-contain-box', class: 'fc-contain-box strict' }, 'contain:strict 容器。子元素重排/重绘被隔离，不影响外部布局。' +
            '长列表/虚拟滚动/卡片网格可用 contain 限制回流范围，提升性能。' +
            '配合 content-visibility:auto 可跳过屏幕外内容渲染。')), h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('strict', { type: 'primary', size: 'sm', onClick: () => this._setContainMode('strict') }), this._btn('layout', { size: 'sm', onClick: () => this._setContainMode('layout') }), this._btn('paint', { size: 'sm', onClick: () => this._setContainMode('paint') }), this._btn('content', { size: 'sm', onClick: () => this._setContainMode('content') }), this._btn('none', { size: 'sm', onClick: () => this._setContainMode('none') })), s.containInfo ? h('p', { class: 'fs-sm mt-sm text-secondary' }, s.containInfo) : null, h('pre', { class: 'code-block mt-md' }, `.contain-box { contain: strict; }
...
/* 是 content-visibility:auto 生效前提 */`));
    ...
    _renderCard4() {
        ...
        }, h('p', { class: 'fs-sm text-secondary' }, 'color(display-p3 1 0 0) 显式声明颜色空间；oklch()/lab()/lch()/oklab() 感知均匀颜色空间；' +
            'Display P3/sRGB/rec2020 色域；color-mix(in oklch, ...)；渐变 color-interpolation-method。' +
            'P3 比 sRGB 更鲜艳，oklch 调色感知均匀。'), h('div', { class: 'fc-color-grid' }, h('div', { id: 'fc-swatch-p3-red', class: 'fc-swatch p3-red' }, 'color(display-p3 1 0 0)'), h('div', { id: 'fc-swatch-p3-green', class: 'fc-swatch p3-green' }, 'color(display-p3 0 1 0)'), h('div', { id: 'fc-swatch-oklch-blue', class: 'fc-swatch oklch-blue' }, 'oklch(50% .2 240)'), h('div', { id: 'fc-swatch-oklch-green', class: 'fc-swatch oklch-green' }, 'oklch(60% .15 145)'), h('div', { id: 'fc-swatch-lab-yellow', class: 'fc-swatch lab-yellow' }, 'lab(80% 40 80)')), h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('读取计算值', { type: 'primary', size: 'sm', disabled: !caps.css, onClick: () => this._readColorInfo() }), h(Tag, { color: caps.oklch ? 'success' : 'error' }, caps.oklch ? 'oklch ✓' : 'oklch ✗')), s.colorFnInfo ? h('pre', { class: 'code-block mt-md', style: { maxHeight: '280px', overflow: 'auto' } }, s.colorFnInfo) : null, h('pre', { class: 'code-block mt-md' }, `.swatch { background: color(display-p3 1 0 0); }   /* Display P3 广色域 */
...
linear-gradient(in oklch, red, blue)                /* 渐变插值色彩空间 */`));
    ...
    _renderCard5() {
        ...
        }, h('p', { class: 'fs-sm text-secondary' }, 'color-mix(in srgb, red, blue 40%) 按比例混合；相对颜色 from 语法 rgb(from red r g b / calc(a+0.1)) 基于基色派生；' +
            'light-dark(lightVal, darkVal) 根据 color-scheme 自动切换。展示调色板生成。'), h('div', { class: 'fs-sm text-secondary mt-sm' }, 'color-mix 红蓝混合比例：'), h('div', { class: 'fc-mix-grid' }, h('div', { id: 'fc-mix-25', class: 'fc-mix-swatch mix-25' }, '25% blue'), h('div', { id: 'fc-mix-50', class: 'fc-mix-swatch mix-50' }, '50% blue'), h('div', { id: 'fc-mix-75', class: 'fc-mix-swatch mix-75' }, '75% blue'), h('div', { id: 'fc-mix-oklch', class: 'fc-mix-swatch mix-oklch' }, 'in oklch')), h('div', { class: 'fs-sm text-secondary mt-sm' }, '相对颜色 rgb(from ...) 派生：'), h('div', { class: 'fc-mix-grid' }, h('div', { id: 'fc-mix-rel-light', class: 'fc-mix-swatch rel-light' }, 'calc(a+0.1)'), h('div', { id: 'fc-mix-rel-dark', class: 'fc-mix-swatch rel-dark' }, 'calc(r-50)')), h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('读取计算值', { type: 'primary', size: 'sm', disabled: !caps.css, onClick: () => this._readMixInfo() }), h(Tag, { color: caps.relativeColor ? 'success' : 'error' }, caps.relativeColor ? 'rgb(from) ✓' : 'rgb(from) ✗'), h(Tag, { color: caps.lightDark ? 'success' : 'error' }, caps.lightDark ? 'light-dark ✓' : 'light-dark ✗')), s.colorMixInfo ? h('pre', { class: 'code-block mt-md', style: { maxHeight: '300px', overflow: 'auto' } }, s.colorMixInfo) : null, h('pre', { class: 'code-block mt-md' }, `.mix  { background: color-mix(in srgb, red, blue 40%); }
...
/* 相对颜色用 from 关键字 + calc 改通道 */`));
    ...
    _toggleDarkSim() {
        const card = this.$('#fc-dark-card');
        if (!card) {
            this._addLog('warn', '未找到深色卡片元素');
            return;
        }
        try {
            this._darkSimOn = !this._darkSimOn;
            if (this._darkSimOn)
                card.classList.add('dark-sim');
            else
                card.classList.remove('dark-sim');
    ...
    _renderCard6() {
        ...
        }, h('p', { class: 'fs-sm text-secondary' }, '@media (prefers-color-scheme: dark) + color-scheme: light dark 声明 + light-dark() 函数；' +
            'matchMedia JS 检测系统偏好；vs 手动 .dark 类切换。展示一个随系统深浅色切换的卡片。'), h('div', {
            id: 'fc-dark-card', class: 'fc-dark-card',
        }, h('p', { class: 'fs-sm', style: { margin: '0 0 6px 0' } }, '本卡片背景/文字/控件使用 light-dark() 自动随 color-scheme 切换：浅色白底深字，深色深底浅字。'), h('div', { class: 'fc-scheme-row' }, h('div', { class: 'fc-scheme-swatch' }), h('div', { class: 'fc-scheme-text' }, 'light-dark(#f0f9ff, #0f172a)'))), h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('检测系统偏好', { type: 'primary', size: 'sm', onClick: () => this._readSchemeInfo() }), this._btn('手动模拟深/浅', { type: 'default', size: 'sm', onClick: () => this._toggleDarkSim() }), h(Tag, { color: caps.lightDark ? 'success' : 'error' }, caps.lightDark ? 'light-dark ✓' : 'light-dark ✗')), s.schemeInfo ? h('pre', { class: 'code-block mt-md', style: { maxHeight: '300px', overflow: 'auto' } }, s.schemeInfo) : null, h('pre', { class: 'code-block mt-md' }, `:root { color-scheme: light dark; }
...
//# sourceMappingURL=CSSFormColorPage.js.map