...
export class CSSBoxModelPositioningPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!f.positionSticky)
            this._addLog('warn', 'position: sticky 不可用（IE 11 不支持，现代浏览器全支持）');
        if (!f.flowRoot)
            this._addLog('warn', 'display: flow-root 不可用（推荐清浮动方案，Chrome/Firefox/Edge/Safari 13+ 支持）');
        if (!f.shapeOutside)
            this._addLog('warn', 'shape-outside 不可用（自定义文字环绕形状，现代浏览器支持）');
        if (!f.alignContentCenter)
            this._addLog('warn', '块级容器 align-content 不可用（仅 Chrome 123+ 实现，Safari/Firefox 未实现）');
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['box-sizing', f.boxSizing]]), h(Tag, { color: 'primary' }, '盒模型基础')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'CSS 盒模型四层：content → padding → border → margin。content-box（默认）width/height 仅含 content，padding/border 额外加；border-box width/height 含 content+padding+border，仅 margin 在外。全局重置推荐 *, *::before, *::after { box-sizing: border-box; } 让尺寸计算直观。margin 不计入元素尺寸（仅视觉外推），box-sizing 全部浏览器支持。'),
                h('div', { class: 'bm-box-sizing-compare' }, h('div', { class: 'bm-box bm-box-content' }, 'content-box\nwidth=200 + 16+16 + 4+4 = 240px'), h('div', { class: 'bm-box bm-box-border' }, 'border-box\nwidth=200px（含 padding+border）')),
                h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('运行 box-sizing 演示', { type: 'primary', size: 'sm', onClick: () => this._runBoxSizingDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.boxSizingInfo || '（点击按钮查看 box-sizing 与盒模型完整用法）')),
            ],
        });
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'position 五种取值：static（默认，正常流，top/z-index 无效）/ relative（相对原位置偏移，不脱离流）/ absolute（脱离流，相对最近非 static 祖先）/ fixed（脱离流，相对视口，transform/filter 会改变 containing block）/ sticky（粘性，正常流+滚动阈值变 fixed）。inset 是 top/right/bottom/left 的简写。z-index 仅 position != static 生效。各 position 的 containing block 计算规则不同。'),
                h('div', { class: 'bm-position-stage' }, h('div', { class: 'bm-pos bm-pos-relative' }, 'relative (top:10 left:10)'), h('div', { class: 'bm-pos bm-pos-absolute' }, 'absolute (top:8 right:8)'), h('div', { class: 'bm-pos bm-pos-sticky' }, 'sticky (top:0)'), h('div', { class: 'bm-pos bm-pos-fixed' }, 'fixed (bottom:12 right:12)')),
                h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('运行 position 演示', { type: 'primary', size: 'sm', onClick: () => this._runPositionDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.positionInfo || '（点击按钮查看 position 定位系统完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['position:sticky', f.positionSticky]]), h(Tag, { color: 'primary' }, '粘性定位')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'position: sticky 在正常文档流中，直到滚动到 top/bottom/left/right 阈值前表现为 relative，到达阈值后变 fixed（但仅在其父容器范围内）。必须设置 top/bottom/left/right 至少一个才生效。父容器 overflow: hidden/auto/scroll 会限制 sticky 范围；父容器滚到底部时 sticky 解锁。与 fixed 区别：fixed 脱离文档流，sticky 保留原位置。支持多级 sticky（嵌套表头/侧边栏）。IE 11 不支持。'),
                h('div', { class: 'bm-sticky-scroll' }, h('div', { class: 'bm-sticky-header' }, '粘性表头 (top:0)'), h('div', { class: 'fs-sm' }, '滚动此容器查看表头吸顶效果（jsdom 不渲染真实滚动，浏览器可见）'), h('div', { class: 'fs-sm', style: { marginTop: '8px' } }, '内容行 1'), h('div', { class: 'fs-sm' }, '内容行 2'), h('div', { class: 'fs-sm' }, '内容行 3'), h('div', { class: 'fs-sm' }, '内容行 4'), h('div', { class: 'fs-sm' }, '内容行 5'), h('div', { class: 'fs-sm' }, '内容行 6'), h('div', { class: 'fs-sm' }, '内容行 7'), h('div', { class: 'fs-sm' }, '内容行 8')),
                h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('运行 sticky 深潜演示', { type: 'primary', size: 'sm', onClick: () => this._runStickyDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.stickyInfo || '（点击按钮查看 position: sticky 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['display:flow-root', f.flowRoot]]), h(Tag, { color: 'primary' }, 'margin 合并')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '块级元素垂直方向 margin 会合并（取较大者），仅垂直方向合并，水平方向不合并。三种场景：相邻块级元素、父元素与第一/最后一个子元素、空块的上下 margin。不合并情况：浮动元素/绝对定位/flex/grid 项目/inline-block/父元素有 padding/border/父元素触发 BFC。解决方案：用 padding 代替 margin / 触发 BFC（display: flow-root）/ 用 flex/grid + gap。writing-mode 影响合并方向。'),
                h('div', { class: 'bm-margin-stage' }, h('div', { class: 'bm-margin-box' }, h('div', {}, '相邻块合并：margin-bottom 20 + margin-top 30 = 30px（取大者）')), h('div', { class: 'bm-margin-box bm-bfc' }, h('div', {}, '父元素 overflow:hidden 阻断：margin 不穿透父元素'))),
                h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('运行 margin collapse 演示', { type: 'primary', size: 'sm', onClick: () => this._runMarginCollapseDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.marginCollapseInfo || '（点击按钮查看 margin collapse 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'BFC 是独立的渲染区域，内部元素不影响外部。触发条件：float != none / position: absolute/fixed / display: flow-root（推荐）/ inline-block / flex / grid / overflow != visible / contain: layout / column-count。作用：清除浮动（父元素 overflow:hidden 或 display:flow-root）/ 阻止 margin 合并 / 阻止元素被浮动覆盖。display: flow-root 专为 BFC 设计，不创建滚动容器，优于 overflow: hidden。'),
                h('div', { class: 'bm-bfc-container bm-no-bfc' }, h('div', { class: 'bm-bfc-float' }, 'float'), h('div', { class: 'bm-bfc-text' }, '无 BFC：文字环绕浮动元素（被覆盖）')),
                h('div', { class: 'bm-bfc-container bm-flow-root' }, h('div', { class: 'bm-bfc-float' }, 'float'), h('div', { class: 'bm-bfc-text' }, 'flow-root BFC：不被浮动覆盖，紧贴右侧')),
                h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('运行 BFC 演示', { type: 'primary', size: 'sm', onClick: () => this._runBfcDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.bfcInfo || '（点击按钮查看 BFC 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'float: left/right/none/inline-start/inline-end 让元素脱离文档流但仍影响文本流（文字环绕）。清除浮动方法：clear 属性 / clearfix hack（::after { content:""; display:table; clear:both; }）/ 父元素触发 BFC（overflow:hidden 或 display:flow-root，推荐 flow-root）。shape-outside 配合 float 实现非矩形环绕（圆形/多边形/图片 alpha）。现代 CSS 中 float 主要用于文字环绕，布局已被 flex/grid 替代。'),
                h('div', { class: 'bm-float-stage bm-clearfix' }, h('div', { class: 'bm-float-img' }, 'float\n+ shape-outside\ncircle()'), h('div', { class: 'bm-bfc-text' }, '浮动图片 + shape-outside: circle() 让文字以圆形环绕。这是 float 的现代用法，传统多栏布局已被 flex/grid 替代。父元素加 display: flow-root 或 clearfix::after 可清除浮动避免高度塌陷。')),
                h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('运行 float 演示', { type: 'primary', size: 'sm', onClick: () => this._runFloatDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.floatInfo || '（点击按钮查看 float 与清除浮动完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard7() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'z-index: auto(默认，等同 0 但不创建上下文) | integer。堆叠上下文形成条件：position!=static 且 z-index!=auto / position:fixed/sticky / opacity<1 / transform/filter/will-change!=auto / mix-blend-mode!=normal / isolation:isolate / flex/grid 子项 z-index!=auto。堆叠顺序（同上下文）：背景 → 负 z-index → block → float → inline → 0/auto → 正 z-index。子元素 z-index 仅在父堆叠上下文内有效，常见陷阱：模态框被父级 opacity:0.99 限制。'),
                h('div', { class: 'bm-z-index-stage' }, h('div', { class: 'bm-z-layer bm-z-bg' }, '背景 z-index:-1'), h('div', { class: 'bm-z-layer bm-z-block' }, 'block z:1'), h('div', { class: 'bm-z-layer bm-z-float' }, 'float z:2'), h('div', { class: 'bm-z-layer bm-z-inline' }, 'inline z:3'), h('div', { class: 'bm-z-layer bm-z-top' }, 'top z:10')),
                h('div', { class: 'bm-trap-parent' }, h('div', { style: { fontSize: '12px', color: '#92400e' } }, '陷阱：父 opacity:0.99 创建堆叠上下文'), h('div', { class: 'bm-trap-modal' }, 'modal z:9999（被父限制）')),
                h('div', { class: 'bm-trap-sibling', style: { fontSize: '12px' } }, 'sibling z:2（外层覆盖 modal）'),
                h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('运行 z-index 演示', { type: 'primary', size: 'sm', onClick: () => this._runZIndexDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.zIndexInfo || '（点击按钮查看 z-index 与堆叠上下文完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _runBlockAlignContentDemo() {
        ...
        if (!supported)
            this._addLog('warn', '块级 align-content 当前环境 CSS.supports 未识别（可能 Safari/Firefox 或 jsdom）');
    }
    _renderCard9() {
        const s = this.state;
        const f = this._flags();
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'CSS Box Alignment Module Level 3 将 align-content 从 Flex/Grid 扩展到普通块级容器（block container），Chrome 123+ 率先实现。取值：normal/start/end/center/space-between/space-around/space-evenly/stretch/baseline。适用条件：容器必须是块级容器（非 flex/grid）+ 有剩余高度 + 仅对块级子元素生效 + 与 writing-mode 协同。与 Flex（分布 flex line）/Grid（分布 grid track）语法相同语义不同。实战：垂直居中块级内容（替代 display:flex + justify-content）/ Footer 置底（min-height:100vh + align-content:space-between）。陷阱：仅 Chrome 123+ 支持，Safari/Firefox 未实现，需 @supports 渐进增强降级到 flex/grid。'),
                h('div', { class: 'bm-block-align-stage' }, h('div', { class: 'bm-block-align-col' }, h('h5', {}, 'align-content: center'), h('div', { class: 'bm-block-align-host is-center' }, h('div', { class: 'bm-block-align-item' }, '块 1'), h('div', { class: 'bm-block-align-item alt' }, '块 2'), h('div', { class: 'bm-block-align-item' }, '块 3'))), h('div', { class: 'bm-block-align-col' }, h('h5', {}, 'align-content: space-between'), h('div', { class: 'bm-block-align-host is-space-between' }, h('div', { class: 'bm-block-align-item' }, '块 1'), h('div', { class: 'bm-block-align-item alt' }, '块 2'), h('div', { class: 'bm-block-align-item' }, '块 3'))), h('div', { class: 'bm-block-align-col' }, h('h5', {}, 'align-content: space-evenly'), h('div', { class: 'bm-block-align-host is-space-evenly' }, h('div', { class: 'bm-block-align-item' }, '块 1'), h('div', { class: 'bm-block-align-item alt' }, '块 2'), h('div', { class: 'bm-block-align-item' }, '块 3'))), h('div', { class: 'bm-block-align-col' }, h('h5', {}, 'align-content: stretch'), h('div', { class: 'bm-block-align-host is-stretch' }, h('div', { class: 'bm-block-align-item' }, '块 1'), h('div', { class: 'bm-block-align-item alt' }, '块 2'), h('div', { class: 'bm-block-align-item' }, '块 3')))),
                h('div', { class: 'fs-sm text-secondary', style: { marginTop: '8px' } }, '注：jsdom 不做真实布局，浏览器中可见 4 种 align-content 取值在块级容器内的分布差异（仅 Chrome 123+ 渲染生效）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('运行块级 align-content 演示', { type: 'primary', size: 'sm', onClick: () => this._runBlockAlignContentDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.blockAlignContentInfo || '（点击按钮查看块级上下文 align-content 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
...
//# sourceMappingURL=CSSBoxModelPositioningPage.js.map