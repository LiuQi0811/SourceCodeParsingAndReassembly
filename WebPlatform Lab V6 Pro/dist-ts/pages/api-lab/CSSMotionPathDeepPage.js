...
export class CSSMotionPathDeepPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!f.offsetPathPath)
            this._addLog('warn', 'offset-path: path() 不可用或 jsdom 未识别（CSS Motion Path L1，Chrome 116+/Firefox 72+/Safari 16+）');
        if (!f.offsetPathRay)
            this._addLog('warn', 'offset-path: ray() 不可用或 jsdom 未识别（CSS Motion Path L1，Chrome 116+/Firefox 113+/Safari 16+）');
        if (!f.offsetPathShape)
            this._addLog('warn', 'offset-path: <basic-shape> 不可用或 jsdom 未识别（如 circle()/ellipse()，Chrome 116+/Safari 16+）');
        if (!f.offsetPathCoordBox)
            this._addLog('warn', 'offset-path: <coord-box> 不可用或 jsdom 未识别（如 content-box/fill-box，Chrome 116+）');
        if (!f.offsetPosition)
            this._addLog('warn', 'offset-position 不可用或 jsdom 未识别（CSS Motion Path L1 较新属性，Chrome 116+/Safari 16+）');
        if (!f.offsetAnchor)
            this._addLog('warn', 'offset-anchor 不可用或 jsdom 未识别（CSS Motion Path L1 较新属性，Chrome 116+/Safari 16+）');
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            children: [
                ...
                h('div', { class: 'mp-path-stage' }, h('svg', { viewBox: '0 0 400 140', preserveAspectRatio: 'none', xmlns: 'http://www.w3.org/2000/svg' }, h('path', { d: 'M20,80 C60,20 140,20 180,80', stroke: '#3b82f6', 'stroke-width': '2', fill: 'none', 'stroke-dasharray': '4,4' })), h('div', { class: 'mp-mover' })),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.overviewInfo || '（点击「读取概述信息」查看 Motion Path 完整说明）')),
                h(Alert, {
                    type: 'info',
    ...
    _readOffsetPathInfo() {
        const f = this._flags();
        try {
            const mover = this.el && this.el?.querySelector('.mp-path-stage .mp-mover');
            let computed = '(未渲染)';
            if (mover) {
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'offset-path 定义运动路径，支持四种形式：path()（SVG path data 字符串，最常用）、ray()（从一点向某角度发射的射线）、<basic-shape>（circle/ellipse/inset/polygon，与 clip-path 共用语法）、url()（引用 SVG <path> 元素的 d 属性）。可附加 <coord-box> 指定参考盒：content-box（默认）/padding-box/border-box/margin-box/fill-box/stroke-box/view-box，影响 ray()/basic-shape 的原点与尺寸。当前规范只支持单条路径；多路径需 JS 拆分或多个元素叠加。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取 offset-path 信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runOffsetPathDemo() })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, 'path() 演示（蓝色小球沿 SVG path 运动）：'),
                h('div', { class: 'mp-path-stage' }, h('svg', { viewBox: '0 0 400 140', preserveAspectRatio: 'none', xmlns: 'http://www.w3.org/2000/svg' }, h('path', { d: 'M20,80 C60,20 140,20 180,80', stroke: '#3b82f6', 'stroke-width': '2', fill: 'none', 'stroke-dasharray': '4,4' })), h('div', { class: 'mp-mover' })),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.offsetPathInfo || '（点击按钮查看 offset-path 四种路径形式完整说明）')),
                h(Alert, {
                    type: 'info',
    ...
    _setPathData(data, desc) {
        this._pathData = data;
        // 更新 .mp-mover 的 offset-path（同时更新 SVG 可视路径）
        this._injectStyle('css-motion-path-data-dynamic', `.mp-path-stage .mp-mover { offset-path: path('${data}'); }`);
        if (this.el) {
            const svgPath = this.el?.querySelector('.mp-path-stage svg path');
            if (svgPath)
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['path()', f.offsetPathPath]]), h(Tag, { color: 'primary' }, 'SVG path data')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'path() 接受 SVG path data 字符串，与 SVG <path> 元素 d 属性共用语法。命令包括 M（Move To）/L（Line To）/H/V（水平/垂直线）/C（三次贝塞尔）/Q（二次贝塞尔）/S/T（平滑贝塞尔）/A（椭圆弧）/Z（闭合）。大写为绝对坐标，小写为相对坐标。offset-distance: 50% 按实际几何长度的 50% 计算（与 pathLength 无关）。path() 字符串是字面量，不支持 CSS 变量插值（CSS Values L5 提案仍草案），替代方案用 JS 动态生成 inline style 或 url() 引用 SVG <path>。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ pathFuncInfo: this._readPathFuncInfo() }) }), this._btn('贝塞尔 C', { size: 'sm', disabled: !f.offsetPathPath, onClick: () => this._setPathData('M20,80 C60,20 140,20 180,80', '三次贝塞尔') }), this._btn('直线 L', { size: 'sm', disabled: !f.offsetPathPath, onClick: () => this._setPathData('M20,100 L380,40', '直线') }), this._btn('三角 Z', { size: 'sm', disabled: !f.offsetPathPath, onClick: () => this._setPathData('M200,20 L380,120 L20,120 Z', '闭合三角') }), this._btn('弧 A', { size: 'sm', disabled: !f.offsetPathPath, onClick: () => this._setPathData('M20,80 A180,60 0 1 1 380,80', '椭圆弧') })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, 'path() 演示（切换按钮观察不同 SVG path 命令）：'),
                h('div', { class: 'mp-path-stage' }, h('svg', { viewBox: '0 0 400 140', preserveAspectRatio: 'none', xmlns: 'http://www.w3.org/2000/svg' }, h('path', { d: 'M20,80 C60,20 140,20 180,80', stroke: '#3b82f6', 'stroke-width': '2', fill: 'none', 'stroke-dasharray': '4,4' })), h('div', { class: 'mp-mover' })),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.pathFuncInfo || '（点击按钮切换 path data 并查看 SVG path 命令说明）')),
                h(Alert, {
                    type: 'warning',
    ...
    _readRayFuncInfo() {
        const f = this._flags();
        try {
            const arrow = this.el && this.el?.querySelector('.mp-ray-arrow');
            let computed = '(未渲染)';
            if (arrow) {
    ...
    _setRay(angle, size) {
        ...
        this._raySize = size;
        this._injectStyle('css-motion-path-ray-dynamic', `.mp-ray-arrow { offset-path: ray(${angle}deg ${size}); offset-rotate: auto; offset-distance: 50%; }`);
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['ray()', f.offsetPathRay]]), h(Tag, { color: 'primary' }, '极坐标 / 雷达扫描')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'ray() 是极坐标路径函数：ray(<angle> <ray-size>? <ray-position>?)。<angle> 为射线方向（0deg 向上，90deg 向右，顺时针）；<ray-size> 为射线长度（closest-side/farthest-side/closest-corner/farthest-corner 默认/sides）；<ray-position> 为起点（<position> 语法，默认 center）。offset-distance: 50% 沿射线从起点到终点的 50% 处，offset-rotate: auto 让元素朝向射线方向。适合"从中心向外辐射""雷达扫描""时钟指针"等场景。可附加 <coord-box> 指定参考盒。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ rayFuncInfo: this._readRayFuncInfo() }) }), this._btn('0° 上', { size: 'sm', disabled: !f.offsetPathRay, onClick: () => this._setRay(0, 'closest-side') }), this._btn('45° 右上', { size: 'sm', disabled: !f.offsetPathRay, onClick: () => this._setRay(45, 'closest-side') }), this._btn('90° 右', { size: 'sm', disabled: !f.offsetPathRay, onClick: () => this._setRay(90, 'closest-side') }), this._btn('135° 右下', { size: 'sm', disabled: !f.offsetPathRay, onClick: () => this._setRay(135, 'farthest-corner') }), this._btn('180° 下', { size: 'sm', disabled: !f.offsetPathRay, onClick: () => this._setRay(180, 'closest-side') })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, 'ray() 演示（绿色小球沿射线方向运动，当前 ' + this._rayAngle + 'deg ' + this._raySize + '）：'),
                h('div', { class: 'mp-ray-stage' }, h('div', { class: 'mp-ray-center' }), h('div', { class: 'mp-ray-arrow' })),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.rayFuncInfo || '（点击按钮切换 ray 角度/size 查看说明）')),
                h(Alert, {
                    type: 'info',
    ...
    _readDistanceInfo() {
        const f = this._flags();
        try {
            const mover = this.el && this.el?.querySelector('.mp-distance-mover');
            let computed = '(未渲染)';
            if (mover) {
    ...
    _setDistance(val) {
        this._distance = val;
        this._injectStyle('css-motion-path-distance-dynamic', `.mp-distance-mover { offset-distance: ${val}; }`);
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['offset-distance', f.offsetDistance]]), h(Tag, { color: 'primary' }, '<length-percentage>')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'offset-distance 控制元素在 offset-path 路径上的位置：<percentage>（0%~100%，相对路径总长度）或 <length>（px/em 等绝对长度）。0% 为路径起点（path 第一个 M 命令位置），100% 为路径终点（最后一个命令终点或 Z 闭合回起点），50% 为路径几何中点。动画 offset-distance 从 0% 到 100% 即"沿路径运动"；反向运动用 100% → 0% 或 animation-direction: reverse；来回运动用 alternate。px 值适合精确控制位置，路径总长度可用 SVG <path>.getTotalLength() 获取。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ distanceInfo: this._readDistanceInfo() }) }), this._btn('0%', { size: 'sm', disabled: !f.offsetDistance, onClick: () => this._setDistance('0%') }), this._btn('25%', { size: 'sm', disabled: !f.offsetDistance, onClick: () => this._setDistance('25%') }), this._btn('50%', { type: 'primary', size: 'sm', disabled: !f.offsetDistance, onClick: () => this._setDistance('50%') }), this._btn('75%', { size: 'sm', disabled: !f.offsetDistance, onClick: () => this._setDistance('75%') }), this._btn('100%', { size: 'sm', disabled: !f.offsetDistance, onClick: () => this._setDistance('100%') }), this._btn('100px', { size: 'sm', disabled: !f.offsetDistance, onClick: () => this._setDistance('100px') })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, 'offset-distance 演示（黄色方块沿直线运动，当前 ' + this._distance + '）：'),
                h('div', { class: 'mp-distance-stage' }, h('div', { class: 'mp-distance-mover' })),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.distanceInfo || '（点击按钮切换 offset-distance 查看说明）')),
                h(Alert, {
                    type: 'info',
    ...
    _readRotateInfo() {
        const f = this._flags();
        try {
            const mover = this.el && this.el?.querySelector('.mp-rotate-mover');
            let computed = '(未渲染)';
            if (mover) {
    ...
    _setRotate(mode) {
        ...
        this._injectStyle('css-motion-path-rotate-dynamic', `.mp-rotate-mover { offset-rotate: ${mode}; }`);
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'offset-rotate 控制元素沿路径运动时的旋转：auto（默认，跟随路径切线方向，元素朝向运动方向）/ reverse（反向跟随切线，倒车效果）/ <angle>（固定角度，0deg 不旋转）/ auto <angle>（切线 + 额外角度，如 auto 90deg 侧身移动）。offset-position 指定路径的初始位置偏移：<position> 语法（与 background-position 共用），对 ray() 作为 <ray-position> 默认值，对 path() 偏移坐标原点。与 offset-anchor 区别：offset-position 是路径位置，offset-anchor 是元素锚点。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ rotateInfo: this._readRotateInfo() }) }), this._btn('auto', { type: 'primary', size: 'sm', disabled: !f.offsetRotateAuto, onClick: () => this._setRotate('auto') }), this._btn('reverse', { size: 'sm', disabled: !f.offsetRotateAuto, onClick: () => this._setRotate('reverse') }), this._btn('0deg', { size: 'sm', disabled: !f.offsetRotateAngle, onClick: () => this._setRotate('0deg') }), this._btn('auto 90deg', { size: 'sm', disabled: !f.offsetRotateAuto, onClick: () => this._setRotate('auto 90deg') }), this._btn('45deg', { size: 'sm', disabled: !f.offsetRotateAngle, onClick: () => this._setRotate('45deg') })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, 'offset-rotate 演示（紫色长条沿曲线运动，当前 ' + this._rotateMode + '）：'),
                h('div', { class: 'mp-rotate-stage' }, h('svg', { viewBox: '0 0 400 120', preserveAspectRatio: 'none', xmlns: 'http://www.w3.org/2000/svg', style: { position: 'absolute', inset: '0', width: '100%', height: '100%' } }, h('path', { d: 'M20,100 C80,20 200,20 360,100', stroke: '#8b5cf6', 'stroke-width': '2', fill: 'none', 'stroke-dasharray': '4,4' })), h('div', { class: 'mp-rotate-mover' })),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.rotateInfo || '（点击按钮切换 offset-rotate 查看说明）')),
                h(Alert, {
                    type: 'info',
    ...
    _readAnchorInfo() {
        const f = this._flags();
        try {
            const mover = this.el && this.el?.querySelector('.mp-anchor-mover');
            let computed = '(未渲染)';
            if (mover) {
    ...
    _setAnchor(mode) {
        ...
        this._injectStyle('css-motion-path-anchor-dynamic', `.mp-anchor-mover { offset-anchor: ${val}; }`);
    ...
    _renderCard7() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['offset-anchor', f.offsetAnchor]]), h(Tag, { color: 'primary' }, '<position>')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'offset-anchor 指定元素的"哪个点"对齐到路径上：<position> 语法（与 background-position 共用）。center（默认，元素中心对齐路径）/ 0% 0%（左上角）/ 100% 100%（右下角）/ 50% 0%（顶部中点）。与 transform-origin 区别：transform-origin 影响 transform 变换中心，offset-anchor 影响 offset-path 运动时元素对齐路径的点；但 offset-rotate: auto 旋转时，旋转中心由 offset-anchor 决定。默认值 auto：若 offset-position 非 auto 则等于 offset-position，否则为 center。适合"钟摆""旗帜飘动""吊灯摇晃"等场景。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ anchorInfo: this._readAnchorInfo() }) }), this._btn('auto', { type: 'primary', size: 'sm', disabled: !f.offsetAnchor, onClick: () => this._setAnchor('auto') }), this._btn('center', { size: 'sm', disabled: !f.offsetAnchor, onClick: () => this._setAnchor('center') }), this._btn('0% 0%', { size: 'sm', disabled: !f.offsetAnchor, onClick: () => this._setAnchor('0% 0%') }), this._btn('100% 100%', { size: 'sm', disabled: !f.offsetAnchor, onClick: () => this._setAnchor('100% 100%') }), this._btn('50% 0%', { size: 'sm', disabled: !f.offsetAnchor, onClick: () => this._setAnchor('50% 0%') })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, 'offset-anchor 演示（绿色方块的红点为锚点，当前 ' + this._anchorMode + '）：'),
                h('div', { class: 'mp-anchor-stage' }, h('svg', { viewBox: '0 0 400 120', preserveAspectRatio: 'none', xmlns: 'http://www.w3.org/2000/svg', style: { position: 'absolute', inset: '0', width: '100%', height: '100%' } }, h('path', { d: 'M20,80 C80,20 200,20 360,80', stroke: '#10b981', 'stroke-width': '2', fill: 'none', 'stroke-dasharray': '4,4' })), h('div', { class: 'mp-anchor-mover' })),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.anchorInfo || '（点击按钮切换 offset-anchor 查看说明）')),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard8() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '五大实战场景：沿 SVG 路径运动动画（path() + offset-distance 动画）、引导线动画（箭头沿折线指引视线）、徽章飘动（促销徽章沿曲线飘动，offset-rotate: 0deg 保持水平）、与 CSS @keyframes 配合（offset-* 可在 keyframes 动画）、offset 简写（同时设置 path/distance/rotate）。五大陷阱：浏览器降级（老 Chrome 用 motion-path，需 -webkit- 前缀）、DevTools 调试（不直接可视化路径，需 SVG 叠加）、性能（GPU 加速 transform，但合成层爆炸风险）、与 Web Animations API 协同（offsetDistance 驼峰）、与 transform: translate() 组合（offset-* 先应用，再叠加 transform）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行实战与陷阱演示', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runPatternsDemo() })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '徽章飘动演示（橙色徽章沿复杂路径来回飘动，offset-rotate: auto）：'),
                h('div', { class: 'mp-badge-stage' }, h('svg', { viewBox: '0 0 480 160', preserveAspectRatio: 'none', xmlns: 'http://www.w3.org/2000/svg', style: { position: 'absolute', inset: '0', width: '100%', height: '100%' } }, h('path', { d: 'M20,40 C100,120 200,10 360,80 C400,110 420,40 460,60', stroke: 'rgba(255,255,255,0.2)', 'stroke-width': '1', fill: 'none', 'stroke-dasharray': '3,3' })), h('div', { class: 'mp-badge' }, 'PROMO')),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.patternsInfo || '（点击按钮查看 5 大实战场景与 5 大陷阱完整说明）')),
                h(Alert, {
                    type: 'warning',
    ...
    _renderLogPanel() {
        const s = this.state;
        return h('div', { class: 'mp-log-panel' }, h('div', { class: 'flex items-center gap-sm mb-sm' }, h('span', { class: 'fs-sm text-secondary' }, '事件日志'), h(Tag, { color: 'primary' }, `${s.logs.length} 条`)), s.logs.length === 0
            ? h('div', { class: 'fs-sm text-secondary' }, '（暂无日志）')
            : h('div', { class: 'mp-log-list' }, ...s.logs.slice().reverse().map((log) => h('div', { class: `mp-log-item mp-log-${log.type}` }, h('span', { class: 'mp-log-time' }, log.time), h('span', {}, log.content)))));
    }
    // =================== 整页渲染 ===================
    render() {
        const s = this.state;
        return h('div', { class: 'api-lab-page css-motion-path-page' }, h('h2', { class: 'section-title' }, 'CSS Motion Path Module Level 1 实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, '本页演示 CSS Motion Path Module Level 1 路径运动动画：offset-path 路径定义（path()/ray()/basic-shape/coord-box）、path() 函数与 SVG 路径（M/L/C/Q/A/Z 命令）、ray() 射线函数（极坐标）、offset-distance 偏移距离（0%~100%）、offset-rotate 与 offset-position（auto/reverse/<angle>）、offset-anchor 锚点、实战与陷阱（沿路径运动/引导线/徽章飘动/简写/降级）。所有特性通过 typeof / CSS.supports 能力检测，不可用时仅记日志，绝不抛异常。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderCard6(), this._renderCard7(), this._renderCard8(), this._renderLogPanel());
    }
}
//# sourceMappingURL=CSSMotionPathDeepPage.js.map