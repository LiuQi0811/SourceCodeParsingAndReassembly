...
export class DOMPartsAPIPage extends Page {
    ...
    componentDidMount() {
        ...
        if (this._dynamicStyles === undefined)
            this._dynamicStyles = [];
        if (this._instances === undefined)
            this._instances = [];
        if (this._listContainer === undefined)
            this._listContainer = null;
        if (this._formContainer === undefined)
            this._formContainer = null;
        if (this._listTpl === undefined)
            this._listTpl = null;
        if (this._formTpl === undefined)
            this._formTpl = null;
        ...
        if (!f.part)
            this._addLog('warn', 'Part 基类不可用（jsdom 无 DOM Parts API，Chrome 部分实现中）');
        if (!f.childNodePart)
            this._addLog('warn', 'ChildNodePart 不可用，子节点部件演示将仅显示说明文本');
        if (!f.partGroup)
            this._addLog('warn', 'PartGroup 不可用，部件分组演示将仅显示说明文本');
        if (!f.documentPartRoot)
            this._addLog('warn', 'DocumentPartRoot 不可用，部件根演示将仅显示说明文本');
        if (!f.getPartRoot)
            this._addLog('warn', 'Element.prototype.getPartRoot 不可用，无法获取元素部件根');
        if (!f.createInstance)
            this._addLog('warn', 'HTMLTemplateElement.prototype.createInstance 不可用，模板实例化演示将仅显示说明文本');
...
const createInstanceOK =
  ...
  typeof HTMLTemplateElement.prototype.createInstance === 'function';`)),
                ...
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.conceptInfo || '（点击按钮查看 DOM Parts API 概念与三方案对比完整说明）')),
            ],
        });
    ...
    `);
        ...
        const info = [
            ...
            '  document',
            '    └─ Element (host)',
            '         └─ ElementPartRoot = (host as any).getPartRoot()',
            '              └─ PartGroup',
            ...
            '  part.value = (e: any) => console.log("clicked", e);',
            '',
            '【实际能力检测演示】',
            `  Part: ${f.part ? '✓' : '✗'}`,
            `  ChildNodePart: ${f.childNodePart ? '✓' : '✗'}`,
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'DocumentPartRoot 是文档级部件根，Element.prototype.getPartRoot() 返回元素的 ElementPartRoot（管理该元素子树的部件）。PartRoot 提供 createChildNodePart(node, metadata) 等工厂方法。部件树与 DOM 树平行：ChildNodePart 指向子节点、AttrPart 指向特性、PropertyPart（::value 语法）指向 JS 属性、BooleanPart 切换布尔特性、EventPart（@event 语法）绑定事件。PartType 枚举：Attribute/Property/Boolean/Event/TextNode。disconnect() 后部件不再跟随 DOM。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行部件根与类型枚举演示', { type: 'primary', size: 'sm', onClick: () => this._runRootDemo() })),
                h('div', { class: 'domparts-root-host domparts-host' }, h('span', { class: 'fs-sm text-secondary' }, '部件根挂载容器（getPartRoot 调用宿主）')),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// Element.prototype.getPartRoot() 获取元素部件根
const host = document.querySelector('.my-host');
const partRoot = (host as any).getPartRoot();
...
eventPart.value = (e: any) => console.log('clicked', e);

// disconnect：断开部件与 DOM 关联
part.disconnect();`)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.rootInfo || '（点击按钮查看 DocumentPartRoot 与 getPartRoot 完整说明）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    `);
        ...
        const info = [
            ...
            '  const group = (host as any).getPartRoot().parts;',
            '  group.forEach((part: any) => {',
            ...
            '    const root = (host as any).getPartRoot();',
            '    host.innerHTML = "";',
            '    const group = [];',
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'Element.createChildNodePart(node, metadata) 创建 ChildNodePart，metadata 含 type（PartType）与 name（绑定状态键）。part.replace(node) 替换为元素节点，part.replace(text) 替换为文本节点，部件引用自动更新。part.disconnect() 断开部件与 DOM 关联（不移除节点）。PartRoot.parts 返回 PartGroup，支持 forEach/index/replaceChildren 批量操作。与 cloneNode 不同，ChildNodePart 建立长期部件引用，可反复 replace 更新，仅触及占位节点无需全量重建。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 createChildNodePart 演示', { type: 'primary', size: 'sm', onClick: () => this._runChildNodeDemo() })),
                h('div', { class: 'domparts-child-host domparts-host' }, h('span', { class: 'fs-sm text-secondary' }, 'ChildNodePart 挂载容器（replace/disconnect 演示宿主）')),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// 创建 ChildNodePart
const host = document.querySelector('.host');
...
function renderList(host: any,  items: any) {
  const root = (host as any).getPartRoot();
  host.innerHTML = '';
  const group = [];
  items.forEach((item: any) => {
  ...
  return group;
                ...
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.childNodeInfo || '（点击按钮查看 createChildNodePart 与 PartGroup 完整说明）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    `);
        ...
        if (container)
            this._instanceContainer = container;
        ...
        const info = [
            ...
            '    { name: "Alice", count: 42 },',
            '    { processCallback: (parts: any, state: any) => { /* 自定义求值 */ } },',
            '  );',
            '',
            '【TemplateInstance 接口】',
            ...
            '  template.processCallback = (parts: any, state: any) => {',
            ...
            '    processCallback: (parts: any, state: any) => {',
            '      parts.forEach((p: any) => {',
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'HTMLTemplateElement.prototype.createInstance(state, options) 从 <template> 创建 TemplateInstance（DocumentFragment 子类），可直接 append 到 DOM。instance.update(state) 批量更新所有匹配的 Part（{{name}}/{{count}} 占位）。processCallback(state) 自定义表达式求值（支持点路径 user.name、过滤器），可覆盖默认求值。协同流程：createInstance 解析模板建立 Part 映射 → 首次 processCallback 求值 → update 触发重新求值 → Part.replace/value 更新 DOM。update 是浅比较，仅触及变化的 Part 无全量 diff。一个 template 可 createInstance 多次生成独立实例。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 createInstance 演示', { type: 'primary', size: 'sm', onClick: () => this._runInstanceDemo() })),
                h('div', { class: 'domparts-instance-host domparts-host' }, h('span', { class: 'fs-sm text-secondary' }, 'TemplateInstance 挂载容器（createInstance + update 演示宿主）')),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// HTML: <template id="greeting">
//   <p>Hello {{name}}, you have {{count}} messages</p>
// </template>
const template = document.querySelector('#my-template');
const instance = template.createInstance();
...
instance.template;        // 关联的 <template> 元素`)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.instanceInfo || '（点击按钮查看 createInstance 与 TemplateInstance 完整说明）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    `);
        ...
        const info = [
            ...
            '    evaluator: (state: any) => state.user.name,  // 自定义求值函数',
            '  });',
            '',
            '  // 读取元数据',
            ...
            '  part.value = (e: any) => console.log("clicked");  // 函数',
            ...
            '          part.value = (e: any) => handlers[part.expression](e, state);',
            '          break;',
            '        case "TextNode":',
            ...
            '    const [path, ...filters] = expr.split("|").map((s: any) => s.trim());',
            '    const value = path.split(".").reduce((o: any, k: any) => o?.[k], state);',
            '    return filters.reduce((v: any, fn: any) => filtersMap[fn]?.(v) ?? v, value);',
            '  }',
            '',
            '  const filtersMap = {',
            '    upper: (v: any) => String(v).toUpperCase(),',
            '    sanitize: (v: any) => String(v).replace(/[<>]/g, ""),',
            ...
            '    }',
            '    set value(v) { (this.el as HTMLElement).style[this.prop] = v; }',
            '  }',
            '',
            '【实际能力检测演示】',
            `  Part: ${f.part ? '✓' : '✗'}`,
...
eventPart.value = (e: any) => console.log('clicked'); // EventPart: 函数
...
// 表达式求值器：点路径 + 过滤器
function evalExpr(state: any,  expr: any) {
  const [path, ...filters] = expr.split('|').map((s: any) => s.trim());
  const value = path.split('.').reduce((o: any, k: any) => o?.[k], state);
  return filters.reduce((v: any, fn: any) => filtersMap[fn]?.(v) ?? v, value);
}
...
class StylePart extends Part {
  ...
  set value(v) { (this.el as HTMLElement).style[this.prop] = v; }
                ...
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.metaInfo || '（点击按钮查看 PartType 元数据与自定义处理器完整说明）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    `);
        ...
        const info = [
            ...
            '  const tpl = document.querySelector("#list-tpl");',
            '  tpl.processCallback = (parts: any, state: any) => {',
            '    for (const part of parts) {',
            '      if (part.metadata.name === "items") {',
            '        // 为每项创建 <li> 节点，part.replace 批量替换',
            '        const nodes = state.items.map((item: any) => {',
            '          const li = document.createElement("li");',
            ...
            '  const instance = (tpl as any).createInstance();',
            ...
            '        part.replace(...nodes);',
            '      };',
            '      this._inst = (tpl as any).createInstance();',
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '声明式响应式列表渲染：模板中 <ul>{{items}}</ul> 占位，processCallback 拦截 items 部件，state.items.map 创建 <li> 节点数组，part.replace(...nodes) 批量替换。响应式更新仅触及 items 部件，无虚拟 DOM diff。与自定义元素协同：constructor 中 attachShadow + createInstance，set items 触发 update 复用同一 TemplateInstance。性能优势：跳过 vdom diff，仅重建 <li>。大列表优化：缓存 ChildNodePart 数组复用，仅追加/移除差异项（手动 key 复用）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行声明式列表渲染演示', { type: 'primary', size: 'sm', onClick: () => this._runListDemo() })),
                h('div', { class: 'domparts-list-host domparts-host' }, h('span', { class: 'fs-sm text-secondary' }, '列表渲染容器（{{items}} 占位 + part.replace 演示宿主）')),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// HTML: <template id="list-tpl"><ul>{{items}}</ul></template>
const tpl = document.querySelector('#list-tpl');
...
class ItemList extends HTMLElement {
  constructor() {
    ...
    tpl.processCallback = (parts: any, state: any) => {
      const part = parts[0];
      part.replace(...state.items.map((item: any) => {
        const li = document.createElement('li');
        li.textContent = item.name + ': ' + item.count;
        return li;
      }));
...
customElements.define('item-list', ItemList);`)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.listInfo || '（点击按钮查看声明式响应式列表渲染完整实战说明）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    `);
        ...
        const info = [
            ...
            '  //     <p>Hello {{name}}</p>',
            '  //   </template>',
            '  // state: { name: "Alice", onInput: (e: any) => state.name = e.target.value }',
            '',
            ...
            '        if (p.metadata.type === "Event") p.value = (e: any) => {',
            ...
            '  ["name", "email", "phone"].forEach((key: any) => {',
    ...
    _renderCard7() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '表单双向绑定：::value PropertyPart 设置 input.value（双冒号区别于 AttrPart 单冒号），@input EventPart 绑定事件。processCallback 按 type 分发：Property 设 value、Event 包装 handler 回写 state 并触发 update、TextNode 用 replace。EventPart 回调中手动 update 实现重渲染（无自动脏检查）。模板片段复用：封装 createField(label, key) 生成可复用模板实例构建多字段表单。与 Declarative Shadow DOM 协同：<template shadowrootmode="open"> 内模板 createInstance，SSR 友好。与 React 受控组件对比：DOM Parts 跳过组件树 diff，字段多时优势明显。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行表单双向绑定演示', { type: 'primary', size: 'sm', onClick: () => this._runFormDemo() })),
                h('div', { class: 'domparts-form-host domparts-host' }, h('span', { class: 'fs-sm text-secondary' }, '表单双向绑定容器（::value + @input 演示宿主）')),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// HTML: <template id="form-tpl">
//   <input ::value="name" @input="onInput">
//   <p>Hello {{name}}</p>
// </template>
const tpl = document.querySelector('#form-tpl');
...
class FormBind extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this._inst.update(this._state);
                ...
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.formInfo || '（点击按钮查看表单双向绑定完整实战说明）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard8() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'primary' }, '生态'), h(Tag, { color: f.createInstance ? 'success' : 'error' }, `DOM Parts ${f.createInstance ? '✓' : '✗'}`)),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '性能对比：Lit html`` 基于早期 Template Instantiation 提案，DOM Parts API 是其标准化演进，原生无需框架层性能更优（~5-8μs vs Lit ~10μs/更新）。无虚拟 DOM diff：update 直接触及占位部件 part.replace/value，跳过树比较，大列表性能优势明显（1000 项更新 ~0.6ms vs React ~3.2ms）。浏览器支持：Chrome ≥120 部分实现，Safari/Firefox 未实现。Polyfill 策略：@github/template-parts（npm install，import 后自动 polyfill，兼容 IE11+）。WC 生态整合：Lit 未来可复用原生 Part，Vue/Solid 可基于原生 Part 标准化内部，原生自定义元素直接 createInstance 无需框架。渐进增强：检测原生 → 不支持动态加载 polyfill → 统一 API。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行性能对比与生态演示', { type: 'primary', size: 'sm', onClick: () => this._runPerfDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// 渐进增强：优先原生，不支持加载 polyfill
const nativeOK = typeof HTMLTemplateElement !== 'undefined'
  && typeof HTMLTemplateElement.prototype.createInstance === 'function';
if (!nativeOK) {
...
// 框架无关：封装响应式状态
function createStateful(template: any,  initialState: any) {
  const inst = template.createInstance();
  let state = initialState;
  return {
    el: inst,
...
//   DOM Parts API:    ~0.6ms (原生部件 replace)`)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.perfInfo || '（点击按钮查看性能对比与生态完整说明）')),
            ],
        });
        this.registerChild(card);
        return card.render();
...
//# sourceMappingURL=DOMPartsAPIPage.js.map