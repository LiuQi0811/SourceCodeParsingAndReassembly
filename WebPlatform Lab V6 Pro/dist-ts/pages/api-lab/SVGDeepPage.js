...
export class SVGDeepPage extends Page {
    ...
    componentDidMount() {
        ...
        if (this._smilAnim === undefined)
            this._smilAnim = null; // Card 3 SMIL <animateTransform> 引用
        if (this._rafId === undefined)
            this._rafId = null; // requestAnimationFrame 句柄
        if (this._viewBoxScale === undefined)
            this._viewBoxScale = 1; // Card 1 当前 viewBox 缩放系数
        if (this._currentFilter === undefined)
            this._currentFilter = null; // Card 4 当前应用的 filter id
        if (this._filterDefs === undefined)
            this._filterDefs = null; // Card 4 filter <defs> 引用
        if (this._clipDef === undefined)
            this._clipDef = null; // Card 5 clipPath 引用
        if (this._maskDef === undefined)
            this._maskDef = null; // Card 5 mask 引用
    ...
    _createSvgElements() {
        ...
        if (!host) {
            this._addLog('warn', '未找到演示容器 #svg-dom-host');
            return;
        }
        try {
            // 清空旧内容
    ...
    _modifyViewBox() {
        ...
        if (!svg) {
            this._addLog('warn', '请先点击「动态创建 SVG 元素」');
            return;
        }
        try {
            // 缩放系数循环：1 → 0.5 → 2 → 1
    ...
    _ensurePath() {
        if (!this._caps().createNs || !this._caps().svgPath)
            return null;
    ...
    _computePathLength() {
        ...
        if (!ref) {
            this._addLog('warn', '未找到演示容器 #svg-path-host');
            return;
        }
        try {
            const { path } = ref;
    ...
    _addSmilRotation() {
        ...
        if (!host) {
            this._addLog('warn', '未找到演示容器 #svg-smil-host');
            return;
        }
        try {
            // 重建以保证干净状态
    ...
    _applyBlur() {
        ...
        if (!svg) {
            this._addLog('warn', '未找到演示容器 #svg-filter-host');
            return;
        }
        try {
            this._clearFilters();
    ...
    _applyClip() {
        ...
        const svg = this._ensureClipHost();
        if (!svg) {
            this._addLog('warn', '未找到演示容器 #svg-clip-host');
            return;
        }
        try {
            this._clearClipDefs();
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.createNs ? 'success' : 'warning' }, caps.createNs ? 'createElementNS 可用' : '不可用'), h(Tag, { color: 'primary' }, 'svg / rect / circle / path')),
            children: [
                ...
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('动态创建 SVG 元素并插入演示区', { type: 'primary', size: 'sm', disabled: !caps.createNs, onClick: () => this._createSvgElements() }), this._btn('修改 viewBox 缩放', { size: 'sm', disabled: !caps.createNs, onClick: () => this._modifyViewBox() })),
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.svgPath ? 'success' : 'warning' }, caps.svgPath ? 'SVGPathElement 可用' : '不可用'), h(Tag, { color: 'primary' }, 'getTotalLength / getPointAtLength')),
            children: [
                ...
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('计算路径总长', { type: 'primary', size: 'sm', disabled: !caps.createNs || !caps.svgPath, onClick: () => this._computePathLength() }), this._btn('沿路径取 5 个点', { size: 'sm', disabled: !caps.createNs || !caps.svgPath, onClick: () => this._samplePathPoints() })),
                ...
                h('pre', { class: 'code-block', style: { maxHeight: '60px', overflow: 'auto' } }, h('code', {}, s.pathLength || '（点击「计算路径总长」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '沿路径取 5 个等距点 getPointAtLength()：'),
                h('pre', { class: 'code-block', style: { maxHeight: '120px', overflow: 'auto' } }, h('code', {}, s.pointList || '（点击「沿路径取 5 个点」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '路径命令速查表（命令 | 名称 | 说明）：'),
                h('div', { class: 'table-wrap', style: { overflowX: 'auto' } }, h('table', { class: 'data-table', style: { width: '100%', borderCollapse: 'collapse', fontSize: '12px' } }, h('thead', {}, h('tr', {}, h('th', { style: thStyle }, '命令'), h('th', { style: thStyle }, '名称'), h('th', { style: thStyle }, '说明'))), h('tbody', {}, ...PATH_COMMANDS_TABLE.map((r) => h('tr', {}, h('td', { style: tdStyle }, h('code', {}, r.cmd)), h('td', { style: tdStyle }, r.name), h('td', { style: tdStyle }, r.desc)))))),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.svgTransform ? 'success' : 'warning' }, caps.svgTransform ? 'SVGTransform 可用' : '不可用'), h(Tag, { color: 'primary' }, 'animateTransform'), h(Tag, { color: 'warning' }, 'SMIL')),
            children: [
                ...
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('添加 SMIL 旋转动画', { type: 'primary', size: 'sm', disabled: !caps.createNs, onClick: () => this._addSmilRotation() }), this._btn('beginElement 启动', { size: 'sm', disabled: !caps.createNs, onClick: () => this._beginSmil() }), this._btn('endElement 停止', { size: 'sm', disabled: !caps.createNs, onClick: () => this._endSmil() })),
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.createNs ? 'success' : 'warning' }, caps.createNs ? 'DOM 可用' : '不可用'), h(Tag, { color: 'primary' }, 'filter / feGaussianBlur / feColorMatrix')),
            children: [
                ...
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('应用高斯模糊', { type: 'primary', size: 'sm', disabled: !caps.createNs, onClick: () => this._applyBlur() }), this._btn('应用阴影滤镜', { type: 'primary', size: 'sm', disabled: !caps.createNs, onClick: () => this._applyShadow() }), this._btn('移除滤镜', { size: 'sm', disabled: !caps.createNs, onClick: () => this._removeFilter() }), h(Tag, { color: s.filterStatus === '未应用' ? 'default' : 'success' }, `当前：${s.filterStatus}`)),
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.createNs ? 'success' : 'warning' }, caps.createNs ? 'DOM 可用' : '不可用'), h(Tag, { color: 'primary' }, 'clipPath / mask')),
            children: [
                ...
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('应用 clipPath 圆形剪裁', { type: 'primary', size: 'sm', disabled: !caps.createNs, onClick: () => this._applyClip() }), this._btn('应用 mask 渐变蒙版', { type: 'primary', size: 'sm', disabled: !caps.createNs, onClick: () => this._applyMask() }), this._btn('移除剪裁/蒙版', { size: 'sm', disabled: !caps.createNs, onClick: () => this._removeClipMask() }), h(Tag, { color: s.clipStatus === '未应用' ? 'default' : 'success' }, `当前：${s.clipStatus}`)),
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'page api-lab-page' }, h('h2', { class: 'section-title' }, 'SVG 矢量图形深度实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, '演示 SVG DOM：createElementNS 命名空间 / SVGPathElement 路径几何(getTotalLength/getPointAtLength) / SVGTransform+SMIL 动画 / filter 滤镜(feGaussianBlur/feColorMatrix) / clipPath+mask 剪裁蒙版。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderLogPanel());
    }
}
...
//# sourceMappingURL=SVGDeepPage.js.map