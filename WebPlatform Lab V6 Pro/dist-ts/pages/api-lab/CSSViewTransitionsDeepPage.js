...
export class CSSViewTransitionsDeepPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!f.viewTransitions)
            this._addLog('info', 'View Transitions API 不可用（Chrome 111+/Safari 18+/Firefox 暂不支持）');
        if (!f.viewTransitionPseudo)
            this._addLog('warn', '::view-transition-* 伪元素不支持');
        if (!f.activeViewTransitionType)
            this._addLog('warn', ':active-view-transition-type() 不支持（Chrome 125+）');
        if (f.prefersReducedMotion)
            this._addLog('warn', '用户偏好减少动态效果（prefers-reduced-motion: reduce），应跳过过渡');
    ...
    _flags() {
        ...
        const supportsStr = (s) => {
            try {
                return hasCSSSupports && CSS.supports(s);
            }
            catch {
                return false;
            }
    ...
    _toggleBasicBox() {
        ...
        if (!box) {
            this._addLog('warn', '演示元素 .vt-basic-box 未找到');
            return;
        }
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'document.startViewTransition(callback) 返回 ViewTransition 对象，含 updateCallbackDone/ready/finished/skipped 四个 Promise 与 skipTransition() 方法。四阶段流程：捕获旧状态快照 -> 执行 DOM 更新 callback -> 捕获新状态快照 -> 交叉淡入淡出过渡（默认 250ms root 容器）。ready 后可修改伪元素动画与 CSS 协同。callback 可异步（返回 Promise）。浏览器支持 Chrome 111+/Safari 18+/Firefox 暂不支持。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行概述演示', { type: 'primary', size: 'sm', onClick: () => this._runOverviewDemo() }), this._btn('切换盒子（触发过渡）', { type: 'default', size: 'sm', onClick: () => this._toggleBasicBox() })),
                h('div', { class: 'fs-sm text-secondary' }, '交互演示（点击上方按钮触发 startViewTransition）：'),
                h('div', { class: 'vt-stage' }, h('div', { class: 'vt-basic-box' }, 'BOX')),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.overviewInfo || '（点击按钮查看 View Transitions API 概述完整用法）')),
            ],
        });
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['::view-transition-old', f.viewTransitionPseudo]]), h(Tag, { color: 'primary' }, '伪元素树')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '::view-transition 根容器覆盖整个视口；::view-transition-group(name) 每个命名过渡元素组；::view-transition-image-pair(name) 包含新旧图像容器；::view-transition-old(name) 旧状态快照图像；::view-transition-new(name) 新状态快照图像。未命名元素全部进入 root 组 ::view-transition-group(root)。伪元素是像素快照非真实 DOM，默认动画 old 淡出 new 淡入。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行伪元素树演示', { type: 'primary', size: 'sm', onClick: () => this._runPseudoTreeDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '伪元素树结构：'),
                h('div', { class: 'vt-tree' }, '::view-transition\n  |\n  +-- ::view-transition-group(root)\n  |     +-- ::view-transition-image-pair(root)\n  |           +-- ::view-transition-old(root)\n  |           +-- ::view-transition-new(root)\n  |\n  +-- ::view-transition-group(header)\n        +-- ::view-transition-image-pair(header)\n              +-- ::view-transition-old(header)\n              +-- ::view-transition-new(header)'),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.pseudoTreeInfo || '（点击按钮查看 ::view-transition 伪元素树完整结构）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _moveNamedBox() {
        ...
        if (!box) {
            this._addLog('warn', '演示元素 .vt-named-box 未找到');
            return;
        }
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['view-transition-name', f.viewTransitionName]]), h(Tag, { color: 'primary' }, '命名过渡')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'view-transition-name: <custom-ident> | none 为元素分配唯一名称使其脱离 root 组独立过渡。名称必须全局唯一（重复会报错并 abort 整个过渡）。命名后该元素的旧/新快照独立交叉淡入淡出，group 还会动画位置/尺寸变化。用于元素位移/缩放/形态变化过渡。命名元素需有稳定 layout 才能匹配新旧状态。列表项可动态分配唯一 name 实现重排过渡。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行命名演示', { type: 'primary', size: 'sm', onClick: () => this._runNameDemo() }), this._btn('移动命名盒子（独立过渡）', { type: 'default', size: 'sm', onClick: () => this._moveNamedBox() })),
                h('div', { class: 'fs-sm text-secondary' }, '交互演示（盒子已设 view-transition-name: vt-named-demo）：'),
                h('div', { class: 'vt-named-host' }, h('div', { class: 'vt-named-box' }, 'NAMED')),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.nameInfo || '（点击按钮查看 view-transition-name 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['自定义动画', f.viewTransitionPseudo]]), h(Tag, { color: 'primary' }, '@keyframes')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '通过 ::view-transition-old(name)/::view-transition-new(name) 设置 animation 自定义过渡，@keyframes vt-fade-out { to { opacity: 0; transform: scale(0.9); } } 新旧状态独立动画。配合 mix-blend-mode/filter 增强效果。ViewTransition.ready.then(() => { /* 修改伪元素动画 */ }) 用 Web Animations API 动态修改（pseudoElement 选项）。skipTransition() 跳过过渡。经典效果：圆形展开（clip-path circle）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行自定义动画演示', { type: 'primary', size: 'sm', onClick: () => this._runCustomAnimDemo() }), this._btn('圆形展开过渡（ready 注入）', { type: 'default', size: 'sm', onClick: () => this._runCustomAnimTransition() })),
                h('div', { class: 'vt-stage' }, h('div', { class: 'vt-basic-box' }, 'CUSTOM')),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.customAnimInfo || '（点击按钮查看自定义动画与 @keyframes 协同完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _slideTypes(direction) {
        const host = document.querySelector('.vt-types-host');
        if (!host) {
            this._addLog('warn', '演示元素 .vt-types-host 未找到');
            return;
        }
        ...
        if (hasObjSig) {
            try {
                ...
                if (t && t.types)
                    t.types = new Set([direction === 'forward' ? 'slide-forward' : 'slide-back']);
            }
        }
        else {
            cb();
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([[':active-view-transition-type', f.activeViewTransitionType]]), h(Tag, { color: 'primary' }, 'types 分类')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'viewTransition.types = Set 设置过渡类型标签，配合 :active-view-transition-type(name) 伪类按类型应用不同动画。document.startViewTransition({ update, types: ["slide"] }) 新签名（Chrome 125+）。SPA 路由不同方向（forward/back）不同动画。类型化的好处：单一样式表声明所有过渡模式，JS 只需设置 types 选择模式。types 必须在 ready 前设置。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 types 演示', { type: 'primary', size: 'sm', onClick: () => this._runTypesDemo() }), this._btn('前进 slide-forward', { type: 'default', size: 'sm', onClick: () => this._slideTypes('forward') }), this._btn('后退 slide-back', { type: 'default', size: 'sm', onClick: () => this._slideTypes('back') })),
                h('div', { class: 'fs-sm text-secondary' }, '交互演示（点击前进/后退触发不同方向滑动过渡）：'),
                h('div', { class: 'vt-types-host', dataset: { idx: '0' } }, h('div', { class: 'vt-types-card' }, 'Page 1 / 5')),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.typesInfo || '（点击按钮查看 ViewTransition.types 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _navigateSpa(page) {
        const host = document.querySelector('.vt-spa-host');
        if (!host) {
            this._addLog('warn', '演示元素 .vt-spa-host 未找到');
            return;
        }
        ...
        const cb = () => {
            ...
            div.textContent = (titles[page]) || `Page ${page}`;
        ...
        if (hasObjSig) {
            try {
                ...
                t = document.startViewTransition(cb);
                if (t && t.types)
                    t.types = new Set([`slide-${direction}`]);
            }
        }
        else {
            cb();
            this._addLog('info', `SPA 导航到 ${(titles[page])}（无过渡）`);
            return;
        }
        if (t) {
            t.finished.then(() => this._addLog('info', `SPA 导航到 ${(titles[page])}（${direction}）finished`)).catch(() => this._addLog('warn', '过渡被跳过/取消'));
        }
    }
    _renderCard6() {
        const s = this.state;
        ...
        const card = new Card({
            title: '6. SPA 路由集成实战',
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['SPA 集成', f.viewTransitions]]), h(Tag, { color: 'primary' }, '路由集成')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '路由 beforeEach 拦截 DOM 更新，router.beforeEach(async (to) => { const t = document.startViewTransition(async () => { await updateDOM(to); }); await t.ready; applyAnimation(t, to); })。新旧状态元素匹配通过 view-transition-name（持久布局匹配）。列表项过渡（每个 item 分配唯一 name）。导航方向感（types 分 forward/back）。注意点：未渲染元素不可命名、隐藏元素快照可能为空、DOM 更新必须在 callback 内。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 SPA 集成演示', { type: 'primary', size: 'sm', onClick: () => this._runSpaDemo() }), this._btn('首页', { type: 'default', size: 'sm', onClick: () => this._navigateSpa(1) }), this._btn('列表', { type: 'default', size: 'sm', onClick: () => this._navigateSpa(2) }), this._btn('详情', { type: 'default', size: 'sm', onClick: () => this._navigateSpa(3) })),
                h('div', { class: 'fs-sm text-secondary' }, '交互演示（模拟 SPA 路由切换，带方向感）：'),
                h('div', { class: 'vt-spa-host', dataset: { page: '1' } }, h('div', { class: 'vt-spa-page vt-spa-page--active' }, '首页 Home')),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.spaInfo || '（点击按钮查看 SPA 路由集成实战完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard7() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['复杂场景', f.viewTransitions]]), h(Tag, { color: 'primary' }, '陷阱清单')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '列表/网格过渡（item 新增/删除/重排，每个 item 分配唯一 name 实现平滑重排）。共享元素过渡（元素 A 变成元素 B，需相同 name，如列表缩略图到详情大图）。3D 翻转（rotateY 配合 perspective）。形态变换（圆形变方形，需 JS 自定义动画因 border-radius 不插值）。滚动位置保持。字体加载闪烁（FOIT/FOUT，用 document.fonts.ready）。陷阱清单：name 全局唯一/新旧状态都存在/旧元素删除需在 callback/iframe 不能触发/动画期间 pointer events/减少动画降级/Safari 滞后/性能快照开销大。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行复杂场景演示', { type: 'primary', size: 'sm', onClick: () => this._runComplexDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.complexInfo || '（点击按钮查看复杂场景与陷阱完整清单）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard8() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '同文档 vs 跨文档（MPA）对比：跨文档用 @view-transition { navigation: auto; } + pageswap/pagereveal 事件（已在 ViewTransitionsL2Page 覆盖），本页专注同文档 SPA 场景。最佳实践：渐进增强（无支持时直接更新 DOM）、prefers-reduced-motion 尊重用户偏好、动画时长 200-400ms、避免过度装饰、可访问性焦点管理、调试技巧（DevTools Animations 面板查看伪元素动画）、性能预算（callback <50ms、命名元素 <20、GPU 内存 <10MB）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行最佳实践演示', { type: 'primary', size: 'sm', onClick: () => this._runBestPracticeDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.bestPracticeInfo || '（点击按钮查看 vs Cross-Document 与最佳实践完整指南）')),
            ],
        });
        this.registerChild(card);
        return card.render();
...
//# sourceMappingURL=CSSViewTransitionsDeepPage.js.map