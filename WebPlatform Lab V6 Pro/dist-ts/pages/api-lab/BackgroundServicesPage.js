...
const SW_STATES = ['parsed', 'installing', 'installed', 'activating', 'activated', 'redundant'];
export class BackgroundServicesPage extends Page {
    ...
    componentDidMount() {
        ...
        if ('cookieStore' in window && window.cookieStore) {
            this._onCookieChange = (event) => {
                try {
                    const changed = (event.changed || []).map((c) => c);
                    const deleted = (event.deleted || []).map((c) => c);
                    if (changed.length)
                        this._addLog('cookie', `change 事件：变更 [${changed.map((c) => c.name).join(', ')}]`);
                    if (deleted.length)
                        this._addLog('cookie', `change 事件：删除 [${deleted.map((c) => c.name).join(', ')}]`);
                }
                catch { /* noop */ }
            };
            try {
                window.cookieStore.addEventListener('change', this._onCookieChange);
        ...
        if ('caches' in window) {
            caches.keys().then((names) => {
                if (this._destroyed || !names.length)
                    return;
    ...
    componentWillUnmount() {
        if (this._onCookieChange && 'cookieStore' in window && window.cookieStore) {
            try {
                window.cookieStore.removeEventListener('change', this._onCookieChange);
            }
            catch { /* noop */ }
        }
    ...
    _input(field, opts = {}) {
        const input = new Input({
            ...
            style: { width: '200px' },
            ...opts,
            onChange: opts.onChange || ((v) => { this.state[field] = v; }),
        });
    ...
    async _updateSW() {
        ...
        try {
            ...
            if (!reg)
                return this._addLog('sw', '无注册，无法调用 update()');
            await reg.update();
    ...
    async _cachePut() {
        if (!('caches' in window))
            return this._addLog('err', 'Cache API 不可用');
    ...
    async _cacheKeysInOne() {
        if (!('caches' in window))
            return this._addLog('err', 'Cache API 不可用');
        try {
            const cache = await caches.open('bg-svc-cache-v1');
            const reqs = await cache.keys();
            this._addLog('cache', `(cache as any).keys() => ${reqs.length} 条 Request：[${reqs.map((r) => r.url).join(', ') || '空'}]`);
        }
        catch (err) {
            this._addLog('err', `cache.keys 异常：${err.message}`);
        }
    }
    async _cacheDeleteAll() {
        if (!('caches' in window))
            return this._addLog('err', 'Cache API 不可用');
        try {
            const names = await caches.keys();
            if (!names.length)
                return this._addLog('cache', '无缓存可删除');
    ...
    async _reqNotifPermission() {
        if (!('Notification' in window))
            return this._addLog('err', 'Notifications API 不可用');
        try {
    ...
    async _pushGetSub() {
        ...
        try {
            const reg = await navigator.serviceWorker.getRegistration();
            if (!reg)
                return this._addLog('push', '无 SW 注册，无法查询订阅');
    ...
    async _pushUnsubscribe() {
        ...
        try {
            const reg = await navigator.serviceWorker.getRegistration();
            if (!reg)
                return this._addLog('push', '无 SW 注册');
            const sub = await reg.pushManager.getSubscription();
            if (!sub) {
    ...
    async _syncRegister() {
        ...
        try {
            ...
            await reg.sync.register('bg-sync-tag');
    ...
    async _cookieSet() {
        ...
        try {
            await window.cookieStore.set({ name, value, path: '/', sameSite: 'lax' });
    ...
    renderPage() {
        ...
        const hasCookie = 'cookieStore' in window && !!window.cookieStore;
        ...
        return [
            ...
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'navigator.serviceWorker.register(swUrl, { scope }) 返回 Promise<ServiceWorkerRegistration>，含 .installing/.waiting/.active 三个 ServiceWorker 实例与 .scope/.update()/.unregister()。SW 状态：parsed → installing → installed → activating → activated → redundant。controller 指向当前控制页面的活动 SW（无则为 null）。可监听 navigator.serviceWorker 的 controllerchange（控制器变更）与 registration 的 updatefound（发现新版本 SW）事件。SW 是离线、Push、Background Sync 的基础，但需要 HTTPS + 独立脚本。'), !hasSW
                ? h(Alert, { type: 'warning', message: '当前环境不支持 Service Worker API（navigator.serviceWorker 未定义）' })
                : h('div', { class: 'flex flex-col gap-sm' }, h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('注册 SW', { type: 'primary', size: 'sm', onClick: () => this._registerSW() }), this._btn('取消注册', { danger: true, size: 'sm', onClick: () => this._unregisterSW() }), this._btn('update()', { size: 'sm', onClick: () => this._updateSW() }), this._btn('查看 controller', { size: 'sm', onClick: () => this._logController() })), h('div', { class: 'flex items-center gap-sm flex-wrap' }, h(Tag, { color: this.state.swState === 'registered' ? 'success' : this.state.swState === 'error' ? 'warning' : 'default' }, `状态：${this.state.swState}`), this.state.swScope ? h(Tag, { color: 'primary' }, `scope: ${this.state.swScope}`) : null, h(Tag, { color: ctrl ? 'success' : 'default' }, `controller: ${ctrl ? ctrl.state : 'null'}`)), this.state.swError ? h('div', { class: 'fs-sm', style: { color: '#ff5555' } }, `错误：${this.state.swError}`) : null, h('div', { class: 'fs-sm text-tertiary' }, `SW 生命周期状态：${SW_STATES.join(' → ')}`)))),
            h(Card, {
                title: '2. Cache API（caches —— 无需 SW 即可使用）',
                extra: h(Tag, { color: hasCaches ? 'success' : 'default' }, hasCaches ? '可用' : '不可用'),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'caches.open(name) 打开 Cache 对象；cache.put(request, response) 写入；cache.add(url) 自动 fetch+put；cache.addAll([...]) 批量；cache.match(request) 读取 Response；cache.matchAll() 读取全部；(cache as any).keys() 列出所有 Request；(caches as any).keys() 列出所有缓存名；caches.delete(name) 删除整个缓存。Cache API 独立于 SW，可在普通页面直接使用。'), !hasCaches
                ? h(Alert, { type: 'warning', message: '当前环境不支持 Cache API（caches 未定义）' })
                : h('div', { class: 'flex flex-col gap-sm' }, h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._input('cacheKey', { placeholder: '缓存 key', style: { width: '160px' } }), this._input('cacheValue', { placeholder: '缓存 value（JSON 字符串）', style: { width: '260px' } })), h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('写入缓存', { type: 'primary', size: 'sm', onClick: () => this._cachePut() }), this._btn('读取缓存', { type: 'primary', size: 'sm', onClick: () => this._cacheRead() }), this._btn('列出所有缓存', { size: 'sm', onClick: () => this._cacheListAll() }), this._btn('列出缓存内 Request', { size: 'sm', onClick: () => this._cacheKeysInOne() }), this._btn('删除全部缓存', { danger: true, size: 'sm', onClick: () => this._cacheDeleteAll() })), h('div', { class: 'flex items-center gap-sm flex-wrap' }, h(Tag, { color: 'primary' }, `缓存名：${this.state.cacheList.length ? this.state.cacheList.join(', ') : '(空)'}`)), this.state.cacheContent ? h('pre', { class: 'code-block', style: { maxHeight: '100px' } }, `cache.match => ${this.state.cacheContent}`) : null))),
            h(Card, {
            ...
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'Notification.requestPermission() 返回 Promise<default|granted|denied>；Notification.permission 反映当前状态。new Notification(title, { body, icon, badge, tag, renotify, requireInteraction, silent, vibrate, dir, lang, data, actions }) 创建通知，实例有 onclick/onshow/onerror/onclose 与 close()。SW 作用域下可用 registration.showNotification()。'), !hasNotif
                ? h(Alert, { type: 'warning', message: '当前环境不支持 Notifications API（Notification 未定义）' })
                : h('div', { class: 'flex flex-col gap-sm' }, h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._input('notifTitle', { placeholder: '通知标题', style: { width: '200px' } }), this._input('notifBody', { placeholder: '通知正文', style: { width: '280px' } })), h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('请求权限', { type: 'primary', size: 'sm', onClick: () => this._reqNotifPermission() }), this._btn('显示通知', { type: 'primary', size: 'sm', onClick: () => this._showNotification() })), h('div', { class: 'fs-sm text-tertiary' }, '选项说明：body 正文 / icon 大图标 / badge 小图标 / tag 分组 / renotify 重新提醒 / requireInteraction 不自动关闭 / silent 静默 / vibrate 震动 / data 附带数据 / actions 按钮动作。')))),
            h(Card, {
                title: '4. Push API（PushManager）',
                extra: h(Tag, { color: hasPush ? 'success' : 'default' }, hasPush ? this.state.pushState : '不可用'),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'registration.pushManager.subscribe({ userVisibleOnly, applicationServerKey }) 返回 Promise<PushSubscription>。订阅对象含 .endpoint / .expirationTime / .options / .getKey(name)（name 取 p256dh 或 auth）。subscription.unsubscribe() 取消；getSubscription() 查询已有；permissionState() 返回 granted/denied/prompt。applicationServerKey 必须是 base64url 编码的 VAPID 公钥。'), !hasPush
                ? h(Alert, { type: 'warning', message: '当前环境不支持 Push API（需 serviceWorker + PushManager）' })
                : h('div', { class: 'flex flex-col gap-sm' }, h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('订阅', { type: 'primary', size: 'sm', onClick: () => this._pushSubscribe() }), this._btn('查询订阅', { size: 'sm', onClick: () => this._pushGetSub() }), this._btn('取消订阅', { danger: true, size: 'sm', onClick: () => this._pushUnsubscribe() })), h('div', { class: 'flex items-center gap-sm flex-wrap' }, h(Tag, { color: 'primary' }, `状态：${this.state.pushState}`)), this.state.pushDetail ? h('div', { class: 'fs-sm text-tertiary', style: { wordBreak: 'break-all' } }, `详情：${this.state.pushDetail}`) : null, h('div', { class: 'fs-sm text-tertiary' }, '提示：Push 订阅必须依赖真实 Service Worker 注册 + 有效 VAPID 公钥；沙箱无 SW，订阅将优雅失败。')))),
            h(Card, {
                title: '5. Background Sync API（registration.sync / periodicSync）',
                extra: h(Tag, { color: 'success' }, this.state.syncState),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'registration.sync.register(tag) 注册一次性后台同步；sync.getTags() 列出标签。Periodic Sync：periodicSync.register(tag, { minInterval }) 周期性同步；periodicSync.getTags() 列出。当网络恢复时，SW 中触发 sync 事件，可延迟执行直至联网。两类 sync 均需活动 SW。'), h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('注册 sync', { type: 'primary', size: 'sm', onClick: () => this._syncRegister() }), this._btn('getTags()', { size: 'sm', onClick: () => this._syncGetTags() }), this._btn('注册 periodicSync', { size: 'sm', onClick: () => this._periodicSyncRegister() })), h('div', { class: 'flex items-center gap-sm flex-wrap' }, h(Tag, { color: 'primary' }, `sync tags：${this.state.syncTags.length ? this.state.syncTags.join(', ') : '(空)'}`)), h('div', { class: 'fs-sm text-tertiary' }, '提示：Background Sync 依赖活动 Service Worker；沙箱无 SW，注册将优雅失败（记为环境限制）。'))),
            h(Card, {
                title: '6. Cookie Store API（cookieStore）',
                extra: h(Tag, { color: hasCookie ? 'success' : 'default' }, hasCookie ? '可用' : '不可用'),
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'cookieStore.set({ name, value, domain, path, expires, maxAge, secure, sameSite, httpOnly }) 或 set(name, value) 写入；get(name) / getAll() 读取；delete(name) 删除。change 事件含 event.changed / event.deleted 两个数组。需安全上下文（HTTPS/localhost）。'), !hasCookie
                ? h(Alert, { type: 'warning', message: '当前环境不支持 Cookie Store API（需安全上下文 HTTPS/localhost）' })
                : h('div', { class: 'flex flex-col gap-sm' }, h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._input('cookieName', { placeholder: 'cookie 名', style: { width: '160px' } }), this._input('cookieValue', { placeholder: 'cookie 值', style: { width: '220px' } })), h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('写入 cookie', { type: 'primary', size: 'sm', onClick: () => this._cookieSet() }), this._btn('获取全部', { type: 'primary', size: 'sm', onClick: () => this._cookieGetAll() }), this._btn('删除 cookie', { danger: true, size: 'sm', onClick: () => this._cookieDelete() })), h('div', { class: 'fs-sm text-tertiary' }, `当前 cookie 列表：${this.state.cookieList.length ? this.state.cookieList.map((c) => `${c.name}=${c.value}`).join('; ') : '(空，点击「获取全部」)'}`), h('div', { class: 'fs-sm text-tertiary' }, '提示：cookieStore change 事件监听已在挂载时注册，写入/删除会触发日志。')))),
            h(Card, {
            ...
            }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'navigator.storage.estimate() 返回 { quota, usage }；persist() 请求持久化存储（不被自动清除）；persisted() 查询当前状态。持久化存储可保护数据免受存储压力下的自动清除。'), !hasStorage
                ? h(Alert, { type: 'warning', message: '当前环境不支持 StorageManager.estimate' })
                : h('div', { class: 'flex flex-col gap-sm' }, h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('获取 estimate', { type: 'primary', size: 'sm', onClick: () => this._storageEstimate() }), this._btn('请求 persist', { type: 'primary', size: 'sm', onClick: () => this._storagePersist() }), this._btn('查询 persisted', { size: 'sm', onClick: () => this._storagePersisted() })), est ? h('div', { class: 'flex flex-col gap-xs' }, h('div', { class: 'fs-sm' }, `配额 quota：${this._formatBytes(est.quota)}，已用 usage：${this._formatBytes(est.usage)}（${usedPct.toFixed(2)}%）`), h('div', { style: { background: '#1e1e1e', borderRadius: '4px', height: '10px', overflow: 'hidden' } }, h('div', { style: { background: '#50fa7b', height: '100%', width: `${usedPct}%` } })), h('div', { class: 'fs-sm text-tertiary' }, `持久化状态：${this.state.persisted ? '已持久化（persisted=true）' : '未持久化（persisted=false）'}`)) : null))),
            h(Card, {
                title: '事件日志',
...
//# sourceMappingURL=BackgroundServicesPage.js.map