// EnvironmentPage.ts —— 环境与设备 API 实验室
...
export class EnvironmentPage extends Page {
    ...
    renderPage() {
        ...
        return [
            ...
            // 1. Page Visibility
            h(Card, { title: '1. Page Visibility 页面可见性', extra: this._capTag('pageVisibility') }, h('div', { class: 'flex flex-col gap-sm' }, h('div', { class: 'flex items-center gap-sm' }, h('span', { class: 'fs-sm text-secondary' }, 'visibilityState：'), h(Tag, { color: this.state.isHidden ? 'warning' : 'success' }, this.state.visibilityState)), h('div', { class: 'flex items-center gap-sm' }, h('span', { class: 'fs-sm text-secondary' }, 'document.hidden：'), h(Tag, { color: this.state.isHidden ? 'error' : 'success' }, String(this.state.isHidden))), this.state.hiddenSince ? h('div', { class: 'fs-sm text-tertiary' }, `后台计时中，已离开 ${((Date.now() - this.state.hiddenSince) / 1000).toFixed(2)}s`) : '', h('p', { class: 'fs-sm text-tertiary' }, '监听 document.visibilitychange。切到其他标签页或最小化窗口时触发；切回会显示离开时长。'))), 
            // 2. requestIdleCallback
            h(Card, { title: '2. requestIdleCallback 空闲回调', extra: this._capTag('idleCallback') }, h('div', { class: 'flex flex-col gap-sm' }, h('div', { class: 'flex flex-wrap gap-sm' }, this._btn('调度 idle 任务', { type: 'primary', size: 'sm', onClick: () => this._runIdleTask() }), this._btn('调度 rAF 任务', { size: 'sm', onClick: () => this._runRafTask() }), this._btn('cancelIdleCallback', { size: 'sm', danger: true, onClick: () => this._cancelIdle() })), this.state.idleResults.length > 0
                ? h('div', { class: 'log-panel', style: { maxHeight: '120px' } }, ...this.state.idleResults.map((r) => h('div', { class: 'log-panel__line' }, h('span', { class: 'log-panel__time' }, r.time), h('span', { class: 'log-panel__tag log-panel__tag--push' }, r.source), h('span', {}, `timeRemaining=${r.timeRemaining.toFixed(1)}ms didTimeout=${r.didTimeout}`))))
                : h('p', { class: 'fs-sm text-tertiary' }, '对比 requestIdleCallback（低优先级，浏览器空闲时执行）与 requestAnimationFrame（下一帧执行）。Safari 不支持时降级为 setTimeout。'))), 
            // 3. Fullscreen
            h(Card, { title: '3. Fullscreen 全屏', extra: this._capTag('fullscreen') }, h('div', { class: 'flex flex-col gap-sm' }, h('div', { class: 'flex flex-wrap gap-sm' }, this._btn('进入全屏', {
            ...
            })), h('div', { class: 'flex items-center gap-sm' }, h('span', { class: 'fs-sm text-secondary' }, 'fullscreenElement：'), h(Tag, { color: this.state.isFullscreen ? 'success' : 'default' }, this.state.fullscreenElement || 'null')), h('div', {
            ...
            }, '全屏目标容器 — 点击「进入全屏」可将此区域铺满屏幕'), h('p', { class: 'fs-sm text-tertiary' }, '调用 element.requestFullscreen() 进入，document.exitFullscreen() 退出，监听 fullscreenchange 事件。'))), 
            // 4. Vibration
            h(Card, { title: '4. Vibration 震动', extra: this._capTag('vibrate') }, h('div', { class: 'flex flex-col gap-sm' }, h('div', { class: 'flex flex-wrap gap-sm' }, this._btn('vibrate(200)', {
                ...
                disabled: !caps.vibrate,
            })), h('p', { class: 'fs-sm text-tertiary' }, 'navigator.vibrate(pattern)：单次震动或 [震动,暂停,震动...] 模式；vibrate(0) 取消。多为移动端支持。'))), 
            // 5. Screen Wake Lock
            h(Card, { title: '5. Screen Wake Lock 屏幕保持唤醒', extra: this._capTag('wakeLock') }, h('div', { class: 'flex flex-col gap-sm' }, h('div', { class: 'flex items-center gap-sm' }, h('span', { class: 'fs-sm text-secondary' }, '状态：'), h(Tag, { color: this.state.wakeLockActive ? 'success' : 'default' }, this.state.wakeLockActive ? '活跃' : '未激活')), h('div', { class: 'flex flex-wrap gap-sm' }, this._btn('request(screen)', {
            ...
            })), h('p', { class: 'fs-sm text-tertiary' }, 'navigator.wakeLock.request("screen") 阻止屏幕熄灭。页面切后台时系统自动释放，切回前台会重新请求。需 HTTPS。'))), 
            // 6. Battery
            h(Card, { title: '6. Battery 电池', extra: this._capTag('battery') }, h('div', { class: 'flex flex-col gap-sm' }, batteryPct != null
                ? h('div', { class: 'api-metric', style: { fontSize: '36px' } }, `${batteryPct}%`)
                : h('div', { class: 'fs-sm text-tertiary' }, '（不支持或未连接）'), h('div', { class: 'fs-sm' }, h('div', {}, h('strong', {}, '充电：'), this.state.batteryCharging == null ? 'N/A'
                : h(Tag, { color: this.state.batteryCharging ? 'success' : 'default' }, String(this.state.batteryCharging))), h('div', { class: 'mt-sm' }, h('strong', {}, '充满还需：'), this._formatBatteryTime(this.state.batteryChargingTime)), h('div', {}, h('strong', {}, '放电时长：'), this._formatBatteryTime(this.state.batteryDischargingTime))), h('p', { class: 'fs-sm text-tertiary' }, 'navigator.getBattery() 返回 BatteryManager，监听 levelchange / chargingchange 事件。Firefox 已移除支持。'))), 
            // 7. Network Information
            h(Card, { title: '7. Network Information 网络信息', extra: this._capTag('connection') }, h('div', { class: 'flex flex-col gap-sm' }, this.state.networkType != null
                ? h('div', { class: 'flex items-center gap-sm' }, h('span', { class: 'fs-sm text-secondary' }, 'effectiveType：'), h(Tag, { color: 'primary' }, this.state.networkType))
                : h('div', { class: 'fs-sm text-tertiary' }, '（不支持）'), h('div', { class: 'fs-sm' }, h('div', {}, h('strong', {}, '下行带宽：'), this.state.networkDownlink != null ? `${this.state.networkDownlink} Mbps` : 'N/A'), h('div', { class: 'mt-sm' }, h('strong', {}, '往返时延：'), this.state.networkRtt != null ? `${this.state.networkRtt} ms` : 'N/A'), h('div', {}, h('strong', {}, '省流模式：'), this.state.networkSaveData != null ? String(this.state.networkSaveData) : 'N/A')), h('p', { class: 'fs-sm text-tertiary' }, 'navigator.connection 读取 effectiveType / downlink / rtt / saveData，监听 change 事件。'))), 
            // 8. Web Share
            h(Card, { title: '8. Web Share 系统分享', extra: this._capTag('share') }, h('div', { class: 'flex flex-col gap-sm' }, this._btn('分享当前页面', {
                type: 'primary', size: 'sm', onClick: () => this._share(),
                disabled: !caps.share,
            }), h('p', { class: 'fs-sm text-tertiary' }, `navigator.share({ title, text, url }) 调起系统分享面板。当前 URL：${window.location.href}`), h('p', { class: 'fs-sm text-tertiary' }, '需 HTTPS 且多为移动端有效；桌面端 Chrome/Edge 亦可调用。')))),
            // 事件日志
