...
export class CSSNestingScopeDeepPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!f.nestingBasic)
            this._addLog('warn', 'CSS Nesting 不可用（Chrome 112+/Firefox 117+/Safari 16.5+）');
        if (!f.atScope)
            this._addLog('warn', '@scope 不可用（Chrome 118+/Safari 17.4+）');
        if (!f.atLayer)
            this._addLog('info', '@layer 不可用（Chrome 99+/Firefox 97+/Safari 15.4+）');
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'CSS Nesting Module Level 1：.parent { color: red; & .child { } & > .direct { } &:hover { } & + .sibling { } }。& 表示父选择器引用，可嵌套任意层级。原生 vs Sass/LESS 预编译差异：原生是浏览器运行时解析、& 是选择器引用；Sass 是编译期字符串拼接。Chrome 112+/Firefox 117+/Safari 16.5+ 支持。CSS.supports(\'selector\', \'& > a\') 检测能力。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 Nesting 基础演示', { type: 'primary', size: 'sm', onClick: () => this._runNestingBasicDemo() })),
                h('div', { class: 'ns-nb-host' }, h('div', { class: 'ns-nb-parent' }, 'parent', h('div', { class: 'child' }, 'child（后代）'), h('div', { class: 'direct' }, 'direct（直接子元素）'))),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, `.parent {
  color: red;
  ...
  /* & 紧贴 = 复合选择器；& 带空格 = 后代选择器 */
                ...
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.nestingBasicInfo || '（点击按钮查看 CSS Nesting 基础与 & 父选择器完整用法）')),
            ],
        });
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '嵌套选择器必须以 & / @ / : / . / # / [ / * 开头否则需显式 &（如标签名 a 必须写 & a）。复杂选择器嵌套 & .a + .b；组合器 & + &、& ~ &；伪元素 &::before；at-rule 直接嵌套 @media / @supports / @container 无需跳出；& 在选择器中间 .a & .b 父级嵌入。解析顺序从外到内逐层展开，& 替换为累积父选择器。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行语法规则演示', { type: 'primary', size: 'sm', onClick: () => this._runSyntaxRulesDemo() })),
                h('div', { class: 'ns-sr-host' }, h('div', { class: 'ns-sr-target is-active' }, 'target（is-active + ::before）'), h('div', { class: 'ns-sr-a' }, 'item A（& + & 应用间距）'), h('div', { class: 'ns-sr-a' }, 'item B（& + & 应用间距）')),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, `.card {
  &::before { content: ""; }
  ...
  .sidebar & { width: 200px; }
                ...
                h('div', { class: 'fs-sm text-secondary' }, '完整参考：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.syntaxRulesInfo || '（点击按钮查看嵌套语法规则与解析顺序完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['@scope', f.atScope]]), h(Tag, { color: 'primary' }, 'Cascading L6')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '@scope (.host) { } 限定选择器只匹配 .host 内元素。@scope (.host) to (.limit) { } 甜甜圈模型 Donut Scope —— 从 .host 开始到 .limit 结束排除中间子树。作用域外不再受规则影响。与后代选择器 .host .child 的区别：作用域可被 scope-end 中断，后代选择器无法中断。@scope 不计入 specificity（与 .host .child 一致）。Chrome 118+/Safari 17.4+ 支持。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 @scope 基础演示', { type: 'primary', size: 'sm', onClick: () => this._runScopeBasicDemo() })),
                h('div', { class: 'ns-sb-host' }, h('div', { class: 'ns-sb-outside' }, 'outside .host（不受 @scope 影响）'), h('div', { class: 'ns-sb-target' }, '.host 内元素（受 @scope 影响）')),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, `@scope (.card) {
...
@scope (.article) to (.comments) {
  p { color: black; }        /* .article 内 .comments 之外的 p */
                ...
                h('div', { class: 'fs-sm text-secondary' }, '完整参考：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.scopeBasicInfo || '（点击按钮查看 @scope 作用域规则基础完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['@scope to()', f.atScope]]), h(Tag, { color: 'primary' }, 'Donut')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '@scope (.article) to (.comments) { p { } } —— .article 内 .comments 之外的 p 受规则影响，.comments 内 p 不受影响。多层 scope boundaries to (.a, .b, .c)。降低优先级避免深度选择器：@scope vs .article .body .content p，作用域更清晰，specificity 计算更明确。包含根边界（scope-start），排除终点（scope-end）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 Donut Scope 演示', { type: 'primary', size: 'sm', onClick: () => this._runDonutScopeDemo() })),
                h('div', { class: 'ns-ds-host' }, h('div', { class: 'ns-ds-article' }, 'article（scope-start，受规则）', h('div', { class: 'ns-ds-comments' }, 'comments（scope-end，不受规则）'))),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, `@scope (.article) to (.comments) {
  p { color: black; }
...
@scope (.article) to (.comments, .sidebar, .footer) {
  p { line-height: 1.7; }
                ...
                h('div', { class: 'fs-sm text-secondary' }, '完整参考：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.donutScopeInfo || '（点击按钮查看 Donut Scope 甜甜圈模型完整深潜）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '@scope 块内 :scope 引用作用域根元素。@scope (.card) { :scope { } :scope:hover { } :scope > .title { } }。:scope vs & 在 @scope 内的语义差异：:scope 是伪类明确表示作用域根，& 是字符串替换；结果等价但语义不同。作用域根的样式设置直接用 :scope 选择器。:scope 伪类 Chrome 27+/Firefox 32+/Safari 7+，@scope 内 :scope 语义需 Chrome 118+/Safari 17.4+。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 :scope 演示', { type: 'primary', size: 'sm', onClick: () => this._runScopePseudoDemo() })),
                h('div', { class: 'ns-sp-host' }, h('div', { class: 'ns-sp-card' }, ':scope 引用 .card（作用域根）')),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, `@scope (.card) {
  :scope {                /* .card 本身 */
    border: 1px solid;
  ...
  /* &:hover 等价 :scope:hover 但语义不同 */
                ...
                h('div', { class: 'fs-sm text-secondary' }, '完整参考：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.scopePseudoInfo || '（点击按钮查看 @scope 与 :scope 伪类完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '嵌套可与 @layer 结合 @layer base { .card { & .title { } } }。@scope 可与 @layer 结合 @layer components { @scope (.card) { } }。@scope 内嵌套 @scope (.host) { & .child { } }。组织大型样式表的三层结构：@layer（层次优先级）+ @scope（作用域范围）+ Nesting（结构组织），三者正交可组合。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行三层协同演示', { type: 'primary', size: 'sm', onClick: () => this._runLayerScopeDemo() })),
                h('div', { class: 'ns-ls-host' }, h('div', { class: 'ns-card' }, h('div', { class: 'ns-card-title' }, '组件库样式架构示例'), h('div', { class: 'ns-card-body' }, '@layer theme/components/overrides + @scope + Nesting'))),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, `@layer reset, base, components, utilities;
...
@layer components {
  @scope (.card) {
    ...
    & .body { color: gray; }
                ...
                h('div', { class: 'fs-sm text-secondary' }, '完整参考：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.layerScopeInfo || '（点击按钮查看 Nesting 与 @layer / @scope 协同完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard7() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['原生 Nesting', f.nestingBasic]]), h(Tag, { color: 'primary' }, '迁移')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '原生 Nesting 与 SCSS 的差异：& 在 SCSS 中是字符串拼接（可 BEM &__child），原生中是选择器引用（不能拼接字符串）。SCSS @at-root 跳出嵌套无原生等价。SCSS @mixin/@include 暂无原生等价（@property + @function 提案中）。SCSS interpolation #{}、控制指令 @if/@for/@each 原生不支持（需 JS 或预处理器）。迁移策略：渐进迁移 + PostCSS postcss-nesting autoprefixer。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 SCSS 迁移演示', { type: 'primary', size: 'sm', onClick: () => this._runScssMigrationDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, `/* SCSS: & 可拼接字符串（BEM） */
.parent {
...
.btn-primary { --btn-color: blue; }`)),
                h('div', { class: 'fs-sm text-secondary' }, '完整参考：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.scssMigrationInfo || '（点击按钮查看 SCSS/Less 迁移与差异完整指南）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard8() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'primary' }, '实战'), h(Tag, { color: f.nestingBasic ? 'success' : 'error' }, `Nesting ${f.nestingBasic ? '✓' : '✗'}`)),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'BEM 替代（嵌套实现模块化）/ 组件库样式组织 / 主题切换嵌套 [data-theme="dark"] { & .card { } } / 响应式嵌套 @media (max-width: 768px) { } 嵌套于组件内 / 状态机嵌套 &:hover/&.is-active/&[disabled]。陷阱清单：& 缺失导致整行被当作属性解析 / 嵌套深度过深难调试 / @scope 边界元素本身是否被包含（包含根边界排除终点）/ 老浏览器降级需 autoprefixer / 嵌套不创建新 specificity 但选择器组合仍计入 / 原生 Nesting 与 Sass 产物差异 / 浏览器支持 Chrome 112+/FF 117+/Safari 16.5+ 较新。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行实战模式演示', { type: 'primary', size: 'sm', onClick: () => this._runPatternDemo() })),
                h('div', { class: 'ns-pt-host' }, h('div', { class: 'ns-pt-card' }, h('div', { class: 'title' }, '组件库样式示例'), h('div', { class: 'body' }, '@media + &:hover + &.is-active')), h('span', { class: 'ns-state' }, 'hover'), h('span', { class: 'ns-state is-active' }, 'is-active'), h('span', { class: 'ns-state', disabled: true }, 'disabled')),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, `/* 主题切换嵌套 */
[data-theme="dark"] {
  & .card {
    background: #1e293b;
    & .title { color: #fbbf24; }
...
//# sourceMappingURL=CSSNestingScopeDeepPage.js.map