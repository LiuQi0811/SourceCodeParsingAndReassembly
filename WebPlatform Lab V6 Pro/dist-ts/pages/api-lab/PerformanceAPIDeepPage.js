...
export class PerformanceAPIDeepPage extends Page {
    ...
    componentDidMount() {
        ...
        const loafEntrySupported = this._safe(() => typeof PerformanceObserver !== 'undefined' &&
            !!PerformanceObserver.supportedEntryTypes &&
            PerformanceObserver.supportedEntryTypes.includes('long-animation-frame'));
        const loafClassSupported = this._safe(() => typeof PerformanceLongAnimationFrameEntry !== 'undefined');
        const summary = '能力检测：performance.now=' + (perfSupported ? '✓' : '✗') +
        ...
        try {
            this._visHandler = () => {
                if (typeof document !== 'undefined' && document.hidden) {
                    this._addLog('info', '页面隐藏，flush Web Vitals：' +
    ...
    _compareNow() {
        ...
        try {
            ...
            const text = 'performance.now() 增量 = ' + perfDelta.toFixed(4) + ' ms（高精度，受 timeOrigin 影响）\n' +
    ...
    _renderEntriesList() {
        ...
        return h('div', { class: 'log-panel', style: { maxHeight: '160px' } }, list.map((e, i) => h('div', { class: 'log-panel__line' }, h('span', { class: 'log-panel__tag log-panel__tag--now' }, e.type), h('span', { class: 'fs-sm' }, `[${i}] ${e.name || '(无名)'} | start=${e.startTime} | dur=${e.duration}`))));
    }
    _renderCard1() {
        const s = this.state;
        return this._card('Card 1 · Performance Timeline 基础', 'performance.now() 返回受 timeOrigin 影响的高精度单调时间戳；' +
            'getEntries / getEntriesByType(type) / getEntriesByName(name, type) 读取缓冲区条目，' +
            'entry 类型含 mark / measure / navigation / resource / paint / longtask / event / first-input / layout-shift / largest-contentful-paint。', h(Tag, { color: 'primary' }, 'Timeline'), [
            h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('performance.now() 对比 Date.now()', {
            ...
            h('pre', { class: 'code-block mt-md' }, `performance.now();              // 高精度时间戳（毫秒，亚毫秒精度）
...
performance.getEntriesByName('gap', 'measure');`),
    ...
    _renderCard2() {
        const s = this.state;
        return this._card('Card 2 · User Timing (mark / measure)', 'performance.mark(name) 创建标记，performance.measure(name, startMark, endMark) 测量区间，' +
            'clearMarks() / clearMeasures() 清除，getEntriesByType("mark"|"measure") 读取。', h(Tag, { color: 'success' }, 'User Timing'), [
            h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('mark-A → 50ms → mark-B → measure', {
            ...
            s.measureResult
                ...
                : null,
            h('pre', { class: 'code-block mt-md' }, `performance.mark('mark-A');
// ...do work...
performance.mark('mark-B');
performance.measure('gap', 'mark-A', 'mark-B');
...
performance.clearMarks(); performance.clearMeasures();`),
            ...
            const observer = new PerformanceObserver((list) => {
                try {
                    const entries = list && typeof list.getEntries === 'function'
                        ? list.getEntries() : [];
                    entries.forEach((e) => {
                        this._addLog('observer', `回调 entryType=${e.entryType} name=${e.name || '(无)'} ` +
    ...
    _renderCard3() {
        const s = this.state;
        return this._card('Card 3 · PerformanceObserver', 'new PerformanceObserver(cb) 异步订阅性能条目；observe({type, buffered:true}) 订阅单个类型并回放历史缓冲，' +
            '或 observe({entryTypes:[...]}) 一次订阅多个类型；disconnect() 停止订阅。componentWillUnmount 会自动 disconnect。', h(Tag, { color: s.observerStatus.startsWith('运行') ? 'success' : 'default' }, s.observerStatus), [
            h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('启动 observer 观察 measure', {
            ...
            h('pre', { class: 'code-block mt-md' }, `const obs = new PerformanceObserver((list) => {
...
obs.disconnect(); // 停止订阅`),
    ...
    _readNavTiming() {
        ...
        this._addLog('nav', `type=${data.type}，TTFB=${ttfb.toFixed(2)}ms，DOM解析=${domParse.toFixed(2)}ms，` +
    ...
    _renderNavTable(nav) {
        ...
        return h('div', {}, h('table', { style: { width: '100%', borderCollapse: 'collapse', marginTop: '8px' } }, h('thead', {}, h('tr', {}, h('th', { style: TH }, '阶段'), h('th', { style: TH }, '起点(ms)'), h('th', { style: TH }, '终点(ms)'), h('th', { style: TH }, '耗时(ms)'))), h('tbody', {}, ...stages.map((row) => {
            const dur = (row[2] || 0) - (row[1] || 0);
            return h('tr', {}, h('td', { style: TD }, row[0]), h('td', { style: TD }, f(row[1])), h('td', { style: TD }, f(row[2])), h('td', { style: TD }, dur.toFixed(2)));
        }))), h('div', { class: 'flex items-center gap-sm flex-wrap mt-sm' }, h(Tag, { color: 'primary' }, 'type: ' + (nav.type || '—')), h(Tag, { color: 'default' }, 'transferSize: ' + (nav.transferSize ?? '—')), h(Tag, { color: 'default' }, 'encodedBodySize: ' + (nav.encodedBodySize ?? '—')), h(Tag, { color: 'default' }, 'decodedBodySize: ' + (nav.decodedBodySize ?? '—')), h(Tag, { color: 'success' }, 'TTFB: ' + nav.ttfb.toFixed(2) + 'ms'), h(Tag, { color: 'warning' }, 'DOM解析: ' + nav.domParse.toFixed(2) + 'ms')));
    }
    _renderResourceList() {
        ...
        return h('table', { style: { width: '100%', borderCollapse: 'collapse', marginTop: '8px' } }, h('thead', {}, h('tr', {}, h('th', { style: TH }, 'name(url)'), h('th', { style: TH }, 'initiatorType'), h('th', { style: TH }, 'duration(ms)'), h('th', { style: TH }, 'transferSize'))), h('tbody', {}, ...list.map((r) => h('tr', {}, h('td', { style: TD, title: r.name }, r.name.length > 40 ? r.name.slice(0, 40) + '…' : r.name), h('td', { style: TD }, r.initiatorType || '—'), h('td', { style: TD }, (r.duration || 0).toFixed(2)), h('td', { style: TD }, r.transferSize ?? '—')))));
    }
    _renderCard4() {
        return this._card('Card 4 · Navigation Timing + Resource Timing', 'performance.getEntriesByType("navigation")[0] 得到 PerformanceNavigationTiming，' +
            '可计算 TTFB = responseStart - requestStart、DOM 解析耗时 = domComplete - responseEnd；' +
            'getEntriesByType("resource") 得到 PerformanceResourceTiming[]（name/initiatorType/duration/transferSize）。', h(Tag, { color: 'warning' }, 'Timing'), [
            h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('读取 navigation timing', {
            ...
            h('pre', { class: 'code-block mt-md' }, `const nav = performance.getEntriesByType('navigation')[0];
// PerformanceNavigationTiming
nav.type;                 // 'navigate' | 'reload' | 'back_forward'
nav.requestStart; nav.responseStart; nav.responseEnd;
(nav as any).domInteractive; nav.domComplete;
  ...
  r.duration; r.transferSize; r.decodedBodySize;
    ...
    async _measureMemory() {
        ...
        try {
            ...
            const parts = breakdown.map((b) => `${b.jsMemoryType || b.type || '?'}=${b.bytes || 0}`);
    ...
    _renderCard5() {
        ...
        return this._card('Card 5 · Web Vitals (LCP / CLS / FID / Long Task) + measureMemory', 'LCP 观察 largest-contentful-paint 的 startTime；CLS 累加 layout-shift 的 value（!hadRecentInput）；' +
            ...
            'performance.measureUserAgentSpecificMemory!() 需 crossOriginIsolated。', h(Tag, { color: s.vitalsStatus.startsWith('运行') ? 'success' : 'default' }, s.vitalsStatus), [
            h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, h(Tag, { color: 'primary' }, 'deviceMemory: ' + deviceMemory), h(Tag, { color: coi ? 'success' : 'warning' }, 'crossOriginIsolated: ' + (coi ? 'true' : 'false'))),
            h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('启动 LCP/CLS/FID/longtask 四合一', {
            ...
            h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, h(Tag, { color: 'success' }, 'LCP: ' + (this._lcp != null ? this._lcp.toFixed(2) + 'ms' : '—')), h(Tag, { color: 'warning' }, 'CLS: ' + ((this._cls || 0).toFixed(4))), h(Tag, { color: 'primary' }, 'FID: ' + (this._fid != null ? this._fid.toFixed(2) + 'ms' : '—'))),
            s.memoryResult
                ? h('pre', { class: 'code-block mt-md' }, s.memoryResult)
                : null,
            h('pre', { class: 'code-block mt-md' }, `// LCP
...
performance.measureUserAgentSpecificMemory!(); // 需 crossOriginIsolated`),
    ...
    _runLoAFDemo() {
        ...
        const supported = this._safe(() => PerformanceObserver.supportedEntryTypes &&
            PerformanceObserver.supportedEntryTypes.includes('long-animation-frame'));
        if (!supported) {
    ...
    _renderLoAFInfo() {
        ...
        if (!info) {
            return h('div', { class: 'log-panel__empty' }, '（暂无 LoAF entry，点击上方按钮启动观察器并制造长任务以触发回调）');
        }
        const TD = { padding: '4px 8px', borderBottom: '1px solid var(--color-border, #f0f0f0)', fontSize: '12px' };
        const TH = { padding: '4px 8px', borderBottom: '1px solid var(--color-border, #eee)', textAlign: 'left', fontSize: '12px' };
        ...
        return h('div', {}, h('div', { class: 'flex items-center gap-sm flex-wrap mt-sm' }, h(Tag, { color: 'warning' }, 'duration: ' + f(info.duration)), h(Tag, { color: 'danger' }, 'blockingDuration: ' + f(info.blockingDuration)), h(Tag, { color: 'primary' }, 'renderStart: ' + f(info.renderStart)), h(Tag, { color: 'primary' }, 'styleAndLayoutStart: ' + f(info.styleAndLayoutStart)), h(Tag, { color: 'default' }, 'firstUIEvent: ' + f(info.firstUIEventTimestamp)), h(Tag, { color: 'default' }, 'startTime: ' + f(info.startTime))), info.scripts && info.scripts.length > 0
            ? h('table', { style: { width: '100%', borderCollapse: 'collapse', marginTop: '8px' } }, h('thead', {}, h('tr', {}, h('th', { style: TH }, 'name(url)'), h('th', { style: TH }, 'duration'), h('th', { style: TH }, 'durationType'), h('th', { style: TH }, 'invoker / invokerType'), h('th', { style: TH }, 'involuntaryType'))), h('tbody', {}, ...info.scripts.map((sc) => h('tr', {}, h('td', { style: TD, title: sc.name || '' }, trimName(sc.name)), h('td', { style: TD }, f(sc.duration)), h('td', { style: TD }, sc.durationType || '—'), h('td', { style: TD }, (sc.invoker || '—') + ' / ' + (sc.invokerType || '—')), h('td', { style: TD }, sc.involuntaryType || '—')))))
            : h('div', { class: 'fs-sm text-secondary mt-sm' }, '（本帧无 scripts 归因）'));
    }
    _renderCard6() {
        const s = this.state;
        const supported = this._safe(() => typeof PerformanceObserver !== 'undefined' &&
            !!PerformanceObserver.supportedEntryTypes &&
            PerformanceObserver.supportedEntryTypes.includes('long-animation-frame'));
        const entryClass = this._safe(() => typeof PerformanceLongAnimationFrameEntry !== 'undefined');
        return this._card('Card 6 · Long Animation Frames API (LoAF)', 'LoAF (Long Animation Frames) 是 PerformanceLongTask 的后继者，观测 >50ms 的长动画帧（含渲染阶段），' +
            ...
            '是 INP (Interaction to Next Paint) 调优的核心工具。', h(Tag, { color: supported ? 'success' : 'warning' }, 'LoAF: ' + (supported ? 'supported' : 'unsupported')), [
            h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, h(Tag, { color: supported ? 'success' : 'default' }, 'supportedEntryTypes 含 long-animation-frame: ' + (supported ? '✓' : '✗')), h(Tag, { color: entryClass ? 'success' : 'default' }, 'PerformanceLongAnimationFrameEntry: ' + (entryClass ? '✓' : '✗')), h(Tag, { color: s.loafStatus.startsWith('运行') ? 'success' : 'default' }, s.loafStatus)),
            h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('启动 LoAF 观察器 + 制造长任务', {
            ...
            h('pre', { class: 'code-block mt-md' }, `// LoAF：观测 >50ms 长动画帧（PerformanceLongAnimationFrameEntry）
...
// 5. 与 longtask 的区别：LoAF 含渲染阶段归因，longtask 仅给 duration 无脚本归因`),
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'page api-lab-page' }, h('h2', { class: 'section-title' }, 'Performance 性能监控实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, '演示 Performance Timeline / User Timing / PerformanceObserver / Navigation & Resource Timing / Web Vitals（LCP/CLS/FID/Long Task）/ Long Animation Frames (LoAF)。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderCard6(), this._renderLogPanel());
    }
}
//# sourceMappingURL=PerformanceAPIDeepPage.js.map