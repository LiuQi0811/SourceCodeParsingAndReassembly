...
export class AdvancedObserverPage extends Page {
    ...
    componentDidMount() {
        ...
        this._clickHandler = ((e) => this._onClickHit(e)); // Card 4 演示区 click 处理器
        ...
        const summary = '能力检测：' + Object.entries(caps)
            .map(([k, v]) => `${k}=${v ? '✓' : '✗'}`).join('，');
        this.setState({ capsSummary: summary });
        this._addLog('cap', summary);
        if (!caps.IntersectionObserver)
            this._addLog('warn', 'IntersectionObserver 不可用（observe 调用会被忽略，回调不触发）');
        if (!caps.ResizeObserver)
            this._addLog('warn', 'ResizeObserver 不可用（observe 调用会被忽略，回调不触发）');
        if (!caps.MutationObserver)
            this._addLog('warn', 'MutationObserver 不可用（observe 调用会被忽略，回调不触发）');
        if (!caps.elementFromPoint)
            this._addLog('warn', 'document.elementFromPoint 不可用，命中测试演示将跳过');
        if (!caps.elementsFromPoint)
            this._addLog('warn', 'document.elementsFromPoint 不可用，列表演示将跳过');
        if (!caps.PerformanceObserver)
            this._addLog('warn', 'PerformanceObserver 不可用，longtask 监听将跳过');
    }
    componentWillUnmount() {
        ...
        try {
            if (this._clickHandler) {
                ...
                if (area)
                    area.removeEventListener('click', this._clickHandler);
            }
        }
        catch { /* noop */ }
    }
    ...
    _startIoObserver() {
        ...
        try {
            ...
            if (!target) {
                this._addLog('warn', '未找到 #io-target 演示元素');
                return;
            }
            const observer = new IntersectionObserver((entries) => {
                try {
                    entries.forEach((e) => {
                        ...
                        this._addLog('entry', `isIntersecting=${e.isIntersecting} ratio=${(e.intersectionRatio * 100).toFixed(0)}% ` +
    ...
    _setIoRootMargin() {
        if (typeof IntersectionObserver === 'undefined') {
            this._addLog('warn', 'IntersectionObserver 不可用，无法设置 rootMargin');
            return;
        }
        if (this._io) {
            try {
    ...
    _ioTakeRecords() {
        if (typeof IntersectionObserver === 'undefined') {
            this._addLog('warn', 'IntersectionObserver 不可用，takeRecords 无效');
            return;
        }
        if (!this._io) {
            this._addLog('warn', '请先点击「启动观察」');
            return;
        }
        try {
            ...
            if (records.length === 0)
                this._addLog('io', '（测试环境观察器已 mock，takeRecords 返回空数组，真实浏览器可见）');
        }
        catch (err) {
    ...
    _renderCard1() {
        const s = this.state;
        return this._card('Card 1 · IntersectionObserver 深入选项', 'new IntersectionObserver(cb, options)：options.root（默认 null=视口）、rootMargin（\'10px 20px\' 形式，可为负）、' +
            'threshold（0~1 单值或数组 [0, 0.25, 0.5, 1]）。entries[] 含 isIntersecting / intersectionRatio / ' +
            'intersectionRect / boundingClientRect / rootBounds / time / target。滚动下方容器可观察目标进入/离开。', h(Tag, { color: 'primary' }, 'Intersection'), [
            h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('启动观察（threshold 数组）', {
            ...
            s.ioEntries.length > 0
                ? h('div', { class: 'log-panel mt-md', style: { maxHeight: '120px' } }, s.ioEntries.map((e, i) => h('div', { class: 'log-panel__line' }, h('span', { class: 'log-panel__tag log-panel__tag--entry' }, 'entry'), h('span', { class: 'fs-sm' }, `[${i}] isIntersecting=${e.isIntersecting} ratio=${(e.ratio * 100).toFixed(0)}% time=${(e.time || 0).toFixed(0)}ms`))))
                : h('div', { class: 'log-panel__empty mt-md' }, '（暂无 entry，点击「启动观察」后滚动容器）'),
            h('pre', { class: 'code-block mt-md' }, `const io = new IntersectionObserver((entries) => {
  entries.forEach((e) => {
...
io.takeRecords(); io.disconnect();`),
    ...
    _startRoObserver() {
        ...
        try {
            const target = this.$('#ro-target');
            if (!target) {
                this._addLog('warn', '未找到 #ro-target 演示元素');
                return;
            }
            const observer = new ResizeObserver((entries) => {
                try {
                    entries.forEach((e) => {
                        ...
                        const size = {
                            contentWidth: e.contentRect ? Math.round(e.contentRect.width) : null,
                            contentHeight: e.contentRect ? Math.round(e.contentRect.height) : null,
                            borderInline: borderBox ? Math.round(borderBox.inlineSize) : null,
                            borderBlock: borderBox ? Math.round(borderBox.blockSize) : null,
                            contentInline: contentBox ? Math.round(contentBox.inlineSize) : null,
                        };
                        this.setState({ roSize: size });
                        this._addLog('size', `borderBox inline=${size.borderInline} block=${size.borderBlock} | ` +
    ...
    _toggleRoBox() {
        ...
        if (typeof ResizeObserver === 'undefined') {
            this._addLog('warn', 'ResizeObserver 不可用，仅切换状态：box=' + this._roBox);
            return;
        }
        if (!this._ro) {
            this._addLog('warn', '请先点击「启动 ResizeObserver」（当前 box=' + this._roBox + '）');
            return;
        }
        try {
            const target = this.$('#ro-target');
            ...
            try {
                this._ro.observe(target, { box: this._roBox });
            }
            catch {
                try {
    ...
    _renderCard2() {
        const s = this.state;
        const size = s.roSize;
        return this._card('Card 2 · ResizeObserver 深入 + borderBoxSize', 'new ResizeObserver(cb)；observer.observe(target, options?)，options.box = \'content-box\' | \'border-box\'（默认 content-box）。' +
            'entries[] 含 target / contentRect（旧，已废弃但仍可用）/ borderBoxSize[]（[{inlineSize, blockSize}]）/ ' +
            'contentBoxSize[] / devicePixelContentBoxSize[]。点击「制造尺寸变化」改 width 观察实时输出。', h(Tag, { color: 'warning' }, 'Resize'), [
            h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('启动 ResizeObserver', {
            ...
            h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, h(Tag, { color: 'default' }, "box: " + this._roBox), size
                ? h(Tag, { color: 'success' }, 'borderBox: ' + (size.borderInline != null ? size.borderInline + '×' + size.borderBlock : '—'))
                : h(Tag, { color: 'default' }, 'borderBox: —'), size
                ? h(Tag, { color: 'primary' }, 'contentRect: ' + (size.contentWidth != null ? size.contentWidth + '×' + size.contentHeight : '—'))
                : null),
            h('div', {
                id: 'ro-target',
                style: {
            ...
            h('pre', { class: 'code-block mt-md' }, `const ro = new ResizeObserver((entries) => {
  entries.forEach((e) => {
...
ro.observe(target, { box: 'border-box' }); ro.disconnect();`),
    ...
    _startMoObserver() {
        ...
        try {
            const target = this.$('#mo-target');
            if (!target) {
                this._addLog('warn', '未找到 #mo-target 演示元素');
                return;
            }
                ...
                try {
                    ...
                    for (const m of records) {
                        ...
                        if (m.type === 'attributes') {
                            this._addLog('mut', `attr ${m.attributeName} → ${m.target.getAttribute(m.attributeName)}` +
                                (m.oldValue != null ? `（旧值=${m.oldValue}）` : ''));
                        }
                        else if (m.type === 'childList') {
                            this._addLog('mut', `childList +${snap.added} -${snap.removed}` +
    ...
    _triggerMoChild() {
        ...
        try {
            ...
            if (target.children.length > 5 && target.lastChild)
    ...
    _moTakeRecords() {
        if (typeof MutationObserver === 'undefined') {
            this._addLog('warn', 'MutationObserver 不可用，takeRecords 无效');
            return;
        }
        if (!this._mo) {
            this._addLog('warn', '请先点击「启动 MutationObserver」');
            return;
        }
        try {
            ...
            if (records.length === 0)
                this._addLog('mo', '（测试环境观察器已 mock，takeRecords 返回空数组，真实浏览器可见）');
        }
        catch (err) {
            this._addLog('warn', 'takeRecords 失败：' + (err && err.message));
        }
    ...
    _renderCard3() {
        const s = this.state;
        return this._card('Card 3 · MutationObserver 深入 + subtree/attributeFilter', 'new MutationObserver(cb)；observer.observe(target, options)。options：childList（子节点增删）/ attributes（属性变化）/ ' +
            'subtree（含后代）/ attributeOldValue / characterData / characterDataOldValue / attributeFilter([\'class\',\'data-x\'] 仅观察指定属性)。' +
            'records[] 含 type / target / addedNodes / removedNodes / attributeName / oldValue / nextSibling / previousSibling。', h(Tag, { color: 'success' }, 'Mutation'), [
            h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('启动 MO（subtree + attributeFilter）', {
            ...
            s.moRecords.length > 0
                ? h('div', { class: 'log-panel mt-md', style: { maxHeight: '120px' } }, s.moRecords.map((r, i) => h('div', { class: 'log-panel__line' }, h('span', { class: 'log-panel__tag log-panel__tag--mut' }, r.type), h('span', { class: 'fs-sm' }, `[${i}] ${r.type} target=${r.target}` +
            ...
            h('pre', { class: 'code-block mt-md' }, `const mo = new MutationObserver((records) => {
  for (const m of records) {
...
mo.takeRecords(); mo.disconnect();`),
    ...
    _targetCenter() {
        ...
        if (!target) {
            this._addLog('warn', '未找到 #hit-target 演示元素');
            return null;
        }
        const rect = typeof target.getBoundingClientRect === 'function'
            ? target.getBoundingClientRect() : null;
        if (!rect) {
            this._addLog('warn', 'getBoundingClientRect 不可用');
            return null;
        }
    ...
    _hitFromCenter() {
        ...
        if (typeof document === 'undefined' || typeof document.elementFromPoint !== 'function') {
            this._addLog('warn', 'document.elementFromPoint 不可用，命中测试跳过');
            return;
        }
        ...
        try {
            ...
            this.setState({
                hitResult: {
                    source: 'center', x: c.cx.toFixed(1), y: c.cy.toFixed(1), tag,
                    rect: { w: c.rect.width.toFixed(0), h: c.rect.height.toFixed(0) }, contains,
                },
            });
            this._addLog('hit', `elementFromPoint(中心 ${c.cx.toFixed(0)}, ${c.cy.toFixed(0)}) → ${tag}（target.contains=${contains}）`);
            if (!el)
                this._addLog('hit', '（jsdom 无布局引擎，elementFromPoint 可能返回 null，真实浏览器可见）');
        }
        catch (err) {
    ...
    _elementsFromPointList() {
        ...
        if (typeof document === 'undefined' || typeof document.elementsFromPoint !== 'function') {
            this._addLog('warn', 'document.elementsFromPoint 不可用，列表演示跳过');
            return;
        }
        const c = this._targetCenter();
        if (!c)
        ...
        try {
            ...
            const names = list.map((el) => el ? (el.tagName + (el.id ? '#' + el.id : '') + (el.className ? '.' + String(el.className).split(/\s+/)[0] : '')) : '?');
            this.setState({ hitResult: { source: 'list', x: c.cx.toFixed(1), y: c.cy.toFixed(1), count: list.length, names } });
            this._addLog('point', `elementsFromPoint(${c.cx.toFixed(0)}, ${c.cy.toFixed(0)}) → ${list.length} 个元素：` +
    ...
    _renderHitResult() {
        ...
        if (!r)
            return h('div', { class: 'log-panel__empty mt-md' }, '（暂无命中结果，点击演示区或上方按钮）');
        let text;
        if (r.source === 'list') {
            text = `elementsFromPoint(${r.x}, ${r.y}) → ${r.count} 个元素（从顶到底）：\n` +
                (r.names && r.names.length ? r.names.map((n, i) => `  [${i}] ${n}`).join('\n') : '  （空，jsdom 无布局引擎时常见）');
        }
    ...
    _renderCard4() {
        return this._card('Card 4 · elementFromPoint / elementsFromPoint + 命中测试', 'document.elementFromPoint(x, y) 返回该坐标最顶层元素；document.elementsFromPoint(x, y) 返回所有重叠元素数组（从顶到底）。' +
            'element.getBoundingClientRect() 获取坐标，element.contains(node) 检查包含关系。' +
            '点击下方演示区，或点击按钮获取 #hit-target 中心点的命中结果。', h(Tag, { color: 'primary' }, 'Hit Test'), [
            h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('获取中心点 elementFromPoint', {
            ...
            }, '点击此处或周围'), h('div', { class: 'fs-sm text-secondary mt-sm' }, '↑ 点击本区域内任意位置进行命中测试')),
            this._renderHitResult(),
            h('pre', { class: 'code-block mt-md' }, `const el = document.elementFromPoint(x, y);    // 最顶层元素
...
target.contains(node);                            // 是否包含某节点`),
    ...
    _startCombo() {
        ...
        const target = this.$('#combo-target');
        if (!target) {
            this._addLog('warn', '未找到 #combo-target 演示元素');
            return;
        }
        // 重置统计
        this._ioCount = 0;
        ...
        const bump = (key, label) => {
            this[key]++;
            this._addLog('combo', `${label} 触发（累计 ${this[key]}）`);
            this._updateComboStats();
        };
        try {
            if (ioOk) {
                this._comboIo = new IntersectionObserver((es) => {
                    try {
                        es.forEach(() => bump('_ioCount', 'Intersection'));
                    }
                    catch { /* noop */ }
                }, { threshold: [0, 0.5, 1] });
                try {
                    this._comboIo.observe(target);
                }
                catch { /* noop */ }
            }
            if (roOk) {
                this._comboRo = new ResizeObserver((es) => {
                    try {
                        es.forEach(() => bump('_roCount', 'Resize'));
                    }
                    catch { /* noop */ }
                });
                try {
                    this._comboRo.observe(target);
                }
                catch { /* noop */ }
            }
            if (moOk) {
                this._comboMo = new MutationObserver((records) => {
                    try {
                ...
                try {
                    this._comboMo.observe(target, { childList: true, attributes: true, subtree: true, attributeOldValue: true });
                }
                catch { /* noop */ }
            }
            ...
            if (poOk) {
                try {
                    this._comboPerfObs = new PerformanceObserver((list) => {
                        try {
                    ...
                    try {
                        this._comboPerfObs.observe({ type: 'longtask', buffered: true });
                    }
                    catch {
                        try {
                            this._comboPerfObs.observe({ entryTypes: ['longtask'] });
                        }
                        catch { /* noop */ }
                    }
                }
            ...
            this._addLog('combo', '三合一观察器已启动：Intersection + Resize + Mutation' +
    ...
    _triggerComboChange() {
        ...
        try {
            ...
            if (target.children.length > 6 && target.firstChild)
    ...
    _comboStatsLog() {
        if (!this._comboIo && !this._comboRo && !this._comboMo) {
            this._addLog('warn', '请先点击「启动三合一观察」');
            return;
        }
        this._updateComboStats();
        this._addLog('stat', `统计 → Intersection=${this._ioCount}，Resize=${this._roCount}，` +
    ...
    _comboDisconnect(silent) {
        ...
        if (!silent) {
            if (any)
                this._addLog('combo', '已 disconnect 全部观察器（Intersection/Resize/Mutation/longtask）');
            else
                this._addLog('warn', '无正在运行的组合观察器');
        }
    }
    _renderCard5() {
        const s = this.state;
        const st = s.comboStats;
        return this._card('Card 5 · 三观察器组合 + PerformanceObserver 监听长任务', '同时启动 Intersection + Resize + Mutation 观察同一容器，配合 PerformanceObserver 观察 longtask（若可用）。' +
            '统计各自触发次数，对比开销。点击「触发综合变化」对容器做一次"改属性 + 改尺寸 + 插入子节点"，三个观察器各触发几次。', h(Tag, { color: st ? 'success' : 'default' }, st ? '运行中' : '未启动'), [
            h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('启动三合一观察', {
            ...
            h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, h(Tag, { color: 'primary' }, 'Intersection: ' + (st ? st.io : 0)), h(Tag, { color: 'warning' }, 'Resize: ' + (st ? st.ro : 0)), h(Tag, { color: 'success' }, 'Mutation: ' + (st ? st.mo : 0)), h(Tag, { color: 'default' }, 'longtask: ' + (st ? st.longtask : 0))),
            h('div', {
            ...
            h('pre', { class: 'code-block mt-md' }, `// 三观察器组合 + longtask
...
io.disconnect(); ro.disconnect(); mo.disconnect(); // 对比开销后释放`),
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'page api-lab-page' }, h('h2', { class: 'section-title' }, '高级观察器与命中测试实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, '演示 IntersectionObserver/ResizeObserver/MutationObserver 深入选项 + elementFromPoint 命中测试 + 三观察器组合。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderLogPanel());
    }
}
//# sourceMappingURL=AdvancedObserverPage.js.map