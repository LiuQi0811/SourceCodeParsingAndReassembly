...
export class ES2025LanguagePage extends Page {
    ...
    componentDidMount() {
        ...
        if (!caps.asyncIter)
            this._addLog('warn', 'AsyncIterator helpers 不可用（AsyncIterator 全局对象或 .map 不存在），ES2025 提案；同步版 Iterator helpers 已在 ModernES2024Page 演示');
        if (!caps.shadowRealm)
            this._addLog('warn', 'ShadowRealm 不可用，TC39 Stage 3 提案');
        if (!caps.errorIsError)
            this._addLog('warn', 'Error.isError 不可用，ES2025 提案');
        if (!caps.regExpEscape)
            this._addLog('warn', 'RegExp.escape 不可用，ES2025 提案');
        if (!caps.mathF16round)
            this._addLog('warn', 'Math.f16round 不可用，ES2025 提案');
        if (!caps.float16Array)
            this._addLog('warn', 'Float16Array 不可用，ES2025 提案');
        if (!caps.suppressedError)
            this._addLog('warn', 'SuppressedError 不可用，ES2025/2026 ERM 提案');
    ...
    async _asyncIterChain() {
        ...
        if (!caps.asyncIter) {
            ...
            try {
                const gen = async function* () { for (let i = 1; i <= 8; i++)
                    yield i; };
                // 等价于 .map((x: any) => x*2).filter((x: any) => x > 5).take(3)
                const collected = [];
                let count = 0;
                ...
                this.setState({
                    asyncIterResult: `【模拟模式】AsyncIterator helpers 不可用，用 async generator + 手动 for await 模拟\n\n` +
                        `目标等价链：gen().map((x: any) => x*2).filter((x: any) => x>5).take(3).toArray()\n\n` +
        ...
        try {
            ...
            const asyncGen = async function* () { for (let i = 1; i <= 8; i++)
                yield i; };
            // 链式：×2 → 保留 >5 → 取前 3 → 转数组
            ...
            this.setState({
                asyncIterResult: `asyncGen() = async function*() { for (i=1..8) yield i; }\n\n` +
                    `.map((x: any) => x*2).filter((x: any) => x>5).take(3).toArray()\n` +
                    `  await chain.toArray() = ${JSON.stringify(arr)}（= [6,8,10]）\n\n` +
                    `惰性求值验证：(asyncGen() as any).map(×10).take(2).toArray() = ${JSON.stringify(lazy)}\n` +
                    `  map 回调仅被调用 ${consumed} 次（take(2) 取够即停，剩余元素不计算）\n\n` +
                    `drop(n) + take(n)：(asyncGen() as any).drop(5).take(2).toArray() = ${JSON.stringify(dropped)}\n\n` +
    ...
    async _asyncIterFrom() {
        ...
        try {
            ...
            this.setState({
                asyncIterResult: `自定义异步可迭代对象 asyncRange = { from:1, to:5, [Symbol.asyncIterator]() {...} }\n` +
                    `AsyncIterator.from(asyncRange) → AsyncIterator 实例（instanceof AsyncIterator = ${isInst}）\n` +
                    `  await it.map((x: any) => x*x).toArray() = ${JSON.stringify(squared)}\n\n` +
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.asyncIter ? 'success' : 'error' }, caps.asyncIter ? 'AsyncIterator ✓' : '不可用'), h(Tag, { color: 'primary' }, '异步惰性链式')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'AsyncIterator.prototype 新增 map / filter / take / drop / toArray 等方法，返回 AsyncIterator 实例（非数组），惰性求值——只有调用 await toArray() 才真正计算。对应 ES2024 同步 Iterator helpers（已在 ModernES2024Page 演示），区别在于回调可为 async、迭代源为 async iterable。AsyncIterator.from(asyncIterable) 把任意异步可迭代对象转为 AsyncIterator。检测方式：typeof AsyncIterator !== "undefined" && typeof AsyncIterator.prototype.map === "function"（注意 AsyncIterator 在老引擎非全局对象，typeof 直接引用会抛 ReferenceError，需 try/catch）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 AsyncIterator helpers', { type: 'primary', size: 'sm', onClick: () => this._asyncIterChain() }), this._btn('AsyncIterator.from', { size: 'sm', disabled: !caps.asyncIterFrom, onClick: () => this._asyncIterFrom() })),
                h('div', { class: 'fs-sm text-secondary' }, 'AsyncIterator 演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '240px', overflow: 'auto' } }, h('code', {}, s.asyncIterResult || '（点击按钮演示 AsyncIterator helpers；不可用时将用普通 async generator 模拟）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '140px', overflow: 'auto' } }, h('code', {}, `const gen = async function*() { for (let i=1; i<=8; i++) yield i; };
// 链式：×2 → 保留 >5 → 取前 3 → await toArray()
const r = await gen().map((x: any) => x*2).filter((x: any) => x>5).take(3).toArray();
// r = [6, 8, 10]
const it = AsyncIterator.from(asyncIterable);   // ES2025 静态方法
const sq = await it.map((x: any) => x*x).toArray();     // Promise<Array>`)),
                h(Alert, {
                    type: 'info',
    ...
    _createShadowRealm() {
        ...
        if (!caps.shadowRealm) {
            ...
            this.setState({
                shadowRealmResult: `ShadowRealm 不可用（TC39 Stage 3 提案，部分浏览器/Node 尚未实现）\n\n` +
        ...
        try {
            ...
            this.setState({
                shadowRealmResult: `const realm = new ShadowRealm()\n\n` +
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.shadowRealm ? 'success' : 'error' }, caps.shadowRealm ? 'ShadowRealm ✓' : 'Stage 3 ✗'), h(Tag, { color: 'primary' }, '沙箱')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'ShadowRealm 提供独立的 JS 执行环境，拥有自己的全局对象、内置函数（Date/Math/JSON 等）与原型链，与主 realm 完全隔离。new ShadowRealm() 创建实例；realm.evaluate(code) 同步执行表达式返回原始值或 callable wrapper（不能直接返回对象）；realm.importValue(specifier, exportName) 异步导入模块导出（返回 Promise）。vs Worker：同步、轻量、无 DOM；vs iframe：无 DOM/window、开销小。检测方式：typeof ShadowRealm !== "undefined"。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('创建 ShadowRealm', { type: 'primary', size: 'sm', disabled: !caps.shadowRealm, onClick: () => this._createShadowRealm() })),
                h('div', { class: 'fs-sm text-secondary' }, 'ShadowRealm 演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '240px', overflow: 'auto' } }, h('code', {}, s.shadowRealmResult || '（点击按钮演示 ShadowRealm；不可用时将显示概念说明）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '140px', overflow: 'auto' } }, h('code', {}, `const realm = new ShadowRealm();
...
const fn = await realm.importValue('./mod.js', 'default');`)),
    ...
    _testErrorIsError() {
        ...
        if (!caps.errorIsError) {
            ...
            try {
                ...
                this.setState({
                    errorRegExpResult: `【模拟模式】Error.isError 不可用（ES2025 提案），展示跨 realm 判定的痛点\n\n` +
                        `const e = new Error('boom')\n` +
        ...
        try {
            ...
            this.setState({
                errorRegExpResult: `Error.isError(x) → boolean：跨 realm 可靠判断是否为 Error 实例\n\n` +
                    checks.join('\n') + '\n\n' +
    ...
    _testRegExpEscape() {
        ...
        if (!caps.regExpEscape) {
            ...
            try {
                ...
                this.setState({
                    errorRegExpResult: (this.state.errorRegExpResult ? this.state.errorRegExpResult + '\n\n' : '') +
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.errorIsError ? 'success' : 'error' }, caps.errorIsError ? 'Error.isError ✓' : 'Error.isError ✗'), h(Tag, { color: caps.regExpEscape ? 'success' : 'error' }, caps.regExpEscape ? 'RegExp.escape ✓' : 'RegExp.escape ✗')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'Error.isError(x) → boolean：跨 realm 可靠判断是否为 Error 实例（内部检查 [[ErrorData]] 内部槽），vs instanceof Error 在跨 realm（ShadowRealm/iframe/Worker）时因原型链指向不同构造器而失效。RegExp.escape(str) → string：转义所有正则元字符（. * + ? ^ $ { } ( ) | [ ] \\），安全把用户输入作为字面字符拼接到正则，避免 ReDoS 与误匹配。检测：typeof Error.isError === "function" / typeof RegExp.escape === "function"。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('测试 Error.isError', { type: 'primary', size: 'sm', onClick: () => this._testErrorIsError() }), this._btn('测试 RegExp.escape', { type: 'primary', size: 'sm', onClick: () => this._testRegExpEscape() })),
                h('div', { class: 'fs-sm text-secondary' }, 'Error.isError / RegExp.escape 演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, s.errorRegExpResult || '（点击按钮演示；不可用时将手动模拟并说明）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '140px', overflow: 'auto' } }, h('code', {}, `// Error.isError：跨 realm 可靠判断
...
re.test('xa.b*cy');  // true（字面匹配，元字符被转义）`)),
    ...
    _float16Demo() {
        ...
        if (!caps.mathF16round && !caps.float16Array) {
            ...
            try {
                ...
                const bytesF16 = 2;
                this.setState({
                    float16Result: `【兜底模式】Math.f16round / Float16Array 不可用（ES2025 提案），用 Math.fround 演示概念\n\n` +
                        `三种浮点格式对比（以 x = ${x} 为例）：\n` +
        ...
        try {
            ...
            this.setState({
                float16Result: `Math.f16round(x)：四舍五入到 IEEE 754 半精度浮点（16-bit）\n` +
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.mathF16round ? 'success' : 'error' }, caps.mathF16round ? 'Math.f16round ✓' : 'Math.f16round ✗'), h(Tag, { color: caps.float16Array ? 'success' : 'error' }, caps.float16Array ? 'Float16Array ✓' : 'Float16Array ✗')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'Math.f16round(x) → number：四舍五入到 IEEE 754 半精度浮点（16-bit），对应 Math.fround（Float32）与 Math.round（整数）。Float16Array：半精度类型化数组，每元素 2 字节（vs Float32Array 4 字节、Float64Array 8 字节），与两者接口完全一致。典型用途：ML 推理（推理阶段对精度不敏感）、WebGPU 纹理/顶点数据交换（GPU 原生支持 half-float）。检测：typeof Math.f16round === "function" / typeof Float16Array !== "undefined"。不可用时用 Math.fround（Float32）兜底演示"精度 vs 内存"概念。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('半精度浮点演示', { type: 'primary', size: 'sm', onClick: () => this._float16Demo() })),
                h('div', { class: 'fs-sm text-secondary' }, 'Float16 演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, s.float16Result || '（点击按钮演示；不可用时将用 Math.fround 兜底演示概念）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '140px', overflow: 'auto' } }, h('code', {}, `Math.f16round(1.1);     // 半精度舍入（Float32 用 Math.fround）
Math.f16round(Math.PI); // 比 Float32 精度更低，但占 2 字节
const arr = new Float16Array([1.5, 2.5, 3.14159]);
arr.BYTES_PER_ELEMENT;  // 2（Float32Array=4，Float64Array=8）
arr.reduce((a: any, b: any) =>a+b,0); // 接口与 Float32Array 完全一致`)),
                h(Alert, {
                    type: 'info',
    ...
    _suppressedErrorDemo() {
        ...
        if (!caps.suppressedError) {
            ...
            try {
                ...
                this.setState({
                    suppressedResult: `【模拟模式】SuppressedError 不可用（ES2025/2026 ERM 提案），演示语义\n\n` +
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.suppressedError ? 'success' : 'error' }, caps.suppressedError ? 'SuppressedError ✓' : 'SuppressedError ✗'), h(Tag, { color: caps.disposableStack ? 'success' : 'error' }, caps.disposableStack ? 'DisposableStack ✓' : 'DisposableStack ✗')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'SuppressedError：using/await using 声明的资源在块结束时自动 [Symbol.dispose]()；若块内抛了"业务异常"，dispose 又抛了"清理异常"，则业务异常被抑制，ES2025 用 new SuppressedError(error, suppressed, message) 把两个异常包装在一起——error 为后抛出的（dispose）异常，suppressed 为被抑制的原业务异常，通过 err.suppressed 访问，避免异常丢失。DisposableStack/using 已在 AdvancedJSRuntimePage 演示，本卡聚焦 SuppressedError 语义。检测：typeof SuppressedError !== "undefined" / typeof DisposableStack !== "undefined"。注意：using 是语法关键字无法运行时 typeof 检测，需编译或原生支持。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('SuppressedError 演示', { type: 'primary', size: 'sm', onClick: () => this._suppressedErrorDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, 'SuppressedError 演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '280px', overflow: 'auto' } }, h('code', {}, s.suppressedResult || '（点击按钮演示；不可用时将模拟语义并说明）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '150px', overflow: 'auto' } }, h('code', {}, `// using 声明：块结束时自动 dispose；若同时抛业务异常 + dispose 异常 → SuppressedError
...
// err.message === 'cleanup failed'（dispose 异常作为主错误）`)),
    ...
    _manualDecorators() {
        try {
            ...
            this.setState({
                decoratorResult: `Decorators 是 TC39 Stage 3 语法特性，无法运行时 typeof 检测\n` +
                    ...
                    `const logged = (target: any, context: any) => (...args) => {\n` +
                    ...
                    `const memoize = (target: any, context: any) => {\n` +
                    `  const cache = new Map();\n` +
                    `  return (...args) => {\n` +
                    `    const key = JSON.stringify(args);\n` +
                    ...
                    `const bound = (target: any, context: any) => function(...args: any[]) { return target.apply(this, args); };\n` +
                    ...
                    `  Stage 3 提案签名：(target: any, context: any) => newTarget | void\n` +
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'warning' }, '语法特性 · 无法 typeof'), h(Tag, { color: 'primary' }, 'Stage 3')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'Decorators 是 TC39 Stage 3 语法层面特性，用 @expr 修饰类/方法/字段/getter/setter，编译时展开为高阶函数调用。无法运行时 typeof 检测——需 TS 5（experimentalDecorators 或 Stage 3 草案）、Babel（@babel/plugin-proposal-decorators）、SWC 编译，或原生支持（Stage 3 未全量落地）。装饰器签名：(target: any, context: any) => newTarget | void，context 含 { name, kind, access, addInitializer, metadata }。常见装饰器工厂：@logged（日志）、@bound（自动 bind this）、@memoize（缓存结果）。元数据通过 Symbol.metadata 静态字段传递。本页用普通 JS 手动实现等价行为（Object.defineProperty + wrapper）作对照演示。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('手动模拟装饰器', { type: 'primary', size: 'sm', onClick: () => this._manualDecorators() })),
                h('div', { class: 'fs-sm text-secondary' }, '手动模拟装饰器结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '300px', overflow: 'auto' } }, h('code', {}, s.decoratorResult || '（点击按钮用普通 JS 手动实现 @logged / @memoize / @bound 等价行为）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '装饰器语法（需编译，无法在未支持环境直接运行）：'),
                h('pre', { class: 'code-block', style: { maxHeight: '200px', overflow: 'auto' } }, h('code', {}, `// 装饰器工厂：返回装饰器函数
function logged(target: any, { name, kind }: any) {
  if (kind === 'method') {
...
function memoize(target: any, { name }: any) {
  const cache = new Map();
  return function(...args: any[]) {
...
D[Symbol.metadata].name; // { type: 'string' }`)),
                h(Alert, {
                    ...
                    description: '装饰器本质是"语法糖"——编译后等价于 Object.defineProperty + wrapper 函数。本页用普通 JS 手动实现 @logged / @memoize / @bound 等价行为，演示装饰器的语义；在不支持装饰器语法的环境（如未启用 TS 5 / Babel 插件的 Node）可作为兜底。Stage 3 装饰器签名 (target: any, context: any) => newTarget | void 与 TS 旧版 experimentalDecorators 签名 (target, key, descriptor) 不同，迁移需注意。Symbol.metadata 用于装饰器间传递类元数据（ORM 字段标注、DI 标记等），同为 ES2025 提案。',
                }),
            ],
        });
        this.registerChild(card);
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'page api-lab-page' }, h('h2', { class: 'section-title' }, 'ES2025 语言新特性实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, '本页演示 ES2025 提案语言新特性：AsyncIterator Helpers、ShadowRealm、Error.isError、RegExp.escape、Math.f16round/Float16Array、SuppressedError/DisposableStack 进阶、Decorators。多数为 TC39 Stage 3/4 提案，需 Node 22+ / 较新浏览器；Decorators 为语法特性需编译。所有 API 调用前做能力检测，不可用时仅记日志说明，绝不抛异常。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderCard6(), this._renderLogPanel());
    }
}
//# sourceMappingURL=ES2025LanguagePage.js.map