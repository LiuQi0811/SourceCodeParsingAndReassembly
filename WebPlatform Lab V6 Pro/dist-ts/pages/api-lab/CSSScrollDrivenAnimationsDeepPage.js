...
export class CSSScrollDrivenAnimationsDeepPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!f.animationTimeline)
            this._addLog('warn', 'animation-timeline: scroll() 不可用（Chrome 115+/Safari 17.4+/Firefox 部分支持）');
        if (!f.animationTimelineView)
            this._addLog('warn', 'animation-timeline: view() 不可用（Chrome 115+/Safari 17.4+/Firefox 部分支持）');
        if (!f.timelineScope)
            this._addLog('info', 'timeline-scope 不可用（Chrome 122+）');
        if (f.prefersReducedMotion)
            this._addLog('warn', '用户偏好减少动态效果（prefers-reduced-motion: reduce），滚动驱动动画应跳过或简化');
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'animation-timeline: <timeline-name> | scroll() | view() | auto | none 声明动画驱动时间线。默认 auto 为时间驱动（duration 表示秒），切换到 scroll()/view() 后 duration 改为表示滚动进度比例。scroll-driven vs time-driven 区别：驱动力（滚动 vs 时间）、duration 语义、iteration 是否有意义、暂停方式。命名时间线通过 animation-timeline: --name 引用。animation 简写会重置 timeline 为 auto，需单独声明。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 animation-timeline 演示', { type: 'primary', size: 'sm', onClick: () => this._runOverviewDemo() })),
                h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, [
                ...
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.overviewInfo || '（点击按钮查看 animation-timeline 概述完整用法）')),
            ],
        });
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['scroll()', f.animationTimeline]]), h(Tag, { color: 'primary' }, 'scroll() 时间线')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'scroll() = scroll([<axis>?, <scroller>?]) 创建滚动进度时间线。axis: block|inline|x|y（block 跟随书写模式，x/y 物理方向）；scroller: nearest|root|self（nearest 最近可滚动祖先，root 整页，self 自身）。animation-timeline: scroll(root block) 整页滚动驱动；scroll(nearest) 最近可滚动祖先；scroll(self inline) 自身水平滚动。参数顺序可互换，单参数可省略另一个。整页进度条用 root，容器内动画用 nearest。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 scroll() 演示', { type: 'primary', size: 'sm', onClick: () => this._runScrollDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '滚动驱动进度条演示（滚动下方容器查看进度条变化）：'),
                h('div', { class: 'sd-scroll-container' }, h('div', { class: 'sd-progress' }, h('div', { class: 'sd-progress__bar' })), h('div', { class: 'sd-scroll-content' }, h('div', { class: 'sd-scroll-item' }, '滚动项 1 —— 向下滚动查看进度条增长'), h('div', { class: 'sd-scroll-item' }, '滚动项 2 —— 进度条由 animation-timeline: scroll() 驱动'), h('div', { class: 'sd-scroll-item' }, '滚动项 3 —— 滚动停止动画即暂停'), h('div', { class: 'sd-scroll-item' }, '滚动项 4 —— 滚到底进度条满 100%'), h('div', { class: 'sd-scroll-item' }, '滚动项 5 —— 反向滚动动画倒退'), h('div', { class: 'sd-scroll-item' }, '滚动项 6 —— 无需 JS 监听 scroll 事件'), h('div', { class: 'sd-scroll-item' }, '滚动项 7 —— 纯 CSS 实现，性能更优'), h('div', { class: 'sd-scroll-item' }, '滚动项 8 —— 结束'))),
                h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, [
                    '/* 整页滚动驱动 */',
                    '.page-progress {',
                    '  animation: grow linear;',
                ...
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.scrollInfo || '（点击按钮查看 scroll() 函数完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['view()', f.animationTimelineView]]), h(Tag, { color: 'primary' }, 'view() 时间线')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'view() = view([<axis>?, <inset>?]) 创建视图进度时间线，跟踪元素进入视口的过程。axis 同 scroll()；inset: auto|<length-percentage>{1,2} 调整起止位置。与 scroll() 区别：scroll 跟踪滚动条位置（整段滚动一个 timeline），view 跟踪元素进入视口过程（每个元素独立 timeline）。用于 reveal/fade-in 动画。timeline range names: cover/entry/exit/contain/entry-crossing/exit-crossing。配合 animation-range 精细控制动画触发阶段。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 view() 演示', { type: 'primary', size: 'sm', onClick: () => this._runViewDemo() })),
                h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, [
                    '/* 卡片 reveal：进入视口时淡入上移 */',
                    '@keyframes reveal {',
                    '  from { opacity: 0; transform: translateY(60px) scale(0.9); }',
                ...
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.viewInfo || '（点击按钮查看 view() 函数完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'scroll-timeline-name: --name 在滚动容器上声明命名滚动时间线，scroll-timeline-axis 指定轴。view-timeline-name: --name 在元素自身声明命名视图时间线，view-timeline-axis 指定轴。子元素通过 animation-timeline: --name 引用。比 scroll()/view() 匿名函数更灵活：可被任意后代引用、跨组件复用、配合 timeline-scope 跨层引用。<dashed-ident> 必须 -- 开头。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行命名时间线演示', { type: 'primary', size: 'sm', onClick: () => this._runNamedDemo() })),
                h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, [
                    '/* 容器声明命名滚动时间线 */',
                    '.scroller {',
                    '  overflow-y: auto;',
                ...
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.namedInfo || '（点击按钮查看命名时间线完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['timeline-scope', f.timelineScope]]), h(Tag, { color: 'primary' }, '跨层作用域')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'timeline-scope: --name | all | none 让祖先元素声明时间线作用域，使子元素可在 DOM 树不同位置引用同一命名时间线。默认命名时间线只在「声明元素的后代」中可见，timeline-scope 在共同祖先上扩展作用域，解决跨分支引用、shadow DOM / 组件树时间线传递问题。all 扩展所有命名时间线（性能开销大）。必须在共同祖先上声明，不是在动画元素上。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 timeline-scope 演示', { type: 'primary', size: 'sm', onClick: () => this._runScopeDemo() })),
                h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, [
                    '/* 共同祖先扩展作用域 */',
                    '.layout {',
                ...
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.scopeInfo || '（点击按钮查看 timeline-scope 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['animation-range', f.animationRange]]), h(Tag, { color: 'primary' }, '动画范围')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'animation-range: <start> <end> 精细控制动画在 timeline 上的触发范围。animation-range-start/end: normal|<length-percentage>|<timeline-range-name> <length-percentage>?。timeline range names: cover|contain|entry|exit|entry-crossing|exit-crossing（仅 view() 时间线有效，scroll() 用百分比）。如 animation-range: entry 0% cover 50% 表示从进入开始到 cover 的 50% 结束。简写单值 entry 等价于 entry 0% entry 100%。默认 cover 0% cover 100%。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 animation-range 演示', { type: 'primary', size: 'sm', onClick: () => this._runRangeDemo() })),
                h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, [
                    '/* 仅进入阶段动画 */',
                    '.card {',
                    '  animation: fadeIn linear;',
                ...
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.rangeInfo || '（点击按钮查看 animation-range 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard7() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['view-timeline-inset', f.viewTimelineInset]]), h(Tag, { color: 'primary' }, '视图内边距')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'view-timeline-inset: auto | <length-percentage>{1,2} 调整 view() 时间线的开始/结束位置，相当于收缩视口范围让元素更早/更晚触发动画。单值起止相同，双值第一个是起始（进入边）第二个是结束（离开边）。正 inset 让动画触发更晚/更早结束，负 inset 让动画提前触发。与 view() 函数的 inset 参数关系：view(auto) 引用属性值，view(20%) 显式覆盖属性。用于提前/延后触发 reveal 动画、sticky 标题渐显。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 view-timeline-inset 演示', { type: 'primary', size: 'sm', onClick: () => this._runInsetDemo() })),
                h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, [
                    '/* 元素进入视口 20% 处才触发动画 */',
                    '.card {',
                    '  view-timeline-name: --reveal;',
                ...
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.insetInfo || '（点击按钮查看 view-timeline-inset 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard8() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'primary' }, '实战'), h(Tag, { color: f.animationTimeline ? 'success' : 'error' }, `scroll() ${f.animationTimeline ? '✓' : '✗'}`)),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '10 种实战模式：滚动驱动进度条（scroll(root) + scaleX）/ 卡片 reveal（view() + opacity/translate）/ 视差滚动（不同 animation-range 速率）/ 滚动旋转（scroll + rotate）/ sticky 标题渐显（view() + entry range）/ 列表项逐项 reveal（view(inline)）/ 回到顶部按钮显隐 / 滚动驱动数字递增（@property + counter）/ 滚动驱动颜色变化 / 横向滚动驱动（scroll(inline)）。陷阱清单：duration 在 timeline 下表示进度比例、iteration-count 无意义、浏览器支持 Chrome 115+/Safari 17.4+/Firefox 部分支持、prefers-reduced-motion 优先级、scroll container 必须有可滚动内容、简写重置 timeline 等。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行实战模式与陷阱演示', { type: 'primary', size: 'sm', onClick: () => this._runPatternDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '滚动驱动进度条演示（滚动下方容器）：'),
                h('div', { class: 'sd-scroll-container' }, h('div', { class: 'sd-progress' }, h('div', { class: 'sd-progress__bar' })), h('div', { class: 'sd-scroll-content' }, h('div', { class: 'sd-reveal-item' }, 'reveal 项 1 —— 滚动驱动进度条 + 卡片 reveal'), h('div', { class: 'sd-reveal-item' }, 'reveal 项 2 —— view() + animation-range'), h('div', { class: 'sd-reveal-item' }, 'reveal 项 3 —— 纯 CSS 实现，无需 JS'), h('div', { class: 'sd-reveal-item' }, 'reveal 项 4 —— 性能更优（GPU 合成）'), h('div', { class: 'sd-reveal-item' }, 'reveal 项 5 —— 反向滚动动画倒退'), h('div', { class: 'sd-reveal-item' }, 'reveal 项 6 —— 结束'))),
                h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, [
...
//# sourceMappingURL=CSSScrollDrivenAnimationsDeepPage.js.map