...
export class CSSGeneratedContentListsPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!f.marker)
            this._addLog('warn', '::marker 不可用（Chrome 86+/Firefox 68+/Safari 14.1+）');
        if (!f.counterStyle)
            this._addLog('warn', '@counter-style 不可用（Firefox 33+/Chrome 91+/Safari 17+）');
    ...
    _renderCard1() {
        ...
        }, h('p', { class: 'fs-sm text-secondary' }, 'CSS Generated Content Module：::before/::after/::marker 伪元素 + content 属性（string/attr/url/counter/counters/open-quote/leader/var）。常用于装饰图标、引号、自动编号、目录页码引导。'), h('div', { class: 'gl-gen-demo' }, h('div', { class: 'gl-quote' }, '使用 open-quote / close-quote 的引号渲染'), h('div', { class: 'gl-attr', 'data-label': '属性值' }, '由 attr(data-label) 注入前缀'), h('div', { class: 'gl-counter' }, h('h4', {}, '第一节'), h('h4', {}, '第二节'), h('h4', {}, '第三节')), h('ol', { class: 'gl-nested' }, h('li', {}, '顶层 1'), h('li', {}, '顶层 2', h('ol', { class: 'gl-nested' }, h('li', {}, '嵌套 2.1'), h('li', {}, '嵌套 2.2'))), h('li', {}, '顶层 3'))), h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('运行生成内容演示', { type: 'primary', size: 'sm', onClick: () => this._runGenContentDemo() })), s.genContentInfo ? h('pre', { class: 'code-block mt-md' }, s.genContentInfo) : null);
    }
    ...
    _renderCard2() {
        ...
        }, h('p', { class: 'fs-sm text-secondary' }, '::marker 伪元素独立设色/字体/content + list-style-type 支持 CSS3 任意字符串 + list-style-position: outside/inside + list-style-image 图片/渐变。'), h('div', { class: 'gl-list-demo' }, h('ul', { class: 'gl-marker-custom' }, h('li', {}, '::marker content: "▶ " + color: red'), h('li', {}, '独立设 marker 颜色与字体'), h('li', {}, 'list-style: none 后用 ::marker 重建')), h('ul', { class: 'gl-marker-emoji' }, h('li', {}, 'list-style-type: "🎯 " 字符串标记'), h('li', {}, 'CSS3 任意字符串支持')), h('ul', { class: 'gl-marker-square' }, h('li', {}, 'list-style-type: square'), h('li', {}, 'list-style-position: inside 标记进内容框'))), h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('运行列表样式演示', { type: 'primary', size: 'sm', onClick: () => this._runListsDemo() })), s.listsInfo ? h('pre', { class: 'code-block mt-md' }, s.listsInfo) : null);
    }
    ...
    _renderCard3() {
        ...
        }, h('p', { class: 'fs-sm text-secondary' }, 'CSS Counter Styles Level 3 @counter-style 规则自定义列表计数样式：system（cyclic/numeric/alphabetic/symbolic/additive/fixed/extends）+ symbols/additive-symbols + prefix/suffix/range/pad/negative/fallback/speak-as。常用于 CJK 序号、希伯来数字、自定义图标列表。'), h('div', { class: 'gl-counter-style-demo' }, h('ul', { class: 'gl-triangle' }, h('li', {}, 'system: cyclic + symbols: "▶"（循环三角形）'), h('li', {}, 'cyclic 超出 symbols 数量时循环使用'), h('li', {}, '最简单的图标列表方案')), h('ul', { class: 'gl-abc' }, h('li', {}, 'system: alphabetic + 🅰🅱🅲🅳🅴'), h('li', {}, '第 6 项变成 🅰🅰（类似 aa, ab, ac）')), h('ul', { class: 'gl-bullets' }, h('li', {}, 'system: fixed ① ② ③ ④ ⑤'), h('li', {}, '超出 5 项使用 fallback')), h('ul', { class: 'gl-roman-ext' }, h('li', {}, 'system: extends lower-roman'), h('li', {}, 'prefix: "(" suffix: ") "'), h('li', {}, '继承已有样式 + 加前后缀')), h('ul', { class: 'gl-binary' }, h('li', {}, 'system: numeric + symbols: 0 1'), h('li', {}, '第 2 项是 10，第 3 项是 11，第 4 项是 100')), h('ul', { class: 'gl-tally' }, h('li', {}, 'system: additive（画正字式）'), h('li', {}, '5 = 五，6 = 五丨'))), h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('运行 @counter-style 演示', { type: 'primary', size: 'sm', onClick: () => this._runCounterStyleDemo() })), s.counterStyleInfo ? h('pre', { class: 'code-block mt-md' }, s.counterStyleInfo) : null);
    }
    ...
    _renderCard4() {
        ...
        }, h('p', { class: 'fs-sm text-secondary' }, 'CSS Paged Media Module @page 规则定义打印页面盒模型 + 16 个 margin box 页眉页脚 + 命名页面 + :first/:left/:right/:blank 伪类。CSS Fragmentation break-before/after/inside + widows/orphans 控制分页断行。'), h('div', { class: 'gl-paged-demo' }, h('div', { class: 'gl-cols' }, h('p', {}, 'break-inside: avoid 演示：这段不会被分栏打断'), h('p', {}, '多栏布局与 fragmentation 协同工作，column-fill: balance 会让各栏尽量平衡'), h('p', {}, '打印时 break-before: page 可让章节标题强制新页'), h('p', {}, 'widows/orphans 控制段落首尾至少保留几行避免孤行'))), h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('运行分页媒体演示', { type: 'primary', size: 'sm', onClick: () => this._runPagedMediaDemo() })), s.pagedMediaInfo ? h('pre', { class: 'code-block mt-md' }, s.pagedMediaInfo) : null);
    }
    _renderLogPanel() {
        const s = this.state;
...
//# sourceMappingURL=CSSGeneratedContentListsPage.js.map