...
export class CSSWritingModesPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!f.wmVerticalRl)
            this._addLog('warn', 'writing-mode: vertical-rl 不可用或 jsdom 未识别（Chrome 48+ / Firefox 41+ / Safari 10.1+ 全部稳定）');
        if (!f.wmSidewaysRl)
            this._addLog('warn', 'writing-mode: sideways-rl 仅 Firefox 支持（CSS Writing Modes L4，Chrome/Safari 未实现）');
        if (!f.textOrientationUpright)
            this._addLog('warn', 'text-orientation: upright 不可用或 jsdom 未识别（Chrome 48+ / Firefox 41+ / Safari 10.1+）');
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'CSS Writing Modes Module Level 3/4 解决多语言排版：水平 vs 垂直（英文横排 vs 古中文/日文竖排）、LTR vs RTL（英文/中文 vs 阿拉伯/希伯来）、CJK 竖排需求（古籍、报纸、漫画）、蒙古文竖排（vertical-lr）。四大核心属性：writing-mode（块流动方向）、direction（行内流动方向）、unicode-bidi（双向文本算法）、text-orientation（竖排字符方向）。浏览器支持完整：Chrome 48+ / Firefox 41+ / Safari 10.1+ 全部稳定，sideways-rl/sideways-lr 仅 Firefox 支持（L4）。'),
                ...
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.overviewInfo || '（点击「读取概述信息」查看 CSS Writing Modes 全景）')),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'writing-mode 定义块流动方向：horizontal-tb（默认水平从上到下，适合英文/现代中文）、vertical-rl（垂直从右到左，古中文/日文/报纸/漫画）、vertical-lr（垂直从左到右，蒙古文/满文）、sideways-rl/sideways-lr（L4 新增，所有字符旋转 90°，仅 Firefox 支持）。vertical-rl/lr 让 CJK 字符直立、拉丁字符自然旋转（配合 text-orientation），sideways 强制所有字符旋转。浏览器支持：Chrome 48+ / Firefox 41+ / Safari 10.1+ 全部稳定，sideways 仅 Firefox。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ writingModeInfo: this._readWritingModeInfo() }) }), this._btn('horizontal-tb', { size: 'sm', disabled: !f.wmHorizontalTb, onClick: () => this._setWritingMode('horizontal-tb') }), this._btn('vertical-rl', { size: 'sm', disabled: !f.wmVerticalRl, onClick: () => this._setWritingMode('vertical-rl') }), this._btn('vertical-lr', { size: 'sm', disabled: !f.wmVerticalLr, onClick: () => this._setWritingMode('vertical-lr') }), this._btn('sideways-rl', { size: 'sm', disabled: !f.wmSidewaysRl, onClick: () => this._setWritingMode('sideways-rl') })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '演示元素（当前 writing-mode: ' + this._writingMode + '）：'),
                h('div', { class: 'wm-demo' }, '床前明月光，疑是地上霜。举头望明月，低头思故乡。——李白《静夜思》'),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.writingModeInfo || '（点击按钮切换 writing-mode 查看说明）')),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'direction 定义行内流动方向（ltr 左到右 / rtl 右到左，阿拉伯/希伯来语）。unicode-bidi 控制双向文本算法：normal（默认）/ embed（嵌入）/ isolate（隔离，防止子元素影响外部 bidi）/ isolate-override / plaintext（按内容自动判断方向）。bidi 算法（UAX #9）按字符 bidi 类别（强/弱/中性）处理混合方向文本，数字是弱字符继承上下文方向。常见场景：阿拉伯语中嵌入英文用 isolate 隔离，用户输入用 plaintext 自动判断方向。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ directionBidiInfo: this._readDirectionBidiInfo() }) }), this._btn('dir: ltr', { size: 'sm', disabled: !f.directionRtl, onClick: () => this._setDirection('ltr') }), this._btn('dir: rtl', { size: 'sm', disabled: !f.directionRtl, onClick: () => this._setDirection('rtl') }), this._btn('bidi: normal', { size: 'sm', disabled: !f.unicodeBidiIsolate, onClick: () => this._setUnicodeBidi('normal') }), this._btn('bidi: isolate', { size: 'sm', disabled: !f.unicodeBidiIsolate, onClick: () => this._setUnicodeBidi('isolate') }), this._btn('bidi: plaintext', { size: 'sm', disabled: !f.unicodeBidiIsolate, onClick: () => this._setUnicodeBidi('plaintext') })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '双向文本演示（direction: ' + this._direction + ' / unicode-bidi: ' + this._unicodeBidi + '）：'),
                h('div', { class: 'bidi-stage' }, h('div', { class: 'bidi-demo' }, 'مرحبا بالعالم Hello 世界 1234 — 双向文本测试')),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.directionBidiInfo || '（点击按钮切换 direction / unicode-bidi 查看说明）')),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'text-orientation 定义竖排时字符方向（仅在 writing-mode: vertical-* 生效）：mixed（默认，CJK 直立 + 拉丁/数字旋转 90°，适合竖排混排）、upright（所有字符直立含拉丁，适合纯 CJK 古籍/蒙古文）、sideways（所有字符旋转 90° 含 CJK，适合表格竖排表头）。与 writing-mode 协同：vertical-rl + mixed 默认竖排混排，vertical-rl + upright 纯 CJK 古籍，vertical-rl + sideways 旋转标题。蒙古文用 vertical-lr + upright（字母直立符合传统）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ textOrientationInfo: this._readTextOrientationInfo() }) }), this._btn('mixed', { size: 'sm', disabled: !f.textOrientationMixed, onClick: () => this._setTextOrientation('mixed') }), this._btn('upright', { size: 'sm', disabled: !f.textOrientationUpright, onClick: () => this._setTextOrientation('upright') }), this._btn('sideways', { size: 'sm', disabled: !f.textOrientationSideways, onClick: () => this._setTextOrientation('sideways') })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '竖排演示（writing-mode: vertical-rl + text-orientation: ' + this._textOrientation + '）：'),
                h('div', { class: 'to-stage' }, h('div', { class: 'to-demo' }, '床前明月光 Hello 2024')),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.textOrientationInfo || '（点击按钮切换 text-orientation 查看说明）')),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['vertical-rl', f.wmVerticalRl], ['vertical-lr', f.wmVerticalLr]]), h(Tag, { color: 'primary' }, '5 大场景')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '嵌套 writing-mode 五大场景：父容器 horizontal-tb 子容器 vertical-rl（横排正文嵌入竖排引用，用 display: inline-block 共存）、横排表格中竖排标题（th 设 vertical-rl 节省横向空间）、混合方向排版（正文横排 + 标题竖排 + 阿拉伯语 RTL）、逻辑属性自适应（嵌套不同 writing-mode 时 padding-inline/margin-block 各自适配方向）、媒体查询切换 writing-mode（桌面横排、移动端竖排）。逻辑属性让嵌套场景各层级自动适配方向。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取嵌套信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runNestingDemo() })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '父横子竖嵌套演示：'),
                h('div', { class: 'nesting-stage' }, h('div', { class: 'parent-h' }, '这是横排正文（horizontal-tb）。', h('span', { class: 'child-v' }, '竖排子元素 vertical-rl'), '继续横排正文，竖排子元素用 inline-block 与横排共存。')),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.nestingInfo || '（点击按钮查看 5 大嵌套场景完整代码）')),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard7() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['vertical-rl', f.wmVerticalRl], ['upright', f.textOrientationUpright]]), h(Tag, { color: 'primary' }, '6 大场景')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '中日韩竖排文学六大场景：古诗竖排（vertical-rl + text-orientation: upright + 楷体 + letter-spacing）、报纸版式（竖排标题 + 横排正文）、漫画竖排 manga（vertical-rl + mixed 假名直立 + 拉丁旋转）、与 Ruby 注音协同（<ruby> 竖排时注音自动到字符右侧）、竖排表单输入（input/textarea 设 vertical-rl）、中文标点直排处理（标点自动旋转到右上角）。配合 text-combine-upright 让横排数字在竖排中组合显示。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取实战信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runCjkVerticalDemo() })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '古诗竖排演示（writing-mode: vertical-rl + text-orientation: upright）：'),
                h('div', { class: 'poem-stage' }, h('div', { class: 'poem' }, h('div', { class: 'line' }, '床前明月光，'), h('div', { class: 'line' }, '疑是地上霜。'), h('div', { class: 'line' }, '举头望明月，'), h('div', { class: 'line' }, '低头思故乡。'))),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.cjkVerticalInfo || '（点击按钮查看 6 大竖排文学场景完整代码）')),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard8() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['direction:rtl', f.directionRtl], ['unicode-bidi:isolate', f.unicodeBidiIsolate]]), h(Tag, { color: 'primary' }, '6 大场景')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '多语言国际化六大场景：阿拉伯语 RTL 切换（html[lang="ar"] + direction: rtl + 逻辑属性自动适配）、希伯来文 RTL（同阿拉伯语处理，数字用 isolate 防翻转）、双向 URL（unicode-bidi: plaintext + direction: ltr 防 bidi 错误翻转斜杠点）、国际化设计系统（逻辑属性 + CSS 变量，一套样式适配所有语言）、logical properties 全套替换物理属性（width→inline-size、margin-left→margin-inline-start、text-align: left→start 等）、DevTools 调试 bidi（Chrome Computed 查看计算值，Firefox 显示 bidi 嵌入层级）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取国际化信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runI18nDemo() })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, 'RTL vs LTR 对比演示：'),
                h('div', { class: 'i18n-stage' }, h('div', { class: 'ltr-demo' }, 'LTR（左到右）：Hello World 1234 — 默认方向，从左到右阅读'), h('div', { class: 'rtl-demo' }, 'RTL（右到左）：مرحبا بالعالم 1234 — 阿拉伯语方向，从右到左阅读')),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.i18nInfo || '（点击按钮查看 6 大国际化场景完整代码）')),
                h(Alert, {
                    type: 'info',
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'api-lab-page css-writing-modes-page' }, h('h2', { class: 'section-title' }, 'CSS Writing Modes Level 3/4 实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, '本页演示 CSS Writing Modes Module Level 3/4 书写模式：writing-mode 属性（horizontal-tb / vertical-rl / vertical-lr / sideways）、direction 与 unicode-bidi（LTR/RTL + bidi 算法 + 双向文本嵌入）、text-orientation 属性（mixed CJK 直立 / upright 全直立 / sideways 全旋转）、逻辑属性基础（inline-size / block-size / margin-block / inset-block）、嵌套 writing-mode 与混合方向（父横子竖 / 表格竖排标题 / 逻辑属性自适应）、中日韩竖排文学（古诗 / 报纸 / 漫画 / Ruby / 表单 / 标点）、多语言国际化（阿拉伯/希伯来 RTL / 双向 URL / 设计系统 / logical properties 全套替换）。所有特性通过 typeof / CSS.supports 能力检测，不可用时仅记日志，绝不抛异常。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderCard6(), this._renderCard7(), this._renderCard8(), this._renderLogPanel());
    }
}
//# sourceMappingURL=CSSWritingModesPage.js.map