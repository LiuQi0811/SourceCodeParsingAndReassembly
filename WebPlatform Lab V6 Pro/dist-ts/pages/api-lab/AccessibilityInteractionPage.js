...
export class AccessibilityInteractionPage extends Page {
    ...
    _onlineHandler = null;
    _pointerCancelUnbind = null;
    _visibilityUnbind = null;
    _vvHandlers = null;
    _vvRAF = null;
    _wakeLock = null;
    _wakeLockAutoReacquire = false;
    // —— 初始 state ——
    ...
    _detectCapabilities() {
        ...
        const caps = {
            ...
            readingFlow: (() => { try {
                return typeof CSS !== 'undefined' && typeof CSS.supports === 'function' && CSS.supports('reading-flow', 'visual');
            }
            catch {
                return false;
            } })(),
    ...
    _renderCard1() {
        const card = new Card({
            ...
            extra: h('span', { class: 'flex items-center gap-xs' }, h(Tag, { color: 'primary' }, 'navigator.wakeLock'), h(Tag, { color: this.state.wakeStatus === 'active' ? 'success' : 'default' }, `status: ${this.state.wakeStatus}`)),
            children: h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'navigator.wakeLock.request("screen") 返回 Promise<WakeLockSentinel>。sentinel.released / sentinel.type 反映状态，sentinel.release() 主动释放。页面隐藏（visibilitychange → hidden）时浏览器会自动释放；恢复可见后需重新 request 才能继续持锁。'), h('div', { class: 'flex flex-wrap gap-sm' }, this._btn('请求 wakeLock', {
            ...
            })), this._kvRow('当前状态', this.state.wakeStatus), h(Alert, {
                type: 'info',
    ...
    _renderCard2() {
        const card = new Card({
            ...
            extra: h('span', { class: 'flex items-center gap-xs' }, h(Tag, { color: 'primary' }, 'requestFullscreen'), h(Tag, { color: 'primary' }, 'visualViewport')),
            children: h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'Element.requestFullscreen() 进入全屏，document.exitFullscreen() 退出，document.fullscreenElement / fullscreenEnabled 反映状态。fullscreenchange / fullscreenerror 事件追踪变化。window.visualViewport 提供 width / height / offsetLeft / offsetTop / pageLeft / scale，pinch-zoom 时 scale 会变化；resize / scroll 事件持续追踪。'), 
            ...
            }, h('div', { class: 'fs-sm', style: { opacity: 0.85 } }, '全屏演示容器'), h('div', { class: 'fs-lg fw-medium' }, '点击「进入全屏」测试 Fullscreen API')), h('div', { class: 'flex flex-wrap gap-sm' }, this._btn('进入全屏', {
                type: 'primary', size: 'sm',
            ...
            })), this._kvRow('fullscreenElement', (typeof document !== 'undefined' && document.fullscreenElement) ? document.fullscreenElement.tagName : 'null'), this._kvRow('fullscreenEnabled', typeof document !== 'undefined' ? String(!!document.fullscreenEnabled) : 'unknown'), this.state.vpInfo
                ? h('div', { class: 'flex flex-col gap-xs' }, this._kvRow('vv.width', this.state.vpInfo.width?.toFixed(1)), this._kvRow('vv.height', this.state.vpInfo.height?.toFixed(1)), this._kvRow('vv.scale', this.state.vpInfo.scale?.toFixed(3)), this._kvRow('vv.offsetLeft', this.state.vpInfo.offsetLeft), this._kvRow('vv.offsetTop', this.state.vpInfo.offsetTop), this._kvRow('vv.pageLeft', this.state.vpInfo.pageLeft))
                : h('p', { class: 'fs-sm text-tertiary' }, '（点击「读取 visualViewport」后，数值显示在这里）'), h('p', { class: 'fs-xs text-tertiary' }, '说明：移动端 pinch-zoom 时 vv.scale 会从 1.0 增大，vv.width/height 随之缩小；vv.offsetLeft/offsetTop 反映视口相对于屏幕的偏移。')),
        });
        this.registerChild(card);
        return card.render();
    }
    _renderCard3() {
        const card = new Card({
            ...
            extra: h('span', { class: 'flex items-center gap-xs' }, h(Tag, { color: 'primary' }, 'navigator.clipboard'), h(Tag, { color: 'warning' }, '需 HTTPS + 用户激活')),
            children: h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'navigator.clipboard.readText() / writeText(text) 处理纯文本；read() / write([ClipboardItem]) 处理富文本与图片。new ClipboardItem({ "text/plain": Blob, "text/html": Blob, "image/png": Blob }) 支持多种 MIME。clipboardchange 事件可监听剪贴板变化（部分浏览器支持）。'), h('div', { class: 'flex flex-wrap gap-sm' }, this._btn('写入 "Hello Clipboard"', {
                type: 'primary', size: 'sm',
            ...
            })), this._kvRow('最近内容', this.state.clipText === '' ? '（空）' : this.state.clipText), h(Alert, {
                type: 'info',
                message: '使用前提',
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('span', { class: 'flex items-center gap-xs' }, h(Tag, { color: 'primary' }, 'navigator.connection'), h(Tag, { color: 'primary' }, 'online/offline'), h(Tag, { color: onlineTagColor }, `online: ${this.state.onlineStatus}`)),
            children: h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'navigator.connection 提供 effectiveType（"4g"/"3g"/"2g"/"slow-2g"）、downlink（Mbps）、rtt（ms）、saveData（boolean）、type（"wifi"/"cellular"/"ethernet"/"none"）。connection.change 事件追踪网络切换。navigator.onLine 反映在线状态，window.online / offline 事件触发回调。'), h('div', { class: 'flex flex-wrap gap-sm' }, this._btn('读取 connection 信息', {
                type: 'primary', size: 'sm',
                ...
                ? h('div', { class: 'flex flex-col gap-xs' }, this._kvRow('effectiveType', this.state.connInfo.effectiveType ?? 'N/A'), this._kvRow('downlink', this.state.connInfo.downlink == null ? 'N/A' : `${this.state.connInfo.downlink} Mbps`), this._kvRow('rtt', this.state.connInfo.rtt == null ? 'N/A' : `${this.state.connInfo.rtt} ms`), this._kvRow('saveData', this.state.connInfo.saveData == null ? 'N/A' : String(this.state.connInfo.saveData)), this._kvRow('type', this.state.connInfo.type ?? 'N/A'))
                : h('p', { class: 'fs-sm text-tertiary' }, '（点击「读取 connection 信息」后，字段显示在这里。jsdom 中通常不可用）'), this._kvRow('navigator.onLine', typeof navigator !== 'undefined' ? String(navigator.onLine) : 'unknown'), h('p', { class: 'fs-xs text-tertiary' }, '注：saveData 表示用户开启了「省流模式」；type 在桌面浏览器多为 N/A，移动端可能返回 wifi / cellular。')),
        });
        this.registerChild(card);
        return card.render();
    }
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('span', { class: 'flex items-center gap-xs' }, h(Tag, { color: 'primary' }, 'InputDeviceCapabilities'), h(Tag, { color: 'primary' }, 'ARIA')),
            children: h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'PointerEvent.prototype.getPredictedEvents() 返回预测事件序列（部分浏览器支持）；pointercancel 事件在指针被中断时触发。new InputDeviceCapabilities({ sourceMotion }) 构造设备能力对象，firesTouchEvents 反映是否触发触摸事件；event.sourceCapabilities（已废弃）提供同类信息。ARIA 基础：role / aria-label / aria-pressed / aria-expanded / aria-live / aria-hidden 用于无障碍语义。'), h('div', { class: 'flex flex-wrap gap-sm' }, this._btn('检测 InputDeviceCapabilities', {
                type: 'primary', size: 'sm',
                ...
                ? h('div', { class: 'flex flex-col gap-xs' }, this._kvRow('InputDeviceCapabilities', info.hasInputDeviceCapabilities ? '支持' : '不支持'), this._kvRow('firesTouchEvents', info.firesTouchEvents == null ? 'N/A' : String(info.firesTouchEvents)), this._kvRow('event.sourceCapabilities', info.hasSourceCapabilities ? '存在（已废弃）' : '不存在'), this._kvRow('PointerEvent.getPredictedEvents', info.hasGetPredictedEvents ? '支持' : '不支持'))
            ...
            }, h('div', { class: 'fs-sm fw-medium' }, 'ARIA toggle 演示'), h('button', {
                type: 'button',
                role: 'button',
                'aria-label': '演示开关按钮',
            ...
            }, this.state.ariaPressed ? '已开启（再次点击关闭）' : '已关闭（点击开启）'), h('div', {
                'aria-live': 'polite',
                'aria-atomic': 'true',
                role: 'status',
                class: 'fs-sm',
            ...
            }, `aria-live 朗读：当前状态为「${this.state.ariaPressed ? '已开启' : '已关闭'}」`), h('div', { class: 'fs-xs text-tertiary' }, `当前 aria-pressed="${this.state.ariaPressed}"；点击按钮可观察 aria-live 区域的朗读变化。`, '辅助技术（屏幕阅读器）会在状态变化时朗读 polite 区域内容。'))),
        });
        this.registerChild(card);
        return card.render();
    }
    ...
    _renderCard9() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: ok ? 'success' : 'error' }, ok ? 'reading-flow ✓' : 'reading-flow ✗'), h(Tag, { color: 'primary' }, 'CSSWG 2024 · A11y')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'reading-flow（CSSWG 2024）在 spatial 布局（grid/flex）中显式声明焦点阅读顺序（visual/flow-visual/flow-relaxed），是无障碍关键特性。与 tabindex 区别：reading-flow 在容器声明一次，子元素自动按布局顺序聚焦，不侵入 DOM；tabindex 需手动指定每个元素序号。与屏幕阅读器协同：即使 grid 视觉重排，辅助技术仍按 reading-flow 顺序朗读。Chrome 开发中，Firefox/Safari 未实现。降级：手动 tabindex 或调整 DOM 顺序。jsdom 不识别，演示仅展示文档。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('展示 reading-flow 文档与代码', { type: 'primary', size: 'sm', onClick: () => this._runReadingFlowDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, 'reading-flow 文档与示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '520px', overflow: 'auto' } }, h('code', {}, s.readingFlowInfo || '（点击按钮查看 reading-flow 完整文档与 grid 焦点顺序示例）')),
                h(Alert, {
                    type: 'info',
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'page api-lab-page' }, h('h2', { class: 'section-title' }, '无障碍与交互 API 实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, '演示 Wake Lock / Fullscreen / Visual Viewport / Clipboard / Network Information / InputDeviceCapabilities / ARIA 属性。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderCard9(), this._renderLogPanel());
    }
}
//# sourceMappingURL=AccessibilityInteractionPage.js.map