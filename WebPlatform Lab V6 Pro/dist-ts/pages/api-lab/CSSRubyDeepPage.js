...
export class CSSRubyDeepPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!f.rubyPosition)
            this._addLog('warn', 'ruby-position 不可用或 jsdom 未识别（CSS Ruby Module；Chrome/Firefox 支持，Safari 旧版需 -webkit- 前缀）');
        if (!f.rubyAlign)
            this._addLog('warn', 'ruby-align 不可用或 jsdom 未识别（start/center/space-between/space-around）');
        if (!f.rubyMerge)
            this._addLog('warn', 'ruby-merge 不可用或 jsdom 未识别（separate/collapse/auto，复合注音关键属性，仍实验性）');
        if (!f.rubyOverhang)
            this._addLog('warn', 'ruby-overhang 不可用或 jsdom 未识别（注音悬挂到相邻字符，仍实验性）');
        if (!f.displayRuby)
            this._addLog('warn', 'display: ruby 不可用或 jsdom 未识别（<ruby> 内部 display 值）');
    ...
    _readRubySemanticInfo() {
        ...
        try {
            ...
            if (stage) {
                ...
                if (rt)
                    rtDisplay = window.getComputedStyle(rt).getPropertyValue('display') || '(空)';
            }
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['display:ruby', f.displayRuby], ['display:ruby-base', f.displayRubyBase], ['display:ruby-text', f.displayRubyText]]), h(Tag, { color: 'primary' }, 'HTML Ruby')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '<ruby> 是注音容器（display: ruby），<rt> 是注音（display: ruby-text，默认 font-size 50%），<rb> 是基底（草案），<rp> 是不支持 <ruby> 时的回退括号（默认 display: none），<rtc> 是双层注音的第二层容器（草案）。HTML 规范定义了完整的 UA 默认样式：ruby/ruby-base/ruby-text/ruby-text-container 四个内部 display 值。单注音每字一音；双注音用 <rtc> 嵌套两层；复合注音用 <rb> 包多字 + ruby-merge: collapse。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取语义信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runRubySemanticDemo() })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '单注音（拼音 / 振假名，每字一音）：'),
                h('div', { class: 'ruby-stage' }, h('ruby', {}, '东', h('rt', {}, 'dōng'), '京', h('rt', {}, 'jīng'), ' · ', h('ruby', {}, '漢', h('rt', {}, 'かん'), '字', h('rt', {}, 'じ')))),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '复合注音（<rb>多字 + 整词注音，配合 ruby-merge: collapse）：'),
                h('div', { class: 'ruby-stage ruby-merge-collapse' }, h('ruby', {}, h('rb', {}, '今日'), h('rt', {}, 'kyō'), ' · ', h('rb', {}, '明日'), h('rt', {}, 'asu'))),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '<rp> 回退括号（不支持 <ruby> 时显示「漢（かん）字（じ）」）：'),
                h('div', { class: 'ruby-stage' }, h('ruby', {}, '漢', h('rp', {}, '（'), h('rt', {}, 'かん'), h('rp', {}, '）'), '字', h('rp', {}, '（'), h('rt', {}, 'じ'), h('rp', {}, '）'))),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '360px', overflow: 'auto' } }, h('code', {}, s.rubySemanticInfo || '（点击「读取语义信息」查看 ruby 元素语义与 UA 默认样式）')),
                h(Alert, {
                    type: 'info',
    ...
    _setRubyPosition(mode) {
        ...
        this._injectStyle('css-ruby-position-demo', `.ruby-pos-demo { ruby-position: ${mode}; -webkit-ruby-position: ${(webkitMap[mode]) || 'before'}; }`);
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['ruby-position', f.rubyPosition]]), h(Tag, { color: 'primary' }, '与 writing-mode 协同')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'ruby-position 控制注音相对主文的位置：over（默认，水平时注音在上方）、under（注音在下方，中文教育常见，用于副标读音）、inter-character（注音在字符之间，繁体中文注音 ㄅㄆㄇㄈ 专用）。与 writing-mode 协同：竖排（vertical-rl）时 over 对应「右侧」。Safari 早期仅支持 -webkit-ruby-position: before|after（before≈over, after≈under），生产建议同时写前缀。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ rubyPositionInfo: this._readRubyPositionInfo() }) }), this._btn('over（上方）', { size: 'sm', disabled: !f.rubyPosition, onClick: () => this._setRubyPosition('over') }), this._btn('under（下方）', { size: 'sm', disabled: !f.rubyPosition, onClick: () => this._setRubyPosition('under') }), this._btn('inter-character', { size: 'sm', disabled: !f.rubyPosition, onClick: () => this._setRubyPosition('inter-character') })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '注音位置演示（当前 ruby-position = ' + this._rubyPosition + '）：'),
                h('div', { class: 'ruby-stage ruby-pos-demo' }, h('ruby', {}, '汉', h('rt', {}, 'hàn'), '语', h('rt', {}, 'yǔ'), '拼', h('rt', {}, 'pīn'), '音', h('rt', {}, 'yīn'))),
                h('p', { class: 'fs-sm text-tertiary mt-sm' }, '真实浏览器中切换可看到注音从上方移到下方；inter-character 主要用于繁体注音符号（如「ㄅㄆㄇㄈ」），将注音置于字符右侧。'),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '320px', overflow: 'auto' } }, h('code', {}, s.rubyPositionInfo || '（点击按钮切换 ruby-position）')),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['ruby-align', f.rubyAlign]]), h(Tag, { color: 'primary' }, '注音相对基底对齐')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'ruby-align 控制注音相对基底（<rb>）的对齐：start（起始对齐，左对齐，日语振假名习惯）、center（居中，默认）、space-between（两端对齐，首尾对齐中间均分）、space-around（分散对齐，每字两侧等距空白）。草案双值语法 ruby-align: <basic> <ruby-text> 分别控制基础与注音区对齐。与 text-align 区别：text-align 控制行内整体，ruby-align 控制注音相对基底。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ rubyAlignInfo: this._readRubyAlignInfo() }) }), this._btn('start', { size: 'sm', disabled: !f.rubyAlign, onClick: () => this._setRubyAlign('start') }), this._btn('center', { size: 'sm', disabled: !f.rubyAlign, onClick: () => this._setRubyAlign('center') }), this._btn('space-between', { size: 'sm', disabled: !f.rubyAlign, onClick: () => this._setRubyAlign('space-between') }), this._btn('space-around', { size: 'sm', disabled: !f.rubyAlign, onClick: () => this._setRubyAlign('space-around') })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '注音对齐演示（当前 ruby-align = ' + this._rubyAlign + '；注音较短时差异细微）：'),
                h('div', { class: 'ruby-stage ruby-align-demo' }, h('ruby', {}, h('rb', {}, '东'), h('rt', {}, 'dōng'), h('rb', {}, '京'), h('rt', {}, 'jīng'), h('rb', {}, '塔'), h('rt', {}, 'tǎ'))),
                h('p', { class: 'fs-sm text-tertiary mt-sm' }, '注音短于汉字时：center 居中、start 起始；注音长于汉字时：space-between 两端对齐、space-around 分散。真实浏览器可看到注音相对汉字的对齐变化。'),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '320px', overflow: 'auto' } }, h('code', {}, s.rubyAlignInfo || '（点击按钮切换 ruby-align）')),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['ruby-merge', f.rubyMerge], ['ruby-overhang', f.rubyOverhang]]), h(Tag, { color: 'primary' }, '复合注音关键')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'ruby-merge 控制复合注音（多字共享一注音）的合并方式：separate（默认，每字独立对齐）、collapse（多字合并为一基底，注音跨整个区域，适合日语熟字訓如「今日=kyō」）、auto（CJK 合并/拉丁独立）。ruby-overhang 控制注音是否悬挂到相邻字符：auto（默认，超出基底宽度时悬挂，不影响行高）、none（不悬挂，限制在基底内，适合精确教育排版）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ rubyMergeInfo: this._readRubyMergeInfo() }) }), this._btn('merge: separate', { size: 'sm', disabled: !f.rubyMerge, onClick: () => this._setRubyMerge('separate') }), this._btn('merge: collapse', { size: 'sm', disabled: !f.rubyMerge, onClick: () => this._setRubyMerge('collapse') }), this._btn('merge: auto', { size: 'sm', disabled: !f.rubyMerge, onClick: () => this._setRubyMerge('auto') }), this._btn('overhang: auto', { size: 'sm', disabled: !f.rubyOverhang, onClick: () => this._setOverhang('auto') }), this._btn('overhang: none', { size: 'sm', disabled: !f.rubyOverhang, onClick: () => this._setOverhang('none') })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '复合注音演示（ruby-merge = ' + this._rubyMerge + '；多字配整词读音）：'),
                h('div', { class: 'ruby-stage ruby-merge-demo' }, h('ruby', {}, h('rb', {}, '今日'), h('rt', {}, 'kyō'), ' · ', h('rb', {}, '大人'), h('rt', {}, 'otona'), ' · ', h('rb', {}, '土産'), h('rt', {}, 'miyage'))),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '注音悬挂演示（ruby-overhang = ' + this._overhang + '；注音长于基底时差异明显）：'),
                h('div', { class: 'ruby-stage ruby-overhang-demo' }, h('ruby', {}, '漢', h('rt', {}, 'kanji-ruby-long'), '字', h('rt', {}, 'ji'), '排', h('rt', {}, 'pái'), '版', h('rt', {}, 'bǎn'))),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '340px', overflow: 'auto' } }, h('code', {}, s.rubyMergeInfo || '（点击按钮切换 ruby-merge / ruby-overhang）')),
                h(Alert, {
                    type: 'warning',
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['ruby-position', f.rubyPosition], ['display:ruby-text', f.displayRubyText]]), h(Tag, { color: 'primary' }, 'complex-ruby')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '双层注音（complex ruby）让一个基底配两层注音：拼音+注音（汉字上方拼音、下方注音 ㄅㄆㄇㄈ）、汉字+罗马字（日语：上方振假名、下方罗马字）、汉字+韩文谚文。结构用 <rtc>（草案）包裹第二层 <rt>，配合 ruby-position: over/under 分别指定位置。<rb>（草案）显式标注基底。兼容性更广的替代方案是嵌套 <ruby>（内层第一层、外层 <rt> 第二层）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取双层注音信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runDoubleRubyDemo() })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '<rtc> 双层结构（汉字 + 罗马字 + 振假名）：'),
                h('div', { class: 'ruby-stage ruby-double' }, h('ruby', {}, h('rb', {}, '東'), h('rb', {}, '京'), h('rtc', {}, h('rt', {}, 'Tōkyō')), h('rtc', {}, h('rt', {}, 'とうきょう')))),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '嵌套 <ruby> 替代方案（兼容性更广；内层拼音 + 外层罗马字）：'),
                h('div', { class: 'ruby-stage ruby-double' }, h('ruby', {}, h('ruby', {}, '東', h('rt', {}, 'dōng'), '京', h('rt', {}, 'jīng')), h('rt', {}, 'Tōkyō'))),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '380px', overflow: 'auto' } }, h('code', {}, s.doubleRubyInfo || '（点击「读取双层注音信息」查看 <rtc>/<rb> 嵌套结构）')),
                h(Alert, {
                    type: 'info',
    ...
    _toggleTrim() {
        ...
        this._injectStyle('css-ruby-cjk-trim', `.ruby-cjk { text-spacing-trim: ${trimVal}; line-break: ${lbVal}; }`);
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['ruby-position', f.rubyPosition]]), h(Tag, { color: 'primary' }, '注音约正文 1/2~1/3')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'Ruby 注音排版与 CJK 文本属性协同：text-spacing-trim 挤压全角标点（避免注音标点顶头）、line-break: strict 禁止标点行首、letter-spacing 控制字距（正文略松 0.05em，注音紧凑 0）、字号比例约定（注音约正文 1/2~1/3，UA 默认 <rt> font-size: 50%）、line-height 足够大（2.0-2.8）给注音留空间避免与上下行重叠。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取 CJK 协同信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ cjkComboInfo: this._readCJKComboInfo() }) }), this._btn(`text-spacing-trim（${this._trimOn ? '开启挤压' : '默认'}）`, { size: 'sm', onClick: () => this._toggleTrim() })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '注音段落（正文 22px，注音 11px，line-height 2.6）：'),
                h('div', { class: 'ruby-cjk' }, h('ruby', {}, '汉', h('rt', {}, 'hàn'), '语', h('rt', {}, 'yǔ'), '拼', h('rt', {}, 'pīn'), '音', h('rt', {}, 'yīn'), '是', h('rt', {}, 'shì'), '中', h('rt', {}, 'zhōng'), '文', h('rt', {}, 'wén'), '排', h('rt', {}, 'pái'), '版', h('rt', {}, 'bǎn'), '的', h('rt', {}, 'de'), '重', h('rt', {}, 'zhòng'), '要', h('rt', {}, 'yào'), '组', h('rt', {}, 'zǔ'), '成', h('rt', {}, 'chéng'), '部', h('rt', {}, 'bù'), '分', h('rt', {}, 'fēn'), '。', h('rt', {}, '。'))),
                h('p', { class: 'fs-sm text-tertiary mt-sm' }, '正文 22px，注音 11px（约 1/2），line-height 2.6 给注音留空间。开启 text-spacing-trim 后句号不再顶头。'),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '380px', overflow: 'auto' } }, h('code', {}, s.cjkComboInfo || '（点击按钮查看 CJK 排版协同完整说明）')),
                h(Alert, {
                    type: 'info',
    ...
    _toggleVertical() {
        ...
        this._injectStyle('css-dict-vertical', `.dict-vertical { writing-mode: ${wm}; -webkit-writing-mode: ${wm}; text-orientation: mixed; ruby-position: over; -webkit-ruby-position: before; max-height: ${this._verticalMode ? '220px' : 'none'}; }`);
    ...
    _renderCard7() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['ruby-position', f.rubyPosition], ['ruby-align', f.rubyAlign]]), h(Tag, { color: 'primary' }, 'writing-mode 协同')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '实战：教育网站字典卡片包含汉字 + 拼音 + 五笔 + 部首 + 释义。Ruby 与垂直排版协同：writing-mode: vertical-rl + ruby-position: over（竖排时 over 对应「右侧」），适合中文古籍、书法、传统日文排版。配合 text-orientation: mixed 让 CJK 字符直立、拉丁字符旋转。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取字典卡片信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ dictCardInfo: this._readDictCardInfo() }) }), this._btn(`writing-mode（${this._verticalMode ? 'vertical-rl 竖排' : 'horizontal-tb 横排'}）`, { size: 'sm', onClick: () => this._toggleVertical() })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '字典卡片（汉字 + 拼音 + 五笔 + 部首 + 释义）：'),
                h('div', { class: 'dict-card' }, h('div', { class: 'word' }, h('ruby', {}, '汉', h('rt', {}, 'hàn'))), h('div', { class: 'meta' }, '部首：氵 · 五笔：IC · 笔画：5 · 注音：ㄏㄢˋ'), h('div', { class: 'meta', style: { marginTop: '4px' } }, '释义：'), h('div', { style: { fontSize: '14px', lineHeight: '1.8' } }, '1. 汉族，中华民族的统称；2. 男子，如「男子汉」；3. 银河，如「星汉灿烂」；4. 朝代名（汉朝）；5. 语言（汉语）。')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, 'Ruby 与垂直排版（writing-mode: vertical-rl + ruby-position: over，竖排时注音在右）：'),
                h('div', { class: 'dict-vertical' }, h('ruby', {}, '春', h('rt', {}, 'chūn'), '眠', h('rt', {}, 'mián'), '不', h('rt', {}, 'bù'), '觉', h('rt', {}, 'jué'), '晓', h('rt', {}, 'xiǎo'))),
                h('p', { class: 'fs-sm text-tertiary mt-sm' }, '竖排时 ruby-position: over 让注音位于竖排汉字「右侧」（横排时为上方）。适合古诗、古籍、书法排版。'),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '400px', overflow: 'auto' } }, h('code', {}, s.dictCardInfo || '（点击按钮查看字典卡片完整代码与说明）')),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard8() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['display:ruby', f.displayRuby], ['display:ruby-text', f.displayRubyText]]), h(Tag, { color: 'primary' }, 'A11y & Fallback')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'Ruby 可访问性：屏幕阅读器（NVDA/VoiceOver/JAWS）朗读 <ruby> 规则因浏览器而异（默认朗读基底，<rt> 可能被忽略或重复；可用 aria-label 显式指定朗读顺序）。<ruby> 元素隐含 role="ruby"，旧阅读器可用显式 role="ruby" + aria-label 兜底。<rp> 括号是优雅降级方案：支持时 display: none 隐藏，不支持时显示括号使注音作为正文。降级策略：<rp> 括号 → CSS @supports 渐进增强 → JS 检测 polyfill → aria-label 兜底。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取可访问性信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runA11yDemo() })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, 'role="ruby" + aria-label（屏幕阅读器整体朗读「汉字，读音 hàn yǔ」）：'),
                h('div', { class: 'ruby-stage a11y-ruby' }, h('ruby', { role: 'ruby', 'aria-label': '汉字，读音 hàn yǔ' }, h('rb', {}, '汉'), h('rt', {}, 'hàn'), h('rb', {}, '语'), h('rt', {}, 'yǔ'))),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '<rp> 括号降级（不支持 <ruby> 时显示「漢（かん）字（じ）」）：'),
                h('div', { class: 'ruby-stage a11y-ruby' }, h('ruby', {}, '漢', h('rp', {}, '（'), h('rt', {}, 'かん'), h('rp', {}, '）'), '字', h('rp', {}, '（'), h('rt', {}, 'じ'), h('rp', {}, '）'))),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '440px', overflow: 'auto' } }, h('code', {}, s.a11yFallbackInfo || '（点击「读取可访问性信息」查看屏幕阅读器规则与降级策略）')),
                h(Alert, {
                    type: 'success',
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'api-lab-page css-ruby-deep-page' }, h('h2', { class: 'section-title' }, 'CSS Ruby 注音排版 实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, '本页演示 CSS Ruby Annotation Layout Module 与 HTML <ruby>/<rt>/<rp>/<rb>/<rtc> 元素：ruby-position（over/under/inter-character）、ruby-align（start/center/space-between/space-around）、ruby-merge（separate/collapse/auto）、ruby-overhang（auto/none）、双层注音（<rtc>/<rb>）、CJK 排版协同（text-spacing-trim/line-break/字号比例）、实战字典卡片（汉字+拼音+五笔+垂直排版）、可访问性与降级（屏幕阅读器/<rp> 括号/@supports）。所有特性通过 typeof / CSS.supports 能力检测，不可用时仅记日志，绝不抛异常。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderCard6(), this._renderCard7(), this._renderCard8(), this._renderLogPanel());
    }
}
//# sourceMappingURL=CSSRubyDeepPage.js.map