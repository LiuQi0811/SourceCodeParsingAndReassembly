...
export class PointerTouchEventsDeepPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!f.setPointerCapture)
            this._addLog('warn', 'setPointerCapture 不可用（拖拽到元素外会丢失事件）');
        if (!f.touchAction)
            this._addLog('warn', 'touch-action 不可用（无法禁用默认手势）');
        if (f.maxTouchPoints > 0)
            this._addLog('info', `设备支持触摸（maxTouchPoints=${f.maxTouchPoints}）`);
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'Pointer Events（W3C Pointer Events Level 2）将鼠标/触摸/触控笔统一为单一 API：pointerdown/pointermove/pointerup/pointercancel/pointerover/pointerout/pointerenter/pointerleave。它是 Mouse Events 与 Touch Events 的超集，通过 pointerType: mouse|touch|pen 区分输入设备。优势：一套代码兼容桌面+移动+创意设备，多指针天然支持（pointerId），触控笔压力/倾斜统一暴露。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行概述演示', { type: 'primary', size: 'sm', onClick: () => this._runOverviewDemo() })),
                h('div', { class: 'pt-ov-host' }, h('div', { class: 'pt-ov-pill pt-ov-mouse' }, 'mouse 鼠标'), h('div', { class: 'pt-ov-pill pt-ov-touch' }, 'touch 触摸'), h('div', { class: 'pt-ov-pill pt-ov-pen' }, 'pen 触控笔')),
                ...
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.overviewInfo || '（点击按钮查看 Pointer Events 概述完整参考）')),
            ],
        });
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'PointerEvent 继承 MouseEvent（clientX/Y/screenX/Y/button 等），独有属性：pointerId 唯一标识（多指针跟踪）、pointerType 设备类型、width/height 接触区域（鼠标为 1）、pressure 压力 0-1（鼠标按住 0.5）、tangentialPressure 切向压力、tiltX/tiltY 触控笔倾斜角（-90 到 90）、twist 旋转角（0-359）、isPrimary 主指针。getCoalescedEvents() 返回高刷屏合并前的所有采样点（绘画必备），getPredictedEvents() 返回预测点（降延迟）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行属性演示', { type: 'primary', size: 'sm', onClick: () => this._runPropsDemo() })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '220px', overflow: 'auto' } }, h('code', {}, codeExample)),
                h('div', { class: 'fs-sm text-secondary' }, '完整参考：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.propsInfo || '（点击按钮查看 PointerEvent 属性全集完整参考）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _runCaptureDemo() {
        ...
        const info = [
            ...
            '',
            '【应用场景 2：自定义滑块（slider）】',
            '  thumb.addEventListener("pointerdown", (e: any) => {',
            '    thumb.setPointerCapture(e.pointerId);',
            '  });',
            '  thumb.addEventListener("pointermove", (e: any) => {',
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'element.setPointerCapture(pointerId) 捕获指针：即使指针移出元素边界仍持续接收 pointermove/pointerup。releasePointerCapture 释放、hasPointerCapture 检测、gotpointercapture/lostpointercapture 事件。应用：拖拽（pointerdown 捕获 → move 更新位置 → up 释放）、自定义滑块、画板。替代已废弃的 element.setCapture()，解决拖拽到元素外事件丢失问题。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行捕获演示', { type: 'primary', size: 'sm', onClick: () => this._runCaptureDemo() })),
                h('div', { class: 'pt-drag-host' }, dragBox),
                statusEl,
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, codeExample)),
                h('div', { class: 'fs-sm text-secondary' }, '完整参考：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.captureInfo || '（点击按钮查看 setPointerCapture 完整参考）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'Touch Events：touchstart/touchmove/touchend/touchcancel。三个 TouchList：touches（屏幕所有触摸点）、targetTouches（当前元素上）、changedTouches（触发本事件的）。Touch 对象属性：identifier（跟踪同一手指）/target/clientX/Y/pageX/Y/screenX/Y/radiusX/Y/rotationAngle/force。event.preventDefault() 阻止滚动/缩放（需 passive: false）。与 Pointer Events 对比：Touch Events 更早、iOS Safari 历史支持，但无触控笔压力细节，鼠标需另写。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 Touch API 演示', { type: 'primary', size: 'sm', onClick: () => this._runTouchApiDemo() })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '220px', overflow: 'auto' } }, h('code', {}, codeExample)),
                h('div', { class: 'fs-sm text-secondary' }, '完整参考：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.touchApiInfo || '（点击按钮查看 Touch Events API 完整参考）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    `);
        const info = [
            ...
            '  area.addEventListener("pointermove", (e: any) => {',
            '    // 不会因浏览器滚动而被打断',
            '  });',
            '',
            '【避免 300ms 点击延迟】',
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['touch-action', f.touchAction]]), h(Tag, { color: 'primary' }, 'CSS 属性')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'touch-action: auto|none|pan-x|pan-y|pan-left|pan-right|pan-up|pan-down|pinch-zoom|manipulation 控制元素上哪些触摸手势由浏览器默认处理。none 禁用所有默认手势（自定义手势必需），pan-x/pan-y 仅允许水平/垂直滚动，pinch-zoom 允许双指缩放，manipulation 仅允许滚动和缩放（禁用双击放大，消除 300ms 延迟）。与 Pointer Events 协同：先设 touch-action: none 再用 pointer 事件实现自定义手势。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 touch-action 演示', { type: 'primary', size: 'sm', onClick: () => this._runTouchActionDemo() })),
                h('div', { class: 'pt-ta-grid' }, h('div', { class: 'pt-ta-cell' }, [h('b', {}, 'none'), h('div', {}, '禁用所有默认手势')]), h('div', { class: 'pt-ta-cell' }, [h('b', {}, 'pan-x'), h('div', {}, '仅水平滚动')]), h('div', { class: 'pt-ta-cell' }, [h('b', {}, 'pan-y'), h('div', {}, '仅垂直滚动')]), h('div', { class: 'pt-ta-cell' }, [h('b', {}, 'pinch-zoom'), h('div', {}, '双指缩放')]), h('div', { class: 'pt-ta-cell' }, [h('b', {}, 'manipulation'), h('div', {}, '滚动+缩放，禁双击')])),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '240px', overflow: 'auto' } }, h('code', {}, codeExample)),
                h('div', { class: 'fs-sm text-secondary' }, '完整参考：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.touchActionInfo || '（点击按钮查看 touch-action 完整参考）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '多点触控跟踪用 Map 缓存 pointerId → 状态。pinch 缩放（两指距离变化计算 scale）、rotate 旋转（两指角度变化用 atan2 计算 rotation）、swipe 滑动（单指位移 + 速度阈值）、long press 长按（pointerdown 后 setTimeout 检测未移动）、double tap 双击（两次 pointerup 时间差）、drag 拖拽。手势状态机设计：idle → detecting → active → end。实战：图片缩放旋转、画板多指、卡片拖拽。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行手势识别演示', { type: 'primary', size: 'sm', onClick: () => this._runGestureDemo() })),
                viz,
                statusEl,
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '完整手势识别代码（pinch + rotate）：'),
                h('pre', { class: 'code-block', style: { maxHeight: '460px', overflow: 'auto' } }, h('code', {}, codeExample)),
                h('div', { class: 'fs-sm text-secondary' }, '完整参考：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.gestureInfo || '（点击按钮查看手势识别完整参考）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard7() {
        const s = this.state;
        const f = this._flags();
        const readoutEl = h('div', { class: 'pt-pen-readout' }, h('div', { class: 'pt-pen-cell' }, 'type: -'), h('div', { class: 'pt-pen-cell' }, 'pressure: -'), h('div', { class: 'pt-pen-cell' }, 'tiltX: -'), h('div', { class: 'pt-pen-cell' }, 'tiltY: -'), h('div', { class: 'pt-pen-cell' }, 'twist: -'), h('div', { class: 'pt-pen-cell' }, 'points: 0'));
        let pointCount = 0;
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'pointerType: "pen" 识别触控笔。pressure 压感（0-1）映射画笔粗细/透明度，tiltX/tiltY 倾斜角（-90 到 90）映射笔锋角度（模拟毛笔/铅笔），twist 旋转（0-359，马克笔/喷雾）。getCoalescedEvents() 获取高频采样点（120Hz 屏幕绘制更平滑曲线），贝塞尔曲线二次平滑，移动平均防抖。实战：签名板、绘画应用（Procreate 风格）、手写笔记，对接 Apple Pencil/Surface Pen。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行触控笔演示', { type: 'primary', size: 'sm', onClick: () => this._runPenDemo() })),
                canvas,
                readoutEl,
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '220px', overflow: 'auto' } }, h('code', {}, codeExample)),
                h('div', { class: 'fs-sm text-secondary' }, '完整参考：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.penInfo || '（点击按钮查看触控笔与压感完整参考）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _updatePenReadout(readoutEl, e, count) {
        if (!readoutEl || !readoutEl.children || readoutEl.children.length < 6)
            return;
        try {
    ...
    _runPatternDemo() {
        ...
        const info = [
            ...
            '  el.addEventListener("pointerdown", (e: any) => recognizers.forEach(r => r.onDown(e)));',
            '',
    ...
    _renderCard8() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'primary' }, '实战'), h(Tag, { color: f.setPointerCapture ? 'success' : 'error' }, `setPointerCapture ${f.setPointerCapture ? '✓' : '✗'}`)),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '完整模式：拖拽（setPointerCapture + transform）、自定义滑块、画板（canvas + pointer + 平滑曲线）、手势识别库设计、响应式交互（鼠标 vs 触摸差异）。陷阱清单：iOS Safari touch-action 滞后 / pointermove 高频需节流或 getCoalescedEvents / 300ms 延迟需 touch-action: manipulation / pointercancel 必须处理 / 多指针用 Map / passive: true 无法 preventDefault / 触控笔压感需设备支持。降级：不支持 Pointer Events 用 mouse + touch 双监听。无障碍：手势需键盘等价操作。'),
                ...
                h('div', { class: 'fs-sm text-secondary' }, '完整参考：'),
                h('pre', { class: 'code-block', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.patternInfo || '（点击按钮查看实战模式与陷阱完整参考）')),
            ],
        });
        this.registerChild(card);
        return card.render();
...
//# sourceMappingURL=PointerTouchEventsDeepPage.js.map