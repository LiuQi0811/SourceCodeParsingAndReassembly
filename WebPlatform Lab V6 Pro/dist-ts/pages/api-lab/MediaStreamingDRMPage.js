...
export class MediaStreamingDRMPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!f.mediaSource)
            this._addLog('warn', 'MediaSource 不可用（jsdom 无原生媒体栈，Chrome 23+/Firefox 42+/Safari 8+/Edge 全部稳定多年）');
        if (!f.sourceBuffer)
            this._addLog('warn', 'SourceBuffer 不可用（MediaSource 的核心子对象，jsdom 无）');
        if (!f.mediaKeys)
            this._addLog('warn', 'MediaKeys 不可用（EME 的密钥管理对象，jsdom 无）');
        if (!f.requestMediaKeySystemAccess)
            this._addLog('warn', 'navigator.requestMediaKeySystemAccess 不可用（EME 入口，jsdom 无）');
        if (!f.mediaKeySession)
            this._addLog('warn', 'MediaKeySession 不可用（DRM 会话对象，jsdom 无）');
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            children: [
                ...
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// 传统 <video src> —— 无法分片/码率切换
...
mediaSource.addEventListener('sourceopen', () => {
  ...
  fetchSegment(url).then((buf: any) => sb.appendBuffer(buf));
});
...
session.addEventListener('message', e => fetchLicense(e.data).then((r: any) => session.update(r)));
session.generateRequest('cenc', initData);`)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.overviewInfo || '（点击按钮查看 MSE + EME 概述与动机完整说明）')),
            ],
        });
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'new MediaSource() 构造（readyState: closed→open→ended），URL.createObjectURL(mediaSource) 拿 blob: URL 赋给 video.src。监听 sourceopen 后 addSourceBuffer(codec) 创建 SourceBuffer。appendBuffer(ArrayBuffer/ArrayBufferView) 追加分片，触发 updateend；remove(start, end) 移除已播放分片节省内存。endOfStream() 标记流结束。duration 设置总时长（直播流设 Infinity）。appendBuffer 并发会抛 InvalidStateError，需等 updating=false。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行生命周期演示', { type: 'primary', size: 'sm', onClick: () => this._runLifecycleDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `const mediaSource = new MediaSource();
video.src = URL.createObjectURL(mediaSource);
...
mediaSource.addEventListener('sourceopen', async () => {
  ...
  mediaSource.endOfStream();   // 流结束
                ...
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.lifecycleInfo || '（点击按钮查看 MediaSource 生命周期完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
...
sb.changeType('video/webm; codecs="vp9"');`)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.sourceBufferInfo || '（点击按钮查看 SourceBuffer 与 ArrayBuffer 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
...
if (videoSb.changeType) {
  ...
  videoSb.appendBuffer(vp9InitSegment);
                ...
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.adaptiveInfo || '（点击按钮查看自适应码率切换完整方案）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    `);
        ...
        const info = [
            ...
            '    video.addEventListener("encrypted", async (e: any) => {',
            '      const session = keys.createSession();',
            '      session.addEventListener("message", async (msg: any) => {',
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'EME 协议分层：Application（业务/license 交换）→ EME API（MediaKeys/MediaKeySession）→ CDM（厂商解密模块）→ Media（MSE SourceBuffer）。navigator.requestMediaKeySystemAccess(keySystem, configs) 探测 DRM 系统（com.widevine.alpha/com.microsoft.playready/com.apple.fps/org.w3.clearkey），createMediaKeys 创建 MediaKeys，setMediaKeys 绑定到 video，createSession 创建会话，generateRequest 发起 license 请求。encryptionScheme（cenc/cbcs/cbc1/cens）+ robustness（SW/HW_SECURE_*）控制安全级别。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 EME 基础演示', { type: 'primary', size: 'sm', onClick: () => this._runEMEBasicsDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// EME 初始化流程
const access = await navigator.requestMediaKeySystemAccess(
  'com.widevine.alpha',
  [{
    initDataTypes: ['cenc'],
...
video.addEventListener('encrypted', async (e: any) => {
  ...
  session.addEventListener('keystatuseschange', () => {
    session.keyStatuses.forEach((status: any, keyId: number) => {
  ...
  await session.generateRequest(e.initDataType, e.initData);
                ...
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.emeBasicsInfo || '（点击按钮查看 EME 基础与 Encrypted Media 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '三大 DRM 系统：Widevine（Google，Chrome/Android/Firefox，L1 硬件/L3 软件）、PlayReady（Microsoft，Edge/IE，SL2000 软件/SL3000 硬件）、FairPlay Streaming（Apple，Safari/iOS，必须 cbcs + setServerCertificate）。license server 交互：session.message 事件触发，POST challenge 到 server，返回 license 用 session.update 注入。persistent-license 持久化到 IndexedDB（断网可播放），load/remove 管理。多 DRM 兼容：服务端打包多 PSSH，客户端按 keySystem 探测。防盗链需 token + 设备指纹。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 DRM 系统对比演示', { type: 'primary', size: 'sm', onClick: () => this._runDRMSystemsDemo() })),
                // 三大 DRM 速查矩阵
                h('div', { class: 'mse-matrix' }, h('div', { class: 'mse-matrix-cell' }, h('div', { class: 'mse-matrix-title' }, 'Widevine'), h('div', {}, 'Google · Chrome/Android · L1硬件/L3软件 · cenc/cbcs')), h('div', { class: 'mse-matrix-cell' }, h('div', { class: 'mse-matrix-title' }, 'PlayReady'), h('div', {}, 'Microsoft · Edge/IE · SL2000软件/SL3000硬件 · cenc')), h('div', { class: 'mse-matrix-cell' }, h('div', { class: 'mse-matrix-title' }, 'FairPlay'), h('div', {}, 'Apple · Safari/iOS · 必须cbcs + FPS证书')), h('div', { class: 'mse-matrix-cell' }, h('div', { class: 'mse-matrix-title' }, 'ClearKey'), h('div', {}, 'W3C 调试用 · 全平台 · 明文 key'))),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// 探测 Widevine 安全级别（L1 vs L3）
const access = await navigator.requestMediaKeySystemAccess(
  'com.widevine.alpha',
  [{
    videoCapabilities: [{
...
for (const ks of ['com.widevine.alpha', 'com.microsoft.playready', 'com.apple.fps']) {
  try { return await navigator.requestMediaKeySystemAccess(ks, configs); }
                ...
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.drmSystemsInfo || '（点击按钮查看三大 DRM 系统对比完整说明）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    `);
        ...
        const info = [
            ...
            '      });',
            '      session.addEventListener("keystatuseschange", () => {',
            '        session.keyStatuses.forEach((status: any, kid: number) => {',
    ...
    _renderCard7() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'license 交换时序：encrypted 事件 → generateRequest(initDataType, initData) → message 事件（license challenge）→ fetch license server → session.update(license) → keystatuseschange 事件（status="usable" 后 CDM 解密）。initDataType：cenc（PSSH）/ webm / keyids（ClearKey 调试）。messageType：license-request/renewal/release。ClearKey 用明文 key 调试，无需真实 license server。keystatuseschange 的 status：usable/expired/output-not-allowed（HDCP 失败）等。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 license 交换演示', { type: 'primary', size: 'sm', onClick: () => this._runLicenseDemo() })),
                // license 交换流程图
                h('div', { class: 'mse-flow' }, h('span', { class: 'mse-flow-node' }, 'encrypted'), h('span', { class: 'mse-flow-arrow' }, '→'), h('span', { class: 'mse-flow-node mse-flow-node--eme' }, 'generateRequest'), h('span', { class: 'mse-flow-arrow' }, '→'), h('span', { class: 'mse-flow-node mse-flow-node--eme' }, 'message'), h('span', { class: 'mse-flow-arrow' }, '→'), h('span', { class: 'mse-flow-node' }, 'license server'), h('span', { class: 'mse-flow-arrow' }, '→'), h('span', { class: 'mse-flow-node mse-flow-node--eme' }, 'update'), h('span', { class: 'mse-flow-arrow' }, '→'), h('span', { class: 'mse-flow-node mse-flow-node--eme' }, 'keystatuseschange'), h('span', { class: 'mse-flow-arrow' }, '→'), h('span', { class: 'mse-flow-node' }, 'usable')),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// 完整 license 交换流程
video.addEventListener('encrypted', async (e: any) => {
  const session = keys.createSession('temporary');
...
const ckAccess = await navigator.requestMediaKeySystemAccess(
  'org.w3.clearkey', [{
    ...
    videoCapabilities: [{ contentType: 'video/mp4; codecs="avc1.42E01E"' }],
                ...
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.licenseInfo || '（点击按钮查看 license 交换流程完整说明）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    `);
        const info = [
            ...
            '  ];',
            '  const codec = codecs.find((c: any) => MediaSource.isTypeSupported(c));',
    ...
    _renderCard8() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '实战：HLS.js（m3u8 解析 + ts fetch + MSE append）与 Shaka Player（DASH mpd）封装完整流程，Safari 原生 HLS 用 video.src。防盗链需 license server 校验 JWT token + Origin 白名单 + 限流。跨域 CORS 必须配置（Access-Control-Allow-Origin + credentials）。MediaSource 跨浏览器 codec 兼容用 isTypeSupported 探测优先级。seek 时检查 buffered 范围，未缓冲需 append 新分片。ClearKey 用 ffmpeg 加密 + 明文 key 调试。10 大陷阱清单覆盖 iOS Safari 限制、FairPlay 证书、CORS、seek、changeType、persistent-license。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行实战与陷阱演示', { type: 'primary', size: 'sm', onClick: () => this._runPitfallsDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// HLS.js 实战
if (video.canPlayType('application/vnd.apple.mpegurl')) {
...
async function fetchLicense(challenge: any) {
  const resp = await fetch('https://license.example.com/widevine', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/octet-stream',
...
].find((c: any) => MediaSource.isTypeSupported(c));
const sb = mediaSource.addSourceBuffer(codec);`)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.pitfallsInfo || '（点击按钮查看实战与陷阱完整说明）')),
                h(Alert, {
                    type: 'warning',
...
//# sourceMappingURL=MediaStreamingDRMPage.js.map