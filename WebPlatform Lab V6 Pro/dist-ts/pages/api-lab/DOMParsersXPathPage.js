...
const XPR = (() => {
    ...
    if (typeof XPathResult !== 'undefined') {
        for (const [k, v] of Object.entries(fallback)) {
            if (typeof (XPathResult[k]) === 'undefined') {
                try {
                    (XPathResult[k]) = v;
                }
                catch { /* 只读则忽略 */ }
...
export class DOMParsersXPathPage extends Page {
    ...
    _detectParserError(doc) {
        if (!doc)
            return null;
        try {
            const direct = doc.querySelector('parsererror');
            if (direct)
                return direct.textContent.trim().slice(0, 200);
    ...
    _parseHtml() {
        ...
        try {
            ...
            if (!doc) {
                this.setState({ parseResult: '解析失败（返回 null）' });
                return;
            }
    ...
    _parseSvg() {
        if (typeof DOMParser === 'undefined') {
            this._addLog('warn', 'DOMParser 不可用');
            return;
        }
        try {
            ...
            if (!doc) {
                this.setState({ parseResult: 'SVG 解析失败（返回 null）' });
                return;
            }
    ...
    _parseErrorXml() {
        ...
        try {
            ...
            if (!doc) {
                this.setState({ parseResult: '错误 XML 解析失败（返回 null）' });
                return;
            }
            const errText = this._detectParserError(doc);
            const lines = [
    ...
    _renderCard1() {
        return this._card('1. DOMParser（HTML / XML / SVG 解析）', 'new DOMParser().parseFromString(string, mimeType)；mimeType 取 text/html（返回完整 Document，可 getElementById）/ application/xml / image/svg+xml / text/xml。text/html 自动补 html/head/body；XML 模式解析错误产生 <parsererror> 元素。', h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'primary' }, 'DOMParser'), h(Tag, { color: 'success' }, 'text/html'), h(Tag, { color: 'warning' }, 'image/svg+xml'), h(Tag, { color: 'default' }, 'application/xml')), [
            h('div', { class: 'flex gap-sm flex-wrap' }, this._btn('解析 HTML 片段', { type: 'primary', size: 'sm', onClick: () => this._parseHtml() }), this._btn('解析 SVG 片段', { size: 'sm', onClick: () => this._parseSvg() }), this._btn('故意解析错误 XML', { size: 'sm', onClick: () => this._parseErrorXml() })),
    ...
    _serializeFragment() {
        if (typeof XMLSerializer === 'undefined') {
            this._addLog('warn', 'XMLSerializer 不可用');
            return;
        }
    ...
    _renderCard2() {
        return this._card('2. XMLSerializer（序列化）', 'new XMLSerializer().serializeToString(node) → string，可序列化 Document/Element/DocumentFragment。element.outerHTML/innerHTML 为 HTML 模式（void 元素无闭合）；XML 模式保留命名空间并以自闭合形式输出空元素。', h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'primary' }, 'XMLSerializer'), h(Tag, { color: 'success' }, 'serializeToString'), h(Tag, { color: 'warning' }, 'outerHTML'), h(Tag, { color: 'warning' }, 'innerHTML')), [
            h('div', { class: 'flex gap-sm flex-wrap' }, this._btn('序列化当前页面元素', { type: 'primary', size: 'sm', onClick: () => this._serializeElement() }), this._btn('序列化 DocumentFragment', { size: 'sm', onClick: () => this._serializeFragment() }), this._btn('对比 innerHTML vs serializeToString', { size: 'sm', onClick: () => this._compareInnerHtml() })),
    ...
    _renderCard3() {
        return this._card('3. XPath 查询（document.evaluate）', 'document.evaluate(xpath, contextNode, nsResolver, resultType, result)。resultType 取 XPathResult 常量：ANY_TYPE(0)/NUMBER(1)/STRING(2)/BOOLEAN(3)/UNORDERED_NODE_ITERATOR(4)/ORDERED_ITERATOR(5)/UNORDERED_SNAPSHOT(6)/ORDERED_SNAPSHOT(7)/ANY_UNORDERED_NODE(8)/FIRST_ORDERED_NODE(9)。迭代器用 iterateNext()，快照用 snapshotItem(i)/snapshotLength，标量用 numberValue/stringValue/booleanValue/singleNodeValue。', h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'primary' }, 'evaluate'), h(Tag, { color: 'success' }, 'iterateNext'), h(Tag, { color: 'success' }, 'snapshotItem'), h(Tag, { color: 'warning' }, 'count()'), h(Tag, { color: 'warning' }, 'singleNodeValue')), [
            h('div', { class: 'flex gap-sm flex-wrap' }, this._btn('evaluate 迭代器', { type: 'primary', size: 'sm', onClick: () => this._xpathIterator() }), this._btn('evaluate 快照', { type: 'primary', size: 'sm', onClick: () => this._xpathSnapshot() }), this._btn('evaluate NUMBER count()', { size: 'sm', onClick: () => this._xpathNumber() }), this._btn('evaluate FIRST_ORDERED_NODE', { size: 'sm', onClick: () => this._xpathFirstNode() })),
    ...
    _fragBuild() {
        ...
        try {
            ...
            const items = Array.from(holder.children).map((li) => `<li data-idx="${li.getAttribute('data-idx')}">${li.textContent}</li>`);
            const lines = [
    ...
    _createHTMLDoc() {
        ...
        try {
            ...
            if (typeof document.implementation.createDocument === 'function') {
                try {
                    xmlDoc = document.implementation.createDocument('urn:demo', 'root', null);
                }
                catch (e) {
                    this._addLog('warn', `createDocument 抛错：${e.message}`);
                }
            }
            const lines = [
    ...
    _templateClone() {
        ...
        try {
            ...
            const clone1 = content.cloneNode(true); // 深克隆
            ...
            const serClone = (frag) => (ser ? ser.serializeToString(frag) : '(无 XMLSerializer)');
            const lines = [
    ...
    _renderCard4() {
        return this._card('4. DocumentFragment + createDocument + 模板', 'document.createDocumentFragment() 创建轻量文档片段；document.implementation.createDocument(ns, qName, doctype) 创建独立 XML Document；createHTMLDocument(title) 创建独立 HTML Document；<template>.content 为 DocumentFragment，template.content.cloneNode(true) 克隆模板后插入 DOM。', h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'primary' }, 'createDocumentFragment'), h(Tag, { color: 'success' }, 'implementation'), h(Tag, { color: 'success' }, 'createHTMLDocument'), h(Tag, { color: 'warning' }, 'template.content')), [
            h('div', { class: 'flex gap-sm flex-wrap' }, this._btn('createDocumentFragment 批量构建', { type: 'primary', size: 'sm', onClick: () => this._fragBuild() }), this._btn('createHTMLDocument', { size: 'sm', onClick: () => this._createHTMLDoc() }), this._btn('template 克隆', { size: 'sm', onClick: () => this._templateClone() })),
    ...
    _rangeSelectContents() {
        ...
        const container = this.$('.dx-range-target');
        if (!container) {
            this._addLog('warn', 'Range 演示容器不存在');
            return;
        }
        try {
            ...
            const lines = [
                ...
                `getBoundingClientRect: ${rect ? `w=${rect.width.toFixed(0)}, h=${rect.height.toFixed(0)}` : '(不可用，jsdom 无布局)'}`,
                setStartInfo,
                '',
    ...
    _rangeExtract() {
        ...
        try {
            ...
            try {
                ...
                this._addLog('warn', `insertNode 失败：${e.message}`);
            }
            const lines = [
    ...
    _renderCard5() {
        return this._card('5. Range + Selection 编辑（与解析结合）', 'document.createRange() 创建 Range；setStart(node,offset)/setEnd(node,offset)/selectNode(node)/selectNodeContents(node)；deleteContents/extractContents（返回 DocumentFragment）/insertNode/surroundContents/cloneContents/getBoundingClientRect。window.getSelection()/removeAllRanges()/addRange(range)/getRangeAt(i)/toString() 管理用户选区。', h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'primary' }, 'Range'), h(Tag, { color: 'success' }, 'selectNodeContents'), h(Tag, { color: 'success' }, 'extractContents'), h(Tag, { color: 'warning' }, 'surroundContents'), h(Tag, { color: 'warning' }, 'Selection')), [
            h('div', { class: 'flex gap-sm flex-wrap' }, this._btn('selectNodeContents', { type: 'primary', size: 'sm', onClick: () => this._rangeSelectContents() }), this._btn('surroundContents 包裹 mark', { size: 'sm', onClick: () => this._rangeSurroundMark() }), this._btn('extractContents 提取', { size: 'sm', onClick: () => this._rangeExtract() }), this._btn('getSelection 读取选中文本', { size: 'sm', onClick: () => this._getSelectionText() })),
            h('div', { class: 'fs-sm text-secondary mt-sm' }, 'Range / Selection 操作目标容器：'),
            h('div', { class: 'dx-range-target', style: { padding: '8px', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-base)', minHeight: '32px' } }, 'Range 演示目标（点击按钮在此容器上执行 selectNodeContents / surroundContents / extractContents / Selection）'),
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'page api-lab-page' }, h('h2', { class: 'section-title' }, 'DOM 解析与 XPath 实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, '演示 DOMParser / XMLSerializer / XPath evaluate / DocumentFragment + implementation / Range + Selection 编辑。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderLogPanel());
    }
}
//# sourceMappingURL=DOMParsersXPathPage.js.map