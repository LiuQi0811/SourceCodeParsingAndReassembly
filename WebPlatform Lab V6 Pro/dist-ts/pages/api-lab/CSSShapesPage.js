...
export class CSSShapesPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!f.circle)
            this._addLog('warn', 'shape-outside: circle() 不可用或 jsdom 未识别（现代浏览器全支持：Chrome 37+/Firefox 62+/Safari 11.1+）');
        if (!f.polygon)
            this._addLog('warn', 'shape-outside: polygon() 不可用或 jsdom 未识别（现代浏览器全支持）');
        if (!f.shapeMargin)
            this._addLog('warn', 'shape-margin 不可用或 jsdom 未识别（现代浏览器全支持）');
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'CSS Shapes Module Level 1（W3C CR 稳定）弥补传统矩形排版局限，允许文本环绕非矩形形状（圆/椭圆/多边形/图像 Alpha 抠图）。提供 shape-outside（定义浮动元素外部环绕形状）、shape-margin（形状外边距）、shape-image-threshold（图像 Alpha 阈值）。与 clip-path 区别：shape-outside 影响布局（文字环绕路径），clip-path 影响渲染（元素可见区域），两者常配合使用。浏览器支持全部稳定（Chrome 37+/Firefox 62+/Safari 11.1+）。核心限制：仅对浮动元素（float 非 none）生效，必须设置 width/height，不影响 flex/grid 子项。'),
                ...
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.overviewInfo || '（点击「读取概述信息」查看 CSS Shapes Module Level 1 全景）')),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'shape-outside 定义浮动元素外部环绕形状，取值：none | [ <basic-shape> || <shape-box> ] | <image>。basic-shape 含 circle()/ellipse()/inset()/polygon()，shape-box 含 margin-box/border-box/padding-box/content-box，image 为 url() 或 gradient。⚠ 必须配合 float: left/right（float: none 无效，flex/grid 子项不浮动无效）。默认参考 margin-box，可显式 shape-outside: circle() margin-box。决定后续内联内容环绕路径，文字沿形状边界流动。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 shape-outside 演示', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runShapeOutsideDemo() })),
                h('div', { class: 'shapes-stage' }, h('div', { class: 'so-basic-shape' }), h('div', { class: 'so-basic-text' }, '这是一段演示文字，会环绕左侧的圆形浮动元素。CSS Shapes Module Level 1 让 Web 排版突破矩形限制，文字可以沿圆形、椭圆、多边形或图像 Alpha 通道的形状边界流动，实现杂志风格的精美排版。真实浏览器中可看到文字沿圆形边界流动的效果（jsdom 不做真实布局）。shape-outside 必须配合 float 使用，仅对浮动元素生效。')),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.shapeOutsideInfo || '（点击「运行 shape-outside 演示」查看完整说明）')),
                h(Alert, {
                    type: 'warning',
    ...
    _setCircleMode(mode) {
        ...
        if (mode === 'circle') {
            this._injectStyle('css-shapes-circle-dynamic', `.shapes-stage .so-circle { shape-outside: circle(${this._circleRadius} at 50% 50%); }` +
                `.shapes-stage .so-ellipse { display: none; }`);
        }
        else {
            this._injectStyle('css-shapes-circle-dynamic', `.shapes-stage .so-ellipse { shape-outside: ellipse(50% 50% at 50% 50%); display: block; }` +
    ...
    _setCircleRadius(radius) {
        ...
        if (this._circleMode === 'circle') {
            this._injectStyle('css-shapes-circle-dynamic', `.shapes-stage .so-circle { shape-outside: circle(${radius} at 50% 50%); }`);
        }
        this.setState({ circleInfo: this._readCircleInfo() });
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['circle()', f.circle], ['ellipse()', f.ellipse]]), h(Tag, { color: 'primary' }, '圆/椭圆')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'circle([radius] [at position])：radius 支持 <length>/<percentage>/closest-side（圆心到最近边）/farthest-side（到最远边），position 支持 <length>/<percentage>/left|center|right|top|bottom。默认 circle(closest-side at center)。ellipse(rx ry [at position]) 两个半径 rx 水平 ry 垂直。percentage 相对参考盒（circle 相对较短边，ellipse rx 相对宽 ry 相对高）。circle(50% at 50% 50%) 是常见完整写法。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ circleInfo: this._readCircleInfo() }) }), this._btn('circle()', { size: 'sm', disabled: !f.circle, onClick: () => this._setCircleMode('circle') }), this._btn('ellipse()', { size: 'sm', disabled: !f.ellipse, onClick: () => this._setCircleMode('ellipse') }), this._btn('半径 50%', { size: 'sm', disabled: !f.circle, onClick: () => this._setCircleRadius('50%') }), this._btn('closest-side', { size: 'sm', disabled: !f.circle, onClick: () => this._setCircleRadius('closest-side') }), this._btn('farthest-side', { size: 'sm', disabled: !f.circle, onClick: () => this._setCircleRadius('farthest-side') })),
                h('div', { class: 'shapes-stage' }, h('div', { class: 'so-circle' }), h('div', { class: 'so-ellipse', style: { display: this._circleMode === 'ellipse' ? 'block' : 'none' } }), h('div', { class: 'so-basic-text' }, 'circle() 与 ellipse() 演示文字。当前模式：' + this._circleMode + '，半径：' + this._circleRadius + '。真实浏览器中文字会沿圆形或椭圆边界流动。circle(50% at 50% 50%) 是最常见的圆形环绕写法，ellipse(50% 50% at 50% 50%) 用于椭圆。closest-side 和 farthest-side 是相对参考盒的智能半径，适合响应式布局。')),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.circleInfo || '（点击按钮切换 circle/ellipse 与半径）')),
                h(Alert, {
                    type: 'info',
    ...
    _setPolygonMode(mode) {
        ...
        if (mode === 'triangle') {
            this._injectStyle('css-shapes-polygon-dynamic', `.shapes-stage .so-polygon { display: block; clip-path: polygon(50% 0, 100% 100%, 0 100%); shape-outside: polygon(50% 0, 100% 100%, 0 100%); }` +
                `.shapes-stage .so-polygon-diamond { display: none; }`);
        }
        else if (mode === 'diamond') {
            this._injectStyle('css-shapes-polygon-dynamic', `.shapes-stage .so-polygon-diamond { display: block; clip-path: polygon(50% 0, 100% 50%, 50% 100%, 0 50%); shape-outside: polygon(50% 0, 100% 50%, 50% 100%, 0 50%); }` +
                `.shapes-stage .so-polygon { display: none; }`);
        }
        else if (mode === 'inset') {
            this._injectStyle('css-shapes-polygon-dynamic', `.shapes-stage .so-inset { shape-outside: inset(10px round ${this._insetRound}); display: block; }` +
                `.shapes-stage .so-polygon { display: none; }` +
                `.shapes-stage .so-polygon-diamond { display: none; }`);
        }
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['inset()', f.inset], ['polygon()', f.polygon]]), h(Tag, { color: 'primary' }, '与 clip-path 共用')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'inset(边距 [round 圆角])：1-4 个边距值（同 margin 语法）+ 可选 round 圆角。inset(10px round 20px) 内缩 10px 圆角 20px，inset(0 round 50%) 等价 circle(50%)。polygon(顶点列表)：至少 3 个 x y 顶点对，坐标原点参考盒左上角，percentage 相对宽/高。polygon(50% 0, 100% 100%, 0 100%) 三角形，polygon(50% 0, 100% 50%, 50% 100%, 0 50%) 菱形。inset()/polygon() 语法在 shape-outside 和 clip-path 完全相同，常配合让视觉与环绕一致。fill-rule: nonzero（默认）/evenodd。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ insetPolygonInfo: this._readInsetPolygonInfo() }) }), this._btn('inset 圆角', { size: 'sm', disabled: !f.inset, onClick: () => this._setPolygonMode('inset') }), this._btn('三角形', { size: 'sm', disabled: !f.polygon, onClick: () => this._setPolygonMode('triangle') }), this._btn('菱形', { size: 'sm', disabled: !f.polygon, onClick: () => this._setPolygonMode('diamond') })),
                h('div', { class: 'shapes-stage' }, h('div', { class: 'so-inset', style: { display: this._polygonMode === 'inset' ? 'block' : 'none' } }), h('div', { class: 'so-polygon', style: { display: this._polygonMode === 'triangle' ? 'block' : 'none' } }), h('div', { class: 'so-polygon-diamond', style: { display: this._polygonMode === 'diamond' ? 'block' : 'none' } }), h('div', { class: 'so-basic-text' }, 'inset() 与 polygon() 演示文字。当前模式：' + this._polygonMode + '。真实浏览器中文字会沿矩形圆角、三角形或菱形边界流动。inset(10px round 20px) 适合圆角卡片环绕，polygon() 适合任意多边形装饰环绕。与 clip-path 共用语法，常配合使用让视觉裁剪与文字环绕一致。')),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.insetPolygonInfo || '（点击按钮切换 inset/三角形/菱形）')),
                h(Alert, {
                    type: 'info',
    ...
    _setShapeMargin(value) {
        ...
        this._injectStyle('css-shapes-margin-dynamic', `.shapes-stage .so-margin { shape-margin: ${value}; }`);
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['shape-margin', f.shapeMargin], ['shape-image-threshold', f.shapeImageThreshold]]), h(Tag, { color: 'primary' }, 'Alpha 阈值')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'shape-margin: <length-percentage> 定义形状外边距（文字与形状边界最小距离，跟随形状轮廓非矩形）。shape-margin: 20px 让文字距圆形 20px 流动。shape-image-threshold: <number>（0.0-1.0）定义图像形状 Alpha 阈值，仅对 shape-outside: url(image) 生效。像素 Alpha ≥ 阈值为形状内（文字避开），< 阈值为形状外（文字流入）。0.0 默认（仅完全不透明），0.5 适合抗锯齿边缘。与 PNG Alpha 通道协同实现抠图环绕。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('读取信息', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this.setState({ marginThresholdInfo: this._readMarginThresholdInfo() }) }), this._btn('margin 0', { size: 'sm', disabled: !f.shapeMargin, onClick: () => this._setShapeMargin('0') }), this._btn('margin 10px', { size: 'sm', disabled: !f.shapeMargin, onClick: () => this._setShapeMargin('10px') }), this._btn('margin 20px', { size: 'sm', disabled: !f.shapeMargin, onClick: () => this._setShapeMargin('20px') }), this._btn('margin 40px', { size: 'sm', disabled: !f.shapeMargin, onClick: () => this._setShapeMargin('40px') })),
                h('div', { class: 'flex items-center gap-sm flex-wrap mt-sm' }, h('span', { class: 'fs-sm text-secondary' }, 'shape-image-threshold：'), this._btn('0.0', { size: 'sm', disabled: !f.shapeImageThreshold, onClick: () => this._setImageThreshold('0.0') }), this._btn('0.5', { size: 'sm', disabled: !f.shapeImageThreshold, onClick: () => this._setImageThreshold('0.5') }), this._btn('1.0', { size: 'sm', disabled: !f.shapeImageThreshold, onClick: () => this._setImageThreshold('1.0') })),
                h('div', { class: 'shapes-stage' }, h('div', { class: 'so-margin' }), h('div', { class: 'so-margin-text' }, 'shape-margin 演示文字。当前 shape-margin：' + this._shapeMargin + '。真实浏览器中文字会距圆形边界 ' + this._shapeMargin + ' 流动，留出呼吸空间提升可读性。shape-margin 跟随形状轮廓（圆形外为同心圆），与 margin（矩形盒外边距）不同。shape-image-threshold：' + this._imageThreshold + '（仅对 url() 图像形状生效）。')),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.marginThresholdInfo || '（点击按钮切换 shape-margin / threshold）')),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['shape-image-threshold', f.shapeImageThreshold]]), h(Tag, { color: 'primary' }, 'PNG 抠图')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'shape-outside: url(image.png) 引用图像，形状由 Alpha 通道决定（Alpha ≥ shape-image-threshold 为形状内文字避开，< 阈值为形状外文字流入）。适合不规则抠图（人物剪影、物体轮廓）、抗锯齿 PNG。gradient 也可作为图像（radial-gradient 等价 circle）。⚠ 跨域图像需 CORS 头或 crossorigin="anonymous"，否则无效。与 mask-image 区别：shape-outside 影响布局（文字环绕），mask-image 影响渲染（裁剪可见），常配合让视觉与环绕一致。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行图像形状演示', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runImageShapeDemo() }), this._btn('threshold 0.0', { size: 'sm', disabled: !f.shapeImageThreshold, onClick: () => this._setImageThreshold('0.0') }), this._btn('threshold 0.5', { size: 'sm', disabled: !f.shapeImageThreshold, onClick: () => this._setImageThreshold('0.5') }), this._btn('threshold 0.8', { size: 'sm', disabled: !f.shapeImageThreshold, onClick: () => this._setImageThreshold('0.8') })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '当前 shape-image-threshold：' + this._imageThreshold),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.imageShapeInfo || '（点击按钮查看图像形状完整说明与代码）')),
                h(Alert, {
                    type: 'warning',
    ...
    _renderCard7() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '杂志式排版三大场景：圆形头像文字环绕（float: left + shape-outside: circle() + border-radius: 50%）、多边形装饰元素环绕（float: right + polygon() + clip-path 配合）、不规则图像抠图环绕（shape-outside: url(mask.png) + shape-image-threshold + mask-image 配合）。与 float 协同：float: left 形状在左文字右环绕，float: right 反之。⚠ Shapes 仅对浮动元素生效，Grid/Flex 子项不浮动无效，需在普通块级容器内用 float。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行杂志排版演示', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runMagazineDemo() })),
                h('div', { class: 'magazine-stage' }, h('div', { class: 'avatar' }, 'A'), h('div', { class: 'decor-polygon' }), h('div', { class: 'article' }, '杂志式排版演示：圆形头像（A）浮动在左侧，文字沿圆形边界环绕；菱形装饰浮动在右侧，文字沿多边形边界穿插。CSS Shapes Module Level 1 让 Web 排版突破矩形限制，实现杂志、书籍、新闻报道的精美排版效果。真实浏览器中可看到文字沿圆形与菱形边界流动的效果。shape-outside 必须配合 float 使用，仅对浮动元素生效。多个浮动形状可叠加，文字依次环绕。注意：Shapes 不影响 Grid/Flex 子项，需在普通块级容器内使用 float。')),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.magazineInfo || '（点击按钮查看 3 大杂志排版场景代码）')),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard8() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '七大陷阱：shape-outside 仅对浮动元素生效（float: none/absolute/flex/grid 无效）；必须设置 width/height（否则参考盒尺寸 0）；形状参考盒模型（默认 margin-box，可 border-box/padding-box/content-box，显式 shape-outside: circle() margin-box）；DevTools Shapes Editor 可视化拖拽（Chrome/Firefox）；响应式适配（percentage + 媒体查询小屏 float: none）；与 Writing Modes 协同（坐标不随 writing-mode 翻转）；性能考虑（< 10 个形状、circle 优于 polygon、避免大图 url）。最佳实践 10 条。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行陷阱与最佳实践演示', { type: 'primary', size: 'sm', disabled: !f.css, onClick: () => this._runPitfallsDemo() }), ...boxes.map((box) => this._btn(box, { size: 'sm', onClick: () => this._setBoxModel(box) }))),
                h('div', { class: 'boxmodel-stage' }, h('div', { class: 'bm-box' }, h('div', { class: 'bm-label bm-margin' }, 'margin-box（默认）'), h('div', { class: 'bm-label bm-border' }, 'border-box'), h('div', { class: 'bm-label bm-padding' }, 'padding-box'), h('div', { class: 'bm-label bm-content' }, 'content-box')), h('div', { class: 'fs-sm text-secondary', style: { marginTop: '8px' } }, '当前参考盒：' + this._boxModel + '（形状坐标相对该盒左上角，percentage 相对该盒尺寸）')),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.pitfallsInfo || '（点击按钮查看 7 大陷阱与 10 条最佳实践）')),
                h(Alert, {
                    type: 'warning',
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'api-lab-page css-shapes-page' }, h('h2', { class: 'section-title' }, 'CSS Shapes Module Level 1 完整实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, '本页演示 W3C CSS Shapes Module Level 1 文本环绕非矩形形状：shape-outside 基础（circle/ellipse/inset/polygon + float）、circle() 与 ellipse()（closest-side/farthest-side）、inset() 与 polygon()（与 clip-path 共用语法）、shape-margin 与 shape-image-threshold（Alpha 阈值）、引用图像形状（url() + PNG 抠图 + CORS）、杂志式排版实战（圆形头像/多边形/抠图环绕）、陷阱与最佳实践（float/尺寸/盒模型/DevTools/响应式/性能）。所有特性通过 CSS.supports 能力检测，不可用时仅记日志，绝不抛异常。jsdom 不做真实布局，真实浏览器可查看文字环绕效果。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderCard6(), this._renderCard7(), this._renderCard8(), this._renderLogPanel());
    }
}
//# sourceMappingURL=CSSShapesPage.js.map