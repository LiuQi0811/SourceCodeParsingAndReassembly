...
export class HTMLMediaElementAdvancedPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!f.setSinkId)
            this._addLog('warn', 'setSinkId 不可用（jsdom 无，Chrome/Edge 110+ 支持，Safari/Firefox 不支持）');
        if (!f.audioSink)
            this._addLog('warn', 'audioSink 属性不可用（Chrome 110+ 实验性，替代 setSinkId）');
        if (!f.preservesPitch)
            this._addLog('warn', 'preservesPitch 不可用（jsdom 无，主流浏览器支持，旧版需 webkit/moz 前缀）');
        if (!f.controlsList)
            this._addLog('warn', 'controlsList 不可用（Chrome 58+，Safari 部分，Firefox 不支持）');
        if (!f.disableRemotePlayback)
            this._addLog('warn', 'disableRemotePlayback 不可用（Chrome/Safari 支持，Firefox 不支持）');
        if (!f.audioTracks)
            this._addLog('warn', 'audioTracks 不可用（Safari 领先，Chrome 需多轨道容器，Firefox 不支持）');
        if (!f.videoTracks)
            this._addLog('warn', 'videoTracks 不可用（Safari 领先，Chrome 部分支持）');
        if (!f.remotePlayback)
            this._addLog('warn', 'RemotePlayback 不可用（Chrome/Edge 支持，Safari 用 webkit- 前缀 AirPlay）');
        if (!f.audioSession)
            this._addLog('warn', 'navigator.audioSession 不可用（Safari 17.4+，Chrome 开发中，jsdom 无音频会话概念）');
    ...
    _registerMediaEl(el) {
        if (el && !this._mediaEls.includes(el))
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'HTMLMediaElement 是 HTMLAudioElement 与 HTMLVideoElement 的共同抽象基类。核心属性按「播放控制 / 音量 / 源 / 状态 / 缓冲 / 控件 / 变速 / 输出 / 轨道 / 远端」分类。canPlayType 返回 probably/maybe/空串。readyState 四态（HAVE_NOTHING..HAVE_ENOUGH_DATA）。本页聚焦 setSinkId/audioSink/preservesPitch/controlsList/disableRemotePlayback/RemotePlayback/audioTracks/videoTracks/seekable 等媒体元素自身高级特性，与 AdvancedMediaPage（MSE/EME/PiP）互补。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行接口总览演示', { type: 'primary', size: 'sm', onClick: () => this._runOverviewDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// HTMLAudioElement 可直接构造，HTMLVideoElement 需 createElement
const a = new Audio('song.mp3');
const v = (document.createElement('video') as any);
...
const caps = {
  ...
  remotePlayback: 'RemotePlayback' in window,
                ...
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.overviewInfo || '（点击按钮查看 HTMLMediaElement 接口总览与浏览器支持矩阵）')),
            ],
        });
    ...
    `);
            ...
            try {
                ...
                if (list) {
                    ...
                    for (const d of audioOutputs) {
                        const item = h('div', { class: 'hme-device-item' }, h('span', { class: 'hme-device-kind hme-device-kind--out' }, 'audiooutput'), h('span', { class: 'hme-device-label' }, d.label || `设备 ${d.deviceId.slice(0, 8)}…`), h('span', { class: 'hme-device-id' }, d.deviceId.slice(0, 16)));
        ...
        const info = [
            ...
            '    stream.getTracks().forEach((t: any) => t.stop()); // 仅用于授权，立即释放',
            '  } catch (e: any) { console.warn("麦克风授权失败:", e); }',
            '',
            '  // 2. 现在可拿到带 label 的设备列表',
            '  const devices = await navigator.mediaDevices.enumerateDevices();',
            '  const outputs = devices.filter((d: any) => d.kind === "audiooutput");',
            '  outputs.forEach((d: any) => console.log(d.label, d.deviceId));',
            '',
            '  // 3. 切换输出设备',
            '  await (audio as any).setSinkId(outputs[1].deviceId); // 切到第二个设备',
            '',
            '【sinkchange 事件】',
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'setSinkId(deviceId) → Promise<void> 切换媒体元素输出到指定音频设备，配合 navigator.mediaDevices.enumerateDevices() 过滤 kind===\'audiooutput\' 获取 deviceId。audioSink 属性（Chrome 110+ 实验性）是新规范方向，直接赋值字符串，逐步替代 setSinkId。两者均触发 sinkchange 事件。必须 SecureContext（HTTPS/localhost），enumerateDevices 返回 label 需先 getUserMedia 授权。Safari/Firefox 不支持。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('枚举设备 + setSinkId 演示', { type: 'primary', size: 'sm', onClick: () => this._runSinkDemo() })),
                h('audio', {
                    class: 'sink-audio', controls: true, preload: 'metadata',
                    style: { width: '100%', maxWidth: '480px', marginTop: '8px' },
                }),
                h('div', { class: 'hme-device-list hme-sink-list' }, h('div', { class: 'hme-device-item' }, '（点击按钮枚举 audiooutput 设备，jsdom 不可用）')),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// 1. 先 getUserMedia 授权以获取带 label 的设备列表
const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
...
const devices = await navigator.mediaDevices.enumerateDevices();
const audioOutputs = devices.filter((d: any) => d.kind === 'audiooutput');
audioOutputs.forEach((d: any) => console.log(d.label, d.deviceId));
...
audioElement.audioSink = '';`)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.sinkInfo || '（点击按钮查看 setSinkId 与 audioSink 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    `);
        ...
        const info = [
            ...
            '【变速 UI 实现】',
            '  // 速率选择按钮组',
            '  [0.5, 0.75, 1, 1.25, 1.5, 2].forEach((rate: any) => {',
            '    btn.onclick = () => {',
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['preservesPitch', f.preservesPitch]]), h(Tag, { color: 'primary' }, '变速不变调')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'playbackRate 控制速率（0.0625-16，默认 1）。preservesPitch=true（默认）变速不变调（SoundTouch/WSOLA 算法保基频），适合语言学习/听书/无障碍慢速；preservesPitch=false 变速变调（花栗鼠/DJ 效果）。旧浏览器需 webkitPreservesPitch/mozPreservesPitch 前缀。ratechange 事件监听速率变化。极端速率下音质下降，移动端保调算法有性能开销。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行变速不变调演示', { type: 'primary', size: 'sm', onClick: () => this._runPitchDemo() })),
                h('audio', {
                    class: 'pitch-audio', controls: true, preload: 'metadata',
                    style: { width: '100%', maxWidth: '480px', marginTop: '8px' },
                }),
                h('div', { class: 'hme-pitch-bar' }, h('span', {}, 'rate'), h('div', { class: 'hme-pitch-meter' }, h('div', { class: 'hme-pitch-fill', style: { width: '50%' } })), h('span', {}, '1.0x')),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// 变速不变调（语言学习/听书）
...
// 兼容旧浏览器前缀
function setPreservesPitch(el: any,  v: any) {
  if ('preservesPitch' in el) el.preservesPitch = v;
  else if ('webkitPreservesPitch' in el) el.webkitPreservesPitch = v;
  else if ('mozPreservesPitch' in el) el.mozPreservesPitch = v;
}
...
[0.5, 0.75, 1, 1.25, 1.5, 2].forEach((rate: any) => {
  btn.onclick = () => {
    ...
    setPreservesPitch(audio, true);
                ...
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.pitchInfo || '（点击按钮查看 preservesPitch 变速不变调完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['controlsList', f.controlsList]]), h(Tag, { color: 'primary' }, '控件禁用')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'controlsList 是 DOMTokenList，四个标准 token：nodownload（隐藏下载）、nofullscreen（隐藏全屏）、noplaybackrate（隐藏速率）、noremoteplayback（隐藏投屏）。配合 disablePictureInPicture 属性隐藏 PiP 按钮（仅 video）。Chrome 58+ 支持，Safari 部分支持，Firefox 不支持。仅隐藏 UI 不影响实际能力，防下载需配合 contextmenu 拦截 + EME DRM。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 controlsList 演示', { type: 'primary', size: 'sm', onClick: () => this._runControlsDemo() })),
                h('video', {
                ...
                h('div', { class: 'hme-controls-host' }, h('div', { class: 'fs-sm text-secondary mb-xs' }, 'controlsList tokens：'), h('div', {}, ...CONTROLS_LIST_TOKENS.map((t) => h('span', { class: 'hme-cl-token hme-cl-token--on' }, t)))),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `<!-- HTML 写法 -->
<video controls controlslist="nodownload nofullscreen noplaybackrate"
...
obs.observe(video, { attributes: true });`)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.controlsInfo || '（点击按钮查看 controlsList 精细控件完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    `);
        ...
        const info = [
            ...
            '    .catch((e: any) => console.error("投屏失败:", e));',
            '',
            '  // 监听可用性变化',
            '  const id = await remote.watchAvailability(available => {',
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'video.remote 返回 RemotePlayback 对象（仅 video），state 为 disconnected/connecting/connected，type 为 airplay/dlna/chromecast。prompt() 触发设备选择弹窗（需用户激活），watchAvailability 监听可用性。disableRemotePlayback=true 彻底禁用投屏。Safari 走 AirPlay，Chrome 走 Chromecast。与 Presentation API 区别：RemotePlayback 投媒体流，Presentation API 投整个网页。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 RemotePlayback 演示', { type: 'primary', size: 'sm', onClick: () => this._runRemoteDemo() }), this._btn('投屏 prompt()', { size: 'sm', onClick: () => this._remotePrompt() })),
                h('video', {
                    class: 'remote-video', controls: true, preload: 'metadata', playsInline: true,
                    style: { width: '100%', maxWidth: '480px', background: '#000', borderRadius: '6px', marginTop: '8px' },
                }),
                h('div', { class: 'hme-remote-host' }, h('span', { class: 'hme-remote-state hme-remote-state--disconnected' }, 'disconnected'), h('span', { class: 'fs-sm text-secondary', style: { marginLeft: '8px' } }, 'state：disconnected / connecting / connected')),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `const video = document.querySelector('video');
const remote = video.remote;  // RemotePlayback 对象
...
video.disableRemotePlayback = true;`)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.remoteInfo || '（点击按钮查看 RemotePlayback 与 disableRemotePlayback 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'audioTracks 返回 AudioTrackList，videoTracks 返回 VideoTrackList。AudioTrack.enabled 控制启用（可多选），VideoTrack.selected 控制选中（单选）。kind 取值 main/alternative/commentary/translation/descriptions/sign。事件 addtrack/removetrack/change。Safari 领先支持（HLS 多轨道原生），Chrome 需多轨道容器 MP4/DASH，Firefox 完全不支持。HLS 在 Chrome 需 hls.js polyfill。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行多轨道演示', { type: 'primary', size: 'sm', onClick: () => this._runTracksDemo() })),
                h('audio', {
                    class: 'tracks-audio', controls: true, preload: 'metadata',
                    style: { width: '100%', maxWidth: '480px', marginTop: '8px' },
                }),
                h('div', { class: 'hme-track-list' }, h('div', { class: 'hme-track-item' }, h('span', { class: 'hme-track-kind hme-track-kind--audio' }, 'audio'), h('span', { class: 'hme-device-label' }, '主音轨（main, zh）'), h('span', { class: 'hme-device-id' }, 'enabled=true')), h('div', { class: 'hme-track-item' }, h('span', { class: 'hme-track-kind hme-track-kind--audio' }, 'audio'), h('span', { class: 'hme-device-label' }, '日语配音（alternative, ja）'), h('span', { class: 'hme-device-id' }, 'enabled=false')), h('div', { class: 'hme-track-item' }, h('span', { class: 'hme-track-kind hme-track-kind--audio' }, 'audio'), h('span', { class: 'hme-device-label' }, '导演评论（commentary, en）'), h('span', { class: 'hme-device-id' }, 'enabled=false'))),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `const video = document.querySelector('video');
...
at.addEventListener('addtrack', e => console.log('新增音轨', e.track));`)),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.tracksInfo || '（点击按钮查看 audioTracks/videoTracks 多轨道完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard7() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'success' }, '✓ TimeRanges'), h(Tag, { color: 'primary' }, '媒体事件')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'buffered/seekable/played 均返回 TimeRanges（length/start(i)/end(i)），可能多段（MSE 分段下载）。22 个媒体事件按加载/播放/寻址/错误分类：loadstart→progress→loadedmetadata→canplay→playing，waiting/canplay 缓冲流程，seeking/seeked 跳转流程。timeupdate 约 4Hz。preload 三态 none/metadata/auto。直播 seekable 通常是尾部滑窗（时移）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行时间区间与事件演示', { type: 'primary', size: 'sm', onClick: () => this._runRangesDemo() })),
                h('audio', {
                    class: 'ranges-audio', controls: true, preload: 'metadata',
                    style: { width: '100%', maxWidth: '480px', marginTop: '8px' },
                }),
                h('div', { class: 'hme-ranges-host' }, h('div', { class: 'fs-sm text-secondary mb-xs' }, 'buffered 区间可视化：'), h('div', { class: 'hme-ranges-bar' }, h('div', { class: 'hme-ranges-seg hme-ranges-seg--buffered', style: { left: '0%', width: '35%' } })), h('div', { class: 'fs-sm text-secondary mb-xs', style: { marginTop: '6px' } }, 'played 区间：'), h('div', { class: 'hme-ranges-bar' }, h('div', { class: 'hme-ranges-seg hme-ranges-seg--played', style: { left: '0%', width: '20%' } })), h('div', { class: 'fs-sm text-secondary mb-xs', style: { marginTop: '6px' } }, 'seekable 区间：'), h('div', { class: 'hme-ranges-bar' }, h('div', { class: 'hme-ranges-seg hme-ranges-seg--seekable', style: { left: '0%', width: '100%' } }))),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// TimeRanges 遍历
for (let i = 0; i < audio.buffered.length; i++) {
  console.log(\`缓冲段 \${i}: \${audio.buffered.start(i)} - \${audio.buffered.end(i)}\`);
}
...
function isBuffered(time: any) {
  ...
  return false;
                ...
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.rangesInfo || '（点击按钮查看 TimeRanges 与媒体事件完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    `);
        ...
        const info = [
            ...
            '    const devices = await navigator.mediaDevices.enumerateDevices();',
            '    const outputs = devices.filter((d: any) => d.kind === "audiooutput");',
            '    outputs.forEach((d: any) => {',
            '      const opt = new Option(d.label, d.deviceId);',
            '      deviceSelect.add(opt);',
            '    });',
            '  }',
            '  deviceSelect.onchange = async () => {',
            '    await (audio as any).setSinkId(deviceSelect.value);',
            '  };',
            '',
            ...
            '      // 多轨道方案',
            '      const t = Array.from(audio.audioTracks).find((t: any) => t.language === lang);',
            '      if (t) { audio.audioTracks.forEach((x: any) => x.enabled = false); t.enabled = true; }',
            '    } else {',
            '      // 降级：切 src',
    ...
    _renderCard8() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'primary' }, '实战'), h(Tag, { color: f.setSinkId || f.audioTracks || f.preservesPitch ? 'success' : 'error' }, `高级特性 ${f.setSinkId || f.audioTracks || f.preservesPitch ? '✓' : '✗'}`)),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '多语言学习播放器综合实战：audioTracks 切配音（Safari/多轨道容器）+ setSinkId 选输出设备（Chrome 110+）+ controlsList 隐藏下载 + preservesPitch 变速不变调听写 + textTracks 字幕同步。9 大陷阱：tracks 浏览器差异（需 hls.js polyfill）、iOS 限制（不支持 setSinkId/JS 触发 play）、无障碍字幕同步错配、setSinkId 权限模型、controlsList 防下载局限、preservesPitch 性能、RemotePlayback 用户激活、媒体事件性能、TimeRanges 多段处理。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行实战综合演示', { type: 'primary', size: 'sm', onClick: () => this._runPatternDemo() })),
                h('audio', {
                    class: 'pattern-audio', controls: true, preload: 'metadata',
                    style: { width: '100%', maxWidth: '480px', marginTop: '8px' },
                }),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, `// 多语言学习播放器：综合 setSinkId + audioTracks + controlsList + preservesPitch
const audio = document.querySelector('audio');
...
async function loadOutputDevices() {
  const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
  stream.getTracks().forEach((t: any) => t.stop()); // 仅授权，立即释放
  const devices = await navigator.mediaDevices.enumerateDevices();
  const outputs = devices.filter((d: any) => d.kind === 'audiooutput');
...
function switchLangFallback(lang: any) {
  const time = audio.currentTime;
  audio.src = \`audio_\${lang}.mp3\`;
  audio.addEventListener('loadedmetadata', () => {
    audio.currentTime = time; audio.play();
                ...
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.patternInfo || '（点击按钮查看实战模式与 9 大陷阱完整清单）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard9() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: f.audioSession ? 'success' : 'error' }, f.audioSession ? 'audioSession ✓' : 'audioSession ✗'), h(Tag, { color: 'primary' }, 'W3C Audio WG 2024-2025')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'AudioSession API（W3C Audio WG 2024-2025）通过 navigator.audioSession 声明当前页面的音频会话类型（ambient/playback/transient/solo-ambient），浏览器据此决定多 tab 音频冲突策略（如新 tab 播放 playback 时暂停旧 tab）。audioSession.type 读写会话类型，audioSession.stateChange 事件通知会话被系统调节。与 HTMLMediaElement 协同：声明 type=playback 后 play()，系统自动管理冲突。Safari 17.4+ 最早实现，Chrome 开发中，Firefox 未实现。降级：不支持时按默认 ambient 混合，可用 Page Visibility API 手动 pause 模拟。jsdom 无音频会话概念，演示仅展示文档与代码。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('展示 AudioSession 文档与代码', { type: 'primary', size: 'sm', onClick: () => this._runAudioSessionDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, 'AudioSession 文档与示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '520px', overflow: 'auto' } }, h('code', {}, s.audioSessionInfo || '（点击按钮查看 AudioSession API 完整文档与多 tab 冲突处理示例）')),
                h(Alert, {
                    type: 'info',
    ...
    renderPage() {
        ...
        return [
            ...
            h('div', { class: 'feature-grid' }, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderCard6(), this._renderCard7(), this._renderCard8(), this._renderCard9()),
            this._renderLogPanel(),
        ];
    }
}
//# sourceMappingURL=HTMLMediaElementAdvancedPage.js.map