...
export class CSSContainerStyleQueriesPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!caps.containerStyle)
            this._addLog('warn', '@container style(--x: 1) 不支持（Chrome 111+ / Safari 17.2+，jsdom 不模拟）');
        if (!caps.containerTypeStyle)
            this._addLog('warn', 'container-type: style 不支持（Chrome 111+）');
        if (!caps.containerTypeInline)
            this._addLog('warn', 'container-type: inline-size 不支持（Chrome 105+ / Safari 16+）');
        if (!caps.registerProperty)
            this._addLog('warn', 'CSS.registerProperty 不支持（Chrome 85+）');
        if (!caps.atProperty)
            this._addLog('warn', '@property at-rule 不支持或 jsdom 未识别（Chrome 85+）');
    ...
    _caps() {
        ...
        if (cssSupports) {
            try {
                containerSize = CSS.supports('@container (min-width: 1px)');
            }
            catch { /* jsdom 不识别 at-rule */ }
            try {
                containerStyle = CSS.supports('@container style(--x: 1)');
            }
            catch { /* noop */ }
            try {
                containerTypeInline = CSS.supports('container-type: inline-size');
            }
            catch { /* noop */ }
            try {
                containerTypeStyle = CSS.supports('container-type: style');
            }
            catch { /* noop */ }
            try {
                containerTypeSize = CSS.supports('container-type: size');
            }
            catch { /* noop */ }
            try {
                containerName = CSS.supports('container-name: sidebar');
            }
            catch { /* noop */ }
            try {
            ...
            try {
                atProperty = CSS.supports('@property --x { syntax: "<length>"; inherits: false; initial-value: 0px; }');
            }
            catch { /* noop */ }
        }
        return {
    ...
    // =================== Card 1：尺寸查询 vs 样式查询 ===================
    _showSizeVsStyleDiff() {
        const caps = this._caps();
        this.setState({ sizeVsStyleInfo: '===== 尺寸查询 vs 样式查询 对比 =====\n\n' +
                '【尺寸查询（Size Queries）】\n' +
    ...
    _showContainerTypeOptions() {
        const caps = this._caps();
        this.setState({ sizeVsStyleInfo: '===== container-type 取值详解 =====\n\n' +
    ...
    _showNamedContainer() {
        const caps = this._caps();
        this.setState({ sizeVsStyleInfo: '===== 命名容器（container-name） =====\n\n' +
                '语法：\n' +
    ...
    _showPerformanceDiff() {
        this.setState({ sizeVsStyleInfo: '===== 性能差异：尺寸查询 vs 样式查询 =====\n\n' +
                '【尺寸查询的性能成本】\n' +
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.containerSize ? 'success' : 'error' }, caps.containerSize ? 'size query ✓' : 'size query ✗'), h(Tag, { color: caps.containerStyle ? 'success' : 'error' }, caps.containerStyle ? 'style query ✓' : 'style query ✗'), h(Tag, { color: 'primary' }, 'container-type'), h(Tag, { color: 'info' }, '性能差异')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '尺寸查询：container-type: inline-size + @container (min-width: 300px) 按容器尺寸响应（触发重排）。样式查询：container-type: style + @container style(--theme: dark) 按自定义属性值响应（仅计算样式，不重排，性能更好）。container-name: sidebar + @container sidebar (min-width: 300px) 命名容器限定。container-type: size（双维度，慎用易递归）vs inline-size（仅宽度）vs style（仅样式，不建立尺寸 containment）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('尺寸 vs 样式对比', { type: 'primary', size: 'sm', onClick: () => this._showSizeVsStyleDiff() }), this._btn('container-type 取值', { size: 'sm', onClick: () => this._showContainerTypeOptions() }), this._btn('命名容器', { size: 'sm', onClick: () => this._showNamedContainer() }), this._btn('性能差异', { size: 'sm', onClick: () => this._showPerformanceDiff() })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '尺寸查询容器（拖动右下角调整宽度，>300px 变绿加粗）：'),
                h('div', { class: 'csq-size-host mt-xs' }, h('div', { class: 'csq-size-child' }, '尺寸查询子元素（@container sizecard (min-width: 300px)）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '样式查询容器（container-type: style，需 JS 设置 --theme: dark 才变深色）：'),
                h('div', { class: 'csq-style-host mt-xs' }, h('div', { class: 'csq-style-child' }, '样式查询子元素（@container style(--theme: dark)）')),
    ...
    // =================== Card 2：查询自定义属性 ===================
    _showCustomPropQuery() {
        const caps = this._caps();
        this.setState({ customPropInfo: '===== 查询自定义属性语法 =====\n\n' +
    ...
    _showMultiConditionAnd() {
        this.setState({ customPropInfo: '===== 多条件 AND 查询 =====\n\n' +
    ...
    _showCustomPropSupport() {
        const caps = this._caps();
        this.setState({ customPropInfo: '===== 浏览器支持检测 =====\n\n' +
                '特性检测方式：\n\n' +
                '【方式 1：CSS.supports()】\n' +
    ...
    _showCustomPropPitfall() {
        this.setState({ customPropInfo: '===== 容器属性设置陷阱 =====\n\n' +
                '陷阱 1：未在容器上设置自定义属性\n' +
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.containerStyle ? 'success' : 'error' }, caps.containerStyle ? 'style() ✓' : 'style() ✗'), h(Tag, { color: 'primary' }, '布尔/字符串/数值'), h(Tag, { color: 'warning' }, '多条件 AND'), h(Tag, { color: 'info' }, '陷阱')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '@container style(--active: 1) 查询布尔型；@container style(--theme: dark) 查询字符串；@container style(--count: 5) 查询数值；@container style(--theme: dark) and style(--compact: 1) 多条件 AND。容器需先设置 --theme: dark，子元素用 @container style(--theme: dark) { ... } 响应。浏览器支持：Chrome 111+，CSS.supports("@container style(--x: 1)") 检测。未注册属性按字符串比较（区分大小写）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('查询语法', { type: 'primary', size: 'sm', onClick: () => this._showCustomPropQuery() }), this._btn('多条件 AND', { size: 'sm', onClick: () => this._showMultiConditionAnd() }), this._btn('浏览器支持检测', { size: 'sm', onClick: () => this._showCustomPropSupport() }), this._btn('容器属性陷阱', { size: 'sm', onClick: () => this._showCustomPropPitfall() })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '样式查询容器（CSS 已声明，需 JS 在 .csq-cp-host 上设置 --active/--theme/--count 才生效）：'),
                h('div', { class: 'csq-cp-host mt-xs' }, h('div', { class: 'csq-cp-item is-active' }, 'is-active（@container style(--active: 1)）'), h('div', { class: 'csq-cp-item is-theme' }, 'is-theme（@container style(--theme: dark)）'), h('div', { class: 'csq-cp-item is-count' }, 'is-count（@container style(--count: 5)）'), h('div', { class: 'csq-cp-item is-multi' }, 'is-multi（@container style(--theme: dark) and style(--compact: 1)）')),
    ...
    // =================== Card 3：后代样式查询 ===================
    _showDescendantPattern() {
        this.setState({ descendantInfo: '===== 后代样式查询模式 =====\n\n' +
    ...
    _compareWithHas() {
        this.setState({ descendantInfo: '===== 样式查询 vs :has() 选择器 对比 =====\n\n' +
    ...
    _showVariantPattern() {
        this.setState({ descendantInfo: '===== 变体适配实战模式 =====\n\n' +
    ...
    _showDescendantLimit() {
        this.setState({ descendantInfo: '===== 后代样式查询的限制 =====\n\n' +
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.containerStyle ? 'success' : 'error' }, caps.containerStyle ? 'descendant ✓' : 'descendant ✗'), h(Tag, { color: 'primary' }, '状态提升'), h(Tag, { color: 'warning' }, 'vs :has()'), h(Tag, { color: 'info' }, '变体适配')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '样式查询只能查询「容器自身」的样式，不能直接查询后代元素。模式：在容器上设置 --state: active，后代元素通过 @container style(--state: active) 响应祖先容器的状态。与 :has() 选择器对比：:has() 查询后代结构（存在性），样式查询查询容器自身属性（任意值）。实战模式：卡片容器 --variant: primary，内部按钮/标题/图标通过样式查询适配变体（嵌套组件可独立主题化）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('后代查询模式', { type: 'primary', size: 'sm', onClick: () => this._showDescendantPattern() }), this._btn('vs :has() 选择器', { size: 'sm', onClick: () => this._compareWithHas() }), this._btn('变体适配实战', { size: 'sm', onClick: () => this._showVariantPattern() }), this._btn('后代查询限制', { size: 'sm', onClick: () => this._showDescendantLimit() })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '变体适配容器（CSS 声明 primary/danger 变体，需 JS 设置 --variant 才生效）：'),
                h('div', { class: 'csq-desc-host mt-xs' }, h('div', { class: 'csq-desc-title' }, '卡片标题（@container style(--variant: ...) 响应）'), h('div', {}, h('button', { class: 'csq-desc-btn', type: 'button' }, '按钮（变体适配）')), h('div', {}, h('span', { class: 'csq-desc-icon' }), '图标 + 文本（变体适配）')),
    ...
    // =================== Card 4：与 @property 注册属性协同 ===================
    _showRegisterProperty() {
        const caps = this._caps();
        this.setState({ registeredPropsInfo: '===== @property 注册属性 =====\n\n' +
    ...
    _showTypedQuery() {
        const caps = this._caps();
        this.setState({ registeredPropsInfo: '===== 类型化样式查询（注册属性） =====\n\n' +
    ...
    _showStringVsTyped() {
        this.setState({ registeredPropsInfo: '===== 字符串比较 vs 类型比较 =====\n\n' +
                '【未注册属性：字符串比较】\n' +
                '  .host { --count: 5; }  /* 未注册 */\n' +
    ...
    _showStartingStyleSynergy() {
        this.setState({ registeredPropsInfo: '===== @property + @starting-style + allow-discrete 协同 =====\n\n' +
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.atProperty ? 'success' : 'error' }, caps.atProperty ? '@property ✓' : '@property ✗'), h(Tag, { color: caps.registerProperty ? 'success' : 'error' }, caps.registerProperty ? 'registerProperty ✓' : 'registerProperty ✗'), h(Tag, { color: 'primary' }, '类型安全'), h(Tag, { color: 'info' }, '可动画')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '@property --hue { syntax: "<angle>"; initial-value: 0deg; inherits: true; } 或 CSS.registerProperty() 注册属性，让自定义属性可被动画/过渡且类型安全。样式查询注册属性：@container style(--hue: 180deg) 查询带类型的属性。未注册属性按字符串比较（"5" !== "05"），注册后按类型比较（如 <length>、<color>、<integer>，10px === 0.625rem）。与 @starting-style、transition-behavior: allow-discrete 协同实现变体切换动画。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('@property 注册', { type: 'primary', size: 'sm', onClick: () => this._showRegisterProperty() }), this._btn('类型化查询', { size: 'sm', onClick: () => this._showTypedQuery() }), this._btn('字符串 vs 类型比较', { size: 'sm', onClick: () => this._showStringVsTyped() }), this._btn('与 @starting-style 协同', { size: 'sm', onClick: () => this._showStartingStyleSynergy() })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '注册属性演示（@property --hue <angle>，CSS 已注入，真实浏览器中 box 颜色随 --hue 变化）：'),
                h('div', { class: 'csq-prop-host mt-xs' }, h('div', { class: 'csq-prop-box' }, 'hsl(var(--hue))')),
    ...
    // =================== Card 5：无尺寸 containment 的纯样式响应 ===================
    _showStyleContainerType() {
        const caps = this._caps();
        this.setState({ pureStyleInfo: '===== container-type: style 不建立尺寸 containment =====\n\n' +
    ...
    _showThemeSwitch() {
        this.setState({ pureStyleInfo: '===== 主题切换模式（无 media query） =====\n\n' +
    ...
    _compareWithPrefersColorScheme() {
        this.setState({ pureStyleInfo: '===== 样式查询 vs prefers-color-scheme 媒体查询 =====\n\n' +
    ...
    _compareWithRootCustomProp() {
        this.setState({ pureStyleInfo: '===== 样式查询 vs :root 自定义属性 + 类名切换 =====\n\n' +
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.containerTypeStyle ? 'success' : 'error' }, caps.containerTypeStyle ? 'container-type:style ✓' : 'container-type:style ✗'), h(Tag, { color: 'primary' }, '主题切换'), h(Tag, { color: 'warning' }, '组件级'), h(Tag, { color: 'info' }, '嵌套独立')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'container-type: style 不建立尺寸 containment（不影响布局），可用于「主题切换」：容器设置 --theme: dark，所有后代无需 media query 即可响应。vs prefers-color-scheme 媒体查询：样式查询是组件级（容器作用域），媒体查询是全局级。vs :root 自定义属性 + 类名切换：样式查询更声明式（@container 块集中管理），组件可独立主题化（嵌套组件可不同主题）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('container-type:style 特性', { type: 'primary', size: 'sm', onClick: () => this._showStyleContainerType() }), this._btn('主题切换模式', { size: 'sm', onClick: () => this._showThemeSwitch() }), this._btn('vs prefers-color-scheme', { size: 'sm', onClick: () => this._compareWithPrefersColorScheme() }), this._btn('vs :root + 类名', { size: 'sm', onClick: () => this._compareWithRootCustomProp() })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '主题切换容器（container-type: style，需 JS 设置 --theme: dark/light 才切换）：'),
                h('div', { class: 'csq-theme-host mt-xs' }, h('div', { class: 'csq-theme-card' }, h('div', { class: 'csq-theme-text' }, '主题卡片（@container style(--theme: dark/light) 响应）'), h('div', { class: 'csq-theme-nested' }, '嵌套容器（可独立设不同主题）'))),
    ...
    _showFallbackStrategy() {
        this.setState({ supportInfo: '===== 降级策略（不支持样式查询时） =====\n\n' +
    ...
    _showSupportsDetection() {
        const caps = this._caps();
        this.setState({ supportInfo: '===== 特性检测方式 =====\n\n' +
    ...
    _showSizeVsStyleRelation() {
        const caps = this._caps();
        this.setState({ supportInfo: '===== 样式查询与尺寸查询的关系 =====\n\n' +
                '历史与定位：\n' +
    ...
    _renderCard6() {
        ...
        const card = new Card({
            title: '6. 浏览器支持与渐进增强',
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.containerStyle ? 'success' : 'error' }, caps.containerStyle ? 'style() ✓' : 'style() ✗'), h(Tag, { color: caps.containerSize ? 'success' : 'error' }, caps.containerSize ? 'size() ✓' : 'size() ✗'), h(Tag, { color: 'primary' }, '渐进增强'), h(Tag, { color: 'warning' }, 'Firefox ✗')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '@supports container: style(--x) { ... } 或 CSS.supports("@container style(--x: 1)") 检测。不支持时回退：用类名 + 普通选择器，或用 :has() 替代。各浏览器版本：Chrome 111+（2023.3）、Safari 17.2+（2023.12）、Firefox 暂不支持（截至 2024）。与 @container 尺寸查询（Chrome 105+/Safari 16+）的关系：样式查询是尺寸查询的扩展（共享语法，新增 style() 查询）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('浏览器支持表', { type: 'primary', size: 'sm', onClick: () => this._showBrowserSupportTable() }), this._btn('降级策略', { size: 'sm', onClick: () => this._showFallbackStrategy() }), this._btn('特性检测方式', { size: 'sm', onClick: () => this._showSupportsDetection() }), this._btn('与尺寸查询关系', { size: 'sm', onClick: () => this._showSizeVsStyleRelation() })),
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'api-lab-page css-container-style-queries-page' }, h('h2', { class: 'section-title' }, 'CSS 容器样式查询'), h('p', { class: 'fs-sm text-secondary mb-md' }, '本页演示 CSS 容器样式查询（Container Style Queries）：尺寸查询 vs 样式查询、查询自定义属性、后代样式查询、与 @property 注册属性协同、无尺寸 containment 的纯样式响应、浏览器支持与渐进增强。所有特性通过 typeof / CSS.supports 能力检测，不可用时仅记日志，绝不抛异常。jsdom 不模拟 CSS 渲染，视觉演示需在真实浏览器查看。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderCard6(), this._renderLogPanel());
    }
}
//# sourceMappingURL=CSSContainerStyleQueriesPage.js.map