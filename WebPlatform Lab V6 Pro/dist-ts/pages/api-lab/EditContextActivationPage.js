...
export class EditContextActivationPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!caps.userActivation)
            this._addLog('warn', 'navigator.userActivation 不可用（Chrome 72+），演示仅记日志');
        if (!caps.editContext)
            this._addLog('warn', 'EditContext 不可用（Chrome 121+），演示仅记日志');
        if (caps.editContext && !caps.editContextOnElement)
            this._addLog('warn', 'EditContext 存在但 element.editContext 绑定未实现');
    ...
    _caps() {
        ...
        try {
            userActivation = typeof navigator !== 'undefined' && navigator.userActivation !== undefined;
        }
        catch {
        ...
        try {
            editContextOnElement = typeof HTMLElement !== 'undefined' && 'editContext' in HTMLElement.prototype;
        }
        catch {
            editContextOnElement = false;
        }
        let clipboardRead = false;
        try {
            clipboardRead = typeof navigator !== 'undefined' && navigator.clipboard && typeof navigator.clipboard.read === 'function';
        }
        catch {
            clipboardRead = false;
        }
    ...
    _renderCard1() {
        ...
        }, h('p', { class: 'fs-sm text-secondary' }, 'navigator.userActivation 统一描述用户激活状态：hasBeenActive（粘性激活——本次会话曾激活过，置位后不再回落）；' +
            'isActive（瞬时激活——最近数秒内发生过用户手势，会随时间衰减）。' +
            '是 Clipboard/Fullscreen/PiP/WebShare/屏幕捕获等权限敏感 API 的统一前置判断，避免 try/catch 反复试错。'), h('div', { class: 'ec-demo-box' }, h('div', { class: 'flex items-center gap-sm flex-wrap' }, caps.userActivation
            ? h('span', { class: 'ec-activation-badge ec-activation-badge--active' }, 'hasBeenActive: 运行时读取')
            : h('span', { class: 'ec-activation-badge ec-activation-badge--unknown' }, 'hasBeenActive: 不可用'), caps.userActivation
            ? h('span', { class: 'ec-activation-badge ec-activation-badge--active' }, 'isActive: 运行时读取')
            : h('span', { class: 'ec-activation-badge ec-activation-badge--unknown' }, 'isActive: 不可用')), h('p', { class: 'fs-sm text-secondary mt-sm' }, '展示规则：页面加载后 hasBeenActive=false；点击下方任意按钮（即用户手势）后变 true 并保持；' +
            'isActive 仅在最近数秒内有手势时为 true，随后回落为 false。')), h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('检查激活状态', { type: 'primary', size: 'sm', onClick: () => this._checkActivation() }), this._btn('触发手势（模拟点击）', { type: 'default', size: 'sm', onClick: () => this._triggerGesture() })), s.activationState ? h('p', { class: 'fs-sm mt-sm text-secondary' }, s.activationState) : null, h('pre', { class: 'code-block mt-md' }, `// 粘性 vs 瞬时
...
// 权限敏感 API 前置判断，避免 try/catch 反复试错`));
    ...
    _createEditContext() {
        ...
        if (!host) {
            this._addLog('warn', '未找到 EditContext 宿主元素');
            return;
        }
        try {
            ...
            try {
                host.textContent = 'Hello EditContext';
            }
            catch { /* noop */ }
        }
        catch (err) {
    ...
    _renderCard2() {
        ...
        }, h('p', { class: 'fs-sm text-secondary' }, 'new EditContext() 构造；element.editContext = ctx 绑定到任意元素；ctx.updateText(text, start, end) 更新文本；' +
            'ctx.updateSelection(start, end) 更新选区；ctx.updateCompositionRange(start, end) 更新 IME 组合范围。' +
            '作为 contentEditable 的现代替代，作者完全控制 DOM 渲染；原生接收 IME/虚拟键盘/手写输入。'), h('div', { class: 'ec-demo-box' }, h('p', { class: 'fs-sm text-secondary' }, '下面是一个普通 div，绑定 EditContext 后由作者控制渲染：'), h('div', { id: 'ec-edit-canvas', class: 'ec-edit-canvas', tabindex: '0' }, '（点击"创建 EditContext 并绑定"后这里会显示初始文本）')), h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('创建 EditContext 并绑定', { type: 'primary', size: 'sm', onClick: () => this._createEditContext() }), caps.editContextOnElement ? h(Tag, { color: 'success' }, 'element.editContext ✓') : h(Tag, { color: 'error' }, 'element.editContext ✗')), s.editContextInfo ? h('p', { class: 'fs-sm mt-sm text-secondary' }, s.editContextInfo) : null, h('pre', { class: 'code-block mt-md' }, `const ctx = new EditContext();
...
// 作者负责把文本渲染到 DOM；浏览器负责 IME/虚拟键盘事件分发`));
        ...
        try {
            ...
            if (typeof ctx.addEventListener === 'function') {
                ctx.addEventListener('textupdate', (e) => {
                    this._addLog('info', `textupdate → updateRangeStart=${e.updateRangeStart} updateRangeEnd=${e.updateRangeEnd} text="${e.text}" selectionStart=${e.selectionStart} selectionEnd=${e.selectionEnd}`);
                });
                ctx.addEventListener('textformatupdate', (e) => {
                    let formats = '(无法读取)';
                    try {
                        formats = JSON.stringify(e.getTextFormats());
                    }
                    catch {
                        formats = '(getTextFormats 不可用)';
                    }
    ...
    _renderCard3() {
        ...
        return h(Card, {
            ...
            extra: h(Tag, { color: caps.editContext ? 'success' : 'error' }, caps.editContext ? '已支持' : '未支持'),
        }, h('p', { class: 'fs-sm text-secondary' }, 'textupdate 事件（e.updateRangeStart/End、e.text、e.selectionStart/End——文本变更）；' +
            'textformatupdate 事件（e.getTextFormats()——IME 强调样式如中文输入下划线）；' +
            'characterboundsupdate 事件（e.rangeStart/End——请求字符边界用于绘制光标）；' +
            'compositionstart/end 事件（IME 组合开始/结束）。'), h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('注册事件监听', { type: 'primary', size: 'sm', onClick: () => this._attachEditContextEvents() }), this._ecEventsAttached ? h(Tag, { color: 'success' }, '已注册') : h(Tag, { color: 'default' }, '未注册')), s.editContextEvents ? h('p', { class: 'fs-sm mt-sm text-secondary' }, s.editContextEvents) : null, h('pre', { class: 'code-block mt-md' }, `ctx.addEventListener('textupdate', e => {
...
ctx.addEventListener('compositionend', e => {});`));
    ...
    _renderCard4() {
        ...
        }, h('p', { class: 'fs-sm text-secondary' }, 'EditContext 把"文本输入"与"DOM 渲染"解耦：浏览器负责 IME/虚拟键盘/手写事件分发，作者负责渲染；' +
            'contentEditable 把两者耦合，简单但难做高性能编辑器。下表为决策矩阵。'), h('table', { class: 'ec-matrix' }, h('thead', {}, h('tr', {}, h('th', { class: 'ec-col-dim' }, '维度'), h('th', {}, 'EditContext'), h('th', {}, 'contentEditable'))), h('tbody', {}, rows.map((r) => h('tr', {}, h('td', { class: 'ec-col-dim' }, r.dim), h('td', {}, r.ec), h('td', {}, r.ce))))), h('p', { class: 'fs-sm mt-sm text-secondary' }, s.matrixInfo || '决策建议：需要高性能/大文档/复杂 IME 支持 → EditContext；需要快速搭建简单富文本 → contentEditable。'), h('pre', { class: 'code-block mt-md' }, `// EditContext：作者全控渲染
const ctx = new EditContext();
host.editContext = ctx;
ctx.addEventListener('textupdate', e => {
...
<div contenteditable="true">可直接编辑</div>`));
    ...
    _renderCard5() {
        ...
        return h(Card, {
        ...
        }, h('p', { class: 'fs-sm text-secondary' }, '依赖用户激活的 API：clipboard.read/write、fullscreen、PiP、WebShare、getDisplayMedia、Keyboard.lock、WakeLock。' +
            ...
            '用 navigator.userActivation 预检可避免 try/catch 反复试错。'), h('div', { class: 'ec-perm-list' }, perms.map((p) => h('div', { class: 'ec-perm-row' }, h('code', {}, p.code), h(Tag, { color: p.cap ? 'success' : 'error' }, p.cap ? '已支持' : '未支持'), h('span', { class: 'fs-sm text-secondary' }, p.consume), this._btn('预检', { type: 'default', size: 'sm', onClick: () => this._preflightPermission(p.name) })))), s.permissionInfo ? h('p', { class: 'fs-sm mt-sm text-secondary' }, s.permissionInfo) : null, h('pre', { class: 'code-block mt-md' }, `// 三态模型
...
//# sourceMappingURL=EditContextActivationPage.js.map