...
export class CSSClipPathDeepPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!f.path)
            this._addLog('warn', 'path() 不可用（Chrome 88+/Firefox 71+/Safari 13.1+ 才支持 path() 作为 basic-shape）');
        if (!f.geometryBox)
            this._addLog('warn', 'geometry-box 不可用（旧 Safari 兼容性问题，需 -webkit-clip-path 前缀）');
        if (!f.urlClip)
            this._addLog('info', 'url(#clip) SVG 引用支持探测返回 false（实际浏览器通常支持，jsdom 限制）');
        if (!f.ovbInset && !f.ovbCircle && !f.ovbNone)
            this._addLog('warn', 'object-view-box 不可用（仅 Chrome 124+ 支持，Safari/Firefox 未实现，可降级到 clip-path）');
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'clip-path: <basic-shape>|<clip-source>|<geometry-box>|none。basic-shape 含 circle()/ellipse()/inset()/polygon()/path() 五种函数。与 overflow:hidden 对比：overflow 仅矩形裁剪基于 padding-box，clip-path 可裁任意形状且不影响布局。与 mask 区别：clip-path 硬切边（锐利边界），mask 用 alpha/luminance 通道渐变切（可羽化）。裁剪区域外内容完全隐藏且不响应指针事件。Chrome/Edge/Firefox 全支持，Safari 部分需 -webkit- 前缀，path() 需 Chrome 88+。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 clip-path 概述演示', { type: 'primary', size: 'sm', onClick: () => this._runOverviewDemo() })),
                ...
                h('div', { class: 'cp-demo' }, h('div', { class: 'cp-box cp-circle' }, 'circle'), h('div', { class: 'cp-box cp-inset' }, 'inset'), h('div', { class: 'cp-box cp-triangle' }, 'triangle'), h('div', { class: 'cp-box cp-star' }, 'star')),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.overviewInfo || '（点击按钮查看 clip-path 概述与基本形状完整用法）')),
            ],
        });
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'circle([<shape-radius>? [at <position>]?]) 圆形剪裁，radius 取 <length>|<percentage>|closest-side|farthest-side，position 圆心定位（center/50% 50%/top left）。ellipse([<rx> <ry>? [at <position>]?]) 椭圆剪裁，rx 水平半径 ry 垂直半径。closest-side 取圆心到最近边距离，farthest-side 取最远边。circle(50%) 在非方形元素上可能非正圆，用 circle(closest-side) 保证正圆。圆形展开动画（View Transitions 常用）：clip-path: circle(0% → 75%)。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 circle/ellipse 演示', { type: 'primary', size: 'sm', onClick: () => this._runCircleDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '320px', overflow: 'auto' } }, h('code', {}, code)),
                h('div', { class: 'fs-sm text-secondary' }, '可视化剪裁效果：'),
                h('div', { class: 'cp-demo' }, h('div', { class: 'cp-box cp-circle' }, 'circle 50%'), h('div', { class: 'cp-box cp-ellipse' }, 'ellipse')),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.circleInfo || '（点击按钮查看 circle()/ellipse() 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['inset()', f.inset]]), h(Tag, { color: 'primary' }, 'basic-shape')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'inset(<length-percentage>{1,4} [round <border-radius>]?) 内嵌矩形剪裁。四值语法顺时针 上 右 下 左（与 margin 一致）。round 圆角剪裁与 border-radius 语法一致但仅作用于裁剪边界，比 border-radius + overflow:hidden 更彻底（强制裁剪子元素）。inset(10%) 是缩进量不是保留量。圆角矩形 inset(0 round 16px)、胶囊 inset(0 round 999px)。中心展开动画 inset(50% → 0%)。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 inset 演示', { type: 'primary', size: 'sm', onClick: () => this._runInsetDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '320px', overflow: 'auto' } }, h('code', {}, code)),
                h('div', { class: 'fs-sm text-secondary' }, '可视化剪裁效果：'),
                h('div', { class: 'cp-demo' }, h('div', { class: 'cp-box cp-inset' }, 'inset 10%/15%'), h('div', { class: 'cp-box cp-inset-round' }, 'inset round')),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.insetInfo || '（点击按钮查看 inset() 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['polygon()', f.polygon]]), h(Tag, { color: 'primary' }, 'basic-shape')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'polygon(<fill-rule>?, <length-percentage> <length-percentage>) 多边形剪裁，至少 3 个顶点。fill-rule: nonzero（默认，非零环绕）|evenodd（奇偶，自相交时可镂空中心）。顶点坐标 x y 百分比相对 border-box。常见形状：三角形/五角星/箭头/梯形/菱形/六边形/八边形/消息气泡。可动画但顶点数必须相同（顶点数变化不可插值）。配合 CSS 变量可动态变形。复杂曲线形状用 path()。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 polygon 演示', { type: 'primary', size: 'sm', onClick: () => this._runPolygonDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '320px', overflow: 'auto' } }, h('code', {}, code)),
                h('div', { class: 'fs-sm text-secondary' }, '可视化剪裁效果：'),
                h('div', { class: 'cp-demo' }, h('div', { class: 'cp-box cp-triangle' }, 'triangle'), h('div', { class: 'cp-box cp-star' }, 'star'), h('div', { class: 'cp-box cp-arrow' }, 'arrow')),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.polygonInfo || '（点击按钮查看 polygon() 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['path()', f.path]]), h(Tag, { color: 'primary' }, 'basic-shape')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'path(<fill-rule>?, <string>) SVG 路径剪裁，string 引用 SVG path d 属性语法。命令：M 移动/L 直线/H 水平/V 垂直/C 三次贝塞尔/Q 二次贝塞尔/S 平滑三次/T 平滑二次/A 椭圆弧/Z 闭合。大写绝对坐标，小写相对坐标。可绘制心形/波浪/blob 等复杂形状。path() vs url(#clipPath)：path() 纯 CSS 无需 SVG 标记但坐标绝对值不可百分比自适应且不可动画；url(#clipPath) 可 objectBoundingBox 自适应且支持 clip-rule。Chrome 88+/Firefox 71+/Safari 13.1+。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 path 演示', { type: 'primary', size: 'sm', onClick: () => this._runPathDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '320px', overflow: 'auto' } }, h('code', {}, code)),
                h('div', { class: 'fs-sm text-secondary' }, '可视化剪裁效果（心形 path）：'),
                h('div', { class: 'cp-demo' }, h('div', { class: 'cp-box cp-heart' }, 'heart'), h('div', { class: 'cp-box cp-wave' }, 'wave')),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.pathInfo || '（点击按钮查看 path() 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['geometry-box', f.geometryBox]]), h(Tag, { color: 'primary' }, 'Masking L1')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'clip-path: <basic-shape> <geometry-box> 修饰参考盒。8 种取值：margin-box（含 margin）/border-box（默认，含 border）/padding-box（含 padding）/content-box（仅内容）/fill-box（SVG 对象边界）/stroke-box（SVG 描边边界）/view-box（SVG 视口）/none。shape 相对参考盒计算百分比与圆心。盒模型层级 margin-box ⊃ border-box ⊃ padding-box ⊃ content-box。SVG 场景用 fill-box/view-box。Safari 部分支持需 -webkit- 前缀。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 geometry-box 演示', { type: 'primary', size: 'sm', onClick: () => this._runGeometryBoxDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '320px', overflow: 'auto' } }, h('code', {}, code)),
                h('div', { class: 'fs-sm text-secondary' }, '可视化剪裁效果（border-box vs content-box）：'),
                h('div', { class: 'cp-demo' }, h('div', { class: 'cp-box cp-geo-border' }, 'border-box'), h('div', { class: 'cp-box cp-geo-content' }, 'content-box')),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.geometryBoxInfo || '（点击按钮查看 geometry-box 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard7() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['url(#clip)', f.urlClip]]), h(Tag, { color: 'primary' }, 'SVG Masking')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'SVG <clipPath> 元素定义剪裁路径，clip-path: url(#myClip) 引用。clipPathUnits: userSpaceOnUse（默认，绝对坐标）|objectBoundingBox（0-1 比例自适应）。clip-rule: nonzero（默认）|evenodd 填充规则，仅对 url(#clipPath) 生效对 basic-shape 无效。SVG 内可组合多形状（circle/rect/path/text）并集作为剪裁区域。文字剪裁 url(#textClip) 实现文字遮罩。可被多元素复用。所有现代浏览器含 IE9+ 支持。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 SVG clip 演示', { type: 'primary', size: 'sm', onClick: () => this._runSvgClipDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '320px', overflow: 'auto' } }, h('code', {}, code)),
                h('div', { class: 'fs-sm text-secondary' }, '可视化剪裁效果（url(#clipPath) 圆形）：'),
                h('div', { class: 'cp-demo' }, h('div', { class: 'cp-box cp-circle' }, 'url clip')),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.svgClipInfo || '（点击按钮查看 clip-rule 与 url(#clipPath) 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard8() {
        ...
        const card = new Card({
            ...
            children: [
                ...
                h('div', { class: 'cp-demo' }, h('div', { class: 'cp-avatar' }), h('div', { class: 'cp-card-bevel' }, 'BEVEL'), h('div', { class: 'cp-text-mask' }, 'CLIP PATH')),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.patternInfo || '（点击按钮查看 9 种实战模式与 10 条陷阱清单完整代码）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard9() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'object-view-box: <basic-shape>|none，CSSWG 提案新属性，专用于 <img>/<video> 等 replaced element 的视图裁剪。接受 inset()/circle()/ellipse()/polygon() 等 basic-shape（与 clip-path 共用语法），默认值 none。与 object-fit 协同：先按 object-view-box 裁剪内容，再由 object-fit 缩放填充元素盒，object-position 定位。vs clip-path：object-view-box 仅裁剪替换元素内容（不影响边框/背景/盒模型），clip-path 裁剪整个元素。仅 Chrome 124+ 支持，Safari/Firefox 未实现，需 @supports 降级到 clip-path。实战：图像方形缩略图、圆形头像、视频去黑边、16:9→4:3 裁剪、响应式裁剪。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 object-view-box 演示', { type: 'primary', size: 'sm', onClick: () => this._runObjectViewBoxDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
                h('pre', { class: 'code-block', style: { maxHeight: '320px', overflow: 'auto' } }, h('code', {}, code)),
                h('div', { class: 'fs-sm text-secondary' }, '可视化效果（object-view-box 仅作用于 img/video，下方 img 示意）：'),
                h('div', { class: 'cp-demo' }, h('img', { class: 'cp-ovb-img cp-ovb-inset', src: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="240" height="160"><rect width="240" height="160" fill="%23f59e0b"/><circle cx="120" cy="60" r="40" fill="%23fff"/><rect x="40" y="110" width="160" height="20" fill="%23fff"/></svg>', alt: 'ovb inset' }), h('img', { class: 'cp-ovb-img cp-ovb-circle', src: 'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="240" height="160"><rect width="240" height="160" fill="%233b82f6"/><circle cx="120" cy="80" r="50" fill="%23fff"/></svg>', alt: 'ovb circle' })),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.objectViewBoxInfo || '（点击按钮查看 object-view-box 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
...
//# sourceMappingURL=CSSClipPathDeepPage.js.map