...
export class CSSObjectModelPage extends Page {
    ...
    componentDidMount() {
        ...
        try {
            ...
            adoptedStyleSheetsSupported = false;
        }
        let cssSupportsSupported = false;
        if (cssDefined) {
            try {
                cssSupportsSupported = typeof CSS.supports === 'function';
            }
            catch {
                cssSupportsSupported = false;
            }
        }
        let cssEscapeSupported = false;
        if (cssDefined) {
            try {
                cssEscapeSupported = typeof CSS.escape === 'function';
            }
            catch {
                cssEscapeSupported = false;
            }
        }
            ...
            try {
                if (typeof document !== 'undefined'
                    ...
                    document.adoptedStyleSheets = document.adoptedStyleSheets
                        .filter((s) => s !== this._constructSheet);
                }
            }
            catch { /* noop */ }
    ...
    _inlineSetProperty() {
        ...
        try {
            ...
            this._addLog('inline', `setProperty 设 color=${value} priority="${priority}"，背景蓝、字号 18px`);
        }
        catch (err) {
    ...
    _inlineCssText() {
        ...
        try {
            ...
            this.setState({ inlineResult: result });
            this._addLog('inline', `cssText 批量设置 → color=#fff, bg=#fa541c, font-size=20px (整体覆盖)`);
        }
        catch (err) {
    ...
    _inlineRemoveProperty() {
        ...
        try {
            ...
            const result = `removeProperty('color') → 返回旧值="${removed}"\n`
                ...
                + `  操作后 cssText: ${after}`;
            this.setState({ inlineResult: result });
            this._addLog('inline', `removeProperty('color') → 旧值="${removed}"，剩余内联样式: ${after}`);
        }
        catch (err) {
    ...
    _inlineEnumerate() {
        ...
        try {
            ...
            const result = `style.length = ${len}\n`
                + `  遍历 style[i] 取属性名：\n`
                + names.map((n, i) => `    [${i}] ${n} = "${decl.getPropertyValue(n)}"`).join('\n')
                + `\n  注：camelCase 写法（style.fontSize）与连字符写法（style.getPropertyValue('font-size')）等价`;
            this.setState({ inlineResult: result });
            this._addLog('inline', `遍历内联样式 → length=${len}，属性名: ${names.length ? names.join(', ') : '(空)'}`);
        }
        catch (err) {
    ...
    _computedRead() {
        ...
        try {
            ...
            this._addLog('computed', `getComputedStyle → color=${color}, font-size=${fontSize}, display=${display}`);
        }
        catch (err) {
    ...
    _computedPseudo() {
        ...
        try {
            ...
            this.setState({ computedResult: result });
            this._addLog('computed', `getComputedStyle(el, '::before') → content="${content}", color=${color}`);
        }
        catch (err) {
    ...
    _computedStyleMap() {
        ...
        try {
            ...
            this.setState({ computedResult: result });
            this._addLog('computed', `computedStyleMap() → font-size=${fsStr}, color=${colorStr}`);
        }
        catch (err) {
    ...
    _sheetInsertRule() {
        ...
        try {
            ...
            const target = this.$('.cssom-dyn-target');
            if (target)
                target.classList.add('dynamic-rule');
            this._addLog('rule', `insertRule("${ruleText.slice(0, 40)}...", ${idx}) → 返回 index=${idx}，cssRules.length=${sheet.cssRules.length}`);
            this._listCssRules();
        }
        catch (err) {
    ...
    _listCssRules() {
        ...
        try {
            ...
            this._addLog('rule', `列出 cssRules → 共 ${rules.length} 条：${list.slice(0, 5).join('; ')}${list.length > 5 ? ' ...' : ''}`);
        }
        catch (err) {
    ...
    _ruleTypeName(rule) {
        try {
            ...
            return map[rule.type] || `type=${rule.type}`;
        }
        catch {
            return `type=${rule.type}`;
        }
    ...
    _sheetDeleteRule() {
        ...
        try {
            ...
            this._addLog('rule', `deleteRule(${idx}) → 移除 ${removed}，剩余 cssRules.length=${sheet.cssRules.length}`);
            this._listCssRules();
        }
        catch (err) {
    ...
    _createConstructSheet() {
      ...
      `);
            ...
            this._addLog('cs', `new CSSStyleSheet() + replaceSync() ✓ —— 构造样式表（不依赖 DOM），待 adopt`);
        }
        catch (err) {
    ...
    _adoptToDocument() {
        ...
        try {
            ...
            const target = this.$('.cssom-adopt-target');
            ...
            this.setState({ adoptStatus: result });
            this._addLog('adopt', `document.adoptedStyleSheets 应用 ✓ —— length=${document.adoptedStyleSheets.length}`);
        }
        catch (err) {
    ...
    _replaceConstructSheet() {
      ...
      `);
            ...
            this.setState({ adoptStatus: result });
            this._addLog('cs', `replaceSync() ✓ —— 替换为橙色主题（同步）`);
    ...
    _supportsCheck() {
        ...
        try {
            ...
            this._addLog('supports', `supports 检测 → grid=${checks[0][2]}, flex=${checks[1][2]}, lab=${checks[2][2]}, color-mix=${checks[3][2]}`);
        }
        catch (err) {
    ...
    // element.matches 演示
    _matchesDemo() {
        const el = this.$('.cssom-match-stage .item.highlight');
        if (!el) {
        ...
        try {
            ...
            const result = `element.matches(selector) 检查元素是否匹配选择器：\n`
                ...
                + escapeInfo;
            this.setState({ supportsResult: result });
            this._addLog('match', `matches → .item=${m1}, .item.highlight=${m2}, .item.warning=${m3}`);
        }
        catch (err) {
    ...
    _closestDemo() {
        ...
        try {
            ...
            const stage = this.$('.cssom-match-stage');
            let scopeInfo = '';
            if (stage) {
            ...
            this.setState({ supportsResult: result });
            this._addLog('match', `closest → .item="${c1info}", .cssom-match-stage="${c2info}"`);
        }
        catch (err) {
    ...
    _matchMediaDemo() {
        ...
        try {
            ...
            this._mqlHandler = (e) => {
                this._addLog('media', `MediaQueryList change → matches=${e.matches}（prefers-color-scheme 切换为 ${e.matches ? 'dark' : 'light'}）`);
            };
            ...
            this._addLog('media', `matchMedia('${query}') → matches=${mql.matches}，已注册 change 监听`);
        }
        catch (err) {
    ...
    _renderCard1() {
        ...
        }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'element.style 是 CSSStyleDeclaration 实例，读写元素内联样式。style.property = value（camelCase）与 style.setProperty(property, value, priority) 等价，后者可设 important。style.cssText 读写整个内联样式字符串。removeProperty 返回旧值并移除属性。getPropertyValue / getPropertyPriority 读取值与优先级。style.length + style[index] 可遍历所有已设置属性名（连字符形式）。'), h('div', { class: 'flex gap-sm flex-wrap' }, this._btn('setProperty 设 color+important', { type: 'primary', size: 'sm', onClick: () => this._inlineSetProperty() }), this._btn('cssText 批量设置', { size: 'sm', onClick: () => this._inlineCssText() }), this._btn('removeProperty 移除', { size: 'sm', onClick: () => this._inlineRemoveProperty() }), this._btn('遍历所有内联属性名', { size: 'sm', onClick: () => this._inlineEnumerate() })), h('div', { class: 'cssom-target' }, '目标元素 .cssom-target —— 点击上方按钮通过 element.style 修改其内联样式（jsdom 不渲染，仅日志可见）'), s.inlineResult && h('pre', { class: 'code-block' }, s.inlineResult)));
    }
    _renderCard2() {
        const s = this.state;
        return h(Card, {
            ...
            extra: h(Tag, { color: 'success' }, '稳定'),
        }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'window.getComputedStyle(element, pseudoElt?) 返回 CSSStyleDeclaration（只读，反映最终计算值）。第二参数传 ":before"/":after"/":hover" 等可读取伪元素样式。计算值与指定值不同：color 总是 rgb() 形式，length 总是 px 形式。getPropertyValue 读取单个属性。element.computedStyleMap() 是 CSS Typed OM 接口，返回 StylePropertyMapReadOnly，get 返回 CSSUnitValue 等类型化对象。'), h('div', { class: 'flex gap-sm flex-wrap' }, this._btn('读取计算样式', { type: 'primary', size: 'sm', onClick: () => this._computedRead() }), this._btn('读取伪元素 ::before', { size: 'sm', onClick: () => this._computedPseudo() }), this._btn('computedStyleMap Typed OM', { size: 'sm', onClick: () => this._computedStyleMap() })), h('div', { class: 'cssom-target' }, '目标元素 .cssom-target —— 含 ::before 伪元素（content: ⚡），点击按钮读取其计算样式'), s.computedResult && h('pre', { class: 'code-block' }, s.computedResult)));
    }
    _renderCard3() {
        const s = this.state;
        return h(Card, {
            ...
            extra: h(Tag, { color: 'success' }, '稳定'),
        }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'document.styleSheets 集合（只读 HTMLLinkElement/HTMLStyleElement 对应的 StyleSheet）。sheet.cssRules / sheet.rules（旧）返回 CSSRuleList。sheet.insertRule(ruleText, index) 插入规则并返回位置，sheet.deleteRule(index) 删除指定位置。CSSRule 子类型：CSSStyleRule（selectorText/style）、CSSMediaRule（media/mediaText）、CSSKeyframesRule（cssRules/findRule/appendRule/deleteRule）、CSSImportRule、CSSFontFaceRule 等。'), h('div', { class: 'flex gap-sm flex-wrap' }, this._btn('insertRule 注入规则', { type: 'primary', size: 'sm', onClick: () => this._sheetInsertRule() }), this._btn('列出 cssRules 类型', { size: 'sm', onClick: () => this._listCssRules() }), this._btn('deleteRule 移除', { size: 'sm', danger: true, onClick: () => this._sheetDeleteRule() })), h('div', { class: 'cssom-dyn-target' }, '动态规则目标 .cssom-dyn-target —— insertRule 注入 .dynamic-rule 后会同步加 class 以便观察'), s.ruleList && h('pre', { class: 'code-block' }, s.ruleList)));
    }
    _renderCard4() {
        const s = this.state;
        ...
        return h(Card, {
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: constructable ? 'success' : 'error' }, constructable ? 'CSSStyleSheet' : 'CSSStyleSheet ✕'), h(Tag, { color: adopted ? 'success' : 'error' }, adopted ? 'adoptedStyleSheets' : 'adoptedStyleSheets ✕')),
        }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'new CSSStyleSheet() 构造独立样式表（不依赖 DOM）。sheet.replaceSync(cssText) 同步替换内容，sheet.replace(cssText) 异步返回 Promise。sheet.insertRule / sheet.deleteRule 同 CSSStyleSheet 接口。document.adoptedStyleSheets = [sheet] 应用到整个文档，ShadowRoot.adoptedStyleSheets 应用于 Shadow DOM。sheet.addRule / sheet.removeRule 为旧 API（已废弃）。'), (!constructable || !adopted) && h(Alert, {
            type: 'warning',
            message: '部分能力当前环境不支持',
        ...
        }), h('div', { class: 'flex gap-sm flex-wrap' }, this._btn('创建 Constructable Stylesheet', { type: 'primary', size: 'sm', onClick: () => this._createConstructSheet() }), this._btn('adoptedStyleSheets 应用到 document', { size: 'sm', onClick: () => this._adoptToDocument() }), this._btn('替换内容', { size: 'sm', onClick: () => this._replaceConstructSheet() })), h('div', { class: 'cssom-adopt-target' }, 'adopted 样式目标 .cssom-adopt-target —— adopt 后通过 adoptedStyleSheets 应用样式（jsdom 不渲染，真实浏览器生效）'), s.adoptStatus && h('pre', { class: 'code-block' }, s.adoptStatus)));
    }
    _renderCard5() {
        const s = this.state;
        return h(Card, {
            ...
            extra: h(Tag, { color: 'success' }, '稳定'),
        }, h('div', { class: 'flex flex-col gap-sm' }, h('p', { class: 'fs-sm text-secondary' }, 'CSS.supports(property, value) 或 CSS.supports(\'(property: value)\') 检测浏览器是否支持某 CSS 特性，返回 boolean。CSS.escape(string) 转义选择器中的特殊字符（如 . # 等）。element.matches(selector) 检查元素是否匹配选择器。element.closest(selector) 向上查找最近匹配祖先。window.matchMedia(query) 返回 MediaQueryList（matches 属性 + change 事件）。querySelectorAll 配合 :scope / :is / :where 实现复杂选择器查询。'), h('div', { class: 'flex gap-sm flex-wrap' }, this._btn('supports 检测 grid/flex/lab/color-mix', { type: 'primary', size: 'sm', onClick: () => this._supportsCheck() }), this._btn('matches 演示', { size: 'sm', onClick: () => this._matchesDemo() }), this._btn('closest 演示', { size: 'sm', onClick: () => this._closestDemo() }), this._btn("matchMedia '(prefers-color-scheme: dark)'", { size: 'sm', onClick: () => this._matchMediaDemo() })), h('div', { class: 'cssom-match-stage' }, h('div', { class: 'item highlight' }, 'item.highlight（matches / closest 演示目标）'), h('div', { class: 'item warning' }, 'item.warning'), h('div', { class: 'item' }, 'item（普通）')), s.supportsResult && h('pre', { class: 'code-block' }, s.supportsResult), s.mediaResult && h('pre', { class: 'code-block' }, s.mediaResult)));
    }
    // ===================== 日志面板 =====================
    _renderLogPanel() {
        const s = this.state;
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'page api-lab-page' }, h('h2', { class: 'section-title' }, 'CSS Object Model 实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, '演示 CSSOM：element.style 内联样式 / getComputedStyle 计算样式 / CSSStyleSheet 规则操作 / Constructable Stylesheets + adoptedStyleSheets / CSS.supports + matchMedia。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderLogPanel());
    }
}
//# sourceMappingURL=CSSObjectModelPage.js.map