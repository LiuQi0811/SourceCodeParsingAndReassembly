...
export class ComputePressureAPIPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!f.pressureObserver)
            this._addLog('warn', 'PressureObserver 不可用（jsdom 不实现；Chrome/Edge 125+ ship，Safari/Firefox 开发中）');
        if (!f.pressureRecord)
            this._addLog('warn', 'PressureRecord 不可用（jsdom 不实现；PressureObserver 输出的记录类型）');
    ...
    _readObserverInfo() {
        ...
        try {
            return `===== PressureObserver 基础 =====\n` +
                `\n` +
                `【构造】\n` +
                `  const observer = new PressureObserver((records: any, observer: any) => {\n` +
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'PressureObserver 通过 new PressureObserver(callback) 构造，callback 在状态变化时异步触发（非轮询）。observe(\'cpu\', { sampleRate }) 开始观察指定 source（sampleRate 限制回调频率上限，防指纹）。unobserve(source) 停止单个 source，disconnect() 全部断开（卸载时必须调用）。takeRecords() 同步取出待处理记录。回调签名 (records: PressureRecord[], observer)，状态升级/降级均触发。sampleRate 默认约 1Hz，过高耗电。Chrome 125+ ship。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行观察者演示', { type: 'primary', size: 'sm', onClick: () => this._runObserverDemo() }), this._btn('cpu', { size: 'sm', onClick: () => this._setObserveSource('cpu') }), this._btn('thermal', { size: 'sm', onClick: () => this._setObserveSource('thermal') })),
                h('div', { class: 'flex items-center gap-sm flex-wrap mt-sm' }, h('span', { class: 'fs-sm text-secondary' }, 'sampleRate：'), this._btn('0.5 Hz', { size: 'sm', onClick: () => this._setSampleRate(0.5) }), this._btn('1 Hz', { size: 'sm', onClick: () => this._setSampleRate(1.0) }), this._btn('2 Hz', { size: 'sm', onClick: () => this._setSampleRate(2.0) })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '当前 source：' + this._observeSource + ' / sampleRate：' + this._sampleRate + ' Hz'),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.observerInfo || '（点击「运行观察者演示」创建 PressureObserver 并查看代码）')),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['cpu source', f.pressureObserver]]), h(Tag, { color: 'primary' }, '3 类 source')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'source 取值：cpu（当前唯一广泛支持，Chrome 125+ ship，反映系统整体 CPU 负载，状态映射 nominal<50%/fair 50-75%/serious 75-90%/critical>90% 近似阈值）/thermal（提案中，反映设备温度，移动端散热受限场景，Chrome 部分 Origin Trial）/power-supply（提案中，电源状态，与 Battery Status API 区别为离散状态非精确数值）。浏览器支持矩阵：cpu Chrome 125+/Edge 125+ ship，thermal/power-supply 均未 ship。安全考虑：仅 4 级离散状态避免指纹识别，sampleRate 限制频率，需 HTTPS。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 source 探测', { type: 'primary', size: 'sm', onClick: () => this._runSourceDemo() }), ...sources.map(([src, name]) => this._btn(name, { size: 'sm', onClick: () => this._setCurrentSource(src) }))),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '当前查看 source：' + this._currentSource),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.sourceInfo || '（点击按钮查看 source 矩阵与能力检测代码）')),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['4 级状态', true]]), h(Tag, { color: 'primary' }, '离散量化')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'state 4 级：nominal（正常，CPU<50%）/fair（轻微，50-75%）/serious（严重，75-90%，建议降级）/critical（危急，>90%，必须降级）。阈值由浏览器/操作系统/硬件共同决定非规范固定。状态转换触发回调（升级/降级均触发，同状态不重复），sampleRate 限制频率。与 CPU 占用率映射为离散量化（4 级），但实际依赖 OS 调度/核心数/热量/电源。应对建议：nominal 维持/fair 准备/serious 降级/critical 紧急降级。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行状态演示', { type: 'primary', size: 'sm', onClick: () => this._runStateDemo() }), ...states.map(([state, name]) => this._btn(name, { size: 'sm', onClick: () => this._setCurrentState(state) }))),
                h('div', { class: 'cp-state-grid' }, states.map(([state, name, cls]) => h('div', { class: 'cp-state-cell ' + cls + (this._currentState === state ? ' active' : '') }, h('div', { class: 'lvl' }, name), h('div', { class: 'desc' }, state)))),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.stateInfo || '（点击按钮切换模拟状态查看说明）')),
                h(Alert, {
                    type: 'info',
    ...
    _readRecordInfo() {
        ...
        try {
            return `===== PressureRecord 结构 =====\n` +
                ...
                `  });\n` +
                `  // 计算压力分布\n` +
                `  const stats = history.reduce((acc: any, r: any) => {\n` +
                ...
                `【完整代码示例】\n` +
                `  const history = [];\n` +
                `  const observer = new PressureObserver((records: any, obs: any) => {\n` +
                ...
                `  // 5 秒后输出统计\n` +
                `  setTimeout(() => {\n` +
                `    const stats = history.reduce((a: any, r: any) => {\n` +
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['PressureRecord', f.pressureRecord]]), h(Tag, { color: 'primary' }, 'DOMHighResTimeStamp')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'PressureRecord 属性：source（压力源 cpu/thermal/power-supply）、state（4 级状态）、time（DOMHighResTimeStamp 毫秒，相对 performance.timeOrigin，与 performance.now() 同源）、toJSON() 序列化为 { source, state, time }。PressureObserver 不维护历史需手动累积（push r.toJSON()）。sampleRate 是回调频率上限（Hz），实际仅在状态变化时触发（持续 nominal 不重复回调）。takeRecords() 同步取出清空队列，适合 disconnect 前最后获取。高 sampleRate 耗电 + 指纹风险，建议 ≤ 1Hz。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 Record 演示', { type: 'primary', size: 'sm', onClick: () => this._runRecordDemo() })),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.recordInfo || '（点击按钮查看 PressureRecord 结构与历史累积代码）')),
                h(Alert, {
                    type: 'info',
    ...
    _readVideoConfInfo() {
        ...
        try {
            return `===== 实战：视频会议自适应降级 =====\n` +
                ...
                `\n` +
                `  // 3. 监听压力并降级\n` +
                `  const observer = new PressureObserver(async (records: any) => {\n` +
                `    for (const r of records) {\n` +
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['PressureObserver', f.pressureObserver]]), h(Tag, { color: 'primary' }, 'MediaStreamTrack')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '视频会议自适应降级：critical 关闭视频（仅音频，最低功耗）/serious 降低分辨率（1280x720→640x360）/fair 降低帧率（30fps→15fps）/nominal 恢复。与 MediaStreamTrack.applyConstraints() 协同热切换采集参数（无需重新 getUserMedia）。WebRTC 协同用 RTCRtpSender.setParameters() 调整 maxBitrate/maxFramerate。PressureObserver 监听 cpu source 触发降级策略。降级表：nominal 1280x720@30/fair 1280x720@15/serious 640x360@15/critical 关闭视频。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行视频降级演示', { type: 'primary', size: 'sm', onClick: () => this._runVideoConfDemo() }), this._btn('nominal 策略', { size: 'sm', onClick: () => this._setVideoPolicy('nominal') }), this._btn('serious 策略', { size: 'sm', onClick: () => this._setVideoPolicy('serious') }), this._btn('critical 策略', { size: 'sm', onClick: () => this._setVideoPolicy('critical') })),
                h('table', { class: 'cp-policy-table' }, h('thead', {}, h('tr', {}, h('th', {}, 'state'), h('th', {}, '分辨率'), h('th', {}, '帧率'), h('th', {}, '动作'))), h('tbody', {}, h('tr', {}, h('td', { class: 'state-nominal' }, 'nominal'), h('td', {}, '1280x720'), h('td', {}, '30fps'), h('td', {}, '恢复')), h('tr', {}, h('td', { class: 'state-fair' }, 'fair'), h('td', {}, '1280x720'), h('td', {}, '15fps'), h('td', {}, '降帧率')), h('tr', {}, h('td', { class: 'state-serious' }, 'serious'), h('td', {}, '640x360'), h('td', {}, '15fps'), h('td', {}, '降分辨率')), h('tr', {}, h('td', { class: 'state-critical' }, 'critical'), h('td', {}, '关闭视频'), h('td', {}, '-'), h('td', {}, '仅音频')))),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.videoConfInfo || '（点击按钮查看视频会议降级完整代码）')),
                h(Alert, {
                    type: 'info',
    ...
    _readGameAdaptInfo() {
        ...
        try {
            return `===== 实战：游戏画质自适应 =====\n` +
                ...
                `    const perfObs = new PerformanceObserver((list: any) => {\n` +
    ...
    _renderCard7() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['PressureObserver', f.pressureObserver]]), h(Tag, { color: 'primary' }, '5 级画质')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '游戏画质自适应：CPU 压力升高时降低粒子数（1000→100）/视距（2000→300）/阴影（高→关）/后处理（全开→全关）/渲染分辨率（1.0→0.5）。与 requestAnimationFrame 配合：PressureObserver 异步设置 targetQuality，rAF 循环每帧读取应用。WebGPU 性能监控：无直接压力 API，通过帧时间统计（>33ms 即 <30fps）间接判断 GPU 压力，与 CPU 压力协同（CPU 降 AI/物理，GPU 降渲染）。长任务降级用 PerformanceObserver longtask 补充。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行游戏自适应演示', { type: 'primary', size: 'sm', onClick: () => this._runGameAdaptDemo() }), ...qualities.map(([q, name]) => this._btn(name, { size: 'sm', onClick: () => this._setGameQuality(q) }))),
                h('div', { class: 'cp-quality-bar' }, qualities.map(([q, name, cls]) => h('div', { class: 'seg ' + cls + (this._gameQuality === q ? '' : ' inactive') }, name))),
                h('pre', { class: 'code-block mt-sm', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.gameAdaptInfo || '（点击按钮查看游戏画质自适应完整代码）')),
                h(Alert, {
                    type: 'info',
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'api-lab-page compute-pressure-api-page' }, h('h2', { class: 'section-title' }, 'Compute Pressure API 完整实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, '本页演示 W3C Compute Pressure API 系统压力感知：PressureObserver 基础（observe/unobserve/disconnect/takeRecords）、压力源 source（cpu/thermal/power-supply）、压力状态 state（nominal/fair/serious/critical 4 级）、PressureRecord 结构（source/state/time/toJSON）、视频会议自适应降级（applyConstraints 协同）、游戏画质自适应（rAF + WebGPU 协同）、陷阱与最佳实践（去重/频率/隐私/deviceMemory/Scheduler/LongTask 降级）。所有特性通过 typeof 能力检测，不可用时仅记日志，绝不抛异常。jsdom 不实现该 API，真实浏览器（Chrome 125+）可运行实际压力监听。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderCard6(), this._renderCard7(), this._renderCard8(), this._renderLogPanel());
    }
}
//# sourceMappingURL=ComputePressureAPIPage.js.map