...
export class WebComponentsAdvancedPage extends Page {
    ...
    componentDidMount() {
        ...
        this._formEl = null;
        this._stateEl = null;
        this._stateInternals = null;
        this._definedEl = null;
        this._composedEl = null;
        this._cssPartEl = null;
        this._observer = null;
        ...
        if (!d.attachInternals)
            this._addLog('warn', 'Element.prototype.attachInternals 不可用（jsdom 通常不支持），form-associated / customStateSet 演示将回退到代码示例');
        if (!d.dsd)
            this._addLog('warn', 'Declarative Shadow DOM 不可用（HTMLTemplateElement.prototype 无 shadowRootMode），DSD 演示将回退到 polyfill 与代码示例');
        if (!d.getHTML)
            this._addLog('warn', 'Element.prototype.getHTML 不可用，序列化演示将回退到 outerHTML');
        if (!d.getName)
            this._addLog('warn', 'customElements.getName 不可用（较新 API，部分浏览器无）');
        if (!d.upgrade)
            this._addLog('warn', 'customElements.upgrade 不可用（较新 API）');
    }
    ...
    _serializeDSD() {
        ...
        try {
            ...
            if (method.startsWith('getHTML'))
                this._addLog('dsd', `getHTML({ serializable: true }) → ${serialized.length} 字符`);
        }
        catch (err) {
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '<template shadowrootmode="open|closed"> 让服务端在 HTML 中声明 Shadow DOM，无需 JS 即可渲染（无 FOUC，禁用 JS 仍可展示）。Element.prototype.attachShadow({ mode }) 是其 JS 等价物。检测：HTMLTemplateElement.prototype 有 shadowRootMode 属性即支持。element.getHTML({ serializable: true }) 序列化 Shadow DOM 含声明式模板；shadowRoot.serializable / delegatesFocus 控制序列化与焦点委托。jsdom 通常不支持 DSD。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('检测能力', { type: 'primary', size: 'sm', onClick: () => this._checkDSD() }), this._btn('注入 DSD', { size: 'sm', onClick: () => this._injectDSD() }), this._btn('序列化 getHTML', { size: 'sm', disabled: !d.getHTML, onClick: () => this._serializeDSD() }), this._btn('polyfill 说明', { size: 'sm', onClick: () => this._showPolyfill() })),
                h('div', { class: 'wc-adv-dsd-host mt-sm' }),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, 'Declarative Shadow DOM 状态 / 序列化：'),
                h('pre', { class: 'code-block', style: { maxHeight: '280px', overflow: 'auto' } }, h('code', {}, s.dsdInfo || '（点击「检测能力」或「注入 DSD」开始 DSD 演示）')),
                h(Alert, {
                    type: 'info',
    ...
    _defineFormAssociated() {
        ...
        try {
            ...
            class WcAdvField extends HTMLElement {
                ...
                formResetCallback() { this.value = ''; }
                formDisabledCallback(_d) { }
                formStateRestoreCallback(_s, _m) { }
            }
    ...
    _setFormValue() {
        ...
        try {
            ...
            if (!internals) {
                this._addLog('warn', 'ElementInternals 不可用（元素未升级或 attachInternals 未执行）');
                return;
            }
            ...
            if (internals.validity && 'valid' in internals.validity)
                lines.push(`internals.validity.valid = ${internals.validity.valid}`);
            if (typeof internals.form !== 'undefined')
                lines.push(`internals.form = ${internals.form ? '<form>' : 'null'}`);
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'static formAssociated = true 声明自定义元素为表单关联；this.attachInternals() 返回 ElementInternals，含 .form / .name / .type / .value / .validity / .validationMessage / .willValidate / .setValidity / .checkValidity / .reportValidity / .labels / .setFormValue(value, state)。回调：formDisabledCallback（fieldset[disabled] 变化）、formStateRestoreCallback(state, mode)（mode="restore"|"autocomplete"）。static formAssociated 必须为静态 getter 返回 true。jsdom 中 attachInternals 通常不可用。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('检测能力', { type: 'primary', size: 'sm', onClick: () => this._checkFormAssociated() }), this._btn('定义 form-associated', { size: 'sm', disabled: !d.ce, onClick: () => this._defineFormAssociated() }), this._btn('设置表单值', { size: 'sm', disabled: !d.attachInternals, onClick: () => this._setFormValue() })),
                h('div', { class: 'wc-adv-field-host mt-sm' }),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, 'form-associated / ElementInternals 状态：'),
                h('pre', { class: 'code-block', style: { maxHeight: '280px', overflow: 'auto' } }, h('code', {}, s.formAssociatedInfo || '（点击「检测能力」或「定义 form-associated 元素」）')),
                h(Alert, {
                    type: 'info',
    ...
    _checkState() {
        ...
        if (!statesSupported)
            this._addLog('warn', 'customStateSet 不可用（jsdom 通常不支持），:state() 演示将回退到代码示例');
    }
    _addStates() {
        const d = this._detected();
        if (!d.attachInternals) {
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([{ label: 'attachInternals', ok: d.attachInternals }]), h(Tag, { color: 'primary' }, ':state()')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'this.attachInternals().states 返回 customStateSet（Set-like）：.add(state) / .delete(state) / .has(state) / .size / .clear() / 可迭代。CSS 用 my-el:state(loading) 选择，是自定义元素的 :checked / :disabled 等价物，替代 hacky 的 [data-error] 属性样式化。状态由作者定义，不暴露给外部 JS 读取（只有元素自己能 set），安全。jsdom 通常不支持。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('检测能力', { type: 'primary', size: 'sm', onClick: () => this._checkState() }), this._btn('添加状态', { size: 'sm', disabled: !d.attachInternals, onClick: () => this._addStates() }), this._btn('切换 error', { size: 'sm', disabled: !d.attachInternals, onClick: () => this._toggleState() }), this._btn('清空状态', { danger: true, size: 'sm', disabled: !d.attachInternals, onClick: () => this._clearStates() })),
                h('div', { class: 'wc-adv-state-host mt-sm' }),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, 'customStateSet / :state() 状态：'),
                h('pre', { class: 'code-block', style: { maxHeight: '280px', overflow: 'auto' } }, h('code', {}, s.stateInfo || '（点击「检测能力」或「添加状态」开始 :state() 演示）')),
                h(Alert, {
                    type: 'info',
    ...
    _demoIsAttribute() {
        ...
        try {
            if (typeof customElements.get === 'function' && customElements.get('wc-adv-div-builtin')) {
                ...
                class WcAdvDiv extends HTMLDivElement {
                    constructor() { super(); }
                }
            ...
            if (container) {
                ...
                try {
                    if (typeof el.matches === 'function')
                        definedMatch = el.matches(':defined') ? 'true（已定义）' : 'false（未定义）';
                }
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, ':defined 伪类匹配已注册定义的自定义元素（注册前用 :not(:defined) 隐藏防 FOUC）。CustomElementRegistry：.define(name, ctor, { extends }) 注册（extends 指定自定义内置元素基类）、.get(name) 按名取构造器、.getName(ctor) 按构造器取名（较新）、.upgrade(node) 同步升级未连接节点、.whenDefined(name) → Promise。自定义内置元素：<div is="my-div"> + define({ extends: "div" })。Safari 曾长期拒绝 is=（现已支持）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('检测能力', { type: 'primary', size: 'sm', onClick: () => this._checkDefined() }), this._btn('is= 自定义内置元素', { size: 'sm', disabled: !d.ce, onClick: () => this._demoIsAttribute() }), this._btn('upgrade 演示', { size: 'sm', disabled: !d.upgrade, onClick: () => this._demoUpgrade() })),
                h('div', { class: 'wc-adv-defined-host mt-sm' }),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, ':defined / CustomElementRegistry 状态：'),
                h('pre', { class: 'code-block', style: { maxHeight: '280px', overflow: 'auto' } }, h('code', {}, s.definedInfo || '（点击「检测能力」或「is= 自定义内置元素」）')),
                h(Alert, {
                    type: 'info',
    ...
    _dispatchComposed() {
        ...
        try {
            ...
            host.addEventListener('inner-action', (e) => {
                ...
                let targetInfo = '（不可读）';
                try {
                    pathInfo = e.composedPath ? e.composedPath().map((n) => n.nodeName || String(n)).join(' → ') : '（无 composedPath）';
                }
                catch { /* noop */ }
                try {
                    targetInfo = e.target ? (e.target.nodeName || String(e.target)) : 'null';
                }
                catch { /* noop */ }
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'Shadow DOM 内事件默认不跨边界；new CustomEvent(name, { composed: true }) 让事件跨 Shadow 边界冒泡。event.composedPath() 返回完整路径（含 Shadow 节点，从 target 到 window）；event.target 跨边界时被重定向为 shadow host（不是内部节点）；event.composed（boolean）标记是否可跨边界。slotchange 在 <slot> 分发内容变化时触发；getRelatedTarget() 在 focus/mouse 跨边界时返回相关目标（防泄露内部节点）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('检测能力', { type: 'primary', size: 'sm', onClick: () => this._checkComposed() }), this._btn('分发 composed 事件', { size: 'sm', disabled: !d.attachShadow, onClick: () => this._dispatchComposed() }), this._btn('composedPath 说明', { size: 'sm', onClick: () => this._showComposedPath() })),
                h('div', { class: 'wc-adv-composed-host mt-sm' }),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, 'Shadow DOM 事件 / composedPath 状态：'),
                h('pre', { class: 'code-block', style: { maxHeight: '280px', overflow: 'auto' } }, h('code', {}, s.composedInfo || '（点击「检测能力」或「分发 composed 事件」）')),
                h(Alert, {
                    type: 'info',
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'CSS 自定义属性（--var）通过继承穿透 Shadow DOM（在 :host 设置，内部 var() 读取）。:host 选择宿主；:host(.class) 匹配带类宿主；:host-context(.dark) 祖先匹配。::part(name) 外部样式化 part="name" 的内部元素（唯一穿透方式，除继承变量）；::part(name):hover 可叠加伪类。exportparts 转发嵌套 Shadow 的 part。::slotted(*) 样式化被分发节点。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('检测能力', { type: 'primary', size: 'sm', onClick: () => this._checkCssPart() }), this._btn('注入 ::part 样式', { size: 'sm', disabled: !d.attachShadow, onClick: () => this._injectPartStyle() }), this._btn('自定义属性穿透', { size: 'sm', onClick: () => this._demoCustomProp() }), this._btn('聚合能力检测', { type: 'primary', size: 'sm', onClick: () => this._aggregateDetect() })),
                h('div', { class: 'wc-adv-css-host mt-sm' }),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, 'CSS 穿透与 ::part() 状态：'),
                h('pre', { class: 'code-block', style: { maxHeight: '320px', overflow: 'auto' } }, h('code', {}, s.cssPartInfo || '（点击「检测能力」或「注入 ::part 样式」）')),
                h(Alert, {
                    type: 'info',
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'api-lab-page web-components-advanced-page' }, h('h2', { class: 'section-title' }, 'Web Components 高级特性 实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, '深入 Declarative Shadow DOM、formAssociated 表单集成、customStateSet :state() 伪类、:defined 与 CustomElementRegistry、Shadow DOM 事件 composedPath、CSS 自定义属性穿透与 ::part()。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderCard6(), this._renderLogPanel());
    }
}
//# sourceMappingURL=WebComponentsAdvancedPage.js.map