...
export class DragDropDeepPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!hasDragEvent)
            this._addLog('warn', 'DragEvent 构造器不可用，将退化为普通 Event 并手动挂 dataTransfer');
        if (!hasDataTransferItem)
            this._addLog('warn', 'DataTransferItem / DataTransferItemList 不可用（jsdom 可能未实现）');
        if (!hasFile)
            this._addLog('warn', 'File 构造器不可用（无法模拟文件拖入）');
    }
            ...
            const handler = (e) => {
                ...
                if (e.type === 'dragstart' && e.dataTransfer) {
                    try {
                        ...
                        extra = ' · setData(text/plain, "payload-42")';
                    }
                    catch (err) {
                        extra = ' · setData 失败';
                    }
                ...
                if (e.type === 'drop' && e.dataTransfer) {
                    let got = '';
                    try {
                        got = e.dataTransfer.getData('text/plain');
                    }
                    catch (err) {
                        got = '(读取失败)';
                    }
    ...
    _runFullLifecycle() {
        ...
        this.setState({
            lifecycleInfo: `模拟完整拖拽生命周期（7 个事件）：\n` +
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.dragEvent ? 'success' : 'warning' }, caps.dragEvent ? 'DragEvent ✓' : 'DragEvent ✗'), h(Tag, { color: caps.dataTransfer ? 'success' : 'error' }, caps.dataTransfer ? 'DataTransfer ✓' : '不可用')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'HTML 拖放 7 个事件：dragstart（源开始拖拽，setData）/ drag（拖拽进行中，高频）/ dragend（源拖拽结束）；dragenter（进入目标，preventDefault 允许放置）/ dragover（悬停，必须 preventDefault 才能触发 drop）/ dragleave（离开目标）/ drop（在目标上释放）。draggable="true" 让元素可拖拽。jsdom 不模拟真实拖拽，故提供「模拟触发」按钮用 dispatchEvent 派发事件。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, h('div', {
                    class: 'dd-source',
                    draggable: true,
                    style: { padding: '12px 16px', border: '2px dashed #1677ff', borderRadius: '6px', cursor: 'grab', userSelect: 'none', color: '#1677ff' },
                }, '⬆ 可拖拽源元素（draggable=true）'), h('div', {
                    class: 'dd-drop',
                ...
                h('div', { class: 'flex items-center gap-sm flex-wrap mt-sm' }, this._btn('模拟 dragstart', { type: 'primary', size: 'sm', disabled: !caps.dataTransfer, onClick: () => this._dispatchLifecycleEvent('source', 'dragstart') }), this._btn('模拟 drag', { size: 'sm', disabled: !caps.dataTransfer, onClick: () => this._dispatchLifecycleEvent('source', 'drag') }), this._btn('模拟 dragenter', { size: 'sm', disabled: !caps.dataTransfer, onClick: () => this._dispatchLifecycleEvent('drop', 'dragenter') }), this._btn('模拟 dragover', { size: 'sm', disabled: !caps.dataTransfer, onClick: () => this._dispatchLifecycleEvent('drop', 'dragover') }), this._btn('模拟 dragleave', { size: 'sm', disabled: !caps.dataTransfer, onClick: () => this._dispatchLifecycleEvent('drop', 'dragleave') }), this._btn('模拟 drop', { type: 'primary', size: 'sm', disabled: !caps.dataTransfer, onClick: () => this._dispatchLifecycleEvent('drop', 'drop') }), this._btn('模拟 dragend', { size: 'sm', disabled: !caps.dataTransfer, onClick: () => this._dispatchLifecycleEvent('source', 'dragend') })),
                h('div', { class: 'flex items-center gap-sm flex-wrap mt-xs' }, this._btn('完整流程', { type: 'primary', size: 'sm', disabled: !caps.dataTransfer, onClick: () => this._runFullLifecycle() }), this._btn('重置 DataTransfer', { size: 'sm', disabled: !caps.dataTransfer, onClick: () => this._clearLifecycleDt() })),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '生命周期说明：'),
                h('pre', { class: 'code-block', style: { maxHeight: '180px', overflow: 'auto' } }, h('code', {}, s.lifecycleInfo || '（点击「完整流程」或单独事件按钮）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '160px', overflow: 'auto' } }, h('code', {}, `const src = document.querySelector('.source');   // draggable=true
const drop = document.querySelector('.drop');
src.addEventListener('dragstart', (e: any) => {
  e.dataTransfer.setData('text/plain', 'payload'); // 写入数据
  e.dataTransfer.effectAllowed = 'copy';
});
drop.addEventListener('dragover', (e: any) => e.preventDefault()); // 必须 preventDefault
...
src.addEventListener('dragend', () => console.log('拖拽结束'));`)),
    ...
    _demoDataTransferFormats() {
        ...
        try {
            ...
            this.setState({
                formatsInfo: `new DataTransfer() → 独立 DataTransfer（无需事件即可用）\n` +
                    `dt.setData("text/plain", "纯文本载荷")\n` +
                    `dt.setData("text/html", "<strong>富文本</strong>")\n` +
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.dataTransfer ? 'success' : 'error' }, caps.dataTransfer ? 'DataTransfer ✓' : '不可用'), h(Tag, { color: 'primary' }, 'text/plain / html / uri-list')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'new DataTransfer() 可独立创建数据传输对象（无需在事件中）。dt.setData(format, data) 写入数据；dt.getData(format) 读出；dt.types 返回当前存储的格式列表（DOMStringList，只读）；dt.clearData(format) 移除单格式，dt.clearData() 无参清空全部。常见格式：text/plain（纯文本）、text/html（富文本）、text/uri-list（URL）、application/json。本卡片演示多格式写入与读取。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('演示多格式', { type: 'primary', size: 'sm', disabled: !caps.dataTransfer, onClick: () => this._demoDataTransferFormats() })),
                h('div', { class: 'fs-sm text-secondary' }, 'DataTransfer 状态：'),
                h('pre', { class: 'code-block', style: { maxHeight: '220px', overflow: 'auto' } }, h('code', {}, s.formatsInfo || '（点击「演示多格式」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '140px', overflow: 'auto' } }, h('code', {}, `const dt = new DataTransfer();          // 独立构造
...
dt.clearData();                          // 清空全部`)),
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.dataTransferItemList ? 'success' : 'warning' }, caps.dataTransferItemList ? 'ItemList ✓' : 'ItemList ✗'), h(Tag, { color: 'primary' }, 'getAsString / getAsFile')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'dataTransfer.items 是 DataTransferItemList，每个元素是 DataTransferItem。item.kind ∈ "string"（字符串）/"file"（文件）；item.type 是 MIME 类型。item.getAsString(callback) 异步读取 string 项内容（回调无返回值）；item.getAsFile() 对 file 项返回 File 对象、对 string 项返回 null。items.add(data, type) 添加字符串项、items.add(file) 添加文件项；items.remove(index) 移除项。与 dt.files 不同，items 可异步读取文件内容（拖入文件夹时）。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('演示 items（异步）', { type: 'primary', size: 'sm', disabled: !caps.dataTransfer, onClick: () => this._demoDataTransferItemList() })),
                h('div', { class: 'fs-sm text-secondary' }, 'DataTransferItem 状态：'),
                h('pre', { class: 'code-block', style: { maxHeight: '240px', overflow: 'auto' } }, h('code', {}, s.itemsInfo || '（点击「演示 items」—— 含异步回调，请稍候）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '160px', overflow: 'auto' } }, h('code', {}, `const dt = new DataTransfer();
...
dt.items.remove(0);                             // 移除第 0 项`)),
    ...
    _demoEffects() {
        ...
        try {
            ...
            this.setState({
                effectsInfo: `effectAllowed 取值（8 种）：${EFFECT_ALLOWED_VALUES.join(' / ')}\n` +
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.dataTransfer ? 'success' : 'error' }, caps.dataTransfer ? 'DataTransfer ✓' : '不可用'), h(Tag, { color: 'primary' }, '8 × 4 组合')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'e.dataTransfer.effectAllowed 在 dragstart 设置（源端约束允许的操作）：none / copy / move / link / copyMove / copyLink / linkMove / all（8 种）。e.dataTransfer.dropEffect 在 dragover / drop 设置（目标端决定最终操作与光标样式）：none / copy / move / link（4 种）。dropEffect 必须是 effectAllowed 允许的子集，否则浏览器忽略该设置。dropEffect 影响 dragend 时源端读取的值，从而决定是复制还是移动。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('遍历 effect 组合', { type: 'primary', size: 'sm', disabled: !caps.dataTransfer, onClick: () => this._demoEffects() })),
                h('div', { class: 'fs-sm text-secondary' }, 'effect 组合结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, s.effectsInfo || '（点击「遍历 effect 组合」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '140px', overflow: 'auto' } }, h('code', {}, `src.addEventListener('dragstart', (e: any) => {
...
src.addEventListener('dragend', (e: any) => {
  console.log(e.dataTransfer.dropEffect);      // 'copy' / 'move' / 'none'
    ...
    _demoSetDragImage() {
        ...
        try {
            ...
            this.setState({
                dragImageInfo: `e.dataTransfer.setDragImage(element, xOffset, yOffset)\n` +
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.dataTransfer ? 'success' : 'error' }, caps.dataTransfer ? 'DataTransfer ✓' : '不可用'), h(Tag, { color: 'primary' }, 'setDragImage')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'e.dataTransfer.setDragImage(element, xOffset, yOffset) 在 dragstart 调用，用指定元素的快照替代浏览器默认的半透明源元素拖拽图像。element 可以是任意已渲染的 DOM 元素（含 canvas，可用 canvas.toDataURL 自绘）；xOffset / yOffset 是鼠标相对于图像左上角的偏移（像素）。jsdom 无渲染引擎，无视觉效果，仅验证 API 可调用。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('演示 setDragImage', { type: 'primary', size: 'sm', disabled: !caps.dataTransfer, onClick: () => this._demoSetDragImage() })),
                h('div', { class: 'fs-sm text-secondary' }, 'setDragImage 结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '200px', overflow: 'auto' } }, h('code', {}, s.dragImageInfo || '（点击「演示 setDragImage」）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '150px', overflow: 'auto' } }, h('code', {}, `src.addEventListener('dragstart', (e: any) => {
  ...
  e.dataTransfer.setDragImage(canvas, 60, 20); // 偏移居中
        ...
        try {
            ...
            if (items && typeof items.add === 'function') {
                try {
                    ...
                    lines.push(`items.add(note.txt) 失败：${e.message}`);
                }
                try {
                    items.add(file2);
                    added++;
                    ...
                    lines.push(`items.add(pic.png) 失败：${e.message}`);
                }
            }
            else {
            ...
            this.setState({
                fileInfo: `new DataTransfer() → 模拟拖入文件\n` +
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: caps.file ? 'success' : 'error' }, caps.file ? 'File ✓' : 'File ✗'), h(Tag, { color: caps.dataTransferItemList ? 'success' : 'warning' }, caps.dataTransferItemList ? 'ItemList ✓' : 'ItemList ✗')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '真实拖入文件时，系统自动填充 e.dataTransfer.files（FileList 同步快照）与 e.dataTransfer.items（DataTransferItemList，支持异步读取）。File 继承自 Blob，可用 file.text() / file.arrayBuffer() / file.slice() 读取内容。本卡片用 new File([...], name, { type }) 构造模拟文件，通过 items.add(file) 添加，再读回 files 与 items。jsdom 可能未把 items.add(file) 同步到 files，真实浏览器会同步。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('模拟拖入文件', { type: 'primary', size: 'sm', disabled: !caps.dataTransfer || !caps.file, onClick: () => this._demoFileDrop() })),
                h('div', { class: 'fs-sm text-secondary' }, '文件拖入结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '260px', overflow: 'auto' } }, h('code', {}, s.fileInfo || '（点击「模拟拖入文件」—— 含异步读取，请稍候）')),
                h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
                h('pre', { class: 'code-block', style: { maxHeight: '170px', overflow: 'auto' } }, h('code', {}, `drop.addEventListener('drop', async (e) => {
  e.preventDefault();
  ...
  for (const item of e.dataTransfer.items) {
    ...
    render() {
        const s = this.state;
        return h('div', { class: 'page api-lab-page' }, h('h2', { class: 'section-title' }, 'HTML Drag and Drop API 深度实验室'), h('p', { class: 'fs-sm text-secondary mb-md' }, 'HTML 拖放 API 让元素可拖拽（draggable="true"）并在放置区交换数据。本页演示 DragEvent 7 个生命周期事件、DataTransfer 多格式数据、DataTransferItemList 异步读取、effectAllowed/dropEffect 光标约束、setDragImage 自定义图像与文件拖入。'), s.capsSummary !== '' ? h(Alert, { type: 'info', message: s.capsSummary }) : null, this._renderCard1(), this._renderCard2(), this._renderCard3(), this._renderCard4(), this._renderCard5(), this._renderCard6(), this._renderLogPanel());
    }
}
//# sourceMappingURL=DragDropDeepPage.js.map