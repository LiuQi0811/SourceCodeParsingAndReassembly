...
export class CSSAnchorPositioningDeepPage extends Page {
    ...
    componentDidMount() {
        ...
        if (!f.anchorName)
            this._addLog('warn', 'anchor-name 不可用（Chrome 125+/Safari TP 起支持，Firefox 仍在实现中）');
        if (!f.positionTryFallbacks)
            this._addLog('warn', 'position-try-fallbacks 不可用（Chrome 125+）');
        if (!f.positionArea)
            this._addLog('warn', 'position-area 不可用（Chrome 125+，早期叫 inset-area）');
    ...
    _renderCard1() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '两步走：(1) 在锚元素上声明 anchor-name: --my-anchor（必须 dashed-ident）；(2) 在目标元素上绑定 position-anchor: --my-anchor。之后该目标的 anchor()/anchor-size() 都基于此锚。浏览器原生管理重排，滚动/缩放/方向变化无需 JS 重算。Chrome 125+ 正式支持。'),
                ...
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.basicsInfo || '（点击按钮查看 anchor-name + position-anchor 完整用法）')),
            ],
        });
    ...
    _renderCard2() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['anchor()', f.anchorFn]]), h(Tag, { color: 'primary' }, '函数值')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'anchor(<anchor-name>? <anchor-side>) 取锚的某条边或中心作为目标定位依据，可用于 top/right/bottom/left/margin。anchor-side 取值：top/bottom/left/right（物理）/start/end/self-start/self-end（逻辑）/center/<percentage>/inner/outer。可与 position-anchor 配合省略锚名，或显式 anchor(--a bottom) 用不同锚。锚 detached 时返回 0，可降级到 @position-try 候选。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 anchor() 演示', { type: 'primary', size: 'sm', onClick: () => this._runAnchorFnDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.anchorFnInfo || '（点击按钮查看 anchor() 函数完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard3() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['anchor-size()', f.anchorSize]]), h(Tag, { color: 'primary' }, '尺寸函数')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'anchor-size(<anchor-name>? width|height|inline-size|block-size) 取锚的尺寸作为目标的 width/height/max-width 等。典型场景：下拉菜单 width: anchor-size(width) 与触发按钮同宽；表单错误提示 max-width: anchor-size(width) 不超过输入框；可配合 calc 减去固定偏移。锚可以是任意元素，不必是父级，可跨层级跨组件。Chrome 125+ 支持。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 anchor-size() 演示', { type: 'primary', size: 'sm', onClick: () => this._runAnchorSizeDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.anchorSizeInfo || '（点击按钮查看 anchor-size() 函数完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard4() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['position-area', f.positionArea]]), h(Tag, { color: 'primary' }, '9 宫格')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'position-area: <row> <col> 用 9 宫格语义声明目标相对锚的位置（如 top/bottom/center × left/right/center）。单值时另一维默认 center。span-* 跨多格子（如 span-left 跨左+中列）。语义化声明比 anchor()+translate 更简洁，自动跟随 writing-mode/direction 翻转。Chrome 119-124 旧名 inset-area，125+ 重命名为 position-area。'),
                h('div', { class: 'ap-area-grid' }, h('div', { class: 'ap-area-cell' }, 'top left'), h('div', { class: 'ap-area-cell' }, 'top'), h('div', { class: 'ap-area-cell' }, 'top right'), h('div', { class: 'ap-area-cell' }, 'left'), h('div', { class: 'ap-area-cell ap-center' }, '锚 center'), h('div', { class: 'ap-area-cell' }, 'right'), h('div', { class: 'ap-area-cell' }, 'bottom left'), h('div', { class: 'ap-area-cell' }, 'bottom'), h('div', { class: 'ap-area-cell' }, 'bottom right')),
                h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' }, this._btn('运行 position-area 演示', { type: 'primary', size: 'sm', onClick: () => this._runPositionAreaDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.positionAreaInfo || '（点击按钮查看 position-area 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard5() {
        ...
        const card = new Card({
            ...
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'position-try-fallbacks: <候选1>, <候选2>, ... 声明候选位置列表（flip-block/flip-inline/flip-start 翻转/--name 引用 @position-try/直接 position-area 值），浏览器按顺序检查，第一个不溢出的采用。position-try-order: most-width/most-height 等策略选可见区域最大的候选。替代 Popper.js/Floating UI 的纯 CSS 方案。Chrome 125+ 支持。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行避让演示', { type: 'primary', size: 'sm', onClick: () => this._runTryFallbacksDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.tryFallbacksInfo || '（点击按钮查看 position-try-fallbacks/order 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard6() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['@position-try', f.positionTryRule]]), h(Tag, { color: 'primary' }, '命名规则')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '@position-try --<name> { ... } 定义带额外属性的命名候选位置（仅 inset/margin/sizing/position-area/align-self 等定位属性可用，background/color 等会被忽略）。比 flip-block 翻转更强大：翻到侧边时可同时让 max-height/width 变化、加滚动条。position-try 简写可同时设 fallbacks + order。position-try-fallbacks: --top-right, --side-left 引用命名候选。Chrome 125+ 支持。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 @position-try 演示', { type: 'primary', size: 'sm', onClick: () => this._runPositionTryRuleDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.positionTryRuleInfo || '（点击按钮查看 @position-try 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard7() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, ...this._caps([['position-visibility', f.positionVisibility]]), h(Tag, { color: 'primary' }, '可见性')),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, 'position-visibility: always(默认)|anchors-visible|no-overflow 控制目标可见性。always 总显示（可能孤悬）；anchors-visible 仅锚至少部分可见时显示（Tooltip 场景）；no-overflow 目标自身不溢出 containing block 时显示（比 fallbacks 更激进直接隐藏）。替代 IntersectionObserver + JS 切换 display 的方案。Chrome 125+ 支持。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行 position-visibility 演示', { type: 'primary', size: 'sm', onClick: () => this._runVisibilityDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } }, h('code', {}, s.visibilityInfo || '（点击按钮查看 position-visibility 完整用法）')),
            ],
        });
        this.registerChild(card);
        return card.render();
    ...
    _renderCard8() {
        ...
        const card = new Card({
            ...
            extra: h('div', { class: 'flex gap-xs' }, h(Tag, { color: 'primary' }, '实战'), h(Tag, { color: f.popover ? 'success' : 'error' }, `Popover ${f.popover ? '✓' : '✗'}`)),
            children: [
                h('p', { class: 'fs-sm text-secondary' }, '完整实战模式：Tooltip 提示（自动翻转 + 锚不可见隐藏）/ 下拉菜单（Popover API + 同宽 + 自动避让）/ Combobox 自动补全（与输入框同宽 + 输入框贴底翻上方）/ Toast 通知（无需 anchor 直接对齐视口）/ 富文本编辑器悬浮工具栏（Range 作为锚）/ 图表数据点 Tooltip（SVG circle 作锚）。配合 anchor-scope 限制作用域、@supports 渐进增强、与 Popper.js/Floating UI 库对比。'),
                h('div', { class: 'flex items-center gap-sm flex-wrap' }, this._btn('运行实战模式演示', { type: 'primary', size: 'sm', onClick: () => this._runPatternDemo() })),
                h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
                h('pre', { class: 'code-block', style: { maxHeight: '600px', overflow: 'auto' } }, h('code', {}, s.patternInfo || '（点击按钮查看典型场景与实战模式完整代码）')),
            ],
        });
        this.registerChild(card);
        return card.render();
...
//# sourceMappingURL=CSSAnchorPositioningDeepPage.js.map