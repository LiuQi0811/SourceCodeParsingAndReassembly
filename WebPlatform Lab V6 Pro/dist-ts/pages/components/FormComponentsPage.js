// FormComponentsPage.ts —— 表单组件：Input / Select / Switch / Checkbox / 表单校验
...
export class FormComponentsPage extends Page {
    ...
    renderPage() {
        ...
        return [
            ...
            h(Card, { title: '表单校验示例（策略模式）' }, h('div', { class: 'flex flex-col gap-md', style: { maxWidth: '480px' } }, 
            // 姓名
            h('div', {}, h('label', { class: 'form-item__label is-required' }, '姓名'), nameInput.render(), h('div', { class: 'form-item__error' }, errors.name || '')), 
            // 邮箱
            h('div', {}, h('label', { class: 'form-item__label is-required' }, '邮箱'), emailInput.render(), h('div', { class: 'form-item__error' }, errors.email || '')), 
            // 城市
            h('div', {}, h('label', { class: 'form-item__label is-required' }, '城市'), citySelect.render(), h('div', { class: 'form-item__error' }, errors.city || '')), 
            // 性别
            h('div', {}, h('label', { class: 'form-item__label' }, '性别'), genderRadio.render()), 
            // 同意条款
            h('div', {}, agreeCheckbox.render(), h('div', { class: 'form-item__error' }, errors.agree || '')), 
            // 订阅
            h('div', { class: 'flex items-center justify-between' }, h('span', {}, '订阅邮件通知'), subscribeSwitch.render()), 
            // 按钮
            h('div', { class: 'flex gap-sm' }, submitBtn.render(), resetBtn.render()))),
            h(Card, { title: 'Input 输入框变体' }, h('div', { class: 'flex flex-col gap-md', style: { maxWidth: '480px' } }, (() => {
                const i = new Input({ placeholder: '默认尺寸' });
                this.registerChild(i);
                return i.render();
            })(), (() => {
                const i = new Input({ placeholder: '大尺寸', size: 'lg' });
                this.registerChild(i);
                return i.render();
            })(), (() => {
                const i = new Input({ placeholder: '小尺寸', size: 'sm' });
                this.registerChild(i);
                return i.render();
            })(), (() => {
                const i = new Input({ placeholder: '带前缀', addonBefore: 'https://' });
                this.registerChild(i);
                return i.render();
            })(), (() => {
                const i = new Input({ placeholder: '禁用', disabled: true });
                this.registerChild(i);
                return i.render();
            })(), (() => {
                const i = new Input({ placeholder: '多行文本', multiline: true, rows: 3 });
                this.registerChild(i);
                return i.render();
            })())),
            h(Card, { title: 'Radio 单选项' }, h('div', { class: 'flex flex-col gap-md' }, h('div', { class: 'demo-row' }, h('span', { class: 'demo-row__label' }, '基本'), (() => {
                const r = new Radio({
            ...
            // DatePicker
            h(Card, { title: 'DatePicker 日期选择器' }, h('div', { class: 'demo-row' }, h('span', { class: 'demo-row__label' }, '基本'), (() => { const dp = new DatePicker({ placeholder: '请选择日期' }); this.registerChild(dp); return dp.render(); })()), h('div', { class: 'demo-row' }, h('span', { class: 'demo-row__label' }, '可清除'), (() => { const dp = new DatePicker({ placeholder: '选择日期', allowClear: true, showToday: true }); this.registerChild(dp); return dp.render(); })()), h('div', { class: 'demo-row' }, h('span', { class: 'demo-row__label' }, '范围选择'), (() => { const rp = new RangePicker({}); this.registerChild(rp); return rp.render(); })())),
            // TimePicker
            h(Card, { title: 'TimePicker 时间选择器' }, h('div', { class: 'demo-row' }, h('span', { class: 'demo-row__label' }, '基本'), (() => { const tp = new TimePicker({ placeholder: '请选择时间' }); this.registerChild(tp); return tp.render(); })()), h('div', { class: 'demo-row' }, h('span', { class: 'demo-row__label' }, '12 小时制'), (() => { const tp = new TimePicker({ placeholder: '选择时间', use12Hours: true }); this.registerChild(tp); return tp.render(); })()), h('div', { class: 'demo-row' }, h('span', { class: 'demo-row__label' }, '步长'), (() => { const tp = new TimePicker({ placeholder: '选择时间', hourStep: 2, minuteStep: 5 }); this.registerChild(tp); return tp.render(); })())),
            // Cascader
            h(Card, { title: 'Cascader 级联选择' }, h('div', { class: 'demo-row' }, h('span', { class: 'demo-row__label' }, '基本'), (() => {
            ...
            // TreeSelect
            h(Card, { title: 'TreeSelect 树选择' }, h('div', { class: 'demo-row' }, h('span', { class: 'demo-row__label' }, '基本'), (() => {
            ...
            // AutoComplete
            h(Card, { title: 'AutoComplete 自动补全' }, h('div', { class: 'demo-row' }, h('span', { class: 'demo-row__label' }, '基本'), (() => {
            ...
            // Mentions
            h(Card, { title: 'Mentions 提及' }, h('div', { class: 'demo-row' }, h('span', { class: 'demo-row__label' }, '基本'), (() => {
            ...
            // ColorPicker
            h(Card, { title: 'ColorPicker 颜色选择器' }, h('div', { class: 'demo-row' }, h('span', { class: 'demo-row__label' }, '基本'), (() => { const c = new ColorPicker({}); this.registerChild(c); return c.render(); })()), h('div', { class: 'demo-row' }, h('span', { class: 'demo-row__label' }, '显示文字'), (() => { const c = new ColorPicker({ showText: true, defaultValue: '#52c41a' }); this.registerChild(c); return c.render(); })()), h('div', { class: 'demo-row' }, h('span', { class: 'demo-row__label' }, '预设色'), (() => {
            ...
            // InputNumber
            h(Card, { title: 'InputNumber 数字输入框' }, h('div', { style: 'display:flex; gap:16px; flex-wrap:wrap; align-items:center;' }, (() => {
                const c = new InputNumber({ min: 0, max: 100, defaultValue: 5, size: 'middle',
                    onChange: (v) => message.info(`值变更：${v}`) });
                this.registerChild(c);
                return c.render();
            })(), (() => {
                ...
                const c = new InputNumber({ min: 0, max: 1000, defaultValue: 100, step: 10,
                    ...
                    parser: (s) => s.replace(/¥\s?|(,*)/g, '') });
                this.registerChild(c);
                return c.render();
            })()), h('p', { style: 'margin-top:12px; color:var(--color-text-tertiary); font-size:13px;' }, '提示：键盘 ↑/↓ 调整数值，Enter 提交。')),
        ];
    }
}
//# sourceMappingURL=FormComponentsPage.js.map