...
export class FloatButton extends Component {
    render() {
        const { icon, type = 'default', shape = 'circle', size = 40, tooltip, placement = 'right', href, target, onClick, badge, fixed = true, } = this.props;
        const classes = [
            'float-button',
            `float-button--${type}`,
            `float-button--${shape}`,
        ...
        const triggerEl = h(href ? 'a' : 'button', {
            ...
            onClick: (e) => onClick?.(e),
        }, h('span', { class: 'float-button__icon' }, icon), badgeEl);
