...
export class InputNumber extends Component {
    ...
    render() {
        const { disabled = false, size = 'middle', controls = true, placeholder = '', autoFocus = false, } = this.props;
        ...
        return h('div', { class: classes }, controls && h('button', {
            type: 'button',
            class: 'input-number__handler input-number__handler--down',
            tabindex: '-1',
            'aria-label': '减小',
            disabled,
            onClick: (e) => { e.preventDefault(); if (!disabled)
                this._stepBy(-1); },
        }, '−'), input, controls && h('button', {
            type: 'button',
