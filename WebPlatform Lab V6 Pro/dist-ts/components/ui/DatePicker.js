...
export class DatePicker extends Component {
    ...
    _renderPanel() {
        ...
        return h('div', { class: 'datepicker__panel', role: 'dialog', onClick: (e) => e.stopPropagation() }, h('div', { class: 'datepicker__header' }, h('button', { type: 'button', class: 'datepicker__btn', onClick: () => this._changeYear(-1) }, '«'), h('button', { type: 'button', class: 'datepicker__btn', onClick: () => this._changeMonth(-1) }, '‹'), h('span', { class: 'datepicker__title' }, `${year}年 ${MONTH_NAMES[month]}`), h('button', { type: 'button', class: 'datepicker__btn', onClick: () => this._changeMonth(1) }, '›'), h('button', { type: 'button', class: 'datepicker__btn', onClick: () => this._changeYear(1) }, '»')), h('div', { class: 'datepicker__weekdays' }, ...WEEK_DAYS.map((w) => h('div', { class: 'datepicker__weekday' }, w))), h('div', { class: 'datepicker__cells' }, ...cells.map((cell) => {
        ...
        })), this.props.showToday && h('div', { class: 'datepicker__footer' }, h('button', { type: 'button', class: 'datepicker__today-btn', onClick: () => this._goToday() }, '今天')));
    }
    render() {
        ...
        }, h('span', { class: text ? '' : 'text-tertiary' }, text || placeholder), h('span', { class: 'datepicker__icon', 'aria-hidden': 'true' }, '📅'), allowClear && text && h('span', {
        ...
        if (this.state.open && !disabled)
            root.appendChild(this._renderPanel());
        return root;
    }
...
export class RangePicker extends Component {
    ...
    _openPortal() {
        ...
        this._portalRAF = requestAnimationFrame(() => {
            ...
            const trigger = this.el.querySelector('.rangepicker__trigger')
                || this.el.querySelector('.datepicker__trigger');
            if (panel && trigger) {
                this._dropdownEl = panel;
                this._portalCleanup = mountDropdown(panel, trigger);
            }
    ...
    _renderPanel() {
        ...
        return h('div', { class: 'rangepicker__panel', role: 'dialog', onClick: (e) => e.stopPropagation() }, h('div', { class: 'datepicker__header' }, h('button', { type: 'button', class: 'datepicker__btn', onClick: () => this._changeMonth(-1) }, '‹'), h('span', { class: 'datepicker__title' }, `${year}年 ${MONTH_NAMES[month]}`), h('button', { type: 'button', class: 'datepicker__btn', onClick: () => this._changeMonth(1) }, '›')), h('div', { class: 'datepicker__weekdays' }, ...WEEK_DAYS.map((w) => h('div', { class: 'datepicker__weekday' }, w))), h('div', { class: 'datepicker__cells' }, ...cells.map((cell) => {
            const k = dateKey(cell.date);
    ...
    render() {
        ...
        const root = h('div', { class: `rangepicker ${this.state.open ? 'is-open' : ''} ${disabled ? 'is-disabled' : ''}` }, h('div', {
        ...
        }, h('span', { class: startText ? '' : 'text-tertiary' }, startText || ph[0]), h('span', { class: 'rangepicker__separator' }, '~'), h('span', { class: endText ? '' : 'text-tertiary' }, endText || ph[1]), allowClear && (startText || endText) && h('span', {
