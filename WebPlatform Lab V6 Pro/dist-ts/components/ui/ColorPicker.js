...
export class ColorPicker extends Component {
    ...
    componentWillUnmount() {
        ...
        this._closePortal();
        if (this._satRAF) {
            cancelAnimationFrame(this._satRAF);
            this._satRAF = null;
        }
        if (this._hueRAF) {
            cancelAnimationFrame(this._hueRAF);
            this._hueRAF = null;
        }
    }
    _setOpen(open) {
    ...
    _renderSaturation(hsv) {
        ...
        const onMove = (e) => { if (this._satDragging)
            handlePointer(e); };
        const onUp = () => {
    ...
    _renderHue(hsv) {
        ...
        const onMove = (e) => { if (this._hueDragging)
            handlePointer(e); };
        const onUp = () => {
    ...
    _renderPanel() {
        ...
        return h('div', { class: 'colorpicker__panel', role: 'dialog', onClick: (e) => e.stopPropagation() }, this._renderSaturation(hsv), this._renderHue(hsv), h('div', { class: 'colorpicker__inputs' }, h('input', {
            type: 'text',
            class: 'colorpicker__hex-input',
            value: this.state.hex,
            onChange: (e) => {
                const v = e.target.value;
                if (/^#([0-9a-f]{6}|[0-9a-f]{3})$/i.test(v) && v.toLowerCase() !== this.state.hex.toLowerCase())
                    this._setHex(v);
            },
        }), showText && h('span', { class: 'colorpicker__display' }, display), h('input', {
            type: 'color',
            class: 'colorpicker__native',
            value: this.state.hex,
            onInput: (e) => {
                const v = e.target.value;
                if (v.toLowerCase() !== this.state.hex.toLowerCase())
                    this._setHex(v);
            },
        })), presets.length > 0 && h('div', { class: 'colorpicker__presets' }, ...presets.map((group) => h('div', { class: 'colorpicker__preset-group' }, group.label && h('div', { class: 'colorpicker__preset-label' }, group.label), h('div', { class: 'colorpicker__preset-colors' }, ...(group.colors || []).map((c) => h('button', {
            type: 'button',
    ...
    render() {
        ...
        const display = formatColor(this.state.hex, format);
        const root = h('div', { class: `colorpicker ${this.state.open ? 'is-open' : ''} ${disabled ? 'is-disabled' : ''}` }, h('div', {
        ...
        }, h('span', { class: 'colorpicker__swatch', style: { background: this.state.hex } }), showText && h('span', { class: 'colorpicker__trigger-text' }, display)));
