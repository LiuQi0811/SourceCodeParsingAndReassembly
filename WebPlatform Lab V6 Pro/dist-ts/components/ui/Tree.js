...
export class Tree extends Component {
    initialState() {
        const { selectedKeys = [], expandedKeys, checkedKeys = [], defaultExpandAll = false, treeData = [], } = this.props;
    ...
    _findPath(nodes, key, path = []) {
        for (const n of nodes) {
            ...
            if (n.key === key)
                return path;
            if (n.children?.length && this._findPath(n.children, key, path))
                return path;
            path.pop();
        }
        return null;
    ...
    render() {
        const { treeData = [], checkable = false, selectable = true, draggable = false, showLine = false, blockNode = false, } = this.props;
        const classes = [
            'tree',
            showLine && 'tree--line',
            blockNode && 'tree--block',
        ].filter(Boolean).join(' ');
        return h('div', { class: classes }, ...(treeData.map((node) => this._renderNode(node, 0))));
    }
    /** 递归渲染单个节点 */
    _renderNode(node, level) {
        ...
        const title = h('span', titleProps, node.icon && h('span', { class: 'tree__icon' }, node.icon), h('span', { class: 'tree__label' }, node.title));
        return h('div', { class: 'tree__node', 'data-key': node.key }, h('div', { class: 'tree__node-content' }, ...indents, switcher, checkbox, title), hasChildren && expanded && h('div', { class: 'tree__children' }, ...node.children.map((c) => this._renderNode(c, level + 1))));
    }
    /** 外部调用：展开/收起 */
