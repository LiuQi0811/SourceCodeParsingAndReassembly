...
export class Cascader extends Component {
    ...
    _resolvePath(values, fn) {
        ...
        for (const v of values) {
            ...
            if (!found)
                break;
            path.push(found);
            if (!found[fn.children] || !found[fn.children].length)
                break;
    ...
    _renderColumn(options, level) {
        ...
        return h('div', { class: 'cascader__column' }, ...(options || []).map((opt) => {
            ...
            return h('div', { class: classes, ...handlers }, h('span', { class: 'cascader__option-label' }, opt[fn.label]), hasChildren && h('span', { class: 'cascader__option-arrow', 'aria-hidden': 'true' }, '›'));
        }));
    }
    _renderPanel() {
        const fn = this.state.fieldNames;
        ...
        return h('div', { class: 'cascader__panel', role: 'dialog', onClick: (e) => e.stopPropagation() }, ...columns);
    }
    render() {
        ...
        const text = this.state.activePath.length
            ...
            : '';
        const root = h('div', { class: `cascader ${this.state.open ? 'is-open' : ''} ${disabled ? 'is-disabled' : ''}` }, h('div', {
        ...
        }, h('span', { class: text ? '' : 'text-tertiary' }, text || placeholder), h('span', { class: 'cascader__arrow', 'aria-hidden': 'true' }, '▾'), allowClear && text && h('span', {
