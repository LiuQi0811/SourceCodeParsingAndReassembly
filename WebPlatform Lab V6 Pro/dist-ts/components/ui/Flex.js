...
function resolveGap(gap) {
    ...
    if (typeof gap === 'number')
        return `${gap}px`;
    if (typeof gap === 'string' && GAP_SIZE_MAP[gap] != null)
        return `${GAP_SIZE_MAP[gap]}px`;
    return String(gap);
}
/** 弹性布局：支持 vertical/wrap/justify/align/gap，子项用 .flex__item 包裹 */
export class Flex extends Component {
    render() {
        const { vertical = false, wrap = false, justify, align, gap, className = '', children, } = this.props;
        const classes = [
            'flex',
        ...
        if (justify)
            style.justifyContent = JUSTIFY_MAP[justify] || justify;
        if (align)
        ...
        const kids = (Array.isArray(children) ? children : [children])
            ...
            if (child && typeof child === 'object' && child.__flexItem) {
                const marker = child;
                const itemStyle = marker.flex != null ? { flex: String(marker.flex) } : null;
