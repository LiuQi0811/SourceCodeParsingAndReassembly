...
export class Mentions extends Component {
    ...
    _onInput(e) {
        ...
        if (mention) {
            ...
            const filtered = opts.filter((o) => (o.label || '').toLowerCase().includes(mention.query.toLowerCase()));
            if (filtered.length) {
    ...
    _renderDropdown() {
        if (!this.state.active)
            return null;
        const opts = this.state.active.options;
        return h('div', { class: 'mentions__dropdown', role: 'listbox', onClick: (e) => e.stopPropagation() }, ...opts.map((opt, i) => h('div', {
