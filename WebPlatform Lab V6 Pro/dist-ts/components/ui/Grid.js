...
export class Row extends Component {
    render() {
        const { gutter = 0, justify = 'start', align = 'top', wrap = true, className = '', children, } = this.props;
...
function parseResponsive(value) {
    ...
    if (typeof value === 'number')
        return { span: value, offset: 0, push: 0, pull: 0 };
...
export class Col extends Component {
    render() {
        const { span, offset = 0, push = 0, pull = 0, className = '', children, ...rest } = this.props;
        ...
        if (push) {
            ...
            style.left = `${(push / 24) * 100}%`;
        }
        if (pull) {
            style.position = 'relative';
            style.right = `${(pull / 24) * 100}%`;
        }
        const col = h('div', { class: classes, style }, ...(Array.isArray(children) ? children : [children]));
        ...
        for (const bp of BREAKPOINTS) {
            ...
            if (!parsed)
                continue;
            if (parsed.span != null)
                col.style.setProperty(`--col-${bp}-span`, String(parsed.span));
            if (parsed.offset)
                col.style.setProperty(`--col-${bp}-offset`, String(parsed.offset));
            if (parsed.span != null) {
