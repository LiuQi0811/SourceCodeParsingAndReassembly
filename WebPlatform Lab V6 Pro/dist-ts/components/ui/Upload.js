...
export class Upload extends Component {
    ...
    render() {
        const { accept = '', multiple = false, maxCount = 0, action = null, listType = 'text', children = null, } = this.props;
        ...
        const triggerWrapper = h('div', {
            ...
            onClick: () => { if (this._input)
                this._input.click(); },
        }, trigger);
        // 文件列表
        const list = fileList.length > 0
            ? h('div', { class: `upload__list upload__list--${listType}` }, ...fileList.map((file, index) => this._renderFile(file, index)))
            : null;
        return h('div', {
    ...
    _renderFile(file, index) {
        ...
        const thumb = isPicture
            ...
            : null;
        return h('div', { class: ['upload__file', `upload__file--${file.status}`] }, thumb, h('div', { class: 'upload__file-info' }, h('div', { class: 'upload__file-name' }, file.name), h('div', { class: 'upload__file-meta' }, file.size != null && h('span', { class: 'upload__file-size' }, this._formatSize(file.size)), h('span', { class: `upload__file-status upload__file-status--${file.status}` }, this._statusText(file.status))), file.status === 'uploading' && h('div', { class: 'upload__file-progress' }, h('div', {
    ...
    _statusText(status) {
        const map = { uploading: '上传中', done: '已完成', error: '失败' };
        return map[status] || '';
    }
    _formatSize(bytes) {
        if (bytes == null)
    ...
    _handleFiles(fileList) {
        ...
        for (const file of files) {
            // 超出最大数量限制
            if (maxCount > 0 && nextList.length >= maxCount)
                break;
