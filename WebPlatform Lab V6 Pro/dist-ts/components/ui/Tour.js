...
export class Tour extends Component {
    ...
    _portalRAF = null;
    _portalCleanup = null;
    initialState() { return { current: this.props.current ?? 0 }; }
    componentDidMount() {
        // 键盘支持：Esc 关闭、← 上一步、→ 下一步
    ...
    _openPortal() {
        ...
        this._portalRAF = requestAnimationFrame(() => {
            if (!this.props.open || !this._card || !this._card.isConnected)
                return;
    ...
    _positionCard(cardEl, targetEl, placement) {
        if (!cardEl)
            return;
        if (!targetEl) {
    ...
    render() {
        ...
        }, step.cover && h('div', { class: 'tour__cover' }, step.cover), h('div', { class: 'tour__head' }, step.title && h('div', { class: 'tour__title' }, step.title), h('button', {
            type: 'button',
            class: 'tour__close',
            'aria-label': '关闭引导',
            onClick: () => this._handleClose(),
        }, '×')), step.description && h('div', { class: 'tour__description' }, step.description), h('div', { class: 'tour__foot' }, 
        // 指示器：圆点序列，激活项高亮
        h('div', { class: 'tour__indicator' }, ...steps.map((_, i) => h('span', {
            class: ['tour__dot', i === idx && 'tour__dot--active'].filter(Boolean).join(' '),
        }))), h('div', { class: 'tour__buttons' }, idx > 0 && h('button', {
            type: 'button',
            class: 'btn',
