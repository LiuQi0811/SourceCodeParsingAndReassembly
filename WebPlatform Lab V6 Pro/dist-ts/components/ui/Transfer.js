...
export class Transfer extends Component {
    ...
    _filter(input, item) {
        if (!this.props.showSearch || !input)
            return true;
    ...
    _toggle(side, key, checked) {
        ...
        this.props.onSelectChange?.(Array.from(this.state.sourceSelected), Array.from(this.state.targetSelected));
    }
    ...
    _renderItem(item, side) {
        ...
        return h('li', {
            ...
            onClick: () => { if (!item.disabled)
                this._toggle(side, item.key, !selected); },
        }, h('span', {
            class: ['transfer__checkbox', selected && 'is-checked', item.disabled && 'is-disabled'].filter(Boolean).join(' '),
        }), h('span', { class: 'transfer__item-content' }, custom != null ? custom : h('span', { class: 'transfer__item-label' }, h('span', { class: 'transfer__item-title' }, item.title), item.description && h('span', { class: 'transfer__item-desc' }, item.description))));
    }
    _renderList(side, data, search) {
        ...
        return h('div', { class: 'transfer__list', style: listStyle || null }, h('div', { class: 'transfer__list-header' }, h('span', { class: 'transfer__checkbox',
            onClick: () => {
                const setKey = side === 'source' ? 'sourceSelected' : 'targetSelected';
        ...
        }), h('span', { class: 'transfer__list-title' }, `${selectedKeys(selectedCount, data.filter((i) => !i.disabled).length)} ${title}`)), showSearch && h('div', { class: 'transfer__search' }, h('input', {
        ...
        })), h('ul', { class: 'transfer__list-body' }, ...data.map((item) => this._renderItem(item, side)), data.length === 0 && h('li', { class: 'transfer__empty' }, '无数据')));
    }
    render() {
        ...
        return h('div', { class: 'transfer' }, this._renderList('source', sourceData, this.state.sourceSearch), h('div', { class: 'transfer__operation' }, h('button', {
            type: 'button',
        ...
        }, ops[1])), this._renderList('target', targetData, this.state.targetSearch));
    }
