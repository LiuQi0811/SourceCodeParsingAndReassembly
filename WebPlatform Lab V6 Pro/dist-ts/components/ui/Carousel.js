...
export class Carousel extends Component {
    ...
    componentDidMount() {
        ...
        if (this.props.pauseOnHover && this.el) {
            ...
            this.on(this.el, 'mouseleave', () => { if (this.props.autoplay)
                this._startAutoplay(); });
        }
    }
    ...
    goTo(slideIndex) {
        ...
        // 动画进行中且要求等待：跳过
        if (waitForAnimate && this.state.animating)
            return;
        let next;
        if (infinite) {
    ...
    render() {
        const { dots = true, dotPosition = 'bottom', effect = 'scrollx', slidesToShow = 1, easing = 'linear', } = this.props;
        const slides = this._getSlides();
        const total = slides.length;
        ...
        const trackEl = h('div', { class: `carousel__track carousel__track--${effect}`, style: trackStyle }, ...slides.map((slide, i) => {
        ...
        // 圆点导航
        const dotsEl = dots && total > 0 ? h('ul', { class: `carousel__dots carousel__dots--${dotPosition}` }, ...slides.map((_, i) => h('li', {
            class: ['carousel__dot', i === current && 'carousel__dot--active'].filter(Boolean).join(' '),
            onClick: () => this.goTo(i),
        }))) : null;
        return h('div', { class: 'carousel' }, h('div', { class: 'carousel__viewport' }, trackEl), dotsEl);
    }
}
