...
export class Anchor extends Component {
    ...
    _handleScroll() {
        ...
        if (active === this.state.activeKey)
            return;
    ...
    _flattenItems(items, depth = 0, result = []) {
        for (const item of items) {
            ...
            if (item.children?.length)
                this._flattenItems(item.children, depth + 1, result);
        }
        return result;
    }
    _renderLink(item) {
        return h('div', { class: 'anchor__link', style: { paddingLeft: `${item.depth * 16 + 8}px` } }, h('a', {
    ...
    render() {
        ...
        return h('div', { class: ['anchor', affix && 'anchor--affix'].filter(Boolean).join(' ') }, h('div', { class: 'anchor__list' }, h('span', { class: 'anchor__ink' }), ...flat.map((item) => this._renderLink(item))));
    }
}
