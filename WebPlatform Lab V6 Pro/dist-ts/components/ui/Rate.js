...
export class Rate extends Component {
    ...
    render() {
        const { count = 5, allowHalf = false, disabled = false, character = '★', } = this.props;
        ...
        for (let i = 1; i <= count; i++) {
            ...
            if (isFull)
                classes.push('rate__star--full');
            else if (isHalf)
                classes.push('rate__star--half');
            else
                classes.push('rate__star--empty');
            stars.push(h('span', {
                class: classes,
        ...
        return h('div', {
            ...
            onMousemove: (e) => {
                ...
                if (next === 0)
                    return;
                if (next !== this.state.hoverValue)
                    this.setState({ hoverValue: next });
            },
            onMouseleave: () => {
                if (disabled)
                    return;
