...
export class Table extends Component {
    ...
    render() {
        ...
        const pagedData = pagination
            ...
            : data;
        return h('div', { class: 'table' }, h('table', {}, h('thead', {}, h('tr', {}, ...columns.map((col) => {
            ...
            }, h('span', {}, col.title), sortable && h('span', { class: 'sort-icon' }, isAsc ? '▲' : isDesc ? '▼' : '⇅'));
        }))), h('tbody', {}, ...pagedData.map((row) => h('tr', {
            key: row[rowKey],
    ...
    _renderPagination(current, total, totalItems) {
        ...
        if (start > 1) {
            pages.push(btn(1, false, () => this.setState({ page: 1 })));
            if (start > 2)
                pages.push(h('span', {}, '…'));
        }
        for (let i = start; i <= end; i++)
            pages.push(btn(i, false, () => this.setState({ page: i })));
        if (end < total) {
            if (end < total - 1)
                pages.push(h('span', {}, '…'));
            pages.push(btn(total, false, () => this.setState({ page: total })));
        }
        return h('div', { class: 'flex items-center justify-between p-md' }, h('span', { class: 'fs-sm text-tertiary' }, `共 ${totalItems} 条`), h('div', { class: 'flex items-center gap-xs' }, btn('‹', current === 1, () => this.setState({ page: current - 1 })), ...pages, btn('›', current === total, () => this.setState({ page: current + 1 }))));
    }
