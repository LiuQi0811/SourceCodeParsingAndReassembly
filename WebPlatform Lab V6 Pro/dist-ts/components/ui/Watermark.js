...
export class Watermark extends Component {
    /** 生成单个水印单元的 dataURL */
    _makeWatermarkUrl(width, height) {
        const { content = '', rotate = -22, font = {}, } = this.props;
        const { color = 'rgba(0,0,0,0.15)', fontSize = 14, fontFamily = 'sans-serif', fontStyle = 'normal', fontWeight = 'normal', } = font;
        const lines = Array.isArray(content) ? content.filter(Boolean) : (content ? [content] : []);
        if (lines.length === 0)
            return null;
    ...
    render() {
        const { content = '', gap = [100, 100], offset = [], width = 120, height = 64, z = 9, inherit = true, children, } = this.props;
        ...
        }, ...kids, h('div', { class: 'watermark__layer', style: layerStyle, 'aria-hidden': 'true' }));
    }
}
