...
function formatTime(t, format = 'HH:mm:ss', use12Hours = false) {
    if (!t)
        return '';
    let h = t.h;
    const suffix = use12Hours ? (h < 12 ? ' AM' : ' PM') : '';
    if (use12Hours) {
        h = h % 12;
        if (h === 0)
            h = 12;
...
export class TimePicker extends Component {
    ...
    _renderColumn(part, max, label) {
        ...
        return h('div', { class: 'timepicker__column' }, h('div', { class: 'timepicker__column-label' }, label), h('div', { class: 'timepicker__column-list' }, ...items));
    }
    _renderPanel() {
        const use12Hours = this.props.use12Hours;
        const hMax = use12Hours ? 12 : 24;
        return h('div', { class: 'timepicker__panel', role: 'dialog', onClick: (e) => e.stopPropagation() }, h('div', { class: 'timepicker__columns' }, this._renderColumn('h', hMax, '时'), this._renderColumn('m', 60, '分'), this._renderColumn('s', 60, '秒')), h('div', { class: 'timepicker__footer' }, h('button', {
        ...
        }, '此刻'), h('button', { type: 'button', class: 'timepicker__btn timepicker__btn--ok', onClick: () => this._confirm() }, '确定')));
    }
    render() {
        ...
        const root = h('div', { class: `timepicker ${this.state.open ? 'is-open' : ''} ${disabled ? 'is-disabled' : ''}` }, h('div', {
        ...
        }, h('span', { class: text ? '' : 'text-tertiary' }, text || placeholder), h('span', { class: 'timepicker__icon', 'aria-hidden': 'true' }, '⏰'), allowClear && text && h('span', {
