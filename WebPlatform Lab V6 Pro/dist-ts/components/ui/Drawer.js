...
export class Drawer extends Component {
    ...
    render() {
        ...
        return h('div', {}, h('div', { class: 'drawer-mask', onClick: () => { if (maskClosable)
                this.close(); } }), h('div', { class: `drawer ${placementClass}`, role: 'dialog', 'aria-modal': 'true',
            'aria-label': typeof title === 'string' ? title : undefined,
            style: { width: `${width}px` } }, h('div', { class: 'drawer__head' }, h('span', {}, title), h('button', { type: 'button', class: 'modal__close', 'aria-label': '关闭',
            onClick: () => this.close() }, '×')), h('div', { class: 'drawer__body' }, children)));
    }
    componentDidMount() {
        if (this.state.open) {
            _lockBody();
    ...
    _bindEsc() {
        if (this._escHandler)
            return;
        this._escHandler = (e) => { if (e.key === 'Escape' && this.state.open)
            this.close(); };
    ...
    _installTrap() {
        ...
        this._teardownTrap();
        const el = this.el;
        const dialogEl = (el?.querySelector('[role="dialog"]') || el);
