...
export class TreeSelect extends Component {
    ...
    _collectKeys(nodes, acc = []) {
        for (const n of nodes) {
            ...
            if (n.children?.length)
                this._collectKeys(n.children, acc);
        }
        return acc;
    }
    /** 在树中查找指定 value 的节点 */
    ...
    _findPath(nodes, value, path = []) {
        for (const n of nodes) {
            ...
            if (n.value === value)
                return path;
            if (n.children?.length && this._findPath(n.children, value, path))
                return path;
            path.pop();
        }
        return null;
    ...
    _renderText() {
        ...
        if (this.state.value == null)
            return '';
        return String(this._findNode(data, this.state.value)?.title || '');
    }
    _clear(e) {
        e.stopPropagation();
    ...
    _renderNode(node, level) {
        ...
        for (let i = 0; i < level; i++)
            indents.push(h('span', { class: 'treeselect__indent' }));
        const switcher = h('span', {
            class: ['treeselect__switcher', isLeaf ? 'is-leaf' : (expanded ? 'is-open' : 'is-close')].join(' '),
            onClick: (e) => { e.stopPropagation(); if (!isLeaf)
                this._toggleExpand(node.value); },
        }, isLeaf ? '' : (expanded ? '▼' : '▶'));
        ...
        return h('div', { class: 'treeselect__node', 'data-value': node.value }, h('div', { class: 'treeselect__node-content' }, ...indents, switcher, checkbox, title), hasChildren && expanded && h('div', { class: 'treeselect__children' }, ...node.children.map((c) => this._renderNode(c, level + 1))));
    }
    _renderPanel() {
        return h('div', { class: 'treeselect__panel', role: 'dialog', onClick: (e) => e.stopPropagation() }, ...(this.props.treeData || []).map((n) => this._renderNode(n, 0)));
    }
    render() {
        const { placeholder = '请选择', disabled = false, allowClear = false } = this.props;
        const text = this._renderText();
        const root = h('div', { class: `treeselect ${this.state.open ? 'is-open' : ''} ${disabled ? 'is-disabled' : ''}` }, h('div', {
        ...
        }, h('span', { class: text ? '' : 'text-tertiary' }, text || placeholder), h('span', { class: 'treeselect__arrow', 'aria-hidden': 'true' }, '▾'), allowClear && text && h('span', {
