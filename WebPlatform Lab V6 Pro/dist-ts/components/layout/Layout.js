...
export class Layout extends Component {
    ...
    render() {
        ...
        }, '跳到主要内容'), header.render(), h('div', { class: 'app-body' }, sidebar.render(), h('main', { class: 'app-content', id: 'main-content' }, h('div', { class: 'app-content__inner' }, this._breadcrumbEl, 
    ...
    _renderBreadcrumb(crumbs) {
        ...
        crumbs.forEach((item, i, arr) => {
            ...
            el.appendChild(h('span', {
                ...
                onClick: () => { if (!last && item.path)
                    this.props.router?.push(item.path); },
            }, item.label));
            if (!last)
                el.appendChild(h('span', { class: 'app-breadcrumb__sep' }, '/'));
        });
    }
    _buildBreadcrumb(to) {
