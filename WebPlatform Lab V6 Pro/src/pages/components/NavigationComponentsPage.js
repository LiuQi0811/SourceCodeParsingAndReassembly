// NavigationComponentsPage.js —— 导航与交互组件：Breadcrumb / Tooltip / Popover / Collapse / Dropdown / Pagination / Steps / Timeline / Rate / Slider / Segmented / Anchor / PageHeader / Affix / FloatButton
...
export class NavigationComponentsPage extends Page {
  ...
  // collapseActive 用实例字段而非 state：
  // 切换面板时若走 setState 会触发整页 _rerender，
  // h(Collapse,...) 每次 render 都 new Collapse() 创建全新实例+DOM，
  // 正在交互的面板被销毁重建 → 闪屏。改为实例字段 + Collapse 内部直接操作 DOM。

  // sliderValue 用实例字段而非 state：
  // 拖动 Slider 时若走 setState 会触发整页 _rerender，
  // h(Slider,...) 每次 render 都 new Slider() 创建全新实例+DOM，
  // 正在被拖动的 input 被销毁重建，浏览器原生拖拽状态丢失 → 闪屏/断掉。
  // 改为实例字段 + 直接 DOM 更新，拖动完全不触发 rerender。
  ...
  renderPage() {
    ...
    const tooltipCard = new Card({
      ...
      children: [
        h('div', { class: 'demo-row' },
          h(Tooltip, { title: '上方提示文字', placement: 'top' }, this._btn('上', {})),
          h(Tooltip, { title: '下方提示', placement: 'bottom' }, this._btn('下', {})),
          h(Tooltip, { title: '左侧提示', placement: 'left' }, this._btn('左', {})),
          h(Tooltip, { title: '右侧提示', placement: 'right' }, this._btn('右', {})),
        ),
      ],
    });
    ...
    // —— Collapse ——
    // 惰性初始化实例字段（首次 render 时）
    if (this._collapseActive == null) this._collapseActive = ['1'];
    const collapseCard = new Card({
      title: 'Collapse 折叠面板',
      children: [
        h(Collapse, {
          activeKey: this._collapseActive,
          onChange: (keys) => { this._collapseActive = keys; },
          items: [
    ...
    const stepsCard = new Card({
      ...
      children: [
        ...
        h('div', { class: 'flex gap-sm mt-md' },
          this._btn('上一步', { disabled: this.state.currentStep === 0, onClick: () => this.setState({ currentStep: Math.max(0, this.state.currentStep - 1) }) }),
          this._btn('下一步', { type: 'primary', disabled: this.state.currentStep >= 3, onClick: () => this.setState({ currentStep: Math.min(3, this.state.currentStep + 1) }) }),
        ),
      ],
    });
    ...
    // —— Rate ——
    // rateValue 用实例字段，onChange 不触发 setState（避免整页 rerender 重建 Rate → 闪屏）
    if (this._rateValue == null) this._rateValue = 3;
    const rateValue = this._rateValue;
    const rateCard = new Card({
      title: 'Rate 评分',
      children: [
        h('div', { class: 'demo-row' },
          h('span', { class: 'demo-row__label' }, '基本'),
          h(Rate, { value: rateValue, allowHalf: true, onChange: (v) => {
            this._rateValue = v;
            this.$$('.js-rate-value').forEach((el) => { el.textContent = `${v} 星`; });
            message.info(`评分：${v} 星`);
          } }),
          h('span', { class: 'ml-sm text-secondary js-rate-value' }, `${rateValue} 星`),
        ),
        h('div', { class: 'demo-row' },
          h('span', { class: 'demo-row__label' }, '禁用'),
          h(Rate, { value: 4, disabled: true }),
        ),
        h('div', { class: 'demo-row' },
          h('span', { class: 'demo-row__label' }, '自定义字符'),
          h(Rate, { value: 3, character: '👍' }),
        ),
      ],
    });
    ...
    // —— Slider ——
    // 惰性初始化实例字段（首次 render 时）
    if (this._sliderValue == null) this._sliderValue = 30;
    const sliderValue = this._sliderValue;
    // onChange 直接操作 DOM，不触发 setState：
    // 更新显示文本 + 联动另一个 Slider 的 range/number input
    const onSliderChange = (v) => {
      this._sliderValue = v;
      this.$$('.js-slider-value').forEach((el) => { el.textContent = `当前值：${v}`; });
      this.$$('.slider__input').forEach((el) => { if (Number(el.value) !== v) el.value = v; });
      this.$$('.slider__number-input').forEach((el) => { if (Number(el.value) !== v) el.value = v; });
    };
    const sliderCard = new Card({
      title: 'Slider 滑动输入条',
      children: [
        h('div', { class: 'mb-md' },
          h(Slider, { value: sliderValue, min: 0, max: 100, onChange: onSliderChange }),
        ),
        h('div', { class: 'mb-md' },
          h(Slider, { value: sliderValue, showInput: true, onChange: onSliderChange }),
        ),
        h('div', { class: 'text-secondary fs-sm js-slider-value' }, `当前值：${sliderValue}`),
      ],
    });
    this.registerChild(sliderCard);

    // —— Segmented ——
    // segmentedValue 用实例字段，onChange 不触发 setState
    if (this._segmentedValue == null) this._segmentedValue = 'a';
    const segmentedValue = this._segmentedValue;
      ...
      children: [
        h('div', { class: 'demo-row' },
          h(Segmented, {
            ...
            onChange: (v) => {
              this._segmentedValue = v;
              this.$$('.js-segmented-value').forEach((el) => { el.textContent = `当前选择：${v}`; });
            },
            options: [
              { value: 'a', label: '选项 A' },
              { value: 'b', label: '选项 B' },
        ...
        h('div', { class: 'mt-sm text-secondary fs-sm js-segmented-value' }, `当前选择：${segmentedValue}`),
      ],
    });
    ...
    const anchorCard = new Card({
      ...
      children: [
        ...
        h('div', { class: 'demo-row' },
          h('span', { class: 'demo-row__label' }, '嵌套'),
          h(Anchor, {
            affix: false,
            items: [
    ...
    const floatButtonCard = new Card({
      ...
      children: [
        ...
        h('div', { class: 'demo-row', style: { minHeight: '60px', alignItems: 'center' } },
          h(FloatButton, { fixed: false, icon: '＋', type: 'primary', shape: 'circle', tooltip: '新增' }),
          h(FloatButton, { fixed: false, icon: '★', type: 'default', shape: 'circle', tooltip: '收藏' }),
          h(FloatButton, { fixed: false, icon: '?', type: 'primary', shape: 'circle', badge: { dot: true }, tooltip: '帮助' }),
        ),
      ],
    });
    ...
    const iconCard = new Card({
      ...
      children: [
        ...
        h('div', { style: 'display:flex; flex-wrap:wrap; gap:24px; font-size:24px; margin-top:12px;' },
          ...Icon.names.map((name) => h('div', {
            ...
            (() => { const c = new Icon({ name, size: 24, color: 'var(--color-primary)' }); this.registerChild(c); return c.render(); })(),
            h('span', { style: 'font-size:11px; color:var(--color-text-tertiary);' }, name),
          )),
        ),
        h('p', { class: 'text-secondary', style: 'margin-top:16px;' }, '旋转图标示例：'),
        h('div', { style: 'display:flex; gap:24px; font-size:24px; align-items:center;' },
          (() => { const c = new Icon({ name: 'reload', size: 24, color: 'var(--color-primary)' }); this.registerChild(c); return c.render(); })(),
          (() => { const c = new Icon({ name: 'reload', size: 32, color: '#52c41a', spin: true }); this.registerChild(c); return c.render(); })(),
          (() => { const c = new Icon({ name: 'setting', size: 28, color: '#faad14', spin: true }); this.registerChild(c); return c.render(); })(),
          (() => { const c = new Icon({ name: 'arrowLeft', size: 24, rotate: 180 }); this.registerChild(c); return c.render(); })(),
        ),
      ],
    });
