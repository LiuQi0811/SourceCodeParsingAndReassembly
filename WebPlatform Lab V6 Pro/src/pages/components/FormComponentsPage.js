...
export class FormComponentsPage extends Page {
  initialState() {
    return {
      ...
      radioChecked: true, // 保留兼容；实际用 _radioChecked 实例字段
    };
  }

  // radioChecked 用实例字段，onChange 不触发 setState（避免整页 rerender 重建 Radio → 闪屏）
      ...
      h(Card, { title: 'Radio 单选项' },
        h('div', { class: 'flex flex-col gap-md' },
          h('div', { class: 'demo-row' }, h('span', { class: 'demo-row__label' }, '基本'),
            (() => {
              if (this._radioChecked == null) this._radioChecked = true;
              const r = new Radio({
                label: '单独的 Radio', value: 'a', checked: this._radioChecked,
                onChange: () => { this._radioChecked = !this._radioChecked; },
              });
