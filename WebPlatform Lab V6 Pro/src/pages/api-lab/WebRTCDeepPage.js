...
export class WebRTCDeepPage extends Page {
  ...
  // 安全读取对象属性（防御性）
  _safeGet(obj, key) { try { return obj[key]; } catch { return undefined; } }
  ...
  _createPeerConnections() {
    ...
    try {
      ...
      this.setState({ pcLifecycle:
        ...
        '  new RTCPeerConnection({ iceServers, iceTransportPolicy, bundlePolicy, rtcpMuxPolicy })\n' +
  ...
  async _wireIceCandidates() {
    ...
    try {
      ...
      const tryAdd = async (pc, who) => {
        ...
        catch (err) { this._addLog('warn', `${who}.addIceCandidate(mockCandidate) 失败：${err.message}`); return false; }
      };
  ...
  async _demoMediaStream() {
    ...
    if (typeof pc1.getSenders === 'function') {
      try { const s = pc1.getSenders(); sendersCount = Array.isArray(s) ? s.length : '?'; lines.push(`   pc1.getSenders() → ${sendersCount} 个 RTCRtpSender`); } catch (err) { lines.push(`   pc1.getSenders() 失败：${err.message}`); }
    } else { lines.push('   pc1.getSenders 不可用（typeof undefined）'); }

    if (typeof pc1.getReceivers === 'function') {
      try { const r = pc1.getReceivers(); receiversCount = Array.isArray(r) ? r.length : '?'; lines.push(`   pc1.getReceivers() → ${receiversCount} 个 RTCRtpReceiver`); } catch (err) { lines.push(`   pc1.getReceivers() 失败：${err.message}`); }
    } else { lines.push('   pc1.getReceivers 不可用（typeof undefined）'); }

    if (addTrackOk && typeof pc1.removeTrack === 'function') {
      try { pc1.removeTrack(null); removeTrackOk = true; lines.push('4) pc1.removeTrack(sender) 已调用（演示 API 表面）'); } catch (err) { lines.push(`4) pc1.removeTrack 失败：${err.message}`); }
    ...
    lines.push('\nAPI 概览：\n  navigator.mediaDevices.getUserMedia({ video, audio }) → Promise<MediaStream>\n' +
      ...
      '  pc.ontrack = (e) => { e.track, e.streams, e.receiver, e.transceiver }\n' +
  ...
  _demoDataChannel() {
    ...
    try {
      ...
      try { dc.onmessage = (e) => { const text = typeof e.data !== 'string' ? `[binary ${(e.data?.byteLength ?? 0)} 字节]` : e.data; this._addLog('dc', `dc.onmessage 收到：${text}`); }; } catch { /* noop */ }
      try { dc.onclose = () => this._addLog('dc', `dc.onclose：通道「${dc.label}」已关闭 (readyState=${dc.readyState})`); } catch { /* noop */ }
      try { dc.onerror = (e) => this._addLog('warn', `dc.onerror：${e?.error?.message || 'unknown'}`); } catch { /* noop */ }
  ...
  async _demoStatsAndSession() {
    ...
    if (typeof pc1.getStats === 'function') {
      try {
        ...
        lines.push('\nRTCStats 类型分布：');
        for (const t of knownTypes) { const n = typeCounts[t] || 0; if (n > 0) lines.push(`  ${t}: ${n}`); }
        const others = Object.keys(typeCounts).filter((k) => !knownTypes.includes(k));
        if (others.length) lines.push(`  其它: ${others.map((k) => `${k}=${typeCounts[k]}`).join(', ')}`);
    ...
    if (typeof pc1.getConfiguration === 'function') {
      ...
      } catch (err) { lines.push(`   getConfiguration 失败：${err.message}`); }
    ...
    if (typeof pc1.setConfiguration === 'function') {
      ...
      } catch (err) { lines.push(`   setConfiguration 失败：${err.message}`); }
    ...
    if (typeof pc1.restartIce === 'function') {
      ...
      catch (err) { lines.push(`   restartIce 失败：${err.message}`); }
