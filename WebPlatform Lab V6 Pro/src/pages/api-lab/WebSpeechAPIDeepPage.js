...
navigator.permissions.query({ name: 'microphone' })
  .then(r => console.log('mic permission:', r.state));`)),
  ...
  _runSynthBasicDemo() {
    ...
    const info = [
      ...
      '【utterance 事件（8 个）】',
      '  u.onstart      = (e) => {};  // 朗读开始',
      '  u.onend        = (e) => {};  // 朗读结束（正常或被 cancel）',
      '  u.onerror      = (e) => {};  // 朗读出错，e.error 含错误类型',
      '  u.onpause      = (e) => {};  // 被 pause() 暂停',
      '  u.onresume     = (e) => {};  // 被 resume() 恢复',
      '  u.onmark        = (e) => {};  // SSML <mark> 标记触发，e.markName',
      '  u.onboundary    = (e) => {};  // 词/句边界触发',
      '  // e.charIndex   边界字符索引',
...
// synth.getVoices(); // 获取可用语音（首次可能为空）`)),
  ...
  _runVoiceDemo() {
    ...
    const info = [
      ...
      '  const jaJP = voices.filter(v => v.lang === "ja-JP");',
      '',
      ...
      '    const voices = window.speechSynthesis.getVoices();',
      '    return voices.find(v => v.voiceURI === uri) || voices.find(v => v.default);',
      '  }',
      '',
      '【浏览器支持】',
      `  speechSynthesis: ${f.speechSynthesis ? '✓' : '✗'}`,
...
function loadVoices() {
  ...
  const local = voices.filter(v => v.localService); // 本地无延迟
...
localStorage.setItem('preferred-voice', u.voice.voiceURI);`)),
  ...
  _runSynthControlDemo() {
    ...
    const info = [
      ...
      '  u.onmark = (e) => console.log("标记", e.markName);  // "here"',
      '  // <break time="1s"/> 插入停顿',
      '  // <emphasis> 强调',
      ...
      '  const male = voices.find(v => v.name.includes("David"));',
      '  const female = voices.find(v => v.name.includes("Huihui"));',
...
u.onmark = (e) => console.log('mark', e.markName);
synth.speak(u);`)),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
  ...
  _runRecogBasicDemo() {
    ...
    const info = [
      ...
      '',
      '【recognition 事件（8 个）】',
      '  recognition.onstart        = (e) => {};  // 识别开始',
      '  recognition.onend          = (e) => {};  // 识别结束（正常/停止/中止）',
      '  recognition.onerror        = (e) => {};  // 出错，e.error 含错误类型',
      '  recognition.onspeechstart  = (e) => {};  // 检测到语音开始',
      '  recognition.onspeechend    = (e) => {};  // 检测到语音结束',
      '  recognition.onsoundstart   = (e) => {};  // 检测到声音（含非语音）',
      '  recognition.onsoundend     = (e) => {};  // 声音结束',
      '  recognition.onresult       = (e) => {};  // 识别结果（核心事件）',
      '',
      ...
      '  };',
      '  rec.onerror = (e) => console.error("错误:", e.error);',
      '  rec.start();  // 需在用户手势中调用',
      '',
      '【安全上下文与权限】',
...
rec.onerror = (e) => console.error('错误:', e.error);
rec.onspeechstart = () => console.log('检测到语音');
rec.onresult = (e) => {
  const transcript = e.results[0][0].transcript;
...
// rec.abort(); // 立即中止`)),
  ...
  _runResultDemo() {
    ...
    const info = [
      ...
      '  sensitive.forEach(w => filtered = filtered.replace(new RegExp(w, "g"), "***"))',
      '',
      '  // 4. 去除首尾空格',
      '  transcript.trim();',
      '',
...
rec.start();`)),
  ...
  _runPatternDemo() {
    ...
    const info = [
      ...
      '  // rec.onresult = (e) => console.log(e.result.text);',
      '',
      '  // 优势：完全离线、隐私友好',
