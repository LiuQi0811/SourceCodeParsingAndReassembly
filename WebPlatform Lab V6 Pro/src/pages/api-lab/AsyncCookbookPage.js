...
// 宏任务：setTimeout / MessageChannel / I/O / UI 事件`)),
  ...
  async _demoAsyncBasics() {
    ...
    try {
      ...
      let caught = null;
      try { await this._delayReject(20, 'intended', 'p-bad'); } catch (err) { caught = err.message; }
  ...
  async _demoCustomAsyncIterator() {
    ...
    try {
      ...
      this.setState({
        asyncGenResult:
          `自定义异步迭代器类（实现 Symbol.asyncIterator 协议）：\n` +
          `class AsyncRange { [Symbol.asyncIterator]() { return this; } next() { return Promise<{value,done}> } }\n` +
    ...
    try {
      ...
      this.setState({
        syncGenResult:
          ...
          `function* dialog() { const a = yield 'Q1'; const b = yield 'Q2'; return ... }\n\n` +
          `逐步执行：\n${steps.join('\n')}\n\n` +
    ...
    try {
      ...
      this.setState({
        syncGenResult:
          ...
          `function* counter() { while(true) { try { const injected = yield i; ... } catch(e) { yield 'caught'; return ... } } }\n` +
          `执行序列：\n${steps.join('\n')}\n\n` +
