...
export class BuiltInAIPage extends Page {
  ...
  // 检测 Translator.availability 并说明状态枚举
  async _checkTranslator() {
    ...
    try {
      this._addLog('info', '检测 Translator.availability()…');
      const availability = await Translator.availability();
      this.setState({ translatorInfo:
        `Translator.availability() = ${JSON.stringify(availability)}\n\n` +
        'Availability 枚举含义：\n' +
      ...
      this._addLog('trans', `Translator.availability() = ${availability}`);
    } catch (err) {
      this._addLog('warn', `Translator.availability 失败：${err.name} - ${err.message}`);
    }
  }
  ...
  async _summarizeSample() {
    ...
    try {
      ...
      for await (const chunk of summarizer.summarizeStreaming(sample)) {
        streamed += chunk;
        chunkCount += 1;
      }
  ...
  async _checkPrompt() {
    ...
    if (!caps.languageModel) {
      this.setState({ promptInfo:
        ...
        '  initialTemp: 0.7, topK: 40, monitor: (p) => {},\n' +
    ...
    try {
      ...
      try {
        const params = await LanguageModel.params();
        paramsLine = `\nLanguageModel.params() = ${JSON.stringify(params)}\n` +
          '  含 defaultTemperature / defaultTopK / maxTemperature / maxTopK';
      } catch (e) {
        paramsLine = `\nLanguageModel.params() 失败：${e.name}`;
      }
      this.setState({ promptInfo:
