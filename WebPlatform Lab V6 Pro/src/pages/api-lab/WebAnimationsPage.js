...
export class WebAnimationsPage extends Page {
  ...
  componentWillUnmount() {
    ...
    const cancelAll = (arr) => {
      try {
        if (Array.isArray(arr)) arr.forEach((a) => { try { a && a.cancel && a.cancel(); } catch { /* noop */ } });
      } catch { /* noop */ }
      return [];
    };
...
el.animate([...], { duration: 1000, fill: 'forwards' });`),
  ...
  _pauseMain() {
    ...
    catch (err) { this._addLog('warn', 'pause 失败：' + (err && err.message)); }
  }
  ...
  _reverseMain() {
    ...
    try {
      ...
      this._syncMainState();
      this._startAnimPoll();
    } catch (err) { this._addLog('warn', 'reverse 失败：' + (err && err.message)); }
  }
  ...
  _finishMain() {
    ...
    catch (err) { this._addLog('warn', 'finish 失败：' + (err && err.message)); }
  }
  ...
  _cancelMain() {
    ...
    catch (err) { this._addLog('warn', 'cancel 失败：' + (err && err.message)); }
  }
  ...
  _commitStylesMain() {
    ...
    try {
      ...
      this._syncMainState();
    } catch (err) { this._addLog('warn', 'commitStyles 失败：' + (err && err.message)); }
  }
...
anim.addEventListener('cancel', (e) => { /* ... */ });`),
      ],
    );
  }
  ...
  _createWave() {
    ...
    if (els.some((e) => !e)) { this._addLog('warn', '未找到演示元素（wa-box-3a/3b/3c）'); return; }
  ...
  _cancelWaveAnims() {
    try {
      if (Array.isArray(this._waveAnims)) {
        this._waveAnims.forEach((a) => { try { a && a.cancel && a.cancel(); } catch { /* noop */ } });
      }
    } catch { /* noop */ }
    this._waveAnims = [];
  }
  ...
  _cancelMultiAnims() {
    try {
      if (Array.isArray(this._multiAnims)) {
        this._multiAnims.forEach((a) => { try { a && a.cancel && a.cancel(); } catch { /* noop */ } });
      }
    } catch { /* noop */ }
    this._multiAnims = [];
  }
...
anim.addEventListener('finish', (e) => { /* ... */ });
anim.addEventListener('cancel',  (e) => { /* ... */ });
anim.addEventListener('remove',  (e) => { /* 自动移除时触发 */ });`),
      ],
    );
  }
