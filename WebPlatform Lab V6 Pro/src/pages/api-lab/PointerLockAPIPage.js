// =====================================================================
// PointerLockAPIPage.js —— Pointer Lock API 鼠标锁定 完整实验室
...
export class PointerLockAPIPage extends Page {
  ...
  _setupDocumentListeners() {
    ...
    const onChange = () => {
      ...
      const out = this.el && this.el.querySelector('.pl-lock-state');
    ...
    try {
      ...
      if (ret && typeof ret.then === 'function') {
        ret.then(() => {
          ...
          this._addLog('warn', `requestPointerLock Promise rejected：${err && err.name ? err.name : 'Error'} - ${err && err.message ? err.message : ''}`);
        });
      ...
      this._addLog('warn', `requestPointerLock 调用异常：${err.name} - ${err.message}`);
    }
  }
  ...
  _tryExitLock() {
    ...
    try {
      ...
      this._addLog('warn', `exitPointerLock 调用异常：${err.name} - ${err.message}`);
    }
  }
  ...
  _readOverviewInfo() {
    ...
    try {
      ...
      return `读取 Pointer Lock 概述信息失败：${err.name} - ${err.message}`;
    }
  }
  ...
  _readRequestInfo() {
    const f = this._flags();
    try {
      const target = this.el && this.el.querySelector('.pl-target');
      ...
      return `===== requestPointerLock 基础 =====\n` +
        ...
        `    const ret = el.requestPointerLock();\n` +
        `    if (ret && typeof ret.then === 'function') {\n` +
        `      ret.then(onLocked).catch(onError);\n` +
        `    } else {\n` +
        ...
        `      if (ret && ret.catch) ret.catch(err => console.warn(err));\n` +
        `    }\n` +
        `  });\n` +
        `\n` +
        `【陷阱】\n` +
      ...
      return `读取 requestPointerLock 信息失败：${err.name} - ${err.message}`;
    }
  }
  ...
  _renderCard2() {
    ...
    const card = new Card({
      ...
      children: [
        ...
        h('div', { class: 'pl-target', onClick: (e) => this._runRequestDemo() },
          '点击我请求 Pointer Lock\n（真实浏览器将锁定鼠标到此元素）\n当前锁定状态见下方',
        ),
  ...
  _readExitInfo() {
    ...
    try {
      ...
      return `读取 exitPointerLock 信息失败：${err.name} - ${err.message}`;
    }
  }
  ...
  _readEventsInfo() {
    ...
    try {
      ...
      return `读取 pointerlock 事件信息失败：${err.name} - ${err.message}`;
    }
  }
  ...
  _readMovementInfo() {
    ...
    try {
      ...
      return `读取 movementX/Y 信息失败：${err.name} - ${err.message}`;
    }
  }

  _setupMovementListener() {
    const f = this._flags();
    const area = this.el && this.el.querySelector('.pl-move-area');
    if (!area) {
    ...
    const onMove = (e) => {
      ...
      this._movementSamples.push({ x, y, cx: e.clientX, cy: e.clientY });
      if (this._movementSamples.length > 8) this._movementSamples.shift();
      const out = this.el && this.el.querySelector('.pl-move-output');
      if (out) {
    ...
    try {
      ...
      this._addLog('warn', `mousemove 监听绑定失败：${err.name} - ${err.message}`);
    }
  }
  ...
  _readFpsInfo() {
    ...
    try {
      ...
      return `读取 3D 相机控制信息失败：${err.name} - ${err.message}`;
    }
  }

  _setupFpsCamera() {
    const f = this._flags();
    const viewport = this.el && this.el.querySelector('.pl-viewport');
    if (!viewport) {
    ...
    const onMove = (e) => {
      ...
      this._yaw += (e.movementX || 0) * sensitivity;
      this._pitch -= (e.movementY || 0) * sensitivity;
    ...
    try {
      ...
      this._addLog('warn', `3D 相机 mousemove 绑定失败：${err.name} - ${err.message}`);
    }
    // 点击请求锁定
  ...
  _updateFpsView() {
    const hud = this.el && this.el.querySelector('.pl-viewport .pl-hud');
    if (hud) {
    ...
    const cube = this.el && this.el.querySelector('.pl-viewport .pl-cube');
    if (cube) {
  ...
  _readPanoramaInfo() {
    ...
    try {
      ...
      return `读取全景图浏览信息失败：${err.name} - ${err.message}`;
    }
  }

  _setupPanorama() {
    const f = this._flags();
    const pano = this.el && this.el.querySelector('.pl-panorama');
    if (!pano) {
    ...
    const onMove = (e) => {
      this._panoAngle += (e.movementX || 0) * 0.3;
    ...
    try {
      ...
      this._addLog('warn', `全景图 mousemove 绑定失败：${err.name} - ${err.message}`);
    }
    pano.addEventListener('click', () => {
  ...
  _updatePanoramaView() {
    const marker = this.el && this.el.querySelector('.pl-pano-marker');
    const hud = this.el && this.el.querySelector('.pl-pano-hud');
    if (marker) {
      // 0-360° 映射到 0-100% 宽度
