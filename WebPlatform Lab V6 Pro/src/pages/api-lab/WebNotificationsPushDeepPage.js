...
export class WebNotificationsPushDeepPage extends Page {
  ...
  componentDidMount() {
    ...
    if (!f.pushManager) this._addLog('warn', 'PushManager 不可用（需 HTTPS + 现代浏览器）');
    if (!f.serviceWorker) this._addLog('warn', 'serviceWorker 不可用（SW 是 Push 的前置依赖）');
    if (f.notification && f.permission === 'denied') this._addLog('warn', '通知权限已被拒绝，requestPermission 不会再弹窗');
  ...
  _flags() {
    ...
    try { permission = hasNotification ? Notification.permission : 'unsupported'; } catch { /* noop */ }
    let maxActions = null;
    try { maxActions = hasNotification && 'maxActions' in Notification ? Notification.maxActions : null; } catch { /* noop */ }
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'Notification 构造器 new Notification(title, options) 创建通知；options 含 body/dir/icon/badge/image/tag/data/silent/requireInteraction/vibrate/renotify/actions/sticky 全字段。Notification.requestPermission() 返回 Promise（或回调）解析为 granted/denied/default。Notification.permission 静态属性读当前权限，Notification.maxActions 检测操作按钮上限。permissionchange 通过 Permissions API 监听。必须 HTTPS 或 localhost；用户激活要求：应在用户交互回调中请求权限。',
        ),
        h('div', { class: 'np-permission-row' },
          h('span', { class: `np-permission-badge ${permClass}` }, `当前权限：${f.permission}`),
          h('span', { class: 'np-permission-badge np-permission-badge--unsupported' }, `maxActions：${f.maxActions === null ? 'N/A' : f.maxActions}`),
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行基础权限演示', { type: 'primary', size: 'sm', onClick: () => this._runBasicDemo() }),
        ),
        ...
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.basicInfo || '（点击按钮查看 Notifications API 基础与权限完整用法）')),
      ],
    });
  ...
  _renderCard2() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['Notification', f.notification]]),
        h(Tag, { color: 'primary' }, 'options'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'title 主标题、body 副标题（支持换行）、dir: auto|ltr|rtl 文字方向、lang BCP 47 语言标签、icon 大图标（192x192 PNG）、badge 状态栏小图标（Android 72x72 单色）、image 大图预览、tag 通知标识符（同 tag 新通知替换旧通知）、data 任意可序列化数据（structuredClone）、silent 静默不发声、requireInteraction 不自动关闭需用户操作、vibrate 震动模式 [200,100,200]、renotify 同 tag 新通知再次提醒、actions 操作按钮数组（最多 2 个，仅 Android/Chrome）、sticky 固定不可清除（Firefox）。',
        ),
        h('div', { class: 'np-action-grid' },
          h('div', { class: 'np-action-cell' }, 'title 必填'),
          h('div', { class: 'np-action-cell' }, 'body 副标题'),
          h('div', { class: 'np-action-cell' }, 'dir 方向'),
          h('div', { class: 'np-action-cell' }, 'lang 语言'),
          h('div', { class: 'np-action-cell' }, 'icon 大图标'),
          h('div', { class: 'np-action-cell' }, 'badge 小图标'),
          h('div', { class: 'np-action-cell' }, 'image 大图'),
          h('div', { class: 'np-action-cell' }, 'tag 标识符'),
          h('div', { class: 'np-action-cell' }, 'data 数据'),
          h('div', { class: 'np-action-cell' }, 'silent 静默'),
          h('div', { class: 'np-action-cell' }, 'requireInteraction'),
          h('div', { class: 'np-action-cell' }, 'vibrate 震动'),
          h('div', { class: 'np-action-cell' }, 'renotify 再提醒'),
          h('div', { class: 'np-action-cell' }, 'actions 按钮'),
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行 options 演示', { type: 'primary', size: 'sm', onClick: () => this._runOptionsDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
        ...
        h('div', { class: 'fs-sm text-secondary' }, '完整参考：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.optionsInfo || '（点击按钮查看 options 全字段完整深潜）')),
      ],
    });
    this.registerChild(card);
    return card.render();
  ...
  _runEventsDemo() {
    ...
    if (f.notification) {
      ...
      if (Notification.permission === 'granted') {
        try {
          ...
          n.onclick = () => { this._addLog('info', 'notification onclick 触发'); try { n.close(); } catch { /* noop */ } };
    ...
    const info = [
      ...
      '  n.onclick   = (e) => { console.log("点击"); };   // 用户点击通知',
      '  n.onshow    = (e) => { console.log("显示"); };   // 通知显示',
      '  n.onerror   = (e) => { console.log("错误"); };   // 通知出错',
      '  n.onclose   = (e) => { console.log("关闭"); };   // 通知关闭',
      '',
      '  // 或用 addEventListener',
      ...
      '  notifications.forEach((n) => n.close());  // 关闭所有',
      '',
      '  // 按 tag 获取',
  ...
  _renderCard3() {
    ...
    const card = new Card({
      ...
      children: [
        ...
        h('div', { class: 'fs-sm text-secondary' }, '完整参考：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.eventsInfo || '（点击按钮查看 Notification 事件与 SW 通知完整用法）')),
      ],
    });
    this.registerChild(card);
    return card.render();
  ...
  _renderCard4() {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'options.actions: [{ action: "reply", title: "回复", icon: "reply.png" }, { action: "archive", title: "归档" }]。event.action 点击的 action 标识，event.notification.data 自定义数据，event.waitUntil(promise) 保持 SW 活跃，event.reply 回复输入（Android quick reply，需 type: "text"）。actions 限制：最多 2 个、仅 Service Worker 通知支持、桌面 Chrome 支持有限（无按钮，需点击通知主体）、Notification.maxActions 检测上限。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行 actions 演示', { type: 'primary', size: 'sm', onClick: () => this._runActionsDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
        h('pre', { class: 'code-block', style: { maxHeight: '320px', overflow: 'auto' } },
          h('code', {}, codeExample)),
        h('div', { class: 'fs-sm text-secondary' }, '完整参考：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.actionsInfo || '（点击按钮查看 actions 操作按钮完整用法）')),
      ],
    });
    this.registerChild(card);
    return card.render();
  ...
  _renderCard5() {
    ...
    const card = new Card({
      ...
      children: [
        ...
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行订阅演示', { type: 'primary', size: 'sm', onClick: () => this._runSubscribeDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
        ...
        h('div', { class: 'fs-sm text-secondary' }, '完整参考：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.subscribeInfo || '（点击按钮查看 Push API 订阅流程完整用法）')),
      ],
    });
    this.registerChild(card);
    return card.render();
  ...
  _renderCard6() {
    ...
    const card = new Card({
      ...
      children: [
        ...
        h('div', { class: 'fs-sm text-secondary' }, '完整参考：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.vapidInfo || '（点击按钮查看 VAPID 与推送加密完整深潜）')),
      ],
    });
    this.registerChild(card);
    return card.render();
  ...
  _renderCard7() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['serviceWorker', f.serviceWorker]]),
        h(Tag, { color: 'primary' }, 'push 事件'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
        ...
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行 push 事件演示', { type: 'primary', size: 'sm', onClick: () => this._runPushEventDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
        ...
        h('div', { class: 'fs-sm text-secondary' }, '完整参考：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.pushEventInfo || '（点击按钮查看 push 事件处理完整用法）')),
      ],
    });
    this.registerChild(card);
    return card.render();
  ...
  _renderCard8() {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '端到端流程：用户订阅 → 发送 subscription 到应用服务器 → 服务器存储 → 触发推送 → 应用服务器用 VAPID 签名 + 加密 payload → 发送到推送服务（FCM/Mozilla/Apple）→ 推送服务转发到用户设备 → 浏览器唤醒 SW → push 事件 → showNotification。各浏览器推送服务：Chrome=FCM、Firefox=mozilla push、Safari=APNs 需 Apple Developer 账号。陷阱：必须 HTTPS/订阅可能过期需 refresh/Safari 16.4+ 才支持/订阅 endpoint 不可跨用户复用/payload 4KB 限制。降级：Safari < 16.4 用 APNs 原生 SDK。隐私：userVisibleOnly 强制显示通知。调试：chrome://gcm-internals/。与 Background Sync/Periodic Background Sync 协同。',
        ),
        h('div', { class: 'fs-sm text-secondary' }, '端到端流程图：'),
        h('div', { class: 'np-flow' },
          h('div', { class: 'np-flow-step np-flow-step--client' },
            h('span', { class: 'np-flow-step-num' }, '1'),
            h('span', {}, '【client】用户点击订阅，pushManager.subscribe({ userVisibleOnly, applicationServerKey })'),
          ),
          h('div', { class: 'np-flow-arrow' }, '↓'),
          h('div', { class: 'np-flow-step np-flow-step--client' },
            h('span', { class: 'np-flow-step-num' }, '2'),
            h('span', {}, '【client】获取 PushSubscription（endpoint + keys.p256dh + keys.auth）'),
          ),
          h('div', { class: 'np-flow-arrow' }, '↓'),
          h('div', { class: 'np-flow-step np-flow-step--client' },
            h('span', { class: 'np-flow-step-num' }, '3'),
            h('span', {}, '【client】POST subscription 到应用服务器 /api/subscribe'),
          ),
          h('div', { class: 'np-flow-arrow' }, '↓'),
          h('div', { class: 'np-flow-step np-flow-step--server' },
            h('span', { class: 'np-flow-step-num' }, '4'),
            h('span', {}, '【server】服务器存储 subscription（关联 userId + deviceInfo）'),
          ),
          h('div', { class: 'np-flow-arrow' }, '↓'),
          h('div', { class: 'np-flow-step np-flow-step--server' },
            h('span', { class: 'np-flow-step-num' }, '5'),
            h('span', {}, '【server】触发推送（定时任务/事件触发，遍历多设备 subscription）'),
          ),
          h('div', { class: 'np-flow-arrow' }, '↓'),
          h('div', { class: 'np-flow-step np-flow-step--server' },
            h('span', { class: 'np-flow-step-num' }, '6'),
            h('span', {}, '【server】VAPID 私钥签 JWT(ES256) + aes128gcm 加密 payload（≤4KB）'),
          ),
          h('div', { class: 'np-flow-arrow' }, '↓'),
          h('div', { class: 'np-flow-step np-flow-step--server' },
            h('span', { class: 'np-flow-step-num' }, '7'),
            h('span', {}, '【server】POST 加密 payload 到 endpoint（Authorization: vapid t=JWT,k=KEY）'),
          ),
          h('div', { class: 'np-flow-arrow' }, '↓'),
          h('div', { class: 'np-flow-step np-flow-step--push' },
            h('span', { class: 'np-flow-step-num' }, '8'),
            h('span', {}, '【push service】FCM/Mozilla/Apple 转发到用户设备（设备长连接）'),
          ),
          h('div', { class: 'np-flow-arrow' }, '↓'),
          h('div', { class: 'np-flow-step np-flow-step--sw' },
            h('span', { class: 'np-flow-step-num' }, '9'),
            h('span', {}, '【SW】浏览器唤醒 Service Worker → push 事件 → showNotification → 用户点击 → notificationclick → 打开/聚焦页面'),
          ),
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' },
          this._btn('运行架构演示', { type: 'primary', size: 'sm', onClick: () => this._runArchitectureDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '代码示例：'),
        ...
        h('div', { class: 'fs-sm text-secondary' }, '完整参考：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.architectureInfo || '（点击按钮查看完整推送系统架构与陷阱清单）')),
      ],
    });
    this.registerChild(card);
    return card.render();
