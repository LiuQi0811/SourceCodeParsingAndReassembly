...
export class AdvancedObserverPage extends Page {
  ...
  componentWillUnmount() {
    ...
    observers.forEach((key) => {
      try { if (this[key]) { this[key].disconnect?.(); this[key] = null; } } catch { /* noop */ }
    });
      ...
      [
        ...
        s.ioEntries.length > 0
          ? h('div', { class: 'log-panel mt-md', style: { maxHeight: '120px' } },
              s.ioEntries.map((e, i) => h('div', { class: 'log-panel__line' },
        ...
        try {
          ...
          for (const m of records) {
            ...
            if (m.type === 'attributes') {
              this._addLog('mut',
                `attr ${m.attributeName} → ${m.target.getAttribute(m.attributeName)}` +
    ...
    return this._card(
      ...
      [
        ...
        s.moRecords.length > 0
          ? h('div', { class: 'log-panel mt-md', style: { maxHeight: '120px' } },
              s.moRecords.map((r, i) => h('div', { class: 'log-panel__line' },
...
mo.takeRecords(); mo.disconnect();`),
  ...
  _hitFromCenter() {
    ...
    try {
      ...
      this._addLog('hit',
        `elementFromPoint(中心 ${c.cx.toFixed(0)}, ${c.cy.toFixed(0)}) → ${tag}（target.contains=${contains}）`);
  ...
  _elementsFromPointList() {
    ...
    try {
      ...
      this._addLog('point',
        `elementsFromPoint(${c.cx.toFixed(0)}, ${c.cy.toFixed(0)}) → ${list.length} 个元素：` +
...
target.contains(node);                            // 是否包含某节点`),
  ...
  _startCombo() {
    ...
    const bump = (key, label) => {
      this[key]++; this._addLog('combo', `${label} 触发（累计 ${this[key]}）`); this._updateComboStats();
    };
    try {
      if (ioOk) {
  ...
  _comboDisconnect(silent) {
    ...
    let any = false;
    keys.forEach((k) => {
      if (this[k]) { try { this[k].disconnect?.(); } catch { /* noop */ } this[k] = null; any = true; }
    });
    if (!silent) {
