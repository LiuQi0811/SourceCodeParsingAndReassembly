...
export class IntlLocalizationPage extends Page {
  ...
  _runNumberFormat() {
    try {
      const numInput = this._numInput;
      const { numLocale, numStyle } = this.state;
  ...
  _runSegmenter() {
    try {
      ...
      const segInput = this._segInput;
      const { segGranularity } = this.state;
  ...
  // =================== 5. Sanitizer API ===================
  _runSanitize() {
    const sanitizeInput = this._sanitizeInput;
    const { sanitizerSupported } = this.state;
  ...
  // =================== 6. URLPattern API ===================
  _runUrlPattern() {
    const urlPattern = this._urlPattern;
    const urlTest = this._urlTest;
    const { urlPatternSupported } = this.state;
  ...
  renderPage() {
    // —— 惰性初始化实例字段 ——
    // 这 5 个高频输入字段从 state 移到实例字段：onInput 只更新实例字段，
    // 不触发 setState，避免 h(Input) 每次 rerender 都 new Input() 销毁正在输入的 DOM，
    // 修复输入闪屏 / 光标丢失（参照 NavigationComponentsPage 的 _sliderValue 模式）。
    if (this._numInput == null) this._numInput = '1234567.89';
    if (this._segInput == null) this._segInput = 'Hello 世界！你好，今天天气真好。人工智能 AI 是未来。';
    if (this._sanitizeInput == null) this._sanitizeInput = '<img src=x onerror=alert(1)><script>alert(1)</script><b>safe</b> 文本 <a href="javascript:alert(1)">恶意链接</a> <i>斜体</i>';
    if (this._urlPattern == null) this._urlPattern = '/users/:id(\\d+)/posts/:slug';
    if (this._urlTest == null) this._urlTest = '/users/42/posts/hello-world';
    return [
      ...
      h(Card, {
        ...
        h('div', { class: 'flex flex-col gap-sm' },
          ...
          h('div', { class: 'demo-row' },
            ...
            h(Input, {
              value: this._numInput,
              style: { width: '180px' },
              onInput: (e) => { this._numInput = e.target.value; },
            }),
          ),
          h('div', { class: 'demo-row' },
            h('span', { class: 'demo-row__label' }, 'Locale'),
      ...
      h(Card, {
        ...
        h('div', { class: 'flex flex-col gap-sm' },
          ...
          h('div', { class: 'demo-row' },
            ...
            h(Input, {
              value: this._segInput,
              style: { flex: '1', minWidth: '240px' },
              onInput: (e) => { this._segInput = e.target.value; },
            }),
          ),
      ...
      h(Card, {
        ...
        h('div', { class: 'flex flex-col gap-sm' },
          ...
          h('div', { class: 'demo-row' },
            ...
            h(Input, {
              value: this._sanitizeInput,
              style: { flex: '1', minWidth: '320px' },
              onInput: (e) => { this._sanitizeInput = e.target.value; },
            }),
      ...
      h(Card, {
        ...
        h('div', { class: 'flex flex-col gap-sm' },
          ...
          h('div', { class: 'demo-row' },
            ...
            h(Input, {
              value: this._urlPattern,
              style: { flex: '1', minWidth: '240px' },
              onInput: (e) => { this._urlPattern = e.target.value; },
            }),
          ),
          h('div', { class: 'demo-row' },
            ...
            h(Input, {
              value: this._urlTest,
              style: { flex: '1', minWidth: '240px' },
              onInput: (e) => { this._urlTest = e.target.value; },
            }),
