// StoragePage.js —— Storage 实验室：localStorage / sessionStorage / Cookie / IndexedDB
...
export class StoragePage extends Page {
  ...
  renderPage() {
    ...
    const renderTable = (title, items, type) => h(Card, { title, extra: h(Tag, { color: 'primary' }, `${items.length} 项`) },
      items.length === 0
        ? h('div', { class: 'fs-sm text-tertiary' }, '（暂无数据，使用上方表单写入）')
        : h('div', { class: 'storage-list' },
          ...items.map((item) => h('div', { class: 'storage-list__item' },
      ...
      h(Card, { title: 'storage 事件（跨标签页通信）', extra: h(Tag, { color: 'warning' }, '打开两个标签页测试') },
        this.state.events.length === 0
          ...
          : h('div', { class: 'log-panel' },
            ...this.state.events.map((e) => h('div', { class: 'log-panel__line' },
