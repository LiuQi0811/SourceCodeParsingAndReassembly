...
export class Sidebar extends Component {
  ...
  render() {
    ...
    const classes = [
      ...
      // 无菜单时（如首页）抽屉为空，标记 aria-hidden 避免空导航干扰
      items.length === 0 ? 'app-sidebar--empty' : '',
    ].filter(Boolean).join(' ');

    return h('aside', { class: classes, id: 'app-sidebar', 'aria-label': '页面导航', 'aria-hidden': items.length === 0 ? 'true' : undefined },
      ...items.map((item) => {
        if (item.group) {
