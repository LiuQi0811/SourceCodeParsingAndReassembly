...
export class ColorPicker extends Component {
  ...
  _setHex(hex) {
    ...
    // setState 会触发 _rerender 重建整个面板 DOM，导致正在交互的元素被销毁重建 → 闪屏。
    // 与 _setHsv 路径对齐：直接改 state + 操作 DOM 同步视图。
    const rgb = hexToRgb(hex);
    const hsv = rgbToHsv(rgb);
    this.state.hex = hex;
    this._paintColor(hex, hsv.h, hsv.s, hsv.v);
  ...
  _paintColor(hex, h, s, v) {
    const panel = this._dropdownEl;
    // saturation 游标
    const satCursor = panel?.querySelector('.colorpicker__saturation-cursor');
    if (satCursor) {
      satCursor.style.left = `${s * 100}%`;
      satCursor.style.top = `${(1 - v) * 100}%`;
    }
    // saturation 背景（随色相变化）
    const sat = panel?.querySelector('.colorpicker__saturation');
    if (sat) sat.style.background = `hsl(${h}, 100%, 50%)`;
    // hue 滑块 thumb
    const hueThumb = panel?.querySelector('.colorpicker__hue-thumb');
    if (hueThumb) hueThumb.style.left = `${(h / 360) * 100}%`;
    // 触发器色块
    const swatch = this.el?.querySelector('.colorpicker__swatch');
    if (swatch) swatch.style.background = hex;
    // hex 输入框
    const hexInput = panel?.querySelector('.colorpicker__hex-input');
    if (hexInput) hexInput.value = hex;
    // 原生 color input
    const nativeInput = panel?.querySelector('.colorpicker__native');
  ...
  _renderSaturation(hsv) {
    ...
    const onUp = () => {
      ...
      // 拖动期间 _setHsv 已把 state.hex 与 DOM 更新到位，无需额外 setState 触发 rerender
      // （setState 会重建面板 DOM，导致闪屏）。
    };
