...
export class Calendar extends Component {
  ...
  /** 切换月份 —— 直接操作 DOM，不触发 setState/rerender 避免整月网格重建闪屏 */
  _changeMonth(delta) {
  ...
  _goToday() {
    ...
    this._changeViewDate(new Date(today.getFullYear(), today.getMonth(), 1));
    this._selectDate(today);
  }

  /** 修改视图日期并局部刷新 DOM（不触发 rerender） */
  _changeViewDate(d) {
  ...
  /** 选择某天 —— 直接操作 DOM 更新选中态，不触发 rerender */
  _selectDate(date) {
  ...
  _syncPanelDom() {
    ...
    // 更新标题文字
    const yearEl = this.el.querySelector('.calendar__title-year');
    const monthEl = this.el.querySelector('.calendar__title-month');
    if (yearEl) yearEl.textContent = `${year}年`;
    if (monthEl) monthEl.textContent = MONTH_NAMES[month];

    // 重建日期网格 cells（只替换 cells 内容，不重建整个组件）
    const cellsContainer = this.el.querySelector('.calendar__cells');
    ...
    // 清空并重建 cells（这是必要的：月份变化后所有日期都变了）
  ...
  _syncSelectionDom() {
    if (!this.el) return;
    const selectedKey = this.state.selectedDate ? this._dateKey(this.state.selectedDate) : null;
    const cells = this.el.querySelectorAll('.calendar__cell');
    cells.forEach((cellEl) => {
      // 从 cell 的 onClick 闭包无法反查 date，用 data-date 属性
      const dateStr = cellEl.getAttribute('data-date-key');
      const isSelected = dateStr === selectedKey;
      cellEl.classList.toggle('calendar__cell--selected', isSelected);
      ...
      h('div', { class: 'calendar__cells' },
        ...cells.map((cell) => {
          ...
          const inner = this.props.dateCellRender
            ? this.props.dateCellRender(cell.date)
            : h('div', { class: 'calendar__date' }, cell.date.getDate());
          // data-date-key：_syncSelectionDom 据此识别选中态
          return h('div', {
            class: classes,
            role: 'gridcell',
            tabindex: valid ? '0' : '-1',
