// App.js —— 全局应用容器组件（对标 antd 5.x App 组件）
// 主要能力：
...
function _scopedNotif(container, type, title, description, duration = 4.5) {
  ...
  const notif = h('div', { class: `notification notification--${type}`, role: 'alert' },
    h('div', { class: 'notification__icon' }, _NOTIF_ICONS[type] || _NOTIF_ICONS.info),
    h('div', { class: 'flex-1' },
...
export const app = {
  useApp() {
    ...
    return { message: globalMessage, notification: globalNotification, modal: { confirm: globalConfirm } };
  },
};
