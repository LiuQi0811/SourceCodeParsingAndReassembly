...
export class Col extends Component {
  render() {
    ...
    // components.css 中的 @media 规则用 [data-{bp}] 属性选择器消费这些变量，
    // 覆盖基础 inline flex/maxWidth，实现 xs/sm/md/lg/xl/xxl 响应式。
    for (const bp of BREAKPOINTS) {
      ...
      // 只要传了该断点（span 或 offset），就标记 data 属性，
      // 供 CSS @media 用属性选择器命中并应用覆盖。
      if (parsed.span != null || parsed.offset) {
        col.dataset[bp] = parsed.span != null
          ? (parsed.offset ? `${parsed.span}:${parsed.offset}` : `${parsed.span}`)
          : `0:${parsed.offset}`;
      }
    }
    return col;
