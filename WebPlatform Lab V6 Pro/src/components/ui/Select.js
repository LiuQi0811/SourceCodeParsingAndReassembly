...
export class Select extends Component {
    ...
    if (this.state.open && !disabled) {
      const dropdown = h('div', { class: 'select__dropdown', role: 'listbox', id: listboxId },
        ...opts.map((opt, i) => h('div', {
          ...
          onMouseEnter: () => {
            if (!opt.disabled) this._setActiveIdx(i);
          },
        }, opt.label)),
      );
  ...
  _moveActive(delta) {
    ...
    // 纯 DOM 切 class + 更新 aria-activedescendant，避免 setState/rerender 导致闪屏
    this._setActiveIdx(idx);
  }

  /** 直接更新 activeIdx + 操作 DOM 同步高亮与 aria-activedescendant，不触发 setState/rerender。
   *  用于 hover 高亮与键盘上下键移动等高频交互：setState 会重建 dropdown DOM 导致闪屏。 */
  _setActiveIdx(i) {
    this.state.activeIdx = i;
    const dropdown = this._dropdownEl;
    if (dropdown) {
      // 移除旧的高亮
      dropdown.querySelectorAll('.is-active-opt').forEach((el) => el.classList.remove('is-active-opt'));
      // 给当前 option 加高亮
      if (i >= 0) {
        const opt = dropdown.querySelector(`#${this._id}-opt-${i}`);
        if (opt) opt.classList.add('is-active-opt');
      }
    }
    // 更新 trigger 的 aria-activedescendant（无障碍：屏幕阅读器跟随活动 option）
    const trigger = this.el?.querySelector('.select__trigger');
    if (trigger) {
      if (this.state.open && i >= 0) {
        trigger.setAttribute('aria-activedescendant', `${this._id}-opt-${i}`);
      } else {
