...
export class DatePicker extends Component {
  ...
  /** 翻月/翻年后直接改 state 字段，仅局部重建 cells 网格，不触发 _rerender */
  _changeViewDate(d) {
    this.state.viewDate = d;
    this._syncPanelViewDom();
  }

  /** 局部重建 panel 的 cells 网格 + 标题（不重建 trigger / 头部按钮） */
  _syncPanelViewDom() {
    const panel = this._dropdownEl || this.el?.querySelector('.datepicker__panel');
    if (!panel) return;
    const cellsGrid = panel.querySelector('.datepicker__cells');
    if (cellsGrid) {
      while (cellsGrid.firstChild) cellsGrid.removeChild(cellsGrid.firstChild);
      for (const cell of this._buildCells(this.state.viewDate)) {
        cellsGrid.appendChild(this._renderCell(cell));
      }
    }
    const title = panel.querySelector('.datepicker__title');
    if (title) {
      title.textContent = `${this.state.viewDate.getFullYear()}年 ${MONTH_NAMES[this.state.viewDate.getMonth()]}`;
    }
  }

  /** 根据 viewDate 生成当月 cells（前后补齐到 42 格） */
  _buildCells(viewDate) {
  ...
  _renderPanel() {
    ...
    return h('div', { class: 'datepicker__panel', role: 'dialog', onClick: (e) => e.stopPropagation() },
      ...
      h('div', { class: 'datepicker__cells' },
        ...this._buildCells(viewDate).map((cell) => this._renderCell(cell)),
      ),
...
export class RangePicker extends Component {
  ...
  /** 局部重建 panel 的 cells 网格 + 标题（不重建 trigger / 头部按钮） */
  _syncPanelViewDom() {
    const panel = this._dropdownEl || this.el?.querySelector('.rangepicker__panel');
    if (!panel) return;
    const cellsGrid = panel.querySelector('.datepicker__cells');
    if (cellsGrid) {
      while (cellsGrid.firstChild) cellsGrid.removeChild(cellsGrid.firstChild);
