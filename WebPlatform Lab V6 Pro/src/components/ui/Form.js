// Form.js —— 表单容器与表单项（参考 antd Form / Form.Item）
...
export class Form extends Component {
  ...
  componentDidMount() {
    ...
    if (this.el) this.el.__form = this;
  ...
  async submit() {
    ...
    if (this._items) {
      for (const item of this._items) {
        const errs = await item.validate(values[item.props.name]);
        if (errs && errs.length) errors.push({ name: item.props.name, errors: errs });
      }
    }
...
export class FormItem extends Component {
  ...
  componentDidMount() {
    ...
    this._unregister = this._form?._registerItem?.(this);
  ...
  _getChildEl() {
    return this.el?.querySelector('.form-item__control')?.firstElementChild || null;
  }
  ...
  _bindChildEvents() {
    ...
    this.on(childEl, 'change', (e) => {
      ...
      const v = e.detail != null ? e.detail : e.target?.value;
      sync(v);
    });
