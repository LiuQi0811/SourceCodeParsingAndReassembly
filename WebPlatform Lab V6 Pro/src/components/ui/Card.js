// Card.js —— 卡片组件
import { Component } from '../../core/Component.js';
import { h } from '../../core/utils.js';

export class Card extends Component {
  render() {
    const { title, extra, hoverable = false, children, bodyStyle = null } = this.props;
    // children 支持函数形式（lazy）：在 render() 内部求值，
    // 让 children 中的 h(子组件) 在本 Card 的 _recycleContext 下执行，
    // 注册到本 Card._children 而非外层组件。
    // 否则 `new Card({ children: [h(Tag,...)] })` 模式中 h(Tag) 在 new 时已求值，
    // 注册到外层 → Card.rerender 时回收池缺失 Tag → 重建 → 闪屏。
    const actualChildren = typeof children === 'function' ? children() : children;
    const classes = ['card', hoverable && 'card--hoverable'].filter(Boolean).join(' ');

    const kids = [];
    if (title || extra) {
      kids.push(h('div', { class: 'card__head' },
        title && h('div', { class: 'card__title' }, title),
        extra && h('div', { class: 'card__extra' }, extra),
      ));
    }
    const body = h('div', { class: 'card__body' }, ...(Array.isArray(actualChildren) ? actualChildren : [actualChildren]));
    if (bodyStyle) Object.assign(body.style, bodyStyle);
    kids.push(body);

    return h('div', { class: classes }, ...kids);
  }
}
