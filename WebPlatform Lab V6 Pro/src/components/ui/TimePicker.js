...
export class TimePicker extends Component {
  ...
  _select(part, val) {
    ...
    this.state.time = t;
    // 局部同步视图：更新对应列的 is-selected 高亮 + trigger 文本
    this._syncColumnSelected(part, t[part]);
    this._syncTriggerText();
    this.props.onChange?.(t, formatTime(t, this.props.format || 'HH:mm:ss', this.props.use12Hours));
  }

  /** 同步某列的 is-selected 高亮（不触发 rerender） */
  _syncColumnSelected(part, val) {
    const panel = this._dropdownEl || this.el?.querySelector('.timepicker__panel');
    if (!panel) return;
    // 列顺序：h=0, m=1, s=2
    const colIdx = part === 'h' ? 0 : part === 'm' ? 1 : 2;
    const columns = panel.querySelectorAll('.timepicker__column');
    const column = columns[colIdx];
    if (!column) return;
    const list = column.querySelector('.timepicker__column-list');
    if (!list) return;
    // 移除旧 is-selected
    const oldSel = list.querySelector('.is-selected');
    if (oldSel) oldSel.classList.remove('is-selected');
    // 给新 option 加 is-selected（按 textContent 匹配 pad(val)）
    const target = pad(val);
    for (const opt of list.querySelectorAll('.timepicker__option')) {
      if (opt.textContent === target) {
  ...
  _syncTriggerText() {
    ...
    if (!textSpan) return;
    const text = this.state.time ? formatTime(this.state.time, this.props.format || 'HH:mm:ss', this.props.use12Hours) : '';
    textSpan.textContent = text || (this.props.placeholder ?? '请选择时间');
      ...
      h('div', { class: 'timepicker__footer' },
        h('button', {
          ...
          onClick: () => {
            ...
            // 直接改 state 字段，绕过 setState，避免 _rerender 重建 DOM 导致闪屏
            this.state.time = t;
            // 局部同步视图：三列 is-selected 高亮 + trigger 文本
            this._syncColumnSelected('h', t.h);
            this._syncColumnSelected('m', t.m);
            this._syncColumnSelected('s', t.s);
            this._syncTriggerText();
            this.props.onChange?.(t, formatTime(t, this.props.format || 'HH:mm:ss', use12Hours));
          },
        }, '此刻'),
