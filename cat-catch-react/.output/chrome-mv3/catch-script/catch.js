(function () {
    class CatCatcher {
         ...
         */
        createUI() {
            const buttonStyle = 'style="margin:2px;padding:6px 10px;background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.12);border-radius:6px;color:#fff;font-size:12px;cursor:pointer;font-family:system-ui,-apple-system,sans-serif;"';
            ...
            <div id="catCatch" style="${style}">
                <style>
                    #CatCatchCatch details { margin-top: 4px; width: 100%; }
                    #CatCatchCatch summary { cursor: pointer; color: rgba(255,255,255,0.8); font-size: 12px; }
                    #CatCatchCatch label { display: flex; align-items: center; gap: 4px; color: rgba(255,255,255,0.8); font-size: 12px; }
                    #CatCatchCatch input[type="checkbox"] { accent-color: #3b82f6; }
                    #CatCatchCatch #tips { color: rgba(255,255,255,0.6); font-size: 12px; }
                    #CatCatchCatch button:hover { opacity: 0.8; }
                    #CatCatchCatch div { color: rgba(255,255,255,0.8); }
                    #CatCatchCatch img { vertical-align: middle; }
                </style>
                <div id="tips"></div>
        ...
        /**
         * 创建 Shadow DOM
         * 简化:直接用 attachShadow(现代浏览器原生支持),避免 iframe 获取原生方法失败降级为普通 div
         */
        createShadowRoot() {
            try {
                ...
                let shadowRoot;
                try {
                    shadowRoot = divShadow.attachShadow({ mode: 'open' });
                } catch (e) {
                    console.error('Shadow DOM 创建失败:', e);
                    // 降级:直接插入(无隔离,但至少能显示)
                    shadowRoot = divShadow;
                }
