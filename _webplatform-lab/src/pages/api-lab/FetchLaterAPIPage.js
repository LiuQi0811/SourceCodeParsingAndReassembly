...
export class FetchLaterAPIPage extends Page {
  ...
  _demoBeaconDataTypes() {
    ...
    try {
      ...
      try { ok = navigator.sendBeacon(url, new Blob(['{"a":1}'], { type: 'application/json' })); results.push(`Blob(application/json) → ${ok}`); } catch (e) { results.push(`Blob → 失败:${e.message}`); }
      try { ok = navigator.sendBeacon(url, new ArrayBuffer(8)); results.push(`ArrayBuffer(8) → ${ok}`); } catch (e) { results.push(`ArrayBuffer → 失败:${e.message}`); }
      try { ok = navigator.sendBeacon(url, new Uint8Array([1, 2, 3, 4])); results.push(`Uint8Array → ${ok}`); } catch (e) { results.push(`Uint8Array → 失败:${e.message}`); }
      try { ok = navigator.sendBeacon(url, new URLSearchParams({ a: '1', b: '2' })); results.push(`URLSearchParams → ${ok}`); } catch (e) { results.push(`URLSearchParams → 失败:${e.message}`); }
      try { const fd = new FormData(); fd.append('x', '1'); ok = navigator.sendBeacon(url, fd); results.push(`FormData → ${ok}`); } catch (e) { results.push(`FormData → 失败:${e.message}`); }
  ...
  _demoBatchSend() {
    ...
    if (caps.fetchLater) {
      ...
      } catch (e) { lines.push(`    调用异常：${e.message}`); }
    } else if (caps.sendBeacon) {
      lines.push('  → navigator.sendBeacon(url, body)');
      try { const ok = navigator.sendBeacon(url, body); lines.push(`    返回 ${ok}`); } catch (e) { lines.push(`    异常：${e.message}`); }
    } else if (caps.fetch) {
