...
//      多候选结果 / 置信度排序 / 短文本识别挑战
//   4. 可用性检测与可用性 —— Translator.availability({ sourceLanguage,
...
export class WebTranslationAPIPage extends Page {
  ...
  componentDidMount() {
    ...
    const parts = [
      ...
      `Translator.availability ${c(f.translatorAvailability)}`,
    ...
    if (!f.translatorAvailability) this._addLog('warn', 'Translator.availability 不可用，无法探测模型下载状态');
  ...
  _runOverviewDemo() {
    ...
    const info = [
      ...
      `  Translator.availability: ${f.translatorAvailability ? '✓' : '✗'}`,
...
detector.destroy();`)),
  ...
  _runTranslatorDemo() {
    ...
    const info = [
      ...
      '  translator.translate(text, ac2.signal).catch((err) => {',
...
// AbortSignal 中止
const ac = new AbortController();
translator.translate(longText, ac.signal).catch((err) => {
  if (err.name === 'AbortError') console.log('已中止');
});
      ...
      try {
        ...
        LanguageDetector.create().then((detector) => {
          ...
          const lines = results.map((r, i) =>
            `"${samples[i]}" → ${r.map((x) => `${x.detectedLanguage}(${(x.confidence * 100).toFixed(0)}%)`).join(', ')}`);
...
detector.destroy(); // 释放模型`)),
  ...
  _runAvailabilityDemo() {
    ...
    if (!f.translatorAvailability) {
      this._addLog('warn', 'Translator.availability 不可用，可用性检测仅展示 API 用法（jsdom 无）');
    }
    const info = [
  ...
  _renderCard4() {
    ...
    const f = this._flags();
    const card = new Card({
      title: '4. 可用性检测 —— Translator.availability / LanguageDetector.availability / 下载状态',
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([
          ['Translator.availability', f.translatorAvailability],
          ['LanguageDetector.availability', f.detectorAvailability],
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'Translator.availability({ sourceLanguage, targetLanguage }) 返回 available（已下载）/ downloadable（可下载，首次 create 触发）/ downloading（下载中）/ after-download（需先触发下载）/ unsupported（不支持）。LanguageDetector.availability() 同语义但无参数。首次使用需 UI 提示模型下载（数十 MB），用户确认后 create 触发下载。浏览器自动缓存模型（IndexedDB），多页面共享，第二次 create 即时返回。降级方案：不支持或拒绝下载时回退到云 API。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
...
recognition.onresult = (e) => {
  ...
  _runBatchDemo() {
    ...
    const info = [
      ...
      '    }',
      '    // 释放所有 Translator',
      '    for (const t of Object.values(translators)) t.destroy();',
      '    detector.destroy();',
      '    return results.join("\\n\\n");',
      '  }',
      '',
      ...
      '',
      '  // UI 进度条',
      '  await translateWithProgress(doc, "zh-Hans", ({ done, total, pct }) => {',
...
// 多语言混合：语言路由
async function translateMixed(chunks, targetLang) {
  const detector = await LanguageDetector.create();
...
await translateWithProgress(text, 'zh-Hans', {
  signal: ac.signal,
  onProgress: ({ done, total, pct }) => updateBar(pct),
});
...
const status = await Translator.availability({ sourceLanguage: 'en', targetLanguage: 'zh-Hans' });
if (status === 'downloadable') {
