...
export class IdentityPage extends Page {
  ...
  _jsonBlock(obj) {
    ...
    try {
      ...
      return h('div', { class: 'fs-sm text-tertiary' }, `（序列化失败：${err.message}）`);
    }
  }
  ...
  async _preventSilentAccess() {
    ...
    try {
      ...
      this._addLog('err', `preventSilentAccess 失败：${err.name} — ${err.message}`);
      this.setState({ credPreventSilentResult: `失败：${err.name}` });
    }
  }
  ...
  async _createAndStorePasswordCredential() {
    ...
    try {
      ...
      this._addLog('err', `PasswordCredential 创建/存储失败：${err.name} — ${err.message}`);
      this.setState({ credStoreResult: `失败：${err.name}` });
    }
  }
  ...
  async _checkPlatformAuthenticator() {
    ...
    try {
      ...
      this._addLog('err', `检测平台认证器失败：${err.name} — ${err.message}`);
  ...
  async _checkConditionalMediation() {
    ...
    try {
      ...
      this._addLog('err', `检测条件 UI 失败：${err.name} — ${err.message}`);
  ...
  _buildWebAuthnOptions() {
    ...
    try {
      ...
      this._addLog('err', `构造 WebAuthn 选项失败：${err.name} — ${err.message}`);
    }
  }
  ...
  _buildPaymentRequest() {
    ...
    try {
      ...
      this._addLog('err', `构造 PaymentRequest 失败：${err.name} — ${err.message}`);
      return null;
    }
  }
  ...
  async _canMakePayment() {
    ...
    try {
      ...
      this._addLog('err', `canMakePayment 失败：${err.name} — ${err.message}`);
  ...
  async _showPayment() {
    ...
    try {
      ...
      try {
        ...
        this._addLog('err', `complete 失败：${e.message}`);
      }
    } catch (err) {
      this._addLog('err', `show 失败：${err.name} — ${err.message}（jsdom / 无支付处理程序时属正常）`);
      this.setState({ paymentShowResult: `失败：${err.name}` });
    }
  }
  ...
  _buildPresentationRequest() {
    ...
    try {
      ...
      try {
        ...
        this._addLog('err', `设置 defaultRequest 失败：${err.message}`);
      }
    } catch (err) {
      this._addLog('err', `构造 PresentationRequest 失败：${err.name} — ${err.message}`);
    }
  }
  ...
  async _idleRequestPermission() {
    ...
    try {
      ...
      this._addLog('err', `requestPermission 失败：${err.name} — ${err.message}`);
    }
  }
  ...
  async _startIdleDetector() {
    ...
    try {
      ...
      this._addLog('err', `IdleDetector.start 失败：${err.name} — ${err.message}`);
  ...
  async _queryLocalFonts() {
    ...
    try {
      ...
      this._addLog('err', `queryLocalFonts 失败：${err.name} — ${err.message}`);
    }
  }
