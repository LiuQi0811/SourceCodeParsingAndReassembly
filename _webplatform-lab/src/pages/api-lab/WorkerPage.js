// WorkerPage.js —— Web Worker 实验室：主线程 vs Worker 对比
...
export class WorkerPage extends Page {
  ...
  _onWorkerMessage = (e) => {
    ...
    if (type === 'sieve:result') {
      this._append('info', `Worker sieve 完成 → ${payload.count} 个素数，耗时 ${formatDuration(payload.elapsed)}`);
      this.setState((s) => ({
        results: [...s.results, { task: 'sieve (Worker)', result: payload.count, elapsed: payload.elapsed, time: formatTime() }].slice(-8),
        running: false,
      }));
    } else if (type === 'fibonacci:result') {
      this._append('info', `Worker fibonacci 完成 → ${payload.digits} 位，耗时 ${formatDuration(payload.elapsed)}`);
      this.setState((s) => ({
        results: [...s.results, { task: 'fib (Worker)', result: payload.value, elapsed: payload.elapsed, time: formatTime() }].slice(-8),
        running: false,
      }));
    }
  };
      ...
      h(Card, { title: '执行结果对比', extra: h(Tag, { color: 'primary' }, `${this.state.results.length} 条`) },
        this.state.results.length === 0
          ...
          : h('div', { class: 'log-panel' },
            ...this.state.results.map((r) => h('div', { class: 'log-panel__line' },
