...
export class WindowPiPViewportPage extends Page {
  ...
  async _demoDocPiP() {
    ...
    try {
      ...
      if (movedEl && pipWindow && pipWindow.document && pipWindow.document.body) {
        ...
        } catch (e) { this._addLog('warn', `移动 DOM 进 PiP 失败：${e.message}`); }
      }
      ...
      this._addLog('warn', `requestWindow 失败：${err.name} - ${err.message}（可能无用户手势或浏览器不支持）`);
      this.setState({ docPiPInfo: `requestWindow 失败：${err.name} - ${err.message}\n常见原因：未由用户手势触发 / 浏览器不支持 / 已有 PiP 窗口。` });
    }
  }
  ...
  _closeDocPiP() {
    ...
    try {
      ...
      this._addLog('warn', `关闭 PiP 失败：${err.name} - ${err.message}`);
    }
    this._pipWindow = null;
    this._pipMovedEl = null;
  }
  ...
  _readViewport() {
    ...
    try {
      ...
      this._addLog('warn', `读取 visualViewport 失败：${err.name} - ${err.message}`);
    }
  }
  ...
  _bindViewport() {
    ...
    try { vv.addEventListener('resize', this._onVVResize); } catch (err) { this._addLog('warn', `注册 resize 失败：${err.message}`); }
    try { vv.addEventListener('scroll', this._onVVScroll); } catch (err) { this._addLog('warn', `注册 scroll 失败：${err.message}`); }
    this.setState({ viewportInfo:
  ...
  async _requestWakeLock() {
    ...
    try {
      ...
      this._addLog('warn', `request wakeLock 失败：${err.name} - ${err.message}（可能无用户手势 / 文档不可见 / 权限拒绝）`);
      this.setState({ wakeLockInfo: `request('screen') 失败：${err.name} - ${err.message}\n常见原因：未由用户手势触发 / 文档不可见 / 浏览器不支持。` });
    }
  }
  ...
  async _releaseWakeLock() {
    ...
    try {
      ...
      this._addLog('warn', `release 失败：${err.name} - ${err.message}`);
    }
    this._wakeLockSentinel = null;
    this._wakeLockReleaseHandler = null;
  }
  ...
  _readOrientation() {
    ...
    try {
      ...
      this._addLog('warn', `读取 orientation 失败：${err.name} - ${err.message}`);
    }
  }
  ...
  async _attemptFullscreen() {
    ...
    try {
      ...
      this._addLog('warn', `requestFullscreen 失败：${err.name} - ${err.message}（通常需用户手势触发）`);
      this.setState({ orientationFsInfo:
        `requestFullscreen 失败：${err.name} - ${err.message}\n常见原因：未由用户手势触发 / 元素不可全屏 / 浏览器拒绝。jsdom 中 fullscreenEnabled 为 false，无法真实进入全屏。` });
    }
  }
  ...
  async _exitFullscreen() {
    ...
    try {
      ...
      this._addLog('warn', `exitFullscreen 失败：${err.name} - ${err.message}`);
    }
  }
  ...
  async _attemptPointerLock() {
    ...
    try {
      ...
      this._addLog('warn', `requestPointerLock 失败：${err.name} - ${err.message}（通常需用户手势触发）`);
      this.setState({ pointerLockInfo:
        `requestPointerLock 失败：${err.name} - ${err.message}\n常见原因：未由用户手势触发 / unadjustedMovement 不被支持 / 用户拒绝。典型场景：FPS 游戏、3D 模型查看器。` });
    }
  }
  ...
  _exitPointerLock() {
    ...
    try {
      ...
      this._addLog('warn', `exitPointerLock 失败：${err.name} - ${err.message}`);
    }
  }
  ...
  _detectVirtualKeyboard() {
    ...
    try {
      ...
      this._addLog('warn', `检测 virtualKeyboard 失败：${err.name} - ${err.message}`);
    }
  }
  ...
  _toggleVirtualKeyboard(show) {
    ...
    try {
      ...
      this._addLog('warn', `${show ? 'show' : 'hide'} 失败：${err.name} - ${err.message}`);
    }
  }
