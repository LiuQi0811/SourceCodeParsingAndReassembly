...
export class WebMIDINFCGamepadPage extends Page {
  ...
  async _midiRequest() {
    ...
    try {
      ...
      this._addLog('warn', `requestMIDIAccess 失败：${err.name || 'Error'} — ${err.message || ''}`);
    }
  }
  ...
  _midiExplain() {
    this.setState({ midiInfo:
      '===== MIDI 消息格式与 sysex 权限 =====\n\n' +
      '【MIDIAccess】.inputs / .outputs（Map）· .sysexEnabled（bool）· .onstatechange = (e) => e.port\n' +
  ...
  async _nfcScan() {
    if (!this._has('nfc')) {
      this.setState({ nfcInfo:
        ...
        "用法：const reader = new NDEFReader();\nreader.onreading = (e) => { e.serialNumber; e.message.records[] };\n" +
    ...
    try {
      ...
      this._addLog('warn', `NFC scan 失败：${err.name || 'Error'} — ${err.message || ''}`);
    }
  }
  ...
  _nfcBuildMessage() {
    ...
    try {
      ...
      try {
        ...
        recordInfo = `（NDEFMessage 构造在此环境抛 ${e.name}，仅展示用法）`;
      }
      this.setState({ nfcInfo:
      ...
      this._addLog('warn', `构造 NDEFMessage 失败：${err.name || 'Error'} — ${err.message || ''}`);
    }
  }
  ...
  async _nfcWriteDemo() {
    ...
    try {
      ...
      this._addLog('warn', `NFC write 失败：${err.name || 'Error'} — ${err.message || ''}`);
    }
  }
  ...
  _renderCard2() {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          "new NDEFReader() 构造。reader.scan({ signal }) 返回 Promise 开始监听标签；reader.write(message, { overwrite, signal }) 写入 NDEF 消息。reader.onreading = (e) => 取 e.serialNumber / e.message（NDEFMessage）；e.message.records 是 NDEFRecord 数组，recordType 含 'empty'|'text'|'url'|'smart-poster'|'absolute-url'|'mime'|'unknown'，可调用 toRecords() / toText() / toJSON() / toDataURL() 解码。仅 Android Chrome 可用。"),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
  ...
  _gamepadScan() {
    ...
    let pads = [];
    try { pads = Array.from(navigator.getGamepads() || []); } catch { pads = []; }
  ...
  _gamepadPoll() {
    ...
    let pads = [];
    try { pads = Array.from(navigator.getGamepads() || []).filter((p) => p); } catch { return; }
  ...
  async _gamepadRumble() {
    ...
    let pads = [];
    try { pads = Array.from(navigator.getGamepads() || []).filter((p) => p); } catch { pads = []; }
    if (pads.length === 0) {
      this.setState({ gamepadInfo:
        'GamepadHapticActuator 振动用法（无真实手柄，仅说明）：\n\n' +
        "const actuator = navigator.getGamepads().filter(p=>p)[0].vibrationActuator;\nawait actuator.playEffect('dual-rumble', {\n" +
    ...
    try {
      ...
      this._addLog('warn', `playEffect 失败：${err.name || 'Error'} — ${err.message || ''}`);
    }
  }
  ...
  async _otpRequest() {
    ...
    try {
      ...
      this._addLog('warn', `credentials.get 失败：${err.name || 'Error'} — ${err.message || ''}`);
      this.setState({ otpInfo: `WebOTP 请求失败：${err.name || 'Error'} — ${err.message || ''}\n（AbortError 为主动取消，属正常）` });
    } finally {
      this._otpAbort = null;
    }
  }
  ...
  _pressureStart() {
    if (!this._has('pressure')) {
      this.setState({ pressureInfo:
        ...
        "用法：const observer = new PressureObserver((changes) => {...}, { sampleRate: 1.0 });\nobserver.observe('cpu'); observer.disconnect(); observer.takeRecords();\n" +
    ...
    try {
      ...
      catch (e) { this._addLog('warn', `observe('cpu') 失败：${e.name || 'Error'} — ${e.message || ''}`); }
      ...
      this._addLog('warn', `创建 PressureObserver 失败：${err.name || 'Error'} — ${err.message || ''}`);
    }
  }
  ...
  _pressureStop() {
    ...
    try {
      ...
      this._addLog('warn', `disconnect 失败：${err.name || 'Error'} — ${err.message || ''}`);
    }
  ...
  async _dgListPurchases() {
    ...
    try {
      ...
      this._addLog('warn', `Digital Goods 调用失败：${err.name || 'Error'} — ${err.message || ''}`);
    }
  }
