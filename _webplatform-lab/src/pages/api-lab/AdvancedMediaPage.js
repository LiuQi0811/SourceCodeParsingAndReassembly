...
export class AdvancedMediaPage extends Page {
    ...
    try {
      ...
      this.on(ms, 'sourceopen', () => {
        ...
        try {
          ...
          this._addLog('error', `addSourceBuffer 失败：${err.message}`);
          return;
        }
        // SourceBuffer 事件
      ...
      this._addLog('error', `MediaSource 创建异常：${err.message}`);
    }
  }
  ...
  _mseAbort() {
    ...
    catch (err) { this._addLog('error', `sourceBuffer.abort 失败：${err.message}`); }
  }
  ...
  _mseRemove() {
    ...
    catch (err) { this._addLog('error', `sourceBuffer.remove 失败：${err.message}`); }
  }
  ...
  _mseEndOfStream() {
    ...
    catch (err) { this._addLog('error', `endOfStream 失败：${err.message}`); }
  }
  ...
  _requestPiP() {
    ...
    v.play().then(() => v.requestPictureInPicture())
      ...
      .catch((err) => this._addLog('error', `requestPictureInPicture 失败：${err.message}`));
  }
  ...
  _exitPiP() {
    ...
    document.exitPictureInPicture()
      ...
      .catch((err) => this._addLog('error', `exitPictureInPicture 失败：${err.message}`));
  }
  ...
  _requestDocPiP() {
    ...
    dpi.requestWindow({ width: 360, height: 240 })
      ...
      .catch((err) => this._addLog('error', `requestWindow 失败：${err.message}`));
  }
  ...
  async _runDecodeInfo() {
    ...
    for (const item of DECODE_CONFIGS) {
      try {
        ...
        results.push({ name: item.name, supported: false, smooth: false, powerEfficient: false, error: err.message });
        this._addLog('error', `decodingInfo("${item.name}") 异常：${err.message}`);
      }
    }
  ...
  async _runEncodeInfo() {
    ...
    for (const item of ENCODE_CONFIGS) {
      try {
        ...
        results.push({ name: item.name, supported: false, smooth: false, powerEfficient: false, error: err.message });
        this._addLog('error', `encodingInfo("${item.name}") 异常：${err.message}`);
      }
    }
  ...
  async _probeKeySystems() {
    ...
    for (const ks of EME_KEY_SYSTEMS) {
      try {
        ...
        results.push({ keySystem: ks, supported: false, detail: err.message });
        this._addLog('error', `requestMediaKeySystemAccess("${ks}") 失败：${err.message}`);
      }
    }
  ...
  async _createMediaKeys() {
    ...
    try {
      ...
      this._addLog('error', `createMediaKeys 链路失败：${err.message}`);
    }
  }
  ...
  async _getUserMedia() {
    ...
    try {
      ...
      this._addLog('error', `getUserMedia 失败：${err.name || ''} ${err.message}`);
    }
  }
  ...
  _applyConstraints() {
    ...
    track.applyConstraints(constraints)
      ...
      .catch((err) => this._addLog('error', `applyConstraints 失败：${err.message}`));
  }
  ...
  _stopTrack() {
    ...
    catch (err) { this._addLog('error', `track.stop 失败：${err.message}`); }
  }
  ...
  _probeGenerator() {
    ...
    try {
      ...
      this._addLog('error', `MediaStreamTrackGenerator 异常：${err.message}`);
    }
  }
