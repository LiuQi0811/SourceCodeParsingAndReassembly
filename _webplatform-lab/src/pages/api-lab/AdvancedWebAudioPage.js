...
export class AdvancedWebAudioPage extends Page {
  ...
  _resumeCtx() {
    ...
    }).catch((err) => this._addLog('error', `resume 失败：${err.message}`));
  }
  ...
  _suspendCtx() {
    ...
    }).catch((err) => this._addLog('error', `suspend 失败：${err.message}`));
  }
  ...
  _closeCtx() {
    ...
    this._audioCtx.close().then(() => {
      ...
      this._audioCtx = null;
    }).catch((err) => this._addLog('error', `close 失败：${err.message}`));
  }
