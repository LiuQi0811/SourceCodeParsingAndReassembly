...
export class SchedulerTasksPage extends Page {
  ...
  componentDidMount() {
    ...
    if (!hasScheduler) this._addLog('warn', 'scheduler 不可用（需 Chromium 94+，且仅在 window / Worker 上下文）');
    if (!hasTaskController) this._addLog('warn', 'TaskController 不可用（需 Chromium 94+）');
    if (!hasTaskSignal) this._addLog('warn', 'TaskSignal 不可用（需 Chromium 94+）');
    if (!hasIdleDetector) this._addLog('warn', 'IdleDetector 不可用（需 Chromium 94+，且 https 与权限授予）');
    if (!hasPressureObserver) this._addLog('warn', 'PressureObserver 不可用（需 Chromium 125+）');
    if (!hasMeasureMemory) this._addLog('warn', 'performance.measureUserAgentSpecificMemory 不可用（需 Chromium 89+ 且跨域隔离）');
  }

  componentWillUnmount() {
    // 释放系统资源：停止 IdleDetector / 断开 PressureObserver / 取消未完成 task
    if (this._idleDetector) {
      try { this._idleDetector.stop?.(); } catch { /* noop */ }
    ...
    if (this._pendingTaskController) {
      try { this._pendingTaskController.abort(); } catch { /* noop */ }
  ...
  async _runPriorityComparison() {
    ...
    try {
      ...
      this.setState({
        priorityResult:
          `scheduler.postTask(callback, { priority }) 三级优先级对比\n` +
      ...
      this._addLog('warn', `优先级对比失败：${err.name} - ${err.message}`);
    }
  }
  ...
  async _compareWithSetTimeout() {
    ...
    try {
      ...
      if (caps.queueMicrotask) queueMicrotask(() => mark('queueMicrotask'));
      else this._addLog('warn', 'queueMicrotask 不可用');
      // setTimeout(0)：会被钳制到 ~4ms（HTML5 规范）
      setTimeout(() => mark('setTimeout(0)'), 0);
      // requestIdleCallback：空闲时执行
      if (caps.requestIdle) requestIdleCallback(() => mark('requestIdleCallback'));
      else this._addLog('warn', 'requestIdleCallback 不可用');
  ...
  _renderCard1() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.scheduler ? 'success' : 'error' }, caps.scheduler ? 'scheduler ✓' : '不可用'),
        h(Tag, { color: 'primary' }, 'user-blocking / visible / background'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'window.scheduler.postTask(callback, options) → Promise 按指定优先级调度任务；options.priority ∈ "user-blocking"（最高，~0ms）/ "user-visible"（默认，~4ms）/ "background"（最低，~50ms）。调度器按优先级而非提交顺序执行；同优先级 FIFO。options.delay（毫秒延迟）。可与 setTimeout / queueMicrotask / requestIdleCallback 对比，理解不同调度机制的优先级与延迟特性。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('三级优先级对比', { type: 'primary', size: 'sm', disabled: !caps.scheduler, onClick: () => this._runPriorityComparison() }),
          this._btn('对比 setTimeout 等', { size: 'sm', disabled: !caps.scheduler, onClick: () => this._compareWithSetTimeout() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '调度结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '220px', overflow: 'auto' } },
          h('code', {}, s.priorityResult || '（点击「三级优先级对比」或「对比 setTimeout 等」）')),
...
requestIdleCallback(cb); // 空闲时才执行`)),
  ...
  async _demoTaskController() {
    ...
    try {
      ...
      const signalInfo = `controller.signal.priority = ${controller.signal.priority}`;
      const taskPromise = scheduler.postTask(
        () => { this._addLog('controller', '任务执行（不应到达，因已被取消）'); return 'done'; },
        { signal: controller.signal, delay: 5000 },
      );
      // 短暂等待后取消
      setTimeout(() => {
      ...
      try {
        ...
        abortInfo = `任务被取消 → 抛出 ${err.name}：${err.message}\n  instanceof AbortError / DOMException(name=AbortError) = ${isAbort}`;
        this._addLog('controller', `捕获 ${err.name}：任务已取消`);
      }
      this._pendingTaskController = null;
      this.setState({
        controllerResult:
          `new TaskController({ priority: 'background' }) → controller\n` +
          `${signalInfo}\n` +
      ...
      this._addLog('warn', `TaskController 演示失败：${err.name} - ${err.message}`);
    }
  }
  ...
  async _demoPriorityChange() {
    ...
    try {
      ...
      const taskPromise = scheduler.postTask(
        () => 'executed',
        { signal: controller.signal, delay: 3000 },
      );
      ...
      this.setState({
        controllerResult:
          `TaskController.setPriority + TaskSignal.prioritychange 演示\n` +
      ...
      this._addLog('warn', `prioritychange 演示失败：${err.name} - ${err.message}`);
    }
  }
  ...
  _renderCard2() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.taskController ? 'success' : 'error' }, caps.taskController ? 'TaskController ✓' : '不可用'),
        h(Tag, { color: caps.taskSignal ? 'primary' : 'warning' }, caps.taskSignal ? 'TaskSignal ✓' : 'TaskSignal ✗'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
        ...
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('abort() 取消任务', { danger: true, size: 'sm', disabled: !caps.scheduler || !caps.taskController, onClick: () => this._demoTaskController() }),
          this._btn('setPriority 变更', { type: 'primary', size: 'sm', disabled: !caps.scheduler || !caps.taskController || !caps.taskSignal, onClick: () => this._demoPriorityChange() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'TaskController 结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '220px', overflow: 'auto' } },
          h('code', {}, s.controllerResult || '（点击「abort() 取消任务」或「setPriority 变更」）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
        h('pre', { class: 'code-block', style: { maxHeight: '180px', overflow: 'auto' } },
          h('code', {},
`const controller = new TaskController({ priority: 'background' });
...
controller.signal.addEventListener('prioritychange', (e) => {
  console.log(e.previousPriority, '→', controller.signal.priority);
});
controller.setPriority('user-blocking');   // 提升优先级

// 取消任务
setTimeout(() => controller.abort(), 100);
try { await taskPromise; } catch (e) { /* AbortError */ }`)),
        h(Alert, {
          type: 'warning',
  ...
  async _requestIdlePermission() {
    ...
    try {
      ...
      this.setState({
        idleState:
          `IdleDetector.requestPermission() → '${perm}'\n` +
      ...
      this._addLog('warn', `requestPermission 失败：${err.name} - ${err.message}`);
      this.setState({ idleState: `requestPermission 失败：${err.name} - ${err.message}` });
    }
  }
  ...
  async _startIdleDetector() {
    ...
    try {
      ...
      this._addLog('warn', `start 失败：${err.name} - ${err.message}`);
      this.setState({ idleState: `start 失败：${err.name} - ${err.message}` });
    }
  }
  ...
  _stopIdleDetector() {
    if (this._idleDetector) {
      try {
        if (this._idleHandler) this._idleDetector.removeEventListener('change', this._idleHandler);
  ...
  _renderCard3() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.idleDetector ? 'success' : 'error' }, caps.idleDetector ? 'IdleDetector ✓' : '不可用'),
        h(Tag, { color: 'primary' }, 'userState / screenState'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
        ...
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('请求权限', { type: 'primary', size: 'sm', disabled: !caps.idleDetector, onClick: () => this._requestIdlePermission() }),
          this._btn('启动检测', { type: 'primary', size: 'sm', disabled: !caps.idleDetector, onClick: () => this._startIdleDetector() }),
          this._btn('停止检测', { danger: true, size: 'sm', disabled: !caps.idleDetector, onClick: () => this._stopIdleDetector() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'IdleDetector 状态：'),
        h('pre', { class: 'code-block', style: { maxHeight: '220px', overflow: 'auto' } },
          h('code', {}, s.idleState || '（点击「请求权限」→「启动检测」）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
...
// 屏幕锁定 → screenState='locked'`)),
    ...
    try {
      ...
      const observer = new PressureObserver((records) => {
        const lines = records.map((r) =>
          `source=${r.source} state=${r.state} time=${new Date(r.time).toISOString()}`
        );
        for (const line of lines) this._addLog('pressure', 'change → ' + line);
        this.setState({
          pressureState:
            `PressureObserver 回调被触发，收到 ${records.length} 条 PressureRecord\n` +
            records.map((r, i) =>
              `  [${i}] source=${r.source}，state=${r.state}，time=${new Date(r.time).toISOString()}`
            ).join('\n') +
      ...
      this.setState({
        pressureState:
          `new PressureObserver(callback) → observer\n` +
          `observer.observe('cpu') ✓\n` +
      ...
      this._addLog('warn', `启动 PressureObserver 失败：${err.name} - ${err.message}`);
    }
  }
  ...
  _takePressureRecords() {
    ...
    try {
      ...
      this.setState({
        pressureState:
          `observer.takeRecords() → ${records.length} 条 PressureRecord\n${text}\n\n` +
  ...
  _stopPressureObserver() {
    if (this._pressureObserver) {
      try {
        ...
        this._addLog('warn', `disconnect 失败：${err.message}`);
      }
      this._pressureObserver = null;
    } else {
  ...
  _renderCard4() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.pressureObserver ? 'success' : 'error' }, caps.pressureObserver ? 'PressureObserver ✓' : '不可用'),
        h(Tag, { color: 'primary' }, 'cpu / nominal / fair / serious / critical'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'new PressureObserver(callback) 创建观察者；observer.observe(source) 订阅压力源（source = "cpu"，未来可能扩展 "gpu" / "thermals"）；observer.unobserve(source) 取消订阅；observer.disconnect() 断开所有订阅；observer.takeRecords() 取出已积累的 PressureRecord。回调参数 PressureRecord[] 每条含 source / state（"nominal"|"fair"|"serious"|"critical"）/ time / history。用途：根据 CPU 压力动态降低视频码率、降帧、降并发等。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('启动监听 cpu', { type: 'primary', size: 'sm', disabled: !caps.pressureObserver, onClick: () => this._startPressureObserver() }),
          this._btn('takeRecords', { size: 'sm', disabled: !caps.pressureObserver, onClick: () => this._takePressureRecords() }),
        ...
        h('pre', { class: 'code-block', style: { maxHeight: '220px', overflow: 'auto' } },
          h('code', {}, s.pressureState || '（点击「启动监听 cpu」）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
        h('pre', { class: 'code-block', style: { maxHeight: '160px', overflow: 'auto' } },
          h('code', {},
`const observer = new PressureObserver((records) => {
...
observer.disconnect();        // 断开所有`)),
  ...
  _startLoAFObserver() {
      ...
      for (const e of entries) {
        ...
        if (entryType === 'long-animation-frame') {
          const scripts = (e.scripts || []).map((s) =>
            `    • name=${s.name || '(anon)'} invoker=${s.invoker || '-'} invokerType=${s.invokerType || '-'} ` +
            `duration=${s.duration?.toFixed(2)}ms source=${s.sourceURL ? s.sourceURL.split('/').pop() : '-'}`
          ).join('\n');
          const line =
            `LoAF：duration=${e.duration.toFixed(1)}ms blocking=${(e.blockingDuration ?? 0).toFixed(1)}ms ` +
          ...
          this.setState({
            loafState:
              `PerformanceObserver 观察 'long-animation-frame' 收到 ${this._loafEntries.length} 条 LoAF 条目\n` +
              `最近一条：\n  ${line}\n` +
          ...
          this.setState({
            loafState:
              `PerformanceObserver 观察 'longtask'（LoAF 不可用的退化分支）收到 ${this._loafEntries.length} 条\n` +
              `最近一条：\n  ${line}\n\n` +
    ...
    try {
      ...
      this.setState({
        loafState:
          `new PerformanceObserver(cb).observe({ entryTypes: ['long-animation-frame'] }) ✓\n` +
          `说明：当浏览器渲染一帧超过 50ms 时（长帧），产生 PerformanceLongAnimationFrameTiming 条目。\n` +
          `等待中…触发长帧操作（如大量同步计算）后会出现条目。`,
      });
    } catch (err) {
      this._addLog('warn', `observe('long-animation-frame') 失败：${err.name} - ${err.message}，尝试退化到 'longtask'`);
      try {
        ...
        this.setState({
          loafState:
            `当前环境不支持 'long-animation-frame'（${err.message}）\n` +
            `已退化为 PerformanceObserver 观察 'longtask'\n` +
            `说明：longtask 是较旧的 API，只给出总耗时；LoAF 提供更细粒度的 renderStart / blockingDuration / scripts 信息。`,
        });
      } catch (err2) {
        this._addLog('warn', `observe('longtask') 也失败：${err2.name} - ${err2.message}`);
        this.setState({ loafState: `LoAF 与 longtask 均不可用：${err2.name} - ${err2.message}` });
      }
    }
  }
  ...
  _renderCard5() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.perfObserver ? 'success' : 'error' }, caps.perfObserver ? 'PerfObserver ✓' : '不可用'),
        h(Tag, { color: 'primary' }, 'long-animation-frame / longtask'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'PerformanceObserver 观察 entryType: "long-animation-frame" 检测超过 50ms 的长动画帧；PerformanceLongAnimationFrameTiming 含 startTime / duration / renderStart / styleAndLayoutStart / blockingDuration（阻塞主线程时长）/ scripts[]（数组：name / invoker / invokerType / executionStart / duration / sourceURL / sourceFunctionName / sourceCharPosition）。比 Long Task API（longtask，仅总耗时）更细粒度。当前环境若不支持 LoAF 会自动退化为 longtask。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('启动 LoAF 监听', { type: 'primary', size: 'sm', disabled: !caps.perfObserver, onClick: () => this._startLoAFObserver() }),
          this._btn('触发长帧', { danger: true, size: 'sm', disabled: !caps.perfObserver, onClick: () => this._triggerLongFrame() }),
          this._btn('停止监听', { size: 'sm', disabled: !caps.perfObserver, onClick: () => this._stopLoAFObserver() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'LoAF 条目：'),
        h('pre', { class: 'code-block', style: { maxHeight: '240px', overflow: 'auto' } },
          h('code', {}, s.loafState || '（点击「启动 LoAF 监听」→「触发长帧」）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
        h('pre', { class: 'code-block', style: { maxHeight: '180px', overflow: 'auto' } },
          h('code', {},
...
o2.observe({ entryTypes: ['longtask'] });`)),
  ...
  async _measureMemory() {
    ...
    try {
      ...
      this.setState({
        memoryState:
          `performance.measureUserAgentSpecificMemory() → { bytes, breakdown } ✓\n` +
      ...
      this._addLog('warn', `measureMemory 失败：${err.name} - ${err.message}`);
      this.setState({ memoryState: `measureMemory 失败：${err.name} - ${err.message}` });
    }
  }
  ...
  _renderCard6() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.measureMemory ? 'success' : 'error' }, caps.measureMemory ? 'measureMemory ✓' : '不可用'),
        h(Tag, { color: 'primary' }, 'bytes / breakdown'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'performance.measureUserAgentSpecificMemory() → Promise<{ bytes, breakdown }> 估算当前页面 JavaScript 堆内存占用。bytes 为总字节数；breakdown 是 [{ attribution, types, bytes }] 数组，按归属（window / worker / SharedArrayBuffer 等）拆分。返回值为「用户代理特定」估计值，不同浏览器实现不同；需跨域隔离（COOP/COEP）才能统计跨域 iframe / window 的内存。用途：检测内存泄漏、监控页面内存增长。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('测量内存', { type: 'primary', size: 'sm', disabled: !caps.measureMemory, onClick: () => this._measureMemory() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '内存测量结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '220px', overflow: 'auto' } },
          h('code', {}, s.memoryState || '（点击「测量内存」）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
        h('pre', { class: 'code-block', style: { maxHeight: '140px', overflow: 'auto' } },
          h('code', {},
`// 需要页面跨域隔离（COOP/COEP 头）
const result = await performance.measureUserAgentSpecificMemory();
...
// 监控内存增长以检测泄漏：定时调用并记录 bytes 变化`)),
  ...
  async _demoYield() {
    ...
    try {
      ...
      this._addLog('error', `scheduler.yield() 演示失败：${err.name} - ${err.message}`);
    }
  }
  ...
  _renderCard7() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: caps.schedulerYield ? 'success' : 'error' }, caps.schedulerYield ? 'yield ✓' : '不可用'),
        h(Tag, { color: 'primary' }, 'Chrome 129+'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'scheduler.yield() 返回 Promise，让出主线程给更高优先级任务（如用户输入响应），随后在「continuation」中恢复执行。是长任务中段让出、优化 INP（Interaction to Next Paint）的关键 API。vs setTimeout(0)：yield 不会被其他定时器插队；vs requestIdleCallback：yield 立即让出不等空闲。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('运行 yield 演示', { type: 'primary', size: 'sm', disabled: !caps.schedulerYield, onClick: () => this._demoYield() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '180px', overflow: 'auto' } },
          h('code', {}, s.yieldState || '（点击「运行 yield 演示」）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
        h('pre', { class: 'code-block', style: { maxHeight: '160px', overflow: 'auto' } },
          h('code', {},
`// 长任务中段让出，避免阻塞用户交互
...
// vs requestIdleCallback：yield 立即让出，不等空闲`)),
  ...
  render() {
    ...
    return h('div', { class: 'page api-lab-page' },
      h('h2', { class: 'section-title' }, '任务调度与系统压力实验室'),
      h('p', { class: 'fs-sm text-secondary mb-md' },
        '本页演示现代浏览器的系统级 Web API：Prioritized Task Scheduling（任务优先级调度）、TaskController（取消与优先级变更）、Idle Detection（空闲检测）、Compute Pressure（CPU 压力）、Long Animation Frames（长帧）、measureUserAgentSpecificMemory（内存监控）。'),
