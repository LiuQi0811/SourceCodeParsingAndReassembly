// =====================================================================
// RealtimeCommsPage.js —— 实时通信 API 实验室
...
export class RealtimeCommsPage extends Page {
  ...
  initialState() {
    return {
      ...
      rtcState: 'idle',         // RTCPeerConnection 整体状态
      dcState: 'idle',          // RTCDataChannel 状态
      iceState: '—',            // iceConnectionState
      signalingState: '—',      // signalingState
      connState: '—',           // connectionState
      messages: [],             // 聊天消息记录
      chatInput: '',            // 聊天输入框值
      wtSupported: false,       // WebTransport 是否支持
      wtSupportDetail: {},      // 各 WT 子 API 支持情况
      wtState: 'idle',          // WebTransport 连接状态
      statsInfo: '',            // getStats 概要文本
      statsEntries: [],         // 过滤后的统计条目
      dcInfo: [],               // 数据通道属性快照
    };
  }
  ...
  async _startWebRTC() {
    ...
    try {
      ...
      pc1.onicecandidate = (e) => {
        if (e.candidate) {
          ...
          pc2.addIceCandidate(e.candidate).catch(
            (err) => this._addLog('err', `pc2.addIceCandidate 失败：${err.message}`),
          );
        }
      };
      pc2.onicecandidate = (e) => {
        if (e.candidate) {
          ...
          pc1.addIceCandidate(e.candidate).catch(
            (err) => this._addLog('err', `pc1.addIceCandidate 失败：${err.message}`),
          );
        }
      };
      ...
      this._addLog('err', `WebRTC 建连失败：${err.message}`);
      ...
      if (isReceiver) {
        ...
        if (dc.readyState === 'open' && !isBinary) {
          try {
            ...
            this._addLog('err', `pc2 回显失败：${err.message}`);
          }
        }
      } else {
        // pc1 收到 pc2 的回显
  ...
  _sendChat(text) {
    ...
    try {
      ...
      this._addLog('err', `发送失败：${err.message}`);
    }
  }
  ...
  _closePc(key) {
    if (this[key]) { try { this[key].close(); } catch { /* noop */ } this[key] = null; }
  }
  ...
  async _tryWtConnect() {
    ...
    try {
      ...
      this._addLog('err', `WebTransport 连接失败：${err.message}`);
  ...
  async _demoWtStreams() {
    ...
    try {
      ...
      this._addLog('err', `单向流写入失败：${err.message}`);
    }
    // 双向流：读 + 写
    try {
      ...
      this._addLog('err', `双向流创建失败：${err.message}`);
    }
    // 数据报（不可靠传输）
    try {
      ...
      this._addLog('err', `datagrams 访问失败：${err.message}`);
    }
  }
  ...
  _refreshDcInfo() {
    ...
    this._addLog('dc', `已刷新通道属性（readyState=${dc.readyState}）`);
  }
  ...
  _sendBinary() {
    ...
    try {
      ...
      this._addLog('err', `二进制发送失败：${err.message}`);
    }
  }
  ...
  _setupBackpressure() {
    ...
    try {
      ...
      this._addLog('err', `背压设置失败：${err.message}`);
    }
  }
  ...
  async _refreshStats() {
    ...
    try {
      ...
      this._addLog('err', `getStats 失败：${err.message}`);
    }
  }
        ...
        h('div', { class: 'flex flex-col gap-sm' },
          ...
          this.state.messages.length === 0
            ...
            : h('div', { class: 'log-panel' },
              ...this.state.messages.map((m) => h('div', { class: 'log-panel__line' },
      ...
      h(Card, {
        ...
        h('div', { class: 'flex flex-col gap-sm' },
          ...
          h('div', { class: 'flex flex-col gap-xs' },
            ...Object.entries(this.state.wtSupportDetail).map(([name, ok]) => h('div', { class: 'flex items-center gap-sm fs-sm' },
      ...
      h(Card, {
        ...
        h('div', { class: 'flex flex-col gap-sm' },
          ...
          this.state.dcInfo.length === 0
            ...
            : h('div', { class: 'flex flex-col gap-xs' },
              ...this.state.dcInfo.map((item) => h('div', { class: 'api-metric' },
          ...
          this.state.statsEntries.length === 0
            ...
            : h('div', { class: 'flex flex-col gap-xs mt-xs' },
              ...this.state.statsEntries.map((s) => h('div', { class: 'api-metric' },
