...
class _MockBroadcastChannel {
  constructor(name) {
    ...
    this._closed = false;
    if (!_BC_REGISTRY.has(this.name)) _BC_REGISTRY.set(this.name, new Set());
...
child.focus();               // 聚焦子窗`)),
  ...
  async _queryPermission(name) {
    if (typeof navigator === 'undefined' || !navigator.permissions || typeof navigator.permissions.query !== 'function') {
      ...
      this.setState({
        permissionInfo:
          ...
          `查询：navigator.permissions.query({ name: '${name}' })\n` +
    ...
    try {
      this._addLog('info', `navigator.permissions.query({ name: '${name}' })…`);
      const status = await navigator.permissions.query({ name });   // → PermissionStatus
      this.setState({
        permissionInfo:
          `navigator.permissions.query({ name: '${name}' }) → PermissionStatus ✓\n` +
      ...
      this.setState({
        permissionInfo:
          `navigator.permissions.query({ name: '${name}' }) 抛错：${err.name} - ${err.message}\n` +
