  ...
  async _asyncIterate() {
    ...
    try {
      ...
      const stream = new ReadableStream({
        start(controller) {
          const timer = setInterval(() => {
            if (i >= pieces.length) { clearInterval(timer); controller.close(); return; }
  ...
  async _pipeDemo() {
    ...
    try {
      ...
      const source = new ReadableStream({
        start(controller) {
          const timer = setInterval(() => {
            if (i >= words.length) { clearInterval(timer); controller.close(); return; }
  ...
  async _streamDownload() {
    ...
    try {
      ...
      const bodyStream = new ReadableStream({
        start(controller) {
          const timer = setInterval(() => {
            if (idx >= pieces.length) { clearInterval(timer); controller.close(); return; }
