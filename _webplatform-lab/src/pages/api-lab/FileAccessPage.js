    ...
    return [
      ...
      h(Card, {
        ...
        h('div', { class: 'flex flex-col gap-sm' },
          h('p', { class: 'fs-sm text-secondary' },
            'window.showOpenFilePicker / showSaveFilePicker / showDirectoryPicker 返回 FileSystemHandle，可跨会话持有。handle.getFile().text() 读取；createWritable().write().close() 写入；for await...of 遍历目录；getFileHandle/getDirectoryHandle({create:true}) 创建；removeEntry 删除；queryPermission/requestPermission 申请持久权限。'),
          ...
          h('div', { class: 'flex flex-col gap-xs' },
            ...
            h('textarea', { class: 'fsa-save-textarea', rows: 4, onInput: (e) => { this.state.saveContent = e.target.value; } }, s.saveContent),
          ),
          ...
          s.dirName && h('div', { class: 'flex flex-col gap-xs' },
            ...
            h('div', { class: 'log-panel' },
              s.dirEntries.length === 0
                ...
                : s.dirEntries.map((e) => h('div', { class: 'log-panel__line' },
      ...
      h(Card, {
        ...
        h('div', { class: 'flex flex-col gap-sm' },
          ...
          h('div', { class: 'flex items-center gap-sm flex-wrap' },
            h('input', { type: 'file', accept: 'image/*,text/*,.json,.csv,.txt', multiple: true, onChange: (e) => this._onFilePick(e) }),
      ...
      h(Card, {
        ...
        h('div', { class: 'flex flex-col gap-sm' },
          ...
          h('div', { class: 'flex flex-col gap-xs' },
            ...
            h('textarea', { class: 'encode-textarea', rows: 3, onInput: (e) => { this.state.encodeInput = e.target.value; } }, s.encodeInput),
          ),
          h('div', { class: 'flex items-center gap-sm flex-wrap' },
        ...
        h('div', { class: 'flex flex-col gap-sm' },
          ...
          !(s.hasBlob && s.hasUrl)
            ...
            : h('div', { class: 'flex flex-col gap-xs' },
                ...
                h('textarea', { class: 'blob-textarea', rows: 4, onInput: (e) => { this.state.blobText = e.target.value; } }, s.blobText),
                h('div', { class: 'flex items-center gap-sm flex-wrap' },
