...
function getMenuForPath(path) {
  ...
  if (path.startsWith('/api-lab')) {
    return [
      ...
      { path: '/api-lab/storage-access', label: '存储多源', icon: 'FilterWhere' },
...
export class Sidebar extends Component {
  ...
  render() {
    ...
    if (items.length === 0) return h('aside'); // 占位，保持网格稳定
    ...
    return h('aside', { class: classes },
      ...items.map((item) => {
        ...
        return h('div', {
          ...
          h('span', { class: 'app-sidebar__item-icon' }, item.icon),
