// =====================================================================
// CSSConditionalLevel5Page.js —— CSS 条件规则 Level 5 与未来提案 实验室
...
export class CSSConditionalLevel5Page extends Page {
  ...
  initialState() {
    return {
      logs: [],
      capsSummary: '',
      whenInfo: '',         // Card 1：@when / @else
      customMediaInfo: '',  // Card 2：@custom-media
      ifFuncInfo: '',       // Card 3：if() 值函数
      customSelInfo: '',    // Card 4：Custom Selectors
      applyInfo: '',        // Card 5：@apply 历史
      timelineInfo: '',     // Card 6：演进时间线
    };
  }
  ...
  componentDidMount() {
    ...
    if (!f.when) this._addLog('warn', '@when 不可用（CSS Conditional Level 5 提案，无浏览器支持）');
    if (!f.customMedia) this._addLog('warn', '@custom-media 不可用（CSS Media Queries Level 5 提案，无浏览器支持）');
    if (!f.ifFunc) this._addLog('warn', 'if() 值函数不可用（CSS Values Level 5 提案，无浏览器支持）');
  }
  ...
  _demoWhenElse() {
    const f = this._flags();
    try {
      this.setState({ whenInfo:
        '===== @when / @else 通用条件规则（CSS Conditional Level 5 提案）=====\n\n' +
      ...
      this._addLog('warn', `@when 演示失败：${err.name} - ${err.message}`);
    }
  }
  ...
  _renderCard1() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['@when', f.when], ['@else', f.elseRule]]),
        h(Tag, { color: 'primary' }, '提案阶段'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '@when 是 CSS Conditional Level 5 提案的通用条件规则，统一 media/supports/style 三类条件查询并支持链式 @else：@when media(...) and supports(...) { } @else when style(...) { } @else { }。替代当前 @media/@supports/@container 分散且无法 else 的繁琐嵌套。条件类型：media()（媒体查询）/ supports()（特性查询）/ style()（自定义属性值查询），可用 and/or/not 组合。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('展示 @when/@else 用法', { type: 'primary', size: 'sm', onClick: () => this._demoWhenElse() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '460px', overflow: 'auto' } },
          h('code', {}, s.whenInfo || '（点击按钮查看 @when/@else 完整用法）')),
        h(Alert, {
          type: 'warning',
  ...
  _demoCustomMedia() {
    const f = this._flags();
    try {
      this.setState({ customMediaInfo:
        '===== @custom-media 自定义媒体查询别名（CSS Media Queries Level 5 提案）=====\n\n' +
      ...
      this._addLog('warn', `@custom-media 演示失败：${err.name} - ${err.message}`);
    }
  }
  ...
  _renderCard2() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['@custom-media', f.customMedia]]),
        h(Tag, { color: 'primary' }, 'Media Queries L5'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '@custom-media 让开发者命名复用复杂媒体查询表达式：@custom-media --tablet (min-width: 768px); 之后 @media (--tablet) { } 即可引用。支持组合（and/or/not）与嵌套别名。命名必须以 -- 开头（与 CSS 自定义属性一致）。替代 Sass 变量在媒体查询中的用法，提供原生语义化方案。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('展示 @custom-media 用法', { type: 'primary', size: 'sm', onClick: () => this._demoCustomMedia() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '420px', overflow: 'auto' } },
          h('code', {}, s.customMediaInfo || '（点击按钮查看 @custom-media 完整用法）')),
      ],
    });
    this.registerChild(card);
    return card.render();
  ...
  _demoIfFunc() {
    ...
    try {
      ...
      this.setState({ ifFuncInfo:
        '===== CSS if() 值函数（CSS Values Level 5 提案）=====\n\n' +
      ...
      this._addLog('warn', `if() 演示失败：${err.name} - ${err.message}`);
    }
  }
  ...
  _renderCard3() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['if()', f.ifFunc], ['if(media)', f.ifMedia]]),
        h(Tag, { color: 'primary' }, 'Values L5'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'if() 是 CSS Values Level 5 提案的值级条件函数：if(<condition>: <true-value>; else: <false-value>) 在单个属性值中写条件，避免用 @media 块包裹整个规则。条件类型：media()（媒体查询）/ style()（自定义属性值）/ supports()（特性查询）。支持嵌套 if()。与 @when 互补：@when 是规则级条件（整个声明块），if() 是值级条件（单个属性值）。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('展示 if() 用法', { type: 'primary', size: 'sm', onClick: () => this._demoIfFunc() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '460px', overflow: 'auto' } },
          h('code', {}, s.ifFuncInfo || '（点击按钮查看 if() 完整用法）')),
      ],
    });
    this.registerChild(card);
    return card.render();
  ...
  _demoCustomSel() {
    const f = this._flags();
    try {
      this.setState({ customSelInfo:
        '===== CSS Custom Selectors :--name（css-extensions 提案）=====\n\n' +
        '动机：复杂选择器重复书写，语义不清晰：\n' +
      ...
      this._addLog('warn', `@custom-selector 演示失败：${err.name} - ${err.message}`);
    }
  }
  ...
  _renderCard4() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['@custom-selector', f.customSel]]),
        h(Tag, { color: 'primary' }, 'css-extensions'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '@custom-selector 让开发者命名复用选择器列表：@custom-selector :--heading h1, h2, h3, h4, h5, h6; 之后 :--heading { } 即可引用。命名必须以 :-- 开头（双连字符）。可在任何选择器位置使用并组合。与 :is()/:where() 互补——后者内联选择器列表无命名复用，@custom-selector 提供跨文件命名别名。替代 Sass @mixin 在选择器层面的用法。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('展示 @custom-selector 用法', { type: 'primary', size: 'sm', onClick: () => this._demoCustomSel() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '420px', overflow: 'auto' } },
          h('code', {}, s.customSelInfo || '（点击按钮查看 @custom-selector 完整用法）')),
      ],
    });
    this.registerChild(card);
    return card.render();
  ...
  _demoApplyHistory() {
    ...
    try {
      ...
      this._addLog('warn', `@apply 演示失败：${err.name} - ${err.message}`);
    }
  }
  ...
  _renderCard5() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['@apply', f.apply]]),
        h(Tag, { color: 'error' }, '已弃用'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          '@apply 曾是 CSS Custom Properties Level 1 草案的特性，允许把一组声明打包到自定义属性中复用（@apply --button-style;）。Chrome 曾实现后移除。弃用原因：与自定义属性语义冲突、可维护性差、与 cascade 交互复杂。替代方案：CSS @function mixins（CSSWG 2024 新提案，支持参数）/ @property 类型化自定义属性 / @layer 层叠层 + 组件类。当前生产环境用 Sass @mixin 仍是最佳实践。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('展示 @apply 历史与替代方案', { type: 'primary', size: 'sm', onClick: () => this._demoApplyHistory() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '500px', overflow: 'auto' } },
          h('code', {}, s.applyInfo || '（点击按钮查看 @apply 历史与替代方案对比）')),
      ],
    });
    this.registerChild(card);
    return card.render();
  ...
  _demoTimeline() {
    const f = this._flags();
    try {
      this.setState({ timelineInfo:
        '===== CSS 条件规则演进时间线（Level 3 → Level 5）=====\n\n' +
        '===== Level 3（已稳定，全浏览器支持）=====\n' +
      ...
      this._addLog('warn', `时间线演示失败：${err.name} - ${err.message}`);
    }
  }
  ...
  _renderCard6() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['@media', f.media], ['@supports', f.supportsRule], ['@container', f.containerStyle]]),
        h(Tag, { color: 'primary' }, '演进对比'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'CSS 条件规则从 Level 3 的 @media/@supports，到 Level 4 的 prefers-* 媒体特性 + @supports selector()/font-tech()，再到 Level 4.5 的 @container 容器查询（已稳定），最终演进到 Level 5 的 @when/@custom-media/if() 统一条件体系（提案阶段）。本卡片对比各 Level 的能力差异与浏览器支持情况，并提供渐进增强策略：基础样式用 Level 3、增强用 Level 4、未来用 Level 5。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('展示演进时间线', { type: 'primary', size: 'sm', onClick: () => this._demoTimeline() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '演示结果：'),
        h('pre', { class: 'code-block', style: { maxHeight: '540px', overflow: 'auto' } },
          h('code', {}, s.timelineInfo || '（点击按钮查看完整演进时间线）')),
      ],
    });
    this.registerChild(card);
    return card.render();
