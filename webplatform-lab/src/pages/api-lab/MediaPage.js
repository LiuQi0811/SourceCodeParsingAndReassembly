// MediaPage.js —— 多媒体 API 实验室：Geolocation / Clipboard / Drag&Drop / Web Audio / Notification / File
// 演示 MDN：navigator.geolocation、Clipboard API、DragEvent、AudioContext、AnalyserNode、Notification、FileReader
import { Page } from '../../core/Component.js';
import { h, formatTime, copyText } from '../../core/utils.js';
...
export class MediaPage extends Page {
  initialState() {
    return {
      ...
      audioRunning: false,
      audioFreq: 0,
      notifPermission: Notification?.permission || 'unsupported',
      events: [],
    };
  }
  ...
  async _readClipboard() {
    try {
      ...
      this._addLog('error', `读取失败：${err.message}`);
    }
  }
  ...
  _setupDnd(target) {
    ...
    this.on(target, 'drop', (e) => {
      ...
      if (textFile) {
        ...
        reader.onload = () => {
          this._addLog('info', `FileReader 读取 ${textFile.name}：${reader.result.slice(0, 80)}...`);
        };
  ...
  _toggleAudio() {
    ...
    try {
      ...
      const step = () => {
        ...
        this.setState({ audioFreq: Math.round(freq) });
      ...
      message.error(`音频失败：${err.message}`);
  ...
  async _requestNotif() {
    if (!('Notification' in window)) {
      message.warning('当前浏览器不支持 Notification');
      return;
    }
  ...
  renderPage() {
    ...
    return [
        ...
        h(Card, { title: 'Geolocation 地理位置' },
          h('div', { class: 'flex flex-col gap-sm' },
            ...
            this.state.geo && h('div', { class: 'fs-sm' },
              h('div', {}, h('strong', {}, '纬度：'), this.state.geo.latitude.toFixed(6)),
              h('div', {}, h('strong', {}, '经度：'), this.state.geo.longitude.toFixed(6)),
              h('div', {}, h('strong', {}, '精度：'), `${this.state.geo.accuracy.toFixed(0)}m`),
      ...
      h(Card, { title: 'Drag & Drop 拖放 + FileReader' },
        ...
        this.state.droppedFiles.length > 0 && h('div', { class: 'mt-sm' },
          ...
          h('div', { class: 'log-panel' },
            ...this.state.droppedFiles.map((f, i) => h('div', { class: 'log-panel__line' },
      ...
      // Web Audio
      h(Card, { title: 'Web Audio 振荡器 + 频谱分析', extra: h(Tag, { color: this.state.audioRunning ? 'success' : 'default' }, this.state.audioRunning ? `峰值 ${this.state.audioFreq}Hz` : '已停止') },
        h('div', { class: 'flex flex-col gap-sm' },
          h('div', { class: 'flex gap-sm' },
      ...
      h('div', { class: 'log-panel' },
        ...this.state.events.map((log) => h('div', { class: 'log-panel__line' },
