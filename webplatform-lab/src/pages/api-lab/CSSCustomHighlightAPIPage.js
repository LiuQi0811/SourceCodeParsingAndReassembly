...
//      CSS.highlights.clear() 清空，CSS.highlights.entries()/keys()/values() 迭代
...
//      highlight.priority = 1;  // 数值越大优先级越高，渲染在上层
...
export class CSSCustomHighlightAPIPage extends Page {
  ...
  componentDidMount() {
    ...
    if (!f.priority) this._addLog('warn', 'priority 属性不可用（Chrome 105+/Safari 17.2+）');
  ...
  _runBasicsDemo() {
    ...
    const info = [
      ...
      '  CSS.highlights.entries();                   // [name, highlight] 迭代',
      '  CSS.highlights.keys();                       // name 迭代',
      '  CSS.highlights.values();                     // highlight 迭代',
  ...
  _runPriorityDemo() {
    ...
    const info = [
      ...
      '  bobHl.priority = 1;    // 同 priority 按注册顺序',
      '  CSS.highlights.set("bob-cursor", bobHl);',
      '',
      '  // 当前用户提升自己的 priority',
      '  if (currentUser === "alice") aliceHl.priority = 5;',
      '',
      '【vs z-index】',
  ...
  _renderCard3() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        ...this._caps([['priority', f.priority]]),
        h(Tag, { color: 'primary' }, '叠层'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'highlight.priority = <number> 控制多重高亮重叠时的叠层顺序（数值越大越上层，默认 0 可为负）。相同 priority 按注册顺序。各属性独立覆盖：A 设 color，B 设 background，即使 A priority 高 B 的 background 仍生效。协同编辑场景：当前用户选区 priority 高于他人。修改 priority 立即生效无需重新注册。Chrome 105+/Safari 17.2+ 支持。',
        ),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
  ...
  _runPatternsDemo() {
    ...
    const info = [
      ...
      '  progressHl.priority = -1;  // 最低优先级，不遮挡其他高亮',
      '  CSS.highlights.set("progress", progressHl);',
      '',
