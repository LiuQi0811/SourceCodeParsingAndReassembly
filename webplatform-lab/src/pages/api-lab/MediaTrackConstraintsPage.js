...
export class MediaTrackConstraintsPage extends Page {
  ...
  componentDidMount() {
    ...
    if (!f.userMedia) this._addLog('warn', 'navigator.mediaDevices.getUserMedia 不可用（需安全上下文 https/localhost + 真实浏览器）');
    if (!f.getCapabilities) this._addLog('warn', 'MediaStreamTrack.prototype.getCapabilities 不可用（需 Chromium 系浏览器）');
    if (!f.applyConstraints) this._addLog('warn', 'MediaStreamTrack.prototype.applyConstraints 不可用');
    if (!f.onoverconstrained) this._addLog('warn', 'track.onoverconstrained 不可用（部分浏览器仅支持 overconstrained 事件 addEventListener）');
    if (f.userMedia) this._addLog('info', 'getUserMedia 可用，但本页不真正调用（避免触发权限弹窗）');
  ...
  _stopMicStream() {
    if (this._micStream) {
      try { this._micStream.getTracks().forEach((t) => t.stop()); } catch { /* noop */ }
      this._micStream = null;
      this._micTrack = null;
    } else if (this._micTrack) {
      try { this._micTrack.stop(); } catch { /* noop */ }
      this._micTrack = null;
    }
  }
  ...
  _runOverviewDemo() {
    ...
    if (f.supportedConstraints) {
      try { supported = navigator.mediaDevices.getSupportedConstraints(); }
      catch (err) { this._addLog('warn', `getSupportedConstraints 失败：${err.name} - ${err.message}`); }
    }
    ...
    const info = [
      ...
      "  track.onoverconstrained = (e) => console.warn('overconstrained', e);",
      '',
    ...
    this._addLog(f.supportedConstraints ? 'info' : 'warn',
      `getSupportedConstraints ${f.supportedConstraints ? '返回 ' + supportedKeys.length + ' 项' : '不可用'}`);
  }
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'MediaTrackConstraints 是 getUserMedia({ audio: {...} }) 中对单条 track 的约束对象，与 MediaTrackCapabilities（设备能力，getCapabilities 返回 min/max）和 MediaTrackSettings（实际选中值，getSettings 返回）三者协同。约束模型支持 ideal（首选，无法满足时降级）/ exact（强制，不满足抛 OverconstrainedError）/ min/max（数值型边界）四种形式。布尔型约束（echoCancellation/noiseSuppression/autoGainControl/voiceIsolation）仅支持 ideal/exact。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('检测能力 + getSupportedConstraints', { type: 'primary', size: 'sm', onClick: () => this._runOverviewDemo() }),
          this._btn('查看约束模型说明', { size: 'sm', onClick: () => this._runOverviewDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '约束模型与能力检测：'),
        h('pre', { class: 'code-block', style: { maxHeight: '320px', overflow: 'auto' } },
          h('code', {}, s.overviewInfo || '（点击「检测能力 + getSupportedConstraints」）')),
...
if (sc.voiceIsolation) {
  ...
  _runAecDemo() {
    ...
    this._addLog(f.userMedia ? 'info' : 'warn',
      `echoCancellation 演示：getUserMedia ${f.userMedia ? '可用（本页不真正调用）' : '不可用'}`);
  }
  ...
  _renderCard2() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: f.userMedia ? 'success' : 'error' }, f.userMedia ? '✓ getUserMedia' : '不可用'),
        h(Tag, { color: 'primary' }, 'AEC3 / NLMS'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'echoCancellation 控制声学回声消除（AEC）算法开关。扬声器播放对方声音 → 麦克风采集到 → 不消除则对方听到自己回声。WebRTC 内置 AEC3（Chrome，分区块自适应滤波）/ 系统 AEC（Safari，CoreAudio）。默认 true，免提通话必备。关闭后裸音频仅适合本地录音（不通话）。getCapabilities().echoCancellation 返回 [true, false] 表示两端可选。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('AEC 原理与用法', { type: 'primary', size: 'sm', onClick: () => this._runAecDemo() }),
          this._btn('查看能力检测代码', { size: 'sm', onClick: () => this._runAecDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'echoCancellation 演示：'),
        h('pre', { class: 'code-block', style: { maxHeight: '300px', overflow: 'auto' } },
          h('code', {}, s.aecInfo || '（点击「AEC 原理与用法」）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
...
const rawStream = await navigator.mediaDevices.getUserMedia({
  ...
  _runNsDemo() {
    ...
    this._addLog(f.userMedia ? 'info' : 'warn',
      `noiseSuppression 演示：${f.userMedia ? '已展示用法' : 'getUserMedia 不可用'}`);
  }
  ...
  _renderCard3() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: f.userMedia ? 'success' : 'error' }, f.userMedia ? '✓ NS 可用' : '不可用'),
        h(Tag, { color: 'primary' }, 'RNNoise / Wiener'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'noiseSuppression 控制噪声抑制（NS）算法开关。WebRTC 内置 NS（噪声估计 + 维纳滤波），Mozilla RNNoise 用 RNN（GRU）实时降噪，对非稳态噪声效果更优。默认 true，通话场景抑制背景噪声。录音场景（音乐录制、现场采访、ASR 训练数据）关闭 NS 保留细节，后期可软件降噪（Audacity / RNNoise WASM + AudioWorklet）。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('NS 原理与用法', { type: 'primary', size: 'sm', onClick: () => this._runNsDemo() }),
          this._btn('RNNoise WASM 方案', { size: 'sm', onClick: () => this._runNsDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'noiseSuppression 演示：'),
        h('pre', { class: 'code-block', style: { maxHeight: '320px', overflow: 'auto' } },
          h('code', {}, s.nsInfo || '（点击「NS 原理与用法」）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
        h('pre', { class: 'code-block', style: { maxHeight: '160px', overflow: 'auto' } },
          h('code', {},
`// 录音场景：关闭 NS/AGC/AEC 保留原始音频
const rawStream = await navigator.mediaDevices.getUserMedia({
  audio: {
    noiseSuppression: false,
...
console.log(track.getSettings().noiseSuppression); // false`)),
  ...
  _runAgcDemo() {
    ...
    const info = [
      ...
      '    output: (chunk, meta) => sendChunk(chunk, meta),',
    ...
    this._addLog(f.userMedia ? 'info' : 'warn',
      `autoGainControl 演示：${f.userMedia ? '已展示 WebCodecs 协同' : 'getUserMedia 不可用'}`);
  }
  ...
  _renderCard4() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: f.userMedia ? 'success' : 'error' }, f.userMedia ? '✓ AGC 可用' : '不可用'),
        h(Tag, { color: 'primary' }, 'WebCodecs 协同'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'autoGainControl 控制自动增益（AGC）算法开关。WebRTC AGC 基于 RMS 估计 + 增益曲线（analog 调硬件 / digital 软件乘系数），自动拉平音量。默认 true，通话场景使对方听感稳定。音乐录制场景关闭 AGC 保留动态范围。关闭所有处理后可用 MediaStreamTrackProcessor 取原始 AudioData 帧，配合 WebCodecs AudioEncoder 编码 Opus/AAC/FLAC（需 Chrome 94+）。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('AGC 原理与用法', { type: 'primary', size: 'sm', onClick: () => this._runAgcDemo() }),
          this._btn('WebCodecs 协同方案', { size: 'sm', onClick: () => this._runAgcDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'autoGainControl 演示：'),
        h('pre', { class: 'code-block', style: { maxHeight: '340px', overflow: 'auto' } },
          h('code', {}, s.agcInfo || '（点击「AGC 原理与用法」）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
        h('pre', { class: 'code-block', style: { maxHeight: '180px', overflow: 'auto' } },
          h('code', {},
`// 关闭 AGC/NS/AEC 取原始音频 + WebCodecs 编码
const rawStream = await navigator.mediaDevices.getUserMedia({
...
while (true) {
  ...
  _runChannelDemo() {
    ...
    this._addLog(f.getSettings ? 'info' : 'warn',
      `channelCount/sampleRate/sampleSize 演示：getSettings ${f.getSettings ? '可用' : '不可用'}`);
  }
  ...
  _renderCard5() {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'channelCount（声道数 1/2/多声道）、sampleRate（采样率 8000/16000/44100/48000）、sampleSize（位深 16/24/32）是数值型约束，支持 ideal/exact/min/max。getSettings() 返回的实际值可能与约束值不同（设备不支持时浏览器降级或 resample）。sampleSize 在 Chrome 已废弃。getCapabilities() 返回 { min, max } 边界。蓝牙耳机受 HFP 协议限制仅支持 8/16kHz 单声道。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('声道/采样率/位深用法', { type: 'primary', size: 'sm', onClick: () => this._runChannelDemo() }),
          this._btn('能力矩阵', { size: 'sm', onClick: () => this._runChannelDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'channelCount/sampleRate/sampleSize 演示：'),
        h('pre', { class: 'code-block', style: { maxHeight: '340px', overflow: 'auto' } },
          h('code', {}, s.channelInfo || '（点击「声道/采样率/位深用法」）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
        h('pre', { class: 'code-block', style: { maxHeight: '160px', overflow: 'auto' } },
          h('code', {},
`// 通话：单声道 16kHz；音乐：立体声 48kHz
...
// 注意：sampleSize 在 Chrome 已废弃，settings.sampleSize 多为 undefined`)),
  ...
  _runLatencyDemo() {
    ...
    this._addLog(f.supportedConstraints ? 'info' : 'warn',
      `latency/voiceIsolation 演示：voiceIsolation 约束 ${scVoice ? '识别' : '不识别（需 Chrome 126+）'}`);
  }
  ...
  _renderCard6() {
    ...
    const card = new Card({
      ...
      extra: h('div', { class: 'flex gap-xs' },
        h(Tag, { color: f.supportedConstraints ? 'success' : 'error' }, f.supportedConstraints ? '✓ getSupportedConstraints' : '不可用'),
        h(Tag, { color: 'primary' }, 'Chrome 126+ voiceIsolation'),
      ),
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'latency 控制音频处理管线目标延迟（秒，0.01 低延迟直播 / 0.2 省电），仅 Chrome 支持。voiceIsolation 是 Chrome 126+ 引入的 AI 人声隔离约束，端侧 ML 模型分离人声与所有非人声（含他人声、乐器），比 noiseSuppression 更激进。getSupportedConstraints().voiceIsolation 检测浏览器是否识别。Safari/Firefox 完全不支持两者。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('latency/voiceIsolation 用法', { type: 'primary', size: 'sm', onClick: () => this._runLatencyDemo() }),
          this._btn('浏览器支持矩阵', { size: 'sm', onClick: () => this._runLatencyDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'latency 与 voiceIsolation 演示：'),
        h('pre', { class: 'code-block', style: { maxHeight: '340px', overflow: 'auto' } },
          h('code', {}, s.latencyInfo || '（点击「latency/voiceIsolation 用法」）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
        h('pre', { class: 'code-block', style: { maxHeight: '150px', overflow: 'auto' } },
          h('code', {},
`// 低延迟直播 + AI 人声隔离（Chrome 126+）
const stream = await navigator.mediaDevices.getUserMedia({
  audio: {
...
console.log(track.getSettings().voiceIsolation); // true / undefined`)),
  ...
  _runApplyDemo() {
    ...
    const info = [
      ...
      '  - track.onoverconstrained = (event) => {}（部分浏览器）',
      ...
      "  track.addEventListener('overconstrained', (event) => {",
    ...
    this._addLog(f.applyConstraints ? 'info' : 'warn',
      `applyConstraints 演示：${f.applyConstraints ? '可用' : '不可用'}，onoverconstrained ${f.onoverconstrained ? '支持' : '不支持'}`);
  }
  ...
  _renderCard7() {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'track.applyConstraints(constraints) 运行时修改约束，无需重新 getUserMedia（避免权限弹窗 + track 重建），适合通话中切换 AEC/NS/AGC。失败时抛 OverconstrainedError（constraint 字段说明哪个约束无法满足）。track.onoverconstrained 事件监听约束无法满足（Safari/Firefox 不触发，需 try/catch Promise）。与 RTCRtpSender.setParameters() 正交：前者调源采集，后者调 RTP 编码。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('applyConstraints 用法', { type: 'primary', size: 'sm', onClick: () => this._runApplyDemo() }),
          this._btn('overconstrained 事件', { size: 'sm', onClick: () => this._runApplyDemo() }),
          this._btn('vs setParameters', { size: 'sm', onClick: () => this._runApplyDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, 'applyConstraints 演示：'),
        h('pre', { class: 'code-block', style: { maxHeight: '340px', overflow: 'auto' } },
          h('code', {}, s.applyInfo || '（点击「applyConstraints 用法」）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
        h('pre', { class: 'code-block', style: { maxHeight: '180px', overflow: 'auto' } },
          h('code', {},
`// 运行时切换 AEC + 监听 overconstrained
const track = stream.getAudioTracks()[0];
track.addEventListener('overconstrained', (e) => {
...
try {
  ...
  _runPatternDemo() {
    ...
    this._addLog(f.userMedia ? 'info' : 'warn',
      `实战模式演示：${f.userMedia ? '已展示通话/录音配置矩阵' : 'getUserMedia 不可用，仅文档'}`);
  }
  ...
  _renderCard8() {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'WebRTC 通话约束调优矩阵（普通/高清/演讲/嘈杂/低延迟/蓝牙场景）。录音 vs 通话差异化配置：录音关闭所有处理保留原始音频，通话开启 AEC/NS/AGC/voiceIsolation。9 大陷阱：移动端默认值差异、Safari 部分约束不支持、getCapabilities() 不完整（Firefox 完全不支持）、sampleRate 被忽略、sampleSize 已废弃、exact 过严、onoverconstrained 不触发、蓝牙 HFP 限制、applyConstraints 瞬时中断。降级方案 + 与 RTCRtpSender.setParameters() 区别。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
          this._btn('调优矩阵 + 陷阱清单', { type: 'primary', size: 'sm', onClick: () => this._runPatternDemo() }),
          this._btn('降级方案', { size: 'sm', onClick: () => this._runPatternDemo() }),
          this._btn('vs setParameters', { size: 'sm', onClick: () => this._runPatternDemo() }),
        ),
        h('div', { class: 'fs-sm text-secondary' }, '实战模式与陷阱：'),
        h('pre', { class: 'code-block', style: { maxHeight: '380px', overflow: 'auto' } },
          h('code', {}, s.patternInfo || '（点击「调优矩阵 + 陷阱清单」）')),
        h('div', { class: 'fs-sm text-secondary mt-sm' }, '参考用法：'),
        h('pre', { class: 'code-block', style: { maxHeight: '180px', overflow: 'auto' } },
          h('code', {},
`// 完整通话约束配置（能力检测 + 渐进增强）
...
const stream = await navigator.mediaDevices.getUserMedia({ audio: constraints });
const track = stream.getAudioTracks()[0];
track.addEventListener('overconstrained', (e) => console.warn(e.constraint));
console.log(track.getSettings());  // 验证实际值
// 注意：与 sender.setParameters() 正交，前者调源，后者调 RTP 编码`)),
  ...
  render() {
    ...
    return h('div', { class: 'page api-lab-page' },
      h('h2', { class: 'section-title' }, 'MediaTrackConstraints 深度实验室'),
      h('p', { class: 'fs-sm text-secondary mb-md' },
        '本页演示 MediaTrackConstraints 约束模型（ideal/exact/min/max）与音频约束深潜：echoCancellation 回声消除（AEC）、noiseSuppression 降噪（NS/RNNoise）、autoGainControl 自动增益（AGC）、channelCount/sampleRate/sampleSize 声道/采样率/位深、latency 低延迟、voiceIsolation Chrome 126+ AI 人声隔离、applyConstraints 运行时动态切换、实战与陷阱（WebRTC 调优矩阵/录音 vs 通话差异化/降级方案/与 RTCRtpSender.setParameters 区别）。所有 API 调用前做 typeof 能力检测，jsdom 不可用时仅记日志说明。'),
