// =====================================================================
// DOMTraversalPage.js —— DOM 遍历 / 事件系统 / 调度对比 / CSS Typed OM 实验室
// 演示 MDN：
...
export class DOMTraversalPage extends Page {
  initialState() {
    return {
      ...
      fragmentPerf: null,        // { count, directMs, fragMs, ratio }
      rangeExtractResult: '',
  ...
  _traverseLiOnly() {
    ...
    try {
      ...
      const filter = { acceptNode(node) { return node.tagName === 'LI' ? NodeFilter.FILTER_ACCEPT : NodeFilter.FILTER_SKIP; } };
  ...
  _testTypedOM() {
    ...
    try {
      ...
      el.attributeStyleMap.set('font-size', CSS.px(24));
      el.attributeStyleMap.set('width', CSS.percent(80));
      const fs = el.attributeStyleMap.get('font-size');
  ...
  _setupNormalListeners() {
    ...
    const mk = (label) => (e) => {
      this._addLog('event',
        ...
        + `target=${e.target.className || '(none)'} | currentTarget=${e.currentTarget.className || '(none)'} | bubbles=${e.bubbles}`);
    };
  ...
  _setupCaptureListeners() {
    ...
    const mk = (label) => (e) => {
      this._addLog('event',
        ...
        + `target=${e.target.className || '(none)'} | currentTarget=${e.currentTarget.className || '(none)'}`);
    };
  ...
  _dispatchCustomEvent() {
    ...
    const onCustom = (e) => {
      this._addLog('event',
        `[CustomEvent] type=${e.type} | detail=${JSON.stringify(e.detail)} | `
        + `bubbles=${e.bubbles} | composed=${e.composed} | cancelable=${e.cancelable}`);
    };
    ...
    return [
      ...
      h(Card, {
        ...
        h('div', { class: 'flex flex-col gap-sm' },
          ...
          s.rangeExtractResult && h('div', { class: 'fs-sm' }, s.rangeExtractResult),
        ),
      ),
      ...
      h(Card, {
        ...
        h('div', { class: 'flex flex-col gap-sm' },
          ...
          s.typedOMInfo && h('pre', { class: 'code-block' }, s.typedOMInfo),
          h('div', { class: 'fs-sm text-secondary mt-xs' },
          ...
          s.highlightInfo && h('pre', { class: 'code-block' }, s.highlightInfo),
        ),
      ),
