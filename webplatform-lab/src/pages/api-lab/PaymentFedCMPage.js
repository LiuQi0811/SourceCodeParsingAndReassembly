...
export class PaymentFedCMPage extends Page {
  ...
  _buildPaymentRequest() {
    ...
    try {
      ...
      this._addLog('warn', `构造 PaymentRequest 失败：${err.name} - ${err.message}`);
    }
  }
  ...
  async _canMakePayment() {
    ...
    try {
      ...
      this._pendingPromise = null;
      this._addLog('warn', `canMakePayment 失败：${err.name} - ${err.message}`);
      this.setState({ paymentRequestInfo: `canMakePayment 失败：${err.name} - ${err.message}` });
    }
  }
  ...
  async _demoShow() {
    ...
    try {
      ...
      catch (e) { this._addLog('warn', `complete 失败：${e.message}`); }
    } catch (err) {
      this._pendingPromise = null;
      this._addLog('warn', `show() 失败：${err.name} - ${err.message}（jsdom / 无支付处理程序 / 无用户手势时属正常）`);
      this.setState({ paymentResponseInfo:
        `show() 调用失败：${err.name} - ${err.message}\n\n` +
        '常见失败原因：\n' +
