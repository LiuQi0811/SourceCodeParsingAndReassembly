...
//   2. Web Locks —— navigator.locks.request(name, {mode, ifAvailable, steal, signal}, cb)、
...
`const root = await navigator.storage.getDirectory();
const fh = await root.getFileHandle('demo.txt', { create: true });
const w = await fh.createWritable();
await w.write('hello OPFS'); await w.close();
await (await fh.getFile()).text();     // 读取
await root.removeEntry('demo.txt');    // 删除
// Worker 同步：const sh = await fh.createSyncAccessHandle();`),
  ...
  _locksExclusiveHold() {
    ...
    if (this._locksAbort) opts.signal = this._locksAbort.signal;
  ...
  _renderLocksCard() {
    ...
    return h(Card, {
      ...
      h('p', { class: 'fs-sm text-secondary' },
        'navigator.locks.request(name, {mode, ifAvailable, steal, signal}, cb) 协调同源跨标签页锁。' +
      ...
      h('pre', { class: 'code-block mt-md' },
`await navigator.locks.request('my-lock', { mode: 'exclusive' }, async (lock) => {
...
const ok = await navigator.storage.persist();  // 请求持久化`),
  ...
  async _idbIndexGetByEmail() {
    ...
    try {
      ...
      if (!user) {
        ...
        this._addLog('pull',
          `index('byEmail').get('bob@demo.dev') → #${user.id} ${user.name}, age=${user.age}`);
      }
    } catch (err) {
  ...
  async _idbKeyRangeAge() {
    ...
    try {
      ...
      this._addLog('pull',
        `index('byAge').getAll(IDBKeyRange.bound(20,30)) → ${result.length} 条：` +
...
// 事务事件：tx.oncomplete / onerror / onabort`),
  ...
  async _cacheDemo() {
    ...
    try {
      ...
      this._addLog('pull', `cache.keys() → ${keys.length} 条 Request`);
      ...
      this._addLog('info', `删除后 cache.keys() → ${keys2.length} 条`);
    } catch (err) {
    ...
    return h(Card, {
      ...
      s.cookieList.length > 0
        ? h('div', { class: 'fs-sm text-tertiary mt-md' },
            `当前 cookie：${s.cookieList.map((c) => `${c.name}=${c.value}`).join('; ') || '(空)'}`)
        : null,
      h('pre', { class: 'code-block mt-md' },
