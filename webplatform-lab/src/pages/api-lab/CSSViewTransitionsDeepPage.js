...
export class CSSViewTransitionsDeepPage extends Page {
  ...
  _runTypesDemo() {
    ...
    const info = [
      ...
      '  const t = document.startViewTransition(() => updateDOM());',
      '  t.types = new Set(["slide", "forward"]);  // 设置类型标签',
      ...
      '  });',
      '  // Chrome 125+ 支持对象签名',
      '  // 等价于：const t = startViewTransition(cb); t.types = new Set(["slide-forward"]);',
      '',
      ...
      '',
      '【多类型组合】',
      '  t.types = new Set(["slide", "highlight"]);',
  ...
  _slideTypes(direction) {
    ...
    if (hasObjSig) {
      try {
        ...
        if (t && t.types) t.types = new Set([direction === 'forward' ? 'slide-forward' : 'slide-back']);
      }
    } else {
      cb();
  ...
  _navigateSpa(page) {
    ...
    if (hasObjSig) {
      try {
        ...
        t = document.startViewTransition(cb);
        if (t && t.types) t.types = new Set([`slide-${direction}`]);
      }
    } else {
      cb();
