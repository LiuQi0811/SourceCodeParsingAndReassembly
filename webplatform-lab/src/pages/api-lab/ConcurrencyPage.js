      ...
      self.onconnect = (e) => {
        ...
        port.onmessage = (ev) => {
          ...
          for (const p of ports) if (p !== port) {
            try { p.postMessage({ kind: 'broadcast', from: id, text }); } catch (_) {}
          }
...
// 主线程非阻塞：Atomics.waitAsync(view, 0, 0, 3000).value.then(...);`),
  ...
  _runSchedulerPriorities() {
    ...
    const record = (label) => {
      this.setState((s) => ({ taskOrder: [...s.taskOrder, label].slice(-12) }));
      this._addLog('push', `执行: ${label}`);
    };

    if (this.state.schedulerSupported) {
  ...
  _runSchedulerAbort() {
    if (this.state.schedulerSupported) {
      ...
      globalThis.scheduler.postTask(
        ...
        this._addLog('pull', `任务#${id} 被取消：${err.name || 'AbortError'}`);
      });
      // 立即取消
      setTimeout(() => {
        try { controller.abort(); } catch (e) { this._addLog('err', `abort 失败: ${e.message}`); }
      }, 0);
