...
export class ModernJSModulesPage extends Page {
  ...
  _renderCard1() {
    ...
    return h(Card, {
      ...
      s.promiseTryInfo ? h('pre', { class: 'code-block mt-md' }, s.promiseTryInfo) : null,
      h('pre', { class: 'code-block mt-md' },
`// 旧写法：fn 内同步抛错可能变成未捕获异常
new Promise(resolve => resolve(fn()));
// ES2025 标准化
Promise.try(fn).then(v => ...).catch(e => ...);
// 情况1：同步抛错 → 被 catch 捕获
// 情况2：同步返回值 → then 收到
// 情况3：异步返回 Promise → then 收到 unwrapped 值`),
  ...
  _resolveImportMap() {
    ...
    const resolve = (specifier) => {
      try {
        // 完全匹配
        if (importMap.imports[specifier]) return importMap.imports[specifier];
        // 前缀映射（带尾斜杠）
...
`{
  ...
  "scopes": {
    "/vendor/": { "react": "https://esm.sh/react@17.0.2" }
      ...
      s.importMapsInfo ? h('pre', { class: 'code-block mt-md' }, s.importMapsInfo) : null,
      h('pre', { class: 'code-block mt-md' },
...
import React from "react";   // bare specifier → 解析到 esm.sh
  ...
  _renderCard3() {
    ...
    return h(Card, {
      ...
      s.importAttrsInfo ? h('p', { class: 'fs-sm mt-sm text-secondary' }, s.importAttrsInfo) : null,
      h('pre', { class: 'code-block mt-md' },
`// 新语法 with（推荐）
...
// - 浏览器仅对未声明类型的模块拒绝执行（安全）`),
  ...
  _serializeShadowDOM() {
    ...
    try {
      ...
      let outer = '';
      try { outer = host.getHTML({ serializableShadowRoots: true }); } catch (e) { this._addLog('warn', 'getHTML 调用失败：' + (e && e.message)); }
  ...
  _renderCard4() {
    ...
    return h(Card, {
      ...
      s.getHtmlInfo ? h('pre', { class: 'code-block mt-md' }, s.getHtmlInfo) : null,
      h('pre', { class: 'code-block mt-md' },
