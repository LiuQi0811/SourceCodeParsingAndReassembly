...
export class Dropdown extends Component {
  ...
  componentDidMount() {
    ...
    if (trigger === 'hover') {
      ...
      this.on(this.el, 'click', (e) => {
        ...
        e.stopPropagation(); // 阻止冒泡到 document，避免立即关闭
        this.toggle();
      });
      this.on(document, 'click', (e) => {
        if (this.state.visible && this.el && !this.el.contains(e.target)) this.hide();
      });
      // 键盘：触发器上按 ↓ 打开，Esc 关闭
      this.on(this.el, 'keydown', (e) => {
        if (e.key === 'ArrowDown' && !this.state.visible) {
          e.preventDefault();
          this.show();
          // 把焦点移到首个可聚焦菜单项
  ...
  render() {
    ...
    this._menu = menuEl;
    if (!this.state.visible) menuEl.style.display = 'none';
