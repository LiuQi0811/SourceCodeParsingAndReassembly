...
export class AdvancedJSRuntimePage extends Page {
  ...
  _demoReflectVsObject() {
    ...
    catch (e) { objectErr = e.constructor.name + ': ' + e.message; }
  ...
  _makePromises() {
    const make = (label, ok, delay = 100 + Math.random() * 400) =>
  ...
  async _runPromiseAnyAllReject() {
    ...
    const allReject = ['X1', 'X2', 'X3'].map((l) =>
      new Promise((_, rej) => setTimeout(() => rej(new Error(l + ' 失败')), 80 + Math.random() * 300))
    );
    try {
  ...
  _demoMisc() {
    ...
    if (c.structuredClone) {
      ...
      L.push(`  cloned.buf.byteLength        → ${cloned.buf.byteLength}`);
      L.push(`  深拷贝独立：cloned.nested !== original.nested → ${cloned.nested !== original.nested}`);
      try { structuredClone({ fn: () => 1 }); }
      catch (err) { L.push(`  structuredClone({fn}) 抛异常：${err.message}（不支持函数/DOM 节点等）`); }
    } else {
