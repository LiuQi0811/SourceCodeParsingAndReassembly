...
export class MediaSessionAPIPage extends Page {
  ...
  _readPlaybackStateInfo() {
    ...
    try {
      let liveState = '(未读取)';
      if (f.playbackState) {
        try { liveState = navigator.mediaSession.playbackState; } catch (e) { liveState = '读取失败：' + e.message; }
      }
  ...
  _setPlaybackState(mode) {
    ...
    const f = this._flags();
    if (f.playbackState) {
      try { navigator.mediaSession.playbackState = mode; } catch (err) {
  ...
  _readActionHandlerInfo() {
    ...
    try {
      ...
      return '===== setActionHandler 动作处理 =====\n' +
        ...
        '   "seekforward", "seekto", "stop", "skipad"].forEach((a) => {\n' +
  ...
  _playerTogglePlay() {
    ...
    const f = this._flags();
    if (f.playbackState) {
      try { navigator.mediaSession.playbackState = this._playerState; } catch (e) { /* noop */ }
    }
