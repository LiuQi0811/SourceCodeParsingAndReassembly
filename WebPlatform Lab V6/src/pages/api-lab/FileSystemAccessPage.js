...
export class FileSystemAccessPage extends Page {
  ...
  _makeMockFileHandle(file) {
    return {
      ...
      createWritable: async (_opts) => {
        let buffer = '';
        return {
          write: async (data) => { if (typeof data === 'string') buffer += data; },
          seek: async (_pos) => { /* mock：seek 不改变追加语义 */ },
  ...
  // 递归遍历：for await (const [name, h] of dir.entries()) 异步迭代器
  ...
  _makeMockDirHandle() {
    const build = (name, tree) => {
      ...
      if (tree.kind === 'directory') {
        ...
        handle.removeEntry = async (_n, _opts) => { /* mock：空操作 */ };
        handle.values = handle.entries;
        handle.keys = () => ({ async *[Symbol.asyncIterator]() {
          for (const k of Object.keys(tree.children || {})) yield k;
        } });
      }
      return handle;
    };
  ...
  _renderCard1() {
    ...
    return this._card(
      ...
      'window.showOpenFilePicker({ multiple, types, excludeAcceptAllOption }) → Promise<FileSystemFileHandle[]>；'
  ...
  _renderCard2() {
    ...
    return this._card(
      ...
      'window.showSaveFilePicker({ suggestedName, types }) → Promise<FileSystemFileHandle>；'
  ...
  _renderCard3() {
    ...
    return this._card(
      ...
      'window.showDirectoryPicker({ mode }) → Promise<FileSystemDirectoryHandle>；'
      + 'dir.entries() / values() / keys() 异步迭代器；getFileHandle(name,{create:true}) / removeEntry(name,{recursive:true})；'
      + 'for await (const [name, handle] of dir.entries()) 递归遍历，handle.kind === "file" / "directory"。',
      h('div', { class: 'flex gap-xs' },
        ...
        s.dirTree
          ? h('div', { class: 'flex flex-col gap-xs mt-sm' },
              ...
              h('div', { class: 'log-panel', style: { maxHeight: '280px', overflow: 'auto' } },
                s.dirTree.entries.map((e) => h('div', { class: 'log-panel__line' },
                  h('span', {
