...
export class AdvancedInputPage extends Page {
  ...
  _readVKInfo() {
    ...
    try {
      ...
      this._addLog('warn', `读取 VirtualKeyboard 失败：${errInfo(err).name} - ${errInfo(err).message}`);
    }
  }
  ...
  _showVK() {
    ...
    try {
      ...
      this._addLog('warn', `show() 失败：${errInfo(err).name} - ${errInfo(err).message}`);
    }
  }
  ...
  _hideVK() {
    ...
    try {
      ...
      this._addLog('warn', `hide() 失败：${errInfo(err).name} - ${errInfo(err).message}`);
    }
  }
  ...
  _toggleOverlays() {
    ...
    try {
      ...
      this._addLog('warn', `切换 overlaysContent 失败：${errInfo(err).name} - ${errInfo(err).message}`);
    }
  }
  ...
  _listenGeometry() {
    ...
    try {
      ...
      this._addLog('warn', `绑定 geometrychange 失败：${errInfo(err).name} - ${errInfo(err).message}`);
    }
  }
  ...
  async _lockKeys() {
    ...
    try {
      ...
      this._addLog('warn', `lock 失败：${errInfo(err).name} - ${errInfo(err).message}`);
      this.setState({ keyboardInfo: `lock 失败：${errInfo(err).name} - ${errInfo(err).message}` });
    }
  }
  ...
  async _getLayoutMap() {
    ...
    try {
      ...
      this._addLog('warn', `getLayoutMap 失败：${errInfo(err).name} - ${errInfo(err).message}`);
      this.setState({ keyboardInfo: `getLayoutMap 失败：${errInfo(err).name} - ${errInfo(err).message}` });
    }
  }
  ...
  _unlockKeys() {
    ...
    try {
      ...
      this._addLog('warn', `unlock 失败：${errInfo(err).name} - ${errInfo(err).message}`);
    }
  }
...
navigator.keyboard.unlock();         // 退出游戏时解锁`)),
  ...
  async _openEyeDropper() {
    ...
    try {
      ...
      this._addLog('warn', `EyeDropper 失败：${errInfo(err).name} - ${errInfo(err).message}（用户取消或环境不支持）`);
      this.setState({ eyeDropperResult: `EyeDropper 失败：${errInfo(err).name} - ${errInfo(err).message}` });
    }
  }
  ...
  async _openWithAbort() {
    ...
    try {
      ...
      this._addLog(isAbort ? 'eye' : 'warn',
        `open({ signal }) 结束：${errInfo(err).name} - ${errInfo(err).message}${isAbort ? '（AbortSignal 已生效）' : ''}`);
      this.setState({
        eyeDropperResult:
          `eyeDropper.open({ signal }) 结果：${errInfo(err).name} - ${errInfo(err).message}\n` +
  ...
  async _requestPipWindow() {
    ...
    try {
      ...
      this._addLog('warn', `requestWindow 失败：${errInfo(err).name} - ${errInfo(err).message}`);
      this.setState({ pipInfo: `requestWindow 失败：${errInfo(err).name} - ${errInfo(err).message}` });
    }
  }
  ...
  _moveToPip() {
    ...
    try {
      ...
      this._addLog('warn', `移动 DOM 到 PiP 失败：${errInfo(err).name} - ${errInfo(err).message}`);
    }
  }
  ...
  _closePip() {
    ...
    try {
      ...
      this._addLog('warn', `close 失败：${errInfo(err).name} - ${errInfo(err).message}`);
    }
  }
  ...
  _listenPipEvents() {
    ...
    try {
      ...
      this._addLog('warn', `绑定 enter / leave 失败：${errInfo(err).name} - ${errInfo(err).message}`);
    }
  }
  ...
  _createSanitizer() {
    ...
    try {
      ...
      this._addLog('warn', `创建 Sanitizer 失败：${errInfo(err).name} - ${errInfo(err).message}`);
    }
  }
  ...
  _sanitizeDiv() {
    ...
    try {
      ...
      this._addLog('warn', `sanitizeFor 失败：${errInfo(err).name} - ${errInfo(err).message}`);
    }
  }
  ...
  _sanitizeFragment() {
    ...
    try {
      ...
      this._addLog('warn', `sanitize 失败：${errInfo(err).name} - ${errInfo(err).message}`);
    }
  }
...
container.replaceChildren(div);    // 安全插入（也可用 sanitize → DocumentFragment）`)),
  ...
  _checkCapabilities() {
    ...
    try {
      ...
      this._addLog('warn', `InputDeviceCapabilities 检测失败：${errInfo(err).name} - ${errInfo(err).message}`);
    }
  }
  ...
  _createTempElement() {
    ...
    try {
      ...
      try {
        ...
        this._addLog('warn', `派发合成事件失败：${errInfo(err).message}`);
        this.setState({
          deviceCapInfo:
      ...
      this._addLog('warn', `创建临时元素失败：${errInfo(err).name} - ${errInfo(err).message}`);
    }
  }
