...
// 非用户手势调用抛 SecurityError；类型不匹配抛 NotSupportedError`)),
    ...
    try {
      ...
      this.setState({
        formDataInfo:
          ...
          `fd.entries() → ${JSON.stringify(entriesArr)}\n` +
          `fd.keys() → ${JSON.stringify(keysArr)}\n` +
          `fd.values() → ${JSON.stringify(valuesArr)}\n\n` +
  ...
  _demoFormDataFileUpload() {
    ...
    try {
      ...
      let count = 0;
      for (const _ of fd.entries()) count++;           // eslint-disable-line no-unused-vars
      this.setState({
        formDataInfo:
...
for (const [k, v] of fd.entries()) { /* 遍历 */ }`)),
        h(Alert, {
          type: 'info',
          message: 'FormData 没有 size 属性',
          description: '与 Map/Set 不同，FormData 没有 size 属性。若需统计字段数，用 Array.from(fd.entries()).length 或遍历计数。append 同名追加多值，set 同名覆盖；get 取第一个，getAll 取全部。fetch 以 FormData 为 body 时浏览器自动设置 Content-Type: multipart/form-data; boundary=...，无需手动设置。',
        }),
      ],
    });
    this.registerChild(card);
  ...
  _demoNativeComponents() {
    ...
    try {
      ...
      } catch (e) { lines.push(`① datalist 检测失败：${e.message}`); }
      // ② progress
      try {
      ...
      } catch (e) { lines.push(`② progress 检测失败：${e.message}`); }
      // ③ meter
      try {
      ...
      } catch (e) { lines.push(`③ meter 检测失败：${e.message}`); }
      // ④ output
      try {
      ...
      } catch (e) { lines.push(`④ output 检测失败：${e.message}`); }
      lines.push('');
...
<form oninput="result.value = Number(a.value) + Number(b.value)">
  ...
  _demoValidateThenSubmit() {
    ...
    try {
      ...
      } catch (e) { lines.push(`① 验证失败抛错：${e.message}`); }
      ...
      } catch (e) { lines.push(`② 验证后提交抛错：${e.name} - ${e.message}`); }
      lines.push('');
