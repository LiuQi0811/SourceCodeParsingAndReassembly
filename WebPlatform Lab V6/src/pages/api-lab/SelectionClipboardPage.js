  ...
  _readSelection() {
    ...
    try {
      ...
      if (this._selHost && typeof this._selHost.select === 'function') {
        ...
        catch (e) { selectLine = `setSelectionRange 失败：${e.message}\n`; }
  ...
  _addRangeAndSelectDemo() {
    ...
    try {
      ...
      if (this._caps().createRange && this._selHost) {
        ...
        } catch (e) { lines.push(`addRange/selectAllChildren/collapse 失败：${e.name} - ${e.message}（jsdom 可能不支持真实选区）`); }
      } else {
...
sel.selectAllChildren(node);              // 选中所有子节点`)),
  ...
  _rangeSetStartEnd() {
    ...
    try {
      ...
      if (firstChild && firstChild.nodeType === 3) { // Node.TEXT_NODE = 3
        ...
        } catch (e) { lines.push(`cloneContents 失败：${e.message}`); }
        try {
        ...
        } catch (e) { lines.push(`getBoundingClientRect/getClientRects 失败：${e.message}（jsdom 无布局）`); }
      } else {
        lines.push('宿主无文本子节点');
      }
  ...
  _rangeSelectAndClone() {
    ...
    try {
      ...
      } catch (e) { lines.push(`selectNode/selectNodeContents/setStartBefore 失败：${e.message}`); }
      try {
      ...
      } catch (e) { lines.push(`cloneRange/compareBoundaryPoints 失败：${e.message}`); }
  ...
  _rangeMutate() {
    ...
    try {
      ...
      } catch (e) { lines.push(`insertNode 失败：${e.name} - ${e.message}`); }
      try {
      ...
      } catch (e) { lines.push(`surroundContents 失败：${e.name} - ${e.message}（须恰好跨完整节点）`); }
      try {
      ...
      } catch (e) { lines.push(`extractContents/deleteContents 失败：${e.name} - ${e.message}`); }
      this.setState({ rangeInfo: lines.join('\n') });
...
range.surroundContents(document.createElement('mark')); // 包裹`)),
  ...
  _execCommandDetect() {
    ...
    try {
      ...
      for (const cmd of cmds) {
        ...
        try { supported = String(document.queryCommandSupported(cmd)); } catch (e) { supported = `err(${e.name})`; }
        try { enabled = String(document.queryCommandEnabled(cmd)); } catch (e) { enabled = `err(${e.name})`; }
        try { value = String(document.queryCommandValue(cmd)); } catch (e) { value = `err(${e.name})`; }
      ...
      const exec = (cmd, ...args) => { try { return String(document.execCommand(cmd, ...args)); } catch (e) { return `err(${e.message})`; } };
  ...
  _execFormatCmds() {
    ...
    try {
      ...
      const host = this._rangeHost;
      const lines = [];
      const exec = (cmd, ...args) => { try { return String(document.execCommand(cmd, ...args)); } catch (e) { return `err(${e.name}: ${e.message})`; } };
  ...
  _dispatchCopyEvent() {
    ...
    try {
      ...
      if (caps.clipboardEvent) {
        ...
        catch (e) { evt = new Event('copy', { bubbles: true, cancelable: true }); }
      } else {
  ...
  _modifyExtendDemo() {
    ...
    try {
      ...
      if (this._selHost) {
        ...
        catch (e) { prepLine = `初始选区失败：${e.message}\n`; }
      ...
      if (modifySupported) {
        ...
        } catch (e) { modifyLine = `modify 抛错：${e.name} - ${e.message}\n`; }
  ...
  _modifyMoveDemo() {
    ...
    try {
      ...
      if (modifySupported) {
        ...
        catch (e) { lines.push(`move 抛错：${e.name} - ${e.message}`); }
      ...
      try {
        ...
        if (anchorNode) {
          ...
          catch (e) { lines.push(`extend 抛错：${e.message}`); }
        } else { lines.push(`anchorNode 为 null，无法 collapse/extend`); }
      } catch (e) { lines.push(`collapse/extend 抛错：${e.name} - ${e.message}`); }
      ...
      if (this._caps().createRange && this._selHost) {
        try {
          ...
          if (!fc) { lines.push('宿主无文本子节点'); }
            ...
            const tryOp = (label, fn) => { try { fn(); lines.push(label); } catch (e) { lines.push(`${label.split(' →')[0]} 抛错：${e.name} - ${e.message}`); } };
        ...
        } catch (e) { lines.push(`操作失败：${e.name} - ${e.message}`); }
