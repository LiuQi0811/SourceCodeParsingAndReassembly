...
export class ScrollEventsPage extends Page {
  ...
  _renderCard1() {
    ...
    return h(Card, {
      ...
      s.scrollendInfo ? h('p', { class: 'fs-sm mt-sm text-secondary' }, s.scrollendInfo) : null,
...
// Chrome 114+ / Edge 114+；Safari/Firefox 部分`),
  ...
  _renderCard2() {
    ...
    return h(Card, {
      ...
      s.compareInfo ? h('p', { class: 'fs-sm mt-sm text-secondary' }, s.compareInfo) : null,
      h('pre', { class: 'code-block mt-md' },
`// 方案一：rAF 节流
let ticking = false;
...
el.addEventListener('scrollend', handle);`),
  ...
  _renderCard3() {
    const s = this.state;
    const caps = this._caps();
    const mkParas = (label) => Array.from({ length: 12 }, (_, i) =>
    ...
    return h(Card, {
      ...
      s.gutterInfo ? h('p', { class: 'fs-sm mt-sm text-secondary' }, s.gutterInfo) : null,
      h('pre', { class: 'code-block mt-md' },
    ...
    return h(Card, {
      ...
      h('div', { class: 'se-demo-box' },
        h('div', {
          ...
          onscroll: () => this._logMatrixEvent('scroll'),
          onscrollend: () => this._logMatrixEvent('scrollend'),
          onwheel: mkWheel,
          onpointermove: (e) => { if (e && e.pointerType === 'touch') this._logMatrixEvent('pointermove'); },
      ...
      h('div', { class: 'flex items-center gap-sm flex-wrap mt-md' },
        this._btn('模拟派发 scroll', { type: 'default', size: 'sm', onClick: () => { try { this.$('#se-matrix-box')?.dispatchEvent(new Event('scroll', { bubbles: true })); } catch { /* noop */ } } }),
        this._btn('模拟派发 scrollend', { type: 'default', size: 'sm', onClick: () => { try { this.$('#se-matrix-box')?.dispatchEvent(new Event('scrollend', { bubbles: true })); } catch { /* noop */ } } }),
        h(Tag, { color: caps.passive ? 'success' : 'error' }, caps.passive ? 'passive ✓' : 'passive ✗'),
      ),
      s.matrixInfo ? h('p', { class: 'fs-sm mt-sm text-secondary' }, s.matrixInfo) : null,
      h('pre', { class: 'code-block mt-md' },
...
// pointermove（pointerType==='touch'）可识别触摸滚动`),
  ...
  _renderCard5() {
    ...
    return h(Card, {
      ...
      s.programInfo ? h('p', { class: 'fs-sm mt-sm text-secondary' }, s.programInfo) : null,
      h('pre', { class: 'code-block mt-md' },
