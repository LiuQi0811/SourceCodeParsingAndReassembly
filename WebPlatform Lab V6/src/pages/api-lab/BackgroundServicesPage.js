// =====================================================================
// BackgroundServicesPage.js —— 后台服务类 Web API 实验室
// 演示 MDN：Service Worker API、Cache API、Notifications API、
//           Push API、Background Sync API、Cookie Store API、StorageManager API
// 说明：Service Worker / Push / Background Sync 需要 HTTPS（或 localhost）与
//       独立脚本作用域；本沙箱环境无 SW，相关注册将优雅失败，重点展示 API 形态
//       与能力检测。Cache API / Cookie Store / StorageManager / Notifications
//       在浏览器中可直接体验（部分仍需安全上下文）。
// =====================================================================
...
// Service Worker 生命周期状态：parsed → installing → installed → activating → activated → redundant
...
export class BackgroundServicesPage extends Page {
  initialState() {
    return {
      ...
      swState: 'unregistered',   // unregistered | registering | registered | error
      swError: '',
      swScope: '',
      // Card 2: Cache API
      cacheList: [],
  ...
  // —— 生命周期 ——
  componentDidMount() {
    // CRITICAL：防止 _rerender 后重复进入导致 setState 循环 / OOM
    if (this._inited) return;
    this._inited = true;

    const patch = {};
    // 一次性能力检测：Notification 权限（仅一次 setState）
    if ('Notification' in window) {
      patch.notifPermission = Notification.permission;
    }
    if (Object.keys(patch).length) this.setState(patch);

    // 监听 cookieStore change 事件（若可用）
    if ('cookieStore' in window && window.cookieStore) {
      this._onCookieChange = (event) => {
        try {
          const changed = (event.changed || []).map((c) => c.name);
          const deleted = (event.deleted || []).map((c) => c.name);
          if (changed.length) this._addLog('cookie', `change 事件：变更 [${changed.join(', ')}]`);
          if (deleted.length) this._addLog('cookie', `change 事件：删除 [${deleted.join(', ')}]`);
        } catch { /* noop */ }
      };
      try {
  ...
  componentWillUnmount() {
    // 移除 cookieStore change 监听
    if (this._onCookieChange && 'cookieStore' in window && window.cookieStore) {
      try { window.cookieStore.removeEventListener('change', this._onCookieChange); }
      catch { /* noop */ }
    }
  ...
  // 受控输入：直接修改 this.state[field]，不触发 setState（避免重渲染导致失焦）
  _input(field, opts = {}) {
  ...
  async _registerSW() {
    ...
    try {
      ...
      // 沙箱无 HTTPS / 无独立 sw 脚本：属环境限制，记为 info 而非 err
  ...
  async _cacheListAll() {
    ...
    try {
      ...
      this.setState({ cacheList: names });
      this._addLog('cache', `caches.keys() => 共 ${names.length} 个：[${names.join(', ') || '空'}]`);
    } catch (err) {
  ...
  async _cacheKeysInOne() {
    ...
    try {
      ...
      this._addLog('cache', `cache.keys() => ${reqs.length} 条 Request：[${reqs.map((r) => r.url).join(', ') || '空'}]`);
    } catch (err) {
  ...
  async _cookieSet() {
    ...
    try {
      // 对象形式：可指定 path/sameSite/expires/maxAge/secure 等
  ...
  async _storageEstimate() {
    ...
    try {
      ...
      this.setState({ storageEstimate: est });
  ...
  renderPage() {
    ...
    const usedPct = est && est.quota ? Math.min(100, (est.usage / est.quota) * 100) : 0;
    ...
    return [
          ...
          !hasSW
            ...
            : h('div', { class: 'flex flex-col gap-sm' },
                ...
                h('div', { class: 'flex items-center gap-sm flex-wrap' },
                  ...
                  this.state.swScope && h(Tag, { color: 'primary' }, `scope: ${this.state.swScope}`),
                  h(Tag, { color: ctrl ? 'success' : 'default' }, `controller: ${ctrl ? ctrl.state : 'null'}`),
                ),
                this.state.swError && h('div', { class: 'fs-sm', style: { color: '#ff5555' } }, `错误：${this.state.swError}`),
      ...
      h(Card, {
        ...
        h('div', { class: 'flex flex-col gap-sm' },
          h('p', { class: 'fs-sm text-secondary' },
            'caches.open(name) 打开 Cache 对象；cache.put(request, response) 写入；cache.add(url) 自动 fetch+put；cache.addAll([...]) 批量；cache.match(request) 读取 Response；cache.matchAll() 读取全部；cache.keys() 列出所有 Request；caches.keys() 列出所有缓存名；caches.delete(name) 删除整个缓存。Cache API 独立于 SW，可在普通页面直接使用。'),
          !hasCaches
            ...
            : h('div', { class: 'flex flex-col gap-sm' },
                ...
                this.state.cacheContent && h('pre', { class: 'code-block', style: { maxHeight: '100px' } },
                  `cache.match => ${this.state.cacheContent}`),
              ),
        ),
      ),
      // ============ 3. Notifications API ============
        ...
        h('div', { class: 'flex flex-col gap-sm' },
          ...
          !hasPush
            ...
            : h('div', { class: 'flex flex-col gap-sm' },
                ...
                this.state.pushDetail && h('div', { class: 'fs-sm text-tertiary', style: { wordBreak: 'break-all' } }, `详情：${this.state.pushDetail}`),
          ...
          !hasStorage
            ...
            : h('div', { class: 'flex flex-col gap-sm' },
                ...
                est && h('div', { class: 'flex flex-col gap-xs' },
                  h('div', { class: 'fs-sm' },
                    `配额 quota：${this._formatBytes(est.quota)}，已用 usage：${this._formatBytes(est.usage)}（${usedPct.toFixed(2)}%）`),
