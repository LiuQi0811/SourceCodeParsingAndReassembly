// =====================================================================
// IdentityPage.js —— 身份认证与支付 API 实验室
// 演示 MDN：
...
export class IdentityPage extends Page {
  ...
  initialState() {
    return {
      ...
      credSupportedTypes: [],          // ['PasswordCredential','FederatedCredential','OTPCredential']
      credPreventSilentResult: '—',    // preventSilentAccess() 调用结果
      credStoreResult: '—',            // store() 调用结果
      // Card 2: WebAuthn
      webauthnPlatformAuth: null,      // boolean | null（未检测）
      webauthnConditionalMed: null,    // boolean | null
      webauthnOptionsPreview: null,    // 构造的 PublicKeyCredentialCreationOptions
      // Card 3: Payment Request
      paymentCanMake: null,            // boolean | null
      paymentPreview: null,            // { methodData, details, options }
      paymentShowResult: '—',          // show() 调用结果
      // Card 4: Presentation + Idle Detection
      presentationRequestPreview: null, // { urls, defaultRequest }
      idlePermission: 'unknown',       // 'granted' | 'denied' | 'unknown'
      idleRunning: false,
      idleUserState: '—',
      idleScreenState: '—',
      ...
      fedcmPreview: null,              // IdentityCredential 配置预览
    };
  }
  ...
  async _preventSilentAccess() {
    ...
    try {
      ...
      this._addLog('err', `preventSilentAccess 失败：${errInfo(err).name} — ${errInfo(err).message}`);
      this.setState({ credPreventSilentResult: `失败：${errInfo(err).name}` });
    }
  }
  ...
  async _createAndStorePasswordCredential() {
    ...
    try {
      ...
      this._addLog('err', `PasswordCredential 创建/存储失败：${errInfo(err).name} — ${errInfo(err).message}`);
      this.setState({ credStoreResult: `失败：${errInfo(err).name}` });
    }
  }
  ...
  async _checkPlatformAuthenticator() {
    ...
    try {
      ...
      this._addLog('err', `检测平台认证器失败：${errInfo(err).name} — ${errInfo(err).message}`);
  ...
  async _checkConditionalMediation() {
    ...
    try {
      ...
      this._addLog('err', `检测条件 UI 失败：${errInfo(err).name} — ${errInfo(err).message}`);
  ...
  _buildWebAuthnOptions() {
    ...
    try {
      ...
      this._addLog('err', `构造 WebAuthn 选项失败：${errInfo(err).name} — ${errInfo(err).message}`);
    }
  }
  ...
  _buildPaymentRequest() {
    ...
    try {
      ...
      this._addLog('err', `构造 PaymentRequest 失败：${errInfo(err).name} — ${errInfo(err).message}`);
      return null;
    }
  }
  ...
  async _canMakePayment() {
    ...
    try {
      ...
      this._addLog('err', `canMakePayment 失败：${errInfo(err).name} — ${errInfo(err).message}`);
  ...
  async _showPayment() {
    ...
    try {
      ...
      this._addLog('err', `show 失败：${errInfo(err).name} — ${errInfo(err).message}（jsdom / 无支付处理程序时属正常）`);
      this.setState({ paymentShowResult: `失败：${errInfo(err).name}` });
    }
  }
  ...
  _buildPresentationRequest() {
    ...
    try {
      ...
      this._addLog('err', `构造 PresentationRequest 失败：${errInfo(err).name} — ${errInfo(err).message}`);
    }
  }
  ...
  async _idleRequestPermission() {
    ...
    try {
      ...
      this._addLog('err', `requestPermission 失败：${errInfo(err).name} — ${errInfo(err).message}`);
    }
  }
  ...
  async _startIdleDetector() {
    ...
    try {
      ...
      this._addLog('err', `IdleDetector.start 失败：${errInfo(err).name} — ${errInfo(err).message}`);
  ...
  async _queryLocalFonts() {
    ...
    try {
      ...
      this._addLog('err', `queryLocalFonts 失败：${errInfo(err).name} — ${errInfo(err).message}`);
    }
  }
