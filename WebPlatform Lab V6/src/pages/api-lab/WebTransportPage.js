...
transport.close({ closeCode: 1000, reason: 'done' });`)),
  ...
  _runConnDemo() {
    ...
    const info = [
      ...
      '    t.closed.then(({ closeCode, reason }) =>',
...
while (true) {
  const { value, done } = await reader.read();
  if (done) break;
  console.log('收到 datagram', value.byteLength, '字节');
}
...
// }`)),
  ...
  _runMarketDemo() {
    ...
    const info = [
      ...
      '      const b = await t.createBidirectionalStream();',
      '      const encoder = new VideoEncoder({',
      '        output: (chunk) => b.writable.getWriter().write(copyChunk(chunk)),',
      '        error: console.error,',
      '      });',
      '      encoder.configure({ codec: "av1", width: 1280, height: 720 });',
      '      track.onFrame = (frame) => encoder.encode(frame);',
      '    }',
      '',
      '【场景 4：IoT 双向遥测】',
      ...
      '      data.set(new Uint8Array(chunk.byteLength), 16);',
...
while (true) {
  ...
  _runDecisionDemo() {
    ...
    const info = [
      ...
      '',
      '【降级链：WebTransport → WebSocketStream → WebSocket → SSE】',
      '  const transport = await createTransport(url).catch(async (err) => {',
