...
CSS.supports('color', 'oklch(70% 0.15 145)'); CSS.supports('width', '10cqw');`)),
  ...
  // CSS.paintWorklet.addModule(url) → Promise<void>：加载 Paint Worklet 模块
  async _addPaintModule() {
    const caps = this._caps();
    if (!caps.paintWorklet) {
      this._addLog('warn', 'CSS.paintWorklet.addModule 不可用（jsdom 无 Worklet 支持）');
      this.setState({
        paintInfo:
          'CSS.paintWorklet.addModule(url) 不可用\n' +
          '当前环境 typeof CSS.paintWorklet === "undefined"（jsdom 无 Worklet 支持）。\n' +
          '真实 Chromium 中：await CSS.paintWorklet.addModule("./paint-worklet.js");\n' +
          '随后 CSS：background: paint(my-paint); Worklet 运行在独立线程，无法访问 window/document。',
      });
      return;
    }
    try {
      this._addLog('paint', 'CSS.paintWorklet.addModule() 可用，尝试加载 mock worklet…');
      ...
      this.setState({
        paintInfo:
          'CSS.paintWorklet.addModule(url) → Promise<void> ✓\n' +
  ...
  _showWorkletOverview() {
    this.setState({
      paintInfo:
        ...
        '1. Paint Worklet —— CSS.paintWorklet.addModule + registerPaint + paint()；CSS：background: paint(name)\n' +
        '2. Layout Worklet —— CSS.layoutWorklet.addModule + registerLayout + layout()；CSS：display: layout(name)\n' +
        '3. Animation Worklet —— CSS.animationWorklet.addModule + registerAnimator + animate()\n' +
  ...
  _renderCard2() {
    ...
    const card = new Card({
      ...
      children: [
        h('p', { class: 'fs-sm text-secondary' },
          'CSS.paintWorklet.addModule(url) 加载 Paint Worklet 模块；worklet 脚本内用 registerPaint(name, class) 注册绘制类，类需实现 static get inputProperties() / inputArguments() / contextOptions() 与 paint(ctx, geometry, properties, args) 方法。CSS 中用 background: paint(name, ...args) 调用。绘制在独立线程执行，输入属性变化时自动重绘。'),
        h('div', { class: 'flex items-center gap-sm flex-wrap' },
...
await CSS.paintWorklet.addModule('./paint-worklet.js');   // 主线程加载
...
input:not([disabled]) { cursor: pointer; }       /* :not 取反 */`)),
  ...
  _createContainer() {
    try {
      ...
      } catch (e) { rulesInfo = `读取 cssRules 抛错：${e.name}（jsdom 可能不解析 @container）`; }
      this.setState({
        containerInfo:
...
.card { padding: 2cqi; }       /* 内联轴的 2% */`)),
  ...
  _injectLayers() {
    try {
      ...
      } catch (e) { rulesInfo = `读取 cssRules 抛错：${e.name}`; }
      this.setState({
        layerInfo:
          `已注入 @layer 规则 ✓\n` +
  ...
  _inspectLayers() {
    ...
    try {
      ...
      if (style && style.sheet) {
        try {
          rulesText = Array.from(style.sheet.cssRules).map((r, i) =>
            `  [${i}] ${r.constructor.name}${r.name ? ` name="${r.name}"` : ''}${r.cssText ? `\n      cssText: ${r.cssText.slice(0, 80)}` : ''}`
          ).join('\n');
        } catch (e) {
