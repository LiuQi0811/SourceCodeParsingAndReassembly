...
// 也可传入 ImageBitmap / Blob / ImageData / HTMLCanvasElement`)),
  ...
  _stopDisplayStream() {
    if (this._displayStream) {
      try { this._displayStream.getTracks().forEach((t) => t.stop()); } catch { /* noop */ }
  ...
  _demoDisplayMediaUsage() {
    ...
    } catch (err) { applyResult = `applyConstraints 失败：${err.message}`; }
  ...
  async _demoImageCaptureUsage() {
    ...
    try {
      ...
      try {
        ...
        constructLine = `new ImageCapture(mockTrack) ✓ 构造成功`;
      } catch (err) {
        constructLine = `new ImageCapture(mockTrack) 失败：${err.name} - ${err.message}（mock track 不被接受，需 getUserMedia 返回的 track）`;
      }
