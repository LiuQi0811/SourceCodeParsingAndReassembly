...
//   3. 装饰器实现 API —— type Decorator = (value, context) => value |
...
export class DecoratorsResourceManagementPage extends Page {
  ...
  _readOverviewInfo() {
    ...
    try {
      return `===== TC39 Stage 3 装饰器与资源管理 概述 =====\n` +
        ...
        `    更简洁的 API：(value, context) => value | { get, set } | void\n` +
  ...
  _readSyntaxInfo() {
    ...
    try {
      return `===== Decorators 语法与种类 =====\n` +
        ...
        `    return function (value, ctx) { // 真正的装饰器\n` +
  ...
  _readApiInfo() {
    ...
    try {
      return `===== 装饰器实现 API =====\n` +
        `\n` +
        `【装饰器函数签名】\n` +
        `  type Decorator = (value, context) => value | { get, set } | void\n` +
  ...
  _renderCard3() {
    ...
    const f = this._flags();
    const card = new Card({
      title: '3. 装饰器实现 API —— (value, context) => value | { get, set } | void',
      extra: h('div', { class: 'flex gap-xs' },
  ...
  _runClassMethodDemo() {
    ...
    this._decoratorCallLog.push('[@memoize] 缓存写入 greet(["world"]) => "hi world"');
  ...
  _readFieldAccessorInfo() {
    ...
    try {
      return `===== 字段与访问器装饰器 =====\n` +
        ...
        `      return function (initial) { return initial * 2; };  // 初始值翻倍\n` +
        `    }\n` +
        ...
        `    emit(prop, val) { console.log(prop + ' changed to ' + val); }\n` +
        `  }\n` +
        `\n` +
        `【陷阱】\n` +
        `  ✗ field 装饰器 value 是 undefined（不是初始值）\n` +
  ...
  _readUsingInfo() {
    ...
    try {
      return `===== 实战：using 资源管理场景 =====\n` +
        ...
        `  navigator.locks.request('res', async (lock) => {\n` +
