...
A.isDisjointFrom(new Set([10,20])); // true（无交集）`)),
        h(Alert, {
          ...
          description: '此前做并集需 new Set([...A, ...B])，交集需 [...A].filter(x => B.has(x))；ES2024 把这些标准化为原型方法，语义清晰且引擎可优化。参数可以是任意 Set 或可迭代对象（如数组）。',
        }),
      ],
    });
    ...
    try {
      ...
      this.setState({
        iteratorResult:
          ...
          `.map(x => x*2).filter(x => x>4).take(3).toArray()\n` +
    ...
    try {
      ...
      this.setState({
        iteratorResult:
          ...
          `  Iterator.from([1,2,3]).flatMap(x => [x, x*10]).toArray() = ${JSON.stringify(flat)}\n\n` +
          `reduce(fn, initial)：归约\n` +
          `  Iterator.from([1,2,3,4]).reduce((a,x) => a+x, 0) = ${sum}\n\n` +
    ...
    try {
      ...
      this.setState({
        iteratorResult:
          ...
          `  .map(x => x*x).toArray() = ${JSON.stringify(squared)}\n\n` +
          `包装生成器：Iterator.from(gen()).filter(s => s!=='y').toArray() = ${JSON.stringify(fromGen)}\n\n` +
...
const r = Iterator.from([1,2,3,4,5])
  .map(x => x * 2).filter(x => x > 4).take(3).toArray(); // [6,8,10]
Iterator.from([1,2,3]).flatMap(x => [x, x*10]).toArray(); // [1,10,2,20,3,30]`)),
        h(Alert, {
          type: 'info',
    ...
    try {
      ...
      this.setState({
        groupByResult:
          ...
          `Object.keys(grouped) = ${JSON.stringify(keys)}\n` +
...
const byType = Map.groupBy(items, x => x.type);         // Map（键可为对象）`)),
  ...
  async _withResolvers() {
    ...
    try {
      ...
      // 等价于手动 new Promise((resolve, reject) => { ... 把 resolve/reject 存到外部 })
      ...
      this.setState({
        stringPromiseResult:
          ...
          `传统写法对比：let r; new Promise((res) => { r = res; }); r(42);\n` +
...
const v = await promise;            // 42`)),
        h(Alert, {
          ...
          description: '传统 new Promise((resolve) => { ... }) 中 resolve 只在 executor 作用域可见，外部触发结算需先声明变量再赋值；withResolvers 直接返回 { promise, resolve, reject }，特别适合流处理、事件回调、ReadableStream 等需要在 promise 外部触发结算的场景。String 两个方法则让 UTF-16 合法性检测与修复标准化。',
        }),
      ],
    });
    this.registerChild(card);
    ...
    try {
      ...
      this.setState({
        bufferResult:
          ...
          `  ab.byteLength = ${ab.byteLength}（初始 8）\n` +
  ...
  _transferBuffer() {
    ...
    try {
      ...
      let accessOldLine = '';
      try { ab.byteLength; } catch (e) { accessOldLine = `\n访问已分离 ab.byteLength → ${e.name}（detached 后无法访问）`; }
      this.setState({
        bufferResult:
...
const ab3 = ab2.transferToFixedLength(); // 转为不可调整（resizable=false）`)),
  ...
  _symbols() {
    ...
    try {
      ...
      const spreadable = [1, 2]; spreadable[Symbol.isConcatSpreadable] = true;
      ...
      const lines = [
        ...
        '',
        `Symbol.hasInstance：4 instanceof IsEven = ${evenCheck}｜5 instanceof IsEven = ${oddCheck}`,
        '',
        ...
        `  ma.map(x=>x*2) instanceof MyArray = ${mappedIsMyArray}（species=Array → 不派生 MyArray）｜ instanceof Array = ${mappedIsArray}`,
      ];
