...
export function h(tag, props = {}, ...children) {
  ...
  if (typeof tag === 'function') {
    ...
    // 否则可变参数 [] 会覆盖 props.children，导致 Upload 这类依赖 children 作为触发器的组件
    // 渲染出空 div，按钮/拖拽区都看不到。
    ...
    // 关键修复：通过 h() 创建的组件实例必须被 attach，
    // 否则 _mounted 永远为 false，setState 触发的 _rerender 会被跳过，
    // 导致 Image 加载完成、Upload 选择文件等交互无法更新视图。
    // _attach() 内部有 _mounted 去重检查，不会重复触发 componentDidMount；
    // 且 _triggerDidMount 用 requestAnimationFrame 延迟调用 componentDidMount，
    // 届时 DOM 已插入文档树，生命周期时序安全。
