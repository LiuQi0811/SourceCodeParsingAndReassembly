...
export class Component {
  ...
  destroy() {
    ...
    this._eventBindings.forEach(({ target, type, handler, options }) => {
      ...
      try { if (target && typeof target.removeEventListener === 'function') target.removeEventListener(type, handler, options); }
      catch { /* noop */ }
    });
    this._eventBindings = [];
    this._storeUnsub?.();
  ...
  _rerender() {
    ...
    // 注意：构造函数里包装过的 this.render() 会在返回前把 this.el 指向新节点，
    // 因此必须先保存旧元素引用，再用旧元素做 replaceWith，否则会变成
    // newEl.replaceWith(newEl) 的空操作，导致旧 DOM 残留在文档中、
    // this.el 指向游离节点，后续 destroy() 也无法移除可见 DOM。
    const oldEl = this.el;
    // 保留离开动画类：若旧 DOM 处于淡出中（.page-leave），
    // 重渲染后需把该类补回新 DOM，否则无感切换的淡出会丢失。
    ...
    this.el = newEl;
    if (wasLeaving) newEl.classList.add('page-leave');
