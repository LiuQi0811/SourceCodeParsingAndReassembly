...
import { Link2, Search, Loader2, ImageOff, Clock, User, RotateCcw, Save, BookOpen } from 'lucide-react'
import { toast } from 'sonner'
import { useConfig } from '@/contexts/ConfigContext'
import { resolveVideoMeta, addBatchTask } from '@/lib/api'
import { QUICK_FORMATS } from '@/lib/yt-dlp/quickFormats'
import { CommandPreview } from '@/components/CommandPreview'
import { InstallGuideDrawer } from '@/components/InstallGuideDrawer'
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          <h2 className="text-lg font-semibold text-foreground text-glow">yt-dlp 工作台</h2>
          <p className="mt-1 text-xs text-muted-foreground">粘贴链接、配置参数，一键生成可执行命令</p>
        </div>
        <Button variant="outline" size="sm" onClick={() => setGuideOpen(true)} className="shrink-0 gap-1.5">
          <BookOpen className="h-3.5 w-3.5" />
          <span className="hidden sm:inline">安装指南</span>
        </Button>
      </div>
      {/* URL 输入与元数据预览 */}
      <Card>
      ...
      {/* 本地安装指南抽屉 */}
      <InstallGuideDrawer open={guideOpen} onOpenChange={setGuideOpen} />
    </div>
  )
}