...
本项目抓取后端为 Python aiohttp 服务（`server.py`，监听 8000 端口），前端依赖它执行真实抓取。若前端提示「抓取后端未运行」，请按以下步骤启动：

```bash
# Step 1: 安装后端依赖
pip install aiohttp beautifulsoup4 lxml cryptography

# Step 2: 启动后端服务（前台运行，Ctrl+C 停止）
python3 server.py

# 或者：后台运行（日志写入 server.log）
nohup python3 server.py > server.log 2>&1 &

# Step 3: 验证服务是否就绪（返回 JSON 即成功）
curl http://localhost:8000/api/status
```

启动后前端顶部状态指示灯将变为 ONLINE，也可以在「9. 命令控制台」执行 `serve` 查看后端状态与启动指引。
