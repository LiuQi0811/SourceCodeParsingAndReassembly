...
def main() -> int:
    ...
    # 必须清空重建：续传记录藏在 .xxx.etag 里，残留会让
    # "脏数据不该被续传"这一项失去前提（以为有记录就敢续传）
    if workdir.exists():
    ...
    # 端口传 0 = 让系统分配空闲端口。写死端口在重复运行时会
    # 撞上上次残留的进程，报 Address already in use
    ...
    url = f"http://127.0.0.1:{httpd.server_address[1]}/big.bin"
