...
| 散装 ts 分片 | 无 m3u8 的一批 `.ts`，自动识别序列并合并 | ✅ `--merge-ts` |
...
| `--merge-ts` | 抓取结束后，把散装 ts 分片合并成完整视频 | - |
| `--merge-dir DIR` | 单独合并某个目录的 ts（不抓取） | - |
| `--delete-parts` | 合并成功后删除原分片 | 保留 |
| `--merge-min N` | 至少 N 个文件才算序列 | 2 |
| `-q` | 只输出摘要 | - |
...
├── tsmerge.py     TS 合并：序列识别、自然排序、加密检测、ffmpeg concat 拼接
...
| 场景 | 框架行为 |
|---|---|
| **有 m3u8 清单** | 自动处理：选流 → 下载分片 → 解密 → 合并成 mp4。无需额外参数 |
| **站点上散着一批 ts**（无 m3u8） | 加 `--merge-ts`，抓取结束后自动合并 |
| **手上已经有一堆 ts 文件** | `--merge-dir 目录`，不抓取只合并 |

```bash
# 抓完自动合并
python -m mharvest https://site.com/page --merge-ts -o ./out

# 已有文件，单独合并
python -m mharvest --merge-dir ./out/videos

# 合并后删掉原分片（默认保留，建议确认能播再删）
python -m mharvest --merge-dir ./out/videos --delete-parts
```
...
分片名若是 `seg_1.ts` ~ `seg_13.ts`（不补零），按字符串排会得到
`seg_1, seg_10, seg_11, seg_12, seg_13, seg_2, ...`——
拼出来的视频**画面会乱跳**。所以排序按文件名里的数字当数值比。

**2. 合并方式影响时长**

| 方式 | 做法 | 后果 |
|---|---|---|
| ffmpeg concat（默认） | 列表文件交给 ffmpeg，`-c copy` | **重算时间戳**，时长正确 |
| 裸拼接（降级） | 直接首尾相接 | 播放器看到的时长只有第一片那么长 |

MPEG-TS 是流式格式，裸拼接能播，但每个分片时间戳都从各自起点开始。
所以只要 ffmpeg 在，一律走 concat。

### 加密分片合不了（这是设计，不是 bug）

如果 ts 是 AES-128 加密的，**必须有密钥**才能解，而密钥只写在 m3u8 的
`#EXT-X-KEY` 里。散装 ts 没有 m3u8 就无从得知密钥。

框架会检测出加密分片并**明确报错**，而不是默默合并出一个
"看起来成功、实际是垃圾"的文件：

```
[跳过] enc: 检测到 5 个疑似 AES 加密的分片（enc_000.ts, enc_001.ts...），无法合并。
  密钥只写在 m3u8 的 #EXT-X-KEY 里，散装 ts 没有 m3u8 就拿不到密钥。
  请改为抓取这条流的 m3u8 地址（框架会自动取密钥并解密）。
```

检测依据是 MPEG-TS 的同步字节：每 188 字节一包、包首固定为 `0x47`，
加密后这个规律会被彻底打乱。

遇到这种情况，去找这条流的 m3u8 地址（F12 网络面板过滤 m3u8），
直接把 m3u8 当入口抓即可。

### 顺带一提：`.ts` 的两义性

`.ts` 既是 MPEG-TS 视频，也是 TypeScript 源码扩展名。
框架现在能发现 `.ts` 链接（否则散装分片根本抓不到），
但**下载后会嗅探文件头**：不是 `0x47` 开头且是纯文本的，判定为源码并丢弃，
不会抓一堆 `.ts` 源码文件回来。
...
python tests/test_tsmerge.py   # TS 合并 36 项（自然排序、分组、加密拒绝、裸拼接降级）
```

`test_tsmerge.py` 里有几个用例是专门用来防回归的：

- 自然排序：`seg_10` 不能排到 `seg_2` 前面
- 分组：`seg_*` 和 `intro_*` 必须分成两组，不能混
- 加密拒绝：加密分片**必须**报错，不能产出坏文件
- 对照实验：乱序合并与正序合并的产物必须不同（证明排序真起作用）
