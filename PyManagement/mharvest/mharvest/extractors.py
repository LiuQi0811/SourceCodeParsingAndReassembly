...
CSS_URL_RE = re.compile(r"url\(\s*(?P<q>['\"]?)(?P<url>[^'\")\s]+)(?P=q)\s*\)", re.I)
...
class HtmlExtractor(HTMLParser):
    ...
    def __init__(self, base_url: str, *, scan_scripts: bool = True,
        ...
        self._text_chunks: list[str] = []    # 所有 script / style 的正文
    ...
    def finish(self) -> Extraction:
        ...
        if text and self.scan_scripts:
            # <style> 里的 background-image，用 CSS 规则拿相对路径
            for url in extract_css_urls(text):
