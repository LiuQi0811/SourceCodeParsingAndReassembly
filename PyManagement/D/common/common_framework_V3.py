...
DEFAULT_CONFIG = {
    ...
    "custom_cookie": "",
    "proxy_url": "",
    "whitelist_domains": [],          # B 白名单域名列表，空=不限制
    "download_rate_limit": 0,         # C 单下载限速 bytes/s，0=不限速，例 500*1024 =500KB/s
    "log_file": "spider.log"          # D 日志文件
}
# =========================================================================
...
logger = setup_logger(DEFAULT_CONFIG["log_file"])
...
# A click命令行入口
@click.command()
@click.option("--url", required=True, help="起始抓取URL")
@click.option("--max-depth", default=DEFAULT_CONFIG["max_depth"], type=int, help="抓取最大深度")
@click.option("--concurrency", default=DEFAULT_CONFIG["concurrency"], type=int, help="并发数")
@click.option("--sleep", default=DEFAULT_CONFIG["sleep_sec"], type=float, help="页面间隔秒")
@click.option("--timeout", default=DEFAULT_CONFIG["timeout"], type=int, help="超时秒")
@click.option("--proxy", default=DEFAULT_CONFIG["proxy_url"], help="代理 http://127.0.0.1:7890")
@click.option("--referer", default=DEFAULT_CONFIG["custom_referer"], help="防盗链Referer")
@click.option("--cookie", default=DEFAULT_CONFIG["custom_cookie"], help="防盗链Cookie字符串")
@click.option("--ffmpeg-path", default=DEFAULT_CONFIG["ffmpeg_path"], help="ffmpeg可执行文件路径")
@click.option("--whitelist", multiple=True, help="B 白名单域名，可多次指定 --whitelist a.com --whitelist b.com")
@click.option("--rate-limit", default=DEFAULT_CONFIG["download_rate_limit"], type=int, help="C 单下载限速 bytes/s，0不限")
@click.option("--no-download", is_flag=True, help="关闭资源下载，只采集链接")
def main(url, max_depth, concurrency, sleep, timeout, proxy, referer, cookie, ffmpeg_path, whitelist, rate_limit, no_download):
    ...
    cfg["timeout"] = timeout
    cfg["proxy_url"] = proxy
    cfg["custom_referer"] = referer
    cfg["custom_cookie"] = cookie
    cfg["ffmpeg_path"] = ffmpeg_path
    cfg["whitelist_domains"] = list(whitelist)
    cfg["download_rate_limit"] = rate_limit
    cfg["download_enable"] = not no_download

    spider = AsyncUniversalSpider(start_url=url, cfg=cfg)
...
# 基础运行
# python spider.py --url "https://example.com"
