...
class MjwoCrawler:
    ...
    def parse_play_page(self, play_url):
        ...
        player_m = re.search(r'var\s+player_*\w*\s*=\s*(\{.*?\});', resp.text, re.S)
        if player_m:
            try:
                import json as _json
                player_data = _json.loads(player_m.group(1))
                ...
                result["player_data_raw"] = player_m.group(1)[:1000]
        ...
        # 查找MacCMS配置中的播放源
        config_m = re.search(r'MacPlayerConfig\s*=\s*(\{.*?\});', resp.text, re.S)
        if config_m:
            result["player_config_raw"] = config_m.group(1)[:1000]
    ...
    def run(self, categories=None, max_items=None):
        """
        运行全站爬虫
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="美剧窝 mjwo.net 全站爬虫")
    parser.add_argument("--mode", default="all", choices=["all", "list", "single", "play"],
                        help="运行模式: all=全站, list=仅列表, single=单条, play=解析播放页")
    parser.add_argument("--categories", nargs="*", default=None,
                        help="指定分类，如 meiju dianying；默认全部")
    parser.add_argument("--max", type=int, default=None, help="限制最大抓取条数（测试用）")
    parser.add_argument("--url", type=str, default=None, help="单抓模式的URL或ID")
